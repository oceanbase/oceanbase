...
TEST_F(TestAllClusterProxy, update_and_load)
{
  ...
  ObClusterInfo info;
  info.cluster_id_ = 0;
  info.cluster_type_ = PRIMARY_CLUSTER;
  ...
  ASSERT_EQ(LEADER, dest_info.cluster_type_);
  ...
  dest_info.reset();
  info.cluster_id_ = 0;
  info.cluster_type_ = STANDBY_CLUSTER;
...
TEST_F(TestAllClusterProxy, set_redo_option)
{
  ...
  ASSERT_EQ(OB_SUCCESS, ret);
  ret = redo_option.construct_redo_option(
      ObString::make_string("reopen = 0 max_failure = 11 sync    net_timeout =    10"));
  ASSERT_EQ(OB_SUCCESS, ret);
  ASSERT_EQ(true, redo_option.is_sync_);
  ASSERT_EQ(10, redo_option.net_timeout_);
