...
TEST_F(TestLogArchiveConfig, log_archive)
{
  ...
  //  ret = log_archive_item.set_value("MANDATORY encryption_mode= transparent_encryption ENCRYPTION_ALGORITHM =
  //  aes-128"); ASSERT_EQ(true, log_archive_item.value_.valid_); ASSERT_EQ(true,
  //  log_archive_item.value_.is_mandatory_); ASSERT_EQ(false, log_archive_item.value_.is_compress_enabled_);
