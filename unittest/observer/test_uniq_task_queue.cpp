...
public:
  ...
  MockTask() : group_id_(common::OB_INVALID_ID), task_id_(common::OB_INVALID_ID)
  {}

  MockTask(const uint64_t group_id, const uint64_t task_id) : group_id_(group_id), task_id_(task_id)
  {}

  virtual ~MockTask()
  {}

  void init(const uint64_t group_id, const uint64_t task_id)
  {
    group_id_ = group_id;
    task_id_ = task_id;
  }
  ...
  int assign_when_equal(const MockTask &other)
  {
    UNUSED(other);
...
public:
  ...
  virtual int batch_process_tasks(const common::ObIArray<MockTask> &tasks, bool &stopped)
  {
    ...
    } else if (tasks.at(0).get_group_id() == 1 && tasks.at(0).get_task_id() == 0) {
