                                         ...
                                         const int64_t nested_obj_count)
{
  ...
  const int64_t key_count = 1;
  const int64_t cfrows_count = 1;
  const int64_t key_idx = 0;
  const OHOperationType hbase_op_type = OHOperationType::PUT;
  ObObj key;
  key.set_varbinary(ObString::make_string("row-0"));

  if (OB_FAIL(serialization::encode(payload, sizeof(payload), payload_pos, credential))) {
  ...
  } else if (OB_FAIL(serialization::encode_vi64(payload, sizeof(payload), payload_pos, key_count))) {
  } else if (OB_FAIL(ObTableSerialUtil::serialize(payload, sizeof(payload), payload_pos, key))) {
  } else if (OB_FAIL(serialization::encode_vi64(payload, sizeof(payload), payload_pos, cfrows_count))) {
  } else if (OB_FAIL(encode_cfrows(payload,
                                   sizeof(payload),
                                   payload_pos,
                                   1,
                                   &key_idx,
                                   ...
                                   nested_cell_num,
                                   nested_obj_count))) {
  } else if (OB_FAIL(serialization::encode(payload, sizeof(payload), payload_pos, hbase_op_type))) {
  } else if (OB_FAIL(append_unis_payload(buf, buf_len, pos, payload, payload_pos))) {
  }
  return ret;
}
...
{
  const int64_t invalid_indexes[] = {-1, 1};
