    ...
    while ((opt = getopt_long(argc, argv, opt_string, longopts, NULL)) != -1) {
      ...
      switch (opt) {
        ...
        case 'c': {
          ...
          snprintf(tmp_file_name, strlen("test_resolver_") + strlen(optarg) + 7, "test_resolver_%s.test", optarg);
...
void TestSqlUtils::init()
{
  ...
  ObTenantManager::get_instance().add_tenant(sys_tenant_id_);
  ObTenantManager::get_instance().set_tenant_mem_limit(sys_tenant_id_, 1024L * 1024L * 1024L, 1024L * 1024L * 1024L);
  if (OB_SUCCESS != (ret = ObPreProcessSysVars::init_sys_var())) {
    ...
    if (OB_FAIL(ret)) {
      ...
      // OK(session_info_.load_sys_variable(sql_mode, type, value, ObSysVarFlag::GLOBAL_SCOPE |
      // ObSysVarFlag::SESSION_SCOPE));
      ...
      ObAddr addr;
      if (OB_FAIL(plan_cache_mgr_.init(&part_cache_, addr))) {
      ...
      if (OB_SUCC(ret)) {
        if (OB_FAIL(session_info_.update_sys_variable_by_name(ObString::make_string(OB_SV_PARALLEL_MAX_SERVERS), 10))) {
          OB_LOG(WARN, "Failed to up date parallel max servers", K(ret));
        }
      }
    }
  }
...
void TestSqlUtils::do_load_sql(const char* query_str, ObStmt*& stmt, bool is_print, enum ParserResultFormat format,
    int64_t expect_error, int64_t case_line)
{
  // ObStmt *stmt = NULL;
...
void TestSqlUtils::do_resolve(const char* query_str, ObStmt*& stmt, bool is_print, enum ParserResultFormat format,
    int64_t expect_error, bool parameterized, bool need_replace_param_expr, int64_t case_line)
{
  UNUSED(need_replace_param_expr);
  ObSQLMode mode = share::is_oracle_mode() ? (SMO_ORACLE | DEFAULT_ORACLE_MODE) : SMO_DEFAULT;
  ...
  ObMaxConcurrentParam::FixParamStore fixed_param_store(ObNewModIds::OB_COMMON_ARRAY, ObWrapperAllocator(&allocator_));
  ...
  if (OB_SUCCESS == ret && stmt->get_stmt_type() == stmt::T_CREATE_TABLE) {
    ...
    schema::ObTableSchema& table_schema = create_table_stmt->get_create_table_arg().schema_;
    ...
    OK(schema_guard_.get_table_id(table_schema.get_tenant_id(),
        database_id,
...
int TestSqlUtils::create_system_table()
{
  ...
  for (int i = 0; OB_SUCC(ret) && NULL != information_schema_table_schema_creators[i]; ++i) {
    ...
    if (OB_SUCCESS != (ret = creator_ptr(table_schema))) {
      ...
      table_schema.set_database_id(combine_id(sys_tenant_id_, next_user_database_id_ + 1));
      table_schema.set_table_id(combine_id(sys_tenant_id_, table_schema.get_table_id()));
      if (OB_FAIL(add_table_schema(table_schema))) {
        _OB_LOG(WARN, "add table schema fail, ret %d", ret);
      }
...
void TestSqlUtils::do_create_database(ObStmt*& stmt)
{
  ...
  database_schema.set_database_id(combine_id(sys_tenant_id_, next_user_database_id_++));
  ...
  if (OB_SUCC(ret)) {
    if (OB_FAIL(schema_guard_.get_tenant_info(database_schema.get_tenant_id(), tenant_schema))) {
      ...
      if (local_mode <= OB_NAME_CASE_INVALID || local_mode >= OB_NAME_CASE_MAX) {
        ...
        if (OB_FAIL(schema_service_->add_database_schema(database_schema, schema_version_++))) {
...
{
  schema::ObDatabaseSchema database_schema;
  database_schema.set_tenant_id(sys_tenant_id_);
  database_schema.set_database_id(combine_id(sys_tenant_id_, OB_SYS_DATABASE_ID));
...
void TestSqlUtils::do_create_user(ObStmt*& stmt)
{
  ...
  for (int64_t i = 0; i < users.count() - 4; i += 3) {
    if (OB_SUCCESS != (ret = users.get_string(i, user_name))) {
      ...
      OK(schema_service_->add_user_schema(user_info, schema_version_++));
    }
  }
}
...
uint64_t TestSqlUtils::get_next_table_id(const uint64_t user_tenant_id)
{
  ...
  if (OB_HASH_NOT_EXIST == next_user_table_id_map_.get_refactored(user_tenant_id, next_table_id)) {
    next_table_id = OB_MIN_USER_TABLE_ID + 1;
...
void TestSqlUtils::generate_index_column_schema(ObCreateIndexStmt& stmt, ObTableSchema& index_schema)
{
  ...
  OK(schema_guard_.get_table_schema(
      index_arg.tenant_id_, index_arg.database_name_, index_arg.table_name_, false, table_schema));
...
void TestSqlUtils::generate_index_schema(ObCreateIndexStmt& stmt)
{
  ...
  OK(schema_guard_.get_table_schema(
      index_arg.tenant_id_, index_arg.database_name_, index_arg.table_name_, false, data_table_schema));
  OB_ASSERT(data_table_schema);
  generate_index_column_schema(stmt, index_schema);
  ObString index_table_name;
  OK(ObTableSchema::build_index_table_name(
      allocator_, data_table_schema->get_table_id(), index_arg.index_name_, index_table_name));
  ...
  if (data_table_schema != NULL) {
    ...
    OK(table_schema.add_simple_index_info(
...
int TestSqlUtils::get_hidden_column_value(ObResolverParams& resolver_ctx, ParamStore& params)
{
  int ret = OB_SUCCESS;
  ...
  for (int64_t i = 0; OB_SUCC(ret) && i < calc_items.count(); i++) {
    ...
    if (OB_SUCCESS != (ret = ObSQLUtils::calc_calculable_expr(
                           stmt::T_NONE, resolver_ctx.session_info_, expr, result, allocator, params))) {
...
int TestSqlUtils::parse_row_from_json(
    const ObString& json_str, ObString& table_name, ObIArray<ObSEArray<ObObj, 3> >& row_array)
{
  int ret = OB_SUCCESS;
