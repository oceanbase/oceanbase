...
void ObExprLeastTest::SetUp()
{
}

void ObExprLeastTest::TearDown()
{
}
...
TEST_F(ObExprLeastTest, basic_test)
{
  ObExprLeast least_op;
  ObExprStringBuf buf;
  ASSERT_EQ(ObExprOperator::MORE_THAN_ZERO, least_op.get_param_num());


  // least when a then b [else c]
  //T_1(least_op, calc_resultN, datetime,1000, datetime, 1000);
  T_2(least_op, calc_resultN, mtime,1000, precise_datetime, 10002, precise_datetime, 1000);
  T_2(least_op, calc_resultN, varchar,"2012-11-12", precise_datetime, 10002, precise_datetime, 10002);
  T_2(least_op, calc_resultN, varchar,"2012-11-12", precise_datetime, 100020000000000000, precise_datetime, 1352649600000000 /*2012-11-12*/);
  T_2(least_op, calc_resultN, bool, true, null, 0, null, 0);
  T_5(least_op, calc_resultN, bool, true, bool, false, bool, true, bool, false, bool, false, bool, false);
  T_5(least_op, calc_resultN, int, 10, int, 20, int, 0, int, -20, int, -20, int, -20);
  //T_5(least_op, calc_resultN, int, 10, double, 20.0, int, 0, float, -20, int, -20, double, -20);
  //T_5(least_op, calc_resultN, int, 10, varchar, "-50", int, 0, float, -20, int, -20, float, -50.0);
}

TEST_F(ObExprLeastTest, fail_test)
{
  ObExprLeast least_op;
  ObExprStringBuf buf;
  ASSERT_EQ(ObExprOperator::MORE_THAN_ZERO, least_op.get_param_num());
  ...
  // least when a then b [else c]
  F_3(least_op, calc_resultN, varchar,"2012-11-12", precise_datetime, 10002, int, 10003, precise_datetime, 10002);
  F_2(least_op, calc_resultN, bool, false, varchar, "hello", null, 0);
  F_2(least_op, calc_resultN, bool, true, varchar, "hello", varchar, "hello");
  F_5(least_op, calc_resultN, int, 10, varchar, "hi", int, 0, float, -20, int, -20, double, -20);
  //F_2(least_op, calc_resultN, mtime,1000, precise_datetime, 10002, mtime, 1000);
}
