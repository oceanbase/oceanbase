...
#define EXPECT_RESULTN1(str_op_object, str_buf, func, type1, v1, v2, ref_type, ref_value)    \
  ...
  while (0)
#define TTN(typ, val1, val2, res_val, res_typ) \
  EXPECT_RESULTN1(obj, &buf, calc_result1, typ, val1, val2, res_val, res_typ)
...
TEST_F(ObExprQuoteTest, basic_test)
{
  ...
  TN(varchar, "dsfedfesdfdgfsf'fdfe'sedf", varchar, "\'dsfedfesdfdgfsf\\'fdfe\\'sedf\'");
