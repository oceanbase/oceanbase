...
struct ObExprMaster
{
  ...
  union {
    EvalVectorFunc eval_vector_func_{nullptr};
    sql::ser_eval_batch_function ser_eval_vector_func_;
  ...
  uint16_t vec_value_tc_;
  // VecValueTypeClass vec_value_tc_;
  int64_t local_session_var_id_{12};
  ObExpr **attrs_{nullptr};
  uint32_t attrs_cnt_{0};
};

OB_DEF_SERIALIZE(ObExprMaster)
{
  int ret = OB_SUCCESS;
...
int ObExprMaster::deserialize(const char *buf, const int64_t data_len, int64_t &pos)
{
  int ret = OB_SUCCESS;
  int64_t version = 0;
  int64_t len = 0;
...
void verify_basic_fields_equal(const ObExprMaster &expr1, const ObExpr &expr2)
{
...
TEST_F(TestExprSerializeCompat, test_42x_to_master)
{
  ...
  verify_basic_fields_equal(expr_master, expr_42x);
}
