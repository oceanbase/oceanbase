...
// Test fixture
class TestTrimNeon : public ::testing::Test
{
public:
  TestTrimNeon() : allocator_(ObModIds::TEST) {}
  virtual void SetUp() {}
...
TEST_F(TestTrimNeon, ltrim_correctness) {
  // Test empty string
  {
    ObString src(0, "");
    ObString pattern(1, " ");
...
TEST_F(TestTrimNeon, rtrim_correctness) {
  // Test empty string
  {
    ObString src(0, "");
    ObString pattern(1, " ");
...
TEST_F(TestTrimNeon, lrtrim_correctness) {
...
TEST_F(TestTrimNeon, compare_with_reference) {
  ...
  for (int t = 0; t < NUM_TESTS; t++) {
    ...
    // Test ltrim
    ObString neon_result;
    EXPECT_EQ(OB_SUCCESS, ObExprTrim::trim(neon_result, TRIM_LTRIM, pattern, src));
    std::string ref_result = ref_trim(s, ' ', true, false);
    EXPECT_EQ(ref_result.length(), neon_result.length())
        << "ltrim length mismatch for string: '" << s << "'";
    EXPECT_EQ(0, strncmp(neon_result.ptr(), ref_result.c_str(), neon_result.length()))
        << "ltrim content mismatch for string: '" << s << "'";

    allocator_.reuse();

    // Test rtrim
    EXPECT_EQ(OB_SUCCESS, ObExprTrim::trim(neon_result, TRIM_RTRIM, pattern, src));
    ...
    // Test lrtrim
    EXPECT_EQ(OB_SUCCESS, ObExprTrim::trim(neon_result, TRIM_LRTRIM, pattern, src));
...
// Edge cases
TEST_F(TestTrimNeon, edge_cases) {
...
TEST_F(TestTrimNeon, simd_boundary_tests) {
...
TEST_F(TestTrimNeon, custom_pattern_tests) {
  // Pattern 'x'
  {
...
// Test rtrim edge cases
TEST_F(TestTrimNeon, rtrim_edge_cases) {
  // Test string with trim position at SIMD boundary
  {
    // 16 spaces followed by content - rtrim should find end correctly
    std::string s = "hello               ";  // "hello" + 15 spaces
    ObString src(s.length(), s.c_str());
    ObString pattern(1, " ");
    ObString result;
    EXPECT_EQ(OB_SUCCESS, ObExprTrim::trim(result, TRIM_RTRIM, pattern, src));
  ...
  // Test multiple SIMD blocks of trailing spaces
  {
    std::string s = "hello" + std::string(32, ' ');  // "hello" + 32 spaces
    ObString src(s.length(), s.c_str());
    ObString pattern(1, " ");
    ObString result;
    EXPECT_EQ(OB_SUCCESS, ObExprTrim::trim(result, TRIM_RTRIM, pattern, src));
...
// Test lrtrim with complex patterns
TEST_F(TestTrimNeon, lrtrim_complex_tests) {
  // Alternating content and pattern at boundaries
  {
    ...
    EXPECT_EQ(OB_SUCCESS, ObExprTrim::trim(result, TRIM_LRTRIM, pattern, src));
    EXPECT_EQ(9, result.length());
    EXPECT_EQ(0, strncmp(result.ptr(), "abc   def", 9));
  }

  // Long string with small content in middle
  {
    std::string s = std::string(100, ' ') + "x" + std::string(100, ' ');
    ObString src(s.length(), s.c_str());
    ObString pattern(1, " ");
    ObString result;
    EXPECT_EQ(OB_SUCCESS, ObExprTrim::trim(result, TRIM_LRTRIM, pattern, src));
