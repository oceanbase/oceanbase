...
class ObPhysicalPlanCtx42x
{
  OB_UNIS_VERSION(2);
public:
  ObPhysicalPlanCtx42x(common::ObIAllocator &allocator)
...
OB_DEF_SERIALIZE(ObPhysicalPlanCtx42x)
{
  int ret = OB_SUCCESS;
...
int ObPhysicalPlanCtx42x::deserialize(const char *buf, const int64_t data_len, int64_t &pos)
{
  int ret = OB_SUCCESS;
  int64_t version = 0;
  int64_t len = 0;
  ...
  if (version == 3) {
    ...
    OB_UNIS_DECODE(enable_rich_format);
  }
...
void fill_physical_plan_ctx(ObPhysicalPlanCtx &ctx)
{
  ...
  ctx.enable_rich_format_ = true;
}

void verify_basic_fields_equal(const ObPhysicalPlanCtx42x &ctx1, const ObPhysicalPlanCtx &ctx2)
{
...
TEST_F(TestPhysicalPlanCtxSerializeCompat, test_42x_to_master)
{
  ...
  ObPhysicalPlanCtx42x ctx_42x(allocator);
  ObPhysicalPlanCtx ctx_master(allocator);
  ...
  verify_basic_fields_equal(ctx_42x, ctx_master);
}
...
TEST_F(TestPhysicalPlanCtxSerializeCompat, test_master_to_42x)
{
  ...
  fill_physical_plan_ctx(ctx_master);
