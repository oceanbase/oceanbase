...
private:
   ...
   * NOTE: Current branch does not support group_distinct on ObHashDistinctVecSpec.
   * This method is a no-op stub.
   */
  int fill_group_distinct(common::ObIAllocator &alloc, ObHashDistinctVecSpec *spec)
  {
    UNUSED(alloc);
    UNUSED(spec);
    // group_distinct is not supported on this branch.
    // spec->group_distinct_exprs_, spec->group_sort_collations_,
    // spec->grouping_id_, spec->has_non_distinct_aggr_params_ are N/A.
    return OB_SUCCESS;
  }
