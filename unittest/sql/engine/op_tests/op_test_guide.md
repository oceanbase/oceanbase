...
15. [Per-Column VectorFormat](#15-per-column-vectorformat)
16. [自定义 Batch Size 函数](#16-自定义-batch-size-函数)
17. [Tracepoint 注入](#17-tracepoint-注入)
18. [Input Skip 注入](#18-input-skip-注入)
19. [性能计时](#19-性能计时)
20. [错误码与 Oracle Runtime 测试](#20-错误码与-oracle-runtime-测试)
...
| `test_framework_extensions` | 框架扩展能力（per-column format / batch size fn / tracepoint / skip / perf） |
...
| `.with_batch_size(fn)` | 按 batch 序号自定义产出行数（lambda） |
| `.with_vector_format(fmt)` | 设置变长列 VectorFormat |
| `.with_col_formats(fmts...)` | 每列独立 VectorFormat（覆盖 `with_vector_format`） |
...
| `.with_tracepoints({{...}})` | 设置 tracepoint（RAII，run() 结束自动复位） |
| `.with_input_skips(fn)` | 注入 skip 位（模拟 filter/predicate 下推） |
| `.with_perf_record(enable)` | 启用性能计时 |
| `.sql_mode(mode)` | 设置 SQL 兼容模式；`expr_unit_test()` 可用 MySQL grammar 解析、Oracle runtime 执行 |
| `.expect_error(error_code)` | 声明 prepare/execute 阶段预期返回的错误码 |
...
## 15. Per-Column VectorFormat

默认情况下 `with_vector_format` 对所有变长列施加同一种格式。`with_col_formats` 允许每列独立指定 VectorFormat，模拟生产中定长/变长列混用的真实场景。

### 基本用法

```cpp
auto result = material_test()
    .table("t", "a int, b varchar(32), c double")
    .select("a, b, c")
    .with_data({{1, "hello", 3.14}, {2, "world", 2.72}})
    .with_col_formats(VEC_FIXED, VEC_DISCRETE, VEC_UNIFORM)
    .run(engine_);
```

### 校验规则

框架在 `build_and_execute` 阶段自动校验：

- **列数必须匹配**：`with_col_formats` 的参数个数必须等于 table 定义的列数，否则 `EXPECT` 失败
- **定长列**（int / double 等）：仅允许 `VEC_FIXED` 或 `VEC_UNIFORM`
- **变长列**（varchar / text 等）：仅允许 `VEC_DISCRETE`、`VEC_CONTINUOUS` 或 `VEC_UNIFORM`
- 非 rich format 模式（`with_rich_format(false)`）下，`col_formats_` 被忽略，整批走 `VEC_UNIFORM`

### 与 with_vector_format 的优先级

`with_col_formats` 优先于 `with_vector_format`：如果 `col_formats_` 非空，则按列使用指定格式；未覆盖的列（`col_formats_` 长度不足时）回退到 `vector_format_` 的默认逻辑。

---

## 16. 自定义 Batch Size 函数

`with_batch_size` 的 lambda 重载允许按 batch 序号返回不同的产出行数，模拟父算子削峰或变长 batch 场景。
...
- 函数签名：`int64_t fn(int64_t batch_idx)`，`batch_idx` 从 0 开始递增
- 返回值 > 0：与 `max_row_cnt` 取较小值作为本批上限
- 返回值 ≤ 0：使用默认 `max_row_cnt`
- 仅影响 MockDataSource 的 `inner_get_next_batch` 产出行数，不影响 `OpTestEngine::collect_batch_results` 的调用方式

---

## 17. Tracepoint 注入

与 `WITH_TENANT_CONFS` 对称，框架提供 `WITH_TRACEPOINTS` 宏和 `with_tracepoints` builder 方法，通过 RAII 自动设置和复位 tracepoint。
...
WITH_TRACEPOINTS(
  ...
  auto result = material_test()
      .table("t", "a int").select("a")
      .with_sql_operator_dump(true).with_hash_area_size(1 * 1024 * 1024)
      .with_data(/* large data */)
      .run(engine_);
}
// 作用域退出后 tracepoint 自动复位
```
...
```cpp
OB_TP(name, error_code, occur, trigger_freq)
```

| 参数 | 说明 |
|------|------|
| `name` | tracepoint 名称，需与 `ob_tracepoint_def.h` 中的注册名一致 |
| `error_code` | 触发时返回的错误码 |
| `occur` | 最大触发次数（>0 时优先于 `trigger_freq`，原子递减至 0 后失效） |
| `trigger_freq` | 触发频率（1 = 每次都触发，N = 1/N 概率触发） |

常见用法：
- 稳定复现：`OB_TP("EN_XXX", -4013, 1, 1)` — 触发 1 次，错误码 -4013
- 强制 dump：`OB_TP("EN_ENABLE_SQL_OP_DUMP", 0, 1, 1)` — 触发 1 次，开启 dump 路径
- 前 N 次出错后恢复：`OB_TP("EN_XXX", -4013, 3, 1)` — 严格触发 3 次后自动停
- 概率注入：`OB_TP("EN_XXX", -4013, 0, 100)` — 1/100 概率触发

### 注意事项

- tracepoint 名需在 `ob_tracepoint_def.h` 中已注册，名字拼错会静默失败（仅打印 `LOG_WARN`）
- 非 errsim 构建（`./build.sh release`）中部分 tracepoint 不存在，`set_event` 返回非 `OB_SUCCESS`
- `WITH_TRACEPOINTS` 基于 C++17 `if (init; cond)` 实现，与 `WITH_TENANT_CONFS` 一致

---

## 18. Input Skip 注入

`with_input_skips` 允许在每次 `get_next_batch` 后向 `brs_.skip_` 注入 skip 位，模拟 filter/predicate 下推或前序算子的过滤结果。
...
auto result = hash_groupby_test()
    .table("t", "a int, b int")
    .select("a, SUM(b)").group_by("a")
    .with_data({{1, 10}, {1, 20}, {2, 30}, {2, 40}})
...
- 函数签名：`void fn(int64_t batch_idx, int64_t batch_size, ObBitVector *skip)`
- `skip` 位图由 `ObOperator` 基类初始化（每位默认 0），lambda 仅叠加置位
- 注入 skip 后，`brs_.size_` **保持不变**，但 `brs_.skip_` 中被置位的行对下游算子不可见
- 因此实际参与运算的行数 ≤ `batch_size`，测试用例的期望值应按过滤后的行构造
- lambda 内置位范围不能超过 `batch_size`，否则越界

---

## 19. 性能计时

`with_perf_record()` 启用后，`run()` 返回的 `OpTestResult` 包含 `PerfStats`，可用于算子性能回归检测。
...
LOG_INFO("operator_rt_us", K(perf.operator_rt_us()));
EXPECT_GT(perf.operator_rt_ns(), 0);
```
...
| 字段 | 类型 | 说明 |
|------|------|------|
| `op_total_ns` | `int64_t` | 算子 open + 全部 get_next_batch + close 的总耗时 (ns) |
| `mock_total_ns` | `int64_t` | MockDataSource 所有 `inner_get_next_batch` 的累加耗时 (ns) |
| `batch_count` | `int64_t` | batch 数量 |
| `row_count` | `int64_t` | 总行数 |
| `operator_rt_ns()` | `int64_t` | 纯算子 RT = `op_total_ns - mock_total_ns` |
| `operator_rt_us()` | `double` | 微秒 |
| `operator_rt_ms()` | `double` | 毫秒 |

### 计时原理

`operator_rt_ns` 从总耗时中扣除 MockDataSource 产数据的时间，得到"纯算子执行时间"。这样下游真实逻辑（HashGroupBy、Merge、Sort 等）的 RT 不会被 mock 的 materialize 时间污染。
...
const auto &perf = result.get_perf_stats();
LOG_INFO("dump_perf", K(perf.operator_rt_ms()), K(perf.batch_count));
```

---

## 20. 错误码与 Oracle Runtime 测试

本节记录用于表达式或算子执行阶段错误码断言的扩展接口，以及表达式测试中覆盖 Oracle runtime 行为的推荐写法。

### 适用场景

- 空输入时表达式不应被求值，例如空表上的 `1 / 0` 不报错。
- 非空输入实际执行表达式时，需要断言指定错误码。
- 验证 `CASE WHEN` 的短路求值，例如未选中的 `ELSE 1 / 0` 不应被执行。
- 表达式 SQL 片段仍走 mock MySQL grammar 解析，但 resolve / codegen / eval 阶段需要切到 Oracle runtime mode。

### `expect_error`

`expect_error(error_code)` 用于声明本用例预期会在 prepare 或 execute 阶段返回某个 OceanBase 错误码。声明后，框架不会把该错误当作通用执行失败，而是校验实际错误码是否等于预期值。
...
- 未调用 `expect_error()` 时，`run()` 默认要求执行结果错误码为 `OB_SUCCESS`。
- 对预期失败用例，建议同时使用 `expect_error()` 和 `get_ret_code()`；前者给框架断言，后者让用例意图更直观。
- `expect_error()` 适用于 prepare 和 execute 两类错误；如果 prepare 已失败，`run()` 会直接返回空结果并校验错误码。

### `OpTestResult::get_ret_code`

`get_ret_code()` 返回本次执行最终错误码。正常扫到 `OB_ITER_END` 会归一为 `OB_SUCCESS`；`open()`、`get_next_batch()`、rescan、close 等阶段的真实错误会记录到结果对象里。
...
### 表达式 Oracle Runtime Mode

`expr_unit_test().sql_mode(SqlMode::ORACLE)` 的语义和普通 `material_test()` 不完全相同：

- mock SELECT SQL 仍按 MySQL grammar 解析，避免 Oracle parser 对当前表达式片段不兼容。
- resolve / codegen / eval 阶段切换到 Oracle runtime mode。
- `run()` 结束后恢复原 SQL mode，避免影响后续测试。

因此它适合覆盖运行时兼容行为，例如 Oracle 模式除零报错、CASE 短路求值等；不适合验证 Oracle parser 专属语法。比如 `CAST('aaa' AS NUMBER)` 这类需要完整 Oracle 语法链路的 SQL，仍建议放在 mysqltest 中覆盖。

### 示例：空输入不求值

空输入时，投影表达式不会被执行，因此危险表达式不应报错。
...
当 `WHEN` 不命中并实际选择 `ELSE` 时，应返回 Oracle 模式除零错误。
