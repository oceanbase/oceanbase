...
struct ObBasicSessionInfo42x
{
  OB_UNIS_VERSION(2);

  ObBasicSessionInfo42x() :
    tenant_id_(500),
...
int ObBasicSessionInfo42x::calc_need_serialize_vars(ObSEArray<share::ObSysVarClassType, 64> &sys_var_ids, ObSEArray<ObString, 32> &user_var_names) const
{
  int ret = OB_SUCCESS;
  sys_var_ids.reset();
...
OB_DEF_SERIALIZE(ObBasicSessionInfo42x)
{
  int ret = OB_SUCCESS;
  ...
  if (OB_SUCC(ret)) {
    ...
    for (int64_t i = 0; OB_SUCC(ret) && i < sys_var_ids.count(); ++i) {
      ...
      } else if (sys_var_idx < 0 || ALL_SYS_VARS_COUNT <= sys_var_idx) {
        ret = OB_ERR_UNEXPECTED;
        LOG_ERROR("sys var idx is invalid", K(sys_var_idx), K(ALL_SYS_VARS_COUNT), K(ret));
      } else if (OB_ISNULL(sys_vars_[sys_var_idx])) {
        ret = OB_ERR_UNEXPECTED;
        LOG_ERROR("sys var is NULL", K(ret), K(sys_var_idx), K(ALL_SYS_VARS_COUNT));
      } else {
...
void verify_basic_fields_equal(const ObBasicSessionInfo42x &info1, const ObBasicSessionInfo &info2)
{
...
void verify_basic_fields_equal(const ObBasicSessionInfo &info1, const ObBasicSessionInfo &info2)
{
  ASSERT_EQ(info1.tenant_id_, info2.tenant_id_);
  ASSERT_EQ(info1.effective_tenant_id_, info2.effective_tenant_id_);
  ASSERT_EQ(info1.user_id_, info2.user_id_);
...
{
  ObBasicSessionInfo42x info_42x;
  ObBasicSessionInfo info_master(OB_SERVER_TENANT_ID);
  init_set(info_master);
  char buf[BUF_SIZE];
  int64_t pos = 0;
  ASSERT_EQ(OB_SUCCESS, info_42x.serialize(buf, sizeof(buf), pos));
  int64_t deserialize_pos = 0;
  ASSERT_EQ(OB_SUCCESS, info_master.deserialize(buf, pos, deserialize_pos));
  verify_basic_fields_equal(info_42x, info_master);
}

TEST_F(TestBasicSessionInfoSerializeCompat, test_master_to_42x)
{
  // ObBasicSessionInfo info_master(500);
  // ObBasicSessionInfo42x info_42x;
  // init_set(info_master);
  ...
  // deserialize for 42x version not implemented, because need copy too much code.
  // int64_t deserialize_pos = 0;
...
int init_master(ObBasicSessionInfo &info_master)
{
  int ret = OB_SUCCESS;
  OBSERVER.init_schema();
  OBSERVER.init_tz_info_mgr();
  common::ObArenaAllocator allocator(ObModIds::OB_SQL_SESSION);
  ObBasicSessionInfo::LockGuard lock_guard(info_master.get_query_lock());
  if (OB_FAIL(ObPreProcessSysVars::init_sys_var())) {
    LOG_ERROR("init sys var failed", K(ret));
  } else if (OB_FAIL(info_master.test_init(0, 0, &allocator))) {
    LOG_ERROR("init session info failed", K(ret));
  } else if (OB_FAIL(ObPreProcessSysVars::change_initial_value())) {
    LOG_ERROR("Change initial value failed !", K(ret));
  } else if (OB_FAIL(info_master.load_default_sys_variable(true, true))) {
    LOG_ERROR("load default sys variable failed", K(ret));
  } else {
    ObString name;
    ObObj type;
    ObObj value;
    ...
    value.set_int(1);
    ret = info_master.update_sys_variable_by_name(name, value);
    if (OB_FAIL(ret)) {
      LOG_ERROR("update sys variable by name failed", K(ret));
    }
  }
  return ret;
}
// TEST_F(TestBasicSessionInfoSerializeCompat, test_master_to_master)
// {
//   int ret = OB_SUCCESS;
//   ObBasicSessionInfo info_master1(OB_SERVER_TENANT_ID);
//   common::ObArenaAllocator allocator(ObModIds::OB_SQL_SESSION);
//   easy_connection_t conn;
//   bool autocommit = false;
//   bool is_valid  = false;
//   ObArenaAllocator calc_buf(ObModIds::OB_SQL_SESSION);
//   ASSERT_EQ(OB_SUCCESS, info_master1.test_init(0, 0, &allocator));
//   {
//     ObString tenant_name = ObString::make_string("yyy");
//     ObString user_name = ObString::make_string("aaa");
//     info_master1.init_tenant(tenant_name, 1);
//     info_master1.set_user(user_name, OB_DEFAULT_HOST_NAME, 1);
...
//     ASSERT_EQ(OB_SUCCESS, info_master1.load_sys_variable(calc_buf, ObString::make_string(OB_SV_AUTOCOMMIT), autocommit_type, autocommit_obj, min_val, max_val,
...
//   }
//   ObBasicSessionInfo info_master2(OB_SERVER_TENANT_ID);
//   ASSERT_EQ(OB_SUCCESS, info_master2.test_init(0, 0, &allocator));
//   {
//     ObString tenant_name = ObString::make_string("yyy");
//     ObString user_name = ObString::make_string("aaa");
//     info_master2.init_tenant(tenant_name, 1);
//     info_master2.set_user(user_name, OB_DEFAULT_HOST_NAME, 1);
//     ObObj autocommit_obj, min_val, max_val;
//     ObObj autocommit_type;
//     autocommit_obj.set_varchar("0");
//     min_val.set_varchar("");
//     max_val.set_varchar("");
//     autocommit_type.set_type(ObIntType);
//     ASSERT_EQ(OB_SUCCESS, info_master2.load_sys_variable(calc_buf, ObString::make_string(OB_SV_AUTOCOMMIT), autocommit_type, autocommit_obj, min_val, max_val,
//                                                          ObSysVarFlag::GLOBAL_SCOPE | ObSysVarFlag::SESSION_SCOPE | ObSysVarFlag::NEED_SERIALIZE, false));
//     //session_info.set_autocommit(autocommit);
//     //session_info.set_conn(&conn);
//   }
//   char buf[BUF_SIZE];
//   int64_t pos = 0;
//   ASSERT_EQ(OB_SUCCESS, info_master1.serialize(buf, sizeof(buf), pos));
//   int64_t deserialize_pos = 0;
//   ASSERT_EQ(OB_SUCCESS, info_master2.deserialize(buf, pos, deserialize_pos));
//   verify_basic_fields_equal(info_master1, info_master2);
// }
