...
struct ObSQLSessionInfoMaster : public common::ObVersionProvider, public ObBasicSessionInfo, public SessionInfoHashValue
{
  OB_UNIS_VERSION(3);
  ObSQLSessionInfoMaster(const uint64_t tenant_id = OB_SERVER_TENANT_ID) :
      ObVersionProvider(),
  ...
  int64_t affected_rows_{1234};
  uint64_t unit_gc_min_sup_proxy_version_;
};

OB_DEF_SERIALIZE(ObSQLSessionInfoMaster)
{
  int ret = OB_SUCCESS;
...
OB_DEF_SERIALIZE_SIZE(ObSQLSessionInfoMaster)
{
  int64_t len = 0;
...
int ObSQLSessionInfoMaster::deserialize(const char *buf, const int64_t data_len, int64_t &pos)
{
  int ret = OB_SUCCESS;
  int64_t version = 0;
  int64_t len = 0;
...
void verify_basic_fields_equal(const ObSQLSessionInfoMaster &sess1, const ObSQLSessionInfo &sess2)
{
...
{
  // ObSQLSessionInfoMaster sess_master(500);
  // ObSQLSessionInfo sess_42x(500);
  // sess_42x.cached_tenant_config_info_.session_ = nullptr;
  // sess_master.cached_tenant_config_info_.session_ = nullptr;
  // init_set(sess_42x);
  // fill_session_info(sess_42x);
  ...
  // // 验证基本字段
  // verify_basic_fields_equal(sess_master, sess_42x);
  // sess_42x.cached_tenant_config_info_.session_ = nullptr;
  // sess_master.cached_tenant_config_info_.session_ = nullptr;
}
