...
TEST_F(TestLocationPartitionId, calc_part_id_by_rowkeys)
{
  ...
  ASSERT_EQ(OB_SUCCESS, session_info.init(0, 0, 0, &allocator_, &tz_info));
  ...
  while (std::getline(if_tests, line)) {
    ...
    if (exact_line.at(exact_line.length() - 1) != ';')
      ...
      ASSERT_EQ(
          OB_SUCCESS, schema_guard_.get_table_schema(sys_tenant_id_, database_name, table_name, false, table_schema));
      ASSERT_EQ(OB_SUCCESS,
          convert_row_array_to_rowkeys(
              row_array, rowkeys, table_schema->get_rowkey_info(), session_info_.get_timezone_info()));
      ASSERT_EQ(OB_SUCCESS,
          table_location.calculate_partition_ids_by_rowkey(
              session_info_, schema_guard_, table_schema->get_table_id(), rowkeys, part_ids, rowkey_lists));
      memset(buf, 0, BUF_LEN);
      of_result << "----------------Case" << case_num << " start----------------" << std::endl;
      of_result << "rowkey json"
                << ": " << total_line << std::endl;
