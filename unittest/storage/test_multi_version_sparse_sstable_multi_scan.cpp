...
class TestMultiVersionSparseSSTableMultiScan : public ObMultiVersionSSTableTest {
public:
  TestMultiVersionSparseSSTableMultiScan() : ObMultiVersionSSTableTest("testmultiversionsparsemultiscan")
  {}
  virtual ~TestMultiVersionSparseSSTableMultiScan()
  {}
...
void TestMultiVersionSparseSSTableMultiScan::prepare_query_param(
...
TEST_F(TestMultiVersionSparseSSTableMultiScan, multi_scan_empty_sstable)
{
  ...
  micro_data[0] = "bigint   var   bigint bigint bigint  bigint    flag    multi_version_row_flag\n"
                  "1        var1  -8      0      18       2       EXIST   C\n"
                  "1        var1  -2      0      12       NOP     EXIST   L\n"
                  "2        var2  -10     0      4        3       EXIST   CL\n"
                  "3        var3  -8      0      7        2       EXIST   C\n"
                  "3        var3  -5      0      31       1       EXIST   N\n"  // 5
...
TEST_F(TestMultiVersionSparseSSTableMultiScan, empty_range_scan)
{
  const int64_t rowkey_cnt = 4;
  const char* micro_data[3];
  micro_data[0] = "bigint   var   bigint bigint  bigint   bigint  flag    multi_version_row_flag\n"
...
TEST_F(TestMultiVersionSparseSSTableMultiScan, multi_scan_normal)
{
  const int64_t rowkey_cnt = 4;
  const char* micro_data[3];
  micro_data[0] = "bigint   var   bigint bigint  bigint   bigint  flag    multi_version_row_flag\n"
  ...
  const char* input_column1 = "int int int int int int\n"
                              ...
                              "16  17  7  8  20  21\n"
                              "16  17  7  8  20  NOP\n";
  const int64_t col_cnt1[] = {6, 5, 6, 6, 6, 5, 5, 6, 5, 5, 6, 6, 6, 5};
  prepare_data(micro_data, 3, rowkey_cnt, 10, "none", SPARSE_ROW_STORE, 0, input_column1, col_cnt1);
  ObMockIterator res_iter;
  ObMockIterator rowkeys_iter;
  ObStoreRowIterator* multi_scanner = NULL;
  ObArray<ObExtStoreRange> ranges;
  ...
  const char* result1 = "bigint   var   bigint   bigint  flag    multi_version_row_flag   is_get  scan_index\n"
                        "1        var1  NOP      12      EXIST   N                        TRUE    0\n"
                        "3        var3  NOP      31      EXIST   N                        FALSE   1\n"
                        "4        var4  6        5       EXIST   N                        FALSE   1\n"
                        "NOP      NOP   NOP      NOP     OP_ROW_DOES_NOT_EXIST   N        TRUE    2\n"
                        "5        var5  4        7       EXIST   N                        FALSE   3\n";
...
TEST_F(TestMultiVersionSparseSSTableMultiScan, test_scan_index)
{
  const int64_t rowkey_cnt = 4;
  const char* micro_data[3];
  micro_data[0] = "bigint   var   bigint bigint  bigint   bigint  flag    multi_version_row_flag\n"
                  "1        var1  -8      0      21       2       EXIST   C\n"
                  "1        var1  -2      0      12       NOP     EXIST   L\n"
                  "2        var2  -5      0      4        3       EXIST   CL\n"  //
                  "3        var3  -8      0      7        2       EXIST   C\n"
                  "3        var3  -5      0      18       NOP     EXIST   N\n"  // 5
                  "3        var3  -3      0      16       NOP     EXIST   L\n"
                  "3        var4  -3      0      6        NOP     DELETE  L\n"
                  "4        var4  -9      0      7        24      EXIST   C\n"  //
                  "4        var4  -7      0      6        5       EXIST   C\n"
                  "4        var4  -3      0      5        NOP     EXIST   L\n";  // 10
  ...
  const char* input_column1 = "int int int int int int\n"
                              ...
                              "16  17  7  8  20  NOP\n"
                              "16  17  7  8  19  NOP\n";  // 15
  const int64_t col_cnt1[] = {6, 5, 6, 6, 5, 5, 5, 6, 6, 5, 5, 6, 6, 5, 5};
  prepare_data(micro_data, 3, rowkey_cnt, 10, "none", SPARSE_ROW_STORE, 0, input_column1, col_cnt1);
  ObMockIterator res_iter;
  ObMockIterator rowkeys_iter;
  ObStoreRowIterator* multi_scanner = NULL;
  ...
  const char* reverse_result1 = "bigint   var   bigint   bigint  flag    multi_version_row_flag   is_get  scan_index\n"
                                "1        var1  NOP      12      EXIST   N                        FALSE   0\n"
                                "5        var5  2        6       EXIST   N                        FALSE   2\n";
  ...
  const char* result2 = "bigint   var   bigint   bigint  flag    multi_version_row_flag   is_get  scan_index\n"
                        "1        var1  NOP      12      EXIST   N                        FALSE   0\n"
                        "3        var3  16       NOP     EXIST   N                        FALSE   1\n"
                        "3        var4  6        NOP     DELETE  N                        FALSE   1\n"
                        "5        var5  2        6       EXIST   N                        FALSE   2\n";

  prepare_query_param(trans_version_range, false);  // 0~7
  ...
  const char* rowkeys_str3 = "bigint   var   flag\n"
                             "1        var1  EXIST\n"
                             "1        var2  EXIST\n"
                             "3        var3  EXIST\n"
                             "3        var8  EXIST\n"
  ...
  const char* result3 = "bigint   var   bigint   bigint  flag    multi_version_row_flag   is_get  scan_index\n"
                        "1        var1  NOP      12      EXIST   N                        TRUE    0\n"
                        "2        var2  4        NOP     EXIST   N                        FALSE   1\n"
                        "3        var3  16       NOP      EXIST   N                        FALSE   1\n"
                        "NOP      NOP   NOP      NOP     OP_ROW_DOES_NOT_EXIST   N        TRUE    2\n"
                        "5        var5  2        6       EXIST   N                        FALSE   3\n";

  prepare_query_param(trans_version_range, false);  // 0~7
  OK(rowkeys_iter.from(rowkeys_str3));
  // 1, 2~3, 4, 5~6
  ext_range.reset();
  ranges.reset();
  OK(rowkeys_iter.get_row(0, start));
...
TEST_F(TestMultiVersionSparseSSTableMultiScan, span_macro_block)
{
  const int64_t rowkey_cnt = 4;
  ...
  multi_scanner->~ObStoreRowIterator();
  ObModItem item;
  get_tenant_mod_memory(500, ObNewModIds::OB_COMMON_ARRAY, item);
  STORAGE_LOG(
      INFO, "memory usage", K(item.hold_), K(item.used_), K(item.count_), K(item.alloc_count_), K(item.free_count_));
  int64_t mem_hold = get_tenant_memory_hold(TENANT_ID);
  STORAGE_LOG(INFO, "memory hold", K(mem_hold));
}

TEST_F(TestMultiVersionSparseSSTableMultiScan, test_skip_single_range)
{
  const int64_t rowkey_cnt = 4;
  const char* micro_data[3];
  micro_data[0] = "bigint   var   bigint bigint  bigint   bigint  flag    multi_version_row_flag\n"
...
int main(int argc, char** argv)
{
  ...
  system("rm -rf test_multi_version_sparse_sstable_multi_scan.log");
  OB_LOGGER.set_file_name("test_multi_version_sparse_sstable_multi_scan.log");
  STORAGE_LOG(INFO, "begin unittest: test_multi_version_sparse_sstable_multi_scan");
