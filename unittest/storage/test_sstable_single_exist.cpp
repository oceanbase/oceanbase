...
TestSSTableSingleExist::TestSSTableSingleExist() : ObSSTableTest("single_exist_sstable")
{}

TestSSTableSingleExist::~TestSSTableSingleExist()
{}

void TestSSTableSingleExist::SetUp()
{
  ObSSTableTest::SetUp();
...
void TestSSTableSingleExist::TearDown()
{
  ObSSTableTest::TearDown();
  ObPartitionService::get_instance().get_pg_index().destroy();
}

TEST_F(TestSSTableSingleExist, single_exist)
{
  int ret = OB_SUCCESS;
...
{
  system("rm -f test_sstable_single_exist.log*");
  OB_LOGGER.set_file_name("test_sstable_single_exist.log", true, true);
  OB_LOGGER.set_log_level("INFO");
  CLOG_LOG(INFO, "begin unittest: test_sstable_single_exist");
