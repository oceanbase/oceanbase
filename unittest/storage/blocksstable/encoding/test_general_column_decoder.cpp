...
TEST_F(TestDictDecoder, black_filter_pushdown_disabled_for_encoding_row_store)
{
  ...
  ASSERT_EQ(common::ENCODING_ROW_STORE, static_cast<common::ObRowStoreType>(decoder.header_->row_store_type_));
...
TEST_F(TestDictDecoder, black_filter_pushdown_disabled_for_selective_encoding_row_store)
{
  ...
  ObMicroBlockData data(encoder_.data_buffer_.data(), encoder_.data_buffer_.length());
  ASSERT_EQ(OB_SUCCESS, decoder.init(data, read_info_));
  ASSERT_EQ(common::SELECTIVE_ENCODING_ROW_STORE, static_cast<common::ObRowStoreType>(decoder.header_->row_store_type_));
