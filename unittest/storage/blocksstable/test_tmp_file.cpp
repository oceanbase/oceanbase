...
TestTmpFileStress::TestTmpFileStress()
    : thread_cnt_(0),
      size_(OB_FILE_SYSTEM.get_macro_block_size()),
      fd_(0),
      is_write_(false),
      is_big_file_(false),
      table_schema_(NULL),
...
int TestTmpFileStress::init(const int fd, const bool is_write, const int64_t thread_cnt, ObTableSchema* table_schema,
...
void TestTmpFileStress::prepare_one_buffer(
    const int64_t macro_block_size, const int64_t start_index, char* buf, int64_t& end_index)
{
  int ret = OB_SUCCESS;
  ObStoreRow row;
  BufHeader header;
...
void TestTmpFileStress::prepare_plain_data(const int64_t buf_capacity, char* buf, ObIArray<int64_t>& size_array)
{
  ObRandom random;
...
TEST_F(TestTmpFile, test_page_io_info_unrelease)
{
  ...
  {
    int64_t begin_used = ObTmpPageCache::get_instance().allocator_.used();
    {
      ObTmpPageCache::ObTmpMultiPageIOCallback callback;
      callback.allocator_ = &(ObTmpPageCache::get_instance().allocator_);
      ...
      int64_t after_alloc_io_buf_used = ObTmpPageCache::get_instance().allocator_.used();
      ASSERT_EQ(after_alloc_io_buf_used, begin_used + tmp_buf_size + DIO_READ_ALIGN_SIZE);

      int64_t after_process_pos = ObTmpPageCache::get_instance().allocator_.used();
    ...
    ASSERT_EQ(begin_used, ObTmpPageCache::get_instance().allocator_.used());
  }
