...
TEST_F(TestSSTableScnRangeCut, sstable_scn_range_no_cross_and_continue)
{
  ...
  ASSERT_EQ(OB_SUCCESS, ret);
  ret = tablet_table_store.cut_ha_sstable_log_ts_range_(minor_sstables, tables_handle);
  ASSERT_EQ(OB_SUCCESS, ret);

  ASSERT_EQ(0, tables_handle.get_table(0)->key_.log_ts_range_.start_log_ts_);
  ASSERT_EQ(100, tables_handle.get_table(0)->key_.log_ts_range_.end_log_ts_);

  ASSERT_EQ(100, tables_handle.get_table(1)->key_.log_ts_range_.start_log_ts_);
  ASSERT_EQ(200, tables_handle.get_table(1)->key_.log_ts_range_.end_log_ts_);

  ASSERT_EQ(200, tables_handle.get_table(2)->key_.log_ts_range_.start_log_ts_);
  ASSERT_EQ(300, tables_handle.get_table(2)->key_.log_ts_range_.end_log_ts_);
}
...
TEST_F(TestSSTableScnRangeCut, sstable_scn_range_contain)
{
  ...
  ASSERT_EQ(200, tables_handle.get_table(2)->key_.log_ts_range_.start_log_ts_);
  ASSERT_EQ(500, tables_handle.get_table(2)->key_.log_ts_range_.end_log_ts_);
