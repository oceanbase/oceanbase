...
protected:
  ...
  virtual int get_gts_(palf::SCN &gts) override;
  virtual int wait_gts_elapse_commit_version_(bool &need_wait) override;
  virtual int get_local_max_read_version_(palf::SCN &local_max_read_version) override;
  virtual int update_local_max_commit_version_(const palf::SCN &) override;
private:
