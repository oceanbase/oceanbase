...
int MockObLocationCache::init()
{
  ...
  if (0 != (tmp_ret = partition_addr_map_.create(BUCKET_NUM, ObModIds::OB_HASH_NODE))) {
    TRANS_LOG(ERROR, "create hashmap for MockObLocationCache error", K(tmp_ret));
...
int MockObLocationCache::add(const ObPartitionKey& partition, const ObAddr& leader)
{
  ...
  if (!partition.is_valid() || !leader.is_valid()) {
    TRANS_LOG(WARN, "invalid argument", K(partition), K(leader));
    ret = OB_INVALID_ARGUMENT;
  } else if (OB_SUCCESS != (tmp_ret = partition_addr_map_.set_refactored(partition, leader))) {
    if (OB_HASH_EXIST != tmp_ret) {
      TRANS_LOG(WARN, "add partition leader pair error", K(tmp_ret), K(partition), K(leader));
      ret = OB_ERR_UNEXPECTED;
    }
  } else {
    // do nothing
...
{
  UNUSEDx(partition, location, expire_renew_time, is_cache_hit);
  return OB_SUCCESS;
}
...
int MockObLocationCache::get_strong_leader(const ObPartitionKey& partition, ObAddr& leader, const bool force_renew)
{
  int ret = OB_SUCCESS;
  int tmp_ret = 0;
  ...
  if (!partition.is_valid()) {
    ...
    TRANS_LOG(WARN, "get leader by partition error", K(tmp_ret), K(partition));
    ret = OB_ERR_UNEXPECTED;
  } else {
    // do nothing
  }
...
int MockObLocationCache::nonblock_get_strong_leader(const ObPartitionKey& partition, ObAddr& leader)
{
  int ret = OB_SUCCESS;
...
int MockObLocationCache::nonblock_renew(
    const ObPartitionKey& partition, const int64_t expire_time, const int64_t cluster_id)
{
  UNUSED(partition);
  UNUSED(expire_time);
  UNUSED(cluster_id);
