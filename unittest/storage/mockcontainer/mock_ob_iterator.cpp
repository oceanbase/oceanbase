...
ObMockIterator::ObMockIterator(bool reverse)
    : rows_(), cursor_(0), reverse_(reverse), trans_id_(ObAddr(999, 888)), allocator_(ObModIds::OB_ST_TEMP)
{}
...
int ObMockIterator::add_row(ObStoreRow* row)
{
  ...
  if (NULL == row) {
    ...
    ObRowStoreType row_type = row->is_sparse_row_ ? SPARSE_ROW_STORE : FLAT_ROW_STORE;
    if (OB_FAIL(malloc_store_row(allocator_, row->row_val_.count_, row_clone, row_type))) {
      ...
      row_clone->dml_ = row->dml_;
      row_clone->first_dml_ = row->first_dml_;
      ...
      if (OB_NOT_NULL(row->trans_id_ptr_)) {
        row_clone->trans_id_ptr_ = row->trans_id_ptr_;
      } else if (row->row_type_flag_.is_uncommitted_row()) {  // make all row point to the same trans_id
        row_clone->trans_id_ptr_ = &trans_id_;
      }
      row_clone->row_val_.count_ = row->row_val_.count_;
      for (int64_t i = 0; OB_SUCC(ret) && i < row->row_val_.count_; ++i) {
        if (OB_SUCCESS != (ret = ob_write_obj(allocator_, row->row_val_.cells_[i], row_clone->row_val_.cells_[i]))) {
          STORAGE_LOG(WARN, "ob write obj error.", K(ret), K(row));
        }
      }
      if (OB_SUCC(ret) && SPARSE_ROW_STORE == row_type) {
        row_clone->is_sparse_row_ = true;
        if (OB_ISNULL(row_clone->column_ids_) || OB_ISNULL(row->column_ids_)) {
          ret = OB_ERR_UNEXPECTED;
          STORAGE_LOG(WARN, "column id is null", K(ret), K(row->column_ids_), K(row_clone->column_ids_));
        } else {
          MEMCPY(row_clone->column_ids_, row->column_ids_, sizeof(uint16_t) * row->row_val_.count_);
          STORAGE_LOG(INFO, "column id copy success", K(ret), K(row->row_val_.count_));
        }
      }
...
bool ObMockIterator::equals(const ObStoreRow& r1, const ObStoreRow& r2, const bool cmp_multi_version_row_flag,
    const bool cmp_is_get_and_scan_index)
{
  bool bool_ret = false;
  if (r1.flag_ != r2.flag_) {
    STORAGE_LOG(WARN, "flag not equals", K(r1), K(r2));
  } else if (!cmp_multi_version_row_flag && r1.flag_ != ObActionFlag::OP_ROW_EXIST) {
    STORAGE_LOG(DEBUG, "flag is not row exist, return true", K(r1), K(r2));
    bool_ret = true;
  } else if (r1.dml_ != r2.dml_ || r1.first_dml_ != r2.first_dml_) {
    STORAGE_LOG(WARN, "dml not equals", K(r1), K(r2));
    bool_ret = false;
    ...
    bool_ret = false;
  } else if (cmp_is_get_and_scan_index && (r1.is_get_ != r2.is_get_ || r1.scan_index_ != r2.scan_index_)) {
...
bool ObMockIterator::equals(int64_t idx, ObNewRow& other_row) const
{
  ...
  if (OB_SUCCESS != get_row(idx, this_row)) {
    ...
    if (!bool_ret) {
      STORAGE_LOG(INFO, "new row is not equal", K(idx), K(this_row->row_val_), K(other_row));
    }
  }
  return bool_ret;
}
...
bool ObMockIterator::equals(int64_t idx, ObStoreRow& other_row) const
{
  ...
  if (OB_SUCCESS != get_row(idx, this_row)) {
    ...
    if (!bool_ret) {
      STORAGE_LOG(WARN, "store row is not equal", K(idx), K(this_row->row_val_), K(other_row));
    }
  }
  return bool_ret;
}
...
ObHashMap<ObString, ObMockIteratorBuilder::ObParseFunc> ObMockIteratorBuilder::str_to_obj_parse_func_;
ObHashMap<ObString, ObMockIteratorBuilder::ObParseFunc> ObMockIteratorBuilder::str_to_info_parse_func_;
ObHashMap<ObString, ObObjMeta*> ObMockIteratorBuilder::str_to_obj_type_;
ObHashMap<ObString, int64_t> ObMockIteratorBuilder::str_to_flag_;
ObHashMap<ObString, storage::ObRowDml> ObMockIteratorBuilder::str_to_dml_;
...
int ObMockIteratorBuilder::parse_varchar(ObIAllocator* allocator, const ObString& word, ObStoreRow& row, int64_t& idx)
{
  OB_ASSERT(allocator);
  int ret = OB_SUCCESS;
  char* buf = NULL;
...
int ObMockIteratorBuilder::parse_lob(ObIAllocator* allocator, const ObString& word, ObStoreRow& row, int64_t& idx)
{
  OB_ASSERT(allocator);
  int ret = OB_SUCCESS;
  char* buf = NULL;
  if (idx >= row.row_val_.count_) {
    ret = OB_ARRAY_OUT_OF_RANGE;
  } else if (NULL == (buf = static_cast<char*>(allocator->alloc(sizeof(ObLobData))))) {
    ret = OB_ALLOCATE_MEMORY_FAILED;
  } else {
    ...
    if ((errno == ERANGE && (val == LONG_MAX || val == LONG_MIN)) || (errno != 0 && val == 0) ||
      ...
      for (int64_t i = 0; i < val; i++) {
        ObLobIndex& lob_index = lob_data->lob_idx_[i];
        lob_index.version_ = 1;
        lob_index.reserved_ = 0;
        lob_index.byte_size_ = lob_index.char_size_ = block_size;
        lob_index.logic_macro_id_.data_seq_ = i;
        lob_index.logic_macro_id_.data_version_ = 0;
      }
      lob_data->version_ = 1;
      lob_data->idx_cnt_ = val;
      lob_data->byte_size_ = lob_data->char_size_ = val * block_size;
      ObObj obj_tmp;
      obj_tmp.set_lob_value(ObLongTextType, lob_data, lob_data->get_handle_size());
      obj_tmp.set_collation_type(CS_TYPE_UTF8MB4_GENERAL_CI);
      ret = ob_write_obj(*allocator, obj_tmp, row.row_val_.cells_[idx++]);
    }
  }
...
int ObMockIteratorBuilder::parse_timestamp(ObIAllocator* allocator, const ObString& word, ObStoreRow& row, int64_t& idx)
{
  UNUSED(allocator);
...
int ObMockIteratorBuilder::parse_int(ObIAllocator* allocator, const ObString& word, ObStoreRow& row, int64_t& idx)
{
  UNUSED(allocator);
  OB_ASSERT(NULL != word.ptr() && 0 <= word.length());
...
int ObMockIteratorBuilder::parse_bigint(ObIAllocator* allocator, const ObString& word, ObStoreRow& row, int64_t& idx)
{
  UNUSED(allocator);
  OB_ASSERT(NULL != word.ptr() && 0 <= word.length());
...
int ObMockIteratorBuilder::parse_number(ObIAllocator* allocator, const ObString& word, ObStoreRow& row, int64_t& idx)
{
  OB_ASSERT(NULL != word.ptr() && 0 <= word.length());
...
int ObMockIteratorBuilder::parse_dml(ObIAllocator* allocator, const ObString& word, ObStoreRow& row, int64_t& idx)
{
  UNUSED(allocator);
  UNUSED(idx);
  OB_ASSERT(NULL != word.ptr() && 0 <= word.length());
  ...
  int ret = OB_SUCCESS;
  if (OB_SUCCESS != str_to_dml_.get_refactored(word, row.dml_)) {
    STORAGE_LOG(WARN, "failed to parse dml", K(word));
    ret = OB_HASH_NOT_EXIST;
  }
  return ret;
}

int ObMockIteratorBuilder::parse_first_dml(ObIAllocator* allocator, const ObString& word, ObStoreRow& row, int64_t& idx)
{
  UNUSED(allocator);
  UNUSED(idx);
  OB_ASSERT(NULL != word.ptr() && 0 <= word.length());
  ...
  int ret = OB_SUCCESS;
  ObRowDml first_dml = T_DML_UNKNOWN;
  if (OB_SUCCESS != str_to_dml_.get_refactored(word, first_dml)) {
    STORAGE_LOG(WARN, "failed to parse first dml", K(word));
    ret = OB_HASH_NOT_EXIST;
  } else {
    row.first_dml_ = first_dml;
    STORAGE_LOG(DEBUG, "parse first dml", K(first_dml), K(row));
  }
  return ret;
}

int ObMockIteratorBuilder::parse_flag(ObIAllocator* allocator, const ObString& word, ObStoreRow& row, int64_t& idx)
{
  UNUSED(allocator);
  UNUSED(idx);
  OB_ASSERT(NULL != word.ptr() && 0 <= word.length());
  ...
  int ret = OB_SUCCESS;
  if (OB_SUCCESS != str_to_flag_.get_refactored(word, row.flag_)) {
...
int ObMockIteratorBuilder::parse_base(ObIAllocator* allocator, const ObString& word, ObStoreRow& row, int64_t& idx)
{
  UNUSED(allocator);
  UNUSED(idx);
  OB_ASSERT(NULL != word.ptr() && 0 <= word.length());
...
int ObMockIteratorBuilder::parse_is_get(ObIAllocator* allocator, const ObString& word, ObStoreRow& row, int64_t& idx)
{
  UNUSED(allocator);
  UNUSED(idx);
  OB_ASSERT(NULL != word.ptr() && 0 <= word.length());
...
int ObMockIteratorBuilder::parse_trans_id(ObIAllocator* allocator, const ObString& word, ObStoreRow& row, int64_t& idx)
{
  UNUSED(allocator);
  UNUSED(idx);
  OB_ASSERT(NULL != word.ptr() && 0 <= word.length());
  ...
  int ret = OB_SUCCESS;
  if (OB_SUCCESS != str_to_trans_id_.get_refactored(word, row.trans_id_ptr_)) {
    STORAGE_LOG(WARN, "failed to parse is_get", K(word));
    ret = OB_HASH_NOT_EXIST;
  } else {
    STORAGE_LOG(DEBUG, "parse str_to_trans_id_", K(row), K(word), K(row.trans_id_ptr_));
  }
  return ret;
}

int ObMockIteratorBuilder::parse_scan_index(
    ObIAllocator* allocator, const ObString& word, ObStoreRow& row, int64_t& idx)
{
  UNUSED(allocator);
  UNUSED(idx);
  OB_ASSERT(NULL != word.ptr() && 0 <= word.length());
...
int ObMockIteratorBuilder::parse_multi_version_row_flag(
    ObIAllocator* allocator, const ObString& word, ObStoreRow& row, int64_t& idx)
{
  UNUSED(allocator);
  UNUSED(idx);
...
int ObMockIteratorBuilder::static_init()
{
  ...
  for (int i = 1; i <= TRANS_ID_NUM; ++i) {
    trans_id_list_[i - 1] = transaction::ObTransID(ObAddr(i, i));
  }

  if (OB_SUCCESS != (ret = str_to_obj_parse_func_.create(cal_next_prime(TYPE_NUM * 10), ObModIds::OB_HASH_BUCKET))) {
    STORAGE_LOG(WARN, "out of memory");
  } else if (OB_SUCCESS != str_to_obj_parse_func_.set_refactored(
                               ObString::make_string("int"), ObMockIteratorBuilder::parse_int) ||
             OB_SUCCESS != str_to_obj_parse_func_.set_refactored(
                               ObString::make_string("bigint"), ObMockIteratorBuilder::parse_bigint) ||
             OB_SUCCESS != str_to_obj_parse_func_.set_refactored(
                               ObString::make_string("varchar"), ObMockIteratorBuilder::parse_varchar) ||
             OB_SUCCESS != str_to_obj_parse_func_.set_refactored(
                               ObString::make_string("lob"), ObMockIteratorBuilder::parse_lob) ||
             OB_SUCCESS != str_to_obj_parse_func_.set_refactored(
                               ObString::make_string("var"), ObMockIteratorBuilder::parse_varchar) ||
             OB_SUCCESS != str_to_obj_parse_func_.set_refactored(
                               ObString::make_string("ts"), ObMockIteratorBuilder::parse_timestamp) ||
             OB_SUCCESS != str_to_obj_parse_func_.set_refactored(
                               ObString::make_string("timestamp"), ObMockIteratorBuilder::parse_timestamp) ||
             OB_SUCCESS != str_to_obj_parse_func_.set_refactored(
                               ObString::make_string("num"), ObMockIteratorBuilder::parse_number) ||
             OB_SUCCESS != str_to_obj_parse_func_.set_refactored(
                               ObString::make_string("number"), ObMockIteratorBuilder::parse_number)) {
    ret = OB_INIT_FAIL;
  ...
  if (OB_SUCCESS != (ret = str_to_obj_type_.create(cal_next_prime(TYPE_NUM * 10), ObModIds::OB_HASH_BUCKET))) {
    STORAGE_LOG(WARN, "out of memory");
  } else if (OB_SUCCESS != str_to_obj_type_.set_refactored(ObString::make_string("int"), &INT_TYPE) ||
             OB_SUCCESS != str_to_obj_type_.set_refactored(ObString::make_string("bigint"), &BIGINT_TYPE) ||
             OB_SUCCESS != str_to_obj_type_.set_refactored(ObString::make_string("varchar"), &VAR_TYPE) ||
             OB_SUCCESS != str_to_obj_type_.set_refactored(ObString::make_string("var"), &VAR_TYPE) ||
             OB_SUCCESS != str_to_obj_type_.set_refactored(ObString::make_string("lob"), &LOB_TYPE) ||
             OB_SUCCESS != str_to_obj_type_.set_refactored(ObString::make_string("ts"), &TS_TYPE) ||
             OB_SUCCESS != str_to_obj_type_.set_refactored(ObString::make_string("timestamp"), &TS_TYPE) ||
             OB_SUCCESS != str_to_obj_type_.set_refactored(ObString::make_string("num"), &NU_TYPE) ||
             OB_SUCCESS != str_to_obj_type_.set_refactored(ObString::make_string("number"), &NU_TYPE)) {
    ret = OB_INIT_FAIL;
  ...
  if (OB_SUCCESS != (ret = str_to_info_parse_func_.create(cal_next_prime(INFO_NUM), ObModIds::OB_HASH_BUCKET))) {
    STORAGE_LOG(DEBUG, "out of memory");
  } else if (OB_SUCCESS != str_to_info_parse_func_.set_refactored(
                               ObString::make_string("dml"), ObMockIteratorBuilder::parse_dml) ||
             OB_SUCCESS != str_to_info_parse_func_.set_refactored(
                               ObString::make_string("first_dml"), ObMockIteratorBuilder::parse_first_dml) ||
             OB_SUCCESS != str_to_info_parse_func_.set_refactored(
                               ObString::make_string("flag"), ObMockIteratorBuilder::parse_flag) ||
             ...
             OB_SUCCESS != str_to_info_parse_func_.set_refactored(
                               ObString::make_string("is_get"), ObMockIteratorBuilder::parse_is_get) ||
             OB_SUCCESS != str_to_info_parse_func_.set_refactored(
                               ObString::make_string("scan_index"), ObMockIteratorBuilder::parse_scan_index) ||
             OB_SUCCESS != str_to_info_parse_func_.set_refactored(
                               ObString::make_string("from_base"), ObMockIteratorBuilder::parse_base) ||
             OB_SUCCESS != str_to_info_parse_func_.set_refactored(
                               ObString::make_string("trans_id"), ObMockIteratorBuilder::parse_trans_id)) {
    ret = OB_INIT_FAIL;
  ...
  if (ret == OB_SUCCESS &&
      OB_SUCCESS != (ret = str_to_flag_.create(cal_next_prime(FLAG_NUM * 2), ObModIds::OB_HASH_BUCKET))) {
    STORAGE_LOG(WARN, "out of memeory");
  } else if (OB_SUCCESS != str_to_flag_.set_refactored(
                               ObString::make_string("OP_ROW_DOES_NOT_EXIST"), +ObActionFlag::OP_ROW_DOES_NOT_EXIST) ||
             OB_SUCCESS !=
                 str_to_flag_.set_refactored(ObString::make_string("EMPTY"), +ObActionFlag::OP_ROW_DOES_NOT_EXIST) ||
             OB_SUCCESS !=
                 str_to_flag_.set_refactored(ObString::make_string("OP_ROW_EXIST"), +ObActionFlag::OP_ROW_EXIST) ||
             OB_SUCCESS != str_to_flag_.set_refactored(ObString::make_string("EXIST"), +ObActionFlag::OP_ROW_EXIST) ||
             OB_SUCCESS != str_to_flag_.set_refactored(ObString::make_string("DELETE"), +ObActionFlag::OP_DEL_ROW) ||
             OB_SUCCESS !=
                 str_to_flag_.set_refactored(ObString::make_string("OP_DEL_ROW"), +ObActionFlag::OP_DEL_ROW)) {
    ret = OB_INIT_FAIL;
  ...
  if (ret == OB_SUCCESS &&
      OB_SUCCESS != (ret = str_to_dml_.create(cal_next_prime(DML_NUM * 2), ObModIds::OB_HASH_BUCKET))) {
    STORAGE_LOG(WARN, "out of memeory");
  } else if (OB_SUCCESS != str_to_dml_.set_refactored(ObString::make_string("T_DML_UNKNOWN"), T_DML_UNKNOWN) ||
             OB_SUCCESS != str_to_dml_.set_refactored(ObString::make_string("UNKNOWN"), T_DML_UNKNOWN) ||
             OB_SUCCESS != str_to_dml_.set_refactored(ObString::make_string("INSERT"), T_DML_INSERT) ||
             OB_SUCCESS != str_to_dml_.set_refactored(ObString::make_string("T_DML_INSERT"), T_DML_INSERT) ||
             OB_SUCCESS != str_to_dml_.set_refactored(ObString::make_string("UPDATE"), T_DML_UPDATE) ||
             OB_SUCCESS != str_to_dml_.set_refactored(ObString::make_string("T_DML_UPDATE"), T_DML_UPDATE) ||
  ...
  if (ret == OB_SUCCESS &&
      OB_SUCCESS != (ret = str_to_base_.create(cal_next_prime(BASE_NUM * 2), ObModIds::OB_HASH_BUCKET))) {
    STORAGE_LOG(WARN, "out of memeory");
  } else if (OB_SUCCESS != str_to_base_.set_refactored(ObString::make_string("TRUE"), true) ||
             OB_SUCCESS != str_to_base_.set_refactored(ObString::make_string("FALSE"), false)) {
    ret = OB_INIT_FAIL;
  ...
  if (ret == OB_SUCCESS &&
      OB_SUCCESS != (ret = str_to_is_get_.create(cal_next_prime(BASE_NUM * 2), ObModIds::OB_HASH_BUCKET))) {
    STORAGE_LOG(WARN, "out of memeory");
  } else if (OB_SUCCESS != str_to_is_get_.set_refactored(ObString::make_string("TRUE"), true) ||
             OB_SUCCESS != str_to_is_get_.set_refactored(ObString::make_string("FALSE"), false)) {
    ret = OB_INIT_FAIL;
  ...
  if (ret == OB_SUCCESS &&
      OB_SUCCESS != (ret = str_to_trans_id_.create(cal_next_prime(BASE_NUM * 3), ObModIds::OB_HASH_BUCKET))) {
    STORAGE_LOG(WARN, "out of memeory");
  } else if (OB_SUCCESS != str_to_trans_id_.set_refactored(ObString::make_string("trans_id_1"), &trans_id_list_[0]) ||
             OB_SUCCESS != str_to_trans_id_.set_refactored(ObString::make_string("trans_id_2"), &trans_id_list_[1]) ||
             OB_SUCCESS != str_to_trans_id_.set_refactored(ObString::make_string("trans_id_3"), &trans_id_list_[2]) ||
             OB_SUCCESS != str_to_trans_id_.set_refactored(ObString::make_string("trans_id_4"), &trans_id_list_[3]) ||
             OB_SUCCESS != str_to_trans_id_.set_refactored(ObString::make_string("trans_id_5"), &trans_id_list_[4]) ||
             OB_SUCCESS != str_to_trans_id_.set_refactored(ObString::make_string("trans_id_6"), &trans_id_list_[5]) ||
             OB_SUCCESS != str_to_trans_id_.set_refactored(ObString::make_string("trans_id_7"), &trans_id_list_[6]) ||
             OB_SUCCESS != str_to_trans_id_.set_refactored(ObString::make_string("trans_id_8"), &trans_id_list_[7]) ||
             OB_SUCCESS != str_to_trans_id_.set_refactored(ObString::make_string("trans_id_9"), &trans_id_list_[8]) ||
             OB_SUCCESS != str_to_trans_id_.set_refactored(ObString::make_string("trans_id_10"), &trans_id_list_[9]) ||
             OB_SUCCESS != str_to_trans_id_.set_refactored(ObString::make_string("trans_id_0"), nullptr)) {
    ret = OB_INIT_FAIL;
  ...
  if (ret == OB_SUCCESS &&
    ...
    STORAGE_LOG(WARN, "out of memeory");
             ...
             OB_SUCCESS != str_to_multi_version_row_flag_.set_refactored(ObString::make_string("C"), 1) ||
             OB_SUCCESS != str_to_multi_version_row_flag_.set_refactored(ObString::make_string("L"), 2) ||
             OB_SUCCESS != str_to_multi_version_row_flag_.set_refactored(ObString::make_string("CL"), 3) ||
             OB_SUCCESS != str_to_multi_version_row_flag_.set_refactored(ObString::make_string("F"), 4) ||
             OB_SUCCESS != str_to_multi_version_row_flag_.set_refactored(ObString::make_string("CF"), 5) ||
             OB_SUCCESS != str_to_multi_version_row_flag_.set_refactored(ObString::make_string("LF"), 6) ||
             OB_SUCCESS != str_to_multi_version_row_flag_.set_refactored(ObString::make_string("CLF"), 7) ||
             OB_SUCCESS != str_to_multi_version_row_flag_.set_refactored(ObString::make_string("U"), 8) ||
             OB_SUCCESS != str_to_multi_version_row_flag_.set_refactored(ObString::make_string("LU"), 10) ||
             OB_SUCCESS != str_to_multi_version_row_flag_.set_refactored(ObString::make_string("LM"), 18)) {
    ret = OB_INIT_FAIL;
  ...
  if (OB_SUCCESS != str_to_obj_parse_func_.set_refactored(
                        ObString::make_string("table_type"), ObMockIteratorBuilder::parse_bigint) ||
      OB_SUCCESS != str_to_obj_parse_func_.set_refactored(
                        ObString::make_string("major_version"), ObMockIteratorBuilder::parse_bigint) ||
      OB_SUCCESS != str_to_obj_parse_func_.set_refactored(
                        ObString::make_string("base_ver"), ObMockIteratorBuilder::parse_bigint) ||
      OB_SUCCESS != str_to_obj_parse_func_.set_refactored(
                        ObString::make_string("multi_ver"), ObMockIteratorBuilder::parse_bigint) ||
      OB_SUCCESS != str_to_obj_parse_func_.set_refactored(
                        ObString::make_string("snapshot_ver"), ObMockIteratorBuilder::parse_bigint) ||
      OB_SUCCESS != str_to_obj_parse_func_.set_refactored(
                        ObString::make_string("start_log_ts"), ObMockIteratorBuilder::parse_bigint) ||
      OB_SUCCESS != str_to_obj_parse_func_.set_refactored(
                        ObString::make_string("end_log_ts"), ObMockIteratorBuilder::parse_bigint) ||
      OB_SUCCESS != str_to_obj_parse_func_.set_refactored(
                        ObString::make_string("max_log_ts"), ObMockIteratorBuilder::parse_bigint)) {
    ret = OB_INIT_FAIL;
    STORAGE_LOG(WARN, "obj parse func hashtable insert failed");
  } else {
    STORAGE_LOG(DEBUG, "init obj parse func hashtable");
  }
  if (OB_SUCCESS != str_to_obj_type_.set_refactored(ObString::make_string("table_type"), &BIGINT_TYPE) ||
      OB_SUCCESS != str_to_obj_type_.set_refactored(ObString::make_string("major_version"), &BIGINT_TYPE) ||
      OB_SUCCESS != str_to_obj_type_.set_refactored(ObString::make_string("base_ver"), &BIGINT_TYPE) ||
      OB_SUCCESS != str_to_obj_type_.set_refactored(ObString::make_string("multi_ver"), &BIGINT_TYPE) ||
      OB_SUCCESS != str_to_obj_type_.set_refactored(ObString::make_string("snapshot_ver"), &BIGINT_TYPE) ||
      OB_SUCCESS != str_to_obj_type_.set_refactored(ObString::make_string("start_log_ts"), &BIGINT_TYPE) ||
      OB_SUCCESS != str_to_obj_type_.set_refactored(ObString::make_string("end_log_ts"), &BIGINT_TYPE) ||
      OB_SUCCESS != str_to_obj_type_.set_refactored(ObString::make_string("max_log_ts"), &BIGINT_TYPE)) {
    ret = OB_INIT_FAIL;
    STORAGE_LOG(WARN, "obj type hashtable insert failed");
  } else {
    STORAGE_LOG(DEBUG, "init obj type hashtable");
...
int ObMockIteratorBuilder::parse(const ObString& str, ObMockIterator& iter, uint16_t* col_id_array_list)
{
  int ret = OB_SUCCESS;
  if (!is_inited_) {
    ret = OB_NOT_INIT;
    ...
    if (OB_ITER_END == ret) {
      ...
      while (OB_SUCC(ret)) {
        ObStoreRow* row = NULL;
        if (OB_FAIL(malloc_store_row(
                *allocator_, obj_num, row, OB_NOT_NULL(col_id_array_list) ? SPARSE_ROW_STORE : FLAT_ROW_STORE))) {
          STORAGE_LOG(WARN, "failed to malloc store row", K(ret));
        } else {
          // set default value
          ...
          if (OB_ITER_END == ret) {
            ...
            iter.add_row(row);
            //            STORAGE_LOG(TRACE, "get row successfully", K(*row), K(row->trans_id_ptr_));
          }
        }
...
int ObMockIteratorBuilder::parse_header(
    const ObString& str, int64_t& pos, ObIArray<ObParseFunc>& header, int64_t& obj_num, ObMockIterator& iter)
{
  ...
  while (OB_SUCCESS == ret && pos < str.length()) {
    ...
    if (OB_FAIL(ret)) {
      ...
      } else if (OB_SUCCESS == str_to_info_parse_func_.get_refactored(word, fp) && NULL != fp) {
        ret = header.push_back(fp);
      } else {
...
int ObMockIteratorBuilder::parse_with_specified_col_ids(const ObString& str, ObMockIterator& iter,
    uint16_t* col_id_array_list /* = nullptr*/, int64_t* result_col_id_array /* = nullptr*/)
{
  int ret = OB_SUCCESS;
  if (!is_inited_) {
    ret = OB_NOT_INIT;
        ...
        if (OB_FAIL(malloc_store_row(*allocator_, obj_num, row))) {
          ...
          if (OB_ITER_END == ret) {
            ...
            if (init_col_id_flag) {
              ...
              STORAGE_LOG(TRACE, "get row successfully", K(idx), K(*row), K(row->trans_id_ptr_));
              ++idx;
            }
            iter.add_row(row);
          }
...
int ObMockIteratorBuilder::parse_row(const ObString& str, int64_t& pos, const ObIArray<ObParseFunc>& header,
    const uint16_t* col_id_array_list, ObStoreRow& row)
{
  OB_ASSERT(is_inited_);
  ...
  for (int64_t i = 0; OB_SUCC(ret) && i <= header.count(); ++i) {
    ...
    if (OB_SUCCESS != (ret = get_next_word(str, pos, word, ext))) {
      ...
      // check col num if flag is OP_ROW_EXIST
      break;
      ...
      switch (ext) {
        ...
        case EXT_MIN_2_TRANS:
          ...
          break;
        case EXT_MAGIC:
          row.row_val_.cells_[idx++].set_int(ObMagicRowManager::MAGIC_NUM);
          break;
        case NOT_EXT:
...
int ObMockIteratorBuilder::get_next_word(const ObString& str, int64_t& pos, ObString& word, int64_t& ext)
{
  OB_ASSERT(is_inited_);
  int ret = OB_SUCCESS;
...
int ObMockIteratorBuilder::write_next_char(const ObString& str, int64_t& pos, ObString& word)
{
  int ret = OB_SUCCESS;
  const char* str_ptr = str.ptr();
  char c = '\0';
