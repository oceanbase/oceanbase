...
TEST_F(FTParserTest, test_append_where_clause)
{
  ...
  ASSERT_EQ(OB_SUCCESS, r1.start_word_.set_token("a", 1));
  ...
  ranges.reset();
  ASSERT_EQ(OB_SUCCESS, r1.start_word_.set_token("a", 1));
  ASSERT_EQ(OB_SUCCESS, r1.end_word_.set_token("m", 1));
  ASSERT_EQ(OB_SUCCESS, ranges.push_back(r1));
  ObMissingRangeInfo r4;
  ASSERT_EQ(OB_SUCCESS, r4.start_word_.set_token("n", 1));
...
TEST_F(FTParserTest, test_collect_missing_ranges)
{
  ...
  ObFTSingleToken start_word;
  ObFTSingleToken end_word;
  ASSERT_EQ(OB_SUCCESS, start_word.set_token("a", 1));
  ASSERT_EQ(OB_SUCCESS, end_word.set_token("m", 1));

  TestFTDictCacheLoaderExec loader;
  ret = loader.collect_missing_ranges(start_word, end_word, 1, 2, 100, &missing_ranges);
  ASSERT_EQ(OB_SUCCESS, ret);
...
TEST_F(FTParserTest, test_partial_rebuild_each_range_matches_full_build)
{
  ...
  for (int32_t missing_range_id = 1; missing_range_id < full_range_count; ++missing_range_id) {
    ...
    ASSERT_EQ(OB_SUCCESS, partial_iter.init(dict_text,
                                            missing_ranges.at(0).start_word_.get_token(),
                                            missing_ranges.at(0).end_word_.get_token()));
