...
void TestMultiVersionSSTableMultiGet::prepare_query_param(const ObVersionRange& trans_version_range)
{
  columns_.reset();
  param_.reset();
  context_.reset();
...
void TestMultiVersionSSTableMultiGet::prepare_rowkeys(
...
TEST_F(TestMultiVersionSSTableMultiGet, multi_get_on_empty_sstable)
{
...
TEST_F(TestMultiVersionSSTableMultiGet, multi_get_normal)
{
  const int64_t rowkey_cnt = 4;
  const char* micro_data[3];
  micro_data[0] = "bigint   var   bigint bigint  bigint   bigint  flag    multi_version_row_flag\n"
  ...
  const int64_t col_cnt[] = {6, 6, 6, 6, 6, 6, 6, 6, 6, 6, 6, 6, 6, 6, 6};
...
TEST_F(TestMultiVersionSSTableMultiGet, multi_get_not_exist)
{
  const int64_t rowkey_cnt = 4;
  const char* micro_data[3];
  micro_data[0] = "bigint   var   bigint bigint  bigint   bigint  flag    multi_version_row_flag\n"
...
TEST_F(TestMultiVersionSSTableMultiGet, span_macro_block)
{
  const int64_t rowkey_cnt = 4;
...
TEST_F(TestMultiVersionSSTableMultiGet, bug_14915264)
{
  const int64_t rowkey_cnt = 4;
  ...
  prepare_data_start(micro_data, rowkey_cnt, 100, "none", FLAT_ROW_STORE, 0);
...
// TEST_F(TestMultiVersionSSTableMultiGet, test_bloom_filter)
//{
// const int64_t rowkey_cnt = 4;
...
TEST_F(TestMultiVersionSSTableMultiGet, sync_io)
{
  const int64_t rowkey_cnt = 4;
  const char* micro_data[7];
  micro_data[0] = "bigint   var   bigint bigint  bigint   bigint  flag    multi_version_row_flag\n"
  ...
  prepare_data_start(micro_data, rowkey_cnt, snapshot_version, "none", FLAT_ROW_STORE, 0);
  for (int i = 0; i < 7; i++) {
    prepare_one_macro(&micro_data[i], 1, snapshot_version);
  }
  prepare_data_end();
...
TEST_F(TestMultiVersionSSTableMultiGet, get_with_magic_row)
{
  const int64_t rowkey_cnt = 4;
  const char* micro_data[7];
  micro_data[0] = "bigint   var   bigint bigint  bigint   bigint  flag    multi_version_row_flag\n"
  ...
  micro_data[3] = "bigint   var   bigint bigint  bigint   bigint  flag    multi_version_row_flag\n"
                  ...
                  "3        var7  -2      0       6        NOP     EXIST   N\n"
                  "3        var7  -1      0       6        NOP     EXIST   CL\n"
                  "3        var8  -9      0       7        NOP     EXIST   CL\n"
                  "3        var9  -9      0       7        NOP     EXIST   CL\n";
  micro_data[4] = "bigint   var   bigint bigint  bigint   bigint  flag    multi_version_row_flag\n"
                  "4        var4  -7      0       6        5       EXIST   C\n"
                  "4        var4  -5      0       6        5       EXIST   N\n"
                  "4        var4  -3      0       6        5       EXIST   CL\n"
                  "4        var4  MAGIC   MAGIC   NOP      NOP     EXIST   LM\n";

  micro_data[5] = "bigint   var   bigint bigint  bigint   bigint  flag    multi_version_row_flag\n"
                  "5        var5  MAGIC   MAGIC   NOP      NOP     EXIST   LM\n";
...
int main(int argc, char** argv)
{
  ...
  system("rm -rf test_multi_version_sstable_multi_get.log");
  OB_LOGGER.set_file_name("test_multi_version_sstable_multi_get.log");
  STORAGE_LOG(INFO, "begin unittest: test_multi_version_sstable_multi_get");
