...
const int64_t COLUMN_CNT = 64;
const int64_t BATCH_SIZE = 500000;
...
protected:
  ...
  ExprFixedArray exprs_;
  ExprFixedArray var_exprs_;
  ExprFixedArray iregular_exprs_;
};

int TestCompactChunk::init_exprs()
{
  int ret = OB_SUCCESS;
  exprs_.set_allocator(&allocator_);
  var_exprs_.set_allocator(&allocator_);
  iregular_exprs_.set_allocator(&allocator_);

  if (OB_FAIL(exprs_.prepare_allocate(COLUMN_CNT))) {
    LOG_WARN("fail to alloc expr", K(ret));
  } else if (OB_FAIL(var_exprs_.prepare_allocate(COLUMN_CNT))) {
    LOG_WARN("fail to alloc expr", K(ret));
  } else if (OB_FAIL(iregular_exprs_.prepare_allocate(COLUMN_CNT))) {
    LOG_WARN("fail to alloc expr", K(ret));
  } else {
    for (int64_t i = 0; OB_SUCC(ret) && i < COLUMN_CNT; i++) {
      ObExpr *expr = new (allocator_.alloc(sizeof(ObExpr))) ObExpr();
      expr->datum_meta_.type_ = ObIntType;
      exprs_.at(i) = expr;
    }
    for (int64_t i = 0; OB_SUCC(ret) && i < COLUMN_CNT; i++) {
      ObExpr *expr = new (allocator_.alloc(sizeof(ObExpr))) ObExpr();
      expr->datum_meta_.type_ = ObVarcharType;
      var_exprs_.at(i) = expr;
    }
    for (int64_t i = 0; OB_SUCC(ret) && i < COLUMN_CNT; i++) {
      ObExpr *expr = new (allocator_.alloc(sizeof(ObExpr))) ObExpr();
      if (i % 2) {
...
void TestCompactChunk::SetUp()
{
  ...
  EXPECT_EQ(OB_SUCCESS, init_exprs());
...
TEST_F(TestCompactChunk, test_read_iter_end)
{
  int ret = OB_SUCCESS;
  ObCompactStore cs_chunk;
  cs_chunk.init(1, 1,
        ObCtxIds::DEFAULT_CTX_ID, "SORT_CACHE_CTX", true, 0, true, true, NONE_COMPRESSOR, &exprs_);
  ...
  int64_t pos = 0;
  for (int64_t i = 0; OB_SUCC(ret) && i < 1; i++) {
  ...
  const StoredRow *dummy_sr;
  EXPECT_EQ(OB_ITER_END, cs_chunk.get_next_row(dummy_sr));
}
...
TEST_F(TestCompactChunk, test_batch)
{
  ...
  ObCompactStore cs_chunk;
  cs_chunk.init(1, 1,
        ObCtxIds::DEFAULT_CTX_ID, "SORT_CACHE_CTX", true, 0, true, true, NONE_COMPRESSOR, &iregular_exprs_);
...
TEST_F(TestCompactChunk, test_read_writer_compact_vardata)
{
  ...
  cs_chunk.init(1, 1,
        ObCtxIds::DEFAULT_CTX_ID, "SORT_CACHE_CTX", true, 0, true, true, NONE_COMPRESSOR, &var_exprs_);
...
TEST_F(TestCompactChunk, test_irregular)
{
  ...
  ObCompactStore cs_chunk;
  cs_chunk.init(1, 1,
        ObCtxIds::DEFAULT_CTX_ID, "SORT_CACHE_CTX", true, 0, false, true, NONE_COMPRESSOR, &var_exprs_);
  StoredRow **sr;
  ret = row_generate_.get_stored_row_irregular(sr);
  ASSERT_EQ(ret, OB_SUCCESS);
