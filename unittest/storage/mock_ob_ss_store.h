...
public:
  ...
  MOCK_METHOD4(get, int(const storage::ObTableAccessParam& param, storage::ObTableAccessContext& context,
                        const common::ObExtStoreRowkey& rowkey, ObStoreRowIterator*& row_iter));
  MOCK_METHOD4(scan, int(const ObTableAccessParam& param, ObTableAccessContext& context,
                         const common::ObExtStoreRange& key_range, ObStoreRowIterator*& row_iter));
  MOCK_METHOD4(
      multi_get, int(const ObTableAccessParam& param, ObTableAccessContext& context,
                     const common::ObIArray<common::ObExtStoreRowkey>& rowkeys, ObStoreRowIterator*& row_iter));
  MOCK_METHOD4(multi_scan, int(const ObTableAccessParam& param, ObTableAccessContext& context,
                               const common::ObIArray<common::ObExtStoreRange>& ranges, ObStoreRowIterator*& row_iter));

  MOCK_METHOD5(
      estimate_get_cost, int(const common::ObQueryFlag query_flag, const uint64_t table_id,
  ...
  MOCK_METHOD5(estimate_scan_cost,
      int(const common::ObQueryFlag query_flag, const uint64_t table_id, const common::ObExtStoreRange& key_range,
          const common::ObIArray<share::schema::ObColDesc>& columns, ObPartitionEst& cost_metrics));

  MOCK_METHOD5(estimate_multi_scan_cost,
      int(const common::ObQueryFlag query_flag, const uint64_t table_id,
  ...
  MOCK_CONST_METHOD0(get_store_type, enum storage::ObStoreType());
};
