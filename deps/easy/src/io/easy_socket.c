...
static ssize_t easy_socket_chain_writev(
    easy_connection_t* conn, easy_list_t* l, struct iovec* iovs, int cnt, int* again);
...
int easy_unix_domain_listen(const char* path, int backlog)
{
  ...
  if (strlen(path) >= sizeof(addr.sun_path)) {
    easy_error_log("file path too long.");
    goto error_exit;
  }
...
int easy_socket_listen(int udp, easy_addr_t* address, int* flags, int backlog)
{
  ...
  if (v) {
    ...
    if (easy_socket_set_opt(fd, SO_REUSEPORT, 1) == 0) {
      ...
      if (address->family == AF_INET && flags && (*flags & EASY_FLAGS_NOLISTEN)) {
        udp = 2;
...
{
  ssize_t n;
  EASY_SOCKET_IO_TIME_GUARD(
      (ev_read_count++, ev_read_time += cost), ", socket recv, fd(%d), conn(%p).", conn->fd, conn);
...
#define EASY_SOCKET_RET_CHECK(ret, size, again) \
  if (ret < 0)                                  \
    return ret;                                 \
  else                                          \
    size += ret;                                \
...
ssize_t easy_socket_tcpwrite(easy_connection_t* conn, easy_list_t* l)
{
  ...
  EASY_SOCKET_IO_TIME_GUARD((ev_write_count++, ev_write_time += cost),
      ", socket write, fd(%d), iov(%ld/%ld), conn(%p).",
      conn->fd,
      wbyte,
      cnt,
      conn);
