...
#define ma_log(level, fd, format, ...) \
  easy_##level##_log("easy dispatch socket: fd=%d remote=%s " format, fd, easy_peername(fd), ##__VA_ARGS__)
...
static void easy_ma_dispatch(int idx, int fd)
{
  ...
  int wb = 0;
  while ((wb = write(g_ma.chan[t], &fd, sizeof(fd))) < 0 && errno == EINTR)
    ;
  if (wb != sizeof(fd)) {
...
static int read_sock_pri(int fd)
{
  int rb = 0;
  uint8_t idx = 0;
  while ((rb = recv(fd, &idx, sizeof(idx), MSG_OOB)) < 0 && EINTR == errno)
    ;
...
error_exit:
  easy_error_log("easy dispatch socket thread start fail: gcount=%d addr=%s lfd=%d efd=%d errno=%d epoll_add_succ=%d",
      g_ma.g_count,
      addr_buf,
      lfd,
      efd,
