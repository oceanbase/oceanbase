...
static int easy_connection_do_accept_one(
    struct ev_loop* loop, ev_io* w, easy_listen_simple_t* listen, int fd, easy_addr_t addr);
...
easy_listen_t* easy_connection_add_listen(easy_io_t* eio, const char* host, int port, easy_io_handler_pt* handler)
{
...
easy_listen_t* easy_add_listen(easy_io_t* eio, const char* host, int port, easy_io_handler_pt* handler, void* args)
{
  easy_addr_t address;
  int udp;
 ...
 */
easy_listen_t* easy_add_listen_addr(easy_io_t* eio, easy_addr_t addr, easy_io_handler_pt* handler, int udp, void* args)
{
  int i, size, cnt, fd;
...
void easy_connection_wakeup_session(easy_connection_t* c)
{
   ...
   *
   */
  if (c->type != EASY_TYPE_SERVER || c->conn_has_error == 0 || easy_list_empty(&c->server_session_list)) {
    return;
  }
...
void easy_connection_destroy(easy_connection_t* c, const char* msg)
{
  ...
  if (c->fd >= 0) {
    ...
    easy_info_log("Socket closed, fd(%d), conn(%s), ev_is_pending(%d), ev_is_active(%d),"
        ...
        ev_is_pending(&c->timeout_watcher),
        ev_is_active(&c->timeout_watcher),
        ev_timer_pending_addr,
        ev_timer_pending,
        &c->timeout_watcher);
    close(c->fd);
    c->fd = -1;
  ...
  if (c->auto_reconn && eio->stoped == 0) {
    ...
    easy_info_log(
        "start autoreconn, conn(%s), reconn_time(%f), reconn_fail(%d).\n", easy_connection_str(c), t, c->reconn_fail);
 ...
 * connect args
 */
easy_session_t* easy_connection_connect_init(
    easy_session_t* s, easy_io_handler_pt* handler, int conn_timeout, void* args, int flags, char* servername)
{
 ...
 * connect async
 */
int easy_connection_connect(
    easy_io_t* eio, easy_addr_t addr, easy_io_handler_pt* handler, int conn_timeout, void* args, int flags)
{
 ...
 * connect sync
 */
easy_connection_t* easy_connection_connect_thread(
    easy_io_t* eio, easy_addr_t addr, easy_io_handler_pt* handler, int conn_timeout, void* args, int flags)
{
  easy_session_t s, *ps;
...
int easy_connection_session_build(easy_session_t* s)
{
  ...
  if (s->type == EASY_TYPE_KEEPALIVE_SESSION) {
    ...
    EASY_PRINT_BT(
        "doing_request_count_inc:%d,c:%s,r:%p,%ld.", c->doing_request_count, easy_connection_str(c), &s->r, s->r.uuid);
#endif
...
static int easy_connection_accept_one(struct ev_loop* loop, ev_io* w)
{
  ...
  easy_info_log(
      "Accepted connect request from client(%s).", easy_inet_addr_to_str(&eaddr, dest_addr, 32));  // easy_debug_log ??
    ...
    struct ev_loop* loop, ev_io* w, easy_listen_simple_t* listen, int fd, easy_addr_t addr)
{
   ...
   */
  if (c->handler->is_ssl && !c->handler->is_ssl_opt && SSM_USE_SSL == c->ssl_sm_ && NULL == c->sc &&
      NULL != ioth->eio->ssl) {
  ...
  easy_info_log(
      "Connection established on server side, conn(%s), cb(%p).\n", easy_connection_str(c), ev_cb(&c->read_watcher));
  return EASY_OK;
}
...
void easy_connection_on_readable(struct ev_loop* loop, ev_io* w, int revents)
{
  ...
  if (unlikely(c->type == EASY_TYPE_SERVER && (c->doing_request_count > EASY_CONN_DOING_REQ_CNT ||
  ...
  do {
     ...
     */
    pending = 0;
    if (SSM_BEFORE_FIRST_PKT == c->ssl_sm_ && c->handler->is_ssl && c->handler->is_ssl_opt &&
        c->type == EASY_TYPE_SERVER) {
    ...
    if ((n = (c->read)(c, m->input->last, m->next_read_len, &pending)) <= 0) {
      ...
      c->conn_has_error = 0;
      if (n == 0) {
        easy_info_log("Read eof from connection. Socket may have been shut down, conn(%s).", easy_connection_str(c));
        if (c->type == EASY_TYPE_CLIENT) {
...
static int easy_connection_do_request(easy_message_t* m)
{
  ...
  while (m->input->pos < m->input->last) {
    ...
    EASY_PRINT_BT(
        "doing_request_count_inc:%d,c:%s,r:%p,%ld.", c->doing_request_count, easy_connection_str(c), r, r->uuid);
#endif
  }
  ...
  if (SSM_USE_SSL == c->ssl_sm_ && c->handler->is_ssl && c->handler->is_ssl_opt && NULL == c->sc &&
      NULL != c->ioth->eio->ssl) {
    ...
    if (EASY_OK != easy_spinrwlock_rdlock(&(c->ioth->eio->ssl_rwlock_))) {
      easy_error_log("easy_spinrwlock_rdlock failed %s, ref_cnt %ld, wait_write %ld\n",
          easy_connection_str(c),
          c->ioth->eio->ssl_rwlock_.ref_cnt,
          c->ioth->eio->ssl_rwlock_.wait_write);
      need_destory = 1;
    } else {
...
void easy_connection_on_writable(struct ev_loop* loop, ev_io* w, int revents)
{
  ...
  if ((ret = easy_connection_write_socket(c)) == EASY_ABORT) {
    easy_warn_log("Failed to write(on_writable), conn(%s)", easy_connection_str(c));
    goto error_exit;
  }
  ...
  if (c->type == EASY_TYPE_CLIENT) {
    ...
    if (c->status == EASY_CONN_CONNECTING) {
      ...
      if (easy_socket_error(c->fd) != 0) {
        c->conn_has_error = 1;
        easy_error_log("Failed to establish connection(%s)", easy_connection_str(c));
        goto error_exit;
      }

      if (0 != easy_ma_handshake(c->fd, c->addr.cidx)) {
        easy_error_log("Failed to do ma handshake on connection(%s).", easy_connection_str(c));
        goto error_exit;
      }

      // on connect
      if (c->handler->on_connect && (c->handler->on_connect)(c) == EASY_ERROR) {
        easy_error_log("Failed to do on_connect on connection(%s)", easy_connection_str(c));
        goto error_exit;
      }
...
static void easy_connection_on_timeout_session(struct ev_loop* loop, ev_timer* w, int revents)
{
  ...
  if (easy_session_process_keep_connection_resilient(s, 1, EASY_TIMEOUT) == EASY_ERROR) {
    easy_error_log("Failed to proccess session (keep resilient), s(%p), conn(%s).", s, easy_connection_str(c));
  ...
  */
  if (conn->status == EASY_CONN_AUTO_CONN) {
    ...
    if (conn->handler->on_idle && (conn->idle_time / 1000.0 < t)) {
      ...
      if (ret == EASY_ERROR) {
        easy_error_log(
            "Easy on_idle failed, and will destroy connection(%s), errno(%d).", easy_connection_str(conn), errno);
        goto destroy_conn;
      }
    }
    ...
    if ((conn->keepalive_enabled) && (ack_timeout_ms > 0) && (conn->magic_ver >= MIN_MAGIC_VERSION_KEEPALIVE)) {
       ...
       */
      if ((conn->tx_keepalive_cnt < 3) || (now > (conn->last_tx_tstamp + ack_timeout_ms / 3000 - 0.1))) {
        if (conn->handler->new_keepalive_packet) {
          ret = (conn->handler->new_keepalive_packet)(conn);
          if (ret != EASY_OK) {
            easy_warn_log("Failed to send keepalive on connection(%s).", easy_connection_str(conn));
            if (ret == EASY_ABORT) {
              /*
    ...
    if (ack_timeout_ms > 0) {
      ...
      if (ret == 0) {
        if (conn->ack_wait_start > 0 && (now > conn->ack_wait_start + ack_timeout_ms / 1000.0 - 0.1)) {
          easy_error_log("Failed to do check_ack_timeout, and will destroy connection(%s).", easy_connection_str(conn));
          conn->conn_has_error = 1;
          goto destroy_conn;
        }
      } else {
        easy_info_log("Unexpected problem, conn(%s), ev_is_pending(%d), ev_is_active(%d),"
                      " ev_timer_pending_addr(%p), ev_timer_pending(%d), timeout_watcher(%p).",
            easy_connection_str(conn),
            ev_is_pending(&conn->timeout_watcher),
            ev_is_active(&conn->timeout_watcher),
            ev_timer_pending_addr,
            ev_timer_pending,
            &conn->timeout_watcher);
        /*
...
static easy_connection_t* easy_connection_do_connect(easy_client_t* client, int fd, int is_ssl_for_test)
{
  ...
  if (connect(fd, (struct sockaddr*)&addr, sizeof(addr)) < 0) {
    if (errno != EINPROGRESS) {
      easy_error_log(
          "Failed to do connect, conn(%s), errno(%d), strerror(%s).\n", easy_connection_str(c), errno, strerror(errno));
      goto error_exit;
    }
  ...
  c->send_queue = thread_local_send_queue
                      ?: (thread_local_send_queue = easy_hash_create_without_pool(EASY_IOTH_SELF->eio->send_qlen, v));
    ...
    if (client->c == NULL) {
      ...
      if (client->c == NULL) {
        ...
        easy_info_log("Establishing connection(%s), s(%p), is_ssl(%d).\n",
            easy_connection_str(client->c),
            s,
            NULL != client ? client->is_ssl : -1);
      }
    }
...
char* easy_connection_str(easy_connection_t* c)
{
  ...
  lnprintf(buffer,
      192,
      "%s_%s_%d_%p tp=%d t=%ld-%ld s=%d r=%d io=%ld/%ld sq=%ld",
      easy_inet_addr_to_str(&local_eaddr, local_addr, 32),
      easy_inet_addr_to_str(&c->addr, dest_addr, 32),
      c->fd,
      c,
      c->type,
      (int64_t)(1000000LL * c->start_time),
      (int64_t)(1000000LL * c->last_time),
      c->status,
      c->doing_request_count,
      c->recv_bytes,
      c->send_bytes,
...
#ifdef HAVE_RECVMMSG
  size =
      sizeof(easy_connection_t) + sizeof(easy_message_t) + sizeof(easy_buf_t) +
      (blen + sizeof(struct sockaddr_storage) + sizeof(struct mmsghdr) + sizeof(struct iovec)) * ioth->eio->recv_vlen;
#else
  size =
      sizeof(easy_connection_t) + sizeof(easy_message_t) + blen + sizeof(easy_buf_t) + sizeof(struct sockaddr_storage);
#endif
...
{
  easy_info_log(
      "Shutting down socket with SHUT_RD, fd(%d), conn(%s), lbt(%s).\n", c->fd, easy_connection_str(c), easy_lbt());
