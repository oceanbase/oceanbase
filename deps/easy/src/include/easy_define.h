...
#define easy_ignore(exp)                        \
  {                                             \
    int ignore __attribute__((unused)) = (exp); \
  }
...
#define EASY_REACH_TIME_INTERVAL(i)                                                          \
  ({                                                                                         \
    ...
    if ((i + last_time) < cur_time && easy_atomic_cmp_set(&last_time, old_time, cur_time)) { \
      bret = 1;                                                                              \
    }                                                                                        \
    bret;                                                                                    \
  })
