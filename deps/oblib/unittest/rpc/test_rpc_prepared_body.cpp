...
TEST_F(TestRpcPreparedBody, prepared_entry_rejects_invalid_body_and_clears_outputs)
{
  ...
  EXPECT_EQ(OB_STATE_NOT_MATCH, rpc_encode_prepared_req(
      proxy, 0, OB_LOG_PUSH_REQ, opts, body, false, req, req_size));
...
TEST_F(TestRpcPreparedBody, cached_encoder_rejects_original_body_over_the_new_limit)
{
  ...
  EXPECT_EQ(OB_RPC_PACKET_TOO_LONG, rpc_encode_req(
      proxy, 0, OB_LOG_PUSH_REQ, args, opts, req, req_size, false));
  EXPECT_EQ(nullptr, req);
  EXPECT_EQ(0, req_size);
  EXPECT_TRUE(body.matches(OB_LOG_PUSH_REQ, TENANT_ID, LZ4_COMPRESSOR));
  EXPECT_EQ(1, args.serialize_count_);
...
// This group is reserved for this test executable. PNIO retains groups and
// allocators for process lifetime; stop/wait releases the running thread.
class ScopedPreparedBodyPnioClient
{
public:
  enum { GROUP_ID = 127 };
  ScopedPreparedBodyPnioClient() : thread_count_(pn_provision(-1, GROUP_ID, 1)) {}
  ~ScopedPreparedBodyPnioClient()
  {
    pn_stop(GROUP_ID);
    pn_wait(GROUP_ID);
  }
  uint64_t gtid() const { return static_cast<uint64_t>(GROUP_ID) << 32; }
  int thread_count_;
private:
  DISALLOW_COPY_AND_ASSIGN(ScopedPreparedBodyPnioClient);
};

class ScopedPreparedBodyPnioRequest
{
public:
  ScopedPreparedBodyPnioRequest() : data_(nullptr), size_(0) {}
  ~ScopedPreparedBodyPnioRequest()
  {
  ...
  int64_t size_;
private:
  DISALLOW_COPY_AND_ASSIGN(ScopedPreparedBodyPnioRequest);
};
...
TEST_F(TestRpcPreparedBody, prepared_entry_copies_outlive_the_cached_body)
{
  ...
  MarkerExtraPayload extra;
  ScopedRuntimePayload runtime_guard(false, extra);
  ScopedPreparedBodyPnioClient client;
  ASSERT_EQ(1, client.thread_count_);
  rpc::frame::ObReqTransport transport(nullptr, nullptr);
  ObRpcProxy proxy;
  ASSERT_EQ(OB_SUCCESS, proxy.init(&transport, 44));
  proxy.set_tenant(TENANT_ID);
  ...
  // Free both real PNIO allocations before stopping the client, even on ASSERT.
  ScopedPreparedBodyPnioRequest first;
  ScopedPreparedBodyPnioRequest second;
  int64_t body_size = 0;
  {
    ObRpcPreparedBody body;
    proxy.set_reusable_body(&body);
    ASSERT_EQ(OB_SUCCESS, rpc_encode_req(proxy, client.gtid(), OB_LOG_PUSH_REQ,
        args, opts, first.data_, first.size_, false));
    ASSERT_TRUE(OB_NOT_NULL(first.data_));
    ASSERT_TRUE(body.matches(OB_LOG_PUSH_REQ, TENANT_ID, LZ4_COMPRESSOR));
    ASSERT_TRUE(body.is_compressed());
    ...
    ASSERT_EQ(OB_SUCCESS, rpc_encode_req(proxy, client.gtid(), OB_LOG_PUSH_REQ,
        args, opts, second.data_, second.size_, false));
    ASSERT_TRUE(OB_NOT_NULL(second.data_));
    EXPECT_NE(first.data_, second.data_);
  ...
  ObRpcPacket packet1;
  ObRpcPacket packet2;
  ASSERT_EQ(OB_SUCCESS, packet1.decode(first.data_, first.size_));
  ASSERT_EQ(OB_SUCCESS, packet2.decode(second.data_, second.size_));
  EXPECT_EQ(ObRpcPacket::get_header_size() + body_size, first.size_);
  EXPECT_EQ(first.size_, second.size_);
  ...
  first.data_[ObRpcPacket::get_header_size()] ^= 1;
