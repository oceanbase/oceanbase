...
void ObLogTestThread::run1()
{
  ...
  ObString str("Our destiny offers not the cup of despair, but the chalice of opportunity. So let us seize it, not in "
               "fear, but in gladness.");
  ...
  if (thread_i++ % 3 == 0) {
    OB_LOGGER.print_trace_buffer(OB_LOG_LEVEL(ERROR));
    OB_LOGGER.set_trace_mode(false);
  }
  if (thread_i++ % 4 == 0) {
    PRINT_OB_LOG_TRACE_BUF(TRACE);
...
void ObLogTestThreadT::run1()
{
  int64_t b_time = ::oceanbase::common::ObTimeUtility::current_time();
  {
    ObLoggerTraceMode trace;
    ObString str("Our destiny offers not the cup of despair, but the chalice of opportunity. So let us seize it, not "
                 "in fear, but in gladness.");
    ...
    trace.print_trace_buffer(OB_LOG_LEVEL(TRACE));
  }
  int64_t e_time = ::oceanbase::common::ObTimeUtility::current_time();
  OB_LOG(WARN, "yangze one thread time", "u_time", e_time - b_time);
}
