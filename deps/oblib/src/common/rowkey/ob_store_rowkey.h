...
public:
  ...
  // FIXME-: the following interfaces are only kept for compatibility
  ...
  {
    return key_.get_obj_cnt();
  }
  inline const ObObj* get_obj_ptr() const
  {
    return key_.get_obj_ptr();
  }
  ...
  inline int is_max(const ObIArrayWrap<ObOrderType>& column_orders, const int64_t rowkey_cnt, bool& is_max_rowkey) const
  {
    return is_min_max(column_orders, rowkey_cnt, is_max_rowkey, MinMaxFlag::MAX);
  }
  inline int is_min(const ObIArrayWrap<ObOrderType>& column_orders, const int64_t rowkey_cnt, bool& is_min_rowkey) const
  {
    return is_min_max(column_orders, rowkey_cnt, is_min_rowkey, MinMaxFlag::MIN);
  }
  inline int set_max(const ObIArray<ObOrderType>& column_orders, ObIAllocator& allocator)
  {
    return set_min_max(column_orders, allocator, MinMaxFlag::MAX);
  }
  inline int set_min(const ObIArray<ObOrderType>& column_orders, ObIAllocator& allocator)
  {
    return set_min_max(column_orders, allocator, MinMaxFlag::MIN);
  }

  inline void assign(ObObj* ptr, const int64_t cnt)
  {
    key_.assign(ptr, cnt);
    if (OB_UNLIKELY(!key_.is_legal())) {
      COMMON_LOG(ERROR, "attempt to assign an illegal key!", KP(ptr), K(cnt));
  ...
  {
    return key_.get_deep_copy_size();
  }
  ...
  inline int checksum(ObBatchChecksum& bc) const
  {
    return key_.checksum(bc);
  }
  ...
  static inline int get_common_prefix_length(const ObStoreRowkey& lhs, const ObStoreRowkey& rhs, int64_t& prefix_len)
  {
  ...
  int compare(const ObStoreRowkey& rhs, const ObRowkeyInfo* rowkey_info, int& cmp) const;
  int compare(const ObStoreRowkey& rhs, const ObIArray<ObOrderType>& column_orders, int& cmp) const;
  OB_INLINE int compare(const ObStoreRowkey& rhs, int& cmp) const
  {
    return key_.compare(rhs.key_, cmp);
  }
  // TODO : remove this function
  OB_INLINE int compare(const ObStoreRowkey& rhs) const
  {
    return key_.compare(rhs.key_);
  }
  OB_INLINE int compare_prefix(const ObStoreRowkey& rhs, int& cmp) const
  {
...
public:
  ...
  {
    return key_.is_max_row();
  }
  inline bool is_min() const
  {
    return key_.is_min_row();
  }
  inline void set_max()
  {
    *this = MAX_STORE_ROWKEY;
  }
  inline void set_min()
  {
    *this = MIN_STORE_ROWKEY;
  }
...
public:
  ...
  int check_use_collation_free(const bool exist_invalid_macro_meta_collation_free, bool& use_collation_free) const;
  int to_collation_free_on_demand_and_cutoff_range(ObIAllocator& allocator);
  int get_possible_range_pos();

  OB_INLINE bool is_range_cutoffed() const
  {
    return range_cut_pos_ >= 0;
  }
  OB_INLINE int16_t get_range_cut_pos() const
  {
    return range_cut_pos_;
  }
  OB_INLINE int16_t get_first_null_pos() const
  {
    return first_null_pos_;
  }
  OB_INLINE bool is_range_check_min() const
  {
    return range_check_min_;
  }
  const ObStoreRowkey& get_store_rowkey() const
  {
    return store_rowkey_;
  }
  ObStoreRowkey& get_store_rowkey()
  {
    return store_rowkey_;
  }
  const ObStoreRowkey& get_collation_free_store_rowkey() const
  {
    return collation_free_store_rowkey_;
  }
  ...
  {
    range_array_idx_ = range_array_idx;
  }
  int64_t get_range_array_idx() const
  {
    return range_array_idx_;
  }
