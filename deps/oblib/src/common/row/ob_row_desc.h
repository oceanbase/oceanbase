...
public:
  struct Desc {
    ...
    Desc() : table_id_(OB_INVALID_ID), column_id_(OB_INVALID_ID)
    {}

    Desc(const uint64_t table_id, const uint64_t column_id) : table_id_(table_id), column_id_(column_id)
    {}
...
{
  int ret = OB_SUCCESS;
  if (idx < 0 || idx >= cells_desc_count_) {
