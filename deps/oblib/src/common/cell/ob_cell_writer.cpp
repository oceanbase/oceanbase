...
int ObCellWriter::write_text(
    const ObObj& obj, const enum ObObjType store_type, const ObString& char_value, ObObj* clone_obj)
{
  int ret = OB_SUCCESS;
  CellMeta meta;
  ...
  if (OB_SUCC(ret) && !old_text_format()) {
    ...
    if (OB_FAIL(append<uint8_t>(static_cast<uint8_t>(lob_scale.get_scale())))) {
      COMMON_LOG(WARN, "fail to append scale, ", K(ret), K(lob_scale));
...
int ObCellWriter::extend_buf(char* buf, int64_t size)
{
  ...
  if (!is_inited_) {
    ...
    // Now back up the data in buf_ to buf, and then modify the pointer position, the size of pos_ does not need to be
    // modified
...
// In order to restore the initial position of the buf pointer, the interface does not need to copy data during the
// revert process
...
int ObCellWriter::append(uint64_t column_id, const ObObj& obj, ObObj* clone_obj)
{
  ...
  if (OB_UNLIKELY(!is_inited_)) {
    ...
    if (OB_SUCCESS == ret && OB_INVALID_ID != column_id && SPARSE == store_type_) {
      if (ObExtendType != obj_type || obj.is_min_value() || obj.is_max_value()) {
