...
public:
  ...
  */
  ...
  ObString(const int64_t length, const char* ptr)
      : buffer_size_(0), data_length_(static_cast<obstr_size_t>(length)), ptr_(const_cast<char*>(ptr))
  {
    if (OB_ISNULL(ptr_)) {
      data_length_ = 0;
    }
  ...
  ObString(const char* ptr) : buffer_size_(0), data_length_(0), ptr_(const_cast<char*>(ptr))
  {
    if (NULL != ptr_) {
  ...
  inline obstr_size_t length() const
  {
    return data_length_;
  }
  inline obstr_size_t size() const
  {
    return buffer_size_;
  }
  ...
  bool operator!() const
  {
    return !(NULL != ptr_ && data_length_ > 0);
  }
  ...
  {
    return (NULL != ptr_ && data_length_ > 0) ? &ObString::operator! : 0;
  }

  // Access a character (no bounds check).
  char operator[](const int64_t n) const
  {
    return ptr_[n];
  }
  ...
  {
    return ptr_ <= p && p < ptr_ + data_length_;
  }
  ...
  {
    return static_cast<const char*>(memchr(ptr_, c, data_length_));
  }
  ...
  //
  // @return A buffer containing data up to but not including @a p.
  ObString split_on(const char c)
  {
    return split_on(find(c));
  }
  ...
  static uint32_t buffer_size_offset_bits()
  {
    return offsetof(ObString, buffer_size_) * 8;
  }
  static uint32_t data_length_offset_bits()
  {
    return offsetof(ObString, data_length_) * 8;
  }
  static uint32_t ptr_offset_bits()
  {
