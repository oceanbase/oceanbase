...
public:
    ...
    Tis620CiSortScanner(const uchar* str, int len, int version = 1)
    ...
    int get_next_character(uchar& next);
    bool has_next();
private:
    void move_next(uchar& ch, uint64_t& move_count);
...
void debug_tis620_sortkey(const uchar *str,
                          size_t len,
                          uchar *dst,
                          size_t dst_len,
