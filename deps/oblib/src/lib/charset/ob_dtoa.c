...
void ob_fcvt_help(char** end, char** dst, char** dend, int sign, int decpt, int* precision, char** res)
{
...
void ob_gcvt_help1(int* width, int* len, char** dend, char** src, char** end, char** dst, int sign, int decpt,
    char* buf, size_t sizeofbuf, char** res, ob_bool* error, double x)
{
  const char* dend_ptr = *dend;
  int i = 0;
  char* dst_ptr = *dst;
...
void ob_gcvt_help2(int* width, int* len, char** dend, char** src, char** end, char** dst, int sign, int decpt,
    char* buf, size_t sizeofbuf, int* exp_len, char** res, ob_bool* error, double x, ob_bool use_oracle_mode)
{
  const char* dend_ptr = *dend;
  char* dst_ptr = *dst;
  char* src_ptr = *src;
    ...
    ob_bool use_force_e_format)
{
  ...
  const int dtoa_width = (type == OB_GCVT_ARG_DOUBLE ? (use_oracle_mode ? MY_MIN(ORACLE_DBL_DIG, width) : width)
  ...
  */
  ...
  if (!use_force_e_format &&
      ((have_space &&
           (decpt >= -TMP_MAX_DECPT_FOR_F_FORMAT + 1 && (decpt <= TMP_MAX_DECPT_FOR_F_FORMAT || len > decpt))) ||
          (!have_space && !force_e_format && (decpt <= width && (decpt >= -1 || decpt == -2))))) {
    ob_gcvt_help1(&width, &len, &dend, &src, &end, &dst, sign, decpt, buf, sizeof(buf), &res, error, x);
  } else {
    ob_gcvt_help2(&width,
        &len,
        &dend,
...
double ob_strtod(const char* str, char** end, int* error)
{
  char buf[DTOA_BUF_MAX_SIZE];
  double res = 0.0;
  if (!(end != NULL && ((str != NULL && *end != NULL) || (str == NULL && *end == NULL)) && error != NULL)) {
    return 0.0;
  }
...
#if defined(WORDS_BIGENDIAN) || (defined(__FLOAT_WORD_ORDER) && (__FLOAT_WORD_ORDER == __BIG_ENDIAN))
...
#define copy_bigint(x, y) memcpy((char*)&x->sign, (char*)&y->sign, 2 * sizeof(int) + y->wds * sizeof(ULong))
...
static const double tinytens[] = {
    1e-16, 1e-32, 1e-64, 1e-128, 9007199254740992. * 9007199254740992.e-256 /* = 2^106 * 1e-53 */
};
/*
...
{
  int scale;
  int bb2, bb5, bbe, bd2, bd5, bbbits, bs2, c = 0, dsign, e, e1, esign, i, j, k, nd, nd0, nf, nz, nz0, sign;
  ...
  for (;;) {
    ...
    if (i < 0) {
      /*
        ...
        a power of two.
      */
      if (dsign || word1(&rv) || word0(&rv) & Bndry_mask || (word0(&rv) & Exp_mask) <= (2 * P + 1) * Exp_msk1) {
#ifdef SET_INEXACT
    ...
    if (i == 0) {
      ...
      if (dsign) {
        if ((word0(&rv) & Bndry_mask1) == Bndry_mask1 &&
            word1(&rv) == ((scale && (y = word0(&rv) & Exp_mask) <= 2 * P * Exp_msk1)
...
static char* dtoa(double dd, int mode, int ndigits, int* decpt, int* sign, char** rve, char* buf, size_t buf_size)
{
  ...
  */
  ...
  int bbits, b2, b5, be, dig, i, ieps, ilim = 0, ilim0, ilim1 = 0, j, j1, k, k0, k_check, leftright, m2, m5, s2, s5,
                                       spec_case, try_quick;
  Long L;
  int denorm;
  ULong x;
  ...
  if (((word0(&u) & Exp_mask) == Exp_mask && (*decpt = DTOA_OVERFLOW)) || (!dval(&u) && (*decpt = 1))) {
    /* Infinity, NaN, 0 */
