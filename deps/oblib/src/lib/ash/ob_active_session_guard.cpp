...
ObRetryWaitEventInfoGuard::ObRetryWaitEventInfoGuard(ObQueryRetryAshInfo *info)
  : is_switch_(false),
    parent_ptr_(nullptr)
{
  if (oceanbase::lib::is_diagnose_info_enabled()) {
    ObDiagnosticInfo *di = ObLocalDiagnosticInfo::get();
    if (OB_NOT_NULL(di)) {
      if (di->get_ash_stat().get_retry_ash_diag_info_ptr() != info) {
        ObQueryRetryAshInfo *parent_ptr_ = di->get_ash_stat().get_retry_ash_diag_info_ptr();
        di->get_ash_stat().set_retry_ash_diag_info_ptr(info);
        is_switch_ = true;
      }
    }
  }
