...
namespace oceanbase
{
  namespace common
  {
        ...
        void *item, ObChunkInfo **chunk_info, const bool do_check /* = false */)
    {
      ...
      if (do_check && (idx >= obj_count_per_chunk_ || (0 != (reinterpret_cast<int64_t>(item) - reinterpret_cast<int64_t>((*chunk_info)->head_)) % type_size_)))
      {
    ...
    void ObObjFreeList::free_to_chunk(ObThreadCache *thread_cache, void *item)
    {
      ...
      *reinterpret_cast<void **>(item) = chunk_info->inner_free_list_;
      ...
      if (0 == chunk_info->allocated_)
      {
        ...
        if (thread_cache->nr_chunks_ > chunk_count_high_mark_ || thread_cache->nr_chunks_ * chunk_byte_size_ > THREAD_CACHE_TOTAL_SIZE)
        {
          ...
          if (1 == obj_count_per_chunk_)
          {
            ...
            while (thread_cache->nr_chunks_ * chunk_byte_size_ > THREAD_CACHE_TOTAL_SIZE || thread_cache->nr_chunks_ > chunk_count_low_mark_)
            {
    ...
    void *ObObjFreeList::privatize_public_freelist(ObThreadCache *thread_cache, const bool alloc)
    {
      ...
      if (NULL != thread_cache->outer_free_list_.head() && NULL != (item = reinterpret_cast<ObFreeObject *>(thread_cache->outer_free_list_.popall())))
      {
                            ...
                            const ObMemCacheType cache_type /* = OP_RECLAIM */)
    {
      ...
      else if (OB_ISNULL(name) || OB_UNLIKELY(obj_size <= 0) || OB_UNLIKELY(obj_count <= 0) || OB_UNLIKELY(obj_count < 0) || OB_UNLIKELY(alignment <= 0) || OB_UNLIKELY(alignment > OB_MALLOC_NORMAL_BLOCK_SIZE))
      {
