...
public:
  ...
  void push_range(common::ObDList<T>& range)
  {
    list_.push_range(range);
  }
  ...
  void set_max_length(int32_t max_length)
  {
    max_length_ = max_length;
  }

  int32_t get_low_water() const
  {
    return low_water_;
  }
  void reset_low_water()
  {
    low_water_ = list_.get_size();
  }

  int32_t get_overlimit_times() const
  {
    return overlimit_times_;
  }
  void set_overlimit_times(int32_t times)
  {
    overlimit_times_ = times;
  }
  void inc_overlimit_times()
  {
    ++overlimit_times_;
  }
...
template <typename T, int64_t MAX_CLASS_NUM, const char* LABEL, int64_t MEM_LIMIT, int32_t MAX_FREE_LIST_LENGTH>
...
ObTCFactory<T, MAX_CLASS_NUM, LABEL, MEM_LIMIT, MAX_FREE_LIST_LENGTH>::ObTCFactory()
    : freelists_(), mem_size_(0), get_count_(), put_count_(), next_(NULL), mem_limit_(MEM_LIMIT)
{
...
template <typename T, int64_t MAX_CLASS_NUM, const char* LABEL, int64_t MEM_LIMIT, int32_t MAX_FREE_LIST_LENGTH>
T* ObTCFactory<T, MAX_CLASS_NUM, LABEL, MEM_LIMIT, MAX_FREE_LIST_LENGTH>::get(int32_t type_id, const uint64_t tenant_id)
{
  UNUSED(tenant_id);
  T* ptr_ret = NULL;
...
template <typename T, int64_t MAX_CLASS_NUM, const char* LABEL, int64_t MEM_LIMIT, int32_t MAX_FREE_LIST_LENGTH>
void ObTCFactory<T, MAX_CLASS_NUM, LABEL, MEM_LIMIT, MAX_FREE_LIST_LENGTH>::put(T* ptr, const uint64_t tenant_id)
{
  UNUSED(tenant_id);
...
void ObTCFactory<T, MAX_CLASS_NUM, LABEL, MEM_LIMIT, MAX_FREE_LIST_LENGTH>::stat()
{
  ...
  while (NULL != tc_factory && OB_SUCC(ret)) {
    ...
    for (int32_t type_id = 0; OB_SUCC(ret) && type_id < MAX_CLASS_NUM; ++type_id) {
      ...
      LIB_LOG(INFO,
          ...
          K(overlimit_times),
          "get_count",
          tc_factory->get_count_[type_id],
          "put_count",
          tc_factory->put_count_[type_id]);
    }
  ...
  for (int32_t type_id = 0; OB_SUCC(ret) && type_id < MAX_CLASS_NUM; ++type_id) {
    LIB_LOG(INFO,
        "[TCFACTORY_STAT] thread=ALL",
        K(type_id),
        "length",
        class_total_length[type_id],
        "max_length",
        class_total_max_length[type_id],
        "allocated",
        allocated_count[type_id]);
  }
  LIB_LOG(INFO,
      "[TCFACTORY_STAT] thread=ALL type_id=ALL",
      "mem_size",
      total_mem_size,
      "length",
      total_length,
