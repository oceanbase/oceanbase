...
public:
  ...
  {}
  ObIteratableHashMapConstIterator(const self_t& other) : map_(other.map_), cur_(other.cur_)
  {}
...
int ObIteratableHashMap<K, V, N, auto_free>::set_refactored(
    const K& key, const V& value, int flag /*= 0*/, int overwrite_key /*= 0*/)
{
  int ret = OB_SUCCESS;
  ...
  uint64_t pos = 0;
  bool exist = false;
  ret = placement_hash_find_set_pos<inner_key_t, N, ModulePageAllocator, auto_free>(
      keys_, flags_, inner_key, flag, pos, exist);
  if (OB_SUCC(ret)) {
    if (!exist) {
      keys_[pos] = inner_key;
      values_[pos] = value;
...
int64_t ObIteratableHashMap<K, V, N, auto_free>::to_string(char* buf, const int64_t buf_len) const
{
  ...
  for (const_iterator_t it = beg; it != end(); ++it) {
    ...
    J_OBJ_START();
    J_KV(N_KEY, (*it).first, N_VALUE, (*it).second);
    J_OBJ_END();
  }
  J_ARRAY_END();
  return pos;
