...
public:
  ...
  virtual int64_t get_header_len() const
  {
    return header_pos_;
  }

  int32_t get_tl_type() const
  {
    return tl_type_;
  }
  void set_tl_type(const int32_t tl_type)
  {
    tl_type_ = tl_type;
  }
  bool is_force_allow() const
  {
    return is_force_allow_;
  }
  void set_force_allow(const bool flag)
  {
    is_force_allow_ = flag;
  }

  bool is_size_overflow() const
  {
    return is_size_overflow_;
  }
  void set_size_overflow()
  {
    is_size_overflow_ = true;
  }
  void reset_size_overflow()
  {
    is_size_overflow_ = false;
  }
  void set_buf_size(const int64_t buf_size)
  {
    buf_size_ = buf_size;
  }
  void set_data_len(const int64_t len)
  {
    pos_ = len;
  }
  void set_header_len(const int64_t len)
  {
    header_pos_ = len;
  }
  ...
  void set_log_level(const int32_t log_level)
  {
  ...
  ObPLogFDType get_fd_type() const
  {
    return fd_type_;
  }
  void set_fd_type(const ObPLogFDType fd_type)
  {
    fd_type_ = fd_type;
  }
  bool is_elec_file() const
  {
    return FD_ELEC_FILE == fd_type_;
  }
  bool is_supported_file() const
  {
    return MAX_FD_FILE != fd_type_;
  }
