...
int64_t ObPerfEventRecorder::to_string(char* buf, const int64_t buf_len) const
{
  ...
  for (int64_t i = 0; i < next_idx_; ++i) {
    ...
    if (NULL == event_name) {
      (void)::oceanbase::common::databuff_printf(buf, buf_len, pos, "|[%hu] u=%ld ", ev.id_, ev.timestamp_ - prev_ts);
    } else {
      (void)::oceanbase::common::databuff_printf(
          buf, buf_len, pos, "|[%s] u=%ld ", event_name, ev.timestamp_ - prev_ts);
    }
  ...
  if (0 == len) {
    ...
    if (rec_len <= 0) {
      ...
      LOG_WARN("the PerfEventRecorder's event buffer is not enough",
          ...
          K(offset),
          "buf_len",
          SIZEOF(ObTimestampEvent) * rec.MAX_EVENT_COUNT);
    } else {
