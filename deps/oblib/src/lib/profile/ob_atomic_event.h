...
#define OB_ATOMIC_EVENT(ev)                                                           \
  ((::oceanbase::common::ObAtomicOpEventPerfConfig::enabled_)                         \
          ? ATOMIC_EVENT_RECORDER.add_event().set_id_time(ID(ev), __LINE__, __FILE__) \
          : (void)0)
...
#define OB_ATOMIC_EVENT_GATHER_DATA()                                                 \
  ((::oceanbase::common::ObAtomicOpEventPerfConfig::enabled_ &&                       \
       ATOMIC_EVENT_RECORDER.need_write(ObAtomicOpEventPerfConfig::sampling_period_)) \
          ? (void)ATOMIC_EVENT_WRITER.write(ATOMIC_EVENT_RECORDER)                    \
          : (void)(0))
