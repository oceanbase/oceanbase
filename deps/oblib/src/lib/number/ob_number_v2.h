...
template <class T>
int poly_mono_mul(const T& multiplicand, const uint64_t multiplier, T& product);

template <class T>
int poly_mono_div(const T& dividend, const uint64_t divisor, T& quotient, uint64_t& remainder);

template <class T>
int poly_poly_add(const T& augend, const T& addend, T& sum);

template <class T>
int poly_poly_sub(const T& minuend, const T& subtrahend, T& remainder, bool& negative, const bool truevalue = true);

template <class T>
int poly_poly_mul(const T& multiplicand, const T& multiplier, T& product);

template <class T>
int poly_poly_div(const T& dividend, const T& divisor, T& quotient, T& remainder);
...
class ObNumber final {
  ...
  public:
    ...
    {
      return (uint32_t*)allocator_.alloc(num);
    }
...
public:
  ...
  static const int32_t OB_REM_BUFFER_SIZE =
      32;  // REM need to change both operands to integer, maximum length = 15 + 15
...
public:
  ...
  ObNumber(const uint32_t desc, uint32_t* digits) : d_(desc), digits_(digits)
  {}
  ObNumber(const number::ObCompactNumber& num) : d_(num.desc_.desc_), digits_(const_cast<uint32_t*>(&(num.digits_[0])))
  {}
...
public:
  ...
  template <class T>
  int from(
      const char* str, T& allocator, int16_t* precision = NULL, int16_t* scale = NULL, const bool do_rounding = true);
  template <class T>
  ...
  template <class T>
  int from(const char* str, const int64_t length, T& allocator, const lib::ObMemAttr& attr, int16_t* precision = NULL,
      int16_t* scale = NULL);
  template <class T>
  int from(const char* str, const int64_t length, T& allocator, number::ObNumberFmtModel* fmt,
      int16_t* precision = NULL, int16_t* scale = NULL);
  template <class T>
  int from(const int16_t precision, const int16_t scale, const char* str, const int64_t length, T& allocator);
  template <class T>
  int from_sci(const char* str, const int64_t length, T& allocator, int16_t* precision, int16_t* scale,
      const bool do_rounding = true);
  template <class T>
  ...
  template <class T>
  int from_sci_opt(const char* str, const int64_t length, T& allocator, int16_t* precision = NULL,
      int16_t* scale = NULL, const bool do_rounding = true);
  ...
  int add_v3(const ObNumber& other, ObNumber& value, ObIAllocator& allocator, const bool strict_mode = true,
  ...
  // strict_mode = true, means only alloc necessary size. otherwise alloc max fixed-length
  int sub_v3(const ObNumber& other, ObNumber& value, ObIAllocator& allocator, const bool strict_mode = true,
      const bool do_rounding = true) const;
  template <class T>
  ...
  int mul_v3(const ObNumber& other, ObNumber& value, ObIAllocator& allocator, const bool strict_mode = true,
      const bool do_rounding = true) const;
  template <class T>
  int div(const ObNumber& other, ObNumber& value, T& allocator, const int32_t qscale = OB_MAX_DECIMAL_DIGIT,
      const bool do_rounding = true) const;
  template <class T>
  ...
  int power(const int64_t exponent, ObNumber& value, ObIAllocator& allocator, const bool do_rounding = true) const;
  int power(const ObNumber& exponent, ObNumber& value, ObIAllocator& allocator, const bool do_rounding = true) const;
  int log(const ObNumber& base, ObNumber& value, ObIAllocator& allocator, const bool do_rounding = true) const;
  inline ObNumber negate() const __attribute__((always_inline));
  inline ObNumber abs() const __attribute__((always_inline));
  int width_bucket(const ObNumber& start, const ObNumber& end, const ObNumber& bucket, ObNumber& value,
  ...
  static int compare(const number::ObNumber::Desc& this_desc, const uint32_t* this_digits,
      const number::ObNumber::Desc& other_desc, const uint32_t* other_digits);
  inline static int compare_v2(const number::ObNumber::Desc& this_desc, const uint32_t* this_digits,
      const number::ObNumber::Desc& other_desc, const uint32_t* other_digits);
  static bool is_equal(const number::ObNumber::Desc& this_desc, const uint32_t* this_digits,
      const number::ObNumber::Desc& other_desc, const uint32_t* other_digits);
  inline static bool is_equal_v2(const number::ObNumber::Desc& this_desc, const uint32_t* this_digits,
      const number::ObNumber::Desc& other_desc, const uint32_t* other_digits);
  ...
  inline bool operator==(const int64_t& other) const __attribute__((always_inline));
  inline bool operator!=(const int64_t& other) const __attribute__((always_inline));
  inline int abs_compare_sys(const uint64_t& other, int64_t exp) const __attribute__((always_inline));
  inline int is_in_uint(int64_t exp, bool& is_uint, uint64_t& num) const __attribute__((always_inline));
  ;
  ...
  inline static int uint32cmp(const uint32_t* s1, const uint32_t* s2, const int64_t n) __attribute__((always_inline));
  inline static int uint32cmp_v2(const uint32_t* s1, const uint32_t* s2, const int64_t n)
      __attribute__((always_inline));
  inline static bool uint32equal(const uint32_t* s1, const uint32_t* s2, const int64_t n)
      __attribute__((always_inline));
  inline static bool uint32equal_v2(const uint32_t* s1, const uint32_t* s2, const int64_t n)
      __attribute__((always_inline));
  inline static uint32_t remove_back_zero(const uint32_t digit, int64_t& count) __attribute__((always_inline));
  ...
  int format_v2(char* buf, const int64_t buf_len, int64_t& pos, int16_t scale) const;
...
protected:
  ...
  int construct_digits_(const char* str, const int64_t start_idx, const int64_t floating_point, const int64_t end_idx,
      uint32_t* digits, int64_t& exp, int64_t& len);
  ...
  int from_(const char* str, IAllocator& allocator, int16_t* precision = NULL, int16_t* scale = NULL,
      const bool do_rounding = true);
  int from_v1_(const char* str, const int64_t length, IAllocator& allocator, int& warning, ObNumberFmtModel* fmt,
      int16_t* precision = NULL, int16_t* scale = NULL, const lib::ObMemAttr* attr = NULL);
  int from_v2_(const char* str, const int64_t length, IAllocator& allocator, int& warning, ObNumberFmtModel* fmt,
      int16_t* precision = NULL, int16_t* scale = NULL, const lib::ObMemAttr* attr = NULL,
      const bool do_rounding = true);
  int from_v3_(const char* str, const int64_t length, IAllocator& allocator, int& warning, ObNumberFmtModel* fmt,
      int16_t* precision = NULL, int16_t* scale = NULL, const lib::ObMemAttr* attr = NULL,
      const bool do_rounding = true);
  inline int from_(const char* str, const int64_t length, IAllocator& allocator, int& warning, ObNumberFmtModel* fmt,
      int16_t* precision = NULL, int16_t* scale = NULL, const lib::ObMemAttr* attr = NULL,
      const bool do_rounding = true)
  {
  ...
  int from_(const int16_t precision, const int16_t scale, const char* str, const int64_t length, IAllocator& allocator);
  ...
  int from_sci_(const char* str, const int64_t length, IAllocator& allocator, int& warning, int16_t* precision,
  ...
  inline static int calc_desc_and_check(
      const uint32_t base_desc, const int64_t exp, const uint8_t len, Desc& desc, const bool is_oracle_mode);
  ...
  int mul_v2_(const ObNumber& other, ObNumber& value, IAllocator& allocator, const bool do_rounding = true) const;
  int div_(const ObNumber& other, ObNumber& value, IAllocator& allocator) const;
  int div_v2_(const ObNumber& other, ObNumber& value, IAllocator& allocator,
      const int32_t qscale = OB_MAX_DECIMAL_DIGIT, const bool do_rounding = true) const;
  ...
  int taylor_series_sin_(const ObNumber& transformed_x, ObNumber& out, ObIAllocator& allocator, const ObNumber& neg_pi,
      const ObNumber& pi) const;
  int taylor_series_cos_(const ObNumber& transformed_x, ObNumber& out, ObIAllocator& allocator,
      const ObNumber& range_low, const ObNumber& range_high) const;
...
protected:
  ...
  int round_scale_v2_(const int64_t scale, const bool using_floating_scale, const bool for_oracle_to_char,
      int16_t* res_precision = NULL, int16_t* res_scale = NULL);
  int round_scale_v3_(const int64_t scale, const bool using_floating_scale, const bool for_oracle_to_char,
      int16_t* res_precision = NULL, int16_t* res_scale = NULL);
  ...
  int round_scale_oracle_(
      const int64_t scale, const bool using_floating_scale, int16_t* res_precision = NULL, int16_t* res_scale = NULL);
  int round_integer_(const int64_t scale, uint32_t* integer_digits, int64_t& integer_length);
  int round_decimal_(const int64_t scale, uint32_t* decimal_digits, int64_t& decimal_length);
  ...
  int rebuild_digits_(const uint32_t* integer_digits, const int64_t integer_length, const uint32_t* decimal_digits,
      const int64_t decimal_length);
  inline static void exp_shift_(const int64_t shift, Desc& desc) __attribute__((always_inline));
  inline static int64_t exp_integer_align_(const Desc d1, const Desc d2) __attribute__((always_inline));
  inline static int64_t get_extend_length_remainder_(const Desc d1, const Desc d2) __attribute__((always_inline));
...
public:
  ...
  {
    uint32_t* ret = NULL;
    if (!is_using_ && num <= (int64_t)(ObNumber::MAX_STORE_LEN * ITEM_SIZE(buffer_))) {
      ret = buffer_;
      is_using_ = true;
    }
    return ret;
...
private:
  ...
  int find_point_v2_(const char* str, int64_t& length, int64_t& integer_start, int64_t& integer_end,
      int64_t& decimal_start, bool& negative, bool& integer_zero, bool& decimal_zero, int& warning);
  // with format string
  int find_point_(const char* str, ObNumberFmtModel* fmt, char* new_str, int64_t& length, int64_t& integer_start,
      int64_t& integer_end, int64_t& decimal_start, bool& negative, bool& integer_zero, bool& decimal_zero,
      int64_t& comma_cnt);
  int build_integer_(const char* str, const int64_t integer_start, const int64_t integer_end, const bool reduce_zero);
  int build_integer_(const char* str, const int64_t integer_start, const int64_t integer_end, const bool reduce_zero,
  ...
  int build_decimal_(const char* str, const int64_t length, const int64_t decimal_start, const bool reduce_zero);
  int build_integer_v2_(const char* str, const int64_t integer_start, const int64_t integer_end, const bool reduce_zero,
      const bool integer_zero, int& warning);
  int build_integer_v2_(const char* str, const int64_t integer_start, const int64_t integer_end, const bool reduce_zero,
      const bool integer_zero, int& warning, ObNumberFmtModel* fmt);
  int build_decimal_v2_(const char* str, const int64_t length, const int64_t decimal_start, const bool reduce_zero,
      const bool decimal_zero, int& warning);
  ...
  // @return OB_SUCCESS if succeed, other error code if error occurs.
  int add_hex_str_(const char* str1, int32_t str1_len, char* str2, int32_t& str2_len) const;
...
public:
  ...
  // C--body decimal
  // D--tail decimal
  enum NextDigitEnum { ND_HEAD_INTEGER = 0, ND_BODY_INTEGER, ND_BODY_DECIMAL, ND_TAIL_DECIMAL, ND_END };
...
public:
  ...
  void knuth_div(ObDivArray a, ObDivArray b, ObDivArray& q, ObDivArray& r)
  {
    ...
    for (int32_t j = m; j >= 0; --j) {
      ...
      //      printf("\nround %d-%lld %lld %lld %lld %lld\n",j,  qq, rr, tmp_value, a.digits_[j + n], a.digits_[j + n -
      //      1]);
      if (qq == BASE || (n >= 2 && qq * b.digits_[n - 2] > BASE * rr + a.digits_[j + n - 2])) {
        --qq;
        rr += b.digits_[n - 1];
      }
      // D4;
      ...
      q.set(j, qq);
      //      printf("\nround %d-%lld %lld %lld %lld %lld\n",j,  qq, rr, tmp_value, a.digits_[j + n], a.digits_[j + n -
      //      1]); q.print(); if (rr == 0 && j > 0 && j + n - 2 <= zerotag) {
      //        --j;
      //        while (j >= 0) {
      //          q.set(j, 0);
      //          --j;
  ...
  void knuth_div_v2(ObDivArray& a, ObDivArray& b, ObDivArray& q, ObDivArray& r)
  {
    ...
    for (int32_t j = m; j >= 0; --j, --aptr) {
      ...
      if (qq == BASE || (n >= 2 && qq * bsmsu > BASE * rr + (*(aptr - 2)))) {
        --qq;
        rr += bmsu;
      }
...
template <class T>
int ObNumber::from(
    const char* str, const int64_t length, T& allocator, int16_t* precision, int16_t* scale, const bool do_rounding)
{
  int ret = OB_SUCCESS;
...
template <class T>
int ObNumber::from_v1(const char* str, const int64_t length, T& allocator, int16_t* precision, int16_t* scale)
{
  int ret = OB_SUCCESS;
  int warning = OB_SUCCESS;
  TAllocator<T> ta(allocator);
...
template <class T>
int ObNumber::from_v2(const char* str, const int64_t length, T& allocator, int16_t* precision, int16_t* scale)
{
  int ret = OB_SUCCESS;
  int warning = OB_SUCCESS;
  TAllocator<T> ta(allocator);
...
template <class T>
int ObNumber::from_v3(const char* str, const int64_t length, T& allocator, int16_t* precision, int16_t* scale)
{
  int ret = OB_SUCCESS;
  int warning = OB_SUCCESS;
  TAllocator<T> ta(allocator);
...
template <class T>
int ObNumber::from(
    const char* str, const int64_t length, T& allocator, const lib::ObMemAttr& attr, int16_t* precision, int16_t* scale)
{
  int ret = OB_SUCCESS;
  int warning = OB_SUCCESS;
  TAllocator<T> ta(allocator);
...
template <class T>
int ObNumber::from(
    const char* str, const int64_t length, T& allocator, ObNumberFmtModel* fmt, int16_t* precision, int16_t* scale)
{
  int ret = OB_SUCCESS;
  int warning = OB_SUCCESS;
  TAllocator<T> ta(allocator);
...
template <class T>
int ObNumber::from(const int16_t precision, const int16_t scale, const char* str, const int64_t length, T& allocator)
{
  TAllocator<T> ta(allocator);
...
template <class IntegerT>
int ObNumber::add(const IntegerT param1, const IntegerT param2, ObNumber& result, ObIAllocator& allocator) const
{
  int ret = OB_SUCCESS;
...
template <class T>
int ObNumber::mul(const ObNumber& other, ObNumber& value, T& allocator, const bool do_rounding) const
{
  TAllocator<T> ta(allocator);
...
template <class T>
int ObNumber::div(
    const ObNumber& other, ObNumber& value, T& allocator, const int32_t qscale, const bool do_rounding) const
{
  TAllocator<T> ta(allocator);
...
int ObNumber::compare(const int64_t& other) const
{
  ...
  if (is_zero()) {
    ...
    if (is_negative()) {
      cmp = (other >= 0) ? -1
                         : -abs_compare_sys(static_cast<uint64_t>((other + 1) * (-1)) + static_cast<uint64_t>(1),
                               EXP_ZERO - d_.exp_);
    } else {
...
inline bool ObNumber::is_equal_v2(const number::ObNumber::Desc& this_desc, const uint32_t* this_digits,
    const number::ObNumber::Desc& other_desc, const uint32_t* other_digits)
{
  bool bret = false;
...
inline int ObNumber::compare_v2(const number::ObNumber::Desc& this_desc, const uint32_t* this_digits,
    const number::ObNumber::Desc& other_desc, const uint32_t* other_digits)
{
  int ret = 0;
...
inline int ObNumber::encode(char* buf, const int64_t buf_size, int64_t& pos, ObNumber* out /* = NULL*/) const
{
  int ret = OB_SUCCESS;
  ...
  if (OB_UNLIKELY(pos + sizeof(uint32_t) + copy_size > buf_size)) {
    ...
    LIB_LOG(ERROR, "buff is null", K(buf), K(copy_size), K(buf_size), K(pos), K(ret));
  } else {
...
inline int64_t ObNumber::get_digit_len_v2(const uint32_t d)
{
  int64_t ret = 0;
...
{
  bool bret = false;
  if ((NEGATIVE == d.sign_ && EXP_ZERO < d.exp_) || (POSITIVE == d.sign_ && 0 < d.exp_ && EXP_ZERO > d.exp_)) {
    bret = true;
  }
  return bret;
}
...
{
  bool bret = false;
  if ((NEGATIVE == d.sign_ && EXP_ZERO >= d.exp_) || (POSITIVE == d.sign_ && EXP_ZERO <= d.exp_)) {
    bret = true;
  }
  return bret;
}
...
inline int ObNumber::exp_check_(const ObNumber::Desc& desc, const bool is_oracle_mode /*false*/)
{
  ...
  } else if (OB_UNLIKELY(MAX_DECIMAL_EXP < exp && is_lt_1_(desc))) {
    ret = OB_DECIMAL_PRECISION_OVERFLOW;
  }
  return ret;
}

inline int ObNumber::calc_desc_and_check(
    const uint32_t base_desc, const int64_t exp, const uint8_t len, Desc& desc, const bool is_oracle_mode)
{
...
int poly_mono_mul(const T& multiplicand, const uint64_t multiplier, T& product)
{
  ...
  if (OB_UNLIKELY(MAX_BASE < base || base <= multiplier || base != product.base() || 0 >= multiplicand_size ||
...
int poly_mono_div(const T& dividend, const uint64_t divisor, T& quotient, uint64_t& remainder)
{
  ...
  if (OB_UNLIKELY(MAX_BASE < base || base <= divisor || base != quotient.base() || 0 >= dividend_size ||
...
int poly_poly_add(const T& augend, const T& addend, T& sum)
{
  ...
  if (OB_UNLIKELY(MAX_BASE < base || base != addend.base() || base != sum.base() || 0 >= augend_size ||
                  0 >= addend_size || add_size != sum_size)) {
    ret = OB_INVALID_ARGUMENT;
    LIB_LOG(ERROR,
        "the input param is invalid",
        K(base),
        K(addend.base()),
        K(sum.base()),
        K(augend_size),
...
int poly_poly_sub(const T& minuend, const T& subtrahend, T& remainder, bool& negative, const bool truevalue /* = true*/)
{
  ...
  if (OB_UNLIKELY(MAX_BASE < base || base != subtrahend.base() || base != remainder.base() || 0 >= minuend_size ||
                  0 >= subtrahend_size || sub_size != remainder_size)) {
    ret = OB_INVALID_ARGUMENT;
    LIB_LOG(ERROR, "the input param is invalid");
  } else {
    uint64_t borrow = 0;
    ...
    negative = (0 != borrow);
    if (OB_SUCCESS == ret && negative && truevalue) {
      uint64_t borrow = 0;
...
template <class T>
int recursion_set_product(const uint64_t base, const uint64_t value, const int64_t start_idx, T& product)
{
  int ret = OB_SUCCESS;
  ...
  while (0 != carry) {
    ...
    if (OB_FAIL(product.set(start_idx - carry_idx, tmp))) {
      LIB_LOG(WARN, "set to product fail", K(ret), K(start_idx), K(carry_idx), K(tmp), K(carry));
      break;
    }
    ++carry_idx;
  }
...
int poly_poly_mul(const T& multiplicand, const T& multiplier, T& product)
{
  ...
  if (OB_UNLIKELY(MAX_BASE < base || base != multiplier.base() || base != product.base() || 0 >= multiplicand_size ||
                  0 >= multiplier_size || mul_size != product_size)) {
    ret = OB_INVALID_ARGUMENT;
    LIB_LOG(ERROR, "the input param is invalid");
  } else {
    // [a, b, c] * [d, e, f]
...
template <class T>
uint64_t knuth_calc_delta(const int64_t j, T& u, T& v)
{
  uint64_t qq = 0;
  ...
  LIB_LOG(DEBUG, "after D3 ", K(j), K(qq));
  return qq;
}

template <class T>
int knuth_probe_quotient(const int64_t j, T& u, const T& v, uint64_t& qq)
{
  int ret = OB_SUCCESS;
  uint64_t base = u.base();
  int64_t n = v.size() - 1;
  ...
  } else if (OB_FAIL(poly_poly_sub(u.ref(j, j + n), qq_mul_v.ref(1, n + 1), u_sub, negative, truevalue))) {
    _OB_LOG(WARN, "%s sub %s fail, ret=%d", to_cstring(u.ref(j, j + n)), to_cstring(qq_mul_v), ret);
  } else if (!negative && OB_FAIL(u.assign(u_sub, j, j + n))) {
    _OB_LOG(WARN, "assign %s to u[%ld,%ld] fail, ret=%d", to_cstring(u_sub), j, j + n, ret);
  } else {
    // do nothing
  }
  LIB_LOG(DEBUG, "after D4", K(j), K(u), K(v), K(qq), K(negative), K(qq_mul_v), K(u_sub), K(ret));

  // D6
  if (negative && OB_SUCCESS == ret) {
    qq -= 1;
    T u_add;
    u_add.set_base(base);
    if (OB_FAIL(u_add.ensure(n + 2))) {
      ...
      _OB_LOG(WARN, "assign %s to u[%ld,%ld] fail, ret=%d", to_cstring(u_add.ref(1, n + 1)), j, j + n, ret);
    } else {
      // do nothing
    }
    LIB_LOG(DEBUG, "after D6 ", K(j), K(u), K(v), K(qq), K(u_add), K(ret));
  }
...
int poly_poly_div(const T& dividend, const T& divisor, T& quotient, T& remainder)
{
  ...
  if (OB_UNLIKELY(MAX_BASE < base || base != divisor.base() || base != quotient.base() || base != remainder.base() ||
                  0 >= divisor_size || divisor_size >= dividend_size || div_size != quotient_size ||
    ...
    if (OB_FAIL(poly_mono_mul(dividend, d, u))) {
      ...
      LIB_LOG(DEBUG, "Knuth Algo start", K(dividend), K(divisor), K(d), K(u), K(v), K(n), K(m));
      ...
      while (true) {
        ...
        // D5
        q.set(j, qq);
        LIB_LOG(DEBUG, "after D5", K(j), K(q));
      ...
      _OB_LOG(DEBUG,
          "Knuth Algo end, %s / %s = %s ... %s",
          to_cstring(dividend),
          to_cstring(divisor),
          to_cstring(quotient),
          to_cstring(remainder));
    }
  }
  return ret;
}
...
inline int64_t ObNumber::get_cap() const
{
  return d_.cap_;
}
...
OB_INLINE int64_t ObNumber::get_scale() const
{
  ...
  if (!is_zero()) {
    ...
    LIB_LOG(DEBUG,
        ...
        K(expr_value),
        K(integer_length),
        K(valid_decimal_length),
        K(decimal_length),
        K(tail_decimal_zero_count),
        K(decimal_count));
  }
