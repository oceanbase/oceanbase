                               ...
                               U32 frame, U32 lastFrameChunk)
{
    ...
    {   size_t const cSize = frame ?
                             ...
                             ZSTD_compressBlock_internal (cctx, dst, dstCapacity, src, srcSize, 0 /* frame mode */);
 ...
 * @returns @blockSize on success, and a ZSTD error otherwise.
 */
static size_t
ZSTD_transferSequences_wBlockDelim(ZSTD_CCtx* cctx,
                                   ZSTD_SequencePosition* seqPos,
        ...
        if (endPosInSequence >= currSeq.litLength + currSeq.matchLength) {
            ...
            if (endPosInSequence > litLength) {
                ...
                if (matchLength > blockSize && firstHalfMatchLength >= cctx->appliedParams.cParams.minMatch) {
                     ...
                     * the last literals. We prefer to do this whenever it is not necessary to split the match, or if doing so
