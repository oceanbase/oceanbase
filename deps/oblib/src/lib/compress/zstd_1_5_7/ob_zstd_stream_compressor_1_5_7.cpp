...
#include "ob_zstd_stream_compressor_1_5_7.h"
...
const char *ObZstdStreamCompressor_1_5_7::get_compressor_name() const
{
  return all_compressor_name[ObCompressorType::STREAM_ZSTD_1_5_7_COMPRESSOR];
}

ObCompressorType ObZstdStreamCompressor_1_5_7::get_compressor_type() const
{
  return ObCompressorType::STREAM_ZSTD_1_5_7_COMPRESSOR;
}

int ObZstdStreamCompressor_1_5_7::create_compress_ctx(void *&ctx)
{
  int ret = OB_SUCCESS;
  ctx = NULL;

  OB_ZSTD_customMem zstd_mem = {ob_zstd_malloc, ob_zstd_free, &allocator_};
  if (OB_FAIL(ObZstdWrapper_1_5_7::create_cctx(zstd_mem, ctx))) {
    LIB_LOG(WARN, "failed to create cctx", K(ret));
  }
  return ret;
}

int ObZstdStreamCompressor_1_5_7::reset_compress_ctx(void *&ctx)
{
  int ret = OB_SUCCESS;
  if (OB_ISNULL(ctx)) {
...
int ObZstdStreamCompressor_1_5_7::free_compress_ctx(void *ctx)
{
  int ret = OB_SUCCESS;
  if (OB_ISNULL(ctx)) {
    ret = OB_INVALID_ARGUMENT;
    ...
    ObZstdWrapper_1_5_7::free_cctx(ctx);
  }
  return ret;
}

// a block is considered not compressible enough,  compressed_size will be zero
int ObZstdStreamCompressor_1_5_7::stream_compress(void *ctx, const char *src, const int64_t src_size,
                                            char *dest, const int64_t dest_capacity, int64_t &dest_size)
{
  ...
  } else if (OB_FAIL(ObZstdWrapper_1_5_7::compress_block(ctx, src, src_size, dest, dest_capacity, compressed_size))) {
...
int ObZstdStreamCompressor_1_5_7::create_decompress_ctx(void *&ctx)
{
  int ret = OB_SUCCESS;
  OB_ZSTD_customMem zstd_mem = {ob_zstd_malloc, ob_zstd_free, &allocator_};
  ctx = NULL;

  if (OB_FAIL(ObZstdWrapper_1_5_7::create_dctx(zstd_mem, ctx))) {
    LIB_LOG(WARN, "failed to create dctx", K(ret));
  }
  return ret;
}

int ObZstdStreamCompressor_1_5_7::reset_decompress_ctx(void *&ctx)
{
  int ret = OB_SUCCESS;
  if (OB_ISNULL(ctx)) {
    ret = OB_INVALID_ARGUMENT;
...
int ObZstdStreamCompressor_1_5_7::free_decompress_ctx(void *ctx)
{
  int ret = OB_SUCCESS;
  if (OB_ISNULL(ctx)) {
    ret = OB_INVALID_ARGUMENT;
    ...
    ObZstdWrapper_1_5_7::free_dctx(ctx);
  }
  return ret;
}

int ObZstdStreamCompressor_1_5_7::stream_decompress(void *ctx, const char *src, const int64_t src_size,
                                              char *dest, const int64_t dest_capacity, int64_t &dest_size)
{
  int ret = OB_SUCCESS;
  ...
  } else if (OB_FAIL(ObZstdWrapper_1_5_7::decompress_block(ctx, src, src_size, dest, dest_capacity, decompressed_size))) {
...
int ObZstdStreamCompressor_1_5_7::get_compress_bound_size(const int64_t src_size, int64_t &bound_size) const
{
  int ret = OB_SUCCESS;
  if (OB_UNLIKELY(0 >= src_size)) {
    ...
    bound_size = ObZstdWrapper_1_5_7::compress_bound(src_size);
  }
  return ret;
}

int ObZstdStreamCompressor_1_5_7::insert_uncompressed_block(void *ctx, const void *block, const int64_t block_size)
{
  int ret = OB_SUCCESS;
  ...
  } else if (OB_FAIL(ObZstdWrapper_1_5_7::insert_block(ctx, block, block_size))) {
