...
#define CLAMPCHECK(val, min, max)                     \
  ...
  {
    U32 const searchLengthMin = ((cParams.strategy == ZSTD_fast) | (cParams.strategy == ZSTD_greedy))
                                    ? ZSTD_SEARCHLENGTH_MIN + 1
    ...
    ZSTD_compressionParameters cPar, unsigned long long srcSize, size_t dictSize)
{
    ...
    cPar.hashLog = cPar.windowLog;
  {
    ...
    if (cPar.chainLog > maxChainLog)
      cPar.chainLog = maxChainLog;
  } /* <= ZSTD_CHAINLOG_MAX */
...
static size_t ZSTD_resetCCtx_advanced(
    ZSTD_CCtx* zc, ZSTD_parameters params, U64 frameContentSize, ZSTD_compResetPolicy_e const crp)
{
...
size_t ZSTD_compressSequences(ZSTD_CCtx* zc, void* dst, size_t dstCapacity, size_t srcSize)
{
...
MEM_STATIC void ZSTD_storeSeq(
    seqStore_t* seqStorePtr, size_t litLength, const void* literals, U32 offsetCode, size_t matchCode)
{
#if 0 /* for debug */
 ...
 */
static size_t ZSTD_count_2segments(
...
void ZSTD_compressBlock_fast_generic(ZSTD_CCtx* cctx, const void* src, size_t srcSize, const U32 mls)
{
  ...
  while (ip < ilimit) { /* < instead of <=, because repcode check at (ip+1) */
    ...
    if (ip <= ilimit) {
      /* Fill Table */
      hashTable[ZSTD_hashPtr(base + current + 2, hBits, mls)] =
...
static void ZSTD_compressBlock_fast_extDict_generic(ZSTD_CCtx* ctx, const void* src, size_t srcSize, const U32 mls)
{
  ...
  /* save reps for next block */
  ctx->savedRep[0] = offset_1;
  ctx->savedRep[1] = offset_2;
...
void ZSTD_compressBlock_doubleFast_generic(ZSTD_CCtx* cctx, const void* src, size_t srcSize, const U32 mls)
{
  ...
  while (ip < ilimit) { /* < instead of <=, because repcode check at (ip+1) */
    ...
    if ((offset_1 > 0) &
        (MEM_read32(ip + 1 - offset_1) == MEM_read32(ip + 1))) { /* note : by construction, offset_1 <= current */
      mLength = ZSTD_count(ip + 1 + 4, ip + 1 + 4 - offset_1, iend) + 4;
      ip++;
      ZSTD_storeSeq(seqStorePtr, ip - anchor, anchor, 0, mLength - MINMATCH);
    } else {
...
static void ZSTD_compressBlock_doubleFast_extDict_generic(
    ZSTD_CCtx* ctx, const void* src, size_t srcSize, const U32 mls)
{
...
static U32 ZSTD_insertBt1(
    ZSTD_CCtx* zc, const BYTE* const ip, const U32 mls, const BYTE* const iend, U32 nbCompares, U32 extDict)
{
  U32* const hashTable = zc->hashTable;
  ...
  *smallerPtr = *largerPtr = 0;
  ...
  if (matchEndIdx > current + 8)
    return matchEndIdx - current - 8;
  return 1;
}

static size_t ZSTD_insertBtAndFindBestMatch(ZSTD_CCtx* zc, const BYTE* const ip, const BYTE* const iend,
    size_t* offsetPtr, U32 nbCompares, const U32 mls, U32 extDict)
{
  U32* const hashTable = zc->hashTable;
  U32 const hashLog = zc->params.cParams.hashLog;
  size_t const h = ZSTD_hashPtr(ip, hashLog, mls);
...
static void ZSTD_updateTree(
    ZSTD_CCtx* zc, const BYTE* const ip, const BYTE* const iend, const U32 nbCompares, const U32 mls)
{
  const BYTE* const base = zc->base;
...
static void ZSTD_updateTree_extDict(
    ZSTD_CCtx* zc, const BYTE* const ip, const BYTE* const iend, const U32 nbCompares, const U32 mls)
{
  const BYTE* const base = zc->base;
  const U32 target = (U32)(ip - base);
...
static size_t ZSTD_BtFindBestMatch_extDict(ZSTD_CCtx* zc, const BYTE* const ip, const BYTE* const iLimit,
  ...
  for (; (matchIndex > lowLimit) & (nbAttempts > 0); nbAttempts--) {
    ...
    if ((!extDict) || matchIndex >= dictLimit) {
      ...
      if (MEM_read32(match) == MEM_read32(ip)) /* assumption : matchIndex <= dictLimit-4 (by table construction) */
        currentMl =
            ZSTD_count_2segments(ip + EQUAL_READ32, match + EQUAL_READ32, iLimit, dictEnd, prefixStart) + EQUAL_READ32;
    }
...
FORCE_INLINE size_t ZSTD_HcFindBestMatch_selectMLS(ZSTD_CCtx* zc, const BYTE* ip, const BYTE* const iLimit,
    size_t* offsetPtr, const U32 maxNbAttempts, const U32 matchLengthSearch)
{
  switch (matchLengthSearch) {
    default:
    case 4:
      return ZSTD_HcFindBestMatch_generic(zc, ip, iLimit, offsetPtr, maxNbAttempts, 4, 0);
    case 5:
      return ZSTD_HcFindBestMatch_generic(zc, ip, iLimit, offsetPtr, maxNbAttempts, 5, 0);
    case 6:
      return ZSTD_HcFindBestMatch_generic(zc, ip, iLimit, offsetPtr, maxNbAttempts, 6, 0);
  }
}

FORCE_INLINE size_t ZSTD_HcFindBestMatch_extDict_selectMLS(ZSTD_CCtx* zc, const BYTE* ip, const BYTE* const iLimit,
    size_t* offsetPtr, const U32 maxNbAttempts, const U32 matchLengthSearch)
{
  switch (matchLengthSearch) {
    default:
    case 4:
      return ZSTD_HcFindBestMatch_generic(zc, ip, iLimit, offsetPtr, maxNbAttempts, 4, 1);
    case 5:
      return ZSTD_HcFindBestMatch_generic(zc, ip, iLimit, offsetPtr, maxNbAttempts, 5, 1);
    case 6:
      return ZSTD_HcFindBestMatch_generic(zc, ip, iLimit, offsetPtr, maxNbAttempts, 6, 1);
  }
}
 ...
 *********************************/
FORCE_INLINE
void ZSTD_compressBlock_lazy_generic(
    ZSTD_CCtx* ctx, const void* src, size_t srcSize, const U32 searchMethod, const U32 depth)
{
  seqStore_t* seqStorePtr = &(ctx->seqStore);
  const BYTE* const istart = (const BYTE*)src;
  const BYTE* ip = istart;
  ...
  typedef size_t (*searchMax_f)(
      ZSTD_CCtx * zc, const BYTE* ip, const BYTE* iLimit, size_t* offsetPtr, U32 maxNbAttempts, U32 matchLengthSearch);
    ...
    ZSTD_CCtx* ctx, const void* src, size_t srcSize, const U32 searchMethod, const U32 depth)
{
  ...
  searchMax_f searchMax =
      searchMethod ? ZSTD_BtFindBestMatch_selectMLS_extDict : ZSTD_HcFindBestMatch_extDict_selectMLS;
    ...
    while (ip <= ilimit) {
      ...
      if (((U32)((dictLimit - 1) - repIndex) >= 3) & (repIndex > lowestIndex)) /* intentional overflow */
        if (MEM_read32(ip) == MEM_read32(repMatch)) {
          /* repcode detected we should take it */
          const BYTE* const repEnd = repIndex < dictLimit ? dictEnd : iend;
          matchLength = ZSTD_count_2segments(ip + EQUAL_READ32, repMatch + EQUAL_READ32, iend, repEnd, prefixStart) +
                        EQUAL_READ32;
          offset = offset_2;
          offset_2 = offset_1;
...
{
  static const ZSTD_blockCompressor blockCompressor[2][8] = {{ZSTD_compressBlock_fast,
...
static size_t ZSTD_compressBlock_internal(ZSTD_CCtx* zc, void* dst, size_t dstCapacity, const void* src, size_t srcSize)
{
  ZSTD_blockCompressor const blockCompressor =
      ZSTD_selectBlockCompressor(zc->params.cParams.strategy, zc->lowLimit < zc->dictLimit);
  const BYTE* const base = zc->base;
  const BYTE* const istart = (const BYTE*)src;
  ...
  if (current > zc->nextToUpdate + 384)
    zc->nextToUpdate =
        current -
        MIN(192,
            (U32)(current - zc->nextToUpdate - 384)); /* update tree not updated after finding very long rep matches */
...
static size_t ZSTD_compress_generic(
    ZSTD_CCtx* cctx, void* dst, size_t dstCapacity, const void* src, size_t srcSize, U32 lastFrameChunk)
{
  ...
  while (remaining) {
    ...
    if (cctx->lowLimit > (1 << 30)) {
      U32 const btplus = (cctx->params.cParams.strategy == ZSTD_btlazy2) |
                         (cctx->params.cParams.strategy == ZSTD_btopt) | (cctx->params.cParams.strategy == ZSTD_btopt2);
      ...
      U32 const newLowLimit =
          (cctx->lowLimit & chainMask) +
          (1 << supLog); /* preserve position % chainSize, ensure current-repcode doesn't underflow */
    ...
    cSize =
        ZSTD_compressBlock_internal(cctx, op + ZSTD_blockHeaderSize, dstCapacity - ZSTD_blockHeaderSize, ip, blockSize);
    if (ZSTD_isError(cSize))
      return cSize;
...
static size_t ZSTD_writeFrameHeader(
    void* dst, size_t dstCapacity, ZSTD_parameters params, U64 pledgedSrcSize, U32 dictID)
{
...
static size_t ZSTD_compressContinue_internal(
    ZSTD_CCtx* cctx, void* dst, size_t dstCapacity, const void* src, size_t srcSize, U32 frame, U32 lastFrameChunk)
{
...
static size_t ZSTD_loadDictionaryContent(ZSTD_CCtx* zc, const void* src, size_t srcSize)
{
  ...
  switch (zc->params.cParams.strategy) {
    ...
    case ZSTD_btopt2:
      ZSTD_updateTree(
          zc, iend - HASH_READ_SIZE, iend, 1 << zc->params.cParams.searchLog, zc->params.cParams.searchLength);
      break;
...
static size_t ZSTD_loadDictEntropyStats(ZSTD_CCtx* cctx, const void* dict, size_t dictSize)
{
...

  {
    ...
    CHECK_E(FSE_buildCTable(cctx->matchlengthCTable, matchlengthNCount, matchlengthMaxValue, matchlengthLog),
...

  {
    ...
    CHECK_E(
        FSE_buildCTable(cctx->litlengthCTable, litlengthNCount, litlengthMaxValue, litlengthLog), dictionary_corrupted);
  ...
  if (cctx->rep[0] >= dictSize)
    return ERROR(dictionary_corrupted);
  cctx->rep[1] = MEM_readLE32(dictPtr + 4);
  if (cctx->rep[1] >= dictSize)
    return ERROR(dictionary_corrupted);
  cctx->rep[2] = MEM_readLE32(dictPtr + 8);
  if (cctx->rep[2] >= dictSize)
    return ERROR(dictionary_corrupted);
  dictPtr += 12;
...
static size_t ZSTD_compressBegin_internal(
    ZSTD_CCtx* cctx, const void* dict, size_t dictSize, ZSTD_parameters params, U64 pledgedSrcSize)
{
...
size_t ZSTD_compress_advanced(ZSTD_CCtx* ctx, void* dst, size_t dstCapacity, const void* src, size_t srcSize,
    const void* dict, size_t dictSize, ZSTD_parameters params)
{
  CHECK_F(ZSTD_checkCParams(params.cParams));
...
size_t ZSTD_compress(void* dst, size_t dstCapacity, const void* src, size_t srcSize, int compressionLevel)
{
  ...
  result =
      ZSTD_compressCCtx(&ctxBody, dst, dstCapacity, src, srcSize, compressionLevel, NULL /*not check zstd_verion*/);
  ZSTD_free(
      ctxBody.workSpace, defaultCustomMem); /* can't free ctxBody itself, as it's on stack; free only heap content */
  return result;
}
...
size_t ZSTD_compressBegin_usingCDict(ZSTD_CCtx* cctx, const ZSTD_CDict* cdict, U64 pledgedSrcSize)
{
  if (cdict->dictContentSize)
    ...
    CHECK_F(ZSTD_compressBegin_advanced(cctx, NULL, 0, cdict->refContext->params, pledgedSrcSize));
  return 0;
}
...
{
  if (zcs->inBuffSize == 0)
    return ERROR(stage_wrong); /* zcs has not been init at least once */
...
size_t ZSTD_initCStream_advanced(
    ZSTD_CStream* zcs, const void* dict, size_t dictSize, ZSTD_parameters params, unsigned long long pledgedSrcSize)
{
  /* allocate buffers */
  {
...
size_t ZSTD_sizeof_CStream(const ZSTD_CStream* zcs)
{
  if (zcs == NULL)
    return 0; /* support sizeof on NULL */
  return sizeof(zcs) + ZSTD_sizeof_CCtx(zcs->cctx) + ZSTD_sizeof_CDict(zcs->cdictLocal) + zcs->outBuffSize +
         zcs->inBuffSize;
}
...
static size_t ZSTD_compressStream_generic(
    ZSTD_CStream* zcs, void* dst, size_t* dstCapacityPtr, const void* src, size_t* srcSizePtr, ZSTD_flush_e const flush)
{
  U32 someMoreWork = 1;
...
size_t ZSTD_endStream(ZSTD_CStream* zcs, ZSTD_outBuffer* output)
{
  ...
  if (zcs->stage != zcss_final) {
    ...
    size_t const notEnded = ZSTD_compressStream_generic(
        zcs, ostart, &sizeWritten, &srcSize, &srcSize, zsf_end); /* use a valid src address instead of NULL */
