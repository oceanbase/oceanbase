...
typedef struct {
  OB_ZSTD_allocFunction customAlloc;
  OB_ZSTD_freeFunction customFree;
  void* opaque;
} OB_ZSTD_customMem;
...
public:
  ...
  static int decompress(OB_ZSTD_customMem& zstd_mem, const char* src_buffer, const size_t src_data_size,
  ...
  static int compress_block(void* ctx, const char* src, const size_t src_size, char* dest, const size_t dest_capacity,
      size_t& compressed_size);
  static int create_dctx(OB_ZSTD_customMem& ob_zstd_mem, void*& ctx);
  static void free_dctx(void*& ctx);
  static int decompress_block(void* ctx, const char* src, const size_t src_size, char* dest, const size_t dest_capacity,
