...
struct ZSTD_DCtx_s {
  ...
  blockType_e bType; /* used in ZSTD_decompressContinue(), to transfer blockType between header decoding and block
                        decoding stages */
  ZSTD_dStage stage;
  U32 litEntropy;
  U32 fseEntropy;
...
{
  return (dctx == NULL) ? 0 : sizeof(ZSTD_DCtx);
}
                  ...
                  (litEncType == set_repeat)
                      ? (singleStream ? HUF_decompress1X_usingDTable(
                                      ...
                                      : HUF_decompress4X_usingDTable(
                                            dctx->litBuffer, litSize, istart + lhSize, litCSize, dctx->HUFptr))
                      : (singleStream ? HUF_decompress1X2_DCtx(
                                            dctx->hufTable, dctx->litBuffer, litSize, istart + lhSize, litCSize)
...
*/
static size_t ZSTD_buildSeqTable(FSE_DTable* DTableSpace, const FSE_DTable** DTablePtr, symbolEncodingType_e type,
    U32 max, U32 maxLog, const void* src, size_t srcSize, const FSE_decode_t4* defaultTable, U32 flagRepeatTable)
{
    ...
    return ERROR(srcSize_wrong); /* minimum possible size */
  {
    ...
    /* Build DTables */
    {
      size_t const llhSize = ZSTD_buildSeqTable(
          dctx->LLTable, &dctx->LLTptr, LLtype, MaxLL, LLFSELog, ip, iend - ip, LL_defaultDTable, dctx->fseEntropy);
    ...
    }
    {
      size_t const ofhSize = ZSTD_buildSeqTable(
          dctx->OFTable, &dctx->OFTptr, OFtype, MaxOff, OffFSELog, ip, iend - ip, OF_defaultDTable, dctx->fseEntropy);
    ...
    }
    {
      size_t const mlhSize = ZSTD_buildSeqTable(
          dctx->MLTable, &dctx->MLTptr, MLtype, MaxML, MLFSELog, ip, iend - ip, ML_defaultDTable, dctx->fseEntropy);
...
size_t ZSTD_execSequenceLast7(BYTE* op, BYTE* const oend, seq_t sequence, const BYTE** litPtr,
    const BYTE* const litLimit_w, const BYTE* const base, const BYTE* const vBase, const BYTE* const dictEnd)
{
...
FORCE_INLINE
size_t ZSTD_execSequence(BYTE* op, BYTE* const oend, seq_t sequence, const BYTE** litPtr, const BYTE* const litLimit_w,
    const BYTE* const base, const BYTE* const vBase, const BYTE* const dictEnd)
{
  ...
  if (iLitEnd > litLimit_w)
    return ERROR(corruption_detected); /* over-read beyond lit buffer */
  if (oLitEnd > oend_w)
    return ZSTD_execSequenceLast7(op, oend, sequence, litPtr, litLimit_w, base, vBase, dictEnd);
...
static size_t ZSTD_decompressSequences(
    ZSTD_DCtx* dctx, void* dst, size_t maxDstSize, const void* seqStart, size_t seqSize)
{
...
static size_t ZSTD_decompressFrame(ZSTD_DCtx* dctx, void* dst, size_t dstCapacity, const void* src, size_t srcSize)
{
  const BYTE* ip = (const BYTE*)src;
  BYTE* const ostart = (BYTE* const)dst;
...
static size_t ZSTD_loadEntropy(ZSTD_DCtx* dctx, const void* const dict, size_t const dictSize)
{
...

  {
    ...
    if (matchlengthLog > MLFSELog)
      return ERROR(dictionary_corrupted);
    CHECK_E(
        FSE_buildDTable(dctx->MLTable, matchlengthNCount, matchlengthMaxValue, matchlengthLog), dictionary_corrupted);
  ...
  if (dctx->rep[0] >= dictSize)
    return ERROR(dictionary_corrupted);
  dctx->rep[1] = MEM_readLE32(dictPtr + 4);
  if (dctx->rep[1] >= dictSize)
    return ERROR(dictionary_corrupted);
  dctx->rep[2] = MEM_readLE32(dictPtr + 8);
  if (dctx->rep[2] >= dictSize)
    return ERROR(dictionary_corrupted);
  dictPtr += 12;
...
#if defined(ZSTD_LEGACY_SUPPORT) && (ZSTD_LEGACY_SUPPORT == 1)
  if (ZSTD_isLegacy(src, srcSize))
    return ZSTD_decompressLegacy(dst, dstCapacity, src, srcSize, ddict->dict, ddict->dictSize);
#endif
...
size_t ZSTD_setDStreamParameter(ZSTD_DStream* zds, ZSTD_DStreamParameter_e paramType, unsigned paramValue)
{
  switch (paramType) {
    default:
      return ERROR(parameter_unknown);
    case ZSTDdsp_maxWindowSize:
...
size_t ZSTD_sizeof_DStream(const ZSTD_DStream* zds)
{
  if (zds == NULL)
    return 0; /* support sizeof on NULL */
  return sizeof(*zds) + ZSTD_sizeof_DCtx(zds->dctx) + ZSTD_sizeof_DDict(zds->ddictLocal) + zds->inBuffSize +
         zds->outBuffSize;
}
      ...
      case zdss_loadHeader: {
...
#if defined(ZSTD_LEGACY_SUPPORT) && (ZSTD_LEGACY_SUPPORT >= 1)
        {
          ...
          if (legacyVersion) {
            ...
            CHECK_F(
                ZSTD_initLegacyStream(&zds->legacyContext, zds->previousLegacyVersion, legacyVersion, dict, dictSize));
      ...
      case zdss_load: {
        ...
        /* decode loaded input */
        {
          ...
          size_t const decodedSize = ZSTD_decompressContinue(
              zds->dctx, zds->outBuff + zds->outStart, zds->outBuffSize - zds->outStart, zds->inBuff, neededInSize);
          if (ZSTD_isError(decodedSize))
            return decodedSize;
