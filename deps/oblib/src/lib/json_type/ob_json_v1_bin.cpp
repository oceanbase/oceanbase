...
#include "ob_json_v1_bin.h"
#include "ob_json_v1_tree.h"
...
int ObJsonBin_v0::get_obtime(ObTime &t) const
{
  int ret = OB_SUCCESS;

  switch (json_type()) {
    case ObJsonNodeType_v0::J_DATE: {
      ret = ObTimeConverter::date_to_ob_time(int_val_, t);
      break;
    }
    case ObJsonNodeType_v0::J_TIME: {
      ret = ObTimeConverter::time_to_ob_time(int_val_, t);
      break;
    }
    case ObJsonNodeType_v0::J_DATETIME: {
      ret = ObTimeConverter::datetime_to_ob_time(int_val_, NULL, t);
      break;
    }
    case ObJsonNodeType_v0::J_TIMESTAMP: {
      ret = ObTimeConverter::datetime_to_ob_time(int_val_, NULL, t);
      break;
    }
    default: {
...
int ObJsonBin_v0::object_add(const common::ObString &key, ObIJsonBase *value)
{
  UNUSED(key);
  UNUSED(value);
  LOG_WARN("json bin not support object add now", K(OB_NOT_SUPPORTED));
  return OB_NOT_SUPPORTED;
}

int ObJsonBin_v0::array_insert(uint64_t index, ObIJsonBase *value)
{
  UNUSED(index);
  UNUSED(value);
  LOG_WARN("json bin not support array insert now", K(OB_NOT_SUPPORTED));
  return OB_NOT_SUPPORTED;
}

int ObJsonBin_v0::array_append(ObIJsonBase *value)
{
  UNUSED(value);
  LOG_WARN("json bin not support array append now", K(OB_NOT_SUPPORTED));
  return OB_NOT_SUPPORTED;
}

int ObJsonBin_v0::merge_tree(ObIAllocator *allocator, ObIJsonBase *other, ObIJsonBase *&result)
{
  UNUSED(allocator);
  UNUSED(other);
  UNUSED(result);
  LOG_WARN("json bin not support merge tree now", K(OB_NOT_SUPPORTED));
  return OB_NOT_SUPPORTED;
}

int ObJsonBin_v0::get_location(ObJsonBuffer &path) const
{
  UNUSED(path);
  LOG_WARN("json bin not support get location now", K(OB_NOT_SUPPORTED));
  return OB_NOT_SUPPORTED;
}

int ObJsonBin_v0::replace(const ObIJsonBase *old_node, ObIJsonBase *new_node)
{
  int ret = OB_SUCCESS;
    ...
    if (j_type != ObJsonNodeType_v0::J_ARRAY && j_type != ObJsonNodeType_v0::J_OBJECT) {
      ...
      for (int i = 0; OB_SUCC(ret) && i < element_count_; i++) {
        ...
        if (OB_SUCC(ret) && child_val_addr == expect_val_addr) {
          ...
          if (OB_FAIL(update(i, static_cast<ObJsonBin_v0 *>(new_node)))) {
...
int ObJsonBin_v0::object_remove(const common::ObString &key)
{
  int ret = OB_SUCCESS;

  if (json_type() != ObJsonNodeType_v0::J_OBJECT) { // check json node type
    ret = OB_ERR_UNEXPECTED;
...
int ObJsonBin_v0::array_remove(uint64_t index)
{
  int ret = OB_SUCCESS;
  ...
  } else if (json_type() != ObJsonNodeType_v0::J_ARRAY) { // check json node type
    ret = OB_ERR_UNEXPECTED;
    LOG_WARN("invalid json node type", K(ret), K(json_type()));
  } else {
...
int ObJsonBin_v0::get_raw_binary(common::ObString &out, ObIAllocator *allocator) const
{
  int ret = OB_SUCCESS;
...
int ObJsonBin_v0::get_key(uint64_t index, common::ObString &key_out) const
{
  int ret = OB_SUCCESS;
  ...
  } else if (json_type() != ObJsonNodeType_v0::J_OBJECT) { // check json node type
    ret = OB_ERR_UNEXPECTED;
    LOG_WARN("invalid json node type", K(ret), K(json_type()));
  } else {
...
int ObJsonBin_v0::get_array_element(uint64_t index, ObIJsonBase *&value) const
{
  int ret = OB_SUCCESS;
  ObString sub;
  ...
  if (index >= element_count()) { // check param
    ...
    if (value != NULL) { // use stack memory
      ...
      buf = allocator->alloc(sizeof(ObJsonBin_v0));
      if (OB_ISNULL(buf)) {
        ret = OB_ALLOCATE_MEMORY_FAILED;
        LOG_WARN("alloc json bin fail", K(ret), K(sizeof(ObJsonBin_v0)));
      }   
    }
    ...
    if (OB_SUCC(ret)) {
      if (OB_FAIL(raw_binary_at_iter(sub))) {
        ...
        ObJsonBin_v0 *j_bin = new (buf) ObJsonBin_v0(sub.ptr(), sub.length(), allocator);
...
int ObJsonBin_v0::get_object_value(uint64_t index, ObIJsonBase *&value) const
{
  int ret = OB_SUCCESS;
  ObString sub;
...
int ObJsonBin_v0::get_object_value(const ObString &key, ObIJsonBase *&value) const
{
  int ret = OB_SUCCESS;
  ObString sub;
...
int ObJsonBin_v0::serialize_json_object(ObJsonObject_v0 *object, ObJsonBuffer &result, uint32_t depth)
{
  UNUSED(depth);
  INIT_SUCC(ret);
  const int64_t st_pos = result.length();
  bool with_key_dict = false; // TODO get from common header
  ObJsonNode_v0 *value = NULL;
  ...
  header.entry_size_ = ObJsonVar_v0::get_var_type(obj_size);
  header.obj_size_size_ = header.entry_size_;
  header.count_size_ = ObJsonVar_v0::get_var_type(count);
  ...
  } else if (OB_FAIL(ObJsonVar_v0::append_var(count, header.count_size_, result))) {
    LOG_WARN("failed to append array header member count", K(ret));
  } else if (OB_FAIL(ObJsonVar_v0::append_var(obj_size, header.obj_size_size_, result))) {
    LOG_WARN("failed to append array header array size", K(ret));
  }
  // [key_entry][val_entry][key][val]
  // push key offset (check if with key dict)
  uint64_t type_size = ObJsonVar_v0::get_var_size(header.entry_size_);
  ...
  if (OB_SUCC(ret)) {
    if (with_key_dict) {
      ...
      for (uint64_t i = 0; OB_SUCC(ret) && i < count; i++) {
        ...
        } else if (OB_FAIL(ObJsonVar_v0::append_var(key_offset, header.entry_size_, result))) { // push key offset to st_pos
          LOG_WARN("append key failed.", K(ret), K(key_offset), K(i));
        } else {
          uint32_t key_len = static_cast<uint32_t>(key.length());
          if (OB_FAIL(ObJsonVar_v0::append_var(key_len, header.entry_size_, result))) { //push key len
    ...
    if (OB_ISNULL(value = object->get_value(i))) {
      ...
      if (!try_update_inline(value, header.entry_size_, &value_entry_offset, result)) {
        ...
        if (OB_FAIL(ret)) {
          ...
          // fill value entry
          if (OB_SUCC(ObJsonVar_v0::set_var(value_offset, header.entry_size_, result.ptr() + value_entry_offset))) {
  ...
  if (OB_SUCC(ret)) {
    ...
    if (ObJsonVar_v0::get_var_type(real_obj_size) > ObJsonVar_v0::get_var_type(obj_size)) {
      ...
      ObJsonVar_v0::set_var(real_obj_size, header->obj_size_size_, header->used_size_ + ObJsonVar_v0::get_var_size(header->count_size_));     
    }
  }
...
int ObJsonBin_v0::serialize_json_array(ObJsonArray_v0 *array, ObJsonBuffer &result, uint32_t depth)
{
  UNUSED(depth);
  INIT_SUCC(ret);
  const int64_t st_pos = result.length();
  ...
  header.entry_size_ = ObJsonVar_v0::get_var_type(array_size);
  header.obj_size_size_ = header.entry_size_;
  header.count_size_ = ObJsonVar_v0::get_var_type(count);
  ...
  if (OB_FAIL(result.append(reinterpret_cast<const char*>(&header), OB_JSON_BIN_ARR_HEADER_LEN))) {
    ...
    LOG_WARN("failed to append array header member count", K(ret));
  } else if (OB_FAIL(ObJsonVar_v0::append_var(array_size, header.obj_size_size_, result))) {
    LOG_WARN("failed to append array header array size", K(ret));
  }
  ...
  if (OB_SUCC(ret)) {
    ...
    if (ObJsonVar_v0::get_var_type(real_array_size) > ObJsonVar_v0::get_var_type(array_size)) {
      if (depth >= OB_JSON_BIN_MAX_SERIALIZE_TIME) {
        ret = OB_ERR_INTERVAL_INVALID;
      ...
      uint64_t count_var_size = ObJsonVar_v0::get_var_size(header->count_size_);
      if (OB_FAIL(ObJsonVar_v0::set_var(real_array_size, header->obj_size_size_, header->used_size_ + count_var_size))) {
...
int ObJsonBin_v0::serialize_json_integer(int64_t value, ObJsonBuffer &result) const
{
  INIT_SUCC(ret);
...
int ObJsonBin_v0::serialize_json_decimal(ObJsonDecimal_v0 *json_dec, ObJsonBuffer &result) const
{
  INIT_SUCC(ret);
...
int ObJsonBin_v0::serialize_json_value(ObJsonNode_v0 *json_tree, ObJsonBuffer &result)
{
  INIT_SUCC(ret);
  switch (json_tree->json_type()) {
    case ObJsonNodeType_v0::J_NULL: {
      if (OB_FAIL(result.append("\0", sizeof(char)))) {
        LOG_WARN("failed to append null json obj", K(ret));
      }
      break;
    }
    case ObJsonNodeType_v0::J_DECIMAL: {
      ObJsonDecimal_v0 *json_dec = static_cast<ObJsonDecimal_v0*>(json_tree);
      if (OB_FAIL(serialize_json_decimal(json_dec, result))) {
        LOG_WARN("failed to serialize json decimal", K(ret));
      }
      break;
    }
    case ObJsonNodeType_v0::J_INT: {
      const ObJsonInt_v0 *i = static_cast<const ObJsonInt_v0*>(json_tree);
    ...
    case ObJsonNodeType_v0::J_UINT: {
      const ObJsonUint_v0 *i = static_cast<const ObJsonUint_v0*>(json_tree);
    ...
    case ObJsonNodeType_v0::J_DOUBLE: {
      const ObJsonDouble_v0 *d = static_cast<const ObJsonDouble_v0*>(json_tree);
    ...
    case ObJsonNodeType_v0::J_STRING: { // [type][length][string]
      const ObJsonString_v0 *sub_obj = static_cast<const ObJsonString_v0*>(json_tree);
    ...
    case ObJsonNodeType_v0::J_OBJECT: {
      ObJsonObject_v0 *object = static_cast<ObJsonObject_v0*>(json_tree);
      if (OB_FAIL(serialize_json_object(object, result))) {
        LOG_WARN("failed to append object json obj", K(ret));
      }
      break;
    }
    case ObJsonNodeType_v0::J_ARRAY: {
      ObJsonArray_v0 *array = static_cast<ObJsonArray_v0*>(json_tree);
      if (OB_FAIL(serialize_json_array(array, result))) {
        LOG_WARN("failed to append array json obj", K(ret));
      }
      break;
    }
    case ObJsonNodeType_v0::J_BOOLEAN: {
      const ObJsonBoolean_v0 *b = static_cast<const ObJsonBoolean_v0*>(json_tree);
    ...
    case ObJsonNodeType_v0::J_DATE: {
      const ObJsonDatetime_v0 *sub_obj = static_cast<const ObJsonDatetime_v0*>(json_tree);
    ...
    case ObJsonNodeType_v0::J_OPAQUE: { // [type][field_type][length][value]
      const ObJsonOpaque_v0 *sub_obj = static_cast<const ObJsonOpaque_v0*>(json_tree);
...
bool ObJsonBin_v0::try_update_inline(const ObJsonNode_v0 *value,
                                  uint8_t var_type,
                                  ...
                                  ObJsonBuffer &result)
{
  ...
  switch (value->json_type()) {
    ...
    case ObJsonNodeType_v0::J_INT: {
      const ObJsonInt_v0 *i = static_cast<const ObJsonInt_v0*>(value);
      if (ObJsonVar_v0::get_var_type(i->value()) <= var_type) {
        inlined_val = ObJsonVar_v0::var_int2uint(i->value());
        is_update_inline = true;
    ...
    case ObJsonNodeType_v0::J_UINT: {
      const ObJsonUint_v0 *i = static_cast<const ObJsonUint_v0*>(value);
      if (ObJsonVar_v0::get_var_type(i->value()) <= var_type) {
        inlined_val = i->value();
        is_update_inline = true;
    ...
    case ObJsonNodeType_v0::J_BOOLEAN: {
      const ObJsonBoolean_v0 *i = static_cast<const ObJsonBoolean_v0*>(value);
  ...
  // set inline
  if (is_update_inline) {
    uint64_t type_size = ObJsonVar_v0::get_var_size(var_type);
    // if inlined set first high bit to 1
    inlined_type |= OB_JSON_TYPE_INLINE_MASK;
    INIT_SUCC(ret);
    if (OB_FAIL(ObJsonVar_v0::set_var(inlined_val, var_type, result.ptr() + *value_entry_offset))) {
...
bool ObJsonBin_v0::try_update_inline(const ObJsonBin_v0 *value,
                                  uint8_t var_type,
                                  int64_t *value_entry_offset,
                                  ObJsonBuffer &result)
{
  bool is_update_inline = false;
  ObJsonNodeType_v0 j_type = value->json_type();
  uint64_t inlined_val;
  ...
  switch (j_type) {
    ...
    case ObJsonNodeType_v0::J_INT: {
      if (ObJsonVar_v0::get_var_type(value->get_int()) <= var_type) {
        inlined_val = ObJsonVar_v0::var_int2uint(value->get_int());
        is_update_inline = true;
        inlined_type =  static_cast<uint8_t>(get_int_vertype());
      }
      break;
    ...
    case ObJsonNodeType_v0::J_UINT: {
      if (ObJsonVar_v0::get_var_type(value->get_uint()) <= var_type) {
...
int ObJsonBin_v0::parse_tree(ObJsonNode_v0 *json_tree)
{
  INIT_SUCC(ret);
  result_.reuse();
  ObJsonNodeType_v0 root_type = json_tree->json_type();
  if (root_type == ObJsonNodeType_v0::J_ARRAY || root_type == ObJsonNodeType_v0::J_OBJECT) {
    if (OB_FAIL(serialize_json_value(json_tree, result_))) { // do recursion
      LOG_WARN("failed to serialize json tree at recursion", K(ret));
      result_.reset();
    }
  } else {
    ObJBVerType ver_type = ObJsonVerType_v0::get_json_vertype(root_type);
    if (!ObJsonVerType_v0::is_opaque_or_string(ver_type) && 
...
int ObJsonBin_v0::to_tree(ObJsonNode_v0 *&json_tree)
{
  INIT_SUCC(ret);
  if (OB_ISNULL(allocator_)) {
    ...
    uint8_t type = type_;
    ObJsonNodeType_v0 curr_type = json_type();
    if (curr_type == ObJsonNodeType_v0::J_OBJECT ||
        curr_type == ObJsonNodeType_v0::J_ARRAY ||
        ObJsonVerType_v0::is_opaque_or_string(static_cast<ObJBVerType>(type_))) {
...
int ObJsonBin_v0::deserialize_json_value(const char *data,
                                      uint64_t length,
                                      uint8_t type,
                                      uint64_t value_offset,
                                      ObJsonNode_v0 *&json_tree)
{
  INIT_SUCC(ret);
  bool is_inlined = OB_JSON_TYPE_IS_INLINE(type);
  ObJBVerType node_vertype = static_cast<ObJBVerType>(OB_JSON_TYPE_GET_INLINE(type));
  ObJsonNodeType_v0 node_type = ObJsonVerType_v0::get_json_type(node_vertype);
  switch (node_type) {
    case ObJsonNodeType_v0::J_NULL: {
      void *buf = allocator_->alloc(sizeof(ObJsonNull_v0));
      if (buf == NULL) {
        ret = OB_ALLOCATE_MEMORY_FAILED;
        LOG_WARN("fail to alloc memory for null json node", K(ret));
      } else {
        ObJsonNull_v0 *node = new(buf)ObJsonNull_v0();
        json_tree = static_cast<ObJsonNode_v0*>(node);
      }
      break;
    }
    case ObJsonNodeType_v0::J_DECIMAL: {
      ...
      if (OB_FAIL(serialization::decode_i16(data, length, pos, &prec))) {
        ...
        void *buf = allocator_->alloc(sizeof(ObJsonDecimal_v0));
        if (buf == NULL) {
          ret = OB_ALLOCATE_MEMORY_FAILED;
          LOG_WARN("fail to alloc memory for decimal json node", K(ret));
        } else {
          ObJsonDecimal_v0 *node = new(buf)ObJsonDecimal_v0(num, prec, scale);
          json_tree = static_cast<ObJsonNode_v0*>(node);
        }
      }
      break;
    ...
    case ObJsonNodeType_v0::J_INT: {
      void *buf = allocator_->alloc(sizeof(ObJsonInt_v0));
      if (buf == NULL) {
        ret = OB_ALLOCATE_MEMORY_FAILED;
        LOG_WARN("fail to alloc memory for int json node", K(ret));
      } else {
        if (is_inlined) {
          ObJsonInt_v0 *node = new(buf)ObJsonInt_v0(ObJsonVar_v0::var_uint2int(value_offset));
          json_tree = static_cast<ObJsonNode_v0*>(node);
        } else {
          int64_t val = 0;
          int64_t pos = 0;
          if (OB_FAIL(serialization::decode_vi64(data, length, pos, &val))) {
            ...
            ObJsonInt_v0 *node = new(buf)ObJsonInt_v0(val);
            json_tree = static_cast<ObJsonNode_v0*>(node);
          }
        }
      }
    ...
    case ObJsonNodeType_v0::J_UINT: {
      void *buf = allocator_->alloc(sizeof(ObJsonUint_v0));
      if (buf == NULL) {
        ret = OB_ALLOCATE_MEMORY_FAILED;
        LOG_WARN("fail to alloc memory for uint json node", K(ret));
      } else {
        if (is_inlined) {
          ObJsonUint_v0 *node = new(buf)ObJsonUint_v0(value_offset);
          json_tree = static_cast<ObJsonNode_v0*>(node);
        } else {
          int64_t val = 0;
          int64_t pos = 0;
          if (OB_FAIL(serialization::decode_vi64(data, length, pos, &val))) {
            ...
            ObJsonUint_v0 *node = new(buf)ObJsonUint_v0(uval);
            json_tree = static_cast<ObJsonNode_v0*>(node);
          }
        }
      }
    ...
    case ObJsonNodeType_v0::J_DOUBLE: {
      void *buf = allocator_->alloc(sizeof(ObJsonDouble_v0));
      if (buf == NULL) {
        ret = OB_ALLOCATE_MEMORY_FAILED;
        ...
        ObJsonDouble_v0 *node = new(buf)ObJsonDouble_v0(val);
        json_tree = static_cast<ObJsonNode_v0*>(node);
      }
      break;
    }
    case ObJsonNodeType_v0::J_STRING: {
      void *buf = allocator_->alloc(sizeof(ObJsonString_v0));
      if (buf == NULL) {
        ret = OB_ALLOCATE_MEMORY_FAILED;
        ...
        if (vertype == ObJBVerType::J_STRING_V0) {
          ...
          if (OB_FAIL(serialization::decode_vi64(data, length, pos, &val))) {
            ...
            if (str_length == 0) {
              ...
              ObJsonString_v0 *empty_str_node = new(buf)ObJsonString_v0(NULL, 0);
              json_tree = static_cast<ObJsonNode_v0*>(empty_str_node);
            } else {
              ...
              if (str_buf == NULL) {
                ...
                ObJsonString_v0 *node = new(buf)ObJsonString_v0(reinterpret_cast<const char*>(str_buf), str_length);
                json_tree = static_cast<ObJsonNode_v0*>(node);
              }
            }
          }
    ...
    case ObJsonNodeType_v0::J_OBJECT: {
      void *buf = allocator_->alloc(sizeof(ObJsonObject_v0));
      if (buf == NULL) {
        ret = OB_ALLOCATE_MEMORY_FAILED;
        LOG_WARN("fail to alloc memory for obj json node", K(ret));
      } else {
        ObJsonObject_v0 *node = new(buf)ObJsonObject_v0(allocator_);
    ...
    case ObJsonNodeType_v0::J_ARRAY: {
      void *buf = allocator_->alloc(sizeof(ObJsonArray_v0));
      if (buf == NULL) {
        ret = OB_ALLOCATE_MEMORY_FAILED;
        LOG_WARN("fail to alloc memory for array json node", K(ret));
      } else {
        ObJsonArray_v0 *node = new(buf)ObJsonArray_v0(allocator_);
    ...
    case ObJsonNodeType_v0::J_BOOLEAN: {
      void *buf = allocator_->alloc(sizeof(ObJsonBoolean_v0));
      if (buf == NULL) {
        ret = OB_ALLOCATE_MEMORY_FAILED;
        ...
        ObJsonBoolean_v0 *node = new(buf)ObJsonBoolean_v0(val);
        json_tree = static_cast<ObJsonNode_v0*>(node);
      }
      break;
    }
    case ObJsonNodeType_v0::J_DATE: {
      void *buf = allocator_->alloc(sizeof(ObJsonDatetime_v0));
      if (buf == NULL) {
        ret = OB_ALLOCATE_MEMORY_FAILED;
        ...
        if (OB_FAIL(ObTimeConverter::date_to_ob_time(value, ob_time))) {
          ...
          ObJsonDatetime_v0 *node = new(buf)ObJsonDatetime_v0(node_type, ob_time);
          json_tree = static_cast<ObJsonNode_v0*>(node);
        }
      }
      break;
    ...
    case ObJsonNodeType_v0::J_OPAQUE: {
      ObJBVerType vertype = *reinterpret_cast<const ObJBVerType*>(data);
      if (vertype == ObJBVerType::J_OPAQUE_V0) {
        ...
        void *buf = allocator_->alloc(sizeof(ObJsonOpaque_v0));
        if (buf == NULL) {
          ret = OB_ALLOCATE_MEMORY_FAILED;
          ...
          ObJsonOpaque_v0 *empy_opa_node = new(buf)ObJsonOpaque_v0(empty_value, field_type);
          json_tree = static_cast<ObJsonNode_v0*>(empy_opa_node);
        } else {
          if (length < need_len + val_length) {
            ...
            if (str_buf == NULL) {
              ...
              ObJsonOpaque_v0 *node = new(buf)ObJsonOpaque_v0(value, field_type);
              json_tree = static_cast<ObJsonNode_v0*>(node);
            }
          }
        }
...
int ObJsonBin_v0::deserialize_json_object_v0(const char *data, uint64_t length, ObJsonObject_v0 *object)
{
  INIT_SUCC(ret);
  ...
  if (length <= OB_JSON_BIN_OBJ_HEADER_LEN) {
    ...
    uint64_t type_size = ObJsonVar_v0::get_var_size(type);
    ...
    for (uint64_t i = 0; OB_SUCC(ret) && i < count; i++) {
      ...
      if (OB_FAIL(ObJsonVar_v0::read_var(key_entry + key_entry_size * i, type, &key_offset))) {
        LOG_WARN("failed to read key offset", K(ret));
      } else if (OB_FAIL(ObJsonVar_v0::read_var(key_entry + key_entry_size * i + type_size, type, &key_len))) {
        LOG_WARN("failed to read key len", K(ret));
      } else if (OB_FAIL(ObJsonVar_v0::read_var(val_entry + val_entry_size * i, type, &value_offset))) {
        LOG_WARN("failed to read val offset", K(ret));
      } else if (OB_FAIL(ObJsonVar_v0::read_var(val_entry + val_entry_size * i + type_size, v_type, &val_type))) {
...
int ObJsonBin_v0::deserialize_json_object(const char *data, uint64_t length, ObJsonObject_v0 *object, ObJBVerType vertype)
{
  INIT_SUCC(ret);
  switch(vertype) {
...
int ObJsonBin_v0::deserialize_json_array_v0(const char *data, uint64_t length, ObJsonArray_v0 *array)
{
  INIT_SUCC(ret);
  uint64_t offset = 0;
  uint8_t node_type, type, obj_size_type;
  ...
  if (length <= OB_JSON_BIN_OBJ_HEADER_LEN) {
    ...
    for (uint64_t i = 0; OB_SUCC(ret) && i < count; i++){
      ...
      if (OB_FAIL(ObJsonVar_v0::read_var(val_entry + val_entry_size * i, type, &val_offset))) {
...
int ObJsonBin_v0::deserialize_json_array(const char *data, uint64_t length, ObJsonArray_v0 *array, ObJBVerType vertype)
{
  INIT_SUCC(ret);
  switch(vertype) {
...
int ObJsonBin_v0::raw_binary(ObString &buf) const
{
  INIT_SUCC(ret);
...
int ObJsonBin_v0::raw_binary(ObString &buf, ObIAllocator *allocator) const
{
  INIT_SUCC(ret);
  ...
  ObJsonNodeType_v0 node_type = ObJsonVerType_v0::get_json_type(vertype);
  if (node_type == ObJsonNodeType_v0::J_ARRAY ||
      node_type == ObJsonNodeType_v0::J_OBJECT ||
      ObJsonVerType_v0::is_opaque_or_string(vertype)) {
...
int ObJsonBin_v0::raw_binary_at_iter(ObString &buf) const
{
  INIT_SUCC(ret);
  if (OB_ISNULL(curr_.ptr())) {
    ret = OB_ERR_NULL_VALUE;
...
int ObJsonBin_v0::get_free_space(size_t &space) const
{
  INIT_SUCC(ret);
  ...
  if (ObJsonVerType_v0::get_json_type(node_type_val) != ObJsonNodeType_v0::J_ARRAY &&
      ObJsonVerType_v0::get_json_type(node_type_val) != ObJsonNodeType_v0::J_OBJECT) {
    space = 0;
  } else {
    used_size = obj_size;
...
int ObJsonBin_v0::reset_iter()
{
  ...
  if (OB_ISNULL(ptr)) {
    ...
    if (!(node_type == ObJsonNodeType_v0::J_ARRAY ||
          node_type == ObJsonNodeType_v0::J_OBJECT ||
          ObJsonVerType_v0::is_opaque_or_string(node_vertype))) {
      pos_ += sizeof(uint8_t);
    }
...
int64_t ObJsonBin_v0::to_string(char *buf, int64_t len) const
{
  int64_t pos = 0;
...
int ObJsonBin_v0::move_iter(ObJsonBuffer& stack, uint32_t start)
{
  INIT_SUCC(ret);
  ...
  for (uint32_t idx = 0; OB_SUCC(ret) && idx < depth; ++idx) {
    ...
    parse_obj_header(data, offset, node_type, type, obj_size_type, count, obj_size);
    if (ObJsonVerType_v0::is_array(static_cast<ObJBVerType>(node_type)) || 
        ObJsonVerType_v0::is_object(static_cast<ObJBVerType>(node_type))) {
      uint64_t type_size = ObJsonVar_v0::get_var_size(type);
      uint64_t key_entry_size = 2 * type_size;
      uint64_t val_entry_size = type_size + sizeof(uint8_t);
      char* val_entry = data + offset + (val_entry_size * node_meta.idx_);
      if (ObJsonVerType_v0::is_object(static_cast<ObJBVerType>(node_type))) {
        val_entry += count * key_entry_size;
      }
      uint64_t val_offset;
      if (OB_FAIL(ObJsonVar_v0::read_var(val_entry, type, &val_offset))) {
  ...
  if (OB_SUCC(ret)) {
    ...
    uint64_t pos = data - result_.ptr();
    if (OB_FAIL(set_curr_by_type(pos, 0, node_meta.ver_type_))) {
...
int ObJsonBin_v0::move_parent_iter()
{
  INIT_SUCC(ret);
...
int ObJsonBin_v0::set_curr_by_type(int64_t new_pos, uint64_t val_offset, uint8_t type, uint8_t entry_size)
{
  INIT_SUCC(ret);
  char *ptr = curr_.ptr();
  data_ = ptr + new_pos;
  ...
  if (!is_inlined && new_pos >= curr_.length()) {
    ...
    switch (node_type) {
      ...
      case ObJsonNodeType_v0::J_INT: {
        if (is_inlined) {
          int_val_ = ObJsonVar_v0::var_uint2int(val_offset, entry_size);
          bytes_ = 0;
        } else {
          int64_t val = 0;
          int64_t pos = 0;
      ...
      case ObJsonNodeType_v0::J_ARRAY: {
        ...
        if (left_len <= OB_JSON_BIN_OBJ_HEADER_LEN) {
          ...
          uint64_t count_size = ObJsonVar_v0::get_var_size(header->count_size_);
          uint64_t obj_size_size = ObJsonVar_v0::get_var_size(header->obj_size_size_);
          if (left_len < count_size + obj_size_size) {
            ...
            if (OB_FAIL(ObJsonVar_v0::read_var(header->used_size_, header->count_size_, &element_count_))) {
              LOG_WARN("fail to read count var.", K(ret), K(header->count_size_));
            } else if (OB_FAIL(ObJsonVar_v0::read_var(header->used_size_ + count_size, header->obj_size_size_, &bytes_))) {
              LOG_WARN("fail to read obj_size var.", K(ret), K(header->obj_size_size_));
            } else {
              left_len -= count_size;
              left_len -= obj_size_size;
              uint64_t entry_size = ObJsonVar_v0::get_var_size(header->entry_size_);
              uint64_t kv_entry_len = element_count_ * (entry_size + 1); // val_entry
              if (node_type == ObJsonNodeType_v0::J_OBJECT) {
...
int ObJsonBin_v0::element(size_t index)
{
  INIT_SUCC(ret);
  ObJsonNodeType_v0 node_type = this->json_type();
  if (node_type != ObJsonNodeType_v0::J_ARRAY && node_type != ObJsonNodeType_v0::J_OBJECT) {
    ...
    } else { // ObJsonNodeType_v0::J_OBJECTs
...
// get key id by key
int ObJsonBin_v0::lookup_index(const ObString &key, size_t *idx) const
{
  INIT_SUCC(ret);
  ObJsonNodeType_v0 node_type = this->json_type();
  if (node_type != ObJsonNodeType_v0::J_OBJECT) {
  ...
  ObJsonKeyCompare_v0 comparator;
  ObString key_iter;
  bool is_found = false;
  int64_t low = 0;
...
int ObJsonBin_v0::lookup(const ObString &key)
{
  INIT_SUCC(ret);
  ObJsonNodeType_v0 node_type = this->json_type();
  if (node_type != ObJsonNodeType_v0::J_OBJECT) {
...
void ObJsonBin_v0::parse_obj_header(const char *data, uint64_t &offset, 
     uint8_t &node_type, uint8_t &type, uint8_t& obj_size_type, uint64_t &count, uint64_t &obj_size) const
{
  ...
  ObJsonVar_v0::read_var(data + offset, header->count_size_, &count);
  offset += ObJsonVar_v0::get_var_size(header->count_size_);
  ObJsonVar_v0::read_var(data + offset, header->obj_size_size_, &obj_size);
  offset += ObJsonVar_v0::get_var_size(header->obj_size_size_);
}

int ObJsonBin_v0::get_element_in_array_v0(size_t index, char **get_addr_only)
{
  INIT_SUCC(ret);
  uint64_t offset = pos_;
  char *data = curr_.ptr();
  ...
  uint64_t val_offset, val_type;
  uint8_t v_type = static_cast<uint8_t>(JBLS_UINT8);
  if (OB_FAIL(ObJsonVar_v0::read_var(val_entry + val_entry_size * index, type, &val_offset))) {
    LOG_WARN("failed to read val offset", K(ret), K(index), K(val_entry_size), K(type));
  } else if (OB_FAIL(ObJsonVar_v0::read_var(val_entry + val_entry_size * index + type_size, v_type, &val_type))) {
...
int ObJsonBin_v0::get_element_in_array(size_t index, char **get_addr_only)
{
  INIT_SUCC(ret);
...
int ObJsonBin_v0::get_element_in_object_v0(size_t i, char **get_addr_only)
{
  INIT_SUCC(ret);
  uint64_t offset = pos_;
  char *data = curr_.ptr();
  ...
  uint8_t v_type = static_cast<uint8_t>(JBLS_UINT8);
  if (OB_FAIL(ObJsonVar_v0::read_var(val_entry + val_entry_size * i, type, &value_offset))) {
    LOG_WARN("failed to read val offset", K(ret), K(i), K(val_entry_size), K(type));
  } else if (OB_FAIL(ObJsonVar_v0::read_var(val_entry + val_entry_size * i + type_size, v_type, &val_type))) {
...
int ObJsonBin_v0::get_element_in_object(size_t i, char **get_addr_only)
{
  INIT_SUCC(ret);
  ObJBVerType vertype = *reinterpret_cast<ObJBVerType*>(curr_.ptr() + pos_);
  switch (vertype) {
...
int ObJsonBin_v0::get_key_in_object_v0(size_t i, ObString &key) const
{
  INIT_SUCC(ret);
  uint64_t offset = pos_;
...
int ObJsonBin_v0::get_key_in_object(size_t i, ObString &key) const
{
  INIT_SUCC(ret);
...
int ObJsonBin_v0::estimate_need_rebuild_kv_entry(ObJsonBuffer &result, ObJsonBuffer& origin_stack, ObJsonBuffer& update_stack,
                                     uint32_t& top_pos, bool& need_rebuild)
{
  ...
  if (OB_FAIL(stack_copy(origin_stack, update_stack))) {
    ...
    for (int idx = top_pos; idx >= 0; --idx) {
      ...
      uint8_t new_offset_type = ObJsonVar_v0::get_var_type(new_offset - path_node.offset_ + path_node.obj_size_);
...
int ObJsonBin_v0::estimate_need_rebuild(ObJsonBuffer& update_stack, int64_t size_change,
                                    int32_t pos, uint32_t& top_pos, bool& need_rebuild)
{
  ...
  if (OB_FAIL(stack_copy(stack_buf_, update_stack))) {
    ...
    for (int64_t idx = pos; size_change && stack_size(update_stack) > 0 && idx >= 0; idx--) {
      ...
      uint8_t new_var_type = ObJsonVar_v0::get_var_type(new_data_size);
      if (new_var_type > path_node.size_type_) {
        top_pos = idx;
        need_rebuild = true;
      }
...
int ObJsonBin_v0::rebuild_with_meta(const char *data, uint64_t length, ObJsonBuffer& old_stack, ObJsonBuffer& new_meta,
                                 uint32_t min, uint32_t max, ObJsonBuffer &result, uint32_t depth)
{
  ...
  if (min <= max) {
    ...
    if ((old_node.ver_type_ == ObJBVerType::J_ARRAY_V0 || old_node.ver_type_ == ObJBVerType::J_OBJECT_V0) &&
      ...
      uint64_t new_count_size = ObJsonVar_v0::get_var_size(header->count_size_); 
      uint64_t new_type_size = ObJsonVar_v0::get_var_size(new_node.entry_type_);
      ...
      if (ObJsonVerType_v0::is_object(static_cast<ObJBVerType>(old_node.ver_type_))) {
      ...
      } else if (OB_FAIL(ObJsonVar_v0::append_var(count, new_header.count_size_, result))) {
        LOG_WARN("failed to append count", K(ret));
      } else if (OB_FAIL(ObJsonVar_v0::append_var(new_node.obj_size_, new_node.size_type_, result))) {
      ...
      } else if (ObJsonVerType_v0::is_object(static_cast<ObJBVerType>(old_node.ver_type_))) {
        if (length < meta_len) {
          ret = OB_ERR_INTERVAL_INVALID;
          ...
          if (OB_FAIL(ObJsonVar_v0::read_var(last_key_offset_ptr, var_type, &key_offset))) {
            LOG_WARN("failed to read key offset", K(ret));
          } else if (OB_FAIL(ObJsonVar_v0::read_var(last_key_len_ptr, var_type, &key_len))) {
            LOG_WARN("failed to read key len", K(ret));
          } else {
            for (int i = 0; OB_SUCC(ret) && i < count; ++i) {
              uint64_t new_key_offset = result.length() - st_pos;
              char* new_key_entry = result.ptr() + st_pos + new_key_entry_offset;
              if (OB_FAIL(ObJsonVar_v0::read_var(key_entry, var_type, &key_offset))) {
                LOG_WARN("failed to read key offset.", K(ret));
              } else if (OB_FAIL(ObJsonVar_v0::read_var(key_entry + type_size, var_type, &key_len))) {
                LOG_WARN("failed to read key len.", K(ret));
              } else if (OB_FAIL(ObJsonVar_v0::set_var(new_key_offset, new_node.entry_type_, new_key_entry))) {
                LOG_WARN("failed to set key len.", K(ret));
              } else if (OB_FAIL(ObJsonVar_v0::set_var(key_len, new_node.entry_type_, new_key_entry + new_type_size))) {
                LOG_WARN("failed to set key len.", K(ret));
                ...
                new_key_entry_offset += new_key_entry_size + new_val_entry_size;
                key_entry += key_entry_size + val_entry_size;
              }
            }
          }
        }
      ...
      if (OB_SUCC(ret) && ObJsonVerType_v0::is_array(static_cast<ObJBVerType>(old_node.ver_type_))) {
      ...
      for (uint64_t i = 0; OB_SUCC(ret) && i < count; i++) {
        ...
        if (OB_FAIL(ObJsonVar_v0::read_var(old_val_entry + val_entry_size * i, var_type, &val_offset))) {
          LOG_WARN("failed to read val offset", K(ret));
        } else if (OB_FAIL(ObJsonVar_v0::read_var(old_val_entry + val_entry_size * i + type_size, v_type, &val_type))) {
          LOG_WARN("failed to read val type", K(ret));
        } else if (OB_FAIL(ObJsonVar_v0::set_var(val_offset, new_node.entry_type_, new_val_entry))) {
          LOG_WARN("failed to set val offset", K(ret));
        } else if (OB_FAIL(ObJsonVar_v0::set_var(val_type, v_type, new_val_entry + new_type_size))) {
          ...
          if (!OB_JSON_TYPE_IS_INLINE(type)) {
            ...
            if (OB_SUCC(ret)) {
              ...
              if (OB_FAIL(ObJsonVar_v0::set_var(new_val_offset, new_node.entry_type_, new_val_entry))) {
      ...
      if (OB_SUCC(ret)) {
        ...
        if (ObJsonVar_v0::get_var_type(actual_obj_size) > new_node.size_type_) {
          if (depth < OB_JSON_BIN_MAX_SERIALIZE_TIME) {
            result.set_length(st_pos);
            new_node.size_type_ = ObJsonVar_v0::get_var_type(actual_obj_size);
        ...
        } else if (OB_FAIL(ObJsonVar_v0::set_var(result.length() - st_pos, new_node.size_type_, obj_size_ptr))) {
...
int ObJsonBin_v0::update_parents(int64_t size_change, bool is_continous)
{
  INIT_SUCC(ret);
  ...
  for (int i = update_begin_pos; OB_SUCC(ret) && !is_rebuild &&  i >= 0; i--) {
    ...
    if (path_node.ver_type_ == ObJBVerType::J_OBJECT_V0 || path_node.ver_type_ == ObJBVerType::J_ARRAY_V0) {
      ...
      uint64_t obj_size;
      uint64_t offset = ObJsonVar_v0::get_var_size(header->count_size_);
      if (OB_FAIL(ObJsonVar_v0::read_var(header->used_size_ + offset, header->obj_size_size_, &obj_size))) {
        LOG_WARN("failed to read obj size.", K(ret), K(header->obj_size_size_));
      } else {
        obj_size += size_change; 
        if (OB_FAIL(ObJsonVar_v0::set_var(obj_size, header->obj_size_size_, header->used_size_ + offset))) {
...
int ObJsonBin_v0::update_offset(uint64_t parent_offset, uint64_t idx, uint64_t value_offset)
{
  INIT_SUCC(ret);
  ...
  if (vertype == ObJBVerType::J_OBJECT_V0 || vertype == ObJBVerType::J_ARRAY_V0) {
    ...
    if (ObJsonVar_v0::set_var(value_offset, var_type, value_offset_ptr)) {
...
bool ObJsonBin_v0::is_discontinuous() const
{
  ObJsonBinObjHeader *header = reinterpret_cast<ObJsonBinObjHeader*>(data_);
  return (header->is_continuous_ == 0);
}

int ObJsonBin_v0::get_update_val_ptr(ObJsonBin_v0 *new_value_bin, char *&val, uint64_t &len, ObJsonBuffer &str)
{
  INIT_SUCC(ret);
  ObJsonNodeType_v0 new_value_type = new_value_bin->json_type();
  switch (new_value_type) {
    case ObJsonNodeType_v0::J_ARRAY: // non-continue array or object do rebuild first
    case ObJsonNodeType_v0::J_OBJECT: {
      size_t free_space;
...
int ObJsonBin_v0::update(const ObString &key, ObJsonBin_v0 *new_value)
{
  INIT_SUCC(ret);
  size_t idx;
  if (OB_FAIL(lookup_index(key, &idx))) {
...
int ObJsonBin_v0::update(int index, ObJsonBin_v0 *new_value)
{
  INIT_SUCC(ret);
  ObJsonNodeType_v0 cur_node_type =  this->json_type();
  ObJBVerType ver_type = this->get_vertype();
  if (OB_ISNULL(new_value)) {
    ret = OB_ERR_NULL_VALUE;
    LOG_WARN("input json binary is null.", K(ret));
  } else if (cur_node_type != ObJsonNodeType_v0::J_ARRAY && cur_node_type != ObJsonNodeType_v0::J_OBJECT) {
    ret = OB_OBJ_TYPE_ERROR;
...
int ObJsonBin_v0::update_v0(int index, ObJsonBin_v0 *new_value)
{
  INIT_SUCC(ret);
  ObJsonNodeType_v0 cur_node_type =  this->json_type();
  ObJBVerType ver_type = this->get_vertype();
  ...
  uint64_t key_entry_size = type_size * 2;
  uint64_t val_entry_size = (type_size + sizeof(uint8_t));
  if (OB_JSON_TYPE_GET_INLINE(node_type) == static_cast<uint8_t>(ObJsonNodeType_v0::J_OBJECT)) {
  ...
  if (!try_update_inline(new_value, var_type, &val_entry_offset, result_)) {
    ...
    if (OB_FAIL(this->element(index))) {
      ...
      ObJsonBin_v0 *new_value_bin = new_value;
      // local is not discontinuous, can not do inplace update
      bool can_do_inplace = true;
      if (this->json_type() == ObJsonNodeType_v0::J_ARRAY || this->json_type() == ObJsonNodeType_v0::J_OBJECT) {
      ...
      if (bytes_changed <= 0 && can_do_inplace) {
        ...
        uint8_t new_var_type = ObJsonVar_v0::get_var_type(new_val_offset - path_node.offset_);
        ...
        if (OB_SUCC(ret)) {
          ...
          type_size = ObJsonVar_v0::get_var_size(path_node.entry_type_);
          key_entry_size = type_size * 2;
          val_entry_size = (type_size + sizeof(uint8_t));
          uint64_t count_size = ObJsonVar_v0::get_var_size(ObJsonVar_v0::get_var_type(count));
          uint64_t obj_size_size = ObJsonVar_v0::get_var_size(path_node.size_type_);
          ...
          if (ObJsonVerType_v0::get_json_type(vertype) == ObJsonNodeType_v0::J_OBJECT) {
            val_offset += key_entry_size * count;
          }
          if (OB_FAIL(ObJsonVar_v0::set_var(new_val_offset - parent_pos, path_node.entry_type_, val_offset))) {
            LOG_WARN("failed to set new value offset.", K(ret));
          } else {
            uint8_t *val_type_ptr = reinterpret_cast<uint8_t*>(val_offset) + ObJsonVar_v0::get_var_size(path_node.entry_type_);
...
*/
int ObJsonBin_v0::remove_v0(size_t index)
{
  INIT_SUCC(ret);
  ObJsonNodeType_v0 node_type = this->json_type();
  ObJBVerType ver_type = this->get_vertype();
  ...
  if (OB_SUCC(ret)) {
    ...
    uint64_t obj_count;
    if (OB_FAIL(ObJsonVar_v0::read_var(header->used_size_, header->count_size_, &obj_count))) {
      LOG_WARN("fail to read header count.", K(ret), KP(header->used_size_), K(header->count_size_));
    } else {
      obj_count -= 1;
      if (OB_FAIL(ObJsonVar_v0::set_var(obj_count, header->count_size_, header->used_size_))) {
    ...
    if (OB_SUCC(ret)) {
      ...
      uint64_t count_var_size = ObJsonVar_v0::get_var_size(header->count_size_);
      if (OB_FAIL(ObJsonVar_v0::read_var(header->used_size_ + count_var_size, header->obj_size_size_, &new_obj_size))) {
        LOG_WARN("fail to read header obj size.", K(ret), K(header->used_size_), K(header->obj_size_size_));
      } else {
        new_obj_size += bytes_changed;
        if (OB_FAIL(ObJsonVar_v0::set_var(new_obj_size, header->obj_size_size_, header->used_size_ + count_var_size))) {
...
int ObJsonBin_v0::remove(size_t index) 
{
  INIT_SUCC(ret);
  ObJsonNodeType_v0 node_type = this->json_type();
  ObJBVerType ver_type = this->get_vertype();
...
int ObJsonBin_v0::remove(const ObString &key)
{
  INIT_SUCC(ret);
  size_t idx;
  ObJsonNodeType_v0 node_type =  this->json_type();
...
int ObJsonBin_v0::rebuild_at_iter(ObJsonBuffer &buf)
{
  INIT_SUCC(ret);
  buf.reuse();
...
int ObJsonBin_v0::rebuild_json_process_value_v0(const char *data, uint64_t length, const char *old_val_entry,
    ...
    if (OB_FAIL(ObJsonVar_v0::read_var(old_val_entry + val_entry_size * i, var_type, &val_offset))) {
      ...
      if (!OB_JSON_TYPE_IS_INLINE(type)) {
        ...
        if (OB_SUCC(ret)) {
          ...
          if (OB_FAIL(ObJsonVar_v0::set_var(new_val_offset, var_type, new_val_entry + val_entry_size * i))) {
            LOG_WARN("failed to set val offset.", K(ret), K(i), K(new_val_offset), K(var_type));
          }
        } else {
          LOG_WARN("rebuild child node failed.", K(ret), K(i), K(val_type));
...
int ObJsonBin_v0::rebuild_json_process_value(const char *data, uint64_t length, const char *old_val_entry,
...
{
  INIT_SUCC(ret);
  ObJsonNodeType_v0 type = ObJsonVerType_v0::get_json_type(cur_vertype);
...
*/
int ObJsonBin_v0::rebuild_json_obj_v0(const char *data, uint64_t length, ObJsonBuffer &result) const
{
  INIT_SUCC(ret);
...
int ObJsonBin_v0::rebuild_json_obj(const char *data, uint64_t length, ObJsonBuffer &result,
...
int ObJsonBin_v0::rebuild_json_array_v0(const char *data, uint64_t length, ObJsonBuffer &result) const
{
  INIT_SUCC(ret);
  int64_t st_pos = result.length();
  uint64_t offset = 0;
...
int ObJsonBin_v0::rebuild_json_array(const char *data, uint64_t length, ObJsonBuffer &result,
                                  ObJBVerType src_vertype, ObJBVerType dest_vertype) const
{
  INIT_SUCC(ret);
...
int ObJsonBin_v0::rebuild_json_value(const char *data,
                                  uint64_t length,
                                  uint8_t src_type,
                                  uint8_t dst_type,
                                  ...
                                  ObJsonBuffer &result) const
{
  ...
  ObJsonNodeType_v0 node_type = ObJsonVerType_v0::get_json_type(src_vertype);
  switch (node_type) {
    case ObJsonNodeType_v0::J_NULL: {
      if (!is_dst_inlined) {
        if (OB_FAIL(result.append("\0", sizeof(char)))) {
...
int ObJsonBin_v0::rebuild(ObJsonBuffer &result)
{
  INIT_SUCC(ret);
  result.reuse();
  char *ptr = curr_.ptr();
...
void ObJsonBin_v0::destroy()
{
  result_.reset();
  stack_buf_.reset();
}

int ObJsonBin_v0::stack_copy(ObJsonBuffer& src, ObJsonBuffer& dst)
{
  INIT_SUCC(ret);
  dst.reset();
...
int ObJsonBin_v0::stack_pop(ObJsonBuffer& stack)
{
  INIT_SUCC(ret);
...
int ObJsonBin_v0::stack_back(ObJsonBuffer& stack, ObJBNodeMeta& node, bool is_pop)
{
  INIT_SUCC(ret);
  uint64_t len = stack.length();
  if (len > 0) {
...
int ObJsonBin_v0::stack_push(ObJsonBuffer& stack, const ObJBNodeMeta& node)
{
  return stack.append(reinterpret_cast<const char*>(&node), JB_PATH_NODE_LEN);
}

int ObJsonBin_v0::stack_at(ObJsonBuffer& stack, uint32_t idx, ObJBNodeMeta& node)
{
  INIT_SUCC(ret);
...
void ObJsonBin_v0::stack_update(ObJsonBuffer& stack, uint32_t idx, const ObJBNodeMeta& new_value)
{
  ObJBNodeMeta* node = reinterpret_cast<ObJBNodeMeta*>(stack.ptr() + idx * JB_PATH_NODE_LEN);
  *node = new_value;
}

int32_t ObJsonBin_v0::stack_size(ObJsonBuffer& stack)
{
  return stack.length() / JB_PATH_NODE_LEN;
}

void ObJsonBin_v0::stack_reset(ObJsonBuffer& stack)
{
  stack.reset();
}

ObJBVerType ObJsonVerType_v0::get_json_vertype(ObJsonNodeType_v0 in_type)
{
  uint8_t in = static_cast<uint8_t>(in_type);
  ObJsonNodeType_v0 type = static_cast<ObJsonNodeType_v0>(OB_JSON_TYPE_GET_INLINE(in));
  ObJBVerType ret_type;

  switch (type) {
    case ObJsonNodeType_v0::J_NULL: {
      ret_type = ObJsonBin_v0::get_null_vertype();
      break;
    }
    case ObJsonNodeType_v0::J_DECIMAL: {
      ret_type = ObJsonBin_v0::get_decimal_vertype();
      break;
    }
    case ObJsonNodeType_v0::J_INT: {
      ret_type = ObJsonBin_v0::get_int_vertype();
      break;
    }
    case ObJsonNodeType_v0::J_UINT: {
      ret_type = ObJsonBin_v0::get_uint_vertype();
      break;
    }
    case ObJsonNodeType_v0::J_DOUBLE: {
      ret_type = ObJsonBin_v0::get_double_vertype();
      break;
    }
    case ObJsonNodeType_v0::J_STRING: {
      ret_type = ObJsonBin_v0::get_string_vertype();
      break;
    }
    case ObJsonNodeType_v0::J_OBJECT: {
      ret_type = ObJsonBin_v0::get_object_vertype();
      break;
    }
    case ObJsonNodeType_v0::J_ARRAY: {
      ret_type = ObJsonBin_v0::get_array_vertype();
      break;
    }
    case ObJsonNodeType_v0::J_BOOLEAN: {
      ret_type = ObJsonBin_v0::get_boolean_vertype();
      break;
    }
    case ObJsonNodeType_v0::J_DATE: {
      ret_type = ObJsonBin_v0::get_date_vertype();
      break;
    }
    case ObJsonNodeType_v0::J_TIME: {
      ret_type = ObJsonBin_v0::get_time_vertype();
      break;
    }
    case ObJsonNodeType_v0::J_DATETIME: {
      ret_type = ObJsonBin_v0::get_datetime_vertype();
      break;
    }
    case ObJsonNodeType_v0::J_TIMESTAMP: {
      ret_type = ObJsonBin_v0::get_timestamp_vertype();
      break;
    }
    case ObJsonNodeType_v0::J_OPAQUE: {
      ret_type = ObJsonBin_v0::get_opaque_vertype();
      break;
    }
    default: {
...
ObJsonNodeType_v0 ObJsonVerType_v0::get_json_type(ObJBVerType type)
{
  ObJsonNodeType_v0 ret_type;

  switch (type) {
    case ObJBVerType::J_NULL_V0: {
      ret_type = ObJsonNodeType_v0::J_NULL;
      break;
    }
    case ObJBVerType::J_DECIMAL_V0: {
      ret_type = ObJsonNodeType_v0::J_DECIMAL;
      break;
    }
    case ObJBVerType::J_INT_V0: {
      ret_type = ObJsonNodeType_v0::J_INT;
      break;
    }
    case ObJBVerType::J_UINT_V0: {
      ret_type = ObJsonNodeType_v0::J_UINT;
      break;
    }
    case ObJBVerType::J_DOUBLE_V0: {
      ret_type = ObJsonNodeType_v0::J_DOUBLE;
      break;
    }
    case ObJBVerType::J_STRING_V0: {
      ret_type = ObJsonNodeType_v0::J_STRING;
      break;
    }
    case ObJBVerType::J_OBJECT_V0: {
      ret_type = ObJsonNodeType_v0::J_OBJECT;
      break;
    }
    case ObJBVerType::J_ARRAY_V0: {
      ret_type = ObJsonNodeType_v0::J_ARRAY;
      break;
    }
    case ObJBVerType::J_BOOLEAN_V0: {
      ret_type = ObJsonNodeType_v0::J_BOOLEAN;
      break;
    }
    case ObJBVerType::J_DATE_V0: {
      ret_type = ObJsonNodeType_v0::J_DATE;
      break;
    }
    case ObJBVerType::J_TIME_V0: {
      ret_type = ObJsonNodeType_v0::J_TIME;
      break;
    }
    case ObJBVerType::J_DATETIME_V0: {
      ret_type = ObJsonNodeType_v0::J_DATETIME;
      break;
    }
    case ObJBVerType::J_TIMESTAMP_V0: {
      ret_type = ObJsonNodeType_v0::J_TIMESTAMP;
      break;
    }
    case ObJBVerType::J_OPAQUE_V0: {
      ret_type = ObJsonNodeType_v0::J_OPAQUE;
      break;
    }
    default: {
      ret_type = static_cast<ObJsonNodeType_v0>(type);
      break;
    }
  }
...
bool ObJsonVerType_v0::is_array(ObJBVerType type)
{
  return (type == J_ARRAY_V0);
}

bool ObJsonVerType_v0::is_object(ObJBVerType type)
{
  return (type == J_OBJECT_V0);
}

bool ObJsonVerType_v0::is_custom(ObJBVerType type)
{
  return (type == J_OPAQUE_V0);
}

bool ObJsonVerType_v0::is_opaque_or_string(ObJBVerType type)
{
  return (type == J_OPAQUE_V0 || type == J_STRING_V0);
}

bool ObJsonVerType_v0::is_scalar(ObJBVerType type)
{
  ObJsonNodeType_v0 node_type = get_json_type(type);

  return (node_type == ObJsonNodeType_v0::J_NULL || 
          node_type == ObJsonNodeType_v0::J_UINT ||
          node_type == ObJsonNodeType_v0::J_INT ||
          node_type == ObJsonNodeType_v0::J_DOUBLE ||
          node_type == ObJsonNodeType_v0::J_STRING ||
          node_type == ObJsonNodeType_v0::J_BOOLEAN ||
          node_type == ObJsonNodeType_v0::J_DATE ||
          node_type == ObJsonNodeType_v0::J_DATETIME ||
          node_type == ObJsonNodeType_v0::J_TIMESTAMP || 
          node_type == ObJsonNodeType_v0::J_OPAQUE);
}

/* var size */
int ObJsonVar_v0::read_var(const char *data, uint8_t type, uint64_t *var)
{
  INIT_SUCC(ret);
  if (OB_ISNULL(data)) {
    ret = OB_ERR_NULL_VALUE;
...
int ObJsonVar_v0::append_var(uint64_t var, uint8_t type, ObJsonBuffer &result)
{
  INIT_SUCC(ret);
  ObJsonBinLenSize size = static_cast<ObJsonBinLenSize>(type);
  switch (size) {
...
int ObJsonVar_v0::reserve_var(uint8_t type, ObJsonBuffer &result)
{
  INIT_SUCC(ret);
  ObJsonBinLenSize size = static_cast<ObJsonBinLenSize>(type);
  switch (size) {
...
int ObJsonVar_v0::set_var(uint64_t var, uint8_t type, char *pos)
{
  INIT_SUCC(ret);
  if (OB_ISNULL(pos)) {
    ret = OB_ERR_NULL_VALUE;
...
uint64_t ObJsonVar_v0::get_var_size(uint8_t type)
{
...
uint8_t ObJsonVar_v0::get_var_type(uint64_t var)
{
...
int ObJsonVar_v0::read_var(const char *data, uint8_t type, int64_t *var)
{
  INIT_SUCC(ret);
  if (OB_ISNULL(data)) {
    ret = OB_ERR_NULL_VALUE;
...
uint64_t ObJsonVar_v0::var_int2uint(int64_t var)
{
  ObJsonBinLenSize size = static_cast<ObJsonBinLenSize>(ObJsonVar_v0::get_var_type(var));
  uint64 val = 0;
  switch (size) {
    case JBLS_UINT8: {
...
int64_t ObJsonVar_v0::var_uint2int(uint64_t var, uint8_t entry_size)
{
  ObJsonBinLenSize size = static_cast<ObJsonBinLenSize>(max(ObJsonVar_v0::get_var_type(var), entry_size));
  int64_t val = 0;
  switch (size) {
    case JBLS_UINT8: {
      if (var > INT8_MAX) {
...
uint8_t ObJsonVar_v0::get_var_type(int64_t var)
{
  ObJsonBinLenSize lsize = JBLS_UINT64;
