...
#ifndef OCEANBASE_SQL_OB_JSON_V1_BIN
#define OCEANBASE_SQL_OB_JSON_V1_BIN
...
class ObJsonNode_v0;
class ObJsonObject_v0;
class ObJsonArray_v0;
class ObJsonDecimal_v0;
class ObJsonPath;
...
class ObJsonVerType_v0 {
public:
  static ObJsonNodeType_v0 get_json_type(ObJBVerType type);
  static ObJBVerType get_json_vertype(ObJsonNodeType_v0 type);
...
private:
  DISALLOW_COPY_AND_ASSIGN(ObJsonVerType_v0);
};

class ObJsonBin_v0 : public ObIJsonBase
{
public:
  ObJsonBin_v0()
      : ObIJsonBase(NULL),
  ...
  explicit ObJsonBin_v0(common::ObIAllocator *allocator)
  ...
  explicit ObJsonBin_v0(const char *data, const int64_t length, common::ObIAllocator *allocator = NULL)
      : ObIJsonBase(allocator),
        allocator_(allocator),
        result_(allocator),
        curr_(length, data),
  ...
  virtual ~ObJsonBin_v0() { result_.reset(); stack_buf_.reset(); }
  ...
  OB_INLINE uint64_t get_data_length() const override
  {
    ...
    ObJsonNodeType_v0 type = json_type();
    if (type == ObJsonNodeType_v0::J_ARRAY || type == ObJsonNodeType_v0::J_OBJECT) {
      data_length = bytes_;
    }
    return data_length;
  }
  ...
  OB_INLINE uint32_t depth() const override { return 0; } // not support now
  OB_INLINE uint64_t get_used_bytes() const { return bytes_; } // return acutal used bytes for curr iter
  OB_INLINE ObJsonNodeType_v0 json_type() const override
  {
    return static_cast<ObJsonNodeType_v0>(ObJsonVerType_v0::get_json_type(get_vertype()));
  }
  ...
  int get_raw_binary(common::ObString &out, ObIAllocator *allocator = NULL) const override;
  ...
  int get_location(ObJsonBuffer &path) const override;
  int merge_tree(ObIAllocator *allocator, ObIJsonBase *other, ObIJsonBase *&result) override;
...
public:
  ...
  */
  int parse_tree(ObJsonNode_v0 *json_tree);
  ...
  @return Returns OB_SUCCESS on success, error code otherwise.
  */
  int to_tree(ObJsonNode_v0 *&json_tree);
  ...
  */
  ...
  @return Returns OB_SUCCESS on success, error code otherwise.
  */
  int update(const ObString &key, ObJsonBin_v0 *new_value);
  ...
  @return Returns OB_SUCCESS on success, error code otherwise.
  */
  int update(int index, ObJsonBin_v0 *new_value);
...
private:
  ...
  int update_v0(int index, ObJsonBin_v0 *new_value);
  ...
  int serialize_json_object(ObJsonObject_v0* object, ObJsonBuffer &result, uint32_t depth = 0);
  int serialize_json_array(ObJsonArray_v0 *array, ObJsonBuffer &result, uint32_t depth = 0);
  int serialize_json_value(ObJsonNode_v0 *json_tree, ObJsonBuffer &result);
  int serialize_json_integer(int64_t value, ObJsonBuffer &result) const;
  int serialize_json_decimal(ObJsonDecimal_v0 *json_dec, ObJsonBuffer &result) const;
  bool try_update_inline(const ObJsonNode_v0 *value,
                        uint8_t var_type,
                        int64_t *value_entry_offset,
                        ObJsonBuffer &result);
  bool try_update_inline(const ObJsonBin_v0 *value,
                        uint8_t var_type,
                        int64_t *value_entry_offset,
                        ObJsonBuffer &result);
  ...
  int deserialize_json_object_v0(const char *data, uint64_t length, ObJsonObject_v0 *object);
  inline int deserialize_json_object(const char *data, uint64_t length, ObJsonObject_v0 *object, ObJBVerType vertype);

  int deserialize_json_array_v0(const char *data, uint64_t length, ObJsonArray_v0 *array);
  inline int deserialize_json_array(const char *data, uint64_t length, ObJsonArray_v0 *array, ObJBVerType vertype);
  ...
  */
  ...
  int get_update_val_ptr(ObJsonBin_v0 *new_value_bin, char *&val, uint64_t &len, ObJsonBuffer &str);
  ...
  #define JB_PATH_NODE_LEN sizeof(ObJBNodeMeta)
...
private:
  ...
  DISALLOW_COPY_AND_ASSIGN(ObJsonBin_v0);
};
...
#endif // OCEANBASE_SQL_OB_JSON_V1_BIN