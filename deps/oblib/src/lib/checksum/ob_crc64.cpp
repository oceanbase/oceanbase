...
namespace oceanbase {
namespace common {
int64_t ObBatchChecksum::to_string(char* buf, const int64_t buf_len) const
{
  int64_t pos = 0;
  J_OBJ_START();
  J_KV(K_(pos), K_(base), K_(buffer));
  J_OBJ_END();
  return pos;
}
...
#define DO_2_STEP(uCRC64, pu8) \
  DO_1_STEP(uCRC64, pu8);      \
  DO_1_STEP(uCRC64, pu8);
#define DO_4_STEP(uCRC64, pu8) \
  DO_2_STEP(uCRC64, pu8);      \
  DO_2_STEP(uCRC64, pu8);
#define DO_8_STEP(uCRC64, pu8) \
  DO_4_STEP(uCRC64, pu8);      \
  DO_4_STEP(uCRC64, pu8);
#define DO_16_STEP(uCRC64, pu8) \
...
#define DO_1_OPTIMIZED_STEP(uCRC64, pu8, u_data, operand1, operand2, table_result0)                              \
  ...
  operand1 = ((uint64_t)s_optimized_crc64_table[(uint8_t)(u_data >> 48)] << 40) |                                \
             ...
             ((uint64_t)s_optimized_crc64_table[(uint8_t)(u_data >> 16)] << 8) | ((uint64_t)table_result0 >> 8); \
...
#define crc32_sse42_byte                                                            \
  asm(".byte 0xf2, 0x48, 0x0f, 0x38, 0xf0, 0x0a" : "=c"(crc) : "c"(crc), "d"(buf)); \
  len--, buf++

/* opcodes taken from objdump of "crc32q (%%rdx), %%rcx"
for RHEL4 support (GCC 3 doesn't support this instruction) */
#define crc32_sse42_quadword                                                        \
  asm(".byte 0xf2, 0x48, 0x0f, 0x38, 0xf1, 0x0a" : "=c"(crc) : "c"(crc), "d"(buf)); \
  len -= 8, buf += 8
#elif defined(__aarch64__)
#include "arm_acle.h"
#define crc32_sse42_quadword            \
  crc = __crc32cd(crc, *(int64_t*)buf); \
  len -= 8, buf += 8
#define crc32_sse42_byte               \
...
uint64_t crc64_sse42_manually(uint64_t crc, const char* buf, int64_t len)
{
   ...
   *   }
   */
  const static uint32_t crc32tab[] = {0x00000000L,
      0xf26b8303L,
      0xe13b70f7L,
      0x1350f3f4L,
      0xc79a971fL,
...
uint64_t fast_crc64_sse42_manually(uint64_t crc, const char* buf, int64_t len)
{
   ...
   */
  const static uint32_t T8_0[] = {0x00000000,
      0xF26B8303,
      0xE13B70F7,
      0x1350F3F4,
      0xC79A971F,
  ...
  const static uint32_t T8_1[] = {0x00000000,
      0x13A29877,
      0x274530EE,
      0x34E7A899,
      0x4E8A61DC,
  ...
  const static uint32_t T8_2[] = {0x00000000,
      0xA541927E,
      0x4F6F520D,
      0xEA2EC073,
      0x9EDEA41A,
  ...
  const static uint32_t T8_3[] = {0x00000000,
      0xDD45AAB8,
      0xBF672381,
      0x62228939,
      0x7B2231F3,
  ...
  const static uint32_t T8_4[] = {0x00000000,
      0x38116FAC,
      0x7022DF58,
      0x4833B0F4,
      0xE045BEB0,
  ...
  const static uint32_t T8_5[] = {0x00000000,
      0xEF306B19,
      0xDB8CA0C3,
      0x34BCCBDA,
      0xB2F53777,
  ...
  const static uint32_t T8_6[] = {0x00000000,
      0x68032CC8,
      0xD0065990,
      0xB8057558,
      0xA5E0C5D1,
  ...
  const static uint32_t T8_7[] = {0x00000000,
      0x493C7D27,
      0x9278FA4E,
      0xDB448769,
      0x211D826D,
...
{
  return serialization::encoded_length_i64(pos_) + serialization::encoded_length_i64(base_) + BUFFER_SIZE;
}
