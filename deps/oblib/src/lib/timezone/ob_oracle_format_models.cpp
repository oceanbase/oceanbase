...
const ObOracleTimeLimiter ObDFMLimit::TIMEZONE_HOUR_ABS = {
    0, 15, OB_INVALID_DATE_VALUE};  // ORA-01874: time zone hour must be between -15 and 15
const ObOracleTimeLimiter ObDFMLimit::TIMEZONE_MIN_ABS = {
    0, 59, OB_INVALID_DATE_VALUE};  // ORA-01875: time zone minute must be between -59 and 59
const ObOracleTimeLimiter ObDFMLimit::JULIAN_DATE = {
    1, 5373484, OB_ERR_INVALID_JULIAN_DATE_VALUE};  // -4712-01-01 ~ 9999-12-31

int ObDFMUtil::match_int_value_with_comma(
    ObDFMParseCtx& ctx, const int64_t expected_len, int64_t& value_len, int32_t& result)
{
  ...
  while (OB_SUCC(ret) && !stop_flag && real_data_len < ctx.remain_len_ &&
...
int ObDFMUtil::special_mode_sprintf(char* buf, const int64_t buf_len, int64_t& pos, const ObTimeConstStr& str,
    const ObDFMElem::UpperCaseMode mode, int64_t padding)
{
  int ret = OB_SUCCESS;
    ...
    const ObDFMElemArr& elements, ObBitSet<ObDFMFlag::MAX_FLAG_NUMBER>& flag_bitmap, uint64_t mode)
{
  ...
  for (int64_t i = 0; OB_SUCC(ret) && i < elements.count(); ++i) {
    ...
    if (OB_SUCC(ret)) {
      ...
      } else if (OB_UNLIKELY(!HAS_TYPE_TIMEZONE(mode) && (ObDFMFlag::TZD == flag || ObDFMFlag::TZR == flag ||
