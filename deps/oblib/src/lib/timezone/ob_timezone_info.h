...
// time_us_ is before time_ctx_ in previous version.
// When implement static engine, we exchange their position for optimization.
// This results in crc64_v2 not compatible with previous version because for ObTimestampNanoType
// type, only fist two bytes of time_ctx_, so crc64_v2 only calculate checksum of first ten bytes.
struct ObOTimestampDataOld {
  int64_t time_us_;
  int32_t time_ctx_;
  ObOTimestampDataOld() : time_us_(0), time_ctx_(0)
  {}
  ObOTimestampDataOld(const int64_t time_us, const int32_t tz_ctx) : time_us_(time_us), time_ctx_(tz_ctx)
  {}
};
...
struct ObTZTransitionStruct {
  ...
  {
    return (offset_sec_ == other.offset_sec_ && tran_type_id_ == other.tran_type_id_ && is_dst_ == other.is_dst_ &&
...
public:
  ...
  {
    return (type_class_ == other.type_class_ && extra_info_ == other.extra_info_ && lower_time_ == other.lower_time_ &&
            info_ == other.info_);
  }
  bool operator!=(const ObTZRevertTypeInfo& other) const
  {
...
struct ObIntervalYMValue {
  ...
  {}
  ObIntervalYMValue(int64_t nmonth) : nmonth_(nmonth)
  {}
  ...
  static inline int64_t get_store_size()
  {
...
struct ObIntervalDSValue {
  ...
  ObIntervalDSValue(int64_t nsecond, int32_t fractional_second)
      : fractional_second_(fractional_second), nsecond_(nsecond)
  {}
  ObIntervalDSValue(
      bool is_negative, int64_t days, int64_t hour, int64_t minute, int64_t second, int32_t fractional_second)
  {
  ...
  inline ObIntervalDSValue calc(const ObIntervalDSValue& other, int32_t op) const
  {
    ...
    if ((fs_calc < 0 && nsecond_calc > 0) || (fs_calc > 0 && nsecond_calc < 0) || std::abs(fs_calc) >= MAX_FS_VALUE) {
  ...
  inline int validate()
  {
    ...
    if (OB_UNLIKELY(std::abs(nsecond_) > MAX_NSECOND_VALUE || std::abs(fractional_second_) >= MAX_FS_VALUE)) {
      ret = OB_ERR_THE_LEADING_PRECISION_OF_THE_INTERVAL_IS_TOO_SMALL;
    } else if (OB_UNLIKELY((nsecond_ < 0 && fractional_second_ > 0) || (nsecond_ > 0 && fractional_second_ < 0))) {
      ret = OB_ERR_UNEXPECTED;
    }
    return ret;
  }
