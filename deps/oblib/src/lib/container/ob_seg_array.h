...
public:
  ...
  bool is_full() const { return (head_ + 1) % SIZE == tail_; }
  bool is_empty() const { return head_ == tail_; }
  int64_t size() const { return (head_ + SIZE - tail_) % SIZE; }
  ...
  int search_boundary(SegPtr &prev,
                      SegPtr &next,
                      const LessThanEqualtoFunctor &le_functor);
...
private:
  inline void advance_(int64_t &v) const { v = (v + 1) % SIZE; }
  inline void decline_(int64_t &v) const { v = (v + SIZE - 1) % SIZE; }
  inline int64_t advanced_val(int64_t v) const { return (v + 1) % SIZE; }
  inline int64_t declined_val(int64_t v) const { return (v + SIZE - 1) % SIZE; }

  template<typename LessThanEqualtoFunctor>
  int binary_search_on_range_(const int64_t small,
                              const int64_t large,
                              SegPtr &prev,
                              SegPtr &next,
                              const LessThanEqualtoFunctor &le_functor);
...
template<typename SegPtr, int64_t SIZE>
template<typename LessThanEqualtoFunctor>
int ObFixedRingDeque<SegPtr, SIZE>::binary_search_on_range_(const int64_t small,
                                                            const int64_t large,
                                                            SegPtr &prev,
                                                            SegPtr &next,
                                                            const LessThanEqualtoFunctor &le_functor)
...
template<typename SegPtr, int64_t SIZE>
template<typename LessThanEqualtoFunctor>
int ObFixedRingDeque<SegPtr, SIZE>::search_boundary(SegPtr &prev,
                                                    SegPtr &next,
                                                    const LessThanEqualtoFunctor &le_functor)
{
  int ret = OB_SUCCESS;
  ...
  if (OB_UNLIKELY(is_empty())) {
    ...
    if (h > t) {
      // there is no reverse
      ret = binary_search_on_range_(t, h, prev, next, le_functor);
    } else {
      // there is reverse, search on two part
      if (le_functor(arr_[SIZE - 1])) {
        // 1. search on the second part
        ret = binary_search_on_range_(0, h, prev, next, le_functor);
        ...
        ret = binary_search_on_range_(t, SIZE - 1, prev, next, le_functor);
      }
    }
  }
  return ret;
...
public:
  ...
  ~Seg() { start_idx_ = 0; end_idx_ = 0; }

  inline int64_t get_count() const { return end_idx_ - start_idx_; }
  const T &get_delegate() const { return arr_[start_idx_]; }
  T &get_min() { return arr_[start_idx_]; }

  inline bool is_empty() const { return start_idx_ == end_idx_; }
  inline bool can_push_back() const { return end_idx_ < SIZE; }
  inline bool can_push_front() const { return start_idx_ > 0; }
...
int64_t Seg<T, SIZE>::to_string(char *buf, const int64_t buf_len) const
{
  ...
  databuff_printf(buf, buf_len, pos, "Seg={count=%ld, start_idx_=%ld, end_idx_ =%ld, arr=[",
                  count, start_idx_, end_idx_);
...
private:
  ...
  {
  public:
    SegLEFunctor(const T *y) : y_(y) {}
    bool operator()(const Seg<T, SEG_STEP> *x) const
    {
      bool bret = false;
...
public:
  ...
  virtual bool is_empty() const override { return 0 == total_count_; }
...
template<typename T, int64_t SEG_STEP, int64_t SEG_COUNT>
int ObSegArray<T, SEG_STEP, SEG_COUNT>::alloc_seg(Seg<T, SEG_STEP> *&seg,
                                                  const SegCreationType type)
{
  int ret = OB_SUCCESS;
  seg = NULL;
  char *buf = NULL;
...
template<typename T, int64_t SEG_STEP, int64_t SEG_COUNT>
int ObSegArray<T, SEG_STEP, SEG_COUNT>::search_boundary_internal_(const T &item, T *&prev_item,
                                                                  T *&next_item)
{
  typedef Seg<T, SEG_STEP> SegT;
  int ret = OB_SUCCESS;
  ...
  if (IS_NOT_INIT) {
    ...
    SegLEFunctor seg_le_functor(&item);

    // search for the lower and upper bound of segment
    ret = dir_.search_boundary(prev_base_seg, next_base_seg, seg_le_functor);
    ...
    // the lower bound of segment exists
      ...
      if (OB_ISNULL(prev_base_seg)) {
        LIB_LOG(ERROR, "dir search_boundary return invalid lower bound",
                K(ret), K(item), K(prev_base_seg), K(next_base_seg));
        ret = OB_ERR_UNEXPECTED;
      } else {
        // reset ret
        ret = OB_SUCCESS;
      ...
      LIB_LOG(ERROR, "dir search_boundary error",
              K(ret), K(item), K(prev_base_seg), K(next_base_seg));
    }
  }
  return ret;
