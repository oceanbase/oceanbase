...
ObIDMap<T, ID_TYPE, TSI_HOLD_NUM>::ObIDMap() : num_(1), overflow_limit_(0), array_(NULL), free_list_()
{
...
int ObIDMap<T, ID_TYPE, TSI_HOLD_NUM>::init(const ID_TYPE num)
{
  ...
  if (NULL != array_) {
  ...
  } else if (NULL == (array_ = (Node*)common::ob_malloc((num + 1) * sizeof(Node), memattr_))) {
    ...
    for (ID_TYPE i = 0; i < (num + 1); i++) {
      ...
      array_[i].id = i;
      if (0 != i && OB_SUCCESS != (ret = free_list_.push(&(array_[i])))) {
        _OB_LOG(WARN, "push to free list fail ret=%d i=%lu", ret, (uint64_t)i);
        break;
      }
    }
    num_ = num + 1;
    overflow_limit_ = (num_ << (calc_clz(num_) - 2));  // only use 30/62-low-bit, make sure integer multiples of num_
  }
  if (OB_FAIL(ret)) {
    destroy();
  }
...
int ObIDMap<T, ID_TYPE, TSI_HOLD_NUM>::assign(T* value, ID_TYPE& id)
{
  ...
  if (NULL == array_) {
    ...
    ret = OB_NOT_INIT;
  } else if (OB_SUCCESS != (ret = free_list_.pop(node)) || NULL == node) {
...
int ObIDMap<T, ID_TYPE, TSI_HOLD_NUM>::get(const ID_TYPE id, T*& value) const
{
  ...
  if (NULL == array_) {
    ...
    T* retv = NULL;
    if (id != array_[pos].id || ST_USING != array_[pos].stat) {
...
T* ObIDMap<T, ID_TYPE, TSI_HOLD_NUM>::fetch(const ID_TYPE id, const FetchMod mod)
{
  ...
  if (NULL != array_) {
    ...
    if (!lock_succ) {
      // do nothing
    } else if (id == array_[pos].id && ST_USING == array_[pos].stat) {
      ret = array_[pos].data;
    } else {
...
void ObIDMap<T, ID_TYPE, TSI_HOLD_NUM>::revert(const ID_TYPE id, const bool erase)
{
  if (NULL != array_) {
    ID_TYPE pos = id % num_;
    if (erase && id == array_[pos].id && ST_USING == array_[pos].stat) {
      ...
      if (OB_SUCCESS != (tmp_ret = free_list_.push(&(array_[pos])))) {
        _OB_LOG(ERROR, "push to free list fail ret=%d free list size=%ld", tmp_ret, free_list_.get_total());
      }
    }
    (void)array_[pos].lock.unlock();
  }
