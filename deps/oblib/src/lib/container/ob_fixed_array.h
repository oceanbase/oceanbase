...
public:
  ...
  ObFixedArrayImpl(AllocatorT* allocator = NULL, int64_t item_count = 0)
      : ObIArray<T>(), allocator_(allocator), init_cnt_(0), copy_assign_ret_(OB_SUCCESS)
  {
  ...
  void set_allocator(AllocatorT* alloc)
  {
    allocator_ = alloc;
  }
  ...
  inline ObFixedArrayImpl(const ObFixedArrayImpl<T, AllocatorT>& other)
      : ObIArray<T>(), allocator_(other.allocator_), init_cnt_(0), capacity_(0), copy_assign_ret_(OB_SUCCESS)
  {
    *this = other;
  }
