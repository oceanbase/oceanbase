...
public:
  ...
  int set(const int64_t sn, T* const& ptr, Func& cond, bool& set);
  ...
  //   else: includes out of memory, all fatal.
  template <typename Func>
  int pop(Func& cond, T*& ptr, bool& popped, bool use_lock = false);
...
// Definitions.
template <typename T>
ObExtendibleRingBuffer<T>::ObExtendibleRingBuffer() : BaseType(), alloc_()
{}

template <typename T>
ObExtendibleRingBuffer<T>::~ObExtendibleRingBuffer()
{}
...
template <typename T>
bool ObExtendibleRingBuffer<T>::is_inited() const
{
  return BaseType::is_inited();
}

template <typename T>
int ObExtendibleRingBuffer<T>::destroy()
{
  return BaseType::destroy();
}

template <typename T>
int ObExtendibleRingBuffer<T>::get(const int64_t sn, T*& ptr) const
{
  return BaseType::get(sn, ptr);
}

template <typename T>
int ObExtendibleRingBuffer<T>::set(const int64_t sn, T* const& ptr)
{
...
template <typename T>
template <typename Func>
int ObExtendibleRingBuffer<T>::set(const int64_t sn, T* const& ptr, Func& cond, bool& set)
{
...
template <typename T>
template <typename Func>
int ObExtendibleRingBuffer<T>::pop(Func& cond, T*& ptr, bool& popped, bool use_lock)
{
  return BaseType::pop(cond, ptr, popped, use_lock);
}

template <typename T>
int64_t ObExtendibleRingBuffer<T>::begin_sn() const
{
  return BaseType::begin_sn();
}

template <typename T>
int64_t ObExtendibleRingBuffer<T>::end_sn() const
{
