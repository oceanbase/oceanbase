...
public:
  ...
  {
    return static_cast<int64_t>(mem_end_of_storage_ - mem_begin_);
  }
...
public:
  ...
  void set_allocator(Allocator& allc)
  {
    pallocator_ = &allc;
  }
...
public:
  ...
  explicit ObSortedVector(
      int64_t size, Allocator* alloc = NULL, const lib::ObLabel& label = ObModIds::OB_MOD_DO_NOT_USE_ME)
...
public:
  ...
  {
    return vector_.remain();
  }
  inline int reserve(int64_t size)
  {
    return vector_.reserve(size);
  }
  inline int remove(iterator pos)
  {
    return vector_.remove(pos);
  }
  inline void clear()
  {
    vector_.clear();
...
public:
  ...
  int replace(const_value_type value, iterator& replace_pos, Compare compare, Equal equal, value_type& replaced_value);
  template <typename Compare, typename Unique>
  int insert_unique(const_value_type value, iterator& insert_pos, Compare compare, Unique unique);
  template <typename Compare>
