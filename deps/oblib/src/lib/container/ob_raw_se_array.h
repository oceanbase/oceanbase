...
public:
  ...
  {
    return offsetof(ObRawSEArray, local_data_) * 8;
  }
  static uint32_t local_data_buf_offset_bits()
  {
    return offsetof(ObRawSEArray, local_data_buf_) * 8;
  }
  static uint32_t block_size_offset_bits()
  {
    return offsetof(ObRawSEArray, block_size_) * 8;
  }
  static uint32_t count_offset_bits()
  {
    return offsetof(ObRawSEArray, count_) * 8;
  }
  static uint32_t capacity_offset_bits()
  {
    return offsetof(ObRawSEArray, capacity_) * 8;
  }
  static uint32_t max_print_count_offset_bits()
  {
    return offsetof(ObRawSEArray, max_print_count_) * 8;
  }
  static uint32_t error_offset_bits()
  {
    return offsetof(ObRawSEArray, error_) * 8;
  }
...
template <typename T, int64_t LOCAL_ARRAY_SIZE, typename BlockAllocatorT, bool auto_free>
int64_t ObRawSEArray<T, LOCAL_ARRAY_SIZE, BlockAllocatorT, auto_free>::get_serialize_size() const
{
  int64_t size = 0;
...
template <typename T, int64_t LOCAL_ARRAY_SIZE, typename BlockAllocatorT, bool auto_free>
OB_INLINE int ObRawSEArray<T, LOCAL_ARRAY_SIZE, BlockAllocatorT, auto_free>::push_back(const T& obj)
{
  int ret = OB_SUCCESS;
...
template <typename T, int64_t LOCAL_ARRAY_SIZE, typename BlockAllocatorT, bool auto_free>
int ObRawSEArray<T, LOCAL_ARRAY_SIZE, BlockAllocatorT, auto_free>::pop_back(T& obj)
{
  int ret = OB_SUCCESS;
  if (OB_UNLIKELY(count_ <= 0)) {
...
template <typename T, int64_t LOCAL_ARRAY_SIZE, typename BlockAllocatorT, bool auto_free>
int ObRawSEArray<T, LOCAL_ARRAY_SIZE, BlockAllocatorT, auto_free>::remove(int64_t idx)
{
  int ret = OB_SUCCESS;
  if (OB_UNLIKELY(0 > idx || idx >= count_)) {
    ret = OB_ARRAY_OUT_OF_RANGE;
...
template <typename T, int64_t LOCAL_ARRAY_SIZE, typename BlockAllocatorT, bool auto_free>
inline int ObRawSEArray<T, LOCAL_ARRAY_SIZE, BlockAllocatorT, auto_free>::prepare_allocate(int64_t capacity)
{
  int ret = OB_SUCCESS;
...
template <typename T, int64_t LOCAL_ARRAY_SIZE, typename BlockAllocatorT, bool auto_free>
int64_t ObRawSEArray<T, LOCAL_ARRAY_SIZE, BlockAllocatorT, auto_free>::to_string(char* buf, int64_t buf_len) const
{
  ...
  if (OB_LIKELY(this != &other)) {
    ...
    if (OB_SUCC(ret)) {
      if (is_memcpy_safe() && typeid(other).hash_code() == typeid(this).hash_code()) {
        memcpy(local_data_,
            static_cast<ObRawSEArray<T, LOCAL_ARRAY_SIZE, BlockAllocatorT, auto_free>*>(
...
inline int databuff_encode_element(char* buf, const int64_t buf_len, int64_t& pos, oceanbase::yson::ElementKeyType key,
    const ObRawSEArray<T, LOCAL_ARRAY_SIZE, BlockAllocatorT, auto_free>& array)
{
