...
public:
  ...
  {
    return offsetof(ObArrayImpl, data_) * 8;
  }
  static uint32_t count_offset_bits()
  {
    return offsetof(ObArrayImpl, count_) * 8;
  }
  static uint32_t valid_count_offset_bits()
  {
    return offsetof(ObArrayImpl, valid_count_) * 8;
  }
  static uint32_t data_size_offset_bits()
  {
    return offsetof(ObArrayImpl, data_size_) * 8;
  }
  static uint32_t block_size_offset_bits()
  {
    return offsetof(ObArrayImpl, block_size_) * 8;
  }
  static uint32_t error_offset_bits()
  {
    return offsetof(ObArrayImpl, error_) * 8;
  }
  static uint32_t reserve_offset_bits()
  {
...
private:
  ...
  inline int extend_buf(int64_t new_size)
  {
    ...
    if (NULL != new_data) {
      ...
      if (0 < data_size_) {
        _OB_LOG(DEBUG,
            "array extend buf, old_size=%ld new_size=%ld old_ptr=%p new_ptr=%p "
      ...
      block_allocator_(alloc)
{
  ...
  if (lib::this_worker().has_req_flag()) {
    if (auto_free) {
      set_block_allocator(BlockAllocatorT(static_cast<ObIAllocator&>(lib::this_worker().get_sql_arena_allocator())));
    }
  }
}
...
T* ObArrayImpl<T, BlockAllocatorT, auto_free, CallBack, ItemEncode>::alloc_place_holder()
{
  ...
  if (count_ < data_size_ / (int64_t)sizeof(T)) {
    ...
    _OB_LOG(WARN,
        ...
        data_size_,
        static_cast<int64_t>(sizeof(T)),
        data_size_ / static_cast<int64_t>(sizeof(T)));
  }
  return ret;
}

template <typename T, typename BlockAllocatorT, bool auto_free, typename CallBack, typename ItemEncode>
int64_t ObArrayImpl<T, BlockAllocatorT, auto_free, CallBack, ItemEncode>::to_string(char* buf, int64_t buf_len) const
{
  int64_t pos = 0;
  J_ARRAY_START();
...
int ObArrayImpl<T, BlockAllocatorT, auto_free, CallBack, ItemEncode>::push_back(const T& obj)
{
  ...
  if (OB_SUCCESS == ret && (count_ < data_size_ / (int64_t)sizeof(T))) {
    ...
    _OB_LOG(WARN,
        "count_=%ld, data_size_=%ld, (int64_t)sizeof(T)=%ld, data_size_/(int64_t)sizeof(T)=%ld, ret=%d",
        count_,
        data_size_,
        static_cast<int64_t>(sizeof(T)),
        data_size_ / static_cast<int64_t>(sizeof(T)),
...
template <typename T, typename BlockAllocatorT, bool auto_free, typename CallBack, typename ItemEncode>
ObArrayImpl<T, BlockAllocatorT, auto_free, CallBack, ItemEncode>::ObArrayImpl(
    const ObArrayImpl<T, BlockAllocatorT, auto_free, CallBack, ItemEncode>& other)
    : ObIArray<T>(),
      valid_count_(0),
      data_size_(0),
...
ObArrayImpl<T, BlockAllocatorT, auto_free, CallBack, ItemEncode>&
    ObArrayImpl<T, BlockAllocatorT, auto_free, CallBack, ItemEncode>::operator=(
        const ObArrayImpl<T, BlockAllocatorT, auto_free, CallBack, ItemEncode>& other)
{
  if (this != &other) {
    this->reset();
