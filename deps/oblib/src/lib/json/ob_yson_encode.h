...
// encoder
template <class T>
inline int databuff_encode_element(
    char* buf, const int64_t buf_len, int64_t& pos, ElementKeyType key, const T& obj, common::TrueType);
template <class T>
inline int databuff_encode_element(
    char* buf, const int64_t buf_len, int64_t& pos, ElementKeyType key, const T& obj, common::FalseType);
...
inline int databuff_encode_element(
    char* buf, const int64_t buf_len, int64_t& pos, ElementKeyType key, const std::pair<T1, T2>& obj);

template <>
inline int databuff_encode_element(
    char* buf, const int64_t buf_len, int64_t& pos, ElementKeyType key, const int8_t& value)
{
  int ret = OB_SUCCESS;
...
template <>
inline int databuff_encode_element(
    char* buf, const int64_t buf_len, int64_t& pos, ElementKeyType key, const bool& value)
{
  int ret = OB_SUCCESS;
...
template <>
inline int databuff_encode_element(
    char* buf, const int64_t buf_len, int64_t& pos, ElementKeyType key, const int32_t& value)
{
  int ret = OB_SUCCESS;
...
template <>
inline int databuff_encode_element(
    char* buf, const int64_t buf_len, int64_t& pos, ElementKeyType key, const int64_t& value)
{
  int ret = OB_SUCCESS;
...
template <>
inline int databuff_encode_element(
    char* buf, const int64_t buf_len, int64_t& pos, ElementKeyType key, const common::ObString& value)
{
  int ret = OB_SUCCESS;
  if (OB_UNLIKELY(value.length() < 0)) {
    ret = OB_INVALID_ARGUMENT;
  } else if (OB_UNLIKELY(
                 buf_len - pos < YSON_ELEMENT_TYPE_LEN + YSON_KEY_LEN + YSON_LEAST_STRING_LEN + value.length())) {
    ret = OB_BUF_NOT_ENOUGH;
  } else {
...
template <>
inline int databuff_encode_element(
    char* buf, const int64_t buf_len, int64_t& pos, ElementKeyType key, const char* const& value)
{
...
template <>
inline int databuff_encode_element(
    char* buf, const int64_t buf_len, int64_t& pos, ElementKeyType key, const uint32_t& value)
{
  int ret = OB_SUCCESS;
  if (OB_UNLIKELY(buf_len - pos < YSON_ELEMENT_TYPE_LEN + YSON_KEY_LEN + YSON_INT32_LEN)) {
    ret = OB_BUF_NOT_ENOUGH;
...
template <>
inline int databuff_encode_element(
    char* buf, const int64_t buf_len, int64_t& pos, ElementKeyType key, const uint64_t& value)
{
  int ret = OB_SUCCESS;
  if (OB_UNLIKELY(buf_len - pos < YSON_ELEMENT_TYPE_LEN + YSON_KEY_LEN + YSON_INT64_LEN)) {
    ret = OB_BUF_NOT_ENOUGH;
...
template <class T>
inline int databuff_encode_array_element(
    char* buf, const int64_t buf_len, int64_t& pos, ElementKeyType key, const common::ObIArrayWrap<T>& array);

template <class T>
inline int databuff_encode_element(
    char* buf, const int64_t buf_len, int64_t& pos, ElementKeyType key, const common::ObArrayWrap<T>& obj)
{
...
template <>
inline int databuff_encode_element(
    char* buf, const int64_t buf_len, int64_t& pos, ElementKeyType key, const common::ObTraceIdAdaptor& value)
{
  int ret = OB_SUCCESS;
...
template <>
inline int databuff_decode_element_value(
    const char* buf, const int64_t buf_len, int64_t& pos, common::ObTraceIdAdaptor& value)
{
  int ret = OB_SUCCESS;
  if (OB_UNLIKELY(buf_len - pos < sizeof(uint64_t) * 2)) {
    ret = OB_INVALID_DATA;
  } else {
...
#define TO_YSON_ELEMENTS(N)                                                                                         \
  ...
  int databuff_encode_elements(                                                                                     \
      char* buf, const int64_t buf_len, int64_t& pos, LST_DO_(N, TO_YSON_ARG_PAIR, (, ), PROC_ONE, ONE_TO_HUNDRED)) \
  {                                                                                                                 \
    int ret = OB_SUCCESS;                                                                                           \
    ...
    char* buf, const int64_t buf_len, int64_t& pos, ElementKeyType key, const std::pair<T1, T2>& obj)
{
  ...
  if (OB_UNLIKELY(buf_len - pos < YSON_ELEMENT_TYPE_LEN + YSON_KEY_LEN + YSON_LEAST_OBJECT_LEN)) {
    ...
    *((ElementKeyType*)(buf + pos)) = key;
    ...
    ret = oceanbase::yson::databuff_encode_elements(buf, buf_len, pos, key_first, obj.first, key_second, obj.second);
    if (OB_SUCC(ret)) {
      *((int32_t*)(buf + obj_size_pos)) = static_cast<int32_t>((pos - (obj_size_pos + sizeof(int32_t))));
    } else {
      pos = obj_size_pos - 1 - sizeof(key);  // revert pos for safety
