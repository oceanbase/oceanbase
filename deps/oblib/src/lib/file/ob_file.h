...
extern int64_t pread_align(const int fd, void* buf, const int64_t count, const int64_t offset, void* align_buffer,
    const int64_t buffer_size, const int64_t align_size);

// Use an aligned auxiliary buffer for dio writing. The buffer may have data reserved for alignment, so it needs to be
// passed in and out of buffer_pos
...
public:
  ...
  int64_t get_file_pos() const
  {
    return NULL == file_ ? -1 : file_->get_file_pos();
  }
  virtual int get_fd() const
  {
    return file_->get_fd();
...
template <class T, int64_t SIZE>
ObWaitablePool<T, SIZE>::ObWaitablePool() : list_(NULL), used_(0)
{
  ObMemAttr memattr;
...
class ObFileAsyncAppender : public ObIFileAppender, ObIWaiter {
  struct AIOCB {
    AIOCB() : buffer_pos(0), buffer(NULL)
    {
  ...
  struct OpenParam {
    OpenParam(const bool dio, const bool is_create, const bool is_trunc, const bool is_excl)
        : flag(O_RDWR), mode(S_IRUSR | S_IWUSR | S_IRGRP | S_IROTH)
    {
      flag |= dio ? O_DIRECT : 0;
