...
WAIT_EVENT_DEF(PARTITION_STORE_NEW_MEMTABLE_LOCK_WAIT, 15130, "partition store new memtable lock wait", "address", "number", "tries", CONCURRENCY, "partition store new memtable lock wait", false)
...
WAIT_EVENT_DEF(MACRO_META_MAJOR_KEY_MAP_LOCK_WAIT, 15142, "latch: macro block meta major key map lock wait", "address", "number", "tries", CONCURRENCY, "latch:  macro block meta major key map lock wait", true)
...
// liboblog
WAIT_EVENT_DEF(OBLOG_PART_MGR_SCHEMA_VERSION_WAIT, 18000, "oblog part mgr schema version wait", "", "", "", CONCURRENCY, "oblog part mgr schema version wait", true)
...
struct ObWaitEventDesc
{
  ...
  int64_t level_;
  int64_t parent_;
  bool is_phy_; // duoqiao.sc: make a distinction between logic wait and phy wait
