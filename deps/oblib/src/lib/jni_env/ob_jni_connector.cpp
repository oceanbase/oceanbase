...
int ObJniConnector::is_valid_loaded_jars_() {
  ...
  if (OB_FAIL(ret)) {
  } else if (OB_LIKELY(cur_version >= CLUSTER_VERSION_4_4_2_3)) {
    if (OB_LIKELY(JAR_VERSION_105 == real_jar_version)) {
      is_valid = true;
    } else {
      LOG_WARN("current major jar version is not 1.0.5", K(ret), K(real_jar_version), K(JAR_VERSION_105));
    }
