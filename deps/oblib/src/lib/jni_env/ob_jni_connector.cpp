...
int ObJniConnector::is_valid_loaded_jars_() {
  ...
  if (OB_FAIL(ret)) {
  } else if (OB_LIKELY(cur_version >= CLUSTER_VERSION_4_6_1_0 ||
                       cur_version == MOCK_CLUSTER_VERSION_4_4_2_1)) {
  ...
  } else if (OB_LIKELY(cur_version == CLUSTER_VERSION_4_6_0_0 ||
...
int ObJniConnector::get_jni_env(JNIEnv *&env) {
  int ret = OB_SUCCESS;
  ObJavaVmManager &helper = ObJavaVmManager::getInstance();
  if (OB_FAIL(helper.get_env(env))) {
    ...
    ret = OB_JNI_ENV_ERROR;
    LOG_WARN("got null jni env from helper", K(ret));
  }
  return ret;
}
