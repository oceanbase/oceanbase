...
int ObSimpleDynamicThreadPool::init(const int64_t thread_num, const char* name, const uint64_t tenant_id)
{
  ...
  if (OB_FAIL(set_adaptive_thread(1, thread_num))) {
    ...
    COMMON_LOG(WARN, "set_thread_count failed", K(ret));
  }
  return ret;
}

int ObSimpleDynamicThreadPool::start()
{
  int ret = OB_SUCCESS;
  if (OB_FAIL(lib::ThreadPool::start())) {
    COMMON_LOG(WARN, "start thread pool fail", K(ret));
  } else {
    ObSimpleThreadPoolDynamicMgr::get_instance().bind(this);
    has_bind_ = true;
  }
  return ret;
}
...
void ObSimpleDynamicThreadPool::destroy()
{
  int64_t ref_cnt = 0;
  if (has_bind_) {
    ObSimpleThreadPoolDynamicMgr::get_instance().unbind(this);
    has_bind_ = false;
  }
...
void ObSimpleDynamicThreadPool::try_expand_thread_count()
{
  ...
  if (min_thread_cnt_ > max_thread_cnt_) {
    ...
    if (inc_cnt > 0) {
      ...
      if (OB_FAIL(Threads::set_thread_count(cur_thread_count + inc_cnt))) { // TODO: fangwu.lcc the memory of io channel threads should allocted in tenant 500
...
void ObSimpleDynamicThreadPool::try_inc_thread_count(int64_t cnt)
{
  ...
  if (min_thread_cnt_ > max_thread_cnt_) {
    ...
    int64_t cur_thread_count = get_thread_count();
    int64_t new_thread_count = cur_thread_count;
  ...
  while (!has_set_stop()) {
    ...
    ObArray<ObSimpleDynamicThreadPool *> dec_pools;
    {
      ...
      DLIST_FOREACH_NORET(pool, simple_thread_pool_list_) {
        ...
        ObSimpleThreadPoolStat &pool_stat = pool->stat_;
...
void ObSimpleThreadPoolDynamicMgr::bind(ObSimpleDynamicThreadPool *pool)
{
  SpinWLockGuard guard(simple_thread_pool_list_lock_);
  simple_thread_pool_list_.add_first(pool);
}

void ObSimpleThreadPoolDynamicMgr::unbind(ObSimpleDynamicThreadPool *pool)
{
  SpinWLockGuard guard(simple_thread_pool_list_lock_);
  simple_thread_pool_list_.remove(pool);
}

int64_t ObSimpleThreadPoolDynamicMgr::get_pool_num() const
{
  return simple_thread_pool_list_.get_size();
}
