...
struct ObSimpleThreadPoolStat {
  ...
  TO_STRING_KV(K_(last_check_time), K_(last_idle_time), K_(last_thread_count));
  int64_t last_check_time_;
  int64_t last_idle_time_;
  int64_t last_thread_count_;
  int64_t last_shrink_time_;
};
class ObSimpleThreadPoolDynamicMgr;

class ObSimpleDynamicThreadPool
    : public lib::ThreadPool, public ObDLinkBase<ObSimpleDynamicThreadPool>
{
public:
...
public:
  ...
  void destroy();
  void run1();
  void bind(ObSimpleDynamicThreadPool *pool);
  void unbind(ObSimpleDynamicThreadPool *pool);
  int64_t get_pool_num() const;
  static ObSimpleThreadPoolDynamicMgr &get_instance();

private:
  ObDList<ObSimpleDynamicThreadPool> simple_thread_pool_list_;
