...
ObLocalDiagnosticInfo::ObLocalDiagnosticInfo()
    : di_ptr_(&ObDiagnosticInfo::dummy_di_), slot_(nullptr)
{}
...
void ObLocalDiagnosticInfo::set_thread_name(const char *name)
{
  ...
  MEMMOVE(instance.get_diagnostic_ptr()->get_ash_stat().program_, name,
    ...
    : need_restore_slot_(false), prev_slot_(nullptr)
{
  ...
  if (oceanbase::lib::is_diagnose_info_enabled()) {
    ...
    if (ObLocalDiagnosticInfo::get()->get_tenant_id() == tenant_id &&
        ObLocalDiagnosticInfo::get()->get_group_id() == group_id) {
      slot = ObLocalDiagnosticInfo::get()->get_summary_slot();
    }
    if (slot == nullptr) {
      if (tenant_id != OB_INVALID_TENANT_ID && tenant_id != OB_SERVER_TENANT_ID &&
          tenant_id != OB_DTL_TENANT_ID) {
        if (using_global) {
          ...
          if (OB_FAIL(c->summarys_.get_di_slot(
                  tenant_id, group_id, ObLocalDiagnosticInfo::get()->get_session_id(), slot))) {
...
ObDiagnosticInfoSwitchGuard::~ObDiagnosticInfoSwitchGuard()
{
  ...
  if (di_switch_success_) {
    ...
    if (2 == cur_di_->ref_cnt_) {
      ...
      ObLocalDiagnosticInfo::get()->get_ash_stat().set_sess_inactive();
    }
    ...
    parent_ptr_(nullptr)
{
  if (oceanbase::lib::is_diagnose_info_enabled()) {
    ObDiagnosticInfo *di = ObLocalDiagnosticInfo::get();
    if (di != &ObDiagnosticInfo::dummy_di_ && OB_NOT_NULL(di)) {
      sql::ObQueryRetryASHDiagInfo * cur_ptr = session.get_retry_info_for_update().get_query_retry_ash_diag_info_ptr();
      if (ObActiveSessionGuard::get_stat().get_retry_ash_diag_info_ptr() != cur_ptr) {
        sql::ObQueryRetryASHDiagInfo *parent_ptr_ = ObActiveSessionGuard::get_stat().get_retry_ash_diag_info_ptr();
        ObActiveSessionGuard::get_stat().set_retry_ash_diag_info_ptr(cur_ptr);
        is_switch_ = true;
      }
    }
  }
...
ObRetryWaitEventInfoGuard::~ObRetryWaitEventInfoGuard()
{
  if (is_switch_) {
    ObDiagnosticInfo *di = ObLocalDiagnosticInfo::get();
    if (di != &ObDiagnosticInfo::dummy_di_ && OB_NOT_NULL(di)) {
      ObActiveSessionGuard::get_stat().set_retry_ash_diag_info_ptr(parent_ptr_);
    } else {
      LOG_ERROR_RET(OB_ERR_UNEXPECTED, "(SHOULD NEVER HAPPEN) di point to dummy_di_ or nullptr", K(di));
    }
  }
}
