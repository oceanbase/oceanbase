...
typedef ObStatArray<ObStatEventSetStat, ObStatEventIds::STAT_EVENT_SET_END - ObStatEventIds::STAT_EVENT_ADD_END - 1>
    ObStatEventSetStatArray;
...
public:
  ...
  void reset();
  int notify_wait_begin(const int64_t event_no, const uint64_t timeout_ms = 0, const uint64_t p1 = 0,
      const uint64_t p2 = 0, const uint64_t p3 = 0);
  ...
  inline ObWaitEventHistory& get_event_history()
  {
    return event_history_;
  }
  inline ObWaitEventStatArray& get_event_stats()
  {
    return event_stats_;
  }
  inline ObStatEventAddStatArray& get_add_stat_stats()
  {
    return stat_add_stats_;
  }
  inline void reset_max_wait()
  {
    max_wait_ = NULL;
  }
  inline void reset_total_wait()
  {
    total_wait_ = NULL;
  }
  inline ObWaitEventDesc* get_max_wait()
  {
    return max_wait_;
  }
  inline ObWaitEventStat* get_total_wait()
  {
    return total_wait_;
  }
  inline bool is_valid() const
  {
    return tenant_id_ < UINT32_MAX;
  }
...
public:
  ...
  inline ObStatEventSetStatArray& get_set_stat_stats()
  {
    return stat_set_stats_;
  }
  inline ObLatchStatArray& get_latch_stats()
  {
    return latch_stats_;
  }
  TO_STRING_EMPTY();
    ...
    if (oceanbase::lib::is_diagnose_info_enabled()) {                                          \
      ...
      if (NULL != session_info) {                                                              \
        oceanbase::common::ObWaitEventStat* stat =                                             \
            session_info->get_event_stats().get(::oceanbase::common::ObWaitEventIds::stat_no); \
        if (NULL != stat) {                                                                    \
  ...
  do {                                                                                    \
    if (oceanbase::lib::is_diagnose_info_enabled()) {                                     \
      oceanbase::common::ObDiagnoseSessionInfo* di =                                      \
          oceanbase::common::ObDiagnoseSessionInfo::get_local_diagnose_info();            \
      if (di) {                                                                           \
        di->notify_wait_begin(oceanbase::common::ObWaitEventIds::stat_no, ##__VA_ARGS__); \
      }                                                                                   \
    }                                                                                     \
  } while (0)
...
#define USLEEP(time)                                                                                           \
  do {                                                                                                         \
    oceanbase::common::ObWaitEventGuard wait_guard(oceanbase::common::ObWaitEventIds::DEFAULT_SLEEP, 0, 0, 0); \
    ::oceanbase::lib::this_routine::usleep(time);                                                              \
  } while (0)
