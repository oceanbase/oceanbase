...
#define FALSE_IT(stmt) \
  ({                   \
    (stmt);            \
    false;             \
  })
#define OB_FALSE_IT(stmt) \
  ({                      \
    (stmt);               \
    false;                \
  })
...
#define htonll(i)                                                                              \
  ((((uint64_t)i & 0x00000000000000ff) << 56) | (((uint64_t)i & 0x000000000000ff00) << 40) |   \
      ...
      (((uint64_t)i & 0x000000ff00000000) >> 8) | (((uint64_t)i & 0x0000ff0000000000) >> 24) | \
      (((uint64_t)i & 0x00ff000000000000) >> 40) | (((uint64_t)i & 0xff00000000000000) >> 56))

#define DEFAULT_TIME_FORMAT "%Y-%m-%d %H:%M:%S"

#define ob_bswap_16(v) (uint16_t)((((uint16_t)v & 0x00ff) << 8) | (((uint16_t)v & 0xff00) >> 8))
...
void hex_dump(
    const void* data, const int32_t size, const bool char_type = true, const int32_t log_level = OB_LOG_LEVEL_DEBUG);
...
int double_expand_storage(
    T*& oldp, const int64_t old_size, const int64_t limit_size, int64_t& new_size, Allocator& allocator)
{
  int ret = OB_SUCCESS;
  new_size = old_size;
  void* newp = NULL;
  if (OB_SUCCESS != (ret = get_double_expand_size(new_size, limit_size))) {
    ...
    if (NULL != oldp) {
      allocator.free(reinterpret_cast<char*>(oldp));
    }
...
template <typename T>
int double_expand_storage(
    T*& oldp, const int64_t old_size, const int64_t limit_size, int64_t& new_size, const lib::ObLabel& label)
{
  ObMalloc allocator;
...
extern int replace_str(char* src_str, const int64_t src_str_buf_size, const char* match_str, const char* replace_str);

inline const char* get_peer_ip(char* buffer, size_t n, easy_request_t* req)
{
  static char mess[8] = "unknown";
  if (OB_LIKELY(NULL != req && NULL != req->ms && NULL != req->ms->c)) {
...
struct CountReporter {
  ...
  bool has_reported()
  {
    return last_report_count_ > 0;
  }
  void inc(const int64_t submit_time)
  {
    int64_t count = __sync_add_and_fetch(&count_, 1);
    int64_t total_cost_time = __sync_add_and_fetch(
        &total_cost_time_, (::oceanbase::common::ObTimeUtility::fast_current_time() - submit_time));
...
///@brief Whether s1 is equal to s2 in no more than N characters, ignoring case and regarding space behind  a word as
/// one blank. If equal, return true. Otherwise, return false.
///
/// For example:
...
struct __has_assign__<true, T> {
  ...
  static yes& chk(type_check<Sign, &_1::assign>*);
  template <typename>
  static no& chk(...);
...
{
  return construct_assign_wrap(
      dest, src, BoolType < __has_assign__<__is_class(T), T>::value && !HAS_MEMBER(T, get_copy_assign_ret) > ());
}
...
public:
  ...
  public:
    void stat()
    {
      ...
      (void)ATOMIC_FAA(&stat_count_, 1);
      if (ATOMIC_LOAD(&last_ts_) + stat_interval_ < cur_ts) {
        LIB_LOG(INFO, NULL == item_ ? "" : item_, K_(stat_count), K_(stat_interval));
...
/*
 *A method for fast calculation according to the localtime calculation formula, instead of the system function
 *localtime_r
 */
struct tm* ob_localtime(const time_t* unix_sec, struct tm* result);

/*
 * Function: Realize the fastest calculation of localtime according to the characteristics of log timestamp.
 * Realization logic: First check whether the input timestamp is the same as the last time (the unit of second is the
 * same, most of the consecutive log second units are the same), if the same, then directly Just use the localtime
 * calculated last time. If it is not the same, use the fast timestamp calculation method ob_localtime to calculate the
 * localtime.
 */
