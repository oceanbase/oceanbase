  ...
  int call(void) const
  {
    ...
    if (OB_LIKELY(trigger_freq == 0)) {
      ...
      if (rand() % trigger_freq == 0) {
        ret = static_cast<int>(error_code_);
        COMMON_LOG(WARN, "[ERRSIM] sim error", K(ret), K_(error_code), K(trigger_freq), K(lbt()));
      } else {
        ret = 0;
      }
    }
...
public:
  ...
  enum {
    ...
    EN_LOG_ARCHIVE_FROZEN_SYS_PIECE_FAILED = 195,
    EN_IGNORE_WRITE_NEXT_LOB_LOG = 196,
