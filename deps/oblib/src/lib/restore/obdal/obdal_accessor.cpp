...
public:
  ...
  ObStorageObdalRetryStrategy(const int64_t timeout_us = OB_STORAGE_MAX_IO_TIMEOUT_US)
...
ObDalLogSpanGuard::ObDalLogSpanGuard()
{
  ...
  const char *trace_id = common::ObCurTraceId::get_trace_id_str(trace_id_buf, sizeof(trace_id_buf));
