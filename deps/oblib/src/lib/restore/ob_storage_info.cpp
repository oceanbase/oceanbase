...
//***********************ObSTSToken***************************
ObSTSToken::ObSTSToken() : data_(nullptr), allocator_(OB_DEVICE_CREDENTIAL_ALLOCATOR)
{}
...
int ObSTSToken::set(const ObString &token)
{
  ...
  if (!token.empty()) {
    if (OB_FAIL(ob_dup_cstring(allocator_, token, data_))) {
...
{
  return set(ObString(token.data_));
}
...
int ObDeviceCredentialKey::init_(const char *role_arn, const char *external_id)
{
  ...
  if (IS_INIT) {
    ...
    // External id is optional, so the length of external id can be 0
      ...
      if (OB_FAIL(get_sts_credential_())) {
        LOG_WARN("failed to get sts credential", K(ret), KPC(this), KP(role_arn), KP(external_id));
      } else {
...
uint64_t ObDeviceCredentialKey::hash() const
{
  ...
  hash_value = murmurhash(role_arn_, static_cast<int32_t>(strlen(role_arn_)), hash_value);
  hash_value = murmurhash(external_id_, static_cast<int32_t>(strlen(external_id_)), hash_value);
  hash_value = murmurhash(sts_ak_, static_cast<int32_t>(strlen(sts_ak_)), hash_value);
  hash_value = murmurhash(sts_sk_, static_cast<int32_t>(strlen(sts_sk_)), hash_value);
  hash_value = murmurhash(sts_url_, static_cast<int32_t>(strlen(sts_url_)), hash_value);
...
int ObDeviceCredentialKey::assign(const ObDeviceCredentialKey &other)
{
  ...
  if (this == &other) {
    ...
    MEMCPY(sts_ak_, other.sts_ak_, sizeof(sts_ak_));
    MEMCPY(sts_sk_, other.sts_sk_, sizeof(sts_sk_));
    MEMCPY(sts_url_, other.sts_url_, sizeof(sts_url_));
...
bool ObDeviceCredentialKey::operator==(const ObDeviceCredentialKey &other) const
{
  return (is_inited_ == other.is_inited_)
         && (0 == STRCMP(role_arn_, other.role_arn_)) && (0 == STRCMP(external_id_, other.external_id_))
         && (0 == STRCMP(sts_ak_, other.sts_ak_)) && (0 == STRCMP(sts_sk_, other.sts_sk_))
         && (0 == STRCMP(sts_url_, other.sts_url_));
}
...
{
  return is_inited_ && OB_NOT_NULL(sts_credential_mgr_);
}

int ObDeviceCredentialKey::get_sts_credential_()
{
  int ret = OB_SUCCESS;
  ...
  if (OB_ISNULL(sts_credential_mgr_)) {
    ...
    LOG_WARN("sts_credential_mgr_ is null", K(ret), KPC(this));
  } else if (OB_FAIL(sts_credential_mgr_->get_sts_credential(sts_credential,
                                                             sizeof(sts_credential)))) {
...
int ObDeviceCredentialKey::construct_signed_url(char *url_buf, const int64_t url_buf_len) const
{
  ...
  if (IS_NOT_INIT) {
    ...
    if (OB_FAIL(params.push_back(ParamType("Action", STS_ACTION)))) {
      ...
      OB_LOG(WARN, "fail to push back", K(ret), K(params.count()), K(params));
    } else if (OB_FAIL(params.push_back(ParamType("ObSignatureKey", sts_ak_)))) {
      OB_LOG(WARN, "fail to push back", K(ret), K(params.count()), K(params));
    } else if (OB_FAIL(params.push_back(ParamType("ObSignatureSecret", sts_sk_)))) {
      OB_LOG(WARN, "fail to push back", K(ret), K(params.count()), K(params));
    ...
    if (FAILEDx(databuff_printf(url_buf, url_buf_len, pos,
        ...
        sts_url_, STS_ACTION, STS_RESOURCE_SOURCE,
        request_id, role_arn, signature_nonce, sts_ak_))) {
...
ObTenantStsCredentialBaseMgr *ObDeviceCredentialKey::sts_credential_mgr_ = nullptr;
int ObDeviceCredentialKey::register_sts_credential_mgr(
...
int check_sts_credential_format(const char *sts_credential, ObDeviceCredentialKey &credential_key)
{
  ...
  if (OB_ISNULL(sts_credential)
      || OB_UNLIKELY(strlen(sts_credential) <= 0
      || strlen(sts_credential) >= OB_MAX_STS_CREDENTIAL_LENGTH)) {
    ret = OB_INVALID_ARGUMENT;
    LOG_WARN("invalid args", K(ret), KP(sts_credential));
  } else {
    // sts_credential: sts_ak=xxx&sts_sk=xxx&sts_url=xxx
    ...
    int64_t sts_credential_len = strlen(sts_credential);
    char *sts_ak = credential_key.sts_ak_;
    char *sts_sk = credential_key.sts_sk_;
    char *sts_url = credential_key.sts_url_;

    MEMCPY(tmp_sts_credential, sts_credential, sts_credential_len);
    ...
    for (char *str = token; OB_SUCC(ret); str = nullptr) {
      ...
      if (nullptr == token) {
        ...
        if (OB_FAIL(ob_set_field(token + strlen(STS_AK), sts_ak, sizeof(credential_key.sts_ak_)))) {
          LOG_WARN("failed to set sts ak", K(ret), KP(token), K(sts_ak),
              K(sizeof(credential_key.sts_ak_)));
        }
      } else if (0 == strncmp(STS_SK, token, strlen(STS_SK))) {
        if (OB_FAIL(ob_set_field(token + strlen(STS_SK), sts_sk, sizeof(credential_key.sts_sk_)))) {
          LOG_WARN("failed to set sts sk", K(ret), KP(token), KP(sts_sk),
              K(sizeof(credential_key.sts_sk_)));
        }
      } else if (0 == strncmp(STS_URL, token, strlen(STS_URL))) {
        if (OB_FAIL(ob_set_field(token + strlen(STS_URL), sts_url, sizeof(credential_key.sts_url_)))) {
          LOG_WARN("failed to set sts url", K(ret), K(token), K(sts_url),
              K(sizeof(credential_key.sts_url_)));
        }
      }
    }
    if (OB_SUCC(ret)) {
