...
int ObAppendableFragmentMeta::parse_from(ObString &fragment_name)
{
  ...
  if (OB_UNLIKELY(!fragment_name.prefix_match(OB_ADAPTIVELY_APPENDABLE_FRAGMENT_PREFIX))) {
    ...
    if (OB_NOT_NULL(fragment_suffix)) {
      size_t fragment_suffix_len = strlen(fragment_suffix);
      if (fragment_suffix_len <= 1 || fragment_suffix_len >= sizeof(suffix_)) {
        ret = OB_INVALID_ARGUMENT;
        OB_LOG(WARN, "invalid fragment suffix", K(ret), K(fragment_suffix));
      } else {
        STRNCPY(suffix_, fragment_suffix, fragment_suffix_len);
        suffix_[fragment_suffix_len] = '\0';
      }
    }
  }
