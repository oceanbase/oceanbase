...
#define SLINK(c, f)                                       \
  ...
  public:                                                 \
    ...
    static const c* next_link(const c* e)                 \
    {                                                     \
      return e->f.next_;                                  \
    }                                                     \
  };                                                      \
...
#define LINK(c, f)                                       \
  ...
  public:                                                \
    ...
    static const c* prev_link(const c* e)                \
    {                                                    \
      return e->f.prev_;                                 \
    }                                                    \
  };                                                     \
...
#define LINK_DEFINITION(c, f)                  \
  inline c*& Link##_##c##_##f::next_link(c* e) \
  {                                            \
    return e->f.next_;                         \
  }                                            \
  inline c*& Link##_##c##_##f::prev_link(c* e) \
  {                                            \
    return e->f.prev_;                         \
  }
...
struct DLL {
  ...
  static T*& next(T* e)
  {
    return reinterpret_cast<T*&>(L::next_link(e));
  }
  static T*& prev(T* e)
  {
    return reinterpret_cast<T*&>(L::prev_link(e));
  }
  static T const* next(const T* e)
  {
    return L::next_link(e);
  }
  static T const* prev(const T* e)
  {
...
struct ObAtomicSLL {
  ...
  T* pop()
  {
    return reinterpret_cast<T*>(al_.pop());
  }
  T* popall()
  {
    return reinterpret_cast<T*>(al_.popall());
  }
  bool empty()
  {
    return al_.empty();
  ...
  T* remove(T* c)
  {
    return reinterpret_cast<T*>(al_.remove(c));
  }
  T* head()
  {
    return reinterpret_cast<T*>(al_.head());
  }
  T* next(T* c)
  {
    return reinterpret_cast<T*>(al_.next(c));
  }
