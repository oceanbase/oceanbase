...
public:
  ...
  int erase(iterator iter)
  {
    ...
    if (0 >= size_) {
      ret = OB_ENTRY_NOT_EXIST;
    } else if (NULL == iter.node_ || iter.node_ == (node_ptr_t)&root_) {
  ...
  int serialize(char* buf, const int64_t buf_len, int64_t& pos) const
  {
    ...
    if (OB_SUCCESS != (ret = serialization::encode_i64(buf, buf_len, tmp_pos, sz))) {
      _OB_LOG(WARN, "serialize size=%ld fail, ret=%d buf=%p buf_len=%ld pos=%ld", sz, ret, buf, buf_len, tmp_pos);
    } else {
      const_iterator iter;
  ...
  int deserialize(const char* buf, const int64_t data_len, int64_t& pos)
  {
    ...
    if (OB_SUCCESS != (ret = serialization::decode_i64(buf, data_len, tmp_pos, &sz))) {
      _OB_LOG(WARN, "deserialize size fail, ret=%d buf=%p date_len=%ld pos=%ld", ret, buf, data_len, tmp_pos);
    } else {
      clear();
      for (int64_t i = 0; i < sz; i++) {
        ...
        if (OB_SUCCESS != (ret = item.deserialize(buf, data_len, tmp_pos))) {
          _OB_LOG(WARN,
              "deserialize item fail, idx=%ld ret=%d buf=%p date_len=%ld pos=%ld",
              i,
              ret,
              buf,
              data_len,
