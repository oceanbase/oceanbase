...
static const crange alphaRangeTable[] = {{0x41, 0x5a}, {0x61, 0x7a}};
...
static const crange digitRangeTable[] = {{0x30, 0x39}};
...
static const chr punctCharTable[] = {0x3a, 0x3b, 0x3f, 0x40, 0x5f, 0x7b, 0x7d};

#define NUM_PUNCT_CHAR (sizeof(punctCharTable) / sizeof(chr))

static const crange spaceRangeTable[] = {{0x09, 0x0d}};

#define NUM_SPACE_RANGE (sizeof(spaceRangeTable) / sizeof(crange))

static const chr spaceCharTable[] = {0x20};

#define NUM_SPACE_CHAR (sizeof(spaceCharTable) / sizeof(chr))

static const crange lowerRangeTable[] = {{0x61, 0x7a}};
...
static const crange upperRangeTable[] = {{0x41, 0x5a}};
...
static const crange graphRangeTable[] = {{0x21, 0x7e}};
...
static const crange printRangeTable[] = {{0x20, 0x7E}};
 ...
 *	is automatically generated by the tools/uniClass.ob script
 *	and used in generic/regc_locale.c.  Do not modify by hand.
 */

/* Unicode: alphabetic characters */

static const crange alphaRangeTable[] = {{0x0041, 0x005a},
    {0x0061, 0x007a},
    {0x00c0, 0x00d6},
    {0x00d8, 0x00f6},
    {0x00f8, 0x021f},
...
static const chr alphaCharTable[] = {0x00aa,
    0x00b5,
    0x00ba,
    0x02d0,
    0x02d1,
...
static const crange digitRangeTable[] = {{0x0030, 0x0039},
    {0x0660, 0x0669},
    {0x06f0, 0x06f9},
    {0x0966, 0x096f},
    {0x09e6, 0x09ef},
...
static const crange punctRangeTable[] = {{0x0021, 0x0023},
    {0x0025, 0x002a},
    {0x002c, 0x002f},
    {0x005b, 0x005d},
    {0x055a, 0x055f},
...
static const chr punctCharTable[] = {0x003a,
    0x003b,
    0x003f,
    0x0040,
    0x005f,
...
static const crange spaceRangeTable[] = {{0x0009, 0x000d}, {0x2000, 0x200b}};

#define NUM_SPACE_RANGE (sizeof(spaceRangeTable) / sizeof(crange))

static const chr spaceCharTable[] = {0x0020, 0x00a0, 0x1680, 0x2028, 0x2029, 0x202f, 0x3000};
...
static const crange lowerRangeTable[] = {{0x0061, 0x007a},
    {0x00df, 0x00f6},
    {0x00f8, 0x00ff},
    {0x017e, 0x0180},
    {0x0199, 0x019b},
...
static const chr lowerCharTable[] = {0x00aa,
    0x00b5,
    0x00ba,
    0x0101,
    0x0103,
...
static const crange upperRangeTable[] = {{0x0041, 0x005a},
    {0x00c0, 0x00d6},
    {0x00d8, 0x00de},
    {0x0189, 0x018b},
    {0x018e, 0x0191},
...
static const chr upperCharTable[] = {0x0100,
    0x0102,
    0x0104,
    0x0106,
    0x0108,
...
static const crange graphRangeTable[] = {{0x0021, 0x007e},
    {0x00a0, 0x011f},
    {0x0121, 0x021f},
    {0x0222, 0x0233},
    {0x0250, 0x02ad},
...
static const chr graphCharTable[] = {0x0374,
    0x0375,
    0x037a,
    0x037e,
    0x038c,
...
static const crange printRangeTable[] = {{0x0020, 0x007E},
    {0x00A0, 0x01F5},
    {0x01FA, 0x0217},
    {0x0250, 0x02A8},
    {0x02B0, 0x02DE},
...
static const chr printCharTable[] = {0x037A,
    0x037E,
    0x038C,
    0x03DA,
    0x03DC,
    ...
    const chr* endp)                /* points just past end of name */
{
   ...
   */
  Ob_DStringInit(&ds);
  np = Ob_UniCharToUtfDString(startp, (int)len, &ds);
    ...
    int cases)                            /* case-independent? */
{
  ...
  for (c = a; c <= b; c++) {
    addchr(cv, c);
    lc = Ob_UniCharToLower((chr)c);
    uc = Ob_UniCharToUpper((chr)c);
    tc = Ob_UniCharToTitle((chr)c);
    if (c != lc) {
      addchr(cv, lc);
    }
    if (c != uc) {
...
static int                 /* predicate */
    before(celt x, celt y) /* collating elements */
{
  if (x < y) {
    return 1;
  }
 ...
 */
static struct cvec* cclass(struct vars* v, /* context */
    const chr* startp,                     /* where the name starts */
    const chr* endp,                       /* just past the end of the name */
    int cases,                             /* case-independent? */
    int oracle_mode)                       /* oracle call? */
{
  size_t len;
  struct cvec* cv = NULL;
  Ob_DString ds;
  ...
  static const char* classNames[] = {"alnum",
      "alpha",
      "ascii",
      "blank",
      "cntrl",
   ...
   */
  // oracle not do this, may be we should compatible oracle,
  if (cases && !oracle_mode && len == 5 && (strncmp("lower", np, 5) == 0 || strncmp("upper", np, 5) == 0)) {
    np = "alpha";
  }
    ...
    case CC_PRINT:
      ...
      if (cv) {
        ...
        for (i = 0; (size_t)i < NUM_PRINT_RANGE; i++) {
          addrange(cv, printRangeTable[i].start, printRangeTable[i].end);
        }
      }
      break;
    case CC_ALNUM:
      ...
      if (cv) {
        ...
        for (i = 0; (size_t)i < NUM_ALPHA_RANGE; i++) {
          addrange(cv, alphaRangeTable[i].start, alphaRangeTable[i].end);
        }
        for (i = 0; (size_t)i < NUM_DIGIT_RANGE; i++) {
          addrange(cv, digitRangeTable[i].start, digitRangeTable[i].end);
        }
      }
      break;
    case CC_ALPHA:
    ...
    case CC_PUNCT:
      ...
      if (cv) {
        for (i = 0; (size_t)i < NUM_PUNCT_RANGE; i++) {
          addrange(cv, punctRangeTable[i].start, punctRangeTable[i].end);
        }
    ...
    case CC_SPACE:
      ...
      if (cv) {
        for (i = 0; (size_t)i < NUM_SPACE_RANGE; i++) {
          addrange(cv, spaceRangeTable[i].start, spaceRangeTable[i].end);
        }
    ...
    case CC_LOWER:
      ...
      if (cv) {
        for (i = 0; (size_t)i < NUM_LOWER_RANGE; i++) {
          addrange(cv, lowerRangeTable[i].start, lowerRangeTable[i].end);
        }
    ...
    case CC_UPPER:
      ...
      if (cv) {
        for (i = 0; (size_t)i < NUM_UPPER_RANGE; i++) {
          addrange(cv, upperRangeTable[i].start, upperRangeTable[i].end);
        }
    ...
    case CC_GRAPH:
      ...
      if (cv) {
        for (i = 0; (size_t)i < NUM_GRAPH_RANGE; i++) {
          addrange(cv, graphRangeTable[i].start, graphRangeTable[i].end);
        }
 ...
 * OB_REG_ICASE backrefs need this.  It should preferably be efficient.
 * Note that it does not need to report anything except equal/unequal.
 * Note also that the length is exact, and the comparison should not
 * stop at embedded NULs!
...
{
  for (; len > 0; len--, x++, y++) {
    if ((*x != *y) && (Ob_UniCharToLower(*x) != Ob_UniCharToLower(*y))) {
      return 1;
    }
  }
  return 0;
