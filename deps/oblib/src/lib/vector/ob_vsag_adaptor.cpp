...
class VidFallbackFunc {
public:
  VidFallbackFunc(FilterInterface *bitmap, bool reverse_filter):
    bitmap_(bitmap), reverse_filter_(reverse_filter)
  {}
  ...
  FilterInterface *bitmap_;
  bool reverse_filter_;
};


class ExinfoFallbackFunc {
public:
  ExinfoFallbackFunc(FilterInterface *bitmap, bool reverse_filter):
    bitmap_(bitmap), reverse_filter_(reverse_filter)
  {}

  bool operator()(const char *data) const {
    if (!reverse_filter_) {
      return bitmap_->test(data);
    } else {
      return !(bitmap_->test(data));
    }
  }
...
public:
  ...
  TO_STRING_KV(KP(this), K_(is_created), K_(is_build), K_(use_static), KCSTRING_(dtype),
      ...
      K_(refine_type), K_(bq_bits_query), K_(bq_use_fht), K_(use_reorder), K_(doc_prune_ratio), K_(doc_prune_ratio), K_(window_size));
                                 ...
                                 float distance_threshold)
{
  ...
  ExinfoFallbackFunc exinfo_filter(bitmap, reverse_filter);
...
{
  int ret = OB_SUCCESS;
  VidFallbackFunc filter(bitmap, reverse_filter);
  ExinfoFallbackFunc exinfo_filter(bitmap, reverse_filter);
