...
int oceanbase::common::ObMemBuf::ensure_space(const int64_t size, const lib::ObLabel& label)
{
  ...
  if (size <= 0 || (NULL != buf_ptr_ && buf_size_ <= 0)) {
    _OB_LOG(WARN,
        "invalid param, size=%ld, buf_ptr_=%p, "
        "buf_size_=%ld",
        size,
        buf_ptr_,
        buf_size_);
...
void* oceanbase::common::ob_malloc_align(const int64_t alignment, const int64_t nbyte, const lib::ObLabel& label)
{
  char* ptr = static_cast<char*>(oceanbase::common::ob_malloc(nbyte + alignment, label));
  char* align_ptr = NULL;
  if (NULL != ptr) {
    align_ptr = reinterpret_cast<char*>(oceanbase::common::upper_align(reinterpret_cast<int64_t>(ptr), alignment));
    if (align_ptr == ptr) {
    ...
    _OB_LOG(WARN,
        "ob_tc_malloc allocate memory failed, alignment[%ld], nbyte[%ld], label[%s].",
        alignment,
        nbyte,
        (const char*)label);
  }
...
void* oceanbase::common::ob_malloc_align(const int64_t alignment, const int64_t nbyte, const ObMemAttr& attr)
{
  ...
  if (NULL != ptr) {
    ...
    _OB_LOG(WARN,
        "ob_tc_malloc allocate memory failed, alignment[%ld], nbyte[%ld], tenant_id[%lu], label[%s].",
        alignment,
        nbyte,
        attr.tenant_id_,
        (const char*)attr.label_);
  }
  return align_ptr;
}
