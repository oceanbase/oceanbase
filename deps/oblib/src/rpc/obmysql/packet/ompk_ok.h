...
public:
  ...
  inline void set_affected_rows(const uint64_t row)
  {
    affected_rows_ = row;
  }
  inline void set_last_insert_id(const uint64_t id)
  {
    last_insert_id_ = id;
  }
  ...
  inline void set_warnings(const uint16_t warnings)
  {
    warnings_ = warnings;
  }
  // shadow copy
  ...
  inline void set_capability(const ObMySQLCapabilityFlags& cap)
  {
    capability_ = cap;
  }
  ...
  inline uint64_t get_last_insert_id() const
  {
  ...
  inline uint16_t get_warnings() const
  {
    return warnings_;
  }
  inline const common::ObString& get_message() const
  {
    return message_;
  }
  inline const common::ObString& get_changed_schema() const
  {
    return changed_schema_;
  };
  inline bool is_state_changed() const
  {
  ...
  inline ObMySQLCapabilityFlags get_capability() const
  {
    return capability_;
  }
...
private:
  ...
  int serialize_string_kv(char* buffer, const int64_t length, int64_t& pos, const ObStringKV& string_kv) const;
...
private:
  ...
  bool state_changed_;
  common::ObSEArray<ObStringKV, 128> system_vars_;
