...
public:
  ...
  static int create(int64_t resp_id, const ObRpcPacket &tmp_pkt, int64_t sz, rpc::ObRequest*& req, int64_t req_arrival_ts);
  ...
  void set_trace_point(int32_t trace_point);
...
public:
  ...
  int update_thread_count(int net_thread_count);
  void stop();
  void wait();
  void destroy();
