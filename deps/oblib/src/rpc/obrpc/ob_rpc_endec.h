...
int rpc_encode_prepared_req(ObRpcProxy &proxy, ObRpcMemPool &pool, ObRpcPacketCode pcode,
...
template <typename T>
int rpc_encode_unprepared_req(ObRpcProxy &proxy, ObRpcMemPool &pool, ObRpcPacketCode pcode,
    ...
    )
{
  ...
  if (use_prepared) {
    ...
    ret = rpc_encode_prepared_req(proxy, pool, pcode, opts, *body, unneed_resp, req, req_sz);
  } else {
    ret = rpc_encode_unprepared_req(proxy, pool, pcode, args, opts, req, req_sz,
