...
int ObVirtualRpcProtocolProcessor::set_next_read_len(
    easy_message_t* ms, const int64_t fallback_len, const int64_t next_read_len)
{
  INIT_SUCC(ret);
  if (OB_UNLIKELY(NULL == ms) || OB_UNLIKELY(NULL == ms->input) || OB_UNLIKELY(fallback_len < 0) ||
 ...
 *   }
 */
int ObVirtualRpcProtocolProcessor::encode_compressed_rpc_packet(
    ObTimeGuard& timeguard, easy_request_t* req, ObRpcPacket*& pkt, int32_t& full_size)
{
  int ret = OB_SUCCESS;
  ...
  if (OB_ISNULL(req) || OB_ISNULL(pkt)) {
    ...
    if (!compress_ctx.is_inited_) {
      ...
      if (rpc_pkt_header_size >= block_size) {
        ...
        timeguard.click();
        MEMCPY(ring_buffer + ring_buffer_pos, ObReqHandler::MAGIC_HEADER_FLAG, 4);
        timeguard.click();
        ring_buffer_pos += 4;
        if (OB_FAIL(encode_i32(
                ring_buffer, ring_buffer_size, ring_buffer_pos, rpc_pkt_size))) {  // next 4 for packet content length
          ...
          if (OB_FAIL(encode_segment(req,
                  compress_ctx,
                  ring_buffer + segment_start_pos,
                  net_pkt_size,
                  header_size,
                  header_buf,
                  compressed_size))) {
            LOG_ERROR("failed to encode segment", K(ret), K(segment_start_pos), K(net_pkt_size), K(header_size));
          } else if (OB_FAIL(header.init(compress_ctx.compress_mode_,
                         (int16_t)(0 == compressed_size ? net_pkt_size : compressed_size),
                         (int16_t)net_pkt_size,
                         0 != compressed_size))) {
            ...
            int32_t total_data_len_after_compress =
                (int32_t)(header_size + (0 == compressed_size ? net_pkt_size : compressed_size));
        ...
        int64_t compressed_size = 0;
        {
          ...
          if (OB_FAIL(encode_i32(
            ...
            timeguard.click();
            if (pkt->get_clen() > 0) {
              packet_cdata_comsumed = static_cast<int16_t>(min(pkt->get_clen(), block_size - rpc_pkt_header_size));
            ...
            if (OB_FAIL(ret)) {
              // do nothing
            } else if (OB_FAIL(encode_segment(req,
                           compress_ctx,
                           ring_buffer + segment_start_pos,
                           head_packet_origin_payload,
                           head_packet_header_size,
                           head_packet_header_buf,
                           compressed_size))) {
              LOG_ERROR("failed to encode segment",
                  K(ret),
                  K(segment_start_pos),
                  K(head_packet_origin_payload),
                  K(head_packet_header_size));
            } else {
              timeguard.click();
              is_head_packet_compressed = (0 != compressed_size);
              total_data_len_after_compress +=
                  (int32_t)(is_head_packet_compressed ? compressed_size : head_packet_origin_payload);
        ...
        if (OB_SUCC(ret)) {
          ...
          while (OB_SUCC(ret) && remain_cdata_size > 0) {
            ...
            int64_t segment_start_pos = ring_buffer_pos;
            timeguard.click();
            MEMCPY(ring_buffer + ring_buffer_pos,
                const_cast<char*>(pkt->get_cdata()) + pkt->get_clen() - remain_cdata_size,
                segment_size);
            timeguard.click();
            ...
            } else if (header.init(compress_ctx.compress_mode_,
                           (int16_t)(0 == compressed_size ? segment_size : compressed_size),
                           (int16_t)segment_size,
                           0 != compressed_size)) {
...
int ObVirtualRpcProtocolProcessor::encode_raw_rpc_packet(
    ObTimeGuard& timeguard, easy_request_t* req, ObRpcPacket*& pkt, int32_t& full_size)
{
  int ret = OB_SUCCESS;
  if (OB_ISNULL(req) || OB_ISNULL(pkt)) {
  ...
  } else if (OB_ISNULL(req->ms) || OB_ISNULL(req->ms->pool)) {
    ret = OB_ERR_UNEXPECTED;
    LOG_ERROR("Easy message session(ms) or ms pool is NULL", "ms", "req->ms", K(ret));
  } else {
    // total data size we'll send via easy
    // [easy_buf] --> point to OB_NET_HEADER below
    // [OB_NET_HEADER]          easy allocated  part1
    // [OB_RPC_PACKET_HAEDER]   easy allocated  part1
    // [easy_buf] --> point to RPC PACKET CONTENT which is part2

    uint32_t pkt_size = static_cast<uint32_t>(pkt->get_encoded_size());
    uint32_t pkt_size_header_size = static_cast<uint32_t>(pkt->get_header_size());
    uint32_t send_size = OB_NET_HEADER_LENGTH + pkt_size;
    uint32_t part1_size = OB_NET_HEADER_LENGTH + pkt_size_header_size;
    uint32_t alloc_size = part1_size + static_cast<uint32_t>(sizeof(easy_buf_t));

    if (pkt->get_clen() > 0) {
      alloc_size += static_cast<uint32_t>(sizeof(easy_buf_t));
    }

    if (send_size > OB_MAX_RPC_PACKET_LENGTH) {
      // We find out the packet size would beyond max size of limitation
      // in OceanBase.
      ret = OB_ERR_UNEXPECTED;
      LOG_WARN("packet size beyond limit", K(ret), K(send_size), "limit", OB_MAX_RPC_PACKET_LENGTH);
    } else {
      timeguard.click();
      ObRpcProxy::PCodeGuard pcode_guard(OB_RPC_STREAM_TEST_ENCODE_RAW_PCODE);
      char* buf = easy_alloc(req->ms->pool, alloc_size);
      if (OB_ISNULL(buf)) {
        ret = OB_ALLOCATE_MEMORY_FAILED;
        LOG_WARN("no memory available in easy", K(alloc_size), K(ret));
      } else {
        timeguard.click();
        easy_buf_t* ebuf = reinterpret_cast<easy_buf_t*>(buf);  // pointer to easy buffer
        char* pbuf = reinterpret_cast<char*>(ebuf + 1);         // pointer to net header
        int64_t pos = 4;                                        // skip 4 bytes for magic flag
        MEMCPY(pbuf, ObReqHandler::MAGIC_HEADER_FLAG, 4);       // first 4 bytes store magic flag

        timeguard.click();
        if (OB_FAIL(encode_i32(pbuf, part1_size, pos, pkt_size))) {  // next 4 for packet content length
          LOG_WARN("failed to encode pkt_size", K(ret));
        } else if (OB_FAIL(encode_i32(pbuf, part1_size, pos, pkt->get_chid()))) {
          // next 4 for channel id
          LOG_WARN("failed to encode chid", K(ret));
        } else if (OB_FAIL(encode_i32(pbuf, part1_size, pos, 0))) {  // skip 4 bytes for reserved
          LOG_WARN("failed to encode reserved", K(ret));
        } else if (OB_FAIL(pkt->encode_header(pbuf, part1_size, pos))) {  // then copy packet header
          LOG_WARN("failed to packet header", K(ret));
        } else {
          timeguard.click();
          easy_buf_set_data(req->ms->pool, ebuf, pbuf, part1_size);
          easy_request_addbuf(req, ebuf);
          timeguard.click();

          if (pkt->get_clen() > 0) {
            easy_buf_t* cur_ebuf = reinterpret_cast<easy_buf_t*>(pbuf + pos);
            easy_buf_set_data(req->ms->pool, cur_ebuf, const_cast<char*>(pkt->get_cdata()), pkt->get_clen());
...
int ObVirtualRpcProtocolProcessor::decode_compressed_packet_data(ObTimeGuard& timeguard, easy_message_t* ms,
      ...
      if (data[0] & (int8_t)0x80) {
        ...
        if (OB_FAIL(header.decode(data, data_len, pos))) {
          ...
          LOG_WARN("obrpc packet payload exceed its limit",
              K(ret),
              K(header.origin_size_),
              "limit",
              OB_MAX_RPC_PACKET_LENGTH);
        } else {
          timeguard.click();
          ...
          if (OB_UNLIKELY(data_len != full_size)) {
            ...
            LOG_WARN("failed allocate memory", K(net_packet_buf_size), K(header), K(ret));
                         ...
                         pos,
                         net_packet_buf_pos))) {
            LOG_WARN("failed to decode_segment",
                KP(data),
                KP(net_packet_buf),
                K(compressed_size),
                K(pos),
                K(decompress_ctx),
                K(net_packet_buf_pos),
                K(header),
                "is_data_compressed",
                header.is_data_compressed(),
                K(decompress_ctx),
        ...
        if (OB_FAIL(header.decode(data, data_len, pos))) {
          ...
          LOG_WARN("obrpc packet payload exceed its limit",
              K(ret),
              K(header.total_data_len_before_compress_),
              "limit",
              OB_MAX_RPC_PACKET_LENGTH);
...
int ObVirtualRpcProtocolProcessor::decode_net_rpc_packet(
    ObTimeGuard& timeguard, char* data, int64_t data_len, ObRpcPacket*& pkt)
{
  int ret = OB_SUCCESS;
  if (OB_ISNULL(data) || OB_UNLIKELY(data_len <= OB_NET_HEADER_LENGTH + sizeof(ObRpcPacket))) {
    ...
    } else if (OB_UNLIKELY(0 != MEMCMP(&mflag[0], ObReqHandler::MAGIC_HEADER_FLAG, 4))) {
      ret = OB_ERR_UNEXPECTED;
...
int ObVirtualRpcProtocolProcessor::decode_raw_net_rpc_packet(
    common::ObTimeGuard& timeguard, easy_message_t* ms, const int64_t preceding_data_len, ObRpcPacket*& pkt)
{
  int ret = OB_SUCCESS;
  ...
  if (OB_ISNULL(ms) || OB_UNLIKELY(preceding_data_len < 0)) {
    ...
    uint32_t recv_len = 0;
    if ((recv_len = static_cast<uint32_t>(ms->input->last - ms->input->pos)) >=
        preceding_data_len + OB_NET_HEADER_LENGTH) {
      ...
      if (OB_ISNULL(in_data)) {
        ...
        timeguard.click();
        if (0 != MEMCMP(&mflag[0], ObReqHandler::MAGIC_HEADER_FLAG, 4)) {
...
int ObVirtualRpcProtocolProcessor::decode_segment(ObRpcCompressDCtx& ctx, char* data, char* net_packet_buf,
    int64_t net_packet_buf_size, bool is_data_compressed, int16_t compressed_size, int16_t original_size, int64_t& pos,
    int64_t& net_packet_buf_pos)
{
  ...
  if (OB_ISNULL(data) || OB_ISNULL(net_packet_buf) || OB_UNLIKELY(compressed_size <= 0) || OB_ISNULL(dctx) ||
    ...
    if (OB_UNLIKELY(original_size > net_packet_buf_size - net_packet_buf_pos)) {
      ret = OB_ERR_UNEXPECTED;
      LOG_ERROR(
          "net packet buf is not enough", K(original_size), K(net_packet_buf_size), K(net_packet_buf_pos), K(ret));
...
int ObVirtualRpcProtocolProcessor::encode_segment(easy_request_t* req, ObRpcCompressCCtx& compress_ctx, char* segment,
    int64_t segment_size, int64_t header_size, char*& header_buf, int64_t& compressed_size)
{
  int ret = OB_SUCCESS;
  ...
  if (OB_ISNULL(req) || OB_ISNULL(req->ms) || OB_ISNULL(pool = req->ms->pool) || OB_ISNULL(compressor) ||
      OB_ISNULL(ctx) || OB_ISNULL(segment) || OB_UNLIKELY(segment_size <= 0)) {
    ret = OB_INVALID_ARGUMENT;
    ...
    if (OB_FAIL(compressor->stream_compress(ctx, segment, segment_size, dest_buf, dest_size, compressed_size))) {
      ...
      // Only the compressed_size of zstd will return OB_SUCCESS with a value of 0, indicating that the data is not
      // compressible and special processing is required to send the data before compression
      timeguard.click();
      ...
      // In order to keep the two compression algorithms consistent, the compressed data is still transmitted if the
      // compressed length is longer
      timeguard.click();
