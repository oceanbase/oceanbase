      ...
      if ((magic != (int8_t)0xff) && (magic & MAGIC_KEEPALIVE_INDICATOR_MASK)) {
        ...
        if (recv_len < common::OB_NET_HEADER_LENGTH) {
          ...
          if (dlen > OB_MAX_RPC_PACKET_LENGTH) {
            ...
            LOG_WARN("obrpc packet payload exceeds its limit",
                K(magic),
                K(result),
                K(dlen),
                "limit",
      ...
      if (OB_FAIL(processor->decode(ms, pkt))) {
        ...
        if (NULL != pkt) {
          ...
          if (!pkt->is_resp() && fly_ts > common::OB_MAX_PACKET_FLY_TS && TC_REACH_TIME_INTERVAL(100 * 1000)) {
            LOG_WARN("packet fly cost too much time",
                "pcode",
                pkt->get_pcode(),
                "fly_ts",
                fly_ts,
...
int ObRpcNetHandler::encode(easy_request_t* req, void* packet)
{
  ...
  if (EASY_OK == eret) {
    ...
    if (!pkt->is_resp() && wait_ts > common::OB_MAX_PACKET_FLY_TS && TC_REACH_TIME_INTERVAL(100 * 1000)) {
      LOG_WARN("packet wait too much time before encode",
          "pcode",
          pkt->get_pcode(),
          "wait_ts",
          wait_ts,
...
void ObRpcNetHandler::set_trace_time(easy_request_t* req, void* packet)
{
  if (OB_ISNULL(req) || OB_ISNULL(packet)) {
    LOG_ERROR("packet should not be NULL");
  } else {
    ObRpcPacket* pkt = static_cast<ObRpcPacket*>(packet);
    req->request_arrival_time = pkt->get_request_arrival_time();
