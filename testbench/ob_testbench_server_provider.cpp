...
ObTestbenchServerProvider::ObTestbenchServerProvider()
    : is_inited_(false), tenant_list_(ObModIds::OB_LOG_SERVER_PROVIDER,
...
void ObTestbenchServerProvider::destroy() {
  if (IS_INIT) {
    ...
    tenant_name_list_.destroy();
    server_list_.destroy();
    tenant_server_map_.destroy();
  }
}
...
int ObTestbenchServerProvider::del_tenant(const uint64_t tenant_id) {
  ...
  if (IS_NOT_INIT) {
    ...
    TESTBENCH_LOG(WARN, "delete tenant_id from tenant_server_map_ failed",
                  K(ret), K(tenant_id));
  }
  return ret;
}
...
int ObTestbenchServerProvider::query_all_tenants() {
  ...
  } else if (OB_FAIL(systable_helper_->query_tenant_list(tenant_list_,
                                                         tenant_name_list_))) {
    TESTBENCH_LOG(ERROR, "get tenant_id_list by systable_helper_ failed",
                  K(ret));
  } else {
    TESTBENCH_LOG(INFO, "query_all_tenants updates tenant_list_",
                  K(tenant_list_));
    TESTBENCH_LOG(INFO, "query_all_tenants updates tenant_name_list_",
                  K(tenant_name_list_));
  }
  ...
  if (IS_NOT_INIT) {
    ...
    for (int tenant_idx = 0; tenant_idx < tenant_list_.count(); tenant_idx++) {
      ...
      if (OB_FAIL(tenant_list_.at(tenant_idx, tenant_id))) {
        TESTBENCH_LOG(WARN, "get_tenant_id failed", KR(ret), K_(tenant_list),
                      K(tenant_idx));
      } else if (OB_UNLIKELY(OB_INVALID_TENANT_ID == tenant_id)) {
        ret = OB_ERR_UNEXPECTED;
        TESTBENCH_LOG(ERROR, "expect valid tenant_id", KR(ret), K_(tenant_list),
                      K(tenant_idx), K(tenant_id));
      } else if (OB_FAIL(
                     tenant_server_map_.get(tenant_id, tenant_server_list))) {
        if (OB_ENTRY_NOT_EXIST == ret) {
          if (OB_FAIL(
                  tenant_server_map_.create(tenant_id, tenant_server_list))) {
            TESTBENCH_LOG(ERROR, "create tenant_server_list failed");
          }
        } else {
          TESTBENCH_LOG(ERROR,
                        "get tenant_server_list from tenant_server_map_ failed",
                        KR(ret), K(tenant_id));
        }
      }
      ...
      if (OB_SUCC(ret)) {
        if (OB_ISNULL(tenant_server_list)) {
          ret = OB_ERR_UNEXPECTED;
          TESTBENCH_LOG(ERROR,
                        "tenant_server_list get/created from "
                        "tenant_server_map_ should not be null",
                        KR(ret), K(tenant_id));
        } else if (OB_FAIL(systable_helper_->query_tenant_sql_server_list(
                       tenant_id, tenant_server_list->get_server_list()))) {
          TESTBENCH_LOG(
              ERROR, "query_tenant_sql_server_list by systable_helper failed",
              KR(ret), K(tenant_id));
        } else if (OB_UNLIKELY(0 == tenant_server_list->get_server_count())) {
          share::schema::TenantStatus tenant_status =
              share::schema::TenantStatus::TENANT_STATUS_INVALID;

          if (OB_FAIL(systable_helper_->query_tenant_status(tenant_id,
                                                            tenant_status))) {
            TESTBENCH_LOG(ERROR, "query_tenant_status failed", KR(ret),
                          K(tenant_id));
          } else if (share::schema::TenantStatus::TENANT_DELETED ==
                     tenant_status) {
            TESTBENCH_LOG(INFO, "tenant_already dropped", K(tenant_id),
                          K(tenant_status));
            ret = OB_ENTRY_NOT_EXIST;
          } else {
            TESTBENCH_LOG(
                WARN, "tenant_server_list is empty but tenant is not dropped",
                K(tenant_id), K(tenant_server_list));
          }
        } else {
          TESTBENCH_LOG(INFO, "find tenant servers", K(tenant_id),
                        K(*tenant_server_list));
        }

        if (OB_NOT_NULL(tenant_server_list)) {
          // revertt tenant_server_list anyway cause already get from
          // tenant_server_map_ succ.
...
int ObTestbenchServerProvider::get_server(const int64_t svr_idx,
  ...
  if (IS_NOT_INIT) {
    ...
    ret = OB_ENTRY_NOT_EXIST;
    TESTBENCH_LOG(WARN, "request server does not exist", K(ret), K(svr_idx),
                  K(server_list_));
  } else if (OB_FAIL(server_list_.at(svr_idx, server))) {
    TESTBENCH_LOG(WARN, "get server at specified position failed", K(ret),
                  K(svr_idx), K(server_list_));
  }
  return ret;
}
...
int ObTestbenchServerProvider::get_tenant_ids(
...
int ObTestbenchServerProvider::get_tenants(
    common::ObIArray<ObTenantName> &tenants) {
  int ret = OB_SUCCESS;
  tenants.reset();
  if (IS_NOT_INIT) {
    ret = OB_NOT_INIT;
...
int ObTestbenchServerProvider::get_tenant_servers(
    const uint64_t tenant_id,
    common::ObIArray<common::ObAddr> &tenant_servers) {
  int ret = OB_SUCCESS;
  tenant_servers.reset();
  TenantServerList *tenant_server_list = nullptr;
  if (IS_NOT_INIT) {
    ret = OB_NOT_INIT;
    TESTBENCH_LOG(ERROR, "ObTestbenchServerProvider not inited", K(ret));
  } else if (OB_FAIL(tenant_server_map_.get(tenant_id, tenant_server_list))) {
    TESTBENCH_LOG(ERROR, "cannot find tenant in tenant_server_map_", K(ret),
                  K(tenant_id));
  } else if (OB_FAIL(tenant_servers.assign(
                 tenant_server_list->get_server_list()))) {
    TESTBENCH_LOG(ERROR, "fail to get server list from tenant_server_list",
                  K(ret), K(tenant_id), K(tenant_servers));
  }

  if (OB_NOT_NULL(tenant_server_list)) {
    tenant_server_map_.revert(tenant_server_list);
    TESTBENCH_LOG(INFO, "tenant_server_list pointer", KP(tenant_server_list));
  }
  return ret;
}

int ObTestbenchServerProvider::refresh_server_list(void) {
  // TODO What's the meaning of refresh_server_list, since we get
  // servers/tenants in the prepare_refresh.
  int ret = OB_SUCCESS;
  return ret;
}
