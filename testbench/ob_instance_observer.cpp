...
static int64_t to_cluster_id_x(const char *sval)
{
  ...
  if (!to_int64(sval, cluster_id))
  {
    MPRINTx("need cluster id: %s", sval);
      ...
      cluster_id >=
          4294967296)
  { // cluster id given by command line must be in range [1,4294967295]
    MPRINTx(
...
int main(int argc, char *argv[])
{
  ...
  char CLOG_DIR[] = "store/clog";
  char SLOG_DIR[] = "store/slog";
  char SSTABLE_DIR[] = "store/sstable";
  ...
  else if (OB_FAIL(FileDirectoryUtils::create_full_path(CLOG_DIR)))
  {
    MPRINT("create store dir fail: ./store/clog/");
  }
  else if (OB_FAIL(FileDirectoryUtils::create_full_path(SLOG_DIR)))
  {
    MPRINT("create store dir fail: ./store/slog/");
  }
  else if (OB_FAIL(FileDirectoryUtils::create_full_path(SSTABLE_DIR)))
  {
    MPRINT("create store dir fail: ./store/sstable/");
  }
