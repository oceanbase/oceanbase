...
int ObTestbenchSystableHelper::init_conn(const libobcdc::MySQLConnConfig &cfg) {
  ...
  if (IS_INIT) {
    ...
    TESTBENCH_LOG(ERROR, "ObTestbenchSystableHelper mysql_conn_ init fail",
                  KR(ret), K(cfg));
  } else {
    is_inited_ = true;
...
int ObTestbenchSystableHelper::query_cluster_info(ClusterInfo &record) {
  ...
  if (IS_NOT_INIT) {
    ...
    TESTBENCH_LOG(ERROR,
                  "ObTestbenchSystableHelper query_cluster_info init fail",
                  KR(ret));
  } else if (OB_FAIL(do_query_(query))) {
    if (OB_NEED_RETRY == ret) {
      TESTBENCH_LOG(WARN,
                    "ObTestbenchSystableHelper do_query_ query_cluster_info "
                    "fail, need retry",
                    KR(ret), "mysql_error_code", query.get_mysql_err_code(),
                    "mysql_error_msg", query.get_mysql_err_msg());
    } else {
      TESTBENCH_LOG(
          ERROR, "ObTestbenchSystableHelper do_query_ query_cluster_info fail",
          KR(ret), "mysql_error_code", query.get_mysql_err_code(),
          "mysql_error_msg", query.get_mysql_err_msg());
    }
    ...
    if (OB_NEED_RETRY == ret) {
      TESTBENCH_LOG(WARN,
                    "ObTestbenchSystableHelper query get_records "
                    "query_cluster_info fail, need retry",
                    KR(ret), "mysql_error_code", query.get_mysql_err_code(),
                    "mysql_error_msg", query.get_mysql_err_msg());
    } else {
      TESTBENCH_LOG(
          ERROR,
          "ObTestbenchSystableHelper query get_records query_cluster_info fail",
          KR(ret), "mysql_error_code", query.get_mysql_err_code(),
          "mysql_error_msg", query.get_mysql_err_msg());
    }
  } else {
    TESTBENCH_LOG(INFO, "query_cluster_info", K(record), "query_svr",
                  query.get_server());
  }
  return ret;
}

int ObTestbenchSystableHelper::query_cluster_min_observer_version(
  ...
  if (IS_NOT_INIT) {
    ...
    TESTBENCH_LOG(ERROR,
                  ...
                  "query_cluster_min_observer_version init fail",
                  KR(ret));
  } else if (OB_FAIL(do_query_(query))) {
    if (OB_NEED_RETRY == ret) {
      TESTBENCH_LOG(WARN,
                    "do query_cluster_min_observer_version fail, need retry",
                    KR(ret), "mysql_error_code", query.get_mysql_err_code(),
                    "mysql_error_msg", query.get_mysql_err_msg());
    } else {
      TESTBENCH_LOG(ERROR, "do query_cluster_min_observer_version fail",
                    KR(ret), "mysql_error_code", query.get_mysql_err_code(),
                    "mysql_error_msg", query.get_mysql_err_msg());
    }
    ...
    if (OB_NEED_RETRY == ret) {
      TESTBENCH_LOG(WARN,
                    "get_records fail while "
                    "query_cluster_min_observer_version, need retry",
                    KR(ret), "mysql_error_code", query.get_mysql_err_code(),
                    "mysql_error_msg", query.get_mysql_err_msg());
    } else {
      TESTBENCH_LOG(ERROR,
  ...
  TESTBENCH_LOG(INFO, "query_cluster_min_observer_version", KR(ret),
                K(min_observer_version), K(records), "query_svr",
                query.get_server());

  return ret;
}

int ObTestbenchSystableHelper::query_timezone_info_version(
    const uint64_t tenant_id, int64_t &timezone_info_version) {
  int ret = OB_SUCCESS;
  BatchSQLQuery query;
  libobcdc::QueryTimeZoneInfoVersionStrategy
      query_timezone_info_version_strategy(tenant_id);
  ...
  if (IS_NOT_INIT) {
    ...
    if (OB_NEED_RETRY == ret) {
      TESTBENCH_LOG(WARN, "do query_timezone_info_version fail, need retry",
                    KR(ret), "mysql_error_code", query.get_mysql_err_code(),
                    "mysql_error_msg", query.get_mysql_err_msg());
    } else {
    ...
    if (OB_NEED_RETRY == ret) {
      TESTBENCH_LOG(
          WARN,
          "get_records fail while query_timezone_info_version, need retry",
          KR(ret), "mysql_error_code", query.get_mysql_err_code(),
          "mysql_error_msg", query.get_mysql_err_msg());
    } else {
      TESTBENCH_LOG(ERROR, "get_records fail while query_timezone_info_version",
                    KR(ret), "mysql_error_code", query.get_mysql_err_code(),
                    "mysql_error_msg", query.get_mysql_err_msg());
    }
  } else {
    if (false == record.is_timezone_info_version_exist_) {
      // 1. 1451 version ob cluster __all_zone table does not have
      // timezone_info_version, query is empty
      // 2. 226 versions of the ob cluster have the timezone table split into
      // tenants, if the tenants are not imported, then the query is not
      // available
  ...
  TESTBENCH_LOG(INFO, "query_timezone_info_version", KR(ret),
                K(timezone_info_version), K(record), "query_svr",
                query.get_server());

  return ret;
}

int ObTestbenchSystableHelper::query_tenant_ls_info(
    const uint64_t tenant_id, TenantLSIDs &tenant_ls_ids) {
  int ret = OB_SUCCESS;
  BatchSQLQuery query;
  ...
  if (IS_NOT_INIT) {
    ...
    if (OB_NEED_RETRY == ret) {
      TESTBENCH_LOG(WARN,
                    "get_records fail while query_tenant_ls_info, need retry",
                    KR(ret), "mysql_error_code", query.get_mysql_err_code(),
                    "mysql_error_msg", query.get_mysql_err_msg());
    } else {
      TESTBENCH_LOG(ERROR, "get_records fail while query_tenant_ls_info",
                    KR(ret), "mysql_error_code", query.get_mysql_err_code(),
                    "mysql_error_msg", query.get_mysql_err_msg());
    }
  }

  TESTBENCH_LOG(INFO, "query_tenant_ls_info", KR(ret), K(tenant_id),
                K(tenant_ls_ids), "query_svr", query.get_server());

  return ret;
}

int ObTestbenchSystableHelper::query_sql_server_list(
  ...
  if (IS_NOT_INIT) {
    ...
    if (OB_NEED_RETRY == ret) {
      TESTBENCH_LOG(WARN,
                    "get_records fail while query_all_server_info, need retry",
                    KR(ret), "mysql_error_code", query.get_mysql_err_code(),
                    "mysql_error_msg", query.get_mysql_err_msg());
    } else {
      TESTBENCH_LOG(ERROR, "get_records fail while query_all_server_info",
                    KR(ret), "mysql_error_code", query.get_mysql_err_code(),
                    "mysql_error_msg", query.get_mysql_err_msg());
    }
  }
...
int ObTestbenchSystableHelper::query_tenant_info_list(
    common::ObIArray<TenantInfo> &tenant_info_list) {
  int ret = OB_SUCCESS;
  BatchSQLQuery query;
  ...
  if (IS_NOT_INIT) {
    ...
    if (OB_NEED_RETRY == ret) {
      TESTBENCH_LOG(WARN,
                    "get_records fail while query_all_tenant_info, need retry",
                    KR(ret), "mysql_error_code", query.get_mysql_err_code(),
                    "mysql_error_msg", query.get_mysql_err_msg());
    } else {
      TESTBENCH_LOG(ERROR, "get_records fail while query_all_tenant_info",
                    KR(ret), "mysql_error_code", query.get_mysql_err_code(),
                    "mysql_error_msg", query.get_mysql_err_msg());
    }
  }
...
int ObTestbenchSystableHelper::query_tenant_list(
    common::ObIArray<uint64_t> &tenant_id_list,
    common::ObIArray<ObTenantName> &tenant_name_list) {
  int ret = OB_SUCCESS;
  BatchSQLQuery query;
  libobcdc::QueryAllTenantStrategy query_all_tenant_strategy;
  ...
  if (IS_NOT_INIT) {
    ...
    TESTBENCH_LOG(ERROR, "query tenant_id, tenant_name list failed", KR(ret),
                  K(tenant_info_list));
  } else {
    ...
    ARRAY_FOREACH_N(tenant_info_list, i, tenant_list_size) {
      ...
      if (OB_FAIL(tenant_id_list.push_back(tenant_id_name.tenant_id))) {
        TESTBENCH_LOG(ERROR, "push tenant_id into tenant_id_list failed",
                      K(tenant_id_name), K(tenant_id_list),
                      K(tenant_info_list));
      } else if (OB_FAIL(
                     tenant_name_list.push_back(tenant_id_name.tenant_name))) {
        TESTBENCH_LOG(ERROR, "push tenant_name into tenant_name_list failed",
                      K(tenant_id_name), K(tenant_name_list),
                      K(tenant_info_list));
      }
    }
  }

  TESTBENCH_LOG(INFO, "query_all_tenant_info", KR(ret), K(tenant_id_list),
                K(tenant_name_list));
...
int ObTestbenchSystableHelper::query_tenant_sql_server_list(
  ...
  int ret = OB_SUCCESS;
  BatchSQLQuery query;
  libobcdc::QueryTenantServerListStrategy query_tenant_serverlist_strategy(
      tenant_id);
  ...
  if (IS_NOT_INIT) {
    ...
    TESTBENCH_LOG(ERROR, "init tenant_server_list query failed", KR(ret),
                  K(tenant_id));
  } else if (OB_FAIL(do_query_(query))) {
    if (OB_NEED_RETRY == ret) {
      TESTBENCH_LOG(WARN, "do query_tenant_serverlist fail, need retry",
                    KR(ret), K(tenant_id), "mysql_error_code",
                    query.get_mysql_err_code(), "mysql_error_msg",
                    query.get_mysql_err_msg());
    } else {
      TESTBENCH_LOG(ERROR, "do query_tenant_serverlist fail", KR(ret),
                    K(tenant_id), "mysql_error_code",
                    query.get_mysql_err_code(), "mysql_error_msg",
                    query.get_mysql_err_msg());
    }
    ...
    if (OB_NEED_RETRY == ret) {
      TESTBENCH_LOG(
          WARN, "get_records fail while query_tenant_serverlist, need retry",
          KR(ret), K(tenant_id), "mysql_error_code", query.get_mysql_err_code(),
          "mysql_error_msg", query.get_mysql_err_msg());
    } else {
      TESTBENCH_LOG(ERROR, "get_records fail while query_tenant_serverlist",
                    KR(ret), K(tenant_id), "mysql_error_code",
                    query.get_mysql_err_code(), "mysql_error_msg",
                    query.get_mysql_err_msg());
    }
  ...
  TESTBENCH_LOG(INFO, "query_tenant_serverlist", KR(ret), K(tenant_id),
...
int ObTestbenchSystableHelper::query_tenant_status(
    const uint64_t tenant_id, share::schema::TenantStatus &tenant_status) {
  int ret = OB_SUCCESS;
  BatchSQLQuery query;
  ...
  if (IS_NOT_INIT) {
    ...
    TESTBENCH_LOG(ERROR, "init tenant_status query failed", KR(ret),
                  K(tenant_id));
  } else if (OB_FAIL(do_query_(query))) {
    if (OB_NEED_RETRY == ret) {
      TESTBENCH_LOG(WARN, "do query_tenant_status fail, need retry", KR(ret),
                    K(tenant_id), "mysql_error_code",
                    query.get_mysql_err_code(), "mysql_error_msg",
                    query.get_mysql_err_msg());
    } else {
    ...
    if (OB_NEED_RETRY == ret) {
      TESTBENCH_LOG(
          WARN, "get_records fail while query_tenant_status, need retry",
          KR(ret), K(tenant_id), "mysql_error_code", query.get_mysql_err_code(),
          "mysql_error_msg", query.get_mysql_err_msg());
    } else {
      TESTBENCH_LOG(ERROR, "get_records fail while query_tenant_status",
                    KR(ret), "mysql_error_code", query.get_mysql_err_code(),
                    "mysql_error_msg", query.get_mysql_err_msg());
    }
  }

  TESTBENCH_LOG(INFO, "query_tenant_status", KR(ret), K(tenant_id),
                K(tenant_status));
