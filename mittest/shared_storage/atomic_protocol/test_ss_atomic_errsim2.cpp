...
{
  {
    ...
    std::cout << "--------test request timeout but unsuccessfully executes--------" << std::endl;
    {
      ...
      // int inject_point_array[] = {3, 9};
      TEST_TIMEOUT_FAIL_REQUEST_WITH_RECONFIRM(write_point_array_in_total[MINI_OP_ID], point_array_total.size(), OB_TIMEOUT)
    }

    {
      // type: SHARE_MINI_CURRENT
      // step 2.A.a.v 2.B.a.v total two writes
      // int inject_point_array[] = {4, 9};
      TEST_TIMEOUT_FAIL_REQUEST_WITH_RECONFIRM(write_point_array_in_total[MINI_CURRENT], point_array_total.size(), OB_TIMEOUT)
    }

    {
      // type: SHARE_MINI_SSTABLE_TASK
      // step 3.B 4.B 5.B 6.B  total four writes
      // int inject_point_array[] = {10, 11, 12, 13};
      TEST_TIMEOUT_FAIL_REQUEST_WITH_RECONFIRM(write_point_array_in_total[MINI_SSTABLE_TASK], point_array_total.size(), OB_TIMEOUT)
    }

    {
      // type: SHARE_MINI_SSTABLE_LIST
      // step 2.A.c 4.D total two writes
      // int inject_point_array[] = {6, 14};
      TEST_TIMEOUT_FAIL_REQUEST_WITH_RECONFIRM(write_point_array_in_total[SHARE_MINI_SSTABLE_LIST], point_array_total.size(), OB_TIMEOUT)
    }
  }
}
...
{
  int ret = 0;
  system("rm -f ./test_atomic_errsim2.log*");
  OB_LOGGER.set_file_name("test_atomic_errsim2.log", true);
