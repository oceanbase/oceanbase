...
TEST_F(TestSNPCachedExternalFileService, test_evict_one_cached_macro)
{
  int ret = OB_SUCCESS;
  if (enable_test_) {
    ObPCachedExternalFileService *accesser = MTL(ObPCachedExternalFileService *);
    ...
    // clear disk cache
    GCONF.external_table_disk_cache_max_percentage = 0;
    ob_usleep(3 * OB_EXTERNAL_FILE_DISK_SPACE_MGR.CHECK_DISK_USAGE_INTERVAL_US);
    GCONF.external_table_disk_cache_max_percentage = 50;
    ASSERT_EQ(0, OB_EXTERNAL_FILE_DISK_SPACE_MGR.cached_macro_count());
    ASSERT_EQ(0, OB_EXTERNAL_FILE_DISK_SPACE_MGR.disk_size_allocated());

    ObStorageObjectHandle macro_handle;
    bool is_evicted = false;
    OK(OB_EXTERNAL_FILE_DISK_SPACE_MGR.evict_one_cached_macro(macro_handle, is_evicted));
    ASSERT_FALSE(is_evicted);
    ...
    // read external table file and check cached
    OK(read_and_check(external_file_info, external_read_info, write_buf, read_buf_size));
    ASSERT_EQ(1, accesser->running_prefetch_tasks_map_.size());
    OK(wait_prefetch_task_finish());
    bool is_cached = false;
    ObPCachedExtMacroKey macro_key(
        external_file_name, "", external_file_info.modify_time_, external_read_info.offset_);
    OK(accesser->get_cached_macro(macro_key, offset, 1/*read_size*/, is_cached, macro_handle));
    ASSERT_TRUE(is_cached);

    // Macro blocks under read cannot be evicted
    ObStorageObjectHandle read_handle;
    OK(accesser->async_read(external_file_info, external_read_info, read_handle));
    OK(OB_EXTERNAL_FILE_DISK_SPACE_MGR.evict_one_cached_macro(macro_handle, is_evicted));
    ASSERT_FALSE(is_evicted);
    // will trigger force evict
    OK(accesser->get_cached_macro(macro_key, offset, 1/*read_size*/, is_cached, macro_handle));
    ASSERT_FALSE(is_cached);
