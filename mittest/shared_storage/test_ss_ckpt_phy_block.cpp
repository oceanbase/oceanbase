             ...
             const common::ObCompressorType compressor_type = ObCompressorType::INVALID_COMPRESSOR)
{
  ...
  if (OB_UNLIKELY(item_buf_len_arr.empty())) {
    ...
    if (OB_FAIL(build_items(item_arr, item_buf_len_arr))) {
      ...
      for (int64_t i = 0; OB_SUCC(ret) && i < blk_id_list.count(); i++) {
        if (OB_UNLIKELY(blk_id_list[i] != entry_blk_id - i)) {
          ret = OB_ERR_UNEXPECTED;
          LOG_WARN("block id unexpected", K(ret), K(i), K(entry_blk_id), K(blk_id_list[i]));
        }
      }
...
TEST_F(TestSSCkptPhyBlock, test_basic_rw)
{
  ...
  const int64_t seg_payload_max_size = blk_payload_max_size - ObSSCkptPhyBlockSegmentHeader::get_serialize_size();
...
TEST_F(TestSSCkptPhyBlock, test_ckpt_segment)
{
  ...
  const int64_t seg_cnt = 100;
  for (int64_t i = 0; i < seg_cnt; ++i) {
    const int64_t seg_item_cnt = i + 1;
    for (int64_t j = 0; j < seg_item_cnt; ++j) {
      ItemInfo item_info;
      const int64_t item_size = ObRandom::rand(50, 4 * 1024);
      ASSERT_EQ(OB_SUCCESS, item_info.init(item_size));
      const bool start_new_seg = ((j == 0) ? true : false);
      bool is_persist = false;
      ASSERT_EQ(OB_SUCCESS, writer.write_item(item_info.buf_, item_info.buf_len_, start_new_seg, is_persist));
    }
    if (writer.ckpt_blk_header_.get_prev_block_id() == SS_INVALID_PHY_BLK_ID) {
