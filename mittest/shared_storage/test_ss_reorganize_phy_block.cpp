  ...
  for (int64_t i = 0; i < macro_cnt; ++i) {
    ...
    for (int32_t j = 0; j < micro_cnt; ++j) {
      ...
      ASSERT_EQ(OB_SUCCESS,
          micro_meta_mgr.add_or_update_micro_block_meta(micro_key, micro_size, micro_crc, mem_blk_handle, first_add));
...
TEST_F(TestSSReorganizePhyBlock, test_reorganize_phy_block_task)
{
  ...
  ASSERT_EQ(0, cache_stat.micro_stat().get_micro_pool_alloc_cnt());
  ...
  const int64_t ori_micro_pool_cnt = cache_stat.micro_stat().get_micro_pool_alloc_cnt();
  ...
  ASSERT_EQ(ori_micro_pool_cnt, cache_stat.micro_stat().get_micro_pool_alloc_cnt());
  ...
  for (int64_t i = 0; i < candidate_cnt; ++i) {
    ...
    ASSERT_LE(40 + SS_SUPER_BLK_COUNT, phy_blk_idx);
    ASSERT_GT(50 + SS_SUPER_BLK_COUNT, phy_blk_idx);
  ...
  for (int64_t i = 0; i < candidate_cnt; ++i) {
    const int64_t phy_blk_idx = reorgan_op.candidate_phy_blks_.at(i).phy_blk_idx_;
    ASSERT_LE(30 + SS_SUPER_BLK_COUNT, phy_blk_idx);
    ASSERT_GT(40 + SS_SUPER_BLK_COUNT, phy_blk_idx);
    ASSERT_EQ(OB_SUCCESS, candidate_idx_arr.push_back(phy_blk_idx));
  }
...
TEST_F(TestSSReorganizePhyBlock, test_evict_and_reorganize_parallel)
{
  ...
  // 3.5 handle reorganized micro_blocks
  {
    ...
    for (int64_t i = 0; i < candidate_micro_cnt; ++i) {
      ...
      ObTabletID tablet_id = micro_reorgan_entry.micro_key().get_major_macro_tablet_id();
      ObLSID ls_id(1);
