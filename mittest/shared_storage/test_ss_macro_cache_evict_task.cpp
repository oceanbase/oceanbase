...
TEST_F(TestSSMacroCacheEvictTask, evict_read_cache)
{
  ...
  // 4. simulate local cache disk space insufficient (95%), so as to trigger macro cache evict
  ObTenantDiskSpaceManager *disk_space_mgr = MTL(ObTenantDiskSpaceManager *);
  ASSERT_NE(nullptr, disk_space_mgr);
  ObSSMacroCacheStat macro_block_cache_stat;
  ASSERT_EQ(OB_SUCCESS, disk_space_mgr->get_macro_cache_stat(ObSSMacroCacheType::MACRO_BLOCK, macro_block_cache_stat));
  const int64_t macro_cache_size = disk_space_mgr->get_macro_cache_size();
  const int64_t macro_block_max_size = macro_cache_size * macro_block_cache_stat.get_max() / 100;
  const int64_t macro_block_used_size = macro_block_cache_stat.used_;
  const int64_t macro_block_avail_size = macro_block_max_size - macro_block_used_size;
  ASSERT_EQ(OB_SUCCESS, disk_space_mgr->alloc_file_size(macro_block_avail_size, ObSSMacroCacheType::MACRO_BLOCK));

  const int64_t macro_cache_used_size = disk_space_mgr->get_macro_cache_used_size();
  const int64_t tmp_file_alloc_size = macro_cache_size * 95 / 100 - macro_cache_used_size;
  ASSERT_EQ(OB_SUCCESS, disk_space_mgr->alloc_file_size(tmp_file_alloc_size, ObSSMacroCacheType::TMP_FILE));
  ...
  ASSERT_EQ(OB_SUCCESS, disk_space_mgr->free_file_size(macro_block_avail_size, ObSSMacroCacheType::MACRO_BLOCK));
  ASSERT_EQ(OB_SUCCESS, disk_space_mgr->free_file_size(tmp_file_alloc_size, ObSSMacroCacheType::TMP_FILE));
...
TEST_F(TestSSMacroCacheEvictTask, evict_tmp_file_write_cache)
{
  ...
  int64_t macro_cache_size = disk_space_mgr->get_macro_cache_size();
  int64_t tmp_file_max_size = macro_cache_size * tmp_file_cache_stat.get_max() / 100;
  int64_t tmp_file_used_size = tmp_file_cache_stat.used_;
  int64_t tmp_file_free_size = tmp_file_max_size - tmp_file_used_size;
  ...
  // 3. simulate local cache disk space insufficient (95%), so as to trigger macro cache evict
  ASSERT_EQ(OB_SUCCESS, disk_space_mgr->get_macro_cache_stat(ObSSMacroCacheType::TMP_FILE, tmp_file_cache_stat));
  tmp_file_max_size = macro_cache_size * tmp_file_cache_stat.get_max() / 100;
  tmp_file_used_size = tmp_file_cache_stat.used_;
  const int64_t tmp_file_avail_size = tmp_file_max_size - tmp_file_used_size;
  ASSERT_EQ(OB_SUCCESS, disk_space_mgr->alloc_file_size(tmp_file_avail_size, ObSSMacroCacheType::TMP_FILE));

  const int64_t macro_cache_used_size = disk_space_mgr->get_macro_cache_used_size();
  const int64_t macro_block_alloc_size = macro_cache_size * 95 / 100 - macro_cache_used_size;
  ASSERT_EQ(OB_SUCCESS, disk_space_mgr->alloc_file_size(macro_block_alloc_size, ObSSMacroCacheType::MACRO_BLOCK));
  ...
  ASSERT_EQ(OB_SUCCESS, disk_space_mgr->free_file_size(macro_block_alloc_size, ObSSMacroCacheType::MACRO_BLOCK));
  ASSERT_EQ(OB_SUCCESS, disk_space_mgr->free_file_size(tmp_file_avail_size, ObSSMacroCacheType::TMP_FILE));
}
...
TEST_F(TestSSMacroCacheEvictTask, fg_trigger_evict)
{
  ...
  const int64_t macro_cache_size = disk_space_mgr->get_macro_cache_size();
  const int64_t macro_block_max_size = macro_cache_size * macro_block_cache_stat.get_max() / 100;
  ASSERT_EQ(OB_SUCCESS, macro_cache_mgr->alloc_file_size(ObSSMacroCacheType::MACRO_BLOCK,
                                                         macro_block_max_size / 2));
  ASSERT_EQ(OB_SERVER_OUTOF_DISK_SPACE, macro_cache_mgr->alloc_file_size(ObSSMacroCacheType::MACRO_BLOCK,
                                                                         macro_block_max_size / 2 + 4096));
