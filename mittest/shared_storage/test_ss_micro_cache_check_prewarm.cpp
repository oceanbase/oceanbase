...
TEST_F(TestSSMicroCacheCheckPrewarm, test_check_prewarm)
{
  ...
  ASSERT_EQ(WRITE_BLK_CNT * micro_cnt + 1, arc_info.seg_info_arr_[SS_ARC_T1].cnt_);
  ...
  #define GET_CURR_ARC_SEG_CNT() \
    int64_t ori_t1_cnt = arc_info.seg_info_arr_[SS_ARC_T1].cnt_; \
    int64_t ori_b1_cnt = arc_info.seg_info_arr_[SS_ARC_B1].cnt_; \
    int64_t ori_t2_cnt = arc_info.seg_info_arr_[SS_ARC_T2].cnt_; \
    int64_t ori_b2_cnt = arc_info.seg_info_arr_[SS_ARC_B2].cnt_; \
  ...
  // 1. try to add existed micro_block
  {
    ...
    ASSERT_EQ(ori_t1_cnt - 1, arc_info.seg_info_arr_[SS_ARC_T1].cnt_);
    ASSERT_EQ(ori_t2_cnt + 1, arc_info.seg_info_arr_[SS_ARC_T2].cnt_);

    // 1.1.2 evict this micro_block
    ObSSMicroBlockMetaInfo evict_micro;
    tmp_micro_meta->get_micro_meta_info(evict_micro);
    ASSERT_EQ(OB_SUCCESS, micro_meta_mgr->try_evict_micro_block_meta(evict_micro));
    CHECK_IN_B2(tmp_micro_meta);
    ASSERT_EQ(ori_t2_cnt, arc_info.seg_info_arr_[SS_ARC_T2].cnt_);
    ASSERT_EQ(ori_b2_cnt + 1, arc_info.seg_info_arr_[SS_ARC_B2].cnt_);
    ...
    ASSERT_EQ(ori_t2_cnt + 1, arc_info.seg_info_arr_[SS_ARC_T2].cnt_);
    ASSERT_EQ(ori_b2_cnt, arc_info.seg_info_arr_[SS_ARC_B2].cnt_);
    ...
    ASSERT_EQ(ori_t2_cnt + 2, arc_info.seg_info_arr_[SS_ARC_T2].cnt_);

    // 1.2.2 evict this micro_block
    ObSSMicroBlockMetaInfo evict_micro1;
    tmp_micro_meta->get_micro_meta_info(evict_micro1);
  ...
  // 3. try to get existed micro_block
  {
    ...
    ASSERT_EQ(ori_t1_cnt - 1, arc_info.seg_info_arr_[SS_ARC_T1].cnt_);
    ASSERT_EQ(ori_b1_cnt + 1, arc_info.seg_info_arr_[SS_ARC_B1].cnt_);
    ...
    CHECK_IN_B1(tmp_micro_meta);
    ASSERT_EQ(hit_type, ObSSMicroCacheHitType::SS_CACHE_MISS);
    ASSERT_EQ(false, tmp_micro_meta->is_valid_field());
    ASSERT_EQ(true, tmp_micro_meta->is_data_persisted_);
    ASSERT_EQ(ori_b1_cnt + 1, arc_info.seg_info_arr_[SS_ARC_B1].cnt_);
    ...
    hit_type = ObSSMicroCacheHitType::SS_CACHE_MISS;
    micro_info.reset();
    ASSERT_EQ(OB_SUCCESS, TestSSCommonUtil::init_io_info(io_info, tmp_micro_key, micro_size, read_buf));
    ASSERT_EQ(OB_SUCCESS, micro_cache->inner_get_micro_block_handle(tmp_micro_key, micro_info,
  ...
  // 4. check some other prewarm interfaces.
  {
    ...
    ASSERT_EQ(ObSSMicroCacheHitType::SS_CACHE_HIT_DISK, hit_type);
