...
TEST_F(TestSSMicroCacheStruct, micro_block_meta)
{
  ...
  ASSERT_EQ(1, cache_stat.micro_stat().micro_pool_fixed_cnt_);
  ...
  ASSERT_EQ(0, cache_stat.micro_stat().micro_pool_fixed_cnt_);
}
...
TEST_F(TestSSMicroCacheStruct, micro_block_meta_3)
{
  ...
  uint32_t crc = 1;
  const uint64_t effective_tablet_id = micro_key.get_major_macro_tablet_id().id();
