...
TEST_F(TestSSMicroCache, test_get_micro_block)
{
  ...
  // 6. fetch this evict micro_block again, just like OB_ENTRY_NOT_EXIST
  {
    ...
    ASSERT_EQ(OB_ENTRY_NOT_EXIST, micro_cache->micro_meta_mgr_.get_micro_block_meta(tmp_micro_key, tmp_micro_handle, false));
    ASSERT_EQ(OB_SUCCESS, TestSSCommonUtil::get_micro_block(cur_info, read_buf));
...
TEST_F(TestSSMicroCache, test_add_micro_block_cache)
{
  ...
  micro_meta_handle.reset();
  ASSERT_EQ(OB_ENTRY_NOT_EXIST, micro_meta_mgr.get_micro_block_meta(micro_key, micro_meta_handle, false));
  micro_meta_handle.reset();
  ASSERT_EQ(OB_SUCCESS, micro_cache->add_micro_block_cache(
            micro_key, data_buf, micro_size, ObSSMicroCacheAccessType::MAJOR_COMPACTION_PREWARM_TYPE));
  ASSERT_EQ(OB_SUCCESS, micro_meta_mgr.get_micro_block_meta(micro_key, micro_meta_handle, false));
  ASSERT_EQ(false, micro_meta_handle()->is_in_l1());
  ASSERT_EQ(false, micro_meta_handle()->is_in_ghost());
...
TEST_F(TestSSMicroCache, test_parallel_add_micro_block_randomly)
{
  ...
  for (auto iter = micro_key_map.begin(); iter != micro_key_map.end(); ++iter) {
    ObSSMicroBlockMetaHandle micro_meta_handle;
    ASSERT_EQ(OB_SUCCESS, micro_meta_mgr.get_micro_block_meta(iter->first, micro_meta_handle, false));
    if (iter->second == 1) {
      ASSERT_EQ(true, micro_meta_handle()->is_in_l1());
    } else {
      ASSERT_EQ(false, micro_meta_handle()->is_in_l1());
...
TEST_F(TestSSMicroCache, test_add_micro_block_cache_for_prewarm)
{
  ...
  ASSERT_EQ(OB_SUCCESS, micro_meta_mgr.get_micro_block_meta(micro_key2, micro_meta_handle, false));
  ASSERT_EQ(false, micro_meta_handle()->is_in_l1());
  ASSERT_EQ(false, micro_meta_handle()->is_in_ghost());
  micro_meta_handle.reset();
}
...
TEST_F(TestSSMicroCache, test_get_not_exist_micro_blocks)
{
  ...
  // key0
  ObSSMicroBlockMetaHandle micro_meta_handle;
  ASSERT_EQ(OB_SUCCESS, micro_meta_mgr.get_micro_block_meta(micro_key0, micro_meta_handle, false));
  ...
  // key1
  micro_meta_handle.reset();
  ASSERT_EQ(OB_SUCCESS, micro_meta_mgr.get_micro_block_meta(micro_key1, micro_meta_handle, false));
  ASSERT_EQ(true, micro_meta_handle()->is_data_persisted());
  micro_meta_handle()->is_in_ghost_ = true;
  micro_meta_handle()->mark_invalid();
  ...
  ASSERT_EQ(OB_SUCCESS, in_arr.push_back(ObSSMicroPrewarmMeta(micro_key0, 0, micro_size, true)));
  ASSERT_EQ(OB_SUCCESS, in_arr.push_back(ObSSMicroPrewarmMeta(micro_key1, 0, micro_size, true)));
  ASSERT_EQ(OB_SUCCESS, in_arr.push_back(ObSSMicroPrewarmMeta(micro_key2, 0, micro_size, true)));
  ASSERT_EQ(OB_SUCCESS, in_arr.push_back(ObSSMicroPrewarmMeta(micro_key3, 0, micro_size, false)));
...
TEST_F(TestSSMicroCache, test_get_batch_la_micro_keys)
{
  ...
  for (int64_t i = 0; i < key_cnt; i++) {
    ...
    const ObSSMicroPrewarmMeta micro_key_meta(micro_key, 0, 1, false);
...
TEST_F(TestSSMicroCache, test_disable_micro_cache)
{
  ...
  ASSERT_EQ(OB_SUCCESS, obj_handle.wait());
  ASSERT_EQ(0, obj_handle.get_data_size());
  ASSERT_EQ(OB_ENTRY_NOT_EXIST, micro_meta_mgr.get_micro_block_meta(micro_key, micro_handle, false));
  ...
  ObSSMicroPrewarmMeta micro_key_meta(micro_key, micro_meta_info.crc_,
  ...
  ObSSMicroPrewarmMeta micro_key_meta2(micro_key2, 0/*data_crc*/, 4096/*data_size*/, false/*is_in_l1*/);
  ...
  micro_handle.reset();
  ASSERT_EQ(OB_SUCCESS, micro_meta_mgr.get_micro_block_meta(micro_key, micro_handle, true));
...
TEST_F(TestSSMicroCache, test_clear_micro_cache)
{
  ...
  for (auto iter = micro_key_map.begin(); iter != micro_key_map.end(); ++iter) {
    ObSSMicroBlockMetaHandle micro_meta_handle;
    ASSERT_EQ(OB_ENTRY_NOT_EXIST, micro_meta_mgr.get_micro_block_meta(iter->first, micro_meta_handle, false));
  }
  ...
  ASSERT_EQ(0, cache_stat.micro_stat().micro_pool_mem_size_);
  ASSERT_EQ(0, cache_stat.micro_stat().micro_map_mem_size_);
  ASSERT_EQ(0, cache_stat.micro_stat().micro_total_mem_size_);
  ...
  ASSERT_LT(0, cache_stat.mem_blk_stat().get_total_mem_blk_used_cnt());
}
