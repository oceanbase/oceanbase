...
{
  UNUSEDx(op_info, commit_version, commit_log_ts, status);
  return OB_SUCCESS;
}
...
TEST_F(TestMemCtxTableLock, clear_table_lock)
{
  ...
  ret = mem_ctx_.clear_table_lock(is_committed, commit_version, commit_log_ts);
  ASSERT_EQ(OB_SUCCESS, ret);
  // 2 COMMIT CLEAR
  is_committed = true;
  commit_version = 1;
...
TEST_F(TestMemCtxTableLock, check_lock_need_replay)
{
  ...
  ret = mem_ctx_.check_lock_need_replay(log_ts,
  ...
  log_ts = 2;
  mem_ctx_.max_durable_log_ts_ = log_ts;
  log_ts = 1;
  ret = mem_ctx_.check_lock_need_replay(log_ts,
                                        DEFAULT_OUT_TRANS_LOCK_OP,
                                        need_replay);
...
TEST_F(TestMemCtxTableLock, get_table_lock_store_info)
{
  ...
  log_ts = 1;
  mem_ctx_.set_log_synced(lock_op_node, log_ts);
  ret = mem_ctx_.get_table_lock_store_info(table_lock_info);
  ASSERT_EQ(OB_SUCCESS, ret);
  op_count = table_lock_info.table_lock_ops_.count();
  ASSERT_EQ(1, op_count);
  ASSERT_EQ(log_ts, table_lock_info.max_durable_log_ts_);
