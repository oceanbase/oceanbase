...
TEST_F(TestMultiVersionMerge, rowkey_cross_three_macro_full_merge)
{
  ...
  prepare_merge_context(MINI_MINOR_MERGE, true/*is_full_merge*/, trans_version_range, merge_context);
  // minor mrege
  ObSSTable *merged_sstable = nullptr;
  ASSERT_EQ(OB_SUCCESS, merger.merge_partition(merge_context, 0));
  build_sstable(merge_context, merged_sstable);
...
TEST_F(TestMultiVersionMerge, test_merge_with_multi_trans)
{
  ...
  for (int64_t i = 1; i <= 4; i++) {
    ...
    if (i < 4) {
      tx_data->commit_version_ = i * 10 + i;
      tx_data->start_log_ts_ = i;
      tx_data->end_log_ts_ = tx_data->commit_version_;
      tx_data->state_ = ObTxData::COMMIT;
    } else {
      tx_data->commit_version_ = INT64_MAX;
      tx_data->start_log_ts_ = i;
      tx_data->end_log_ts_ = 30;
...
TEST_F(TestMultiVersionMerge, rowkey_cross_two_macro_with_commit_version_less_multi_version_start)
{
  int ret = OB_SUCCESS;
  ObPartitionMinorMerger merger;
  ObTabletMergeDagParam param;
