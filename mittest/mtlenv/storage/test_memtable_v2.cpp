...
public:
                   ...
                   const int64_t prepare_version)
  {
    ...
    tx_ctx->exec_info_.prepare_version_ = prepare_version;
    mt_ctx->trans_version_ = prepare_version;
  }
                 ...
                 const int64_t expire_time = 10000000000)
  {
    ...
    store_ctx->mvcc_acc_ctx_.snapshot_.version_ = snapshot;
                ...
                const int64_t expire_time = 10000000000)
  {
    ...
    start_stmt(wtx, snapshot, expire_time);
  ...
  void lock_tx(ObStoreCtx *ltx,
               ...
               const int64_t expire_time = 10000000000)
   {
     ...
     start_stmt(ltx, snapshot, expire_time);
                ...
                const int64_t expire_time = 10000000000)
  {
    ...
    start_stmt(rtx, snapshot, expire_time);
    ...
    while (can_continue) {
      if (OB_FAIL(mmi.iterate_next_row())) {
        ...
        TRANS_LOG(INFO, "TEST_MEMTABLE V2: replay row",
                  K(*store_ctx));
        store_ctx->mvcc_acc_ctx_.mem_ctx_->set_redo_log_timestamp(replay_log_ts);
  ...
  int mock_trans_version_range()
  {
    ...
    trans_version_range_.snapshot_version_ = common::ObVersionRange::MAX_VERSION - 2;
    return OB_SUCCESS;
  }
                    ...
                    const uint8_t ndt_type = NDT_NORMAL)
  {
    ...
    EXPECT_EQ(trans_version, tnode->trans_version_);
    EXPECT_EQ(log_ts, tnode->log_timestamp_);
                  ...
                  int64_t v)
  {
    ...
    EXPECT_EQ(INT64_MAX, cb->log_ts_);
    ...
    EXPECT_EQ(INT64_MAX, tnode->trans_version_);
    EXPECT_EQ(INT64_MAX, tnode->log_timestamp_);
    EXPECT_EQ(seq_no, tnode->seq_no_);
    EXPECT_EQ(prev, tnode->prev_);
    EXPECT_EQ(next, tnode->next_);
    EXPECT_EQ(modify_count, tnode->modify_count_);
...
TEST_F(TestMemtableV2, test_parallel_lock_with_same_txn)
{
  ...
  wtx->mvcc_acc_ctx_.snapshot_.version_ = 1000;
