...
void TestIndexTree::SetUp()
{
  ...
  table_key_.version_range_.snapshot_version_ = INT64_MAX - 2;
...
TEST_F(TestIndexTree, test_micro_block_size)
{
  ...
  ASSERT_EQ(sstable_builder.build_meta_tree(res), OB_NOT_SUPPORTED);
}
...
TEST_F(TestIndexTree, test_merge_info_build_row)
{
  ...
  ObSSTableMergeRes res;
  mock_compaction(test_row_num, data_write_ctxs, index_write_ctxs, merge_info_list, res);
...
TEST_F(TestIndexTree, test_meta_builder)
{
  ...
  ASSERT_EQ(OB_SUCCESS, meta_builder.close(allocator_, root_desc));
  ASSERT_TRUE(root_desc.is_valid());
  ASSERT_TRUE(root_desc.is_mem_type());
  int64_t root_size = root_desc.addr_.size_;
  char *root_buf = root_desc.buf_;
...
TEST_F(TestIndexTree, test_reuse_macro_block)
{
  ...
  ObSSTableMergeRes res;
  mock_compaction(test_row_num, data_write_ctxs, index_write_ctxs, macro_metas, res);
  ASSERT_NE(nullptr, macro_metas);
