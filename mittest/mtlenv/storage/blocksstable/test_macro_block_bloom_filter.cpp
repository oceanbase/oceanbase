...
TEST_F(TestMacroBlockBloomFilter, test_append_micro_block)
{
  ...
  ASSERT_EQ(OB_SUCCESS, data_writer.flush_macro_block(current_block, true));
  ...
  ASSERT_EQ(OB_SUCCESS, micro_block_iter.open_for_whole_range(macro_block_buf, macro_block_size, false));
  ...
  while (OB_SUCC(ret)) {
    ...
    ret = micro_block_iter.get_next_micro_block_desc(micro_block_desc, micro_index_data, rowkey_allocator);
    if (ret == OB_ITER_END) {
      ret = OB_SUCCESS;
      break;
    } else {
    ...
    ASSERT_EQ(OB_SUCCESS, data_writer_test.append_micro_block(micro_block_desc, micro_index_data));
  }
  ASSERT_EQ(OB_SUCCESS, ret);
}
