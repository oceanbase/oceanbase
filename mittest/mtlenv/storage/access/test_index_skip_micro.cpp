...
TEST_F(TestSkipMicro, test_reverse_scan_with_spec_prefetch)
{
  int ret = OB_SUCCESS;
  ObTableHandleV2 handle;
  const char *micro_data[2];
  ...
  micro_data[0] =
    "bigint  bigint  varchar  bigint  bigint  bigint  flag  flag_type  multi_version_row_flag\n"
    "1004  1  6.12.234.22  40000  -1767480554103993000  -2  DELETE  NORMAL  C\n";

  micro_data[1] =
    "bigint  bigint  varchar  bigint  bigint  bigint  flag  flag_type  multi_version_row_flag\n"
    "1004  1  6.12.234.22  40000  -1767480554103992000  -1  INSERT  NORMAL  CL\n";
  ...
  ObSSTableRowScanner<> skip_scanner;
  skip_scanner.prefetcher_.is_prefetch_end_ = true; // 控制init不要触发预取
  ASSERT_EQ(OB_SUCCESS, skip_scanner.init(iter_param_, context_, table, &scan_range_));
  skip_scanner.prefetcher_.is_prefetch_end_ = false;
  const ObDatumRow *skip_prow = nullptr;

  // 控制预取1个微块
  skip_scanner.prefetcher_.need_check_prefetch_depth_ = true;
  ObLimitParam limit_param;
  limit_param.offset_ = 0;
  limit_param.limit_ = 1;
  context_.limit_param_ = &limit_param;
  ret = skip_scanner.inner_get_next_row(skip_prow);
  ASSERT_EQ(OB_SUCCESS, ret);
  ASSERT_NE(nullptr, skip_prow);
  ASSERT_TRUE(skip_prow->row_flag_.is_delete());
