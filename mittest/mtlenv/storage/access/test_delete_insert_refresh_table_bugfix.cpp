...
TEST_P(TestDeleteInsertRefreshTableBugfix, DISABLED_test_issue_2026060500116565627_before_fix)
{
  ...
  // mini merge 不允许 encoding row store，见 ObDataStoreDesc::is_store_type_valid()
  row_store_type_ = FLAT_ROW_STORE;
  reset_writer(snapshot_version, MINI_MERGE);
...
TEST_P(TestDeleteInsertRefreshTableBugfix, test_issue_2026060500116565627_after_fix)
{
  const int64_t max_batch_size = 4;
  int ret = OB_SUCCESS;
  ObTableStoreIterator table_store_iter;
