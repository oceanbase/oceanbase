...
TEST_F(TestTabletStatus, misc)
{
  ...
  ret = tablet->init(ls_id_, tablet_id, tablet_id, empty_tablet_id, empty_tablet_id,
      create_scn, snapshot_version, table_schema, compat_mode, store_flag, table_handle, freezer);
  ASSERT_EQ(OB_SUCCESS, ret);
  ...
  const int64_t log_ts = INT64_MAX - 1;
  const bool for_replay = false;
  tx_data.tx_id_ = 1;
  tx_data.tx_log_ts_ = INT64_MAX;
  ...
  trans_flags.log_ts_ = log_ts;
  trans_flags.for_replay_ = for_replay;

  const int64_t memtable_log_ts = ObLogTsRange::MAX_TS;
  ret = ObTabletCreateDeleteHelper::set_tablet_final_status(tablet_handle, ObTabletStatus::NORMAL,
      trans_flags.log_ts_, memtable_log_ts, trans_flags.for_replay_);
  ASSERT_EQ(OB_SUCCESS, ret);
