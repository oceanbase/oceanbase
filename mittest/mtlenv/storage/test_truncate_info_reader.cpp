  ...
  */
  ...
  { // last truncate info have largest commit version, use last truncate info to compare
    ...
    if (tablet_handle.is_valid()) {
      ...
      bool equal = 0;
      COMMON_LOG(INFO, "compare", KR(ret), KPC(distinct_array.truncate_info_array_.at(0)), K(info));
      ASSERT_EQ(OB_SUCCESS, info.compare(*distinct_array.truncate_info_array_.at(0), equal));
  ...
  */
  ...
  { // last truncate info have largest commit version, use last truncate info to compare
    ...
    if (tablet_handle.is_valid()) {
      ...
      bool equal = 0;
      ASSERT_EQ(OB_SUCCESS, info.compare(*distinct_array.truncate_info_array_.at(1), equal));
      ASSERT_EQ(equal, true);

      ObTruncateInfoArray cached_array;
      ObTruncateInfoCacheKey cache_key(MTL_ID(), tablet_id, info.schema_version_, tablet_handle.get_obj()->get_last_major_snapshot_version());
  ...
  */
  ...
  ASSERT_EQ(OB_SUCCESS, insert_truncate_info(trans_id + 100, commit_version + 100, info));
  {
    ...
    if (tablet_handle.is_valid()) {
      ...
      ASSERT_EQ(2, distinct_array.count());
      bool equal = 0;
      COMMON_LOG(INFO, "compare", KR(ret), KPC(distinct_array.truncate_info_array_.at(1)), K(info));
      ASSERT_EQ(OB_SUCCESS, info.compare(*distinct_array.truncate_info_array_.at(1), equal));
      ASSERT_EQ(equal, true);
    }
  }
...
TEST_F(TestTruncateInfoReader, test_truncate_info_cache)
{
  ...
  ObTruncateInfoArray mock_array;
  ASSERT_EQ(OB_SUCCESS, TruncateInfoHelper::batch_mock_truncate_info(allocator_, key_data, mock_array));
  const int64_t commit_cnt = 5;
  const int64_t max_cnt = 6;
  ...
  */
  ...
  ASSERT_EQ(OB_SUCCESS, wait_mds_mini_finish(tablet_id, mock_array.at(idx - 1)->commit_version_));

  for ( ; idx < mock_array.count(); ++idx) {
    ASSERT_EQ(OB_SUCCESS, insert_truncate_info(*mock_array.at(idx)));
  } // for
  ret = MediumInfoCommon::get_tablet(tablet_id, tablet_handle);
  ASSERT_EQ(OB_SUCCESS, ret);
  ...
  if (tablet_handle.is_valid()) {
    ...
    */
    ...
    ASSERT_EQ(mock_array.at(commit_cnt - 1)->commit_version_, truncate_info_cache.newest_commit_version_);

    truncate_info_cache.reset();
    ASSERT_EQ(OB_SUCCESS, insert_truncate_info(*mock_array.at(commit_cnt))); // set one abort info
