...
int64_t insert_start_time = 0;
int64_t inserted_cnt = 0;
const int64_t MOD_NS = 100001651;
...
private:
  ...
  void check_commit_version_row_(ObTxDataMemtableScanIterator &iter, ObTxDataMemtable *freezing_memtable);
...
{
  insert_start_time = ObTimeUtil::current_time_ns();
  ...
  while (true) {
    ...
    tx_data->start_log_ts_ = insert_start_time + (rand64(ObTimeUtil::current_time()) % MOD_NS);
    tx_data->commit_version_ = is_abort_tx ? -1 : (tx_data->start_log_ts_ + (rand64(ObTimeUtil::current_time()) % MOD_NS));
    tx_data->end_log_ts_ = is_abort_tx ? tx_data->start_log_ts_ : tx_data->commit_version_;
...
void TestTxDataTable::insert_rollback_tx_data_()
{
  ...
  for (int i = 0; i < 200; i++) {
    ...
    // fill in data
    tx_data->tx_id_ = tx_id;
    tx_data->start_log_ts_ = i % 2 ? insert_start_time + (rand() % 100_s) : INT64_MAX;
    tx_data->commit_version_ = insert_start_time + ((rand() % 10000));
    tx_data->end_log_ts_ = tx_data->commit_version_;
    if (tx_data->end_log_ts_ > max_end_log_ts) {
      max_end_log_ts = tx_data->end_log_ts_;
    }
    ...
    ASSERT_EQ(nullptr, tx_data);
    {
      ...
      ASSERT_EQ(max_end_log_ts, guard.tx_data().end_log_ts_);
    }
  }
}
...
{
  insert_start_time = ObTimeUtil::current_time();
  ObTxData *tx_data = nullptr;
  transaction::ObTransID tx_id;
  ...
  // fill in data
  tx_data->tx_id_ = tx_id;
  tx_data->start_log_ts_ = insert_start_time + MOD_NS + 1; // bigger than all tx datas to be the last one
  tx_data->commit_version_ = -1;
  tx_data->end_log_ts_ = tx_data->start_log_ts_;
...
{
  int64_t start_log_ts = insert_start_time - 300_s;
  int64_t commit_version = start_log_ts + 2_s;
  for (int i = 0; i < 500; i++) {
    past_commit_versions.array_.push_back(ObCommitVersionsArray::Node(start_log_ts, commit_version));
    start_log_ts = start_log_ts + 1_s + (rand() % 1_s);
    commit_version = std::max(commit_version, start_log_ts) + (rand() % 2_s);
  }
}
...
void TestTxDataTable::init_memtable_mgr_(ObTxDataMemtableMgr *memtable_mgr)
{
  ...
  ASSERT_EQ(OB_SUCCESS, memtable_mgr->create_memtable(1, 1));
                                    ...
                                    ObTxDataMemtable *&active_memtable)
{
  ...
  ASSERT_EQ(freezing_memtable->get_end_log_ts(), active_memtable->get_start_log_ts());

  // set frozen state
  freezing_memtable->set_state(ObTxDataMemtable::State::FROZEN);
}

void TestTxDataTable::check_commit_version_row_(ObTxDataMemtableScanIterator &iter, ObTxDataMemtable *freezing_memtable)
{
  // int ret = OB_SUCCESS;
  ...
  auto &cur_array = cur_commit_versions.array_;
  auto &past_array = past_commit_versions.array_;
  auto &merged_array = merged_commit_versions.array_;
  int64_t max_commit_version = -1;
  int64_t max_start_log_ts = -1;

  // check sort commit version result
  {
    ASSERT_EQ(OB_SUCCESS, freezing_memtable->do_sort_by_start_log_ts_());
    int64_t pre_start_ts = INT64_MIN;
    ...
    while (nullptr != cur_node) {
      ...
      ASSERT_GE(tx_data->start_log_ts_, pre_start_ts);
      if (ObTxData::COMMIT == tx_data->state_) {
        ...
        max_start_log_ts = std::max(max_start_log_ts, tx_data->start_log_ts_);
      }
      STORAGETEST_LOG(DEBUG,
                      "check_commit_version_row",
                      KPC(tx_data),
                      KTIME(tx_data->start_log_ts_),
                      KTIME(tx_data->end_log_ts_));
      pre_start_ts = tx_data->start_log_ts_;
      cur_node = cur_node->next_;
      cnt++;
    }
    last_commit_tx_data->commit_version_ = max_commit_version + 1;
    max_commit_version = last_commit_tx_data->commit_version_;
    ASSERT_EQ(cnt, inserted_cnt);
    fprintf(stdout, "total insert %ld tx data\n", cnt);

    ASSERT_EQ(OB_SUCCESS, iter.fill_in_cur_commit_versions_(cur_commit_versions));
    STORAGETEST_LOG(INFO, "cur_commit_versions count", K(cur_commit_versions.array_.count()));
    ASSERT_NE(0, cur_commit_versions.array_.count());
    for (int i = 1; i < cur_array.count() - 1; i++) {
      ASSERT_GE(cur_array.at(i).start_log_ts_, cur_array.at(i - 1).start_log_ts_ + iter.PERIODICAL_SELECT_INTERVAL_NS);
      ASSERT_GE(cur_array.at(i).commit_version_, cur_array.at(i - 1).commit_version_);
    }
    int i = cur_array.count() - 1;
    ASSERT_GE(cur_array.at(i).start_log_ts_, cur_array.at(i-1).start_log_ts_);
    ASSERT_GE(cur_array.at(i).commit_version_, cur_array.at(i-1).commit_version_);
    ASSERT_EQ(cur_array.at(i).start_log_ts_, max_start_log_ts);
    ASSERT_EQ(cur_array.at(i).commit_version_, max_commit_version);
  }

  // generate a fake past commit versions
  {
    generate_past_commit_version_(past_commit_versions);
    ASSERT_NE(0, past_commit_versions.array_.count());
    for (int i = 1; i < past_array.count() - 1; i++) {
      ASSERT_GE(past_array.at(i).start_log_ts_, past_array.at(i - 1).start_log_ts_);
      ASSERT_GE(past_array.at(i).commit_version_, past_array.at(i - 1).commit_version_);
    }
    int i = past_array.count() - 1;
    ASSERT_GE(past_array.at(i).start_log_ts_, past_array.at(i-1).start_log_ts_);
    ASSERT_GE(past_array.at(i).commit_version_, past_array.at(i-1).commit_version_);
  }
  ...
  // check merged result
  {
    int64_t recycle_ts = insert_start_time - (100LL * 1000LL * 1000LL/*100 seconds*/);
    ASSERT_EQ(OB_SUCCESS, iter.merge_cur_and_past_commit_verisons_(recycle_ts, cur_commit_versions,
                                                                   past_commit_versions,
                                                                   merged_commit_versions));
    for (int i = 1; i < merged_array.count() - 1; i++) {
      ASSERT_GE(merged_array.at(i).start_log_ts_, merged_array.at(i - 1).start_log_ts_);
      ASSERT_GE(merged_array.at(i).commit_version_, merged_array.at(i - 1).commit_version_);
    }
    int i = merged_array.count() -1;
    ASSERT_GE(merged_array.at(i).start_log_ts_, merged_array.at(i-1).start_log_ts_);
    ASSERT_GE(merged_array.at(i).commit_version_, merged_array.at(i-1).commit_version_);
    fprintf(stdout,
  ...
  {
    int64_t m_size = merged_commit_versions.get_serialize_size();
    ObArenaAllocator allocator;
    ObTxLocalBuffer buf_(allocator);
    buf_.reserve(m_size);

    int64_t pos = 0;
    ASSERT_EQ(OB_SUCCESS, merged_commit_versions.serialize(buf_.get_ptr(), m_size, pos));

    void *ptr = allocator_.alloc(sizeof(ObCommitVersionsArray));
    ObCommitVersionsArray *deserialize_commit_versions = new (ptr) ObCommitVersionsArray();
    pos = 0;
    const ObString str = buf_.get_ptr();
    ASSERT_EQ(OB_SUCCESS, deserialize_commit_versions->deserialize(str.ptr(), str.length(), pos));

    const auto &deserialize_array = deserialize_commit_versions->array_;
    ASSERT_EQ(merged_array.count(), deserialize_array.count());
    for (int i = 0; i < merged_commit_versions.array_.count(); i++) {
  ...
  tx_data_table_.calc_upper_trans_version_cache_.commit_versions_ = merged_commit_versions;

  // check the situation when sstable_end_log_ts is greater than the greatest start_log_ts in
  // merged_array
  {
    sstable_end_log_ts = max_start_log_ts + 1;
    upper_trans_version = 0;
    ASSERT_EQ(OB_SUCCESS,
              tx_data_table_.calc_upper_trans_version_(sstable_end_log_ts, upper_trans_version));
    ASSERT_EQ(max_commit_version, upper_trans_version);
  }

  // check the normal calculation
  {
    int64_t second_max_start_log_ts = merged_array.at(merged_array.count() - 2).start_log_ts_ - 1;
    for (int i = 0; i < 100; i++) {
      sstable_end_log_ts = second_max_start_log_ts - (rand() % 1100_s);
      upper_trans_version = INT64_MAX;
      ASSERT_EQ(OB_SUCCESS,
                tx_data_table_.calc_upper_trans_version_(sstable_end_log_ts, upper_trans_version));
      ASSERT_NE(INT64_MAX, upper_trans_version);
      ASSERT_NE(max_commit_version, upper_trans_version);
    }
  }
}
...
void TestTxDataTable::do_basic_test()
{
  ...
  check_commit_version_row_(iter, freezing_memtable);
...
void TestTxDataTable::test_serialize_with_action_cnt_(int cnt)
{
    ...
    tx_data->commit_version_ = ObTimeUtil::current_time();
    tx_data->end_log_ts_ = ObTimeUtil::current_time();
    tx_data->start_log_ts_ = tx_data->end_log_ts_ - 10000;
...
void TestTxDataTable::test_commit_versions_serialize_()
{
  ...
  int64_t start_log_ts = ObTimeUtil::current_time_ns();
  int64_t MOD = 137171;
  int64_t recycle_ts = start_log_ts + 1000 * MOD;

  int array_cnt = 50000;
  STORAGE_LOG(INFO, "start generate past array");
  for (int i = 0; i < array_cnt; i++) {
    start_log_ts += (rand() % MOD);
    ObCommitVersionsArray::Node node(start_log_ts, start_log_ts + (rand() % MOD));
  ...
  ObTxDataMemtableScanIterator iter(tx_data_table_.get_read_schema().iter_param_);
  ASSERT_EQ(OB_SUCCESS, iter.merge_cur_and_past_commit_verisons_(recycle_ts, cur_array, past_array,
                                                                 merged_array));
...
void TestTxDataTable::do_repeat_insert_test() {
  ...
  for (int i = 1; i <= 100; i++) {
    ...
    // fill in data
    tx_data->tx_id_ = tx_id;
    tx_data->start_log_ts_ = insert_start_time;
    tx_data->commit_version_ = transaction::ObTransVersion::INVALID_TRANS_VERSION;
    tx_data->end_log_ts_ = insert_start_time + i;
    tx_data->state_ = ObTxData::RUNNING;
