...
TEST_F(TestObSimpleLogClusterSingleReplica, test_gc_block)
{
  ...
  LogEngine *log_engine = &leader.palf_handle_.palf_handle_impl_->log_engine_;
  LogStorage *log_meta_storage = &log_engine->log_meta_storage_;
  block_id_t min_block_id; int64_t min_block_ts;
  EXPECT_EQ(OB_ENTRY_NOT_EXIST, log_engine->get_min_block_info_for_gc(min_block_id, min_block_ts));
  ...
  EXPECT_EQ(OB_ERR_OUT_OF_UPPER_BOUND, log_engine->get_min_block_info_for_gc(min_block_id, min_block_ts));
  EXPECT_EQ(OB_SUCCESS, submit_log(leader, 1, leader_idx, MAX_LOG_BODY_SIZE));
  EXPECT_EQ(OB_SUCCESS, wait_lsn_until_flushed(leader.palf_handle_.palf_handle_impl_->get_max_lsn(), leader));
  block_id_t expect_block_id = 1;
  int64_t expect_ts;
  EXPECT_EQ(OB_SUCCESS, log_engine->get_min_block_info_for_gc(min_block_id, min_block_ts));
  EXPECT_EQ(OB_SUCCESS, log_engine->get_block_min_ts_ns(expect_block_id, expect_ts));
  EXPECT_EQ(expect_ts, min_block_ts);
  EXPECT_EQ(OB_SUCCESS, log_engine->delete_block(0));
  EXPECT_EQ(OB_INVALID_TIMESTAMP, log_engine->min_block_max_ts_ns_);
