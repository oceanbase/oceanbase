...
TEST(TestArchiveSendTask, test_archive_send_task)
{
  ...
  int64_t max_log_ts = 10000000;
  const int64_t data_len = 1024;
  char data[data_len] = "test_data_archive";
  LSN start_offset(0);
  LSN end_offset(1024);
  ObLSID id(1001);
  ObArchivePiece piece(1024000000, 10000, 1024000000, 1);
  ObArchiveSendTask send_task;
  ArchiveWorkStation station(ArchiveKey(1, 1, 1), ObArchiveLease(1, 0, 0));
  EXPECT_EQ(OB_SUCCESS, send_task.init(tenant_id, id, station, piece, start_offset, end_offset, max_log_ts, data, data_len));

  start_offset = LSN(1024);
  end_offset = LSN(0);
  EXPECT_EQ(OB_INVALID_ARGUMENT, send_task.init(tenant_id, id, station, piece, start_offset, end_offset, max_log_ts, data, data_len));
  ...
  EXPECT_EQ(OB_SUCCESS, send_task2.init(tenant_id, id, station, piece, start_offset, end_offset, max_log_ts, data, data_len));
