...
struct FakeArchiveComponent
{
  ...
  TO_STRING_KV(K(round_id_), K(start_ts_), K(end_ts_), K(min_piece_id_), K(max_piece_id_));
};
...
private:
  ...
  virtual int get_round_(const int64_t start_ts) override;
