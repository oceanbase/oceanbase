...
int FakeArchivePieceContext::get_round_(const int64_t start_ts)
{
  int ret = OB_SUCCESS;
...
int FakeArchivePieceContext::load_round_(const int64_t round_id, RoundContext &round_context, bool &exist)
{
  ...
  if (OB_FAIL(rounds_->get_round(round_id, component))) {
    ...
    round_context.start_ts_ = component->start_ts_;
    round_context.base_piece_id_ = component->base_piece_id_;
    round_context.base_piece_ts_ = component->base_piece_ts_;
    ...
    if (end_exist) {
      ...
      round_context.end_ts_ = component->end_ts_;;
