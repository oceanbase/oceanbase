...
TEST_F(TestObSimpleLogClusterAccessMode, basic_change_access_mode)
{
  ...
  EXPECT_EQ(OB_EAGAIN, leader.palf_handle_.palf_handle_impl_->mode_mgr_.change_access_mode(mode_version, palf::AccessMode::RAW_WRITE, 0));
  ...
  while(OB_EAGAIN == tmp_ret) {
    tmp_ret = leader.palf_handle_.palf_handle_impl_->mode_mgr_.change_access_mode(mode_version, palf::AccessMode::RAW_WRITE, 0);
    usleep(10 * 1000);
  }
  EXPECT_EQ(OB_SUCCESS, leader.get_access_mode(mode_version, curr_access_mode));
  ...
  EXPECT_EQ(OB_SUCCESS, leader.change_access_mode(curr_proposal_id, mode_version, palf::AccessMode::FLASHBACK, 0));
  EXPECT_EQ(OB_NOT_MASTER, submit_log(leader, 1, id));
  // base_ts: 0.5 hour later
  EXPECT_EQ(OB_SUCCESS, leader.get_access_mode(mode_version, curr_access_mode));
  EXPECT_EQ(OB_SUCCESS, leader.get_role(unused_role, curr_proposal_id));
  EXPECT_EQ(OB_SUCCESS, leader.change_access_mode(curr_proposal_id, mode_version, AccessMode::APPEND, ref_ts_ns));
  std::vector<LSN> lsn_array;
  std::vector<int64_t> log_ts_array;
  EXPECT_EQ(OB_SUCCESS, submit_log(leader, 50, id, lsn_array, log_ts_array));
  for (auto log_ts : log_ts_array)
  {
    EXPECT_GT(log_ts, ref_ts_ns);
  }
  // 40 minutes later
  ...
  EXPECT_EQ(OB_STATE_NOT_MATCH, leader.change_access_mode(curr_proposal_id, mode_version, AccessMode::APPEND, ref_ts_ns));
  // can not APPEND -> FLASHBACK
  EXPECT_EQ(OB_STATE_NOT_MATCH, leader.change_access_mode(curr_proposal_id, mode_version, AccessMode::FLASHBACK, ref_ts_ns));
  EXPECT_EQ(OB_SUCCESS, leader.change_access_mode(curr_proposal_id, mode_version, AccessMode::RAW_WRITE, ref_ts_ns));
  EXPECT_EQ(OB_SUCCESS, leader.get_access_mode(mode_version, curr_access_mode));
  EXPECT_EQ(OB_SUCCESS, leader.get_role(unused_role, curr_proposal_id));
  EXPECT_EQ(OB_SUCCESS, leader.change_access_mode(curr_proposal_id, mode_version, AccessMode::APPEND, ref_ts_ns));
  lsn_array.clear();
...
TEST_F(TestObSimpleLogClusterAccessMode, switch_leader1)
{
  ...
  EXPECT_EQ(OB_SUCCESS, leader.change_access_mode(curr_proposal_id, mode_version, palf::AccessMode::RAW_WRITE, 0));
  EXPECT_EQ(OB_SUCCESS, leader.get_role(unused_role, curr_proposal_id));
  EXPECT_EQ(OB_SUCCESS, leader.get_access_mode(mode_version, curr_access_mode));
...
TEST_F(TestObSimpleLogClusterAccessMode, prev_log_slide)
{
  ...
  EXPECT_EQ(OB_EAGAIN, leader.palf_handle_.palf_handle_impl_->mode_mgr_.switch_state_(AccessMode::RAW_WRITE, 0, false));
