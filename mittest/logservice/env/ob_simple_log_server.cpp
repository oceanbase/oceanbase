...
int ObSimpleLogServer::init_ls_service_for_role_change_()
{
  int ret = OB_SUCCESS;
  if (ls_service_.is_inited_) {
  } else if (OB_FAIL(ls_service_.ls_allocator_.init(common::OB_MALLOC_NORMAL_BLOCK_SIZE,
                                                    "mittest",
                                                    node_id_,
                                                    1024 * 1024 * 1024))) {
                                                   ...
                                                   IPalfHandleImpl *&palf_handle_impl)
{
  ...
  } else if (OB_FAIL(init_ls_service_for_role_change_())) {
    CLOG_LOG(WARN, "init_ls_service_for_role_change_ failed", K(ret), K(palf_id));
  } else if (OB_FAIL(palf_env_impl->create_palf_handle_impl(
      palf_id, access_mode, base_info, replica_type, palf_handle_impl))) {
                                                   ...
                                                   IPalfHandleImpl *&palf_handle_impl)
{
  ...
  if (LogReplicaType::LOGONLY_REPLICA == replica_type) {
    ret = create_ls_with_log_service_(
        palf_id, access_mode, base_info, replica_type, palf_handle_impl);
...
int ObSimpleLogServer::remove_ls(const int64_t palf_id)
{
  ...
  if (OB_ISNULL(palf_env_impl)) {
    ...
    if (OB_FAIL(log_service_.remove_ls(
        ObLSID(palf_id), ls->log_handler_, ls->restore_handler_))) {
    ...
    CLOG_LOG(WARN, "remove palf handle impl failed", K(ret), K(palf_id));
  } else {
