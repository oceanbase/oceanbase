...
int ObSimpleLogClusterTestEnv::create_paxos_group(const int64_t id, const int64_t create_ts, int64_t &leader_idx, PalfHandleGuard &leader)
{
  int ret = OB_SUCCESS;
  PalfBaseInfo palf_base_info;
  palf_base_info.generate_by_default();
...
int ObSimpleLogClusterTestEnv::submit_log(PalfHandleGuard &leader, int count, int id)
{
  ...
  return submit_log_impl(leader, count, id, 0, lsn_array, log_ts_ns_array);
}
...
int ObSimpleLogClusterTestEnv::submit_log(PalfHandleGuard &leader, int count, int id, int data_len)
{
  std::vector<LSN> lsn_array;
  std::vector<int64_t> log_ts_ns_array;
  return submit_log_impl(leader, count, id, data_len, lsn_array, log_ts_ns_array);
}

int ObSimpleLogClusterTestEnv::submit_log(PalfHandleGuard &leader, int count, int id, std::vector<LSN> &lsn_array, std::vector<int64_t> &log_ts_ns_array)
{
  return submit_log_impl(leader, count, id, 0, lsn_array, log_ts_ns_array);
}

int ObSimpleLogClusterTestEnv::submit_log(PalfHandleGuard &leader, int count, int data_len, int id, std::vector<LSN> &lsn_array, std::vector<int64_t> &log_ts_ns_array)
{
  return submit_log_impl(leader, count, id, data_len, lsn_array, log_ts_ns_array);
}

int ObSimpleLogClusterTestEnv::submit_log(PalfHandleGuard &leader, LSN &lsn, int64_t &log_ts)
{
  const int64_t ref_ts = ObTimeUtility::current_time_ns();
  return submit_log(leader, ref_ts, lsn, log_ts);
}

int ObSimpleLogClusterTestEnv::submit_log(PalfHandleGuard &leader, const int64_t ref_ts, LSN &lsn, int64_t &log_ts)
{
  int ret = OB_SUCCESS;
  std::vector<LSN> lsn_array;
  std::vector<int64_t> log_ts_ns_array;
  if (OB_FAIL(submit_log_impl(leader, 1, 1, 0, ref_ts, lsn_array, log_ts_ns_array))) {
    //do nothing
  } else {
    lsn = lsn_array[0];
    log_ts = log_ts_ns_array[0];
  }
  return ret;
}
...
{
  const int64_t ref_ts = ObTimeUtility::current_time_ns();
  return submit_log_impl(leader, count, id, data_len, ref_ts, lsn_array, log_ts_ns_array);
}
  ...
  if (OB_FAIL(generate_data(buf, buf_len, real_log_data_size, data_len))) {
    ...
    for (int i  = 0; i < count && OB_SUCC(ret); i++) {
      do {
        ...
        LSN lsn;
        int64_t log_ts = 0;
        ret = leader.append(opts, buf, real_log_data_size, ref_ts, lsn, log_ts);
        if (OB_SUCC(ret) && lsn.is_valid()) {
          lsn_array.push_back(lsn);
          log_ts_ns_array.push_back(log_ts);
          usleep(200);
          PALF_LOG(INFO, "submit_log success", KR(ret), K(id), K(lsn), K(log_ts));
        } else {
...
int ObSimpleLogClusterTestEnv::change_access_mode_to_raw_write(PalfHandleGuard &leader)
{
  ...
  if (OB_FAIL(leader.get_access_mode(mode_version, curr_access_mode))) {
    ...
    PALF_LOG(WARN, "get_access_mode failed", K(ret));
  } else if (OB_FAIL(leader.change_access_mode(proposal_id, mode_version, new_access_mode, 0))) {
...
int ObSimpleLogClusterTestEnv::read_log(PalfHandleGuard &leader, const LSN &lsn)
{
  ...
  if (OB_FAIL(leader.seek(lsn, iterator))) {
    ...
    while (OB_SUCCESS == ret) {
      ...
      } else if (OB_FAIL(iterator.get_entry(buf, buf_len, ts, log_offset, is_raw_write))) {
    ...
    if (OB_ITER_END == ret) {
      ...
      if (OB_FAIL(leader_raw_write.seek(LSN(0), iterator_raw_write))) {
        ...
        while (OB_SUCCESS == ret) {
          ...
          } else if (OB_FAIL(iterator_raw_write.get_entry(buffer, nbytes, ts, lsn, is_raw_write))) {
