...
TEST_F(TestObSimpleLogApplyFunc, apply)
{
    ...
    {
      {
        ...
        ret = submit_log(leader, i, lsn, log_ts);
        if (OB_SUCC(ret)) {
          EXPECT_EQ(true, lsn.is_valid());
          cb_array[i]->__set_lsn(lsn);
          cb_array[i]->__set_ts_ns(log_ts);
          EXPECT_EQ(OB_SUCCESS, apply_status->push_append_cb(cb_array[i]));
        } else {
          break;
        }
      }
      CLOG_LOG(INFO, "submit log finish", K(i), K(lsn), K(log_ts));
    }
  } while (0);
  int64_t log_ts = 0;
  EXPECT_EQ(OB_SUCCESS, ap_sv.get_min_unapplied_log_ts_ns(ls_id, log_ts));
  ...
  do {
    ...
    for (int i = truncate_count; i < task_count; i++)
    {
      EXPECT_EQ(OB_SUCCESS, ap_sv.get_min_unapplied_log_ts_ns(ls_id, log_ts));
      {
        ...
        ls_adapter.critical_guard();
        CLOG_LOG(INFO, "submit log start", K(i));
        EXPECT_EQ(OB_SUCCESS, submit_log(leader, ref_ts, lsn, log_ts));
        EXPECT_EQ(true, lsn.is_valid());
        cb_array[i]->__set_lsn(lsn);
        cb_array[i]->__set_ts_ns(log_ts);
        palf_end_ts = log_ts;
  ...
  EXPECT_EQ(OB_SUCCESS, ap_sv.switch_to_follower(ls_id));
  EXPECT_EQ(OB_SUCCESS, ap_sv.get_min_unapplied_log_ts_ns(ls_id, log_ts));
  EXPECT_EQ(log_ts - 1, palf_end_ts);
