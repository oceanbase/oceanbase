...
TEST_F(TestObSimpleLogClusterRebuild, test_follower_rebuild)
{
  ...
  int64_t min_ts_ns;
  while (true) {
    EXPECT_EQ(OB_SUCCESS, leader.palf_handle_.palf_handle_impl_->get_min_block_info_for_gc(leader_min_block_id, min_ts_ns));
    if (1 == leader_min_block_id) {
      break;
    } else {
      sleep(2);
  ...
  PalfBaseInfo base_info_after_rebuild;
  // check block_id
  EXPECT_EQ(OB_SUCCESS, rebuild_server->palf_handle_.palf_handle_impl_->get_min_block_info_for_gc(follower_min_block_id, min_ts_ns));
