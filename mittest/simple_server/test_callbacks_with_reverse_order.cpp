    ...
    ObRowsInfo &rows_info)
{
  ...
  TRANS_LOG(INFO, "qqc debug", K(run_ctx.store_ctx_.mvcc_acc_ctx_.tx_id_));

  // if (check_exists && OB_FAIL(tablet_handle.get_obj()->rowkeys_exists(
  //     run_ctx.store_ctx_, table, rows_info, exists))) {
  //   TRANS_LOG(WARN, "fail to check the existence of rows", K(ret), K(rows_info), K(exists));
  // } else if (exists) {
  ...
  TRANS_LOG(INFO, "qqc debug3", K(run_ctx.store_ctx_.mvcc_acc_ctx_.tx_id_));
  ...
  if (OB_ERR_PRIMARY_KEY_DUPLICATE == ret && !run_ctx.dml_param_.is_ignore_) {
    ...
    if (table.is_index_table()) {
      ...
      if (OB_TMP_FAIL(table.get_index_name(index_name))) {
        TRANS_LOG(WARN, "get_index_name failed", K(tmp_ret));
      }
    }
