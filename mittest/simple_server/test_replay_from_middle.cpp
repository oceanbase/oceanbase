...
int64_t     SSTABLE_END_SCN = 0;
int64_t     REPLAY_BARRIER = 0;
int64_t     KEEP_ALIVE_SCN = 0;
int64_t     MAX_DECIDED_SCN_AFTER_COMMIT = 0;
ObLSID      LS_ID;
TestRunCtx  RunCtx;
...
void ObReplayFromMiddleTest::minor_freeze_once()
{
...

  {
    ...
    HEAP_VAR(ObMySQLProxy::MySQLResult, res)
    {
      ...
      ASSERT_EQ(OB_SUCCESS, result->get_int("end_log_scn", SSTABLE_END_SCN));
      fprintf(stdout,
              "get end_log_scn from mini sstable : %ld ls_id : %ld tenant_id : %ld\n",
              SSTABLE_END_SCN,
              LS_ID.id(),
              RunCtx.tenant_id_);
    }
...
void ObReplayFromMiddleTest::advance_checkpoint()
{
  ...
  MTL_SWITCH(RunCtx.tenant_id_)
  {
    ...
    int ret = ls->checkpoint_executor_.advance_checkpoint_by_flush(SSTABLE_END_SCN + 1);
    ASSERT_EQ(true, OB_SUCCESS == ret || OB_NO_NEED_UPDATE == ret);
    while (--retry_times > 0) {
      checkpoint = ls->get_clog_checkpoint_ts();
      if (checkpoint > SSTABLE_END_SCN) {
        break;
      } else {
        ::sleep(1);
        fprintf(stdout,
                "waiting advance checkpoint, checkpoint = %ld SSTABLE_END_SCN = %ld\n",
                checkpoint,
                SSTABLE_END_SCN);
      }
    }
    ASSERT_GT(ls->get_clog_checkpoint_ts(), SSTABLE_END_SCN);
    ...
    ASSERT_NE(0, min_start_scn);
    ASSERT_NE(0, keep_alive_scn);
    ASSERT_EQ(MinStartScnStatus::HAS_CTX, status);
    KEEP_ALIVE_SCN = keep_alive_scn;
    fprintf(
        stdout,
        "advance checkpoint done, checkpoint = %ld SSTABLE_END_SCN = %ld min_start_scn = %ld keep_alive_scn = %ld\n",
        checkpoint,
        SSTABLE_END_SCN,
        min_start_scn,
        keep_alive_scn);
...
void ObReplayFromMiddleTest::flush_tx_ctx_table()
{
  ...
  MTL_SWITCH(RunCtx.tenant_id_)
  {
    int64_t tx_ctx_rec_scn_before_flush = 0;
    int64_t tx_ctx_rec_scn_after_flush = 0;
    int64_t retry_times = 20;
    ObLS *ls = get_ls(RunCtx.tenant_id_, LS_ID);
    ASSERT_EQ(OB_SUCCESS, ls->ls_tx_svr_.mgr_->get_rec_log_ts(tx_ctx_rec_scn_before_flush));
    ASSERT_EQ(OB_SUCCESS, ls->ls_tx_svr_.flush_ls_inner_tablet(LS_TX_CTX_TABLET));
    while (--retry_times > 0) {
      ASSERT_EQ(OB_SUCCESS, ls->ls_tx_svr_.mgr_->get_rec_log_ts(tx_ctx_rec_scn_after_flush));
      if (tx_ctx_rec_scn_after_flush > tx_ctx_rec_scn_before_flush) {
        break;
      } else {
        fprintf(stdout,
                "wait flushing tx ctx table, tx_ctx_rec_scn_before_flusht = %ld tx_ctx_rec_scn_after_flush = %ld\n",
                tx_ctx_rec_scn_before_flush,
                tx_ctx_rec_scn_after_flush);
      }
    }
  }
}
...
{
  ObLS *ls = get_ls(RunCtx.tenant_id_, LS_ID);
  ASSERT_EQ(OB_SUCCESS, ls->get_max_decided_log_ts_ns(MAX_DECIDED_SCN_AFTER_COMMIT));
  ...
  fprintf(
      outfile, "%ld\n%ld\n%ld\n%ld\n%ld\n", RunCtx.tenant_id_, LS_ID.id(), SSTABLE_END_SCN, MAX_DECIDED_SCN_AFTER_COMMIT, KEEP_ALIVE_SCN);
  fclose(outfile);
  fprintf(stdout,
          "write communication file done, tenant_id = %ld ls_id = %ld sstable_end_scn = %ld max_decided_scn = %ld "
          "keep_alive_scn = %ld\n",
          RunCtx.tenant_id_,
          LS_ID.id(),
          SSTABLE_END_SCN,
          MAX_DECIDED_SCN_AFTER_COMMIT,
          KEEP_ALIVE_SCN);
}
...
public:
  ...
  void restart_test();
  void wait_replay(ObLS *ls, const int64_t expected_log_ts);
...
void ObReplayRestartTest::restart_test()
{
  ...
  MTL_SWITCH(RunCtx.tenant_id_)
  {
    ...
    ASSERT_EQ(OB_SUCCESS, ls->get_max_decided_log_ts_ns(max_decided_log_ts));
    ASSERT_NE(0, max_decided_log_ts);

    tx_data_table = ls->get_tx_table()->get_tx_data_table();

    {
      // 场景一： keep alive日志没有被回放，min_start_scn为初始值状态，跳过计算upper_trans_version
      ASSERT_EQ(0, tx_data_table->calc_upper_info_.min_start_scn_in_ctx_);
      upper_trans_version = 0;
      ASSERT_EQ(OB_SUCCESS,
                tx_data_table->get_upper_trans_version_before_given_log_ts(SSTABLE_END_SCN, upper_trans_version));
    }

    {
      // 场景2： keep alive日志被回放，min_start_scn小于end_scn，跳过计算upper_trans_version
      REPLAY_BARRIER = KEEP_ALIVE_SCN + 1;
      int64_t retry_times = 100;
      while (--retry_times > 0) {
        tx_data_table->update_calc_upper_info_(INT64_MAX /*max_decided_log_ts*/);
        if (0 != tx_data_table->calc_upper_info_.min_start_scn_in_ctx_) {
          break;
        } else {
          ::sleep(1);
      ...
      ASSERT_EQ(OB_SUCCESS,
                tx_data_table->get_upper_trans_version_before_given_log_ts(SSTABLE_END_SCN, upper_trans_version));
      ASSERT_GE(upper_trans_version, INT64_MAX);
    }

    {
      // 场景3： 所有日志回放结束，事务数据也发生冻结转储，上下文及事务数据表都已没有事务
      REPLAY_BARRIER = INT64_MAX;
      ...
      while (--retry_times > 0) {
        ...
        if (tx_data_table->calc_upper_info_.min_start_scn_in_ctx_ > SSTABLE_END_SCN) {
          ...
          ::sleep(1);
          fprintf(stdout,
                  "waiting all tx ctx clear, min_start_scn = %ld keep_alive_scn = %ld status = %d\n",
                  min_start_scn,
                  keep_alive_scn,
                  status);
        }
      ...
      while (--retry_times > 0) {
        ASSERT_EQ(OB_SUCCESS, ls->get_max_decided_log_ts_ns(max_decided_log_ts));
        if (max_decided_log_ts > tx_data_table->calc_upper_info_.keep_alive_scn_) {
          break;
        } else {
          ::sleep(1);
          fprintf(stdout, "waiting max decided log ts, max_decided_log_ts = %ld keep_alive_scn = %ld\n", max_decided_log_ts, tx_data_table->calc_upper_info_.keep_alive_scn_);
        }
      }
      ASSERT_GT(max_decided_log_ts, tx_data_table->calc_upper_info_.keep_alive_scn_);

      upper_trans_version = 0;
      ASSERT_EQ(OB_SUCCESS,
                tx_data_table->get_upper_trans_version_before_given_log_ts(SSTABLE_END_SCN, upper_trans_version));
      STORAGE_LOG(INFO, "finish restart test", K(upper_trans_version), K(SSTABLE_END_SCN), K(tx_data_table->calc_upper_info_));
      ASSERT_LT(upper_trans_version, INT64_MAX);
    }
  }
}

void ObReplayRestartTest::wait_replay(ObLS *ls, const int64_t expected_log_ts)
{
  ...
  while (true) {
    ...
    ASSERT_EQ(OB_SUCCESS, ls->get_max_decided_log_ts_ns(max_decided_log_ts));
    if (max_decided_log_ts >= expected_log_ts || (--retry_times < 0)) {
      break;
    } else {
      fprintf(stdout, "wait replaying...  max_decided_log_ts = %ld expected_log_ts = %ld\n", max_decided_log_ts, expected_log_ts);
      ::sleep(1);
    }
  }
  ASSERT_GE(max_decided_log_ts, expected_log_ts);
}

void ObReplayRestartTest::flush_tx_data(ObLS *ls)
{
  int64_t tx_ctx_rec_scn = 0;
  int64_t retry_times = 10;
...
int ObLSService::enable_replay()
{
  ...
  if (helper_ptr->is_restart_) {
    ...
    fscanf(infile, "%ld\n%ld\n%ld\n%ld\n%ld\n", &(RunCtx.tenant_id_), &int_ls_id, &SSTABLE_END_SCN, &MAX_DECIDED_SCN_AFTER_COMMIT, &KEEP_ALIVE_SCN);
    OB_ASSERT(RunCtx.tenant_id_ > 0);
    OB_ASSERT(SSTABLE_END_SCN > 0);
    OB_ASSERT(int_ls_id > 0);
    OB_ASSERT(MAX_DECIDED_SCN_AFTER_COMMIT > 0);
    REPLAY_BARRIER = SSTABLE_END_SCN + 1;
    LS_ID = int_ls_id;
  }
  /**************************** addtional code *****************************/
...
int ObLSAdapter::replay(ObLogReplayTask *replay_task)
{
  ...
  /**************************** addtional code *****************************/
  } else if (MTL_ID() == RunCtx.tenant_id_ && LS_ID == ls->get_ls_id() && replay_task->log_ts_ > REPLAY_BARRIER) {
    if (EXECUTE_COUNT_PER_SEC(1)) {
      fprintf(stdout,
              "stop replaying. tenant_id = %ld ls_id = %ld replay_log_ts = %ld replay_barrier = %ld\n",
              RunCtx.tenant_id_,
              LS_ID.id(),
              replay_task->log_ts_,
              REPLAY_BARRIER);
