...
TEST_F(TestLSStatusOperator, LSLifeAgent)
{
  ...
  ret = ls_life.create_new_ls(info, create_ts, zone_priority.str());
  ASSERT_EQ(OB_INVALID_ARGUMENT, ret);
  ...
  //设置日志流offline的参数检查
  ret = ls_life.set_ls_offline(tenant_id_, SYS_LS, share::OB_LS_DROPPING, -1);
  ASSERT_EQ(OB_INVALID_ARGUMENT, ret);
  ret = ls_life.set_ls_offline(tenant_id_, SYS_LS, share::OB_LS_NORMAL, 0);
  ASSERT_EQ(OB_INVALID_ARGUMENT, ret);

  //更新__all_ls_status表为空
  ret = ls_life.set_ls_offline(tenant_id_, ls_id, share::OB_LS_DROPPING, 0);
  ...
  ret = status_operator.update_ls_status(tenant_id_, ls_id, share::OB_LS_CREATING, share::OB_LS_NORMAL, share::NORMAL_SWITCHOVER_STATUS,
      get_curr_simple_server().get_observer().get_mysql_proxy());
  ASSERT_EQ(OB_SUCCESS, ret);
  ret = ls_life.set_ls_offline(tenant_id_, ls_id, share::OB_LS_NORMAL, 0);
  ASSERT_EQ(OB_INVALID_ARGUMENT, ret);
  ...
  ret = status_operator.update_ls_status(tenant_id_, ls_id, share::OB_LS_NORMAL, share::OB_LS_DROPPING, share::NORMAL_SWITCHOVER_STATUS,
      get_curr_simple_server().get_observer().get_mysql_proxy());
  ASSERT_EQ(OB_SUCCESS, ret);
  ret = ls_life.set_ls_offline(tenant_id_, ls_id, share::OB_LS_TENANT_DROPPING, 0);
  ASSERT_EQ(OB_NEED_RETRY, ret);
  ret = ls_life.set_ls_offline(tenant_id_, ls_id, share::OB_LS_DROPPING, 100);
  ASSERT_EQ(OB_SUCCESS, ret);
  ...
  ret = recovery_stat.init_only_recovery_stat(tenant_id_, ls_id, 99, 99);
  ASSERT_EQ(OB_SUCCESS, ret);
  ret = recovery_op.update_ls_recovery_stat(recovery_stat, get_curr_simple_server().get_observer().get_mysql_proxy());
  ASSERT_EQ(OB_SUCCESS, ret);
  ret = ls_life.drop_ls(tenant_id_, ls_id);
  ASSERT_EQ(OB_NEED_RETRY, ret);
  ret = recovery_stat.init_only_recovery_stat(tenant_id_, ls_id, 98, 100);
  ASSERT_EQ(OB_SUCCESS, ret);
  //recovery_stat的取最大值
  ret = recovery_op.update_ls_recovery_stat(recovery_stat, get_curr_simple_server().get_observer().get_mysql_proxy());
  ASSERT_EQ(OB_SUCCESS, ret);
  ...
  ret = ls_life.drop_ls(tenant_id_, ls_id);
  ASSERT_EQ(OB_NEED_RETRY, ret);
  ret = recovery_stat.init_only_recovery_stat(tenant_id_, ls_id, 100, 100);
  ASSERT_EQ(OB_SUCCESS, ret);
  ret = recovery_op.update_ls_recovery_stat(recovery_stat, get_curr_simple_server().get_observer().get_mysql_proxy());
  ASSERT_EQ(OB_SUCCESS, ret);
  ret = ls_life.drop_ls(tenant_id_, ls_id);
