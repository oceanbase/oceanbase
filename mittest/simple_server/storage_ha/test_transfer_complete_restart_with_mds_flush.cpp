...
static const char *BORN_CASE_NAME = "TestTransferHandlerCompleteWFlush";
static const char *RESTART_CASE_NAME = "TestTransferRestartCompleteWFlush";
static ObTransferTask g_task;

class TestTransferHandlerCompleteWFlush : public unittest::ObSimpleClusterTestBase
{
public:
  TestTransferHandlerCompleteWFlush() : unittest::ObSimpleClusterTestBase(TEST_FILE_NAME) {}
...
int TestTransferHandlerCompleteWFlush::read_sql(
...
int TestTransferHandlerCompleteWFlush::gen_mock_data(const ObTransferTaskID task_id, const ObTransferStatus &status, ObTransferTask &task)
{
  int ret = OB_SUCCESS;
  ObLSID src_ls(1001);
  ObLSID dest_ls(1002);
...
class TestTransferRestartCompleteWFlush : public ObSimpleClusterTestBase
{
public:
  TestTransferRestartCompleteWFlush() : ObSimpleClusterTestBase(TEST_FILE_NAME) {}
  void select_existed_data();
};

void TestTransferRestartCompleteWFlush::select_existed_data()
{
  sleep(10);
...
TEST_F(TestTransferHandlerCompleteWFlush, prepare_valid_data)
{
...
TEST_F(TestTransferHandlerCompleteWFlush, test_transfer_1001_to_1002_with_transfer_finish_without_mds_flush)
{
  int ret = OB_SUCCESS;
...
TEST_F(TestTransferRestartCompleteWFlush, observer_restart_when_transfer_finish_without_flush)
{
