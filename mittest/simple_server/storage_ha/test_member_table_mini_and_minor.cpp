...
static const char *TEST_FILE_NAME = "member_table_mini_and_minor";
    ...
    const int64_t sstable_count)
{
  ...
  if (OB_ISNULL(ls_svr = MTL(ObLSService *))) {
    ...
    while (OB_SUCC(ret)) {
      ...
      ObTabletID tablet_id(ObTabletID::LS_MEMBER_TABLET_ID);
...
int TestMemberTable::check_member_table_recycle_scn()
{
  ...
  ObMemberTableService *table_service = nullptr;
  ...
  if (OB_ISNULL(ls_svr = MTL(ObLSService *))) {
    ...
    ret = OB_ERR_UNEXPECTED;
    LOG_WARN("ls should not be NULL", K(ret), KP(ls), K(ls_id));
  } else if (OB_ISNULL(member_table = ls->get_member_table())) {
    ret = OB_ERR_UNEXPECTED;
    LOG_WARN("member table should not be NULL", K(ret), KPC(ls));
  } else if (OB_ISNULL(table_service = MTL(ObMemberTableService *))) {
    ret = OB_ERR_UNEXPECTED;
    LOG_WARN("ls service should not be NULL", K(ret));
  } else {
    ...
    SCN can_recycle_scn;
    while (OB_SUCC(ret)) {
      if (OB_FAIL(member_table->get_can_recycle_scn(can_recycle_scn))) {
    ...
    if (OB_SUCC(ret)) {
      if (OB_FAIL(member_table->get_can_recycle_scn(g_recycle_scn))) {
...
int TestMemberTable::generate_member_table_data()
{
  ...
  ObArray<ObMemberTableData> member_table_data;
  ...
  for (int64_t i = g_index; OB_SUCC(ret) && i < g_index + 100; i++) {
    ...
    if (OB_FAIL(ObMemberTableDataGenerator::gen_transfer_member_data(tablet_id, reorganization_scn, transfer_out_status,
        dest_ls_id, transfer_out_transfer_seq, transfer_out_scn, src_reorgainzation_scn, member_table_data))) {
      LOG_WARN("failed to gen member data", K(ret), K(tablet_id));
    }
  }
  ...
  for (int64_t i = g_index; OB_SUCC(ret) && i < g_index + 100; i++) {
    ...
    if (OB_FAIL(ObMemberTableDataGenerator::gen_transfer_member_data(tablet_id, reorganization_scn, transfer_in_status,
  ...
  ObMemberTableWriteOperator write_op;
  if (OB_FAIL(ret)) {
...
int TestMemberTable::check_member_table_data_recycle()
{
  int ret = OB_SUCCESS;
  ObLSID ls_id(1);
  ObMemberTableReadOperator read_op;
