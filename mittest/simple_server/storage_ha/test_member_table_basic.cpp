...
static const char *TEST_FILE_NAME = "member_table_basic";
...
TEST_F(TestMemberTable, insert_with_trans)
{
  ...
  ret = ObMemberTableDataGenerator::gen_transfer_member_data(tablet_id, reorganization_scn, tablet_status,
      relative_ls_id, transfer_seq, transfer_scn, src_reorgainzation_scn, member_table_data);
  ASSERT_EQ(OB_SUCCESS, ret);

  //insert
  ObLSID ls_id(1);
  ObMySQLTransaction trans1;
  ObMemberTableWriteOperator write_op1;
  ASSERT_EQ(OB_SUCCESS, trans1.start(&inner_sql_proxy, g_tenant_id));
  ret = write_op1.init(trans1, g_tenant_id, ls_id);
  ASSERT_EQ(OB_SUCCESS, ret);
  ret = write_op1.insert_rows(member_table_data);
  ASSERT_EQ(OB_SUCCESS, ret);
  ...
  const ObMemberTableData &old_data = member_table_data.at(0);
  ObMemberTableData get_data;
...
TEST_F(TestMemberTable, insert_without_trans)
{
  ...
  ASSERT_EQ(OB_SUCCESS, ret);
  ret = write_op.insert_rows(member_table_data);
