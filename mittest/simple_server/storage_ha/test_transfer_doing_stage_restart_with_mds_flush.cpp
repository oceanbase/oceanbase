...
static const char *BORN_CASE_NAME = "TestTransferHandlerDoingWFlush";
static const char *RESTART_CASE_NAME = "TestTransferRestartDoingWFlush";

class TestTransferHandlerDoingWFlush : public unittest::ObSimpleClusterTestBase
{
public:
  TestTransferHandlerDoingWFlush() : unittest::ObSimpleClusterTestBase(TEST_FILE_NAME) {}
...
int TestTransferHandlerDoingWFlush::read_sql(
...
int TestTransferHandlerDoingWFlush::gen_mock_data(const ObTransferTaskID task_id, const ObTransferStatus &status, ObTransferTask &task)
{
  int ret = OB_SUCCESS;
  ObLSID src_ls(1001);
  ObLSID dest_ls(1002);
...
class TestTransferRestartDoingWFlush : public ObSimpleClusterTestBase
{
public:
  TestTransferRestartDoingWFlush() : ObSimpleClusterTestBase(TEST_FILE_NAME) {}
  void select_existed_data();
};

void TestTransferRestartDoingWFlush::select_existed_data()
{
  sleep(10);
...
TEST_F(TestTransferHandlerDoingWFlush, prepare_valid_data)
{
...
TEST_F(TestTransferHandlerDoingWFlush, test_transfer_1001_to_1_with_transfer_start_with_mds_flush)
{
  int ret = OB_SUCCESS;
...
TEST_F(TestTransferRestartDoingWFlush, observer_restart_when_transfer_start_without_flush)
{
