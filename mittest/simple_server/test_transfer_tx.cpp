...
int ObTransferTx::do_transfer_start_abort(uint64_t tenant_id, ObLSID dest_ls_id, ObLSID src_ls_id, ObTransferTabletInfo tablet_info)
{
  ...
  MTL_SWITCH(tenant_id) {
    ...
    } else if (OB_FAIL(transfer_handler->get_config_version_(config_version))) {
    ...
    } else if (OB_FAIL(transfer_handler->do_trans_transfer_start_v2_(task_info, config_version, SCN::min_scn(), timeout_ctx, trans))) {
