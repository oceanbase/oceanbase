...
void ObTxDataTableTest::check_start_tx_scn(ObTxDataTable *tx_data_table)
{
  ...
  int64_t pre_start_tx_scn = -1;
  while (!ATOMIC_LOAD(&stop)) {
    int64_t start_tx_scn = 0;
    ...
    ASSERT_EQ(OB_SUCCESS, tx_data_table->get_recycle_ts(recycle_ts));
    ASSERT_LE(start_tx_scn, recycle_ts);
    pre_start_tx_scn = start_tx_scn;
    OB_LOG(INFO, "check start tx scn : ", K(start_tx_scn), K(recycle_ts));
...
void ObTxDataTableTest::check_tx_data_minor_succeed()
{
  ...
  ASSERT_EQ(OB_SUCCESS,
      sql.assign("select count(*) as row_cnt from oceanbase.__all_virtual_tablet_compaction_history where "
                 "tenant_id=1002 and ls_id=1001 and (type='MINI_MINOR_MERGE' or type = 'MINOR_MERGE');"));
...
void ObTxDataTableTest::basic_test()
{
  ...
  ls = ls_handle.get_ls();
  int64_t clog_checkpoint_before_freeze = ls->get_ls_meta().get_clog_checkpoint_ts();
...

  {
    ...
    int ret = ls->get_checkpoint_executor()->advance_checkpoint_by_flush(first_rollback_ts_);
  ...
  while (--wait_update_count > 0) {
    ...
    clog_checkpoint_after_freeze = ls->get_ls_meta().get_clog_checkpoint_ts();
