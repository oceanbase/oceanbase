...
void ObTabletFlushTest::prepare_ddl_lock_table_data(ObLS *&ls)
{
  ...
  ASSERT_EQ(OB_SUCCESS, ls->ls_freezer_.get_max_consequent_callbacked_log_ts(max_decided_log_ts));
  ...
  ASSERT_EQ(INT64_MAX, lock_memtable->get_rec_log_ts());
  ObTxIDSet unused;
  ...
  ASSERT_EQ(OB_SUCCESS, lock_memtable->update_lock_status(table_lock_op2,
                                                          max_decided_log_ts - 1,
                                                          max_decided_log_ts - 1,
                                                          LOCK_OP_COMPLETE));
  ASSERT_EQ(max_decided_log_ts - 2, lock_memtable->get_rec_log_ts());
}
...
TEST_F(ObTabletFlushTest, test_special_tablet_flush)
{
  ...
  ASSERT_NE(lock_memtable->get_rec_log_ts(), INT64_MAX);
  ASSERT_NE(tx_ctx_memtable->get_rec_log_ts(), INT64_MAX);
  ASSERT_NE(tx_data_mgr->get_rec_log_ts(), INT64_MAX);
  ...
  ASSERT_EQ(lock_memtable->get_rec_log_ts(), INT64_MAX);
  ASSERT_EQ(tx_ctx_memtable->get_rec_log_ts(), INT64_MAX);
  ASSERT_EQ(tx_data_mgr->get_rec_log_ts(), INT64_MAX);
}
