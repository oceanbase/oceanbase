...
public:
  ...
  virtual int get_server_version(const uint64_t tenant_id, palf::SCN &version) const = 0;

  /// get CLUSTER level wrs version
  ///
  /// @param [in]  tenant_id   target tenant ID
  /// @param [out] version     wrs version
  virtual int get_cluster_version(const uint64_t tenant_id, palf::SCN &version) = 0;
