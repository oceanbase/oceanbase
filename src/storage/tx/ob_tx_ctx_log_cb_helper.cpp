...
void ObPartTransCtx::reset_log_cbs_()
{
  ...
  if (!extra_cb_group_list_.is_empty()) {
    TRANS_LOG_RET(INFO, tmp_ret, "Prepare to free extra log cb group", K(ret), K(trans_id_),
                  K(ls_id_), K(extra_cb_group_list_.get_size()));
  }
#endif
