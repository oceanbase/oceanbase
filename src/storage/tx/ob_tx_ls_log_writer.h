...
public:
  int submit_start_working_log(const int64_t &leader_epoch, palf::SCN &log_ts);
                                    ...
                                    palf::SCN &log_ts)
{
  ...
  } else if (OB_FAIL(tx_log_adapter_->submit_log(cb->get_log_buf(), cb->get_log_pos(), palf::SCN::min_scn(), cb, need_nonblock))) {
    return_log_cb_(cb);
    cb = nullptr;
