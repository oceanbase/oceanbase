...
{
public:
  CtxLock() : ctx_lock_(), access_lock_(), flush_redo_lock_(), ctx_(NULL), lock_start_ts_(0), waiting_lock_cnt_(0) {}
  ~CtxLock() {}
...
public:
  ...
  {
    request_ts_ = ObClockGenerator::getClock();
    if (mode_ & ACCESS) {
      if (need_lock) {
        lock_->wrlock_access();
      }
    ...
    hold_ts_ = ObClockGenerator::getClock();
  }
