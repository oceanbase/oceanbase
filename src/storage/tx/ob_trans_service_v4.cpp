                                              ...
                                              int64_t &uncertain_bound)
{
  ...
  } else if (OB_FAIL(ts_mgr_->get_gts_sync(tenant_id_, request_time, timeout_us, snapshot, receive_gts_ts))) {
                                      ...
                                      SCN &trans_version)
{
  ...
  if (OB_SUCC(get_tx_ctx_for_standby_(ls_id, tx_id, ctx))) {
    ...
    if (GET_MIN_CLUSTER_VERSION() >=  CLUSTER_VERSION_4_2_5_6 /*TODO : fix with release version*/
