...
{
  int ret = OB_SUCCESS;
  memtable::ObTxFillRedoCtx ctx(helper_->callbacks_);
  ...
  while (OB_SUCC(ret) && !stop) {
    if (submit_if_not_full_ && OB_FAIL(prepare_())) {
      ...
      if (OB_UNLIKELY(ctx.fill_count_ <= 0)) {
      ...
      if (ctx.fill_count_ > 0 && !skip_submit) {
        const int64_t replay_hint = tx_id_.get_id() + (is_parallel_logging ? ctx.list_idx_ : 0);
...
int ObTxRedoSubmitter::submit_log_block_out_(const int64_t replay_hint, bool &submitted)
{
  int ret = OB_SUCCESS;
  submitted = false;
  if (OB_UNLIKELY(helper_->callbacks_.count() == 0)) {
    ret = OB_ERR_UNEXPECTED;
    TRANS_LOG(WARN, "empty callbacks", K(ret));
  } else if (OB_NOT_NULL(log_cb_) && OB_FAIL(log_cb_->reserve_callbacks(helper_->callbacks_.count()))) {
    ...
    helper_->callback_redo_submitted_ = !parallel_logging;
    ret = tx_ctx_.submit_redo_log_out(*log_block_, log_cb_, *helper_, replay_hint, parallel_logging, submitted_scn);
    if (OB_SUCC(ret) && parallel_logging) {
      ret = after_submit_redo_out_();
    }
    if (OB_NOT_NULL(log_cb_)) {
      int tmp_ret = OB_ERR_UNEXPECTED;
      TRANS_LOG(ERROR, "log_cb should be null", K(tmp_ret), KPC(this));
#ifdef ENABLE_DEBUG_LOG
      ob_abort();
#endif
    }
...
int ObTxRedoSubmitter::fill_log_block_(memtable::ObTxFillRedoCtx &ctx)
{
  ...
  do {
    ...
    if (OB_FAIL(log_block_->prepare_mutator_buf(log))) {
      TRANS_LOG(WARN, "prepare mutator buf fail", K(ret));
    } else {
      ...
      ctx.callback_scopes_.reset();
      ctx.max_seq_no_.reset();
      ctx.data_size_ = 0;
      int64_t start_ts = common::ObClockGenerator::getClock();
      ret = mt_ctx_.fill_redo_log(ctx);
      ctx.fill_time_ = common::ObClockGenerator::getClock() - start_ts;
      helper_->data_size_ = ctx.data_size_;
      helper_->max_seq_no_ = ctx.max_seq_no_;
      int save_ret = ret;
      int64_t real_buf_pos = ctx.fill_count_ > 0 ? ctx.buf_pos_ : 0;
      if (OB_FAIL(log_block_->finish_mutator_buf(log, real_buf_pos))) {
        TRANS_LOG(WARN, "finish mutator buf fail", K(ret));
        ...
        TRANS_LOG(WARN, "add new log fail", K(ret));
      } else {
        ret = save_ret;
      }
    }
  } while(OB_UNLIKELY(need_retry));
