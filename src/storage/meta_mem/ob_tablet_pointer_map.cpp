    ...
    bool &is_in_memory)
{
  ...
  if (OB_FAIL(ResourceMap::hash_func_(key, hash_val))) {
    ...
    common::ObBucketHashRLockGuard lock_guard(ResourceMap::bucket_lock_, hash_val);
    ret = try_get_in_memory_meta_obj_without_lock(key, ptr_hdl, guard, is_in_memory);
  }
  return ret;
}

int ObTabletPointerMap::try_get_in_memory_meta_obj_without_lock(
    const ObTabletMapKey &key,
    ObTabletPointerHandle &ptr_hdl,
    ObMetaObjGuard<ObTablet> &guard,
    bool &is_in_memory)
    ...
    ObITabletFilterOp *op)
{
  ...
  if (OB_SUCC(ret)) {
    if ((nullptr == op && CLICK_FAIL(try_get_in_memory_meta_obj(key, ptr_hdl, tmp_guard, is_in_memory)))
        || (nullptr != op && CLICK_FAIL(try_get_in_memory_meta_obj_with_filter(key, *op, ptr_hdl, tmp_guard, is_in_memory)))) {
      if (OB_ENTRY_NOT_EXIST == ret) {
        STORAGE_LOG(DEBUG, "meta obj does not exist", K(ret), K(key));
      } else {
        STORAGE_LOG(WARN, "fail to try get in memory meta obj", K(ret), K(key));
    ...
    if (OB_ISNULL(buf = allocator.alloc(sizeof(ObTablet)))) {
      ...
      do {
        ...
        if (CLICK_FAIL(load_meta_obj(key, t_ptr, allocator, disk_addr, t))) {
          ...
          if (CLICK_FAIL(try_get_in_memory_meta_obj_without_lock(key, tmp_ptr_hdl, tmp_guard, is_in_memory))) {
            STORAGE_LOG(WARN, "fail to try get in memory meta obj", K(ret), K(key));
          } else if (is_in_memory) {
            tmp_guard.set_wash_priority(WashTabletPriority::WTP_LOW);
          }
