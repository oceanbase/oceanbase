...
ObTabletPointer::ObTabletPointer()
  : ObTabletBasePointer(),
    ...
    ddl_info_(),
    flying_(false),
    ddl_kv_mgr_lock_(common::ObLatchIds::TABLET_POINTER_LOCK),
    mds_table_handler_(),
...
int ObTabletPointer::dump_meta_obj(ObTabletHandle &guard, void *&free_obj)
{
  ...
  if (OB_ISNULL(obj_.ptr_)) {
    ...
    LOG_WARN("failed to get min data version", K(ret), K(min_data_version));
  } else {
    ...
    const ObTabletPersisterParam param(min_data_version, ls_id, ls_handle_.get_ls()->get_ls_epoch(), tablet_id, obj_.ptr_->get_transfer_seq(), 0);
