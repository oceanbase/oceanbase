...
int ObAdvanceSrcLSCheckpointP::process()
{
  ...
  MTL_SWITCH(arg_.tenant_id_) {
    ...
    if (OB_ISNULL(ls_service = MTL(ObLSService *))) {
      ...
      LOG_WARN("failed to update the requested ls checkpoint", K(ret), K(arg_));
    }
  }
    ...
    const share::SCN &recycle_scn)
{
  ...
  if (!is_inited_) {
    ret = OB_NOT_INIT;
    STORAGE_LOG(WARN, "storage rpc is not inited", K(ret));
  } else if (OB_INVALID_ID == tenant_id || !src_info.is_valid()
      || !ls_id.is_valid() || !recycle_scn.is_valid()) {
    ret = OB_INVALID_ARGUMENT;
    STORAGE_LOG(WARN, "invalid argument", K(ret), K(tenant_id), K(src_info), K(ls_id), K(recycle_scn));
  } else if (OB_FAIL(rpc_proxy_->to(src_info.src_addr_).dst_cluster_id(src_info.cluster_id_)
      .by(tenant_id)
      .group_id(share::OBCG_STORAGE)
