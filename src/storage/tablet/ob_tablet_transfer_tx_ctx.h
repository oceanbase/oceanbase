...
public:
  ...
  TO_STRING_KV(K_(tx_id), K_(epoch), K_(session_id), K_(associated_session_id),
               K_(tx_state), K_(trans_version), K_(prepare_version),
               K_(cluster_id), K_(cluster_version), K_(scheduler),
               K_(tx_expired_time), K_(xid), K_(last_seq_no), K_(commit_version),
               K_(max_submitted_seq_no), K_(tx_start_scn), K_(tx_end_scn),
               K_(is_sub2pc), K_(happened_before), K_(table_lock_info));
};
