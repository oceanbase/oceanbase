...
OB_SERIALIZE_MEMBER(CollectTxCtxInfo, src_ls_id_, dest_ls_id_, task_id_,
                    transfer_epoch_, transfer_scn_, args_);
OB_SERIALIZE_MEMBER(ObTxCtxMoveArg, tx_id_, epoch_, session_id_, tx_state_,
                    trans_version_, prepare_version_, commit_version_,
                    cluster_id_, cluster_version_, scheduler_, tx_expired_time_,
                    xid_, last_seq_no_, max_submitted_seq_no_, tx_start_scn_,
                    tx_end_scn_, is_sub2pc_, happened_before_, table_lock_info_,
                    associated_session_id_);
OB_SERIALIZE_MEMBER(ObTransferDestPrepareInfo, task_id_, src_ls_id_, dest_ls_id_);
OB_SERIALIZE_MEMBER(ObTransferMoveTxParam, src_ls_id_, transfer_epoch_, transfer_scn_,
                    op_scn_, op_type_, is_replay_, is_incomplete_replay_);
...
void ObTransferOutTxCtx::on_redo(const share::SCN &redo_scn)
{
  ...
  while (true) {
    ...
    } else if (OB_FAIL(MTL(ObLSService*)->get_ls(src_ls_id_, ls_handle, ObLSGetMod::STORAGE_MOD, ObLSAccessAttr::DISABLE_LOGONLY))) {
...
int ObStartTransferMoveTxHelper::on_register(const char* buf, const int64_t len, mds::BufferCtx &ctx)
{
  ...
  } else if (OB_FAIL(MTL(ObLSService*)->get_ls(collect_tx_info.dest_ls_id_, ls_handle, ObLSGetMod::STORAGE_MOD, ObLSAccessAttr::DISABLE_LOGONLY))) {
      ...
      mds::BufferCtx &ctx)
{
  ...
  } else if (OB_FAIL(MTL(ObLSService*)->get_ls(info.dest_ls_id_, ls_handle, ObLSGetMod::STORAGE_MOD, ObLSAccessAttr::DISABLE_LOGONLY))) {
...
void ObTransferDestPrepareTxCtx::on_abort(const share::SCN &abort_scn)
{
  ...
  while (true) {
    ...
    if (!info.is_valid()) {
      ret = OB_ERR_UNEXPECTED;
      LOG_WARN("transfer dest prepare info is invalid", KR(ret), K(tx_id), KP(this), KPC(this));
    } else if (OB_FAIL(MTL(ObLSService*)->get_ls(transfer_dest_prepare_info_.dest_ls_id_, ls_handle, ObLSGetMod::STORAGE_MOD, ObLSAccessAttr::DISABLE_LOGONLY))) {
