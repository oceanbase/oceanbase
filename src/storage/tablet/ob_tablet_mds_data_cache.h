...
public:
  ObMdsInfoCache()
    : newest_commit_version_(INT64_MAX),
      ...
      replay_seq_(0)
  {}
  ~ObMdsInfoCache() { reset(); }
  bool is_valid() const
  {
    return INT64_MAX != newest_commit_version_ && cnt_ >= 0;
  }
  ...
  void replay_mds_info()
  {
    newest_commit_version_ = INT64_MAX;
    cnt_ = 0;
    replay_seq_++;
  }
  void set_empty() { set_value(0, 0); }
  void set_value(
  ...
  TO_STRING_KV(K_(newest_commit_version), K_(cnt), K_(replay_seq));
private:
...
using ObTruncateInfoCache = ObMdsInfoCache;
using ObTTLFilterInfoCache = ObMdsInfoCache;
