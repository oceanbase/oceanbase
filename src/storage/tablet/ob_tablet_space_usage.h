...
public:
  ...
  TO_STRING_KV(K_(all_sstable_occupy_size), K_(all_sstable_required_size), K_(all_sstable_meta_size), K_(ss_public_sstable_occupy_size), K_(tablet_clustered_meta_size), K_(tablet_clustered_sstable_data_size), K_(backup_bytes));
  int serialize(char *buf, const int64_t buf_len, int64_t &pos) const;
  int deserialize(const char *buf, const int64_t data_len, int64_t &pos);
  int32_t get_serialize_size() const;
  bool is_valid() const
  {
    return (OB_INVALID_SIZE != all_sstable_occupy_size_)
        && (OB_INVALID_SIZE != all_sstable_required_size_)
...
public:
  // all sstable occupy_size, include data blocks and meta blocks
  int64_t all_sstable_occupy_size_; // compat : occupy_bytes_
  // all sstable required_size, (data_block_count + meta_block_count) * 2MB
  int64_t all_sstable_required_size_; // compat : data_size_
  // deprecated, old: sstable_meta_block_count * 2MB, now: 0
