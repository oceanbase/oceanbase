...
public:
  ...
  int init(
      ...
      transaction::ObTxDataSourceType mds_op_type);
protected:
...
int ObTabletStartTransferOutReplayExecutor::do_replay_(ObTabletHandle &tablet_handle)
{
  ...
  if (!is_inited_) {
    ...
    if (mds_op_type_ == transaction::ObTxDataSourceType::START_TRANSFER_OUT_PREPARE) {
    ...
    ObTabletHandle &tablet_handle)
{
  ...
  } else if (mds_op_type_ == transaction::ObTxDataSourceType::START_TRANSFER_OUT && (
  ...
  } else if (mds_op_type_ == transaction::ObTxDataSourceType::START_TRANSFER_OUT_PREPARE && (
        ObTabletStatus::NORMAL != user_data.tablet_status_ ||
        !is_committed)) {
    ret = OB_ERR_UNEXPECTED;
    LOG_WARN("tablet status is unexpected", K(ret), KPC(tablet), K(tablet_info_), K(user_data));
  ...
  // } else if (mds_op_type_ == transaction::ObTxDataSourceType::START_TRANSFER_OUT_V2 && (
    ...
    mds::BufferCtx &ctx)
{
  ...
  transaction::ObTxDataSourceType mds_op_type = transaction::ObTxDataSourceType::START_TRANSFER_OUT;
  ...
  } else if (CLICK_FAIL(ls_service->get_ls(tx_start_transfer_out_info.src_ls_id_, ls_handle, ObLSGetMod::STORAGE_MOD, ObLSAccessAttr::DISABLE_LOGONLY))) {
    ...
    mds::BufferCtx &ctx)
{
  ...
  if (!tablet_info.is_valid() || OB_ISNULL(ls)) {
    ...
    LOG_WARN("check src transfer tablets get invalid argument", K(ret), K(tablet_info), KP(ls));
             ...
             mds_op_type_ != transaction::ObTxDataSourceType::START_TRANSFER_OUT_PREPARE &&
             mds_op_type_ != transaction::ObTxDataSourceType::START_TRANSFER_OUT_V2) {
    ret = OB_ERR_UNEXPECTED;
    ...
    LOG_WARN("failed to get tx data", K(ret), KPC(tablet), K(tablet_info));
  } else if ((mds_op_type_ == transaction::ObTxDataSourceType::START_TRANSFER_OUT || mds_op_type_ == transaction::ObTxDataSourceType::START_TRANSFER_OUT_PREPARE) && (
        ObTabletStatus::NORMAL != user_data.tablet_status_ ||
...
int ObTabletStartTransferOutCommonHelper::set_transfer_tablets_freeze_flag_(const ObTXStartTransferOutInfo &tx_start_transfer_out_info)
{
  ...
  } else if (OB_FAIL(ls_service->get_ls(tx_start_transfer_out_info.src_ls_id_, ls_handle, ObLSGetMod::HA_MOD, ObLSAccessAttr::DISABLE_LOGONLY))) {
    ...
    mds::BufferCtx &ctx)
{
  ...
  } else if (mds_op_type_ != transaction::ObTxDataSourceType::START_TRANSFER_OUT_PREPARE && CLICK_FAIL(try_enable_dest_ls_clog_replay(scn, tx_start_transfer_out_info.dest_ls_id_))) {
    LOG_WARN("failed to try enable dest ls clog replay", K(ret), K(scn), K(tx_start_transfer_out_info));
  } else if (mds_op_type_ != transaction::ObTxDataSourceType::START_TRANSFER_OUT_PREPARE && CLICK_FAIL(set_transfer_tablets_freeze_flag_(tx_start_transfer_out_info))) {
    ...
    mds::BufferCtx &ctx)
{
  ...
  ObLSHandle ls_handle;
  ObLS *ls = nullptr;
  transaction::ObTxDataSourceType mds_op_type = transaction::ObTxDataSourceType::START_TRANSFER_OUT_PREPARE;
  ObTabletStartTransferOutCommonHelper transfer_out_helper(mds_op_type);
  ...
  if (OB_ISNULL(buf) || len < 0) {
    ...
    ret = OB_ERR_UNEXPECTED;
    LOG_WARN("tx start transfer out info is unexpected", K(ret), K(tx_start_transfer_out_info));
  } else if (CLICK_FAIL(MTL(ObLSService *)->get_ls(tx_start_transfer_out_info.src_ls_id_, ls_handle, ObLSGetMod::STORAGE_MOD, ObLSAccessAttr::DISABLE_LOGONLY))) {
    LOG_WARN("fail to get ls", KR(ret), K(tx_start_transfer_out_info));
  } else if (OB_UNLIKELY(nullptr == (ls = ls_handle.get_ls()))) {
    ret = OB_ERR_UNEXPECTED;
    LOG_WARN("ls should not be NULL", KR(ret), K(tx_start_transfer_out_info), KP(ls));
    ...
    mds::BufferCtx &ctx)
{
  ...
  transaction::ObTxDataSourceType mds_op_type = transaction::ObTxDataSourceType::START_TRANSFER_OUT_V2;
  ObTabletStartTransferOutCommonHelper transfer_out_helper(mds_op_type);
  ...
  if (OB_ISNULL(buf) || len < 0) {
  ...
  } else if (OB_FAIL(MTL(ObLSService*)->get_ls(info.src_ls_id_, ls_handle, ObLSGetMod::STORAGE_MOD, ObLSAccessAttr::DISABLE_LOGONLY))) {
    ...
    param.op_type_ = transaction::NotifyType::REGISTER_SUCC;
    ...
    const bool for_replay)
{
  ...
  if (!tx_start_transfer_in_info.is_valid()) {
    ...
    ret = OB_ERR_UNEXPECTED;
    LOG_WARN("ls service should not be null", K(ret), KP(ls_service));
  } else if (CLICK_FAIL(ls_service->get_ls(tx_start_transfer_in_info.dest_ls_id_, ls_handle, ObLSGetMod::STORAGE_MOD, ObLSAccessAttr::DISABLE_LOGONLY))) {
    ...
    bool &skip_replay)
{
  ...
  if (!scn.is_valid() || !tx_start_transfer_in_info.is_valid()) {
    ...
    ret = OB_ERR_UNEXPECTED;
    LOG_WARN("ls service should not be null", K(ret), KP(ls_service));
  } else if (OB_FAIL(ls_service->get_ls(tx_start_transfer_in_info.dest_ls_id_, ls_handle, ObLSGetMod::STORAGE_MOD, ObLSAccessAttr::DISABLE_LOGONLY))) {
    LOG_WARN("fail to get ls", KR(ret), K(tx_start_transfer_in_info));
  } else if (OB_UNLIKELY(nullptr == (ls = ls_handle.get_ls()))) {
    ret = OB_ERR_UNEXPECTED;
    LOG_WARN("ls should not be NULL", KR(ret), K(tx_start_transfer_in_info), KP(ls));
    ...
    const bool check_src_tablet_status)
{
  ...
  if ((!scn.is_valid() && for_replay) || !tx_start_transfer_in_info.is_valid()) {
    ...
    ret = OB_ERR_UNEXPECTED;
    LOG_ERROR("ls srv should not be NULL", K(ret), KP(ls_srv));
  } else if (CLICK_FAIL(ls_srv->get_ls(tx_start_transfer_in_info.src_ls_id_, src_ls_handle, ObLSGetMod::STORAGE_MOD, ObLSAccessAttr::DISABLE_LOGONLY))) {
    ...
    mds::BufferCtx &ctx)
{
  ...
  if ((!scn.is_valid() && for_replay) || !tx_start_transfer_in_info.is_valid()) {
    ...
    ret = OB_ERR_UNEXPECTED;
    LOG_ERROR("ls srv should not be NULL", K(ret), KP(ls_srv));
  } else if (CLICK_FAIL(ls_srv->get_ls(tx_start_transfer_in_info.dest_ls_id_, dest_ls_handle, ObLSGetMod::STORAGE_MOD, ObLSAccessAttr::DISABLE_LOGONLY))) {
    ...
    mds::BufferCtx &ctx)
{
  ...
  if (OB_ISNULL(buf) || len < 0 || !scn.is_valid()) {
    ...
    ret = OB_ERR_UNEXPECTED;
    LOG_WARN("tx start transfer in info is unexpected", K(ret), K(tx_start_transfer_in_info));
  } else if (tx_start_transfer_in_info.data_version_ >= DATA_VERSION_4_2_3_0) {
    b_ret = true;
  ...
  if (b_ret) {
    ...
    for (int64_t i = 0; OB_SUCC(ret) && i < tx_start_transfer_in_info.tablet_meta_list_.count(); ++i) {
      ...
      } else if (OB_SUCCESS != (tmp_ret = reporter->submit_tablet_update_task(MTL_ID(),
    ...
    const char *&can_not_do_reason)
{
  ...
  if (!tx_start_transfer_in_info.is_valid() || (for_replay && !scn.is_valid())) {
    ...
    ret = OB_ERR_UNEXPECTED;
    LOG_WARN("ls service should not be null", K(ret), KP(ls_service));
  } else if (OB_FAIL(ls_service->get_ls(tx_start_transfer_in_info.dest_ls_id_, ls_handle,
    ...
    const share::ObLSRestoreStatus &ls_restore_status)
{
  ...
  if (!tx_start_transfer_in_info.is_valid() || !scn.is_valid()
  ...
  } else if (ls_restore_status.is_in_restore() && ObMigrationStatus::OB_MIGRATION_STATUS_NONE != migration_status) {
    ...
    if (OB_FAIL(ObTransferUtils::get_gts(tenant_id, gts_scn))) {
      ...
      if (OB_ISNULL(ls_service = MTL(ObLSService *))) {
        ret = OB_ERR_UNEXPECTED;
        LOG_WARN("ls service should not be null", K(ret), KP(ls_service));
      } else if (OB_FAIL(ls_service->get_ls(tx_start_transfer_in_info.src_ls_id_, ls_handle,
                                            ObLSGetMod::STORAGE_MOD,
                                            ObLSAccessAttr::DISABLE_LOGONLY))) {
        if (OB_LS_NOT_EXIST == ret) {
          ret = OB_EAGAIN;
