    ...
    mds::BufferCtx &ctx)
{
  ...
  if (OB_ISNULL(ls_svr = MTL(ObLSService *))) {
    ret = OB_ERR_UNEXPECTED;
    LOG_WARN("ls svr should not be NULL", K(ret), KP(ls_svr));
  } else if (CLICK_FAIL(ls_svr->get_ls(src_ls_id, ls_handle, ObLSGetMod::STORAGE_MOD, ObLSAccessAttr::DISABLE_LOGONLY))) {
    ...
    mds::BufferCtx &ctx)
{
  ...
  if (OB_ISNULL(ls_svr = MTL(ObLSService *))) {
    ret = OB_ERR_UNEXPECTED;
    LOG_WARN("ls svr should not be NULL", K(ret), KP(ls_svr));
  } else if (CLICK_FAIL(ls_svr->get_ls(tx_finish_transfer_out_info.src_ls_id_, ls_handle, ObLSGetMod::STORAGE_MOD, ObLSAccessAttr::DISABLE_LOGONLY))) {
...
int ObTabletFinishTransferInReplayExecutor::do_replay_(ObTabletHandle &tablet_handle)
{
  ...
  if (!is_inited_) {
    ...
    ret = OB_ERR_UNEXPECTED;
    LOG_WARN("ls svr should not be NULL", K(ret), KP(ls_svr));
  } else if (OB_FAIL(ls_svr->get_ls(dest_ls_id_, ls_handle, ObLSGetMod::STORAGE_MOD, ObLSAccessAttr::DISABLE_LOGONLY))) {
...
int ObTabletFinishTransferInReplayExecutor::try_make_dest_ls_rebuild_()
{
  ...
  if (!is_inited_) {
    ...
    ret = OB_ERR_UNEXPECTED;
    LOG_WARN("ls svr should not be NULL", K(ret), KP(ls_svr));
  } else if (OB_FAIL(ls_svr->get_ls(src_ls_id_, ls_handle, ObLSGetMod::STORAGE_MOD, ObLSAccessAttr::DISABLE_LOGONLY))) {
    ...
    mds::BufferCtx &ctx)
{
  ...
  if (OB_ISNULL(ls_svr = MTL(ObLSService *))) {
    ret = OB_ERR_UNEXPECTED;
    LOG_WARN("ls svr should not be NULL", K(ret), KP(ls_svr));
  } else if (CLICK_FAIL(ls_svr->get_ls(dest_ls_id, ls_handle, ObLSGetMod::STORAGE_MOD, ObLSAccessAttr::DISABLE_LOGONLY))) {
    ...
    mds::BufferCtx &ctx)
{
  ...
  if (OB_ISNULL(ls_svr = MTL(ObLSService *))) {
    ret = OB_ERR_UNEXPECTED;
    LOG_WARN("ls svr should not be NULL", K(ret), KP(ls_svr));
  } else if (CLICK_FAIL(ls_svr->get_ls(tx_finish_transfer_in_info.dest_ls_id_, ls_handle, ObLSGetMod::STORAGE_MOD, ObLSAccessAttr::DISABLE_LOGONLY))) {
    ...
    const char *&can_not_do_reason)
{
  ...
  if (OB_ISNULL(buf) || buf_len < 0 || (for_replay && !log_scn.is_valid())) {
    ...
    ret = OB_ERR_UNEXPECTED;
    LOG_WARN("tx finish transfer in info is unexpected", K(ret), K(tx_finish_transfer_in_info));
  } else if (tx_finish_transfer_in_info.data_version_ < DATA_VERSION_4_2_3_0) {
    ...
    const char *&can_not_do_reason)
{
  ...
  if (!tx_finish_transfer_in_info.is_valid() || !finish_transfer_scn.is_valid()) {
    ...
    ret = OB_ERR_UNEXPECTED;
    LOG_WARN("ls svr should not be NULL", K(ret), KP(ls_svr));
  } else if (OB_FAIL(ls_svr->get_ls(tx_finish_transfer_in_info.dest_ls_id_, ls_handle, ObLSGetMod::STORAGE_MOD, ObLSAccessAttr::DISABLE_LOGONLY))) {
