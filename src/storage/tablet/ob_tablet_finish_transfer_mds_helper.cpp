    ...
    mds::BufferCtx &ctx)
{
  ...
  ObTransferUtils::clear_transfer_module();
  if (OB_SUCC(ret)) {
    ObTransferUtils::reset_related_info(tx_finish_transfer_out_info.dest_ls_id_);
  }
  if (OB_FAIL(ret)) {
    ...
    mds::BufferCtx &ctx)
{
  ...
  if (tx_finish_transfer_in_info.is_valid()) {
    ...
    diag.cur_phase_ = share::ObStorageHADiagTaskType::TRANSFER_FINISH_IN;
    diag.update_cost_item(ret, share::ObStorageHACostItemName::ON_REGISTER_SUCCESS);
    ...
    mds::BufferCtx &ctx)
{
  ...
  if (tx_finish_transfer_in_info.is_valid()) {
    share::ObHAInflightDiagState diag;
    diag.cur_phase_ = share::ObStorageHADiagTaskType::TRANSFER_FINISH_IN;
    diag.update_cost_item(ret, share::ObStorageHACostItemName::ON_REPLAY_SUCCESS);
    ObTransferUtils::merge_transfer_diag(
        tx_finish_transfer_in_info.dest_ls_id_,
        tx_finish_transfer_in_info.task_id_,
        diag);
    // Wake the local transfer service so this follower's handler ticks soon
    // and notices `__all_transfer_task` being gone — otherwise it waits up
    // to `_transfer_service_wakeup_interval` (10s default), and the inflight
    // diag row lingers until then.
    ObTransferService *transfer_service = MTL(ObTransferService *);
    if (OB_NOT_NULL(transfer_service)) {
