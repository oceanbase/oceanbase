...
public:
  ...
  int get_block_list_level(const int64_t flushing_page_num,
  ...
  int insert_block_into_flush_priority_list(const BlockFlushLevel level, ObTmpFileBlock &block);
  int remove_block_from_flush_priority_list(const BlockFlushLevel level, ObTmpFileBlock &block);
...
private:
  bool is_inited_;
  ObTmpFileBlockHandleList flush_lists_[to_flush_level_idx(BlockFlushLevel::MAX)];
};
