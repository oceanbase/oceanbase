...
namespace tmp_file
{
ObTenantTmpFileManager::ObTenantTmpFileManager()
  : is_inited_(false),
...
int ObTenantTmpFileManager::init()
{
  ...
  if (OB_UNLIKELY(is_inited_)) {
    ret = OB_INIT_TWICE;
    LOG_WARN("ObTenantTmpFileManager init twice", KR(ret), K(is_inited_));
    ...
    is_inited_ = true;
    LOG_INFO("ObTenantTmpFileManager init successful", K(tenant_id_), KP(this));
  }
  return ret;
}
...
int ObTenantTmpFileManager::start()
{
  ...
  if (IS_NOT_INIT) {
    ...
    LOG_INFO("ObTenantTmpFileManager start successful", K(tenant_id_), KP(this));
  }
  return ret;
}
...
void ObTenantTmpFileManager::stop()
{
  ...
  LOG_INFO("ObTenantTmpFileManager stop successful", K(tenant_id_), KP(this));
}

void ObTenantTmpFileManager::wait()
{
  page_cache_controller_.wait();
  LOG_INFO("ObTenantTmpFileManager wait successful", K(tenant_id_), KP(this));
}
...
void ObTenantTmpFileManager::destroy()
{
  ...
  LOG_INFO("ObTenantTmpFileManager destroy", K(tenant_id_), KP(this));
}
...
void ObTenantTmpFileManager::refresh_meta_memory_limit()
{
  int ret = OB_SUCCESS;
...
int ObTenantTmpFileManager::aio_read(const ObTmpFileIOInfo &io_info, ObTmpFileIOHandle &io_handle)
{
  ...
  if (IS_NOT_INIT) {
    ...
    LOG_WARN("fail to init io handle", KR(ret), K(io_info), K(tmp_file_handle));
...
int ObTenantTmpFileManager::aio_write(const ObTmpFileIOInfo &io_info, ObTmpFileIOHandle &io_handle)
{
  ...
  if (IS_NOT_INIT) {
    ...
    LOG_WARN("fail to get tmp file io handle", KR(ret), K(io_info));
  } else if (OB_FAIL(io_handle.init_write(io_info, tmp_file_handle))) {
    LOG_WARN("fail to init io handle", KR(ret), K(io_info), K(tmp_file_handle));
...
int ObTenantTmpFileManager::get_tmp_file(const int64_t fd, ObTmpFileHandle &file_handle)
{
  int ret = OB_SUCCESS;
...
int ObTenantTmpFileManager::get_tmp_file_size(const int64_t fd, int64_t &size)
{
  int ret = OB_SUCCESS;
  ObTmpFileHandle tmp_file_handle;
  if (IS_NOT_INIT) {
...
int ObTenantTmpFileManager::get_macro_block_list(common::ObIArray<blocksstable::MacroBlockId> &macro_id_list)
{
  int ret = OB_SUCCESS;
  if (IS_NOT_INIT) {
    ret = OB_NOT_INIT;
...
int ObTenantTmpFileManager::get_macro_block_count(int64_t &macro_block_count)
{
  int ret = OB_SUCCESS;
  if (IS_NOT_INIT) {
    ret = OB_NOT_INIT;
