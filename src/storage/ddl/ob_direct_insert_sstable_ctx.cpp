...
int ObSSTableInsertTabletContext::prepare_reshape(const ObTableSchema *table_schema,
    ObTableSchemaParam &schema_param, ObRelativeTable &relative_table) const
{
  int ret = OB_SUCCESS;
  ...
  } else if (OB_FAIL(relative_table.init(&schema_param,
      tablet_handle_.get_obj()->get_tablet_meta().tablet_id_))) {
    LOG_WARN("fail to init relative_table", K(ret), K(schema_param),
        K(tablet_handle_.get_obj()->get_tablet_meta()));
  }
  return ret;
}

int ObSSTableInsertTabletContext::check_null(
    const ObTableSchema &table_schema, const ObNewRow &row_val, const int64_t rowkey_column_num) const
{
  int ret = OB_SUCCESS;
  if (table_schema.is_index_table()) {
    // index table is index-organized but can have null values in index column
  } else if (OB_UNLIKELY(rowkey_column_num > row_val.count_)) {
    ret = OB_INVALID_ARGUMENT;
    LOG_WARN("invalid rowkey column number", K(ret), K(rowkey_column_num), K(row_val));
  } else {
    for (int64_t i = 0; OB_SUCC(ret) && i < rowkey_column_num; i++) {
      const ObObj &cell = row_val.cells_[i];
      if (cell.is_null()) {
        ret = OB_ER_INVALID_USE_OF_NULL;
        LOG_WARN("invalid null cell for row key column", K(ret), K(cell));
      }
    }
  }
  return ret;
    ...
    int64_t &affected_rows)
{
  int ret = OB_SUCCESS;
  affected_rows = 0;
  ObDDLSSTableRedoWriter data_redo_writer;
  const ObTabletID &tablet_id = build_param.tablet_id_;
  ObLSID ls_id;
  ...
  if (OB_FAIL(ret)) {
    ...
    // maybe the index builder is better built in macro block writer
    data_desc.sstable_index_builder_ = index_builder_;
    data_desc.is_ddl_ = true;
    ObSSTableInsertRowIterator *tablet_row_iter = reinterpret_cast<ObSSTableInsertRowIterator *>(&iter);
    HEAP_VAR(ObMacroBlockWriter, writer) {
      ...
      if (OB_FAIL(get_table_key(table_key))) {
        ...
        ret = OB_INVALID_ARGUMENT;
        LOG_WARN("invalid argument", K(ret), K(table_key));
      } else if (OB_FAIL(callback.init(DDL_MB_DATA_TYPE, table_key, &data_redo_writer))) {
        LOG_WARN("fail to init data callback", K(ret));
      } else if (OB_FAIL(writer.open(data_desc, start_seq, &callback))) {
        LOG_WARN("open macro block writer failed", K(ret), K(tablet_id), K(data_desc));
      }
      ...
      if (OB_FAIL(ret)) {
      } else if (OB_FAIL(prepare_reshape(table_schema, schema_param, relative_table))) {
        ...
        STORAGE_LOG(WARN, "Failed to init datum row", K(ret), K(col_descs));
      }

      int64_t rowkey_column_num = table_schema->get_rowkey_column_num();
      ...
      while (OB_SUCC(ret)) {
        if (OB_FAIL(THIS_WORKER.check_status())) {
          LOG_WARN("check status failed", K(ret));
                    ...
                    build_param.table_id_, rowkey_column_num, snapshot_version, row_val, row_tablet_id))) {
          if (OB_ITER_END != ret) {
          ...
          if (OB_FAIL(ObRowReshapeUtil::reshape_table_rows(
                  row_val, reshape_ptr, col_descs.count(), &row, 1, sql_mode_for_ddl_reshape))) {
            LOG_WARN("failed to malloc and reshape row", K(ret));
          } else if (OB_FAIL(check_null(*table_schema, row.row_val_, rowkey_column_num))) {
            LOG_WARN("unexpected null value in row", K(ret), K(row));
          } else if (OB_FAIL(datum_row.from_store_row(row))) {
            STORAGE_LOG(WARN, "Failed to transfer store row ", K(ret), K(row));
          }
          for (int64_t i = 0; OB_SUCC(ret) && i < col_descs.count(); i++) {
            ObStorageDatum &datum = datum_row.storage_datums_[i];
            if (col_descs.at(i).col_type_.is_lob_v2() && !datum.is_nop() && !datum.is_null()) {
              lob_cnt++;
              const int64_t timeout_ts = ObTimeUtility::current_time() + ObInsertLobColumnHelper::LOB_ACCESS_TX_TIMEOUT;
              if (OB_FAIL(ObInsertLobColumnHelper::insert_lob_column(
                  lob_allocator, ls_id, tablet_id, col_descs.at(i), datum, timeout_ts))) {
          ...
          if (OB_FAIL(ret)) {
            ...
            if (OB_ERR_PRIMARY_KEY_DUPLICATE == ret && table_schema->is_unique_index()) {
              ...
              int64_t task_id = 0;
              index_key.assign(row.row_val_.cells_, rowkey_column_num);
              if (OB_SUCCESS != ObDDLErrorMessageTableOperator::extract_index_key(*table_schema, index_key, index_key_buffer, OB_TMP_BUF_SIZE_256)) {   // read the unique key that violates the unique constraint
                LOG_WARN("extract unique index key failed", K(index_key), K(index_key_buffer));
            ...
            LOG_DEBUG("sstable insert op append row", K(row));
            ++affected_rows;
          }
          if (lob_cnt++ % ObInsertLobColumnHelper::LOB_ALLOCATOR_RESET_CYCLE == 0) {
...
ObSSTableInsertTableParam::ObSSTableInsertTableParam()
  : exec_ctx_(nullptr), context_id_(0), dest_table_id_(OB_INVALID_ID), write_major_(false), schema_version_(0),
    snapshot_version_(0), task_cnt_(0), execution_id_(0), ddl_task_id_(0), cluster_version_(0), ls_tablet_ids_()
{
}
