  ...
  int64_t &real_sleep_us)
{
  ...
  if (OB_FAIL(ret)) {
    ...
    while (OB_SUCC(ret) && is_need_stop_write) {
      ...
      if (0 == loop_cnt % 100) {
        int64_t unused_data_format_version = 0;
        share::ObDDLTaskStatus task_status = share::ObDDLTaskStatus::PREPARE;
        if (OB_TMP_FAIL(ObDDLUtil::get_data_format_version(tenant_id, task_id, unused_data_format_version, task_status))) {
      ...
      if (is_need_stop_write && OB_TMP_FAIL(check_need_stop_write(ddl_kv_mgr_handle, is_need_stop_write))) {
        LOG_WARN("fail to check need stop write", K(tmp_ret), K(ddl_kv_mgr_handle));
      }
      loop_cnt++;
    }
    ...
    const int64_t data_format_version)
{
  ...
  } else if (OB_FAIL(write_commit_log_with_retry(tablet_handle, ddl_kv_mgr_handle, true, table_key, commit_scn, is_remote_write))) {
...
int ObDDLSSTableRedoWriter::write_commit_log_with_retry(ObTabletHandle &tablet_handle,
                       ObDDLKvMgrHandle &ddl_kv_mgr_handle,
                       const bool allow_remote_write,
                       const ObITable::TableKey &table_key,
                       ...
                       bool &is_remote_write)
{
  ...
  do {
    if (OB_FAIL(THIS_WORKER.check_status())) {
      LOG_WARN("check status failed", K(ret));
    } else if (OB_FAIL(write_commit_log(tablet_handle, ddl_kv_mgr_handle, allow_remote_write, table_key, commit_scn, is_remote_write))) {
...
int ObDDLRedoLogWriterCallback::retry(const int64_t timeout_us)
{
  ...
  if (IS_NOT_INIT) {
    ...
    while (ObTimeUtility::fast_current_time() - start_ts < timeout_us) { // ignore ret
      if (OB_FAIL(THIS_WORKER.check_status())) {
        LOG_WARN("check status failed", K(ret));
      } else if (OB_FAIL(ddl_writer_->write_redo_log(redo_info_, macro_block_id_, true/*allow remote write*/, task_id_, tablet_handle_, ddl_kv_mgr_handle_))) {
        LOG_WARN("fail to write ddl redo log", K(ret), K(ret));
      } else if (OB_FAIL(ddl_writer_->wait_redo_log_finish(redo_info_, macro_block_id_))) {
