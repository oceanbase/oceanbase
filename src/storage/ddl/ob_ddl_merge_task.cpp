...
int ObDDLTableMergeTask::process()
{
  ...
  if (OB_UNLIKELY(!is_inited_)) {
    ret = OB_NOT_INIT;
    LOG_WARN("not init", K(ret));
  } else if (OB_FAIL(MTL(ObLSService *)->get_ls(merge_param_.ls_id_, ls_handle, ObLSGetMod::DDL_MOD, ObLSAccessAttr::DISABLE_LOGONLY))) {
                                        ...
                                        blocksstable::ObSSTable &sstable)
{
  ...
  if (OB_UNLIKELY(nullptr == sstable_index_builder || !ddl_param.is_valid())) {
    ...
    if (OB_FAIL(param.init_for_ddl(sstable_index_builder, ddl_param, first_ddl_sstable,
        *storage_schema, create_schema_version_on_tablet))) {
      LOG_WARN("fail to init param for ddl",
          K(ret), K(create_schema_version_on_tablet),
                                            ...
                                            blocksstable::ObSSTable &sstable)
{
  ...
  if (OB_UNLIKELY(!ddl_param.is_valid())) {
    ...
    if (OB_FAIL(ls_service->get_ls(ddl_param.ls_id_, ls_handle, ObLSGetMod::DDL_MOD, ObLSAccessAttr::DISABLE_LOGONLY))) {
                                         ...
                                         const int64_t data_format_version)
{
  ...
  if (OB_UNLIKELY(!ls_id.is_valid() || !tablet_id.is_valid() || OB_INVALID_ID == ddl_task_id
        || !is_valid_id(table_id) || 0 == table_id || execution_id < 0 || data_format_version < 0)) {
    ret = OB_INVALID_ARGUMENT;
    LOG_WARN("invalid argument", K(ret), K(ls_id), K(tablet_id), K(table_id), K(execution_id), K(data_format_version));
