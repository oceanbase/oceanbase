...
int ObComplementDataParam::prepare_task_ranges()
{
  ...
  if (OB_UNLIKELY(!is_inited_)) {
    ...
    } else if (!is_remote_exec && OB_FAIL(split_task_ranges(orig_ls_id_, orig_tablet_id_, orig_schema_tablet_size_, user_parallelism_))) {
  ...
  HEAP_VARS_3((ObMacroBlockWriter, writer),
    ...
    if (OB_UNLIKELY(!is_inited_)) {
      ...
      if (OB_FAIL(ObMultiVersionSchemaService::get_instance().get_tenant_schema_guard(
        ...
        ret = OB_ERR_UNEXPECTED;
        LOG_WARN("the dag of this task is null", K(ret), KPC(param_));
      } else if (FALSE_IT(sstable_redo_writer.set_start_scn(context_->data_sstable_redo_writer_.get_start_scn()))) {
    ...
    while (OB_SUCC(ret)) {      //get each row from row_iter
      ...
      dag_yield();
      if (OB_FAIL(DDL_SIM(param_->dest_tenant_id_, param_->task_id_, DDL_INSERT_SSTABLE_GET_NEXT_ROW_FAILED))) {
        LOG_WARN("ddl sim failure", K(ret), KPC(param_));
...
int ObComplementMergeTask::add_build_hidden_table_sstable()
{
  ...
  if (OB_UNLIKELY(!is_inited_)) {
    ...
    LOG_WARN("fail to get hidden table key", K(ret), K(hidden_table_key));
      ...
      param_->dest_ls_id_, hidden_table_key, param_->dest_table_id_, param_->execution_id_, param_->task_id_, param_->data_format_version_))) {
