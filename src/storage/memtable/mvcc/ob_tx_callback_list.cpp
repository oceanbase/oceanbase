        ...
        if (parallel_start_pos_
          ...
          if ((deleted && OB_FAIL(ret)) || (!deleted && OB_FAIL(iter->del()))) {
            ...
            if (iter->need_submit_log()) {
              if (OB_UNLIKELY(!iter->need_fill_redo())) {
                ret = OB_ERR_UNEXPECTED;
                TRANS_LOG(ERROR, "unexpected callback state", KR(ret), KPC(iter), KPC(this));
#ifdef ENABLE_DEBUG_LOG
                ob_abort();
#endif
              }
...
int ObTxCallbackList::submit_log_succ(const ObCallbackScope &callbacks)
{
  ...
  ATOMIC_STORE(&logged_, logged_ + (int64_t)callbacks.cnt_);
  ATOMIC_STORE(&logged_data_size_, logged_data_size_ + callbacks.data_size_);
  return ret;
}
...
int ObTxCallbackList::sync_log_fail(const ObCallbackScope &callbacks, int64_t &removed_cnt)
{
  int ret = OB_SUCCESS;
