...
public:
  ...
  void trans_start();
  int calc_checksum_all(ObSEArray<uint64_t, 1> &checksum);
...
private:
  ...
  // since 4.2.4, support multi-callback-list logging, extended callback-list(s)
...
public:
  ...
  virtual int log_sync(const share::SCN scn, ObIMemtable *&last_mt) override;
