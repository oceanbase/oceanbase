...
public:
  ...
  virtual bool is_iter_end(ObITransCallback *callback) const override
  {
    ...
    if (NULL == callback) {
      ...
      is_iter_end = true;
    } else if (callback->need_fill_redo()) {
...
public:
  ...
  int operator()(ObITransCallback *callback)
  {
    ...
    if (NULL == callback) {
      ...
      // NOTE: submit and log sync callback in concurrency
...
class ObITxFillRedoFunctor
{
public:
  virtual int operator()(ObITransCallback *start,
                         ...
                         int32_t &cnt,
                         int64_t &data_size,
                         transaction::ObTxSEQ &max_seq_no) = 0;
};
