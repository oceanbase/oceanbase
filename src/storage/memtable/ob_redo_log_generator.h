...
struct ObTxFillRedoCtx final
{
  ObTxFillRedoCtx(ObSEArray<ObCallbackScope, 1> &callback_scopes) :
...
public:
  ...
  void inc_fill_log_cnt(const int cnt)
  {
    ATOMIC_AAF(&redo_filled_cnt_, cnt);
  }
  void inc_sync_log_succ_cnt(const int cnt)
  {
    redo_sync_succ_cnt_ += cnt;
  }
