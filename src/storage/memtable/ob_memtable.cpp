  ...
  if (IS_NOT_INIT) {
    ...
    if (OB_FAIL(parameter_mtk.encode(out_cols, &rowkey.get_store_rowkey()))) {
      ...
      if (OB_SUCC(ret)) {
        ...
        ObNopBitMap bitmap;
        int64_t row_scn = 0;
        if (OB_FAIL(bitmap.init(request_cnt,
                                store_rowkey->get_obj_cnt(),
                                read_info->get_extra_rowkey_count(),
                                read_info->get_trans_col_index()))) {
                     ...
                     ObStoreRowIterator *&row_iter)
{
  ...
  if (IS_NOT_INIT) {
    ...
    } else if (OB_UNLIKELY(param.is_index_skip_scan())) {
