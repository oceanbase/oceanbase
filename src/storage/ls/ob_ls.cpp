                                 ...
                                 const SCN &create_scn)
{
  ...
  if (ObReplicaTypeCheck::is_log_replica(ls_meta_.get_replica_type())) {
    // do nothing
  } else if (OB_FAIL(tx_table_.create_tablet(compat_mode, create_scn))) {
...
int ObLS::remove_ls_inner_tablet()
{
  ...
  if (ObReplicaTypeCheck::is_log_replica(ls_meta_.get_replica_type())) {
    // do nothing
  } else if (OB_FAIL(tx_table_.remove_tablet())) {
    ...
    for (int64_t i = 0; OB_SUCC(ret) && i < col_count; ++i) {
      ...
      switch (col_id) {
        ...
        case static_cast<uint64_t>(ObAllVirtualLSInfoColumnId::WEAK_READ_TIMESTAMP): {
          // weak_read_timestamp
          if (is_logonly) {
            ls_info.weak_read_scn_ = SCN::min_scn();
...
int ObLS::finish_storage_meta_replay()
{
  ...
  // For L replica, skip reorg_info_table_.init_tablet_for_compat
  // because L replica does not have tablet and reorg_info_table_ is not inited.
  } else if (!is_logonly_replica()
      && ls_meta_.get_persistent_state().is_normal_state()
