               ...
               observer::ObIMetaReport *reporter)
{
  ...
  if (!ls_id.is_valid() ||
    ...
    if (OB_FAIL(replica_checkpoint_collector_.init(ls_id))) {
      LOG_WARN("failed to init replica checkpoint collector", KR(ret), K(ls_id));
      ...
      if (OB_FAIL(ls_freezer_.init(this))) {
        ...
        if (OB_SUCC(ret) && ls_id.is_sys_ls()) {
          REGISTER_TO_LOGSERVICE(logservice::DBMS_SCHEDULER_LOG_BASE_TYPE, MTL(rootserver::ObDBMSSchedService *));
          if (is_user_tenant(tenant_id)) {
            //user
...
bool ObLS::safe_to_destroy()
{
  ...
  if (ObReplicaTypeCheck::is_log_replica(ls_meta_.get_replica_type())) {
    if (OB_FAIL(ls_restore_handler_.safe_to_destroy(is_ls_restore_handler_safe))) {
  ...
  if (OB_SUCC(ret)) {
    if (!is_safe) {
      if (REACH_TIME_INTERVAL(60 * 1000 * 1000)) {
        LOG_WARN("this ls is not safe to destroy", K(is_safe), K(ls_meta_.get_replica_type()),
...
void ObLS::destroy()
{
  ...
  if (ObReplicaTypeCheck::is_log_replica(ls_meta_.get_replica_type())) {
    ...
    tenant_id_ = OB_INVALID_TENANT_ID;
    need_delay_resource_recycle_ = false;
    replica_checkpoint_collector_.reset();
  } else {
    ...
    if (ls_meta_.ls_id_.is_sys_ls()) {
      UNREGISTER_FROM_LOGSERVICE(logservice::DBMS_SCHEDULER_LOG_BASE_TYPE, MTL(rootserver::ObDBMSSchedService *));
      if (is_user_tenant(MTL_ID())) {
...
int ObLS::offline_(const int64_t start_ts)
{
  ...
  if (IS_NOT_INIT) {
    ...
    if (OB_FAIL(ls_restore_handler_.offline())) {
    ...
    if (FALSE_IT(checkpoint_executor_.offline())) {
...
int ObLS::online()
{
  ...
  if (IS_NOT_INIT) {
    ...
    if (OB_FAIL(log_handler_.online(ls_meta_.get_clog_base_lsn(),
                                    ls_meta_.get_clog_checkpoint_scn(),
                                    true/*is_logonly_replica*/))) {
    ...
    if (OB_FAIL(ls_tablet_svr_.online())) {
...
int ObLS::get_ls_info(const ObIArray<uint64_t> &output_column_ids, ObLSVTInfo &ls_info)
{
  ...
  if (IS_NOT_INIT) {
    ret = OB_NOT_INIT;
    LOG_WARN("ls is not inited", K(ret));
  } else if (FALSE_IT(is_logonly = ObReplicaTypeCheck::is_log_replica(ls_meta_.get_replica_type()))) {
    // shall never be here
  } else if (col_count <= 0) {
    // do nothing
  } else {
    ...
    for (int64_t i = 0; OB_SUCC(ret) && i < col_count; ++i) {
      ...
      switch (col_id) {
        ...
        case static_cast<uint64_t>(ObAllVirtualLSInfoColumnId::TABLET_COUNT): {
          // tablet_count
          ls_info.tablet_count_ = is_logonly ? 0 : ls_tablet_svr_.get_tablet_count();
          break;
        }
        ...
        case static_cast<uint64_t>(ObAllVirtualLSInfoColumnId::REQUIRED_DATA_DISK_SIZE): {
          ...
          if (is_logonly) {
            ls_info.required_data_disk_size_ = 0;
          } else if (OB_FAIL(ls_tablet_svr_.get_ls_migration_required_size(required_data_disk_size))) {
    ...
    ObTabletHandle &handle) const
{
  ...
  if (IS_NOT_INIT) {
    ...
    } else if (OB_FAIL(MTL(logservice::ObLogService*)->get_log_replay_service()->get_max_replayed_scn(ls_meta_.ls_id_, max_scn))) {
...
int ObLS::check_can_replay_clog(bool &can_replay)
{
  ...
  if (is_need_gc()) {
    ...
    can_replay = false;
    LOG_INFO("log replica should not enable replay", "ls_id", get_ls_id(), K_(ls_meta));
  }
  return ret;
}
    ...
    bool &allow_read)
{
  ...
  if (!ObMigrationStatusHelper::is_valid(migration_status) || !restore_status.is_valid()) {
    ...
    && (restore_status.is_restore_none() || is_sys_ls())) {
    allow_read = true;
  } else {
    allow_read = false;
  }
