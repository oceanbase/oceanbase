...
int ObVectorRefreshIndexExecutor::set_attribute(pl::ObPLExecCtx &ctx, const ObVectorRebuildIndexArg &arg)
{
  ...
  OX(tenant_id_ = session_info_->get_effective_tenant_id());
  OZ(ObVectorRefreshIndexExecutor::check_min_data_version(
      tenant_id_, DATA_VERSION_4_4_1_0,
      "tenant's data version is below 4.4.1.0, refreshing vector index is not "
      "supported."));
    ...
    const int64_t tenant_id, const ObString &value, const bool is_rebuild)
{
  ...
  } else if (OB_FAIL(dbms_scheduler::ObDBMSSchedJobUtils::check_is_valid_repeat_interval(value))) {
