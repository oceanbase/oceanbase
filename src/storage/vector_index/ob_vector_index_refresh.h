...
public:
  ObVectorRefreshIndexCtx()
      : allocator_("VecRefCtx"), tenant_id_(OB_INVALID_TENANT_ID),
        base_tb_id_(OB_INVALID_ID), delta_buf_tb_id_(OB_INVALID_ID),
        index_id_tb_id_(OB_INVALID_ID), trans_(nullptr),
        refresh_method_(share::schema::ObVectorRefreshMethod::MAX) {}
  bool is_valid() const {
    return OB_INVALID_TENANT_ID != tenant_id_ &&
           OB_INVALID_ID != delta_buf_tb_id_ && OB_INVALID_ID != base_tb_id_ &&
  ...
  TO_STRING_KV(K_(tenant_id), K_(base_tb_id), K_(delta_buf_tb_id),
...
private:
  ...
  static int lock_delta_buf_tb(ObVectorRefreshIdxTransaction &trans,
                               const uint64_t tenant_id,
                               const uint64_t delta_buf_tb_id,
  ...
  int lock_delta_buf_table_for_refresh();
  int do_refresh();
  int do_rebuild();
