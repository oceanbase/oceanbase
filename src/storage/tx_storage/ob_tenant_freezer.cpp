...
bool ObTenantFreezer::exist_ls_freezing()
{
  ...
  if (IS_NOT_INIT) {
    ret = OB_NOT_INIT;
    LOG_WARN("[TenantFreezer] tenant freezer not inited", KR(ret));
  } else if (OB_FAIL(ls_srv->get_ls_iter(iter, ObLSGetMod::TXSTORAGE_MOD, ObLSAccessAttr::DISABLE_LOGONLY))) {
...
int ObTenantFreezer::ls_freeze_all_unit_(ObLS *ls, const int64_t abs_timeout_ts)
{
  ...
  do {
    ...
    } else if (OB_SUCC(ls->advance_checkpoint_by_flush(SCN::max_scn(), abs_timeout_ts, true /* is_tenant_freeze */))) {
    } else {
...
int ObTenantFreezer::tenant_freeze_data_()
{
  ...
  if (OB_FAIL(ls_srv->get_ls_iter(iter, ObLSGetMod::TXSTORAGE_MOD, ObLSAccessAttr::DISABLE_LOGONLY))) {
    LOG_WARN("[TenantFreezer] fail to get log stream iterator", KR(ret));
  } else {
    ObLS *ls = nullptr;
    int ls_cnt = 0;
...
int ObTenantFreezer::ls_freeze_all_unit(const share::ObLSID &ls_id)
{
  ...
  if (IS_NOT_INIT) {
    ...
    LOG_WARN("[TenantFreezer] tenant freezer not inited", KR(ret));
                                                  ...
                                                  abs_timeout_ts))) {
    LOG_WARN("get timeout ts failed", KR(ret));
  } else if (OB_FAIL(ls_srv->get_ls(ls_id, handle, ObLSGetMod::TXSTORAGE_MOD, ObLSAccessAttr::DISABLE_LOGONLY))) {
...
int ObTenantFreezer::do_freeze_diagnose()
{
  ...
  if (current_time - freezer_stat_.last_captured_timestamp_ >= 30 * 1_min) {
    int64_t current_retire_clock = MTL(ObSharedMemAllocMgr *)->memstore_allocator().get_retire_clock();
    ...
    if (freezer_stat_.last_captured_timestamp_ == 0) {
      ...
      ATOMIC_FAA(&freezer_stat_.captured_data_size_,
                 current_retire_clock - ATOMIC_LOAD(&freezer_stat_.last_captured_retire_clock_));
      ATOMIC_SET(&freezer_stat_.last_captured_retire_clock_, current_retire_clock);
...
ObTenantFreezerStat::ObFreezerMergeType ObTenantFreezerStat::switch_to_freezer_merge_type(const ObMergeType type)
{
...
void ObTenantFreezerStat::add_merge_event(const ObMergeType type, const int64_t cost)
{
