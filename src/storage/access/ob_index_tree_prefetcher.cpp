...
int ObIndexTreePrefetcher::lookup_in_index_tree(ObSSTableReadHandle &read_handle)
{
  ...
  while (OB_SUCC(ret) && !found) {
    ...
    } else if (index_block_info.is_macro_node() && OB_FAIL(check_bloom_filter(index_block_info, false, read_handle))) {
    ...
    ObSSTableReadHandle &read_handle)
{
  ...
  if (!index_info.is_valid() || !index_info.is_macro_node()) {
    ...
    // We must recognize this situation by setting the is_rowkey_begin_idx_overlap_ in ObIndexBlockRowScanner::skip_to_next_valid_position.
    ...
    // Otherwise, this border row may be filtered out incorrectly.
      const int64_t tmp_rowkey_begin_idx = index_info.is_rowkey_begin_idx_overlap_ ?
          index_info.rowkey_begin_idx_ + 1 : index_info.rowkey_begin_idx_;
      ...
      } else if (index_info.is_rowkey_begin_idx_overlap_) {
        is_contain = true;
      }
    ...
    while (OB_SUCC(ret) && prefetched_cnt < prefetch_depth) {
      if (OB_FAIL(drill_down())) {
        ...
        while (OB_SUCC(ret) && prefetched_cnt < prefetch_depth) {
          ...
          if (OB_FAIL(tree_handles_[cur_level_].get_next_data_row(is_multi_check(), block_info))) {
            if (OB_UNLIKELY(OB_ITER_END != ret)) {
  ...
  if (fetch_idx_ >= prefetch_idx_) {
    ...
    if (OB_SUCC(ret)) {
      if (index_info.can_blockscan(has_lob_out)) {
        ...
        LOG_WARN("Fail to update_blockscan", K(ret), K_(prefetcher.border_rowkey));
      }
