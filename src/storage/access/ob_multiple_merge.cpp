...
ObMultipleMerge::ObMultipleMerge()
    : padding_allocator_("MULTI_MERGE_PAD"),
      ...
      refresh_table_state_(RefreshTableState::NONE),
    ...
    ObTabletReadTables &tablet_read_tables)
{
  ...
  bool user_filter_contain_rowscn = false;
  ...
  if (OB_UNLIKELY(inited_)) {
    ...
    STORAGE_LOG(WARN, "The ObMultipleMerge has been inited, ", K(ret));
          ...
          || OB_UNLIKELY(!tablet_read_tables.is_valid())) {
    ret = OB_INVALID_ARGUMENT;
    STORAGE_LOG(WARN, "Invalid argument", K(ret), K(param), K(context), K(tablet_read_tables));
  ...
  if (OB_SUCC(ret)) {
    ...
    access_param_->iter_param_.set_tablet_handle(tablet_read_tables.tablet_iter_.get_tablet_handle_ptr());
  ...
  LOG_DEBUG("init multiple merge", K(ret), KP(this), K(param), K(context), K(tablet_read_tables));
  return ret;
}
    ...
    ObTabletReadTables &tablet_read_tables)
{
  ...
  STORAGE_LOG(TRACE, "switch param", K(ret), KP(this), K(param), K(context), K(tablet_read_tables));
  return ret;
}
    ...
    ObTabletReadTables &tablet_read_tables)
{
  ...
  LOG_DEBUG("switch table", K(ret), KP(this), K(param), K(context), K(tablet_read_tables));
  return ret;
}
...
int ObMultipleMerge::save_curr_rowkey()
{
  ...
  } else if (OB_UNLIKELY(ScanState::BATCH == scan_state_)) {
    ret = OB_ERR_UNEXPECTED;
  ...
  if (OB_SUCC(ret)) {
    ...
    if (ScanState::SINGLE_ROW == scan_state_) {
      ...
      if (OB_FAIL(di_base_sstable_row_scanner_->get_blockscan_border_rowkey(tmp_rowkey))) {
        LOG_WARN("fail to get blockscan border rowkey", K(ret), KPC(di_base_sstable_row_scanner_));
      } else if ((access_ctx_->query_flag_.is_reverse_scan() && tmp_rowkey.is_min_rowkey()) ||
  ...
  FLOG_INFO("save refresh table info", K(ret), K(curr_rowkey_), K(curr_scan_index_), K(scan_state_), KPC(di_base_sstable_row_scanner_));
  return ret;
}
...
int ObMultipleMerge::get_next_row(ObDatumRow *&row)
{
  ...
  if (OB_UNLIKELY(!inited_)) {
      ...
      if (OB_SUCC(ret)) {
        if (OB_FAIL(fill_group_idx_if_need(unprojected_row_))) {
          ...
          if (OB_UNLIKELY(nullptr != group_by_cell_)) {
            ...
            if (OB_FAIL(group_by_cell_->copy_single_output_row(access_param_->iter_param_, access_param_->get_op()->get_eval_ctx()))) {
    ...
    if (OB_SUCC(ret)) {
      ++access_ctx_->table_store_stat_.out_row_cnt_;
...
int ObMultipleMerge::get_next_normal_rows(int64_t &count, int64_t capacity)
{
  ...
  if (OB_UNLIKELY(!inited_)) {
        ...
        switch (scan_state_) {
          ...
          case ScanState::DI_BASE: {
            ...
            if (OB_FAIL(inner_get_next_rows())) {
              if (OB_UNLIKELY(OB_PUSHDOWN_STATUS_CHANGED != ret && OB_ITER_END != ret)) {
                LOG_WARN("fail to get next di base rows", K(ret), K(is_unprojected_row_valid_), KPC(vector_store), KPC(di_base_sstable_row_scanner_));
              } else {
                if (OB_ITER_END == ret) {
                  need_scan_di_base_ = false;
                }
                ...
                if (RefreshTableState::NONE != refresh_table_state_) {
                  // if vector_store is empty, must switch to NONE to avoid get SINGLE_ROW before refresh table
                  // if vector_store is not empty, must switch to SUSPEND_REFRESH to prevent loss of the last batch
                  RefreshTableState next_state = vector_store->is_end()
                                                   ? RefreshTableState::SUSPEND_REFRESH
                                                   : RefreshTableState::NONE;
                  if (OB_FAIL(switch_refresh_table_state(next_state))) {
                    LOG_WARN("fail to switch refresh table state", K(ret), K(next_state));
                  }
                }
              }
            }
            ...
            if (OB_FAIL(inner_get_next_row(unprojected_row_))) {
              if (OB_UNLIKELY(OB_PUSHDOWN_STATUS_CHANGED != ret && OB_ITER_END != ret)) {
                ...
                if (OB_FAIL(di_base_sstable_row_scanner_->prepare_di_base_blockscan(true/*di_base_only*/, nullptr/*row*/))) {
                  LOG_WARN("Fail to prepare di base blockscan", K(ret), K(is_unprojected_row_valid_), K_(unprojected_row), KPC(di_base_sstable_row_scanner_));
                } else {
                  break;
                }
              }
            // `unprojected_row_` is used to generate range in pause
              ...
              if (unprojected_row_.is_di_delete()) {
                if (need_scan_di_base_) {
                  scan_state_ = ScanState::DI_BASE;
                  if (OB_FAIL(di_base_sstable_row_scanner_->prepare_di_base_blockscan(false/*di_base_only*/, &unprojected_row_))) {
                    LOG_WARN("Fail to prepare di base blockscan", K(ret), K(is_unprojected_row_valid_), K_(unprojected_row), KPC(di_base_sstable_row_scanner_));
                  ...
                  LOG_WARN("Unexpected di row itered in di case", K(ret), K_(is_unprojected_row_valid),
                           K(tables_.count()), K(iters_.count()), K_(unprojected_row), KPC_(access_param));
    ...
    access_ctx_->table_store_stat_.out_row_cnt_ += count;
  }
  if (OB_ITER_END == ret) {
    update_and_report_tablet_stat();
    scan_state_ = ScanState::NONE;
...
int ObMultipleMerge::get_next_aggregate_row(ObDatumRow *&row)
{
  ...
  if (OB_UNLIKELY(!inited_)) {
    ...
    LOG_WARN("the ObMultipleMerge has not been inited, ", K(ret));
              ...
              || nullptr == block_row_store_
              || access_param_->iter_param_.need_fill_group_idx())) {
    ret = OB_ERR_UNEXPECTED;
    ...
    ObAggregatedStoreVec *agg_store_vec = access_param_->iter_param_.plan_use_new_format()
    ...
    ScanState last_scan_state = ScanState::NONE;
    ...
    while (OB_SUCC(ret) && !batch_row_store->is_end()) {
      ...
      if (OB_FAIL(refresh_table_on_demand())) {
        ...
        is_unprojected_row_valid_ = false;
        need_init_expr_header = last_scan_state != scan_state_;
        last_scan_state = scan_state_;
        switch(scan_state_) {
          case ScanState::BATCH: {
            bool can_batch = false;
            if (nullptr != agg_store_vec && OB_FAIL(agg_store_vec->prepare_batch_scan())) {
            ...
            if (OB_FAIL(ret)) {
              ...
              if (OB_UNLIKELY(OB_PUSHDOWN_STATUS_CHANGED != ret && OB_ITER_END != ret)) {
                LOG_WARN("fail to get next aggregate rows fast", K(ret), K(is_unprojected_row_valid_), KPC(di_base_sstable_row_scanner_));
              } else {
          ...
          case ScanState::DI_BASE: {
            ...
            if (OB_FAIL(ret)) {
              ...
              if (OB_UNLIKELY(OB_PUSHDOWN_STATUS_CHANGED != ret && OB_ITER_END != ret)) {
                LOG_WARN("fail to get next di base rows", K(ret), K(is_unprojected_row_valid_), KPC(di_base_sstable_row_scanner_));
              } else {
                if (OB_ITER_END == ret) {
                  need_scan_di_base_ = false;
                }
                ...
                scan_state_ = ScanState::SINGLE_ROW;
                if (RefreshTableState::NONE != refresh_table_state_) {
                  if (OB_FAIL(switch_refresh_table_state(RefreshTableState::NONE))) {
                    LOG_WARN("fail to switch refresh table state", K(ret));
                  }
                }
              }
            }
          ...
          case ScanState::SINGLE_ROW: {
            if (OB_FAIL(inner_get_next_row(unprojected_row_))) {
              ...
              if (unprojected_row_.is_di_delete()) {
                if (need_scan_di_base_) {
                  scan_state_ = ScanState::DI_BASE;
                  if (OB_FAIL(di_base_sstable_row_scanner_->prepare_di_base_blockscan(false, &unprojected_row_))) {
                    LOG_WARN("Fail to prepare di base blockscan", K(ret), K_(is_unprojected_row_valid),
                             K(tables_.count()), K(iters_.count()), K_(unprojected_row), KPC_(access_param));
                  } else if (unprojected_row_.row_flag_.is_delete() || unprojected_row_.is_insert_filtered_) {
                    break;
            ...
            if (OB_FAIL(ret)) {
              ...
              if (access_param_->iter_param_.use_new_format() &&
                ...
                if (nullptr != agg_store_vec && access_param_->iter_param_.vectorized_enabled_ &&
                    OB_FAIL(access_param_->get_op()->deep_copy(access_param_->output_exprs_, agg_store_vec->get_row_count()))) {
                  LOG_WARN("fail to deep copy row", K(ret));
    ...
    if (OB_ITER_END == ret && !batch_row_store->is_end()) {
      ...
      } else if (OB_FAIL(agg_store_base->collect_aggregated_result())) {
  ...
  LOG_TRACE("[Vectorized] get next aggregate row", K(ret), KPC(row));
  return ret;
}
...
void ObMultipleMerge::report_tablet_stat()
{
  if (OB_ISNULL(access_ctx_) || OB_ISNULL(access_ctx_->table_scan_stat_) || OB_ISNULL(access_param_)) {
    ...
    tablet_stat.scan_logical_row_cnt_ = access_ctx_->table_store_stat_.out_row_cnt_ + access_ctx_->table_store_stat_.storage_filtered_row_cnt_;
...
void ObMultipleMerge::update_and_report_tablet_stat()
{
  if (OB_ISNULL(access_ctx_)) {
    ...
    if (NULL != access_ctx_->table_scan_stat_) {
      ...
      access_ctx_->table_scan_stat_->access_row_cnt_ += access_ctx_->table_store_stat_.out_row_cnt_ +
...
void ObMultipleMerge::reuse()
{
  ...
  reuse_iter_array();
  if (nullptr != di_base_sstable_row_scanner_) {
    di_base_sstable_row_scanner_->reuse();
  }
  range_idx_delta_ = 0;
...
void ObMultipleMerge::reset_iter_array(const bool can_reuse)
{
  if (can_reuse) {
    reclaim_iter_array();
    if (nullptr != di_base_sstable_row_scanner_) {
      di_base_sstable_row_scanner_->reclaim_iter_array();
    }
  } else {
    ...
    iters_.reset();
    if (nullptr != di_base_sstable_row_scanner_) {
      di_base_sstable_row_scanner_->reset_iter_array();
    }
  }
}
...
void ObMultipleMerge::reuse_iter_array()
{
  ...
  if (nullptr != di_base_sstable_row_scanner_) {
    di_base_sstable_row_scanner_->reuse_iter_array();
  }
}
...
int ObMultipleMerge::open()
{
  ...
  if (OB_SUCC(ret)) {
    if (access_param_->iter_param_.is_delete_insert_ && !is_level_order_scan() && OB_FAIL(init_di_base_sstable_row_scanner())) {
      LOG_WARN("fail to init di base sstable row scanner", K(ret));
    }
  }
  return ret;
}
...
int ObMultipleMerge::alloc_row_store(ObTableAccessContext &context, const ObTableAccessParam &param)
{
  ...
  } else if (OB_UNLIKELY((param.iter_param_.enable_pd_aggregate() || param.iter_param_.enable_pd_group_by())
                         && param.iter_param_.use_new_format() && !param.iter_param_.plan_use_new_format())) {
    ret = OB_ERR_UNEXPECTED;
    LOG_WARN("Unexpected state, table scan use new format but plan not", K(ret), K(param.iter_param_.use_new_format()), K(param.iter_param_));
    ...
    if (param.iter_param_.plan_use_new_format()) {
      if (OB_ISNULL(buf = context.stmt_allocator_->alloc(sizeof(ObAggregatedStoreVec)))) {
        ...
        block_row_store_ = new (buf) ObAggregatedStoreVec(
            param.iter_param_.vectorized_enabled_ ? param.get_op()->get_batch_size() : 1,
  ...
  if (OB_SUCC(ret) && param.iter_param_.enable_pd_group_by() && !param.iter_param_.vectorized_enabled_) {
    if (param.iter_param_.plan_use_new_format()) {
      if (OB_ISNULL(buf = context.stmt_allocator_->alloc(sizeof(ObGroupByCellVec)))) {
        ret = common::OB_ALLOCATE_MEMORY_FAILED;
        LOG_WARN("fail to alloc group by cell", K(ret));
      } else {
        group_by_cell_ = new (buf) ObGroupByCellVec(1, param.get_op()->get_eval_ctx(), skip_bit_, *context.stmt_allocator_);
      }
    } else {
      if (OB_ISNULL(buf = context.stmt_allocator_->alloc(sizeof(ObGroupByCell)))) {
        ...
        group_by_cell_ = new (buf) ObGroupByCell(1, *context.stmt_allocator_);
      }
    }
    if (OB_SUCC(ret) && OB_FAIL(group_by_cell_->init_for_single_row(param, context, param.get_op()->get_eval_ctx()))) {
...
int ObMultipleMerge::check_filtered(const ObDatumRow &row, bool &filtered)
{
  ...
  ObMDSFilterMgr *mds_filter_mgr = access_ctx_->mds_filter_mgr_;
  ...
  } else if (!filtered && nullptr != mds_filter_mgr && OB_FAIL(mds_filter_mgr->filter(unprojected_row_, filtered))) {
    LOG_WARN("failed to do mds filter", K(ret), KPC(mds_filter_mgr));
...
int ObMultipleMerge::refresh_filter_params_on_demand(const bool is_open)
{
  ...
  if (OB_UNLIKELY(!inited_)) {
    ...
    if (has_split_filter && (is_open || (OB_NOT_NULL(iter_param.need_update_tablet_param_) && *iter_param.need_update_tablet_param_))) {
      ...
      ObTablet *tablet = tablet_read_tables_->tablet_iter_.get_tablet_handle().get_obj();
    ...
    if (OB_SUCC(ret) && has_split_filter && GCONF.enable_defensive_check()) {
      ObTablet *tablet = tablet_read_tables_->tablet_iter_.get_tablet();
...
int ObMultipleMerge::prepare_read_tables(bool refresh)
{
  ...
  if (OB_UNLIKELY(NULL == tablet_read_tables_ || !access_param_->is_valid() || NULL == access_ctx_)) {
    ret = OB_NOT_INIT;
    LOG_WARN("ObMultipleMerge has not been inited", K(ret), K_(tablet_read_tables), KP_(access_param),
        KP_(access_ctx));
  } else if (!refresh) {
    // make sure tables are prepared when not refresh
    // if (OB_UNLIKELY(!tablet_read_tables_->tablet_iter_.table_iter()->is_valid())) {
    //   ret = OB_ERR_UNEXPECTED;
    //   LOG_WARN("tables are not prepared", K(ret), KPC_(access_ctx), KPC_(access_param), K(tablet_read_tables_->tablet_iter_.table_iter()));
    // }
  } else if (OB_UNLIKELY(is_mds_query)) {
    ret = OB_NOT_SUPPORTED;
    LOG_WARN("mds query does not support refresh table currently", K(ret), K(refresh), K_(access_param));
  } else {
    tablet_read_tables_->tablet_iter_.table_iter()->reset();
    const bool need_split_src_table = access_param_->iter_param_.is_tablet_spliting();
    const bool need_split_dst_table = refresh ? true : tablet_read_tables_->need_split_dst_table_;
    if (OB_UNLIKELY(tablet_read_tables_->frozen_version_ != -1)) {
      if (!tablet_read_tables_->sample_info_.is_no_sample()) {
        ret = OB_NOT_SUPPORTED;
        LOG_WARN("sample query does not support frozen_version", K(ret), K_(tablet_read_tables), KP_(access_param));
      } else if (OB_FAIL(tablet_read_tables_->tablet_iter_.refresh_read_tables_from_tablet(
          tablet_read_tables_->frozen_version_,
          false/*allow_not_ready*/,
          true/*major_sstable_only*/,
          need_split_src_table,
          need_split_dst_table))) {
        LOG_WARN("get table iterator fail", K(ret), K_(tablet_read_tables), KP_(access_param));
      }
    } else if (OB_FAIL(tablet_read_tables_->tablet_iter_.refresh_read_tables_from_tablet(
  ...
  if (OB_SUCC(ret)) {
    if (OB_FAIL(prepare_tables_from_iterator(
        *tablet_read_tables_->tablet_iter_.table_iter(),
        !is_mds_query && nullptr != tablet_read_tables_->tablet_iter_.get_split_extra_tablet_handles_ptr(),
        &tablet_read_tables_->sample_info_))) {
      LOG_WARN("failed to prepare tables from iter", K(ret), K(tablet_read_tables_->tablet_iter_.table_iter()));
    }
#ifdef ENABLE_DEBUG_LOG
    if (OB_SUCC(ret) && access_param_->iter_param_.is_tablet_spliting() && GCONF.enable_defensive_check()) {
      if (OB_FAIL(ObPartitionSplitQuery::check_split_prepare_read_tables(access_param_->iter_param_, *tablet_read_tables_->tablet_iter_.get_tablet()))) {
        LOG_WARN("failed to check split prepare read tables", K(ret));
      }
    }
#endif
  }
  LOG_DEBUG("prepare read tables", K(ret), K(refresh), K_(tablet_read_tables), K_(tables), K_(major_table_version));
  return ret;
}
...
int ObMultipleMerge::prepare_tables_from_iterator(ObTableStoreIterator &table_iter, const bool has_split_extra_tables, const common::SampleInfo *sample_info)
{
  ...
  if (OB_UNLIKELY(table_iter.count() == 0)) {
    // skip
  } else if (OB_FAIL(init_major_version(table_iter))) {
    LOG_WARN("failed to init major version", K(ret));
  } else if ((access_param_->iter_param_.need_truncate_filter() || access_param_->iter_param_.need_ttl_filter() || access_ctx_->mds_filter_mgr_ != nullptr)
             && OB_FAIL(prepare_mds_filter())) {
  ...
  while (OB_SUCC(ret)) {
    ...
    if (OB_FAIL(table_iter.get_next(table_ptr))) {
      ...
      if (SampleInfo::SAMPLE_ALL_DATA == sample_info->scope_) {
        ...
        need_table = table_ptr->is_major_type_sstable();
      } else if (SampleInfo::SAMPLE_INCR_DATA == sample_info->scope_) {
        need_table = !table_ptr->is_major_type_sstable();
      }
    }
    if (OB_SUCC(ret) && need_table) {
      if (OB_FAIL(check_table_need_read(table_ptr, need_table))) {
        LOG_WARN("fail to check table need read", K(ret), KPC(table_ptr));
      }
    }
    if (OB_SUCC(ret) && need_table) {
      ...
      } else if (table_ptr->is_inc_major_related_sstable()) {
  ...
  if (OB_SUCC(ret)) {
    ...
    } else if (tables_.count() > common::MAX_TABLE_CNT_IN_STORAGE + common::MAX_INC_MAJOR_SSTABLE_CNT) {
      ret = OB_ERR_UNEXPECTED;
...
int ObMultipleMerge::refresh_table_on_demand()
{
  ...
  if (OB_UNLIKELY(!inited_)) {
    ret = OB_NOT_INIT;
    STORAGE_LOG(WARN, "ObMultipleMerge has not been inited", K(ret));
  } else if (RefreshTableState::NONE != refresh_table_state_) {
    // already in drain refresh table process, skip check
    if (OB_FAIL(switch_refresh_table_state_on_demand())) {
      STORAGE_LOG(WARN, "fail to switch refresh table state on demand", K(ret), K(refresh_table_state_));
    }
  } else if (OB_FAIL(check_need_refresh_table(need_refresh, need_retry))) {
    STORAGE_LOG(WARN, "fail to check need refresh table", K(ret));
  } else if (need_retry) {
    ret = OB_SNAPSHOT_DISCARDED;
    FLOG_INFO("long time in level order scan, retry to scan", K(ret));
    ...
    if (nullptr != block_row_store_
      ...
      if (need_scan_di_base_) {
        ...
        bool is_di_base_changed = true;
        if (OB_FAIL(di_base_sstable_row_scanner_->stash_tablet_iter(tablet_read_tables_->tablet_iter_, tables_))) {
          STORAGE_LOG(WARN, "fail to stash tablet iterator", K(ret));
          ...
          STORAGE_LOG(WARN, "fail to prepare read tables", K(ret));
        } else if (di_base_table_cnt > 1) {
          // not support fast refresh table for multi di base
          fast_refresh_table = false;
        } else if (OB_FAIL(di_base_sstable_row_scanner_->check_di_base_changed(tables_, is_di_base_changed))) {
          STORAGE_LOG(WARN, "di base snapshot version changed", K(ret));
        } else {
          // fast refresh only when single di base is unchanged
          // prohibit tablet refresh in do_refresh_table: a second refresh may load newer
          // tables and make di base changed, invalidating the fast path
          fast_refresh_table = !is_di_base_changed;
          can_refresh_tablet = is_di_base_changed;
        }
      }
      if (OB_FAIL(ret)) {
        ...
        if (OB_FAIL(switch_refresh_table_state(RefreshTableState::HOLD_DRAIN))) {
          STORAGE_LOG(WARN, "fail to switch refresh table state", K(ret));
        }
      }
    }
  }
...
int ObMultipleMerge::do_refresh_table(const bool can_refresh_tablet)
{
  int ret = OB_SUCCESS;
  if (FALSE_IT(access_param_->iter_param_.set_use_stmt_iter_pool())) {
  ...
  } else if (!can_refresh_tablet) {
    // can not refresh tablet, avoid di base changed
  } else if (tablet_read_tables_->tablet_iter_.is_valid() &&
             !tablet_read_tables_->tablet_iter_.table_iter()->check_store_expire()) {
    // tablet has been refreshed and store not expired, skip refresh tablet
    access_param_->iter_param_.rowkey_read_info_ =
      &(tablet_read_tables_->tablet_iter_.get_tablet()->get_rowkey_read_info());
  } else if (OB_FAIL(refresh_tablet_iter())) {
    STORAGE_LOG(WARN, "fail to refresh tablet iter", K(ret));
  } else if (OB_FAIL(prepare_read_tables(true/*refresh*/))) {
    STORAGE_LOG(WARN, "fail to prepare read tables", K(ret));
  ...
  if (OB_FAIL(ret)) {
    ...
    di_base_sstable_row_scanner_->clear_stash();
    // need switch param, because di base may changed
    if (OB_FAIL(init_di_base_sstable_row_scanner())) {
      STORAGE_LOG(WARN, "fail to init di base sstable row scanner", K(ret));
    }
  }
  if (OB_FAIL(ret)) {
  ...
  if (OB_FAIL(ret)) {
    ...
    FLOG_INFO("table refreshed", "tablet_id", access_param_->iter_param_.tablet_id_,
      K(curr_scan_index_), K(scan_state_), K(refresh_table_state_), K(major_table_version_), KPC(di_base_sstable_row_scanner_));
  }
  return ret;
}

int ObMultipleMerge::switch_refresh_table_state_on_demand()
{
  int ret = OB_SUCCESS;
  switch (refresh_table_state_) {
    case RefreshTableState::HOLD_DRAIN:
      // check if time threshold is reached, if reached, switch to clone drain
      if (ObTimeUtility::current_time_ns() - start_time_ns_ > RETRY_QUERY_THRESHOLD_NS) {
        if (OB_FAIL(switch_refresh_table_state(RefreshTableState::CLONE_DRAIN))) {
          STORAGE_LOG(WARN, "fail to switch refresh table state", K(ret));
        }
      }
      break;
    case RefreshTableState::CLONE_DRAIN:
      // check new tablet is expired, if expired, reset tablet iter
      if (tablet_read_tables_->tablet_iter_.is_valid() &&
          tablet_read_tables_->tablet_iter_.table_iter()->check_store_expire()) {
        tablet_read_tables_->tablet_iter_.reset();
      }
      break;
    case RefreshTableState::SUSPEND_REFRESH:
      if (OB_FAIL(switch_refresh_table_state(RefreshTableState::NONE))) {
        STORAGE_LOG(WARN, "fail to switch refresh table state", K(ret));
      }
      break;
 ...
 *      │                                        │
 *      ▼                                        │
 *  HOLD_DRAIN ── drain end ────────────────────►┤
 *      │                                        │
 *      │ time reached                           │
 *      │                                        │
 *      ▼                                        │
 * CLONE_DRAIN ── drain end ────────────────────►┘
 *
 */
  ...
  switch (refresh_table_state) {
    case RefreshTableState::NONE:
      ...
      if (OB_UNLIKELY(RefreshTableState::HOLD_DRAIN != refresh_table_state_ &&
                      RefreshTableState::CLONE_DRAIN != refresh_table_state_ &&
                      RefreshTableState::SUSPEND_REFRESH != refresh_table_state_)) {
        ret = OB_ERR_UNEXPECTED;
        LOG_WARN("unexpected refresh table state", K(ret), K(refresh_table_state_), K(refresh_table_state));
      } else if (OB_FAIL(do_refresh_table())) {
        LOG_WARN("fail to refresh table", K(ret));
      } else {
        start_time_ns_ = 0;
      }
      break;
    case RefreshTableState::HOLD_DRAIN:
      // continue DI_BASE scan with the stashed old tablet until time reach threshold
      if (OB_UNLIKELY(RefreshTableState::NONE != refresh_table_state_)) {
        ret = OB_ERR_UNEXPECTED;
        LOG_WARN("unexpected refresh table state", K(ret), K(refresh_table_state_), K(refresh_table_state));
      } else {
        start_time_ns_ = ObTimeUtility::current_time_ns();
        access_param_->iter_param_.rowkey_read_info_ = di_base_sstable_row_scanner_->get_rowkey_read_info();
        if (nullptr != block_row_store_ && OB_FAIL(block_row_store_->reuse_for_refresh_table())) {
          LOG_WARN("fail to reuse block row store", K(ret));
        } else if (ScanState::SINGLE_ROW == scan_state_) {
          if (OB_UNLIKELY(!is_unprojected_row_valid_)) {
            ret = OB_ERR_UNEXPECTED;
            LOG_WARN("unexpected unprojected row invalid", K(ret));
          } else if (OB_FAIL(di_base_sstable_row_scanner_->prepare_di_base_blockscan(false/*di_base_only*/, &unprojected_row_))) {
            LOG_WARN("fail to prepare di base blockscan", K(ret));
          } else {
            scan_state_ = ScanState::DI_BASE;
          }
      ...
      break;
    case RefreshTableState::CLONE_DRAIN:
      // clone tablet and continue DI_BASE scan to the border
      if (OB_UNLIKELY(RefreshTableState::HOLD_DRAIN != refresh_table_state_)) {
        ret = OB_ERR_UNEXPECTED;
        LOG_WARN("unexpected refresh table state", K(ret), K(refresh_table_state_), K(refresh_table_state));
        ...
        FLOG_INFO("clone sstable drain di base", "tablet_id", access_param_->iter_param_.tablet_id_,
          K(curr_scan_index_), K(scan_state_), K(refresh_table_state_), K(major_table_version_), KPC(di_base_sstable_row_scanner_));
      }
      break;
    case RefreshTableState::SUSPEND_REFRESH:
      // drain end with pending batch in vector_store, defer refresh table until it is returned
      if (OB_UNLIKELY(RefreshTableState::HOLD_DRAIN != refresh_table_state_ &&
                      RefreshTableState::CLONE_DRAIN != refresh_table_state_)) {
        ret = OB_ERR_UNEXPECTED;
        LOG_WARN("unexpected refresh table state", K(ret), K(refresh_table_state_), K(refresh_table_state));
      } else {
        FLOG_INFO("drain di base end, suspend refresh", "tablet_id", access_param_->iter_param_.tablet_id_,
          K(curr_scan_index_), K(scan_state_), K(refresh_table_state_), K(major_table_version_), KPC(di_base_sstable_row_scanner_));
      }
      break;
    default:
...
int ObMultipleMerge::refresh_tablet_iter()
{
  ...
  const common::ObTabletID &tablet_id = access_ctx_->tablet_id_;
  ...
  if (OB_UNLIKELY(remain_timeout <= 0)) {
    ...
    LOG_WARN("failed to refresh tablet iterator", K(ret), K(ls_id), K_(tablet_read_tables), KP_(access_param));
  } else {
    tablet_read_tables_->refreshed_merge_ = this;
    access_param_->iter_param_.rowkey_read_info_ =
      &(tablet_read_tables_->tablet_iter_.get_tablet_handle().get_obj()->get_rowkey_read_info());
  }
  return ret;
}
...
OB_INLINE void ObMultipleMerge::reuse_lob_locator()
{
  if (NULL != access_ctx_->lob_locator_helper_) {
...
OB_INLINE void ObMultipleMerge::set_base_version() const
{
...
{
  return tablet_read_tables_->sample_info_.is_no_sample() ?
         access_ctx_->store_ctx_->mvcc_acc_ctx_.get_snapshot_version().get_val_for_tx() : INT64_MAX;
}

int ObMultipleMerge::check_table_need_read(const ObITable *table, bool &need_table) const
{
  int ret = OB_SUCCESS;
  need_table = true;
  ...
  if (table->is_inc_major_related_sstable()) {
    ...
    if (OB_ISNULL(sstable)) {
      ret = OB_ERR_UNEXPECTED;
      LOG_WARN("get unexpected table", K(ret), KPC(table));
...
int ObMultipleMerge::prepare_mds_filter()
{
  int ret = OB_SUCCESS;
  ...
  if (OB_ISNULL(access_param_) || OB_ISNULL(access_ctx_) || OB_ISNULL(access_ctx_->stmt_allocator_)) {
    ...
    mds_filter_flags
        .add_type_if(access_param_->iter_param_.need_truncate_filter(), ObMDSFilterFlags::MDSFilterType::TRUNCATE_FILTER)
    ...
    const ObIArray<ObTabletHandle> *split_extra_tablet_handles
        = access_param_->iter_param_.is_tablet_spliting()
              ? tablet_read_tables_->tablet_iter_.get_split_extra_tablet_handles_ptr()
              : nullptr;
...
int ObMultipleMerge::build_iter_array_for_all_tables()
{
  int ret = OB_SUCCESS;

  if (OB_UNLIKELY((iters_.count() > 0 || get_di_base_iter_cnt() > 0) && (iters_.count() + get_di_base_iter_cnt() > tables_.count()))) {
    ret = OB_ERR_UNEXPECTED;
    LOG_WARN("iter cnt is not equal to table cnt", K(ret), K(iters_.count()), K(get_di_base_iter_cnt()), K(tables_), KPC(di_base_sstable_row_scanner_));
  ...
  } else if (get_di_base_table_cnt() > 0 && OB_FAIL(di_base_sstable_row_scanner_->construct_iters())) {
    LOG_WARN("fail to construct di base iters", K(ret));
  } else if (tables_.count() > get_di_base_table_cnt()) {
    LOG_TRACE("construct iters begin", K(tables_.count()), K(iters_.count()), K_(access_ctx_->trans_version_range), K_(tables), KPC_(access_param), KPC_(di_base_sstable_row_scanner));
    ObITable *table = NULL;
    ObStoreRowIterator *iter = NULL;
    ...
    for (int64_t i = table_cnt; OB_SUCC(ret) && i >= get_di_base_table_cnt(); --i) {
      if (OB_FAIL(tables_.at(i, table))) {
        ...
        if (OB_FAIL(build_iter(table, iter_param, iter))) {
      ...
      } else if (OB_FAIL(init_iter(table, iter_param, iter))) {
...
int ObMultipleMerge::init_di_base_sstable_row_scanner()
{
  int ret = OB_SUCCESS;
  const bool is_multi_scan = type_ == ObQRIterType::T_MULTI_SCAN || type_ == ObQRIterType::T_LEVEL_ORDER_MULTI_SCAN;
  // TODO: zhanghuidong.zhd, reuse di base sstable row scanner
  if (nullptr == di_base_sstable_row_scanner_) {
    void *buf = nullptr;
    if (OB_ISNULL(buf = access_ctx_->stmt_allocator_->alloc(sizeof(ObDIBaseSSTableRowScanner)))) {
      ret = OB_ALLOCATE_MEMORY_FAILED;
      LOG_WARN("fail to allocate memory", K(ret));
    } else {
      di_base_sstable_row_scanner_ = new (buf) ObDIBaseSSTableRowScanner();
    }
  }
  if (FAILEDx(di_base_sstable_row_scanner_->switch_param(access_param_, access_ctx_, tables_, is_multi_scan))) {
    LOG_WARN("fail to switch param di base sstable row scanner", K(ret));
  } else {
    // di base maybe changed after refresh table, reset need_scan_di_base
    need_scan_di_base_ = get_di_base_table_cnt() > 0;
  }
  return ret;
}
...
OB_INLINE int ObMultipleMerge::check_need_refresh_table(bool &need_refresh, bool &need_retry)
{
  ...
  need_refresh = false;
  need_retry = false;
  if (tablet_read_tables_->sample_info_.is_block_sample() ||
      tablet_read_tables_->sample_info_.is_ddl_block_sample() ||
    ...
    STORAGE_LOG(DEBUG, "skip refresh table for block sample, aggregated in prefetch or group by pushdown",
        K(tablet_read_tables_->sample_info_.is_block_sample()),
        K(tablet_read_tables_->sample_info_.is_ddl_block_sample()),
        KPC(block_row_store_));
  } else if (access_param_->iter_param_.is_mds_query_) {
    // skip refresh table for mds query
  } else if (tablet_read_tables_->tablet_iter_.table_iter()->check_store_expire() || OB_SUCCESS != tmp_ret) {
    if (type_ == T_LEVEL_ORDER_SCAN || type_ == T_LEVEL_ORDER_MULTI_SCAN) {
...
OB_INLINE int ObMultipleMerge::init_major_version(ObTableStoreIterator &table_iter)
{
  int ret = OB_SUCCESS;
...
int ObMultipleMerge::calc_scan_range_by_rowkey(ObTableAccessContext *access_ctx,
                                               const ObDatumRange *&range,
                                               ObDatumRange &cow_range,
                                               const ObDatumRowkey &curr_rowkey,
                                               const bool calc_di_base_range)
{
  ...
  if (curr_rowkey.is_valid()) {
    ...
    cow_range.change_boundary(curr_rowkey, access_ctx->query_flag_.is_reverse_scan(), calc_di_base_range);
    ...
    if (access_ctx->query_flag_.is_reverse_scan() && curr_rowkey.is_min_rowkey()) {
      cow_range.start_key_.set_min_rowkey();
    } else if (!access_ctx->query_flag_.is_reverse_scan() && curr_rowkey.is_max_rowkey()) {
...
int ObMultipleMerge::calc_scan_range_by_rowkey(ObTableAccessParam *access_param,
                                               ObTableAccessContext *access_ctx,
                                               const ObIArray<ObDatumRange> *&ranges,
                                               ObIArray<ObDatumRange> &cow_ranges,
                                               ...
                                               const bool calc_di_base_range)
{
  ...
  } else if (NULL == access_param || NULL == access_ctx) {
    ret = OB_ERR_UNEXPECTED;
    STORAGE_LOG(WARN, "multiple multi scan merge not inited", K(ret), KP(access_param), KP(access_ctx));
  } else if (OB_ISNULL(ranges)) {
    ret = OB_ERR_UNEXPECTED;
    STORAGE_LOG(WARN, "ranges is NULL", K(ret));
  } else if (OB_ISNULL(read_info = access_param->iter_param_.get_read_info())) {
    ret = OB_ERR_UNEXPECTED;
    STORAGE_LOG(WARN, "Unexpected null read info", K(ret));
  } else {
    ObSEArray<ObDatumRange, 32> tmp_ranges;
    if (OB_FAIL(tmp_ranges.assign(*ranges))) {
      STORAGE_LOG(WARN, "fail to assign ranges", K(ret));
    }
        ...
        if (curr_scan_index == i) {
          ...
          if (OB_FAIL(range_key.compare(curr_rowkey, read_info->get_datum_utils(), cmp_ret))) {
            ...
            // notice, ranges should be pushed in when curr_rowkey is min/max
                     ...
                     (calc_di_base_range && cmp_ret == 0) || // right border is closed in di base
                     (((curr_scan_index + 1) == r) && access_param->iter_param_.is_delete_insert_)) {
            range.change_boundary(curr_rowkey, is_reverse_scan, calc_di_base_range);
            // As memtable will use reverse scan when start rowkey is greater than end rowkey instead of
            // empty result, make the range correct
            if (is_reverse_scan && curr_rowkey.is_min_rowkey()) {
              range.start_key_.set_min_rowkey();
            } else if (!is_reverse_scan && curr_rowkey.is_max_rowkey()) {
