...
template <int64_t STACK_QUEUE_SIZE>
inline void MdsTableHighPriorityFlusher<STACK_QUEUE_SIZE>::flush_by_order(MdsTableMap &map,
                                                                          share::SCN limit,
                                                                          share::SCN max_decided_scn,
                                                                          const int64_t trace_id) {
  int ret = OB_SUCCESS;
  int64_t flush_count = 0;
  FlushOp op(limit, flush_count, max_decided_scn, trace_id);
...
void MdsTableOrderFlusher<STACK_QUEUE_SIZE, true>::flush_by_order(MdsTableMap &map,
  ...
  int64_t flush_count = 0;
  FlushOp op(limit, flush_count, max_decided_scn, trace_id);
  high_priority_flusher_.flush_by_order(map, limit, max_decided_scn, trace_id);
