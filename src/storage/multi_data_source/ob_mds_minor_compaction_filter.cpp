    ...
    ObFilterRet &filter_ret)
{
  ...
  constexpr uint8_t tablet_truncate_mds_unit_id = mds::TupleTypeIdx<mds::NormalMdsTable, mds::MdsUnit<mds::DummyKey, ObTabletTruncateMdsUserData>>::value;
  ...
  } else if (tablet_truncate_mds_unit_id == kv_adapter.get_type())  {
    ret = filter_tablet_truncate_data(row, kv_adapter, filter_ret);
  } else {
    filter_ret = FILTER_RET_NOT_CHANGE;
    LOG_DEBUG("not medium info/truncate info/tablet truncate data", K(ret), K(row), K_(last_major_snapshot), K_(truncate_filter_snapshot), K(kv_adapter));
  }
...
int ObMdsMinorFilter::filter_tablet_truncate_data(
  const blocksstable::ObDatumRow &row,
  const mds::MdsDumpKVStorageAdapter &kv_adapter,
  ObFilterRet &filter_ret)
{
  ...
  if (OB_UNLIKELY(row.is_uncommitted_row()
    ...
    ret = OB_INVALID_ARGUMENT;
    LOG_WARN("uncommitted row or uncompacted row in mds table", K(ret), K(row));
  } else if (OB_FAIL(truncate_data.deserialize(kv_adapter.get_user_data().ptr(), kv_adapter.get_user_data().length(), pos))) {
    LOG_WARN("fail to deserialize truncate info", K(ret), K(kv_adapter));
  } else if (!row.is_first_multi_version_row() || !row.is_last_multi_version_row()) {
    filter_ret = FILTER_RET_NOT_CHANGE;
    LOG_DEBUG("skip filtering multi-version tablet truncate mds data", K(ret), K(row),
      K_(clog_checkpoint_scn), K_(last_major_snapshot), K(truncate_data), K(kv_adapter));
  } else if (truncate_data.truncate_commit_scn_ < clog_checkpoint_scn_
             && truncate_data.truncate_commit_version_ < last_major_snapshot_) {
    filter_ret = FILTER_RET_REMOVE;
    // TODO(fanyin): change into debug log later
    LOG_INFO("tablet truncate mds data is filtered", K(ret), K(filter_ret), K(row),
      K_(clog_checkpoint_scn), K_(last_major_snapshot), K(truncate_data), K(kv_adapter));
  } else {
    filter_ret = FILTER_RET_NOT_CHANGE;
  }
