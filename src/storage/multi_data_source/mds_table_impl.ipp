...
struct GetLatestHelper {
  ...
  struct InnerHelper<DummyKey, V> {
    ...
    int operator()(const MdsUnit<DummyKey, V> &mds_unit) {
      return mds_unit.get_latest(ReadNodeOp(helper_), helper_.read_seq_);
    }
    GetLatestHelper &helper_;
  };
  template <typename K, typename V>
...
struct IsLockedByOthersHelper {
  ...
  struct InnerHelper {
    ...
    int operator()(const MdsUnit<K, V> &mds_unit) {
      return mds_unit.get_latest(*reinterpret_cast<K*>(helper_.key_), ReadNodeOp(helper_), 0);
    }
    IsLockedByOthersHelper &helper_;
  };
  template <typename V>
  struct InnerHelper<DummyKey, V> {
    InnerHelper(IsLockedByOthersHelper &helper) : helper_(helper) {}
    int operator()(const MdsUnit<DummyKey, V> &mds_unit) { return mds_unit.get_latest(ReadNodeOp(helper_), 0); }
    IsLockedByOthersHelper &helper_;
  };
  template <typename K, typename V>
  int operator()(const MdsUnit<K, V> &mds_unit) {
...
int MdsTableImpl<MdsTableType>::for_each_unit_from_small_key_to_big_from_old_node_to_new_to_dump(
  ...
  } else if (MDS_FAIL(const_cast<MdsTableImpl<MdsTableType>*>(this)->for_each_unit_from_small_key_to_big_from_old_node_to_new_to_dump(
    for_each_op, mds_construct_sequence, for_flush
  ))) {
    MDS_LOG(WARN, "mds table switch state failed", KR(ret), K(*this));
  }
  return ret;
  ...
  if (MDS_FAIL(ret)) {
    ...
    do {
      ...
      if (MDS_FAIL(for_each_scan_row(FowEachRowAction::CALCULATE_REC_SCN, op))) {// lock all rows failed, retry until lock all rows success
        ...
        if (max_aborted_scn_.atomic_load() > flush_scn) {// can only be advanced to flush scn cause must consider about aborted scn between flush scn and max_aborted_scn_
          MDS_LOG_FLUSH(INFO, "max_aborted_scn_on_mds_table is greater than flush scn, rec_scn can only be advanced to flush_scn");
          new_rec_scn = share::SCN::scn_inc(flush_scn);
        }
...
struct ForcelyReleaseAllNodeOp
{
  template<typename K, typename V>
  struct ForEachOp {
    ForEachOp(ForcelyReleaseAllNodeOp &op, const MdsRow<K, V> &row) : op_(op), row_(row) {}
    bool operator()(const UserMdsNode<K, V> &mds_node) {
      UserMdsNode<K, V> &cast_node = const_cast<UserMdsNode<K, V> &>(mds_node);
      if (!cast_node.is_committed_()) {
        MDS_LOG_RET(INFO, OB_SUCCESS, "release uncommitted node");
      }
      const_cast<MdsRow<K, V> &>(row_).sorted_list_.del((ListNodeBase*)(ListNode<UserMdsNode<K, V>>*)(&cast_node));
      MdsFactory::destroy(&cast_node);
      return false;
    }
    ForcelyReleaseAllNodeOp &op_;
    const MdsRow<K, V> &row_;
  };
