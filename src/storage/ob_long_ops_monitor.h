...
struct ObILongOpsTaskStat
{
  ...
  ObILongOpsTaskStat()
    : task_id_(0), cpu_cost_(0), io_cost_(0), state_(TaskState::INIT), type_(TaskType::INVALID)
  {}
  virtual ~ObILongOpsTaskStat() = default;
  virtual int estimate_cost() = 0;
  double get_cost() { return cpu_cost_ + io_cost_; }
  bool is_valid() const
  {
    return task_id_ >= 0;
  }
...
{
  ObCreateIndexScanTaskStat()
    : macro_count_(0)
  {}
  virtual ~ObCreateIndexScanTaskStat() = default;
  virtual int estimate_cost() override { io_cost_ = static_cast<double>(macro_count_); return common::OB_SUCCESS; }
...
{
  ObCreateIndexSortTaskStat()
    : macro_count_(0), run_count_(0)
  {}
  virtual ~ObCreateIndexSortTaskStat() {}
  virtual int estimate_cost() override { io_cost_ = static_cast<double>(run_count_) * static_cast<double>(macro_count_); return common::OB_SUCCESS; }
  virtual int assign(const ObILongOpsTaskStat &task_stat) override;
...
{
  ObCreateIndexKey()
    : index_table_id_(common::OB_INVALID_ID), partition_id_(common::OB_INVALID_ID)
  {}
  virtual ~ObCreateIndexKey() {}
  virtual int to_key_string() override;
  bool is_valid() const
  {
    return common::OB_INVALID_ID != index_table_id_
...
struct ObCommonOpsStatValue
{
  ...
  void reset();
  TO_STRING_KV(K_(start_time), K_(finish_time), K_(elapsed_time), K_(remaining_time),
      K_(percentage), K_(last_update_time));
  int64_t start_time_;
...
struct ObILongOpsStat
{
  ...
  virtual int estimate_cost() = 0;
  virtual int64_t get_deep_copy_size() const { return sizeof(*this); }
...
struct ObCreateIndexPartitionStat : public ObILongOpsStat
{
  ...
  Type get_type() const override { return ObILongOpsStat::Type::CREATE_INDEX; }
  virtual bool is_valid() const override
  {
    return key_.is_valid();
  }
  virtual int check_can_purge(bool &can_purge) override;
  const ObILongOpsKey &get_key() const override { return key_; }
