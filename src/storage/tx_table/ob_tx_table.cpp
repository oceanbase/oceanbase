...
int ObTxTable::get_ss_recycle_scn(share::SCN &ss_recycle_scn)
{
  ...
  } else if (FALSE_IT(ss_recycle_scn = recycle_scn_from_all_cn_)) {
  ...
  FLOG_INFO("finish get ss_recycle_scn", K(ret), K(recycle_scn_from_all_cn_), K(ss_recycle_scn), K(ls_id_));
  return ret;
}
...
void ObTxTable::CollectRecycleSCNTask::runTimerTask()
{
  ...
  if (OB_FAIL(do_collect_recycle_scn_(min_tx_recycle_scn))) {
    LOG_WARN("do collect recycle scn failed", KR(ret));
  } else if (OB_FAIL(do_collect_recycle_scn_(min_tx_recycle_scn_again))) {
    LOG_WARN("do collect recycle scn failed", KR(ret));
  }
  ...
  if (OB_SUCC(ret)) {
    if (min_tx_recycle_scn.is_valid_and_not_min() && !min_tx_recycle_scn.is_max() &&
        ...
        min_tx_recycle_scn > host_tx_table_.get_recycle_scn_from_all_cn()) {
 ...
 *
 */
int ObTxTable::CollectRecycleSCNTask::do_collect_recycle_scn_(share::SCN &min_tx_recycle_scn)
{
  int ret = OB_SUCCESS;
  ...
  if (OB_FAIL(SVR_TRACER.get_all_tenant_servers(tenant_id, all_servers, renew_time))) {
    ...
    ObSSRpcCollectCNTxRecycleSCNArg arg(tenant_id, ls_id);
    for (int i = 0; i < all_servers.count() && OB_SUCC(ret); ++i) {
      ...
      if (OB_FAIL(SVR_TRACER.check_server_permanent_offline(server, is_permanet_offline))) {
        ...
        STORAGE_LOG(INFO, "finish get tx_recycle_scn from one compute node", K(ls_id), K(server), K(result));
      }
    }
  }
  return ret;
