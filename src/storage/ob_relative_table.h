...
public:
  ...
  ObRelativeTable()
      : tablet_iter_(), allow_not_ready_(false), schema_param_(nullptr), mds_filter_mgr_(nullptr),
  ...
  int prepare_mds_filter(common::ObIAllocator &allocator,
                         const int64_t read_snapshot,
                         const bool need_truncate_filter);
  ObMDSFilterMgr *get_mds_filter_mgr() const { return mds_filter_mgr_; }
