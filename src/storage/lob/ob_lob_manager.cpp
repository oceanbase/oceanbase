  ...
  if (OB_UNLIKELY(!is_inited_)) {
    ...
    if (OB_FAIL(ret)) {
      ...
      if (OB_FAIL(init_out_row_ctx(param, data.length(), param.op_type_))) {
        ...
        if (!param.tablet_id_.is_inner_tablet() && param.byte_size_ == 0) {
          // only insert
          // mock iter as row iter
          ObLobMetaWriteRowIter mock_write_iter(&param, &iter);
          if (OB_FAIL(lob_ctx.lob_meta_mngr_->batch_write(param, &mock_write_iter))) {
            LOG_WARN("write lob meta row failed.", K(ret));
          }
        } else {
