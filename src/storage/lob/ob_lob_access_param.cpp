...
int ObLobAccessParam::init_seq_no(const uint64_t modified_len)
{
  ...
  if (OB_ISNULL(tx_desc_)) {
    ret = OB_INVALID_ARGUMENT;LOG_WARN("tx desc is null", K(ret), KPC(this));
    LOG_WARN("tx desc is null", K(ret), KPC(this));
  } else if (this->seq_no_st_.is_valid()) {
    ret = OB_INVALID_ARGUMENT;
