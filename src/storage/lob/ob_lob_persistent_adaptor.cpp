...
int ObPersistentLobApator::init_table_param()
{
  ...
  HEAP_VAR(ObTableSchema, meta_schema, &tmp_allocator) {
    ...
    } else if (OB_FAIL(meta_table_param_->convert(meta_schema, meta_column_ids))) {
...
{
  int ret = OB_SUCCESS;
  if (OB_FAIL(build_lob_meta_table_dml(param))) {
...
int ObPersistentLobApator::build_lob_meta_table_dml(ObLobAccessParam& param)
{
  int ret = OB_SUCCESS;
    ...
    ObTabletHandle &handle)
{
  ...
  if (OB_FAIL(MTL(ObLSService *)->get_ls(param.ls_id_, ls_handle, ObLSGetMod::STORAGE_MOD, ObLSAccessAttr::DISABLE_LOGONLY))) {
...
int ObPersistentLobApator::prepare_lob_tablet_id(ObLobAccessParam& param)
{
  ...
  } else if (OB_FAIL(MTL(ObLSService *)->get_ls(param.ls_id_, ls_handle, ObLSGetMod::STORAGE_MOD, ObLSAccessAttr::DISABLE_LOGONLY))) {
    LOG_WARN("failed to get log stream", K(ret), K(param.ls_id_));
  } else if (OB_ISNULL(ls_handle.get_ls())) {
    ret = OB_ERR_UNEXPECTED;
