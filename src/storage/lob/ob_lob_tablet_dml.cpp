    ...
    blocksstable::ObStorageDatum &datum)
{
  ...
  if (OB_FAIL(prepare_lob_write(run_ctx, row, column, row_idx, col_idx, old_disk_locator, datum, need_do_write))) {
    LOG_WARN("prepare_lob_write fail", K(ret), K(column), K(row_idx), K(col_idx), K(datum), K(row));
  } else if (! need_do_write) {
  } else if(OB_FAIL(insert_lob_col(run_ctx, row, column, datum, nullptr, old_disk_locator/*empty data*/))) {
    LOG_WARN("insert_lob_col fail", K(ret), K(column), K(row_idx), K(col_idx), K(datum), K(row));
  }
  return ret;
}
    ...
    blocksstable::ObStorageDatum &new_datum)
{
  ...
  if (OB_FAIL(ObLobTabletDmlHelper::prepare_lob_write(run_ctx, new_row, column, row_idx, col_idx, old_disk_locator, new_datum, need_do_write))) {
    LOG_WARN("prepare_lob_write fail", K(ret), K(column), K(row_idx), K(col_idx), K(new_datum));
  } else if (! need_do_write) {
  } else if (OB_FAIL(ObLobTabletDmlHelper::update_lob_col(run_ctx, old_row, new_row, data_tbl_rowkey_change, column, old_datum, new_datum))) {
    LOG_WARN("[STORAGE_LOB]failed to to update lob col", K(ret), K(column), K(row_idx), K(col_idx), K(new_datum));
  }
  return ret;
}
    ...
    ObArray<ObLobMetaInfo> *lob_meta_list)
{
  ...
  if (OB_ISNULL(lob_mngr)) {
    ...
    // do nothing
  } else if (OB_FAIL(build_common_lob_param_for_dml(run_ctx, data_row, column, disk_locator_data, lob_param))) {
    LOG_WARN("build_common_lob_param_for_dml fail", K(ret), K(column));
  } else {
    ...
    ObLobAccessParam &lob_param)
{
  ...
  if (OB_ISNULL(lob_mngr)) {
    ...
    if (data.length() < sizeof(ObLobCommon)) {
      ...
      if (OB_FAIL(build_common_lob_param_for_dml(run_ctx, data_row, column, dup_data, lob_param))) {
        LOG_WARN("build_common_lob_param_for_dml fail", K(ret), K(column), K(datum), K(data));
    ...
    blocksstable::ObStorageDatum &new_datum)
{
  ...
  } else if (OB_FAIL(delete_lob_col(run_ctx, old_row, column, old_datum, lob_common, lob_param))) {
    LOG_WARN("[STORAGE_LOB]failed to erase old lob col", K(ret), K(old_row));
  } else if (data_tbl_rowkey_change) {
    if (OB_FAIL(insert_lob_col(run_ctx, new_row, column, new_datum, nullptr, disk_locator_data/*empty data*/))) { // no need del_param
      LOG_WARN("[STORAGE_LOB]failed to insert new lob col.", K(ret), K(new_row));
    }
  } else if (OB_FALSE_IT(disk_locator_data.assign_ptr(reinterpret_cast<char*>(lob_common), lob_param.handle_size_))) {
  } else if (OB_FAIL(insert_lob_col(run_ctx, new_row, column, new_datum, &lob_param, disk_locator_data))) {
    LOG_WARN("[STORAGE_LOB]failed to insert new lob col.", K(ret), K(new_row));
  }
  return ret;
}
    ...
    blocksstable::ObStorageDatum &datum)
{
  ...
  if (OB_ISNULL(lob_mngr)) {
    ...
    // TODO copy old
    } else if (OB_FAIL(build_common_lob_param_for_dml(run_ctx, data_row, column, old_disk_lob, lob_param))) {
      LOG_WARN("build_common_lob_param_for_dml fail", K(ret), K(column), K(datum), K(old_disk_lob));
    ...
    bool &need_do_write)
{
  ...
  if (is_sys_table(run_ctx.relative_table_.get_table_id())) {
    ...
    need_do_write = true;
    LOG_DEBUG("before 4.3.4", K(data_version), K(column), K(row_idx), K(col_idx));
  } else {
    ...
    if (OB_FAIL(build_common_lob_param_for_dml(run_ctx, data_row, column, old_disk_locator, lob_param))) {
      LOG_WARN("build_common_lob_param_for_dml fail", K(ret), K(column), K(src_data_locator));
    } else if (OB_FAIL(lob_mngr->prepare_insert_task(lob_param, is_outrow, info))) {
      LOG_WARN("prepare_insert_task fail", K(ret), K(column), K(src_data_locator));
    } else if (is_outrow) {
