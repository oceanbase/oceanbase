...
int ObLobMetaIterator::get_next_row(ObLobMetaInfo &row)
{
  ...
  if (OB_ISNULL(row_iter_)) {
  ...
  } else if (OB_FAIL(row_iter_->get_next_row(new_row))) {
    if (ret != OB_ITER_END) {
    ...
    LOG_WARN("get meta info from row fail", K(ret), K(new_row), KPC(this));
  }
  return ret;
}
...
int ObLobPersistDeleteIter::get_next_row(ObNewRow *&row)
{
  ...
  } else if (OB_FAIL(param_->update_out_row_ctx(nullptr/*new_info*/, result_.info_/*new_info*/))) { // new row
    LOG_WARN("failed update checksum.", K(ret));
  } else if (OB_FAIL(param_->update_handle_data_size(&result_.info_/*new_info*/, nullptr/*new_info*/))) {
