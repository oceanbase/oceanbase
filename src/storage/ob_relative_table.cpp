...
int ObRelativeTable::prepare_mds_filter(common::ObIAllocator &allocator,
                                        ...
                                        const bool need_truncate_filter)
{
  ...
  if (IS_NOT_INIT) {
    ...
    // TODO(menglan): need base version filter should just depend has_merged_with_mds_info
    ObMDSFilterFlags mds_filter_flags;
    const bool need_ttl_filter = schema_param_ != nullptr && schema_param_->get_read_info().has_ttl_definition();
    const bool need_base_version_filter = (need_truncate_filter || need_ttl_filter) && (tablet != nullptr && tablet->has_merged_with_mds_info());
    mds_filter_flags
        .add_type_if(need_truncate_filter, ObMDSFilterFlags::MDSFilterType::TRUNCATE_FILTER)
        .add_type_if(need_ttl_filter, ObMDSFilterFlags::MDSFilterType::TTL_FILTER)
        .add_type_if(need_base_version_filter, ObMDSFilterFlags::MDSFilterType::BASE_VERSION_FILTER);

    if (OB_UNLIKELY(!tablet_iter_.table_iter()->is_valid())) {
      // empty tablet, no need to build filter
      LOG_DEBUG("[MDS FILTER INFO], empty tablet", KPC(tablet_iter_.table_iter()));
      ...
      if (OB_UNLIKELY(nullptr != table_ptr && table_ptr->is_major_sstable()
        ...
        LOG_DEBUG("[MDS FILTER INFO]", K(ret), K(read_version_range), KPC_(mds_filter_mgr));
      }
    }
  }
