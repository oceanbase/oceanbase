...
ObTmpFilePageBuddy::ObTmpFilePageBuddy()
  : max_cont_page_nums_(0), buf_(NULL), allocator_(NULL),
    is_inited_(false)
{
...
int ObTmpFilePageBuddy::init(common::ObIAllocator &allocator)
{
  ...
  if (OB_UNLIKELY(is_inited_)) {
    ...
    buf_ = reinterpret_cast<ObTmpFileArea *>(allocator_->alloc(sizeof(ObTmpFileArea)
        * (std::pow(2, MAX_ORDER) - std::pow(2, MIN_ORDER))));
    if (NULL == buf_) {
...
int ObTmpMacroBlock::get_wash_io_info(ObTmpBlockIOInfo &info)
{
  ...
  if (OB_UNLIKELY(!is_inited_)) {
    ...
    info.offset_ = 0;
    info.size_ = OB_TMP_FILE_STORE.get_block_size();
...
ObTmpTenantMacroBlockManager::ObTmpTenantMacroBlockManager()
  : allocator_(), blocks_(), is_inited_(false)
{
}
...
int ObTmpTenantFileStore::alloc(const int64_t dir_id, const uint64_t tenant_id, const int64_t size,
    ObTmpFileExtent &extent)
{
  ...
  if (OB_UNLIKELY(!is_inited_)) {
    ...
    if (alloc_size > max_cont_size_per_block) {
      ...
      if (OB_FAIL(tmp_mem_block_manager_.alloc_extent(dir_id, tenant_id, alloc_size,
          extent, free_blocks))) {
        if (OB_ITER_END == ret) {
          ret = OB_SUCCESS;
          if (free_blocks.count() > 0) {
            ...
            STORAGE_LOG(WARN, "cannot allocate a tmp macro block", K(ret), K(dir_id), K(tenant_id));
          } else if (OB_FAIL(tmp_mem_block_manager_.alloc_extent(dir_id,
                                                                tenant_id,
                                                                alloc_size,
                                                                extent,
                                                                free_blocks))) {
