    ...
    const int64_t storage_meta_cache_priority)
{
  ...
  if (OB_UNLIKELY(is_inited_)) {
  ...
  } else if (OB_FAIL(mds_info_cache_.init("mds_info_cache", MDS_INFO_KV_CACHE_PRIORITY))) {
    ...
    STORAGE_LOG(ERROR, "fail to init tablet split cache", K(ret));
  } else {
    is_inited_ = true;
  }
    ...
    const int64_t storage_meta_cache_priority)
{
  ...
  } else if (OB_FAIL(mds_info_cache_.set_priority(MDS_INFO_KV_CACHE_PRIORITY))) {
    STORAGE_LOG(ERROR, "fail to set priority for mds info cache", K(ret));
