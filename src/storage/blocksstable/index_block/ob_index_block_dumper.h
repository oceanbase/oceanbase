...
public:
  ...
  ~ObIndexBlockInfo();
  void reset();
  OB_INLINE bool is_valid() const { return empty() || in_mem() || in_disk(); }
  OB_INLINE bool empty() const
  {
    return micro_block_desc_ == nullptr && block_write_ctx_ == nullptr
        && micro_block_cnt_ == 0 && row_count_ == 0 && in_mem_ == false && need_rewrite_ == false
        && next_level_rows_list_ == nullptr && agg_info_ == nullptr;
  }
  OB_INLINE bool in_disk() const
  {
    return !in_mem_ && micro_block_desc_ == nullptr && agg_info_ == nullptr
  ...
  {
    return in_mem_ && !need_rewrite_ && micro_block_desc_ != nullptr
        && block_write_ctx_ == nullptr && next_level_rows_list_ == nullptr;
  }
  OB_INLINE bool not_need_load_block() const {
    return (!in_mem_ && next_level_rows_list_ != nullptr && agg_info_ == nullptr && !need_rewrite_) ||
        (in_mem_ && next_level_rows_list_ == nullptr && agg_info_ != nullptr && !need_rewrite_);
  }
...
public:
  ...
  ObIndexRowAggInfo *agg_info_;// with next level;
  int64_t micro_block_cnt_;
  int64_t row_count_;
  bool in_mem_;
  bool need_rewrite_;
  TO_STRING_KV(K_(in_mem), K_(need_rewrite), K_(row_count), K_(micro_block_cnt),
      KPC_(micro_block_desc), KPC_(agg_info), KPC_(block_write_ctx), KP_(next_level_rows_list));
private:
...
public:
  ...
  TO_STRING_KV(K_(is_inited), K_(row_count), K_(micro_block_cnt), K_(last_rowkey),
      ...
      KP_(container_store_desc), KP_(index_store_desc), KP_(callback));
protected:
...
protected:
  ...
  ObIMacroBlockFlushCallback *callback_;  // for macro writer
