...
int ObIndexBlockTreeCursor::drill_down()
{
  ...
  if (OB_FAIL(idx_row_parser_.get_header(idx_row_header))) {
    ...
    if (OB_FAIL(get_next_level_block(idx_row_header->get_macro_id(), *idx_row_header))) {
      LOG_WARN("Fail to get micro block data handle", K(ret), KPC(idx_row_header));
    ...
    bool &is_beyond_the_range)
{
  ...
  if (OB_UNLIKELY(!rowkey.is_valid())) {
    ret = OB_INVALID_ARGUMENT;
    LOG_WARN("Rowkey is not valid", K(ret), K(rowkey));
    ...
    int64_t &end_idx)
{
  ...
  if (OB_UNLIKELY(!range.is_valid())) {
    ...
    if (OB_FAIL(reader_->locate_range(range, true, true, begin_idx, end_idx, true))) {
      LOG_WARN("Fail to locate range with micro block reader", K(ret), K(range));
    }
  } else {
    bool equal = false;
    const ObIndexBlockDataHeader *idx_data_header = nullptr;
    ...
    const ObIndexBlockRowHeader &idx_row_header)
{
  ...
  if (OB_FAIL(ret)) {
    ...
    if (OB_UNLIKELY(OB_ENTRY_NOT_EXIST != ret)) {
      ...
      ObIMicroBlockCache::BaseBlockCache *base_cache = nullptr;
      ...
      } else if (OB_FAIL(ObMicroBlockHeader::deserialize_and_check_record(
          macro_handle.get_buffer(),
          idx_row_header.get_block_size(),
          MICRO_BLOCK_HEADER_MAGIC))) {
        LOG_WARN("Fail to check magic code in micro block header", K(ret));
...
int ObIndexBlockTreeCursor::read_next_level_row(const int64_t row_idx)
{
  ...
  if (curr_path_item_->is_block_transformed_) {
    ...
    } else if (OB_FAIL(idx_data_header->get_index_data(row_idx, idx_data_buf))) {
