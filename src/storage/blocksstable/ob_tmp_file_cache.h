...
public:
  ...
  int get_block(const ObTmpBlockCacheKey &key, ObTmpBlockValueHandle &handle);
  int alloc_buf(const ObTmpBlockCacheKey &key, ObTmpBlockValueHandle &handle);
  int alloc_extent(const int64_t dir_id, const uint64_t tenant_id, const int64_t size,
  ...
  int try_sync(const int64_t block_id);
  int try_wash(common::ObIArray<ObTmpMacroBlock *> &free_blocks);
  int add_macro_block(ObTmpMacroBlock *&t_mblk);
  int wait_write_io_finish();
  OB_INLINE bool check_need_wait_write() { return write_handles_.count() > 0; }
...
private:
  int get_macro_block(const int64_t dir_id, const uint64_t tenant_id, const int64_t page_nums,
      ObTmpMacroBlock *&t_mblk, common::ObIArray<ObTmpMacroBlock *> &free_blocks);
  int wash(const int64_t block_nums,
      common::ObIArray<ObTmpMacroBlock *> &free_blocks);
  int wash(ObTmpMacroBlock *wash_block, bool &is_empty);
  int wash_block(ObTmpMacroBlock *wash_block, bool &is_empty);
  int write_io(
      const ObTmpBlockIOInfo &io_info,
      ObMacroBlockHandle &handle);
  int refresh_dir_to_blk_map(const int64_t dir_id, const ObTmpMacroBlock *t_mblk);
  int erase_block_from_dir_map(const int64_t block_id);
