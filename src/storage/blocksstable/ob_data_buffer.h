...
template <bool>
struct BufferWriteWrap;
template <>
struct BufferWriteWrap<true> {
  template <typename T, typename U>
  int operator()(T& t, const U& u)
  {
    return t.write_serialize(u);
  }
};
template <>
struct BufferWriteWrap<false> {
  ...
  int operator()(T& t, const U& u)
  {
    return t.write_pod(u);
  }
};

template <bool>
struct BufferReadWrap;
template <>
struct BufferReadWrap<true> {
  template <typename T, typename U>
  int operator()(T& t, U& u)
  {
    return t.read_serialize(u);
  }
};
template <>
...
public:
  ...
  int write_pod(const T& value)
  {
    ...
    if (remain() >= (int64_t)sizeof(T)) {
      do_write(value);
    } else if (common::OB_SUCCESS == (ret = expand(static_cast<int64_t>(sizeof(T))))) {
