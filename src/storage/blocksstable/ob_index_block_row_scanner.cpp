    ...
    bool &can_blockscan)
{
  ...
  if (IS_NOT_INIT) {
    ...
    if (IndexFormat::RAW_DATA == index_format_) {
      ...
      } else if (OB_FAIL(last_endkey.compare(rowkey, index_read_info_->get_datum_utils(), cmp_ret, false))) {
        LOG_WARN("Fail to compare rowkey", K(ret), K(last_endkey), K(rowkey));
      }
    } else if (OB_FAIL((idx_data_header_->rowkey_array_ + end_)->compare(rowkey, index_read_info_->get_datum_utils(), cmp_ret, false))) {
