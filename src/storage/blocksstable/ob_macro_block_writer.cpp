...
int ObMacroBlockWriter::flush_macro_block(ObMacroBlock &macro_block, const bool is_close_flush)
{
  ...
  if (OB_FAIL(ret)) {
      ...
      && OB_FAIL(builder_->generate_macro_row(macro_block, ddl_start_row_offset))) {
...
{
  int ret = OB_SUCCESS;
  if (check_can_flush_small_sstable(is_close_flush)) {
...
int ObMacroBlockWriter::init_pre_agg_util(const ObDataStoreDesc &data_store_desc)
{
  ...
  if (!need_pre_aggregation) {
    ...
    ret = OB_INVALID_ARGUMENT;
    LOG_WARN("invalid agg meta for mini / minor sstable", K(ret), K(full_agg_metas));
  } else {
