...
ERRSIM_POINT_DEF(EN_FORCE_CO_MERGE_MICRO_BLOCK);

class ObIntConstUniform
{
public:
  ObIntConstUniform(const int64_t v) : eval_info_(), vec_(&datum_, &eval_info_) {
    datum_.set_int(v);
  }
  const ObIVector *get_vector() const { return &vec_; }
private:
...
static const ObIntConstUniform DEFAULT_TRANS_SEQUENCE_CONST_UNIFORM(0);
static const ObIntConstUniform DELETE_INSERT_TRANS_SEQUENCE_CONST_UNIFORM(-common::DELETE_INSERT_TRANS_SEQUENCE);
    ...
    common::ObIAllocator &allocator)
{
  ...
  if (OB_FAIL(ret)) {
    ...
    // @cuiyuntian.cyt decide whether to disable micro block verification on minor / mini sstables
...
ObMacroBlockWriter::ObMacroBlockWriter(const bool is_need_macro_buffer)
  : data_store_desc_(nullptr),
    ...
    is_curr_block_pre_allocated_(false),
    micro_block_bf_(),
    reader_helper_(),
    micro_helper_(),
    ...
    pre_warmer_(NULL),
    io_buf_(nullptr),
    can_append_batch_(false),
    merge_micro_block_read_info_(nullptr),
    ...
    const storage::ObITableReadInfo *merge_micro_block_read_info)
{
  ...
  if (OB_UNLIKELY(!data_store_desc.is_valid() || parallel_idx < 0 ||
    ...
    STORAGE_LOG(WARN, "invalid macro block writer input argument.", K(ret), K(data_store_desc), K(parallel_idx),
      K(macro_seq_param), K(pre_warm_param));
  } else if (OB_UNLIKELY(EXEC_MODE_LOCAL != data_store_desc.get_exec_mode())) {
    ret = OB_NOT_SUPPORTED;
    STORAGE_LOG(WARN, "non-local exec mode is not supported by macro block writer",
                K(ret), "exec_mode", exec_mode_to_str(data_store_desc.get_exec_mode()));
    ...
    data_store_desc_ = &data_store_desc;
    merge_micro_block_read_info_ = merge_micro_block_read_info;
    ...
    if (OB_FAIL(init_macro_seq_generator(macro_seq_param))) {
      ...
      STORAGE_LOG(WARN, "Failed to open micro helper", K(ret), K(data_store_desc));
                                          ...
                                          allocator_,
                                          micro_writer_,
                                          micro_helper_.get_micro_block_merge_verify_level()))) {
      ...
      can_append_batch_ =
          ObRowStoreType::CS_ENCODING_ROW_STORE == data_store_desc.get_row_store_type() &&
          !compaction::is_multi_version_merge(data_store_desc.get_merge_type());
    }
    ...
    } else if (FALSE_IT(is_curr_block_pre_allocated_ = is_pre_alloc())) {
...
int ObMacroBlockWriter::append_batch(const ObBatchDatumRows &datum_rows, const int64_t split_size)
{
  ...
  if (OB_SUCC(ret)) {
    bool is_split = false;
    const ObBatchDatumRows *append_datum_rows = &datum_rows;
    if (!data_store_desc_->is_cg() && OB_FAIL(update_cs_row_sql_sequence_info(append_datum_rows))) {
      LOG_WARN("fail to update cs row sql sequence info", KR(ret));
    } else if (OB_FAIL(append(*append_datum_rows))) {
    ...
    compaction::ObMergeVectorStore *read_vector_store)
{
  ...
  if (NULL == data_store_desc_) {
    ...
    if (OB_SUCC(ret)) {
      ...
      } else if (FALSE_IT(last_key_with_L_flag_ = micro_block.header_.is_last_row_last_flag())) {
    ...
    if (OB_FAIL(merge_micro_block(micro_block, read_vector_store))) {
      STORAGE_LOG(WARN, "merge_micro_block failed", K(micro_block), K(ret), KP(read_vector_store));
    } else {
...
int ObMacroBlockWriter::update_cs_row_sql_sequence_info(const ObBatchDatumRows *&datum_rows)
{
  ...
  const int64_t sql_sequence_col_idx =
    ObMultiVersionRowkeyHelpper::get_sql_sequence_col_store_index(
      data_store_desc_->get_schema_rowkey_col_cnt(),
      data_store_desc_->get_rowkey_column_count() != data_store_desc_->get_schema_rowkey_col_cnt());
  if (OB_UNLIKELY(nullptr == datum_rows ||
                  datum_rows->get_column_count() != data_store_desc_->get_row_column_count() ||
                  sql_sequence_col_idx >= datum_rows->get_column_count())) {
    ret = OB_INVALID_ARGUMENT;
    ...
    if (is_delete_insert_merge) {
      ...
      datum_rows_.vectors_.at(sql_sequence_col_idx) =
        const_cast<ObIVector *>(DEFAULT_TRANS_SEQUENCE_CONST_UNIFORM.get_vector());
    }
...
int ObMacroBlockWriter::append_index_micro_block(ObMicroBlockDesc &micro_block_desc)
{
  ...
  ObBlockWriterConcurrentGuard guard(concurrent_lock_);
  const bool need_pre_warm = is_for_index() && (NULL != pre_warmer_);
  ...
  } else if (OB_FAIL(write_micro_block_and_prewarm(false, need_pre_warm, micro_block_desc))) {
    STORAGE_LOG(WARN, "fail to write micro block and prewarm", K(ret), K(micro_block_desc));
  }
...
int ObMacroBlockWriter::write_micro_block_and_prewarm(const bool dump_diag_info,
                                                      ...
                                                      ObMicroBlockDesc &micro_block_desc)
{
  ...
  if (OB_FAIL(micro_helper_.compress_encrypt_micro_block(micro_block_desc,
    ...
    if (OB_FAIL(write_micro_block(micro_block_desc, need_pre_warm))) {
      STORAGE_LOG(WARN, "fail to write micro block ", K(ret), K(micro_block_desc));
    } else if (OB_UNLIKELY(is_curr_block_pre_allocated_ && !micro_block_desc.macro_id_.is_valid())) {
      ret = OB_ERR_UNEXPECTED;
      STORAGE_LOG(WARN, "macro_id is invalid", K(ret), K(micro_block_desc));
    }
    if (OB_SUCC(ret) && need_pre_warm_while_pre_alloc) {
...
int ObMacroBlockWriter::build_micro_block()
{
  ...
  if (OB_UNLIKELY(nullptr == micro_writer_ || micro_writer_->get_row_count() <= 0)) {
    ...
    const bool need_pre_warm = (NULL != pre_warmer_);
    if (OB_FAIL(write_micro_block_and_prewarm(true, need_pre_warm, micro_block_desc))) {
      STORAGE_LOG(WARN, "fail to write micro block and prewarm", K(ret), K(micro_block_desc));
    ...
    ObMicroBlockHeader &header_for_rewrite)
{
  ...
  if (OB_SUCC(ret) && need_fill_meta_info) {
    ...
    micro_block_desc.is_first_row_first_flag_ = micro_block.header_.is_first_row_first_flag();
    micro_block_desc.single_version_rows_ = micro_block.header_.single_version_rows_;
    if (OB_NOT_NULL(micro_block.micro_index_info_->minor_meta_info_)) {
      micro_block_desc.row_count_delta_ = static_cast<int32_t>(micro_block.micro_index_info_->minor_meta_info_->row_count_delta_);
    }
  }
...
int ObMacroBlockWriter::index_builder_append_row(const ObMicroBlockDesc &micro_block_desc,
...
int ObMacroBlockWriter::write_micro_block(ObMicroBlockDesc &micro_block_desc, const bool need_pre_warm)
{
  ...
  if (FAILEDx(wait_io_and_alloc_block())) {
    ...
    } else if (OB_FAIL(index_builder_append_row(micro_block_desc, macro_blocks_[current_index_]))) {
      if (OB_BUF_NOT_ENOUGH != ret) {
        ...
        if (OB_FAIL(index_builder_append_row(micro_block_desc, macro_blocks_[current_index_]))) {
          STORAGE_LOG(WARN, "fail to append row", K(ret), K(micro_block_desc));
        }
      }
    }
...
int ObMacroBlockWriter::prewarm_micro_blocks(const ObMacroBlock &macro_block,
                                             const MacroBlockId &macro_block_id,
                                             const bool need_fill_logic_id)
{
  int ret = OB_SUCCESS;
  ObMicroBlockBareIterator micro_block_iter;
  compaction::ObLocalArena iter_allocator_temp("MaBlkWrPrew");
  if (!macro_block_id.is_valid()) {
    ...
    1. Uncompressed micro blocks are used for executing prewarm->reserve(), which requires the header and payload of
        ...
        1.1 The micro block header in ObMicroBlockDesc points to a complete micro block buffer, and
        the column_checksums_ pointer in ObMicroBlockHeader is valid, which is set in
        ObMacroBlockReader::do_decrypt_and_decompress_data > header.deep_copy(...).
    2.  micro-blocks stored within the macro block (maybe not compressed) are used for executing prewarm->add(), which
        do not require the header and payload to be in contiguous memory.
        ...
        2.3 For compressed micro blocks, must use compressed buf instead of uncompressed micro block,
            as they have different buf sizes which serve as cache key
    */
    ...
    while (OB_SUCC(ret)) {
      ...
      if (OB_FAIL(micro_block_iter.get_next_micro_block_desc(uncompressed_micro_block_desc, micro_block_desc, iter_allocator_temp))) {
        if (ret == OB_ITER_END) {
          ...
          LOG_WARN("fail to get next micro block", K(ret), K(micro_block_idx), K(micro_block_iter));
        }
      } else if (FALSE_IT(micro_block_desc.macro_id_ = macro_block_id)) {
      } else if (OB_UNLIKELY(!micro_block_desc.is_valid() || !uncompressed_micro_block_desc.is_valid())) {
        ret = OB_ERR_UNEXPECTED;
        LOG_WARN("unexpected iterated valid micro block desc", K(ret), K(micro_block_desc), K(uncompressed_micro_block_desc));
      } else if (OB_UNLIKELY(micro_block_desc.logic_micro_id_.is_valid())) {
        ret = OB_ERR_UNEXPECTED;
        LOG_WARN("unexpected valid logic micro id", K(ret), K(micro_block_desc.logic_micro_id_), K(need_fill_logic_id),
                K(micro_block_desc.logic_micro_id_.is_valid()));
      ...
      } else if (current_micro_block_need_prewarm) {
        if (need_fill_logic_id) {
          ObLogicMacroBlockId cur_logic_id;
          gen_logic_macro_id(cur_logic_id);
          micro_block_desc.logic_micro_id_.init(micro_block_desc.block_offset_, cur_logic_id);
    ...
    if (FAILEDx(macro_block.get_pre_warm_list_count() != micro_block_idx ||
      ...
      ret = OB_ERR_UNEXPECTED;
      LOG_WARN("micro block count not equal.", K(macro_block.get_pre_warm_list_count()),
...
int ObMacroBlockWriter::cluster_micro_blocks(const ObMacroBlock &macro_block,
                                             const MacroBlockId &macro_block_id,
                                             const bool need_fill_logic_id)
{
  int ret = OB_SUCCESS;
  ObMicroBlockBareIterator micro_block_iter;
  compaction::ObLocalArena iter_allocator_temp("MaBlkWrClust");
  if (!macro_block_id.is_valid()) {
    ret = OB_INVALID_ARGUMENT;
    LOG_WARN("invalid macro block id", K(ret), K(macro_block_id));
  } else if (OB_FAIL(micro_block_iter.open_for_whole_range(
                macro_block.get_data_buf(),
                macro_block.get_data_size(),
                false))) {
    LOG_WARN("fail to open macro block", K(ret));
    ...
    while (OB_SUCC(ret)) {
      iter_allocator_temp.reuse();
      if (OB_FAIL(micro_block_iter.get_next_micro_block_desc(micro_block_desc, micro_index_data, iter_allocator_temp))) {
        if (ret == OB_ITER_END) {
          ret = OB_SUCCESS;
          break;
        } else {
          LOG_WARN("fail to get next micro block", K(ret), K(micro_block_iter));
        }
      } else if (FALSE_IT(micro_block_desc.macro_id_ = macro_block_id)) {
      } else if (OB_UNLIKELY(!micro_block_desc.is_valid())) {
        ret = OB_ERR_UNEXPECTED;
        LOG_WARN("unexpected iterated invalid micro block desc", K(ret), K(micro_block_desc));
      } else if (OB_UNLIKELY(need_fill_logic_id && !micro_block_desc.logic_micro_id_.is_valid())) {
        // The valid logic micro id has been set in write_micro_block(...) if need_fill_logic_id is true
        ret = OB_ERR_UNEXPECTED;
        LOG_WARN("unexpected invalid logic micro id", K(ret), K(micro_block_desc.logic_micro_id_), K(need_fill_logic_id),
                K(micro_block_desc.logic_micro_id_.is_valid()));
      } else if (OB_FAIL(builder_->clustered_index_agg_micro_block(micro_index_data, micro_block_desc))) {
        LOG_WARN("fail to aggregate micro block", K(ret), K(micro_block_desc), K(micro_index_data));
      } else if (OB_FAIL(builder_->clustered_index_append_row(micro_block_desc))) {
        LOG_WARN("fail to append row to clustered index block", K(ret), K(micro_block_desc));
      }
    }

    if (OB_SUCC(ret) && OB_FAIL(builder_->write_clustered_index_micro_block())) {
      LOG_WARN("fail to write clustered index micro block", K(ret));
    }
  }
  return ret;
}
...
bool ObMacroBlockWriter::is_alloc_block_needed() const
{
  ...
  return data_store_desc_->get_need_submit_io()
        || OB_NOT_NULL(callback_)
        || is_curr_block_pre_allocated_;
}
...
// Used to check before writing data macro block whether the current conditions
// definitely do not satisfy small sstable requirements.
// If not satisfied, directly adopt the pre-allocated macro block id logic.
bool ObMacroBlockWriter::is_definitely_not_small_sstable() const
{
  ObSSTableIndexBuilder *sstable_index_builder = OB_ISNULL(data_store_desc_) ? nullptr : data_store_desc_->sstable_index_builder_;
  return merge_block_info_.macro_block_count_ != 0 ||
         OB_ISNULL(sstable_index_builder) ||
         ObSSTableIndexBuilder::ObSpaceOptimizationMode::ENABLE != sstable_index_builder->get_optimization_mode() ||
         OB_ISNULL(data_store_desc_) ||
         !data_store_desc_->is_for_sstable_data() ||
         data_store_desc_->get_concurrent_cnt() != 1;
}
...
int ObMacroBlockWriter::prepare_default_macro_block_flusher(const bool is_close_flush)
{
  ...
  if (!macro_handle.get_macro_id().is_valid()) {
    if (OB_UNLIKELY(is_curr_block_pre_allocated_)) {
      ret = OB_ERR_UNEXPECTED;
      STORAGE_LOG(WARN, "is_curr_block_pre_allocated_ must be false when macro_id is invalid.", K(ret), K(is_curr_block_pre_allocated_), K(macro_handle.get_macro_id()));
  ...
  } else if (OB_UNLIKELY(!is_curr_block_pre_allocated_)) {
    ret = OB_ERR_UNEXPECTED;
    STORAGE_LOG(WARN, "macro_id must be invalid when is_curr_block_pre_allocated_ is false.", K(ret), K(is_curr_block_pre_allocated_), K(macro_handle.get_macro_id()));
  }
...
int ObMacroBlockWriter::post_flush_small_sstable_data_macro_block(const ObBlockInfo &block_info)
{
  ...
  if (OB_UNLIKELY(OB_ISNULL(builder_) || !macro_block_id.is_valid()
    ...
    ret = OB_NOT_SUPPORTED;
    STORAGE_LOG(WARN, "small sstable is not supported with clustered micro index",
                K(ret), K(micro_index_clustered()));
                                                      ...
                                                      const MacroBlockId &macro_block_id)
{
  ...
  } else if (!is_curr_block_pre_allocated_) {
    if (macro_block.need_pre_warm() && OB_FAIL(prewarm_micro_blocks(macro_block, macro_block_id, need_fill_logic_id))) {
      STORAGE_LOG(WARN, "fail to prewarm micro blocks.", K(ret));
    } else if (micro_index_clustered() && OB_FAIL(cluster_micro_blocks(macro_block, macro_block_id, need_fill_logic_id))) {
      STORAGE_LOG(WARN, "fail to cluster micro blocks.", K(ret));
    }
  } else if (micro_index_clustered() && OB_FAIL(builder_->write_clustered_index_micro_block())) {
    LOG_WARN("fail to write clustered index micro block", K(ret));
  }
  return ret;
}
...
int ObMacroBlockWriter::post_flush(ObMacroBlock &macro_block)
{
  ...
  if (OB_FAIL(macro_seq_generator_->get_next(current_macro_seq))) {
    ...
    /* if use buffer, init in wait_io_finish */
     ...
     LOG_WARN("macro block writer fail to init.", K(ret), K(cur_mb_row_count));
  }
  if (OB_SUCC(ret)) {
    // set is_pre_allocated state for next macro block
    is_curr_block_pre_allocated_ = is_pre_alloc();
  }
  return ret;
}
...
int ObMacroBlockWriter::wait_io_and_alloc_block()
{
  ...
  } else if (is_curr_block_pre_allocated_ && OB_FAIL(alloc_block())) {
...
#ifdef ERRSIM
      if (OB_UNLIKELY(EN_FORCE_CO_MERGE_MICRO_BLOCK)) {
        need_merge = true;
        SERVER_EVENT_SYNC_ADD("merge_errsim", "force_co_merge_micro_block",
            "tablet_id", data_store_desc_->get_tablet_id());
      }
#endif
    }
...
int ObMacroBlockWriter::merge_micro_block(const ObMicroBlock &micro_block, compaction::ObMergeVectorStore *read_vector_store)
{
  int ret = OB_SUCCESS;
  ObIMicroBlockReader *micro_reader = NULL;
  const bool deep_copy_des_meta = false;
  ...
  if (OB_ISNULL(data_store_desc_)) {
    ...
    ret = OB_INVALID_ARGUMENT;
    STORAGE_LOG(WARN, "invalid micro_block", K(micro_block), K(ret));
  } else if (OB_FAIL(reader_helper_.get_reader(micro_block.header_, micro_reader))) {
    ...
    const bool is_append_batch = nullptr != read_vector_store && can_append_batch_;
    ...
    } else if (is_append_batch) {
      if (OB_ISNULL(merge_micro_block_read_info_)) {
        ret = OB_ERR_UNEXPECTED;
        STORAGE_LOG(WARN, "unexpected null merge micro block read info", K(ret), KP(merge_micro_block_read_info_));
      } else if (OB_FAIL(micro_reader->init(decompressed_data, *merge_micro_block_read_info_))) {
        STORAGE_LOG(WARN, "micro_block_reader init failed", K(micro_block), K(ret), KPC(merge_micro_block_read_info_));
      }
    ...
    if (OB_SUCC(ret)) {
      if (is_append_batch) {
        if (OB_FAIL(merge_micro_block_append_rows_by_batch(*micro_reader, *read_vector_store, split_size))) {
          STORAGE_LOG(WARN, "merge micro block append by batch failed", K(ret), K(split_size));
        }
      } else if (OB_FAIL(merge_micro_block_append_rows_by_row(*micro_reader, split_size))) {
        STORAGE_LOG(WARN, "merge micro block append by row failed", K(ret), K(split_size));
      }

      if (OB_SUCC(ret)
          && micro_writer_->get_row_count() > 0
          && micro_writer_->get_block_size() >= data_store_desc_->get_micro_block_size()) {
        if (OB_FAIL(build_micro_block())) {
...
int ObMacroBlockWriter::flush_read_vector_store(
    compaction::ObMergeVectorStore &read_vector_store,
    const int64_t split_size)
{
  int ret = OB_SUCCESS;
  blocksstable::ObBatchDatumRows batch_rows;
  if (read_vector_store.is_empty()) {
    ret = OB_ERR_UNEXPECTED;
    STORAGE_LOG(WARN, "unexpected empty read vector store", K(ret), K(read_vector_store));
  } else if (OB_FAIL(read_vector_store.get_batch_datum_rows(batch_rows))) {
    STORAGE_LOG(WARN, "get_batch_datum_rows failed", K(ret));
  } else if (OB_UNLIKELY(batch_rows.row_count_ <= 0)) {
    ret = OB_ERR_UNEXPECTED;
    LOG_WARN("unexpected empty batch rows", K(ret), K(batch_rows), K(read_vector_store));
  } else if (OB_FAIL(append_batch(batch_rows, split_size))) {
    STORAGE_LOG(WARN, "append_batch failed", K(ret), K(batch_rows), K(split_size));
  } else {
    read_vector_store.reuse();
  }
  return ret;
}

int ObMacroBlockWriter::merge_micro_block_append_rows_by_batch(
    ObIMicroBlockReader &micro_reader,
    compaction::ObMergeVectorStore &read_vector_store,
    const int64_t split_size)
{
  ...
  const int64_t row_count = micro_reader.row_count();
  read_vector_store.reuse();

  while (OB_SUCC(ret) && begin_index < row_count) {
    if (OB_FAIL(read_vector_store.fill_rows_from_reader(nullptr, micro_reader, begin_index, row_count))) {
      STORAGE_LOG(WARN, "fill rows from micro reader failed", K(ret), K(begin_index), K(row_count));
    } else if (OB_UNLIKELY(read_vector_store.is_empty())) {
      ret = OB_ERR_UNEXPECTED;
      STORAGE_LOG(WARN, "unexpected empty read vector store", K(ret), K(begin_index), K(row_count));
    } else if (OB_FAIL(flush_read_vector_store(read_vector_store, split_size))) {
      STORAGE_LOG(WARN, "flush read vector store failed", K(ret), K(begin_index), K(row_count));
    }
    // Some readers reuse temporary buffers on the next batch, so flush before reading more rows.
  }
  return ret;
}

int ObMacroBlockWriter::merge_micro_block_append_rows_by_row(
    ObIMicroBlockReader &micro_reader,
    const int64_t split_size)
{
  int ret = OB_SUCCESS;
                                           ...
                                           const int64_t verify_level)
{
  ...
  if (data_store_desc->encoding_enabled()) {
    ...
    if (ObStoreFormat::is_row_store_type_with_pax_encoding(data_store_desc->get_row_store_type())) {
      ...
      bool is_multi_version = compaction::is_multi_version_merge(data_store_desc->get_merge_type()) && !data_store_desc->is_for_index_or_meta();
      if (is_multi_version) {
        ret = inner_build_micro_writer<ObMicroBlockCSEncoder<true /* IS_MULTI_VERSION */>>(allocator, micro_writer, verify_level, encoding_ctx);
      } else {
        ret = inner_build_micro_writer<ObMicroBlockCSEncoder<false /* IS_MULTI_VERSION */>>(allocator, micro_writer, verify_level, encoding_ctx);
      }
      if (OB_FAIL(ret)) {
        STORAGE_LOG(WARN, "Fail to init micro block encoder", K(ret));
      }
