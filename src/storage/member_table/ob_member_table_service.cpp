...
ObMemberTableService::ObMemberTableService()
  : is_inited_(false),
    thread_cond_(),
    wakeup_cnt_(0),
    ls_service_(nullptr)
...
int ObMemberTableService::mtl_init(ObMemberTableService *&member_service)
{
  int ret = OB_SUCCESS;
  ...
  } else if (OB_FAIL(member_service->init(ls_service))) {
    LOG_WARN("failed to init member service", K(ret), KP(ls_service));
  }
  return ret;
}

int ObMemberTableService::init(
    ObLSService *ls_service)
{
  ...
  if (is_inited_) {
    ret = OB_INIT_TWICE;
    LOG_WARN("member table service is already init", K(ret));
  } else if (OB_ISNULL(ls_service)) {
    ret = OB_INVALID_ARGUMENT;
    LOG_WARN("init member table service get invalid argument", K(ret), KP(ls_service));
...
void ObMemberTableService::wakeup()
{
...
void ObMemberTableService::destroy()
{
  if (is_inited_) {
    COMMON_LOG(INFO, "ObMemberTableService starts to destroy");
    thread_cond_.destroy();
    wakeup_cnt_ = 0;
    is_inited_ = false;
    COMMON_LOG(INFO, "ObMemberTableService destroyed");
  }
}

void ObMemberTableService::stop()
{
  if (is_inited_) {
    COMMON_LOG(INFO, "ObMemberTableService starts to stop");
    ThreadPool::stop();
    wakeup();
    COMMON_LOG(INFO, "ObMemberTableService stopped");
  }
}

void ObMemberTableService::wait()
{
  if (is_inited_) {
    COMMON_LOG(INFO, "ObMemberTableService starts to wait");
    ThreadPool::wait();
    COMMON_LOG(INFO, "ObMemberTableService finish to wait");
  }
}

int ObMemberTableService::start()
{
  int ret = OB_SUCCESS;
  if (!is_inited_) {
    ret = OB_NOT_INIT;
    LOG_WARN("member table service do not init", K(ret));
  } else {
    if (OB_FAIL(lib::ThreadPool::start())) {
      COMMON_LOG(WARN, "ObMemberTableService start thread failed", K(ret));
    } else {
      COMMON_LOG(INFO, "ObMemberTableService start");
    }
  }
  return ret;
}

void ObMemberTableService::run1()
{
  int tmp_ret = OB_SUCCESS;
  lib::set_thread_name("MemberService");
...
int ObMemberTableService::calc_max_recycle_scn_()
{
  int ret = OB_SUCCESS;
  int tmp_ret = OB_SUCCESS;
...
int ObMemberTableService::calc_ls_max_recycle_scn_(const share::ObLSID &ls_id)
{
  int ret = OB_SUCCESS;
  ...
  if (!is_inited_) {
    ...
    if (OB_SUCC(ret)) {
      ...
      if (OB_FAIL(ls->get_member_table()->update_can_recycle_scn(recycle_scn))) {
...
int ObMemberTableService::check_can_recycle_(
    ObLS &ls,
    ...
    bool &can_recycle)
{
  ...
  if (!is_inited_) {
    ...
    LOG_WARN("member table tablet status do not supported", K(ret), K(data));
  }
  return ret;
}

int ObMemberTableService::check_transfer_tablet_(
    ObLS &ls,
    const ObMemberTableData &data,
    bool &can_recycle)
{
...
int ObMemberTableService::check_transfer_in_tablet_finish_(
    ObLS &ls,
    const ObMemberTableData &data,
...
int ObMemberTableService::check_transfer_out_tablet_finish_(
    ObLS &ls,
    const ObMemberTableData &data,
    const ObTransferDataValue &data_value,
    bool &is_finish)
