...
ObMemberTableSchemaDef::ObMemberTableSchemaDef()
{
}
...
int ObMemberTableSchemaDef::get_table_column_info(
    ...
    int64_t &rowkey_position)
{
  ...
  if (column_idx < 0) {
  ...
  } else if (column_idx >= MEMBER_ROW_COLUMN_CNT) {
    ...
    const MEMBER_TABLE_SCHEMA_COL_IDX type = static_cast<MEMBER_TABLE_SCHEMA_COL_IDX>(column_idx);
    switch (type) {
    case MEMBER_TABLE_SCHEMA_COL_IDX::TABLET_ID : {
      meta.set_int();
      rowkey_position = 1;
      break;
    }
    case MEMBER_TABLE_SCHEMA_COL_IDX::REORGANIZATION_SCN : {
      meta.set_int();
      rowkey_position = 2;
      break;
    }
    case MEMBER_TABLE_SCHEMA_COL_IDX::DATA_TYPE : {
      meta.set_int();
      rowkey_position = 3;
      break;
    }
    case MEMBER_TABLE_SCHEMA_COL_IDX::DATA_VALUE : {
      meta.set_binary();
      rowkey_position = 0;
      break;
    }
...
ObMemberTableSchemaHelper::ObMemberTableSchemaHelper()
  : is_inited_(false),
    allocator_("MTSchema"),
    table_schema_(),
    storage_schema_(),
...
ObMemberTableSchemaHelper &ObMemberTableSchemaHelper::get_instance()
{
  static ObMemberTableSchemaHelper helper;
  return helper;
}

int ObMemberTableSchemaHelper::init()
{
  int ret = OB_SUCCESS;
  ...
  if (OB_UNLIKELY(is_inited_)) {
    ...
    if (OB_FAIL(build_table_schema_(tenant_id, ObMemberTableSchemaDef::DATABASE_ID, ObMemberTableSchemaDef::MEMBER_TABLE_ID,
        ObMemberTableSchemaDef::MEMBER_TABLE_NAME, table_schema_))) {
...
void ObMemberTableSchemaHelper::reset()
{
...
const ObStorageSchema *ObMemberTableSchemaHelper::get_storage_schema() const
{
  int ret = OB_SUCCESS;
...
const share::schema::ObTableSchema *ObMemberTableSchemaHelper::get_table_schema() const
{
  int ret = OB_SUCCESS;
...
const ObRowkeyReadInfo *ObMemberTableSchemaHelper::get_rowkey_read_info() const
{
  int ret = OB_SUCCESS;
...
int ObMemberTableSchemaHelper::build_table_schema_(
    ...
    share::schema::ObTableSchema &table_schema)
{
  ...
  table_schema.set_rowkey_column_num(ObMemberTableSchemaDef::ROWKEY_COLUMN_NUM);
  table_schema.set_compress_func_name("none");
  table_schema.set_row_store_type(ObRowStoreType::FLAT_ROW_STORE);
  table_schema.set_table_name(ObMemberTableSchemaDef::MEMBER_TABLE_NAME);
  table_schema.set_schema_version(ObMemberTableSchemaDef::MEMBER_SCHEMA_VERSION);

  ObColumnSchemaV2 column_schema;
  for (int64_t i = 0; OB_SUCC(ret) && i < ObMemberTableSchemaDef::MEMBER_ROW_COLUMN_CNT; ++i) {
    column_schema.reset();
    if (OB_FAIL(ObMemberTableSchemaDef::get_table_column_info(i, column_id, column_name, column_length, meta, rowkey_position))) {
      LOG_WARN("failed to get table column info", K(ret), K(i));
        ...
        column_id,
        column_name,
        ObMemberTableSchemaDef::COLUMN_SCHEMA_VERSION,
        rowkey_position,
        ObOrderType::ASC,
        meta,
        column_length,
...
int ObMemberTableSchemaHelper::build_rowkey_read_info_(
...
int ObMemberTableSchemaHelper::build_column_schema_(
    const uint64_t tenant_id,
    const uint64_t table_id,
...
int ObMemberTableSchemaHelper::build_column_ids_(
...
int ObMemberTableSchemaHelper::get_column_ids(
    const bool is_query,
    common::ObIArray<uint64_t> &column_ids)
{
  ...
  if (OB_UNLIKELY(!is_inited_)) {
    ...
    int64_t i = 0;
    for (; OB_SUCC(ret) && i < ObMemberTableSchemaDef::ROWKEY_COLUMN_NUM; ++i) {
    ...
    for (; OB_SUCC(ret) && i < ObMemberTableSchemaDef::MEMBER_ROW_COLUMN_CNT; ++i) {
      if (OB_FAIL(column_ids.push_back(column_ids_.at(i)))) {
        LOG_WARN("failed to push column id into array", K(ret), K(i), K(column_ids_));
      }
    }
