...
ObMemberTableIterator::ObMemberTableIterator()
  : is_inited_(false),
    allocator_(),
...
int ObMemberTableIterator::init(
    ...
    const int64_t abs_timeout_ts)
{
  ...
  if (is_inited_) {
    ret = OB_INIT_TWICE;
    LOG_WARN("member table iterator init twice", K(ret));
  } else if (OB_INVALID_ID == tenant_id || !ls_id.is_valid() || !new_range.is_valid() || abs_timeout_ts <= 0) {
    ret = OB_INVALID_ARGUMENT;
    LOG_WARN("member table iterator init get invalid argument", K(ret), K(tenant_id), K(ls_id),
...
int ObMemberTableIterator::prepare_scan_iter_(
    const uint64_t tenant_id,
    const share::ObLSID &ls_id,
    const common::ObNewRange &new_range,
    const bool is_get,
    const int64_t abs_timeout_ts)
{
  int ret = OB_SUCCESS;
  share::SCN snapshot;
  if (OB_FAIL(get_local_read_scn_(tenant_id, abs_timeout_ts, snapshot))) {
    LOG_WARN("failed to get local read scn", K(ret), K(tenant_id), K(ls_id));
...
int ObMemberTableIterator::get_local_read_scn_(
    const uint64_t tenant_id,
    const int64_t abs_timeout_ts,
    share::SCN &read_scn)
{
...
int ObMemberTableIterator::build_read_ctx_(
    const share::ObLSID &ls_id,
    ...
    const share::SCN &snapshot)
{
  ...
  if (OB_ISNULL(buf = allocator_.alloc(sizeof(ObInnerTableReadCtx)))) {
    ...
    ctx_->ls_id_ = ls_id;
    ctx_->tablet_id_ = LS_MEMBER_TABLET;
...
int ObMemberTableIterator::build_scan_iter_()
{
  int ret = OB_SUCCESS;
...
int ObMemberTableIterator::inner_get_next_row_(
    ...
    int64_t &sql_no)
{
  ...
  member_table_data.reset();
  trans_scn.reset();
  sql_no = 0;

  if (!is_inited_) {
    ret = OB_NOT_INIT;
    LOG_WARN("member table iterator do not init", K(ret));
  } else if (OB_FAIL(scan_iter_->get_next_row(datum_row))) {
    if (OB_ITER_END != ret) {
      LOG_WARN("failed to get next row", K(ret));
    }
  } else if (OB_FAIL(member_table_data.row_2_data(datum_row, trans_scn, sql_no))) {
    LOG_WARN("failed to convert row to member table data", K(ret), KPC(datum_row));
  }
  return ret;
}

int ObMemberTableIterator::get_next_row(
    ObMemberTableData &member_table_data,
    share::SCN &trans_scn)
{
  int ret = OB_SUCCESS;
  ...
  if (!is_inited_) {
    ret = OB_NOT_INIT;
    LOG_WARN("member table iterator do not init", K(ret));
  } else if (OB_FAIL(inner_get_next_row_(member_table_data, trans_scn, sql_no))) {
    if (OB_ITER_END != ret) {
      LOG_WARN("failed to get next row", K(ret));
    }
  }
    ...
    ObMemberTableData &member_table_data)
{
  ...
  if (!is_inited_) {
    ret = OB_NOT_INIT;
    LOG_WARN("member table iterator do not init", K(ret));
  } else if (OB_FAIL(inner_get_next_row_(member_table_data, unused_trans_scn, unused_sql_no))) {
    if (OB_ITER_END != ret) {
      LOG_WARN("failed to get next row", K(ret));
    }
  }
...
void ObMemberTableIterator::free_scan_iter_(ObNewRowIterator *iter)
{
  int ret = OB_SUCCESS;
  if (OB_NOT_NULL(iter)) {
    ObAccessService *access_service = nullptr;
