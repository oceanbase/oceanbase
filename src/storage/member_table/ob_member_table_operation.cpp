...
int ObMemberTableDataGenerator::gen_transfer_member_data(
    ...
    common::ObIArray<ObMemberTableData> &member_table_data)
{
  ...
  if (!tablet_id.is_valid() || !reorganization_scn.is_valid() || !tablet_status.is_valid()
    ...
    LOG_WARN("gen member table data get invalid argument", K(ret), K(tablet_id), K(reorganization_scn), K(tablet_status),
    ...
    data.key_.type_ = tablet_status == ObTabletStatus::TRANSFER_IN ? ObMemberTableDataType::TRANSFER_IN : ObMemberTableDataType::TRANSFER_OUT;
    ...
    } else if (OB_FAIL(member_table_data.push_back(data))) {
...
ObMemberTableWriteOperator::ObMemberTableWriteOperator()
  : is_inited_(false),
    allocator_(),
    sql_proxy_(nullptr),
...
int ObMemberTableWriteOperator::init(
    ...
    const share::ObLSID &ls_id)
{
  ...
  if (is_inited_) {
    ret = OB_INIT_TWICE;
    LOG_WARN("member table write operator already init", K(ret));
  } else if (OB_INVALID_ID == tenant_id || !ls_id.is_valid()) {
    ret = OB_INVALID_ARGUMENT;
    LOG_WARN("member table write operator init get invalid argument", K(ret), K(tenant_id), K(ls_id));
...
int ObMemberTableWriteOperator::insert_row(
    const ObMemberTableData &data)
{
  ...
  if (!is_inited_) {
    ret = OB_NOT_INIT;
    LOG_WARN("member table write operator do not init", K(ret));
  } else if (!data.is_valid()) {
    ret = OB_INVALID_ARGUMENT;
...
int ObMemberTableWriteOperator::insert_rows(
    const common::ObIArray<ObMemberTableData> &member_table_data)
{
  ...
  if (!is_inited_) {
    ...
    for (int64_t i = 0; OB_SUCC(ret) && i < member_table_data.count(); ++i) {
      const ObMemberTableData &data = member_table_data.at(i);
      if (!data.is_valid()) {
        ret = OB_INVALID_ARGUMENT;
        LOG_WARN("member table data is invalid", K(ret), K(data));
      } else if (OB_FAIL(inner_insert_row_(data, affected_rows))) {
...
int ObMemberTableWriteOperator::inner_insert_row_(
    const ObMemberTableData &data,
    int64_t &affected_rows)
{
  ...
  if (!is_inited_) {
    ...
    LOG_WARN("member table write operator do not init", K(ret));
      ...
      ObMemberTableSchemaDef::MEMBER_ROW_COLUMN_CNT, datum_row))) {
...
int ObMemberTableWriteOperator::fill_data_(
...
int ObMemberTableWriteOperator::flush_and_wait_()
{
  int ret = OB_SUCCESS;
  ...
  if (0 == data_buffer_.get_position()) {
    ...
    LOG_WARN("conn is NULL", KR(ret));
      ...
      LS_MEMBER_TABLET, data_buffer_.get_data(), data_buffer_.get_position(), affected_rows))) {
...
ObMemberTableReadOperator::ObMemberTableReadOperator()
  : is_inited_(false),
    new_range_(),
    iter_(),
    allow_read_(true)
{
  for (int64_t i = 0; i < ObMemberTableSchemaDef::ROWKEY_COLUMN_NUM; ++i) {
...
int ObMemberTableReadOperator::init(
    const uint64_t tenant_id,
    const share::ObLSID &ls_id,
    ...
    const int64_t timeout_us)
{
  ...
  if (is_inited_) {
    ret = OB_NOT_INIT;
    LOG_WARN("member table read operation is already init", K(ret));
  } else if (OB_INVALID_ID == tenant_id || !ls_id.is_valid() || !key.is_valid() || timeout_us <= 0) {
    ret = OB_INVALID_ARGUMENT;
    LOG_WARN("init member table read operation get invalid argument", K(ret),
    ...
    const ObRowkey rowkey(start_key_obj_, ObMemberTableSchemaDef::ROWKEY_COLUMN_NUM);
    if (OB_FAIL(new_range_.build_range(ObMemberTableSchemaDef::MEMBER_TABLE_ID, rowkey))) {
    ...
    const int64_t timeout_us)
{
  ...
  if (is_inited_) {
    ...
    new_range_.table_id_ = ObMemberTableSchemaDef::MEMBER_TABLE_ID;
    new_range_.start_key_.assign(start_key_obj_, ObMemberTableSchemaDef::ROWKEY_COLUMN_NUM);
    new_range_.end_key_.assign(end_key_obj_, ObMemberTableSchemaDef::ROWKEY_COLUMN_NUM);
    ...
    const int64_t timeout_us)
{
  ...
  if (is_inited_) {
    ret = OB_NOT_INIT;
    LOG_WARN("member table read operation is already init", K(ret));
  } else if (OB_INVALID_ID == tenant_id || !ls_id.is_valid() || !ObMemberTableDataType::is_valid(start_type)
      || !ObMemberTableDataType::is_valid(end_type)) {
    ret = OB_INVALID_ARGUMENT;
    LOG_WARN("init member table read operation get invalid argument", K(ret),
...
int ObMemberTableReadOperator::inner_init_(
    const uint64_t tenant_id,
    const share::ObLSID &ls_id,
    const bool is_get,
    const int64_t timeout_us)
...
int ObMemberTableReadOperator::get_next(
    ObMemberTableData &data)
{
  int ret = OB_SUCCESS;
  data.reset();
  ...
  if (!is_inited_) {
    ret = OB_NOT_INIT;
    LOG_WARN("member table read operation do not init", K(ret));
  } else if (!allow_read_) {
    ret = OB_ITER_END;
    ...
    const ObMemberTableDataType::TYPE &end_type)
{
  ...
  } else if (OB_INVALID_ID == tenant_id || !ls_id.is_valid() || !ObMemberTableDataType::is_transfer(start_type)
      || !ObMemberTableDataType::is_transfer(end_type)) {
    ret = OB_INVALID_ARGUMENT;
    ...
    ObTransferDataValue &transfer_value)
{
  ...
  } else if (!ObMemberTableDataType::is_transfer(data.key_.type_)) {
    ret = OB_ERR_UNEXPECTED;
