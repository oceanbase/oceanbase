...
static const char *MEMBER_TABLE_DATA_TYPE[] = {
    "TRANSFER_IN",
    "TRANSFER_OUT",
    "SPLIT_SRC",
    "SPLIT_DST",
...
const char *ObMemberTableDataType::get_str(const TYPE &type)
{
  ...
  if (type < 0 || type >= MAX) {
    ...
    str = MEMBER_TABLE_DATA_TYPE[type];
  }
  return str;
}

ObMemberTableDataKey::ObMemberTableDataKey()
  : tablet_id_(),
    reorganization_scn_(),
    type_(ObMemberTableDataType::MAX)
{
}
...
void ObMemberTableDataKey::reset()
{
  tablet_id_.reset();
...
bool ObMemberTableDataKey::is_valid() const
{
  return tablet_id_.is_valid()
      && reorganization_scn_.is_valid()
      && ObMemberTableDataType::is_valid(type_);
}

OB_SERIALIZE_MEMBER(ObMemberTableDataKey,
    tablet_id_, reorganization_scn_, type_);


ObMemberTableDataValue::ObMemberTableDataValue()
  : pos_(0)
{
  value_[0] = '\0';
}
...
int ObMemberTableDataValue::assign(const char *str, const int64_t buf_len)
{
  int ret = OB_SUCCESS;
  if (NULL == str) {
    pos_ = 0;
...
int ObMemberTableDataValue::assign(const ObMemberTableDataValue &other)
{
  int ret = OB_SUCCESS;
  if (this != &other) {
    if (OB_FAIL(assign(other.value_, other.pos_))) {
      LOG_WARN("failed to assign member table data value", K(other), K(ret));
    }
  }
  return ret;
}

ObMemberTableDataValue &ObMemberTableDataValue::operator =(
    const ObMemberTableDataValue &value)
{
  int ret = OB_SUCCESS;
  if (this != &value) {
...
bool ObMemberTableDataValue::is_empty() const
{
  return (0 == pos_);
}

bool ObMemberTableDataValue::is_valid() const
{
  return !is_empty();
}

int ObMemberTableDataValue::write_value(const ObIMemberTableDataValue &data_value)
{
  int ret = OB_SUCCESS;
  if (!data_value.is_valid()) {
    ret = OB_INVALID_ARGUMENT;
    LOG_WARN("member table data value get invalid argument", K(ret), K(data_value));
  } else {
...
int ObMemberTableDataValue::write_value(const char *buf, const int64_t buf_len)
{
  int ret = OB_SUCCESS;
...
int ObMemberTableDataValue::get_value(ObIMemberTableDataValue &data_value) const
{
  int ret = OB_SUCCESS;
  data_value.reset();
  int64_t pos = 0;
  if (is_empty()) {
    ret = OB_ERR_UNEXPECTED;
    LOG_WARN("member table data is empty, unexpected", K(ret));
...
int ObMemberTableDataValue::get_value(char *buf, const int64_t buf_len, int64_t &pos) const
{
  int ret = OB_SUCCESS;
  pos = 0;
...
int64_t ObMemberTableDataValue::to_string(char *buf, const int64_t buf_len) const
{
  int64_t pos = 0;
  if (buf_len < pos_) {
    pos = 0;
...
ObMemberTableData::ObMemberTableData()
  : key_(),
    value_()
{
}
...
void ObMemberTableData::reset()
{
  key_.reset();
  value_.reset();
}

bool ObMemberTableData::is_valid() const
{
  return key_.is_valid()
      && value_.is_valid();
}

int ObMemberTableData::data_2_datum_row(
    ...
    blocksstable::ObDatumRow *datum_row) const
{
  ...
  if (!is_valid()) {
    ret = OB_NOT_INIT;
    LOG_WARN("member table data is invalid", K(ret), KPC(this));
  } else if (OB_ISNULL(datum_row)) {
    ret = OB_INVALID_ARGUMENT;
    LOG_WARN("data 2 datum row get invalid argument", K(ret), KP(datum_row), KPC(this));
  } else if (OB_ISNULL(table_schema = ObMemberTableSchemaHelper::get_instance().get_table_schema())) {
    ret = OB_ERR_UNEXPECTED;
  ...
  } else if (column_list.count() != ObMemberTableSchemaDef::MEMBER_ROW_COLUMN_CNT) {
    ret = OB_ERR_UNEXPECTED;
    ...
    for (int64_t column_idx = 0; OB_SUCC(ret) && column_idx < column_list.count(); ++column_idx) {
      const ObMemberTableSchemaDef::MEMBER_TABLE_SCHEMA_COL_IDX type = static_cast<ObMemberTableSchemaDef::MEMBER_TABLE_SCHEMA_COL_IDX>(column_idx);
      switch (type) {
        case ObMemberTableSchemaDef::MEMBER_TABLE_SCHEMA_COL_IDX::TABLET_ID: {
          datum_row->storage_datums_[column_idx].set_uint(key_.tablet_id_.id());
          break;
        }
        case ObMemberTableSchemaDef::MEMBER_TABLE_SCHEMA_COL_IDX::REORGANIZATION_SCN : {
          datum_row->storage_datums_[column_idx].set_int(key_.reorganization_scn_.get_val_for_tx());
          break;
        }
        case ObMemberTableSchemaDef::MEMBER_TABLE_SCHEMA_COL_IDX::DATA_TYPE : {
          datum_row->storage_datums_[column_idx].set_int(static_cast<int64_t>(key_.type_));
          break;
        }
        case ObMemberTableSchemaDef::MEMBER_TABLE_SCHEMA_COL_IDX::DATA_VALUE: {
          const int64_t buffer_len = value_.size();
          char *buffer = nullptr;
          int64_t pos = 0;
          if (nullptr == (buffer = (char *)allocator.alloc(buffer_len))) {
            ret = OB_ALLOCATE_MEMORY_FAILED;
            STORAGE_LOG(WARN, "fail to serialize member table value, cause buffer allocated failed",
                              KR(ret), K(*this));
...
int ObMemberTableData::row_2_data(
    ...
    int64_t &sql_no)
{
  ...
  if (OB_ISNULL(new_row)) {
  ...
  } else if (new_row->get_column_count() != ObMemberTableSchemaDef::MEMBER_MULTI_VERSION_ROW_COLUMN_CNT) {
    ret = OB_ERR_UNEXPECTED;
    ...
    for (int64_t column_idx = 0; OB_SUCC(ret) && column_idx < new_row->get_column_count(); ++column_idx) {
      ...
      const ObMemberTableSchemaDef::MEMBER_SSTABLE_COL_IDX type = static_cast<ObMemberTableSchemaDef::MEMBER_SSTABLE_COL_IDX>(column_idx);
      switch (type) {
        case ObMemberTableSchemaDef::MEMBER_SSTABLE_COL_IDX::TABLET_ID : {
          key_.tablet_id_ = obj.get_int();
          break;
        }
        case ObMemberTableSchemaDef::MEMBER_SSTABLE_COL_IDX::REORGANIZATION_SCN : {
          SCN tmp_scn;
        ...
        case ObMemberTableSchemaDef::MEMBER_SSTABLE_COL_IDX::DATA_TYPE : {
          SCN tmp_scn;
          key_.type_ = static_cast<ObMemberTableDataType::TYPE>(obj.get_int());
          break;
        }
        case ObMemberTableSchemaDef::MEMBER_SSTABLE_COL_IDX::TRANS_VERSION : {
          SCN tmp_scn;
          if (OB_FAIL(tmp_scn.convert_for_tx(obj.get_int()))) {
            LOG_WARN("failed to convert for tx", K(ret), K(obj));
          } else {
        ...
        case ObMemberTableSchemaDef::MEMBER_SSTABLE_COL_IDX::SQL_SEQ : {
          sql_no = obj.get_int();
          break;
        }
        case ObMemberTableSchemaDef::MEMBER_SSTABLE_COL_IDX::DATA_VALUE : {
          const ObString &tmp_string = obj.get_string();
          if (OB_FAIL(value_.write_value(tmp_string.ptr(), tmp_string.length()))) {
            LOG_WARN("failed to write member table value", K(ret), K(tmp_string));
          }
          break;
        }
        default: {
...
int ObMemberTableData::get_transfer_data_value(ObTransferDataValue &data_value) const
{
  int ret = OB_SUCCESS;
  data_value.reset();
  if (!is_valid()) {
    ret = OB_NOT_INIT;
    LOG_WARN("member table data is invalid", K(ret), KPC(this));
  } else if (!ObMemberTableDataType::is_transfer(key_.type_)) {
    ret = OB_ERR_UNEXPECTED;
    LOG_WARN("member table data is not transfer, unexpected", K(ret), KPC(this));
...
int64_t ObMemberTableData::to_string(char *buf, const int64_t buf_len) const
{
  int64_t pos = 0;
...
int64_t ObMemberTableData::to_value_string(char *buf, const int64_t buf_len) const
{
  int64_t pos = 0;
  int tmp_ret = OB_SUCCESS;
  const int64_t value_buf_length = ObMemberTableSchemaDef::DATA_VALUE_COLUMN_LENGTH;
  char value_buf[value_buf_length] = {0};
  if (ObMemberTableDataType::is_transfer(key_.type_)) {
...
int ObMemberTable::init(ObLS *ls)
{
  int ret = OB_SUCCESS;
  if (is_inited_) {
    ret = OB_INIT_TWICE;
    LOG_WARN("member table is already init", K(ret));
  } else if (OB_ISNULL(ls)) {
    ret = OB_INVALID_ARGUMENT;
    LOG_WARN("init member table get invalid argument", K(ret), KP(ls));
  } else {
    ls_ = ls;
    ls_id_ = ls->get_ls_id();
    is_inited_ = true;
...
void ObMemberTable::stop()
{
  LOG_INFO("tx table stop finish", KPC(this));
}

void ObMemberTable::destroy()
{
  ls_id_.reset();
  ls_ = nullptr;
  is_inited_ = false;
...
int ObMemberTable::offline()
{
  ...
  if (!is_inited_) {
  ...
  } else if (OB_FAIL(ls_tablet_svr->get_tablet(LS_MEMBER_TABLET, handle))) {
    ...
    LOG_WARN("failed to release member table tablet memtables", K(ret), KPC(ls_));
  }
  return ret;
}

int ObMemberTable::online()
{
  int ret = OB_SUCCESS;
  ObTabletHandle handle;
  ObTablet *tablet = nullptr;
  ...
  if (!is_inited_) {
    ...
    ret = OB_ERR_UNEXPECTED;
    LOG_WARN("member tablet should not be NULL", K(ret), KP(tablet));
  }
  return ret;
}

int ObMemberTable::remove_tablet()
{
  int ret = OB_SUCCESS;
  if (!is_inited_) {
    ret = OB_NOT_INIT;
    LOG_WARN("member table is not init.", KR(ret));
  } else if (OB_FAIL(ls_->remove_ls_inner_tablet(ls_id_, LS_MEMBER_TABLET))) {
    LOG_WARN("remove ls inner tablet failed", K(ret), K(ls_id_), K(LS_MEMBER_TABLET));
    if (OB_TABLET_NOT_EXIST == ret) {
      //do nothing
    }
  }
...
int ObMemberTable::create_tablet(
    const share::SCN &create_scn)
{
  ...
  if (!is_inited_) {
    ...
    ret = OB_INVALID_ARGUMENT;
    LOG_WARN("member table create tablet get invalid argument", K(ret), K(create_scn));
  } else if (OB_ISNULL(table_schema = ObMemberTableSchemaHelper::get_instance().get_table_schema())) {
    ret = OB_ERR_UNEXPECTED;
    ...
    LOG_WARN("failed to init storage schema", KR(ret), K(table_schema));
      ...
      create_scn))) {
    LOG_WARN("create member table tablet failed", K(ret), K(ls_id_),
        K(LS_MEMBER_TABLET), K(ObLS::LS_INNER_TABLET_FROZEN_SCN),
...
int ObMemberTable::update_already_recycled_scn(
...
int ObMemberTable::update_can_recycle_scn(
...
int ObMemberTable::get_can_recycle_scn(share::SCN &can_recycle_scn)
{
  int ret = OB_SUCCESS;
...
int ObMemberTable::init_tablet_for_compat()
{
  int ret = OB_SUCCESS;
  bool is_exist = false;
  ...
  if (!is_inited_) {
    ret = OB_NOT_INIT;
    LOG_WARN("member table is not init.", KR(ret));
  } else if (OB_FAIL(ls_->get_tablet_svr()->is_tablet_exist(LS_MEMBER_TABLET, is_exist))) {
...
ObMemberTable::RecycleSCNCache::RecycleSCNCache()
  : lock_(),
...
void ObMemberTable::RecycleSCNCache::reset()
{
...
int ObMemberTable::RecycleSCNCache::update_already_recycled_scn(
    const share::SCN &already_recycled_scn)
{
  int ret = OB_SUCCESS;
  common::SpinWLockGuard guard(lock_);
...
int ObMemberTable::RecycleSCNCache::update_can_recycle_scn(
    const share::SCN &can_recycle_scn)
{
  int ret = OB_SUCCESS;
  common::SpinWLockGuard guard(lock_);
...
SCN ObMemberTable::RecycleSCNCache::get_already_recycle_scn()
{
  common::SpinRLockGuard guard(lock_);
  return already_recycled_scn_;
}

SCN ObMemberTable::RecycleSCNCache::get_can_reycle_scn()
{
  common::SpinRLockGuard guard(lock_);
