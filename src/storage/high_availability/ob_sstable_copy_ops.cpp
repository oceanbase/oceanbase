    ...
    ObMergeType &merge_type)
{
  ...
  // No inc_major sstable type, so no is_inc_major_*_sstable predicates.
...
{
  int ret = OB_SUCCESS;
  // No inc_major_ddl_sstable predicate.
...
// now explicit params, and get_transfer_seq() supplies the transfer epoch.
// ---------------------------------------------------------------------------
    ...
    ObWholeDataStoreDesc &out_desc)
{
  ...
  if (OB_UNLIKELY(!tablet_id.is_valid() || cluster_version < 0)) {
    ...
    if (OB_UNLIKELY(cg_idx < 0 || cg_idx >= storage_schema->get_column_group_count())) {
      ret = OB_ERR_UNEXPECTED;
      LOG_WARN("unexpected cg idx", K(ret), K(cg_idx), KPC(storage_schema));
    } else {
    ...
    // NB: cluster_version param is intentionally unused -- desc.init takes 0
    // here to match the pre-extraction behavior of
    // ObSSTableCopyFinishTask::prepare_data_store_desc_.
    UNUSED(cluster_version);
    if (OB_SUCC(ret)) {
      const int64_t tablet_transfer_seq = tablet->get_transfer_seq();
      if (OB_FAIL(out_desc.init(
          false /*is_ddl*/,
          *storage_schema,
          ls.get_ls_id(),
          tablet_id,
        ...
        LOG_WARN("failed to init data store desc", K(ret), K(cg_idx), K(sstable_param), K(cg_schema));
      } else {
        const int64_t column_cnt = sstable_param.basic_meta_.column_cnt_;
        if (OB_FAIL(out_desc.get_col_desc().mock_valid_col_default_checksum_array(column_cnt))) {
          LOG_WARN("fail to mock valid col default checksum array", K(ret));
        } else if (OB_FAIL(out_desc.get_desc().update_basic_info_from_macro_meta(sstable_param.basic_meta_))) {
    ...
    ObCopyMacroBlockReaderInitParam &out_init_param)
{
  ...
  out_init_param.bandwidth_throttle_ = copy_ctx.bandwidth_throttle_;
  out_init_param.svr_rpc_proxy_ = copy_ctx.svr_rpc_proxy_;
  ...
  out_init_param.copy_macro_range_info_ = &copy_macro_range_info;
  ...
  if (OB_FAIL(build_data_version_for_macro_block_reuse_(
          copy_ctx, sstable_param, out_init_param))) {
    LOG_WARN("failed to build data version for macro block reuse",
        K(ret), K(copy_ctx), K(sstable_param));
  }
  return ret;
}

int ObSSTableCopyOps::build_data_version_for_macro_block_reuse_(
    const ObPhysicalCopyCtx &copy_ctx,
    const ObMigrationSSTableParam &sstable_param,
    ObCopyMacroBlockReaderInitParam &out_init_param)
{
  ...
  if (OB_ISNULL(copy_ctx.macro_block_reuse_mgr_)) {
    // Reuse is disabled for non-major keys.
  } else if (OB_FAIL(GET_MIN_DATA_VERSION(copy_ctx.tenant_id_, compat_version))) {
    LOG_WARN("failed to get min data version", K(ret), K(copy_ctx));
  } else if (compat_version < DATA_VERSION_4_3_4_0) {
    LOG_INFO("skip macro block reuse for compatibility",
        K(compat_version), K(copy_ctx));
  } else if (sstable_param.is_small_sstable_) {
    LOG_INFO("skip macro block reuse for small sstable", K(copy_ctx));
  } else if (OB_FAIL(copy_ctx.macro_block_reuse_mgr_->get_major_snapshot_version(
                 copy_ctx.table_key_, snapshot_version,
                 co_base_snapshot_version))) {
    if (OB_ENTRY_NOT_EXIST == ret) {
      ret = OB_SUCCESS;
      LOG_INFO("major snapshot version not found, skip macro block reuse",
          K(copy_ctx));
    } else {
      LOG_WARN("failed to get reusable major snapshot version",
          K(ret), K(copy_ctx));
    }
  } else if (FALSE_IT(src_co_base_snapshot_version =
                          sstable_param.basic_meta_.get_co_base_snapshot_version())) {
  } else if (co_base_snapshot_version != src_co_base_snapshot_version) {
    LOG_INFO("co base snapshot version mismatch, skip macro block reuse",
        K(snapshot_version), K(co_base_snapshot_version),
        K(src_co_base_snapshot_version), K(copy_ctx));
  } else {
    out_init_param.data_version_ = snapshot_version;
  }
  return ret;
}
    ...
    ObMacroBlocksWriteCtx &copied_ctx)
{
  ...
  if (OB_UNLIKELY(task_idx < 0 || copy_task_concurrent_cnt <= 0
    ...
    LOG_WARN("failed to init index block rebuilder", K(ret), K(copy_ctx.table_key_));
                 ...
                 copy_ctx, sstable_param, copy_macro_range_info, reader_param))) {
  ...
  if (OB_UNLIKELY(copy_ctx.is_leader_restore_)) {
    ...
    if (need_rebuild_index
      ...
      for (int64_t i = 0; OB_SUCC(ret) && i < range_cnt; ++i) {
        const ObCopyMacroRangeInfo &cur_range =
