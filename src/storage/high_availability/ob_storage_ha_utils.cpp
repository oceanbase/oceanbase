...
void ObTransferUtils::merge_transfer_diag(
    const share::ObLSID &ls_id,
    const share::ObTransferTaskID &task_id,
    const share::ObHAInflightDiagState &local)
{
  ...
  ObLSHandle ls_handle;
  if (!ls_id.is_valid()) {
    // diag info incomplete; skip silently.
  } else if (OB_FAIL(get_transfer_handler_(ls_handle, ls_id, transfer_handler))) {
    LOG_WARN("failed to get transfer handler for diag merge", K(ret), K(ls_id));
  } else if (OB_ISNULL(transfer_handler)) {
    // handler not yet initialized on this LS.
  } else if (OB_FAIL(transfer_handler->merge_external_diag(task_id, local))) {
    LOG_WARN("failed to merge external diag", K(ret), K(ls_id), K(task_id));
  }
}
