    ...
    ObScnRange &scn_range)
{
  ...
  if (!is_inited_) {
    ...
    if (OB_SUCC(ret)) {
      ...
      if (GET_MIN_CLUSTER_VERSION() < CLUSTER_VERSION_4_2_1_5) {
        if (OB_FAIL(get_minor_scn_before_4215_(tablet->get_tablet_meta().tablet_id_, scn_range.start_scn_, scn_range.end_scn_))) {
...
int ObStorageHATabletsBuilder::get_minor_scn_before_4215_(
    const common::ObTabletID &tablet_id,
    share::SCN &start_scn,
