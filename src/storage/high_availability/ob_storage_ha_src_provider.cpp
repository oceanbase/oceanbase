    ...
    obrpc::ObFetchLSMetaInfoResp &ls_info)
{
  ...
  if (!is_inited_) {
    ...
    LOG_WARN("do not choose this src, local checkpoint scn check failed",
        K(ret), K(tenant_id_), K(ls_id_), K(addr), K(dst), K(learner_list), K(local_clog_checkpoint_scn_), K(ls_info));
    ...
    LOG_WARN("do not choose this src, parent checkpoint scn check failed",
        K(ret), K(tenant_id_), K(ls_id_), K(addr), K(dst), K(learner_list), K(palf_parent_checkpoint_scn_), K(ls_info));
  }
  ...
  if (addr != dst.get_server()
      && OB_TMP_FAIL(choose_source_info_.add_choose_source_info(addr, result, ls_info, ret))) {
    LOG_WARN_RET(tmp_ret, "failed to add choose source info", K(ret), K(addr), K(result), K(src_replica_type), K(ls_info));
  }
