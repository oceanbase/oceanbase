    ...
    const common::ObReplicaType replica_type, share::SCN &parent_checkpoint_scn)
{
  ...
  } else if (OB_FAIL(check_ls_info_valid_(ls_info, is_ls_info_valid))) {
    LOG_WARN("failed to check ls info valid", K(ret), K(ls_info));
  } else if (!is_ls_info_valid) {
    ret = OB_ERR_UNEXPECTED;
    LOG_WARN("ls_info is invalid!", K(ret), K(ls_info));
  } else {
    ...
    const common::GlobalLearnerList &learner_list, obrpc::ObFetchLSMetaInfoResp &ls_info)
{
  ...
  } else if (OB_FAIL(check_replica_validity_(ls_info))) {
...
int ObStorageHASrcProvider::check_ls_info_valid_(const obrpc::ObFetchLSMetaInfoResp &ls_info, bool &is_valid) const
{
  int ret = OB_SUCCESS;
  is_valid = false;
  if (!ls_info.is_valid() && !ls_info.is_valid_for_4_1_0_0()) {
    ret = OB_INVALID_ARGUMENT;
    LOG_WARN("invalid argument", K(ret), K(ls_info));
  } else if (ls_info.version_ < CLUSTER_VERSION_4_2_1_0) {
    is_valid = ls_info.is_valid_for_4_1_0_0();
  } else {
    is_valid = ls_info.is_valid();
  }
  return ret;
}

int ObStorageHASrcProvider::check_replica_validity_(const obrpc::ObFetchLSMetaInfoResp &ls_info) const
{
  int ret = OB_SUCCESS;
  ObMigrationStatus migration_status;
  share::ObLSRestoreStatus restore_status;
  bool is_ls_info_valid = false;
  if (OB_FAIL(check_ls_info_valid_(ls_info, is_ls_info_valid))) {
    LOG_WARN("failed to check ls info valid", K(ret), K(ls_info));
  } else if (!is_ls_info_valid) {
    ret = OB_INVALID_ARGUMENT;
    LOG_WARN("ls_info is invalid!", K(ret), K(ls_info));
