...
ERRSIM_POINT_DEF(EN_UPDATE_ALL_SESSION_TABLET_TO_TEMPORARY_TABLE_FAILED);
...
#ifdef ERRSIM
#define TRANSFER_ERRSIM(errsim_point) \
  if (OB_SUCC(ret)) { \
    ret = errsim_point ? : OB_SUCCESS; \
    if (OB_FAIL(ret)) { \
      STORAGE_LOG(ERROR, "errsim " #errsim_point, K(ret)); \
    } \
  }
#else
#define TRANSFER_ERRSIM(errsim_point)
...
ObTransferHandler::~ObTransferHandler()
{
  ...
  if (OB_TMP_FAIL(diag_ctx_.flush_history(OB_SUCCESS))) {
    LOG_WARN_RET(tmp_ret, "failed to flush inflight diag on destroy");
  }
}
    ...
    common::ObMySQLProxy *sql_proxy)
{
  ...
  } else if (OB_FAIL(diag_ctx_.init(ls->get_ls_id()))) {
    LOG_WARN("failed to init diag ctx", K(ret), KP(ls));
  } else{
    ls_ = ls;
    ha_svc_ctx_.bandwidth_throttle_ = bandwidth_throttle;
    ha_svc_ctx_.svr_rpc_proxy_ = svr_rpc_proxy;
    ha_svc_ctx_.storage_rpc_ = storage_rpc;
    ha_svc_ctx_.sql_proxy_ = sql_proxy;
    is_inited_ = true;
  }
  return ret;
}
...
// query from __all_transfer_task
    ...
    bool &task_exist)
{
  ...
  if (OB_FAIL(ObTransferTaskOperator::get_by_src_ls(
      *ha_svc_ctx_.sql_proxy_, tenant_id, src_ls_id, task, share::OBCG_STORAGE))) {
    LOG_WARN("failed to get transfer task", K(ret), K(tenant_id), K(src_ls_id));
  } else if (OB_FAIL(task_info.convert_from(tenant_id, task))) {
    LOG_WARN("failed to convert from transfer task", K(ret), K(task));
  } else if (OB_FAIL(check_task_exist_(task_info, true/*find_by_src_ls*/, task_exist))) {
    LOG_WARN("failed to get task exist", K(ret), K(task_info));
  }
    ...
    bool &task_exist)
{
  ...
  if (OB_FAIL(ObTransferTaskOperator::get_by_dest_ls(
      *ha_svc_ctx_.sql_proxy_, tenant_id, dest_ls_id, task, share::OBCG_STORAGE))) {
    LOG_WARN("failed to get transfer task by dest ls", K(ret), K(tenant_id), K(dest_ls_id));
  } else if (OB_FAIL(task_info.convert_from(tenant_id, task))) {
    LOG_WARN("failed to convert from transfer task", K(ret), K(task));
  } else if (OB_FAIL(check_task_exist_(task_info, false/*find_by_src_ls*/, task_exist))) {
    LOG_WARN("failed to get task exist", K(ret), K(task_info));
  }
  if (OB_ENTRY_NOT_EXIST == ret || OB_TABLE_NOT_EXIST == ret) {
    task_exist = false;
...
int ObTransferHandler::do_leader_transfer_()
{
  ...
  if (!is_inited_) {
    ...
    if (diag_ctx_.is_registered()) {
      task_gone = true; // flush diag ctx
    }
    ...
    diag_ctx_.register_task(task_info.task_id_);
    diag_ctx_.set_tablet_count(task_info.tablet_list_.count());
    switch (task_info.status_) {
    case ObTransferStatus::START: {
      diag_ctx_.set_phase(share::ObStorageHADiagTaskType::TRANSFER_START);
    ...
    case ObTransferStatus::DOING : {
      diag_ctx_.set_phase(share::ObStorageHADiagTaskType::TRANSFER_DOING);
    ...
    case ObTransferStatus::ABORTED : {
      diag_ctx_.set_phase(share::ObStorageHADiagTaskType::TRANSFER_ABORT);
    ...
    // Each state step likely advances or ends the task; re-tick soon so the
    // next `task_gone` check doesn't wait up to `_transfer_service_wakeup_interval`.
    wakeup_();
  }
  if (task_gone) {
    LOG_INFO("[TRANSFER_DIAG] process() task_gone — fallback flush",
        "ls_id", ls_->get_ls_id(), K(ret), K(is_leader),
        "retry_count", diag_ctx_.get_retry_count());
    if (OB_TMP_FAIL(diag_ctx_.flush_history(ret))) {
      LOG_WARN("failed to flush inflight diag history", K(tmp_ret), K(ret));
    }
  }
  return ret;
}
...
int ObTransferHandler::do_with_start_status_(const share::ObTransferTaskInfo &task_info)
{
  ...
  if (!is_inited_) {
  ...
  } else if (OB_FAIL(init_transfer_ls_info_(task_info))) {
    ...
    if (OB_FAIL(ret)) {
    } else if (OB_FAIL(lock_src_and_dest_ls_member_and_learner_list_(task_info))) {
    ...
    } else if (OB_FAIL(ObTransferUtils::get_transfer_need_check_addr_list(task_info.src_ls_id_, task_info.dest_ls_id_,
        ls_transfer_info_, member_list, learner_list, addr_list))) {
      LOG_WARN("failed to get transfer need check addr list", K(ret), K(task_info));
    } else if (OB_FAIL(precheck_ls_replay_scn_(task_info, addr_list))) {
      LOG_WARN("failed to precheck ls replay scn", K(ret), K(task_info), K(addr_list));
    ...
    } else if (OB_FAIL(check_start_status_transfer_tablets_(task_info, addr_list, timeout_ctx))) {
      LOG_WARN("failed to check start status transfer tablets", K(ret), K(task_info), K(addr_list));
    ...
    OB_FAIL(do_trans_transfer_start_(task_info, config_version, dest_max_desided_scn, addr_list, timeout_ctx, trans, is_update_transfer_meta))) {
    ...
    OB_FAIL(do_trans_transfer_start_v2_(task_info, dest_max_desided_scn, addr_list, timeout_ctx, trans, is_update_transfer_meta))) {
      LOG_WARN("failed to do trans transfer start", K(ret), K(task_info));
    } else {
#ifdef ERRSIM
  if (OB_SUCC(ret)) {
    ...
    if (OB_FAIL(ret)) {
      if (timeout_ctx.is_timeouted()) {
        ...
        const int64_t elapsed_us = ObTimeUtil::current_time() - start_ts;
        LOG_WARN("[TRANSFER] transfer start trans already timeout, error code will change to OB_TIMEOUT. "
                 "consider increasing tenant config _transfer_start_trans_timeout if this happens repeatedly",
                 K(original_ret), K(elapsed_us), "configured_stmt_timeout_us", stmt_timeout,
                 "task_id", task_info.task_id_, "trace_id", task_info.trace_id_,
                 "src_ls", task_info.src_ls_id_, "dest_ls", task_info.dest_ls_id_,
                 "tablet_count", task_info.tablet_list_.count());
        ret = OB_TIMEOUT;
      }
    }
    ...
    diag_ctx_.update_cost_item(ret, ObStorageHACostItemName::START_TRANS_COMMIT);
  ...
  if (OB_FAIL(ret)) {
    if (!is_leader) {
      ...
      if (!new_transfer && OB_TMP_FAIL(unblock_tx_(task_info.tenant_id_, task_info.src_ls_id_, gts_seq_))) {
        diag_ctx_.update_cost_item(tmp_ret, ObStorageHACostItemName::UNLOCK_MEMBER_LIST_IN_START);
        LOG_WARN("failed to unblock tx", K(tmp_ret), K(ret));
      } else if (OB_TMP_FAIL(unlock_src_and_dest_ls_member_and_learner_list_(task_info, true/*need_check_palf_leader*/, task_info.src_ls_id_))) {
        LOG_WARN("failed to unlock src and dest ls member list", K(tmp_ret), K(ret), K(task_info));
      } else {
        diag_ctx_.set_cost_item(ObStorageHACostItemName::UNLOCK_MEMBER_LIST_IN_START);
      }
  ...
  if (OB_SUCCESS != (tmp_ret = record_server_event_(ret, round_, task_info))) {
    LOG_WARN("failed to record server event", K(tmp_ret), K(ret),
        "retry_count", diag_ctx_.get_retry_count(), K(task_info));
  }
  // if START stage execution failed and can retry, just wakeup self
  // if START stage execution succeeded or cannot retry, try to wakeup dest ls leader to go to DOING stage or ABORTED stage
  if (OB_FAIL(ret)) {
    if (can_retry) {
      wakeup_(); // wakeup self
    } else if (OB_TMP_FAIL(wakeup_dest_ls_leader_(task_info))) {
  ...
  TRANSFER_ERRSIM(EN_TRANSFER_DIAGNOSE_START_FAILED);
  diag_ctx_.update_cost_item(ret, ObStorageHACostItemName::TRANSFER_START_END);

  finish_parallel_tablet_info_dag_(task_info, timeout_ctx);

  LOG_INFO("[TRANSFER] finish do with start status", K(ret), K(task_info), "cost_ts", ObTimeUtil::current_time() - start_ts);
  return ret;
}

int ObTransferHandler::lock_src_and_dest_ls_member_and_learner_list_(
    const share::ObTransferTaskInfo &task_info)
{
  ...
  common::GlobalLearnerList learner_list;
  ObSEArray<share::ObLSID, 2> lock_ls_list;
  const uint64_t tenant_id = task_info.tenant_id_;
  ...
  if (!is_inited_) {
    ret = OB_NOT_INIT;
    LOG_WARN("transfer handler do not init", K(ret));
  } else if (!task_info.is_valid() || !src_ls_id.is_valid() || !dest_ls_id.is_valid()) {
    ret = OB_INVALID_ARGUMENT;
    LOG_WARN("lock src and dest ls member list get invalid argument", K(ret), K(task_info), K(status),
       K(src_ls_id), K(dest_ls_id), K(status));
  } else if (OB_FAIL(check_ls_member_list_and_learner_list_same_(src_ls_id, dest_ls_id, member_list, learner_list,
      !ls_transfer_info_.allow_learner_list_not_same(), is_same))) {
  ...
  } else if (OB_FAIL(ObMemberListLockUtils::batch_lock_ls_member_and_learner_list(tenant_id, task_id,
      lock_ls_list, member_list, learner_list, status, share::OBCG_STORAGE, src_ls_id, *ha_svc_ctx_.sql_proxy_))) {
    LOG_WARN("failed to batch lock ls member and learner list", K(ret));
  } else if (OB_FAIL(check_ls_member_list_and_learner_list_same_(src_ls_id, dest_ls_id, member_list, learner_list,
      !ls_transfer_info_.allow_learner_list_not_same(), is_same))) {
    LOG_WARN("failed to check ls member list same", K(ret), K(src_ls_id), K(dest_ls_id));
  } else if (!is_same) {
    ret = OB_TRANSFER_MEMBER_LIST_NOT_SAME;
    LOG_WARN("src ls and dest ls member list or learner list is not same", K(ret), K(src_ls_id), K(dest_ls_id));
  } else {
    TRANSFER_ERRSIM(EN_LOCK_TRANSFER_MEMBER_LIST_FAILED);
  }
  LOG_INFO("[TRANSFER] finish lock src and dest ls member list and learner list",
      K(src_ls_id), K(dest_ls_id), "cost_ts",  ObTimeUtil::current_time() - start_ts);
#ifdef ERRSIM
  ...
  diag_ctx_.update_cost_item(ret, ObStorageHACostItemName::LOCK_MEMBER_LIST);
  return ret;
}
...
int ObTransferHandler::unlock_src_and_dest_ls_member_and_learner_list_(
    const share::ObTransferTaskInfo &task_info,
    ...
    const share::ObLSID &need_check_palf_leader_ls_id)
{
  ...
  } else if (OB_FAIL(inner_unlock_ls_member_and_learner_list_(task_info, task_info.src_ls_id_, fake_member_list, fake_learner_list, status, need_check_palf_leader, need_check_palf_leader_ls_id))) {
    LOG_WARN("failed to inner unlock ls member and learner list", K(ret), K(task_info));
  } else if (OB_FAIL(inner_unlock_ls_member_and_learner_list_(task_info, task_info.dest_ls_id_, fake_member_list, fake_learner_list, status, need_check_palf_leader, need_check_palf_leader_ls_id))) {
    LOG_WARN("failed to inner unlock ls member and learner list", K(ret), K(task_info));
  }
  return ret;
}

int ObTransferHandler::inner_unlock_ls_member_and_learner_list_(
    const share::ObTransferTaskInfo &task_info,
    ...
    const share::ObLSID &need_check_palf_leader_ls_id)
{
  ...
  if (!is_inited_) {
    ret = OB_NOT_INIT;
    LOG_WARN("transfer handler do not init", K(ret));
  } else if (!task_info.is_valid() || !ls_id.is_valid() || !status.is_valid()) {
    ret = OB_INVALID_ARGUMENT;
    LOG_WARN("inner lock ls member list get invalid argument", K(ret), K(task_info), K(ls_id), K(member_list), K(status));
  } else if (OB_FAIL(ObMemberListLockUtils::unlock_ls_member_and_learner_list(
      tenant_id, ls_id, task_id, member_list, learner_list, status, share::OBCG_STORAGE, need_check_palf_leader, need_check_palf_leader_ls_id, *ha_svc_ctx_.sql_proxy_))) {
    LOG_WARN("failed to unlock ls member and learner list", K(ret), K(task_info), K(ls_id), K(member_list));
  }
  return ret;
}

int ObTransferHandler::check_ls_member_list_and_learner_list_same_(
    ...
    bool &is_same)
{
  ...
  if (!is_inited_) {
    ...
    ret = OB_INVALID_ARGUMENT;
    LOG_WARN("check ls member list and learner list same get invalid argument", K(ret), K(src_ls_id), K(dest_ls_id));
  } else if (OB_FAIL(ObTransferUtils::get_ls_member_list_and_learner_list(dest_ls_id, dest_ls_member_list, dest_ls_learner_list))) {
    LOG_WARN("failed to get dest ls member list and learner list", K(ret), KPC(ls_));
  } else if (OB_FAIL(ObTransferUtils::get_ls_member_list_and_learner_list(src_ls_id, src_ls_member_list, src_ls_learner_list))) {
    LOG_WARN("failed to get src ls member list and learner list", K(ret), K(src_ls_id));
  } else if (OB_FAIL(ObTransferUtils::check_list_is_same(dest_ls_member_list, src_ls_member_list, is_same))) {
    LOG_WARN("failed to check member list is same", K(ret), K(src_ls_id), K(dest_ls_id),
      K(src_ls_member_list), K(dest_ls_member_list));
  } else if (!is_same) {
    LOG_WARN("member list not same", K(ret), K(src_ls_id), K(dest_ls_id), K(src_ls_member_list), K(dest_ls_member_list));
  } else if (need_check_learner_list) {
    if (OB_FAIL(ObTransferUtils::check_list_is_same(dest_ls_learner_list, src_ls_learner_list, is_same))) {
      LOG_WARN("failed to check learner list is same", K(ret), K(src_ls_id), K(dest_ls_id),
        K(src_ls_learner_list), K(dest_ls_learner_list), K(need_check_learner_list));
    } else if (!is_same) {
      LOG_WARN("learner list not same", K(ret), K(src_ls_id), K(dest_ls_id), K(src_ls_learner_list),
        K(dest_ls_learner_list), K(need_check_learner_list));
    }
  }

  if (OB_SUCC(ret) && is_same) {
    if (OB_FAIL(member_list.deep_copy(src_ls_member_list))) {
      LOG_WARN("failed to deep copy member list", K(ret), K(src_ls_member_list));
    } else if (OB_FAIL(learner_list.deep_copy(src_ls_learner_list))) {
      LOG_WARN("failed to deep copy learner list", K(ret), K(src_ls_learner_list));
    }
  }
#ifdef ERRSIM
  if (OB_SUCC(ret)) {
    ...
    if (OB_FAIL(ret)) {
      LOG_WARN("errsim EN_TRANSFER_CHECK_MEMBER_LIST_NOT_SAME", K(ret), K(src_ls_id), K(dest_ls_id),
          K(src_ls_member_list), K(dest_ls_member_list));
    }
  }
#endif
    ...
    const int64_t expected_active_trans_count)
{
  ...
  if (!is_inited_) {
    ...
    TRANSFER_ERRSIM(EN_CHECK_ACTIVE_TRANS_FAILED);
    LOG_INFO("get active trans count", K(src_ls_id), K(active_trans_count));
  }
  diag_ctx_.update_cost_item(ret, ObStorageHACostItemName::CHECK_SRC_LS_HAS_ACTIVE_TRANS);
  return ret;
}
    ...
    share::SCN &new_mv_merge_scn)
{
  ...
  } else if (OB_FAIL(ObTransferUtils::get_ls_leader(task_info.dest_ls_id_, dest_ls_leader))) {
    ...
    ObTimeoutCtx &timeout_ctx)
{
  ...
  ObArray<obrpc::ObRpcProxy::NoneT> responses;
  ...
  if (!is_inited_) {
    ...
    ret = OB_TIMEOUT;
    LOG_WARN("check start status transfer tablet already timeout", K(ret), K(task_info));
  ...
  } else if (OB_FAIL(async_rpc_arg.set_ha_async_arg(task_info.tenant_id_, group_id, rpc_timeout, addr_list))) {
    LOG_WARN("failed to set ha async arg", K(ret), K(task_info), K(group_id), K(rpc_timeout), K(addr_list));
  } else {
  ...
  if (OB_SUCC(ret)) {
    ...
    TRANSFER_ERRSIM(EN_CHECK_START_TRANSFER_STATUS_FAILED);

  }
  diag_ctx_.update_cost_item(ret, ObStorageHACostItemName::CHECK_START_STATUS_TRANSFER_TABLETS);
  return ret;
}
    ...
    bool &is_update_transfer_meta)
{
  ...
  if (!is_inited_) {
    ret = OB_NOT_INIT;
    LOG_WARN("transfer handler do not init", K(ret));
  } else if (!task_info.is_valid() || !config_version.is_valid() || !dest_max_desided_scn.is_valid()) {
    ret = OB_INVALID_ARGUMENT;
    LOG_WARN("do trans transfer start get invalid argument", K(ret), K(task_info), K(config_version), K(dest_max_desided_scn));
  } else if (OB_FAIL(do_tx_start_transfer_out_(task_info, dest_max_desided_scn, trans,
  ...
  } else if (OB_FAIL(wait_src_ls_replay_to_start_scn_(task_info, start_scn, addr_list, timeout_ctx))) {
    ...
    LOG_WARN("failed to do parallel get transfer tablets meta", K(ret), K(task_info));
      ...
      && OB_FAIL(update_transfer_meta_info_(task_info, start_scn, addr_list, timeout_ctx, is_update_transfer_meta))) {
    ...
    ObMySQLTransaction &trans)
{
  ...
  } else if (OB_FAIL(ha_svc_ctx_.storage_rpc_->submit_tx_log(task_info.tenant_id_, addr_info, task_info.dest_ls_id_, data_scn))) {
    LOG_WARN("failed to submit tx log", K(ret), K(task_info));
    ...
    bool &is_update_transfer_meta)
{
  ...
  ObArray<ObTransID> move_tx_ids_obj;
  ObArray<ObTransID> *move_tx_ids = (task_info.data_version_ >= DATA_VERSION_4_3_2_0 ? &move_tx_ids_obj : nullptr);
  ...
  } else if (OB_FAIL(ObTransferUtils::get_ls_leader(task_info.src_ls_id_, src_ls_leader))) {
    LOG_WARN("failed to get src ls leader", K(ret), K(task_info));
  } else if (OB_FAIL(tablet_list.reserve(task_info.tablet_list_.count()))) {
    LOG_WARN("failed to reserve tablet list", K(ret), K(task_info));
  } else {
    for (int64_t idx = 0; OB_SUCC(ret) && idx < task_info.tablet_list_.count(); idx++) {
      if (OB_FAIL(tablet_list.push_back(task_info.tablet_list_.at(idx).tablet_id()))) {
  ...
  } else if (OB_NOT_NULL(move_tx_ids) && OB_FAIL(ls_->filter_tx_need_transfer(tablet_list, data_end_scn, *move_tx_ids))) {
  ...
  // transfer in
  } else if (ObTransferUtils::enable_transfer_dml_ctrl(task_info.data_version_) && OB_FAIL(update_transfer_meta_info_(task_info, start_scn, addr_list, timeout_ctx, is_update_transfer_meta))) {
    LOG_WARN("failed to update transfer meta info", K(ret), K(task_info));
  } else if (OB_FAIL(do_tx_start_transfer_in_(task_info, start_scn, timeout_ctx, trans))) {
    LOG_WARN("failed to do tx start transfer in", K(ret), K(task_info), K(start_scn));
    ...
    ObMySQLTransaction &trans)
{
  ...
  if (!is_inited_) {
    ret = OB_NOT_INIT;
    LOG_WARN("transfer handler do not init", K(ret));
  } else if (stmt_timeout <= 0 || group_id < 0) {
    ret = OB_INVALID_ARGUMENT;
    LOG_WARN("start trans get invalid argument", K(ret), K(stmt_timeout), K(group_id));
  } else if (OB_FAIL(timeout_ctx.set_trx_timeout_us(stmt_timeout))) {
    LOG_WARN("fail to set trx timeout", K(ret), K(stmt_timeout));
  } else if (OB_FAIL(timeout_ctx.set_timeout(stmt_timeout))) {
    LOG_WARN("set timeout context failed", K(ret));
  } else if (OB_FAIL(trans.start(ha_svc_ctx_.sql_proxy_, tenant_id, with_snapshot, group_id))) {
...
int ObTransferHandler::get_start_trans_timeout_(
...
int ObTransferHandler::get_abort_trans_timeout_(
    int64_t &stmt_timeout)
{
  int ret = OB_SUCCESS;
    ...
    common::ObISQLClient &trans)
{
  ...
  if (!is_inited_) {
    ...
    TRANSFER_ERRSIM(EN_LOCK_TRANSFER_TASK_FAILED);
  }
  return ret;
}
    ...
    ObIArray<ObTransID> *move_tx_ids)
{
  ...
  const int64_t start_ts = ObTimeUtil::current_time();
  share::SCN src_max_desided_scn;

  if (!is_inited_) {
    ret = OB_NOT_INIT;
    LOG_WARN("transfer handler do not init", K(ret));
  } else if (!task_info.is_valid() || !dest_max_desided_scn.is_valid()) {
    ret = OB_INVALID_ARGUMENT;
    LOG_WARN("do tx start transfer out get invalid argument", K(ret), K(task_info), K(dest_max_desided_scn));
  } else if (OB_FAIL(ls_->get_max_decided_scn(src_max_desided_scn))) {
    ...
    } else if (OB_NOT_NULL(move_tx_ids)) {
    ...
    if (OB_SUCC(ret)) {
      ...
      flag.mds_base_scn_ = std::max(src_max_desided_scn, dest_max_desided_scn);
      if (OB_ISNULL(buf)) {
    ...
    TRANSFER_ERRSIM(EN_START_TRANSFER_OUT_FAILED);

    DEBUG_SYNC(AFTER_START_TRANSFER_OUT);

  }
  diag_ctx_.update_cost_item(ret, ObStorageHACostItemName::REGISTER_TRANSFER_START_OUT);
  return ret;
}
    ...
    share::SCN &start_scn)
{
  ...
  if (!is_inited_) {
    ...
    TRANSFER_ERRSIM(EN_GET_TRANSFER_START_SCN_FAILED);
    DEBUG_SYNC(AFTER_START_TRANSFER_GET_START_SCN);
  }
  diag_ctx_.update_cost_item(ret, ObStorageHACostItemName::DEST_LS_GET_START_SCN);
  return ret;
}
    ...
    ObTimeoutCtx &timeout_ctx)
{
  ...
  } else if (OB_FAIL(wait_ls_replay_event_(task_info.src_ls_id_, task_info, addr_list, start_scn, group_id, timeout_ctx))) {
    LOG_WARN("failed to wait ls replay event", K(ret), K(task_info), K(addr_list), K(start_scn));
  } else {
  ...
  TRANSFER_ERRSIM(EN_WAIT_SRC_REPALY_TO_START_SCN_FAILED);
  diag_ctx_.update_cost_item(ret, ObStorageHACostItemName::WAIT_SRC_LS_REPLAY_TO_START_SCN);
  return ret;
}

int ObTransferHandler::precheck_ls_replay_scn_(
    const share::ObTransferTaskInfo &task_info,
    const common::ObIArray<ObAddr> &addr_list)
{
  int ret = OB_SUCCESS;
  ...
  if (OB_FAIL(ret)) {
    ...
    LOG_WARN("failed to get max decided scn", K(ret), K(task_info));
  } else if (OB_FAIL(wait_ls_replay_event_(task_info.src_ls_id_, task_info, addr_list, check_scn, group_id, timeout_ctx))) {
    LOG_WARN("failed to wait ls replay event", K(ret), K(task_info), K(addr_list), K(check_scn));
    if (OB_TIMEOUT == ret) {
  ...
  diag_ctx_.update_cost_item(ret, ObStorageHACostItemName::PRECHECK_LS_REPALY_SCN);
  return ret;
}
...
int ObTransferHandler::wait_ls_replay_event_(
    const share::ObLSID &ls_id,
    const share::ObTransferTaskInfo &task_info,
    const common::ObIArray<ObAddr> &total_addr_list,
...
// use MDS to create transfer in tablets in trans
    ...
    common::ObMySQLTransaction &trans)
{
  ...
  diag_ctx_.update_cost_item(ret, ObStorageHACostItemName::REGISTER_TRANSFER_START_IN);
  return ret;
}
    ...
    common::ObISQLClient &trans)
{
  ...
  if (!is_inited_) {
    ...
    TRANSFER_ERRSIM(EN_UPDATE_ALL_TABLET_TO_LS_FAILED);
  ...
  diag_ctx_.update_cost_item(ret, ObStorageHACostItemName::UPDATE_ALL_TABLET_TO_LS);
  return ret;
}
  ...
  common::ObISQLClient &trans)
{
  ...
  if (!is_inited_) {
    ...
    TRANSFER_ERRSIM(EN_UPDATE_ALL_SESSION_TABLET_TO_TEMPORARY_TABLE_FAILED);
    ...
    common::ObMySQLTransaction &trans)
{
  ...
  diag_ctx_.update_cost_item(ret, ObStorageHACostItemName::LOCK_TABLET_ON_DEST_LS_FOR_TABLE_LOCK);
  return ret;
}
    ...
    common::ObISQLClient &trans)
{
  ...
  if (!is_inited_) {
    ...
    TRANSFER_ERRSIM(EN_UPDATE_TRANSFER_TASK_FAILED);
    }
  }
  return ret;
}
    ...
    const int32_t result)
{
  ...
  if (!task_info.is_valid()) {
    ...
    bool_ret = true;
    diag_ctx_.inc_retry_count();
  } else if (ObTransferStatus::START == task_info.status_) {
    if (ObTransferUtils::is_need_retry_error(result)
        && diag_ctx_.get_retry_count() < max_transfer_start_retry_count) {
      diag_ctx_.inc_retry_count();
      bool_ret = true;
    } else {
      bool_ret = false;
...
int ObTransferHandler::do_with_doing_status_(const share::ObTransferTaskInfo &task_info)
{
  ...
  const share::ObLSID &src_ls_id = task_info.src_ls_id_;
  const share::ObLSID &dest_ls_id = task_info.dest_ls_id_;
  diag_ctx_.set_cost_item(ObStorageHACostItemName::TRANSFER_FINISH_BEGIN);
#ifdef ERRSIM
  ...
  if (!task_info.is_valid()) {
    ...
    LOG_WARN("failed to init transfer ls info", K(ret), K(task_info));
      ...
      task_id, tenant_id, src_ls_id, dest_ls_id, *ha_svc_ctx_.sql_proxy_, &ls_transfer_info_))) {
    LOG_WARN("failed to init finish transfer", K(ret), K(task_info));
  }
  if (OB_FAIL(ret)) {
    diag_ctx_.record_error(ret, ObStorageHACostItemName::TRANSFER_FINISH_BEGIN);
  } else if (OB_FAIL(finish_transfer.process(round_))) {
    diag_ctx_.record_error(ret, ObStorageHACostItemName::TRANSFER_FINISH_END);
    LOG_WARN("failed to process", K(ret));
  } else {
    diag_ctx_.set_cost_item(ObStorageHACostItemName::TRANSFER_FINISH_END);
  ...
  // Note: do NOT call diag_ctx_.finalize here on success. `finish_transfer.
  // process(round_)` returns OB_SUCCESS per-tick-without-error, not "task
  // actually committed". The real terminator is `get_transfer_task_` coming
  // back OB_ENTRY_NOT_EXIST on a later tick — that's what fires the task_gone
  // fallback flush. Calling finalize here would write one history row per
  // tick for a task that's still making progress.
  return ret;
}
...
{
  int ret = OB_SUCCESS;
  LOG_INFO("[TRANSFER_DIAG] do_with_aborted_status_ enter",
      "task_id", task_info.task_id_, "status", task_info.status_);
  if (!is_inited_) {
    ret = OB_NOT_INIT;
    LOG_WARN("transfer handler do not init", K(ret));
  } else if (!task_info.is_valid()) {
    ...
    LOG_INFO("[TRANSFER_DIAG] do_with_aborted_status_ (before_4230)",
        "task_id", task_info.task_id_, "data_version", task_info.data_version_);
    if (OB_FAIL(inner_do_with_abort_status_before_4230_(task_info))) {
      LOG_WARN("failed to inner do with abort status before 4230", K(ret), K(task_info));
    }
  } else {
    LOG_INFO("[TRANSFER_DIAG] do_with_aborted_status_ (new)",
        "task_id", task_info.task_id_, "data_version", task_info.data_version_);
    if (OB_FAIL(inner_do_with_abort_status_(task_info))) {
      LOG_WARN("failed to inner do with abort status", K(ret), K(task_info));
    }
  }
  LOG_INFO("[TRANSFER_DIAG] do_with_aborted_status_ return",
      "task_id", task_info.task_id_, K(ret));
  // Flush is handled uniformly by the next tick's task_gone path once the
  // task row is removed from __all_transfer_task — same rationale as the
  // DOING branch.
  return ret;
}

int ObTransferHandler::init_transfer_ls_info_(const share::ObTransferTaskInfo &task_info)
{
  int ret = OB_SUCCESS;
  ls_transfer_info_.reset();
  if (!is_inited_) {
    ret = OB_NOT_INIT;
    LOG_WARN("transfer handler do not init", K(ret));
  } else if (!task_info.is_valid()) {
    ret = OB_INVALID_ARGUMENT;
    LOG_WARN("init transfer ls info get invalid argument", K(ret), K(task_info));
  } else if (OB_FAIL(ObTransferUtils::get_transfer_ls_info(task_info.tenant_id_,
      task_info.src_ls_id_, task_info.dest_ls_id_, *ha_svc_ctx_.sql_proxy_, ls_transfer_info_))) {
    LOG_WARN("failed to get transfer ls info", K(ret), K(task_info));
  }
  return ret;
}
    ...
    bool &succ_block_tx)
{
  ...
  diag_ctx_.update_cost_item(ret, ObStorageHACostItemName::BLOCK_AND_KILL_TX);
  return ret;
}
    ...
    const share::SCN &gts)
{
  ...
  diag_ctx_.update_cost_item(ret, ObStorageHACostItemName::UNBLOCK_TX);
  return ret;
}
...
int ObTransferHandler::stop_tablets_schedule_medium_(const ObIArray<ObTabletID> &tablet_ids, bool &succ_stop)
{
  ...
  if (OB_FAIL(ret)) {
    diag_ctx_.record_error(ret, ObStorageHACostItemName::STOP_LS_SCHEDULE_MEMDIUM);
  }
  return ret;
}
    ...
    ObTimeoutCtx &timeout_ctx)
{
  ...
  ObStorageHASrcInfo addr_info;
  addr_info.cluster_id_ = GCONF.cluster_id;
  if (OB_FAIL(ObTransferUtils::get_ls_leader(task_info.dest_ls_id_, dest_ls_leader))) {
    LOG_WARN("failed to get src ls leader", K(ret), K(task_info));
  } else if (FALSE_IT(addr_info.src_addr_ = dest_ls_leader)) {
  } else {
     ...
     while (OB_SUCC(ret)) {
       if (OB_FAIL(ha_svc_ctx_.storage_rpc_->get_transfer_dest_prepare_scn(task_info.tenant_id_,
                                                               addr_info,
                                                               task_info.dest_ls_id_,
    ...
    bool &task_exist) const
{
  ...
  if (!task_info.is_valid()) {
    ret = OB_INVALID_ARGUMENT;
    LOG_WARN("check task exist get invalid argument", K(ret), K(task_info));
  } else if (!ObTransferUtils::enable_transfer_dml_ctrl(task_info.data_version_)) {
    if (OB_FAIL(inner_check_task_exist_before_4230_(task_info.status_, find_by_src_ls, task_exist))) {
      LOG_WARN("failed to do inner check task exist before 4230", K(ret), K(task_info));
    }
  } else {
    if (OB_FAIL(inner_check_task_exist_(task_info.status_, find_by_src_ls, task_exist))) {
      LOG_WARN("failed to do inner check task exist", K(ret), K(task_info));
    }
  }
  return ret;
}

// Only src ls could work when task status is START.
// Conversely dest ls work when task status is DOING or ABORT.
...
// The ABORT status will change to FAILED status in dest ls work time.
int ObTransferHandler::inner_check_task_exist_(
    const ObTransferStatus &status, const bool find_by_src_ls, bool &task_exist) const
{
  ...
  } else if (find_by_src_ls && status.is_start_status()) {
    task_exist = true;
  } else if (!find_by_src_ls && (status.is_doing_status() || status.is_aborted_status())) {
    task_exist = true;
  } else {
    task_exist = false;
  }
...
int ObTransferHandler::inner_check_task_exist_before_4230_(
    const ObTransferStatus &status, const bool find_by_src_ls, bool &task_exist) const
{
  int ret = OB_SUCCESS;
  task_exist = false;
...
int ObTransferHandler::wait_transfer_in_tablet_abort_(
    const share::ObTransferTaskInfo &task_info,
    const common::ObMemberList &member_list)
{
  int ret = OB_SUCCESS;
  ...
  if (!is_inited_) {
    ret = OB_NOT_INIT;
    LOG_WARN("transfer handler do not init", K(ret));
  } else if (!task_info.is_valid() || !member_list.is_valid()) {
    ret = OB_INVALID_ARGUMENT;
    LOG_WARN("wait transfer in tablet abort get invalid argument", K(ret), K(task_info));
  } else if (OB_FAIL(member_list.get_addr_array(total_addr_list))) {
    LOG_WARN("failed to get addr array", K(ret), K(task_info));
  } else if (OB_FAIL(timeout_ctx.set_timeout(OB_CHECK_TRANSFER_IN_ABORT_TIMEOUT))) {
    LOG_WARN("failed to set timeout", K(ret), K(task_info));
  } else if (OB_FAIL(get_max_decided_scn_(task_info.tenant_id_, task_info.dest_ls_id_, max_decided_scn))) {
    LOG_WARN("failed to get max decided scn", K(ret), K(task_info));
  } else if (OB_FAIL(wait_ls_replay_event_(task_info.dest_ls_id_, task_info, total_addr_list, max_decided_scn, group_id, timeout_ctx))) {
    LOG_WARN("failed to wait ls replay event", K(ret), K(task_info), K(total_addr_list), K(max_decided_scn));
  } else {
    while (OB_SUCC(ret)) {
      if (timeout_ctx.is_timeouted()) {
        ret = OB_TIMEOUT;
...
int ObTransferHandler::do_trans_transfer_aborted_(
    const share::ObTransferTaskInfo &task_info,
    ObTimeoutCtx &timeout_ctx,
    ObMySQLTransaction &trans)
{
  LOG_INFO("start do transfer in aborted", K(task_info));
  int ret = OB_SUCCESS;
  observer::ObInnerSQLConnection *conn = NULL;
  ...
  if (!is_inited_) {
    ...
    ret = OB_TIMEOUT;
    LOG_WARN("already timeout", K(ret), K(task_info));
  } else if (OB_FAIL(ObTransferUtils::get_gts(task_info.tenant_id_, gts))) {
    LOG_WARN("failed to get gts", K(ret), K(task_info));
  } else if (OB_ISNULL(conn = static_cast<observer::ObInnerSQLConnection *>(trans.get_connection()))) {
    ret = OB_ERR_UNEXPECTED;
    LOG_WARN("conn_ is NULL", KR(ret));
  } else {
    transfer_in_aborted_info.dest_ls_id_ = task_info.dest_ls_id_;
    transfer_in_aborted_info.data_version_ = task_info.data_version_;
    if (OB_FAIL(transfer_in_aborted_info.tablet_list_.assign(task_info.tablet_list_))) {
      LOG_WARN("failed to assign transfer tablet list", K(ret), K(task_info));
    } else {
      int64_t buf_len = transfer_in_aborted_info.get_serialize_size();
      int64_t pos = 0;
      char *buf = (char*)allocator.alloc(buf_len);
      ObRegisterMdsFlag flag;
      flag.need_flush_redo_instantly_ = true;
      flag.mds_base_scn_ = gts;
      if (OB_ISNULL(buf)) {
        ret = OB_ALLOCATE_MEMORY_FAILED;
        LOG_WARN("fail alloc memory", KR(ret));
      } else if (OB_FAIL(transfer_in_aborted_info.serialize(buf, buf_len, pos))) {
        LOG_WARN("fail to serialize start transfer out info", KR(ret), K(transfer_in_aborted_info));
      } else if (OB_FAIL(conn->register_multi_data_source(task_info.tenant_id_, task_info.dest_ls_id_,
          transaction::ObTxDataSourceType::TRANSFER_IN_ABORTED, buf, buf_len, flag))) {
        LOG_WARN("failed to register multi data source", K(ret), K(task_info));
      } else {
        LOG_INFO("[TRANSFER] success register transfer in aborted", "cost", ObTimeUtil::current_time() - start_ts);
      }
    }
  }
  return ret;
    ...
    bool &is_update_transfer_meta)
{
  ...
  storage::ObUpdateTransferMetaInfoProxy batch_rpc_proxy(
      *(GCTX.storage_rpc_proxy_), &obrpc::ObStorageRpcProxy::update_transfer_meta_info);
  const int64_t rpc_timeout = timeout_ctx.get_timeout();
  ...
  if (!is_inited_) {
    ...
    ret = OB_INVALID_ARGUMENT;
    LOG_WARN("do tx start transfer out get invalid argument", K(ret), K(task_info), K(start_scn));
  } else if (rpc_timeout < 0) {
    ret = OB_TIMEOUT;
    LOG_WARN("update transfer meta info is already timeout", K(ret), K(task_info));
  } else if (OB_FAIL(build_transfer_meta_info_(task_info, start_scn, transfer_meta_info))) {
    LOG_WARN("failed to build transfer meta info", K(ret), K(task_info));
  } else if (OB_FAIL(async_rpc_arg.set_ha_async_arg(tenant_id, group_id, rpc_timeout, addr_list))) {
    LOG_WARN("failed to set ha async arg", K(ret), K(tenant_id), K(group_id), K(rpc_timeout), K(addr_list));
  } else {
...
#endif
    if (responses.count() > addr_list.count()) {
      tmp_ret = OB_ERR_UNEXPECTED;
      ret = OB_SUCCESS == ret ? tmp_ret : ret;
      LOG_WARN("unexpected responses", K(ret), K(tmp_ret), K(responses), K(addr_list));
...
int ObTransferHandler::build_transfer_meta_info_(
    const share::ObTransferTaskInfo &task_info,
    const share::SCN &start_scn,
    ObLSTransferMetaInfo &transfer_meta_info)
{
  ...
  if (!is_inited_) {
    ...
    for (int64_t i = 0; OB_SUCC(ret) && i < task_info.tablet_list_.count(); ++i) {
      const ObTransferTabletInfo &tablet_info = task_info.tablet_list_.at(i);
      if (OB_FAIL(tablet_id_list.push_back(tablet_info.tablet_id_))) {
        LOG_WARN("failed to push tablet id into list", K(ret), K(tablet_info));
      }
    }

    if (OB_SUCC(ret)) {
      if (OB_FAIL(transfer_meta_info.set_transfer_info(task_info.src_ls_id_, start_scn, trans_status, tablet_id_list, task_info.data_version_))) {
        LOG_WARN("failed to set transfer info", K(ret), K(task_info), K(start_scn), K(trans_status));
      }
    }
  }
  return ret;
    ...
    share::SCN &ls_decided_scn)
{
  ...
  } else if (OB_FAIL(ObTransferUtils::get_ls_leader(ls_id, leader_addr))) {
...
int ObTransferHandler::get_local_ls_member_list_and_learner_list_(
    common::ObMemberList &member_list,
    common::GlobalLearnerList &learner_list)
{
  int ret = OB_SUCCESS;
  ...
  if (IS_NOT_INIT) {
    ret = OB_NOT_INIT;
    LOG_WARN("ls transfer handler do not init", K(ret));
  } else if (ls_transfer_info_.must_get_learner_list_from_src_ls()) {
    ls = ls_;
  } else if (ls_transfer_info_.must_get_learner_list_from_dest_ls()) {
    if (OB_FAIL(MTL(ObLSService*)->get_ls(task_info_.dest_ls_id_, ls_handle, ObLSGetMod::HA_MOD))) {
      LOG_WARN("failed to get ls handle", K(ret), K(task_info_));
    } else if (OB_ISNULL(ls = ls_handle.get_ls())) {
      ret = OB_ERR_UNEXPECTED;
      LOG_WARN("ls should not be NULL", K(ret), KP(ls), K(task_info_));
    }
  } else {
    ls = ls_;
  }
...
int ObTransferHandler::check_transfer_in_tablet_abort_(
    const share::ObTransferTaskInfo &task_info,
...
{
  int ret = OB_SUCCESS;
  storage::ObCheckTransferInTabletAbortProxy batch_rpc_proxy(
      *(GCTX.storage_rpc_proxy_), &obrpc::ObStorageRpcProxy::check_transfer_in_tablet_aborted);
  const uint64_t group_id = share::OBCG_STORAGE;
  ObTransferTabletInfoArg arg;
  const int64_t rpc_timeout = timeout_ctx.get_timeout();
  ObHAAsyncRpcArg async_rpc_arg;
  ObArray<obrpc::ObTransferInTabletAbortedRes> responses;
  ...
  if (IS_NOT_INIT) {
    ...
    ret = OB_INVALID_ARGUMENT;
    LOG_WARN("check transfer in tablet abort get invalid argument", K(ret), K(task_info));
  } else if (rpc_timeout < 0) {
    ret = OB_TIMEOUT;
    LOG_WARN("check transfer in tablet abort is already timeout", K(ret), K(task_info));
  } else if (OB_FAIL(async_rpc_arg.set_ha_async_arg(task_info.tenant_id_, group_id, rpc_timeout, member_addr_list))) {
    LOG_WARN("failed to set ha async arg", K(ret), K(task_info), K(group_id), K(rpc_timeout), K(member_addr_list));
  } else {
    arg.tenant_id_ = task_info.tenant_id_;
    arg.src_ls_id_ = task_info.src_ls_id_;
    arg.dest_ls_id_ = task_info.dest_ls_id_;
    if (OB_FAIL(arg.tablet_list_.assign(task_info.tablet_list_))) {
      LOG_WARN("failed to assign tablet array", K(ret), K(task_info));
    } else if (OB_FAIL(ObHAAsyncRpc::send_async_rpc(async_rpc_arg, arg, batch_rpc_proxy, responses))) {
      LOG_WARN("failed to send async rpc", K(ret), K(async_rpc_arg), K(arg));
    } else {
      for (int64_t i = 0; OB_SUCC(ret) && i < responses.count(); ++i) {
        const obrpc::ObTransferInTabletAbortedRes &res = responses.at(i);
        if (res.is_aborted_ && OB_FAIL(finished_addr_list.push_back(member_addr_list.at(i)))) {
...
int ObTransferHandler::inner_do_with_abort_status_(
    const share::ObTransferTaskInfo &task_info)
{
  int ret = OB_SUCCESS;
  int tmp_ret = OB_SUCCESS;
  ...
  if (!is_inited_) {
  ...
  } else if (OB_FAIL(unlock_src_and_dest_ls_member_and_learner_list_(task_info, false/*need_check_palf_leader*/, task_info.dest_ls_id_))) {
    diag_ctx_.record_error(ret, ObStorageHACostItemName::UNLOCK_MEMBER_LIST_IN_ABORT);
    ...
    if (OB_FAIL(lock_transfer_task_(task_info, trans))) {
      LOG_WARN("failed to lock transfer task", K(ret), K(task_info));
    } else if (OB_FAIL(get_local_ls_member_list_and_learner_list_(member_list, learner_list))) {
...
#endif
      diag_ctx_.set_cost_item(ObStorageHACostItemName::UNLOCK_MEMBER_LIST_IN_ABORT);
    }
    ...
    if (OB_TMP_FAIL(commit_trans_(ret, trans))) {
      ...
      if (OB_SUCCESS == ret) {
        ret = tmp_ret;
        diag_ctx_.record_error(ret, ObStorageHACostItemName::ABORT_TRANS_COMMIT);
      }
    } else if (OB_SUCCESS == ret) {
      round_ = 0;
    }
    if (OB_SUCC(ret)) {
      diag_ctx_.set_cost_item(ObStorageHACostItemName::ABORT_TRANS_COMMIT);
    }
  }
...
#endif
    if (need_unlock && OB_SUCCESS != (tmp_ret = inner_unlock_ls_member_and_learner_list_(
        task_info, task_info.dest_ls_id_, member_list, learner_list, status, true/*need_check_palf_leader*/, task_info.dest_ls_id_))) {
      LOG_WARN("failed to unlock dest ls member and learner list", K(tmp_ret), K(task_info));
    }
  ...
  const int before_errsim_ret = ret;
  TRANSFER_ERRSIM(EN_TRANSFER_DIAGNOSE_ABORT_FAILED);
  LOG_INFO("[TRANSFER_DIAG] abort tail", "task_id", task_info.task_id_,
      K(before_errsim_ret), "after_errsim_ret", ret);
  diag_ctx_.update_cost_item(ret, ObStorageHACostItemName::TRANSFER_ABORT_END);
  return ret;
}

int ObTransferHandler::inner_do_with_abort_status_before_4230_(
    const share::ObTransferTaskInfo &task_info)
{
  int ret = OB_SUCCESS;
  int tmp_ret = OB_SUCCESS;
  ...
  if (!is_inited_) {
    ...
    if (OB_FAIL(start_trans_(stmt_timeout, group_id, timeout_ctx, trans))) {
      LOG_WARN("failed to start trans", K(ret), K(task_info));
    } else {
      const SCN scn = task_info.start_scn_;
      const int32_t result = task_info.result_;
    ...
    ObTimeoutCtx &timeout_ctx)
{
  ...
  if (!is_inited_) {
    ret = OB_NOT_INIT;
    LOG_WARN("transfer handler do not init", K(ret));
  } else if (OB_FAIL(generate_parallel_tablet_info_dag_(task_info, timeout_ctx))) {
  ...
  TRANSFER_ERRSIM(EN_GET_TRANSFER_TABLET_META_FAILED);

  DEBUG_SYNC(AFTER_START_TRANSFER_GET_TABLET_META);
  diag_ctx_.update_cost_item(ret, ObStorageHACostItemName::SRC_LS_GET_TABLET_META);
    ...
    ObTimeoutCtx &timeout_ctx)
{
  ...
  } else if (OB_FAIL(scheduler->check_dag_exist(&fake_dag, exist, unused_is_emergency))) {
    ...
    ObTimeoutCtx &timeout_ctx)
{
  ...
  } else if (OB_FAIL(ObTransferBuildTabletInfoHelper::loop_to_build_tablet_infos(ls_, timeout_ctx, ctx_))) {
    LOG_WARN("failed to build tablet infos", K(ret), K(task_info));
  }
  return ret;
}
    ...
    ObTimeoutCtx &timeout_ctx)
{
  ...
  } else if (OB_FAIL(wait_parallel_tablet_info_dag_finish_(task_info, timeout_ctx))) {
