...
int ObTabletCopyFinishTask::init(const ObTabletCopyFinishTaskParam &param)
{
  ...
  if (is_inited_) {
    ...
    LOG_WARN("failed to set copy tablet status", K(ret), K(param));
  } else {
    param_ = param;
...
int ObTabletCopyFinishTask::process()
{
  ...
  if (!is_inited_) {
    ...
    LOG_WARN("failed to create new table store with ddl", K(ret), K_(param));
    ...
    LOG_WARN("failed to update tablet data status", K(ret), K(param_));
  }
...
int ObTabletCopyFinishTask::create_new_table_store_with_ddl_()
{
  ...
  if (!is_inited_) {
    ...
    FLOG_INFO("only replace major, no need build ddl tables", K(ret), "tablet_id", param_.tablet_id_);
  } else if (OB_FAIL(ObStorageHATabletBuilderUtil::build_table_with_ddl_tables(param_.ls_, param_.tablet_id_, ddl_tables_handle_))) {
...
int ObTabletCopyFinishTask::create_new_table_store_with_minor_()
{
  ...
  } else if (OB_FAIL(ObStorageHATabletBuilderUtil::build_table_with_minor_tables(param_.ls_, param_.tablet_id_,
      param_.src_tablet_meta_, minor_tables_handle_, param_.restore_action_))) {
    LOG_WARN("failed to build table with ddl tables", K(ret));
  }
  return ret;
}
...
int ObTabletCopyFinishTask::create_new_table_store_with_major_()
{
  ...
  if (!is_inited_) {
    ...
    // do nothing
  } else if (OB_FAIL(ObStorageHATabletBuilderUtil::build_tablet_with_major_tables(param_.ls_,
      param_.tablet_id_, major_tables_handle_, param_.src_tablet_meta_->storage_schema_,
      param_.src_tablet_meta_->medium_info_list_, param_.is_only_replace_major_))) {
    LOG_WARN("failed to build tablet with major tables", K(ret), K(major_tables_handle_), K(param_));
  }
  return ret;
}
...
int ObTabletCopyFinishTask::check_finish_copy_tablet_data_valid_()
{
  ...
  if (!is_inited_) {
    ...
    ret = OB_ERR_UNEXPECTED;
    LOG_WARN("tablet should not be NULL", K(ret), K(param_), KP(tablet));
    ...
    if (OB_SUCC(ret)) {
      int tmp_ret = OB_SUCCESS;
      if (OB_SUCCESS != (tmp_ret = param_.reporter_->submit_tablet_update_task(param_.ls_->get_tenant_id(),
          param_.ls_->get_ls_id(), param_.tablet_id_))) {
        LOG_WARN("failed to submit tablet update task", K(tmp_ret), K(param_));
      }
    }
  }
  return ret;
