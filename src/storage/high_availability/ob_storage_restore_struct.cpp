...
int ObTabletRestoreAction::need_restore_sstable(const ACTION &action, const ObITable::TableType table_type, bool &need_restore)
{
  ...
  } else if (ObITable::is_inc_major_ddl_dump_sstable(table_type)) {
    need_restore = need_restore_inc_major_ddl_sstable(action);
  } else if (ObITable::is_inc_major_type_sstable(table_type)) {
    need_restore = need_restore_inc_major_sstable(action);
  } else {
    ret = OB_ERR_UNEXPECTED;
