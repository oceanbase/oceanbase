...
int ObParallelCreateTabletsInfo::record_tablet_simple_info_(
    ObMigrationCtx &ctx,
    const obrpc::ObCopyTabletInfo &tablet_info)
{
  // The lock must be held by caller. tablet_simple_info_map_ carries the
  // status_/data_size_ that ObDataTabletsMigrationTask::build_tablet_group_info_
  // relies on; missing it makes every data tablet be skipped as "not exist".
  int ret = OB_SUCCESS;
  ...
  if (OB_FAIL(ctx.tablet_simple_info_map_.set_refactored(
      tablet_info.tablet_id_, tablet_simple_info, overwrite))) {
    LOG_WARN("failed to set tablet simple info into map", K(ret), K(tablet_info), K(tablet_simple_info));
  }
  return ret;
}
    ...
    const obrpc::ObCopyTabletInfo &tablet_info)
{
  ...
  if (OB_FAIL(logic_tablet_id.init(tablet_info.tablet_id_,
    ...
    } else if (OB_FAIL(record_tablet_simple_info_(ctx, tablet_info))) {
      LOG_WARN("failed to record tablet simple info", K(ret), K(tablet_info));
    }
  }
  return ret;
}

int ObParallelCreateTabletsInfo::record_data_tablet_id(
    ObMigrationCtx &ctx,
    const obrpc::ObCopyTabletInfo &tablet_info)
{
  int ret = OB_SUCCESS;
  ...
  if (OB_FAIL(logic_tablet_id.init(tablet_info.tablet_id_,
    ...
    common::ObSpinLockGuard guard(lock_);
    if (OB_FAIL(ctx.data_tablet_id_array_.push_back(logic_tablet_id))) {
      LOG_WARN("failed to push tablet id into array", K(ret), K(tablet_info), K(logic_tablet_id));
    } else if (OB_FAIL(record_tablet_simple_info_(ctx, tablet_info))) {
      LOG_WARN("failed to record tablet simple info", K(ret), K(tablet_info));
    }
    ...
    ObCopyLSViewInfoObReader *ob_reader)
{
  ...
  if (OB_SUCC(ret) && need_create_tablets) {
    // sys_tablet_id_array_/data_tablet_id_array_ were already populated by
    // choose_src_() from the inline ls info, but the parallel create tablets
    // path re-populates both from the reader stream via record_sys_tablet_id()/
    // record_data_tablet_id(). Clear them to avoid duplicated tablet ids, which
    // would otherwise route the same tablet into two batches and trigger
    // "tablet table info mgr already init" when building sstable info.
    ...
    const obrpc::ObCopyTabletInfo &tablet_info)
{
  ...
  if (!is_inited_) {
    ...
    if (OB_FAIL(ctx_->parallel_create_tablets_info_.record_data_tablet_id(*ctx_, tablet_info))) {
      LOG_WARN("failed to record data tablet id", K(ret), K(tablet_info));
    }
  }
  return ret;
}
...
void ObMigrationCreateTabletsFinishTask::record_server_event_()
{
  ...
  const int64_t data_tablet_count = ctx_->data_tablet_id_array_.count();
