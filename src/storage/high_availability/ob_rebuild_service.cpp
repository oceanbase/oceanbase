...
int ObLSRebuildMgr::generate_rebuild_ls_task_()
{
  ...
  if (!is_inited_) {
    ...
    ObLSID ls_id;
    observer::ObService *ob_service = nullptr;
  #ifdef ERRSIM
      if (OB_SUCC(ret)) {
    ...
    } else if (OB_ISNULL(ob_service = GCTX.ob_service_)) {
      ret = OB_ERR_UNEXPECTED;
      LOG_WARN("ob service should not be NULL", K(ret));
    } else if (OB_FAIL(ob_service->fill_ls_replica(tenant_id, ls_id, ls_replica))) {
