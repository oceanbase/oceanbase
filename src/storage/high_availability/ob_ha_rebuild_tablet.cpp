...
int ObRebuildTabletCtx::fill_comment(char *buf, const int64_t buf_len) const
{
  ...
  if (!is_valid()) {
    ...
    int64_t pos = 0;
    ret = databuff_printf(buf, buf_len, pos, "rebuild tablet : task_id = ");
...
int ObRebuildTabletDagNet::fill_comment(char *buf, const int64_t buf_len) const
{
  ...
  if (!is_inited_) {
    ...
    int64_t pos = 0;
    ret = databuff_printf(buf, buf_len, pos, "ObRebuildTabletDagNet: tenant_id=%lu, ls_id=", ctx_->tenant_id_);
...
int ObRebuildTabletDagNet::fill_dag_net_key(char *buf, const int64_t buf_len) const
{
  ...
  if (!is_inited_) {
    ...
    int64_t pos = 0;
    ret = databuff_printf(buf, buf_len, pos, "ObRebuildTabletDagNet: ls_id = ");
    OB_SUCCESS != ret ? : ret = databuff_printf(buf, buf_len, pos, ctx_->arg_.ls_id_);
    OB_SUCCESS != ret ? : ret = databuff_printf(buf, buf_len, pos,
...
int ObInitialRebuildTabletDag::fill_dag_key(char *buf, const int64_t buf_len) const
{
  ...
  if (!is_inited_) {
    ...
    int64_t pos = 0;
    ret = databuff_printf(buf, buf_len, pos, "ObInitialRebuildTabletDag: ls_id = ");
...
int ObInitialRebuildTabletTask::generate_rebuild_tablet_dags_()
{
  ...
  if (OB_FAIL(ret)) {
    if (OB_NOT_NULL(scheduler) && OB_NOT_NULL(finish_rebuild_tablet_dag)) {
      scheduler->free_dag(*finish_rebuild_tablet_dag, start_rebuild_tablet_dag);
      finish_rebuild_tablet_dag = nullptr;
    }
    if (OB_NOT_NULL(scheduler) && OB_NOT_NULL(start_rebuild_tablet_dag)) {
      scheduler->free_dag(*start_rebuild_tablet_dag, initial_rebuild_tablet_dag);
...
int ObStartRebuildTabletDag::fill_dag_key(char *buf, const int64_t buf_len) const
{
  ...
  if (!is_inited_) {
    ...
    int64_t pos = 0;
    ret = databuff_printf(buf, buf_len, pos, "ObStartRebuildTabletDag: ls_id = ");
    OB_SUCCESS != ret ? : ret = databuff_printf(buf, buf_len, pos, ctx->arg_.ls_id_);
    OB_SUCCESS != ret ? : ret = databuff_printf(buf, buf_len, pos,
        ", migration_type = %s", ObMigrationOpType::get_str(ctx->arg_.type_));
    if (OB_FAIL(ret)) {
  ...
  if (!is_inited_) {
    ...
    if (OB_FAIL(ret)) {
      if (OB_NOT_NULL(tablet_rebuild_dag)) {
        scheduler->free_dag(*tablet_rebuild_dag, start_rebuild_tablet_dag);
...
int ObTabletRebuildMajorDag::fill_dag_key(char *buf, const int64_t buf_len) const
{
  ...
  if (!is_inited_) {
    ...
    int64_t pos = 0;
    ret = databuff_printf(buf, buf_len, pos, "ObTabletRebuildMajorDag: log_stream_id = ");
    OB_SUCCESS != ret ? : ret = databuff_printf(buf, buf_len, pos, ctx->arg_.ls_id_);
    ...
    share::ObIDag *finish_dag)
{
  ...
  if (is_inited_) {
    ...
    // Rebuild dag pins this handle for its full lifetime while the dest tablet keeps serving
    // writes; without this, dumped memtables cannot be freed.
    ...
    if (OB_FAIL(ls->ha_get_tablet_without_memtables(WashTabletPriority::WTP_LOW,
      ...
      if (OB_TABLET_NOT_EXIST == ret) {
        ...
        LOG_WARN("failed to get tablet without memtables", K(ret), K(map_key));
      }
    } else {
...
int ObFinishRebuildTabletDag::fill_dag_key(char *buf, const int64_t buf_len) const
{
  ...
  if (!is_inited_) {
    ...
    int64_t pos = 0;
    ret = databuff_printf(buf, buf_len, pos, "ObFinishRebuildTabletDag: log_stream_id = ");
    OB_SUCCESS != ret ? : ret = databuff_printf(buf, buf_len, pos, ctx->arg_.ls_id_);
    OB_SUCCESS != ret ? : ret = databuff_printf(buf, buf_len, pos,
        ", migration_type = %s", ObMigrationOpType::get_str(ctx->arg_.type_));
    if (OB_FAIL(ret)) {
...
int ObFinishRebuildTabletTask::generate_rebuild_tablet_init_dag_()
{
  ...
  if (!is_inited_) {
    ...
    if (OB_NOT_NULL(initial_rebuild_tablet_dag) && OB_NOT_NULL(scheduler)) {
      scheduler->free_dag(*initial_rebuild_tablet_dag, finish_rebuild_tablet_dag);
      initial_rebuild_tablet_dag = nullptr;
    }
  }
  return ret;
