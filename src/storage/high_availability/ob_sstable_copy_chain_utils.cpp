...
// Inserts one middle task of type T between parent_task and next_task:
// parent -> T -> next. The middle task topologies only differ in the type of this middle
// task (ObPhysicalCopyTask actually transfers the macro block data, ObSSTableCopyStartTask
// only fetches the macro block id array of the sstable), so the wiring is shared here.
// next_task is finish_task itself, except for the copy start task of
// START_THEN_COPY_MACRO_BLOCKS, which is followed by the physical copy task.
template <typename T>
static int insert_middle_task(
    share::ObIDag *dag,
    share::ObITask *parent_task,
    share::ObITask *next_task,
    ObSSTableCopyFinishTask *finish_task)
{
  ...
  } else if (OB_FAIL(middle_task->add_child(*next_task))) {
    LOG_WARN("failed to add next task as child of middle task", K(ret));
...
// Inserts the two middle tasks of START_THEN_COPY_MACRO_BLOCKS between parent_task and
// finish_task: parent -> copy start -> physical copy -> finish. The copy task is built
// (and linked to finish_task) first, so that it can be the next task of the copy start
// task; like insert_middle_task, every task is fully wired before it is added to the dag.
static int insert_start_then_copy_tasks(
    share::ObIDag *dag,
    share::ObITask *parent_task,
    ObSSTableCopyFinishTask *finish_task)
{
  ...
  if (OB_FAIL(dag->alloc_task(copy_task))) {
    LOG_WARN("failed to alloc physical copy task", K(ret));
  } else if (OB_FAIL(copy_task->init(finish_task->get_copy_ctx(), finish_task))) {
    LOG_WARN("failed to init physical copy task", K(ret));
  } else if (OB_FAIL(copy_task->add_child(*finish_task))) {
    LOG_WARN("failed to add finish task as child of physical copy task", K(ret));
  } else if (OB_FAIL(insert_middle_task<ObSSTableCopyStartTask>(
      dag, parent_task, copy_task /* next_task */, finish_task))) {
    LOG_WARN("failed to insert sstable copy start task", K(ret));
  } else if (OB_FAIL(dag->add_task(*copy_task))) {
    LOG_WARN("failed to add physical copy task to dag", K(ret));
  }
  return ret;
}
  ...
  if (OB_ISNULL(dag) || OB_ISNULL(parent_task) || OB_ISNULL(child_task)) {
    ...
    switch (topology) {
      case COPY_MACRO_BLOCKS: // parent -> physical copy -> finish -> child
        ret = insert_middle_task<ObPhysicalCopyTask>(dag, parent_task, finish_task, finish_task);
        break;
      case START_THEN_COPY_MACRO_BLOCKS: // parent -> copy start -> physical copy -> finish -> child
        ret = insert_start_then_copy_tasks(dag, parent_task, finish_task);
        break;
      case START_THEN_FINISH: // parent -> copy start -> finish -> child
        ret = insert_middle_task<ObSSTableCopyStartTask>(dag, parent_task, finish_task, finish_task);
        break;
...
int ObTabletCopyChainDriverTaskBase::generate_next_sstable_chain_()
{
  ...
  if (OB_FAIL(ObSSTableCopyChainScanner::find_next_copy_table_key(
    ...
    // The sink was already added to the dag by ObTabletMigrationTask / ObTabletRestoreTask and
    // is therefore TASK_STATUS_WAITING, so the child status check must be skipped. Re-parenting
    // it here only bumps its indegree (1 -> 2), and this driver's inner_finish_task() drops it
    // back to 1, so the sink is still never scheduled before the last driver finishes.
  } else if (OB_FAIL(next_driver_task->add_child(*tablet_copy_finish_task_,
      false /*check_child_task_status*/))) {
