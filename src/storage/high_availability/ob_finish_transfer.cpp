    ...
    const share::ObLSID &src_ls_id, const share::ObLSID &dest_ls_id, int64_t &round)
{
  ...
  if (OB_TMP_FAIL(record_server_event_(ret, is_ready, round_))) {
    ...
    const uint64_t tenant_id, const share::ObLSID &ls_id, storage::ObLSHandle &ls_handle)
{
  ...
  } else if (OB_FAIL(ls_service->get_ls(ls_id, ls_handle, ObLSGetMod::STORAGE_MOD, ObLSAccessAttr::DISABLE_LOGONLY))) {
