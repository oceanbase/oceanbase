  ...
  if (!task_id.is_valid() || OB_INVALID_ID == tenant_id) {
    ...
    if (OB_FAIL(ObTransferTaskOperator::get(trans, tenant_id, task_id, for_update, transfer_task))) {
      ...
      LOG_WARN("finish scn not expected", K(ret), K(transfer_task), K(finish_scn));
                   ...
                   trans, tenant_id, task_id, transfer_task.get_status(), finish_scn))) {
      LOG_WARN("failed to update finish scn", K(ret), K(tenant_id), K(task_id), K(finish_scn));
                   ...
                   trans, tenant_id, task_id, transfer_task.get_status(), next_status, result, ObTransferTaskComment::EMPTY_COMMENT))) {
