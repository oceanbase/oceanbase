...
// ObPhysicalCopyCtx has no service ctx; it carries the raw bandwidth_throttle_
// + svr_rpc_proxy_ fields used throughout.
//
...
private:
  ...
  // Resolve data_version_ for source-side major macro-block reuse, mirroring
  // ObPhysicalCopyTask::build_data_version_for_macro_block_reuse_.
  static int build_data_version_for_macro_block_reuse_(
      const ObPhysicalCopyCtx &copy_ctx,
      const blocksstable::ObMigrationSSTableParam &sstable_param,
      ObCopyMacroBlockReaderInitParam &out_init_param);
