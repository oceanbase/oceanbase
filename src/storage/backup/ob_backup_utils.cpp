    ...
    share::SCN &start_replay_scn)
{
  ...
  ARRAY_FOREACH_X(ls_meta_infos.ls_meta_packages_, i, cnt, OB_SUCC(ret)) {
    ...
    if (is_default_palf_base_info) {
      ...
      share::ObLSAttrOperator ls_attr_operator(tenant_id, &sql_proxy);
      if (OB_FAIL(ls_attr_operator.get_ls_attr(ls_id, false /*for_update*/, sql_proxy,
    ...
    const storage::ObTabletTableStore &tablet_table_store, common::ObIArray<storage::ObITable *> &sstable_array)
{
  ...
  if (!backup_data_type.is_valid() || !tablet_table_store.is_valid()) {
    ...
    } else if (OB_FAIL(tablet_handle.get_obj()->get_medium_info_list(allocator, medium_info_list))) {
...
int ObBackupUtils::check_ls_valid_for_backup(const uint64_t tenant_id, const share::ObLSID &ls_id, const int64_t local_rebuild_seq)
{
  ...
  } else if (OB_FAIL(ls_service->get_ls(ls_id, handle, ObLSGetMod::STORAGE_MOD, ObLSAccessAttr::DISABLE_LOGONLY))) {
    ...
    const common::ObTabletID &tablet_id, ObBackupTabletHandleRef *&tablet_ref)
{
  ...
  if (OB_ISNULL(ls_backup_ctx_)) {
    ...
    if (OB_EAGAIN == ret) {
      LOG_ERROR("backup get tablet without memtables need retry", K(ret), K(key));
    } else {
