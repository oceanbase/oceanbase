...
ObBackupMetaCacheReader::ObBackupMetaCacheReader() : is_inited_(false), tenant_id_(), path_(), storage_info_(NULL), meta_kv_cache_(NULL) {}
    ...
    common::ObIAllocator &allocator, blocksstable::ObBufferReader &buffer)
{
  ...
  } else if (OB_FAIL(ObLSBackupRestoreUtil::pread_file(path, storage_info, offset, length, buf))) {
