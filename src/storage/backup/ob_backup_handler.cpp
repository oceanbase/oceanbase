...
namespace {
template <typename DagNetT>
int submit_backup_dag_net_(
    ...
    ObLSBackupDagNetInitParam &param)
{
  ...
  param.report_ctx_.location_service_ = GCTX.location_service_;
  param.report_ctx_.sql_proxy_ = GCTX.sql_proxy_;
  param.report_ctx_.rpc_proxy_ = GCTX.srv_rpc_proxy_;
  if (OB_UNLIKELY(nullptr == sql_proxy || nullptr == GCTX.location_service_ || nullptr == GCTX.srv_rpc_proxy_)) {
  ...
  } else if (OB_FAIL(dag_scheduler->create_and_add_dag_net<DagNetT>(&param))) {
    LOG_WARN("failed to create backup dag net", K(ret), K(param));
  } else {
    FLOG_INFO("success to create backup dag net", K(ret), K(param));
  }
  return ret;
}
}  // anonymous namespace
    ...
    const int64_t turn_id, const int64_t retry_id, const SCN &start_scn)
{
  ...
  if (OB_UNLIKELY(!job_desc.is_valid() || !backup_dest.is_valid() || OB_INVALID_ID == tenant_id
    ...
    LOG_WARN("get invalid args", K(ret), K(job_desc), K(backup_dest), K(tenant_id),
        K(backup_set_desc), K(ls_id), K(turn_id), K(retry_id), K(start_scn), K(ret));
  } else {
    ObLSBackupDagNetInitParam param;
    ...
    ret = submit_backup_dag_net_<ObLSBackupMetaDagNet>(tenant_id, backup_dest, param);
  }
  return ret;
}
    ...
    const int64_t turn_id, const int64_t retry_id, const share::ObBackupDataType &backup_data_type)
{
  ...
  if (OB_UNLIKELY(!job_desc.is_valid() || !backup_dest.is_valid() || OB_INVALID_ID == tenant_id
    ...
    ret = submit_backup_dag_net_<ObLSBackupDataDagNet>(tenant_id, backup_dest, param);
  }
  return ret;
}
    ...
    const int64_t turn_id, const int64_t retry_id, const share::ObBackupDataType &backup_data_type)
{
  ...
  if (OB_UNLIKELY(!job_desc.is_valid() || !backup_dest.is_valid() || OB_INVALID_ID == tenant_id
    ...
    ret = OB_INVALID_ARGUMENT;
    LOG_WARN("get invalid args", K(ret), K(job_desc), K(backup_dest), K(tenant_id),
        K(backup_set_desc), K(turn_id), K(backup_data_type));
  } else {
    ObLSBackupDagNetInitParam param;
    param.job_desc_ = job_desc;
    param.tenant_id_ = tenant_id;
    ...
    param.retry_id_ = retry_id;
    param.backup_data_type_ = backup_data_type;
    ret = submit_backup_dag_net_<ObBackupBuildTenantIndexDagNet>(tenant_id, backup_dest, param);
  }
  return ret;
}
    ...
    const share::ObLSID &ls_id, const SCN &start_scn, const SCN &end_scn, const bool is_only_calc_stat)
{
  ...
  if (OB_UNLIKELY(!job_desc.is_valid() || !backup_dest.is_valid() || OB_INVALID_ID == tenant_id
    ...
    ret = submit_backup_dag_net_<ObBackupComplementLogDagNet>(tenant_id, backup_dest, param);
  }
  return ret;
}
    ...
    const uint64_t tenant_id, const share::ObBackupSetDesc &backup_set_desc, const share::ObLSID &ls_id, const int64_t turn_id, const int64_t retry_id)
{
  ...
  if (OB_UNLIKELY(!job_desc.is_valid() || !backup_dest.is_valid() || OB_INVALID_ID == tenant_id
    ...
    param.turn_id_ = turn_id;
    param.retry_id_ = retry_id;
    ret = submit_backup_dag_net_<ObBackupTabletFuseDagNet>(tenant_id, backup_dest, param);
  }
  return ret;
}
