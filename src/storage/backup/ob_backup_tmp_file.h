...
private:
  int get_io_info_(const char *buf, const int64_t size, blocksstable::ObTmpFileIOInfo &io_info);
...
int ObBackupIndexBufferNode::get_backup_index(T &backup_index)
{
  ...
  } else if (OB_FAIL(blocksstable::ObTmpFileManager::get_instance().pread(io_info, read_offset_, timeout_ms, handle))) {
