...
int ObBackupComplementLogDagNet::start_running()
{
  ...
  if (IS_NOT_INIT) {
    ...
    if (OB_FAIL(ret) && OB_NOT_NULL(dag_scheduler) && OB_NOT_NULL(finish_dag)) {
      ...
      if (add_finish_dag_success && !add_complement_dag_success) {
        if (OB_TMP_FAIL(dag_scheduler->cancel_dag(finish_dag, nullptr/*parent_dag*/))) {
  ...
  if (OB_FAIL(ret) && OB_NOT_NULL(dag_scheduler) && OB_NOT_NULL(complement_dag)) {
    dag_scheduler->free_dag(*complement_dag, nullptr/*parent*/);
  }
  if (OB_FAIL(ret) && OB_NOT_NULL(dag_scheduler) && OB_NOT_NULL(finish_dag)) {
    dag_scheduler->free_dag(*finish_dag, nullptr/*parent*/);
  }
...
{
  int ret = OB_SUCCESS;
  if (OB_FAIL(databuff_printf(buf, buf_len, "ls_id=%ld", ctx_->ls_id_.id()))) {
...
ObBackupLSLogGroupTask::ObBackupLSLogGroupTask()
  : ObITask(TASK_TYPE_BACKUP_LS_LOG_GROUP_TASK),
    is_inited_(false),
    ls_id_(),
    ctx_(NULL),
    current_idx_(0),
...
int ObBackupLSLogGroupTask::generate_ls_dag_()
{
  ...
  while (OB_SUCC(ret)) {
    ...
    if (OB_FAIL(get_next_ls_id_(ls_id))) {
      ...
      if (OB_ISNULL(dag_net = this->get_dag()->get_dag_net())) {
        ...
        LOG_WARN("failed to push ls backup log dag into array", K(ret), K(*ctx_));
      ...
      if (OB_FAIL(ret) && OB_NOT_NULL(ls_dag)) {
        ...
        scheduler->free_dag(*ls_dag, nullptr/*parent*/);
        ls_dag = nullptr;
      }
    }
  }
  if (OB_FAIL(ret) && OB_NOT_NULL(scheduler)) {
    for (int64_t idx = 0; idx < ls_dag_array.count(); ++idx) {
      if (OB_TMP_FAIL(scheduler->cancel_dag(ls_dag_array.at(idx), nullptr/*parent*/))) {
...
{
  int ret = OB_SUCCESS;
  if (OB_FAIL(databuff_printf(buf, buf_len, "ls_id=%ld", ls_id_.id()))) {
    LOG_WARN("failed to fill dag_key", K(ret), K_(ls_id));
  }
  return ret;
}
...
ObBackupLSLogTask::ObBackupLSLogTask()
  : ObITask(TASK_TYPE_BACKUP_LS_LOG_TASK),
    is_inited_(false),
    ls_id_(),
    ctx_(NULL),
    bandwidth_throttle_(NULL),
    ...
    const int64_t archive_dest_id, const share::ObLSID &ls_id)
{
  ...
  } else if (OB_FAIL(deal_with_piece_meta_(piece_list))) {
    LOG_WARN("failed to deal with piece meta", K(ret), K(piece_list), K_(file_list));
  } else if (OB_FAIL(generate_ls_copy_task_(ls_id))) {
...
int ObBackupLSLogTask::inner_deal_with_piece_meta_(const ObTenantArchivePieceAttr &piece_attr)
{
  ...
  if (OB_FAIL(transform_and_copy_meta_file_(piece_attr))) {
    ...
    LOG_WARN("failed to copy piece end file", K(ret), K(piece_attr));
  }
  return ret;
}

int ObBackupLSLogTask::generate_ls_copy_task_(const share::ObLSID &ls_id)
{
  int ret = OB_SUCCESS;
...
ObBackupLSLogFileTask::ObBackupLSLogFileTask()
  : ObITask(TASK_TYPE_BACKUP_LS_LOG_FILE_TASK),
    is_inited_(false),
    ls_id_(),
    ctx_(NULL),
    finish_task_(NULL),
...
int ObBackupLSLogFileTask::transfer_clog_file_(const ObBackupPath &src_path, const ObBackupPath &dst_path)
{
  ...
  if (OB_FAIL(util.open_with_access_type(
          device_handle, fd, ctx_->backup_dest_.get_storage_info(), dst_path.get_obstr(), OB_STORAGE_ACCESS_MULTIPART_WRITER))) {
...
ObBackupLSLogFinishTask::ObBackupLSLogFinishTask()
  : ObITask(TASK_TYPE_BACKUP_LS_LOG_FINISH_TASK),
    is_inited_(false),
    mutex_(),
    idx_(),
    ctx_(NULL),
...
ObBackupLSLogGroupFinishTask::ObBackupLSLogGroupFinishTask()
  : ObITask(TASK_TYPE_BACKUP_LS_LOG_GROUP_FINISH_TASK),
    is_inited_(false),
    ctx_(NULL)
{
}
