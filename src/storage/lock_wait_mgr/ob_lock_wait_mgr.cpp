...
void ObLockWaitMgr::handle_exec_result(int exec_errcode, ObTxDesc &tx)
{
  int ret = OB_SUCCESS;
  ...
  } else if (OB_TRY_LOCK_ROW_CONFLICT != exec_errcode
  && (node = lockwaitmgr::ObLockWaitMgr::get_thread_node()) != NULL) {
    // remove placeholder when execution result is not lock-conflict
    lock_wait_mgr->remove_placeholder_node(node);
    node->reset_need_wait();
  } else if (OB_TRY_LOCK_ROW_CONFLICT == exec_errcode) {
    // save conflict infos to scheduler lock wait node
    lock_wait_mgr->on_lock_conflict(tx);
  }
}
  ...
  if (OB_TMP_FAIL(tmp_ret)
   && OB_UNLIKELY(ObDeadLockDetectorMgr::is_deadlock_enabled())) {
    ...
    ret = tmp_ret;
                               ...
                               node.get_abs_timeout(), query_str))) {
                                                 ...
                                                 ObLockWaitMgrRpcResult &result)
{
  ...
  if (OB_ISNULL(ptr)) {
    ...
    remote_exec_side_node->abs_timeout_ = msg.get_abs_timeout_ts();
        ...
        if (tenant_config->_enable_wait_remote_lock) {
          if (GET_MIN_CLUSTER_VERSION() < CLUSTER_VERSION_4_2_5_0) {
            if (REACH_TIME_INTERVAL(1 * 1000 * 1000)) {
              TRANS_LOG(TRACE, "observer not upgrade to 4_2_5_0");
            }
          } else {
...
void ObLockWaitMgr::on_lock_conflict(ObTxDesc &tx) {
  ...
  if (tx.has_conflict_infos()){
    ...
    on_lock_conflict(cflict_infos, &tx, tx.get_assoc_session_id());
  }
  TRANS_LOG_RET(TRACE, OB_SUCCESS, "end stmt handle lock conflict end", K(ret), K(tx));
}
...
void ObLockWaitMgr::on_lock_conflict(ObSArray<ObRowConflictInfo> &cflict_infos, ObTxDesc *tx, uint32_t session_id)
{
  // add remote conflict info
  rpc::ObLockWaitNode* node = NULL;
  if ((node = get_thread_node()) == NULL) {
    ...
    bool wait_cflict_idx = 0;
    bool find_local_cflict = false;
    ARRAY_FOREACH_NORET(cflict_infos, i) {
      if (cflict_infos.at(i).conflict_hash_ == last_wait_hash) {
      ...
      if (!find_local_cflict && cflict_infos.at(i).conflict_happened_addr_ == addr_) {
        // priority wait local conflict
        find_local_cflict = true;
        wait_cflict_idx = i;
      }
    }
...
ObLink* ObLockWaitMgr::check_timeout()
{
  ...
  }
  {
    ...
    while(NULL != (iter = hash_->quick_next(iter))) {
      ...
      } else if (iter->is_timeout() || has_set_stop()) {
