...
int ObMViewExplainRefreshExecutor::do_explain_nested_refresh(ObSqlString &output)
{
  ...
  if (OB_ISNULL(ctx_->get_sql_proxy())) {
    ret = OB_ERR_UNEXPECTED;
    LOG_WARN("sql proxy is nullptr", KR(ret));
  } else if (OB_FAIL(sql::ObMVDepUtils::get_mview_ids_in_topo_refresh_order(
                      *ctx_->get_sql_proxy(), tenant_id_, target_mview_id, nested_mview_ids))) {
    LOG_WARN("fail to get mview ids in topo refresh order", KR(ret), K(target_mview_id));
                                                        ...
                                                        ObSqlString &output)
{
  ...
  if (OB_FAIL(ret)) {
    ...
    if (OB_FAIL(get_mview_full_name(mview_id, full_name))) {
      ...
      if (mview_info.get_refresh_dop() > 0) {
        parallelism = mview_info.get_refresh_dop();
      } else if (OB_FAIL(session_info_->get_mview_refresh_dop(session_var_refresh_dop))) {
        LOG_WARN("fail to get mview refresh dop", KR(ret));
      } else if (session_var_refresh_dop > 0) {
        parallelism = session_var_refresh_dop;
      }
      if (OB_FAIL(ret)) {
      } else if (OB_FAIL(GET_MIN_DATA_VERSION(tenant_id_, data_version))) {
        LOG_WARN("fail to get data version", KR(ret), K(tenant_id_));
      } else if (OB_FAIL(ObMViewRefresher::collect_based_schema_object_infos(
                     tenant_id_, data_version, schema_guard, dependency_infos,
                     based_schema_object_infos, direct_dep_cnt))) {
        LOG_WARN("fail to collect based schema object infos", KR(ret));
      } else if (OB_FAIL(ObMVProvider::get_complete_refresh_mview_str(
                     *mview_table_schema,
                     schema_guard,
      ...
      ObArray<uint64_t> tables_without_delete;
      ObArray<uint64_t> tables_without_insert;
      ...
      if (OB_FAIL(mv_provider.print_mv_operators(session_info_, &refresh_info, str_alloc,
        ...
        ret = OB_ERR_UNEXPECTED;
        LOG_WARN("unexpected empty refresh operators", KR(ret), K(operators));
      }
