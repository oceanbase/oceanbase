...
{
  ObFTSingleToken start_word_;
  ObFTSingleToken end_word_;
  int32_t start_range_id_;
  int64_t range_count_;
  int64_t snapshot_version_;

  TO_STRING_KV(K_(start_word), K_(end_word), K_(start_range_id), K_(range_count), K_(snapshot_version));
};
...
protected:
  ...
  using CacheValidator = std::function<int(const ObFTDictDesc &desc,
                                           ObFTCacheRangeContainer &range_container,
                                           bool &is_valid)>;
  ...
  // Helper: wrap try_load_cache as validator
  int validate_by_load(const ObFTDictDesc &desc,
                       ObFTCacheRangeContainer &range_container,
                       bool &is_valid);
...
private:
  ...
  int collect_missing_ranges(const ObFTSingleToken &start_word,
...
private:
  // On cache miss returns OB_SUCCESS and sets is_valid=false.
  int check_cache_valid(const ObFTDictDesc &desc,
                        bool &is_valid,
                        ObFTCacheRangeContainer *range_container = nullptr);
  int check_cache_valid_for_bind(const ObFTDictDesc &desc,
                                  ObFTCacheRangeContainer &range_container,
                                  bool &is_valid);
