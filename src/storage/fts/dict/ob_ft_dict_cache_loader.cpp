                                                          ...
                                                          int64_t &row_count)
{
  ...
  SMART_VAR(ObISQLClient::ReadResult, result)
  {
    ...
    } else if (OB_FAIL(sql_proxy->read(result, tenant_id_, sql_string.ptr()))) {
...
int ObFTDictCacheLoaderBase::validate_by_load(const ObFTDictDesc &desc,
                                               ObFTCacheRangeContainer &range_container,
                                               bool &is_valid)
{
  int ret = OB_SUCCESS;
                                                   ...
                                                   CacheValidator validator)
{
  ...
  if (OB_FAIL(check_row_scn(desc, is_need_build, row_scn_info))) {
    ...
    if (OB_FAIL(validator(desc, range_container, is_valid))) {
  ...
  while (OB_SUCC(ret) && !range_end) {
    ...
    if (OB_FAIL(iter.get_key(key))) {
      ...
      if (OB_FAIL(ObCharset::first_valid_char(desc.coll_type_,
                                              ...
                                              first_char_len))) {
        LOG_WARN("First char is not valid", K(ret));
      } else if (range_end_char.get_token() != ObString(first_char_len, key.ptr())) {
        range_end = true;
      }
    }
...
int ObFTDictCacheLoaderBase::collect_missing_ranges(const ObFTSingleToken &start_word,
                                                     const ObFTSingleToken &end_word,
                                                     ...
                                                     ObIArray<ObMissingRangeInfo> *missing_ranges)
{
  ...
  if (OB_UNLIKELY(OB_ISNULL(missing_ranges))) {
    ...
    missing_range.start_word_ = start_word;
    missing_range.end_word_ = end_word;
                                                    ...
                                                    ObIArray<ObMissingRangeInfo> *missing_ranges)
{
  ...
  if (range_count == range_container.get_handles().size()) {
    ...
    if (OB_SUCC(ret) && prev_range_id + 1 < range_count && OB_NOT_NULL(prev_handle)) {
      ObFTSingleToken start_word = prev_handle->value_->get_dat_block()->end_token_;
      int32_t missing_start_range_id = prev_range_id + 1;
      int64_t missing_count = range_count - missing_start_range_id;
      if (OB_FAIL(collect_missing_ranges(start_word, ObFTSingleToken(), missing_start_range_id,
...
int ObFTDictCacheLoaderDDL::build_and_fetch_cache(const ObFTDictDesc &desc,
                                                   ...
                                                   ObFTCacheRangeContainer &range_container)
{
  CacheValidator validator = std::bind(&ObFTDictCacheLoaderDDL::validate_by_load, this,
                                       std::placeholders::_1, std::placeholders::_2, std::placeholders::_3);
  return refresh_cache_common(desc, row_scn_info, range_container, validator);
}
                                                   ...
                                                   ObFTCacheRangeContainer *range_container)
{
  ...
  if (OB_FAIL(ret) && OB_NOT_NULL(range_container)) {
    range_container->reset();
  }
...
int ObFTDictCacheLoaderRefresh::check_cache_valid_for_bind(const ObFTDictDesc &desc,
                                                            ObFTCacheRangeContainer &range_container,
                                                            bool &is_valid)
{
                                                        ...
                                                        ObFTDictMgr::DictTableSnapshot &row_scn_info)
{
  ...
  CacheValidator validator = std::bind(
      &ObFTDictCacheLoaderRefresh::check_cache_valid_for_bind, this,
      std::placeholders::_1, std::placeholders::_2, std::placeholders::_3);
  if (OB_FAIL(refresh_cache_common(desc, row_scn_info, range_container, validator))) {
    LOG_WARN("Failed to refresh cache", K(ret), K(desc));
  }
  range_container.reset();
  return ret;
