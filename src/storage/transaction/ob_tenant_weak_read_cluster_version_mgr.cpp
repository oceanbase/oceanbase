...
bool ObTenantWeakReadClusterVersionMgr::find_match_server(
    int64_t& pre_count, const common::ObAddr& addr, ServerInfo*& psi)
{
  int exist = false;
  int64_t i = 0;
...
int ObTenantWeakReadClusterVersionMgr::update_server_version(const common::ObAddr& addr, const int64_t version,
    const int64_t valid_part_count, const int64_t total_part_count, const int64_t generate_tstamp, bool& is_new_server)
{
  int ret = OB_SUCCESS;
  ...
  if (!server_exist) {
    ...
    if (OB_FAIL(rwlock_.unlock())) {
      ...
      if (OB_FAIL(svr_array_.push_back(si))) {
        LOG_WARN("add new server info entry fail",
            KR(ret),
            K(si),
            K(svr_array_),
            K(addr),
...
int64_t ObTenantWeakReadClusterVersionMgr::get_version(
    const int64_t base_version, int64_t& skip_server_count, const bool force_print) const
{
  ...
  for (int64_t i = 0; i < svr_array_.count(); i++) {
    ...
    LOG_DEBUG("[WEAK_READ_SERVER_VERSION_MGR] get version from server",
        K_(tenant_id),
        K(i),
        K(need_skip),
        K(base_version),
  ...
  if (need_print) {
    LOG_INFO("[WEAK_READ_SERVER_VERSION_MGR] compute version",
        K_(tenant_id),
        K(min_version),
        K(base_version),
        "server_count",
    ...
    LOG_DEBUG("[WEAK_READ_SERVER_VERSION_MGR] compute version",
        K_(tenant_id),
        K(min_version),
        K(base_version),
        "server_count",
...
ObTenantWeakReadClusterVersionMgr::ServerInfo::ServerInfo()
    : addr_(), version_(0), valid_part_count_(0), total_part_count_(0), generate_tstamp_(0), is_skipped_(false), lock_()
{}

ObTenantWeakReadClusterVersionMgr::ServerInfo::ServerInfo(
    const ObAddr& addr, const int64_t version, const int64_t valid_part_count, const int64_t total_part_count)
{
  addr_ = addr;
  version_ = version;
...
void ObTenantWeakReadClusterVersionMgr::ServerInfo::update(const int64_t version, const int64_t valid_part_count,
    const int64_t total_part_count, const int64_t generate_tstamp)
{
  SpinLockGuard guard(lock_);
...
int64_t ObTenantWeakReadClusterVersionMgr::ServerInfo::get_version(bool& need_skip, bool& is_first_skipped) const
{
  ...
  if (0 == valid_part_count_ || (cur_tstamp - generate_tstamp_) > MAX_SERVER_ALIVE_HEARTBEAT_INTERVAL) {
    need_skip = true;
    // Determine whether it is the first need_skip
    if (!is_skipped_) {
