...
int ObXATransHeartbeatWorker::init(ObTransService* trans_service)
{
  int ret = OB_SUCCESS;
  if (OB_UNLIKELY(is_inited_)) {
    ret = OB_INIT_TWICE;
  } else if (NULL == trans_service) {
    TRANS_LOG(WARN, "ps or trans_service is null", KP(trans_service));
...
void ObXATransHeartbeatWorker::run1()
{
  ...
  while (!has_set_stop()) {
    ...
    if (OB_UNLIKELY(!is_inited_)) {
      ...
      if (OB_FAIL(trans_service_->xa_scheduler_hb_req())) {
    ...
    if (REACH_TIME_INTERVAL(5 * 1000 * 1000)) {
      TRANS_LOG(INFO, "XA scheduler heartbeat task statistics", "avg_time", total_time / loop_count);
      total_time = 0;
      loop_count = 0;
    }
