...
int ObTableLockDetectFuncList::do_session_alive_detect()
{
  ...
  ObArray<ObTuple<int64_t, int64_t>> owner_ids;
  ...
  if (OB_FAIL(databuff_printf(
        full_table_name, OB_MAX_TABLE_NAME_BUF_LENGTH, "%s.%s", OB_SYS_DATABASE_NAME, OB_ALL_DETECT_LOCK_INFO_V2_TNAME))) {
    LOG_WARN("generate full table_name failed", K(OB_SYS_DATABASE_NAME), K(OB_ALL_DETECT_LOCK_INFO_V2_TNAME));
  } else if (OB_FAIL(
                     ObTableAccessHelper::read_multi_row(MTL_ID(), {"owner_type", "owner_id"}, full_table_name, where_cond, owner_ids))) {
    if (OB_ITER_END == ret) {
      ret = OB_SUCCESS;
    } else {
    ...
    for (int64_t i = 0; i < owner_ids.count() && OB_SUCC(ret); i++) {
      ...
      if (OB_FAIL(owner_id.convert_from_value(static_cast<ObLockOwnerType>(owner_ids[i].element<0>()),
                                              owner_ids[i].element<1>()))) {
                                                             ...
                                                             int64_t &cnt)
{
  ...
  if (OB_FAIL(ret) || OB_ISNULL(inner_conn)) {
                                     ...
                                     OB_SYS_DATABASE_NAME,
                                     OB_ALL_DETECT_LOCK_INFO_V2_TNAME))) {
    LOG_WARN("generate full table_name failed", KR(ret), K(OB_SYS_DATABASE_NAME), K(OB_ALL_DETECT_LOCK_INFO_V2_TNAME));
  } else if (OB_FAIL(add_pk_column_to_dml_(task_type, lock_req, dml))) {
    LOG_WARN(
      "add_pk_column_to_dml_ for remove_detect_info_from_inner_table failed", KR(ret), K(task_type), K(lock_req));
  } else if (OB_FAIL(get_cnt_of_lock_(full_table_name, inner_conn, dml, cnt))) {
                                                       ...
                                                       bool &is_existed)
{
  ...
  if (OB_FAIL(ret)) {
  } else if (OB_FAIL(sql.append_fmt("SELECT 1 FROM %s WHERE task_type = %d AND obj_type = %" PRIu64
                                    " AND obj_id = %" PRIu64
                                    " AND owner_id = %" PRId64
                                    " AND owner_type = %d",
                                    table_name,
                                    ...
                                    obj_type,
                                    obj_id,
                                    lock_req.owner_id_.id(),
                                    lock_req.owner_id_.type()))) {
                                               ...
                                               share::ObDMLSqlSplicer &dml)
{
  ...
  if (OB_FAIL(ret)) {
             ...
             || OB_FAIL(dml.add_pk_column("owner_id", lock_req.owner_id_.id()))
             || OB_FAIL(dml.add_pk_column("owner_type", lock_req.owner_id_.type()))) {
