...
int ObTableLockService::ObOBJLockGarbageCollector::garbage_collect_for_all_ls_()
{
  ...
  } else if (OB_FAIL(ls_service->get_ls_iter(ls_iter_guard, ObLSGetMod::TABLELOCK_MOD, ObLSAccessAttr::DISABLE_LOGONLY))) {
                                           ...
                                           const ObTableLockOwnerID lock_owner)
{
  ...
  if (!ctx.is_in_trans_ && OB_FAIL(start_tx_(ctx))) {
    ...
    if (OB_FAIL(process_alone_tablet_lock_task_(ctx, lock_mode, lock_owner))) {
                                               ...
                                               const ObTableLockOwnerID lock_owner)
{
  ...
  } else if (ctx.is_enable_lock_priority_ && !ctx.is_unlock_task() && OB_FAIL(process_obj_lock_with_prio_(ctx, lock_mode, lock_owner, ls_lock_map))) {
    LOG_WARN("add obj lock into queue failed", K(ret), K(ctx));
  } else if (OB_FAIL(process_obj_lock_(ctx, lock_mode, lock_owner, ls_lock_map))) {
                                                 ...
                                                 const ObTableLockOwnerID lock_owner)
{
  ...
  if (OB_FAIL(ret)) {
  } else if (OB_FAIL(get_lock_id(ctx.table_id_, table_lock_id))) {
    LOG_WARN("get lock id failed", K(ret), K(ctx));
  } else if (is_part_table_lock_(ctx.task_type_)
             && OB_FAIL(get_table_lock_mode_(ctx.task_type_, lock_mode, table_lock_mode))) {
    ...
    // locking queue upon transaction commitment.
             ...
             && !ctx.is_unlock_task()
             && OB_FAIL(
               process_table_tablet_lock_with_prio_(ctx, lock_mode, table_lock_mode, lock_owner, table_ls_lock_map))) {
    LOG_WARN("add table and tablet lock into queue failed", K(ret), K(ctx));
  } else if (OB_FAIL(process_table_tablet_lock_(ctx, lock_mode, table_lock_mode, lock_owner, table_ls_lock_map))) {
    LOG_WARN("lock table and tablet failed", K(ret), K(ctx));
  }
  return ret;
}
                                                        ...
                                                        const ObTableLockOwnerID lock_owner)
{
  ...
  if (OB_FAIL(get_ls_lock_map_(ctx, ctx.tablet_list_, ls_lock_map))) {
    LOG_WARN("fail to get ls lock map", K(ret), K(ctx.get_tablet_cnt()));
             ...
             && OB_FAIL(process_obj_lock_with_prio_(ctx, lock_mode, lock_owner, ls_lock_map))) {
                                                             ...
                                                             const ObLSLockMap &table_ls_lock_map)
{
  ...
  if (OB_FAIL(process_obj_lock_with_prio_(ctx, table_lock_mode, lock_owner, table_ls_lock_map))) {
    ...
    LOG_WARN("failed to get_tablet_ls_lock_map_", K(ret), K(ctx), K(lock_mode), K(lock_owner));
  } else if (OB_FAIL(process_obj_lock_with_prio_(ctx, lock_mode, lock_owner, tablet_ls_lock_map))) {
                                                   ...
                                                   const ObLSLockMap &table_ls_lock_map)
{
  ...
  if (OB_FAIL(process_obj_lock_(ctx, table_lock_mode, lock_owner, table_ls_lock_map))) {
    LOG_WARN("lock table failed", K(ret), K(ctx), K(table_lock_mode), K(lock_owner));
  }
  ...
  if (FAILEDx(get_tablet_ls_lock_map_(lock_mode, ctx, tablet_ls_lock_map))) {
    LOG_WARN("get tablet ls_lock_map failed", K(ret), K(ctx), K(lock_mode), K(lock_owner));
  } else  if (!ctx.is_enable_lock_priority_ && OB_FAIL(pre_check_lock_(ctx, lock_mode, lock_owner, tablet_ls_lock_map))) {
    LOG_WARN("failed to pre_check_lock_", K(ret), K(ctx), K(lock_mode), K(lock_owner));
  } else if (OB_FAIL(process_obj_lock_(ctx, lock_mode, lock_owner, tablet_ls_lock_map))) {
    LOG_WARN("lock tablet failed", K(ret), K(ctx), K(lock_mode), K(lock_owner));
  }
  return ret;
}
                                                    ...
                                                    const ObLSLockMap &ls_lock_map)
{
  ...
  LOG_DEBUG(
    "ObTableLockService::process_obj_lock_with_prio_", K(ret), K(ctx), K(ctx.task_type_), K(lock_mode), K(lock_owner));
...

{
  ...
  do {
    ...
    if (ctx.is_timeout()) {
      ...
      LOG_WARN("failed to start sub tx", K(ret), K(ctx));
                                                     ...
                                                     lock_mode,
                                                     lock_owner))) {
      LOG_WARN("fail to lock objects", K(ret), K(ctx));
  ...
  LOG_DEBUG("ObTableLockService::process_obj_lock_", K(ret), K(ctx), K(ctx.task_type_), K(lock_mode), K(lock_owner));
  return ret;
}
