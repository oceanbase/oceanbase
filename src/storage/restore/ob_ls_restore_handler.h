...
public:
  ...
  using Comment = common::ObFixedLengthString<common::MAX_TABLE_COMMENT_LENGTH>;
...
protected:
  ...
  int upload_wait_restore_tablet_();
  int reload_miss_tablet_(bool &all_finish);
...
private:
  ...
  int do_create_user_tablet_(const ObSArray<ObTabletID> &tablet_need_restore);
...
private:
  ...
  int do_quick_restore_(const ObSArray<ObTabletID> &tablet_need_restore);
...
private:
  ...
  int do_restore_major_(const ObSArray<ObTabletID> &tablet_need_restore);
