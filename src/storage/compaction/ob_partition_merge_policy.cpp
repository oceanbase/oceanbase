    ...
    int64_t &least_medium_snapshot)
{
  ...
  } else if (!schema_on_tablet->is_global_index_table()) {
    LOG_DEBUG("only read truncate info for global_index tablet", KR(ret), K(ls_id), K(tablet_id), K(schema_on_tablet->is_global_index_table()));
