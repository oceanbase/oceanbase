    ...
    bool &create_dag)
{
  ...
  } else if (compat_version < DATA_VERSION_4_2_1_5) {
    // do nothing, buffer table opt (including meta major merge) only supported in version >= 4.2.1.5
    if (REACH_TENANT_TIME_INTERVAL(2 * 60 * 1000 * 1000L /*120s*/)) {
      LOG_INFO("compat_version is smaller than DATA_VERSION_4_2_1_5, no need to schedule adaptive merge after mini", K(compat_version));
    }
