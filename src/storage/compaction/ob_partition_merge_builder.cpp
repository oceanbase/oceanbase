...
int64_t ObMacroBlockBuilder::to_string(char* buf, const int64_t buf_len) const
{
  int64_t pos = 0;
  J_OBJ_START();
  J_KV(K_(desc), K_(sstable_merge_info), K_(writer));
  J_OBJ_END();
  return pos;
}
int64_t ObMacroBlockEstimator::to_string(char* buf, const int64_t buf_len) const
{
  int64_t pos = 0;
  J_OBJ_START();
  J_KV(KP(component_));
