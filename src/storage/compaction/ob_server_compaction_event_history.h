...
public:
  ...
  int generate_event_str(const ObServerCompactionEvent &last_event, char *buf, const int64_t buf_len) const;
  TO_STRING_KV(K_(tenant_id), "merge_type", merge_type_to_str(merge_type_), K_(compaction_scn),
      "event", get_comp_event_str(event_), K_(timestamp), K_(comment));
...
#define DEFINE_COMPACTION_EVENT_PRINT_KV(n)                                                     \
  ...
  int ADD_COMPACTION_EVENT(const int64_t tenant_id,                                              \
  ...
  {                                                                                              \
    ...
    SIMPLE_TO_STRING_##n                                                                      \
    if (OB_FAIL(MTL(ObServerCompactionEventHistory*)->add_event(event_item))) {                \
