...
ObColumnChecksumCalculator::ObColumnChecksumCalculator()
    : is_inited_(false), allocator_(ObModIds::OB_SSTABLE_CHECKSUM_CALCULATOR), column_checksum_(NULL), column_cnt_(0)
{}
...
int ObColumnChecksumCalculator::calc_column_checksum(const int64_t checksum_method, const storage::ObStoreRow* new_row,
    const storage::ObStoreRow* old_row, const bool* is_column_changed)
{
  ...
  if (OB_UNLIKELY(!is_inited_)) {
  ...
  } else if (ObActionFlag::OP_DEL_ROW == new_row->flag_) {
    ...
    } else if (ObActionFlag::OP_ROW_EXIST == old_row->flag_) {
      if (OB_FAIL(calc_column_checksum(checksum_method, old_row->row_val_, false, NULL, column_checksum_))) {
        STORAGE_LOG(WARN, "fail to calculate checksum of old row", K(*old_row), K(ret));
      }
    }
  } else if (ObActionFlag::OP_ROW_EXIST == new_row->flag_) {
    const int64_t old_row_flag = NULL == old_row ? ObActionFlag::OP_ROW_DOES_NOT_EXIST : old_row->flag_;
    if (ObActionFlag::OP_ROW_EXIST == old_row_flag) {
      if (OB_FAIL(
              calc_column_checksum(checksum_method, old_row->row_val_, false, is_column_changed, column_checksum_))) {
        STORAGE_LOG(WARN, "fail to calculate checksum of old row", K(*old_row), K(ret));
      } else if (OB_FAIL(calc_column_checksum(
                     checksum_method, new_row->row_val_, true, is_column_changed, column_checksum_))) {
        STORAGE_LOG(WARN, "fail to calculate checksum of new row", K(*new_row), K(ret));
      }
    } else if (ObActionFlag::OP_ROW_DOES_NOT_EXIST == old_row_flag) {
      if (OB_FAIL(
              calc_column_checksum(checksum_method, new_row->row_val_, true, is_column_changed, column_checksum_))) {
        STORAGE_LOG(WARN, "fail to calculate checksum of new row", K(*new_row), K(ret));
      }
    } else {
...
int ObColumnChecksumCalculator::calc_column_checksum(const int64_t checksum_method, const common::ObNewRow& row,
    const bool new_row, const bool* column_changed, int64_t* column_checksum)
{
  int ret = OB_SUCCESS;
  if (OB_UNLIKELY(!is_inited_)) {
    ret = OB_NOT_INIT;
    LOG_WARN("ObColumnChecksumCalculator has not been inited", K(ret));
  } else if (NULL == column_checksum || row.is_invalid() || row.count_ != column_cnt_ ||
             !(checksum_method >= CCM_TYPE_AND_VALUE && checksum_method <= CCM_VALUE_ONLY)) {
    ret = OB_INVALID_ARGUMENT;
    STORAGE_LOG(WARN,
        "row was invalid",
        K(ret),
    ...
    for (int64_t i = 0; i < row.count_; ++i) {
      ...
      if (CCM_TYPE_AND_VALUE == checksum_method) {
        tmp_checksum = row.cells_[i].checksum(0);
      } else if (CCM_VALUE_ONLY == checksum_method) {
        tmp_checksum = row.cells_[i].checksum_v2(0);
      }
      if (new_row) {
...
ObColumnChecksumAccumulator::ObColumnChecksumAccumulator()
    : is_inited_(false),
      column_checksum_(NULL),
      allocator_(ObModIds::OB_SSTABLE_CHECKSUM_CALCULATOR),
      column_cnt_(0),
      lock_()
{}
...
int ObColumnChecksumAccumulator::add_column_checksum(const int64_t column_cnt, const int64_t* column_checksum)
{
  int ret = OB_SUCCESS;
  if (OB_UNLIKELY(!is_inited_)) {
    ret = OB_NOT_INIT;
...
int ObSSTableColumnChecksum::init(const int64_t concurrent_cnt, const share::schema::ObTableSchema& table_schema,
    const bool need_org_checksum, const int64_t checksum_method,
    const ObIArray<const blocksstable::ObSSTableMeta*>& org_sstable_metas)
{
  ...
  if (OB_UNLIKELY(is_inited_)) {
    ...
    LOG_WARN("invalid arguments",
        ...
        K(table_schema),
        K(need_org_checksum),
        K(org_sstable_metas.count()));
  } else if (OB_FAIL(init_checksum_calculators(concurrent_cnt, table_schema))) {
    LOG_WARN("fail to init checksum calculators", K(ret));
  } else if (OB_FAIL(init_column_checksum(table_schema, need_org_checksum, checksum_method, org_sstable_metas))) {
...
int ObSSTableColumnChecksum::init_checksum_calculators(
    const int64_t concurrent_cnt, const share::schema::ObTableSchema& table_schema)
{
  int ret = OB_SUCCESS;
  ...
  } else if (OB_FAIL(table_schema.get_store_column_count(column_cnt))) {
...
int ObSSTableColumnChecksum::init_column_checksum(const share::schema::ObTableSchema& table_schema,
    const bool need_org_checksum, const int64_t checksum_method,
    const ObIArray<const blocksstable::ObSSTableMeta*>& sstable_metas)
{
  ...
  int64_t column_cnt = 0;
  if (OB_UNLIKELY(!table_schema.is_valid() || (need_org_checksum && sstable_metas.count() < 1))) {
    ret = OB_INVALID_ARGUMENT;
        ...
        if (OB_FAIL(table_schema.get_column_ids(columns))) {
          ...
          for (int64_t i = 0; OB_SUCC(ret) && i < columns.count(); ++i) {
            ...
            if (OB_ISNULL(col_schema)) {
              ...
              // checksum of virtual columns for index sstable needs to be calculated and stored, but not need for data
              // sstable
            } else if (!col_schema->is_column_stored_in_sstable() && !table_schema.is_storage_index_table()) {
              // skip virtual column
            } else {
...
int ObSSTableColumnChecksum::get_checksum_calculator(const int64_t idx, ObColumnChecksumCalculator*& checksum_calc)
{
  int ret = OB_SUCCESS;
  checksum_calc = NULL;
  if (OB_UNLIKELY(!is_inited_)) {
...
int64_t ObColumnChecksumCalculator::to_string(char* buf, const int64_t buf_len) const
{
  int64_t pos = 0;
  J_OBJ_START();
  J_KV(KP_(column_checksum), K_(column_cnt));
  J_OBJ_END();
  return pos;
}
