...
struct ObBasicScheduleTabletFunc
{
  ...
  int64_t get_merge_version() const { return merge_version_; }
  int64_t get_loop_cnt() const { return loop_cnt_; }
  VIRTUAL_TO_STRING_KV(K_(merge_version), K_(ls_status),
    K_(ls_could_schedule_new_round), K_(ls_could_schedule_merge), K_(tablet_cnt), K_(loop_cnt));
  /*
   * diagnose section
  */
...
protected:
  ...
  virtual int check_with_schedule_scn(
    ...
    const int64_t need_freeze_snapshot = 0);
