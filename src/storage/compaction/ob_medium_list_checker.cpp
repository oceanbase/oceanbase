    ...
    const int64_t truncate_commit_version)
{
  ...
  if (skip_validate) {
    // do nothing
  } else if (OB_FAIL(inner_check_medium_list(
      extra_info, medium_info_array, last_major_snapshot, truncate_commit_version))) {
    ...
    const int64_t truncate_commit_version)
{
  ...
  if (OB_FAIL(check_extra_info(extra_info, last_major_snapshot, truncate_commit_version))) {
    LOG_WARN("failed to check extra info", KR(ret), K(last_major_snapshot),
        K(truncate_commit_version), K(extra_info));
  } else if (nullptr == medium_info_array || medium_info_array->empty()) {
    // do nothing
  } else if (OB_FAIL(filter_finish_medium_info(
      *medium_info_array, last_major_snapshot, truncate_commit_version, next_medium_info_idx))) {
    LOG_WARN("failed to filter finish medium info", KR(ret), K(last_major_snapshot),
        K(truncate_commit_version), K(next_medium_info_idx));
  ...
  } else if (OB_FAIL(check_next_schedule_medium(*medium_info_array->at(next_medium_info_idx),
      last_major_snapshot, truncate_commit_version, false/*force_check*/))) {
    LOG_WARN("failed to check next schedule medium info", KR(ret), K(last_major_snapshot),
        K(truncate_commit_version), KPC(medium_info_array), K(next_medium_info_idx));
  }
  return ret;
}
    ...
    int64_t &next_medium_info_idx)
{
  ...
  for ( ; OB_SUCC(ret) && idx < medium_info_array.count(); ++idx) {
    ...
    if (OB_ISNULL(info)) {
      ret = OB_ERR_UNEXPECTED;
      LOG_WARN("medium info ist null", K(ret), KPC(info), K(idx), K(medium_info_array));
    } else if (info->medium_snapshot_ > MAX(last_major_snapshot, truncate_commit_version)) {
      break;
    }
  } // end of for
  if (OB_SUCC(ret)) {
...
{
  int ret = OB_SUCCESS;
  if (truncate_commit_version > 0
      && extra_info.last_medium_scn_ <= truncate_commit_version) {
    // extra info was generated before tablet truncate, no need to check
  } else if (last_major_snapshot > 0 && extra_info.last_medium_scn_ > 0) {
    ...
    const bool force_check)
{
  int ret = OB_SUCCESS;
  if (truncate_commit_version > 0
      && next_medium_info.last_medium_snapshot_ <= truncate_commit_version) {
    LOG_INFO("skip checking medium info generated before tablet truncate", K(next_medium_info),
