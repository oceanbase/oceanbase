...
public:
  ...
  OB_INLINE common::ObLength get_length() const
  {
    ...
    if (!is_string_type() && !is_enum_or_set() && !is_enumset_inner_type()
                          && !is_ext() && !is_lob_locator()) {
      if (OB_FAIL(common::ObField::get_field_mb_length(get_type(),
                                                       get_accuracy(),
                                                       common::CS_TYPE_INVALID,
                                                       length))) {
        SQL_RESV_LOG(WARN, "failed to get length", K(ret), KCSTRING(common::lbt()), N_TYPE, get_type());
      }
    }
    return length;
  }
  ...
  { return static_cast<common::ObScale>(
      accuracy_.get_scale() == -1 ? NOT_FIXED_DEC : accuracy_.get_scale()); }
