...
namespace clog {
int ObSimpleMemberList::add_server(const common::ObAddr& server)
{
  int ret = OB_SUCCESS;
  bool found = false;
...
bool ObSimpleMemberList::contains(const common::ObAddr& server) const
{
  bool bool_ret = false;
  for (int64_t i = 0; i < count_; i++) {
    if (server_[i] == server) {
...
int64_t ObSimpleMemberList::to_string(char* buf, const int64_t buf_len) const
{
  int64_t pos = 0;
  databuff_printf(buf, buf_len, pos, "%c", count_);
  for (int64_t i = 0; i < count_; i++) {
    pos += server_[i].to_string(buf, buf_len);
    databuff_printf(buf, buf_len, pos, "%s", ";");
  }
  return pos;
}

int ObSimpleMemberList::deep_copy(const common::ObMemberList& member_list)
{
  int ret = OB_SUCCESS;
  reset();
  ObAddr server;
  ...
  if (OB_SUCC(ret) && get_count() != member_list.get_member_number()) {
    ...
    CLOG_LOG(ERROR, "count not match", K(ret), "dst_count", get_count(), "src_count", member_list.get_member_number());
  }
  return ret;
}

int ObSimpleMemberList::deep_copy_to(common::ObMemberList& member_list)
{
  int ret = OB_SUCCESS;
  member_list.reset();
  ...
  if (OB_SUCC(ret) && get_count() != member_list.get_member_number()) {
    ret = OB_ERR_UNEXPECTED;
    CLOG_LOG(ERROR, "count not match", K(ret), "src_count", get_count(), "dst_count", member_list.get_member_number());
  }
  return ret;
}

int ObSimpleMemberList::remove_server(const common::ObAddr& server)
{
  int ret = OB_SUCCESS;
...
int ObAckList::add_server(const common::ObAddr& server)
{
  int ret = OB_SUCCESS;
  bool found = false;
...
int64_t ObAckList::get_count() const
{
  int64_t count = 0;
  for (int64_t i = 0; i < ACK_LIST_SERVER_NUM; i++) {
...
int64_t ObAckList::to_string(char* buf, const int64_t buf_len) const
{
  int64_t pos = 0;
  const int64_t count = get_count();
