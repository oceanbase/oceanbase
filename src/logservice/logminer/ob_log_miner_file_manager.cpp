    ...
    if (OB_SUCC(ret)) {
      if (0 < data_len && nullptr != data && OB_FAIL(append_data_file_(file_id, data, data_len))) {
        ...
        if (OB_INVALID_TIMESTAMP == meta->range_.min_commit_ts_ ||
          ...
          LOG_ERROR("range max_commit_ts rollbacked", KPC(meta), K(batch_record));
        }

        if (OB_FAIL(write_meta_(file_id, *meta))) {
          LOG_ERROR("write meta failed when appending batch_record", K(file_id), KPC(meta));
        }
      }
    }
  }
...
int ObLogMinerFileManager::create_data_file_(const int64_t file_id)
{
  ...
  if (OB_ISNULL(meta)) {
    ret = OB_ALLOCATE_MEMORY_FAILED;
    LOG_ERROR("allocate oblogminer file meta failed", K(meta));
  } else if (OB_FAIL(generate_data_file_header_(alloc, header_data, header_len))) {
    ...
    if (OB_FAIL(write_meta_(file_id, *meta))) {
      LOG_ERROR("failed to write meta when writing file", K(file_id), KPC(meta));
    } else if (OB_FAIL(meta_map_.insert(file_id, meta))) {
      LOG_ERROR("failed to insert meta into meta_index", K(file_id), KPC(meta));
    }
  }
...
int ObLogMinerFileManager::append_data_file_(const int64_t file_id, const char *data, const int64_t data_len)
{
  ...
  ObLogMinerFileMeta *meta = nullptr;
  ...
  if (OB_FAIL(meta_map_.get(FileIdWrapper(file_id), meta))) {
    ...
    LOG_ERROR("get a null meta from meta_index, unexpected", K(file_id), K(meta));
  } else if (OB_FAIL(data_file_uri_(file_id, data_file_uri, sizeof(data_file_uri), uri_len))) {
...
int ObLogMinerFileManager::write_index_(const int64_t file_id)
{
  ...
  if (OB_FAIL(meta_map_.get(FileIdWrapper(file_id), meta))) {
    ...
    ret = OB_ERR_UNEXPECTED;
    LOG_ERROR("get a null file_meta from meta_index", K(file_id), K(meta));
