...
DEFINE_DESERIALIZE(ObDataDictMetaInfoItem)
{
  ...
  if (OB_UNLIKELY(version_ < 1)) {
    ...
    LOG_WARN("expect valid version while deserializing DictMetaInfo", KR(ret), K_(version));
  } else {
    ...
    if (OB_FAIL(ret)) {
      ...
      if (version_ > 1) {
        if (OB_FAIL(common::serialization::decode(buf, data_len, pos, end_scn_))) {
          LOG_WARN("deserialize end_scn field failed", KR(ret), K(data_len), K(pos), K(version_));
        }
      }
    ...
    const int64_t data_size)
{
  ...
  if (OB_ISNULL(data)) {
    ...
    LOG_WARN("data is null", K(ret), K(data));
  } else {
    ...
    const int64_t data_size) const
{
  ...
  if (DATADICT_METAINFO_HEADER_MAGIC != magic_) {
    bret = false;
    LOG_WARN_RET(OB_ERR_UNEXPECTED, "error magic number of datadict metainfo", KPC(this));
  } else if (OB_ISNULL(data)) {
    bret = false;
    LOG_WARN_RET(OB_ERR_UNEXPECTED, "data is null when check integrity", K(data), K(data_size_), KPC(this));
  } else if (data_size < data_size_) {
    bret = false;
    LOG_WARN_RET(OB_ERR_UNEXPECTED, "the size of buffer being deserialized is smaller than the data size",
        K(data_size), K(data_size_), KPC(this));
  } else {
    const int64_t data_crc_code = ob_crc64(data, data_size_);
    if (data_crc_code != checksum_) {
      bret = false;
      LOG_WARN_RET(OB_ERR_UNEXPECTED, "check sum mismatch", K(data), K(data_size_), K(data_crc_code), KPC(this));
    } else {
      // succ
    }
  }
...
DEFINE_SERIALIZE(ObDataDictMetaInfoHeader)
{
  ...
  if (OB_FAIL(serialization::encode_i16(buf, buf_len, pos, magic_))) {
    LOG_WARN("serialize magic failed", K(ret), K(buf), K(buf_len), K(pos), K(magic_));
  } else if (OB_FAIL(serialization::encode_i16(buf, buf_len, pos, meta_version_))) {
    LOG_WARN("serialize meta_version failed", K(ret), K(buf), K(buf_len), K(pos), K(meta_version_));
  } else if (OB_FAIL(serialization::encode_i64(buf, buf_len, pos, tenant_id_))) {
    LOG_WARN("serialize tenant_id_ failed", K(ret), K(buf), K(buf_len), K(pos), K(tenant_id_));
  } else if (OB_FAIL(serialization::encode_i32(buf, buf_len, pos, item_cnt_))) {
    LOG_WARN("serialize item_cnt failed", K(ret), K(buf), K(buf_len), K(pos), K(item_cnt_));
  } else if (OB_FAIL(serialization::encode_i64(buf, buf_len, pos, min_snapshot_scn_))) {
    LOG_WARN("serialize min_snapshot_scn failed", K(ret), K(buf), K(buf_len), K(pos), K(min_snapshot_scn_));
  } else if (OB_FAIL(serialization::encode_i64(buf, buf_len, pos, max_snapshot_scn_))) {
    LOG_WARN("serialize max_snapshot_scn failed", K(ret), K(buf), K(buf_len), K(pos), K(max_snapshot_scn_));
  } else if (OB_FAIL(serialization::encode_i64(buf, buf_len, pos, data_size_))) {
    LOG_WARN("serialize data_size failed", K(ret), K(buf), K(buf_len), K(pos), K(data_size_));
  } else if (OB_FAIL(serialization::encode_i64(buf, buf_len, pos, checksum_))) {
    LOG_WARN("serialize checksum failed", K(ret), K(buf), K(buf_len), K(pos), K(checksum_));
  }
...
DEFINE_DESERIALIZE(ObDataDictMetaInfoHeader)
{
  ...
  if (OB_FAIL(serialization::decode_i16(buf, data_len, pos, &magic_))) {
    LOG_WARN("deserialize magic failed", K(ret), K(buf), K(data_len), K(pos), K(magic_));
  } else if (OB_FAIL(serialization::decode_i16(buf, data_len, pos, &meta_version_))) {
    LOG_WARN("deserialize meta_version failed", K(ret), K(buf), K(data_len), K(pos), K(meta_version_));
  } else if (OB_FAIL(serialization::decode_i64(buf, data_len, pos, (int64_t *)&tenant_id_))) {
    LOG_WARN("deserialize tenant_id_ failed", K(ret), K(buf), K(data_len), K(pos), K(tenant_id_));
  } else if (OB_FAIL(serialization::decode_i32(buf, data_len, pos, &item_cnt_))) {
    LOG_WARN("deserialize item_cnt failed", K(ret), K(buf), K(data_len), K(pos), K(item_cnt_));
  } else if (OB_FAIL(serialization::decode_i64(buf, data_len, pos, (int64_t *)&min_snapshot_scn_))) {
    LOG_WARN("deserialize min_snapshot_scn failed", K(ret), K(buf), K(data_len), K(pos), K(min_snapshot_scn_));
  } else if (OB_FAIL(serialization::decode_i64(buf, data_len, pos, (int64_t *)&max_snapshot_scn_))) {
    LOG_WARN("deserialize max_snapshot_scn failed", K(ret), K(buf), K(data_len), K(pos), K(max_snapshot_scn_));
  } else if (OB_FAIL(serialization::decode_i64(buf, data_len, pos, &data_size_))) {
    LOG_WARN("deserialize data_size failed", K(ret), K(buf), K(data_len), K(pos), K(data_size_));
  } else if (OB_FAIL(serialization::decode_i64(buf, data_len, pos, &checksum_))) {
    LOG_WARN("deserialize checksum failed", K(ret), K(buf), K(data_len), K(pos), K(checksum_));
  }
...
int ObDataDictMetaInfo::push_back(const ObDataDictMetaInfoItem &item)
{
  ...
  if(OB_FAIL(item_arr_.push_back(item))) {
    LOG_WARN("push back data dict meta info item failed", K(ret), K(item), K(item_arr_));
  }
  return ret;
}
...
bool ObDataDictMetaInfo::check_integrity() const
{
  ...
  if (item_arr_count != header_.get_item_count()) {
    bret = false;
    LOG_WARN_RET(OB_ERR_UNEXPECTED, "item count in array and header is not equal", K(item_arr_count), K(header_));
  } else {
    ...
    if (max_scn_item.snapshot_scn_ != header_.get_max_snapshot_scn() &&
      ...
      bret = false;
      LOG_WARN_RET(OB_ERR_UNEXPECTED, "min scn and max scn record in header is inconsistent with item_arr", K(header_),
...
DEFINE_SERIALIZE(ObDataDictMetaInfo)
{
  ...
  if (OB_FAIL(header_.serialize(buf, buf_len, new_pos))) {
    LOG_WARN("failed to serialize datadict metainfo header", K(ret),
    ...
    for (int64 i = 0; OB_SUCC(ret) && i < item_cnt; i++) {
      ...
      if (OB_FAIL(item.serialize(buf, buf_len, new_pos))) {
        LOG_WARN("failed to serialize datadict metainfo item", K(ret), K(header_),
...
DEFINE_DESERIALIZE(ObDataDictMetaInfo)
{
  ...
  if (OB_FAIL(header_.deserialize(buf, data_len, pos))) {
    LOG_WARN("failed to deserialize datadict metainfo header", K(ret),
        K(buf), K(data_len), K(pos));
  } else if (! header_.check_integrity(buf + pos, header_.get_data_size())) {
    ret = OB_INVALID_DATA;
    LOG_WARN("data may be invalid", K(ret), K(buf), K(pos), K(data_len), K(header_));
  } else {
    const int32_t item_cnt = header_.get_item_count();
    if (0 >= item_cnt) {
      ret = OB_ERR_UNEXPECTED;
      LOG_WARN("item count is less or equal than 0, unexpected", K(ret), K(header_));
    }
    for (int32_t i = 0; OB_SUCC(ret) && i < item_cnt && pos < data_len; i++) {
      ...
      if (OB_FAIL(item.deserialize(buf, data_len, pos))) {
        LOG_WARN("datadict metainfo item deserialize failed", K(ret),
            K(buf), K(data_len), K(pos));
      } else if (OB_FAIL(item_arr_.push_back(item))){
        LOG_WARN("item_arr push back datadict metainfo item failed", K(ret), K(item), K(item_arr_));
      }
    }
  }
  return ret;
    ...
    share::SCN &scn)
{
  ...
  if (OB_UNLIKELY(! base_scn.is_valid_and_not_min())
    ...
    ret = OB_INVALID_ARGUMENT;
    LOG_WARN("invalid args to get data_dict data", KR(ret), K(base_scn), KP(data), K(data_size));
  } else if (OB_FAIL(get_data_dict_meta_info_(base_scn, item_arr))) {
    LOG_WARN("get data dict meta info failed", K(ret), K_(tenant_id), K(base_scn));
  } else if (item_arr.count() <= 0) {
    ret = OB_ENTRY_NOT_EXIST;
    int tmp_ret = OB_SUCCESS;

    if (OB_TMP_FAIL(mark_dump_data_dict_())) {
      LOG_WARN("mark_dump_data_dict_ failed", KR(ret), KR(tmp_ret), K_(tenant_id), K(base_scn));
    } else {
      LOG_WARN("no valid result was fetched from result", KR(ret), K_(tenant_id), K(base_scn));
    }
  } else if (OB_FAIL(generate_data_(item_arr, data, data_size, real_size, scn))) {
    LOG_WARN("generate data from item_arr failed", KR(ret), K_(tenant_id), K(base_scn));
  }
    ...
    share::SCN &scn)
{
  ...
  for (int64_t i = 0; OB_SUCC(ret) && i < arr_size && have_enough_buffer; i++) {
    ...
    if (meta_data_pos + item_size > data_size) {
      ...
      LOG_WARN("serialize datadict metainfo item failed", K(ret), K(data),
  ...
  if (OB_SUCC(ret)) {
    ...
    if (OB_FAIL(header.generate(tenant_id_, serialized_item_cnt, max_scn_item.snapshot_scn_,
      ...
      LOG_WARN("generate metainfo header failed", K(ret), K(serialized_item_cnt),
          K(data), K(header_size), K(meta_data_pos));
    } else if (OB_FAIL(header.serialize(data, header_size, header_pos))) {
      LOG_WARN("serialize metainfo header failed", K(ret), K(data), K(header_size),
          K(header_pos));
    } else {
      real_size = meta_data_pos;
      if (OB_FAIL(scn.convert_for_inner_table_field(max_scn_item.snapshot_scn_))) {
        LOG_WARN("convert snapshot_scn(bigint) to scn failed", K(ret), K(max_scn_item));
      }
    }
  }
...
int MetaInfoQueryHelper::get_data_dict_meta_info_(const share::SCN &base_scn, DataDictMetaInfoItemArr &item_arr)
{
  ...
  SMART_VAR(ObISQLClient::ReadResult, result) {
    if (OB_FAIL(sql.assign_fmt(QUERY_META_INFO_SQL_STR, DATA_DICT_META_TABLE_NAME))) {
      LOG_WARN("assign format to sqlstring failed", KR(ret), KCSTRING(QUERY_META_INFO_SQL_STR),
          KCSTRING(DATA_DICT_META_TABLE_NAME));
    } else if (OB_FAIL(sql_proxy_.read(result, tenant_id_, sql.ptr()))) {
      LOG_WARN("sql proxy failed to read result when querying datadict metainfo", K(ret),
          K(tenant_id_), "sql", sql.ptr());
    } else if (OB_ISNULL((sql_result = result.get_result()))) {
      ret = OB_ERR_UNEXPECTED;
      LOG_WARN("get sql result failed", K(ret), "sql", sql.ptr());
    } else if (OB_FAIL(parse_record_from_result_(base_scn, *sql_result, record_count, valid_record_count, item_arr))) {
      LOG_WARN("parse record from result failed", KR(ret),
          K_(tenant_id), "sql", sql.ptr(), K(base_scn), K(record_count), K(valid_record_count));
    } else {
      LOG_INFO("get_data_dict_meta_info", K(tenant_id_), K(base_scn), K(record_count), K(valid_record_count));
    }
  }
    ...
    DataDictMetaInfoItemArr &item_arr)
{
  ...
  while (OB_SUCC(ret)) {
    if (OB_FAIL(result.next())) {
      if (OB_ITER_END != ret) {
        LOG_WARN("get next result failed",K(ret), K(record_count));
      }
    } else {
      record_count++;
      ObDataDictMetaInfoItem item;
      ...
      if (OB_SUCC(ret)
        ...
        if (OB_FAIL(item_arr.push_back(item))) {
          LOG_WARN("push item into item_arr failed", K(ret), K(item), K(item_arr),
...
int MetaInfoQueryHelper::mark_dump_data_dict_()
{
  ...
  if (OB_UNLIKELY(OB_INVALID_TENANT_ID == ctx_tenant_id || ctx_tenant_id != tenant_id_)) {
    ...
    LOG_WARN("current mtl ctx tenant is invalid or not match", KR(ret), K(ctx_tenant_id), K_(tenant_id));
  } else if (OB_ISNULL(dict_service = MTL(ObDataDictService*))) {
    ret = OB_ERR_UNEXPECTED;
    LOG_WARN("data_dict_service is not valid, server may not has resource for ctx_tenant", KR(ret), K(ctx_tenant_id));
  } else {
    dict_service->mark_force_dump_data_dict();
    LOG_INFO("mark_force_dump_data_dict", K(ctx_tenant_id));
  }
