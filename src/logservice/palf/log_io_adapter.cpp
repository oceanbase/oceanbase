                             ...
                             ObDeviceManager *device_manager)
{
  ...
  if (OB_ISNULL(clog_dir) || 0 >= disk_io_thread_count || 0 >= max_io_depth ||
    ...
    ObIODOpts iod_opts;
    if (OB_FAIL(device_manager->get_device(storage_info, storage_type_prefix, device_handle))) {
      PALF_LOG(WARN, "failed to get device from ObDeviceManager", K(ret), K(storage_info), K(storage_type_prefix), KP(device_handle));
      ...
      is_inited_ = true;
      PALF_LOG(INFO, "log_local_device_ init successfully", K(storage_info), KP_(log_local_device));
    }
