...
public:
  ...
  ~LSNAllocator();
public:
  int init(const int64_t log_id, const SCN &log_scn, const LSN &start_lsn);
  void reset();
  ...
  // 功能: 为一条日志分配lsn, log_scn.
  //
  // @param [in] base_scn: log_scn下界值
  // @param [in] size: 日志体长度,如果是聚合日志需包含LogHeader长度
  //
  // @param [out] lsn: 分配的lsn
  // @param [out] log_scn: 分配的log_scn
  ...
  int inc_update_last_log_info(const LSN &lsn, const int64_t log_id, const SCN &log_scn);
  // inc update log_scn base, called by change access mode and to leader active
  int inc_update_log_scn_base(const SCN &log_scn);
  int truncate(const LSN &lsn, const int64_t log_id, const SCN &log_scn);
  // 获取last_lsn和log_timestamp
  TO_STRING_KV("max_log_id", get_max_log_id(), "max_lsn", lsn_ts_meta_.lsn_val_,
      "max_log_scn", get_max_log_scn());
private:
  static const int32_t LOG_ID_DELTA_BIT_CNT = 28;  // log_id_delta部分的位数，可生成25万个log_id
  static const int32_t LOG_TS_DELTA_BIT_CNT = 35;  // log_ts_delta部分的位数, ns级别，约可用32秒
...
private:
  union LSNTsMeta
  {
    ...
    struct
    {
      ...
      uint64_t log_ts_delta_ : LOG_TS_DELTA_BIT_CNT;
