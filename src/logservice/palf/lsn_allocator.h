...
public:
  ...
  ~LSNAllocator();
public:
  int init(const int64_t log_id, const int64_t log_ts, const LSN &start_lsn);
  void reset();
  ...
  // 功能: 为一条日志分配lsn, log_ts.
  //
  // @param [in] base_ts: log_ts下界值
  // @param [in] size: 日志体长度,如果是聚合日志需包含LogHeader长度
  //
  // @param [out] lsn: 分配的lsn
  // @param [out] log_ts: 分配的log_ts
  ...
  // inc update log_ts base, called by change access mode and to leader active
  int inc_update_log_ts_base(const int64_t log_ts);
  int truncate(const LSN &lsn, const int64_t log_id, const int64_t log_ts);
  // 获取last_lsn和log_timestamp
  TO_STRING_KV("max_log_id", get_max_log_id(), "max_lsn", lsn_ts_meta_.lsn_val_,
      "max_log_ts", get_max_log_ts());
private:
