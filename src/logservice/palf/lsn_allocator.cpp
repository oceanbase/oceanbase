                               ...
                               int64_t &padding_len)
{
  ...
  if (IS_NOT_INIT) {
    ret = OB_NOT_INIT;
  } else if (size <= 0 || log_id_upper_bound <= 0 || !lsn_upper_bound.is_valid()) {
    ret = OB_INVALID_ARGUMENT;
    PALF_LOG(WARN, "invalid arguments", K(ret), K(base_ts), K(size), K(log_id_upper_bound), K(lsn_upper_bound));
  } else {
