...
LogSlidingWindow::LogSlidingWindow()
  : self_(),
    ...
    quorum_policy_(NULL),
    lsn_allocator_(),
    group_buffer_lock_(common::ObLatchIds::PALF_SW_GROUP_BUFFER_LOCK),
    group_buffer_(),
    ...
    max_flushed_info_lock_(common::ObLatchIds::OB_PALF_SW_MAX_FLUSHED_INFO_LOCK),
    max_flushed_lsn_(),
    max_flushed_end_lsn_(),
    max_flushed_log_pid_(INVALID_PROPOSAL_ID),
    committed_info_lock_(common::ObLatchIds::OB_PALF_SW_COMMITTED_INFO_LOCK),
    committed_end_lsn_(),
...
int LogSlidingWindow::to_leader_active()
{
  ...
  if (IS_NOT_INIT) {
    ...
    // max_accepted_lsn of reconfirm(it has phantom logs generated by old leader).
    PALF_LOG(WARN, "reset_match_lsn_map_ failed", K(ret), K_(palf_id), K_(self));
  } else {
    reconfirm_fetch_dest_.reset();
...
int LogSlidingWindow::gen_committed_end_lsn_(LSN &new_committed_end_lsn)
{
  ...
  if (OB_FAIL(mm_->get_log_sync_member_list_for_generate_committed_lsn(prev_member_list,
    ...
    //    Otherwise, if the config log is not committed and B is elected to be new leader later,
                                        ...
                                        LSN &result_lsn) const
{
  ...
  int64_t accept_quorum = 0;
  if (OB_UNLIKELY(replica_num <= 0 || OB_MAX_MEMBER_NUMBER < replica_num || OB_ISNULL(quorum_policy_))) {
    ret = OB_INVALID_ARGUMENT;
    PALF_LOG(ERROR, "invalid arguments", K(ret), K_(palf_id), K_(self), K(member_list),
        K(replica_num), KP_(quorum_policy));
  } else if (FALSE_IT(accept_quorum = quorum_policy_->get_accept_quorum(replica_num))) {
  } else if (OB_UNLIKELY(accept_quorum <= 0 || OB_MAX_MEMBER_NUMBER < accept_quorum)) {
    ret = OB_ERR_UNEXPECTED;
    PALF_LOG(ERROR, "accept quorum is invalid", K(ret), K_(palf_id), K_(self),
        K(replica_num), K(accept_quorum));
  }
  if (OB_SUCC(ret)) {
    ObSpinLockGuard guard(match_lsn_map_lock_);
    ObAddr tmp_server;
  ...
  if (OB_FAIL(ret)) {
    ...
    PALF_LOG(WARN, "match_lsn_map does not reach accept quorum", K(ret), K_(palf_id), K_(self),
        K(member_list), K(replica_num), K(valid_member_cnt), K(accept_quorum));
  } else {
