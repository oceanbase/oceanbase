...
public:
  ...
  void reset();
  // NB: not thread safe
  void update_lsn_and_scn(const LSN &lsn, const SCN &log_scn);
  void update_palf_id_and_curr_block_id(const int64_t palf_id, const block_id_t curr_block_id);
  void mark_block_can_be_reused(const SCN &max_log_scn);
  block_id_t get_curr_block_id() const;
  const SCN &get_min_log_scn() const;
  LSN get_min_lsn() const;

  // This interface is just used for PalfIterator
  //
  // when iterate LogBlockHeader, this interface will return the min log_scn of this block,
  // if the 'log_scn_' of next LogGroupEntry is smaller than the return value, means this is
  // an empty block.
  //
  // if this block is a reused block, return 'max_log_scn_', otherwise return 'min_log_scn_'.
  const SCN get_log_scn_used_for_iterator() const;
  void calc_checksum();
  bool check_integrity() const;
  NEED_SERIALIZE_AND_DESERIALIZE;
  TO_STRING_KV(K_(magic), K_(version), K_(min_lsn), K_(min_log_scn), K_(curr_block_id), K_(palf_id));
