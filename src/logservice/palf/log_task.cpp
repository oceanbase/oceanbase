...
LogTaskHeaderInfo& LogTaskHeaderInfo::operator=(const LogTaskHeaderInfo &rval)
{
  if (this != &rval) {
    ...
    this->min_log_scn_ = rval.min_log_scn_;
    this->max_log_scn_ = rval.max_log_scn_;
...
void LogTask::inc_update_max_log_scn(const SCN &log_scn)
{
  header_.max_log_scn_ = log_scn;
}
...
int LogTask::set_initial_header_info(const LogTaskHeaderInfo &header_info)
{
  ...
  if (is_valid()) {
    ...
    header_.min_log_scn_ = header_info.min_log_scn_;
    update_data_len(header_info.data_len_);
    // Note: 这里不能直接赋值，需用inc_update，因为其他log_entry可能已经更新了max_log_scn_
    inc_update_max_log_scn(header_info.max_log_scn_);
...
int LogTask::set_group_header(const LSN &lsn, const SCN &log_scn, const LogGroupEntryHeader &group_entry_header)
{
  // caller need hold lock
  // this function is called when receiving log
  int ret = OB_SUCCESS;
  if (!lsn.is_valid() || !group_entry_header.is_valid()) {
    ...
    PALF_LOG(WARN, "log_task has been valid", K(ret), K(lsn), K(log_scn), K(group_entry_header), KPC(this));
  } else {
    ...
    header_.min_log_scn_ = log_scn;
    header_.max_log_scn_ = group_entry_header.get_max_scn();
