...
{
  return (magic_ == LogEntryHeader::MAGIC && log_size_ > 0 && log_scn_.is_valid());
}
...
bool LogEntryHeader::get_header_parity_check_res_() const
{
  ...
  bool_ret ^= parity_check((log_scn_.get_val_for_lsn_allocator()));
...
{
  int ret = OB_SUCCESS;
  if (NULL == log_data || data_len <= 0 || !log_scn.is_valid()) {
...
DEFINE_SERIALIZE(LogEntryHeader)
{
  ...
  if (OB_UNLIKELY(NULL == buf || buf_len <= 0)) {
    ret = OB_INVALID_ARGUMENT;
             ...
             || OB_FAIL(log_scn_.fixed_serialize(buf, buf_len, new_pos))
...
DEFINE_DESERIALIZE(LogEntryHeader)
{
  ...
  if (OB_UNLIKELY(NULL == buf || data_len <= 0)) {
    ret = OB_INVALID_ARGUMENT;
              ...
              || OB_FAIL(log_scn_.fixed_deserialize(buf, data_len, new_pos))
...
DEFINE_GET_SERIALIZE_SIZE(LogEntryHeader)
{
  ...
  size += log_scn_.get_fixed_serialize_size();
