...
LogReconfirm::LogReconfirm()
    : state_(INITED),
      ...
      prepare_quorum_cnt_(0),
      prepare_quorum_max_accepted_log_server_(),
      ...
      wait_accept_quorum_time_us_(OB_INVALID_TIMESTAMP),
...
void LogReconfirm::reset_state()
{
  ...
  if (state_ != INITED) {
    ...
    prepare_quorum_cnt_ = 0;
    prepare_quorum_max_accepted_log_server_.reset();
    prepare_quorum_max_accepted_log_pid_ = INVALID_PROPOSAL_ID;
...
int LogReconfirm::init_reconfirm_()
{
  ...
  if (OB_FAIL(mm_->get_alive_member_list_with_arb(member_list, replica_num))) {
    ...
    PALF_LOG(INFO, "init_reconfirm_ success", K(ret), K_(palf_id), K(member_list), K_(self),
        K_(prepare_quorum_cnt), K(replica_num));
  }
  return ret;
}
...
int LogReconfirm::submit_prepare_log_()
{
  ...
  if (OB_FAIL(sw_->get_max_flushed_log_info(unused_prev_lsn, max_flushed_end_lsn, max_flushed_log_pid))) {
    ...
    prepare_log_ack_list_.reset();
    prepare_quorum_max_accepted_log_server_ = self_;
    prepare_quorum_max_accepted_lsn_ = max_flushed_end_lsn;
    // the init 'prepare_quorum_max_accepted_log_pid_' should be the maximum of max_flushed_log_proposl_id and
    ...
    if (INVALID_PROPOSAL_ID != max_flushed_log_pid) {
      prepare_quorum_max_accepted_log_pid_ = max_flushed_log_pid;
    }
    const int64_t local_accept_proposal_id = mm_->get_accept_proposal_id();
    if (INVALID_PROPOSAL_ID != local_accept_proposal_id
        && INVALID_PROPOSAL_ID != prepare_quorum_max_accepted_log_pid_
        && local_accept_proposal_id > prepare_quorum_max_accepted_log_pid_) {
      prepare_quorum_max_accepted_log_pid_ = local_accept_proposal_id;
    }
    ...
    if (OB_FAIL(state_mgr_->handle_prepare_request(self_, new_proposal_id_))) {
      ...
      PALF_LOG(INFO, "submit_prepare_meta_req success", K(ret), K_(palf_id),
          K(curr_paxos_follower_list_), K(prepare_log_ack_list_), K(old_proposal_id),
          K(new_proposal_id_), K(self_), K(prepare_quorum_max_accepted_lsn_),
          K(prepare_quorum_max_accepted_log_server_),
          K(prepare_quorum_max_accepted_log_pid_), K(now_us), K(old_prepare_time_us),
          K(last_submit_prepare_req_time_us_), K(max_flushed_log_pid),
                                          ...
                                          const LSN &committed_end_lsn)
{
  ...
  if (IS_NOT_INIT) {
    ...
    // NB: no need change 'prepare_quorum_max_accepted_log_server_' after FETCH_MAX_LOG_LSN
  ...
  } else if (INVALID_PROPOSAL_ID != prepare_quorum_max_accepted_log_pid_
             && accept_proposal_id < prepare_quorum_max_accepted_log_pid_) {
    // accept_proposal_id is smaller than current max accepted value, ignore
    PALF_LOG(INFO, "server's proposal_id is smaller than me, ignore it",
        K(ret), K_(palf_id), K(server), K(src_proposal_id), K(accept_proposal_id),
        K(last_lsn), K(prepare_quorum_max_accepted_log_pid_), K(prepare_quorum_max_accepted_lsn_));
  } else {
    // if server's proposal_id is equal with me, compare with lsn.
    if (accept_proposal_id == prepare_quorum_max_accepted_log_pid_) {
      if (last_lsn > prepare_quorum_max_accepted_lsn_) {
        prepare_quorum_max_accepted_log_server_ = server;
        prepare_quorum_max_accepted_lsn_ = last_lsn;
      }
    } else if (INVALID_PROPOSAL_ID == prepare_quorum_max_accepted_log_pid_
               || accept_proposal_id > prepare_quorum_max_accepted_log_pid_) {
      prepare_quorum_max_accepted_lsn_ = last_lsn;
      prepare_quorum_max_accepted_log_pid_ = accept_proposal_id;
      prepare_quorum_max_accepted_log_server_ = server;
      PALF_LOG(INFO, "accept_proposal_id is greater than prepare_quorum_max_accepted_log_pid_",
          K(ret), K_(palf_id), K(server), K(src_proposal_id), K(accept_proposal_id),
          K(last_lsn));
    } else {
      // do nothing
    ...
    // which will lead to waiting accept quorum sync timeout.
  ...
  PALF_LOG(INFO, "handle_prepare_response finished", K(ret), K_(palf_id), K(server), K(accept_proposal_id), K(last_lsn),
      K(prepare_quorum_max_accepted_log_pid_), K(prepare_quorum_max_accepted_lsn_), K(prepare_log_ack_list_));
  return ret;
}
...
bool LogReconfirm::is_fetch_log_finished_()
{
  ...
  if (prepare_quorum_max_accepted_lsn_ == max_flushed_end_lsn) {
    bool_ret = true;
  } else {
    ...
    if (OB_SUCCESS != (tmp_ret = purge_throttling_())) {
      PALF_LOG_RET(WARN, tmp_ret, "purge throttling failed", K_(palf_id),
          K_(prepare_quorum_max_accepted_lsn), K(max_flushed_end_lsn), K(max_lsn));
    }
  }

  PALF_LOG(TRACE, "is_fetch_log_finished_", K_(palf_id), K(bool_ret),
      K_(prepare_quorum_max_accepted_lsn), K(max_flushed_end_lsn), K(max_lsn));
  return bool_ret;
}
...
int LogReconfirm::try_fetch_log_()
{
  ...
  if (OB_INVALID_TIMESTAMP == last_fetch_log_time_us_
    ...
    if (OB_FAIL(sw_->try_fetch_log_for_reconfirm(
        prepare_quorum_max_accepted_log_server_,
        prepare_quorum_max_accepted_lsn_,
        is_fetched))) {
      ...
      PALF_LOG(INFO, "try_fetch_log_ success", K(ret), K_(palf_id),
          K_(prepare_quorum_max_accepted_lsn),
          K_(prepare_quorum_max_accepted_log_server), K_(self), K_(last_record_sw_start_id));
    }
  } else if (sw_start_id != last_record_sw_start_id_) {
    last_record_sw_start_id_ = sw_start_id;
    last_fetch_log_time_us_ = now_us;
  } else {
    PALF_LOG(INFO, "no need fetch log in current round", K(ret), K_(palf_id),
        K_(prepare_quorum_max_accepted_log_server),
        K_(prepare_quorum_max_accepted_lsn), K(sw_start_id), K_(last_record_sw_start_id));
  }
  return ret;
}
...
bool LogReconfirm::can_receive_log() const
{
  ...
  // Or it may receive logs with old proposal_id whose lsn is larger than prepare_quorum_max_accepted_lsn.
...
{
  // degrade operation can be executed after prepare message reaches prepare quorum
...
bool LogReconfirm::is_accept_quorum_catch_up_()
{
  ...
  if (OB_SUCCESS != (tmp_ret = sw_->get_majority_match_lsn(majority_match_lsn))) {
    ...
    PALF_LOG_RET(ERROR, OB_INVALID_ARGUMENT, "majority_match_lsn is invalid, unexpected",
        K_(palf_id), K(majority_match_lsn));
  } else if (majority_match_lsn < prepare_quorum_max_accepted_lsn_) {
    PALF_LOG_RET(WARN, OB_SUCCESS,
        "majority_match_lsn is smaller than prepare_quorum_max_accepted_lsn_, need wait",
        K_(palf_id), K_(prepare_quorum_max_accepted_lsn), K(majority_match_lsn));
    if (palf_reach_time_interval(100 * 1000, last_notify_fetch_time_us_)) {
      ObMemberList lagged_list;
      if (OB_SUCCESS != (tmp_ret = sw_->get_lagged_member_list(prepare_quorum_max_accepted_lsn_, lagged_list))) {
        ...
        PALF_LOG_RET(WARN, tmp_ret, "submit_notify_fetch_log_req failed",
            K(tmp_ret), K_(palf_id), K(lagged_list));
      } else {
        PALF_LOG(INFO, "notify_fetch_log success", K(tmp_ret), K_(palf_id),
            K_(prepare_quorum_max_accepted_lsn), K(lagged_list));
      }
    }
  } else {
    bool_ret = true;
    ...
    switch (state_) {
      case INITED: {
        if (OB_FAIL(init_reconfirm_())) {
          ...
          PALF_EVENT("Reconfirm come into WAITING_LOG_FLUSHED state", palf_id_, K_(self),
              K_(prepare_quorum_cnt), K_(curr_paxos_follower_list));
        }
      ...
      case WAITING_LOG_FLUSHED: {
        if (OB_FAIL(purge_throttling_())) {
          ...
          last_purge_throttling_time_us_ = OB_INVALID_TIMESTAMP;
          PALF_EVENT("Reconfirm come into FETCH_MAX_LOG_LSN state", palf_id_, K_(self),
              K_(prepare_quorum_max_accepted_log_pid));
        }
        break;
      }
      case FETCH_MAX_LOG_LSN: {
        const int64_t cost_ts = ObTimeUtility::current_time() - last_submit_prepare_req_time_us_;
        // prepare_log_ack_list_ only records remote followers; count self separately.
        if (prepare_log_ack_list_.get_count() + 1 < prepare_quorum_cnt_) {
          // not reach prepare quorum in RECONFIRM_PREPARE_RETRY_INTERVAL_US, retry PREPARE phase
          ...
          if (RECONFIRM_PREPARE_RETRY_INTERVAL_US <= cost_ts) {
            ...
            PALF_LOG(WARN,
                "prepare_log_ack_list_ does not reach prepare quorum, retry prepare",
                K(cost_ts), K(prepare_log_ack_list_), K(prepare_quorum_cnt_), K_(palf_id));
          }
        } else {
          state_ = RECONFIRM_MODE_META;
          PALF_EVENT("Reconfirm come into RECONFIRM_MODE_META state", palf_id_,
              K(prepare_log_ack_list_), K_(prepare_quorum_max_accepted_log_server),
              K_(prepare_quorum_max_accepted_lsn));
        }
      ...
      case RECONFIRM_FETCH_LOG: {
        ...
        } else if (prepare_quorum_max_accepted_log_server_ != self_ && !is_fetch_log_finished_()) {
      ...
      case RECONFIRMING: {
        ...
        // Wait accept quorum match lsn catching up with prepare_quorum_max_accepted_lsn_.
        // It needs accept quorum catch-up check only when cluster_version reaches 4100.
        } else if (is_cluster_already_4100
            && need_skip_log_barrier == false
            && false == is_accept_quorum_catch_up_()) {
          if (palf_reach_time_interval(1 * 1000 * 1000, wait_accept_quorum_time_us_)) {
            PALF_LOG(INFO, "is_accept_quorum_catch_up_ return false, need wait and retry",
                K(ret), K_(self), K_(palf_id), K_(prepare_quorum_max_accepted_lsn));
          }
          ...
          // Record the end_lsn when entering START_WORKING.
          saved_end_lsn_ = last_end_lsn;
          state_ = START_WORKING;
          PALF_EVENT("Reconfirm come into START_WORKING state", palf_id_,
              K(new_proposal_id_), K(prepare_quorum_max_accepted_log_server_), K(prepare_quorum_max_accepted_lsn_),
              K(last_lsn), K(last_log_proposal_id), K(last_end_lsn));
        }
      ...
      case START_WORKING: {
        ...
        if (OB_SUCC(mm_->confirm_start_working_log(new_proposal_id_, leader_epoch, sw_config_version_))) {
          // check_ms_log_committed() retries appending the START_WORKING log
          // if it has not been committed.
          // The retry keeps using the previous lsn and proposal_id,
          // so saved_end_lsn_ does not need to be refreshed.
          const LSN curr_max_lsn = sw_->get_max_lsn();
          // After the START_WORKING log is committed, advance committed_end_lsn to the saved point,
          // which is expected to equal the current max_lsn.
          if (curr_max_lsn != saved_end_lsn_) {
            ret = OB_ERR_UNEXPECTED;
            PALF_LOG(ERROR, "max_lsn is not equal to saved_end_lsn_, unexpected",
                K_(palf_id), K_(self), K(curr_max_lsn), K_(saved_end_lsn));
            ...
            // Some committed logs have not slid out yet; wait and retry.
        ...
        if (state_ != FINISHED) {
          PALF_LOG(WARN, "wait start_working log committed or log slide", K_(palf_id), K(new_proposal_id_),
              K(prepare_quorum_max_accepted_log_server_), K(prepare_quorum_max_accepted_lsn_),
              K(prepare_quorum_max_accepted_log_pid_));
          break;
        }
      }
      case FINISHED: {
