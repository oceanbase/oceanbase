 ...
 * 由于Sequencer模块本身无法感知到日志流日志offset与log_scn关系, sequencer根据fetcher模块进度
 * 以及已分配任务数量作为归档进度的参考
 *
...
public:
  ...
  void get_min_log_info(ObLSID &id, palf::SCN &min_log_scn);
  TO_STRING_KV(K_(incarnation), K_(round), K_(succ_count), K_(total_count), K_(min_log_scn), K_(id));
