    ...
    IObCDCPartTransResolver *&ptr)
{
  ...
  if (OB_ISNULL(task_pool_) || OB_ISNULL(dispatcher_) || OB_ISNULL(cluster_id_filter_) || OB_ISNULL(lsn_filter_) || OB_ISNULL(trans_id_filter_)) {
    LOG_ERROR("not init", K(task_pool_), K(dispatcher_), K(cluster_id_filter_), K(lsn_filter_), K(trans_id_filter_));
    ret = OB_NOT_INIT;
  } else {
    ...
    if (OB_ISNULL(obj)) {
      ...
      ptr = new(obj) ObCDCPartTransResolver(tls_id_str, *task_pool_, task_map_, *dispatcher_, *cluster_id_filter_, *lsn_filter_, *trans_id_filter_, source_cluster_id_);
    }
  }
