    ...
    volatile bool &stop_flag)
{
  ...
  while (! stop_flag && ! rpc_succ && cur_ts < end_ts) {
    ...
    if (OB_UNLIKELY(! rpc_succ || has_valid_feedback)) {
      ...
      misslog_task.need_change_server_ = (cluster_version >= CLUSTER_VERSION_4_2_1_7);
...
{
  const obrpc::ObCdcLSFetchLogResp::FeedbackType& feedback = resp.get_feedback_type();
  has_valid_feedback = (feedback != obrpc::ObCdcLSFetchLogResp::FeedbackType::INVALID_FEEDBACK);
  rpc_fetch_no_log = (obrpc::ObCdcLSFetchLogResp::FeedbackType::LOG_NOT_IN_THIS_SERVER == feedback);

  if (has_valid_feedback) {
    const char *rpc_feedback_info = nullptr;
    switch (feedback) {
      case obrpc::ObCdcLSFetchLogResp::FeedbackType::LAGGED_FOLLOWER:
        rpc_feedback_info = "fetching log on lagged follower";
        break;
      case obrpc::ObCdcLSFetchLogResp::FeedbackType::LOG_NOT_IN_THIS_SERVER:
        rpc_feedback_info = "log not in this server, may be recycled";
        break;
      case obrpc::ObCdcLSFetchLogResp::FeedbackType::LS_OFFLINED:
        rpc_feedback_info = "fetching log on offline logstream";
        break;
      case obrpc::ObCdcLSFetchLogResp::FeedbackType::ARCHIVE_ITER_END_BUT_LS_NOT_EXIST_IN_PALF:
