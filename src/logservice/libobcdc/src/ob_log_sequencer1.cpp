...
ObLogSequencer::ObLogSequencer()
  : inited_(false),
    ...
    ready_trans_queue_(),
    trans_queue_lock_(common::ObLatchIds::OB_CDC_COMMON_LOCK),
    seq_trans_queue_(),
    checkpoint_cond_(ObCond::SPIN_WAIT_NUM, common::ObWaitEventIds::CDC_COMMON_COND_WAIT),
    ready_queue_cond_(ObCond::SPIN_WAIT_NUM, common::ObWaitEventIds::CDC_COMMON_COND_WAIT),
    seq_queue_cond_(ObCond::SPIN_WAIT_NUM, common::ObWaitEventIds::CDC_COMMON_COND_WAIT),
...
int ObLogSequencer::recycle_resources_after_trans_ready_(TransCtx &trans_ctx, ObLogTenant &tenant, volatile bool &stop_flag)
{
  ...
  // } else if (OB_UNLIKELY(! trans_ctx.is_sequenced())) {
    ...
    while (NULL != participant && ! stop_flag) {
      if (participant->is_dml_trans() || participant->is_ddl_trans() || participant->is_ls_op_trans()) {
