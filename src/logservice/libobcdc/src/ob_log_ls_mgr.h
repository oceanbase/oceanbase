...
public:
  ...
  int offline_ls(const TenantLSID &tls_id);

  int offline_and_recycle_ls(const TenantLSID &tls_id);
  ...
  /// @retval OB_SUCCESS                Success
  /// @retval Other return values       Fail
  int dec_ls_trans_count(const TenantLSID &tls_id);
...
private:
  ...
  bool is_ls_served_(const TenantLSID &tls_id) const;
  ...
  int offline_ls_(const TenantLSID &tls_id,
      const bool ensure_recycled_when_offlined = false);
  int recycle_ls_(const TenantLSID &tls_id,
      ObLogLSInfo *info);
  // Notify the modules that the LS is ready for recycling
  int call_recycle_ls_callbacks_(const TenantLSID &tls_id);
