...
int ObLogHandlerBase::prepare_switch_role(int64_t &curr_proposal_id,
                                          int64_t &new_proposal_id,
                                          ...
                                          palf::SyncMode &new_sync_mode) const
{
  ...
  if (IS_NOT_INIT) {
    ...
    ret = OB_NOT_RUNNING;
  } else if (OB_FAIL(palf_handle_->get_proposal_id_and_sync_mode(new_proposal_id, new_sync_mode, is_pending_state))) {
    PALF_LOG(WARN, "PalfHandle get_proposal_id_and_sync_mode failed", K(ret), K_(id), K(curr_proposal_id));
  } else {
    curr_proposal_id = proposal_id_;
    curr_sync_mode = sync_mode_;
    curr_role = role_;
    PALF_LOG(INFO, "prepare_switch_role with sync_mode success", K(ret), K_(id), K(curr_proposal_id),
                                                                    ...
                                                                    const char *reason)
{
  ...
  if (IS_NOT_INIT) {
    ...
    ret = palf_handle_->advance_election_epoch_and_downgrade_priority(proposal_id_, downgrade_priority_time_us, reason);
  }
  return ret;
}
...
int ObLogHandlerBase::change_leader_to(const common::ObAddr &dst_addr)
{
  ...
  if (IS_NOT_INIT) {
    ...
    ret = OB_NOT_RUNNING;
  } else if (OB_FAIL(palf_handle_->change_leader_to(dst_addr))) {
    PALF_LOG(WARN, "palf change_leader failed", K(ret), K(dst_addr), KP(palf_handle_));
  } else {
    PALF_LOG(INFO, "ObLogHandler change_laeder success", K(ret), K(dst_addr), KP(palf_handle_));
  }
  return ret;
}
