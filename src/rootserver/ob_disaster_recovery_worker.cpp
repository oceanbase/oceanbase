    ...
    for (int64_t i = 0; OB_SUCC(ret) && i < dr_ls_info_.get_locality().count(); ++i) {
      ...
      if (OB_FAIL(check_zone_exist_in_unit_list_(zone, zone_exist_in_unit_list))) {
        ...
        if (dr_ls_info_.is_duplicate_ls()
          ...
          // L-zone should not have R-replica
          zone_replica_desc_array->is_readonly_all_server_ = true;
          zone_replica_desc_array->readonly_memstore_percent_ = 100;
        } else {
    ...
    ObRemoveLSReplicaTask &remove_replica_task)
{
  ...
  if (OB_UNLIKELY(!inited_)) {
    ...
    ObReplicaMember remove_member(member_to_remove.get_server(), member_to_remove.get_timestamp(), replica_type);
    if (FALSE_IT(task_id.init(self_addr_))) {
    } else if (OB_FAIL(check_and_generate_new_paxos_replica_num_(
    ...
    ObReplicaMember &data_source)
{
  ...
  */
  ...
  if (OB_UNLIKELY(!inited_)) {
    ...
    } else if (ls_replica.get_restore_status().is_restore_failed()) {
      ret = OB_OP_NOT_ALLOW;
      LOG_USER_ERROR(OB_OP_NOT_ALLOW, "Data source replica restore failed, which is");
    ...
    int64_t &acc_dr_task)
{
  ...
  if (OB_UNLIKELY(!inited_)) {
    ...
    // bypass
    if (REACH_TENANT_TIME_INTERVAL(DR_WORKER_PRINT_INTERVAL)) {
  ...
  if (OB_UNLIKELY(!inited_)) {
    ...
    for (int64_t index = 0; OB_SUCC(ret) && index < replica_cnt; ++index) {
      ...
      if (OB_FAIL(dr_ls_info.get_replica_stat(
        ...
        //       for R-replica: execute remove_learner task directly
        ...
        if (dr_ls_info.is_duplicate_ls()) {
          if (0 != ls_status_info->ls_group_id_
            ...
            if (OB_FAIL(try_remove_readonly_replica_for_deleting_and_gts_unit_(
                            *ls_replica,
                            only_for_display,
                            dr_ls_info,
                            acc_dr_task,
...
int ObDRWorker::try_remove_readonly_replica_for_deleting_and_gts_unit_(
    const share::ObLSReplica &ls_replica,
    const bool &only_for_display,
    DRLSInfo &dr_ls_info,
    int64_t &acc_dr_task,
    ...
    const bool &is_unit_deleting)
{
  ...
  if (OB_UNLIKELY(!inited_)) {
    ret = OB_NOT_INIT;
    LOG_WARN("not init", KR(ret));
  } else if (OB_UNLIKELY(!ls_replica.is_valid() || comment.empty())) {
    ret = OB_INVALID_ARGUMENT;
    ...
    int64_t &acc_dr_task)
{
  ...
  if (OB_UNLIKELY(!inited_)) {
    ret = OB_NOT_INIT;
    LOG_WARN("not init", KR(ret));
  } else if (OB_ISNULL(unit_in_group_stat_info)) {
    ret = OB_INVALID_ARGUMENT;
    ...
    int64_t &acc_dr_task)
{
  ...
  if (OB_UNLIKELY(!inited_)) {
    ...
    LOG_WARN("fail to build type transform task", KR(ret), K(task_key), K(tenant_id), K(ls_id),
             K(data_size), K(dst_replica), K(src_member), K(old_paxos_replica_number),
    ...
    bool &is_unit_in_group_related)
{
  ...
  if (OB_UNLIKELY(!inited_)) {
    ...
    LOG_WARN("replica stat unexpected",
                 ...
                 KP(unit_stat_info),
                 KP(unit_in_group_stat_info));
  } else if (REPLICA_TYPE_READONLY == ls_replica->get_replica_type() && dr_ls_info.is_duplicate_ls()) {
    // only process F replica for duplicate ls, ignore R replica
    need_generate = false;
  } else if (REPLICA_STATUS_NORMAL == ls_replica->get_replica_status()
          && unit_stat_info->is_in_pool()
