...
int ObAlterLSCommand::pre_check_for_zone_deploy_mode_()
{
  ...
  } else if (meta_tenant_data_version < DATA_VERSION_4_2_5_5) {
    is_unit_list_enabled_ = false;
    if (OB_UNLIKELY(arg_.has_unit_list())) {
      ret = OB_NOT_SUPPORTED;
      LOG_WARN("To set UNIT_LIST, meta_tenant_data_version should be [4.2.5.5, +infinity)",
          KR(ret), K(meta_tenant_data_version));
      LOG_USER_ERROR(OB_NOT_SUPPORTED, "meta_tenant_data_version is smaller than 4.2.5.5, setting UNIT_LIST is");
    }
  } else {
