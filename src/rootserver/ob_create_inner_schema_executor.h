...
public:
  ...
  static int ur_exists(
      share::schema::ObSchemaGetterGuard& schema_guard, uint64_t tenant_id, const uint64_t ur_id, bool& exists);
  static int add_inner_role(
      uint64_t tenant_id, share::schema::ObSchemaGetterGuard& schema_guard, common::ObMySQLProxy* sql_proxy);
  static int add_password_and_lock_security_sys_users(
      uint64_t tenant_id, share::schema::ObSchemaGetterGuard& schema_guard, common::ObMySQLProxy* sql_proxy);
  static int check_and_create_default_profile(
      share::schema::ObSchemaGetterGuard& schema_guard, int64_t tenant_id, obrpc::ObCommonRpcProxy* rpc_proxy);
  static int add_audit_user(
      uint64_t tenant_id, share::schema::ObSchemaGetterGuard& schema_guard, common::ObMySQLProxy* sql_proxy);
  static int do_create_inner_schema_by_tenant(uint64_t tenant_id, oceanbase::lib::Worker::CompatMode compat_mode,
