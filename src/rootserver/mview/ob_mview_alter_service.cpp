    ...
    common::ObMySQLTransaction &trans)
{
  ...
  } else if (data_version < DATA_VERSION_4_4_2_4
             && (OB_DDL_DROP_COLUMN == column_operation_type || OB_DDL_CHANGE_COLUMN == column_operation_type)) {
    ret = OB_NOT_SUPPORTED;
    LOG_WARN("drop or change column on materialized view base table is not supported before 4.4.2.4",
             K(ret), K(data_version), K(new_table_schema), K(new_table_schema));
    LOG_USER_ERROR(OB_NOT_SUPPORTED,
                   "drop or change column on materialized view base table before 4.4.2.4 is");
