    ...
    int64_t &upgrade_cnt)
{
  ...
  if (OB_UNLIKELY(!ctx_.is_inited())) {
    ...
    ObDDLSQLTransaction trans(ctx_.schema_service_, true, true);
    ObSchemaGetterGuard schema_guard;
    ...
    ObHashSet<uint64_t> &dropped_table_ids)
{
  ...
  if (FAILEDx(ctx_.ddl_service_->get_tenant_schema_guard_with_version_in_inner_table(
    ...
    } else if (FALSE_IT(drop_table_id = exist_schema->get_table_id())) {
    } else if (OB_HASH_EXIST == dropped_table_ids.exist_refactored(drop_table_id)) {
    ...
    } else if (OB_FAIL(dropped_table_ids.set_refactored(drop_table_id))) {
      LOG_WARN("failed to add table_id to dropped_table_id set", KR(ret), K(drop_table_id));
    }
  }
  // 2. try drop table first
  exist_schema = NULL;
  if (OB_FAIL(ret)) {
  } else if (OB_HASH_EXIST == dropped_table_ids.exist_refactored(table.get_table_id())) {
    // the table is dropped in this trans
