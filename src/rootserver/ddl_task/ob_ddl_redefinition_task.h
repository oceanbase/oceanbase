...
protected:
  ...
  int sync_stats_info_accross_tenant(common::ObMySQLTransaction &trans,
                                     ...
                                     const ObTableSchema &data_table_schema,
                                     const ObTableSchema &new_table_schema);
  int get_src_table_level_stat(const ObTableSchema &data_table_schema,
                               ObOptTableStat &table_stat);
  int get_src_part_level_stats(const ObTableSchema &data_table_schema,
                               ObIArray<ObOptTableStatHandle> &part_stats_handles);
  int get_column_table_level_stats(const ObTableSchema &data_table_schema,
                                   ObIArray<ObOptColumnStatHandle> &column_stat_handles);
  int get_column_partition_level_stats(const ObTableSchema &data_table_schema,
                                       ObIArray<ObOptColumnStatHandle> &column_stat_handles);
  int sync_table_level_stats_info_accross_tenant(common::ObMySQLTransaction &trans,
                                                 const ObTableSchema &new_table_schema,
                                                 const ObOptTableStat &src_table_stat);
  int sync_partition_level_stats_info_accross_tenant(common::ObMySQLTransaction &trans,
                                                     const ObTableSchema &data_table_schema,
                                                     const ObTableSchema &new_table_schema,
                                                     const ObIArray<ObOptTableStatHandle> &src_part_stats_handle);
  int sync_column_table_level_stats_info_accross_tenant(common::ObMySQLTransaction &trans,
                                                        ObSchemaGetterGuard *dst_tenant_schema_guard,
                                                        const ObTableSchema &data_table_schema,
                                                        const ObTableSchema &new_table_schema,
                                                        const ObIArray<ObOptColumnStatHandle> &src_column_stat_handles);
  int sync_column_partition_level_stats_info_accross_tenant(common::ObMySQLTransaction &trans,
                                                            ObSchemaGetterGuard *dst_tenant_schema_guard,
                                                            const ObTableSchema &data_table_schema,
                                                            const ObTableSchema &new_table_schema,
                                                            const ObIArray<ObOptColumnStatHandle> &src_column_stat_handles);
