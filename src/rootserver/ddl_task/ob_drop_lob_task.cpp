...
int ObDropLobTask::prepare(const ObDDLTaskStatus new_status)
{
  ...
  if (OB_UNLIKELY(!is_inited_)) {
    ret = OB_NOT_INIT;
    LOG_WARN("ObDropLobTask has not been inited", K(ret));
...
int ObDropLobTask::drop_lob_impl()
{
  ...
  if (OB_ISNULL(root_service_)) {
    ...
    ret = OB_SCHEMA_ERROR;
    LOG_WARN("data table schema is null", K(ret), K(object_id_));
    ...
    if (OB_FAIL(ObDDLUtil::get_ddl_rpc_timeout(2 * data_table_schema->get_all_part_num(), ddl_rpc_timeout))) {
      ...
      LOG_WARN("drop lob failed", K(ret), K(ddl_rpc_timeout));
    }
    LOG_INFO("finish drop lob", K(ret), K(arg));
  }
  return ret;
}
...
int ObDropLobTask::drop_lob(const ObDDLTaskStatus new_status)
{
  ...
  if (OB_FAIL(drop_lob_impl())) {
    LOG_WARN("send drop lob rpc failed", K(ret));
  } else if (OB_FAIL(switch_status(new_status, true, ret))) {
    LOG_WARN("switch status failed", K(ret));
  }
  return ret;
...
int ObDropLobTask::fail()
{
  int ret = OB_SUCCESS;
  if (OB_FAIL(drop_lob_impl())) {
    LOG_WARN("drop lob impl failed", K(ret));
    ...
    switch (status) {
      ...
      case ObDDLTaskStatus::DROP_SCHEMA:
        if (OB_FAIL(drop_lob(SUCCESS))) {
          LOG_WARN("drop lob failed", K(ret));
        }
        break;
...
int ObDropLobTask::check_switch_succ_()
{
  ...
  if (OB_UNLIKELY(!is_inited_)) {
    ...
    LOG_WARN("faild to get_table_schema", K(tenant_id_), K(object_id_), K(ret));
    ...
    LOG_INFO("lob has been dropped", K(ret), KPC(this), KPC(data_table_schema_ptr));
  }
  return ret;
}
...
int ObDropLobTask::deserlize_params_from_message(const uint64_t tenant_id, const char *buf, const int64_t buf_size, int64_t &pos)
{
  int ret = OB_SUCCESS;
...
void ObDropLobTask::flt_set_task_span_tag() const
{
...
void ObDropLobTask::flt_set_status_span_tag() const
{
  switch (task_status_) {
  case ObDDLTaskStatus::PREPARE: {
