    ...
    const bool is_retryable_ddl)
{
  ...
  if (OB_FAIL(ret)) {
  } else if (use_doc_id_ && index_schema->is_rowkey_doc_id() && (snapshot_version <= 0) && !create_index_arg.is_offline_or_restore()) {
...
int ObFtsIndexBuildTask::prepare_aux_index_tables()
{
  ...
  if (OB_UNLIKELY(!is_inited_)) {
    ...
    LOG_WARN("failed to lock dictionary tables", K(ret), K(*this));
  }
  if (OB_SUCC(ret)) {
#ifdef ERRSIM
    if (OB_SUCC(ret)) {
