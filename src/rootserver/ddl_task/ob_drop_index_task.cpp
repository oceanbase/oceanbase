    ...
    const ObTableSchema *second_container_schema)
{
  ...
  if (OB_UNLIKELY(OB_INVALID_ID == tenant_id || task_id <= 0 || OB_INVALID_ID == data_table_id
    ...
    if (OB_NOT_NULL(second_container_schema)) {
      drop_index_arg_.second_container_table_id_ = second_container_schema->get_table_id();
    }
  }
  return ret;
}
...
int ObDropIndexTask::drop_index_impl()
{
  ...
  if (OB_ISNULL(root_service_)) {
    ...
    drop_index_arg.second_container_table_id_ = drop_index_arg_.second_container_table_id_;
