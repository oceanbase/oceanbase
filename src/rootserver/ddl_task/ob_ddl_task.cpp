...
int ObDDLTask::switch_status(const ObDDLTaskStatus new_status, const bool enable_flt, const int ret_code)
{
  ...
  if (OB_ISNULL(root_service = GCTX.root_service_)) {
  ...
  } else if (OB_FAIL(DDL_SIM(tenant_id_, task_id_, CHECK_TENANT_STANDBY_FAILED))) {
    LOG_WARN("ddl sim failure: check tenant standby failed", K(ret), K(tenant_id_), K(task_id_));
    ...
    if (OB_CANCELED == real_ret_code || ObDDLTaskStatus::FAIL == task_status_) {
      (void)ObDDLTaskRecordOperator::kill_task_inner_sql(root_service->get_sql_proxy(),
          trace_id_, dst_tenant_id_, task_id_, snapshot_version_, sql_exec_addrs_, killed_sessions_); // ignore return code
...
int ObDDLTask::report_error_code(const ObString &forward_user_message, const int64_t affected_rows)
{
  ...
  if (OB_UNLIKELY(!is_inited_)) {
    ...
    LOG_WARN("ddl sim failure: report ddl ret code failed", K(ret), K(tenant_id_), K(task_id_));
  ...
  if (OB_UNLIKELY(OB_INVALID_ID == tenant_id
    ...
    SMART_VAR(ObMySQLProxy::MySQLResult, res) {
      ...
      if (OB_FAIL(sql_string.assign_fmt("SELECT COUNT(*) as cnt FROM %s WHERE object_id = %lu AND ddl_type IN (%d, %d)",
          OB_ALL_DDL_TASK_STATUS_TNAME, data_table_id, ObDDLType::DDL_CREATE_INDEX, ObDDLType::DDL_CREATE_PARTITIONED_LOCAL_INDEX))) {
        LOG_WARN("assign sql string failed", K(ret));
      } else if (OB_FAIL(proxy.read(res, tenant_id, sql_string.ptr()))) {
        LOG_WARN("query ddl task record failed", K(ret), K(sql_string));
      } else if (OB_ISNULL(result = res.get_result())) {
    ...
    ObDDLTaskRecord &record)
{
  ...
  if (OB_UNLIKELY(!record.is_valid())) {
    ...
    } else if (tenant_data_version < DATA_VERSION_4_2_2_0 && record.consensus_schema_version_ != OB_INVALID_VERSION) {
      ret = OB_NOT_SUPPORTED;
