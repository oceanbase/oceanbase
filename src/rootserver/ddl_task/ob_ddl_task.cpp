...
int ObDDLTask::report_error_code(const ObString &forward_user_message, const int64_t affected_rows)
{
  ...
  } else if (OB_FAIL(DDL_SIM(tenant_id_, task_id_, REPORT_DDL_RET_CODE_FAILED))) {
    ...
    bool &has_index_task)
{
  ...
  if (OB_UNLIKELY(OB_INVALID_ID == tenant_id
    ...
    SMART_VAR(ObMySQLProxy::MySQLResult, res) {
      sqlclient::ObMySQLResult *result = NULL;
      if (OB_FAIL(sql_string.assign_fmt("SELECT EXISTS(SELECT 1 FROM %s WHERE object_id = %lu AND target_object_id = %lu AND ddl_type IN (%d, %d)) as has",
