...
int ObSetCommentHelper::check_table_legitimacy_()
{
  ...
  if (OB_FAIL(check_inner_stat_())) {
    ...
    LOG_WARN("orig_table_schema_ is nullptr", KR(ret), K_(table_id));
                        ...
                        || (!orig_table_schema_->is_user_table()
...
int ObSetCommentHelper::lock_for_common_ddl_()
{
  ...
  } else if (OB_FAIL(ObDDLLock::lock_for_common_ddl_in_trans(*orig_table_schema_, trans_))) {
    LOG_WARN("fail to lock for common ddl", KR(ret));
  }
...
int ObSetCommentHelper::operate_schemas_()
{
  int ret = OB_SUCCESS;
  ...
  if (OB_FAIL(check_inner_stat_())) {
    ...
    ret = OB_ERR_UNEXPECTED;
    LOG_WARN("new_table_schema_ or orig_table_schema_ is null", KR(ret), KP(new_table_schema_), KP(orig_table_schema_));
    ...
    for (uint64_t column_idx = 0; OB_SUCC(ret) && column_idx < arg_.column_name_list_.size(); column_idx++) {
      ObColumnSchemaV2 *new_column_schema = new_column_schemas_.at(column_idx);
      if (OB_ISNULL(new_column_schema)) {
        ret = OB_ERR_UNEXPECTED;
...
int ObSetCommentHelper::init_()
{
  int ret = OB_SUCCESS;
  return ret;
}

int ObSetCommentHelper::construct_and_adjust_result_(int &return_ret)
{
  int ret = return_ret;
...
int ObSetCommentHelper::operation_before_commit_()
{
  int ret = OB_SUCCESS;
  // do nothing
  return ret;
...
int ObSetCommentHelper::clean_on_fail_commit_()
{
  int ret = OB_SUCCESS;
  // do nothing
  return ret;
