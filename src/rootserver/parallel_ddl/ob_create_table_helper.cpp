...
int ObCreateTableHelper::generate_table_schema_()
{
  ...
  } else if (compat_version < DATA_VERSION_4_2_1_5 && arg_.schema_.is_new_queuing_table_mode()) {
    ret = OB_NOT_SUPPORTED;
    LOG_WARN("moderate/super/extreme table mode is not supported in data version less than 4.2.1.5", K(ret), K_(tenant_id), K(compat_version), K(arg_));
