...
ObCreateTableHelper::ObCreateTableHelper(
  ...
  : ObTableHelper(schema_service, tenant_id, "[parallel create table]", enable_ddl_parallel),
    arg_(arg),
    res_(res),
...
int ObCreateTableHelper::lock_objects_by_id_()
{
  ...
  // 5. parent table/mock fk parent table
  for (int64_t i = 0; OB_SUCC(ret) && i < arg_.foreign_key_arg_list_.count(); i++) {
    const obrpc::ObCreateForeignKeyArg &foreign_key_arg = arg_.foreign_key_arg_list_.at(i);
  ...
  // 6. lock mock fk parent table for replacement
  if (OB_SUCC(ret)) {
    const ObString &table_name = table.get_table_name();
...
int ObCreateTableHelper::check_table_name_()
{
  ...
  if (OB_FAIL(check_inner_stat_())) {
    ...
    if (OB_FAIL(schema_guard_wrapper_.get_synonym_id(database_id, table_name, synonym_id))) {
...
int ObCreateTableHelper::generate_schemas_()
{
  ...
  if (OB_FAIL(check_inner_stat_())) {
    LOG_WARN("fail to check inner stat", KR(ret));
  } else if (OB_FAIL(inner_generate_schemas_(new_tables_, new_sequences_, new_audits_))) {
...
int ObCreateTableHelper::generate_foreign_keys_()
{
  ...
  if (OB_FAIL(check_inner_stat_())) {
    ...
    if (OB_SUCC(ret)) {
      ObMockFKParentTableSchema *new_mock_fk_parent_table = NULL;
      if (OB_FAIL(try_replace_mock_fk_parent_table_(replace_mock_fk_parent_table_id_, new_tables_, new_mock_fk_parent_table))) {
...
int ObCreateTableHelper::calc_schema_version_cnt_()
{
  ...
  if (OB_FAIL(check_inner_stat_())) {
    LOG_WARN("fail to check inner stat", KR(ret));
  } else if (OB_FAIL(inner_calc_schema_version_cnt_(new_tables_, new_sequences_, new_audits_, new_mock_fk_parent_tables_))) {
    LOG_WARN("fail to check inner stat", KR(ret));
  }
  return ret;
}
...
int ObCreateTableHelper::operate_schemas_() {
  ...
  if (OB_FAIL(check_inner_stat_())) {
    LOG_WARN("fail to check inner stat", KR(ret));
  } else if (OB_FAIL(inner_create_table_(new_tables_, &arg_.ddl_stmt_str_, new_sequences_, new_audits_,
                                         replace_mock_fk_parent_table_id_, new_mock_fk_parent_tables_))) {
