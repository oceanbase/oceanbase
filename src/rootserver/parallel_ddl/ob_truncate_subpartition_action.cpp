...
int ObTruncateSubpartitionAction::init()
{
  int ret = OB_SUCCESS;
  if (OB_FAIL(check_inner_stat_())) {
    LOG_WARN("fail to check inner stat", KR(ret));
  } else if (OB_UNLIKELY(obrpc::ObAlterTableArg::TRUNCATE_SUB_PARTITION != get_arg().alter_part_type_)) {
    ret = OB_INVALID_ARGUMENT;
    LOG_WARN("alter_part_type is not TRUNCATE_SUB_PARTITION", KR(ret),
             "alter_part_type", get_arg().alter_part_type_);
  }
  return ret;
}

int ObTruncateSubpartitionAction::calc_schema_version_cnt()
{
  int ret = OB_SUCCESS;
  ...
  if (OB_FAIL(check_inner_stat_())) {
    ...
    //    truncate_table_subpartitions 1v + update_table_attribute 1v
...
int ObTruncateSubpartitionAction::before_commit()
{
  int ret = OB_SUCCESS;
  const ObTableSchema *orig_table_schema = nullptr;
  if (OB_FAIL(check_inner_stat_())) {
