...
int ObDropSubpartitionAction::init()
{
  int ret = OB_SUCCESS;
  if (OB_FAIL(check_inner_stat_())) {
    LOG_WARN("fail to check inner stat", KR(ret));
  } else if (OB_UNLIKELY(obrpc::ObAlterTableArg::DROP_SUB_PARTITION != get_arg().alter_part_type_)) {
    ret = OB_INVALID_ARGUMENT;
    LOG_WARN("alter_part_type is not DROP_SUB_PARTITION", KR(ret),
             "alter_part_type", get_arg().alter_part_type_);
  }
  return ret;
}

int ObDropSubpartitionAction::calc_schema_version_cnt()
{
  int ret = OB_SUCCESS;
  ...
  if (OB_FAIL(check_inner_stat_())) {
    ...
    //    drop_inc_subpart_info 1v + update_table_attribute 1v
    //    (subpart drop does not consume a separate update_partition_option version)
    const int64_t table_cnt = new_table_schemas_.count();
    int64_t cnt = table_cnt * 2;
    // 2. handle_tablets_for_alter_partitions: +1v (gen_new_schema_version)
    cnt += 1;
...
int ObDropSubpartitionAction::before_commit()
{
  int ret = OB_SUCCESS;
  const ObTableSchema *orig_table_schema = nullptr;
  if (OB_FAIL(check_inner_stat_())) {
