...
int ObDropTableHelper::operation_before_commit_()
{
  ...
  if (OB_FAIL(check_inner_stat_())) {
  ...
  } else if (!err_table_list_.empty() || table_schemas_.empty()) {
    ...
    // NEITHER of the two scenarios should commit transaction
    ret = OB_ERR_BAD_TABLE;
    LOG_WARN("table not exist", KR(ret));
  }
  return ret;
}
...
int ObDropTableHelper::prefetch_table_schemas_()
{
  ...
  if (OB_FAIL(check_inner_stat_())) {
    ...
    for (int64_t i = 0; OB_SUCC(ret) && i < table_items_.count(); i++) {
      ...
      } else if (!table_schema->is_user_table() && !table_schema->is_oracle_tmp_table() && !table_schema->is_external_table()) {
        // skip, parallel drop table temporarily only supports:
        // - user table
        // - oracle tmp table (mysql tmp table is not implemented yet)
        // - external table
        LOG_WARN("this table type is not supported", KR(ret), KPC(table_schema));
        if (OB_FAIL(log_table_not_exist_msg_(table_items_.at(i)))) {
          LOG_WARN("fail to log table not exsit msg", KR(ret));
        }
    ...
    if (FAILEDx(lock_existed_objects_by_id_())) {
      ...
      for (int64_t i = 0; OB_SUCC(ret) && i < table_schemas_.count(); i++) {
        ...
        if (OB_ISNULL(table_schema)) {
          ...
          const uint64_t table_id = table_schema->get_table_id();
          if (OB_FAIL(ObDependencyInfo::collect_all_dep_objs(tenant_id_, table_id, *sql_proxy_, dep_objs_after_lock))) {
            LOG_WARN("fail to collect dep obj", KR(ret), K_(tenant_id), K(table_id));
          } else if (dep_objs_after_lock.count() != dep_objs_before_lock_array.at(i).count()) {
            ret = OB_ERR_PARALLEL_DDL_CONFLICT;
            LOG_WARN("dep objs count not consistent", KR(ret), K(dep_objs_after_lock.count()), K(dep_objs_before_lock_array.at(i).count()));
          } else {
            lib::ob_sort(dep_objs_before_lock_array.at(i).begin(), dep_objs_before_lock_array.at(i).end(), dep_compare_func_);
            lib::ob_sort(dep_objs_after_lock.begin(), dep_objs_after_lock.end(), dep_compare_func_);
            for (int64_t j = 0; OB_SUCC(ret) && j < dep_objs_after_lock.count(); j++) {
              if (dep_objs_before_lock_array.at(i).at(j).first != dep_objs_after_lock.at(j).first) {
                ret = OB_ERR_PARALLEL_DDL_CONFLICT;
                LOG_WARN("dep obj in double check not in exist list", KR(ret),
                        K(dep_objs_after_lock.at(j).first), K(dep_objs_after_lock.at(j).second),
                        K(dep_objs_before_lock_array.at(i).at(j).first), K(dep_objs_before_lock_array.at(i).at(j).second));
              } else if (dep_objs_before_lock_array.at(i).at(j).second != dep_objs_after_lock.at(j).second) {
                ret = OB_ERR_UNEXPECTED;
          ...
          if (FAILEDx(dep_objs_.push_back(dep_objs_after_lock))) {
            LOG_WARN("fail to push back obj", KR(ret), K(dep_objs_after_lock));
          }
        }
      } // end for
    }
  ...
  if (OB_FAIL(check_inner_stat_())) {
    ...
    for (int64_t i = 0; OB_SUCC(ret) && i < trigger_ids.count(); i++) {
      ...
      if (OB_FAIL(latest_schema_guard_.get_trigger_info(trigger_id, trigger_info))) {
        ...
        if (is_to_recyclebin_(table_schema) && !table_schema.is_view_table()) {
          // compatible with oracle, triggers of view will be dropped directly
...
int ObDropTableHelper::drop_trigger_(const ObTriggerInfo &trigger_info, const ObTableSchema &table_schema)
{
  ...
  if (OB_FAIL(check_inner_stat_())) {
    ...
    ObDDLOperator ddl_operator(*schema_service_, *sql_proxy_);
    if (OB_FAIL(ddl_operator.drop_trigger(trigger_info,
                                          trans_,
                                          NULL/*ddl_stmt_str*/,
