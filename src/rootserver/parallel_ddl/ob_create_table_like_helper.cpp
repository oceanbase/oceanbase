...
ObCreateTableLikeHelper::ObCreateTableLikeHelper(
    ...
    ObTableHelper(schema_service, tenant_id, "[create table like]", enable_ddl_parallel),
    arg_(arg),
    res_(res),
...
int ObCreateTableLikeHelper::generate_schemas_()
{
  int ret = OB_SUCCESS;
  common::ObArray<ObSequenceSchema *> sequences;
  if (OB_FAIL(check_inner_stat_())) {
    LOG_WARN("fail to check inner stat", KR(ret));
  } else if (OB_FAIL(inner_generate_schemas_(new_tables_, sequences, new_audits_))) {
...
int ObCreateTableLikeHelper::generate_table_schema_()
{
  ...
  if (OB_FAIL(check_inner_stat_())) {
    ...
    } else if (FAILEDx(gen_object_ids_(obj_cnt, id_generator))) {
    ...
    for (int64_t i = 0; OB_SUCC(ret) && i < simple_index_infos.count(); ++i) {
      ...
      if (OB_FAIL(schema_guard_wrapper_.get_table_schema(simple_index_infos.at(i).table_id_, index_table_schema))) {
        ...
        if (OB_FAIL(new_index_schema.assign(*index_table_schema))) {
          ...
          if (OB_FAIL(new_tables_.push_back(new_index_schema))) {
...
int ObCreateTableLikeHelper::calc_schema_version_cnt_()
{
  int ret = OB_SUCCESS;
  common::ObArray<ObSequenceSchema *> sequences;
  if (OB_FAIL(check_inner_stat_())) {
    LOG_WARN("fail to check inner stat", KR(ret));
  } else if (OB_FAIL(inner_calc_schema_version_cnt_(new_tables_, sequences, new_audits_, new_mock_fk_parent_tables_))) {
    LOG_WARN("fail to check inner stat", KR(ret));
  }
  return ret;
}
...
int ObCreateTableLikeHelper::operate_schemas_() {
  int ret = OB_SUCCESS;
  common::ObArray<ObSequenceSchema *> new_sequences;
  if (OB_FAIL(check_inner_stat_())) {
    LOG_WARN("fail to check inner stat", KR(ret));
  } else if (OB_FAIL(inner_create_table_(new_tables_, &arg_.ddl_stmt_str_, new_sequences, new_audits_,
