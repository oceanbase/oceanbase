...
int ObDropPartitionAction::init()
{
  int ret = OB_SUCCESS;
  if (OB_FAIL(check_inner_stat_())) {
    LOG_WARN("fail to check inner stat", KR(ret));
  } else if (OB_UNLIKELY(obrpc::ObAlterTableArg::DROP_PARTITION != get_arg().alter_part_type_)) {
    ret = OB_INVALID_ARGUMENT;
    LOG_WARN("alter_part_type is not DROP_PARTITION", KR(ret),
             "alter_part_type", get_arg().alter_part_type_);
  }
  return ret;
}

int ObDropPartitionAction::calc_schema_version_cnt()
{
  int ret = OB_SUCCESS;
  ...
  if (OB_FAIL(check_inner_stat_())) {
    ...
    // 1. Each table (data + local index + lob + mlog): 3v
    //    drop_inc_part_info 1v + update_partition_option 1v + update_table_attribute 1v
    const int64_t table_cnt = new_table_schemas_.count();
    int64_t cnt = table_cnt * 3;
...
int ObDropPartitionAction::before_commit()
{
  int ret = OB_SUCCESS;
  const ObTableSchema *orig_table_schema = nullptr;
  if (OB_FAIL(check_inner_stat_())) {
