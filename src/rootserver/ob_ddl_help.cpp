    ...
    ObSchemaGetterGuard &schema_guard)
{
  ...
  } else if (tablegroup.is_sharding_subpartition()) {
                                           ...
                                           ObSchemaGetterGuard &schema_guard)
{
  ...
  if (arg.alter_option_bitset_.has_member(ObAlterTablegroupArg::SHARDING)) {
    ...
    } else if (new_tablegroup_schema.is_sharding_subpartition()) {
                                        ...
                                        common::ObMySQLTransaction &trans)
{
  ...
  if (arg.alter_option_bitset_.has_member(ObAlterTablegroupArg::SCOPE)) {
    ...
    } else if (data_version < MOCK_DATA_VERSION_4_4_2_1 ||
        (data_version >= DATA_VERSION_4_5_0_0 && data_version < DATA_VERSION_4_6_1_0)) {
      ret = OB_NOT_SUPPORTED;
      LOG_WARN("can not modify scope type for tablegroup before 4.6.1", KR(ret), K(tenant_id));
      LOG_USER_ERROR(OB_NOT_SUPPORTED, "modify scope type for tablegroup before 4.6.1 is");
    } else if (OB_FAIL(new_tablegroup_schema.assign(tablegroup_schema))) {
      LOG_WARN("fail to assign tablegroup schema", KR(ret), K(tablegroup_schema));
