...
#include "rootserver/ob_balance_info.h" // for ObDataSourceCandidateChecker
    ...
    ObAdminDRTaskRetComment &ret_comment)
{
  ...
  // STEP 1: parse parameters from ob_admin command directly
    ...
    ret = OB_INVALID_ARGUMENT;
    LOG_WARN("invalid argument", KR(ret), K(target_server));
  ...
  if (OB_SUCC(ret)) {
    ...
    if (!type_checker.is_candidate(data_source_replica_type)) {
      ret = OB_INVALID_ARGUMENT;
      LOG_WARN("data source replica type not allowed", KR(ret), K(data_source_replica_type), K(replica_type));
    ...
    ObLSDropNonPaxosReplicaArg &remove_nonpaxos_arg)
{
  ...
  if (OB_UNLIKELY(!ls_id.is_valid_with_tenant(tenant_id))
    ...
    ret = OB_INVALID_ARGUMENT;
    LOG_WARN("invalid tenant_id or ls_id", KR(ret), K(tenant_id), K(ls_id), K(target_server), K(replica_type));
  } else if (OB_FAIL(get_local_palf_stat_(tenant_id, ls_id, palf_stat, ret_comment))) {
    LOG_WARN("fail to get local palf stat", KR(ret), K(tenant_id), K(ls_id));
  } else if (OB_UNLIKELY(!palf_stat.is_valid())) {
    ret = OB_INVALID_ARGUMENT;
