    ...
    if (OB_FAIL(iterator.get_entry(log_entry, target_lsn))) {
      ...
      if (OB_SUCC(ret)) {
        ...
        if (logservice::GC_LS_LOG_BASE_TYPE == header.get_log_type()) {
          ...
          if (OB_FAIL(gc_log.deserialize(log_body, log_body_size, pos))) {
            ...
            //can not be there
            ret = OB_ERR_UNEXPECTED;
            LOG_ERROR("sys ls iterator offline scn", KR(ret), K(sync_scn));
          }
          // nothing
      ...
      if (last_sync_scn.is_valid() && (OB_FAIL(ret) || OB_INVALID_TIMESTAMP == last_report_ts_
        ...
        if (OB_TMP_FAIL(report_sys_ls_recovery_stat_(last_sync_scn, false, comment))) {
          LOG_WARN("failed to report ls recovery stat", KR(ret), KR(tmp_ret), K(last_sync_scn), K(comment));
        }
      }
    }
  }//end for each log
  ...
  if (!share::is_ls_create_pre_op(ls_attr.get_ls_operation_type()) || OB_INVALID_VERSION == switchover_epoch) {
    ...
    if (OB_ISNULL(GCTX.schema_service_)) {
      ...
      if (OB_FAIL(ObLSServiceHelper::create_new_ls_in_trans(ls_attr.get_ls_id(),
              ...
              switchover_epoch, tenant_stat, trans, ls_flag))) {
...
int ObRecoveryLSService::do_standby_balance_()
{
  ...
  } else if (DATA_VERSION_4_2_5_5 > meta_data_version) {
    ret = OB_NEED_RETRY;
    LOG_WARN("can not do standby balance, need wait meta tenant upgrade",
  ...
  if (OB_UNLIKELY(!inited_)) {
    ...
    if (OB_FAIL(ret)) {
      ...
      if (OB_FAIL(ObLSServiceHelper::check_transfer_task_replay(
              ...
              can_remove))) {
        LOG_WARN("failed to check transfer replay", KR(ret), K(tenant_id_), K(ls_balance_task));
      }
...
int ObRecoveryLSService::do_ls_balance_alter_task_(
    const share::ObBalanceTaskHelper &ls_balance_task,
    common::ObMySQLTransaction &trans)
{
  int ret = OB_SUCCESS;
