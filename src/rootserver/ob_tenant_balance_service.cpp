  ...
  if (OB_UNLIKELY(!inited_)) {
    ...
    while (!has_set_stop()) {
      ...
      if (OB_SUCC(ret) && 0 == job_cnt) {
        ...
        if (OB_FAIL(GET_MIN_DATA_VERSION(gen_meta_tenant_id(tenant_id_), meta_data_version))) {
          LOG_WARN("failed to get min data version", KR(ret), K(tenant_id_));
        } else if (meta_data_version < DATA_VERSION_4_2_5_5) {
          ret = OB_NEED_RETRY;
          LOG_WARN("can not ls balance, wait meta tenant upgrade to 4255", KR(ret), KDV(meta_data_version));
    ...
    ObIArray<share::ObUnit> &unit_array)
{
  ...
  if (OB_UNLIKELY(!is_valid_tenant_id(tenant_id))) {
    ...
    if (OB_FAIL(get_tenant_schema(tenant_id, tenant_schema))) {
      ...
      if (OB_SUCC(ret)) {
        bool enable_transfer = tenant_config->enable_transfer;
        bool enable_rebalance = tenant_config->enable_rebalance;
    ...
    common::ObSqlString &comment)
{
  ...
  if (OB_UNLIKELY(!inited_ || !ATOMIC_LOAD(&loaded_) || !job_desc_.is_valid())) {
    ...
    // case 1
    need_cancel = true;
    if (OB_TMP_FAIL(comment.assign_fmt("Canceled due to meta tenant data_version < 4.2.5.5"))) {
    ...
    common::ObMySQLTransaction &trans)
{
  ...
  if (OB_UNLIKELY(!job.is_valid() || ls_array.empty())) {
    ret = OB_INVALID_ARGUMENT;
  ...
  const uint64_t tenant_id = job.get_tenant_id();
  if (FAILEDx(lock_and_check_balance_job_(trans, tenant_id))) {
    ...
    const int64_t balance_timeout)
{
  ...
  if (OB_UNLIKELY(!inited_)) {
    ...
    LOG_WARN("trigger partition balance not support", K(tenant_id), K(data_version));
...
int ObTenantBalanceService::precheck_for_trigger_(const uint64_t tenant_id)
{
  ...
  if (OB_UNLIKELY(!is_user_tenant(tenant_id))) {
    ...
    LOG_USER_ERROR(OB_OP_NOT_ALLOW, "enable_rebalance or enable_transfer is disabled, trigger partition balance is");
