  ...
  if (OB_ISNULL(job_attr_) || OB_ISNULL(ls_attr_) || OB_ISNULL(set_task_attr_)) {
    ...
    if (OB_FAIL(ret)) {
      ...
      if (ObTimeUtility::current_time() < ls_attr_->end_ts_ + OB_BACKUP_RETRY_TIME_INTERVAL) {
        ...
        // for complement log, the set task row must be locked before the ls task row is
        // updated by redo_ls_task, keeping the same lock order as the observer side
        // report_progress_ (set task -> ls task), otherwise the two transactions may deadlock.
        const bool is_compl_log_task = ObBackupDataTaskType::BACKUP_PLUS_ARCHIVE_LOG == ls_attr_->task_type_.type_;
        ObBackupSetTaskAttr lock_set_task_attr;
        if (is_compl_log_task && OB_FAIL(ObBackupTaskOperator::get_backup_task(trans,
            job_attr_->job_id_, ls_attr_->tenant_id_, true/*for update*/, lock_set_task_attr))) {
          LOG_WARN("[DATA_BACKUP]failed to lock backup set task", K(ret), KPC(job_attr_), KPC(ls_attr_));
        ...
        } else if (is_compl_log_task && OB_FAIL(reset_compl_log_progress_(trans, lock_set_task_attr))) {
