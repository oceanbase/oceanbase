...
int ObBackupServiceProxy::handle_backup_validate_cancel(const obrpc::ObBackupManageArg &arg)
{
  int ret = OB_SUCCESS;
  ObBackupMgrService *validate_service = nullptr;
  MAKE_TENANT_SWITCH_SCOPE_GUARD(guard);
  if (ObBackupManageArg::CANCEL_BACKUP_VALIDATE != arg.type_) {
    ret = OB_INVALID_ARGUMENT;
    LOG_WARN("handle backup validate cancel get invalid argument", K(ret), K(arg));
  } else if (OB_FAIL(guard.switch_to(OB_SYS_TENANT_ID))) {
    LOG_WARN("failed to switch to sys tenant", K(ret));
  } else if (OB_ISNULL(validate_service = MTL(ObBackupMgrService *))) {
    ret = OB_ERR_UNEXPECTED;
    LOG_WARN("backup service must not be nullptr", K(ret));
  } else if (OB_FAIL(validate_service->handle_backup_validate_cancel(arg.tenant_id_, arg.managed_tenant_ids_))) {
    LOG_WARN("failed handle backup validate cancel", K(ret));
  }
  return ret;
}
...
{
  int ret = OB_SUCCESS;
  ObBackupMgrService *backup_service = nullptr;
  MAKE_TENANT_SWITCH_SCOPE_GUARD(guard);
  if (OB_FAIL(guard.switch_to(OB_SYS_TENANT_ID))) {
    LOG_WARN("failed to switch to sys tenant", K(ret));
  } else if (OB_ISNULL(backup_service = MTL(ObBackupMgrService *))) {
    ret = OB_ERR_UNEXPECTED;
    LOG_WARN("backup service must not be nullptr", K(ret));
...
int ObBackupServiceProxy::handle_backup_validate(const obrpc::ObBackupValidateArg &arg)
{
  ...
  if (!arg.is_valid()) {
    ...
    ret = OB_ERR_UNEXPECTED;
    LOG_WARN("backup service must not be nullptr", K(ret));
  } else if (OB_FAIL(validate_service->handle_backup_validate(arg))) {
    LOG_WARN("failed handle backup validate", K(ret));
  }
  return ret;
}