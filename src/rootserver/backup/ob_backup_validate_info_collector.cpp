    ...
    bool &is_skip) const
{
  ...
  if (OB_ISNULL(job_attr_)) {
    ...
    } else if (is_basic_validate && backup_set_info.tenant_compatible_ < DATA_VERSION_4_4_2_2) {
      // File list based validate metadata is available in this branch from 4.4.2.2.
      is_skip = true;
      LOG_INFO("[BACKUP_VALIDATE]backup set tenant compatible is less than 4.4.2.2, skip", K(backup_set_info));
    }
  }
    ...
    common::ObArray<share::ObPieceKey> &piece_keys)
{
  ...
  } else if (backup_set_info.tenant_compatible_ >= DATA_VERSION_4_4_2_2) {
