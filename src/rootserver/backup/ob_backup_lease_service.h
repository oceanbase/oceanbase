...
public:
  ...
  int register_scheduler(ObIBackupScheduler &scheduler);
	int register_mgr(ObBackupService &mgr);
  ...
  virtual int64_t get_lease_version() const override;
...
private:
  ...
  void wait_backup_scheduler_stop_();
	int start_backup_service_(); //4.0
  void stop_backup_service_(); //4.0
  void wait_mgr_stop_(); // 4.0
  virtual void run1() final override;
  void do_idle(const int32_t result);
  int renew_lease_();
  int clean_backup_lease_info_(const int64_t next_round);
  int set_backup_lease_info_(const share::ObBackupLeaseInfo &lease_info,
      const char *msg);
  OB_INLINE bool can_be_leader_() const { return 0 != can_be_leader_ts_; }
...
private:
  ...
  common::ObSEArray<ObIBackupScheduler *, 8> backup_schedulers_;// There are less than 8 at present, can be adjusted larger as needed
