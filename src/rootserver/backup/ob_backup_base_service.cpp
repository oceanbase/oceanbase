  ...
  if (trans.is_started()) {
    if (OB_SUCCESS == upstream_ret) {
      ...
      if (OB_FAIL(check_leader())) {
        LOG_WARN("[BACKUP_SERVICE]failed to recheck leader before commit", K(ret));
        if (OB_TMP_FAIL(trans.end(false))) {
          LOG_WARN("[BACKUP_SERVICE]failed to rollback after leader check fail", K(ret), K(tmp_ret));
        }
      } else if (OB_FAIL(trans.end(true))) {
        LOG_WARN("[BACKUP_SERVICE]failed to commit trans", K(ret));
      }
    } else {
      // Upper layer failed, rollback (preserve original error)
      if (OB_FAIL(trans.end(false))) {
        LOG_WARN("[BACKUP_SERVICE]failed to rollback trans", K(upstream_ret), K(ret));
      }
    }
  }
  return ret;
