...
public:
  ...
  int init(share::schema::ObMultiVersionSchemaService& schema_service, ObFreezeInfoManager& freeze_info_manager,
...
private:
  ...
  int try_recycle_schema_history(const uint64_t tenant_id, const int64_t recycle_schema_version);
  int check_can_skip_tenant(const uint64_t tenant_id, bool& skip);
  int calc_recycle_schema_versions(const common::ObIArray<uint64_t>& tenant_ids);
  int get_recycle_schema_version_by_server(const common::ObIArray<uint64_t>& tenant_ids,
      common::hash::ObHashMap<uint64_t, int64_t>& recycle_schema_versions);
  int get_recycle_schema_version_for_index(common::hash::ObHashMap<uint64_t, int64_t>& recycle_schema_versions);
  int get_recycle_schema_version_by_standby_cluster(const common::ObIArray<uint64_t>& tenant_ids,
      common::hash::ObHashMap<uint64_t, int64_t>& recycle_schema_versions);
  int get_recycle_schema_version_by_global_stat(const common::ObIArray<uint64_t>& tenant_ids,
      common::hash::ObHashMap<uint64_t, int64_t>& recycle_schema_versions);
  int fill_recycle_schema_versions(const uint64_t tenant_id, const int64_t schema_version,
      common::hash::ObHashMap<uint64_t, int64_t>& recycle_schema_versions);
...
private:
  ...
  virtual int retrieve_schema_history(
      common::sqlclient::ObMySQLResult& result, ObFirstSchemaKey& key, ObRecycleSchemaValue& value);
  virtual int fill_schema_history_key(const ObFirstSchemaKey& cur_key, ObFirstSchemaKey& key);
  virtual int fill_schema_history(const ObFirstSchemaKey& key, const ObRecycleSchemaValue& value);

  virtual int gen_batch_recycle_schema_history_sql(
      const common::ObIArray<ObFirstSchemaKey>& dropped_schema_keys, common::ObSqlString& sql);
  virtual int batch_recycle_schema_history(const common::ObIArray<ObFirstSchemaKey>& dropped_schema_keys);
...
private:
  ...
  virtual int gen_fill_schema_history_sql(int64_t start_idx, common::ObSqlString& sql);
  virtual int retrieve_schema_history(
      common::sqlclient::ObMySQLResult& result, ObSecondSchemaKey& key, ObRecycleSchemaValue& value);
  virtual int fill_schema_history_key(const ObSecondSchemaKey& cur_key, ObSecondSchemaKey& key);
  virtual int fill_schema_history(const ObSecondSchemaKey& key, const ObRecycleSchemaValue& value);
...
private:
  ...
  virtual int gen_fill_schema_history_sql(int64_t start_idx, common::ObSqlString& sql);
  virtual int retrieve_schema_history(
      common::sqlclient::ObMySQLResult& result, ObThirdSchemaKey& key, ObRecycleSchemaValue& value);
  virtual int fill_schema_history_key(const ObThirdSchemaKey& cur_key, ObThirdSchemaKey& key);
  virtual int fill_schema_history(const ObThirdSchemaKey& key, const ObRecycleSchemaValue& value);
...
private:
  ...
  common::hash::ObHashMap<ObThirdSchemaKey, ObRecycleSchemaValue, common::hash::NoPthreadDefendMode>
      schema_history_map_;
...
private:
  ...
  virtual int gen_fill_schema_history_sql(int64_t start_idx, common::ObSqlString& sql);
  virtual int retrieve_schema_history(
      common::sqlclient::ObMySQLResult& result, ObSystemVariableSchemaKey& key, ObRecycleSchemaValue& value);
  virtual int fill_schema_history_key(const ObSystemVariableSchemaKey& cur_key, ObSystemVariableSchemaKey& key);
  virtual int fill_schema_history(const ObSystemVariableSchemaKey& key, const ObRecycleSchemaValue& value);

  virtual int gen_batch_compress_schema_history_sql(
      const ObIArray<ObSystemVariableCompressSchemaInfo>& compress_schema_infos, common::ObSqlString& sql);
