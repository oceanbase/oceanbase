  ...
  if (IS_NOT_INIT) {
    ...
    if (OB_TABLE_NOT_EXIST == ret) {
      ...
      if (table_schema->is_global_index_table()) {
        ...
        LOG_WARN("table not exists after locking table", KR(ret), K(part_info),
            K(tablet_id), K(part_idx), K(subpart_idx), K(ori_schema_version), KPC(table_schema));
      }
    } else {
      LOG_WARN("get latest table schema failed", KR(ret), K(part_info), K(ori_schema_version));
    }
  } else if (OB_ISNULL(new_table_schema)) {
    ret = OB_ERR_UNEXPECTED;
    LOG_WARN("null new table schema", KR(ret), K(part_info),
        K(part_info), K(tablet_id), K(ori_schema_version), KPC(table_schema));
    ...
    if (OB_ENTRY_NOT_EXIST == ret) {
      ...
      LOG_WARN("get tablet and partition idx by object_id failed", KR(ret), K(part_info),
          K(tablet_id), K(part_idx), K(subpart_idx), K(ori_schema_version), K(new_schema_version), KPC(new_table_schema));
    }
