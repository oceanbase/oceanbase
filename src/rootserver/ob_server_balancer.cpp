...
int ObServerBalancer::init(share::schema::ObMultiVersionSchemaService& schema_service, ObUnitManager& unit_manager,
...
int ObServerBalancer::tenant_group_balance()
{
  ...
  ObString switch_config_str = GCONF.__balance_controller.str();
  {
    ...
    } else if (OB_FAIL(unit_mgr_->get_soft_limit(soft_limit))) {
      LOG_WARN("fail to get soft limit", K(ret));
    } else if (soft_limit < 1.0 &&
               (GCONF.is_rebalance_enabled() || balance_controller.at(ObRootBalanceHelp::ENABLE_SERVER_BALANCE))) {
...
int ObServerBalancer::balance_servers()
{
  ...
  DEBUG_SYNC(START_UNIT_BALANCE);
  {
    ...
    } else if (OB_FAIL(distribute_for_unit_intersect())) {
      LOG_WARN("fail to distribute for special unit", K(ret));
    } else {
      // When the server status changes,
      // try to adjust the unit on the server without configuration item control
      if (GCONF.is_rereplication_enabled() || GCONF.is_rebalance_enabled() ||
    ...
    const share::ObResourcePool& pool, const common::ObIArray<common::ObAddr>& sys_unit_server_array)
{
  ...
  if (OB_UNLIKELY(OB_INVALID_ID == pool.resource_pool_id_)) {
    ...
    for (int64_t i = 0; OB_SUCC(ret) && i < pool_unit_array->count(); ++i) {
      ...
      if (OB_UNLIKELY(nullptr == unit)) {
      ...
      } else if (OB_FAIL(unit_mgr_->get_excluded_servers(*unit, unit_stat, excluded_servers))) {
        ...
        LOG_WARN("fail tp append sys unit server array", K(ret));
                     ...
                     unit_mgr_->choose_server_for_unit(*unit_config, unit->zone_, excluded_servers, migrate_server))) {
        if (OB_MACHINE_RESOURCE_NOT_ENOUGH == ret) {
          ret = OB_SUCCESS;
          LOG_WARN("has no place to migrate unit", "unit", *unit, K(excluded_servers));
        } else {
          LOG_WARN("fail to choose server for unit", K(ret), K(unit), K(excluded_servers));
        }
...
int ObServerBalancer::distribute_for_standalone_sys_unit()
{
  ...
  if (!check_inner_stat()) {
    ...
    LOG_WARN("sys unit standalone deployment is disabled", K(ret));
                 ...
                 OB_SYS_TENANT_ID, empty_zone, empty_processed_id_array, sys_unit_server_array))) {
    ...
    for (; OB_SUCC(ret) && iter != end; ++iter) {
      ...
      } else if (OB_FAIL(distribute_pool_for_standalone_sys_unit(*iter->second, sys_unit_server_array))) {
...
// After version 14x, two resouce_pools of the same tenant are allowed to overlap on the zone
// Therefore, after multiple resource_pools are assigned to tenants,
// the units corresponding to these resource_pools need to be re-distributed to the machine
// Avoid having two units of the same server belong to the same tenant
int ObServerBalancer::distribute_for_unit_intersect()
{
  int ret = OB_SUCCESS;
  LOG_INFO("start distribute for tenant unit intersect");
  if (!check_inner_stat()) {
    ret = OB_INNER_STAT_ERROR;
    LOG_WARN("check_inner_stat failed", K_(inited), K(ret));
  }
  ObHashMap<uint64_t, ObArray<share::ObResourcePool*>*>::const_iterator it = unit_mgr_->get_tenant_pools_map().begin();
  ...
  ObArray<ObAddr> server_list;  // Record the location information of the unit under a single tenant
  ObArray<ObUnitStat> in_migrate_unit_stat;
  ObArray<ObAddr> excluded_servers;
  ObArray<ObAddr> sys_unit_servers;
  const common::ObZone empty_zone;  // means all zones
  const common::ObArray<uint64_t> empty_processed_id_array;
  if (OB_FAIL(unit_mgr_->get_tenant_unit_servers(
          OB_SYS_TENANT_ID, empty_zone, empty_processed_id_array, sys_unit_servers))) {
    ...
    for (; OB_SUCC(ret) && it != it_end; ++it) {
      ...
      if (OB_UNLIKELY(OB_INVALID_ID == tenant_id)) {
        ...
        // For each tenant, get the unit under the resource_pool corresponding to the tenant;
        // check whether the machine where the unit is located overlaps
        ObIArray<share::ObResourcePool*>& pools = *(it->second);
        ObZone zone;
        ObAddr migrate_server;
        ObUnitStat unit_stat;
        for (int64_t i = 0; i < pools.count() && OB_SUCC(ret); i++) {
          unit_infos.reset();
          if (OB_FAIL(unit_mgr_->inner_get_unit_infos_of_pool(pools.at(i)->resource_pool_id_, unit_infos))) {
            LOG_WARN("fail to get unit infos", K(ret), K(pools));
          } else {
            for (int64_t j = 0; j < unit_infos.count() && OB_SUCC(ret); j++) {
              ObUnitInfo& unit = unit_infos.at(j);
              zone = unit.unit_.zone_;
              unit_stat.reset();
              in_migrate_unit_stat.reset();
              excluded_servers.reset();
              if (!has_exist_in_array(server_list, unit.unit_.server_)) {
                if (OB_FAIL(server_list.push_back(unit.unit_.server_))) {
                  LOG_WARN("fail to push back", K(ret), K(unit));
                }
              } else if (OB_FAIL(unit_stat_mgr_->get_unit_stat(unit.unit_.unit_id_, unit_stat))) {
                LOG_WARN("fail to locate unit", K(ret), "unit", unit.unit_);
              } else if (OB_FAIL(unit_mgr_->get_excluded_servers(unit.unit_, unit_stat, excluded_servers))) {
                LOG_WARN("fail to get exclude servers", K(ret), K(unit.unit_));
                ...
                LOG_WARN("fail to append sys unit servers", K(ret));
                             ...
                             unit_mgr_->choose_server_for_unit(unit.config_, zone, excluded_servers, migrate_server))) {
                LOG_WARN("fail to choose server for unit", K(ret), K(unit), K(excluded_servers));
...
int ObServerBalancer::distribute_for_server_status_change()
{
  ...
  for (; OB_SUCC(ret) && it != it_end; ++it) {
    if (OB_SYS_TENANT_ID != it->first) {
      ...
      LOG_WARN("distribute_by_tenant failed", "tenant_id", it->first, K(ret));
    }
  }
  ObHashMap<uint64_t, ObArray<share::ObResourcePool*>*>::const_iterator it2 = unit_mgr_->get_tenant_pools_map().begin();
  ...
  for (; OB_SUCC(ret) && it2 != it2_end; ++it2) {
    if (OB_SYS_TENANT_ID == it2->first) {
      ...
      LOG_WARN("distribute_by_tenant failed", "tenant_id", it2->first, K(ret));
    }
  }

  ObHashMap<uint64_t, share::ObResourcePool*>::const_iterator pool_it = unit_mgr_->get_id_pool_map().begin();
  ObHashMap<uint64_t, share::ObResourcePool*>::const_iterator pool_it_end = unit_mgr_->get_id_pool_map().end();
  for (; OB_SUCC(ret) && pool_it != pool_it_end; ++pool_it) {
    if (NULL == pool_it->second) {
      ...
      // bypass, already distribute in distribute_by_tenant
    } else if (OB_FAIL(distribute_by_pool(pool_it->second))) {
      LOG_WARN("distribute_by_pool failed", "pool", *(pool_it->second), K(ret));
    }
  }
...
int ObServerBalancer::distribute_by_tenant(const uint64_t tenant_id, const ObArray<share::ObResourcePool*>& pools)
{
  int ret = OB_SUCCESS;
...
int ObServerBalancer::distribute_zone_unit(const ObUnitManager::ZoneUnit& zone_unit)
{
  ...
  if (!check_inner_stat()) {
    ...
    FOREACH_CNT_X(unit_info, zone_unit.unit_infos_, OB_SUCCESS == ret)
    {
      ...
      } else if (status.is_permanent_offline() || status.is_deleting() || status.is_taken_over_by_rs()) {
        if (OB_FAIL(distribute_for_permanent_offline_or_delete(status, *unit_info))) {
          LOG_WARN("distribute for permanent offline or delete failed", K(status), "unit_info", *unit_info, K(ret));
        }
      }

      // should go on doing even some unit choose dst server failed
      if (OB_MACHINE_RESOURCE_NOT_ENOUGH == ret) {
        ret = OB_SUCCESS;
      }
    }
...
int ObServerBalancer::distribute_for_active(const ObServerStatus& status, const ObUnitInfo& unit_info)
{
  int ret = OB_SUCCESS;
  if (!check_inner_stat()) {
    ret = OB_INNER_STAT_ERROR;
    ...
    if ((status.is_migrate_in_blocked()) && unit_info.unit_.migrate_from_server_.is_valid()) {
      LOG_INFO("find unit server active but can't migrate in, "
               "migrate_from_server is set",
          "unit",
          unit_info.unit_);
...
int ObServerBalancer::distribute_for_permanent_offline_or_delete(
    const ObServerStatus& status, const ObUnitInfo& unit_info)
{
  int ret = OB_SUCCESS;
  ...
  if (!check_inner_stat()) {
    ...
    LOG_INFO("server is permanent offline or in deleting status, need migrate unit", K(unit_info), K(status));
  } else {
    ...
    if (OB_FAIL(try_cancel_migrate_unit(unit_info.unit_, is_canceled))) {
      ...
      if (!status.is_with_partition()) {
        ...
        if (OB_FAIL(unit_mgr_->end_migrate_unit(unit_info.unit_.unit_id_, op))) {
          ...
          need_migrate_unit = true;
          LOG_INFO("unit has no partition, abort the migration", K(ret), K(unit_info), K(op), K(status));
        }
      }
    } else {
  ...
  if (OB_FAIL(ret) || !need_migrate_unit) {
    ...
    ret = OB_ERR_UNEXPECTED;
    LOG_WARN("invalid tenatn stat", K(ret), K(tenant_stat_));
  } else if (OB_FAIL(unit_stat_mgr_->get_unit_stat(unit_info.unit_.unit_id_, unit_stat))) {
    LOG_WARN("fail to locate unit", K(ret), "unit", unit_info.unit_);
  } else if (OB_FAIL(unit_mgr_->get_tenant_unit_servers(
                 OB_SYS_TENANT_ID, empty_zone, empty_processed_id_array, sys_unit_servers))) {
    LOG_WARN("fail to get tenant unit servers", K(ret));
  } else {
    if (OB_FAIL(unit_mgr_->get_excluded_servers(unit_info.unit_, unit_stat, excluded_servers))) {
      LOG_WARN("get_excluded_servers failed", "unit", unit_info.unit_, K(ret));
    } else if (OB_SYS_TENANT_ID != unit_info.pool_.tenant_id_ && enable_sys_unit_standalone &&
               OB_FAIL(append(excluded_servers, sys_unit_servers))) {
      LOG_WARN("fail to append sys unit servers", K(ret));
    } else if (OB_FAIL(unit_mgr_->choose_server_for_unit(unit_info.config_, zone, excluded_servers, migrate_server))) {
      LOG_WARN("choose_unit_server failed", "config", unit_info.config_, K(zone), K(excluded_servers), K(ret));
    }
    if (OB_SUCC(ret)) {
      ObArray<ObUnitStat> in_migrate_unit_stat;
...
int ObServerBalancer::distribute_for_migrate_in_blocked(const ObUnitInfo& unit_info)
{
  ...
  if (!check_inner_stat()) {
  ...
  } else if (OB_FAIL(server_mgr_->get_server_status(unit_info.unit_.migrate_from_server_, status))) {
    LOG_WARN("get_server_status failed", "server", unit_info.unit_.migrate_from_server_, K(ret));
  } else if (OB_FAIL(tenant_stat_->unit_stat_map_.locate(unit_info.unit_.unit_id_, u))) {
    LOG_WARN("fail to get unit info", K(ret), "unit", unit_info.unit_);
    ...
    if (status.can_migrate_in()) {
    ...
    } else if (OB_ISNULL(u) || u->v_.inside_replica_cnt_ != 0) {
      // nothing todo
      LOG_INFO("unit migrate_from_server can not migrate in, but dest_observer has parititon, wait for a while",
          "unit",
          unit_info.unit_);
    } else {
      LOG_INFO("unit migrate_from_server can not migrate in, and unit_server is empty now"
               "migrate unit to new server",
          "unit",
          unit_info.unit_);
      ObAddr new_server;
      ObArray<ObAddr> excluded_servers;
      ...
      if (OB_FAIL(unit_mgr_->end_migrate_unit(unit_info.unit_.unit_id_, op))) {
        ...
        LOG_WARN("get_excluded_servers failed", "unit", unit_info.unit_, K(ret));
                     ...
                     unit_info.config_, unit_info.unit_.zone_, excluded_servers, new_server))) {
...
int ObServerBalancer::get_unit_resource_reservation(
    uint64_t unit_id, const ObAddr& new_server, ObIArray<ObUnitStat>& in_migrate_unit_stat)
{
  int ret = OB_SUCCESS;
  UNUSED(unit_id);
    ...
    if (OB_FAIL(unit_mgr_->get_loads_by_server(new_server, unit_loads))) {
      ...
      for (int64_t i = 0; OB_SUCC(ret) && i < unit_loads->count(); ++i) {
        ...
        if (!load.is_valid()) {
          ...
          // by pass
        } else if (OB_FAIL(unit_stat_mgr_->get_unit_stat(load.unit_->unit_id_, in_mig_stat))) {
          LOG_WARN("get_unit_stat fail", "unit_id", load.unit_->unit_id_, K(ret));
...
bool ObServerBalancer::UnitMigrateStatCmp::operator()(const UnitMigrateStat& left, const UnitMigrateStat& right)
{
  bool bool_ret = false;
  if (OB_SUCCESS != ret_) {
    // do nothing
...
int ObServerBalancer::check_can_execute_rebalance(const common::ObZone& zone, bool& can_execute_rebalance)
{
  int ret = OB_SUCCESS;
  HEAP_VAR(share::ObZoneInfo, zone_info)
  {
    ...
    if (OB_UNLIKELY(!inited_)) {
    ...
    } else if (OB_UNLIKELY(NULL == server_mgr_ || NULL == unit_mgr_ || NULL == zone_mgr_)) {
      ret = OB_ERR_UNEXPECTED;
      ...
      for (int64_t i = 0; can_execute_rebalance && OB_SUCC(ret) && i < server_list.count(); ++i) {
        ...
        } else if (fabs(server_status.resource_info_.report_cpu_assigned_ - sum_load.min_cpu_) > CPU_EPSILON ||
                   fabs(server_status.resource_info_.report_cpu_max_assigned_ - sum_load.max_cpu_) > CPU_EPSILON ||
                   server_status.resource_info_.report_mem_assigned_ != sum_load.min_memory_ ||
                   server_status.resource_info_.report_mem_max_assigned_ != sum_load.max_memory_) {
          can_execute_rebalance = false;
...
int ObServerBalancer::rebalance_servers_v2(const common::ObZone& zone)
{
  int ret = OB_SUCCESS;
  ...
  } else if (OB_FAIL(do_rebalance_servers_v2(zone, group_tenant_array, standalone_units, not_grant_units))) {
    ...
    for (int64_t row = 0; OB_SUCC(ret) && row < tenant_name_group.row_; ++row) {
      for (int64_t column = 0; OB_SUCC(ret) && column < tenant_name_group.column_; ++column) {
        ...
        if (idx >= tenant_name_group.tenants_.count()) {
          ret = OB_ERR_UNEXPECTED;
          LOG_WARN("idx value unexpected", K(ret), K(idx), "tenants_count", tenant_name_group.tenants_.count());
        } else if (OB_FAIL(schema_guard.get_tenant_info(tenant_name_group.tenants_.at(idx), tenant_schema))) {
          LOG_WARN("fail to get tenant info", K(ret), "tenant_name", tenant_name_group.tenants_.at(idx));
          ...
          LOG_WARN(
              "fail to set tenant id matrix", K(ret), K(row), K(column), "tenant_id", tenant_schema->get_tenant_id());
        } else {
        }  // next matrix cell
      }
    }
    ...
    const uint64_t tenant_id, const common::ObIArray<Matrix<uint64_t> >& group_tenant_array)
{
  ...
  for (int64_t i = 0; !exist && i < group_tenant_array.count(); ++i) {
    ...
    }  // not contain, go on next
  }
  return exist;
}

int ObServerBalancer::generate_standalone_tenant_array(
    const common::ObIArray<Matrix<uint64_t> >& group_tenant_array, common::ObIArray<uint64_t>& standalone_tenant_array)
{
  int ret = OB_SUCCESS;
  share::schema::ObSchemaGetterGuard schema_guard;
...
int ObServerBalancer::get_zone_ttg_string(const common::ObIArray<common::ObAddr>& server_list,
    common::ObString& ttg_string, common::ObIAllocator& allocator, bool& got)
{
  int ret = OB_SUCCESS;
  ...
  if (OB_UNLIKELY(!inited_)) {
    ret = OB_NOT_INIT;
    LOG_WARN("not init", K(ret));
  } else if (OB_UNLIKELY(nullptr == server_mgr_ || nullptr == root_service)) {
    ret = OB_ERR_UNEXPECTED;
    LOG_WARN("server_mgr_ or root_service ptr is null", K(ret));
  } else {
    got = false;
    for (int64_t i = 0; !got && OB_SUCC(ret) && i < server_list.count(); ++i) {
      ObString my_ttg_string;
      ttg_string.reset();
      const common::ObAddr& this_server = server_list.at(i);
      ...
      if (OB_FAIL(server_mgr_->check_server_active(this_server, is_active))) {
        ...
        // pass
      } else if (OB_FAIL(root_service->get_rpc_proxy().to(this_server).get_remote_tenant_group_string(my_ttg_string))) {
        LOG_WARN("fail to get tenant group string", K(ret));
      } else if (OB_FAIL(ob_write_string(allocator, my_ttg_string, ttg_string))) {
        LOG_WARN("fail to deep copy string", K(ret), K(my_ttg_string));
      } else {
        got = true;
      }
      if (OB_FAIL(ret)) {
...
int ObServerBalancer::generate_tenant_array(const common::ObZone& zone,
    common::ObIArray<Matrix<uint64_t> >& group_tenant_array, common::ObIArray<uint64_t>& standalone_tenant_array)
{
  int ret = OB_SUCCESS;
  common::ObArray<common::ObAddr> server_list;
  ...
  if (OB_UNLIKELY(!inited_)) {
    ...
    LOG_WARN("fail to get servers of zone", K(ret), K(zone));
  } else if (OB_FAIL(get_zone_ttg_string(server_list, ttg_string, allocator, got_remote_ttg_string))) {
    LOG_WARN("fail to get zone ttg string", K(ret), K(zone));
  } else if (!got_remote_ttg_string) {
    LOG_WARN("do not got remote ttg string", K(zone));
  } else if (OB_FAIL(ttg_parser.parse_tenant_groups(ttg_string, tenant_groups))) {
    LOG_WARN("fail to parse tenant groups", K(ret));
  } else if (OB_FAIL(generate_group_tenant_array(tenant_groups, group_tenant_array))) {
    LOG_WARN("fail to generate group tenant array", K(ret));
  } else if (OB_FAIL(generate_standalone_tenant_array(group_tenant_array, standalone_tenant_array))) {
    LOG_WARN("fail to generate standalone tenant array", K(ret));
  } else {
  }  // no more to do
  LOG_INFO("generate tenant array finish", K(ret), K(zone), K(standalone_tenant_array), K(group_tenant_array));
  return ret;
}

int ObServerBalancer::fill_pool_units_in_zone(
    const common::ObZone& zone, share::ObResourcePool& pool, common::ObIArray<ObUnitManager::ObUnitLoad>& unit_loads)
{
  int ret = OB_SUCCESS;
  if (OB_UNLIKELY(!inited_)) {
    ret = OB_NOT_INIT;
    ...
    const common::ObZone& zone, common::ObIArray<ObUnitManager::ObUnitLoad>& not_grant_units)
{
  ...
  if (OB_UNLIKELY(!inited_)) {
    ...
    for (iter = unit_mgr_->id_pool_map_.begin(); OB_SUCC(ret) && iter != unit_mgr_->id_pool_map_.end(); ++iter) {
...
int ObServerBalancer::generate_standalone_units(const common::ObZone& zone,
...
int ObServerBalancer::generate_available_servers(const common::ObZone& zone, const bool excluded_sys_unit_server,
  ...
  HEAP_VAR(share::ObZoneInfo, zone_info)
  {
    ...
    if (OB_UNLIKELY(!inited_)) {
      ...
      LOG_WARN("fail to get servers of zone", K(ret), K(zone));
                   ...
                   OB_SYS_TENANT_ID, zone, empty_processed_id_array, sys_unit_server_array))) {
      LOG_WARN("fail to get tenant unit server array", K(ret));
    } else {
      ...
      for (int64_t i = 0; OB_SUCC(ret) && i < server_list.count(); ++i) {
        ...
        if (OB_FAIL(server_mgr_->get_server_status(server_list.at(i), server_status))) {
          ...
          ret = OB_STATE_NOT_MATCH;
          LOG_WARN("server in zone is not stable, stop balance servers",
              K(ret),
              K(server_status),
              "is_temporary_offline",
              server_status.is_temporary_offline(),
              "is_stopped",
              server_status.is_stopped());
        } else if (excluded_sys_unit_server && has_exist_in_array(sys_unit_server_array, server_list.at(i))) {
          // bypass
...
int ObServerBalancer::try_balance_single_unit_by_disk(const ObUnitManager::ObUnitLoad& unit_load,
    ...
    bool& do_balance)
{
  ...
  } else if (OB_FAIL(unit_stat_mgr_->get_unit_stat(unit_load.unit_->unit_id_, unit_stat))) {
    LOG_WARN("fail to get unit stat", K(ret), "unit", *unit_load.unit_);
  } else {
    do_balance = false;
    for (int64_t i = 0; !do_balance && OB_SUCC(ret) && i < zone_disk_statistic_.server_disk_statistic_array_.count();
         ++i) {
      ServerTotalLoad* server_load = NULL;
      ServerDiskStatistic& server_disk_statistic = zone_disk_statistic_.server_disk_statistic_array_.at(i);
      ...
      } else if (OB_FAIL(pick_server_load(server_disk_statistic.server_, server_load_ptrs_sorted, server_load))) {
...
int ObServerBalancer::do_update_src_server_load(
    const ObUnitManager::ObUnitLoad& unit_load, ServerTotalLoad& this_server_load, const share::ObUnitStat& unit_stat)
{
  int ret = OB_SUCCESS;
  ...
  if (OB_FAIL(ret)) {
                 ...
                 zone_disk_statistic_.reduce_server_disk_use(this_server_load.server_, my_unit_stat.required_size_))) {
...
int ObServerBalancer::do_update_dst_server_load(const ObUnitManager::ObUnitLoad& unit_load,
    const UnitMigrateStat& unit_migrate_stat, ServerTotalLoad& this_server_load, const share::ObUnitStat& unit_stat,
    common::ObIArray<PoolOccupation>& pool_occupation)
{
  ...
  if (OB_FAIL(ret)) {
  } else if (OB_FAIL(pool_occupation.push_back(PoolOccupation(
                 unit_load.pool_->tenant_id_, unit_load.pool_->resource_pool_id_, this_server_load.server_)))) {
    LOG_WARN("fail to push back", K(ret));
  } else if (OB_FAIL(
                 zone_disk_statistic_.raise_server_disk_use(this_server_load.server_, my_unit_stat.required_size_))) {
...
int ObServerBalancer::try_balance_single_unit_by_cm(const ObUnitManager::ObUnitLoad& unit_load,
    common::ObArray<ServerTotalLoad*>& server_load_ptrs_sorted, common::ObIArray<UnitMigrateStat>& task_array,
    const common::ObIArray<common::ObAddr>& excluded_servers, common::ObIArray<PoolOccupation>& pool_occupation,
    bool& do_balance)
{
  ...
  if (OB_UNLIKELY(!inited_)) {
    ...
    do_balance = false;
    for (int64_t j = server_load_ptrs_sorted.count() - 1; !do_balance && OB_SUCC(ret) && j >= 0; --j) {
      ...
      if (OB_UNLIKELY(NULL == server_load)) {
        ...
        if (server_load->wild_server_) {
          ...
          LOG_WARN("fail to append load", K(ret));
        } else if (OB_FAIL(check_single_server_resource_enough(this_load, unit_stat, *server_load, enough))) {
...
int ObServerBalancer::do_balance_sys_tenant_single_unit(const ObUnitManager::ObUnitLoad& unit_load,
    common::ObArray<ServerTotalLoad*>& server_load_ptrs_sorted, common::ObIArray<UnitMigrateStat>& task_array,
    common::ObIArray<PoolOccupation>& pool_occupation)
{
  int ret = OB_SUCCESS;
  ...
  if (OB_UNLIKELY(!inited_)) {
    ...
    ret = OB_INVALID_ARGUMENT;
    LOG_WARN("invalid argument", K(ret), K(unit_load));
  } else if (OB_FAIL(get_pool_occupation_excluded_dst_servers(unit_load, pool_occupation, excluded_servers))) {
    LOG_WARN("fail to get excluded servers", K(ret));
  } else if (OB_FAIL(try_balance_single_unit_by_cm(
                 unit_load, server_load_ptrs_sorted, task_array, excluded_servers, pool_occupation, do_balance))) {
...
int ObServerBalancer::do_balance_sys_tenant_units(const common::ObIArray<ObUnitManager::ObUnitLoad>& sys_tenant_units,
    const common::ObIArray<common::ObAddr>& available_servers, common::ObArray<ServerTotalLoad>& server_loads)
{
  ...
  if (OB_UNLIKELY(!inited_)) {
    ...
    if (OB_FAIL(ret)) {
      ...
      LOG_WARN("fail to migrate unit task", K(ret));
    } else {
    }  // no more to do
  }
  return ret;
...
int ObServerBalancer::try_balance_sys_tenant_units(const common::ObZone& zone, bool& do_execute)
{
  int ret = OB_SUCCESS;
  ...
  if (OB_UNLIKELY(!inited_)) {
    ...
    ret = OB_INVALID_ARGUMENT;
    LOG_WARN("invalid argument", K(ret), K(zone));
  } else if (OB_FAIL(generate_available_servers(zone, false /* sys unit not excluded*/, available_servers))) {
  ...
  } else if (OB_FAIL(unit_mgr_->get_tenant_zone_all_unit_loads(common::OB_SYS_TENANT_ID, zone, sys_tenant_units))) {
    if (OB_ENTRY_NOT_EXIST == ret) {
      ret = OB_SUCCESS;
    } else {
      LOG_WARN("fail to get tenant zone unit loads", K(ret), K(zone));
    }
  } else if (OB_FAIL(check_need_balance_sys_tenant_units(sys_tenant_units, available_servers, need_balance))) {
    LOG_WARN("fail to check need balance sys tenant units", K(ret));
  } else if (!need_balance) {
    do_execute = false;
  } else if (OB_FAIL(calc_global_balance_resource_weights(zone, available_servers, g_res_weights, RES_MAX))) {
    LOG_WARN("fail to calc whole balance resource weights", K(ret));
  } else if (OB_FAIL(generate_complete_server_loads(zone, available_servers, g_res_weights, RES_MAX, server_loads))) {
    LOG_WARN("fail to generate complete server loads", K(ret));
  } else if (OB_FAIL(do_balance_sys_tenant_units(sys_tenant_units, available_servers, server_loads))) {
...
int ObServerBalancer::try_balance_non_sys_tenant_units(const common::ObZone& zone,
    ...
    const common::ObIArray<ObUnitManager::ObUnitLoad>& not_grant_units)
{
  ...
  if (OB_UNLIKELY(!inited_)) {
  ...
  } else if (OB_FAIL(generate_available_servers(zone, enable_sys_unit_standalone, available_servers))) {
    ...
    for (int64_t i = 0; OB_SUCC(ret) && i < balance_info_array.count(); ++i) {
      ...
      } else if (OB_FAIL(generate_original_unit_matrix(*balance_info.tenant_id_matrix_,
                     zone,
      ...
      } else if (OB_FAIL(single_unit_migrate_stat_matrix_balance(balance_info, available_servers))) {
        LOG_WARN("fail to do migrate stat matrix balance", K(ret));
      } else {
        LOG_INFO("finish single ttg balance", "tenant_id_matrix", *balance_info.tenant_id_matrix_);
      }
    }
    if (OB_FAIL(ret)) {
    } else if (OB_FAIL(try_amend_and_execute_unit_balance_task(
                   zone, balance_info_array, standalone_units, not_grant_units, available_servers))) {
...
int ObServerBalancer::generate_tenant_balance_info_array(const common::ObZone& zone,
    const common::ObIArray<Matrix<uint64_t> >& group_tenant_array,
  ...
  if (OB_UNLIKELY(!inited_)) {
    ...
    for (int64_t i = 0; OB_SUCC(ret) && i < group_tenant_array.count(); ++i) {
      ...
      if (OB_FAIL(check_and_get_tenant_matrix_unit_num(
              group_tenant_array.at(i), zone, unit_num_match, balance_info.column_unit_num_array_))) {
      ...
      }  // no tenant of this group exists in this zone
    }
    if (OB_FAIL(ret)) {
...
int ObServerBalancer::try_generate_degraded_balance_info_array(const common::ObZone& zone,
    ...
    common::ObIArray<TenantGroupBalanceInfo>& balance_info_array)
{
  ...
  if (OB_UNLIKELY(!inited_)) {
    ...
    for (int64_t i = 0; OB_SUCC(ret) && i < unit_num_not_match_array.count(); ++i) {
      ...
      } else if (OB_FAIL(do_degrade_tenant_group_matrix(*this_tenant_group, degraded_tenant_group_array))) {
    ...
    for (int64_t i = 0; OB_SUCC(ret) && i < degraded_tenant_group_array.count(); ++i) {
      ...
      if (OB_FAIL(check_and_get_tenant_matrix_unit_num(
              degraded_tenant_group_array.at(i), zone, unit_num_match, balance_info.column_unit_num_array_))) {
        LOG_WARN("fail to check and get tenant matrix unit num", K(ret));
      } else if (!unit_num_match) {
        ret = OB_ERR_UNEXPECTED;
        LOG_WARN("unit num not match unexpected, since this tenant group is a vector",
            K(ret),
            "vector tenant group",
            degraded_tenant_group_array.at(i));
      } else if (balance_info.column_unit_num_array_.count() > 0) {
        if (OB_FAIL(balance_info_array.push_back(balance_info))) {
          LOG_WARN("fail to push back", K(ret));
        }
...
int ObServerBalancer::do_rebalance_servers_v2(const common::ObZone& zone,
    const common::ObIArray<Matrix<uint64_t> >& group_tenant_array,
    const common::ObIArray<ObUnitManager::ObUnitLoad>& standalone_units,
    const common::ObIArray<ObUnitManager::ObUnitLoad>& not_grant_units)
{
  ...
  } else if (OB_FAIL(try_balance_non_sys_tenant_units(zone, group_tenant_array, standalone_units, not_grant_units))) {
    LOG_WARN("fail to try balance non sys tenant units", K(ret));
  }
  return ret;
}

int ObServerBalancer::try_amend_and_execute_unit_balance_task(const common::ObZone& zone,
    common::ObIArray<TenantGroupBalanceInfo>& balance_info_array,
    const common::ObIArray<ObUnitManager::ObUnitLoad>& standalone_units,
    const common::ObIArray<ObUnitManager::ObUnitLoad>& not_grant_units,
    const common::ObIArray<common::ObAddr>& available_servers)
{
  ...
  if (OB_UNLIKELY(!inited_)) {
  ...
  } else if (OB_FAIL(divide_tenantgroup_balance_info(balance_info_array, stable_tenant_group, unstable_tenant_group))) {
    LOG_WARN("fail to divide tenantgroup balance info", K(ret));
  } else if (OB_FAIL(try_amend_and_execute_stable_tenantgroup(zone,
                 unstable_tenant_group,
                 stable_tenant_group,
                 standalone_units,
                 not_grant_units,
                 available_servers,
    ...
    if (unstable_tenant_group.count() > 0) {
      if (OB_FAIL(try_amend_and_execute_unstable_tenantgroup(zone,
              unstable_tenant_group,
              stable_tenant_group,
              standalone_units,
              not_grant_units,
              available_servers,
              do_amend_unstable_ttg))) {
    ...
    if (OB_FAIL(ret)) {
      ...
      if (OB_FAIL(
              try_balance_disk_by_stable_tenantgroup(zone, stable_tenant_group, available_servers, do_balance_disk))) {
...
int ObServerBalancer::generate_simple_ug_loads(
    common::ObIArray<SimpleUgLoad>& simple_ug_loads, TenantGroupBalanceInfo& balance_info)
{
  int ret = OB_SUCCESS;
  if (OB_UNLIKELY(!inited_)) {
    ret = OB_NOT_INIT;
    LOG_WARN("not init", K(ret));
  } else if (balance_info.unitgroup_load_array_.count() != balance_info.unit_migrate_stat_matrix_.get_column_count()) {
    ret = OB_ERR_UNEXPECTED;
    LOG_WARN("unitgroup load count and unit migrate stat matrix column not match",
        K(ret),
        "unitgroup_count",
        balance_info.unitgroup_load_array_.count(),
        "unit_matrix_column",
        balance_info.unit_migrate_stat_matrix_.get_column_count());
  } else if (balance_info.unit_migrate_stat_matrix_.get_row_count() <= 0) {
    ret = OB_ERR_UNEXPECTED;
    LOG_WARN("unit matrix row unexpected",
        K(ret),
        "unit_matrix_row",
        balance_info.unit_migrate_stat_matrix_.get_row_count());
  } else {
    simple_ug_loads.reset();
    for (int64_t column = 0; OB_SUCC(ret) && column < balance_info.unit_migrate_stat_matrix_.get_column_count();
         ++column) {
...
int ObServerBalancer::get_ug_exchange_excluded_dst_servers(const int64_t ug_idx,
    const Matrix<UnitMigrateStat>& unit_migrate_stat_matrix, common::ObIArray<common::ObAddr>& excluded_servers)
{
  ...
  if (OB_UNLIKELY(!inited_)) {
    ...
    LOG_WARN("invalid argument",
        K(ret),
        K(ug_idx),
        "unit migrate stat matrix column",
        unit_migrate_stat_matrix.get_column_count());
  } else if (OB_UNLIKELY(NULL == unit_mgr_)) {
    ret = OB_ERR_UNEXPECTED;
    LOG_WARN("unit_mgr_ ptr is null", K(ret));
  }
...
int ObServerBalancer::check_cm_resource_enough(
    const LoadSum& this_load, const ServerTotalLoad& server_load, bool& enough)
{
  int ret = OB_SUCCESS;
  ...
  if (OB_UNLIKELY(!inited_)) {
    ...
    enough = ((this_load.load_sum_.max_cpu_ + server_load.load_sum_.load_sum_.max_cpu_ <=
                  server_load.resource_info_.cpu_ * hard_limit) &&
              (this_load.load_sum_.min_cpu_ + server_load.load_sum_.load_sum_.min_cpu_ <=
                  server_load.resource_info_.cpu_) &&
              (static_cast<double>(this_load.load_sum_.max_memory_) +
                      static_cast<double>(server_load.load_sum_.load_sum_.max_memory_) <=
                  static_cast<double>(server_load.resource_info_.mem_total_) * hard_limit) &&
              (static_cast<double>(this_load.load_sum_.min_memory_) +
                      static_cast<double>(server_load.load_sum_.load_sum_.min_memory_) <=
                  static_cast<double>(server_load.resource_info_.mem_total_)));
  }
  return ret;
}

int ObServerBalancer::check_exchange_ug_make_sense(common::ObIArray<ServerTotalLoad>& server_loads,
    const int64_t left_idx, const int64_t right_idx, common::ObIArray<SimpleUgLoad>& simple_ug_loads,
    Matrix<UnitMigrateStat>& unit_migrate_stat_matrix, bool& do_make_sense)
{
  ...
  if (OB_UNLIKELY(!inited_)) {
    ret = OB_NOT_INIT;
    LOG_WARN("not init", K(ret));
  } else if (left_idx >= simple_ug_loads.count() || right_idx >= simple_ug_loads.count() ||
    ...
    for (int64_t row = 0; OB_SUCC(ret) && row < unit_migrate_stat_matrix.get_row_count(); ++row) {
      ...
      if (OB_FAIL(unit_migrate_stat_matrix.get(row, left_idx, left_unit_migrate))) {
      ...
      } else if (OB_FAIL(unit_stat_mgr_->get_unit_stat(left_unit_migrate.unit_load_.unit_->unit_id_, left_unit_stat))) {
        ...
        LOG_WARN("unit load is invalid", K(ret));
                     ...
                     unit_stat_mgr_->get_unit_stat(right_unit_migrate.unit_load_.unit_->unit_id_, right_unit_stat))) {
        LOG_WARN("fail to get unit stat", K(ret));
      } else {
    ...
    if (OB_SUCC(ret)) {
      ...
      if (OB_FAIL(zone_disk_statistic_.get_server_disk_statistic(
        ...
        LOG_WARN("fail to get server disk statistic", K(ret));
                     ...
                     simple_ug_loads.at(left_idx).original_server_, server_load_ptrs, left_server_load))) {
        LOG_WARN("fail to get server load", K(ret));
      } else if (OB_FAIL(pick_server_load(
                     simple_ug_loads.at(right_idx).original_server_, server_load_ptrs, right_server_load))) {
        LOG_WARN("fail to get server load", K(ret));
      } else if (NULL == left_server_load || NULL == right_server_load) {
        ret = OB_ERR_UNEXPECTED;
        LOG_WARN("left server or right server load ptr is null", K(ret));
      } else if (OB_FAIL(
                     check_cm_resource_enough(simple_ug_loads.at(left_idx).load_sum_, *right_server_load, enough))) {
        LOG_WARN("fail to check cpu memory resource enough", K(ret));
      } else if (!enough) {
        do_make_sense = false;
      } else if (OB_FAIL(
                     check_cm_resource_enough(simple_ug_loads.at(right_idx).load_sum_, *left_server_load, enough))) {
        LOG_WARN("fail to check cpu memory resource enough", K(ret));
      } else if (!enough) {
        do_make_sense = false;
      } else if (left_unit_use < right_unit_use) {
        do_make_sense = false;  // Cannot lower the left disk after swap
      } else if (static_cast<double>(right_disk_statistic.disk_in_use_ - right_unit_use + left_unit_use) >
      ...
      } else if (static_cast<double>(right_disk_statistic.disk_in_use_ + left_unit_use) >
      ...
      } else if (static_cast<double>(left_disk_statistic.disk_in_use_ + right_unit_use) >
                 disk_usage_limit * static_cast<double>(left_disk_statistic.disk_total_)) {
        do_make_sense = false;
      } else if (static_cast<double>(left_unit_use - right_unit_use) <
...
int ObServerBalancer::coordinate_unit_migrate_stat_matrix(const int64_t left_idx, const int64_t right_idx,
    common::ObIArray<SimpleUgLoad>& simple_ug_loads, Matrix<UnitMigrateStat>& unit_migrate_stat_matrix)
{
  ...
  if (OB_UNLIKELY(!inited_)) {
    ret = OB_NOT_INIT;
    LOG_WARN("not init", K(ret));
  } else if (OB_UNLIKELY(left_idx >= simple_ug_loads.count() || right_idx >= simple_ug_loads.count())) {
    ret = OB_INVALID_ARGUMENT;
    LOG_WARN("invalid argument", K(ret), K(left_idx), K(right_idx), "simple ug loads count", simple_ug_loads.count());
  } else {
...
int ObServerBalancer::generate_exchange_ug_migrate_task(const int64_t left_idx, const int64_t right_idx,
    common::ObIArray<SimpleUgLoad>& simple_ug_loads, Matrix<UnitMigrateStat>& unit_migrate_stat_matrix,
...
int ObServerBalancer::try_balance_server_disk_onebyone(common::ObIArray<ServerTotalLoad>& server_loads,
    ...
    bool& do_balance_disk)
{
  ...
  if (OB_UNLIKELY(!inited_)) {
    ...
    for (int64_t i = 0;
         !do_balance_disk && OB_SUCC(ret) && i < zone_disk_statistic_.server_disk_statistic_array_.count();
         ++i) {
      ...
      if (has_exist_in_array(left_excluded_servers, disk_statistic.server_)) {
        ...
        for (int64_t j = 0; !do_balance_disk && OB_SUCC(ret) && j < simple_ug_loads.count(); ++j) {
          ...
          if (left_ug_load.column_tenant_id_ == right_ug_load.column_tenant_id_) {
            ...
            // not in this server
          } else if ((left_ug_load.load_sum_.load_sum_.min_cpu_ != right_ug_load.load_sum_.load_sum_.min_cpu_) ||
                     (left_ug_load.load_sum_.load_sum_.min_memory_ != right_ug_load.load_sum_.load_sum_.min_memory_)) {
          ...
          } else if (OB_FAIL(check_exchange_ug_make_sense(server_loads,
                         ug_idx,
                         j,
                         simple_ug_loads,
                         balance_info.unit_migrate_stat_matrix_,
            ...
            LOG_WARN("fail to coordinate unit migrate stat matrix", K(ret));
                         ...
                         ug_idx, j, simple_ug_loads, balance_info.unit_migrate_stat_matrix_, task_array))) {
...
int ObServerBalancer::try_balance_server_disk_by_ttg(common::ObIArray<ServerTotalLoad>& server_loads,
    const common::ObAddr& src_server, TenantGroupBalanceInfo& balance_info,
    const common::ObIArray<common::ObAddr>& available_servers,
    const common::ObIArray<common::ObAddr>& disk_over_servers, common::ObIArray<UnitMigrateStat>& task_array,
    bool& do_balance_disk)
{
  ...
  if (OB_UNLIKELY(!inited_)) {
    ...
    for (int64_t i = 0; OB_SUCC(ret) && i < simple_ug_loads.count(); ++i) {
      ...
      } else if (OB_FAIL(try_balance_server_disk_onebyone(server_loads,
                     balance_info,
                     simple_ug_loads,
                     i,
                     available_servers,
...
int ObServerBalancer::try_balance_disk_by_stable_tenantgroup(const common::ObZone& zone,
  ...
  if (OB_UNLIKELY(!inited_)) {
    ...
    for (int64_t i = 0; OB_SUCC(ret) && i < disk_over_servers.count(); ++i) {
      ...
      for (int64_t j = 0; OB_SUCC(ret) && j < stable_ttg.count(); ++j) {
        ...
        } else if (OB_FAIL(try_balance_server_disk_by_ttg(server_loads,
                       src_server,
                       *balance_info,
                       available_servers,
                       disk_over_servers,
    ...
    if (OB_SUCC(ret) && task_array.count() > 0) {
      if (OB_FAIL(do_migrate_unit_task(task_array))) {
...
int ObServerBalancer::try_amend_and_execute_stable_tenantgroup(const common::ObZone& zone,
    ...
    const common::ObIArray<common::ObAddr>& available_servers, bool& do_amend)
{
  ...
  if (OB_UNLIKELY(!inited_)) {
    ...
    do_amend = false;
  } else if (OB_FAIL(choose_balance_info_amend(stable_tenant_group, is_stable_ttg, info_may_amend, other_stable_ttg))) {
  ...
  } else if (OB_FAIL(try_amend_and_execute_tenantgroup(zone,
                 info_may_amend,
                 other_stable_ttg,
                 unstable_tenant_group,
                 standalone_units,
...
int ObServerBalancer::try_amend_and_execute_unstable_tenantgroup(const common::ObZone& zone,
    const common::ObIArray<TenantGroupBalanceInfo*>& unstable_tenant_group,
    const common::ObIArray<TenantGroupBalanceInfo*>& stable_tenant_group,
    const common::ObIArray<ObUnitManager::ObUnitLoad>& standalone_units,
    const common::ObIArray<ObUnitManager::ObUnitLoad>& not_grant_units,
...
int ObServerBalancer::get_sys_tenant_unitgroup_loads(
    const common::ObZone& zone, common::ObIArray<UnitGroupLoad>& sys_tenant_ug_loads)
{
  int ret = OB_SUCCESS;
  ...
  if (OB_UNLIKELY(!inited_)) {
    ...
    ret = OB_ERR_UNEXPECTED;
    LOG_WARN("unit_mgr_ ptr is null", K(ret));
  } else if (OB_FAIL(unit_mgr_->get_tenant_zone_unit_loads(common::OB_SYS_TENANT_ID, zone, replica_type, unit_loads))) {
    if (OB_ENTRY_NOT_EXIST == ret) {
      ret = OB_SUCCESS;
    } else {
...
int ObServerBalancer::try_amend_and_execute_tenantgroup(const common::ObZone& zone, TenantGroupBalanceInfo* info_amend,
    const common::ObIArray<TenantGroupBalanceInfo*>& stable_tenant_group,
    const common::ObIArray<TenantGroupBalanceInfo*>& unstable_tenant_group,
    const common::ObIArray<ObUnitManager::ObUnitLoad>& standalone_units,
    const common::ObIArray<ObUnitManager::ObUnitLoad>& not_grant_units,
    const common::ObIArray<common::ObAddr>& available_servers, bool& do_amend)
{
  ...
  } else if (OB_FAIL(calc_inter_ttg_weights(info_amend,
                 stable_tenant_group,
                 sys_tenant_ug_loads,
                 available_servers,
                 info_amend->inter_weights_,
                 RES_MAX))) {
  ...
  } else if (OB_FAIL(do_amend_inter_ttg_balance(info_amend, stable_tenant_group, info_amend->server_load_array_))) {
    LOG_WARN("fail to do amend inter ttg balance", K(ret));
  } else if (OB_FAIL(try_execute_unit_balance_task(zone,
                 info_amend,
                 standalone_units,
                 not_grant_units,
                 unstable_tenant_group,
...
int ObServerBalancer::calc_inter_ttg_weights(TenantGroupBalanceInfo* info_need_amend,
    const common::ObIArray<TenantGroupBalanceInfo*>& stable_tenant_group,
    ...
    const int64_t weights_count)
{
  ...
  if (OB_UNLIKELY(!inited_)) {
    ret = OB_NOT_INIT;
    LOG_WARN("not init", K(ret));
  } else if (OB_UNLIKELY(NULL == resource_weights || RES_MAX != weights_count || NULL == info_need_amend)) {
    ret = OB_INVALID_ARGUMENT;
    LOG_WARN("invalid argument", K(ret), KP(info_need_amend));
    ...
    LoadSum load_sum;
    for (int64_t i = 0; OB_SUCC(ret) && i < info_need_amend->unitgroup_load_array_.count(); ++i) {
    ...
    for (int64_t i = 0; OB_SUCC(ret) && i < stable_tenant_group.count(); ++i) {
      const TenantGroupBalanceInfo* balance_info = stable_tenant_group.at(i);
      if (OB_UNLIKELY(NULL == balance_info)) {
        ret = OB_ERR_UNEXPECTED;
        LOG_WARN("balance info ptr is null", K(ret));
      } else {
        for (int64_t j = 0; OB_SUCC(ret) && j < balance_info->unitgroup_load_array_.count(); ++j) {
    ...
    for (int32_t i = RES_CPU; i < RES_MAX; ++i) {
      ...
      if (RES_DISK == resource_type) {
        resource_weights[resource_type] = 0.0;
      } else if (required <= 0 || capacity <= 0) {
        resource_weights[resource_type] = 0.0;
...
int ObServerBalancer::append_alien_ug_loads(ServerLoad& server_load,
  ...
  if (OB_UNLIKELY(!inited_)) {
    ...
    for (int64_t i = 0; OB_SUCC(ret) && i < tenant_groups.count(); ++i) {
      ...
      if (OB_UNLIKELY(NULL == balance_info)) {
        ...
        for (int64_t i = 0; OB_SUCC(ret) && i < balance_info->unitgroup_load_array_.count(); ++i) {
...
int ObServerBalancer::generate_inter_ttg_server_loads(TenantGroupBalanceInfo* info_need_amend,
    const common::ObIArray<TenantGroupBalanceInfo*>& stable_tenant_group,
    common::ObIArray<UnitGroupLoad>& sys_tenant_ug_loads, const common::ObIArray<common::ObAddr>& available_servers)
{
  ...
  if (OB_UNLIKELY(!inited_)) {
    ...
    ret = OB_INVALID_ARGUMENT;
    LOG_WARN("invalid argument", K(ret), KP(info_need_amend), "server_count", available_servers.count());
  } else {
    ...
    for (int64_t i = 0; OB_SUCC(ret) && i < target_server_loads.count(); ++i) {
      ...
      if (OB_FAIL(append_alien_ug_loads(target_server_loads.at(i), stable_tenant_group, sys_tenant_ug_loads))) {
...
int ObServerBalancer::do_amend_inter_ttg_balance(TenantGroupBalanceInfo* balance_info,
    const common::ObIArray<TenantGroupBalanceInfo*>& stable_tenant_group, common::ObIArray<ServerLoad>& server_loads)
{
  ...
  if (OB_UNLIKELY(!inited_)) {
    ret = OB_NOT_INIT;
    LOG_WARN("not init", K(ret));
  } else if (OB_UNLIKELY(NULL == balance_info || server_loads.count() <= 0)) {
    ret = OB_INVALID_ARGUMENT;
    LOG_WARN("invalid argument", K(ret), KP(balance_info), "server_load_count", server_loads.count());
    ...
    if (OB_SUCC(ret)) {
      ...
      if (OB_FAIL(cmp.get_ret())) {
        ...
        LOG_WARN("fail to make inter ttg server under load", K(ret));
                     ...
                     inner_ttg_balance_strategy_.coordinate_all_column_unit_group(balance_info->column_unit_num_array_,
                         balance_info->unit_migrate_stat_matrix_,
...
int ObServerBalancer::sort_server_loads_for_balance(common::ObIArray<ServerLoad*>& complete_server_loads,
...
int ObServerBalancer::make_inter_ttg_server_under_load(TenantGroupBalanceInfo* balance_info,
    common::ObIArray<ServerLoad*>& over_server_loads, common::ObIArray<ServerLoad*>& under_server_loads,
  ...
  if (OB_UNLIKELY(!inited_)) {
    ...
    for (int64_t i = 0; OB_SUCC(ret) && i < over_server_loads.count(); ++i) {
      ...
      for (int64_t j = under_server_loads.count() - 1; OB_SUCC(ret) && j >= 0; --j) {
        ...
        if (OB_FAIL(amend_ug_inter_ttg_server_load(over_server_load, under_server_load, balance_info, upper_limit))) {
...
int ObServerBalancer::amend_ug_inter_ttg_server_load(ServerLoad* src_inter_load, ServerLoad* dst_inter_load,
    TenantGroupBalanceInfo* balance_info, const double inter_ttg_upper_lmt)
{
  ...
  if (OB_UNLIKELY(!inited_)) {
    ret = OB_NOT_INIT;
    LOG_WARN("not init", K(ret));
  } else if (OB_UNLIKELY(NULL == src_inter_load || NULL == dst_inter_load || NULL == balance_info)) {
    ret = OB_INVALID_ARGUMENT;
    LOG_WARN("invalid argument", K(ret), KP(src_inter_load), KP(dst_inter_load), KP(balance_info));
...
int ObServerBalancer::CountBalanceStrategy::amend_ug_inter_ttg_server_load(ServerLoad* src_inter_load,
    ServerLoad* dst_inter_load, TenantGroupBalanceInfo* balance_info, const double inter_ttg_upper_lmt)
{
  int ret = OB_SUCCESS;
  UNUSED(inter_ttg_upper_lmt);
  if (OB_UNLIKELY(NULL == src_inter_load || NULL == dst_inter_load || NULL == balance_info)) {
    ret = OB_INVALID_ARGUMENT;
    LOG_WARN("invalid argument", K(ret), KP(src_inter_load), KP(dst_inter_load), KP(balance_info));
  } else {
    ...
    if (unitgroup_loads.count() <= 0) {
      ...
      if (src_inter_load->unitgroup_loads_.count() == dst_inter_load->unitgroup_loads_.count()) {
        if (OB_FAIL(try_exchange_ug_balance_inter_ttg_load(
                *src_inter_load, *dst_inter_load, balance_info, unitgroup_loads, inter_ttg_upper_lmt))) {
...
int ObServerBalancer::CountBalanceStrategy::try_exchange_ug_balance_inter_ttg_load(ServerLoad& src_inter_load,
    ServerLoad& dst_inter_load, TenantGroupBalanceInfo* balance_info, common::ObIArray<UnitGroupLoad>& unitgroup_loads,
    const double inter_ttg_upper_lmt)
{
  int ret = OB_SUCCESS;
  if (OB_UNLIKELY(NULL == balance_info)) {
    ...
    int64_t max_times = src_inter_load.unitgroup_loads_.count() + dst_inter_load.unitgroup_loads_.count();
    ...
    int64_t times = 0;
    while (OB_SUCC(ret) && times < max_times &&
      ...
      if (OB_FAIL(try_exchange_ug_balance_inter_ttg_load_foreach(
              src_inter_load, dst_inter_load, balance_info, unitgroup_loads, inter_ttg_upper_lmt, do_exchange))) {
...
int ObServerBalancer::CountBalanceStrategy::try_exchange_ug_balance_inter_ttg_load_foreach(ServerLoad& left_inter_load,
    ServerLoad& right_inter_load, TenantGroupBalanceInfo* balance_info,
    common::ObIArray<UnitGroupLoad>& unitgroup_loads, const double inter_ttg_upper_lmt, bool& do_exchange)
{
  int ret = OB_SUCCESS;
  do_exchange = false;
  if (OB_UNLIKELY(NULL == balance_info)) {
    ret = OB_INVALID_ARGUMENT;
    LOG_WARN("invalid argument", K(ret), KP(balance_info));
  } else if (left_inter_load.inter_ttg_load_value_ - right_inter_load.inter_ttg_load_value_ <= EPSILON) {
    // ignore, since this almost has no effect for load balance
  } else {
    for (int64_t i = 0; !do_exchange && OB_SUCC(ret) && i < left_inter_load.unitgroup_loads_.count(); ++i) {
      common::ObArray<common::ObAddr> excluded_servers;
      ...
      if (OB_UNLIKELY(NULL == left_ug)) {
        ...
        break;
      } else if (OB_FAIL(get_ug_balance_excluded_dst_servers(*left_ug, unitgroup_loads, excluded_servers))) {
        LOG_WARN("fail to get excluded server", K(ret));
      } else if (has_exist_in_array(excluded_servers, right_inter_load.server_)) {
        // ignore this
      } else {
        for (int64_t j = right_inter_load.unitgroup_loads_.count() - 1; !do_exchange && OB_SUCC(ret) && j >= 0; --j) {
          ...
          if (NULL == right_ug) {
            ...
            // ignore this
          } else if (OB_FAIL(get_ug_balance_excluded_dst_servers(*right_ug, unitgroup_loads, excluded_servers))) {
            LOG_WARN("fail to get excluded servers", K(ret));
          ...
          } else if (right_inter_load.inter_ttg_load_value_ + left_ug_load_value - right_ug_load_value >=
                     inter_ttg_upper_lmt) {
            // ignore this
          ...
          } else if (OB_FAIL(coordinate_single_column_unit_group(left_ug->start_column_idx_,
                         left_ug->column_count_,
                         balance_info->unit_migrate_stat_matrix_,
                         balance_info->unitgroup_load_array_))) {
            LOG_WARN("fail to coordinate single column unit", K(ret));
          } else if (OB_FAIL(coordinate_single_column_unit_group(right_ug->start_column_idx_,
                         right_ug->column_count_,
                         balance_info->unit_migrate_stat_matrix_,
                         balance_info->unitgroup_load_array_))) {
            LOG_WARN("fail to coordinate single column unit", K(ret));
          } else {
    ...
    if (OB_SUCC(ret)) {
      ...
      std::sort(left_inter_load.unitgroup_loads_.begin(), left_inter_load.unitgroup_loads_.end(), unitgroup_load_cmp);
    ...
    if (OB_SUCC(ret)) {
      ...
      std::sort(right_inter_load.unitgroup_loads_.begin(), right_inter_load.unitgroup_loads_.end(), unitgroup_load_cmp);
      if (OB_FAIL(unitgroup_load_cmp.get_ret())) {
        LOG_WARN("fail to sort", K(ret));
      }
    }
...
int ObServerBalancer::CountBalanceStrategy::try_move_single_ug_balance_inter_ttg_load(ServerLoad& src_inter_load,
    ServerLoad& dst_inter_load, TenantGroupBalanceInfo* balance_info, const double inter_ttg_upper_lmt)
{
  int ret = OB_SUCCESS;
  if (OB_UNLIKELY(NULL == balance_info)) {
    ...
    if (src_inter_load.unitgroup_loads_.count() <= dst_inter_load.unitgroup_loads_.count()) {
      ...
      for (int64_t i = src_inter_load.unitgroup_loads_.count() - 1; OB_SUCC(ret) && i >= 0; --i) {
        UnitGroupLoad& ug_load = *src_inter_load.unitgroup_loads_.at(i);
        double ug_load_value = 0.0;
        bool can_move = false;
        excluded_dst_servers.reset();
        if (OB_FAIL(get_ug_balance_excluded_dst_servers(ug_load, unitgroup_loads, excluded_dst_servers))) {
          LOG_WARN("fail to get excluded server", K(ret));
          ...
          // ignore, since this amendment will violate the balance in terms of inter ttg
                       ...
                       check_can_move_ug_balance_inter_ttg_load(src_inter_load, ug_load, dst_inter_load, can_move))) {
          LOG_WARN("fail to check can move ug balance inter ttg load", K(ret));
        } else if (!can_move) {
          // ignore
        } else if (OB_FAIL(move_ug_between_server_loads(src_inter_load, dst_inter_load, ug_load, i))) {
          LOG_WARN("fail to move ug between server loads", K(ret));
        } else if (OB_FAIL(coordinate_single_column_unit_group(ug_load.start_column_idx_,
                       ug_load.column_count_,
                       balance_info->unit_migrate_stat_matrix_,
                       balance_info->unitgroup_load_array_))) {
          LOG_WARN("fail to coordinate single column unit", K(ret));
        } else {
      ...
      if (OB_SUCC(ret)) {
        ...
        std::sort(src_inter_load.unitgroup_loads_.begin(), src_inter_load.unitgroup_loads_.end(), unitgroup_load_cmp);
        if (OB_FAIL(unitgroup_load_cmp.get_ret())) {
          LOG_WARN("fail to sort", K(ret));
        }
      }
      if (OB_SUCC(ret)) {
        ...
        std::sort(dst_inter_load.unitgroup_loads_.begin(), dst_inter_load.unitgroup_loads_.end(), unitgroup_load_cmp);
        if (OB_FAIL(unitgroup_load_cmp.get_ret())) {
          LOG_WARN("fail to sort", K(ret));
        }
      }
...
int ObServerBalancer::try_execute_unit_balance_task(const common::ObZone& zone, TenantGroupBalanceInfo* info_need_amend,
    const common::ObIArray<ObUnitManager::ObUnitLoad>& standalone_units,
    const common::ObIArray<ObUnitManager::ObUnitLoad>& not_grant_units,
    const common::ObIArray<TenantGroupBalanceInfo*>& unstable_tenant_group,
    const common::ObIArray<common::ObAddr>& available_servers, bool& do_amend)
{
  ...
  } else if (OB_FAIL(task_array.reserve(info_need_amend->unit_migrate_stat_matrix_.get_element_count()))) {
    LOG_WARN("fail to reserve", K(ret));
  } else if (OB_FAIL(generate_unit_balance_task(*info_need_amend, task_array))) {
    LOG_WARN("fail to generate unit balance task", K(ret));
  } else if (task_array.count() <= 0) {
    if (OB_FAIL(vacate_space_for_ttg_balance(zone,
            *info_need_amend,
            standalone_units,
            not_grant_units,
            unstable_tenant_group,
...
int ObServerBalancer::do_migrate_unit_task(const common::ObIArray<UnitMigrateStat>& task_array)
{
  int ret = OB_SUCCESS;
  if (OB_UNLIKELY(!inited_)) {
    ret = OB_NOT_INIT;
    ...
    for (int64_t i = 0; OB_SUCC(ret) && i < task_array.count(); ++i) {
      ...
      if (!unit_migrate_stat.unit_load_.is_valid()) {
      ...
      } else if (OB_FAIL(unit_mgr_->check_can_migrate_in(unit_migrate_stat.arranged_pos_, can_migrate_in))) {
        LOG_WARN("fail to check can migrate in", K(ret));
      } else if (!can_migrate_in) {
        // bypass
      } else if (OB_FAIL(unit_stat_mgr_->get_unit_stat(unit_migrate_stat.unit_load_.unit_->unit_id_, unit_stat))) {
        LOG_WARN("fail to get unit stat", K(ret));
      } else if (OB_FAIL(get_unit_resource_reservation(unit_migrate_stat.unit_load_.unit_->unit_id_,
        ...
        if (OB_OP_NOT_ALLOW == ret) {
          LOG_INFO("cannot migrate unit, server full",
              "unit",
              *unit_migrate_stat.unit_load_.unit_,
              "dst_server",
              unit_migrate_stat.arranged_pos_);
          ret = OB_SUCCESS;  // ingore this unit
        } else {
          LOG_WARN("fail to migrate unit",
              K(ret),
              "unit",
              *unit_migrate_stat.unit_load_.unit_,
              "dst_server",
...
int ObServerBalancer::generate_unit_balance_task(
    TenantGroupBalanceInfo& balance_info, common::ObIArray<UnitMigrateStat*>& task_array)
{
  int ret = OB_SUCCESS;
  LOG_INFO("tenant group migrate unit stat matrix", "unit migrate stat matrix", balance_info.unit_migrate_stat_matrix_);
  if (OB_UNLIKELY(!inited_)) {
    ret = OB_NOT_INIT;
    LOG_WARN("not init", K(ret));
  } else if (OB_FAIL(try_generate_square_task_from_ttg_matrix(balance_info.unit_migrate_stat_matrix_, task_array))) {
    LOG_WARN("fail to generate balance task from ttg matrix", K(ret));
  } else if (task_array.count() > 0) {
    // good, has task generated
  } else if (OB_FAIL(try_generate_line_task_from_ttg_matrix(balance_info.unit_migrate_stat_matrix_, task_array))) {
    LOG_WARN("fail to generate balance task from ttg matrix", K(ret));
  } else if (task_array.count() > 0) {
    // good, has task generated
  } else if (OB_FAIL(try_generate_dot_task_from_ttg_matrix(balance_info.unit_migrate_stat_matrix_, task_array))) {
    LOG_WARN("fail to generate balance task from ttg matrix", K(ret));
  } else if (task_array.count() > 0) {
    // good, has task generated
  } else {
    ...
    const UnitMigrateStat& unit_migrate_stat, common::ObIArray<ServerLoadSum>& server_load_sums)
{
  ...
  if (OB_UNLIKELY(!inited_)) {
    ...
    } else if (OB_FAIL(server_load_sums.at(idx).load_sum_.append_load(*unit_migrate_stat.unit_load_.unit_config_))) {
      LOG_WARN("fail to append load", K(ret));
    } else if (OB_FAIL(unit_stat_mgr_->get_unit_stat(unit_migrate_stat.unit_load_.unit_->unit_id_, unit_stat))) {
      LOG_WARN("fail to get unit stat", K(ret));
    } else {
...
int ObServerBalancer::check_servers_resource_enough(
    const common::ObIArray<ServerLoadSum>& server_load_sums, bool& enough)
{
  int ret = OB_SUCCESS;
  double hard_limit = 0.0;
  double disk_waterlevel = 0.0;
  if (OB_UNLIKELY(!inited_)) {
    ...
    for (int64_t i = 0; OB_SUCC(ret) && enough && i < server_load_sums.count(); ++i) {
      ...
      if (OB_SUCC(ret)) {
        if (load_sum.load_sum_.max_cpu_ > server_status.resource_info_.cpu_ * hard_limit ||
            load_sum.load_sum_.min_cpu_ > server_status.resource_info_.cpu_ ||
            static_cast<double>(load_sum.load_sum_.max_memory_) >
                static_cast<double>(server_status.resource_info_.mem_total_) * hard_limit ||
            static_cast<double>(load_sum.load_sum_.min_memory_) >
    ...
    Matrix<UnitMigrateStat>& unit_migrate_stat_task, common::ObIArray<UnitMigrateStat*>& task_array)
{
  ...
  if (OB_UNLIKELY(!inited_)) {
    ...
    for (int64_t i = 0; !tenant_unit_collide && OB_SUCC(ret) && i < unit_migrate_stat_task.get_row_count(); ++i) {
      for (int64_t j = 0; !tenant_unit_collide && OB_SUCC(ret) && j < unit_migrate_stat_task.get_column_count(); ++j) {
        ...
        } else if (OB_FAIL(check_unit_migrate_stat_unit_collide(*ptr, dummy_collide_unit, tenant_unit_collide))) {
          LOG_WARN("fail to check unit migrate stat unit collide", K(ret));
        } else if (tenant_unit_collide) {
          LOG_INFO("migrate unit collide", "migrate_unit", *ptr, "collide_unit", dummy_collide_unit);
    ...
    UnitMigrateStat& unit_migrate_stat, common::ObIArray<UnitMigrateStat*>& task_array)
{
  ...
  if (OB_UNLIKELY(!inited_)) {
    ...
    // ignore since this do not need to migrate
                 ...
                 check_unit_migrate_stat_unit_collide(unit_migrate_stat, dummy_collide_unit, tenant_unit_collide))) {
    ...
    Matrix<UnitMigrateStat>& unit_migrate_stat_task, common::ObIArray<UnitMigrateStat*>& task_array)
{
  ...
  if (OB_UNLIKELY(!inited_)) {
    ret = OB_NOT_INIT;
    LOG_WARN("not init", K(ret));
  } else if (unit_migrate_stat_task.get_row_count() <= 0 || unit_migrate_stat_task.get_column_count() <= 0) {
    ret = OB_ERR_UNEXPECTED;
    LOG_WARN("unit migrate stat task array unexpected", K(ret), K(unit_migrate_stat_task));
  } else {
    bool first_row_stable = true;
    for (int64_t column = 0; first_row_stable && OB_SUCC(ret) && column < unit_migrate_stat_task.get_column_count();
         ++column) {
    ...
    if (OB_FAIL(ret)) {
      ...
      bool finish = false;
      for (int64_t i = 1; !finish && OB_SUCC(ret) && i < unit_migrate_stat_task.get_row_count(); ++i) {
        for (int64_t j = 0; !finish && OB_SUCC(ret) && j < unit_migrate_stat_task.get_column_count(); ++j) {
          UnitMigrateStat* ptr = unit_migrate_stat_task.get(i, j);
          if (OB_UNLIKELY(NULL == ptr)) {
            ret = OB_ERR_UNEXPECTED;
            LOG_WARN("unit migrate stat ptr is null", K(ret));
          ...
          }  // not finish, go on next
        }
      }
    } else {
      bool finish = false;
      for (int64_t column = 0; !finish && OB_SUCC(ret) && column < unit_migrate_stat_task.get_column_count();
           ++column) {
        UnitMigrateStat* ptr = unit_migrate_stat_task.get(0, column);
        if (OB_UNLIKELY(NULL == ptr)) {
          ret = OB_ERR_UNEXPECTED;
...
int ObServerBalancer::vacate_space_for_ttg_balance(const common::ObZone& zone, TenantGroupBalanceInfo& balance_info,
    const common::ObIArray<ObUnitManager::ObUnitLoad>& standalone_units,
    const common::ObIArray<ObUnitManager::ObUnitLoad>& not_grant_units,
    const common::ObIArray<TenantGroupBalanceInfo*>& unstable_tenant_group,
    const common::ObIArray<common::ObAddr>& available_servers, bool& do_amend)
{
  ...
  if (OB_UNLIKELY(!inited_)) {
    ...
    for (int64_t i = 0; !find && OB_SUCC(ret) && i < migrate_matrix.get_row_count(); ++i) {
      for (int64_t j = 0; !find && OB_SUCC(ret) && j < migrate_matrix.get_column_count(); ++j) {
        ...
        if (OB_UNLIKELY(NULL == unit_migrate_stat)) {
          ...
          // ignore this, since this do not need to amend
                       ...
                       check_unit_migrate_stat_unit_collide(*unit_migrate_stat, collide_unit, tenant_unit_collide))) {
          LOG_WARN("fail to check unit migrate stat unit collide", K(ret));
        } else if (tenant_unit_collide) {
          if (OB_FAIL(do_migrate_out_collide_unit(*unit_migrate_stat, collide_unit, server_loads, do_amend))) {
            LOG_WARN("fail to do migrate out collide unit", K(ret));
          } else {
            find = true;
          }
        } else {
          if (OB_FAIL(do_vacate_space_for_ttg_balance(*unit_migrate_stat,
                  standalone_units,
                  not_grant_units,
                  unstable_tenant_group,
                  server_loads,
...
int ObServerBalancer::do_migrate_out_collide_unit(UnitMigrateStat& unit_migrate_stat,
    ObUnitManager::ObUnitLoad& collide_unit_load, common::ObArray<ServerTotalLoad>& server_loads, bool& do_amend)
{
  int ret = OB_SUCCESS;
  common::ObArray<ObUnitManager::ObUnitLoad> zone_units;
  ...
  if (OB_UNLIKELY(!inited_)) {
    ret = OB_NOT_INIT;
    LOG_WARN("not init", K(ret));
  } else if (OB_UNLIKELY(common::OB_INVALID_ID == unit_migrate_stat.tenant_id_ || !collide_unit_load.is_valid())) {
    ret = OB_INVALID_ARGUMENT;
  ...
  } else if (OB_FAIL(unit_stat_mgr_->get_unit_stat(collide_unit_load.unit_->unit_id_, unit_stat))) {
    ...
    ret = OB_ERR_UNEXPECTED;
    LOG_WARN("server not match",
        K(ret),
        "left_server",
        server_load_to_vacate->server_,
        "right_server",
        collide_unit_load.unit_->server_);
  } else if (OB_FAIL(get_pool_occupation_excluded_dst_servers(collide_unit_load, pool_occupation, excluded_servers))) {
    LOG_WARN("fail to get excluded servers", K(ret));
  } else {
    do_amend = false;
    ...
    for (int64_t j = server_load_ptrs_sorted.count() - 1; OB_SUCC(ret) && j >= 0; --j) {
      ...
      if (OB_UNLIKELY(NULL == server_load)) {
        ...
        if (OB_FAIL(do_update_src_server_load(collide_unit_load, *server_load_to_vacate, unit_stat))) {
    ...
    if (OB_FAIL(ret)) {
      ...
      LOG_WARN("fail to do migrate task", K(ret));
    } else {
      do_amend = true;
    }
  }
...
int ObServerBalancer::pick_vacate_server_load(common::ObIArray<ServerTotalLoad>& server_loads,
    ...
    ServerTotalLoad*& vacate_server_load)
{
  ...
  if (OB_UNLIKELY(!inited_)) {
    ...
    if (OB_SUCC(ret)) {
      ServerTotalLoadCmp cmp;
      if (FALSE_IT(std::sort(server_load_ptrs_sorted.begin(), server_load_ptrs_sorted.end(), cmp))) {
...
int ObServerBalancer::do_vacate_space_for_ttg_balance(UnitMigrateStat& unit_migrate_stat,
    const common::ObIArray<ObUnitManager::ObUnitLoad>& standalone_units,
    const common::ObIArray<ObUnitManager::ObUnitLoad>& not_grant_units,
    const common::ObIArray<TenantGroupBalanceInfo*>& unstable_tenant_group,
    common::ObArray<ServerTotalLoad>& server_loads, bool& do_amend)
{
  ...
  if (OB_UNLIKELY(!inited_)) {
    ...
    LOG_WARN("fail to get hard limit", K(ret));
  } else if (OB_FAIL(unit_stat_mgr_->get_unit_stat(unit_load_to_vacate.unit_->unit_id_, unit_stat))) {
    ...
    if (OB_FAIL(load_to_vacate.append_load(unit_migrate_stat.unit_load_))) {
      LOG_WARN("fail to append load", K(ret));
    } else if (OB_FAIL(vacate_space_by_unit_array(load_to_vacate,
                   unit_load_to_vacate,
                   *server_load_to_vacate,
                   task_array,
                   not_grant_units,
                   server_load_ptrs_sorted))) {
      ...
      // good, enough
    } else if (OB_FAIL(vacate_space_by_tenantgroup(load_to_vacate,
                   unit_load_to_vacate,
                   *server_load_to_vacate,
                   task_array,
                   unstable_tenant_group,
...
int ObServerBalancer::generate_pool_occupation_array(
    const common::ObIArray<ObUnitManager::ObUnitLoad>& unit_loads, common::ObIArray<PoolOccupation>& pool_occupation)
{
  int ret = OB_SUCCESS;
  ...
  for (int64_t i = 0; OB_SUCC(ret) && i < unit_loads.count(); ++i) {
    ...
    if (OB_UNLIKELY(!unit_load.is_valid())) {
      ...
      LOG_WARN("unit load unexpected", K(ret), K(unit_load));
                   ...
                   unit_load.pool_->tenant_id_, unit_load.pool_->resource_pool_id_, unit_load.unit_->server_)))) {
      LOG_WARN("fail to push back", K(ret));
    } else {
    }
  }
...
int ObServerBalancer::vacate_space_by_tenantgroup(LoadSum& load_to_vacate,
    const ObUnitManager::ObUnitLoad& unit_load_to_vacate, ServerTotalLoad& server_load_to_vacate,
    common::ObIArray<UnitMigrateStat>& task_array,
    const common::ObIArray<TenantGroupBalanceInfo*>& unstable_tenant_group,
    common::ObArray<ServerTotalLoad*>& server_load_ptrs_sorted)
{
...
int ObServerBalancer::vacate_space_by_unit_array(LoadSum& load_to_vacate,
    const ObUnitManager::ObUnitLoad& unit_load_to_vacate, ServerTotalLoad& server_load_to_vacate,
    ...
    for (int64_t i = 0; OB_SUCC(ret) && i < units.count(); ++i) {
      ...
      if (OB_UNLIKELY(!unit_load.is_valid())) {
        ...
        // unit is not belong to vacate, skip
                     ...
                     server_load_to_vacate, unit_load, task_array, pool_occupation, server_load_ptrs_sorted))) {
...
int ObServerBalancer::check_single_server_resource_enough(const LoadSum& this_load, const ObUnitStat& unit_stat,
    const ServerTotalLoad& server_load, bool& enough, const bool mind_disk_waterlevel)
{
  int ret = OB_SUCCESS;
  ServerDiskStatistic disk_statistic;
  double hard_limit = 0.0;
  ...
  if (OB_UNLIKELY(!inited_)) {
    ...
    LOG_WARN("fail to get hard limit", K(ret));
  } else if (OB_FAIL(zone_disk_statistic_.get_server_disk_statistic(server_load.server_, disk_statistic))) {
...
int ObServerBalancer::vacate_space_by_single_unit(ServerTotalLoad& server_load_to_vacate,
    const ObUnitManager::ObUnitLoad& unit_load, common::ObIArray<UnitMigrateStat>& task_array,
    common::ObIArray<PoolOccupation>& pool_occupation, common::ObArray<ServerTotalLoad*>& server_load_ptrs_sorted)
{
  int ret = OB_SUCCESS;
  ObUnitStat unit_stat;
  ObArray<common::ObAddr> excluded_servers;
  if (OB_UNLIKELY(!inited_)) {
    ret = OB_NOT_INIT;
    LOG_WARN("not init", K(ret));
  } else if (OB_UNLIKELY(!unit_load.is_valid() || server_load_to_vacate.server_ != unit_load.unit_->server_)) {
    ret = OB_INVALID_ARGUMENT;
    LOG_WARN(
        "invalid argument", K(ret), "unit_server", unit_load.unit_->server_, "server", server_load_to_vacate.server_);
  } else if (OB_UNLIKELY(NULL == unit_stat_mgr_)) {
    ret = OB_ERR_UNEXPECTED;
    LOG_WARN("unit_stat_mgr_ ptr is null", K(ret), KP(unit_stat_mgr_));
  } else if (OB_FAIL(unit_stat_mgr_->get_unit_stat(unit_load.unit_->unit_id_, unit_stat))) {
...
int ObServerBalancer::get_pool_occupation_excluded_dst_servers(const ObUnitManager::ObUnitLoad& this_load,
    const common::ObIArray<PoolOccupation>& pool_occupation, common::ObIArray<common::ObAddr>& excluded_servers)
{
  ...
  if (OB_UNLIKELY(!inited_)) {
    ...
    for (int64_t i = 0; OB_SUCC(ret) && i < pool_occupation.count(); ++i) {
      ...
      } else if (common::OB_INVALID_ID != this_load.pool_->tenant_id_ && this_load.pool_->tenant_id_ != po.tenant_id_) {
...
int ObServerBalancer::generate_complete_server_loads(const common::ObZone& zone,
    const common::ObIArray<common::ObAddr>& available_servers, double* const resource_weights,
    const int64_t weights_count, common::ObArray<ServerTotalLoad>& server_loads)
{
  ...
  if (OB_UNLIKELY(!inited_)) {
    ret = OB_NOT_INIT;
    LOG_WARN("not init", K(ret));
  } else if (OB_UNLIKELY(NULL == resource_weights || RES_MAX != weights_count || zone.is_empty())) {
    ret = OB_INVALID_ARGUMENT;
    ...
    if (OB_SUCC(ret)) {  // Defensive inspection
      for (int64_t i = 0; OB_SUCC(ret) && i < available_servers.count(); ++i) {
        if (!has_exist_in_array(zone_servers, available_servers.at(i))) {
          ret = OB_ERR_UNEXPECTED;
          LOG_WARN("available server is not a member of zone servers",
              K(zone_servers),
              "available server",
              available_servers.at(i));
        } else {
        }  // good, in zone servers array
      }
    }
  }
  return ret;
...
int ObServerBalancer::divide_complete_server_loads_for_balance(
    const common::ObIArray<common::ObAddr>& available_servers, common::ObIArray<ServerTotalLoad>& server_loads,
    common::ObIArray<ServerTotalLoad*>& wild_server_loads, common::ObIArray<ServerTotalLoad*>& over_server_loads,
    common::ObIArray<ServerTotalLoad*>& under_server_loads, double& upper_lmt)
{
  int ret = OB_SUCCESS;
  if (OB_UNLIKELY(!inited_)) {
    ret = OB_NOT_INIT;
    LOG_WARN("not init", K(ret));
  } else if (OB_UNLIKELY(server_loads.count() <= 0 || available_servers.count() <= 0)) {
    ret = OB_INVALID_ARGUMENT;
    LOG_WARN("invalid argument",
        K(ret),
        "server_load_count",
        server_loads.count(),
        "available_servers count",
        available_servers.count());
  } else if (OB_UNLIKELY(NULL == unit_mgr_)) {
    ret = OB_ERR_UNEXPECTED;
    LOG_WARN("unit_mgr_ ptr is null", K(ret));
...
int ObServerBalancer::generate_available_server_loads(ObIArray<ServerTotalLoad*>& over_server_loads,
...
int ObServerBalancer::make_single_wild_server_empty_by_units(common::ObIArray<PoolOccupation>& pool_occupation,
    ...
    common::ObArray<ServerTotalLoad*>& available_server_loads, common::ObIArray<UnitMigrateStat>& task_array)
{
  ...
  for (int64_t i = 0; OB_SUCC(ret) && i < units.count(); ++i) {
    ...
    if (OB_UNLIKELY(!unit_load.is_valid())) {
      ...
      LOG_WARN("fail to get excluded servers", K(ret));
    } else if (FALSE_IT(std::sort(available_server_loads.begin(), available_server_loads.end(), cmp))) {
      // sort cannot fail
    } else if (OB_FAIL(cmp.get_ret())) {
      LOG_WARN("fail to sort", K(ret));
    } else if (try_balance_single_unit_by_cm(
                   unit_load, available_server_loads, task_array, excluded_servers, pool_occupation, do_balance)) {
...
int ObServerBalancer::make_wild_servers_empty(const common::ObIArray<ObUnitManager::ObUnitLoad>& standalone_units,
    const common::ObIArray<ObUnitManager::ObUnitLoad>& not_grant_units,
    common::ObIArray<ServerTotalLoad*>& wild_server_loads, common::ObIArray<ServerTotalLoad*>& over_server_loads,
    common::ObIArray<ServerTotalLoad*>& under_server_loads)
{
  ...
  if (OB_UNLIKELY(!inited_)) {
    ret = OB_NOT_INIT;
    LOG_WARN("not init", K(ret));
  } else if (OB_FAIL(generate_available_server_loads(over_server_loads, under_server_loads, available_server_loads))) {
    ...
    LOG_WARN("fail to do migrate unit task", K(ret));
  } else {
  }  // no more to do
  return ret;
}

int ObServerBalancer::make_wild_servers_empty_by_units(common::ObIArray<ServerTotalLoad*>& wild_server_loads,
    const common::ObIArray<ObUnitManager::ObUnitLoad>& units, common::ObArray<ServerTotalLoad*>& available_server_loads,
    common::ObIArray<UnitMigrateStat>& task_array)
{
  int ret = OB_SUCCESS;
...
int ObServerBalancer::get_disk_over_available_servers(common::ObIArray<common::ObAddr>& disk_over_servers)
{
  int ret = OB_SUCCESS;
  double disk_waterlevel = 0.0;
  if (OB_UNLIKELY(!inited_)) {
    ...
    for (int64_t i = 0; OB_SUCC(ret) && i < zone_disk_statistic_.server_disk_statistic_array_.count(); ++i) {
      ServerDiskStatistic& disk_statistic = zone_disk_statistic_.server_disk_statistic_array_.at(i);
...
int ObServerBalancer::make_non_ttg_balance_under_load_by_disk(common::ObIArray<PoolOccupation>& pool_occupation,
  ...
  if (OB_UNLIKELY(!inited_)) {
    ...
    for (int64_t i = 0; OB_SUCC(ret) && i < zone_disk_statistic_.server_disk_statistic_array_.count(); ++i) {
      ...
      if (OB_FAIL(zone_disk_statistic_.check_server_over_disk_waterlevel(
        ...
        LOG_WARN("fail to get unit stat", K(ret), "unit", *unit_load.unit_);
      } else if (OB_FAIL(pick_server_load(disk_statistic.server_, available_server_loads, dst_server_load))) {
        LOG_WARN("fail to pick server load", K(ret));
        ...
        LOG_WARN("fail to append load", K(ret));
      } else if (OB_FAIL(check_single_server_resource_enough(this_load, unit_stat, *dst_server_load, enough))) {
  ...
  if (OB_UNLIKELY(!inited_)) {
    ...
    for (int64_t i = 0; OB_SUCC(ret) && i < disk_over_servers.count(); ++i) {
      ...
      for (int64_t j = 0; OB_SUCC(ret) && j < standalone_unit_ptrs.count(); ++j) {
        ...
        } else if (OB_FAIL(pick_server_load(src_server, available_server_loads, src_server_load))) {
          LOG_WARN("fail to pick server load", K(ret));
        } else if (NULL == src_server_load) {
          ret = OB_ERR_UNEXPECTED;
          LOG_WARN("dst server load ptr is null", K(ret));
        } else if (OB_FAIL(make_non_ttg_balance_under_load_by_disk(pool_occupation,
                       task_array,
                       *unit_load,
                       src_server_load,
                       disk_over_servers,
                       available_server_loads))) {
    ...
    if (OB_SUCC(ret) && task_array.count() > 0) {
      if (OB_FAIL(do_migrate_unit_task(task_array))) {
        LOG_WARN("fail to migrate task", K(ret));
      }
    }
  }
  return ret;
    ...
    const double upper_lmt, double* const g_res_weights, const int64_t weights_count)
{
  ...
  if (OB_UNLIKELY(!inited_)) {
    ...
    common::ObArray<UnitMigrateStat> task_array;
    if (OB_FAIL(do_non_ttg_unit_balance_by_cm(task_array,
            not_grant_units,
            over_server_loads,
            under_server_loads,
            upper_lmt,
    ...
    } else if (OB_FAIL(do_non_ttg_unit_balance_by_cm(task_array,
                   standalone_units,
                   over_server_loads,
                   under_server_loads,
                   upper_lmt,
    ...
    common::ObArray<ServerDiskStatistic*>& available_servers_disk_statistic)
{
  ...
  if (OB_UNLIKELY(!inited_)) {
    ...
    if (OB_SUCC(ret)) {
      ...
      std::sort(available_servers_disk_statistic.begin(), available_servers_disk_statistic.end(), cmp);
      if (OB_FAIL(cmp.get_ret())) {
        LOG_WARN("fail to sort", K(ret));
      }
    }
...
int ObServerBalancer::divide_available_servers_disk_statistic(
    ...
    common::ObIArray<ServerDiskStatistic*>& under_disk_statistic, double& upper_lmt)
{
  int ret = OB_SUCCESS;
  if (OB_UNLIKELY(!inited_)) {
    ret = OB_NOT_INIT;
    ...
    if (OB_SUCC(ret)) {
      ...
      const double average_value = total_value / static_cast<double>(available_servers_disk_statistic.count());
...
int ObServerBalancer::make_server_disk_underload_by_unit(ServerTotalLoad* src_server_load,
    const ObUnitManager::ObUnitLoad& unit_load, const double upper_lmt,
    ...
    common::ObArray<ServerDiskStatistic*>& under_disk_statistic, common::ObIArray<UnitMigrateStat>& task_array)
{
  ...
  if (OB_UNLIKELY(!inited_)) {
    ...
    ret = OB_ERR_UNEXPECTED;
    LOG_WARN(
        "server not match", K(ret), "left_server", src_server_load->server_, "right_server", unit_load.unit_->server_);
  } else if (OB_UNLIKELY(NULL == unit_stat_mgr_)) {
    ret = OB_ERR_UNEXPECTED;
    LOG_WARN("unit stat mgr ptr is null", K(ret));
  } else {
    ...
    ObUnitStat unit_stat;
    LoadSum this_load;
    if (OB_FAIL(get_pool_occupation_excluded_dst_servers(unit_load, pool_occupation, excluded_servers))) {
      ...
      for (int64_t j = under_disk_statistic.count() - 1; OB_SUCC(ret) && j >= 0; --j) {
        ...
        if (OB_UNLIKELY(NULL == under_server)) {
          ...
          // in excluded server
        } else if (OB_FAIL(pick_server_load(under_server->server_, available_server_loads, dst_server_load))) {
          LOG_WARN("fail to pick server load", K(ret));
          ...
          // resource not enough
        } else if (under_server->get_disk_used_percent_if_add(unit_stat.required_size_) > upper_lmt) {
          ...
          if (OB_FAIL(do_update_src_server_load(unit_load, *src_server_load, unit_stat))) {
            ...
            LOG_WARN("fail to push back", K(ret));
          } else if (FALSE_IT(std::sort(under_disk_statistic.begin(), under_disk_statistic.end(), cmp))) {
          } else if (OB_FAIL(cmp.get_ret())) {
            LOG_WARN("fail to sort", K(ret));
          } else {
            break;
...
int ObServerBalancer::make_server_disk_underload(ServerDiskStatistic& src_disk_statistic,
  ...
  if (OB_UNLIKELY(!inited_)) {
    ...
    for (int64_t i = 0; OB_SUCC(ret) && i < standalone_unit_ptrs.count(); ++i) {
      ...
      if (src_disk_statistic.get_disk_used_percent() < upper_lmt) {
        ...
        ret = OB_ERR_UNEXPECTED;
        LOG_WARN("unit load ptr is null", K(ret));
      } else if (OB_FAIL(pick_server_load(src_disk_statistic.server_, available_server_loads, src_server_load))) {
        LOG_WARN("fail to pick server load", K(ret));
        ...
        // bypass
      } else if (OB_FAIL(make_server_disk_underload_by_unit(src_server_load,
                     *unit_load,
                     upper_lmt,
                     pool_occupation,
                     available_server_loads,
                     under_disk_statistic,
                     task_array))) {
    ...
    common::ObIArray<ServerTotalLoad*>& available_server_loads)
{
  ...
  if (OB_UNLIKELY(!inited_)) {
    ...
    LOG_WARN("fail to generate sort available server disk statistic", K(ret));
                 ...
                 available_servers_disk_statistic, over_disk_statistic, under_disk_statistic, upper_limit))) {
    ...
    for (int64_t i = 0; OB_SUCC(ret) && i < over_disk_statistic.count(); ++i) {
      ...
      if (OB_UNLIKELY(NULL == src_disk_statistic)) {
        ret = OB_ERR_UNEXPECTED;
        LOG_WARN("disk statistic ptr is null", K(ret));
      } else if (OB_FAIL(make_server_disk_underload(*src_disk_statistic,
                     standalone_units,
                     upper_limit,
                     pool_occupation,
                     available_server_loads,
    ...
    const double upper_lmt, double* const g_res_weights, const int64_t weights_count)
{
  ...
  if (OB_UNLIKELY(!inited_)) {
    ...
    ObArray<ServerTotalLoad*> available_server_loads;
    if (OB_FAIL(generate_available_server_loads(over_server_loads, under_server_loads, available_server_loads))) {
      LOG_WARN("fail to generate available server loads", K(ret));
    } else if (OB_FAIL(make_available_servers_disk_balance(standalone_units, available_server_loads))) {
    ...
    if (OB_FAIL(generate_available_server_loads(over_server_loads, under_server_loads, available_server_loads))) {
      LOG_WARN("fail to generate available server loads", K(ret));
    } else if (OB_FAIL(make_available_servers_balance_by_disk(standalone_units, available_server_loads))) {
      LOG_WARN("fail to make available servers balance by disk", K(ret));
    }
  } else {
    if (OB_FAIL(make_available_servers_balance_by_cm(standalone_units,
            not_grant_units,
            over_server_loads,
            under_server_loads,
            upper_lmt,
...
int ObServerBalancer::do_non_tenantgroup_unit_balance_task(const common::ObZone& zone,
    const common::ObIArray<ObUnitManager::ObUnitLoad>& standalone_units,
    const common::ObIArray<ObUnitManager::ObUnitLoad>& not_grant_units,
    const common::ObIArray<common::ObAddr>& available_servers)
{
  ...
  if (OB_UNLIKELY(!inited_)) {
    ...
    LOG_WARN("fail to generate complete server loads", K(ret));
  } else if (OB_FAIL(divide_complete_server_loads_for_balance(available_servers,
                 server_loads,
                 wild_server_loads,
                 over_server_loads,
                 under_server_loads,
    ...
    if (OB_FAIL(make_wild_servers_empty(
            standalone_units, not_grant_units, wild_server_loads, over_server_loads, under_server_loads))) {
      LOG_WARN("fail to make wild servers empty", K(ret));
    }
  } else {
    if (OB_FAIL(make_available_servers_balance(standalone_units,
            not_grant_units,
            over_server_loads,
            under_server_loads,
            upper_lmt,
...
int ObServerBalancer::do_non_ttg_unit_balance_by_cm(common::ObIArray<UnitMigrateStat>& task_array,
    ...
    for (int64_t i = 0; OB_SUCC(ret) && i < over_server_loads.count(); ++i) {
      ...
      for (int64_t j = under_server_loads.count() - 1; OB_SUCC(ret) && j >= 0; --j) {
        ...
        if (OB_FAIL(make_non_ttg_balance_under_load_by_cm(
                pool_occupation, task_array, units, src, dst, upper_lmt, g_res_weights, weights_count))) {
...
int ObServerBalancer::make_non_ttg_balance_under_load_by_cm(common::ObIArray<PoolOccupation>& pool_occupation,
    common::ObIArray<UnitMigrateStat>& task_array, const common::ObIArray<ObUnitManager::ObUnitLoad>& units,
    ServerTotalLoad* src_server_load, ServerTotalLoad* dst_server_load, const double upper_lmt,
    double* const g_res_weights, const int64_t weights_count)
{
  ...
  if (OB_UNLIKELY(!inited_)) {
    ret = OB_NOT_INIT;
    LOG_WARN("not init", K(ret));
  } else if (NULL == src_server_load || NULL == dst_server_load || NULL == g_res_weights || RES_MAX != weights_count) {
    ret = OB_INVALID_ARGUMENT;
    ...
    for (int64_t i = 0; OB_SUCC(ret) && i < units.count(); ++i) {
      ...
      } else if (OB_FAIL(this_load.calc_load_value(g_res_weights, weights_count, *dst_server_load, dst_load_value))) {
...
int ObServerBalancer::choose_balance_info_amend(const common::ObIArray<TenantGroupBalanceInfo*>& tenant_group,
    const bool is_stable_tenantgroup, TenantGroupBalanceInfo*& info_need_amend,
...
int ObServerBalancer::divide_tenantgroup_balance_info(common::ObIArray<TenantGroupBalanceInfo>& balance_info_array,
...
int ObServerBalancer::calc_global_balance_resource_weights(const common::ObZone& zone,
    const common::ObIArray<common::ObAddr>& available_servers, double* const resource_weights,
    const int64_t weights_count)
{
  int ret = OB_SUCCESS;
  ...
  if (OB_UNLIKELY(!inited_)) {
    ret = OB_NOT_INIT;
    LOG_WARN("not init", K(ret));
  } else if (OB_UNLIKELY(zone.is_empty() || NULL == resource_weights || RES_MAX != weights_count)) {
    ret = OB_INVALID_ARGUMENT;
    LOG_WARN("invalid argument", K(ret), K(zone));
  } else if (OB_UNLIKELY(NULL == unit_mgr_ || NULL == server_mgr_)) {
    ret = OB_ERR_UNEXPECTED;
...
int ObServerBalancer::check_and_get_tenant_matrix_unit_num(const Matrix<uint64_t>& tenant_id_matrix,
    const common::ObZone& zone, bool& unit_num_match, ObIArray<int64_t>& column_unit_num_array)
{
  int ret = OB_SUCCESS;
  if (OB_UNLIKELY(!inited_)) {
    ret = OB_NOT_INIT;
    ...
    for (int64_t i = 0; OB_SUCC(ret) && unit_num_match && i < column_count; ++i) {
      ...
      if (OB_SUCC(ret)) {
        int64_t unit_num = 0;
        if (OB_FAIL(check_and_get_tenant_column_unit_num(tenant_column, zone, unit_num_match, unit_num))) {
    ...
    const ObIArray<uint64_t>& tenant_id_column, const common::ObZone& zone, bool& unit_num_match, int64_t& unit_num)
{
  ...
  if (OB_UNLIKELY(!inited_)) {
    ...
    for (int64_t i = 0; unit_num_match && OB_SUCC(ret) && i < tenant_id_column.count(); ++i) {
      ...
      } else if (OB_FAIL(unit_mgr_->inner_get_tenant_zone_full_unit_num(tenant_id, zone, this_unit_num))) {
...
int ObServerBalancer::generate_original_unit_matrix(const Matrix<uint64_t>& tenant_id_matrix,
    const common::ObZone& zone, const ObIArray<int64_t>& column_unit_num_array,
    Matrix<UnitMigrateStat>& unit_migrate_stat_matrix)
{
  ...
  if (OB_UNLIKELY(!inited_)) {
    ...
    if (tenant_matrix_row_count <= 0 || tenant_matrix_row_count >= INT32_MAX || tenant_matrix_column_count <= 0 ||
        tenant_matrix_column_count >= INT32_MAX) {
      ret = OB_ERR_UNEXPECTED;
      LOG_WARN("tenant matrix row or column count unexpected",
          K(ret),
          K(tenant_matrix_row_count),
          K(tenant_matrix_column_count));
    ...
    } else if (OB_FAIL(unit_migrate_stat_matrix.init(unit_matrix_row_count, unit_matrix_column_count))) {
      ...
      for (int64_t row = 0; OB_SUCC(ret) && row < tenant_matrix_row_count; ++row) {
        ...
        for (int64_t column = 0; OB_SUCC(ret) && column < tenant_matrix_column_count; ++column) {
          ...
          if (OB_FAIL(tenant_id_matrix.get(row, column, tenant_id))) {
            ...
            ret = OB_ERR_UNEXPECTED;
            LOG_WARN("unit_mgr_ is null", K(ret), KP(unit_mgr_));
          } else if (OB_FAIL(unit_mgr_->get_tenant_zone_unit_loads(tenant_id, zone, replica_type, unit_loads))) {
            if (OB_ENTRY_NOT_EXIST != ret) {
              LOG_WARN("fail to get tenant zone unit infos", K(ret));
            } else {
              ret = OB_SUCCESS;
...
int ObServerBalancer::fill_unit_migrate_stat_matrix(Matrix<UnitMigrateStat>& unit_migrate_stat_matrix,
    const uint64_t tenant_id, const int64_t row, const int64_t start_column_id,
    const common::ObIArray<ObUnitManager::ObUnitLoad>& unit_loads)
{
  ...
  if (OB_UNLIKELY(!inited_)) {
    ...
    if (row >= unit_matrix_row_count || start_column_id >= unit_matrix_column_count ||
      ...
      LOG_WARN("invalid argument",
          ...
          K(start_column_id),
          K(unit_loads),
          K(unit_matrix_row_count),
          K(unit_matrix_column_count));
    } else {
      for (int64_t i = 0; OB_SUCC(ret) && i < unit_loads.count(); ++i) {
        const ObUnitManager::ObUnitLoad& unit_load = unit_loads.at(i);
        if (!unit_load.is_valid()) {
    ...
    TenantGroupBalanceInfo& balance_info, const common::ObIArray<common::ObAddr>& available_servers)
{
  ...
  if (OB_UNLIKELY(!inited_)) {
    ...
    ret = OB_INVALID_ARGUMENT;
    LOG_WARN("invalid argument", K(ret));
  } else if (OB_FAIL(balance_row_units(balance_info,
                 column_unit_num_array,
                 unit_migrate_stat_matrix,
                 unitgroup_loads,
                 server_loads,
                 available_servers))) {
...
int ObServerBalancer::InnerTenantGroupBalanceStrategy::coordinate_all_column_unit_group(
    const common::ObIArray<int64_t>& column_unit_num_array, Matrix<UnitMigrateStat>& unit_migrate_stat_matrix,
    const common::ObIArray<UnitGroupLoad>& unitgroup_loads)
{
  int ret = OB_SUCCESS;
  if (column_unit_num_array.count() <= 0 || unit_migrate_stat_matrix.get_row_count() <= 0 ||
      unitgroup_loads.count() <= 0) {
    ret = OB_INVALID_ARGUMENT;
    LOG_WARN("invalid argument", K(ret), K(column_unit_num_array), K(unit_migrate_stat_matrix), K(unitgroup_loads));
  } else {
    ...
    for (int64_t i = 0; OB_SUCC(ret) && i < column_unit_num_array.count(); ++i) {
      if (OB_FAIL(coordinate_single_column_unit_group(
              accumulate_column_idx, column_unit_num_array.at(i), unit_migrate_stat_matrix, unitgroup_loads))) {
        LOG_WARN("fail to coordinate single column unit", K(ret));
      } else {
...
int ObServerBalancer::InnerTenantGroupBalanceStrategy::coordinate_single_column_unit_group(
    const int64_t start_column_idx, const int64_t this_column_count, Matrix<UnitMigrateStat>& unit_migrate_stat_matrix,
    const common::ObIArray<UnitGroupLoad>& unitgroup_loads)
{
  int ret = OB_SUCCESS;
  if (start_column_idx < 0 || this_column_count <= 0 ||
      start_column_idx + this_column_count > unit_migrate_stat_matrix.get_column_count() ||
      unit_migrate_stat_matrix.get_row_count() <= 0 || unit_migrate_stat_matrix.get_column_count() <= 0 ||
      unitgroup_loads.count() <= 0 || unitgroup_loads.count() != unit_migrate_stat_matrix.get_column_count()) {
    ret = OB_INVALID_ARGUMENT;
    LOG_WARN("invalid argument",
        K(ret),
        K(start_column_idx),
        K(this_column_count),
        K(unit_migrate_stat_matrix),
        K(unitgroup_loads));
  } else {
    ...
    if (OB_SUCC(ret)) {
      if (OB_FAIL(do_coordinate_single_column_unit_group(
              start_column_idx, this_column_count, unit_migrate_stat_matrix, dest_server_array))) {
...
int ObServerBalancer::InnerTenantGroupBalanceStrategy::do_coordinate_single_column_unit_group(
    const int64_t start_column_idx, const int64_t column_count, Matrix<UnitMigrateStat>& unit_migrate_stat_matrix,
    const common::ObIArray<common::ObAddr>& dest_servers)
{
  int ret = OB_SUCCESS;
  if (start_column_idx < 0 || column_count <= 0 || dest_servers.count() <= 0 ||
      start_column_idx + column_count > unit_migrate_stat_matrix.get_column_count() ||
      unit_migrate_stat_matrix.get_row_count() <= 0 || unit_migrate_stat_matrix.get_column_count() <= 0 ||
      dest_servers.count() != column_count) {
    ret = OB_INVALID_ARGUMENT;
    LOG_WARN(
        "invalid argument", K(ret), K(start_column_idx), K(column_count), K(unit_migrate_stat_matrix), K(dest_servers));
  } else {
    for (int64_t row = 0; OB_SUCC(ret) && row < unit_migrate_stat_matrix.get_row_count(); ++row) {
      if (OB_FAIL(do_coordinate_single_unit_row(
              row, start_column_idx, column_count, dest_servers, unit_migrate_stat_matrix))) {
  ...
  // the matrix is sorted vertically according to the server to facilitate the observation of information during
  // debugging.
  UnitMigrateStatCmp cmp_operator;
  if (OB_FAIL(ret)) {
    // failed in the previous procedure
  } else if (OB_FAIL(unit_migrate_stat_matrix.sort_column_group(start_column_idx, column_count, cmp_operator))) {
...
int ObServerBalancer::InnerTenantGroupBalanceStrategy::do_coordinate_single_unit_row(const int64_t row,
    const int64_t start_column_idx, const int64_t column_count,
    const common::ObIArray<common::ObAddr>& candidate_servers, Matrix<UnitMigrateStat>& unit_migrate_stat_matrix)
{
  ...
  if (row < 0 || start_column_idx < 0 || column_count <= 0 ||
      start_column_idx + column_count > unit_migrate_stat_matrix.get_column_count() ||
      unit_migrate_stat_matrix.get_row_count() <= 0 || unit_migrate_stat_matrix.get_column_count() <= 0 ||
      row >= unit_migrate_stat_matrix.get_row_count() || candidate_servers.count() <= 0) {
    ret = OB_INVALID_ARGUMENT;
    LOG_WARN("invalid argument",
        K(ret),
        K(row),
    ...
    for (int64_t column = start_column_idx; OB_SUCC(ret) && column < end_column_idx; ++column) {
      ...
      if (OB_FAIL(unit_migrate_stat_matrix.get(row, column, unit_migrate_stat))) {
        ...
        if (OB_FAIL(arrange_unit_migrate_stat_pos(candidate_servers, unit_migrate_stat, excluded_array))) {
...
int ObServerBalancer::InnerTenantGroupBalanceStrategy::arrange_unit_migrate_stat_pos(
    const common::ObIArray<common::ObAddr>& candidate_servers, UnitMigrateStat& unit_migrate_stat,
    const common::ObIArray<common::ObAddr>& excluded_array)
{
  ...
  if (OB_UNLIKELY(candidate_servers.count() <= 0)) {
    ...
    ret = OB_INVALID_ARGUMENT;
    LOG_WARN("unit migrate stat already in candidate", K(ret), K(unit_migrate_stat), K(candidate_servers));
  } else {
    bool find = false;
    ...
    if (OB_FAIL(ret)) {
      ...
      ret = OB_ERR_UNEXPECTED;
      LOG_WARN("cannot find a new pos for unit migrate stat", K(ret), K(candidate_servers), K(excluded_array));
    } else { /* good */
    }
  }
  return ret;
...
int ObServerBalancer::balance_row_units(TenantGroupBalanceInfo& balance_info,
    const common::ObIArray<int64_t>& column_unit_num_array, Matrix<UnitMigrateStat>& unit_migrate_stat_matrix,
    ...
    const common::ObIArray<common::ObAddr>& available_servers)
{
  ...
  if (OB_UNLIKELY(!inited_)) {
    ret = OB_NOT_INIT;
    LOG_WARN("not init", K(ret));
  } else if (column_unit_num_array.count() <= 0 || unit_migrate_stat_matrix.get_row_count() <= 0 ||
             unit_migrate_stat_matrix.get_column_count() <= 0 || available_servers.count() <= 0) {
    ret = OB_INVALID_ARGUMENT;
    LOG_WARN("invalid argument", K(ret), K(column_unit_num_array), K(available_servers), K(unit_migrate_stat_matrix));
  } else if (OB_FAIL(generate_unitgroup_and_server_load(
                 unit_migrate_stat_matrix, column_unit_num_array, available_servers, unitgroup_loads, server_loads))) {
  ...
  } else if (OB_FAIL(update_server_load_value(balance_info.intra_weights_, RES_MAX, server_loads))) {
    LOG_WARN("fail to update unitgroup and server load value", K(ret));
  } else if (OB_FAIL(do_balance_row_units(balance_info, unit_migrate_stat_matrix, unitgroup_loads, server_loads))) {
...
int ObServerBalancer::generate_unitgroup_and_server_load(const Matrix<UnitMigrateStat>& unit_migrate_stat_matrix,
    ...
    common::ObIArray<UnitGroupLoad>& unitgroup_loads, common::ObIArray<ServerLoad>& server_loads)
{
  ...
  if (OB_UNLIKELY(!inited_)) {
    ret = OB_NOT_INIT;
    LOG_WARN("not init", K(ret));
  } else if (unit_migrate_stat_matrix.get_row_count() <= 0 || unit_migrate_stat_matrix.get_column_count() <= 0 ||
             column_unit_num_array.count() <= 0 || available_servers.count() <= 0) {
    ret = OB_INVALID_ARGUMENT;
    LOG_WARN("invalid argument", K(ret), K(unit_migrate_stat_matrix), K(available_servers));
  } else if (OB_FAIL(generate_unitgroup_load(unit_migrate_stat_matrix, column_unit_num_array, unitgroup_loads))) {
...
int ObServerBalancer::generate_unitgroup_load(const Matrix<UnitMigrateStat>& unit_migrate_stat_matrix,
    ...
    for (int64_t i = 0; OB_SUCC(ret) && i < column_unit_num_array.count(); ++i) {
      for (int64_t j = 0; OB_SUCC(ret) && j < column_unit_num_array.at(i); ++j) {
        ...
        if (OB_FAIL(generate_column_unitgroup_load(
                column, unit_migrate_stat_matrix, accumulate_idx, column_unit_num_array.at(i), unitgroup_loads))) {
...
int ObServerBalancer::generate_column_unitgroup_load(const int64_t column,
    const Matrix<UnitMigrateStat>& unit_migrate_stat_matrix, const int64_t start_column_idx, const int64_t column_count,
    common::ObIArray<UnitGroupLoad>& unitgroup_loads)
{
  ...
  if (OB_UNLIKELY(!inited_)) {
    ...
    ret = OB_INVALID_ARGUMENT;
    LOG_WARN("invalid argument", K(ret), K(start_column_idx), K(column), K(column_count), K(unit_migrate_stat_matrix));
  } else {
    ...
    for (int64_t row = 0; row < unit_migrate_stat_matrix.get_row_count(); ++row) {
      ...
      } else if (OB_FAIL(unitgroup_load.unit_loads_.push_back(*unit_migrate_stat.unit_load_.unit_config_))) {
    ...
    if (OB_FAIL(ret)) {
      ...
      ret = OB_ERR_UNEXPECTED;
      LOG_WARN("server addr unexpected",
          K(ret),
          K(column),
          K(server_addr),
          K(column_tenant_id),
...
int ObServerBalancer::generate_server_load(const common::ObIArray<common::ObAddr>& available_servers,
    common::ObIArray<UnitGroupLoad>& unitgroup_loads, common::ObIArray<ServerLoad>& server_loads)
{
  int ret = OB_SUCCESS;
  if (OB_UNLIKELY(!inited_)) {
    ...
    for (int64_t i = 0; OB_SUCC(ret) && i < available_servers.count(); ++i) {
      ...
      if (OB_FAIL(server_mgr_->get_server_status(server_load.server_, server_status))) {
        LOG_WARN("fail to get server status", K(ret));
      } else if (OB_FAIL(try_regulate_intra_ttg_resource_info(server_status.resource_info_, intra_ttg_resource_info))) {
...
int ObServerBalancer::arrange_server_load_for_wild_unitgroup(
    UnitGroupLoad& unitgroup_load, common::ObIArray<ServerLoad>& server_loads)
{
  int ret = OB_SUCCESS;
  bool find = false;
  for (int64_t i = 0; !find && OB_SUCC(ret) && i < server_loads.count(); ++i) {
    ...
    for (int64_t j = 0; !same_pool_found && OB_SUCC(ret) && j < server_load.unitgroup_loads_.count(); ++j) {
  ...
  if (OB_FAIL(ret)) {
    ...
    ret = OB_MACHINE_RESOURCE_NOT_ENOUGH;
    LOG_WARN("no available server to hold more unit", K(ret), "tenant_id", unitgroup_load.column_tenant_id_);
  }
  return ret;
}

int ObServerBalancer::calc_row_balance_resource_weights(const common::ObIArray<ServerLoad>& server_loads,
    const common::ObIArray<UnitGroupLoad>& unitgroup_loads, double* const resource_weights, const int64_t weights_count)
{
  int ret = OB_SUCCESS;
  if (OB_UNLIKELY(!inited_)) {
    ret = OB_NOT_INIT;
    LOG_WARN("not init", K(ret));
  } else if (OB_UNLIKELY(server_loads.count() <= 0 || unitgroup_loads.count() <= 0 || NULL == resource_weights ||
    ...
    for (int32_t i = RES_CPU; i < RES_MAX; ++i) {
      ...
      if (RES_DISK == resource_type) {
        resource_weights[resource_type] = 0.0;
      } else if (sum_assigned <= 0 || sum_capacity <= 0) {
        resource_weights[resource_type] = 0.0;
...
int ObServerBalancer::do_balance_row_units(TenantGroupBalanceInfo& balance_info,
...
int ObServerBalancer::CountBalanceStrategy::do_balance_row_units(TenantGroupBalanceInfo& balance_info,
    Matrix<UnitMigrateStat>& unit_migrate_stat_matrix, common::ObIArray<UnitGroupLoad>& unitgroup_loads,
    common::ObIArray<ServerLoad>& server_loads)
{
  int ret = OB_SUCCESS;
  if (OB_UNLIKELY(unitgroup_loads.count() <= 0 || server_loads.count() <= 0)) {
    ret = OB_INVALID_ARGUMENT;
    LOG_WARN("invalid argument", K(ret), K(server_loads), K(unitgroup_loads));
  } else if (OB_FAIL(make_count_balanced(balance_info, unit_migrate_stat_matrix, unitgroup_loads, server_loads))) {
    LOG_WARN("fail to make count balanced", K(ret));
  } else if (OB_FAIL(move_or_exchange_ug_balance_ttg_load(balance_info, unitgroup_loads, server_loads))) {
...
// until the exchange cannot reduce the difference between the two or the difference between the two is less than
// tolerance
int ObServerBalancer::CountBalanceStrategy::move_or_exchange_ug_balance_ttg_load(TenantGroupBalanceInfo& balance_info,
    common::ObIArray<UnitGroupLoad>& unitgroup_loads, common::ObIArray<ServerLoad>& server_loads)
{
  int ret = OB_SUCCESS;
  UNUSED(balance_info);
    ...
    if (OB_SUCC(ret)) {
      ...
      if (OB_FAIL(cmp.get_ret())) {
        LOG_WARN("fail to sorted", K(ret));
                     ...
                     do_move_or_exchange_ug_balance_ttg_load(server_load_ptrs_sorted, unitgroup_loads, tolerance))) {
...
int ObServerBalancer::CountBalanceStrategy::do_move_or_exchange_ug_balance_ttg_load(
    common::ObArray<ServerLoad*>& server_load_ptrs_sorted, common::ObIArray<UnitGroupLoad>& unitgroup_loads,
    const double tolerance)
{
  int ret = OB_SUCCESS;
  if (server_load_ptrs_sorted.count() <= 0 || unitgroup_loads.count() <= 0 || unitgroup_loads.count() >= INT32_MAX) {
    ret = OB_INVALID_ARGUMENT;
    ...
    while (OB_SUCC(ret) && do_operate && times < max_times) {
      ...
      } else if (max_load_server->intra_ttg_load_value_ < tolerance + min_load_server->intra_ttg_load_value_) {
        break;  // load balanced
      } else if (max_load_server->unitgroup_loads_.count() <= min_load_server->unitgroup_loads_.count()) {
        if (OB_FAIL(try_exchange_ug_balance_ttg_load_foreach(
                *max_load_server, *min_load_server, unitgroup_loads, do_operate))) {
          LOG_WARN("fail to exchange ug", K(ret));
        }
      } else if (max_load_server->unitgroup_loads_.count() > min_load_server->unitgroup_loads_.count() + 1) {
        ret = OB_ERR_UNEXPECTED;
        LOG_WARN("unitgroup load count unexpected", K(ret), K(*max_load_server), K(*min_load_server));
      } else {
...
int ObServerBalancer::CountBalanceStrategy::try_move_ug_balance_ttg_load_foreach(ServerLoad& max_server_load,
    ServerLoad& min_server_load, common::ObIArray<UnitGroupLoad>& unitgroup_loads, bool& do_move)
{
  int ret = OB_SUCCESS;
  do_move = false;
  if (max_server_load.unitgroup_loads_.count() <= min_server_load.unitgroup_loads_.count()) {
    ...
    ObArray<common::ObAddr> excluded_dst_servers;
    for (int64_t i = max_server_load.unitgroup_loads_.count() - 1; !do_move && OB_SUCC(ret) && i >= 0; --i) {
      ...
      if (OB_FAIL(get_ug_balance_excluded_dst_servers(ug_load, unitgroup_loads, excluded_dst_servers))) {
        ...
        // in excluded servers, ignore
                     ...
                     check_can_move_ug_balance_intra_ttg_load(max_server_load, ug_load, min_server_load, can_move))) {
        LOG_WARN("fail to check can move ug balance intra ttg load", K(ret));
      } else if (!can_move) {
        // ignore
      } else if (OB_FAIL(move_ug_between_server_loads(max_server_load, min_server_load, ug_load, i))) {
        LOG_WARN("fail to move ug between server loads", K(ret));
      } else {
        do_move = true;
      }
    ...
    if (OB_SUCC(ret)) {
      ...
      std::sort(max_server_load.unitgroup_loads_.begin(), max_server_load.unitgroup_loads_.end(), unitgroup_load_cmp);
      if (OB_FAIL(unitgroup_load_cmp.get_ret())) {
        LOG_WARN("fail to sort", K(ret));
      }
    }
    if (OB_SUCC(ret)) {
      ...
      std::sort(min_server_load.unitgroup_loads_.begin(), min_server_load.unitgroup_loads_.end(), unitgroup_load_cmp);
      if (OB_FAIL(unitgroup_load_cmp.get_ret())) {
        LOG_WARN("fail to sort", K(ret));
      }
    }
...
int ObServerBalancer::CountBalanceStrategy::try_exchange_ug_balance_ttg_load_foreach(ServerLoad& max_server_load,
    ServerLoad& min_server_load, common::ObIArray<UnitGroupLoad>& unitgroup_loads, bool& do_exchange)
{
  int ret = OB_SUCCESS;
  do_exchange = false;
  if (max_server_load.intra_ttg_load_value_ - min_server_load.intra_ttg_load_value_ < EPSILON) {
    do_exchange = false;
  } else if (max_server_load.unitgroup_loads_.count() <= 0 || min_server_load.unitgroup_loads_.count() <= 0) {
    do_exchange = false;
  } else {
    for (int64_t i = 0; !do_exchange && OB_SUCC(ret) && i < max_server_load.unitgroup_loads_.count(); ++i) {
      common::ObArray<common::ObAddr> excluded_servers;
      ...
      if (OB_UNLIKELY(NULL == left_ug)) {
        ...
        for (int64_t j = min_server_load.unitgroup_loads_.count() - 1; !do_exchange && OB_SUCC(ret) && j >= 0; --j) {
...
// until the difference between the largest server load and the smallest server load is less than or equal to 1
// unitgroup
int ObServerBalancer::CountBalanceStrategy::make_count_balanced(TenantGroupBalanceInfo& balance_info,
    Matrix<UnitMigrateStat>& unit_migrate_stat_matrix, common::ObIArray<UnitGroupLoad>& unitgroup_loads,
    common::ObIArray<ServerLoad>& server_loads)
{
  int ret = OB_SUCCESS;
  ...
  if (OB_UNLIKELY(unitgroup_loads.count() <= 0 || server_loads.count() <= 0)) {
    ...
    if (OB_SUCC(ret)) {
      ...
      std::sort(server_cnt_ptrs_sorted.begin(), server_cnt_ptrs_sorted.end(), cmp);
      if (OB_FAIL(cmp.get_ret())) {
        LOG_WARN("fail to sort", K(ret));
...
int ObServerBalancer::CountBalanceStrategy::do_make_count_balanced_foreach(
    ServerLoad& src_server_load, ServerLoad& dst_server_load, common::ObIArray<UnitGroupLoad>& unitgroup_loads)
{
  int ret = OB_SUCCESS;
  if (unitgroup_loads.count() <= 0) {
    ret = OB_INVALID_ARGUMENT;
    ...
    for (int64_t i = src_server_load.unitgroup_loads_.count() - 1; OB_SUCC(ret) && i >= 0; --i) {
      common::ObArray<common::ObAddr> excluded_servers;
      UnitGroupLoad* ug_load = src_server_load.unitgroup_loads_.at(i);
      if (src_server_load.unitgroup_loads_.count() <= dst_server_load.unitgroup_loads_.count() + 1) {
        break;
      } else if (OB_UNLIKELY(NULL == ug_load)) {
        ret = OB_ERR_UNEXPECTED;
        LOG_WARN("ug_load ptr is null", K(ret), KP(ug_load));
      } else if (OB_FAIL(get_ug_balance_excluded_dst_servers(*ug_load, unitgroup_loads, excluded_servers))) {
        LOG_WARN("fail to get excluded servers", K(ret));
      } else if (has_exist_in_array(excluded_servers, dst_server_load.server_)) {
        // ignore this ug load, since it cannot move to dst server
      } else if (OB_FAIL(move_ug_between_server_loads(src_server_load, dst_server_load, *ug_load, i))) {
        LOG_WARN("fail to move ug", K(ret));
      } else {
      }  // no more to do
    }
    if (OB_SUCC(ret)) {
      ...
      std::sort(src_server_load.unitgroup_loads_.begin(), src_server_load.unitgroup_loads_.end(), unitgroup_load_cmp);
      if (OB_FAIL(unitgroup_load_cmp.get_ret())) {
        LOG_WARN("fail to sort", K(ret));
      }
    }
    if (OB_SUCC(ret)) {
      ...
      std::sort(dst_server_load.unitgroup_loads_.begin(), dst_server_load.unitgroup_loads_.end(), unitgroup_load_cmp);
      if (OB_FAIL(unitgroup_load_cmp.get_ret())) {
        LOG_WARN("fail to sort", K(ret));
      }
    }
...
int ObServerBalancer::InnerTenantGroupBalanceStrategy::get_ug_balance_excluded_dst_servers(
    const UnitGroupLoad& unitgroup_load, const common::ObIArray<UnitGroupLoad>& unitgroup_loads,
...
int ObServerBalancer::InnerTenantGroupBalanceStrategy::exchange_ug_between_server_loads(UnitGroupLoad& left_ug,
    const int64_t left_idx, ServerLoad& left_server, UnitGroupLoad& right_ug, const int64_t right_idx,
    ServerLoad& right_server)
{
  int ret = OB_SUCCESS;
  if (left_idx < 0 || left_idx >= left_server.unitgroup_loads_.count() || right_idx < 0 ||
      right_idx >= right_server.unitgroup_loads_.count()) {
    ret = OB_INVALID_ARGUMENT;
    LOG_WARN("invalid argument",
        K(ret),
        K(left_idx),
        "left_server_ug_cnt",
        left_server.unitgroup_loads_.count(),
        K(right_idx),
        "right_server_ug_cnt",
        right_server.unitgroup_loads_.count());
...
int ObServerBalancer::CountBalanceStrategy::check_can_exchange_ug_balance_intra_ttg_load(UnitGroupLoad& left_ug,
    ServerLoad& left_server, UnitGroupLoad& right_ug, ServerLoad& right_server, bool& can_exchange)
{
  int ret = OB_SUCCESS;
  can_exchange = false;
  double left_before = left_server.intra_ttg_load_value_;
...
int ObServerBalancer::CountBalanceStrategy::check_can_exchange_ug_balance_inter_ttg_load(UnitGroupLoad& left_ug,
    ServerLoad& left_server, UnitGroupLoad& right_ug, ServerLoad& right_server, bool& can_exchange)
{
  int ret = OB_SUCCESS;
  can_exchange = false;
...
{
  bool bool_ret = OB_INVALID_ID != column_tenant_id_ && start_column_idx_ >= 0 && column_count_ > 0 &&
                  unit_loads_.count() > 0 && server_.is_valid() && load_sum_.is_valid();
...
int ObServerBalancer::UnitGroupLoad::get_intra_ttg_load_value(
    const ServerLoad& target_server_load, double& intra_ttg_load_value) const
{
  int ret = OB_SUCCESS;
  intra_ttg_load_value = 0.0;
  for (int32_t i = RES_CPU; i < RES_MAX; ++i) {
    ObResourceType res_type = static_cast<ObResourceType>(i);
    if (target_server_load.get_intra_ttg_resource_capacity(res_type) <= 0 || load_sum_.get_required(res_type) <= 0) {
      ...
      double quotient = load_sum_.get_required(res_type) / target_server_load.get_intra_ttg_resource_capacity(res_type);
...
int ObServerBalancer::UnitGroupLoad::get_inter_ttg_load_value(
    const ServerLoad& target_server_load, double& inter_ttg_load_value) const
{
  int ret = OB_SUCCESS;
  inter_ttg_load_value = 0.0;
  for (int32_t i = RES_CPU; i < RES_MAX; ++i) {
    ObResourceType res_type = static_cast<ObResourceType>(i);
    if (target_server_load.get_true_capacity(res_type) <= 0 || load_sum_.get_required(res_type) <= 0) {
      // has no effect on load value
      ...
      double quotient = load_sum_.get_required(res_type) / target_server_load.get_true_capacity(res_type);
...
bool ObServerBalancer::UnitGroupLoadCmp::operator()(const UnitGroupLoad* left, const UnitGroupLoad* right)
{
...
double ObServerBalancer::ServerResourceLoad::get_true_capacity(const ObResourceType resource_type) const
{
  ...
  switch (resource_type) {
    ...
    case RES_MEM:
      ...
      break;
    case RES_DISK:
      ret = static_cast<double>(resource_info_.disk_total_);
      break;
    default:
      ret = -1;
      break;
...
double ObServerBalancer::ServerLoad::get_intra_ttg_resource_capacity(const ObResourceType resource_type) const
{
  double ret = -1;
  switch (resource_type) {
    case RES_CPU:
    ...
    case RES_MEM:
      ...
      break;
    case RES_DISK:
      ret = static_cast<double>(intra_ttg_resource_info_.disk_total_);
      break;
    default:
      ret = -1;
      break;
...
bool ObServerBalancer::ServerTotalLoadCmp::operator()(const ServerTotalLoad* left, const ServerTotalLoad* right)
{
  bool bool_ret = false;
  if (OB_UNLIKELY(OB_SUCCESS != ret_)) {
    // ignore
...
bool ObServerBalancer::IntraServerLoadCmp::operator()(const ServerLoad* left, const ServerLoad* right)
{
  bool bool_ret = false;
  if (OB_UNLIKELY(OB_SUCCESS != ret_)) {
    // ignore
...
bool ObServerBalancer::InterServerLoadCmp::operator()(const ServerLoad* left, const ServerLoad* right)
{
  bool bool_ret = false;
  if (OB_UNLIKELY(OB_SUCCESS != ret_)) {
    // ignore
...
bool ObServerBalancer::ServerLoadUgCntCmp::operator()(const ServerLoad* left, const ServerLoad* right)
{
  bool bool_ret = false;
  if (OB_UNLIKELY(OB_SUCCESS != ret_)) {
    // ignore
...
{
  return load_sum_.min_cpu_ >= 0 && load_sum_.max_cpu_ >= load_sum_.min_cpu_ && load_sum_.min_memory_ >= 0 &&
         load_sum_.max_memory_ >= load_sum_.min_memory_ && load_sum_.max_disk_size_ > 0;
}
...
double ObServerBalancer::LoadSum::get_required(const ObResourceType resource_type) const
{
  ...
  switch (resource_type) {
    case RES_CPU:
      ret = load_sum_.min_cpu_;
      break;
    case RES_MEM:
      ret = static_cast<double>(load_sum_.min_memory_);
      break;
    case RES_DISK:
      ret = static_cast<double>(load_sum_.max_disk_size_);
      break;
    default:
      ret = -1;
      break;
...
int ObServerBalancer::LoadSum::calc_load_value(double* const resource_weights, const int64_t weights_count,
    const ServerResourceLoad& server_resource, double& load_value)
{
  ...
  if (NULL == resource_weights || RES_MAX != weights_count) {
    ...
    for (int32_t i = RES_CPU; i < RES_MAX; ++i) {
      ObResourceType res_type = static_cast<ObResourceType>(i);
      if (server_resource.get_true_capacity(res_type) <= 0 || get_required(res_type) <= 0) {
        // has no effect on load value
...
int ObServerBalancer::LoadSum::remove_load(const ObUnitManager::ObUnitLoad& unit_load)
{
  int ret = OB_SUCCESS;
  if (OB_UNLIKELY(!unit_load.is_valid())) {
    ret = OB_INVALID_ARGUMENT;
...
int ObServerBalancer::LoadSum::append_load(const share::ObUnitConfig& load)
{
  int ret = OB_SUCCESS;
  load_sum_ += load;
  return ret;
...
int ObServerBalancer::LoadSum::append_load(const ObUnitManager::ObUnitLoad& unit_load)
{
  int ret = OB_SUCCESS;
  if (OB_UNLIKELY(!unit_load.is_valid())) {
    ret = OB_INVALID_ARGUMENT;
...
int ObServerBalancer::LoadSum::append_load(const common::ObIArray<ObUnitManager::ObUnitLoad>& unit_loads)
{
  int ret = OB_SUCCESS;
  for (int64_t i = 0; OB_SUCC(ret) && i < unit_loads.count(); ++i) {
    const ObUnitManager::ObUnitLoad& unit_load = unit_loads.at(i);
...
int ObServerBalancer::LoadSum::append_load(const ObServerBalancer::LoadSum& load)
{
  int ret = OB_SUCCESS;
...
{
  return resource_sum_.cpu_ > 0 && resource_sum_.mem_total_ > 0 && resource_sum_.disk_total_ > 0;
}
...
double ObServerBalancer::ResourceSum::get_capacity(const ObResourceType resource_type) const
{
  ...
  switch (resource_type) {
    ...
    case RES_MEM:
      ...
      break;
    case RES_DISK:
      ret = static_cast<double>(resource_sum_.disk_total_);
      break;
    default:
      ret = -1;
      break;
...
int ObServerBalancer::ResourceSum::append_resource(const share::ObServerResourceInfo& resource)
{
  int ret = OB_SUCCESS;
...
int ObServerBalancer::ResourceSum::append_resource(const ResourceSum& resource)
{
  int ret = OB_SUCCESS;
...
int ObServerBalancer::TenantGroupBalanceInfo::get_trick_id(uint64_t& trick_id)
{
  int ret = OB_SUCCESS;
  if (OB_UNLIKELY(NULL == tenant_id_matrix_)) {
    ret = OB_ERR_UNEXPECTED;
    LOG_WARN("tenant id matrix ptr is null", K(ret), KP(tenant_id_matrix_));
  } else if (tenant_id_matrix_->get_row_count() <= 0 || tenant_id_matrix_->get_row_count() >= INT32_MAX ||
             tenant_id_matrix_->get_column_count() <= 0 || tenant_id_matrix_->get_column_count() >= INT32_MAX) {
    ret = OB_ERR_UNEXPECTED;
    LOG_WARN("tenant id matrix size unexpected",
        K(ret),
        "row_count",
        tenant_id_matrix_->get_row_count(),
        "column_count",
        tenant_id_matrix_->get_column_count());
  } else {
    trick_id = UINT64_MAX;
    for (int64_t row = 0; OB_SUCC(ret) && row < tenant_id_matrix_->get_row_count(); ++row) {
      for (int64_t column = 0; OB_SUCC(ret) && column < tenant_id_matrix_->get_column_count(); ++column) {
...
ObServerBalancer::ServerLoad* ObServerBalancer::TenantGroupBalanceInfo::get_server_load(const common::ObAddr& server)
{
  ...
  if (OB_UNLIKELY(!inited_)) {
    ...
    for (int64_t i = 0; OB_SUCC(ret) && i < tenant_groups.count() && legal; ++i) {
      ...
      for (int64_t j = 0; OB_SUCC(ret) && j < tenant_name_group.tenants_.count(); ++j) {
        const share::schema::ObTenantSchema* tenant_schema = NULL;
        if (OB_FAIL(schema_guard.get_tenant_info(tenant_name_group.tenants_.at(j), tenant_schema))) {
...
int ObServerBalancer::get_server_data_disk_usage_limit(double& data_disk_usage_limit) const
{
  int ret = OB_SUCCESS;
  if (OB_UNLIKELY(!inited_)) {
    ret = OB_NOT_INIT;
...
int ObServerBalancer::get_server_balance_critical_disk_waterlevel(double& server_balance_critical_disk_waterlevel) const
{
  int ret = OB_SUCCESS;
  if (OB_UNLIKELY(!inited_)) {
    ret = OB_NOT_INIT;
...
int ObServerBalancer::generate_zone_server_disk_statistic(const common::ObZone& zone)
{
  int ret = OB_SUCCESS;
  common::ObArray<common::ObAddr> server_list;
  double disk_waterlevel = 0.0;
...
bool ObServerBalancer::ServerDiskStatisticCmp::operator()(
    const ServerDiskStatistic& left, const ServerDiskStatistic& right)
{
  bool bool_ret = false;
...
int ObServerBalancer::ZoneServerDiskStatistic::append(ServerDiskStatistic& server_disk_statistic)
{
  int ret = OB_SUCCESS;
  if (OB_FAIL(server_disk_statistic_array_.push_back(server_disk_statistic))) {
    ...
    std::sort(server_disk_statistic_array_.begin(), server_disk_statistic_array_.end(), cmp);
  }
  return ret;
}

int ObServerBalancer::ZoneServerDiskStatistic::raise_server_disk_use(
    const common::ObAddr& server, const int64_t unit_required_size)
{
  int ret = OB_SUCCESS;
  if (OB_UNLIKELY(!server.is_valid())) {
    ...
    bool find = false;
    for (int64_t i = 0; OB_SUCC(ret) && !find && i < server_disk_statistic_array_.count(); ++i) {
...
int ObServerBalancer::ZoneServerDiskStatistic::get_server_disk_statistic(
    const common::ObAddr& server, ServerDiskStatistic& server_disk_statistic)
{
  int ret = OB_SUCCESS;
  if (OB_UNLIKELY(!server.is_valid())) {
    ret = OB_INVALID_ARGUMENT;
  ...
  if (OB_UNLIKELY(!server.is_valid())) {
    ...
    for (int64_t i = 0; !find && i < server_disk_statistic_array_.count(); ++i) {
      ...
      if (server != disk_statistic.server_) {
        ...
        find = true;
        disk_over_waterlevel = (static_cast<double>(disk_statistic.disk_in_use_) >
                                disk_waterlevel * static_cast<double>(disk_statistic.disk_total_));
      }
    }
    if (!find) {
    ...
    const double disk_waterlevel, bool& all_available_servers_over_disk_waterlevel)
{
  ...
  for (int64_t i = 0;
       all_available_servers_over_disk_waterlevel && OB_SUCC(ret) && i < server_disk_statistic_array_.count();
       ++i) {
    ServerDiskStatistic& disk_statistic = server_disk_statistic_array_.at(i);
    all_available_servers_over_disk_waterlevel = (static_cast<double>(disk_statistic.disk_in_use_) >
                                                  disk_waterlevel * static_cast<double>(disk_statistic.disk_total_));
  }
  return ret;
}
...
bool ObServerBalancer::UnitLoadDiskCmp::operator()(
    const ObUnitManager::ObUnitLoad* left, const ObUnitManager::ObUnitLoad* right)
{
  bool bool_ret = false;
  ...
  } else if (OB_SUCCESS != (ret_ = unit_stat_mgr_.get_unit_stat(left->unit_->unit_id_, left_stat))) {
    LOG_WARN("fail to get left unit stat", K(ret_));
  } else if (OB_SUCCESS != (ret_ = unit_stat_mgr_.get_unit_stat(right->unit_->unit_id_, right_stat))) {
