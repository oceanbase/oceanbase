...
public:
  ...
  virtual int finish_zone_merge(const common::ObZone &zone,
                                ...
                                const palf::SCN &last_merged_scn,
                                const palf::SCN &all_merged_scn);
  ...
  int check_need_broadcast(const palf::SCN &frozen_scn, bool &need_broadcast);
  int set_global_freeze_info(const palf::SCN &frozen_scn, const int64_t expected_epoch);
  ...
  virtual int generate_next_global_broadcast_scn(const int64_t expected_epoch, palf::SCN &next_scn);
...
public:
  ...
  virtual int check_need_broadcast(const palf::SCN &frozen_scn,
                                   bool &need_broadcast);
  virtual int set_global_freeze_info(const palf::SCN &frozen_scn, const int64_t expected_epoch);
