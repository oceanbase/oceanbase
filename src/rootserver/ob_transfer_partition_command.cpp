...
int ObTransferPartitionCommand::check_data_version_for_cancel_(const uint64_t tenant_id)
{
  ...
  if (OB_UNLIKELY(!is_valid_tenant_id(tenant_id))) {
    ...
    LOG_WARN("fail to get data version", KR(ret), K(tenant_id));
      ...
      || (compat_version >= DATA_VERSION_4_2_2_0 && compat_version < DATA_VERSION_4_2_4_0)) {
    ret = OB_NOT_SUPPORTED;
    LOG_USER_ERROR(OB_NOT_SUPPORTED, "Tenant COMPATIBLE is below 4.2.4.0. Operation is");
...
int ObTransferPartitionCommand::check_data_version_for_balance_job_(const ObTransferPartitionArg &arg)
{
  ...
  if (OB_UNLIKELY(!arg.is_valid() || !arg.is_balance_job_op())) {
    ...
    if (OB_FAIL(GET_MIN_DATA_VERSION(tenant_id, compat_version))) {
      ...
      ret = OB_NOT_SUPPORTED;
      LOG_USER_ERROR(OB_NOT_SUPPORTED, "Tenant COMPATIBLE is below 4.2.5.2. Operation is");
      LOG_WARN("Tenant COMPATIBLE is below target version, command is not supported",
