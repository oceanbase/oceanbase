...
int ObIndexBuilder::drop_index(const ObDropIndexArg &arg, obrpc::ObDropIndexRes &res)
{
  ...
  if (OB_SUCC(ret)) {
    ...
    const ObTableSchema *second_container_table_schema = NULL;
    if (OB_INVALID_ID != arg.index_table_id_) {
      ...
      } else if (OB_INVALID_ID != arg.container_table_id_) {
        if (OB_FAIL(schema_guard.get_table_schema(tenant_id, arg.container_table_id_, container_table_schema))) {
          LOG_WARN("fail to get index table schema", K(ret), K(tenant_id), K(arg.container_table_id_));
        }
      }
      if (OB_SUCC(ret) && OB_INVALID_ID != arg.second_container_table_id_) {
        if (OB_FAIL(schema_guard.get_table_schema(tenant_id, arg.second_container_table_id_, second_container_table_schema))) {
          LOG_WARN("fail to get index table schema", K(ret), K(tenant_id), K(arg.second_container_table_id_));
        }
      }
    } else {
    ...
    } else if (index_table_schema->is_using_ivf_index() &&
               OB_ISNULL(container_table_schema)) {
      ObString container_table_name;
      int64_t index_table_id = OB_INVALID_ID;
      if (FALSE_IT(index_table_id = index_table_schema->get_table_id())) {
      } else if (OB_FAIL(ObTableSchema::build_index_table_name(
          allocator, index_table_id, arg.index_name_, container_table_name))) {
        LOG_WARN("build_index_table_name failed", K(arg), K(index_table_id), K(ret));
      } else if (OB_FAIL(schema_guard.get_table_schema(tenant_id,
              table_schema->get_database_id(),
              container_table_name,
              true,
              container_table_schema))) {
        LOG_WARN("fail to get table schema", K(ret), K(tenant_id), K(container_table_schema));
      }
      if (OB_SUCC(ret) && index_table_schema->is_using_ivfpq_index() && OB_ISNULL(second_container_table_schema)) {
        ObString second_container_table_name;
        int64_t container_table_id = OB_INVALID_ID;
        if (FALSE_IT(container_table_id = container_table_schema->get_table_id())) {
        } else if (OB_FAIL(ObTableSchema::build_index_table_name(
            allocator, container_table_id, arg.index_name_, second_container_table_name))) {
          LOG_WARN("build_index_table_name failed", K(arg), K(container_table_id), K(ret));
        } else if (OB_FAIL(schema_guard.get_table_schema(tenant_id,
                table_schema->get_database_id(),
                second_container_table_name,
                true,
                second_container_table_schema))) {
          LOG_WARN("fail to get table schema", K(ret), K(tenant_id), K(second_container_table_schema));
        }
      }
    }
    if (FAILEDx(ddl_service_.check_index_on_foreign_key(index_table_schema,
                                                               foreign_key_infos,
                                                               have_index))) {
      ...
      SMART_VAR(ObTableSchema, new_index_schema) {
        ...
        } else if (OB_FAIL(submit_drop_index_task(trans, *table_schema, new_index_schema, new_index_schema.get_schema_version(), arg, allocator, task_record, container_table_schema, second_container_table_schema))) {
      ...
      ObTableItem container_table_item;
      ObTableItem second_container_table_item;
      if (OB_NOT_NULL(container_table_schema)) {
        container_table_item.database_name_ = arg.database_name_;
        container_table_item.table_name_ =  container_table_schema->get_table_name();
        container_table_item.is_hidden_ = container_table_schema->is_user_hidden_table();
      }
      if (OB_NOT_NULL(second_container_table_schema)) {
        second_container_table_item.database_name_ = arg.database_name_;
        second_container_table_item.table_name_ =  second_container_table_schema->get_table_name();
        second_container_table_item.is_hidden_ = second_container_table_schema->is_user_hidden_table();
      }
      ...
      if (OB_NOT_NULL(second_container_table_schema) && OB_FAIL(drop_table_arg.tables_.push_back(second_container_table_item))) {
        LOG_WARN("failed to add table item!", K(second_container_table_item), K(ret));
      } else if (OB_NOT_NULL(container_table_schema) && OB_FAIL(drop_table_arg.tables_.push_back(container_table_item))) {
        LOG_WARN("failed to add table item!", K(container_table_item), K(ret));
    ...
    obrpc::ObAlterTableRes &res)
{
  ...
  HEAP_VARS_2((ObTableSchema, new_table_schema),
    ...
    if (OB_FAIL(schema_guard.get_schema_version(tenant_id, refreshed_schema_version))) {
    ...
    } else if (OB_FAIL(generate_container_schema(new_arg, new_table_schema, index_schema, true/*generate_id*/))) {
      LOG_WARN("fail to generate ivf container schema", K(ret));
    } else {
      if (gen_columns.empty()) {
        ...
        } else if (new_arg.is_vector_ivf_index()) {
          if (OB_FAIL(create_container_table(new_arg, trans, allocator, index_schema, new_table_schema, 0, tenant_data_version, true))) {
            LOG_WARN("fail to create ivf container table", K(ret));
          } else if (USING_IVFPQ == new_arg.index_using_type_ &&
                     OB_FAIL(create_container_table(new_arg, trans, allocator, new_arg.vector_help_schema_.at(0), new_table_schema, 1, tenant_data_version, true))) {
            LOG_WARN("fail to create ivfpq second container table", K(ret));
          }
        }
      } else {
      ...
      if (OB_FAIL(ret)) {
                                                 ...
                                                 allocator,
                                                 task_record,
                                                 new_arg.vector_help_schema_.empty() ? nullptr : &new_arg.vector_help_schema_.at(0),
                                                 new_arg.vector_help_schema_.count() < 2 ? nullptr : &new_arg.vector_help_schema_.at(1)))) {
...
int ObIndexBuilder::create_container_table(obrpc::ObCreateIndexArg &arg,
...
{
  int ret = OB_SUCCESS;
  ObTableSchema &container_table_schema = arg.vector_help_schema_.at(offset);
  ObString index_table_name = arg.index_name_;
  const int64_t index_table_id = index_schema.get_table_id();
  if (OB_FAIL(ObTableSchema::build_index_table_name(
            allocator, index_table_id, arg.index_name_, index_table_name))) {
    LOG_WARN("build_index_table_name failed", K(index_table_id), K(arg), K(ret));
  } else if (OB_FAIL(container_table_schema.set_table_name(index_table_name))) {
    LOG_WARN("set_table_name failed", K(index_table_name), K(arg), K(ret));
  } else if (!is_global_index && OB_FAIL(ddl_service_.create_index_table(arg, tenant_data_version, container_table_schema, trans))) {
    LOG_WARN("fail to create local container table", K(ret), K(container_table_schema));
  } else if (is_global_index && OB_FAIL(ddl_service_.create_global_index(trans, arg, new_table_schema, tenant_data_version, container_table_schema))) {
    LOG_WARN("fail to create global container table", K(ret), K(container_table_schema));
  }
  return ret;
}
    ...
    obrpc::ObAlterTableRes &res)
{
  ...
  HEAP_VARS_3((ObTableSchema, index_schema),
    ...
    LOG_WARN("get tenant data version failed", K(ret));
      ...
      } else if (INDEX_TYPE_VECTOR_GLOBAL == my_arg.index_type_) {
        ret = OB_NOT_SUPPORTED;
        LOG_WARN("global vector index is not supported now", K(ret));
      }
      if (OB_FAIL(ObIndexBuilderUtil::adjust_expr_index_args(
      ...
      } else if (OB_FAIL(generate_container_schema(my_arg, new_table_schema, index_schema, true/*generate_id*/))) {
        LOG_WARN("fail to generate ivf container schema", K(ret));
        ...
        } else if (my_arg.is_vector_ivf_index()) {
          if (OB_FAIL(create_container_table(my_arg, trans, allocator, index_schema, new_table_schema, 0, tenant_data_version))) {
            LOG_WARN("fail to create ivf container table", K(ret));
          } else if (USING_IVFPQ == my_arg.index_using_type_ &&
                     OB_FAIL(create_container_table(my_arg, trans, allocator, my_arg.vector_help_schema_.at(0), new_table_schema, 1, tenant_data_version))) {
            LOG_WARN("fail to create ivfpq second container table", K(ret));
          }
        }
      } else {
      ...
      if (OB_FAIL(ret)) {
                                                 ...
                                                 allocator,
                                                 task_record,
                                                 my_arg.vector_help_schema_.empty() ? nullptr : &my_arg.vector_help_schema_.at(0),
                                                 my_arg.vector_help_schema_.count() < 2 ? nullptr : &my_arg.vector_help_schema_.at(1)))) {
    ...
    obrpc::ObAlterTableRes &res)
{
  ...
  if (!ddl_service_.is_inited()) {
    ...
    LOG_WARN("check whether the foreign key related table is executing ddl failed", K(ret));
             ...
             || INDEX_TYPE_VECTOR_LOCAL == arg.index_type_) {
    if (OB_FAIL(do_create_local_index(schema_guard, arg, *table_schema, res))) {
      LOG_WARN("fail to do create local index", K(ret), K(arg));
             ...
             || INDEX_TYPE_VECTOR_GLOBAL == arg.index_type_) {
...
int ObIndexBuilder::generate_container_schema(
    ...
    const bool generate_id)
{
  ...
  if (arg.is_vector_ivf_index()) {
    if (0 >= arg.vector_help_schema_.count()) {
      ret = OB_ERR_UNEXPECTED;
      LOG_WARN("unexpected empty ivf container table schema", K(ret));
    } else {
      ObTableSchema &container_table_schema = arg.vector_help_schema_.at(0);
      if (OB_FAIL(container_table_schema.assign(index_schema))) {
        LOG_WARN("failed to assign table schema", K(ret));
      } else {
        // container_table_schema.reuse_partition_schema();
        container_table_schema.reset_column_info();
        container_table_schema.set_vec_ivf_container_table(ObVecIvfContainerTableFlag::IS_IVF_CONTAINER_TABLE);
        // rename table_name by index table id after create index table
        if (OB_FAIL(set_index_table_columns(arg, data_schema, container_table_schema, ObIndexTableType::CONTAINER_TABLE))) {
          LOG_WARN("set_container_table_columns failed", K(arg), K(data_schema), K(ret));
        } else if (generate_id) {
          if (OB_FAIL(ddl_service_.generate_object_id_for_partition_schema(container_table_schema))) {
            LOG_WARN("fail to generate object_id for partition schema", KR(ret), K(container_table_schema));
          } else if (OB_FAIL(ddl_service_.generate_tablet_id(container_table_schema))) {
            LOG_WARN("fail to fetch new table id", KR(ret), K(container_table_schema));
          }
        }

        if (OB_SUCC(ret) && USING_IVFPQ == arg.index_using_type_) {
          if (1 >= arg.vector_help_schema_.count()) {
            ret = OB_ERR_UNEXPECTED;
            LOG_WARN("unexpected missing ivfpq container table schema", K(ret));
          } else {
            ObTableSchema &second_container_table_schema = arg.vector_help_schema_.at(1);
            if (OB_FAIL(second_container_table_schema.assign(container_table_schema))) {
              LOG_WARN("failed to assign table schema", K(ret));
            } else {
              // second_container_table_schema.reuse_partition_schema();
              second_container_table_schema.reset_column_info();
              second_container_table_schema.set_vec_ivf_container_table(ObVecIvfContainerTableFlag::IS_NOT_IVF_CONTAINER_TABLE);
              second_container_table_schema.set_vec_ivfpq_second_container_table(ObVecIvfpqSecondContainerTableFlag::IS_IVFPQ_SECOND_CONTAINER_TABLE);
              // rename table_name by index table id after create index table
              if (OB_FAIL(set_index_table_columns(arg, data_schema, second_container_table_schema, ObIndexTableType::SECOND_CONTAINER_TABLE))) {
                LOG_WARN("set_second_container_table_columns failed", K(arg), K(data_schema), K(ret));
              } else if (generate_id) {
                if (OB_FAIL(ddl_service_.generate_object_id_for_partition_schema(second_container_table_schema))) {
                  LOG_WARN("fail to generate object_id for partition schema", KR(ret), K(second_container_table_schema));
                } else if (OB_FAIL(ddl_service_.generate_tablet_id(second_container_table_schema))) {
                  LOG_WARN("fail to fetch new table id", KR(ret), K(second_container_table_schema));
                }
              }
            }
          }
      ...
      if (OB_SUCC(ret)) {
        LOG_INFO("success to generate container table schema", K(ret), K(container_table_schema));
      }
    }
  }
  return ret;
...
int ObIndexBuilder::set_index_table_columns(const ObCreateIndexArg &arg,
                                            const ObTableSchema &data_schema,
                                            ObTableSchema &schema,
                                            const ObIndexTableType index_table_type)
{
  int ret = OB_SUCCESS;
  if (!ddl_service_.is_inited()) {
    ret = OB_INNER_STAT_ERROR;
    LOG_WARN("ddl_service not init", "ddl_service inited", ddl_service_.is_inited(), K(ret));
  } else if (OB_FAIL(ObIndexBuilderUtil::set_index_table_columns(arg, data_schema, schema, true, index_table_type))) {
                                            ...
                                            share::schema::ObTableSchema &schema)
{
  ...
  if (!ddl_service_.is_inited()) {
    ...
    schema.set_vector_pq_seg(arg.index_schema_.get_vector_pq_seg());
