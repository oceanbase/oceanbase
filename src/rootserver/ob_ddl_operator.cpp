                                            ...
                                            const bool is_truncate_table)
{
  ...
  } else if (OB_UNLIKELY(table_schema.table_referenced_by_mv())) {
    ret = OB_ERR_UNEXPECTED;
    ...
    const bool is_oracle_mode)
{
  ...
  if (OB_SUCC(ret) && is_oracle_mode) {
    ...
    if (is_ora_standby_replication_role(user.get_user_id())) {
      ...
      GRANT_OBJ_PRIV_TO_USER(OB_ORA_SYS_SCHEMA_NAME, OB_DBA_OB_LOG_RESTORE_SOURCE_ORA_TNAME, OB_DBA_OB_LOG_RESTORE_SOURCE_ORA_TID, ObObjectType::TABLE, SELECT);
    }
  }
  return ret;
}
