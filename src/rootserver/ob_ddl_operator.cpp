        ...
        for (int64_t i = 0; OB_SUCC(ret) && i < table_ids.count(); ++i) {
          ...
          if (OB_FAIL(schema_service_.get_tenant_schema_guard(tenant_id, schema_guard))) {
            ...
            if (OB_FAIL(drop_trigger_cascade(*table, trans))) {
              LOG_WARN("drop trigger failed", KR(ret), K(table_id));
            } else if (OB_FAIL(drop_table(*table, trans, NULL, false, NULL, true))) {
              LOG_WARN("drop table failed", KR(ret), K(table_id));
            }
          }
        }
      }
         ...
         if (OB_ISNULL(package_schema)) {
          ...
          LOG_WARN("fail to gen new schema_version", K(ret), K(tenant_id));
           ...
           if (OB_FAIL(schema_guard.get_audit_schema_in_owner(tenant_id,
             ...
             LOG_DEBUG("no need to delete audit_schema from drop packege", K(audits), KPC(package_schema));
           }
         }
       }
     }
    ...
    common::ObMySQLTransaction &trans)
{
  ...
  if (OB_ISNULL(schema_service)) {
    ...
    LOG_WARN("delete column update prev id failed", K(ret));
  } else {
...
int ObDDLOperator::alter_table_rename_index(
    const schema::ObTableSchema &orig_index_table_schema,
    ...
    schema::ObTableSchema &new_index_table_schema)
{
  ...
  if (OB_ISNULL(schema_service)) {
    ret = OB_ERR_UNEXPECTED;
    LOG_WARN("schema_service is NULL", K(ret));
  } else if (OB_UNLIKELY(!orig_index_table_schema.is_valid() || !new_index_table_schema.is_valid())) {
    ret = OB_INVALID_ARGUMENT;
    LOG_WARN("invalid arg", K(ret), K(orig_index_table_schema), K(new_index_table_schema));
  } else if (OB_UNLIKELY(orig_index_table_schema.is_in_recyclebin())) {
    ret = OB_ERR_OPERATION_ON_RECYCLE_OBJECT;
    ...
    ObArenaAllocator allocator(ObModIds::OB_SCHEMA);
    const uint64_t tenant_id = orig_index_table_schema.get_tenant_id();
    const uint64_t data_table_id = orig_index_table_schema.get_data_table_id();
    ...
    if (OB_FAIL(ObTableSchema::build_index_table_name(allocator,
      ...
      if (OB_FAIL(ret)) {
        ...
        RS_LOG(WARN, "failed to set new table name!", K(new_index_table_schema), K(ret));
                  ...
                  trans,
                  orig_index_table_schema,
                  new_index_table_schema,
                  orig_index_table_schema.is_global_index_table() ? OB_DDL_RENAME_GLOBAL_INDEX: OB_DDL_RENAME_INDEX))) {
        RS_LOG(WARN, "schema service update_table_options failed", K(orig_index_table_schema), K(ret));
      }
    }
  }
  return ret;
                                 ...
                                 ObSArray<ObDependencyInfo> &dep_infos)
{
  ...
  if (OB_FAIL(ObDependencyInfo::delete_schema_object_dependency(trans, tenant_id,
    ...
    LOG_WARN("Delete schema obj dependency failed.", K(ret), K(routine_info));
  } else if (OB_FAIL(ObDependencyInfo::insert_dependency_infos(trans, dep_infos, tenant_id,
                              routine_info.get_routine_id(),
                              routine_info.get_schema_version(),
                              routine_info.get_owner_id()))) {
    LOG_WARN("Insert schema obj dependency failed.", K(ret), K(routine_info));
  } else if (OB_FAIL(error_info.handle_error_info(trans, &routine_info))) {
                              ...
                              const ObString *ddl_stmt_str)
{
  ...
  if (OB_SUCC(ret) && !is_inner_pl_udt_id(new_udt_id)) {
    OZ (ObDependencyInfo::insert_dependency_infos(trans, dep_infos, tenant_id, new_udt_id,
                                new_schema_version,
                                udt_info.get_database_id()));
  }
  if (OB_SUCC(ret)) {
                                                    ...
                                                    ObTableSchema &new_data_table_schema)
{
  ...
  for (int64_t i = 0; OB_SUCC(ret) && i < index_info.get_size(); ++i) {
    ...
    if (OB_ISNULL(index_col = data_table->get_column_schema(
      ...
      ret = OB_ERR_UNEXPECTED;
      LOG_WARN("get index column schema failed", K(ret), K(tenant_id),
          "column_id", index_info.get_column(i)->column_id_, K(index_info));
