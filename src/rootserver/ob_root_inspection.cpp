...
int ObRootInspection::wait_and_check_rpc_response(rootserver::ObCheckSysTableSchemaProxy &proxy)
{
  ...
  const ObIArray<const obrpc::ObCheckSysTableSchemaResult *> &results = proxy.get_results();
  const ObIArray<obrpc::ObCheckSysTableSchemaArg> &args = proxy.get_args();
  if (OB_FAIL(proxy.wait_all(return_code))) {
    ...
    for (int64_t i = 0; i < return_code.count() && OB_SUCC(ret); i++) {
      if (OB_ISNULL(results.at(i))) {
        ...
        const obrpc::ObCheckSysTableSchemaResult &result = *results.at(i);
...
int ObRootInspection::check_sys_table_schemas(const ObIArray<uint64_t> &tenant_ids)
{
  ...
  if (OB_UNLIKELY(!inited_)) {
  ...
  } else if (cluster_version < CLUSTER_VERSION_4_2_5_7) {
    ...
    ObCheckSysTableSchemaProxy proxy(*GCTX.srv_rpc_proxy_, &obrpc::ObSrvRpcProxy::check_sys_table_schema);
    ObArray<int> return_code;
    for (int64_t i = 0; i < tenant_ids.count() && OB_SUCC(ret); i++) {
      const uint64_t tenant_id = tenant_ids.at(i);
      if (OB_FAIL(check_cancel())) {
...
{
  int ret = OB_SUCCESS;
  obrpc::ObCheckSysTableSchemaArg arg;
