    ...
    const ObPartitionScatterMode &scatter_mode)
{
  ...
  } else if (OB_FAIL(create_part_group_container_(bg_id, balanced_ls_num, scatter_mode))) {
    ...
    const uint64_t part_group_uid)
{
  ...
  if (part_group_uid != last_part_group_uid_) {
    ...
    if (OB_ISNULL(buf)) {
      ...
      LOG_WARN("part group container is null", KR(ret), K_(pg_container));
    } else if (OB_FAIL(pg_container_->append(table_schema, part_group_uid, part_group))) {
    ...
    const ObPartitionScatterMode &scatter_mode)
{
  ...
  if (OB_UNLIKELY(inited_ || nullptr != pg_container_)) {
    ...
    ObSimplePartGroupContainer *pg_container_simple = nullptr;
    if (OB_ISNULL(pg_container_simple = reinterpret_cast<ObSimplePartGroupContainer *>(
        alloc_.alloc(sizeof(ObSimplePartGroupContainer))))) {
      ret = OB_ALLOCATE_MEMORY_FAILED;
      LOG_WARN("alloc mem fail", KR(ret));
    } else if (OB_ISNULL(pg_container_simple =
        new(pg_container_simple) ObSimplePartGroupContainer(alloc_))) {
      ret = OB_ALLOCATE_MEMORY_FAILED;
      LOG_WARN("alloc mem fail", KR(ret));
    } else if (OB_FAIL(pg_container_simple->init())) {
      LOG_WARN("failed to init simple part group container", KR(ret));
    } else {
      pg_container_ = pg_container_simple;
    }
  } else if (SCATTER_ROUND_ROBIN == scatter_mode) {
    ...
    ret = OB_INVALID_ARGUMENT;
    LOG_WARN("unknown scatter mode", K(scatter_mode));
  }
  return ret;
}
    ...
    ObIPartGroupInfo *&pg_info)
{
  ...
  if (OB_UNLIKELY(!inited_)) {
    ...
    ret = OB_ERR_UNEXPECTED;
    LOG_WARN("part group container is null", KR(ret), K_(pg_container), K_(dst_bg_info.pg_container));
