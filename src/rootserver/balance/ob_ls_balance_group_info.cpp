  ...
  if (OB_UNLIKELY(!inited_)) {
    ...
    FOREACH_X(iter, bg_map_, OB_SUCC(ret)) {
      ...
      if (OB_ISNULL(src_bg_info)
        ...
        const int64_t data_size_threshold = ceil(
            static_cast<double>(src_bg_info->get_part_groups_data_size())
        ...
        if (OB_UNLIKELY(remove_count >= avail_count)) {
          ...
          if (remove_count > 0 && OB_FAIL(selected_part_groups.reserve(remove_count))) {
            LOG_WARN("reserve selected part groups failed", KR(ret), K(remove_count));
          }
  ...
  if (OB_ISNULL(src_bg_info) || OB_ISNULL(dst_bg_info)) {
    ...
    while (OB_SUCC(ret)
      ...
      if (OB_FAIL(get_largest_part_group_from_array_(
        ...
        ret = OB_ERR_UNEXPECTED;
        LOG_WARN("invalid largest pg index", KR(ret), K(largest_pg_idx), K(selected_part_groups));
