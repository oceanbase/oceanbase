...
int64_t BalanceStat::UnitCounter::to_string(char* buf, const int64_t buf_len) const
{
  int64_t pos = 0;
  J_OBJ_START();
  J_KV(K_(unit_id), K_(count));
  J_OBJ_END();
  return pos;
}
int64_t BalanceStat::to_string(char* buf, const int64_t buf_len) const
{
  int64_t pos = 0;
  J_OBJ_START();
  J_KV(K_(row_unit));
