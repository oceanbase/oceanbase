...
int ObTransformSimplify::get_push_down_conditions(
    ObDMLStmt *stmt, JoinedTable *join_table, ObIArray<ObRawExpr *> &join_conds, ObIArray<ObRawExpr *> &push_down_conds)
{
  int ret = OB_SUCCESS;
...
int ObTransformSimplify::try_push_down_outer_join_conds(ObDMLStmt *stmt, JoinedTable *join_table, bool &trans_happened)
{
  ...
  if (OB_ISNULL(stmt) || OB_ISNULL(join_table) || OB_ISNULL(ctx_)) {
    ...
    if (OB_FAIL(get_push_down_conditions(stmt, join_table, join_table->get_join_conditions(), push_down_conds))) {
