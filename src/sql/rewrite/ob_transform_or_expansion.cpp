                                                         ...
                                                         bool &is_match)
{
  ...
  if (OB_SUCC(ret) && !is_match) {
    ...
    if (expr->has_flag(CNT_COLUMN) &&
      ...
      is_match = true;
    } else if ((T_OP_EQ <= com_type && T_OP_NE >= com_type) || T_OP_IS == com_type || T_OP_IS_NOT == com_type) {
  ...
  if (OB_ISNULL(op) || OB_ISNULL(op->get_plan())
    ...
    bool include_unprecise =
        op->get_plan()->get_optimizer_context()
          .get_query_ctx()->check_opt_compat_version(COMPAT_VERSION_4_2_5_BP8);
  ...
  if (OB_ISNULL(op)) {
    ...
    if (OB_FAIL(add_range_exprs_from_array(table_scan->get_range_conditions(),
      ...
      if (pre_range != NULL) {
        ...
        } else if (OB_FAIL(add_range_exprs_from_array(
                             pre_range->get_ss_range_exprs(), range_exprs))) {
          LOG_WARN("failed to add ss range exprs", K(ret));
        }
      }
    }
  } else {
