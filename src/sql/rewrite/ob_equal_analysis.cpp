...
{
  int ret = OB_SUCCESS;
  if (OB_FAIL(column_set_.create(128, ObModIds::OB_SQL_OPTIMIZER_EQUAL_SETS, ObModIds::OB_SQL_OPTIMIZER_EQUAL_SETS))) {
...
{
  int ret = OB_SUCCESS;
  if (OB_ISNULL(expr) || (T_OP_EQ == expr->get_expr_type() &&
                             (OB_ISNULL(expr->get_param_expr(0)) || OB_ISNULL(expr->get_param_expr(1))))) {
    ret = OB_ERR_UNEXPECTED;
    LOG_WARN("expr is null", K(ret));
  } else if (T_OP_EQ == expr->get_expr_type() &&
             (expr->get_param_expr(0)->has_flag(CNT_COLUMN) || expr->get_param_expr(0)->has_flag(CNT_SET_OP)) &&
             (expr->get_param_expr(1)->has_flag(CNT_COLUMN) || expr->get_param_expr(1)->has_flag(CNT_SET_OP))) {
...
int ObEqualAnalysis::add_equal_cond(const ObOpRawExpr& expr)
{
  ...
  if (OB_UNLIKELY(expr.get_expr_type() != T_OP_EQ) || OB_ISNULL(expr.get_param_expr(0)) ||
...
int ObEqualAnalysis::get_equal_sets(ObIAllocator* alloc, EqualSets& equal_sets) const
{
  ...
  DLIST_FOREACH(equal_set, equal_sets_)
  {
    ...
    for (ObExprEqualSet::ColumnIterator iter = equal_set->column_begin();
         OB_SUCC(ret) && iter != equal_set->column_end();
         ++iter) {
      if (OB_FAIL(raw_equal_set.push_back(const_cast<ObRawExpr*>(iter.get_expr_info())))) {
        LOG_WARN("push back equal expr info failed", K(ret));
      }
    }
    if (OB_SUCC(ret)) {
      if (OB_FAIL(ObRawExprSetUtils::add_expr_set(alloc, raw_equal_set, equal_sets))) {
...
int ObEqualAnalysis::expr_can_be_add_to_equal_set(
    const ObExprEqualSet& equal_set, const ObRawExpr* same_expr, const ObRawExpr* new_expr, bool& can_be) const
{
  can_be = true;
  int ret = OB_SUCCESS;
  if (equal_set.get_column_num() <= 1 || OB_ISNULL(same_expr) || OB_ISNULL(new_expr)) {
    ...
    for (ObExprEqualSet::ColumnIterator iter = equal_set.column_begin();
         OB_SUCC(ret) && can_be && iter != equal_set.column_end();
         ++iter) {
...
int ObEqualAnalysis::check_whether_can_be_merged(
    const ObExprEqualSet& equal_set, const ObExprEqualSet& another_set, bool& can_be) const
{
  ...
  if (!equal_set.intersect_equal_set(another_set)) {
    ...
    for (ObExprEqualSet::ColumnIterator iter = another_set.column_begin();
         OB_ISNULL(same_expr) && iter != another_set.column_end();
         ++iter) {
      same_expr = equal_set.get_expr(iter.get_expr_idx());
    }
  }
  for (ObExprEqualSet::ColumnIterator iter = another_set.column_begin();
       OB_SUCC(ret) && can_be && iter != another_set.column_end();
       ++iter) {
...
int ObEqualAnalysis::finish_feed()
{
  ...
  DLIST_FOREACH(equal_set, equal_sets_)
  {
    ...
    if (OB_ISNULL(equal_set) || OB_ISNULL(another_set = equal_set->get_next())) {
      ret = OB_ERR_UNEXPECTED;
      LOG_WARN("equal set is null", K(equal_set), K(another_set));
    } else if (equal_sets_.get_header() != another_set) {
      bool can_be = false;
      if (OB_FAIL(check_whether_can_be_merged(*equal_set, *another_set, can_be))) {
        ...
        for (ObExprEqualSet::ColumnIterator it = another_set->column_begin();
             OB_SUCC(ret) && it != another_set->column_end();
             ++it) {
    ...
    const ObRawExpr& cur_expr, const ObRawExpr& same_expr, const ObRawExpr& new_expr, bool& can_be)
{
  ...
  if (OB_FAIL(check_stack_overflow(is_stack_overflow))) {
  ...
  } else if (T_OP_ROW == cur_expr.get_expr_type() && T_OP_ROW == new_expr.get_expr_type() &&
    ...
    if (cur_expr.get_param_count() != same_expr.get_param_count() ||
      ...
      for (int64_t i = 0; OB_SUCC(ret) && can_be && i < cur_expr.get_param_count(); i++) {
        ...
        if (OB_ISNULL(c_cur = cur_expr.get_param_expr(i)) || OB_ISNULL(c_same = same_expr.get_param_expr(i)) ||
    ...
    if (OB_FAIL(ObRelationalExprOperator::is_equivalent(
            cur_expr.get_result_type(), same_expr.get_result_type(), new_expr.get_result_type(), can_be))) {
...
int ObEqualAnalysis::compute_equal_set(
    ObIAllocator* allocator, const ObIArray<ObRawExpr*>& eset_conditions, EqualSets& equal_sets)
{
  int ret = OB_SUCCESS;
...
int ObEqualAnalysis::compute_equal_set(ObIAllocator* allocator, const ObIArray<ObRawExpr*>& eset_conditions,
