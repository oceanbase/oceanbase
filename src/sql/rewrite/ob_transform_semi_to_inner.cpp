    ...
    common::ObIArray<ObParentDMLStmt> &parent_stmts, ObDMLStmt *&stmt, bool &trans_happened)
{
  ...
  if (OB_ISNULL(stmt) || OB_ISNULL(ctx_)) {
    ...
    for (int64_t i = 0; OB_SUCC(ret) && i < semi_infos.count() && !spj_view_added; ++i) {
      ...
      } else if (need_check_cost && OB_FAIL(ObTransformUtils::partial_cost_eval_validity_check(
                                            *ctx_, parent_stmts, stmt, true, partial_cost_check))) {
                                                 ...
                                                 TransformParam& trans_param)
{
  ...
  if (OB_FAIL(ret)) {
    ...
    if (OB_ISNULL(right_table_ref_query = right_table->ref_query_)) {
      // do nothing
    } else if (OB_FAIL(ObTransformUtils::check_query_from_dual(right_table_ref_query, query_from_dual))) {
  ...
  if (OB_SUCC(ret) && is_valid && need_check_cost
      && (ctx_->eval_cost_ || !ObTransformUtils::is_cost_based_trans_enable(*ctx_, query_hint->global_hint_))) {
    is_valid = false;
  }
  return ret;
}
...
int ObTransformSemiToInner::non_sens_dul_vals_need_check_cost(ObDMLStmt &stmt,
  ...
  if (OB_ISNULL(right_table = stmt.get_table_item_by_id(semi_info.right_table_id_))) {
    ...
    LOG_WARN("failed to calculate column repeat rate", K(ret));
