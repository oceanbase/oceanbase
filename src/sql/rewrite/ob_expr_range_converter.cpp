  ...
  if (OB_ISNULL(expr)) {
    ...
    if (OB_FAIL(convert_is_expr(expr, expr_depth, range_node))) {
      LOG_WARN("failed to convert is expr");
             ...
             GET_MIN_CLUSTER_VERSION() >= CLUSTER_VERSION_4_2_5_6 &&
             ctx_.optimizer_features_enable_version_ >= COMPAT_VERSION_4_2_5_BP6) {
...
int ObExprRangeConverter::convert_in_expr(const ObRawExpr *expr, int64_t expr_depth, ObRangeNode *&range_node)
{
  ...
  if (OB_ISNULL(expr) ||
    ...
    if (OB_FAIL(get_implicit_set_collation_in_range(l_expr, r_expr, expr->get_result_type(),
...
int ObExprRangeConverter::convert_not_in_expr(const ObRawExpr *expr, int64_t expr_depth, ObRangeNode *&range_node)
{
  ...
  if (OB_ISNULL(expr) ||
    ...
    // do nothing
  } else if (GET_MIN_CLUSTER_VERSION() >= CLUSTER_VERSION_4_2_3_0 &&
             ctx_.optimizer_features_enable_version_ >= COMPAT_VERSION_4_2_3 &&
                                                  ...
                                                  ObRangeNode *&range_node)
{
  ...
  if (OB_ISNULL(column_expr) ||
    ...
    calc_type.set_calc_meta(column_expr->get_result_type().get_obj_meta());
                                                           ...
                                                           ObRangeNode *&range_node)
{
  ...
  if (OB_SUCC(ret) && OB_NOT_NULL(collation_expr) &&
    ...
    if (OB_FAIL(check_can_extract_implicit_collation_range(cmp_type,
      ...
      // do nothing
    } else if (OB_FALSE_IT(calc_type.set_calc_meta(inner_expr->get_result_type()
                                                              .get_obj_meta()))) {
