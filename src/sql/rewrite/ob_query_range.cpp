                                        ...
                                        const ObDataTypeCastParams &dtc_params)
{
  ...
  if (OB_ISNULL(b_expr) || OB_ISNULL(query_range_ctx_) || OB_ISNULL(query_range_ctx_->exec_ctx_)
    ...
    ret = OB_INVALID_ARGUMENT;
    LOG_WARN("get unexpected null", K(b_expr), K_(query_range_ctx), K(session), K(query_ctx));
  } else if (2 != b_expr->get_param_count()) {//binary op expr
    ret = OB_ERR_UNEXPECTED;
    LOG_WARN("b_expr must has 2 arguments", K(ret));
                                     ...
                                     const ObDataTypeCastParams &dtc_params)
{
  ...
  if (OB_ISNULL(geo_expr) || OB_ISNULL(query_range_ctx_)) {
    ...
    if (OB_ISNULL(l_expr) || OB_ISNULL(r_expr)) {
      ...
      if (OB_FAIL(ret)) {
      } else if (column_item->get_data_type() != ObObjType::ObGeometryType) { // not geo column, do not extract geo query range
        GET_ALWAYS_TRUE_OR_FALSE(true, out_key_part);
      } else {
                                                    ...
                                                    const ObDataTypeCastParams &dtc_params) const
{
  ...
  } else if (OB_FAIL(search_state.init_search_state(column_count_, true, table_graph_.key_part_head_->id_.table_id_))) {
...
if (OB_SUCC(ret) ) { \
  ...
  if (!start.is_min_value() && !start.is_max_value() && !start.is_unknown() \
    ...
    ObCastCtx cast_ctx(&allocator, &dtc_params, cur_datetime, CM_WARN_ON_FAIL, expect_type.get_collation_type()); \
