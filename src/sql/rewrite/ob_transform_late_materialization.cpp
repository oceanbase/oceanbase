                                                                         ...
                                                                         bool &need)
{
  ...
  } else if (OB_UNLIKELY(COMPAT_VERSION_4_2_5 >
                         stmt.get_query_ctx()->optimizer_features_enable_version_)) {
    /* need_transform = false; */
    LOG_TRACE("not late materialization stmt, optimizer feature enable is lower than 4_2_5");
  ...
  if (OB_ISNULL(ctx_) || OB_ISNULL(schema_guard = ctx_->sql_schema_guard_) ||
    ...
    if (OB_SUCC(ret)) {
      ...
      if (OB_FAIL(ObQueryHint::create_hint(ctx_->allocator_, T_NO_MERGE_HINT, no_merge_hint))) {
        LOG_WARN("failed to create hint", K(ret));
      } else if (OB_FAIL(ObQueryHint::create_hint(ctx_->allocator_, T_NO_EXPAND, or_expand_hint))) {
        LOG_WARN("failed to create hint", K(ret));
      } else {
        no_merge_hint->set_parent_qb_name(parent_qb_name);
        no_merge_hint->set_qb_name(view_qb_name);
        or_expand_hint->set_qb_name(parent_qb_name);
        if (OB_FAIL(select_stmt.get_stmt_hint().merge_hint(*no_merge_hint,
                                                           HINT_DOMINATED_EQUAL,
                                                           conflict_hints))) {
          LOG_WARN("merge no expand hint failed", K(ret));
        } else if (OB_FAIL(select_stmt.get_stmt_hint().merge_hint(*or_expand_hint,
                                                                  HINT_DOMINATED_EQUAL,
                                                                  conflict_hints))) {
          LOG_WARN("merge no expand hint failed", K(ret));
        }
                                                             ...
                                                             bool &is_expected)
{
  ...
  if (OB_ISNULL(top) || OB_ISNULL(ctx_) || OB_ISNULL(ctx_->sql_schema_guard_)) {
    ...
    if (OB_SUCC(ret) && is_expected) {
      ...
      } else if (!index_scan->is_index_scan()) {
        is_expected = false;
