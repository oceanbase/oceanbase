...
int ObTransformPreProcess::add_no_merge_hint_for_set_view(ObSelectStmt *parent_stmt,
                                                           ObSelectStmt *child_stmt)
{
  ...
  if (OB_ISNULL(ctx_) || OB_ISNULL(ctx_->allocator_) || OB_ISNULL(parent_stmt) || OB_ISNULL(child_stmt)) {
    ...
    no_merge_hint->set_trans_added(true);
                                                  ...
                                                  bool &trans_happened)
{
  ...
  if (tenant_config.is_valid()) {
    config_enabled = tenant_config->_preserve_order_for_groupby;
  }
  if (OB_ISNULL(stmt) || OB_ISNULL(ctx_) ||
      OB_ISNULL(ctx_->session_info_)) {
    ret = OB_ERR_UNEXPECTED;
  ...
  } else if (!config_enabled && !has_hint) {
