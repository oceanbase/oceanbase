...
%left	INTERSECT
%left LOWER_ON
%left JOIN CROSS LEFT FULL RIGHT INNER ON USING OUTER NATURAL STRAIGHT_JOIN WINDOW
%left SET_VAR
%left OR OR_OP
%left XOR
%left AND AND_OP
...
DISABLE_PARALLEL_DML ENABLE_PARALLEL_DML ENABLE_PARALLEL_DAS_DML DISABLE_PARALLEL_DAS_DML MONITOR NO_PARALLEL CURSOR_SHARING_EXACT
MAX_CONCURRENT DOP TRACING NO_QUERY_TRANSFORMATION NO_COST_BASED_QUERY_TRANSFORMATION GROUP_COMMIT TABLE_LOCK_MODE
// transform hint
...
DECORRELATE NO_DECORRELATE COALESCE_AGGR NO_COALESCE_AGGR WITH_PULLUP WO_PULLUP
// optimize hint
INDEX_HINT FULL_HINT NO_INDEX_HINT USE_DAS_HINT NO_USE_DAS_HINT PUSH_SUBQ NO_PUSH_SUBQ
INDEX_SS_HINT INDEX_SS_ASC_HINT INDEX_SS_DESC_HINT
LEADING_HINT ORDERED
USE_NL USE_MERGE USE_HASH NO_USE_HASH NO_USE_MERGE NO_USE_NL USE_ADAPTIVE NO_USE_ADAPTIVE
...
//-----------------------------non_reserved keyword begin-------------------------------------------
        ACCESS ACCESS_INFO ACCOUNT ACTION ACTIVE ADDDATE AFTER AGAINST AGGREGATE ALGORITHM ALL_META ALL_USER ALWAYS ANALYSE ANY
        APPROX_COUNT_DISTINCT APPROX_COUNT_DISTINCT_SYNOPSIS APPROX_COUNT_DISTINCT_SYNOPSIS_MERGE
        ARBITRATION ASCII AT ATTRIBUTE AUTHORS AUTO AUTOEXTEND_SIZE AUTO_INCREMENT AUTO_INCREMENT_MODE AUTO_INCREMENT_CACHE_SIZE
        AVG AVG_ROW_LENGTH ACTIVATE AVAILABILITY ARCHIVELOG AUDIT ADMIN
        ...
        EXTENDED_NOADDR EXTENT_SIZE EXTRACT EXCEPT EXPIRED ENCODING EMPTY_FIELD_AS_NULL EXTERNAL EXTERNAL_STORAGE_DEST EXCLUSIVE
        ...
        MIGRATE MIN MIN_CPU MIN_IOPS MINOR MIN_ROWS MINUS MINUTE MODE MODIFY MODULE MONTH MOVE
        ...
        OBSOLETE OBJECT_NAME_DISPLAY OCCUR OF OFF OFFSET OLD OLD_PASSWORD ONE ONE_SHOT ONLY OPEN OPTIONS ORDINALITY ORIG_DEFAULT OWNER OLD_KEY OVER
        OBCONFIG_URL OJ
        OBJECT_ID
        ...
        SIZE SKEWONLY SEQUENCE SLOG STATEMENT_ID SKIP_HEADER SKIP_BLANK_LINES LICENSE SKIP
        ...
        TRANSFER TENANT_STS_CREDENTIAL TABLETS

        UNCOMMITTED UNDEFINED UNDO_BUFFER_SIZE UNDOFILE UNICODE UNINSTALL UNIT UNIT_GROUP UNIT_LIST UNIT_NUM UNLOCKED UNTIL
...
%type <node> column_name relation_name opt_relation_name function_name column_label var_name relation_name_or_string row_format_option
...
%type <node> opt_hint_list hint_option select_with_opt_hint update_with_opt_hint delete_with_opt_hint hint_list_with_end global_hint transform_hint optimize_hint opt_optimize_hints
...
%type <node> path_info opt_access_info opt_attribute
...
stmt:
  ...
  | lock_tables_stmt           { $$ = $1; check_question_mark($$, result); }
  | unlock_tables_stmt         { $$ = $1; check_question_mark($$, result); }
...
| create_with_opt_hint special_table_type TABLE opt_if_not_exists relation_factor IGNORE select_stmt
{
  (void)($1);
  ...
  malloc_non_terminal_node($$, result->malloc_pool_, T_CREATE_TABLE, 10,
                           ...
                           $7,                   /* select_stmt */
                           $1,                   /* hints */
                           ignore_node);         /* opt_ignore_or_replace*/
  $$->reserved_ = 0;
}
| create_with_opt_hint special_table_type TABLE opt_if_not_exists relation_factor REPLACE select_stmt
{
  ParseNode *replace_node = NULL;
  malloc_terminal_node(replace_node, result->malloc_pool_, T_REPLACE);
  malloc_non_terminal_node($$, result->malloc_pool_, T_CREATE_TABLE, 10,
                           $2,                   /* temporary option */
                           $4,                   /* if not exists */
                           $5,                   /* table name */
                           ...
                           $7,                   /* select_stmt */
                           $1,                   /* hints */
                           replace_node);                  /* opt_ignore_or_replace*/
  $$->reserved_ = 0;
}
...
| create_with_opt_hint special_table_type TABLE opt_if_not_exists relation_factor IGNORE AS select_stmt
{
  ParseNode *ignore_node = NULL;
  malloc_terminal_node(ignore_node, result->malloc_pool_, T_IGNORE);
  malloc_non_terminal_node($$, result->malloc_pool_, T_CREATE_TABLE, 10,
...
| create_with_opt_hint special_table_type TABLE opt_if_not_exists relation_factor REPLACE AS select_stmt
{
  ParseNode *replace_node = NULL;
  malloc_terminal_node(replace_node, result->malloc_pool_, T_REPLACE);
  malloc_non_terminal_node($$, result->malloc_pool_, T_CREATE_TABLE, 10,
...
| UNION opt_equal_mark '(' opt_table_list ')'
{
  (void)($2);
  (void)($4);
  $$ = NULL;
...
| GROUP_COMMIT '(' BOOL_VALUE ')'
{
  malloc_non_terminal_node($$, result->malloc_pool_, T_GROUP_COMMIT, 1, $3);
}
;
...
opt_optimize_hints:
opt_optimize_hints optimize_hint
{
  malloc_non_terminal_node($$, result->malloc_pool_, T_LINK_NODE, 2, $1, $2);
}
| /*EMPTY*/
...
| USE_ADAPTIVE '(' qb_name_option relation_factor_in_use_join_hint_list opt_optimize_hints ')'
{
  ParseNode *table_list = NULL;
  ParseNode *opt_hint_list = NULL;
  merge_nodes(table_list, result, T_RELATION_FACTOR_IN_USE_JOIN_HINT_LIST, $4);
  merge_nodes(opt_hint_list, result, T_HINT_OPTION_LIST, $5);
  malloc_non_terminal_node($$, result->malloc_pool_, T_USE_ADAPTIVE, 3, $3, table_list, opt_hint_list);
}
| NO_USE_ADAPTIVE '(' qb_name_option relation_factor_in_use_join_hint_list ')'
{
  ParseNode *table_list = NULL;
  merge_nodes(table_list, result, T_RELATION_FACTOR_IN_USE_JOIN_HINT_LIST, $4);
  malloc_non_terminal_node($$, result->malloc_pool_, T_NO_USE_ADAPTIVE, 2, $3, table_list);
}
...
|
alter_with_opt_hint SYSTEM alter_or_change_or_modify EXTERNAL_STORAGE_DEST path_info opt_set opt_access_info opt_attribute
{
  (void)($1);
  (void)($3);
  (void)($6);
...
|
REPAIR opt_repair_mode TABLE table_list opt_repair_option_list
{
  (void)$2;
  (void)$4;
  (void)$5;
...
|
CHECKSUM TABLE table_list opt_checksum_option
{
  (void)$3;
  (void)$4;
...
| index_name handler_rkey_function {
  (void)($1);
  (void)($2);
  (void)($$);
}
