        ...
        if (OB_UNLIKELY(5 != refresh_info_node->num_child_)) {
          ...
          if (-1 != refresh_method) {
            ...
            if (OB_UNLIKELY(data_version < MOCK_DATA_VERSION_4_3_5_3
              ...
              ret = OB_NOT_SUPPORTED;
              LOG_WARN("tenant data version is less than 4.3.5.3 or between 4.4.0.0 and 4.4.2.0 alter mv nested refresh mode is not supported", K(ret), K(data_version));
  ...
  if (table_schema.required_by_mv_refresh()) {
    ...
    if (T_COLUMN_ADD == type || T_COLUMN_MODIFY == type || T_COLUMN_ALTER == type) {
      ...
      if (OB_FAIL(GET_MIN_DATA_VERSION(table_schema.get_tenant_id(), data_version))) {
        ...
        is_allowed = data_version >= DATA_VERSION_4_4_2_4;
      }
    }
