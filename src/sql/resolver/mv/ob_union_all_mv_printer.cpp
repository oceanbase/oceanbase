  ...
  if (OB_ISNULL(mlog_tables_)) {
    ...
    switch (refresh_type) {
      case OB_MV_FAST_REFRESH_SIMPLE_MAV: {
        ObSimpleMAVPrinter printer(ctx_, mv_schema_, mv_container_schema_, child_sel_stmt, *mlog_tables_);
      ...
      case OB_MV_FAST_REFRESH_SIMPLE_JOIN_MAV: {
        ObSimpleJoinMAVPrinter printer(ctx_, mv_schema_, mv_container_schema_, child_sel_stmt, *mlog_tables_);
        if (OB_FAIL(printer.gen_child_refresh_dmls_for_union_all(dml_stmts))) {
          LOG_WARN("failed to gen child refresh dmls for union all", K(ret));
        }
        break;
      ...
      case OB_MV_FAST_REFRESH_OUTER_JOIN_MAV: {
        ObOuterJoinMAVPrinter printer(ctx_, mv_schema_, mv_container_schema_, child_sel_stmt, *mlog_tables_);
        if (OB_FAIL(printer.gen_child_refresh_dmls_for_union_all(dml_stmts))) {
          LOG_WARN("failed to gen child refresh dmls for union all", K(ret));
        }
        break;
