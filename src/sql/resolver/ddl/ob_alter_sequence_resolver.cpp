...
ObAlterSequenceResolver::ObAlterSequenceResolver(ObResolverParams& params) : ObStmtResolver(params)
{}
...
int ObAlterSequenceResolver::resolve(const ParseNode& parse_tree)
{
  ...
  if (OB_SUCC(ret)) {
    ...
    if (OB_FAIL(resolve_ref_factor(parse_tree.children_[1], session_info_, sequence_name, db_name))) {
      ...
      } else if (OB_FAIL(schema_checker_->get_sequence_id(tenant_id, db_name, sequence_name, sequence_id))) {
        if (ret == OB_ERR_SEQ_NOT_EXIST) {
          if (!allow_inexistence) {
            ret = OB_OBJECT_NAME_NOT_EXIST;
            LOG_WARN("sequence not exists", K(ret), K(sequence_name));
          } else {
            ret = OB_SUCCESS;
  ...
  /* sequence options */
  if (OB_SUCC(ret) && mystmt->get_arg().get_seq_items().size() == 1) {
