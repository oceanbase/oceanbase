...
int ObUserProfileResolver::fill_arg(int64_t type, ObObj &value, obrpc::ObProfileDDLArg &arg)
{
  ...
  if (OB_ISNULL(params_.allocator_)) {
    ...
    if (OB_ISNULL(params_.session_info_)) {
      ...
      LOG_WARN("PASSWORD_ROLLOVER_TIME is not supported in current data version", K(ret), K(compat_version));
      LOG_USER_ERROR(OB_NOT_SUPPORTED, "PASSWORD_ROLLOVER_TIME in current data version");
    }
  }
  if (OB_FAIL(ret)) {
    // do nothing
