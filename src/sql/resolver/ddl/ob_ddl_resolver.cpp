...
int ObDDLResolver::deep_copy_column_expr_name(common::ObIAllocator &allocator, ObIArray<ObRawExpr *> &exprs)
{
  int ret = OB_SUCCESS;
    ...
    for (int64_t i = 0; OB_SUCC(ret) && i < column_exprs.count(); ++i) {
      ...
      if (OB_ISNULL(column_exprs.at(i)) || !column_exprs.at(i)->is_column_ref_expr()) {
        ...
        LOG_WARN("failed to write string");
                     ...
                     ob_write_string(allocator, column_expr->get_database_name(), column_expr->get_database_name()))) {
        LOG_WARN("failed to write string");
      }
    }
  }
