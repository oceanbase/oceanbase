  ...
  if (NULL != def_node) {
    ...
    if (OB_FAIL(ret)) {
      ...
      case T_OP_NEG: {
        ...
        ret = SMART_CALL(resolve_default_value(def_val->children_[0], old_res));
        if (OB_FAIL(ret)) {
          SQL_RESV_LOG(WARN, "Resolve default const value failed", K(ret));
        } else if (!old_res.is_literal_) {
          // do nothing
...
int ObDDLResolver::resolve_table_options(ParseNode *node, bool is_index_option)
{
  ...
  if (NULL != node) {
    ...
    if(T_TABLE_OPTION_LIST != node->type_) {
      ret = OB_ERR_UNEXPECTED;
      SQL_RESV_LOG(WARN, "invalid parse node", K(ret), K(node->type_));
    } else if ( OB_ISNULL(session_info_)) {
      ret = OB_ERR_UNEXPECTED;
      SQL_RESV_LOG(WARN, "session_info_ is null", K(session_info_), K(ret));
    } else {
      ...
      case T_TABLE_MODE: {
        ...
        if (OB_FAIL(GET_MIN_DATA_VERSION(tenant_id, tenant_data_version))) {
          ...
          if (OB_FAIL(ret)) {
            ...
            ret = OB_NOT_SUPPORTED;
            LOG_WARN("moderate/super/extreme table mode is not supported in data version < 4.2.1.5 or 4.2.2 <= data_version < 4.2.3", K(ret), K(table_mode_str), K(tenant_data_version));
...
int ObDDLResolver::resolve_lob_storage_parameters(const ParseNode *node)
{
  ...
  if (OB_ISNULL(schema_checker_) || OB_ISNULL(stmt_) ||
    ...
    ret = OB_NOT_SUPPORTED;
    LOG_USER_ERROR(OB_NOT_SUPPORTED, "tenant version is less than 4.2.2, chunk size attribute");
    ...
    bool is_explicit_order)
{
  ...
  if (OB_SUCC(ret) && index_keyname_ == SPATIAL_KEY) {
    ...
    } else if (column_schema.is_stored_generated_column() && tenant_data_version < DATA_VERSION_4_2_5_1) {
      ret = OB_NOT_SUPPORTED;
      LOG_WARN("tenant version is less than 4.25bp1, spatial index on stored generated column isnot supported", K(ret),
        K(is_geo_column), K(is_spatial_index));
      LOG_USER_ERROR(OB_NOT_SUPPORTED, "tenant version is less than 4.25bp1, spatial index on stored generated column");
