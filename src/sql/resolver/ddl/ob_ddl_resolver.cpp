...
//        }
      ...
      case T_VEC_INDEX_PARAMS: {
        if (index_keyname_ != VEC_KEY) {
          ...
          } else if (OB_FAIL(ObVectorIndexUtil::check_vec_index_in_stmt(*stmt_, vec_index_type_, index_params_))) {
      ...
      case T_PARSER_NAME: {
        if (OB_ISNULL(option_node->children_[0])) {
          ...
          } else if (storage::ObFTParser::is_analyzer_parser_name(parser_name_)) {
            if (tenant_data_version < MIN_DATA_VERSION_FOR_FTS_ANALYZER) {
              ret = OB_NOT_SUPPORTED;
              LOG_WARN("analyzer syntax is not supported before version 4.6.0.1", K(ret));
              LOG_USER_ERROR(OB_NOT_SUPPORTED,
                  "WITH PARSER analyzer before version 4.6.0.1");
            }
