    ...
    common::ObString &parser_property)
{
  ...
  if (OB_UNLIKELY(parse_tree.num_child_ <= 0)) {
    ...
    for (int64_t i = 0; OB_SUCC(ret) && i < parse_tree.num_child_; ++i) {
      ...
      } else if (OB_FAIL(resolve_fts_index_parser_properties(index_database_name, parse_tree.children_[i], tenant_id, property, allocator, schema_checker, analysis_json))) {
        LOG_WARN("fail to resolve fts index parser properties", K(ret));
      }
    }

    if (OB_FAIL(ret)) {
    } else if (!analysis_json.empty() && !property.is_empty()) {
      ret = OB_INVALID_ARGUMENT;
      LOG_WARN("analysis cannot be mixed with other parser properties", K(ret), K(analysis_json));
      LOG_USER_ERROR(OB_INVALID_ARGUMENT,
          "analysis cannot be combined with other PARSER_PROPERTIES options");
    } else if (!analysis_json.empty()) {
      // analysis JSON path: validate and store the raw JSON string
      ObString validated_json;
      if (OB_FAIL(resolve_analysis_json(analysis_json, allocator, validated_json))) {
        LOG_WARN("fail to resolve analysis json", K(ret), K(analysis_json));
      } else {
        parser_property = validated_json;
      }
...
int ObFTParserResolverHelper::resolve_analysis_json(
    const common::ObString &analysis_json,
    common::ObIAllocator &allocator,
    common::ObString &validated_json)
{
  int ret = OB_SUCCESS;
  storage::ObAnalyzerSpec *analyzer_spec = nullptr;

  if (analysis_json.empty()) {
    ret = OB_INVALID_ARGUMENT;
    LOG_WARN("analysis json is empty", K(ret));
    LOG_USER_ERROR(OB_INVALID_ARGUMENT, "analysis JSON must not be empty");
  } else if (OB_FAIL(storage::ObAnalyzerSpecFactory::create_analyzer_spec(
                 analysis_json, allocator, analyzer_spec))) {
    LOG_WARN("fail to create analyzer spec", K(ret), K(analysis_json));
    if (OB_NOT_SUPPORTED == ret) {
      LOG_USER_ERROR(OB_NOT_SUPPORTED, "analyzer type");
    } else {
      LOG_USER_ERROR(OB_INVALID_ARGUMENT, "analysis JSON");
    }
  } else if (OB_FAIL(ob_write_string(allocator, analysis_json, validated_json))) {
    LOG_WARN("fail to deep copy analysis json", K(ret), K(analysis_json));
  }
  storage::ObAnalyzerSpecFactory::destroy_analyzer_spec(allocator, analyzer_spec);
  return ret;
}
    ...
    common::ObString &analysis_json)
{
  ...
  if (OB_ISNULL(node) || node->num_child_ != 1 || OB_ISNULL(node->children_[0])){
    ...
    switch (node->type_) {
      ...
      case T_PARSER_ANALYSIS: {
        if (OB_ISNULL(node->children_[0])) {
          ret = OB_ERR_UNEXPECTED;
          LOG_WARN("option_node child is nullptr", K(ret));
        } else if (OB_UNLIKELY(!analysis_json.empty())) {
          ret = OB_INVALID_ARGUMENT;
          LOG_WARN("duplicate analysis option is not allowed", K(ret), K(analysis_json));
          LOG_USER_ERROR(OB_INVALID_ARGUMENT, "duplicate analysis option is not allowed");
        } else if (OB_UNLIKELY(node->children_[0]->str_len_ <= 0)) {
          ret = OB_INVALID_ARGUMENT;
          LOG_WARN("analysis json string is empty", K(ret));
          LOG_USER_ERROR(OB_INVALID_ARGUMENT, "analysis JSON must not be empty");
        } else {
          int32_t str_len = static_cast<int32_t>(node->children_[0]->str_len_);
          analysis_json.assign_ptr(node->children_[0]->str_value_, str_len);
        }
        break;
      }
