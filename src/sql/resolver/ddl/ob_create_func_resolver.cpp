...
int ObCreateFuncResolver::resolve(const ParseNode& parse_tree)
{
  ...
  if (OB_ISNULL(create_func_node) || T_CREATE_FUNC != create_func_node->type_ || 4 != create_func_node->num_child_ ||
    ...
    if (OB_FAIL(plugin_path.get_string(plugin_path_str))) {
      ...
      create_func_arg.udf_.set_name(
          ObString(create_func_node->children_[1]->str_len_, create_func_node->children_[1]->str_value_));
