...
ObAlterTableStmt::ObAlterTableStmt(common::ObIAllocator *name_pool)
    : ObTableStmt(name_pool, stmt::T_ALTER_TABLE), is_comment_table_(false),
      is_alter_system_(false), fts_arg_allocator_(nullptr),
      is_alter_triggers_(false), interval_expr_for_set_interval_(NULL),
      transition_expr_for_set_interval_(NULL),
      interval_expr_for_add_partition_(NULL),
      transition_expr_for_add_partition_(NULL), alter_table_action_count_(0),
      alter_external_table_type_(0), is_add_interval_partition_(false),
      exchange_db_name_(), exchange_table_name_() {}

ObAlterTableStmt::ObAlterTableStmt()
    : ObTableStmt(stmt::T_ALTER_TABLE), is_comment_table_(false),
      is_alter_system_(false), fts_arg_allocator_(nullptr),
      is_alter_triggers_(false), interval_expr_for_set_interval_(NULL),
      transition_expr_for_set_interval_(NULL),
      interval_expr_for_add_partition_(NULL),
