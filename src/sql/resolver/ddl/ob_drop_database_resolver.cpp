...
ObDropDatabaseResolver::ObDropDatabaseResolver(ObResolverParams& params) : ObDDLResolver(params)
{}
...
int ObDropDatabaseResolver::resolve(const ParseNode& parse_tree)
{
  ...
  if (OB_ISNULL(node) || OB_UNLIKELY(node->type_ != T_DROP_DATABASE) ||
      OB_UNLIKELY(node->num_child_ != DB_NODE_COUNT) || OB_ISNULL(node->children_)) {
    ...
    if (OB_SUCC(ret)) {
      ...
      if (OB_FAIL(session_info_->get_sys_variable(share::SYS_VAR_CHARACTER_SET_SERVER, coll_cs_server_int64))) {
        LOG_WARN("failed to get character_set_server", K(ret));
      } else if (OB_FAIL(session_info_->get_sys_variable(share::SYS_VAR_COLLATION_SERVER, coll_server_int64))) {
        ...
        ret = OB_ERR_UNEXPECTED;
        LOG_WARN("invalid collation type", K(ret), K(coll_cs_server_int64), K(coll_server_int64));
      } else if (OB_FAIL(ObCharset::charset_name_by_coll(
                     static_cast<ObCollationType>(coll_cs_server_int64), server_charset))) {
        LOG_WARN("fail to get charset name by collation type", K(ret), K(coll_cs_server_int64));
      } else {
    ...
    if (OB_SUCC(ret)) {
      ...
      bool drop_purge = (NULL != parse_tree.children_[DROP_PURGE_NODE]) ? true : false;
      if (drop_purge) {
