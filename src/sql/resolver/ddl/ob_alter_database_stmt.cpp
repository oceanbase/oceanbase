...
ObAlterDatabaseStmt::ObAlterDatabaseStmt(common::ObIAllocator* name_pool)
    : ObDDLStmt(name_pool, stmt::T_ALTER_DATABASE), alter_database_arg_()
{}
...
{
  alter_database_arg_.database_schema_.add_zone(zone);
}

int ObAlterDatabaseStmt::set_primary_zone(const common::ObString& zone)
{
  return alter_database_arg_.database_schema_.set_primary_zone(zone);
}
