...
int ObCreateWrappedResolver::verify_plwrap_version_compatible()
{
  ...
  if (OB_SUCC(ret) && DATA_VERSION_4_2_5_7 > tenant_data_version) {
    ...
    if (OB_SUCC(ret) && DATA_VERSION_4_2_5_7 > target_data_version) {
      ret = OB_NOT_SUPPORTED;
      LOG_WARN("target data version is less than 4.2.5.7, wrapped PL/SQL unit is not supported",
               K(ret), K(tenant_data_version));
      LOG_USER_ERROR(
          OB_NOT_SUPPORTED,
          "target data version is less than 4.2.5.7, wrapped PL/SQL unit is not supported");
    }
  }
  return ret;
}
