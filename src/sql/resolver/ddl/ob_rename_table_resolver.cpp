...
ObRenameTableResolver::ObRenameTableResolver(ObResolverParams& params) : ObDDLResolver(params)
{}
...
int ObRenameTableResolver::resolve_rename_action(const ParseNode& rename_action_node)
{
  ...
  if (OB_SUCC(ret)) {
    ...
    if (OB_ISNULL(origin_node) || OB_ISNULL(new_node)) {
      ret = OB_ERR_PARSE_SQL;
      LOG_WARN("origin_node or new node is null", K(origin_node), K(new_node), K(ret));
    ...
    } else if (OB_FAIL(resolve_table_relation_node(origin_node, origin_table_name, origin_db_name))) {
      LOG_WARN("failed to resolve origin table node.", K(origin_table_name), K(origin_db_name), K(ret));
    } else if (OB_FAIL(resolve_table_relation_node(new_node, new_table_name, new_db_name))) {
      LOG_WARN("failed to resolve new table node.", K(new_table_name), K(new_db_name), K(ret));
    } else {
