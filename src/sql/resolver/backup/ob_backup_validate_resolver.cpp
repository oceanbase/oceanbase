...
int ObBackupValidateResolver::resolve(const ParseNode &parse_tree)
{
  ...
  if (OB_UNLIKELY(T_BACKUP_VALIDATE != parse_tree.type_)) {
    ...
    ret = OB_NOT_SUPPORTED;
    LOG_WARN("BACKUP_VALIDATE command is not supported in data version less than 4.4.2.2", KR(ret), K(data_version));
    LOG_USER_ERROR(OB_NOT_SUPPORTED, "BACKUP_VALIDATE command is not supported in data version less than 4.4.2.2");
