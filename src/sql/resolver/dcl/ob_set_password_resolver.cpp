...
int ObSetPasswordResolver::resolve(const ParseNode &parse_tree)
{
  ...
  if (OB_FAIL(GET_MIN_DATA_VERSION(session_info_->get_effective_tenant_id(), compat_version))) {
    ...
    if (OB_SUCC(ret) && OB_NOT_NULL(node->children_[6])) {
      if (!((compat_version >= MOCK_DATA_VERSION_4_4_2_1 && compat_version < DATA_VERSION_4_5_0_0)
            || compat_version >= DATA_VERSION_4_6_1_0)) {
        ret = OB_NOT_SUPPORTED;
        LOG_WARN("RETAIN CURRENT PASSWORD/DISCARD OLD PASSWORD/EXPIRE PASSWORD ROLLOVER PERIOD not supported if data version not in [4.4.2.1,4.5.0.0) or [4.6.1.0, +infinity)", K(ret), K(compat_version));
        LOG_USER_ERROR(OB_NOT_SUPPORTED, "RETAIN CURRENT PASSWORD/DISCARD OLD PASSWORD/EXPIRE PASSWORD ROLLOVER PERIOD if data version not in [4.4.2.1,4.5.0.0) or [4.6.1.0, +infinity)");
      } else {
    ...
    if (OB_SUCC(ret)
        ...
        ((compat_version >= MOCK_DATA_VERSION_4_4_2_1 && compat_version < DATA_VERSION_4_5_0_0)
          || compat_version >= DATA_VERSION_4_6_1_0)) {
    ...
    if (OB_SUCC(ret)) {
      if (OB_FAIL(check_dcl_on_inner_user(node->type_,
        ...
        LOG_WARN("failed to check dcl on inner-user or unsupport to modify reserved user",
                 K(ret),
                 K(params_.session_info_->get_priv_user_id()),
                 K(user_name));
      } else if (OB_SUCC(ret) && ObSchemaChecker::is_ora_priv_check()
