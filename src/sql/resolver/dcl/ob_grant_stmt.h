...
public:
  ...
  ObGrantStmt();
  virtual ~ObGrantStmt();
  int add_user(const common::ObString& user_name, const common::ObString& host_name, const common::ObString& pwd,
  ...
  void set_need_create_user(bool need_create_user)
  {
    need_create_user_ = need_create_user;
  }
  ...
  void set_option(uint64_t option)
  {
    option_ = option;
  }
  ...
  bool get_need_create_user() const
  {
    return need_create_user_;
  }
  bool need_create_user_priv() const
  {
    return need_create_user_priv_;
  }
  const common::ObStrings& get_users() const
  {
    return users_;
  }
  const common::ObSArray<common::ObString>& get_role_names() const
  {
    return grant_arg_.roles_;
  };
  ...
  share::schema::ObPrivLevel get_grant_level() const
  {
    return grant_level_;
  }
  share::schema::ObObjectType get_object_type() const
  {
    return object_type_;
  }
  const ObSEArray<uint64_t, 4> get_ins_col_ids() const
  {
    return ins_col_ids_;
  }
  const ObSEArray<uint64_t, 4> get_upd_col_ids() const
  {
    return upd_col_ids_;
  }
  const ObSEArray<uint64_t, 4> get_ref_col_ids() const
  {
    return ref_col_ids_;
  }
  void set_object_type(share::schema::ObObjectType object_type)
  {
  ...
  void set_ref_query(ObSelectStmt* ref_query)
  {
    ref_query_ = ref_query;
  }
  int add_grantee(const common::ObString& grantee);

  const share::ObRawPrivArray& get_priv_array() const
  {
    return sys_priv_array_;
  }
  const share::ObRawObjPrivArray& get_obj_priv_array() const
  {
    return obj_priv_array_;
  }

  uint64_t get_option() const
  {
    return option_;
  };
  ...
  ObSelectStmt* get_ref_query() const
  {
    return ref_query_;
  }
  ...
  const common::ObStrings& get_grantees() const
  {
    return grantees_;
  }
