...
int ObModuleDataResolver::resolve_target_tenant_id(const ParseNode *node, const table::ObModuleDataArg::ObExecModule mod,
                                                   uint64_t &target_tenant_id)
{
  ...
  if (login_tenant_id != OB_SYS_TENANT_ID) {
  ...
  } else if (lib::Worker::CompatMode::ORACLE == mode && mod != table::ObModuleDataArg::ObExecModule::GIS) {
    ret = OB_NOT_SUPPORTED;
    ...
    ret = OB_NOT_SUPPORTED;
    LOG_USER_ERROR(OB_NOT_SUPPORTED, "target tenant versions lower than 4.2.5.0");
    LOG_WARN("target tenant versions lower than 4.2.5.0 not supported",
...
int ObModuleDataResolver::resolve_file_path(const ParseNode *node, ObString &abs_path)
{
  ...
  if (OB_ISNULL(node)) {
    abs_path.reset();
  } else if (OB_FAIL(Util::resolve_string(node,abs_path))) {
...
int ObModuleDataResolver::resolve(const ParseNode &parse_tree)
{
  ...
  if (OB_ISNULL(stmt)) {
    ...
    ret = OB_NOT_SUPPORTED;
    LOG_WARN("module_data is not supported under cluster version 4.2.5.0", K(ret));
    LOG_USER_ERROR(OB_NOT_SUPPORTED, "module_data in cluster under version 4.2.5.0");
  } else {
    ...
    if (OB_FAIL(resolve_exec_type(parse_tree.children_[TYPE_IDX], arg.op_))) {
    ...
    } else if (OB_FAIL(resolve_target_tenant_id(parse_tree.children_[TENANT_IDX], arg.module_, arg.target_tenant_id_))) {
      ...
      ret = OB_NOT_SUPPORTED;
      LOG_WARN("module_data is not supported with tenant under version 4.2.5.0", K(ret));
      LOG_USER_ERROR(OB_NOT_SUPPORTED, "module_data with tenant under version 4.2.5.0");
