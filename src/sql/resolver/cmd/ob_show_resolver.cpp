  ...
  if (OB_SUCC(ret)) {
    ...
    switch (parse_tree.type_) {
      ...
      case T_SHOW_SENSITIVE_RULES: {
        ...
        if (OB_FAIL(GET_MIN_DATA_VERSION(real_tenant_id, min_version))) {
            LOG_WARN("get min data_version failed", K(ret), K(real_tenant_id));
        } else if ((is_oracle_mode && !((min_version >= DATA_VERSION_4_4_2_0 && min_version < DATA_VERSION_4_5_0_0)))
                   || (!is_oracle_mode
                       && !((min_version >= MOCK_DATA_VERSION_4_3_5_3 && min_version < DATA_VERSION_4_4_0_0)
                            || (min_version >= DATA_VERSION_4_4_2_0 && min_version < DATA_VERSION_4_5_0_0)))) {
          ret = OB_NOT_SUPPORTED;
