...
int ObRawExprResolverImpl::fold_cnn_expr(ObOpRawExpr *ori_cnn_expr, ObOpRawExpr *&folded_cnn_expr)
{
  ...
  if (OB_ISNULL(ori_cnn_expr) || OB_UNLIKELY(2 > ori_cnn_expr->get_param_count())) {
    ...
    const ObExprResType* current_type = NULL;
    ...
    if (OB_FAIL(ret)) {
      ...
      for (int64_t i = 1; OB_SUCC(ret) && i < final_param_exprs.count(); ++i) {
        ...
        } else if (OB_FAIL(new_cnn->set_param_exprs(current, final_param_exprs.at(i)))) {
                                                          ...
                                                          ObIArray<ObRawExpr*> &final_param_exprs)
{
  ...
  if (OB_ISNULL(ori_cnn_expr) || OB_UNLIKELY(0 == same_type_exprs.count())) {
    ...
    for (int64_t i = 0; OB_SUCC(ret) && i < same_type_exprs.count(); ++i) {
      if (OB_FAIL(cnn_expr_with_same_type->add_param_expr(same_type_exprs.at(i)))) {
        LOG_WARN("fail to add param expr to cnn expr with same type", K(ret));
      }
    }
    if (OB_FAIL(ret)) {
