    ...
    if (OB_FAIL(ret)) {
      ...
      if ((T_OP_GET_PACKAGE_VAR == new_expr->get_expr_type() && new_expr->get_param_count() != 5)
        ...
        if (OB_ISNULL(result_type_expr)
          ...
          ObExprResType* orig_result_type = reinterpret_cast<ObExprResType *>(value.get_int());
          ObExprResType* new_result_type = NULL;
          if (OB_ISNULL(orig_result_type)) {
            ret = OB_ERR_UNEXPECTED;
            LOG_WARN("unexpected orig result type pointer", K(ret));
          } else if (OB_ISNULL(new_result_type
                                = reinterpret_cast<ObExprResType*>(
                                    expr_factory_.get_allocator().alloc(sizeof(ObExprResType))))) {
            ret = OB_ALLOCATE_MEMORY_FAILED;
            LOG_WARN("failed to alloc memory for new result type", K(ret));
          } else {
            new (new_result_type) ObExprResType();
 ...
 * @param is_root : whether the expr node is the root of expr tree
 * @return
 */
                                     ...
                                     const bool is_root /*= true*/)
{
  ...
  if (OB_SUCC(ret) && NULL == to_expr) {
    ...
    for (int64_t i = 0; OB_SUCC(ret) && i < to_expr->get_param_count(); ++i) {
      ...
      if (OB_FAIL(SMART_CALL(copy_on_replace(param, new_param, replacer, false)))) {
