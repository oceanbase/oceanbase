...
int ObRawExprWrapEnumSet::wrap_enum_set(ObDMLStmt& stmt)
{
  ...
  if (stmt.is_select_stmt()) {
    ...
    current_level_ = insert_stmt.get_current_level();
...
{
  int ret = OB_SUCCESS;
  common::ObIArray<ObRawExpr*>& column_conv_exprs = stmt.get_column_conv_functions();
  for (int64_t i = 0; OB_SUCC(ret) && i < column_conv_exprs.count(); ++i) {
    ...
    if (OB_ISNULL(conv_expr)) {
      ...
      // do nothing
    } else if (OB_UNLIKELY(conv_expr->get_param_count() != ObExprColumnConv::PARAMS_COUNT_WITH_COLUMN_INFO &&
      ...
      if (OB_UNLIKELY(!conv_expr->get_param_expr(0)->is_const_expr())) {
        ret = OB_ERR_UNEXPECTED;
        LOG_WARN("convert expr have invalid param", K(ret));
      } else if (OB_FAIL(
                     static_cast<ObConstRawExpr*>(conv_expr->get_param_expr(0))->get_value().get_int32(const_value))) {
...
int ObRawExprWrapEnumSet::wrap_value_vector(ObInsertStmt& stmt)
{
  ...
  if ((desc_count > 0)) {
    // Only when there is an enum or set column in value_desc,
    ...
    int64_t vector_count = stmt.get_value_vectors().count();
    const bool is_same_need = true;
    for (int64_t i = 0; OB_SUCC(ret) && need_check && i < vector_count; ++i) {
      ObRawExpr*& value_expr = stmt.get_value_vectors().at(i);
      if (OB_ISNULL(value_expr)) {
        ...
        if (OB_FAIL(wrap_type_to_str_if_necessary(value_expr,
                stmt.get_values_desc().at(index)->get_data_type(),
                stmt.get_current_level(),
                is_same_need,
                new_expr))) {
          LOG_WARN("failed to wrap_type_to_str_if_necessary", K(i), K(ret));
...
int ObRawExprWrapEnumSet::wrap_target_list(ObSelectStmt& select_stmt)
{
  ...
  for (int64_t i = 0; OB_SUCC(ret) && i < select_items.count(); ++i) {
    ...
    if (OB_ISNULL(target_expr)) {
      ...
      ObSysFunRawExpr* new_expr = NULL;
      if (OB_FAIL(ObRawExprUtils::create_type_to_str_expr(
              expr_factory_, target_expr, select_stmt.get_current_level(), new_expr, my_session_, is_type_to_str))) {
...
int ObRawExprWrapEnumSet::analyze_all_expr(ObDMLStmt& stmt)
{
  ...
  current_level_ = stmt.get_current_level();
...
int ObRawExprWrapEnumSet::visit(ObFunMatchAgainst& expr)
{
  UNUSED(expr);
  return OB_SUCCESS;
}
...
int ObRawExprWrapEnumSet::visit(ObOpRawExpr& expr)
{
  ...
  if (!expr.has_enum_set_column()) {
    // not contain enum or set, don't need to do any conversion
    ...
    if (OB_ISNULL(op = expr.get_op())) {
      ...
      if (ObExprOperator::NOT_ROW_DIMENSION != row_dimension) {
        ...
        } else if (left_expr->has_enum_set_column() && OB_FAIL(visit_left_expr(expr, row_dimension, cmp_types))) {
          LOG_WARN("failed to visit left expr", K(expr), K(ret));
        } else if (right_expr->has_enum_set_column() &&
                   OB_FAIL(visit_right_expr(*right_expr, row_dimension, cmp_types))) {
          LOG_WARN("failed to visit right expr", K(expr), K(ret));
        } else { /*do nothing*/
        }
      } else if (expr.get_input_types().count() == expr.get_param_count()) {
        // Handling of scalar cases
...
int ObRawExprWrapEnumSet::visit_left_expr(
    ObOpRawExpr& expr, int64_t row_dimension, const ObIArray<ObExprCalcType>& cmp_types)
{
  int ret = OB_SUCCESS;
  ...
  if (OB_ISNULL(left_expr)) {
    ...
    int64_t idx = 0;
    ObSysFunRawExpr* wrapped_expr = NULL;
    if (OB_FAIL(check_and_wrap_left(*left_expr, idx, cmp_types, row_dimension, get_current_level(), wrapped_expr))) {
    ...
    for (int64_t i = 0; OB_SUCC(ret) && i < left_expr->get_param_count(); ++i) {
      ...
      } else if (OB_FAIL(check_and_wrap_left(
                     *param_expr, i, cmp_types, row_dimension, get_current_level(), wrapped_expr))) {
        LOG_WARN("failed to check_and_wrap_left", K(ret));
      } else if (NULL != wrapped_expr) {
    ...
    if (OB_ISNULL(left_ref)) {
      ...
      if (OB_ISNULL(ref_stmt)) {
        ...
        ret = OB_ERR_UNEXPECTED;
        LOG_WARN("select item size and row_dimension should be equal",
            "slect_item_size",
        ...
        for (int64_t i = 0; OB_SUCC(ret) && i < ref_stmt->get_select_item_size(); ++i) {
          ...
          if (OB_ISNULL(target_expr)) {
            ...
            LOG_WARN("expr of select item is NULL", K(left_expr), K(ret));
                         ...
                         *target_expr, i, cmp_types, row_dimension, get_current_level() + 1, wrapped_expr))) {
            LOG_WARN("failed to check_and_wrap_left", K(ret));
          } else if (NULL != wrapped_expr) {
...
int ObRawExprWrapEnumSet::check_and_wrap_left(ObRawExpr& expr, int64_t idx, const ObIArray<ObExprCalcType>& cmp_types,
    int64_t row_dimension, int32_t expr_level, ObSysFunRawExpr*& wrapped_expr) const
{
  int ret = OB_SUCCESS;
  wrapped_expr = NULL;
  ...
  if (OB_UNLIKELY(idx >= row_dimension)) {
    ...
    for (int64_t i = 0; OB_SUCC(ret) && i < target_num && !(need_numberic && need_varchar); ++i) {
      ...
      if (OB_FAIL(ObRawExprUtils::need_wrap_to_string(
              expr_type, cmp_types.at(row_dimension * i + idx).get_type(), is_same_type_need, need_wrap))) {
    ...
    if (OB_SUCC(ret) && need_varchar) {
      ...
      if (OB_FAIL(ObRawExprUtils::create_type_to_str_expr(
              expr_factory_, &expr, expr_level, wrapped_expr, my_session_, is_type_to_str))) {
...
int ObRawExprWrapEnumSet::visit_right_expr(
    ObRawExpr& right_expr, int64_t row_dimension, const ObIArray<ObExprCalcType>& cmp_types)
{
  int ret = OB_SUCCESS;
  ...
  if (OB_UNLIKELY(row_dimension < 1)) {
      ...
      for (int64_t i = 0; OB_SUCC(ret) && i < right_op_expr.get_param_count(); ++i) {
        ...
        if (OB_ISNULL(param_expr)) {
          ...
          if (OB_UNLIKELY(row_dimension != param_expr->get_param_count())) {
            ret = OB_ERR_UNEXPECTED;
            LOG_WARN("pram_count should be equal to row_dimension",
                K(row_dimension),
                "param_count",
                param_expr->get_param_count(),
                K(ret));
          } else {
            ...
            for (int64_t j = 0; OB_SUCC(ret) && j < param_op_expr->get_param_count(); ++j) {
              ...
              ObSysFunRawExpr* wrapped_expr = NULL;
              if (OB_FAIL(wrap_type_to_str_if_necessary(
                      inner_param_expr, calc_type, get_current_level(), is_same_need, wrapped_expr))) {
                LOG_WARN("failed to wrap_type_to_str_if_necessary", K(i), K(ret));
    ...
    if (OB_ISNULL(ref_stmt)) {
      ...
      for (int64_t i = 0; OB_SUCC(ret) && i < row_dimension; ++i) {
        ...
        ObObjType calc_type = cmp_types.at(i).get_type();
        if (OB_FAIL(wrap_type_to_str_if_necessary(
                target_expr, calc_type, get_current_level() + 1, is_same_need, wrapped_expr))) {
          LOG_WARN("failed to wrap_type_to_str_if_necessary", K(i), K(ret));
        } else if (NULL != wrapped_expr) {
          ref_stmt->get_select_item(i).expr_ = wrapped_expr;
...
int ObRawExprWrapEnumSet::wrap_type_to_str_if_necessary(
    ObRawExpr* expr, ObObjType dest_type, int32_t expr_level, bool is_same_need, ObSysFunRawExpr*& wrapped_expr)
{
  int ret = OB_SUCCESS;
  bool need_wrap = false;
  const bool is_type_to_str = true;
  ...
  if (OB_ISNULL(expr)) {
    ret = OB_ERR_UNEXPECTED;
    LOG_WARN("expr is NULL", K(ret));
  } else if (OB_FAIL(ObRawExprUtils::need_wrap_to_string(expr->get_data_type(), dest_type, is_same_need, need_wrap))) {
    LOG_WARN("failed to check_need_wrap_to_string", K(ret));
  } else if (need_wrap && OB_FAIL(ObRawExprUtils::create_type_to_str_expr(
                              expr_factory_, expr, expr_level, wrapped_expr, my_session_, is_type_to_str))) {
    LOG_WARN("failed to create_type_to_string_expr", KPC(expr), K(is_type_to_str), K(ret));
  } else {
    LOG_DEBUG("finish wrap_type_to_str_if_necessary",
        K(ret),
        K(need_wrap),
        K(dest_type),
        KPC(expr),
...
int ObRawExprWrapEnumSet::visit(ObCaseOpRawExpr& expr)
{
  ...
  if (OB_UNLIKELY(T_OP_CASE != expr.get_expr_type() && T_OP_ARG_CASE != expr.get_expr_type())) {
    ...
    if (T_OP_ARG_CASE == expr.get_expr_type()) {
      ...
      if (OB_ISNULL(arg_param_expr)) {
        ...
        for (int64_t i = 0; OB_SUCC(ret) && i < expr.get_when_expr_size(); ++i) {
          ...
          bool need_wrap = false;
          if (OB_FAIL(wrap_type_to_str_if_necessary(
                  when_expr, calc_type, get_current_level(), is_same_need, wrapped_expr))) {
            LOG_WARN("failed to wrap_type_to_str_if_necessary", K(i), K(ret));
        ...
        if (OB_SUCC(ret) && ob_is_enumset_tc(arg_type) && need_varchar) {
          ...
          wrapped_expr = NULL;
          if (OB_FAIL(ObRawExprUtils::create_type_to_str_expr(
                  expr_factory_, arg_param_expr, get_current_level(), wrapped_expr, my_session_, is_type_to_str))) {
      ...
      for (int64_t i = 0; OB_SUCC(ret) && i < expr.get_when_expr_size(); i++) {
        ...
        ObRawExpr* when_expr = expr.get_when_param_expr(i);
        if (OB_FAIL(
                wrap_type_to_str_if_necessary(when_expr, calc_type, get_current_level(), is_same_need, wrapped_expr))) {
          LOG_WARN("failed to wrap_type_to_str_if_necessary", K(i), K(ret));
        } else if (NULL != wrapped_expr && OB_FAIL(expr.replace_when_param_expr(i, wrapped_expr))) {
          LOG_WARN("failed to replace_when_param_expr", K(i), K(ret));
        } else { /*do nothing*/
    ...
    for (int64_t i = 0; OB_SUCC(ret) && i < expr.get_then_expr_size(); i++) {
      ...
      wrapped_expr = NULL;
      if (OB_FAIL(
              wrap_type_to_str_if_necessary(then_expr, result_type, get_current_level(), is_same_need, wrapped_expr))) {
        LOG_WARN("failed to wrap_type_to_str_if_necessary", K(i), K(ret));
    ...
    if (OB_SUCC(ret) && expr.get_default_param_expr() != NULL) {
      ...
      wrapped_expr = NULL;
      if (OB_FAIL(wrap_type_to_str_if_necessary(
              default_expr, result_type, get_current_level(), is_same_need, wrapped_expr))) {
  ...
  if (expr.has_enum_set_column() && (T_FUN_GROUP_CONCAT == expr.get_expr_type() ||
    ...
    for (int64_t i = 0; OB_SUCC(ret) && i < real_parm_exprs.count(); ++i) {
      ...
      if (OB_ISNULL(real_param_expr)) {
        ...
        ObSysFunRawExpr* wrapped_expr = NULL;
        if (OB_FAIL(wrap_type_to_str_if_necessary(
                real_param_expr, calc_type, get_current_level(), is_same_need, wrapped_expr))) {
          LOG_WARN("failed to wrap_type_to_str_if_necessary", K(i), K(ret));
...
int ObRawExprWrapEnumSet::visit(ObSysFunRawExpr& expr)
{
  ...
  if (OB_UNLIKELY(NULL == op)) {
    ...
    ret = OB_ERR_UNEXPECTED;
    LOG_WARN("param_count is not equal to input_types_count", K(ret));
  } else if (expr.has_enum_set_column() && !expr.is_type_to_str_expr()) {
    ...
    for (int i = 0; i < param_count && OB_SUCC(ret); i += 2) {
      ...
      if (OB_ISNULL(left_param) || OB_ISNULL(right_param)) {
        ...
        if (ob_is_enumset_tc(left_param->get_data_type())) {
          ObObjType calc_type = expr.get_input_types().at(i).get_calc_type();
          const bool is_same_need = false;
          const bool is_type_to_str = false;
          bool need_wrap = false;
          ObSysFunRawExpr* wrapped_expr = NULL;
          if (OB_FAIL(ObRawExprUtils::need_wrap_to_string(
                  left_param->get_data_type(), calc_type, is_same_need, need_wrap))) {
            LOG_WARN("failed to check if need_wrap", K(ret));
          } else if (need_wrap &&
                     OB_FAIL(ObRawExprUtils::create_type_to_str_expr(
                         expr_factory_, left_param, get_current_level(), wrapped_expr, my_session_, is_type_to_str))) {
            LOG_WARN("failed to create_type_to_string_expr", K(ret));
          } else if ((NULL != wrapped_expr) && OB_FAIL(expr.replace_param_expr(i, wrapped_expr))) {
        ...
        if (OB_SUCC(ret) && ob_is_enumset_tc(right_param->get_data_type())) {
          ...
          const bool is_same_need = false;
          if (OB_FAIL(wrap_type_to_str_if_necessary(
                  right_param, calc_type, get_current_level(), is_same_need, wrapped_expr))) {
            LOG_WARN("failed to wrap_type_to_str_if_necessary", K(ret));
          } else if ((NULL != wrapped_expr) && OB_FAIL(expr.replace_param_expr(i + 1, wrapped_expr))) {
