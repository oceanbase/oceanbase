                                                                    ...
                                                                    ObRawExpr *&generated_ref_expr)
{
  ...
  for (int i = 0; OB_SUCC(ret) && i < generated_column_info.dependcy_column_infos_.count(); i++) {
    ...
    } else if (OB_FAIL(init_column_ref_expr(dependcy_column_info, column_expr))) {
                                                     ...
                                                     ObColumnRefRawExpr *&column_expr)
{
  ...
  if (OB_ISNULL(column_info) || OB_ISNULL(column_expr)) {
    ...
    if (OB_SUCC(ret) && column_info->is_enum_or_set()) {
      if (OB_FAIL(column_expr->set_enum_set_values(column_info->extended_type_info_))) {
                                                              ...
                                                              ObRawExpr *&generated_ref_expr)
{
  ...
  if (OB_FAIL(resolve_sql_mode_from_solidified_var(generated_column_info, sql_mode))) {
    ...
    ret = OB_ERR_UNEXPECTED;
    LOG_WARN("column_expr is null");
  } else if (OB_FAIL(init_column_ref_expr(&generated_column_info, generated_column_expr))) {
...
int ObCalcGeneratedColumn::create_ctx_before_calc()
{
  ...
  if (OB_UNLIKELY(NULL == (exec_ctx_buf = inner_alloc_.alloc(sizeof(ObExecContext))))) {
    ret = OB_ALLOCATE_MEMORY_FAILED;
    ...
    exec_ctx_->get_task_executor_ctx()->set_min_cluster_version(CLUSTER_VERSION_4_2_3_0);
