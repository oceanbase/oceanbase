  ...
  if (OB_SUCC(ret)) {
    ...
    if (1 != tas.count() && share::is_oracle_mode()) {
      ...
      for (int j = 0; j < tas.count(); ++j) {
        ObTableAssignment& ta = tas.at(j);
        if (OB_INVALID_ID == ta.table_id_) {
          ret = OB_ERR_UNEXPECTED;
          LOG_ERROR("invalid table assignment", K(ta.table_id_));
        } else if (OB_FAIL(resolve_check_constraints(update_stmt->get_table_item_by_id(ta.table_id_)))) {
          ...
          for (uint64_t i = cst_count; OB_SUCC(ret) && i < update_stmt->get_check_constraint_exprs_size(); ++i) {
            if (OB_FAIL(ObTableAssignment::expand_expr(
                    ta.assignments_, update_stmt->get_check_constraint_exprs().at(i)))) {
              LOG_WARN("expand generated column expr failed", K(ret));
            }
          }
          cst_count = update_stmt->get_check_constraint_exprs_size();
        }
      }
    }
