    ...
    ObArray<ObQualifiedName> *output_columns/* = NULL*/)
{
  ...
  if (OB_SUCC(ret)) {
    ...
    ctx.is_need_print_ = params_.need_print_stmt();
...
int ObDMLResolver::resolve_columns(ObRawExpr *&expr, ObArray<ObQualifiedName> &columns)
{
      ...
      if (ret == OB_ERR_BAD_FIELD_ERROR) {
        if (i + 1 < columns.count() && columns.at(i + 1).access_idents_.count() > 0
          ...
          if (is_cloumn_ref && ObSequenceNamespaceChecker::is_curr_or_next_val(access_name)) {
            ...
            resolve_seq_funcs = true;
            if (OB_FAIL(resolve_qualified_identifier(q_name, columns, real_exprs, real_ref_expr, true))) {
  ...
  if (OB_FAIL(ret)) {
  } else if (!expr->is_column_ref_expr() && !resolve_seq_funcs) {
    // do nothing
                                                      ...
                                                      const ObTableSchema &table_schema)
{
  ...
  bool enable_nested_mview = false;
  if (OB_UNLIKELY((OB_E(EventTable::EN_MVIEW_DEFENSIVE_CHECK) OB_SUCCESS) != OB_SUCCESS)) {
    enable_nested_mview = true;
  } else if (tenant_config.is_valid()) {
    enable_nested_mview = tenant_config->_enable_mv_nested;
  }
  ...
  if (OB_SUCC(ret)) {
    if (!stmt->is_select_stmt() && OB_NOT_NULL(time_node)) {
      ...
      case T_RELATION_FACTOR: {
        if (parse_tree.value_ == T_EXTERNAL_FILE_LOCATION) {  // 临时外表
                                      ...
                                      TableItem &table_item)
{
  ...
  if (NULL != part_node) {
    ...
    if (T_NAME_LIST == part_node->children_[0]->type_) {
      if (OB_FAIL(ObResolverUtils::resolve_partition_hints(
              *part_node, table_schema, table_item.part_ids_, table_item.part_names_))) {
        LOG_WARN("failed to resolve partition hints", K(ret));
      }
    ...
    ObDMLStmt *stmt /* = NULL */)
{
  ...
  bool is_support_reuse_cid_expr = ((GET_MIN_CLUSTER_VERSION() >= MOCK_CLUSTER_VERSION_4_3_5_5 && GET_MIN_CLUSTER_VERSION() < CLUSTER_VERSION_4_4_0_0) || GET_MIN_CLUSTER_VERSION() >= CLUSTER_VERSION_4_4_2_0);
  if (OB_SUCC(ret)) {
