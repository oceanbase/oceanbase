...
int ObGlobalHint::print_global_hint(PlanText &plan_text) const
{
  ...
  if (OB_SUCC(ret) && (has_valid_opt_features_version())) {
    int64_t cur_pos = 0;
    // if enabled trace point outline valid check tp_no = 551 and opt_features_version_ is DEFAULT_USED_VERSION,
    // just print OPTIMIZER_FEATURES_ENABLE('') to avoid mysqltest changed repeatedly after upgrade DEFAULT_USED_VERSION
