...
int ObTscCgService::generate_tsc_ctdef(ObLogTableScan &op, ObTableScanCtDef &tsc_ctdef)
{
  ...
  if (OB_SUCC(ret)) {
    ...
    DASScanCGCtx cg_ctx;
    if (OB_UNLIKELY(op.use_index_merge() || op.has_es_match() || op.get_is_hybrid_search())) {
  ...
  if (OB_SUCC(ret) && op.get_is_hybrid_search()) {
    tsc_ctdef.is_hybrid_search_ = true;
  }

  // Assign all TABLE SCAN OPERATOR LEVEL information before this line,
  // to ensure correct execution of the hybrid search code generation.
  bool need_attach = false;
  if (OB_FAIL(ret)) {
  } else if (op.get_is_hybrid_search()) {
    if (OB_FAIL(hybrid_search_cg_service_.generate_hybrid_search_ctdef(op, tsc_ctdef, root_ctdef))) {
      LOG_WARN("failed to generate hybrid search ctdef", K(ret));
    } else {
      need_attach = true;
    }
  } else if (OB_FAIL(generate_attach_ctdef(op, tsc_ctdef, root_ctdef, need_attach))) {
    LOG_WARN("failed to generate attach ctdef", K(ret));
  }

  if (OB_SUCC(ret) && need_attach) {
    const ObPushDownTopNInfo &push_down_top_n_info = op.get_push_down_top_n_info();
    ObDASPushDownTopN &push_down_topn = tsc_ctdef.scan_ctdef_.push_down_topn_;
    ...
    if (push_down_top_n_info.is_push_into_index_) {
      if (OB_NOT_NULL(push_down_top_n_info.limit_count_expr_) &&
          OB_FAIL(cg_.generate_rt_expr(*push_down_top_n_info.limit_count_expr_, push_down_topn.limit_expr_))) {
        LOG_WARN("failed to generate rt limit expr", K(ret));
      } else if (OB_NOT_NULL(push_down_top_n_info.limit_offset_expr_) &&
                OB_FAIL(cg_.generate_rt_expr(*push_down_top_n_info.limit_offset_expr_, push_down_topn.offset_expr_))) {
        LOG_WARN("failed to generate rt offset expr", K(ret));
      } else if (OB_NOT_NULL(push_down_top_n_info.sort_key_.expr_)) {
        if (OB_FAIL(cg_.generate_rt_expr(*push_down_top_n_info.sort_key_.expr_, push_down_topn.sort_key_))) {
          LOG_WARN("failed to generate rt sort key expr", K(ret));
        } else {
          push_down_topn.order_type_ = push_down_top_n_info.sort_key_.order_type_;
        }
      }
      if (OB_SUCC(ret)) {
        push_down_topn.with_ties_ = push_down_top_n_info.with_ties_;
        push_down_topn.is_push_into_index_ = push_down_top_n_info.is_push_into_index_;
      }
    }
  }
...
int ObTscCgService::generate_attach_ctdef(
  ObLogTableScan &op,
  ...
  bool &need_attach)
{
  ...
  if (OB_FAIL(ret)) {
    ...
    if (OB_UNLIKELY(root_ctdef != &tsc_ctdef.scan_ctdef_)) {
      ret = OB_ERR_UNEXPECTED;
      LOG_WARN("unexpected error, root ctdef isn't equal to scan ctdef", K(ret));
    } else if (OB_FAIL(generate_das_scan_ctdef_with_domain_id(op, tsc_ctdef, &tsc_ctdef.scan_ctdef_, domain_id_merge_ctdef))) {
      LOG_WARN("fail to generate das scan ctdef with doc id", K(ret));
    } else {
      tsc_ctdef.scan_ctdef_.multivalue_idx_ = op.get_multivalue_col_idx();
      tsc_ctdef.scan_ctdef_.multivalue_type_ = op.get_multivalue_type();
      root_ctdef = domain_id_merge_ctdef;
      need_attach = true;
    }
  }
  ...
  if (OB_SUCC(ret) && OB_NOT_NULL(op.get_vector_index_info()) && op.get_vector_index_info()->is_hnsw_vec_scan()) {
...
int ObTscCgService::generate_tsc_filter(const ObLogTableScan &op, ObTableScanSpec &spec)
{
  ...
  if (OB_FAIL(ret)) {
    ...
    if (OB_ISNULL(attach_ctdef)) {
      ret = OB_ERR_UNEXPECTED;
      LOG_WARN("unexpected nullptr attach ctdef", K(ret));
    } else if (is_dsl_query) {
      if (!op.get_filter_exprs().empty()) {
        ret = OB_ERR_UNEXPECTED;
        LOG_WARN("dsl query should not have additional filters", K(ret), K(op.get_ref_table_id()));
      }
    } else if (ObDASOpType::DAS_OP_INDEX_PROJ_LOOKUP == attach_ctdef->op_type_) {
      ret = OB_INVALID_ARGUMENT;
      LOG_WARN("invalid index proj lookup ctdef in hybrid search with SQL query", K(ret));
    } else if (OB_FAIL(op.extract_nonpushdown_filters(op.get_filter_exprs(), nonpushdown_filters, lookup_pushdown_filters))) {
      LOG_WARN("failed to extract non pushdown filters", K(ret));
    } else {
      // Only prepare nonpushdown filters for SQL query, lookup pushdown filters are handled in @generate_main_scan_ctdef.
      lookup_pushdown_filters.reset();
    }
    ...
    LOG_WARN("generate pd storage flag for scan ctdef failed", K(ret));
      ...
      OB_FAIL(generate_pd_storage_flag(op,
                                       op.get_ref_table_id(),
                                       op.get_access_exprs(),
                                       op.get_type(),
  ...
  if (OB_SUCC(ret) && !lookup_pushdown_filters.empty() && lookup_ctdef != nullptr) {
  ...
  if (OB_SUCC(ret) && !nonpushdown_filters.empty()) {
    ObArray<ObRawExpr *> filters_without_match;
    ObArray<ObRawExpr *> filter_column_raw_exprs;
    ObArray<ObExpr *> filter_column_exprs;
    if (OB_FAIL(cg_.generate_rt_exprs(nonpushdown_filters, spec.filters_))) {
      ...
      // skip check for oracle mapping real virtual table
    } else if (OB_FAIL(filters_without_match.assign(nonpushdown_filters))) {
      LOG_WARN("failed to assign nonpushdown filters", K(ret));
    } else if (OB_FAIL(filter_out_match_exprs(filters_without_match))) {
      LOG_WARN("failed to filter out match exprs", K(ret));
    } else if (OB_FAIL(ObRawExprUtils::extract_column_exprs(filters_without_match, filter_column_raw_exprs))) {
      LOG_WARN("failed to extract column exprs from nonpushdown filters", K(ret));
    } else if (OB_FAIL(cg_.generate_rt_exprs(filter_column_raw_exprs, filter_column_exprs))) {
      LOG_WARN("failed to generate rt exprs", K(ret));
    } else {
  ...
  LOG_TRACE("generate tsc filter", K(ret), K(op.is_hybrid_search()), K(op.use_index_merge()),
    K(op.get_real_ref_table_id()), K(nonpushdown_filters), K(scan_pushdown_filters), K(lookup_pushdown_filters));
  return ret;
}


int ObTscCgService::generate_pd_storage_flag(const ObLogTableScan &op,
                                             ...
                                             ObDASScanCtDef &scan_ctdef)
{
  ...
  if (is_sys_table(ref_table_id) || is_virtual_table(ref_table_id)) {
    // do nothing
  } else if (OB_FAIL(cg_.generate_tsc_flags(op, scan_ctdef))) {
                                                 ...
                                                 ObIArray<ObRawExpr*> &access_exprs)
{
  ...
  if (op.is_vec_idx_scan() && OB_ISNULL(vc_info)) {
    ret = OB_ERR_UNEXPECTED;
    LOG_WARN("unexpected null vector index info", K(ret), KP(vc_info));
  }
  const bool is_text_retrieval_aux_type = (!op.is_vec_idx_scan() || (OB_NOT_NULL(vc_info) && !vc_info->is_vec_aux_table_id(scan_table_id))) &&
  ...
  const bool is_doc_id_index_back = (!op.is_vec_idx_scan() || (OB_NOT_NULL(vc_info) && !vc_info->is_vec_aux_table_id(scan_table_id))) && op.need_doc_id_index_back() && scan_table_id == op.get_doc_id_index_table_id();
  if (OB_FAIL(ret)) {
                                             ...
                                             ObIArray<ObRawExpr*> &access_exprs)
{
  ...
  if (OB_FAIL(ret)) {
    ...
    if (cg_ctx.non_ror_filters_ == nullptr) {
    ...
    if (OB_SUCC(ret)) {
      ...
      ObArray<ObRawExpr *> lookup_pushdown_filters;
      if (cg_ctx.non_ror_filters_ != nullptr) {
        if (OB_FAIL(scan_pushdown_filters.assign(*cg_ctx.non_ror_filters_))) {
          LOG_WARN("failed to assign non ror filters", K(ret));
        }
      } else if (use_index_merge) {
        if (OB_FAIL(scan_pushdown_filters.assign(op.get_index_filters(scan_ctdef.index_merge_idx_)))) {
          LOG_WARN("failed to assign index merge filters", K(ret));
        }
      } else if (OB_FAIL(const_cast<ObLogTableScan &>(op).extract_pushdown_filters(nonpushdown_filters,
                                                                            scan_pushdown_filters,
                                                                            lookup_pushdown_filters))) {
        LOG_WARN("extract pushdown filters failed", K(ret));
      }
                                            ...
                                            bool &has_rowscn)
{
  ...
  if (OB_SUCC(ret) && scan_ctdef.is_index_merge_) {
    ...
    } else if (cg_ctx.non_ror_filters_ != nullptr) {
      if (OB_FAIL(scan_pushdown_filters.assign(*cg_ctx.non_ror_filters_))) {
        LOG_WARN("failed to assign non ror filters", K(ret));
      }
                                                  ...
                                                  ObDASScanCtDef *&block_max_scan_ctdef)
{
  ...
  if (OB_ISNULL(vc_info)) {
    ret = OB_ERR_UNEXPECTED;
    LOG_WARN("unexpected null vector index info", K(ret), KP(vc_info));
  } else if (OB_FAIL(generate_vec_aux_table_ctdef(op, spiv_scan_type, vc_info->get_aux_table_id(ObVectorAuxTableIdx::VEC_FIRST_AUX_TBL_IDX), spiv_scan_ctdef))) {
                                           ...
                                           ObDASBaseCtDef *&root_ctdef)
{
  ...
  if (OB_ISNULL(op.get_vector_index_info())) {
    ret = OB_ERR_UNEXPECTED;
    ...
    if (OB_ISNULL(schema_guard)) {
      ...
      if (OB_FAIL(schema_guard->get_table_schema(op.get_vector_index_info()->get_aux_table_id(hybrid_embedded_tid), hybrid_embedded_table_schema))) {
    ...
    if (OB_SUCC(ret)) {
      ...
      if (OB_FAIL(generate_vec_aux_idx_tbl_ctdef(op, first_aux_ctdef, second_aux_ctdef, third_aux_ctdef, fourth_aux_ctdef, fifth_aux_ctdef))) {
      ...
      if (OB_SUCC(ret)) {
        ...
        if (OB_ISNULL(vec_scan_ctdef->children_ = OB_NEW_ARRAY(ObDASBaseCtDef*, &ctdef_alloc, vec_child_task_cnt))) {
          ...
          vec_scan_ctdef->strategy_ = vc_info.strategy_;
          vec_scan_ctdef->pre_filtering_timeout_ = vc_info.pre_filtering_timeout_;
                                           ...
                                           ObDASBaseCtDef *&root_ctdef)
{
  ...
  if (OB_FAIL(ret)) {
    ...
    bool push_down_topk = tr_info.need_block_max_scan() ||
                          op.get_push_down_top_n_info().is_push_into_index_;

    if (OB_SUCC(ret) && push_down_topk) {
    ...
    if (OB_SUCC(ret)) {
      ...
      if (OB_ISNULL(ir_scan_ctdef->children_ = OB_NEW_ARRAY(ObDASBaseCtDef*, &ctdef_alloc, ir_scan_children_cnt))) {
        ...
        if (use_approx_pre_agg) {
          ...
          ir_scan_ctdef->has_block_max_scan_ = push_down_topk;
        } else {
          ir_scan_ctdef->children_[0] = inv_idx_scan_ctdef;
          ir_scan_ctdef->children_[1] = inv_idx_agg_ctdef;
          ir_scan_ctdef->children_[2] = doc_agg_ctdef;
  ...
  if (OB_SUCC(ret)) {
    ...
    if (OB_FAIL(generate_text_ir_spec_exprs(tr_info, op.get_push_down_top_n_info(), *ir_scan_ctdef))) {
  ...
  if (OB_SUCC(ret) && tr_info.need_sort() && !cg_ctx.is_es_match_) {
    ObSEArray<OrderItem, 2> order_items;
                                                    ...
                                                    ObDASIndexMergeCtDef *&root_ctdef)
{
  ...
  if (OB_ISNULL(node) || OB_UNLIKELY(!node->is_merge_node())) {
    ...
    if (OB_FAIL(ObDASTaskFactory::alloc_das_ctdef(DAS_OP_INDEX_MERGE, alloc, merge_ctdef))) {
      ...
      bool has_dynamic_id_filter = false;
      for (int64_t i = 0; OB_SUCC(ret) && i < children_cnt; ++i) {
        ...
        if (OB_ISNULL(child)) {
          ...
          has_dynamic_id_filter |= child->has_dynamic_id_filter_;
          ObDASScanCtDef *scan_ctdef = nullptr;
      ...
      if (OB_SUCC(ret) && !node->non_ror_filters_.empty()) {
        // generate das scan ctdef for main scan to calc non ror filters
        ObDASScanCtDef *scan_ctdef = nullptr;
        if (OB_FAIL(ObDASTaskFactory::alloc_das_ctdef(DAS_OP_TABLE_SCAN, alloc, scan_ctdef))) {
          LOG_WARN("failed to allocate scan ctdef", K(ret));
        } else if (OB_ISNULL(scan_ctdef)) {
          ret = OB_ERR_UNEXPECTED;
          LOG_WARN("unexpected null scan ctdef", K(ret));
        } else {
          scan_ctdef->ref_table_id_ = op.get_ref_table_id();
          scan_ctdef->is_index_merge_ = true;
          cg_ctx.non_ror_filters_ = &node->non_ror_filters_;
          if (OB_FAIL(generate_das_scan_ctdef(op, cg_ctx, *scan_ctdef, has_rowscn))) {
            LOG_WARN("failed to generate das scan ctdef", KPC(scan_ctdef), K(ret));
          } else {
            merge_ctdef->main_scan_ctdef_ = scan_ctdef;
          }
          cg_ctx.non_ror_filters_ = nullptr;
        }
      }
      ...
      if (OB_SUCC(ret) && OB_NOT_NULL(merge_ctdef)) {
        ...
        if (OB_FAIL(rowkey_exprs.assign(op.get_rowkey_exprs()))) {
          ...
          merge_ctdef->has_dynamic_id_filter_ = has_dynamic_id_filter;
    ...
    ObIArray<ObRawExpr*> &access_exprs)
{
  ...
  if (OB_ISNULL(vec_info)) {
    ret = OB_ERR_UNEXPECTED;
    LOG_WARN("unexpected null vector index info", K(ret), KP(vec_info));
  } else if (scan_ctdef.ref_table_id_ == op.get_doc_id_index_table_id()) {
    ...
    switch (scan_ctdef.ir_scan_type_) {
      ...
      case ObTSCIRScanType::OB_VEC_SNAPSHOT_SCAN: {
        if (OB_FAIL(add_var_to_array_no_dup(access_exprs, static_cast<ObRawExpr*>(vec_info->get_aux_table_column(HNSW_SNAPSHOT_KEY_COL))))) {
          ...
          LOG_WARN("failed to add document length column to access exprs", K(ret));
        } else if (vec_info->has_get_visible_column() &&
                   OB_FAIL(add_var_to_array_no_dup(access_exprs, static_cast<ObRawExpr*>(vec_info->get_aux_table_column(HNSW_SNAPSHOT_VISIBLE_COL))))) {
          LOG_WARN("failed to add document length column to access exprs", K(ret));
        }
        break;
      }
                                              ...
                                              ObIArray<ObRawExpr*> &access_exprs)
{
  ...
  if (OB_ISNULL(vec_info)) {
    ret = OB_ERR_UNEXPECTED;
    LOG_WARN("unexpected null vector index info", K(ret), KP(vec_info));
  } else switch (scan_ctdef.ir_scan_type_) {
    case ObTSCIRScanType::OB_VEC_IVF_CENTROID_SCAN: {
  ...
  if (OB_ISNULL(vec_info)) {
    ...
    case ObTSCIRScanType::OB_VEC_SNAPSHOT_SCAN: {
      if (OB_ISNULL(vec_info->get_aux_table_column(HNSW_SNAPSHOT_KEY_COL))
        ...
        || (vec_info->has_get_visible_column() && OB_ISNULL(vec_info->get_aux_table_column(HNSW_SNAPSHOT_VISIBLE_COL)))) {
        ret = OB_ERR_UNEXPECTED;
        LOG_WARN("unexpected null column", K(ret), KP(vec_info->get_aux_table_column(HNSW_SNAPSHOT_KEY_COL)),
                                                      ...
                                                      KP(vec_info->get_aux_table_column(HNSW_SNAPSHOT_VISIBLE_COL)));
      } else if (OB_FAIL(output_cids.push_back(
      ...
      } else if (vec_info->has_get_visible_column() && OB_FAIL(output_cids.push_back(
          static_cast<ObColumnRefRawExpr *>(vec_info->get_aux_table_column(HNSW_SNAPSHOT_VISIBLE_COL))->get_column_id()))) {
        LOG_WARN("failed to push output snapshot_data col id", K(ret));
      }
      break;
    }
                                                         ...
                                                         ObIArray<uint64_t> &output_cids)
{
  ...
  } else if (OB_FAIL(vec_info->check_vec_aux_column_is_all_inited(is_column_all_inited))) {
                                                  ...
                                                  ObIArray<ObExpr*> &result_output)
{
  ...
  if (OB_ISNULL(op.get_vector_index_info())) {
    ret = OB_ERR_UNEXPECTED;
    LOG_WARN("unexpected null vector index info", K(ret));
  } else if (op.get_vector_index_info()->get_extra_info_columns_count() != vec_ir_scan_ctdef.extra_column_count_) {
    ret = OB_ERR_UNEXPECTED;
    LOG_WARN("extra info column count not match", K(ret), K(op.get_vector_index_info()->get_extra_info_columns_count()), K(vec_ir_scan_ctdef.extra_column_count_));
  } else {
    const ObVecIndexInfo &vec_info = *op.get_vector_index_info();
                                                ...
                                                ObDASIRScanCtDef &text_ir_scan_ctdef)
{
  ...
  if (OB_SUCC(ret) && (tr_info.need_block_max_scan() || push_down_topn_info.is_push_into_index_)) {
    bool is_skip_index_valid = false;
    if (tr_info.need_block_max_scan() && OB_FAIL(check_skip_index_validity(tr_info, is_skip_index_valid))) {
      LOG_WARN("failed to check skip index validity", K(ret));
    } else if (tr_info.need_block_max_scan() && !is_skip_index_valid) {
      text_ir_scan_ctdef.has_block_max_scan_ = false;
    } else if (OB_FAIL(generate_text_block_max_scan_ctdef(tr_info, push_down_topn_info, text_ir_scan_ctdef))) {
...
int ObTscCgService::check_skip_index_validity(const ObTextRetrievalInfo &tr_info, bool &is_valid) const
{
  ...
  // } else
  if (OB_FAIL(schema_guard->get_table_schema(tr_info.inv_idx_tid_, inv_idx_schema))) {
                                                       ...
                                                       ObDASIRScanCtDef &text_ir_scan_ctdef)
{
  ...
  } else if (tr_info.need_block_max_scan()) {
    if (OB_FAIL(cg_.generate_rt_expr(*tr_info.topk_limit_expr_, text_ir_scan_ctdef.topk_limit_expr_))) {
  ...
  } else if (push_down_topn_info.is_push_into_index_) {
    if (OB_FAIL(cg_.generate_rt_expr(*push_down_topn_info.limit_count_expr_, text_ir_scan_ctdef.topk_limit_expr_))) {
      LOG_WARN("cg rt expr for topk limit expr failed", K(ret));
    } else if (nullptr != push_down_topn_info.limit_offset_expr_ &&
        OB_FAIL(cg_.generate_rt_expr(*push_down_topn_info.limit_offset_expr_, text_ir_scan_ctdef.topk_offset_expr_))) {
      LOG_WARN("cg rt expr for topk offset expr failed", K(ret));
    }
  }
                                                  ...
                                                  ObExpr* vid_col)
{
  ...
  if (OB_ISNULL(schema_guard) || OB_ISNULL(vec_info)) {
    ret = OB_ERR_UNEXPECTED;
    LOG_WARN("unexpected nullptr to schema guard or vector index info", K(ret), KP(schema_guard), KP(vec_info));
                                               ...
                                               ObDASBaseCtDef *&root_ctdef)
{
  ...
  if (OB_SUCC(ret) && match_tr_infos.count() > 0 && match_tr_infos.at(0).need_sort()) {
    ObDASSortCtDef *sort_ctdef = nullptr;
    ObSEArray<OrderItem, 2> order_items;
    if (OB_FAIL(order_items.push_back(match_tr_infos.at(0).sort_key_))) {
      LOG_WARN("append order item array failed", K(ret));
    } else if (OB_FAIL(generate_das_sort_ctdef(
        order_items,
        match_tr_infos.at(0).with_ties_,
        match_tr_infos.at(0).topk_limit_expr_,
        match_tr_infos.at(0).topk_offset_expr_,
        root_ctdef,
        sort_ctdef))) {
      LOG_WARN("generate sort ctdef failed", K(ret));
    } else {
...
int ObTscCgService::remove_virtual_generated_access_exprs(
  const ObLogTableScan &op,
  ObStaticEngineCG &cg,
  ObIArray<ObRawExpr*> &access_exprs,
  const bool is_vec_com_aux_scan)
{
  int ret = OB_SUCCESS;
  ObArray<ObRawExpr *> tmp_access_exprs;
  ObOptimizerContext *opt_ctx = nullptr;
  ObSchemaGetterGuard *schema_guard = nullptr;
  if (OB_ISNULL(opt_ctx = cg.opt_ctx_) ||
      OB_ISNULL(schema_guard = opt_ctx->get_schema_guard())) {
    ret = OB_ERR_UNEXPECTED;
    LOG_WARN("unexpected nullptr opt ctx or schema guard", K(ret), K(opt_ctx), K(schema_guard));
  }
  for (int64_t i = 0; OB_SUCC(ret) && i < access_exprs.count(); ++i) {
    ObRawExpr *expr = access_exprs.at(i);
    if (OB_ISNULL(expr)) {
      ret = OB_ERR_UNEXPECTED;
      LOG_WARN("unexpected null access expr", K(ret), K(i));
      ...
      if (OB_FAIL(schema_guard->get_table_schema(MTL_ID(), op.get_ref_table_id(), table_schema))) {
        ...
        bool need_add = false;
        for (int64_t i = 0; i < domain_tids.count() && !need_add; i++) {
