...
public:
  ...
  bool has_active_autocommit_trans(transaction::ObTransID &trans_id) const;
  bool has_active_local_started_trans(transaction::ObTransID & trans_id) const;
...
//  int store_sys_var(int64_t store_idx, share::ObBasicSysVar *sys_var);
  ...
  virtual int64_t get_truncated_sql_len(const ObString &stmt);
protected:
