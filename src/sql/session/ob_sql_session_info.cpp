...
int ObSQLSessionInfo::serialize(char *buf, const int64_t buf_len, int64_t &pos) const
{
  int ret = OB_SUCCESS;
  ...
  if (OB_SUCC(ret)) {
    ...
    if (OB_FAIL(serialize_(buf, buf_len, pos))) {
      ...
      int64_t tmp_pos = 0;
      CHECK_SERIALIZE_SIZE(ObSQLSessionInfo, serial_size);
...
int ObSQLSessionInfo::serialize_(char *buf, const int64_t buf_len, int64_t &pos) const
{
  int ret = OB_SUCCESS;
...
int ObSQLSessionInfo::deserialize(const char *buf, const int64_t data_len, int64_t &pos)
{
  ...
  if (OB_SUCC(ret)) {
    ...
    LST_DO_CODE(OB_UNIS_DECODE,
        ...
        gtt_trans_scope_ids_);
    if (version == 3) {
      int64_t mock_affect_rows = 0;
      OB_UNIS_DECODE(mock_affect_rows);
    }
                                            ...
                                            int32_t v_len)
{
  ...
  sess_diag_info_[type][index].set_sess_id(get_server_sid());
