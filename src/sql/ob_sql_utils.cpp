    ...
    if (stmt->is_dml_stmt()) {
      ...
      if (OB_SUCC(ret) && OB_INVALID_ID == final_choosed_group_id) {
        ...
        if (OB_FAIL(G_RES_MGR.get_mapping_rule_mgr().get_group_id_by_user(
          ...
          // if not set consumer_group for current user, use OBCG_DEFAULT by default.
          final_choosed_group_id = share::OBCG_DEFAULT;
        }
      }
      // if worker had been scheduled into LQ from DEFAULT as a large query,
      // use OBCG_LQ as the final chosen group id.
      if (OB_SUCC(ret)) {
                                                ...
                                                bool &is_hive_table)
{
  ...
  if (GET_MIN_CLUSTER_VERSION() < CLUSTER_VERSION_4_4_2_2) {
    is_hive_table = false;
