...
int ObSQLUtils::is_charset_data_version_valid(ObCharsetType charset_type, const int64_t tenant_id)
{
  ...
  if (OB_FAIL(GET_MIN_DATA_VERSION(tenant_id, data_version))) {
    ...
    ret = OB_NOT_SUPPORTED;
    SQL_LOG(WARN, "charset not supported when data_version < 4_2_5_0 ",K(charset_type), K(ret));
    LOG_USER_ERROR(OB_NOT_SUPPORTED, "tenant data version is less than 4.2.5, charset is");
  }
  return ret;
}
                                   ...
                                   bool force_copy_extend_type)
{
  ...
  CREATE_WITH_TEMP_CONTEXT(param) {
    ...
    int tmp_ret = OB_E(EventTable::EN_DISABLE_CONST_EXPR_CALC_OPT) OB_SUCCESS;
      ...
      if (OB_SUCC(ret)) {
        ...
        if (OB_SUCC(ret)) {
          ...
          if (obj_params.at(obj_params.count() - 1).get_meta().is_null()) {
            OX (obj_params.at(obj_params.count() - 1).set_meta_type(coll->get_element_type().get_meta_type()));
          }
        }
      }
    }
