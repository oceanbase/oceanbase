...
int ObDASDeleteOp::init_task_info()
{
  ...
  if (!write_buffer_.is_inited()
      && OB_FAIL(write_buffer_.init(op_alloc_, DAS_ROW_EXTEND_SIZE, tenant_id_, "DASDeleteBuffer"))) {
...
int ObDASDeleteOp::write_row(const ExprFixedArray &row, ObEvalCtx &eval_ctx, bool &buffer_full, ObChunkDatumStore::StoredRow* &stored_row)
{
  int ret = OB_SUCCESS;
  bool added = false;
  buffer_full = false;
