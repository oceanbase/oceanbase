...
int ObDASGroupFoldIter::inner_get_next_rows(int64_t &count, int64_t capacity)
{
  ...
  if (available_group_idx_ > cur_group_idx_) {
    ret = OB_ITER_END;
    LOG_INFO("available_group_idx > cur_group_idx, no available rows", K_(available_group_idx), K_(cur_group_idx));
  } else {
  ...
  if (OB_SUCC(ret)) {
    if (available_group_idx_ == cur_group_idx_) { // there are rows available in row_store_.
      ...
      LOG_INFO("all new rows from storage layer have greater group idx", K_(available_group_idx), K_(cur_group_idx));
      ret = OB_ITER_END;
    }
  }
  count = ret_count;
