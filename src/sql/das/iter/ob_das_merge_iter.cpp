...
int ObDASMergeIter::create_das_task(const ObDASTabletLoc *tablet_loc, ObDASScanOp *&scan_op, bool &reuse_op)
{
  ...
  // when the cluster version is less than 4.2.3, a DAS_OP_TABLE_BATCH_SCAN task is sent on group rescan situtations
  // for compatibility considerations.
  ObDASOpType op_type = (nullptr != group_id_expr_ && GET_MIN_CLUSTER_VERSION() < CLUSTER_VERSION_4_2_3_0) ? DAS_OP_TABLE_BATCH_SCAN : DAS_OP_TABLE_SCAN;
  reuse_op = false;
  ...
  if (das_tasks_arr_.count() == 1) {
    ...
    if (OB_SUCC(ret)) {
      if (output_idx == OB_INVALID_INDEX) {
        ...
        int64_t ret_count = std::min(store_rows.row_cnt_with_cur_group_idx(), capacity);
