...
private:
  ...
  // When the location cache of a stale GTT v2 tablet has expired, location resolution fails
  // before a DAS task can be created. Verify that the tablet mapping has been deleted, remove
  // the stale session-map entry, and keep the original location error for statement retry.
  int try_remove_stale_gtt_session_tablet_on_location_error_(
