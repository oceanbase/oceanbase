...
namespace oceanbase
{
  ...
  namespace sql
  {
    ...
    {
      int ret = OB_SUCCESS;
      if (OB_ISNULL(opt_stat_mgr) || OB_ISNULL(transport) || OB_ISNULL(vt_partition_service))
      {
    ...
    int ObSql::fill_result_set(ObResultSet &result_set, ObSqlCtx *context, const bool is_ps_mode, ObStmt &basic_stmt)
    {
      ...
      else
      {
        ...
        case stmt::T_DELETE:
        {
          ...
          for (int64_t i = 0; OB_SUCC(ret) && i < size; i++)
          {
            ...
            if (OB_SUCC(ret))
            {
              ...
              if (ob_is_string_or_lob_type(static_cast<ObObjType>(expr->get_data_type())) || ob_is_raw(static_cast<ObObjType>(expr->get_data_type())) || ob_is_enum_or_set_type(static_cast<ObObjType>(expr->get_data_type())))
              {
          ...
          for (int64_t i = 0; OB_SUCC(ret) && i < size; ++i)
          {
            ...
            if (OB_SUCC(ret))
            {
              ...
              if (OB_NOT_NULL(type))
              {
                ...
                // Setup Collation and Collation levl
                if (ob_is_string_or_lob_type(out_obj_type) || ob_is_raw(out_obj_type) || ob_is_enum_or_set_type(out_obj_type))
                {
              ...
              else if (ObCharType == out_obj_type || ObVarcharType == out_obj_type || ob_is_nstring_type(out_obj_type))
              {
                if (-1 == field.accuracy_.get_length())
                {
                  field.length_ = ObCharType == out_obj_type ? OB_MAX_ORACLE_CHAR_LENGTH_BYTE : OB_MAX_ORACLE_VARCHAR_LENGTH;
                }
                else
                {
                  field.length_ = field.accuracy_.get_length();
        ...
        const int64_t question_marks_count = pctx->get_is_ps_rewrite_sql() ? pctx->get_orig_question_mark_cnt() : stmt->get_query_ctx()->get_prepare_param_count();
        if (OB_SUCC(ret) && is_ps_mode && question_marks_count > 0)
                                      ...
                                      ObStmt &basic_stmt, ObField &field)
    {
      ...
      { //  for select into, no rows return
        // do nothing.
      }
      else
      {
        ...
        for (int64_t i = 0; OB_SUCC(ret) && i < size; ++i)
        {
          ...
          {
            if (ob_is_string_or_lob_type(expr->get_data_type()) && CS_TYPE_BINARY != expr->get_collation_type() && ObCharset::is_valid_collation(collation_type))
            {
          ...
          {
            if ((expr->is_query_ref_expr() && static_cast<ObQueryRefRawExpr *>(expr)->is_cursor()) || (expr->is_udf_expr() && static_cast<ObUDFRawExpr *>(expr)->get_is_return_sys_cursor()))
            {
          ...
          if (OB_SUCC(ret))
          {
            ...
            if (expr->is_column_ref_expr() || T_FUN_SYS_CALC_UROWID == expr->get_expr_type() || T_FUN_SET_TO_STR == expr->get_expr_type() || T_FUN_ENUM_TO_STR == expr->get_expr_type())
            {
                                                     ...
                                                     static_cast<ObColumnRefRawExpr *>(expr)->get_column_name())))
              {
                ...
                { // mock empty column expr for generate table
                  is_contain_column_ref = true;
                               ...
                               int32_t returning_into_parm_num)
    {
      ...
      else
      {
        ...
        if (OB_FAIL(ret) || duplicate_prepare)
        { // dec ref count
                               ...
                               bool is_inner_sql)
    {
      ...
      ParseMode parse_mode = context.is_dbms_sql_ ? DBMS_SQL_MODE : (context.is_dynamic_sql_ || !is_inner_sql) ? DYNAMIC_SQL_MODE
      ...
      if (OB_FAIL(ret))
      {
               ...
               || (stmt::T_ANONYMOUS_BLOCK == stmt_type && context.is_prepare_protocol_ && context.is_prepare_stage_ && context.is_pre_execute_))
      {
        param_cnt = parse_result.question_mark_ctx_.count_;
        normalized_sql = sql;
        if (stmt::T_ANONYMOUS_BLOCK == stmt_type && context.is_prepare_protocol_ && context.is_prepare_stage_ && context.is_pre_execute_)
        {
      ...
      else
      {
        ...
        else if (ObStmt::is_dml_stmt(stmt_type) && !ObStmt::is_show_stmt(stmt_type) && !is_inner_sql && !is_from_pl && !(ObStmt::is_dml_write_stmt(stmt_type) && // returning into from oci not need parameterization
        ...
        else if (stmt::T_CALL_PROCEDURE == basic_stmt->get_stmt_type() && FALSE_IT(result.set_cmd(dynamic_cast<ObICmd *>(basic_stmt))))
        {
        }
...
#define NEED_CHECK_SESS_MAX_PS_HANDLE_LIMIT(v) (0 == v ? false : true)
      ...
      if (OB_SUCC(ret))
      {
        ...
        else
        {
          ...
          else if (!is_inner_sql && NEED_CHECK_SESS_MAX_PS_HANDLE_LIMIT(open_cursors_limit) && cur_ps_handle_size >= open_cursors_limit)
          {
    ...
    {
      int ret = OB_SUCCESS;
      if (lib::is_oracle_mode() && ((param.is_varchar() && 0 == param.get_varchar().length()) || (param.is_char() && 0 == param.get_char().length()) || (param.is_nstring() && 0 == param.get_string_len())))
      {
        ...
        else
        {
          ...
          else if (stmt::T_ANONYMOUS_BLOCK == stmt_type && !context.is_pre_execute_)
          {
            ...
            else
            {
              ...
              {
                if (ObRoutineParamInOut::SP_PARAM_INOUT == static_cast<ObRoutineParamInOut>(param_fields.at(i).inout_mode_) || ObRoutineParamInOut::SP_PARAM_OUT == static_cast<ObRoutineParamInOut>(param_fields.at(i).inout_mode_))
                {
                  field_column_cnt++;
                }
              }
          ...
          else
          {
            ...
            ObParser parser(allocator, session.get_sql_mode(),
                            session.get_local_collation_connection());
            ParseResult parse_result;
            ParseMode parse_mode = context.is_dbms_sql_ ? DBMS_SQL_MODE : context.is_dynamic_sql_ ? DYNAMIC_SQL_MODE
                                   ...
                                   ObCacheObjGuard &guard)
    {
      ...
      { //没有从plan cache中拿到plan, 走长路径生成plan
    ...
    OB_INLINE int ObSql::handle_text_query(const ObString &stmt, ObSqlCtx &context, ObResultSet &result)
    {
      ...
      else
      {
        ...
        {
          //是否为COMMIT语句
          is_begin_commit_stmt = (0 == STRNCASECMP(trimed_stmt.ptr(), "commit", 6) && !context.multi_stmt_item_.is_batched_multi_stmt());
        }
        else if (trimed_stmt.length() == 5)
        {
          is_begin_commit_stmt = (0 == STRNCASECMP(trimed_stmt.ptr(), "begin", 5) && !context.multi_stmt_item_.is_batched_multi_stmt());
        }
      ...
      int tmp_ret = ret;
      if (!is_begin_commit_stmt && GCONF.enable_perf_event && OB_FAIL(handle_large_query(tmp_ret, result, ectx.get_need_disconnect_for_update(), ectx)))
      {
        // do nothing
      }
      if (OB_SUCC(ret) && !result.get_is_from_plan_cache())
      { //没有从plan cache中拿到plan, 走长路径生成plan
      ...
      if (OB_SUCC(ret))
      {
        if (!context.is_prepare_protocol_ && OB_FAIL(after_get_plan(*pc_ctx, session, result.get_physical_plan(),
                             ...
                             ParseResult *outline_parse_result)
    {
      ...
      if (OB_SUCC(ret))
      {
        ...
        resolver_ctx.external_param_info_.by_name_ = parse_result.question_mark_ctx_.by_name_ || NULL != context.secondary_namespace_; // static sql in PL must be by name
      ...
      else if (OB_ISNULL(parse_result.result_tree_) || OB_ISNULL(parse_result.result_tree_->children_) || OB_ISNULL(parse_result.result_tree_->children_[0]))
      {
        ret = OB_INVALID_ARGUMENT;
      ...
      if (OB_SUCC(ret))
      {
        ...
        if (stmt::T_ANONYMOUS_BLOCK == context.stmt_type_ && context.is_prepare_protocol_ && !context.is_prepare_stage_ && !context.is_pre_execute_)
        {
          ParseNode tmp_node;
        ...
        else
        {
          ...
          if (NULL != stmt && NULL != resolver_ctx.query_ctx_)
          {
            ...
            bool in_pl = NULL != resolver_ctx.secondary_namespace_ || (resolver_ctx.is_dynamic_sql_ && OB_NOT_NULL(result.get_session().get_pl_context())) || resolver_ctx.is_dbms_sql_;
                                      ...
                                      ParseResult *outline_parse_result /* null */)
    {
      ...
      if (OB_FAIL(sanity_check(sql_ctx)))
      { // check sql_ctx.session_info_ and sql_ctx.schema_guard_
        LOG_WARN("Failed to do sanity check", K(ret));
      }
      ...
      else if (!is_begin_commit_stmt && OB_FAIL(ObPrivilegeCheck::check_privilege_new(sql_ctx,
                                                                                      basic_stmt,
                                                                                      stmt_need_privs,
                                                                                      stmt_ora_need_privs)))
      {
      ...
      if (OB_SUCC(ret))
      {
        if (basic_stmt->is_dml_stmt() || basic_stmt->is_explain_stmt() || basic_stmt->is_help_stmt())
        {
        ...
        else
        {
          ...
          else
          {
            ...
            if (!is_begin_commit_stmt && OB_FAIL(fill_result_set(result, &sql_ctx, is_ps_mode, *basic_stmt)))
            {
              LOG_WARN("Failed to fill result set", K(ret));
            }
          }
        ...
        ObPhysicalPlan *&phy_plan)
    {
      ...
      ObPhysicalPlanCtx *pctx = result.get_exec_context().get_physical_plan_ctx();
      bool use_jit = false;
      if (OB_ISNULL(stmt) || OB_ISNULL(logical_plan) || OB_ISNULL(stmt->get_query_ctx()) || OB_ISNULL(phy_plan) || OB_ISNULL(sql_ctx.session_info_) || OB_ISNULL(pctx))
      {
        ret = OB_INVALID_ARGUMENT;
      ...
      else
      {
        ...
        else
        {
          ...
          if (stmt->is_insert_stmt())
          {
            ...
            phy_plan->set_is_plain_insert(!insert_stmt->value_from_select() && !insert_stmt->is_insert_up() && insert_stmt->get_subquery_exprs().empty() && !insert_stmt->is_replace());
          }
                           ...
                           bool &need_disconnect)
    {
      ...
      {
        if (OB_EAGAIN == ret || OB_REACH_MAX_CONCURRENT_NUM == ret || OB_ARRAY_BINDING_ROLLBACK == ret || OB_ERR_PROXY_REROUTE == ret || OB_BATCHED_MULTI_STMT_ROLLBACK == ret)
        {
          /*do nothing*/
        }
      ...
      { // get plan 成功
        ...
        else
        {
          ...
          if (!session->has_user_super_privilege() && !pc_ctx.sql_ctx_.is_remote_sql_ && GCONF.enable_perf_event)
          {
            ...
            if (ObStmt::is_write_stmt(plan->get_stmt_type(), false) && OB_FAIL(pc_ctx.sql_ctx_.schema_guard_->verify_read_only(
                                ...
                                bool &is_enable_transform_tree)
    {
      ...
      else
      {
        ...
        else if (pc_ctx.is_ps_mode_ && OB_FAIL(construct_param_store_from_ps_param(pc_ctx, pctx->get_param_store_for_update())))
        {
          LOG_WARN("construct param store failed", K(ret));
        }
        if (OB_SUCC(ret))
        ...
        else if (OB_LIKELY(OB_ERR_PARSE_SQL == ret || OB_ERR_EMPTY_QUERY == ret || OB_SIZE_OVERFLOW == ret || OB_ERR_ILLEGAL_NAME == ret || OB_ERR_STR_LITERAL_TOO_LONG == ret || OB_ERR_NOT_VALID_ROUTINE_NAME == ret || OB_ERR_CONSTRUCT_MUST_RETURN_SELF == ret || OB_ERR_ONLY_FUNC_CAN_PIPELINED == ret || OB_ERR_NO_ATTR_FOUND == ret || OB_ERR_NON_INT_LITERAL == ret))
        {
        ...
        if (OB_SUCC(ret))
        {
          ...
          else if (OB_ISNULL(parse_result.result_tree_->children_) || OB_UNLIKELY(parse_result.result_tree_->num_child_ < 1))
          {
            ret = OB_INVALID_ARGUMENT;
          ...
          else
          {
            ...
            if (OB_ISNULL(children_node))
            {
              ...
              //除了普通的dml stmt,explain stmt中存在?也需要这里一起判断
                     ...
                     (children_node->type_ == T_EXPLAIN || IS_DML_STMT(children_node->type_)) && (children_node->value_ > 0))
            {
      ...
      else
      {
        ...
        bool flag = false;
        if ((add_plan_to_pc && !is_show_variables) || ((T_CREATE_OUTLINE == parse_result.result_tree_->children_[0]->type_ || T_ALTER_OUTLINE == parse_result.result_tree_->children_[0]->type_) && (INT64_MAX != parse_result.result_tree_->children_[0]->value_)))
        {
          flag = true;
        ...
        if (flag)
        {
          bool is_transform_outline = (T_CREATE_OUTLINE == parse_result.result_tree_->children_[0]->type_ || T_ALTER_OUTLINE == parse_result.result_tree_->children_[0]->type_);
                           ...
                           bool &plan_added)
    {
      ...
      else if (OB_FAIL(result.to_plan(pc_ctx.is_ps_mode_, phy_plan)))
      {
        LOG_WARN("Failed copy field to pplan", K(ret));
                                       ...
                                       pc_ctx.is_ps_mode_ ? pc_ctx.raw_sql_ : pc_ctx.sql_ctx_.spm_ctx_.bl_key_.constructed_sql_,
      ...
      else
      {
        ...
        {
          if (REACH_TIME_INTERVAL(1000000))
          { // 1s, 当内存达到上限时, 该日志打印会比较频繁, 所以以1s为间隔打印
        ...
        else if (OB_FAIL(ret))
        {
          ...
          else if (OB_REACH_MAX_CONCURRENT_NUM != ret)
          {                   //如果是达到限流上限, 则将错误码抛出去
                              ...
                              const ParamStore *ps_params)
    {
      ...
      if (NULL != pctx)
      {
        if (NULL != phy_plan)
        {
          ...
          if (OB_SUCC(ret) && phy_plan->is_remote_plan() && !phy_plan->contains_temp_table() && !enable_send_plan)
          {
            //处理远程plan转发SQL的情况
            ...
            else if (phy_plan->temp_sql_can_prepare() && pc_ctx.neg_param_index_.is_empty() && !pc_ctx.sql_ctx_.multi_stmt_item_.is_batched_multi_stmt())
            {
        ...
        {
          if (phy_plan->is_contain_oracle_session_level_temporary_table() || phy_plan->contains_temp_table())
          {
                                                     ...
                                                     bool &is_valid)
    {
      ...
      if (add_plan_to_pc || (ObSQLUtils::is_enable_explain_batched_multi_statement() && T_EXPLAIN == type))
      {
        is_valid = true;
                                ...
                                int err_code)
    {
      ...
      if (OB_SUCCESS == err_code && parse_result.result_tree_->children_[0]->type_ == T_SP_CALL_STMT)
      {
        signature_sql = pc_ctx.sql_ctx_.spm_ctx_.bl_key_.constructed_sql_;
      }
      else if (add_plan_to_pc == false || pc_ctx.is_ps_mode_ || OB_SUCCESS != err_code)
      {
    ...
    int ObSql::check_need_reroute(ObPlanCacheCtx &pc_ctx, ObPhysicalPlan *plan, bool &need_reroute)
    {
      ...
      if (OB_NOT_NULL(plan) && pc_ctx.sql_ctx_.can_reroute_sql_ && (OB_PHY_PLAN_REMOTE == plan->get_plan_type() || (!das_ctx.is_partition_hit() && !das_ctx.get_table_loc_list().empty())))
      {
        // reroute request,
             ...
             i++)
        {
          ...
          if (TABLE_SCHEMA == schema_obj.get_schema_type() && is_virtual_table(schema_obj.object_id_))
          {
