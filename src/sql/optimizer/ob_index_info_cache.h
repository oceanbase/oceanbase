...
public:
  QueryRangeInfo()
      : is_valid_(false),
        ...
        range_columns_()
  {}
  const ObQueryRange* get_query_range() const
  {
    return query_range_;
  }
  const ObQueryRangeArray& get_ranges() const
  {
    return ranges_;
  }
  const common::ObIArray<ColumnItem>& get_range_columns() const
  {
    return range_columns_;
  }
  uint64_t get_equal_prefix_count() const
  {
    return equal_prefix_count_;
  };
  uint64_t get_equal_prefix_null_count() const
  {
    return equal_prefix_null_count_;
  };
  uint64_t get_range_prefix_count() const
  {
    return range_prefix_count_;
  };
  uint64_t get_index_column_count() const
  {
    return index_column_count_;
  };
  bool get_contain_always_false() const
  {
    return contain_always_false_;
  }

  ObQueryRangeArray& get_ranges()
  {
    return ranges_;
  }
  void set_query_range(ObQueryRange* query_range)
  {
    query_range_ = query_range;
  }
  common::ObIArray<ColumnItem>& get_range_columns()
  {
    return range_columns_;
  }
  void set_valid()
...
public:
  ...
  const common::ObIArray<ObRawExpr*>& get_index_keys() const
  {
    return index_keys_;
  }
  const common::ObIArray<ObRawExpr*>& get_ordering() const
  {
    return ordering_;
  }

  common::ObIArray<ObRawExpr*>& get_index_keys()
  {
    return index_keys_;
  }
  common::ObIArray<ObRawExpr*>& get_ordering()
  {
    return ordering_;
  }
  void set_scan_direction(const ObOrderDirection direction)
  {
...
public:
  ...
  {}
  QueryRangeInfo& get_range_info()
  {
    return range_info_;
  }
  const QueryRangeInfo& get_range_info() const
  {
    return range_info_;
  }
  OrderingInfo& get_ordering_info()
  {
    return ordering_info_;
  }
  const OrderingInfo& get_ordering_info() const
  {
    return ordering_info_;
  }
  int64_t get_interesting_order_info() const
  {
    return interesting_order_info_;
  }
  void set_interesting_order_info(int64_t info)
  {
    interesting_order_info_ = info;
  }
  int64_t get_interesting_order_prefix_count() const
  {
    return interesting_order_prefix_count_;
  }
  void set_interesting_order_prefix_count(int64_t count)
  {
    interesting_order_prefix_count_ = count;
  }
  uint64_t get_index_id() const
  {
    return index_id_;
  }
  void set_index_id(const uint64_t index_id)
  {
    index_id_ = index_id;
  }
  ...
  bool is_valid_unique_index() const
  {
    return is_unique_index_ &&
           !((lib::is_oracle_mode() && range_info_.equal_prefix_all_null()) ||
               (lib::is_mysql_mode() && (range_info_.equal_prefix_has_null() || !range_info_.is_index_column_get())));
  }
  void set_is_unique_index(const bool is_unique_index)
  {
    is_unique_index_ = is_unique_index;
  }
  bool is_index_back() const
  {
    return is_index_back_;
  }
  void set_is_index_back(const bool is_index_back)
  {
    is_index_back_ = is_index_back;
  }
  bool is_index_global() const
  {
    return is_index_global_;
  }
  void set_is_index_global(const bool is_index_global)
  {
...
public:
  ...
  int get_query_range(const uint64_t table_id, const uint64_t index_id, const QueryRangeInfo*& query_range_info) const;
  int get_access_path_ordering(
      const uint64_t table_id, const uint64_t index_id, const OrderingInfo*& ordering_info) const;
  int get_index_info_entry(const uint64_t table_id, const uint64_t index_id, IndexInfoEntry*& entry) const;
  void set_table_id(const uint64_t table_id)
  {
    table_id_ = table_id;
  }
  void set_base_table_id(const uint64_t base_table_id)
  {
