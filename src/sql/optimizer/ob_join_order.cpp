                                     ...
                                     bool is_adaptive)
{
  ...
  const LogTableHint *aj_table_hint = NULL;
  if (is_adaptive) {
     const ObLogPlanHint &log_hint = get_plan()->get_log_plan_hint();
     const LogJoinHint *log_join_hint = log_hint.get_join_hint(get_tables());
     if (NULL != log_join_hint) {
      aj_table_hint = &log_join_hint->aj_scan_hint_;
     }
  }
  if (OB_ISNULL(get_plan()) ||
      OB_ISNULL(stmt = get_plan()->get_stmt()) ||
                                     ...
                                     const LogTableHint *aj_table_hint)
{
  ...
  const LogTableHint *log_table_hint = aj_table_hint;
  if (OB_ISNULL(get_plan()) ||
      OB_ISNULL(stmt = get_plan()->get_stmt()) ||
      OB_ISNULL(schema_guard = OPT_CTX.get_sql_schema_guard())) {
    ret = OB_INVALID_ARGUMENT;
    ...
    if (OB_FAIL(valid_index_ids.push_back(table_item->ref_id_))) {
      LOG_WARN("failed to push back array", K(ret));
             ...
             FALSE_IT(log_table_hint = get_plan()->get_log_plan_hint().get_index_hint(table_id))) {
  ...
  if (OB_ISNULL(get_plan()) ||
    ...
    for (int64_t i = 0; OB_SUCC(ret) && i < access_paths.count(); ++i) {
      ...
      if (OB_ISNULL(ap) || OB_ISNULL(ap->get_sharding())) {
        ...
        if (OB_FAIL(add_path(*get_plan(), ap))) {
                                              ...
                                              ObIArray<AccessPath *> &access_paths)
{
  ...
  } else if (OB_FAIL(filter_exprs.assign(helper.filters_))) {
    LOG_WARN("failed to append filters", K(ret));
  } else if (OB_FAIL(ObOptimizerUtil::classify_subquery_exprs(helper.subquery_exprs_,
                                                              filter_exprs,
                                                              pushdown_exprs,
                                                              false /* with_onetime */ ))) {
                                                  ...
                                                  const ObJoinType join_type)
{
  ...
  } else if (OPT_CTX.get_query_ctx()->optimizer_features_enable_version_ >= COMPAT_VERSION_4_2_5) {
    // do nothing
  ...
  if (OB_ISNULL(left_tree) || OB_ISNULL(right_tree)) {
    ...
    if (IS_LEFT_SEMI_ANTI_JOIN(join_type)
      ...
      LOG_WARN("failed to append const exprs", K(ret));
                                                            ...
                                                            output_const_exprs_))) {
      LOG_WARN("failed to compute const exprs", K(ret));
    } else if (IS_SEMI_JOIN(join_type)
               && OB_FAIL(ObOptimizerUtil::compute_const_exprs(join_info_->on_conditions_,
                                                               output_const_exprs_))) {
  ...
  if (OB_ISNULL(left_tree) || OB_ISNULL(right_tree) || OB_ISNULL(join_info_)) {
    ...
    if (LEFT_OUTER_JOIN == join_type || RIGHT_OUTER_JOIN == join_type) {
      ...
      if (OB_FAIL(ObEqualAnalysis::compute_equal_set(allocator_,
                                                     ...
                                                     child_eset,
                                                     output_equal_sets_))) {
        LOG_WARN("failed to compute equal sets for left/right outer join", K(ret));
      }
    } else if (FULL_OUTER_JOIN == join_type) {
      if (OB_FAIL(ObEqualAnalysis::compute_equal_set(allocator_,
                                                     join_info_->where_conditions_,
                                                     output_equal_sets_))) {
        LOG_WARN("failed to compute equal sets for full outer join", K(ret));
      }
    }
  } else if (IS_SEMI_ANTI_JOIN(join_type)) {
    const EqualSets &input_equal_sets = IS_LEFT_SEMI_ANTI_JOIN(join_type)
                                        ? left_tree->output_equal_sets_
                                        : right_tree->output_equal_sets_;
    ObSEArray<ObRawExpr*, 8> eset_conditions;
    if (OB_FAIL(eset_conditions.assign(join_info_->where_conditions_))) {
      LOG_WARN("failed to assign filter exprs", K(ret));
    } else if (IS_SEMI_JOIN(join_type)
               && OB_FAIL(append(eset_conditions, join_info_->on_conditions_))) {
      ...
      LOG_WARN("failed to compute equal sets for semi/anti join", K(ret));
    } else { /*do nothing*/ }
  } else { /*do nothing*/ }
  return ret;
}
...
*/
int ObJoinOrder::add_path(ObLogPlan &plan, Path* path)
{
  int ret = OB_SUCCESS;
...
int ObJoinOrder::update_cost_and_cardinality(const Path &path)
{
  ...
  if (JOIN != type_ || OB_ISNULL(OPT_CTX.get_query_ctx()) ||
      !OPT_CTX.get_query_ctx()->check_opt_compat_version(COMPAT_VERSION_4_2_5_BP8)) {
                                           ...
                                           DominateRelation &relation)
{
  ...
  if (OB_ISNULL(get_plan()) || OB_ISNULL(stmt = get_plan()->get_stmt()) ||
    ...
    OPT_TRACE("left path dominate right path because of pushdown prefix filters");
             ...
             NESTED_LOOP_JOIN == static_cast<const JoinPath&>(first_path).get_join_algo() &&
             NESTED_LOOP_JOIN == static_cast<const JoinPath&>(second_path).get_join_algo() &&
             !static_cast<const JoinPath&>(first_path).is_adaptive_join() &&
             static_cast<const JoinPath&>(second_path).is_adaptive_join()) {
    relation = DominateRelation::OBJ_RIGHT_DOMINATE;
    OPT_TRACE("right path dominate left path because of adaptive join");
  } else if (first_path.is_join_path() &&
             second_path.is_join_path() &&
             NESTED_LOOP_JOIN == static_cast<const JoinPath&>(first_path).get_join_algo() &&
             NESTED_LOOP_JOIN == static_cast<const JoinPath&>(second_path).get_join_algo() &&
             ...
             !static_cast<const JoinPath&>(second_path).is_adaptive_join()) {
    relation = DominateRelation::OBJ_LEFT_DOMINATE;
    OPT_TRACE("left path dominate right path because of adaptive join");
  } else {
                                                ...
                                                DominateRelation &relation)
{
  ...
  if (OB_SUCC(ret) && DominateRelation::OBJ_EQUAL == relation
    ...
    if (OB_FAIL(need_compare_batch_rescan(first_path, second_path, need_compare ))) {
...
int ObJoinOrder::need_compare_batch_rescan(const JoinPath &first_path,
                                           const JoinPath &second_path,
                                           bool &need_compare)
{
  int ret = OB_SUCCESS;
  ...
  if (OB_ISNULL(first_path.right_path_) || OB_ISNULL(second_path.right_path_)) {
    ...
    if (OB_FAIL(get_rescan_path_index_id(*first_path.right_path_,
                                         simple_rescan_path,
                                         first_table_id,
                                         first_index_id,
                                         range_row_count))) {
...
int ObJoinOrder::get_rescan_path_index_id(const Path &path,
                                          ...
                                          double &range_row_count)
{
  ...
  if (!path.subquery_exprs_.empty()) {
    ...
    index_id = ap->index_id_;
    range_row_count = ap->query_range_row_count_;
  } else if (!path.is_subquery_path()) {
    simple_rescan_path = false;
  } else if (OB_FAIL(ObOptimizerUtil::get_rescan_path_index_id(static_cast<const SubQueryPath*>(&path)->root_,
                                                               simple_rescan_path,
                                                               table_id,
                                                               index_id,
                                                               range_row_count))) {
                                          ...
                                          bool &is_valid)const
{
  ...
  if (OB_ISNULL(parent_) || OB_ISNULL(plan = parent_->get_plan()) ||
    ...
    is_valid = false;
    LOG_TRACE("disable adjust index cost", K(opt_stats_cost_percent),
...
int JoinPath::compute_join_path_sharding()
{
  ...
  if (OB_ISNULL(left_path_) || OB_ISNULL(right_path_) ||
    ...
    } else if (DistAlgo::DIST_NONE_NONE == join_dist_algo_) {
      strong_sharding_ = left_path_->strong_sharding_;
      inherit_sharding_index_ = 0;
      if (OB_FAIL(append(weak_sharding_, left_path_->weak_sharding_))) {
        LOG_WARN("failed to append weak sharding", K(ret));
                                                         ...
                                                         ObIArray<common::ObAddr> &server_list)
{
  ...
  if (OB_ISNULL(left_path) || OB_ISNULL(right_path)) {
    ...
    } else if (DistAlgo::DIST_NONE_NONE == join_dist_algo) {
      parallel = left_path->parallel_;
      server_cnt = left_path->server_cnt_;
      if (OB_FAIL(server_list.assign(left_path->server_list_))) {
        LOG_WARN("failed to assign server list", K(ret));
...
int JoinPath::check_right_is_local_scan(int64_t &local_scan_type) const
{
  ...
  if (OB_ISNULL(left_path_) || OB_ISNULL(right_path_)) {
    ...
    local_scan_type = 0;
  } else if (OB_FAIL(check_contain_dist_das(get_server_list(), right_path_, contain_dist_das))) {
...
int JoinPath::do_re_estimate_cost(EstimateCostInfo &info, double &card, double &op_cost, double &cost)
{
  ...
  if (OB_ISNULL(left_path_) || OB_ISNULL(right_path_) || OB_UNLIKELY(info.need_batch_rescan_)) {
    ...
    } else if (is_adaptive_) {
      card = left_output_rows;
    }
    ...
    if ((OB_FAIL(cost_hash_join(info.need_parallel_,
      ...
      //ignore hash join cost for adaptive join
      cost -= op_cost;
      cost -= right_cost;
    }
  } else {
                               ...
                               double &row_count)
{
  ...
  if (OB_ISNULL(left_path_) || OB_ISNULL(right_path_) || OB_ISNULL(parent_) ||
    ...
    LOG_WARN("get unexpected null", K(left_path_), K(right_path_), K(plan), K(ret));
             ...
             HASH_JOIN == join_algo_ && (pushdown_join_filter_infos.empty()  || is_adaptive_join())) {
    ...
    if (IS_NOT_INNER_JOIN(join_type_)) {
    ...
    if (query_ctx->check_opt_compat_version(COMPAT_VERSION_4_2_5_BP7)) {
      if (1.0 > left_rows
          && (!right_path_->is_access_path()
              || ObJoinOrder::PRUNING_ROW_COUNT_THRESHOLD <= static_cast<const AccessPath*>(right_path_)->phy_query_range_row_count_)) {
        left_rows = 1.0;
      }
    } else {
  ...
  if (OB_ISNULL(get_plan()) || OB_ISNULL(stmt = get_plan()->get_stmt()) || OB_ISNULL(allocator_)) {
    ...
    if (OB_FAIL(json_path->set_parallel_and_server_info_for_match_all())) {
      LOG_WARN("failed set parallel and server info for match all", K(ret));
                                                                ...
                                                                json_path->filter_,
                                                                false /* with_onetime */ ))) {
      LOG_WARN("failed to classify filters", K(ret));
    ...
    if (OB_FAIL(ret)) {
    } else if (OB_FAIL(json_path->estimate_cost())) {
      LOG_WARN("failed to estimate cost", K(ret));
    } else if (OB_FAIL(json_path->compute_pipeline_info())) {
      LOG_WARN("failed to compute pipelined path", K(ret));
    } else if (OB_FAIL(create_plan_for_path_with_subq(json_path))) {
      LOG_WARN("failed to create plan for path with subq", K(ret));
    } else if (OB_FAIL(add_path(*get_plan(), json_path))) {
...
int ObJoinOrder::generate_function_table_paths()
{
  ...
  if (OB_ISNULL(get_plan()) || OB_ISNULL(stmt = get_plan()->get_stmt()) || OB_ISNULL(allocator_)) {
    ...
    if (OB_FAIL(ret)) {
      ...
      LOG_WARN("failed to compute pipelined path", K(ret));
    } else if (OB_FAIL(create_plan_for_path_with_subq(func_path))) {
      LOG_WARN("failed to create plan for path with subq", K(ret));
    } else if (OB_FAIL(add_path(*get_plan(), func_path))) {
      LOG_WARN("failed to add path", K(ret));
    } else { /*do nothing*/ }
  }
  return ret;
                                           ...
                                           ObShardingInfo *sharding)
{
  ...
  if (OB_ISNULL(get_plan()) ||
      OB_ISNULL(ap = reinterpret_cast<CteTablePath*>(allocator_->alloc(sizeof(CteTablePath))))) {
    ret = OB_ALLOCATE_MEMORY_FAILED;
    ...
    if (OB_FAIL(ap->set_parallel_and_server_info_for_match_all())) {
      ...
      LOG_WARN("failed to create plan for path with subq", K(ret));
    } else if (OB_FAIL(add_path(*get_plan(), ap))) {
      LOG_WARN("failed to add path", K(ret));
    } else {
      /* do nothing */
    }
...
int ObJoinOrder::generate_normal_base_table_paths()
{
  ...
  } else if (OB_FAIL(generate_base_table_paths(helper, is_adaptive))) {
    LOG_WARN("failed to generate access paths", K(ret));
  }
  return ret;
}

int ObJoinOrder::generate_base_table_paths(PathHelper &helper, bool is_adaptive)
{
  int ret = OB_SUCCESS;
...
int ObJoinOrder::generate_temp_table_paths()
{
  ...
  if (OB_ISNULL(get_plan()) || OB_ISNULL(stmt = get_plan()->get_stmt()) ||
    ...
    if (OB_FAIL(ObOptimizerUtil::classify_subquery_exprs(get_restrict_infos(),
                                                         temp_table_path->subquery_exprs_,
                                                         temp_table_path->filter_,
                                                         false /* with_onetime */ ))) {
      LOG_WARN("failed to classify filters", K(ret));
      ...
      LOG_WARN("failed to create plan for path with subq", K(ret));
    } else if (OB_FAIL(add_path(*get_plan(), temp_table_path))) {
      LOG_WARN("failed to add path", K(ret));
...
int ObJoinOrder::generate_normal_subquery_paths()
{
  ...
  ObSEArray<ObRawExpr*, 4> remain_quals;
  helper.is_inner_path_ = false;
  ...
  } else if (OB_FAIL(append(candi_nonpushdown_quals, remain_quals))) {
    LOG_WARN("failed to append non push down quals", K(ret));
  } else if (OB_FAIL(ObOptimizerUtil::classify_subquery_exprs(candi_nonpushdown_quals,
                                                              helper.subquery_exprs_,
                                                              helper.filters_,
                                                              false /* with_onetime */ ))) {
    LOG_WARN("failed to classify filters", K(ret));
        ...
        if (OB_ISNULL(root = candidate_plans.at(i).plan_tree_)) {
          ...
          if (OB_FAIL(append(sub_path->filter_, helper.filters_))) {
            ...
            LOG_WARN("failed to compute pipelined path", K(ret));
                     ...
                     && OB_FALSE_IT(sub_path->inner_row_count_ = inner_row_count)) {
          } else if (OB_FAIL(create_plan_for_path_with_subq(sub_path))) {
            LOG_WARN("failed to create plan for path with subq", K(ret));
          } else if (helper.is_inner_path_) {
          ...
          } else if (OB_FAIL(add_path(*get_plan(), sub_path))) {
            LOG_WARN("failed to add path", K(ret));
          } else {
                                                 ...
                                                 bool is_temp_table_path)
{
  ...
  if (OB_ISNULL(allocator_) || OB_ISNULL(get_plan()) || OB_ISNULL(get_plan()->get_stmt()) ||
    ...
    if (OB_FAIL(convert_subplan_scan_order_item(*get_plan(),
      ...
      path->exchange_allocated_ = is_temp_table_path ? false : root->is_exchange_allocated();
                                       ...
                                       const JoinInfo *join_info)
{
  ...
  if (OB_ISNULL(left_tree) || OB_ISNULL(right_tree) || OB_ISNULL(join_info)) {
    ...
    LOG_WARN("failed to compute fd item set for join", K(ret));
                                                   ...
                                                   right_tree,
                                                   IS_NOT_INNER_JOIN(join_info->join_type_) ?
                                           ...
                                           const ValidPathInfo &reverse_path_info)
{
  ...
  // generate adaptive join paths
  if (OB_SUCC(ret) &&
      OB_FAIL(check_need_generate_adaptive_join(path_info,
                                                !hash_join_conditions.empty(),
                                                naaj_info.is_naaj_,
                                                need_adaptive_join))) {
    LOG_WARN("failed to check need generate adaptive join", K(ret));
  } else if (OB_SUCC(ret) && need_adaptive_join) {
    LOG_TRACE("start to generate adaptive join paths");
    get_plan()->get_selectivity_ctx().init_join_ctx(path_info.join_type_,
                                                    &left_tree.get_tables(),
                                                    &right_tree.get_tables(),
                                                    left_tree.get_output_rows(),
    ...
    if (OB_FAIL(ObOptSelectivity::calculate_selectivity(
      ...
      LOG_WARN("failed to calculate selectivity", K(ret), K(hash_join_conditions));
        ...
        get_plan()->get_predicate_selectivities()))) {
      LOG_WARN("failed to calculate selectivity", K(ret), K(hash_join_filters));
    } else if (ADAPTIVE_JOIN & path_info.local_methods_ &&
                OB_FAIL(generate_aj_paths(equal_sets,
                                          left_paths,
                                          right_paths,
                                          left_hash_keys,
                                          right_hash_keys,
                                          ...
                                          where_conditions,
                                          has_equal_condition))) {
      LOG_WARN("generate adaptive join paths failed", K(ret));
    } else if (ADAPTIVE_JOIN & reverse_path_info.local_methods_ &&
                OB_FAIL(generate_aj_paths(equal_sets,
                                          right_paths,
                                          left_paths,
                                          right_hash_keys,
      ...
      int64_t adaptive_join_path_num = interesting_paths_.count() - path_number;
      path_number = interesting_paths_.count();
      if (adaptive_join_path_num > 0) {
        has_non_nl_path = true;
      }
      OPT_TRACE("generate", adaptive_join_path_num, "adaptive join path");
      LOG_TRACE("succeed to generate adaptive join paths", K(hash_join_conditions.count()),
          "adaptive_path_count", adaptive_join_path_num);
    }
  }
  return ret;
}

int ObJoinOrder::check_need_generate_adaptive_join(const ValidPathInfo &path_info,
                                                   const bool has_equal_join_cond,
                                                   const bool is_naaj,
                                                   bool &need)
{
  int ret = OB_SUCCESS;
  ...
  if (OB_ISNULL(get_plan()) || OB_ISNULL(OPT_CTX.get_query_ctx())) {
    ...
    for (int64_t i = 0; OB_SUCC(ret) &&
                        !need &&
                        i < interesting_paths_.count(); ++i) {
      Path *path = interesting_paths_.at(i);
      if (OB_ISNULL(path) || OB_UNLIKELY(!path->is_join_path())) {
        ret = OB_ERR_UNEXPECTED;
        LOG_WARN("unexpected interesting path type", KPC(path), K(ret));
      } else {
        JoinPath *join_path = static_cast<JoinPath*>(path);
        if (OB_ISNULL(join_path->right_path_)) {
          ret = OB_ERR_UNEXPECTED;
          LOG_WARN("right path is null", K(ret));
        } else if (NESTED_LOOP_JOIN == join_path->get_join_algo() &&
            !join_path->right_path_->nl_params_.empty()) {
          need = true;
        }
      }
    }
                                   ...
                                   const bool has_equal_cond)
{
  ...
  const ObIArray<ObRawExpr*> &join_conditions =
      IS_NOT_INNER_JOIN(path_info.join_type_) ? on_conditions : where_conditions;
...
int ObJoinOrder::generate_aj_paths(const EqualSets &equal_sets,
                                     const ObIArray<ObSEArray<Path*, 16>> &left_paths,
                                     const ObIArray<ObSEArray<Path*, 16>> &right_paths,
                                     const ObIArray<ObRawExpr*> &left_join_keys,
                                     const ObIArray<ObRawExpr*> &right_join_keys,
                                     const ObIArray<bool> &null_safe_info,
                                     const ObIArray<ObRawExpr*> &hash_join_conditions,
                                     const ObIArray<ObRawExpr*> &hash_join_filters,
                                     const ObIArray<ObRawExpr*> &filters,
                                     const double equal_cond_sel,
                                     const double other_cond_sel,
                                     const ValidPathInfo &path_info,
                                     ...
                                     bool has_equal_condition)
{
  ...
  ObSEArray<Path*, 8> left_best_paths;
  ObSEArray<Path*, 8> right_best_paths;
  ObSEArray<Path*, 8> hj_right_best_paths;
  ObSEArray<Path*, 8> nlj_right_best_paths;
  ObSEArray<ObRawExpr*, 4> normal_filters;
  ObSEArray<ObRawExpr*, 4> subquery_filters;
  if (path_info.is_reverse_path_) {
    OPT_TRACE_TITLE("Consider Reverse ADAPTIVE", ob_join_type_str(path_info.join_type_));
  } else {
    OPT_TRACE_TITLE("Consider ADAPTIVE", ob_join_type_str(path_info.join_type_));
  }
  if (OB_UNLIKELY(left_paths.empty()) || OB_UNLIKELY(right_paths.empty()) || OB_ISNULL(get_plan()) ||
      OB_UNLIKELY(left_paths.at(0).empty()) || OB_ISNULL(left_tree = left_paths.at(0).at(0)->parent_) ||
      OB_UNLIKELY(right_paths.at(0).empty()) || OB_ISNULL(right_tree = right_paths.at(0).at(0)->parent_)) {
    ...
    LOG_WARN("failed to check valid for inner path", K(ret));
  } else if (!need_inner_path) {
    // do nothing.
    LOG_TRACE("do not need inner path");
  } else if (IS_NOT_INNER_JOIN(path_info.join_type_)
             && OB_FAIL(ObOptimizerUtil::classify_subquery_exprs(where_conditions,
                                                                 subquery_filters,
                                                                 normal_filters,
                                                                 false))) {
    LOG_WARN("classify subquery exprs failed", K(ret));
  } else if (!subquery_filters.empty()) {
    LOG_TRACE("do not generate aj path if subquery filters not empty", K(subquery_filters));
  } else if (OB_FAIL(get_cached_inner_paths(join_conditions,
                                            *left_tree,
                                            *right_tree,
                                            false /* force_inner_nl */,
                                            path_info.join_type_,
                                            nlj_right_best_paths,
                                            true /* is_adaptive */))) {
    LOG_WARN("failed to generate best inner paths", K(ret));
  } else if (!nlj_right_best_paths.empty()) {
    if (OB_FAIL(find_minimal_cost_path(left_paths, left_best_paths))) {
      LOG_WARN("failed to find minimal cost path", K(ret));
    } else if (OB_FAIL(find_minimal_cost_path(right_paths, hj_right_best_paths))) {
      LOG_WARN("failed to find minimal cost path", K(ret));
    }
    Path *left_path = NULL;
    Path *hj_right_path = NULL;
    Path *nlj_right_path = NULL;
    for (int64_t i = 0; OB_SUCC(ret) && i < left_best_paths.count(); i++) {
      if (OB_ISNULL(left_path = left_best_paths.at(i))) {
        ret = OB_ERR_UNEXPECTED;
        LOG_WARN("left path is null", K(ret));
      }
      ObSEArray<int64_t, 8> nlj_dist_methods;
      for (int64_t j = 0; j < nlj_right_best_paths.count() && OB_SUCC(ret); j++) {
        int64_t dist_method = 0;
        if (OB_ISNULL(nlj_right_path = nlj_right_best_paths.at(j))) {
          ret = OB_ERR_UNEXPECTED;
          LOG_WARN("get unexpected null", K(ret), K(nlj_right_best_paths));
        } else if (OB_FAIL(get_distributed_join_method(*left_path,
                                                        *nlj_right_path,
                                                        equal_sets,
                                                        left_join_keys,
                                                        ...
                                                        false /* is_naaj */,
                                                        dist_method,
                                                        false /* allow_right_shuffle */))) {
          LOG_WARN("get distribute join method failed", K(ret));
        } else if (OB_FAIL(nlj_dist_methods.push_back(dist_method))) {
          LOG_WARN("push back failed", K(ret));
        }
      }
      for (int64_t j = 0; OB_SUCC(ret) && j < hj_right_best_paths.count(); j++) {
        int64_t hj_dist_method = 0;
        if (OB_ISNULL(hj_right_path = hj_right_best_paths.at(j))) {
          ret = OB_ERR_UNEXPECTED;
          LOG_WARN("get unexpected null", K(hj_right_path), K(ret));
        } else if (!hj_right_path->is_access_path()) {
          continue;
        } else if (OB_FAIL(get_distributed_join_method(*left_path,
                                                        *hj_right_path,
                                                        equal_sets,
          ...
          for (int64_t idx = 0; idx < nlj_right_best_paths.count() && OB_SUCC(ret); idx++) {
            nlj_right_path = nlj_right_best_paths.at(idx);
            if (!nlj_right_path->is_access_path()) {
              continue;
            }
            bool is_equal = false;
            if (OB_ISNULL(hj_right_path->get_sharding()) ||
                OB_ISNULL(nlj_right_path->get_sharding())) {
              ret = OB_ERR_UNEXPECTED;
              LOG_WARN("get unexpected null", K(hj_right_path->get_sharding()),
                      K(nlj_right_path->get_sharding()), K(ret));
            } else if (hj_right_path->get_sharding()->is_match_all() ||  // One of them is das.
                       nlj_right_path->get_sharding()->is_match_all()) {
            } else if (OB_FAIL(ObShardingInfo::is_sharding_equal(hj_right_path->get_sharding(),
                                                          nlj_right_path->get_sharding(),
                                                          equal_sets,
                                                          is_equal))) {
            ...
            for (int64_t k = DistAlgo::DIST_BASIC_METHOD;
                  OB_SUCC(ret) && k < DistAlgo::DIST_MAX_JOIN_METHOD; k = k << 1) {
              if ((hj_dist_method & k) && (nlj_dist_method & k)) {
                DistAlgo dist_algo = get_dist_algo(k);
                if (OB_FAIL(create_and_add_aj_path(left_path,
                                                    hj_right_path,
                                                    nlj_right_path,
                                                    path_info.join_type_,
                                                    dist_algo,
                                                    false, // no support slave mapping in adaptive join
                                                    hash_join_conditions,
                                                    hash_join_filters,
                                                    filters,
                                                    equal_cond_sel,
                                                    ...
                                                    where_conditions,
                                                    has_equal_condition))) {
                  LOG_WARN("failed to create and add adaptive path", K(ret));
                } else { /*do nothing*/ }
              }
            }
          }
...
int ObJoinOrder::create_plan_for_path_with_subq(Path *path)
{
  int ret = OB_SUCCESS;
  ...
  if (OB_ISNULL(path) || OB_ISNULL(get_plan())) {
    ...
    LOG_WARN("failed to create plan from path", K(ret), KPC(path));
...
int ObJoinOrder::create_plan_tree_from_access_paths(ObIArray<AccessPath*> &access_paths)
{
  int ret = OB_SUCCESS;
  for (int64_t i = 0; OB_SUCC(ret) && i < access_paths.count(); ++i) {
    if (OB_FAIL(create_plan_for_path_with_subq(access_paths.at(i)))) {
      LOG_WARN("failed to create plan for path with subq", K(ret));
    }
  }
  return ret;
    ...
    for (int64_t i = 0; OB_SUCC(ret) && i < left_paths.count(); i++) {
      ...
      for (int64_t j = DistAlgo::DIST_BASIC_METHOD;
        ...
        if (dist_method & j) {
          ...
          bool tmp_need_mat = need_mat;
          bool tmp_need_no_mat = need_no_mat;
          if (!ObOptimizerUtil::is_right_need_exchange(*right_path->get_sharding(), dist_algo) &&
            ...
            tmp_need_mat = true;
            tmp_need_no_mat = false;
            OPT_TRACE("distributed method", ob_dist_algo_str(dist_algo), "force need materialize");
            LOG_TRACE("distributed method force need materialize", K(dist_algo), KPC(right_path));
          }
          if ((tmp_need_mat || right_need_exchange) &&
          ...
          } else if (tmp_need_no_mat && !right_need_exchange &&
                     OB_FAIL(create_and_add_nl_path(left_paths.at(i),
                                                    right_path,
                                                    path_info.join_type_,
                                                    dist_algo,
...
int ObJoinOrder::get_distributed_join_method(Path &left_path,
                                             ...
                                             const bool allow_right_shuffle /*true*/)
{
  int ret = OB_SUCCESS;
  bool is_basic = false;
  ...
  if (OB_SUCC(ret)) {
    OPT_TRACE("succeed to get valid distributed method:");
    OPT_TRACE_BEGIN_SECTION;
    for (int64_t j = DistAlgo::DIST_BASIC_METHOD; j < DistAlgo::DIST_MAX_JOIN_METHOD; j = (j << 1)) {
      if (distributed_methods & j) {
        OPT_TRACE(ob_dist_algo_str(get_dist_algo(j)));
      }
    }
...
{
  int ret = OB_SUCCESS;
  if (OB_FAIL(extract_mergejoin_conditions(IS_NOT_INNER_JOIN(join_type) ? on_condition : where_condition,
                                           left_tree.get_tables(),
                                           right_tree.get_tables(),
  ...
  } else if (IS_NOT_INNER_JOIN(join_type) && OB_FAIL(append(filters, where_condition))) {
                                ...
                                const ObIArray<ObRawExpr*> &where_conditions)
{
  ...
  } else if (IS_NOT_INNER_JOIN(join_type)) {
                                          ...
                                          const NullAwareAntiJoinInfo &naaj_info)
{
  ...
  if (OB_ISNULL(get_plan())) {
    ret = OB_INVALID_ARGUMENT;
    LOG_WARN("get unexpected null", K(get_plan()), K(ret));
  } else if (OB_FAIL(create_hash_join_path(left_path,
                                           right_path,
                                           join_type,
                                           join_dist_algo,
                                           is_slave_mapping,
    ...
    OPT_TRACE("create new HASH Join path:", static_cast<JoinPath*>(join_path));
  }
  if (OB_FAIL(ret)) {
                                        ...
                                        const bool has_equal_cond)
{
  ...
  Path *nlj_path = NULL;
  Path *hj_path = NULL;
  if (OB_ISNULL(left_path) || OB_ISNULL(hj_right_path) || OB_ISNULL(nlj_right_path)) {
    ret = OB_ERR_UNEXPECTED;
    LOG_WARN("path is null", K(ret), K(left_path), K(hj_right_path), K(nlj_right_path));
  } else if (OB_FAIL(create_nl_join_path(left_path,
                                          nlj_right_path,
                                          join_type,
                                          join_dist_algo,
                                          is_slave_mapping,
                                          ...
                                          false,  //is_normal_nl
                                          false,  //need mat
                                          true,   //is adaptive join
                                          nlj_path))) {
    LOG_WARN("failed to create nl join path", K(ret));
    ...
    if (OB_FAIL(add_path(*get_plan(), hj_path))) {
      LOG_WARN("failed to add path", K(ret));
    }
  }
  return ret;
...
int ObJoinOrder::create_nl_join_path(const Path *left_path,
                                const Path *right_path,
                                const ObJoinType join_type,
                                const DistAlgo join_dist_algo,
                                const bool is_slave_mapping,
                                ...
                                Path* &nl_path)
{
  ...
  if (OB_ISNULL(left_path) || OB_ISNULL(right_path) ||
    ...
    join_path->is_adaptive_ = is_adaptive_join;
...
int ObJoinOrder::create_hash_join_path(const Path *left_path,
                                  const Path *right_path,
                                  const ObJoinType join_type,
                                  const DistAlgo join_dist_algo,
                                  const bool is_slave_mapping,
                                                ...
                                                ObIArray<JoinFilterInfo> &join_filter_infos)
{
  ...
  if (OB_ISNULL(plan) ||
      ...
      OB_ISNULL(query_ctx = OPT_CTX.get_query_ctx())) {
    ret = OB_ERR_UNEXPECTED;
    LOG_WARN("get unexpected null", K(ret), K(plan), K(left_tree), K(stmt), K(query_ctx));
  }
  for (int i = 0; OB_SUCC(ret) && i < join_filter_infos.count(); ++i) {
    JoinFilterInfo &info = join_filter_infos.at(i);
    double join_filter_sel = 1.0;
    int64_t bloom_filter_ratio = GCONF._bloom_filter_ratio;
    ...
    } else if (OB_FAIL(query_ctx->get_global_hint().opt_params_.get_integer_opt_param(ObOptParamHint::BLOOM_FILTER_RATIO, bloom_filter_ratio))) {
      LOG_WARN("failed to get opt param bloom filter ratio", K(ret));
    } else {
      double rate = 1 - join_filter_sel;
      double threshold = 0.6;
      double misjudgment_rate = (static_cast<double>(bloom_filter_ratio) / 100.0);
      if (info.in_current_dfo_) {
        threshold = 0.9;
      }
      info.join_filter_selectivity_ = join_filter_sel;
      if (query_ctx->optimizer_features_enable_version_ >= COMPAT_VERSION_4_2_4 &&
...
{
  int ret = OB_SUCCESS;
  if (OB_FAIL(extract_hashjoin_conditions(IS_NOT_INNER_JOIN(join_type)
  ...
  LOG_TRACE("classify_hashjoin_conditions", K(join_type), K(on_conditions), K(where_filters),
          K(equal_join_conditions.count()), K(other_join_conditions.count()), K(filters.count()));
  return ret;
}
                                               ...
                                               ValidPathInfo &path_info)
{
  ...
  if (OB_ISNULL(get_plan()) || OB_ISNULL(get_plan()->get_stmt()) ||
    ...
    LOG_TRACE("get join hint", K(ignore_dist_hint), K(both_access), KPC(log_join_hint));
    if (ignore_dist_hint) {
      // do nothing.
    ...
    if (NULL != log_join_hint && !log_join_hint->local_method_hints_.empty()) {
      ...
      LOG_TRACE("get_valid_path_info_from_hint", K(path_info));
                                     ...
                                     ValidPathInfo &path_info)
{
  ...
  if (OB_ISNULL(get_plan()) || OB_UNLIKELY(left_paths.empty() || right_paths.empty())
    ...
    if (CONNECT_BY_JOIN == path_info.join_type_) {
      ...
      path_info.local_methods_ = NESTED_LOOP_JOIN | MERGE_JOIN | HASH_JOIN | ADAPTIVE_JOIN;
    ...
    if (!ignore_hint
      ...
      path_info.local_methods_ &= ~ADAPTIVE_JOIN;
      OPT_TRACE("right or full outer join can not use nested loop/adaptive join");
    } else if (RIGHT_SEMI_JOIN == path_info.join_type_ ||
               RIGHT_ANTI_JOIN == path_info.join_type_) {
      path_info.local_methods_ &= ~NESTED_LOOP_JOIN;
      path_info.local_methods_ &= ~MERGE_JOIN;
      path_info.local_methods_ &= ~ADAPTIVE_JOIN;
      OPT_TRACE("right semi/anti join can not use nested loop/merge/adaptive join");
    }
    if (OB_SUCC(ret)) {
    ...
    if (OB_SUCC(ret) && !opt_ctx.enable_new_query_range()) {
      // disable adaptive join if use old query range because
      // we do not copy and replace old query range when copy log table scan.
      path_info.local_methods_ &= ~ADAPTIVE_JOIN;
      LOG_TRACE("disable adaptive join since new query range disabled");
    }
  ...
  if (NULL == query_range_provider) {
    ...
    if (OB_ISNULL(get_plan()) || OB_ISNULL(get_plan()->get_stmt()) ||
      ...
      for (int64_t i = 0; OB_SUCC(ret) && i < all_filters.count(); i++) {
        ...
        bool enable_unprecise_filter_opt = query_ctx->check_opt_compat_version(COMPAT_VERSION_4_2_1_BP10, COMPAT_VERSION_4_2_2,
                                                                               COMPAT_VERSION_4_2_5_BP1);
        if (OB_ISNULL(filter = all_filters.at(i))) {
          ...
          /*do nothing*/
                                                               ...
                                                               expr_column_ids))) {
          LOG_WARN("failed to extract column ids", K(ret));
        } else if (enable_unprecise_filter_opt &&
                   ObOptimizerUtil::overlap(expr_column_ids, index_column_ids) &&
          ...
          LOG_PRINT_EXPR(TRACE, "Found unprecise range filter", filter, K(is_dynamic), K(ex_prefix_column_ids));
          OPT_TRACE("Found unprecise range filter :", filter);
        } else if (!ObOptimizerUtil::is_subset(expr_column_ids, index_column_ids)) {
          ret = est_cost_info.table_filters_.push_back(filter);
        } else if (!enable_unprecise_filter_opt &&
                   OB_FAIL(can_extract_unprecise_range(est_cost_info.table_id_, filter,
                                                       ex_prefix_column_ids,
                                                       unprecise_range_exprs,
                                                       can_extract))) {
          LOG_WARN("failed to extract column ids", K(ret));
                                                ...
                                                ObIArray<uint64_t> *equal_column_ids)
{
  ...
  if (OB_ISNULL(filter)) {
    ...
    LOG_WARN("unexpected null expr", K(ret));
             ...
             T_OP_LIKE != filter->get_expr_type() &&   // ignore filters such as `c1 like '%123'`
             T_OP_OR != filter->get_expr_type() &&     // ignore filters such as `c1 > 1 and c2 = 1 or c1 > 2 and c2 = 2`
             ObOptimizerUtil::find_equal_expr(unprecise_exprs, filter)) {
    can_extract = true;
    ...
    if (OB_SUCC(ret) && can_extract) {
      if (NULL != equal_column_ids) {
        ret = equal_column_ids->push_back(column->get_column_id());
      }
      is_dynamic = filter->has_flag(CNT_DYNAMIC_PARAM);
    }
  } else if (T_OP_LIKE == filter->get_expr_type()) {
    ...
    if (OB_ISNULL(first_expr) || OB_ISNULL(patten_expr) || OB_ISNULL(escape_expr)) {
      ...
      if (column->get_table_id() != table_id
        ...
        if (patten_expr->is_dynamic_const_expr()) {
          ...
          can_extract = !is_start_with && !all_is_percent_sign;
        }
      } else {
        can_extract = false;
      }
    ...
    can_extract = true;
    ObSEArray<uint64_t, 4> tmp_equal_column_ids;
    ObSEArray<uint64_t, 4> child_equal_column_ids;
    for (int64_t i = 0; OB_SUCC(ret) && can_extract && i < filter->get_param_count(); i ++) {
      ...
      child_equal_column_ids.reuse();
      if (OB_FAIL(SMART_CALL(can_extract_unprecise_range_v2(
              table_id, filter->get_param_expr(i), prefix_column_ids, unprecise_exprs,
              child_can_extract, child_is_dynamic, false, &child_equal_column_ids)))) {
        LOG_WARN("failed to check child can extract", K(ret));
      } else if (!child_can_extract) {
        can_extract = false;
      } else if (i == 0) {
        ret = tmp_equal_column_ids.assign(child_equal_column_ids);
        is_dynamic |= child_is_dynamic;
      } else {
        ret = ObOptimizerUtil::intersect(tmp_equal_column_ids,
                                         child_equal_column_ids,
                                         tmp_equal_column_ids);
        can_extract = !tmp_equal_column_ids.empty();
        is_dynamic |= child_is_dynamic;
      }
    }
    if (OB_SUCC(ret) && NULL != equal_column_ids && can_extract) {
      ret = equal_column_ids->assign(tmp_equal_column_ids);
    }
    ...
    for (int64_t i = 0; OB_SUCC(ret) && i < filter->get_param_count(); i ++) {
      ...
      if (OB_FAIL(SMART_CALL(can_extract_unprecise_range_v2(
              table_id, filter->get_param_expr(i), prefix_column_ids, unprecise_exprs,
              child_can_extract, child_is_dynamic, false, equal_column_ids)))) {
        LOG_WARN("failed to check child can extract", K(ret));
      } else if (child_can_extract) {
        can_extract = true;
        is_dynamic |= child_is_dynamic;
                                        ...
                                        bool need_mat)
{
  ...
  if (OB_ISNULL(get_plan())) {
    ...
    OPT_TRACE("create new NL Join path:", static_cast<JoinPath*>(join_path));
  }
  if (OB_FAIL(ret)) {
  } else if (OB_FAIL(add_path(*get_plan(), join_path))) {
    LOG_WARN("failed to add path", K(ret));
        ...
        if (OB_FAIL(ret)) {
          ...
          for (int64_t i = 0; OB_SUCC(ret) && i < refetch_col_ids.count(); ++i) {
            ...
            if (OB_ISNULL(col_meta)) {
              ret = OB_ERR_UNEXPECTED;
              LOG_WARN("get unexpected null", K(ret));
            } else if (OB_UNLIKELY(col_meta->get_column_id() != refetch_col_ids.at(i))) {
              ret = OB_ERR_UNEXPECTED;
                                  ...
                                  const JoinInfo &join_info)
{
  ...
  if (OB_ISNULL(left_tree) || OB_ISNULL(right_tree) ||
    ...
    if (!OPT_CTX.get_query_ctx()->check_opt_compat_version(COMPAT_VERSION_4_2_5_BP8)) {
      for (int64_t i = 0; i < ambient_card_.count(); i ++) {
                                        ...
                                        ObIArray<double> &ambient_card)
{
  ...
  if (OB_SUCC(ret)) {
    ...
    tmp_join_info.on_conditions_.reuse();
    ...
    } else if (IS_NOT_INNER_JOIN(join_type) && OB_FAIL(append(tmp_join_info.on_conditions_, join_info.on_conditions_))) {
      LOG_WARN("failed to append on conditions", K(ret));
    } else if (IS_INNER_JOIN(join_type) && OB_FAIL(append(tmp_join_info.on_conditions_, join_info.where_conditions_))) {
      LOG_WARN("failed to append", K(ret));
  ...
  if (OB_SUCC(ret) && IS_NOT_INNER_JOIN(join_type) && !join_info.where_conditions_.empty()) {
  ...
  OPT_TRACE("outer/semi/anti join filter selectivity :", where_sel_for_oj);
                                         ...
                                         const ObJoinType assumption_type)
{
  ...
  for (int64_t i = 0; OB_SUCC(ret) && i < join_info.on_conditions_.count(); ++i) {
    ...
    if (OB_FAIL(check_direct_join_condition(join_info.on_conditions_.at(i),
                                            equal_sets,
                                            table_id,
                                            exclusion_ids,
                                            is_direct_condition))) {
      ...
      // do nothing
    } else if (OB_FAIL(table_join_info.on_conditions_.push_back(join_info.on_conditions_.at(i)))) {
      LOG_WARN("failed to push back expr", K(ret));
    }
  }
  if (OB_FAIL(ret)) {
  } else if (table_join_info.on_conditions_.empty()) {
    ambient_card_sel = 1.0;
  } else if (OB_FAIL(calc_join_output_rows(plan,
                                           left_tree.get_tables(),
                                           right_tree.get_tables(),
                                       ...
                                       const EqualSets &equal_sets)
{
  ...
  if (OB_ISNULL(plan)) {
    ...
    LOG_WARN("get unexpected null", K(ret));
                                                                   ...
                                                                   right_output_rows,
                                                                   &equal_sets))) {
  } else if (IS_INNER_JOIN(join_type)) {
    ...
    if (OB_FAIL(check_and_remove_is_null_qual(plan,
      ...
      JoinInfo tmp_join_info;
      new_rows = 0.0;
      if (OB_FAIL(tmp_join_info.on_conditions_.assign(join_info.on_conditions_))) {
        LOG_WARN("failed to assign on conditions", K(ret));
    ...
    double qual_sel = 1.0;
    if (join_info.on_conditions_.empty()) {
      ...
      selectivity = 1.0;
                                                                    ...
                                                                    selectivity,
                                                                    plan->get_predicate_selectivities()))) {
      LOG_WARN("Failed to calc join condition selectivities", K(ret));
    } else {
    ...
    if (OB_FAIL(ret)) {
    } else if (join_info.where_conditions_.empty()) {
      // do nothing
    } else if (OB_FAIL(ObOptSelectivity::calculate_join_selectivity(plan->get_update_table_metas(),
                                                                    plan->get_selectivity_ctx(),
                                                                    join_info.where_conditions_,
                                              ...
                                              const ObJoinType join_type)
{
  ...
  if (OB_SUCC(ret)) {
    ...
    LOG_WARN("failed to compute fd item set for join", K(ret), K(join_type));
  }
  return ret;
}
                                                        ...
                                                        const ObJoinType join_type)
{
  int ret = OB_SUCCESS;
  if (OB_ISNULL(left_tree) || OB_ISNULL(right_tree) ||
      OB_ISNULL (join_info) || !IS_SEMI_ANTI_JOIN(join_type)) {
    ret = OB_ERR_UNEXPECTED;
    LOG_WARN("params are invalid", K(ret), K(left_tree), K(right_tree), K(join_info), K(join_type));
  } else if (IS_LEFT_SEMI_ANTI_JOIN(join_type)) {
    ObSEArray<ObRawExpr*, 8> input_quals;
    ...
    } else if (OB_FAIL(input_quals.assign(join_info->where_conditions_))) {
      LOG_WARN("failed to assign exprs", K(ret));
                                                 ...
                                                 bool is_adaptive)
{
  ...
  if (OB_ISNULL(get_plan()) || OB_ISNULL(stmt = get_plan()->get_stmt())) {
    ...
    // do nothing
  } else if (OB_FAIL(all_quals.assign(right_tree.get_restrict_infos()))) {
  ...
  } else if (OB_FAIL(append(all_quals, helper.pushdown_filters_))) {
    LOG_WARN("failed to append pushdown quals", K(ret));
  } else if (OB_FAIL(ObOptimizerUtil::classify_subquery_exprs(all_quals,
                                                              helper.subquery_exprs_,
                                                              helper.filters_,
                                                              false /* with_onetime */ ))) {
  } else if (OB_FAIL(right_tree.generate_base_table_paths(helper, is_adaptive))) {
    LOG_WARN("failed to generate access paths", K(ret));
  ...
  if (ACCESS == left_tree.get_type() ||
    ...
    if (left_tree.interesting_paths_.empty()) {
      ...
      if (OB_ISNULL(join_path) ||
        ...
        const JoinPath *child_join_path = NULL;
        const Path *next_check_path = join_path->left_path_;
        if (JOIN != join_path->left_path_->parent_->get_type()) {
          // do nothing.
        } else if (FALSE_IT(child_join_path = static_cast<const JoinPath*>(join_path->left_path_))) {
        } else if (child_join_path->is_adaptive_ && NESTED_LOOP_JOIN == child_join_path->join_algo_) {
          LOG_TRACE("adaptive join interesting paths", K(left_tree.interesting_paths_));
          next_check_path = child_join_path->left_path_;
          if (OB_ISNULL(next_check_path->parent_)) {
            ret = OB_ERR_UNEXPECTED;
            LOG_WARN("unexpect null path", K(ret));
          }
        }
        if (OB_FAIL(ret)) {
        } else if (OB_FAIL(SMART_CALL(check_filter_is_redundant(*next_check_path->parent_,
                                                              expr,
                                                              context,
                                                              is_redunant)))) {
          LOG_WARN("failed to check filter is redunant", K(ret));
                                          ...
                                          const ObIArray<ObRawExpr*> &pushdown_quals)
{
  ...
  for (int64_t i = 0; OB_SUCC(ret) && i < inner_paths.count(); ++i) {
    ...
    if (OB_ISNULL(inner_path)) {
      ret = OB_ERR_UNEXPECTED;
      LOG_WARN("unexpected null path", K(ret));
    } else if (OB_FAIL(compute_valid_inner_path(inner_path, pushdown_quals))) {
                                          ...
                                          const ObIArray<ObRawExpr*> &pushdown_filters)
{
  ...
  if (OB_ISNULL(get_plan())) {
    ret = OB_ERR_UNEXPECTED;
    LOG_WARN("get unexpected null", K(ret));
  } else if (OB_FAIL(append(all_pushdown_filters, pushdown_filters))) {
...
int ObJoinOrder::extract_range_filters(Path *inner_path, ObIArray<ObRawExpr*> &all_range_filters)
{
  ...
  if (OB_ISNULL(inner_path)) {
    ...
    if (OB_ISNULL(access_path)) {
      ret = OB_ERR_UNEXPECTED;
      LOG_WARN("unexpected null path", K(ret));
    } else if (OB_FAIL(append(all_range_filters, access_path->est_cost_info_.pushdown_prefix_filters_))) {
      LOG_WARN("failed to append pushdown filters", K(ret));
    } else {
      ObQueryRangeProvider *pre_range = access_path->get_query_range_provider();
      if (OB_ISNULL(pre_range)) {
        //do nothing
...
int ObJoinOrder::extract_range_filters(ObLogicalOperator *root, ObIArray<ObRawExpr*> &all_range_filters)
{
  ...
  if (OB_ISNULL(root)) {
    ...
    if (OB_ISNULL(est_cost_info) || OB_FAIL(ret)) {
      //do nothing
                                          ...
                                          InnerPathInfo &inner_path_info)
{
  ...
  for (int64_t i = 0; OB_SUCC(ret) && i < right_tree.get_interesting_paths().count(); ++i) {
    ...
    if (OB_ISNULL(right_path)) {
      ...
      if (OB_FAIL(create_plan_for_path_with_subq(inner_path))) {
        LOG_WARN("failed to create plan for path with subq", K(ret));
      }
    }
  }
                                                   ...
                                                   PathHelper &helper)
{
  ...
  if (T_OP_LIKE == type) {
    ...
    ObObj result;
    ObOptimizerContext *opt_ctx = NULL;
    const number::ObNumber num_one = number::ObNumber::get_positive_one();
    ...
    } else if (!got_result || result.is_null() || (is_oracle_mode() &&
               (!result.is_oracle_decimal() || result.get_number().compare(num_one) < 0))) {
...
int ObJoinOrder::generate_values_table_paths()
{
  ...
  if (OB_ISNULL(get_plan()) || OB_ISNULL(stmt = get_plan()->get_stmt()) || OB_ISNULL(allocator_)) {
    ...
    if (OB_FAIL(values_path->set_parallel_and_server_info_for_match_all())) {
      ...
      LOG_WARN("failed to create plan for path with subq", K(ret));
    } else if (OB_FAIL(add_path(*get_plan(), values_path))) {
      LOG_WARN("failed to add path", K(ret));
    } else { /*do nothing*/ }
  }
