...
struct MergeKeyInfoHelper
{
  ...
  public:
    ...
    : allocator_(&allocator),
      ...
      join_info_(NULL),
      used_conflict_detectors_(allocator),
      restrict_info_set_(allocator),
      interesting_paths_(allocator),
      is_at_most_one_row_(false),
      output_equal_sets_(allocator),
      output_const_exprs_(allocator),
      table_opt_info_(allocator),
      available_access_paths_(allocator),
      diverse_path_count_(0),
      fd_item_set_(allocator),
      candi_fd_item_set_(allocator),
      not_null_columns_(allocator),
      inner_path_infos_(allocator),
      deduced_exprs_info_(allocator),
      cnt_rownum_(false),
      total_path_num_(0),
      ambient_card_(allocator),
      current_join_output_rows_(-1.0),
      current_join_ambient_card_(allocator),
      best_cost_(-1.0)
    {
    }
    virtual ~ObJoinOrder();
