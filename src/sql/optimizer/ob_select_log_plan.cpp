                                                     ...
                                                     const ObIArray<bool> *pushdown_info /* default null */)
{
  ...
  if (OB_ISNULL(top)) {
    ...
    if (range_dist_keys_cnt > 0) {
      window_function->set_range_dist_parallel(true);
                                                 ...
                                                 bool &is_valid)
{
  ...
  if (WinDistAlgo::WIN_DIST_NONE == method) {
    ...
    bool range_dist_supported = false;
      ...
      switch (method) {
        ...
        case WIN_DIST_LIST: {
          if (pby_oby_cnt <= 0 || pby_oby_cnt == pby_cnt) {
            is_valid = false;
          } else if (OB_FAIL(check_wf_range_dist_supported(win_func_exprs.at(i), range_dist_supported))) {
...
{
  int ret = OB_SUCCESS;
  if (OB_UNLIKELY(split_idx < 0 || split_idx >= split.count()
                  || split.count() != methods.count()
                  || WinDistAlgo::WIN_DIST_INVALID == methods.at(split_idx)
                  || pby_oby_prefixes.count() != ordered_win_func_exprs.count()
                  || split.at(split_idx) > ordered_win_func_exprs.count())) {
    ret = OB_ERR_UNEXPECTED;
    LOG_WARN("unexpected params", K(ret), K(split_idx), K(split), K(methods),
                                  K(pby_oby_prefixes), K(ordered_win_func_exprs.count()));
  } else {
    win_func_helper.win_dist_method_ = methods.at(split_idx);
    ...
    const int64_t start = 0 == split_idx ? 0 : split.at(split_idx - 1);
    const int64_t end = split.at(split_idx);
