...
const char *ObLogPxObjectSample::get_name() const
{
...
int ObLogPxObjectSample::set_sort_keys(const common::ObIArray<OrderItem> &order_keys)
{
  int ret = OB_SUCCESS;
...
int ObLogPxObjectSample::get_op_exprs(ObIArray<ObRawExpr*> &all_exprs)
{
  int ret = OB_SUCCESS;
...
int ObLogPxObjectSample::inner_replace_op_exprs(ObRawExprReplacer &replacer)
{
  int ret = OB_SUCCESS;
  for (int64_t i = 0; OB_SUCC(ret) && i < sort_keys_.count(); i++) {
    if (OB_ISNULL(sort_keys_.at(i).expr_)) {
