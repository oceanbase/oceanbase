...
ObPwjComparer::ObPwjComparer(bool is_strict)
  ...
  if (OB_ISNULL(table.phy_table_loc_info_)) {
    ...
    if (OB_ISNULL(part_array = tablet_id_group_.alloc_place_holder())){
      ...
      LOG_WARN("failed to assign ordered partition ids", K(ret));
             ...
             OB_FAIL(check_logical_equal_and_calc_match_map(pwj_tables_.at(0),
                                                            pwj_tables_.at(pwj_tables_.count() - 1),
                                                            is_match_pwj))) {
    LOG_WARN("failed to calc match map", K(ret));
  } else if (!is_match_pwj) {
    // do nothing
  } else if (OB_FAIL(check_if_match_partition_wise(pwj_tables_.at(0),
                                                   pwj_tables_.at(pwj_tables_.count() - 1),
                                                   is_match_pwj))) {
...
int ObPwjComparer::get_used_partition_indexes(const int64_t part_count,
...
int ObPwjComparer::check_logical_equal_and_calc_match_map(const PwjTable &l_table,
...
bool ObPwjComparer::is_same_part_type(const ObPartitionFuncType part_type1,
...
int ObPwjComparer::is_first_partition_logically_equal(const PwjTable &l_table,
                                                      const PwjTable &r_table,
                                                      bool &is_equal)
{
  int ret = OB_SUCCESS;
...
int ObPwjComparer::is_sub_partition_logically_equal(const PwjTable &l_table,
                                                    const PwjTable &r_table,
                                                    bool &is_equal)
{
  int ret = OB_SUCCESS;
...
int ObPwjComparer::get_subpartition_indexes_by_part_index(const PwjTable &table,
...
int ObPwjComparer::check_hash_partition_equal(const PwjTable &l_table,
                                              const PwjTable &r_table,
...
int ObPwjComparer::check_range_partition_equal(ObPartition **l_partition_array,
...
int ObPwjComparer::check_list_partition_equal(ObPartition **l_partition_array,
                                              ObPartition **r_partition_array,
                                              const ObIArray<int64_t> &l_indexes,
                                              const ObIArray<int64_t> &r_indexes,
                                              ObIArray<std::pair<uint64_t,uint64_t> > &part_tablet_id_map,
...
int ObPwjComparer::check_if_match_partition_wise(const PwjTable &l_table,
                                                 const PwjTable &r_table,
...
int ObPwjComparer::is_simple_physically_equal_partitioned(const PwjTable &l_table,
                                                          const PwjTable &r_table,
                                                          bool &is_physical_equal)
{
  ...
  if (left_locations.count() != right_locations.count()) {
    ...
    for (int64_t i = 0; OB_SUCC(ret) && i < left_locations.count(); ++i) {
      ...
      } else if (OB_FAIL(l_servers.push_back(l_replica_loc.get_server()))) {
        LOG_WARN("failed to push back server addr", K(ret));
      } else if (OB_FAIL(r_servers.push_back(r_replica_loc.get_server()))) {
        LOG_WARN("failed to push back server addr", K(ret));
      }
    }
    if (OB_SUCC(ret)) {
      ...
      for (int64_t i = 0; is_physical_equal && i < l_servers.count(); ++i) {
        ...
        for (int64_t j = 0; OB_SUCC(ret) && !find && j < r_servers.count(); ++j) {
          if (matched_partitions.has_member(j) ||
            ...
            find = true;
            int64_t offset = -1;
            if (ObOptimizerUtil::find_item(l_table.ordered_tablet_ids_,
                                           left_locations.at(i).get_partition_location().get_tablet_id().id(),
                                           &offset)) {
              r_array->at(offset) = right_locations.at(j).get_partition_location().get_tablet_id().id();
            }
          }
        }
        if (OB_SUCC(ret) && !find) {
...
int ObPwjComparer::is_physically_equal_partitioned(const PwjTable &l_table,
                                                   const PwjTable &r_table,
                                                   bool &is_physical_equal)
{
  int ret = OB_SUCCESS;
    ...
    if (OB_FAIL(l_tablet_id_map.create(std::max(N, MIN_ID_LOCATION_BUCKET_NUMBER),
      ...
      for (int64_t i = 0; OB_SUCC(ret) && is_physical_equal && i < N; ++i) {
        ...
        if (OB_FAIL(phy_part_map_.get_refactored(l_tablet_id, r_tablet_id))) {
          ...
          LOG_WARN("failed to get refactored", K(ret), K(l_tablet_id), K(r_tablet_id),
                    K(l_table), K(r_table));
...
int ObPwjComparer::check_replica_location_match(const ObCandiTabletLoc &left_location,
                                                const ObCandiTabletLocIArray &right_locations,
...
int ObPwjComparer::get_part_tablet_id_by_part_index(const PwjTable &table,
                                                    const int64_t part_index,
                                                    uint64_t &tablet_id)
{
  int ret = OB_SUCCESS;
...
int ObPwjComparer::get_sub_part_tablet_id(const PwjTable &table,
                                          const int64_t &part_index,
