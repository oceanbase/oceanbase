...
int ObExchangeInfo::init_calc_part_id_expr(ObOptimizerContext &opt_ctx)
{
  ...
  if (part_expr_cnt <= 0 || part_expr_cnt > 2) {
    ...
    if (OB_ISNULL(repartition_func_exprs_.at(0))) {
      ...
      } else if (OB_FAIL(op_row_expr->init_param_exprs(repartition_keys_.count()))) {
  ...
  if (OB_SUCC(ret) && part_expr_cnt == 2) {
    ...
    if (OB_ISNULL(repartition_func_exprs_.at(1))) {
      ...
      if (OB_FAIL(expr_factory.create_raw_expr(T_OP_ROW, op_row_expr))) {
        LOG_WARN("fail to create raw expr", K(ret));
      } else if (OB_FAIL(op_row_expr->init_param_exprs(repartition_sub_keys_.count()))) {
        LOG_WARN("failed to init param exprs", K(ret));
      } else {
...
int ObExchangeInfo::append_hash_dist_expr(const common::ObIArray<ObRawExpr *> &exprs)
{
  ...
  for (int64_t i = 0; OB_SUCC(ret) && i < exprs.count(); i++) {
    ...
    } else if (OB_FAIL(hash_dist_exprs_.push_back(HashExpr(raw_expr)))) {
                                             ...
                                             ObIArray<ObRawExpr*> &shard_exprs)
{
  ...
  if (OB_ISNULL(raw_expr)) {
    ...
    /*do nothing*/
  } else if (T_OP_ROW == raw_expr->get_expr_type() || T_FUN_SYS_INNER_ROW_CMP_VALUE == raw_expr->get_expr_type()) {
    /*do nothing*/
  ...
  if (ObOptimizerUtil::find_item(ctx.inseparable_exprs_, raw_expr) || raw_expr->is_match_against_expr()) {
    // do nothing
  } else {
      ...
      for (int i = 0; i < get_num_of_child() && OB_SUCC(ret); ++i) {
        ...
        if (0 == i) {
          enable_px_batch_rescans.push_back(false);
                   ...
                   || static_cast<ObLogSubPlanFilter *>(this)->get_initplan_idxs().has_member(i)) {
          find_px = false;
  ...
  if (OB_FAIL(ret)) {
    ...
    if (replacer != NULL) {
      for (int64_t i = 0; OB_SUCC(ret) && i < output_exprs_.count(); ++i) {
        if (OB_FAIL(replace_expr_action(*replacer, output_exprs_.at(i)))) {
