                                                 ...
                                                 ObIAllocator &allocator)
{
  ...
  if (OB_UNLIKELY(interest_column_ids.count() != const_column_info.count())) {
    ...
    ObSEArray<uint64_t, 8> filtered_interest_column_ids;
    for (int64_t i = 0; OB_SUCC(ret) && i < interest_column_ids.count(); ++i) {
      if (!const_column_info.at(i)) {
        if (OB_FAIL(filtered_interest_column_ids.push_back(interest_column_ids.at(i)))) {
          LOG_WARN("failed to push back column id", K(ret));
        }
      }
    }
    if (OB_SUCC(ret)) {
      if (filtered_interest_column_ids.count() > 0) {
        dim->set_interesting_order(true);
        if (OB_FAIL(dim->add_interest_prefix_ids(filtered_interest_column_ids))) {
