                                                       ...
                                                       bool &is_success)
{
  ...
  for (int64_t i = 0; OB_SUCC(ret) && i < paths.count(); ++i) {
    ...
    SMART_VAR(ObTablePartitionInfo, index_part_info) {
      ...
      if (OB_FAIL(ret)) {
        ...
        LOG_WARN("failed to assign table part info", K(ret));
                ...
                OB_FAIL(index_part_info.replace_final_location_key(*ctx.get_exec_ctx(),
                                                                 ap->index_id_,
                                                                 true))) {
                                                              ...
                                                              bool &is_reliable)
{
  ...
  if (is_reliable) {
    for (int64_t i = 0; OB_SUCC(ret) && i < result_helpers.count(); ++i) {
      ...
      } else if (OB_FAIL(fill_cost_table_scan_info(ctx,
                                                   path->est_cost_info_,
                                                      ...
                                                      double &index_back_row_count)
{
  ...
  if (OB_ISNULL(ctx.get_query_ctx())) {
    ...
    if (ctx.get_query_ctx()->check_opt_compat_version(COMPAT_VERSION_4_2_5_BP7)) {
      // refine row count
                                                          ...
                                                          AccessPath *path)
{
  ...
  if (OB_ISNULL(path) || OB_ISNULL(table_meta_info = path->est_cost_info_.table_meta_info_)) {
    ...
    OZ (fill_cost_table_scan_info(ctx,
                                  est_cost_info,
                                  path->output_row_count_,
                                  logical_row_count,
                                  physical_row_count,
...
int ObAccessPathEstimation::update_table_stat_info_by_default(AccessPath *path)
{
  ...
  } else if (path->output_row_count_ > 0) {
    OptTableMetas &table_metas = path->parent_->get_plan()->get_basic_table_metas();
    OptTableMeta *table_meta = table_metas.get_table_meta_by_table_id(path->table_id_);
    ...
    for (int64_t i = 0; OB_SUCC(ret) && i < paths.count(); ++i) {
      if (OB_ISNULL(paths.at(i))) {
        ...
        double &index_back_row_count = est_cost_info.index_meta_info_.is_index_back_ ?
                                        paths.at(i)->index_back_row_count_ :
                                        dummy_row_count;
