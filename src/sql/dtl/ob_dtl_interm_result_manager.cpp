      ...
      ObDTLIntermResultInfo *> &entry)
{
  ...
  if (OB_NOT_NULL(block_buf_)) {
    if (OB_SUCCESS != entry.second->ret_) {
      ...
      if (OB_FAIL(DTL_IR_STORE_DO(*entry.second, append_block, block_buf_, size_, true))) {
      ...
      ObDTLIntermResultInfo *> &entry)
{
  ...
  if (OB_NOT_NULL(block_buf_)) {
    if (OB_SUCCESS != entry.second->ret_) {
      ...
      if (OB_FAIL(DTL_IR_STORE_DO(*entry.second, append_block_payload,
                                           ...
                                           ObDTLMemProfileInfo *mem_profile_info)
{
  ...
  if (OB_ISNULL(mem_profile_info)) {
    ...
    if (OB_SUCC(ret) && dumped) {
      ...
      ++gc_.dump_count_;
      if (OB_FAIL(DTL_IR_STORE_DO(result_info, dump, false, true))) {
