...
int ObPlanCache::cache_evict_by_glitch_node()
{
  ...
  if (get_mem_hold() > get_mem_high()) {
    ...
    if (OB_FAIL(cache_key_node_map_.foreach_refactored(traverse_op))) {
      ...
      // Evict least-recently-used first: StmtStat::weight() is higher when last_active_timestamp_
      // is newer (smaller time interval). Sort ascending by weight so the front is LRU.
