...
#define MAKE_EXPR_BUFFER(expr_buffer, expr_idx, expr_count, result)            \
  do {                                                                         \
    CK (OB_NOT_NULL(ctx));                                                     \
    CK (OB_NOT_NULL(ctx->func_));                                              \
    if (OB_SUCC(ret) && expr_idx != nullptr && expr_count > 0) {               \
      if (expr_count > (expr_buffer.local_buffer_cnt_ - expr_buffer.already_used_cnt_)) { \
        int64_t alloc_size = expr_count * sizeof(decltype(*result));            \
        result = static_cast<decltype(result)>(expr_buffer.allocator_.alloc(alloc_size)); \
        if (OB_ISNULL(result)) {                                               \
          ret = OB_ALLOCATE_MEMORY_FAILED;                                     \
          LOG_WARN("failed to alloc memory for sql expr buffer", K(expr_count)); \
        }                                                                      \
      } else {                                                                 \
        result = reinterpret_cast<decltype(result)>(reinterpret_cast<void*>    \
                 (expr_buffer.local_buffer_ + expr_buffer.already_used_cnt_)); \
        expr_buffer.already_used_cnt_ += expr_count;                           \
        expr_buffer.local_used_cnt_ += expr_count;                             \
      }                                                                        \
...
static const int SPI_EXPR_BUFFER_LOCAL_CNT = 50;
RLOCAL(ObSqlExpression*, SPI_EXPR_BUFFER_LOCAL_BUFFER[SPI_EXPR_BUFFER_LOCAL_CNT]);
RLOCAL_INIT(int32_t, already_used_cnt, 0);

class ObSPIExprBuffer
{
public:
  ObSPIExprBuffer()
    : already_used_cnt_(already_used_cnt),
      local_buffer_cnt_(SPI_EXPR_BUFFER_LOCAL_CNT),
      local_used_cnt_(0),
      local_buffer_(SPI_EXPR_BUFFER_LOCAL_BUFFER),
      allocator_("SPIExprBuffer", OB_MALLOC_NORMAL_BLOCK_SIZE, MTL_ID()) {}
...
public:
  ...
  int32_t local_used_cnt_;
  ObSqlExpression** local_buffer_;
  ObArenaAllocator allocator_;
};

PlMemEntifyDestroyGuard::~PlMemEntifyDestroyGuard()
{
  if (nullptr != ref_ && need_destory_) {
...
int ObSPIResultSet::init_memory_context(sql::ObSQLSessionInfo &session_info, lib::MemoryContext mem_context)
{
  int ret = OB_SUCCESS;
  lib::ContextParam param;
  if (mem_context != nullptr) {
    ...
    if (OB_FAIL(CURRENT_CONTEXT->CREATE_CONTEXT(mem_context_, param))) {
      ...
      need_destory_mem_context_ = true;
    }
  }
  if (OB_SUCC(ret)) {
...
int ObSPIResultSet::init_result_set(sql::ObSQLSessionInfo &session_info)
{
  int ret = OB_SUCCESS;
  CK (OB_NOT_NULL(mem_context_));
  if (OB_SUCC(ret) && !is_inited_result_set_) {
    is_inited_result_set_ = true;
    result_set_ = new (buf_) ObResultSet(session_info, mem_context_->get_arena_allocator());
...
int ObSPIResultSet::begin_nested_session(ObSQLSessionInfo &session)
{
  ...
  OX (origin_audit_record_ = &session.get_raw_audit_record());
  OX (session.switch_audit_record_ptr(&audit_record_));
  return ret;
}
...
{
  int ret = OB_SUCCESS;
  CK (OB_NOT_NULL(origin_audit_record_));
  OX (session.switch_audit_record_ptr(origin_audit_record_));
...
int ObSPIResultSet::init_spi_cursor(ObSQLSessionInfo &session_info, uint64_t mem_limit  )
{
  int ret = OB_SUCCESS;
  cursor_prefetch_cache_ = (ObSPICursor*)get_memory_ctx()->get_arena_allocator().alloc(sizeof(ObSPICursor));
  OV (OB_NOT_NULL(cursor_prefetch_cache_), OB_ALLOCATE_MEMORY_FAILED);
  OX (new (cursor_prefetch_cache_) ObSPICursor(get_memory_ctx()->get_arena_allocator(), &session_info));
  if (OB_SUCC(ret)) {
    mem_limit = mem_limit == OB_INVALID_SIZE ? GCONF._chunk_row_store_mem_limit : mem_limit;
    OZ (cursor_prefetch_cache_->row_store_.init(mem_limit,
                                       ...
                                       ObObjParam &result)
{
  ...
  OZ (ctx->exec_ctx_->get_pl_top_context(pl_top_context));
  CK (OB_NOT_NULL(pl_ctx->get_pl_exec_info()));
  OX (expr_alloc = pl_ctx->get_pl_exec_info()->is_cached_ ? &pl_top_context->get_exec_state_allocator() : pl_ctx->get_pl_exec_info()->expr_ctx_op_alloc_);
  ...
  if (OB_SUCC(ret)) {
    ...
    if (1 == expr.get_expr_items().count()) { // 没有入参, 直接计算
      ...
      OZ (ObSQLUtils::calc_sql_expression_without_row(*ctx->exec_ctx_, expr, result, expr_alloc, true));
    }
    ObExprResType type;
...
{
  int ret = OB_SUCCESS;
  const ParamStore local_param_store((ObWrapperAllocator(*allocator)));
  const ParamStore *v_param_store = OB_NOT_NULL(param_store) ? param_store : &local_param_store;
  CK (OB_NOT_NULL(session));
  CK (OB_NOT_NULL(result));
  OZ (ObSQLUtils::se_calc_const_expr(
    session, expr, *v_param_store, *allocator, session->get_cur_exec_ctx(), *result));
                                       ...
                                       ObObjParam *result)
{
  ...
  if (OB_FAIL(ret)) {
    ...
    OZ (ctx->get_calc_once_result(expr_idx, local_result));
    if (OB_FAIL(ret)) {
    } else if (OB_NOT_NULL(local_result)) {
...
int ObSPIService::spi_set_symbol(pl::ObPLExecCtx *ctx,
                                                 const int64_t result_idx,
                                                 ObObjParam *result)
{
  int ret = OB_SUCCESS;
  ...
  if (OB_SUCC(ret) && result_idx != OB_INVALID_INDEX) {
    ...
    if (!result->is_ext()) { // baisic variable path
      ...
      if (local_variable.get_meta().get_scale() != SCALE_UNKNOWN_YET) {
        result->ObObj::set_scale(local_variable.get_meta().get_scale());
        result->set_accuracy(local_variable.get_accuracy());
      } else if (result->get_accuracy() == invalid_accuracy) {
        result->set_accuracy(local_variable.get_accuracy());
      }
      if (result->is_null()) {
        result->set_param_meta(local_variable.get_param_meta());
      }
      if (has_lob_header) {
        result->ObObj::set_has_lob_header();
      }
      if (is_ref_cursor && result->is_null()) {
        OZ (spi_add_ref_cursor_refcount(ctx, &local_variable, -1)); // close local variable cursor
    ...
    } else if (!is_ref_cursor) { // composite path, not cursor
      CK (result->get_meta().get_extend_type() != PL_REF_CURSOR_TYPE);
      OX (result->set_udt_id(local_variable.get_udt_id()));
      OZ (ObUserDefinedType::deep_copy_obj(*ctx->allocator_, *result, tmp));
      OZ (ObUserDefinedType::destruct_objparam(
        *ctx->allocator_, local_variable, ctx->exec_ctx_->get_my_session()));
      OX (local_variable = tmp);
    }
    OX (local_variable.set_param_meta());
  }
  return ret;
}
                                ...
                                ObObjParam *result)
{
  ...
  CK (OB_NOT_NULL(ctx));
  CK (OB_NOT_NULL(expr_allocator = ctx->get_top_expr_allocator()));

  // calculate expression
  if (OB_SUCC(ret)) {
    const ObPostExprItem &item = expr->get_expr_items().at(0);
    ObItemType expr_type = item.get_item_type();
    if (IS_CONST_TYPE(expr_type)) {
      const ObObj &value = item.get_obj();
      ...
      OZ (calc_common_expr(ctx, expr, result));
    }
  }

  OX (result->set_is_pl_mock_default_param(expr->get_is_pl_mock_default_expr()));
  OX (result->set_param_meta());
...
int ObSPIService::calc_common_expr(ObPLExecCtx *ctx, const ObSqlExpression *expr, ObObjParam *result)
{
  int ret = OB_SUCCESS;
  ...
  if (OB_SUCC(ret)) {
    ObExprResType result_type;
    ObIAllocator *cache_alloc = pl_ctx->get_pl_exec_info()->is_cached_ ? &pl_top_context->get_exec_state_allocator()
                                                                       : pl_ctx->get_pl_exec_info()->expr_ctx_op_alloc_;
    OZ (ObSQLUtils::calc_sql_expression_without_row(*ctx->exec_ctx_, *expr, *result, cache_alloc, true), KPC(expr));
    OZ (get_result_type(*ctx, *expr, result_type));
    OX (result->set_param_meta(result_type));
      ...
      if (OB_SUCCESS != ret && ctx->exec_ctx_->get_my_session()->is_in_transaction()) {
        if (has_implicit_savepoint) {
...
#ifdef OB_BUILD_ORACLE_PL
        } else if (ctx->exec_ctx_->get_my_session()->associated_xa() && OB_TRANS_XA_BRANCH_FAIL != ret) {
          if (OB_SUCCESS !=
              (tmp_ret = ObDbmsXA::xa_rollback_savepoint(*ctx->exec_ctx_))) {
            LOG_WARN("xa trans rollback to save point failed", K(ret), K(tmp_ret));
          }
#endif
          ...
          if (OB_SUCCESS !=
            ...
            LOG_WARN("failed to implicit end trans", K(tmp_ret));
          }
        }
      } else if (OB_SUCCESS == ret
                 && ctx->exec_ctx_->get_my_session()->get_local_autocommit()
                 && !ctx->exec_ctx_->get_my_session()->has_explicit_start_trans()
                 && !ctx->exec_ctx_->get_my_session()->associated_xa()) {
        int tmp_ret = OB_SUCCESS;
        if (OB_SUCCESS !=
            (tmp_ret = ObPLContext::implicit_end_trans(*ctx->exec_ctx_->get_my_session(), *ctx->exec_ctx_, false, true))) {
                                           ...
                                           ObObjParam *result)
{
  ...
  ObPLExecInfo *pl_exec_info = nullptr;
  ...
  CK (OB_NOT_NULL(pl_exec_info = state->get_cur_pl_ctx()->get_pl_exec_info()));
  ...
  if (OB_SUCC(ret)) {
    ...
    exec_ctx_bak.backup(*exec_ctx);
    exec_ctx->set_physical_plan_ctx(pl_exec_info->phy_plan_ctx_);
    exec_ctx->set_expr_op_ctx_store(pl_exec_info->expr_op_ctx_store_);
    exec_ctx->set_expr_op_size(pl_exec_info->expr_op_size_);
                                           ...
                                           ObObjParam *result)
{
  ...
  if (OB_SUCC(ret)) {
    ...
    OZ (ObSQLUtils::calc_sql_expression_without_row(exec_ctx, *sql_expr, *result, &allocator, true));
                                   ...
                                   bool is_default)
{
  ...
  if (OB_ISNULL(ctx) || OB_ISNULL(expr) || OB_ISNULL(value)) {
    ...
    if (T_OP_GET_SYS_VAR == expr_type || T_OP_GET_USER_VAR == expr_type) {
      ...
      OV (3 <= expr->get_expr_items().count(), OB_ERR_UNEXPECTED, expr->get_expr_items().count());
                                    ...
                                    bool is_dbms_sql)
{
  ...
  if (OB_SUCC(ret)) {
    ObSPIResultSet *spi_result = nullptr;
    OZ (ctx->get_spi_result_set(spi_result));
    if (OB_SUCC(ret)) {
      ...
      // in mysql mode, select into @user_var stmt will nested enter spi_inner_execute,
      // for second time which is set stmt, if use get_mem_context_for_sql to obatin memory context, will reuse memory, which is not expected.
      if (type != stmt::T_VARIABLE_SET) {
        OZ (ctx->get_mem_context_for_sql(mem_context));
      }
      OZ (spi_result->init_memory_context(*session, mem_context));
      OX (spi_result->set_flags(is_forall, is_dynamic_sql, is_dbms_sql, false));
...
// #endif
      ...
      if (OB_SUCC(ret) && is_ddl_stmt) {
        ...
        OX (session->get_pl_top_context()->set_disable_pl_exec_cache(true));
      }
    ...
    if (OB_FAIL(ret)) {
      ObString sql_str = nullptr == sql ? ps_sql : sql;
      ObString print_sql(MIN(pl::ObPLParser::MAX_PRINT_LEN, sql_str.length()), sql_str.ptr());
      LOG_WARN("spi_execute failed", K(ret), K(print_sql));
    }
  }
...
void ObSPIService::spi_restore_sqlcode(pl::ObPLExecCtx *ctx)
{
  if (OB_NOT_NULL(ctx)) {
    ctx->restore_sql_code_info();
  }
}

void ObSPIService::spi_save_sqlcode(pl::ObPLExecCtx *ctx)
{
  if (OB_NOT_NULL(ctx)) {
    ctx->set_saved_sql_code_info();
  }
                                          ...
                                          bool for_update)
{
  ...
  ObSPIExprBuffer expr_buffer;
  const ObSqlExpression **into_exprs = nullptr;

  MAKE_EXPR_BUFFER(expr_buffer, into_exprs_idx, into_count, into_exprs);
                              ...
                              bool for_update)
{
  ...
  const ObSqlExpression **into_exprs = nullptr;
  MAKE_EXPR_BUFFER(expr_buffer, param_exprs_idx, param_count, param_exprs);
  MAKE_EXPR_BUFFER(expr_buffer, into_exprs_idx, into_count, into_exprs);
    ...
    SMART_VAR(PLPrepareResult, pl_prepare_result) {
      ...
      if (OB_SUCC(ret)) {
        ...
        if (OB_SUCC(ret)) {
          ...
          for (int64_t i = 0; OB_SUCC(ret) && i < pl_prepare_result.result_set_->get_value_exprs().count(); ++i) {
            expr = NULL;
            if (OB_FAIL(ObPLExprCopier::copy_expr(expr_factory,
                                                  pl_prepare_result.result_set_->get_value_exprs().at(i),
                                                  expr))) {
              LOG_WARN("failed to copy expr", K(i), K(pl_prepare_result.result_set_->get_value_exprs().at(i)), K(ret));
            } else if (OB_ISNULL(expr)) {
              ret = OB_ERR_UNEXPECTED;
              LOG_WARN("failed to copy expr", K(expr), K(ret));
            } else if (OB_FAIL(prepare_result.value_exprs_.push_back(expr))) {
              LOG_WARN("push_back error", K(ret));
            }
          }
                                  ...
                                  common::ColumnsFieldArray *field_list)
{
  ...
  CK (OB_NOT_NULL(GCTX.sql_engine_));
  if (OB_SUCC(ret)) {
    HEAP_VARS_2((ObSPIResultSet, spi_result), (ObResultSet, result_set, *session, allocator)) {
      stmt_type = stmt::T_NONE;
      bool is_prepare_with_param = !is_dbms_sql && OB_NOT_NULL(params);
      ObSqlCtx &context = spi_result.get_sql_ctx();
      ObSchemaGetterGuard schema_guard(share::schema::ObSchemaMgrItem::MOD_PL_PREPARE_RESULT);
      OV ((is_dbms_sql && NULL != field_list)
          || (!is_dbms_sql && NULL == field_list),
          OB_ERR_UNEXPECTED, is_dbms_sql, field_list);
      if (OB_SUCC(ret)) {
        spi_result.set_result_set(&result_set);
        context.is_dynamic_sql_ = true;
        context.is_dbms_sql_ = is_dbms_sql;
        context.is_cursor_ = false;
        ...
        context.prepare_params_ = lib::is_mysql_mode() ? nullptr : params;
        context.is_from_pl_ = true;
        context.is_prepare_with_params_ = !is_dbms_sql && OB_NOT_NULL(params);
        OZ (GCTX.schema_service_->get_tenant_schema_guard(session->get_effective_tenant_id(), schema_guard));
        OX (context.schema_guard_ = &schema_guard);
      }
#ifdef ERRSIM
      OX (ret = OB_E(EventTable::EN_SPI_SQL_EXEC) OB_SUCCESS);
#endif
      if (OB_SUCC(ret)) {
        ...
        DISABLE_SQL_MEMLEAK_GUARD;
        OZ (GCTX.sql_engine_->stmt_prepare(sql_str.string(), context, *spi_result.get_result_set(), true));
        OX (stmt_id = static_cast<ObPsStmtId>(spi_result.get_result_set()->get_statement_id()));
        OX (stmt_type = static_cast<stmt::StmtType>(spi_result.get_result_set()->get_stmt_type()));
        OZ (ob_write_string(allocator, spi_result.get_result_set()->get_stmt_ps_sql(), ps_sql, true));
        OX (for_update = spi_result.get_result_set()->get_is_select_for_update());
        OX (hidden_rowid = spi_result.get_result_set()->has_hidden_rowid());
        OX (into_cnt = spi_result.get_result_set()->get_external_retrieve_info().into_exprs_cnt_);
        OX (skip_locked = spi_result.get_result_set()->is_skip_locked());

        if (OB_SUCC(ret) && NULL != field_list) {
          CK (OB_NOT_NULL(spi_result.get_result_set()->get_field_columns()));
          if (OB_SUCC(ret) && spi_result.get_result_set()->get_field_columns()->count() > 0) {
        ...
        if (OB_SUCC(ret)) {
          ...
          if (lib::is_mysql_mode() || !is_prepare_with_param) {
            exec_param_cnt = ObStmt::is_dml_stmt(stmt_type)
                              ? spi_result.get_result_set()->get_external_params_cnt()
                              : spi_result.get_result_set()->get_param_fields()->count();
          } else {
            exec_param_cnt = spi_result.get_result_set()->get_parse_question_mark_cnt();
          }
          if (spi_result.get_result_set()->is_returning() && 0 == into_cnt) {
              ret = OB_ERR_MISSING_INTO_KEYWORD;
              LOG_WARN("ORA-00925: missing INTO keyword", K(ret),
                      K(spi_result.get_result_set()->is_returning()), K(into_cnt));
          } else {
            /*!
        ...
        if (OB_SUCC(ret)) {
          bool remove_into = false;
            ...
            } else if (spi_result.get_result_set()->get_into_exprs_cnt() == 0) {
              ret = OB_ERR_MISSING_INTO_KEYWORD;
          ...
          } else if (stmt::T_SELECT == stmt_type || spi_result.get_result_set()->is_returning()) {
            /*
          ...
          if (OB_SUCC(ret) && remove_into) {
            CK (!spi_result.get_result_set()->get_stmt_ps_sql().empty());
            CK ((spi_result.get_result_set()->get_stmt_ps_sql().length() + 1) < OB_MAX_SQL_LENGTH);
            OX (sql_str.reset());
            OZ (sql_str.append(spi_result.get_result_set()->get_stmt_ps_sql()));
          }
        }
                                             ...
                                             int64_t stmt_type)
{
  ...
  if (OB_SUCC(ret) && is_forall && OB_NOT_NULL(exec_params)) {
    OZ (prepare_forall_batch_params(spi_result, exec_params, allocator));
  }
  return ret;
}
                                        ...
                                        bool is_type_record)
{
  ...
  for (int64_t i = 0; OB_SUCC(ret) && i < param_count; ++i) {
    ...
    if (OB_SUCC(ret)) {
      ...
      if (params[i]->is_pl_extend() &&
          ob_is_xml_pl_type(params[i]->get_type(), params[i]->get_udt_id())) {
        ...
        OZ (GCTX.schema_service_->get_tenant_schema_guard(session->get_effective_tenant_id(),
                                                          schema_guard));
        OZ (cast_ctx.exec_ctx_->get_subschema_id_by_udt_id(udt_id,
                                                            subschema_id,
                                                            &schema_guard));
        OX (new_param.set_subschema_id(subschema_id));
        OZ (ObObjCaster::to_type(ObUserDefinedSQLType, cast_ctx, *params[i], new_param));
        OX (new_param.set_udt_id(udt_id));
  ...
  if (OB_SUCC(ret) && is_forall) {
    ...
    ParamStore forall_param_store((ObWrapperAllocator(allocator)));
    OZ (forall_param_store.reserve(param_store.count()));
    OZ (ObSQLUtils::get_one_group_params(actual_pos, param_store, forall_param_store));
    OZ (param_store.assign(forall_param_store));
  }
  ...
  if (OB_FAIL(ret)) {
  ...
  } else if (is_forall && !ObStmt::is_pdml_supported_stmt(stmt_type)) {
    /* forall need rollback to for loop */
    ret = OB_BATCHED_MULTI_STMT_ROLLBACK;
    LOG_TRACE("cannot batch execute", K(ret), K(sql), K(stmt_type));
  } else {
    OZ (SMART_CALL(spi_inner_execute(ctx,
                                     *ctx->get_top_expr_allocator(),
                                     ...
                                     for_update,
                                     true, /*is_dynamic_sql*/
                                     &out_using_params)), K(param_store));
  }
  return ret;
}
                                              ...
                                              ObObjParam &param)
{
  ...
  if (OB_SUCC(ret)) {
    ...
    OZ (pl_manager.get_package_var_val(
      resolve_ctx, *exec_ctx, package_id, index, value));
                                  ...
                                  int64_t cursor_index)
{
  ...
  if (OB_SUCC(ret)) {
    ...
    MAKE_EXPR_BUFFER(expr_buffer, sql_param_exprs_idx, sql_param_count, sql_param_exprs);
                                  ...
                                  bool skip_locked)
{
  ...
  MAKE_EXPR_BUFFER(expr_buffer, sql_param_idx, sql_param_count, sql_param_exprs);
  MAKE_EXPR_BUFFER(expr_buffer, actual_param_idx, cursor_param_count,
                    actual_param_exprs);
...
// #endif
    ...
    do {
      {
        ...
        OZ (spi_result->init_result_set(session_info));

        ObSPIRetryCtrlGuard retry_guard(retry_ctrl, *spi_result, session_info, ctx, ret, is_retry, !cursor.is_ps_cursor());
        ...
        OX (cursor.get_cursor_handler()->set_enable_streaming_cursor_prefetch(session_info.enable_streaming_cursor_prefetch()));
  ...
  if (OB_FAIL(ret)) {
    ObString sql_str = sql.empty() ? ps_sql : sql;
    ObString print_sql(MIN(pl::ObPLParser::MAX_PRINT_LEN, sql_str.length()), sql_str.ptr());
    LOG_WARN("streaming_cursor_open failed", K(ret), K(print_sql));
  }
  return ret;
}
                                          ...
                                          int64_t orc_max_ret_rows)
{
  ...
  if (OB_SUCC(ret)) {
    ...
    CK (OB_NOT_NULL(ctx->exec_ctx_->get_sql_ctx()));
    OZ (ctx->get_mem_context_for_sql(mem_context));
    OZ (spi_result->init_memory_context(session_info, mem_context));
    OX (spi_result->set_flags(false, false, is_dbms_cursor, false));
...
// #endif
    ...
    if (OB_FAIL(ret)) {
      ObString sql_str = sql.empty() ? ps_sql : sql;
      ObString print_sql(MIN(pl::ObPLParser::MAX_PRINT_LEN, sql_str.length()), sql_str.ptr());
      LOG_WARN("unstreaming_cursor_open failed", K(ret), K(print_sql));
    }
  }
  return ret;
}
                                  ...
                                  bool is_type_record)
{
  ...
  if (OB_SUCC(ret)
    ...
    */
    //if has async pl commit , wait preceding transaction end
    if (session->is_pl_async_commit()) {
      ...
      if (OB_NOT_NULL(pl_end_trans_cb.get_tx_desc())) {
        if (OB_FAIL(pl_end_trans_cb.wait_tx_end(plan_ctx))) {
          LOG_WARN("[PL_ASYNC_COMMIT] Cursor fetch wait preceding transaction end failed", K(ret));
        }
      }
    }
    if (OB_FAIL(ret)) {
  ...
  bool fetch_prefetch_data = false;
  if (OB_FAIL(ret)) {
  } else if (cursor->is_streaming()) {
    CK (OB_NOT_NULL(spi_result = cursor->get_cursor_handler()));
    OX (fetch_prefetch_data = (OB_NOT_NULL(spi_result) && OB_NOT_NULL(spi_result->get_spi_cursor()) && spi_result->get_spi_cursor()->cur_ < spi_result->get_spi_cursor()->row_store_.get_row_cnt()));
    if (OB_SUCC(ret) && !fetch_prefetch_data) {
...
#define GET_RESULT                                                                    \
  do {                                                                                \
    if (OB_FAIL(ret)) {                                                               \
    } else if (!cursor->is_streaming() || fetch_prefetch_data) {                      \
      bool can_retry = true;                                                          \
      void *spi_cursor =  cursor->is_streaming() ? static_cast<void*>(spi_result->get_spi_cursor()) : static_cast<void*>(cursor->get_cursor_handler()); \
      ret = get_result(ctx,                                                           \
                   spi_cursor,                                                        \
                   false,                                                             \
                   into_exprs,                                                        \
...
#undef GET_RESULT
    ...
    if (cursor->is_streaming() && !fetch_prefetch_data) {
      spi_result->end_cursor_stmt(ctx, ret);
      cursor->set_last_execute_time(ObTimeUtility::current_time());
    }
...
int ObSPIService::dbms_cursor_close(ObExecContext &exec_ctx, ObPLCursorInfo &cursor)
{
  ...
  if (cursor.isopen()) {
    ...
    OZ (cursor.ObPLCursorInfo::close(*exec_ctx.get_my_session(),
...
int ObSPIService::spi_adjust_error_trace(pl::ObPLExecCtx *ctx, int level)
{
  ...
  CK (OB_NOT_NULL(ctx->exec_ctx_));
  CK (OB_NOT_NULL(ctx->exec_ctx_->get_my_session()));
  CK (OB_NOT_NULL(pl_top_context = ctx->exec_ctx_->get_my_session()->get_pl_top_context()));
  CK (OB_NOT_NULL(exec_stack = pl_top_context->get_exec_stack()));
  if (OB_SUCC(ret) && OB_NOT_NULL(pl_top_context->get_call_stack_trace_directly())) {
    OZ (pl_top_context->get_call_stack_trace_directly()->format_error_trace(exec_stack->count(), level));
    CK (OB_NOT_NULL(sqlcode_info = ctx->exec_ctx_->get_my_session()->get_pl_sqlcode_info()));
    OX (sqlcode_info->set_sqlcode(pl_top_context->get_call_stack_trace_directly()->get_last_error_code()));
  }
#endif
  return ret;
}
  ...
  pl::ObPLExecCtx *ctx, int64_t code, bool is_pop_warning_buf, int level)
{
  ...
  if (is_pop_warning_buf && OB_NOT_NULL(stack_warning_buf) && stack_warning_buf->count() > 0) {
    int64_t idx = stack_warning_buf->count() - 1;
    OX (stack_warning_buf->at(idx).~ObWarningBuffer());
    OX (stack_warning_buf->pop_back());
    OX (ctx->exec_ctx_->get_my_session()->set_show_warnings_buf(OB_SUCCESS));
  }

#ifdef OB_BUILD_ORACLE_PL
  if (OB_FAIL(ret) || lib::is_mysql_mode() || OB_ISNULL(pl_top_context->get_call_stack_trace())) {
  } else if (!is_pop_warning_buf) {
    OZ (pl_top_context->get_call_stack_trace()->format_error_trace(*exec_stack, level));
  } else {
    OZ (pl_top_context->get_call_stack_trace()->format_error_trace(exec_stack->count(), level));
  }
#endif
...
int ObSPIService::spi_get_pl_exception_code(pl::ObPLExecCtx *ctx, int64_t *code)
{
  ...
  if (lib::is_mysql_mode()
      && OB_NOT_NULL(wb = common::ob_get_tsi_warning_buffer())) {
    ObIArray<ObWarningBuffer> *stack_warning_buf = NULL;
    OZ (sqlcode_info->get_stack_warning_buf(stack_warning_buf));
    CK (OB_NOT_NULL(stack_warning_buf));
    OZ (stack_warning_buf->push_back(*wb));
  }
  return ret;
}
  ...
  if (OB_FAIL(ret)) {
    ...
    if (!is_signal && *error_code != -1 && NULL == sql_state) {
      ...
      if (type == ObPLConditionType::NOT_FOUND) {
        sqlcode = ER_SIGNAL_NOT_FOUND;
      } else {
        sqlcode = ER_SIGNAL_EXCEPTION;
      }
    }
  } else {
  ...
  if (OB_SUCC(ret)) {
    if (OB_ISNULL(errmsg_expr)) {
      if (is_signal) {
        ...
        OX (error_msg = stack_warning_buf->at(idx).get_err_msg());
      }
    } else {
  ...
  if (OB_FAIL(ret)) {
    ...
    if (ObPLConditionType::SQL_WARNING == type) {
      if (OB_ISNULL(errcode_expr)) {
        ...
        if (OB_NOT_NULL(wb)) {
          wb->append_warning(err_msg, sqlcode, sql_state);
        }
      }
      if (is_signal) {
        sqlcode = OB_SUCCESS;
      ...
      if (is_signal) {
        LOG_MYSQL_USER_ERROR(OB_SP_RAISE_APPLICATION_ERROR,
                            ...
                            error_msg.ptr());
        if (OB_NOT_NULL(wb)) {
          wb->set_error_code(sqlcode);
          wb->set_sql_state(sql_state);
        }
      } else {
        if (OB_NOT_NULL(wb)) {
          wb->set_error(err_msg, sqlcode);
                                      ...
                                      int32_t upper)
{
  ...
  if (OB_SUCC(ret)) {
    ...
    if (OB_SUCC(ret)) {
      ObSEArray<int32_t, OB_DEFAULT_SE_ARRAY_COUNT> indices;
      for (int32_t i = lower; OB_SUCC(ret) && i <= upper; ++i) {
        OZ (indices.push_back(i));
      }
      OZ (spi_sub_nestedtable_generate_collection(ctx, src_expr, dst_type, dst_coll_obj, index_obj, indices));
    }
  }
#endif
  return ret;
...
int ObSPIService::spi_sub_nestedtable_indices_of_collection(ObPLExecCtx *ctx,
                                                            int64_t src_expr_idx,
                                                            int64_t dst_coll_idx,
                                                            int64_t index_idx,
                                                            int32_t lower,
                                                            ...
                                                            int64_t expr_next)
{
  int ret = OB_SUCCESS;
#ifndef OB_BUILD_ORACLE_PL
  UNUSEDx(ctx, src_expr_idx, dst_coll_idx, index_idx, lower, upper, with_between_bound, expr_first, expr_last, expr_next);
#else
  CK (OB_NOT_NULL(ctx));
  CK (OB_NOT_NULL(ctx->params_));
  CK (OB_NOT_NULL(ctx->func_));
  CK (OB_LIKELY(src_expr_idx >= 0 && src_expr_idx < ctx->func_->get_expressions().count()));
  CK (OB_LIKELY(expr_first >= 0 && expr_first < ctx->func_->get_expressions().count()));
  CK (OB_LIKELY(expr_last >= 0 && expr_last < ctx->func_->get_expressions().count()));
  CK (OB_LIKELY(expr_next >= 0 && expr_next < ctx->func_->get_expressions().count()));
  CK (OB_LIKELY(dst_coll_idx >= 0 && dst_coll_idx < ctx->func_->get_variables().count()));
  CK (OB_LIKELY(dst_coll_idx >= 0 && dst_coll_idx < ctx->params_->count()));
  if (OB_SUCC(ret) && with_between_bound) {
    CK (OB_LIKELY(lower <= upper));
  }
  ...
  if (OB_SUCC(ret)) {
    ...
    if (OB_SUCC(ret)) {
      ObObjParam first_obj, next_obj;
      if (with_between_bound) {
        // exists is checked when ObPLCodeGenerator::generate_indices_with_between_bound
...
int ObSPIService::spi_sub_nestedtable_values_of_index_collection(ObPLExecCtx *ctx,
                                                                int64_t src_expr_idx,
                                                                int64_t dst_coll_idx,
                                                                int64_t index_idx,
                                                                int64_t expr_first,
                                                                ...
                                                                int64_t expr_value)
{
  int ret = OB_SUCCESS;
#ifndef OB_BUILD_ORACLE_PL
  UNUSEDx(ctx, src_expr_idx, dst_coll_idx, index_idx, expr_first, expr_last, expr_next, expr_value);
#else
  CK (OB_NOT_NULL(ctx));
  CK (OB_NOT_NULL(ctx->params_));
  CK (OB_NOT_NULL(ctx->func_));
  ...
  CK (OB_LIKELY(expr_last >= 0 && expr_last < ctx->func_->get_expressions().count()));
  CK (OB_LIKELY(expr_next >= 0 && expr_next < ctx->func_->get_expressions().count()));
  CK (OB_LIKELY(expr_value >= 0 && expr_value < ctx->func_->get_expressions().count()));
  CK (OB_LIKELY(dst_coll_idx >= 0 && dst_coll_idx < ctx->func_->get_variables().count()));
  CK (OB_LIKELY(dst_coll_idx >= 0 && dst_coll_idx < ctx->params_->count()));
...
int ObSPIService::spi_sub_nestedtable_generate_collection(pl::ObPLExecCtx *ctx,
                                               ...
                                               ObIArray<int32_t> &indices)
{
  int ret = OB_SUCCESS;
#ifndef OB_BUILD_ORACLE_PL
  UNUSEDx(ctx, src_expr, dst_type, dst_coll_obj, index_obj, indices);
#else
  const ObUserDefinedType *user_type = NULL;
    ...
    if (OB_SUCC(ret)) {
      ...
      for (; OB_SUCC(ret) && i < indices.count(); ++i) {
        ...
        if (OB_ERR_SUBSCRIPT_OUTSIDE_LIMIT == ret) {
          ...
          LOG_WARN("ORA-22160: element at index does not exist", K(indices.at(i)), K(ret));
        }
        OX (dst_data[i].reset());
        if (OB_FAIL(ret)) {
      ...
      if (OB_FAIL(ret)) {
        if (OB_NOT_NULL(dst_data)) {
          for (int64_t j = 0; j < i - 1; ++j) {
            ObUserDefinedType::destruct_objparam(*coll_alloc, dst_data[j], ctx->exec_ctx_->get_my_session());
          }
                                                 ...
                                                 bool &has_init_value)
{
  ...
  if (OB_FAIL(ret)) {
    ...
    has_init_value = true;
    if (is_pkg_type) {
      OZ (ObSPIService::spi_calc_package_expr(pl_ctx, package_id, member->get_default(), true, &result));
    } else {
                                       ...
                                       ObObj *new_element)
{
  ...
  int64_t init_size = OB_INVALID_SIZE;
  CK (OB_NOT_NULL(ns));
  CK (OB_NOT_NULL(new_element));
  CK (OB_NOT_NULL(allocator));
  OZ (ns->get_user_type(collection_id, type));
  CK (OB_NOT_NULL(type));
  ...
  #ifdef OB_BUILD_ORACLE_PL
      if (OB_SUCC(ret) && collection_type->is_associative_array_type()
                        && (collection_type->get_element_type().is_package_type()
...
#define SET_COLLECTION_INFO \
  do { \
    if (OB_SUCC(ret)) { \
      coll.set_data(reinterpret_cast<ObObj*>(data), new_capacity); \
  ...
  if (OB_SUCC(ret)) {
    if (extend_mode && n > 0) {
      ...
      if (OB_SUCC(ret)) {
        int64_t new_capacity = coll.get_inner_capacity();
        if ((coll.get_count() + n) < coll.get_inner_capacity()) {
          data = coll.get_data();
        } else {
          new_capacity
            = (coll.get_inner_capacity() * 2) > (coll.get_count() + n)
              ? (coll.get_inner_capacity() * 2) : (coll.get_count() + n);
          data = coll.get_allocator()->alloc(sizeof(ObObj) * (new_capacity));
          if (OB_ISNULL(data)) {
            ret = OB_ALLOCATE_MEMORY_FAILED;
            ...
            for (int64_t i = coll.get_count(); OB_SUCC(ret) && i < new_capacity; ++i) {
...
#define ALLOC_ASSOC_ARRAY(TYPE, PROPERTY) \
  do { \
    if (OB_SUCC(ret)) { \
      ...
      } else if (old_capacity == assoc_array.get_inner_capacity()) { \
      } else if (OB_ISNULL(addr = static_cast<TYPE *>(allocator->get_allocator()->alloc(sizeof(TYPE) * assoc_array.get_inner_capacity())))) { \
        ret = OB_ALLOCATE_MEMORY_FAILED; \
    ...
    if (OB_SUCC(ret)) {
      for (int64_t i = 0; i < assoc_array.get_inner_capacity(); ++i) {
...
#define REALLOC_ASSOC_ARRAY(TYPE, PROPERTY) \
  do { \
    if (OB_SUCC(ret)) { \
      ...
      } else if (OB_ISNULL(addr = reinterpret_cast<TYPE*>(allocator->get_allocator()->alloc(sizeof(TYPE) * assoc_array.get_inner_capacity())))) { \
        ret = OB_ALLOCATE_MEMORY_FAILED; \
        LOG_WARN("alloc failed", K(ret), K(assoc_array), K(n), K(sizeof(TYPE) * (assoc_array.get_count() - n)), K(sizeof(TYPE) * assoc_array.get_count())); \
      } \
      if (old_capacity != assoc_array.get_inner_capacity()) { \
    ...
    // NOTE: init extend key avoid to deep copy core!!!
    if (OB_SUCC(ret) && old_capacity != assoc_array.get_inner_capacity()) {
      for (int64_t i = assoc_array.get_count() - n; i < assoc_array.get_inner_capacity(); ++i) {
        ObObj &key = assoc_array.get_key()[i];
        new(&key)ObObj();
      }
    }
  ...
  if (OB_SUCC(ret)) {
    ...
    if (OB_FAIL(ret)) {
      ...
      if (need_convert) {
        ...
        OZ (ObPLComposite::copy_element(src_tmp, *dest, *copy_allocator, true, ctx, ctx->exec_ctx_->get_my_session()));
        ...
        OZ (ObPLComposite::copy_element(src_ref, *dest, *copy_allocator, true, ctx, ctx->exec_ctx_->get_my_session()));
      }
...
int ObSPIService::spi_interface_impl(pl::ObPLExecCtx *ctx, const char *interface_name)
{
  ...
  if (OB_UNLIKELY(nullptr == interface_name || nullptr == ctx)) {
    ...
    if (nullptr != fp) {
      ret = fp(*ctx, *ctx->params_, tmp_result);
      if (OB_FAIL(ret)) {
      } else if ((ctx->func_->get_ret_type().is_composite_type() ||
        ...
        if (tmp_result.is_pl_extend() &&
          ...
          OZ (ObUserDefinedType::deep_copy_obj(*ctx->result_allocator_, tmp_result, *ctx->result_));
          ...
          OZ (deep_copy_obj(*ctx->result_allocator_, tmp_result, *ctx->result_));
        } else {
...
{
  int ret = OB_SUCCESS;
  UNUSEDx(ctx, into_exprs, into_count, out_params);
  ...
  for (int64_t i = 0; OB_SUCC(ret) && i < param_count; ++i) {
    ...
    if OB_SUCC (ret) {
      ...
      if (OB_FAIL(ret)) {
        ...
        const bool skip_set_accuracy_set = OB_NOT_NULL(ctx->func_)
                                          && pl::STANDALONE_ANONYMOUS == ctx->func_->get_proc_type()
                                          && ob_is_string_tc(result_type.get_type())
                                          && -1 == result_type.get_length();
        if (!skip_set_accuracy_set) {
      ...
      if (OB_SUCC(ret)) { //执行期参数必须从PL的内存空间拷贝到SQL自己的内存空间，以防止在SQL执行过程中参数被改掉
        ...
        if (!is_forall) { // forall场景, 不做拷贝, 上层代码transform_pl_ext_type会进行paramstore整体拷贝
          ...
          } else if (need_copy || into_count == 0) {
...
int ObSPIService::store_params_string(sql::ObSQLSessionInfo &session,
                                      ParamStore *params,
                                      char *params_value,
                                      int64_t &params_value_len)
{
  ...
  for (int i = 0; OB_SUCC(ret) && i < params->count(); ++i) {
    ...
    if (param.is_ext()) {
      ...
      OZ (param.print_sql_literal(params_value, params_value_len, pos, TZ_INFO(&session)));
      if (i != params->count() - 1) {
        OZ (databuff_printf(params_value, params_value_len, pos, ","));
      }
    }
  }
  OX (params_value_len = pos);
  return ret;
}
  ...
  ObPLExecCtx *ctx, ObSPIResultSet &spi_result, ParamStore *exec_params)
{
  ...
  if (OB_SUCC(ret)
      ...
      && OB_NOT_NULL(ctx->exec_ctx_)
      && OB_NOT_NULL(ctx->exec_ctx_->get_my_session())) {
    char *tmp_ptr = spi_result.get_local_param_value_buffer();
    int64_t tmp_len = spi_result.get_local_param_value_buffer_len();
...
int ObSPIService::prepare_forall_batch_params(ObSPIResultSet &spi_result,
                                              ParamStore *&exec_params,
                                              ObIAllocator &allocator)
{
  int ret = OB_SUCCESS;
  ...
  if (OB_SUCC(ret)) {
    if (array_binding_count <= 0) {
      ...
      LOG_WARN("array_binding_count is wrong", K(array_binding_count), K(ret));
        ...
        *exec_params, array_binding_count, allocator, batch_params, true))) {
      LOG_WARN("transform failed", K(ret));
    } else if (OB_ISNULL(batch_params)) {
      ret = OB_ERR_UNEXPECTED;
      LOG_WARN("unexpected null", K(ret));
    } else {
      exec_params = batch_params;
                                            ...
                                            ParamStore *&curr_params)
{
  ...
  curr_params = NULL;
  if (spi_result.is_stream_cursor()) { // stream cursor may cross multi sql stmt, cannot share same param store with static sql
    if (OB_ISNULL(exec_params = reinterpret_cast<ParamStore *>(param_allocator.alloc(sizeof(ParamStore))))) {
      ret = OB_ALLOCATE_MEMORY_FAILED;
      LOG_WARN("failed to alloc memory for exec params", K(ret));
    } else {
      new (exec_params) ParamStore( (ObWrapperAllocator(param_allocator)) );
    }
  } else {
    OZ (ctx->get_param_store_for_sql(*ctx->get_top_expr_allocator(), exec_params));
  }
  ...
  if (NULL == sql) {
    OZ (construct_exec_params(ctx, param_allocator, param_exprs, param_count,
      ...
      K(type), K(param_count), K(out_params), KPC(exec_params));

    OZ (store_params_string(ctx, spi_result, exec_params));
  }

  if (OB_SUCC(ret) && is_forall && OB_NOT_NULL(exec_params)) {
    OZ (prepare_forall_batch_params(spi_result, exec_params, param_allocator));
    OX (curr_params = exec_params);
  }
                             ...
                             ParamStore **array_params)
{
  ...
  bool is_forall = spi_result.is_forall();
  bool is_dynamic_sql = spi_result.is_dynamic_sql();
                                         ...
                                         bool is_type_record)
{
  ...
  if (OB_SUCC(ret)) {
    ...
    int64_t time_gap = cur_time - last_exec_time;
    ...
    OX (session->set_query_start_time(cur_time)); // query start time will restore by end_cursor_stmt when cursor fetch end
    ...
    if (OB_SUCC(ret)) {
      ...
      do {
        ret = OB_SUCCESS;
        bool can_retry = true;
        ObSPIRetryCtrlGuard retry_guard(retry_ctrl, spi_result, *session, ctx, ret, false);
        ...
        if (OB_SUCC(ret) && !is_bulk && spi_result.enable_streaming_cursor_prefetch()) {
          if (OB_FAIL(prefetch_streaming_cursor(session, cursor_info))) {
            if (!for_cursor || (for_cursor && ret != OB_READ_NOTHING)) {
               LOG_WARN("failed to prefetch_streaming_cursor, check if need retry", K(ret));
            }
          }
        }
        if (can_retry) {
  ...
  if (into_count > 1) {
    ...
    for (int64_t i = 0; OB_SUCC(ret) && i < actual_column_count; ++i) {
      ...
      if (is_null_row) {
        ...
        obj = cur_row.get_cell(i);
        const ObDataType *return_type = return_types != nullptr ? &return_types[i] : nullptr;
        OZ (try_cast_obj(ctx, into_exprs[i], column_types[i], exprs_not_null[i], pl_integer_ranges[i], row_desc.at(i), is_strict, cast_ctx, obj, return_type));
      }
      OZ (store_result(ctx, into_exprs[i], &column_types[i], obj));
    }
    ...
    */
    ...
    OZ (try_cast_multi_obj(ctx, into_exprs[0], column_types, type_count, exprs_not_null, pl_integer_ranges, row_desc, is_strict, cast_ctx, tmp_result, return_types, return_type_count));
    OZ (store_record_result(ctx, into_exprs[0], tmp_result));
  } else {
    /*
    ...
    */
    ...
    if (OB_SUCC(ret)) {
      ...
      if (is_null_row) {
        ...
        obj = cur_row.get_cell(0);
        const ObDataType *return_type = return_types != nullptr ? &return_types[0] : nullptr;
        OZ (try_cast_obj(ctx, into_exprs[0], column_types[0], exprs_not_null[0], pl_integer_ranges[0], row_desc.at(0), is_strict, cast_ctx, obj, return_type));
      }
      OZ (store_result(ctx, into_exprs[0], column_types, obj));
    }
  }
  return ret;
}
                             ...
                             bool is_type_record)
{
  ...
  if (OB_FAIL(ret)) {
    ...
    if (OB_ISNULL(result_set)
      ...
      // Step1: 获取结果集行描述
      ObSEArray<ObDataType, OB_DEFAULT_SE_ARRAY_COUNT> row_desc;
      if (!is_streaming) { //Mysql模式的Cursor或者可更新游标或者游标表达式会缓存数据
          ...
          if (OB_FAIL(ret)) {
            ...
            for (int64_t i = 0; OB_SUCC(ret) && i < actual_column_count; ++i) {
              ...
              if (obj.is_pl_extend()) {
                ...
                ObObj tmp;
                OZ (convert_obj(ctx, cast_ctx, is_strict, NULL, row_desc.at(i), obj, column_types[i], tmp));
                OZ (deep_copy_obj(*current_expr_allocator, tmp, *out_using_params->at(i)));
    ...
    if (ob_result_set->is_with_rows()) { // SELECT或DML RETURNING
      ...
      if (lib::is_oracle_mode()) { // ORACLE Mode: only iterate to end
        ...
        ObSQLSessionInfo *session_info = NULL;
        ObQueryDriver *query_driver = NULL;
        CK (OB_NOT_NULL(session_info = exec_ctx->get_my_session()));
        if (OB_SUCC(ret) && OB_NOT_NULL(query_driver = session_info->get_pl_query_sender())) {
          OZ (query_driver->response_query_result(*ob_result_set,
...
int ObSPIService::convert_obj(ObPLExecCtx *ctx,
                              ...
                              const ObDataType &current_type,
                              ObObj &src_obj,
                              const ObDataType &in_result_type,
                              ObObj &dst_obj)
{
  int ret = OB_SUCCESS;
  CK(OB_NOT_NULL(ctx));
  ...
  ObObj &obj = src_obj;
  if (OB_SUCC(ret)) {
    LOG_DEBUG("column convert", K(obj.get_meta()), K(in_result_type.get_meta_type()),
              K(current_type), K(in_result_type.get_accuracy()));
    if (obj.is_pl_extend()/* && pl::PL_RECORD_TYPE == obj.get_meta().get_extend_type()*/
      ...
      dst_obj = obj;
    } else if (obj.get_meta() == in_result_type.get_meta_type()
        && (current_type.get_accuracy() == in_result_type.get_accuracy()
            || (in_result_type.get_meta_type().is_number() // NUMBER目标类型精度未知直接做赋值
                && PRECISION_UNKNOWN_YET == in_result_type.get_accuracy().get_precision()
                && ORA_NUMBER_SCALE_UNKNOWN_YET == in_result_type.get_accuracy().get_scale())
            || (in_result_type.get_meta_type().is_character_type() // CHAR/VARCHAR长度未知,直接赋值
      ...
      dst_obj = obj;
               ...
               && !obj.is_null()
               && in_result_type.get_meta_type().is_ext()
               && !ob_is_xml_pl_type(in_result_type.get_obj_type(), in_result_type.get_udt_id())) {
      ...
      if (ob_is_enum_or_set_type(in_result_type.get_obj_type())) {
      ...
      ObExprResType result_type;
      OX (result_type.set_meta(in_result_type.get_meta_type()));
      if (in_result_type.get_meta_type().is_number()
          && in_result_type.get_precision() == -1
          && in_result_type.get_scale() == -1
          && current_type.get_meta_type().is_number()) {
        OX (result_type.set_accuracy(current_type.get_accuracy()));
      } else {
        OX (result_type.set_accuracy(in_result_type.get_accuracy()));
      ...
      if (OB_FAIL(ret)) {
        ...
        if (((obj.get_meta().is_ext())  // xmltype can not convert with other type in pl
            ...
            || (obj.is_null() && current_type.get_meta_type().is_xml_sql_type()
                && !ob_is_xml_pl_type(result_type.get_type(), result_type.get_accuracy().get_accuracy())
                && !ob_is_string_tc(result_type.get_type()))) {
          ret = OB_ERR_INVALID_TYPE_FOR_OP;
          LOG_WARN("xml type can not convert other type in pl", K(ret), K(obj), K(current_type), K(result_type));
          ...
          if (!ob_is_string_tc(result_type.get_type()) &&
              (current_type.get_meta_type().is_xml_sql_type()
              || (current_type.get_meta_type().is_ext() && current_type.get_accuracy().get_accuracy() == T_OBJ_XML))) {
  #ifdef OB_BUILD_ORACLE_PL
          ...
          } else if (in_result_type.get_meta_type().is_ext() &&
                    (PL_RECORD_TYPE == in_result_type.get_meta_type().get_extend_type() ||
                     PL_NESTED_TABLE_TYPE == in_result_type.get_meta_type().get_extend_type() ||
                     PL_VARRAY_TYPE == in_result_type.get_meta_type().get_extend_type())) {
            int64_t udt_id = in_result_type.get_udt_id();
...
int ObSPIService::try_cast_obj(ObPLExecCtx *ctx,
                                const ObSqlExpression *result_expr,
                                const ObDataType &result_type,
                                const bool not_null_flag,
                                const int64_t pl_integer_ranges,
                                const ObDataType &row_desc,
                                bool is_strict,
                                ObCastCtx &cast_ctx,
                                ObObj &obj,
                                const ObDataType *return_type)
{
  int ret = OB_SUCCESS;

  //  检查not null修饰符是否生效
  if (not_null_flag && obj.is_null()) {
    ret = OB_ERR_NUMERIC_OR_VALUE_ERROR;
...
int ObSPIService::try_cast_multi_obj(ObPLExecCtx *ctx,
                                      const ObSqlExpression *result_expr,
                                      const ObDataType *result_types,
                                      int64_t type_count,
                                      ...
                                      int64_t return_type_count)
{
  ...
  CK (return_types != nullptr ? return_type_count == obj_array.count() : true);
  ...
  for (int64_t i = 0; OB_SUCC(ret) && i < obj_array.count(); ++i) {
    const ObDataType *return_type = return_types != nullptr ? &return_types[i] : nullptr;
    OZ(try_cast_obj(ctx, result_expr, result_types[i], not_null_flags[i], pl_integer_ranges[i], row_desc.at(i), is_strict,
                    cast_ctx, obj_array.at(i), return_type));
  }
...
int ObSPIService::get_objaccess_address_and_allocator(ObPLExecCtx *ctx,
                                                      const ObSqlExpression *result_expr,
                                                      ObObjParam &result_address,
                                                      ObIAllocator *&alloc,
                                                      bool is_type_record)
{
  ...
  if (OB_FAIL(ret)) {
    ...
    OX (alloc = composite_allocator);
  } else if (OB_NOT_NULL(composite_allocator)) {
    OX (alloc = composite_allocator);
  } else {
    if (result_expr->get_expr_items().count() > 1
      ...
      OX (alloc = pkg_allocator);
    } else {
      OX (alloc = ctx->allocator_);
    }
  }
...
int ObSPIService::store_record_result(ObPLExecCtx *ctx,
                                      const ObSqlExpression *result_expr,
                                      ObIArray<ObObj> &obj_array)
{
  int ret = OB_SUCCESS;
  ...
  CK (is_obj_access_expression(*result_expr));
  OZ (get_objaccess_address_and_allocator(ctx, result_expr, result_address, alloc, true));
  CK (OB_NOT_NULL(alloc));

  OZ (store_record_datums(result_address, obj_array, alloc, ctx->exec_ctx_->get_my_session()));
                               ...
                               ObObj &src_obj)
{
  ...
  if (OB_SUCC(ret)) {
    ...
    if (is_obj_access_expression(*result_expr)) { //通过ObjAccess访问得到的基础变量或record
      ObIAllocator *alloc = nullptr;
      OZ (get_objaccess_address_and_allocator(ctx, result_expr, result_address, alloc, false));
      CK (OB_NOT_NULL(alloc));
      OZ (store_datums(result_address, src_obj, *alloc, ctx->exec_ctx_->get_my_session()));
      ...
      if (param_idx >= params->count() || param_idx < 0) {
        ...
        ObObjParam &param = params->at(param_idx);
        if (param.is_pl_extend()
            && param.get_ext() != 0
            && param.get_meta().get_extend_type() != PL_REF_CURSOR_TYPE
            && param.get_meta().get_extend_type() != PL_CURSOR_TYPE
            && param.get_ext() != src_obj.get_ext()) {
          OZ (ObUserDefinedType::destruct_objparam(*ctx->allocator_, param, ctx->exec_ctx_->get_my_session()));
        }
        ObObj result;
        ...
        if (OB_FAIL(ret)) {
          ...
          if (!src_obj.is_pl_extend()) {
            ret =OB_ERR_EXPRESSION_WRONG_TYPE;
            LOG_WARN("expr is wrong type", K(ret));
          } else if (PL_REF_CURSOR_TYPE == src_obj.get_meta().get_extend_type() ||
                    PL_OPAQUE_TYPE == src_obj.get_meta().get_extend_type()) {
            ObIAllocator *tmp_alloc = PL_OPAQUE_TYPE == src_obj.get_meta().get_extend_type() ? ctx->allocator_
                                                                                             : &ctx->exec_ctx_->get_allocator();
            if (PL_REF_CURSOR_TYPE == src_obj.get_meta().get_extend_type()) {
              OX (result = src_obj);  //ref cursor need to shallow copy
            } else {
              OZ (ObUserDefinedType::deep_copy_obj(*tmp_alloc, src_obj, result, true));
            }
          } else if (PL_CURSOR_TYPE == src_obj.get_meta().get_extend_type()) {
            ret = OB_ERR_UNEXPECTED;
            LOG_WARN("unexpected PL_CURSOR_TYPE to store datum", K(src_obj.get_meta()), K(ret));
          } else {
            dst_id = var_type.get_user_type_id();
            ObPLComposite *composite = reinterpret_cast<ObPLComposite*>(src_obj.get_ext());
            if (NULL == composite) {
              ret = OB_ERR_UNEXPECTED;
              LOG_WARN("unexpected composite to store datum", KPC(composite), K(ret));
              ...
              OZ (pl::ObUserDefinedType::deep_copy_obj(*ctx->allocator_, src_obj, result));
            }
          }
        } else if (var_type.is_obj_type() && var_type.get_data_type() != NULL &&
                   var_type.get_data_type()->get_meta_type().is_null() &&
                   src_obj.is_pl_extend()) {
          OZ (pl::ObUserDefinedType::deep_copy_obj(*ctx->allocator_, src_obj, result));
        } else if (src_obj.is_pl_extend()) {
          ret =OB_ERR_EXPRESSION_WRONG_TYPE;
          LOG_WARN("expr is wrong type", K(ret));
        } else {
          bool enable_new_padding = false;
        ...
        if (OB_SUCC(ret)) {
          ...
          is_ref_cursor = param.is_ref_cursor_type();
          if (param.is_pl_extend()) {
            //do nothing
          } else {
          ...
          if (is_ref_cursor   //cursor use spi_copy_ref_cursor to copy
              || (param.is_pl_extend() && (param.get_meta().get_extend_type() == PL_REF_CURSOR_TYPE || param.get_meta().get_extend_type() == PL_CURSOR_TYPE))) {
            OZ (spi_copy_ref_cursor(ctx, &ctx->exec_ctx_->get_allocator(), &result, &param));
          } else {
            OX (param = result);
          }
          OX (param.set_is_ref_cursor_type(is_ref_cursor));
          OX (param.set_param_meta());
        }
        OZ (spi_process_nocopy_params(ctx, param_idx));
        OX (param.set_accuracy(accuracy));
      }
      ...
      if (OB_SUCC(ret) && value.is_lob_storage()) {
        if (OB_FAIL(ObTextStringIter::convert_outrow_lob_to_inrow_templob(value, value, nullptr, &tmp_alloc, true/*allow_persist_inrow*/))) {
      ...
      ret = OB_ERR_UNEXPECTED;
      LOG_WARN("Invalid result address", K(*result_expr), K(result_address), K(ret));
    }
  }
  return ret;
}
                                ...
                                ObSQLSessionInfo *session_info)
{
  ...
  if (dest_addr.is_pl_extend()) {
    if (PL_OPAQUE_TYPE == dest_addr.get_meta().get_extend_type()) {
#ifdef OB_BUILD_ORACLE_PL
      if (src_obj.is_pl_extend()) {
        if (PL_OPAQUE_TYPE != src_obj.get_meta().get_extend_type()) {
          ret = OB_ERR_INTO_EXPR_ILLEGAL;
          LOG_WARN("PLS-00597: expression 'string' in the INTO list is of wrong type", K(ret));
        } else {
          ObPLOpaque *src = reinterpret_cast<ObPLOpaque *>(src_obj.get_ext());
          ...
          if (OB_FAIL(ObSPIService::spi_copy_opaque(NULL, NULL, *src, dst))) { // dst buffer has enough size?
            LOG_WARN("opaque data store datum failed ", KPC(src), K(dest_addr), K(src_obj), K(ret));
          }
        }
      }
#endif
      ...
      ret = OB_ERR_EXPRESSION_WRONG_TYPE;
      LOG_WARN("expr is wrong type", K(ret));
    } else if (dest_addr.get_meta().get_extend_type() != src_obj.get_meta().get_extend_type()) {
      ret = OB_ERR_INTO_EXPR_ILLEGAL;
      LOG_WARN("PLS-00597: expression 'string' in the INTO list is of wrong type", K(ret));
    } else {
      OZ (check_and_deep_copy_result(alloc, src_obj, dest_addr));
    }
  } else {
    int64_t current_datum = reinterpret_cast<int64_t>(dest_addr.get_ext());
    ObObj tmp;
    OZ (ob_write_obj(alloc, src_obj, tmp));
    if (OB_FAIL(store_datum(current_datum, tmp, session_info, &alloc))) {
      int tmp_ret = OB_SUCCESS;
      if ((tmp_ret = ObUserDefinedType::destruct_objparam(alloc, tmp, session_info)) != OB_SUCCESS) {
        LOG_WARN("failed to destruct obj, memory may leak", K(ret), K(tmp_ret), K(src_obj));
      }
      LOG_WARN("failed to arrange store", K(dest_addr), K(tmp), K(src_obj), K(ret));
    }
  }
...
int ObSPIService::store_record_datums(ObObj &dest_addr,
                                      ObIArray<ObObj> &obj_array,
                                      ObIAllocator *alloc,
                                      ObSQLSessionInfo *session_info)
{
  ...
  if (OB_SUCC(ret)) {
    ...
    for (int64_t i = 0; OB_SUCC(ret) && i < obj_array.count(); ++i) {
      ObObj tmp;
      ObObj &current_obj = obj_array.at(i);
      if (current_obj.is_pl_extend()) {
        if (PL_CURSOR_TYPE == current_obj.get_meta().get_extend_type() ||
            PL_REF_CURSOR_TYPE == current_obj.get_meta().get_extend_type()) {
          ret = OB_ERR_UNEXPECTED;
          LOG_WARN("unexcpected cursor type in obj access expression", K(ret), K(current_obj.get_meta().get_extend_type()));
        } else {
          OZ (pl::ObUserDefinedType::deep_copy_obj(*datum_allocator, current_obj, tmp));
        }
      } else {
        OZ (ob_write_obj(*datum_allocator, current_obj, tmp));
      }
      if (OB_SUCC(ret)) {
        if (OB_FAIL(store_datum(current_datum, tmp, session_info, datum_allocator))) {
          int tmp_ret = OB_SUCCESS;
          if ((tmp_ret = ObUserDefinedType::destruct_objparam(*datum_allocator, tmp, session_info)) != OB_SUCCESS) {
            LOG_WARN("failed to destruct obj, memory may leak", K(ret), K(tmp_ret), K(i), K(obj_array));
          }
          LOG_WARN("failed to arrange store", K(dest_addr), K(i), K(tmp), K(obj_array), K(ret));
        }
      }
    }
  }
...
int ObSPIService::prefetch_streaming_cursor(ObSQLSessionInfo *session_info, pl::ObPLCursorInfo &cursor)
{
  int ret = OB_SUCCESS;
  ObSPIResultSet *spi_result = cursor.get_cursor_handler();
  CK (OB_NOT_NULL(session_info));
  CK (OB_NOT_NULL(spi_result));
  if (OB_SUCC(ret) && OB_ISNULL(spi_result->get_spi_cursor())) {
    uint64_t mem_limit = OB_INVALID_SIZE;
    OZ (session_info->get_tmp_table_size(mem_limit));
    OZ (spi_result->init_spi_cursor(*session_info, mem_limit));
    CK (OB_NOT_NULL(spi_result->get_spi_cursor()));
    CK (OB_NOT_NULL(spi_result->get_result_set()));
    CK (OB_NOT_NULL(spi_result->get_result_set()->get_field_columns()));
    OZ (spi_result->get_spi_cursor()->init_row_desc(*spi_result->get_result_set()));
    OZ (ObDbmsInfo::deep_copy_field_columns(spi_result->get_memory_ctx()->get_arena_allocator(),
                                            spi_result->get_result_set()->get_field_columns(),
                                            spi_result->get_spi_cursor()->fields_));
  }
  CK (OB_NOT_NULL(spi_result->get_spi_cursor()));
  OX (spi_result->get_spi_cursor()->cur_ = 0);
  OX (spi_result->get_spi_cursor()->row_store_.reuse());
  CK (OB_NOT_NULL(spi_result->get_result_set()))

  uint64_t batch_fill_count = 1;
  bool is_iter_end = false;
  const ObNewRow *curr_row = NULL;
  bool need_set_current_row = false;
  if (OB_SUCC(ret) && cursor.get_current_row().is_valid()) { //save current_row to row_store's first row
    OZ (ObSPIService::fill_cursor_row(spi_result->get_spi_cursor(), cursor.get_current_row()));
    OX (need_set_current_row = true);
  }
  OX (batch_fill_count = spi_result->get_batch_fill_count());
  OZ (fill_cursor(spi_result->get_memory_ctx(), *spi_result->get_result_set(), spi_result->get_spi_cursor(), ObTimeUtility::current_time(), is_iter_end, batch_fill_count, true));
  OX (spi_result->update_batch_fill_count());

  if (OB_SUCC(ret) && is_iter_end) {
    OZ (spi_result->get_spi_cursor()->row_store_.finish_add_row());
  }

  // Restore the current_row from row_store after prefetch, must be done after all row_store operations are completed.
  if (OB_SUCC(ret) && need_set_current_row) {
    OZ (spi_result->get_spi_cursor()->row_store_.get_row(spi_result->get_spi_cursor()->cur_, curr_row));
    CK (OB_NOT_NULL(curr_row));
    OX (spi_result->get_spi_cursor()->cur_++);
    OX (cursor.get_current_row() = *curr_row);
  }
  return ret;
}
                              ...
                              bool is_cursor_prefetch)
{
  ...
  if (OB_ISNULL(entity)) {
    ...
    WITH_CONTEXT(entity) {
      if (OB_FAIL(fill_cursor(result_set, cursor, new_query_start_time, is_iter_end, orc_max_ret_rows, is_cursor_prefetch))) {
...
#endif
      // For streaming cursor, check if adding more rows would trigger dump to disk
      if (is_cursor_prefetch && cursor->row_store_.need_dump()) {
        LOG_INFO("streaming cursor stops filling due to memory limit, avoid dump to disk",
                  K(i), K(cursor->row_store_.get_row_cnt()),
                  K(cursor->row_store_.get_mem_hold()),
                  K(cursor->row_store_.get_mem_limit()));
        break;
      }
                            ...
                            ObNewRow &cur_row)
{
  ...
  if (OB_ISNULL(result_set)) {
    ...
    const ObNewRow *row = NULL;
    if (OB_FAIL(cursor->row_store_.get_row(cursor->cur_, row))) {
...
int ObSPIService::spi_update_location(pl::ObPLExecCtx *ctx, uint64_t location)
{
  int ret = OB_SUCCESS;
  ObPLTopContext *pl_top_context = nullptr;
  if (OB_NOT_NULL(ctx) && OB_NOT_NULL(ctx->get_exec_ctx())) {
    ObIArray<ObPLExecState *> *stack = nullptr;
    OZ (ctx->get_exec_ctx()->get_pl_top_context(pl_top_context));
    CK (OB_NOT_NULL(pl_top_context));
    CK (OB_NOT_NULL(stack = pl_top_context->get_exec_stack()));
    if (OB_SUCC(ret) && stack->count() > 0) {
...
int ObSPIService::spi_pl_profiler_before_record(pl::ObPLExecCtx *ctx, int64_t line, int64_t level, int64_t column)
{
  ...
  CK (OB_NOT_NULL(pl_top_context = session->get_pl_top_context()));
  CK (OB_NOT_NULL(curr_state = pl_top_context->get_current_state()));
  ...
  if (OB_SUCC(ret) && OB_NOT_NULL(code_coverage = session->get_pl_code_coverage())
    ...
    hash::ObHashSet<std::pair<uint64_t, uint64_t>> *coverage_info = curr_state->get_coverage_info();
    CK (OB_NOT_NULL(coverage_info));
    OX (coverage_info->set_refactored(std::make_pair(line, column)));
  }
#endif // OB_BUILD_ORACLE_PL
...
int ObSPIService::ps_cursor_open(ObPLExecCtx *ctx,
                                 ObPsCursorInfo &ps_cursor)
{
  int ret = OB_SUCCESS;
  ParamStore &exec_params = ps_cursor.get_exec_params();
  ObString sql_str;
  ObResultSet *rs = NULL;
  ...
  if (OB_ISNULL(ctx) || OB_ISNULL(ctx->exec_ctx_)) {
    ret = OB_INVALID_ARGUMENT;
    LOG_WARN("pointer is NULL", K(ret), K(ctx));
...
#define CHECK_CURSOR_HANDLER                                                  \
  do {                                                                        \
    CK (OB_NOT_NULL(ps_cursor.get_cursor_handler()));                         \
    OV (OB_NOT_NULL(rs = ps_cursor.get_cursor_handler()->get_result_set()));  \
    OV (OB_NOT_NULL(spi_cursor = ps_cursor.get_cursor_store()));              \
  } while (0)

#define CLOSE_SPI_RESULT                                                \
  do {                                                                  \
  ...
  if (ps_cursor.is_async()) {
    ...
    while (OB_SUCC(ret)
           && OB_SUCCESS == ps_cursor.get_store_ret()
           && !store_end) {
      // NOTE: Do not delete the following log line !!!
      LOG_INFO("async fill ps cursor", K(ret), K(ps_cursor.get_id()), K(session.get_server_sid()),
               K(session.get_effective_tenant_id()));
      ObSpinLockGuard guard(ps_cursor.get_spin_lock());
      if (!ps_cursor.is_client_close()) {
        int64_t max_cnt = ps_cursor.get_fetch_size();
        CHECK_CURSOR_HANDLER;
        if (OB_SUCC(ret)) {
          int64_t v = ps_cursor.get_ora_max_ret_rows() - spi_cursor->row_store_.get_row_cnt();
          max_cnt = MIN(max_cnt, v);
          if (pre_store_size > 0) {
        ...
        if (FAILEDx((fill_cursor(*rs,
                                ...
                                is_iter_end,
                                max_cnt)))) {
          LOG_WARN("fill cursor failed", K(ret));
        } else {
#ifdef ERRSIM
          ret = OB_E(EventTable::EN_FILL_PS_CUROSR_ERROR) OB_SUCCESS;
          if (OB_FAIL(ret)) {
            LOG_WARN("async cursor EN_FILL_PS_CUROSR_ERROR", K(ret));
          }
#endif
        }
        if (OB_FAIL(ret)) {
          ps_cursor.set_store_ret(ret);
        } else if (is_iter_end || (spi_cursor->row_store_.get_row_cnt() >= ps_cursor.get_ora_max_ret_rows())) {
          if (OB_FAIL(spi_cursor->row_store_.finish_add_row())) {
            LOG_WARN("finish add row failed", K(ret));
            ps_cursor.set_store_ret(ret);
          } else {
            ps_cursor.set_store_ret(OB_ITER_END);
          }
        } else if (pre_store_size > 0) {
          store_end = true;
        }
      } else {
        ps_cursor.set_store_ret(OB_ITER_END);
      }
      if (OB_FAIL(ret) || ps_cursor.is_client_close() || OB_SUCCESS != ps_cursor.get_store_ret()) {
        store_end = true;
        CLOSE_SPI_RESULT;
      }
    }
    ...
    if (FAILEDx(fill_cursor(*rs,
                            ...
                            ObTimeUtility::current_time(),
                            is_iter_end,
                            ps_cursor.get_ora_max_ret_rows()))) {
      LOG_WARN("fill cursor failed", K(ret));
    } else {
#ifdef ERRSIM
      ret = OB_E(EventTable::EN_FILL_PS_CUROSR_ERROR) OB_SUCCESS;
      if (OB_FAIL(ret)) {
        LOG_WARN("sync cursor EN_FILL_PS_CUROSR_ERROR", K(ret));
      }
#endif
    }
    if (OB_FAIL(ret)) {
...
int ObSPIService::ps_cursor_fetch(ObPsCursorInfo &ps_cursor, ObSQLSessionInfo &session)
{
  int ret = OB_SUCCESS;
  ObPLASHGuard guard(ObPLASHGuard::ObPLASHStatus::IS_SQL_EXECUTION);
  ObSPICursor *cursor = ps_cursor.get_cursor_store();
  ObNewRow &cur_row = ps_cursor.get_current_row();
  const ObNewRow *row = NULL;
  ...
  if (FAILEDx(cursor->row_store_.get_row(cursor->cur_, row))) {
    if (OB_INDEX_OUT_OF_RANGE == ret
        && (0 > cursor->cur_ || cursor->cur_ >= cursor->row_store_.get_row_cnt())) {
      ret = OB_ITER_END;
    } else {
      LOG_WARN("get row from row_store failed", K(cursor->cur_), K(ret));
    }
  } else {
    CK (OB_NOT_NULL(row));
    OX (cur_row = *row);
  ...
  LOG_TRACE("ps cursor fetch row", K(ret), K(session.get_server_sid()), K(ps_cursor.get_id()), K(cur_row));
  return ret;
}
...
{
  if (OB_NOT_NULL(ctx)
      && OB_NOT_NULL(ctx->get_exec_ctx())
      && OB_NOT_NULL(ctx->get_exec_ctx()->get_pl_top_context())
      && OB_NOT_NULL(ctx->get_exec_ctx()->get_pl_top_context()->get_exec_stack())
      && ctx->get_exec_ctx()->get_pl_top_context()->get_exec_stack()->count() > 0) {
    // hold old time and reset sub time
    ObIArray<ObPLExecState *> *stack = ctx->get_exec_ctx()->get_pl_top_context()->get_exec_stack();
...
ObSPIRetryCtrlGuard::ObSPIRetryCtrlGuard(
  ObQueryRetryCtrl &retry_ctrl, ObSPIResultSet &spi_result, ObSQLSessionInfo &session_info, ObPLExecCtx *ctx, int &ret, bool need_reset, bool can_opt_schema_guard)
  : retry_ctrl_(retry_ctrl), spi_result_(spi_result), session_info_(session_info), ret_(ret), init_(false)
{
  ...
  } else if (can_opt_schema_guard && OB_FAIL(GCTX.schema_service_->get_tenant_refreshed_schema_version(eff_tenant_id, refreshed_schema_version))) {
    LOG_WARN("failed to get tenant refreshed schema version", K(ret), K(eff_tenant_id), K(refreshed_schema_version));
  } else if (can_opt_schema_guard && OB_FAIL(schema_guard->get_schema_version(eff_tenant_id, current_schema_version))) {
    LOG_WARN("failed to get current tenant schema version", K(ret), K(eff_tenant_id), K(current_schema_version), K(refreshed_schema_version));
  } else if (OB_UNLIKELY(!can_opt_schema_guard || refreshed_schema_version != current_schema_version)) {
    if (OB_FAIL(GCTX.schema_service_->get_tenant_schema_guard(eff_tenant_id, spi_result_.get_scheme_guard()))) {
      LOG_WARN("failed to get tenant schema guard", K(ret), K(eff_tenant_id), K(refreshed_schema_version));
    } else {
      schema_guard = &spi_result_.get_scheme_guard();
    }
  }
  if (OB_SUCC(ret)) {
    int64_t tenant_version = 0;
    int64_t sys_version = 0;
    if (OB_FAIL(schema_guard->get_schema_version(eff_tenant_id, tenant_version))) {
  ...
  if (complex_obj.is_pl_extend() && pl::PL_REF_CURSOR_TYPE == complex_obj.get_meta().get_extend_type()) {
    ...
    if (OB_NOT_NULL(cursor) && OB_NOT_NULL(session_info_)) {
      ...
      if (0 == cursor->get_ref_count()) {
         CK (cursor->is_session_cursor()); //ref cursor == 0 must be session cursor
        if (OB_FAIL(session_info_->close_cursor(cursor->get_id()))) {
          LOG_WARN("fail to close session cursor source", K(ret), K(cursor));
        }
      }
    }
...
int ObSPICursor::init_row_desc(sql::ObResultSet &result_set)
{
  ...
  OZ (row_desc_.reserve(fields->count()));
