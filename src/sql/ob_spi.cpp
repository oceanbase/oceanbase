        ...
        do {
          ...
          ObArenaAllocator allocator(GET_PL_MOD_STRING(PL_MOD_IDX::OB_PL_STATIC_SQL_EXEC), OB_MALLOC_NORMAL_BLOCK_SIZE, MTL_ID());
          {
            ...
            ObPLSqlAuditGuard audit_guard(
              *(ctx->exec_ctx_), *(session), spi_result, audit_record, ret, (sql != NULL ? sql : ps_sql), retry_ctrl, trace_id_guard);
    ...
    do {
      {
        ...
        ObPLSqlAuditGuard audit_guard(
          *(ctx->exec_ctx_), session_info, *spi_result, audit_record, ret, (sql != NULL ? sql : ps_sql), retry_ctrl, trace_id_guard);
...
#define GET_RESULT                                                                    \
  do {                                                                                \
    if (OB_FAIL(ret)) {                                                               \
      ...
      ObPLSubPLSqlTimeGuard guard(ctx);                                               \
      ObPLSqlAuditGuard audit_guard(                                                  \
        *(ctx->exec_ctx_), *session, *spi_result, audit_record, ret, ps_sql, retry_ctrl, trace_id_guard); \
    ...
    if (OB_SUCC(ret)) {
      ...
      if (lower <= 0 // 检查是否越界, lower,upper从1开始
        ...
        LOG_WARN("OBE-22160: element at index does not exist",
                 K(lower), K(upper), K(src_coll->get_count()), K(ret));
      } else if (OB_ISNULL(data_ptr)) {
        ret = OB_ERR_UNEXPECTED;
        LOG_WARN("invalid data", K(ret));
