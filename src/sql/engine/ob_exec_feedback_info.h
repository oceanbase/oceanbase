...
public:
  ObExecFeedbackNode(int64_t op_id) : op_id_(op_id), output_row_count_(0),
      ...
      op_last_row_time_(0), cpu_time_(0), block_time_(0), worker_count_(0), pdml_op_write_rows_(0),
  ...
  TO_STRING_KV(K_(op_id), K_(output_row_count), K_(op_open_time),
               ...
               K_(cpu_time), K_(block_time), K_(pdml_op_write_rows), K_(ssstore_read_row_count),
...
public:
  ...
  int64_t cpu_time_;    // rdtsc cpu cycles spend on this op, include cpu instructions & io
...
class ObExecFeedbackInfo final
{
  OB_UNIS_VERSION(1);
public:
  ObExecFeedbackInfo() : nodes_(), total_cpu_time_(0), is_valid_(true) {}
  ...
  int64_t get_total_cpu_time() const { return total_cpu_time_; }
  int64_t &get_total_cpu_time() { return total_cpu_time_; }
  void reset() { nodes_.reset(); }
  TO_STRING_KV(K_(nodes), K_(total_cpu_time), K_(is_valid));
private:
  common::ObSArray<ObExecFeedbackNode> nodes_;
  int64_t total_cpu_time_;
  bool is_valid_;
};
