...
ObPxObjectSampleSpec::ObPxObjectSampleSpec(common::ObIAllocator &alloc, const ObPhyOperatorType type)
  : ObOpSpec(alloc, type),
...
OB_SERIALIZE_MEMBER((ObPxObjectSampleSpec, ObOpSpec),
                    is_none_partition_,
                    sort_exprs_,
                    sort_collations_,
                    sort_cmp_funs_,
...
ObPxObjectSampleOp::ObPxObjectSampleOp(ObExecContext &ctx_, const ObOpSpec &spec, ObOpInput *input)
...
void ObPxObjectSampleOp::destroy()
{
...
int ObPxObjectSampleOp::inner_open()
{
  int ret = OB_SUCCESS;
  if (OB_ISNULL(child_)) {
    ret = OB_ERR_UNEXPECTED;
    LOG_WARN("child is null", K(ret));
  } else if (ObPxObjectSampleType::SAMPLE_SORT == MY_SPEC.type_) {
    by_pass_ = false;
...
int ObPxObjectSampleOp::inner_close()
{
...
int ObPxObjectSampleOp::inner_rescan()
{
  int ret = OB_SUCCESS;
  iter_end_ = false;
  if (ObPxObjectSampleType::SAMPLE_SORT == MY_SPEC.type_) {
    sort_impl_.reset();
    by_pass_ = true;
    non_partition_row_count_ = 0;
...
int ObPxObjectSampleOp::set_no_need_sample()
{
  int ret = OB_SUCCESS;
  ObOperator *tsc = NULL;
...
int ObPxObjectSampleOp::find_sample_scan(ObOperator *op, ObOperator *&tsc)
{
  int ret = OB_SUCCESS;
...
int ObPxObjectSampleOp::inner_get_next_row()
{
  int ret = OB_SUCCESS;
  if (iter_end_) {
    ret = OB_ITER_END;
...
int ObPxObjectSampleOp::inner_get_next_batch(const int64_t max_row_cnt)
{
  int ret = OB_SUCCESS;
...
int ObPxObjectSampleOp::generate_sample_partition_range(int64_t batch_size)
{
  int ret = OB_SUCCESS;
  if (!is_vectorized()) {
    while (OB_SUCC(ret)) {
...
int ObPxObjectSampleOp::split_partition_range()
{
  int ret = OB_SUCCESS;
...
int ObPxObjectSampleOp::collect_row_count_in_partitions(
    bool is_vectorized,
...
int ObPxObjectSampleOp::update_partition_row_count()
{
  int ret = OB_SUCCESS;
  int64_t tablet_id = 0;
  int64_t *count_ptr = nullptr;
...
int ObPxObjectSampleOp::get_tablet_id(int64_t &tablet_id)
{
  int ret = OB_SUCCESS;
...
bool ObPxObjectSampleOp::is_none_partition()
{
