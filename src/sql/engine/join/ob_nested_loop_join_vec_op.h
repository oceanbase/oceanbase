...
class ObNLJCacheMemChecker
{
public:
  explicit ObNLJCacheMemChecker(int64_t row_cnt)
    : type_(ROW_CNT), cur_row_cnt_(row_cnt), sql_mem_processor_(nullptr)
  {}
  explicit ObNLJCacheMemChecker(ObSqlMemMgrProcessor &sql_mem_processor)
    : type_(EXTEND_MEM), cur_row_cnt_(0), sql_mem_processor_(&sql_mem_processor)
  {}
  bool operator()(int64_t val)
  {
    bool need_check = false;
    if (ROW_CNT == type_) {
      need_check = cur_row_cnt_ > val;
    } else {
      need_check = sql_mem_processor_->get_data_size() > val * 0.8;
    }
    return need_check;
  }
private:
  enum Type { ROW_CNT, EXTEND_MEM };
  Type type_;
