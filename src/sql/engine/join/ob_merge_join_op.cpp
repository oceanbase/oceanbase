...
ObMergeJoinOp::ObMergeJoinOp(ObExecContext &exec_ctx, const ObOpSpec &spec, ObOpInput *input)
  : ObJoinOp(exec_ctx, spec, input),
    ...
    left_first_row_holder_(),
    right_first_row_holder_(),
...
int ObMergeJoinOp::inner_open()
{
  ...
  if (OB_FAIL(ObJoinOp::inner_open())) {
    ...
    void *left_fetcher_buf = NULL;
    void *right_fetcher_buf = NULL;
    if (OB_ISNULL(left_fetcher_buf = allocator.alloc(sizeof(ChildBatchFecther)))
              || OB_ISNULL(right_fetcher_buf = allocator.alloc(sizeof(ChildBatchFecther)))) {
      ret = OB_ALLOCATE_MEMORY_FAILED;
      LOG_WARN("allocate memory failed", K(ret), K(left_fetcher_buf), K(right_fetcher_buf));
    } else {
      const ObIArray<ObMergeJoinSpec::EqualConditionInfo> &equal_cond_infos =
                                                            MY_SPEC.equal_cond_infos_;
      if (OB_FAIL(left_first_row_holder_.init(left_outputs, eval_ctx_))) {
        LOG_WARN("init left brs holder failed", K(ret));
      } else if (OB_FAIL(right_first_row_holder_.init(right_outputs, eval_ctx_))) {
        LOG_WARN("init left brs holder failed", K(ret));
      } else {
        if (OB_SUCC(ret)) {
          uint64_t tenant_id = ctx_.get_my_session()->get_effective_tenant_id();
          const ObIArray<ObMergeJoinSpec::EqualConditionInfo> &equal_cond_infos = MY_SPEC.equal_cond_infos_;
          if (OB_FAIL(left_brs_fetcher_.init(
...
int ObMergeJoinOp::trans_to_fill_cache()
{
  ...
  if (OB_FAIL(calc_other_conds(is_match))) {
    ...
    if (OB_ISNULL(mem_context_)) {
      ...
      // 此处暂时不能落盘, 因为数据落盘后， 再读取出来时， 会需要修改extra中is_match标记，
      // 但该标记并不会再写到磁盘中，当这些修改后标记在内存释放后就丢了,
      // 后面可考虑使用bitmap处理某行匹配的标记
...
int ObMergeJoinOp::ChildBatchFecther::init(
    ...
    const ExprFixedArray *all_exprs)
{
  ...
  if (OB_FAIL(datum_store_.init(UINT64_MAX/*mem_limit*/,
                                tenant_id,
                                ObCtxIds::WORK_AREA,
                                ObModIds::OB_SQL_MERGE_JOIN,
                                false,
...
int ObMergeJoinOp::ChildBatchFecther::get_next_batch(const int64_t max_row_cnt)
{
  int ret = OB_SUCCESS;
...
int ObMergeJoinOp::ChildBatchFecther::backup_remain_rows()
{
  int ret = OB_SUCCESS;
...
int ObMergeJoinOp::ChildBatchFecther::get_next_nonskip_row(bool &got_next_batch)
{
  int ret = OB_SUCCESS;
...
int ObMergeJoinOp::ChildBatchFecther::get_next_small_group(int64_t &cmp_res)
{
  int ret = OB_SUCCESS;
  bool greater_found = false;
  bool all_batch_finished = false;
  bool enough_datums = merge_join_op_.has_enough_datums();
  JoinRowList row_list(row_list_buffer_cnt_);
  ...
  while (OB_SUCC(ret) && !all_batch_finished && !greater_found && !enough_datums) {
    if (need_store_unmatch && OB_LIKELY(cur_idx_ < brs_.size_)) {
      ...
      if (OB_FAIL(datum_store_.add_row(*all_exprs_, &(merge_join_op_.eval_ctx_), &stored_row))) {
        ...
        // TODO: shanting left store row need no extra info.
        ObStoredJoinRow *stored_join_row = static_cast<ObStoredJoinRow *>(stored_row);
        stored_join_row->set_is_match(false);
        if (OB_FAIL(push_back_list_row(stored_join_row, row_list))) {
          LOG_WARN("push back stored join row failed", K(ret));
        }
      }
    }
    if (OB_SUCC(ret)) {
...
int ObMergeJoinOp::ChildBatchFecther::get_next_equal_group(JoinRowList &row_list,
                                                    ...
                                                    const bool is_unique)
{
  ...
  LOG_DEBUG("end get next equal group, need store match", K(cur_idx_),
            K(row_list), K(row_list_buffer_cnt_), K(is_unique));
  return ret;
}

int ObMergeJoinOp::ChildBatchFecther::push_back_list_row(ObStoredJoinRow *stored_row,
                                                         JoinRowList &list)
{
  int ret = OB_SUCCESS;
  if (OB_UNLIKELY(row_list_buffer_cnt_ >= row_list_buffer_size_)) {
    ObIAllocator &allocator = merge_join_op_.ctx_.get_allocator();
    const int64_t init_size = batch_size_ * BATCH_MULTIPLE_TIMES;
    int64_t new_buffer_size = MAX(row_list_buffer_size_ * 2, init_size);
    ObStoredJoinRow **new_row_list_buffer = NULL;
    if (OB_ISNULL(new_row_list_buffer = static_cast<ObStoredJoinRow **>
                                  (allocator.alloc(sizeof(ObStoredJoinRow *) * new_buffer_size)))) {
      ret = OB_ALLOCATE_MEMORY_FAILED;
      LOG_WARN("allocate memory failed", K(ret), K(row_list_buffer_size_), K(init_size));
    } else {
      MEMCPY(new_row_list_buffer, row_list_buffer_, sizeof(ObStoredJoinRow *) * row_list_buffer_cnt_);
      LOG_DEBUG("expand row list buffer", K(row_list_buffer_cnt_), K(new_buffer_size));
      row_list_buffer_size_ = new_buffer_size;
      row_list_buffer_ = new_row_list_buffer;
    }
  }
  if (OB_SUCC(ret)) {
    row_list_buffer_[row_list_buffer_cnt_++] = stored_row;
    list.end_++;
  }
  return ret;
}

int ObMergeJoinOp::ChildBatchFecther::get_list_row(int64_t idx, ObStoredJoinRow *&stored_row)
{
  int ret = OB_SUCCESS;
  if (OB_UNLIKELY(idx >= row_list_buffer_size_)) {
    ret = OB_ERR_UNEXPECTED;
    LOG_WARN("get list row failed, idx out of range", K(idx), K(row_list_buffer_size_), K(lbt()));
  } else if (OB_ISNULL(stored_row = row_list_buffer_[idx])) {
    ret = OB_ERR_UNEXPECTED;
    LOG_WARN("get null row from list", K(idx), K(row_list_buffer_size_), K(lbt()));
  }
  return ret;
}
...
int ObMergeJoinOp::calc_equal_conds_with_stored_row(const ObChunkDatumStore::StoredRow *stored_row,
                                                    int64_t batch_idx,
                                                    int64_t &cmp_res)
{
  int ret = OB_SUCCESS;
...
int ObMergeJoinOp::store_group_first_row(
    ChildBatchFecther &child_fetcher, JoinRowList &row_list,
    ObChunkDatumStore::StoredRow *&stored_row,
    ObEvalCtx::BatchInfoScopeGuard &guard)
{
  int ret = OB_SUCCESS;
  ObChunkDatumStore::StoredRow *res_row = NULL;
  ...
  if (OB_FAIL(child_fetcher.datum_store_.add_row(*child_fetcher.all_exprs_,
    ...
    ObStoredJoinRow *row = static_cast<ObStoredJoinRow *>(res_row);
    row->set_is_match(false);
    if (OB_FAIL(child_fetcher.push_back_list_row(row, row_list))) {
      LOG_WARN("push back failed", K(ret));
    } else {
      stored_row = res_row;
    }
...
{
  int ret = OB_SUCCESS;
  JoinRowList l_row_list(left_brs_fetcher_.row_list_buffer_cnt_);
  JoinRowList r_row_list(right_brs_fetcher_.row_list_buffer_cnt_);
  ObChunkDatumStore::StoredRow *l_row = NULL;
  ObChunkDatumStore::StoredRow *r_row = NULL;
  if (OB_FAIL(store_group_first_row(left_brs_fetcher_, l_row_list, l_row, guard))) {
...
{
  int ret = OB_SUCCESS;
  left_brs_fetcher_.datum_store_.reset();
  right_brs_fetcher_.datum_store_.reset();
  ...
  if (OB_FAIL(left_brs_fetcher_.brs_holder_.restore())) {
    ...
    if (OB_SUCC(ret)) {
      const bool need_dump = false;
      if (OB_FAIL(left_brs_fetcher_.datum_store_.finish_add_row(need_dump))) {
        LOG_WARN("left datum store finish add row failed", K(ret));
      } else if (OB_FAIL(right_brs_fetcher_.datum_store_.finish_add_row(need_dump))) {
    ...
    if (OB_SUCC(ret)) {
      ...
      switch (join_type) {
        case INNER_JOIN :
          ret = match_group_rows<false, false, INNER_JOIN>();
          break;
        case LEFT_SEMI_JOIN :
          ret = match_group_rows<false, true, LEFT_SEMI_JOIN>();
          break;
        case LEFT_ANTI_JOIN :
          ret = match_group_rows<false, true, LEFT_ANTI_JOIN>();
          break;
        case LEFT_OUTER_JOIN :
          ret = match_group_rows<false, true, LEFT_OUTER_JOIN>();
          break;
        case RIGHT_OUTER_JOIN :
          ret = match_group_rows<true, false, RIGHT_OUTER_JOIN>();
          break;
        case FULL_OUTER_JOIN :
          ret = match_group_rows<true, true, FULL_OUTER_JOIN>();
          break;
        default : {
          ret = OB_ERR_UNEXPECTED;
    ...
    if (OB_SUCC(ret) && output_cache_.count() > 0) {
      ...
      LOG_DEBUG("goto output store, save datums", K(MY_SPEC.max_batch_size_));
    } else if (left_brs_fetcher_.iter_end()) {
...
int ObMergeJoinOp::match_group_rows()
{
  LOG_DEBUG("start match group rows", K(left_empty_allowed), K(right_empty_allowed),
              K(match_groups_.count()), K(output_cache_.count()));
  int ret = OB_SUCCESS;
  // we put params of other conds in the first row of batch, so we need to backup the first row
  // of batch and restore them in the end.
  ...
  if (MY_SPEC.other_join_conds_.count() > 0) {
    if (OB_FAIL(left_first_row_holder_.save(1))) {
      LOG_WARN("save left first row failed", K(ret));
    } else if (OB_FAIL(right_first_row_holder_.save(1))) {
      LOG_WARN("save right first row failed", K(ret));
    }
  }
  for(int64_t i = 0; i < match_groups_.count() && OB_SUCC(ret); i++) {
    RowsListPair rows_list_pair = match_groups_.at(i);
    /*
    ...
    */
    JoinRowList left_group = rows_list_pair.first;
    JoinRowList right_group = rows_list_pair.second;
    if (OB_UNLIKELY(0 == left_group.count() && 0 == right_group.count())) {
      ret = OB_ERR_UNEXPECTED;
      LOG_WARN("group is empty", K(left_group), K(right_group));
    } else if (left_empty_allowed && 0 == left_group.count()) {
      for(int64_t i = right_group.start_; i < right_group.end_ && OB_SUCC(ret); i++) {
        ObStoredJoinRow *stored_row = NULL;
        if (OB_FAIL(right_brs_fetcher_.get_list_row(i, stored_row))) {
          LOG_WARN("get row in list failed", K(ret));
        } else if (OB_FAIL(output_cache_.push_back(std::make_pair(static_cast<ObStoredJoinRow *>(NULL),
                                                            stored_row)))) {
          LOG_WARN("output cache push back failed", K(ret));
        }
      }
    } else if (right_empty_allowed && 0 == right_group.count()) {
      ObStoredJoinRow *stored_row = NULL;
      for(int64_t i = left_group.start_; i < left_group.end_ && OB_SUCC(ret); i++) {
        if (OB_FAIL(left_brs_fetcher_.get_list_row(i, stored_row))) {
          LOG_WARN("get row in list failed", K(ret));
        } else if (OB_FAIL(output_cache_.push_back(std::make_pair(stored_row,
                                                          static_cast<ObStoredJoinRow *>(NULL))))) {
          LOG_WARN("output cache push back failed", K(ret));
        }
      }
    } else {
      // calc other conds, then push back output cache according to is_match and join type.
      for(int64_t l_idx = left_group.start_; l_idx < left_group.end_ && OB_SUCC(ret); l_idx++) {
        bool left_matched = false;
        ObStoredJoinRow *l_stored_row = NULL;
        if (OB_FAIL(left_brs_fetcher_.get_list_row(l_idx, l_stored_row))) {
          LOG_WARN("get row in list failed", K(ret));
        } else if (MY_SPEC.other_join_conds_.count() > 0
            && OB_FAIL(l_stored_row->to_expr(left_output, eval_ctx_, left_output.count()))) {
          LOG_WARN("left datums to expr failed", K(ret));
        }
        for (int64_t r_idx = right_group.start_; r_idx < right_group.end_ && OB_SUCC(ret); r_idx++) {
          bool is_match = false;
          ObStoredJoinRow *r_stored_row = NULL;
          clear_evaluated_flag();
          if (OB_FAIL(right_brs_fetcher_.get_list_row(r_idx, r_stored_row))) {
            LOG_WARN("get row in list failed", K(ret));
          } else if (MY_SPEC.other_join_conds_.count() > 0) {
            if (OB_FAIL(r_stored_row->to_expr(right_output, eval_ctx_, right_output.count()))) {
          ...
          if (OB_SUCC(ret)) {
            if (INNER_JOIN != join_type && is_match) {
              left_matched = true;
              if (need_right_join()) {
                r_stored_row->set_is_match(true);
              }
              if (LEFT_SEMI_JOIN == MY_SPEC.join_type_ || LEFT_ANTI_JOIN == MY_SPEC.join_type_) {
                break;
              }
            ...
            if (output_equal_rows_directly() && is_match) {
              if (OB_FAIL(output_cache_.push_back(std::make_pair(l_stored_row, r_stored_row)))) {
                LOG_WARN("output cache push back failed", K(ret));
              }
            }
          }
        ...
        if (OB_FAIL(ret)) {
        } else if (need_store_left_unmatch_rows() && !left_matched) {
          if (OB_FAIL(output_cache_.push_back(std::make_pair(l_stored_row,
                                                          static_cast<ObStoredJoinRow *>(NULL))))) {
            LOG_WARN("output cache push back failed", K(ret));
          }
        } else if (LEFT_SEMI_JOIN == join_type && left_matched) {
          if (OB_FAIL(output_cache_.push_back(std::make_pair(l_stored_row,
                                                          static_cast<ObStoredJoinRow *>(NULL))))) {
            LOG_WARN("output cache push back failed", K(ret));
          }
      ...
      if (need_right_join() && OB_SUCC(ret)) {
        for (int64_t r_idx = right_group.start_; r_idx < right_group.end_ && OB_SUCC(ret); r_idx++) {
          ObStoredJoinRow *r_stored_row = NULL;
          if (OB_FAIL(right_brs_fetcher_.get_list_row(r_idx, r_stored_row))) {
            LOG_WARN("get row in list failed", K(ret));
          } else if (!r_stored_row->is_match()
              && OB_FAIL(output_cache_.push_back(std::make_pair(
                                            static_cast<ObStoredJoinRow *>(NULL), r_stored_row)))) {
            LOG_WARN("output cache push back failed", K(ret));
          }
        }
      }
  ...
  clear_evaluated_flag();
  if (OB_SUCC(ret) && MY_SPEC.other_join_conds_.count() > 0) {
    if (OB_FAIL(left_first_row_holder_.restore())) {
      LOG_WARN("restore left first row failed", K(ret));
    } else if (OB_FAIL(right_first_row_holder_.restore())) {
      LOG_WARN("restore right first row failed", K(ret));
    }
  }
  LOG_DEBUG("end match group rows", K(match_groups_.count()), K(output_cache_.count()));
  match_groups_.reset();
  left_brs_fetcher_.row_list_buffer_cnt_ = 0;
  right_brs_fetcher_.row_list_buffer_cnt_ = 0;
  return ret;
}

int ObMergeJoinOp::output_cache_rows(const int64_t max_row_cnt)
{
  int ret = OB_SUCCESS;
  ObJoinType join_type = MY_SPEC.join_type_;
  switch (join_type) {
    ...
    case LEFT_SEMI_JOIN :
    case LEFT_ANTI_JOIN :
      ret = output_cache_rows<false, false>(max_row_cnt);
      break;
    case LEFT_OUTER_JOIN :
      ret = output_cache_rows<false, true>(max_row_cnt);
      break;
    case RIGHT_OUTER_JOIN :
      ret = output_cache_rows<true, false>(max_row_cnt);
      break;
    case FULL_OUTER_JOIN :
      ret = output_cache_rows<true, true>(max_row_cnt);
      break;
    default : {
      ret = OB_ERR_UNEXPECTED;
      LOG_WARN("unexpected join type", K(ret), K(join_type));
...
{
  int ret = OB_SUCCESS;
  const int64_t output_end_idx = output_idx_ + MIN3(max_row_cnt, MY_SPEC.max_batch_size_,
                                                    get_output_cache_remain_rows_cnt());
  ObEvalCtx::BatchInfoScopeGuard guard(eval_ctx_);
  guard.set_batch_idx(-1);
  ObStoredJoinRow *left_row = NULL;
  ObStoredJoinRow *right_row = NULL;
  LOG_DEBUG("start output rows in cache", K(output_idx_), K(output_end_idx),
             K(output_cache_.count()));
  const ExprFixedArray &left_output = left_->get_spec().output_;
  const ExprFixedArray &right_output = right_->get_spec().output_;
  brs_.size_ = output_end_idx - output_idx_;
  for (; output_idx_ < output_end_idx && OB_SUCC(ret); output_idx_++) {
    guard.set_batch_idx(eval_ctx_.get_batch_idx() + 1);
    RowsPair output = output_cache_.at(output_idx_);
    left_row = output.first;
    right_row = output.second;
    if (NULL != left_row) {
      if (OB_FAIL(left_row->to_expr(left_output, eval_ctx_, left_output.count()))) {
    ...
    if (OB_FAIL(ret)) {
    } else if (NULL != right_row) {
      if (OB_FAIL(right_row->to_expr(right_output, eval_ctx_, right_output.count()))) {
  ...
  if (output_cache_.count() == output_idx_) {
    output_cache_.reuse();
    output_idx_ = 0;
  ...
  LOG_DEBUG("output rows in cache", K(output_idx_), K(brs_.size_),
              K(output_cache_.count()), K(batch_join_state_),
...
{
  int64_t cur_datums_cnt = left_brs_fetcher_.datum_store_.get_row_cnt()
...
int ObMergeJoinOp::output_side_rows(ChildBatchFecther &batch_fetcher,
    ...
    if (OB_SUCC(ret)) {
      switch (batch_join_state_) {
        case BJS_OUTPUT_STORE : {
          if (OB_FAIL(output_cache_rows(max_row_cnt))) {
