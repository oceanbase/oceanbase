...
public:
  ...
  int add_row(const ObChunkDatumStore::StoredRow &sr)
  {
    int ret = OB_SUCCESS;
    if (is_compact_) {
      ret = compact_store_.add_row(sr);
    } else {
      ret =  datum_store_.add_row(sr);
    }
    return ret;
  }
