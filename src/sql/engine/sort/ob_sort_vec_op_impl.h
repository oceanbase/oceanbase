...
public:
  explicit ObSortVecOpImpl(ObMonitorNode &op_monitor_info, lib::MemoryContext &mem_context, ObSqlWorkAreaType profile_type) :
    ...
    eval_ctx_(nullptr), comp_(allocator_), stable_comp_(comp_), sk_store_(&allocator_), addon_store_(&allocator_),
    ...
    compress_type_(NONE_COMPRESSOR), tempstore_read_alignment_size_(0), seq_num_(0)
  {}
...
protected:
  ...
  int build_chunk(const int64_t level, Input &input, const int64_t chunk_idx = -1)
  {
    ...
    if (OB_SUCC(ret)) {
      ...
      //
      // Add the `chunk_idx_` field to ensure stable comparison between chunks.
      // +-------------------------------------------------------------------------------+
