                                        ...
                                        obrpc::ObCommonRpcProxy *common_rpc_proxy)
{
  ...
  HEAP_VAR(obrpc::ObAlterTableArg, alter_table_arg) {
    ...
    if (OB_SUCC(ret)) {
      ...
      } else if (data_version >= DATA_VERSION_4_2_5_3) {// 2. create table
  ...
  if (OB_FAIL(ret)) {
    ...
    if (OB_ISNULL(task_exec_ctx = GET_TASK_EXECUTOR_CTX(ctx))) {
      ...
      if (OB_FAIL(ret)) {
        ...
        if (!is_parallel_create && data_version < DATA_VERSION_4_2_5_3) {
          if (OB_FAIL(common_rpc_proxy->create_table(create_table_arg, res))) {
            LOG_WARN("rpc proxy create table failed", KR(ret), "dst", common_rpc_proxy->get_server());
          }
        } else {
          DEBUG_SYNC(BEFORE_SEND_PARALLEL_CREATE_TABLE);
          create_table_arg.is_parallel_ = is_parallel_create;
          if (OB_FAIL(ObDDLExecutorUtil::execute_pcreate_table(*common_rpc_proxy, my_session, "[parallel create table]",
    ...
    const bool is_sync_ddl_user)
{
  ...
  if (OB_SUCC(ret)) {
    ...
    for (int64_t i = 0; OB_SUCC(ret) && i < ddl_ress.count(); ++i) {
      ...
      if (!alter_table_arg.is_update_global_indexes_ && OB_FAIL(ObDDLExecutorUtil::wait_ddl_finish(ddl_res.tenant_id_, ddl_res.task_id_, my_session, common_rpc_proxy, is_support_cancel))) {
      ...
      if (OB_SUCC(ret)) {
        if (is_parallel_drop) {
          ...
          if (OB_FAIL(common_rpc_proxy->drop_table(drop_table_arg, res))) {
            ...
            // TODO(ziqian.zzq): Remove this. Drop table doesn't use ddl scheduler now
...
int ObCreateTableLikeExecutor::execute(ObExecContext &ctx, ObCreateTableLikeStmt &stmt)
{
  ...
  if (OB_FAIL(stmt.get_first_stmt(first_stmt))) {
    ...
    if (OB_FAIL(ret)) {
    } else if (data_version >= DATA_VERSION_4_2_5_3) {
      DEBUG_SYNC(BEFORE_SEND_PARALLEL_CREATE_TABLE);
