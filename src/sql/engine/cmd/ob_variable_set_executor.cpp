      ...
      ObObj &value_obj)
{
  ...
  } else if (value_obj.is_pl_extend() || value_obj.is_user_defined_sql_type()) {
    ret = OB_NOT_SUPPORTED;
    LOG_WARN("pl extend or user defined sql type is not supported in set stmt", K(ret), K(value_obj));
  }
  return ret;
}
