...
#define DEF_SUBSCHEMA_ENTRY(SUBSCHEMATYPE, CLZ)         \
  {                                                     \
    ...
    subschema_value_destroy<SUBSCHEMATYPE, CLZ>,        \
  }
...
template<ObSubSchemaType TYPE, typename CLZ>
int subschema_value_destroy(void *&value, ObIAllocator &allocator)
{
  int ret = OB_SUCCESS;
  if (OB_ISNULL(value)) {
    ret = OB_INVALID_ARGUMENT;
    LOG_WARN("value is null", K(ret), KP(value));
  } else {
    reinterpret_cast<CLZ *>(value)->destroy();
...
OB_DEF_DESERIALIZE(ObSubSchemaValue)
{
  ...
  if (OB_FAIL(ret)) {
    ...
    ret = OB_ERR_UNEXPECTED;
    LOG_WARN("allocator in deserialize is null", K(ret));
  } else if (is_valid_type(type_)) {
    ObIAllocator *alloc = alloc_;
    void *temp_value = NULL;
    if (OB_FAIL(SUBSCHEMA_FUNCS[type_].init(temp_value, *alloc))) {
      LOG_WARN("fail to init value", K(ret));
    } else if (OB_FAIL(SUBSCHEMA_FUNCS[type_].value_deserialize(temp_value, buf, data_len, pos))) {
      LOG_WARN("fail to deserialize subschema data", K(ret), K(type_), K(signature_));
    } else if (OB_FAIL(deep_copy_value(temp_value, *alloc_))) { // copy value from buffer to local memory
      LOG_WARN("deep copy value failed", K(ret));
    } else if (OB_FAIL(SUBSCHEMA_FUNCS[type_].destroy(temp_value, *alloc))) {
      LOG_WARN("fail to destroy temp value", K(ret));
    }
  }
  return ret;
}
...
int ObSubSchemaCtx::set_subschema(uint16_t subschema_id, ObSubSchemaValue &value)
{
  ...
  if (OB_FAIL(get_subschema(subschema_id, tmp_value))) {
