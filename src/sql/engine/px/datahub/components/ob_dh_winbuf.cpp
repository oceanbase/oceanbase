...
int ObWinbufPieceMsgListener::on_message(
    ObWinbufPieceMsgCtx& ctx, common::ObIArray<ObPxSqcMeta*>& sqcs, const ObWinbufPieceMsg& pkt)
{
  int ret = OB_SUCCESS;
  if (pkt.op_id_ != ctx.op_id_) {
    ...
    ret = OB_ERR_UNEXPECTED;
    LOG_WARN("should not receive any more pkt. already get all pkt expected", K(pkt), K(ctx));
  } else if (pkt.is_end_) {
    /*do nothing*/
  } else if (pkt.is_datum_) {
    if (!ctx.whole_msg_.datum_store_.is_inited() &&
        OB_FAIL(ctx.whole_msg_.datum_store_.init(
    ...
    if (!ctx.whole_msg_.row_store_.is_inited() &&
        OB_FAIL(ctx.whole_msg_.row_store_.init(
            UNLIMITED_MEM, ctx.tenant_id_, common::ObCtxIds::WORK_AREA, "PXDhWinbuf", false))) {
      LOG_WARN("fail to init row store", K(ret));
  ...
  // all piece received,send whole to all SQC
  // and SQC will broadcast whole to its tasks
  if (OB_SUCC(ret) && ctx.received_ == ctx.task_cnt_) {
    ctx.whole_msg_.is_datum_ = pkt.is_datum_;
    ctx.whole_msg_.op_id_ = ctx.op_id_;
    ctx.whole_msg_.is_empty_ = (!ctx.whole_msg_.row_store_.is_inited()) && (!ctx.whole_msg_.datum_store_.is_inited());
...
int ObWinbufPieceMsgCtx::alloc_piece_msg_ctx(
    const ObWinbufPieceMsg& pkt, ObExecContext& ctx, int64_t task_cnt, ObPieceMsgCtx*& msg_ctx)
{
  ...
  if (OB_ISNULL(ctx.get_my_session()) || OB_ISNULL(ctx.get_physical_plan_ctx())) {
    ...
    if (OB_ISNULL(buf)) {
      ...
      msg_ctx = new (buf) ObWinbufPieceMsgCtx(pkt.op_id_,
          task_cnt,
...
OB_DEF_SERIALIZE(ObWinbufPieceMsg)
{
  ...
  if (OB_SUCC(ret)) {
    if (is_datum_) {
      LST_DO_CODE(OB_UNIS_ENCODE, is_end_, is_datum_, col_count_, row_size_, payload_len_);
      if (OB_SUCC(ret)) {
        if (row_size_ > 0) {
          if (OB_ISNULL(datum_row_) || row_size_ != datum_row_->row_size_ ||
              row_size_ < sizeof(ObChunkDatumStore::StoredRow) || payload_len_ < 0) {
            ret = OB_ERR_UNEXPECTED;
            LOG_WARN("datum row is null or size is unexpected",
                K(ret),
                K(row_size_),
                K(datum_row_->row_size_),
                K(payload_len_));
          } else {
            ...
            for (int i = 0; i < datum_row_->cnt_; ++i) {
              pointer2off(*(const char**)&cells[i].ptr_, datum_row_->payload_);
            }
            pos += payload_len_;
          }
        }
      ...
      LST_DO_CODE(OB_UNIS_ENCODE, is_end_, is_datum_, col_count_, row_);
    }
  }
  return ret;
}
...
OB_DEF_DESERIALIZE(ObWinbufPieceMsg)
{
  ...
  if (OB_SUCC(ret)) {
    LST_DO_CODE(OB_UNIS_DECODE, is_end_, is_datum_, col_count_);
    if (is_datum_) {
      OB_UNIS_DECODE(row_size_);
      OB_UNIS_DECODE(payload_len_);
      if (OB_SUCC(ret) && row_size_ > 0) {
        char* datum_row_ptr = NULL;
        if (OB_ISNULL(datum_row_ptr = static_cast<char*>(deseria_allocator_.alloc(row_size_)))) {
          ret = OB_ALLOCATE_MEMORY_FAILED;
          ...
          if (pos + payload_len_ > data_len) {
            ...
            for (int64_t i = 0; i < datum_row_->cnt_; ++i) {
              off2pointer(*(const char**)&cells[i].ptr_, datum_row_->payload_);
            }
            pos += payload_len_;
          }
        }
      ...
      if (col_count_ > 0 && OB_FAIL(ob_create_row(deseria_allocator_, col_count_, row_))) {
...
OB_DEF_SERIALIZE_SIZE(ObWinbufPieceMsg)
{
  ...
  if (is_datum_) {
    LST_DO_CODE(OB_UNIS_ADD_LEN, is_end_, is_datum_, col_count_, row_size_, payload_len_);
    len += payload_len_ > 0 ? payload_len_ : 0;
  } else {
    LST_DO_CODE(OB_UNIS_ADD_LEN, is_end_, is_datum_, col_count_, row_);
  }
...
OB_DEF_DESERIALIZE(ObWinbufWholeMsg)
{
  ...
  if (OB_SUCC(ret)) {
    LST_DO_CODE(OB_UNIS_DECODE, is_empty_, is_datum_)
...
OB_DEF_SERIALIZE(ObWinbufWholeMsg)
{
  ...
  if (OB_SUCC(ret)) {
    LST_DO_CODE(OB_UNIS_ENCODE, is_empty_, is_datum_);
    if (is_datum_ && !is_empty_) {
...
OB_DEF_SERIALIZE_SIZE(ObWinbufWholeMsg)
{
  ...
  LST_DO_CODE(OB_UNIS_ADD_LEN, is_empty_, is_datum_);
  if (is_datum_ && !is_empty_) {
...
int ObWinbufWholeMsg::assign(const ObWinbufWholeMsg& other, common::ObIAllocator* allocator)
{
  ...
  if (!is_empty_) {
    ...
    if (is_datum_) {
      ...
      } else if (OB_FAIL(other.datum_store_.serialize(static_cast<char*>(ser_ptr), ser_len, ser_pos))) {
        LOG_WARN("fail serialzie init task arg", KP(ser_ptr), K(ser_len), K(ser_pos), K(ret));
      } else if (OB_FAIL(datum_store_.deserialize(static_cast<const char*>(ser_ptr), ser_pos, des_pos))) {
      ...
      if (OB_ISNULL(ser_ptr = allocator->alloc(ser_len))) {
        ret = OB_ALLOCATE_MEMORY_FAILED;
        LOG_WARN("fail alloc memory", K(ser_len), KP(ser_ptr), K(ret));
      } else if (OB_FAIL(other.row_store_.serialize(static_cast<char*>(ser_ptr), ser_len, ser_pos))) {
        LOG_WARN("fail serialzie init task arg", KP(ser_ptr), K(ser_len), K(ser_pos), K(ret));
      } else if (OB_FAIL(row_store_.deserialize(static_cast<const char*>(ser_ptr), ser_pos, des_pos))) {
        LOG_WARN("fail des task arg", KP(ser_ptr), K(ser_pos), K(des_pos), K(ret));
      } else if (ser_pos != des_pos) {
        ret = OB_DESERIALIZE_ERROR;
        LOG_WARN("data_len and pos mismatch", K(ser_len), K(ser_pos), K(des_pos), K(ret));
