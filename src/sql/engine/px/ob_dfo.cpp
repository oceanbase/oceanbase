...
int ObPxSqcMeta::assign(const ObPxSqcMeta &other)
{
  ...
  if (OB_FAIL(ret)) {
  } else if (OB_FAIL(px_tablets_info_.assign(other.px_tablets_info_))) {
    LOG_WARN("failed to assign px_tablets_info_");
  }
  return ret;
}
                                          ...
                                          ObIAllocator &alloc)
{
  ...
  if (OB_ISNULL(ser_ptr = alloc.alloc(ser_arg_len))) {
    ...
    ObPhyPlanVersionGuard phy_plan_version_guard(*inner_phy_plan_);
    if (OB_FAIL(src.serialize(static_cast<char *>(ser_ptr), ser_arg_len, ser_pos))) {
