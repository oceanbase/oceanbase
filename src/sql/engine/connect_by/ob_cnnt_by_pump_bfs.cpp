...
bool ObConnectByOpBFSPump::RowComparer::operator()(const PumpNode& pump_node1, const PumpNode& pump_node2)
{
  ...
  } else if (OB_ISNULL(l) || OB_ISNULL(r) || OB_UNLIKELY(l->cnt_ != r->cnt_)) {
...
int ObConnectByOpBFSPump::sort_sibling_rows()
{
  ...
  } else if (0 != sort_collations_->count() && !sort_stack_.empty()) {
...
int ObConnectByOpBFSPump::add_root_row(const ObIArray<ObExpr*>& root_row, const ObIArray<ObExpr*>& output_row)
{
  int ret = OB_SUCCESS;
...
int ObConnectByOpBFSPump::push_back_row_to_stack(const ObIArray<ObExpr*>& left_row, const ObIArray<ObExpr*>& output_row)
{
  int ret = OB_SUCCESS;
  PumpNode pump_node;
  ...
  // We check cycle with the value of prior c1, and c1 is from left table which is got from right child.
  // So we save left_prior_expr first, then calc_path_node and check cycle, and restore left_prior_expr in the end.
  if (OB_FAIL(deep_copy_row(left_row, new_left_row))) {
    ...
    LOG_DEBUG("connect by pump node",
        K(pump_node),
        K(left_row),
        K(output_row),
        K(ObToStringExprRow(*eval_ctx_, *connect_by_prior_exprs_)));
...
int ObConnectByOpBFSPump::append_row(const ObIArray<ObExpr*>& right_row, const ObIArray<ObExpr*>& output_row)
{
  int ret = OB_SUCCESS;
...
int ObConnectByOpBFSPump::init(
    ObNLConnectByWithIndexSpec& connect_by, ObNLConnectByWithIndexOp& connect_by_op, ObEvalCtx& eval_ctx)
{
  int ret = OB_SUCCESS;
...
int ObConnectByOpBFSPump::get_next_row(
    const ObChunkDatumStore::StoredRow*& pump_row, const ObChunkDatumStore::StoredRow*& output_row)
{
  int ret = OB_SUCCESS;
  PumpNode pump_node;
  if (OB_FAIL(free_record_rows())) {
...
int ObConnectByOpBFSPump::add_path_node(PumpNode& pump_node)
{
  ...
  } else if (OB_FAIL(deep_copy_row(*connect_by_prior_exprs_, node.prior_exprs_result_))) {
    LOG_WARN("fail to deep copy row", K(ret));
  } else {
...
int ObConnectByOpBFSPump::calc_path_node(PumpNode& pump_node)
{
  ...
  if (OB_ISNULL(connect_by_prior_exprs_)) {
    ...
    LOG_DEBUG(
        "connect by after prior exprs", K(cur_level_), K(ObToStringExprRow(*eval_ctx_, *connect_by_prior_exprs_)));
...
int ObConnectByOpBFSPump::check_cycle_path()
{
  ...
  for (int64_t i = 0; OB_SUCC(ret) && i < path_stack_.count() && !always_false; ++i) {
    ...
    if (connect_by_prior_exprs_->count() == 0) {
      ...
      ret = OB_ERR_UNEXPECTED;
      LOG_WARN("unexpected status: the column count is not match",
          K(ret),
          "expr cnt",
          connect_by_prior_exprs_->count(),
          "prior result expr cnt",
          cur_node.prior_exprs_result_->cnt_);
    } else {
      int cmp = 0;
  ...
  LOG_DEBUG("trace compare row", K(ObToStringExprRow(*eval_ctx_, *connect_by_prior_exprs_)), K(never_meet_cycle_));
  return ret;
}
...
int64_t ObConnectByOpBFSPump::PumpNode::to_string(char* buf, const int64_t buf_len) const
{
  int64_t pos = 0;
  J_OBJ_START();
...
int64_t ObConnectByOpBFSPump::PathNode::to_string(char* buf, const int64_t buf_len) const
{
  int64_t pos = 0;
  J_OBJ_START();
