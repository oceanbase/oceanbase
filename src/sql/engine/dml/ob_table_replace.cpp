...
OB_SERIALIZE_MEMBER(
    (ObTableReplace, ObTableModify), only_one_unique_key_, primary_key_ids_, res_obj_types_, res_collation_types_);

void ObTableReplace::reset()
{
  only_one_unique_key_ = false;
  res_obj_types_.reset();
  res_collation_types_.reset();
  ObTableModify::reset();
}
...
int ObTableReplace::add_column_collation_type(const ObCollationType type)
{
  int ret = OB_SUCCESS;
  if (OB_FAIL(res_collation_types_.push_back(type))) {
    LOG_WARN("failed to push res obj type", K(ret));
  }
  return ret;
}
...
int ObTableReplace::do_type_cast(ObExecContext& ctx, const common::ObNewRow*& row) const
{
  ...
  if (OB_ISNULL(replace_ctx = GET_PHY_OPERATOR_CTX(ObTableReplaceCtx, ctx, get_id()))) {
    ...
    int64_t collation_size = res_collation_types_.count();
    ...
    if (type_size != cell_size) {
    ...
    } else if (0 != collation_size && collation_size != type_size) {
       // colaltion size is not zero means this is a plan generated by new versions, should equal to type size
      ret = OB_ERR_UNEXPECTED;
      LOG_WARN("collation size is not zero, obj type size is not equal to collation size", K(type_size), K(collation_size));
      ...
      for (int64_t cell_idx = 0; OB_SUCC(ret) && cell_idx < row->get_count(); cell_idx++) {
        ...
        if (collation_size == cell_size) {
          // res_collation_types_ is a new element of the replace operator
          // For the sake of version compatibility, during the multi-version run-off phase,
          // the plan send by old version to the new version follows the logic of the old version
          // the plan send between new version with this commit follows the logic of the new version
          ObCollationType expect_collation_type = res_collation_types_.at(cell_idx);
          replace_ctx->expr_ctx_.column_conv_ctx_.expect_obj_collation_ = expect_collation_type;
          replace_ctx->expr_ctx_.column_conv_ctx_.dest_collation_ = expect_collation_type;
        }
        if (obj.is_null()) {
          // do nothing
