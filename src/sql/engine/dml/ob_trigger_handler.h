...
public:
  ...
  static int init_trigger_params(ObDMLRtCtx &das_ctx,
                                ...
                                const ObTrigDMLCtDef &trig_ctdef,
                                ObTrigDMLRtDef &trig_rtdef);
  // lazy-init helper functions for row trigger execute args
  static int begin_trigger_row(ObExecContext &exec_ctx,
                               const ObTrigDMLCtDef &trig_ctdef,
                               ObTrigDMLRtDef &trig_rtdef);
  static int ensure_trigger_execute_args_ready(ObExecContext &exec_ctx,
                                               const ObTrigDMLCtDef &trig_ctdef,
                                               ObTrigDMLRtDef &trig_rtdef);
...
private:
  ...
  static int calc_when_condition(ObExecContext &exec_ctx,
                                 uint64_t trigger_id,
                                 bool &need_fire);
  static int calc_before_row(ObTableModifyOp &dml_op,
                             ObTrigDMLRtDef &trig_rtdef,
                             uint64_t trigger_id,
                             int64_t trigger_idx);
  static int calc_after_row(ObTableModifyOp &dml_op,
                            ObTrigDMLRtDef &trig_rtdef,
                            uint64_t trigger_id,
                            int64_t trigger_idx);
  ...
  static int check_trigger_arg(ParamStore &params, const pl::ObPLFunction &func, bool is_system_trigger, ObExecContext &ctx);
  ...
  static int calc_trigger_routine_cold_path(ObExecContext &exec_ctx,
                                            ObTrigDMLRtDef &trig_rtdef,
                                            uint64_t trigger_id,
                                            uint64_t routine_id,
                                            ParamStore &params,
  ...
  static int init_trigger_execute_args(ObExecContext &exec_ctx,
                                       const ObTrigDMLCtDef &trig_ctdef,
                                       ObTrigDMLRtDef &trig_rtdef);
  static int init_trigger_execute_arg_array(ObIAllocator &alloc,
                                            int64_t trigger_arg_count,
                                            pl::ObPLExecuteArg *&execute_args);
  static int prepare_trigger_execute_arg(ObExecContext &exec_ctx,
                                         uint64_t trigger_id,
                                         uint64_t routine_id,
                                         pl::ObPLExecuteArg &execute_arg);
  static int check_trigger_record_count(const pl::ObPLFunction &func,
                                        const ObTrigDMLRtDef &trig_rtdef);
  static int get_trigger_execute_arg(ObTrigDMLRtDef &trig_rtdef,
                                     uint64_t routine_id,
                                     int64_t trigger_idx,
                                     pl::ObPLExecuteArg *&execute_arg);
  static void free_trigger_execute_args(ObTrigDMLRtDef &trig_rtdef);
  static int obtain_trigger_routine(pl::ObPLExecuteArg &execute_arg,
                                    ObExecContext &exec_ctx,
                                    uint64_t trigger_id,
                                    uint64_t routine_id,
