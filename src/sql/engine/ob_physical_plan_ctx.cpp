...
int ObPhysicalPlanCtx::serialize(char *buf, const int64_t buf_len, int64_t &pos) const
{
  int ret = OB_SUCCESS;
  ...
  if (OB_SUCC(ret)) {
    ...
    if (OB_FAIL(serialize_(buf, buf_len, pos))) {
      ...
      int64_t tmp_pos = 0;
      CHECK_SERIALIZE_SIZE(ObPhysicalPlanCtx, serial_size);
...
int ObPhysicalPlanCtx::serialize_(char *buf, const int64_t buf_len, int64_t &pos) const
{
  int ret = OB_SUCCESS;
...
int ObPhysicalPlanCtx::deserialize(const char *buf, const int64_t data_len, int64_t &pos)
{
  ...
  if (OB_SUCC(ret)) {
    ...
    if (version == 3) {
      ...
      OB_UNIS_DECODE(mock_enable_rich_format);
    }
                                                     ...
                                                     ObSchemaGetterGuard *schema_guard)
{
  ...
  if (OB_NOT_NULL(phy_plan_)) { // physical plan exist, use subschema ctx on phy plan
    ...
    if (OB_HASH_NOT_EXIST != ret) {
      ...
      if (OB_ISNULL(schema_guard)) {
        ret = OB_ERR_UNEXPECTED;
        LOG_WARN("No schema gurad to generate udt_meta", K(ret));
      } else if (OB_FAIL(src_meta.deep_copy(allocator_, dst_meta))) {
