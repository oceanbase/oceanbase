...
ObRowSampleScan::ObRowSampleScan(common::ObIAllocator& allocator) : ObTableScan(allocator)
{}

int ObRowSampleScan::prepare_scan_param(ObExecContext& ctx) const
{
  int ret = OB_SUCCESS;
...
inline int ObRowSampleScan::init_op_ctx(ObExecContext& ctx) const
{
  int ret = OB_SUCCESS;
  ObPhyOperatorCtx* op_ctx = NULL;
  ObTableScanInput* scan_input = NULL;
  if (OB_ISNULL(scan_input = GET_PHY_OP_INPUT(ObTableScanInput, ctx, get_id()))) {
    if (OB_FAIL(CREATE_PHY_OP_INPUT(ObRowSampleScanInput, ctx, get_id(), get_type(), scan_input))) {
  ...
  if (OB_FAIL((ret))) {
    // do nothing
  } else if (OB_FAIL(CREATE_PHY_OPERATOR_CTX(ObRowSampleScanCtx, ctx, get_id(), get_type(), op_ctx))) {
...
int ObRowSampleScan::inner_get_next_row(ObExecContext& ctx, const common::ObNewRow*& row) const
{
  int ret = OB_SUCCESS;
...
OB_SERIALIZE_MEMBER((ObRowSampleScan, ObTableScan), sample_info_);
