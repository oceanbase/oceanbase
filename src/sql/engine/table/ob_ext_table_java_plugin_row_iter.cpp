...
#include "plugin/legacy/external_table/ob_external_arrow_status.h"
...
////////////////////////////////////////////////////////////////////////////////
// ObExtTableJavaPluginRowIterator
ObExtTableJavaPluginRowIterator::ObExtTableJavaPluginRowIterator()
...
int ObExtTableJavaPluginRowIterator::init(const ObTableScanParam *scan_param)
{
  int ret = OB_SUCCESS;
...
int ObExtTableJavaPluginRowIterator::rescan(const ObTableScanParam *scan_param)
{
  int ret = OB_SUCCESS;
...
void ObExtTableJavaPluginRowIterator::reset()
{
...
void ObExtTableJavaPluginRowIterator::destroy()
{
  reset();
...
int ObExtTableJavaPluginRowIterator::open_scanner(const ObTableScanParam *scan_param)
{
  int ret = OB_SUCCESS;
...
int ObExtTableJavaPluginRowIterator::get_next_row()
{
  int64_t count = 0;
  return get_next_rows(count, 1);
}

int ObExtTableJavaPluginRowIterator::get_next_rows(int64_t &count, int64_t capacity)
{
  int ret = OB_SUCCESS;
...
int ObExtTableJavaPluginRowIterator::get_next_record_batch()
{
  int ret = OB_SUCCESS;
  ObEvalCtx *eval_ctx = nullptr;
  int64_t batch_size = 0;
...
int ObExtTableJavaPluginRowIterator::init_plugin_column_index_map_if_need(RecordBatch *record_batch)
{
  int ret = OB_SUCCESS;
...
int ObExtTableJavaPluginRowIterator::init_external_scan_param(const storage::ObTableScanParam *scan_param)
{
  int ret = OB_SUCCESS;
...
#if 0
int ObExtTableJavaPluginRowIterator::load_data(ObExpr *expr, arrow::Array &arrow_array, LoadFunctor loader)
{
  int ret = OB_SUCCESS;
