...
public:
  ...
  {
    void *value_;
    common::ObIAllocator *alloc_; // used for deserialize only
  };
};
...
typedef int (*ob_subschema_value_destroy)(void *&value, ObIAllocator &allocator);
...
struct ObSubSchemaFuncs
{
  ...
  ob_subschema_value_destroy destroy;
};
...
template <ObSubSchemaType TYPE, typename CLZ> int64_t subschema_value_serialize_size(void *value);
template <ObSubSchemaType TYPE, typename CLZ>
...
public:
  ...
  int get_subschema_id(uint64_t value_signature, ObSubSchemaType type,
