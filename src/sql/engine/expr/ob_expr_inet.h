...
class ObExprInetCommon {
  public :
  static bool str_to_ipv4(int len, const char *str, in_addr *ipv4addr);
  static bool str_to_ipv6(int len, const char *str, in6_addr *ipv6addr);
  static void ip_to_str(int type, const void *ip, void *result);
};
...
private:
  // helper func
  template <typename T>
  static int ob_inet_aton(T& result, const common::ObString& text);
...
public:
  ...
  virtual int calc_result_type1(ObExprResType& type, ObExprResType& text, common::ObExprTypeCtx& type_ctx) const;
  static int calc(common::ObObj& result, const common::ObObj& text, ObExprCtx& expr_ctx, common::ObExprStringBuf& string_buf);
  virtual int calc_result1(common::ObObj& result, const common::ObObj& text, common::ObExprCtx& expr_ctx) const;
  virtual int cg_expr(ObExprCGCtx& op_cg_ctx, const ObRawExpr& raw_expr, ObExpr& rt_expr) const override;
...
private:
  // helper func
  static int inet6_ntoa(common::ObObj& result, const common::ObString& text, common::ObExprStringBuf& string_buf);
  static int inet6_ntoa(common::ObDatum& result, const common::ObString& text);
...
private:
  // helper func
  static int inet6_aton(common::ObObj& result, const common::ObString& text, common::ObExprStringBuf& string_buf);
  static int inet6_aton(common::ObDatum& result, const common::ObString& text);
  static bool check_ip(const char *ip_buf, int len);
...
private:
  // helper func
  template <typename T>
  static int is_ipv4_mapped(T& result, const common::ObString& num_val);
...
private:
  // helper func
  template <typename T>
  static int is_ipv4_compat(T& result, const common::ObString& num_val);
