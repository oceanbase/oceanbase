    ...
    ObObj& result, const ObObj& unit, const ObObj& left, const ObObj& right, ObExprCtx& expr_ctx) const
{
  ...
  if (OB_ISNULL(expr_ctx.my_session_)) {
    ...
    TYPE_CHECK(left, ObDateTimeType);
    TYPE_CHECK(right, ObDateTimeType);
    int64_t res = 0;
    bool is_null = false;
...
int ObExprTimeStampDiff::eval_timestamp_diff(const ObExpr& expr, ObEvalCtx& ctx, ObDatum& res)
{
  ...
  if (OB_ISNULL(session)) {
    ...
    bool is_null = false;
    if (OB_FAIL(
            calc(res_int, is_null, u->get_int(), l->get_datetime(), r->get_datetime(), session->get_timezone_info()))) {
...
int ObExprTimeStampDiff::cg_expr(ObExprCGCtx& ctx, const ObRawExpr& raw_expr, ObExpr& rt_expr) const
{
  ...
  CK(ObIntType == rt_expr.args_[0]->datum_meta_.type_ && ObDateTimeType == rt_expr.args_[1]->datum_meta_.type_ &&
