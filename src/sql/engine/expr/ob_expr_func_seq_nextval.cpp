...
ObExprFuncSeqNextval::ObExprFuncSeqNextval(ObIAllocator& alloc)
    : ObFuncExprOperator(alloc, T_FUN_SYS_MYSQL_SEQ_NEXTVAL, N_MYSQL_SEQ_NEXTVAL, 1, NOT_ROW_DIMENSION)
{}

ObExprFuncSeqNextval::~ObExprFuncSeqNextval()
{}

int ObExprFuncSeqNextval::calc_result_type1(ObExprResType& type, ObExprResType& type1, ObExprTypeCtx& type_ctx) const
{
  UNUSED(type_ctx);
  int ret = OB_SUCCESS;
  if (!type1.is_uint64()) {
...
int ObExprFuncSeqNextval::calc_result1(ObObj& result, const ObObj& obj, ObExprCtx& expr_ctx) const
{
  int ret = OB_SUCCESS;
