...
namespace sql
{
class ObExprDocLength : public ObFuncExprOperator
{
public:
  explicit ObExprDocLength(common::ObIAllocator &alloc);
  virtual ~ObExprDocLength() = default;
  ...
  static int generate_doc_length(
          const ObExpr &raw_ctx,
          ObEvalCtx &eval_ctx,
          ObDatum &expr_datum);
private :
  //disallow copy
  DISALLOW_COPY_AND_ASSIGN(ObExprDocLength);
};
}
}
#endif //SQL_ENGINE_EXPR_OB_EXPR_DOC_LENGTH_H_
