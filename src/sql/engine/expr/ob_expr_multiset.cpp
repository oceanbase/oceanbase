...
int ObExprMultiSet::eval_multiset_composite(ObExecContext &exec_ctx,
  ...
  static constexpr char MULTISET_EXCEPT_PL[] =
      ...
      "cmp1 boolean;\n"
      "cmp2 boolean;\n"
      "rhs_atomic_null boolean;\n"
      "lhs_atomic_null boolean;\n"
      "begin\n"
      "if lhs_count > 0 then\n"
      "  if rhs_count > 0 then\n"
      "    for i in :rhs.first..:rhs.last loop\n"
      "      if not :rhs.exists(i) then continue; end if;\n"
      "      rhs_atomic_null := (:rhs(i) is null);\n"
      "      if rhs_atomic_null then\n"
      "        cmp1 := NULL;\n"
      "      else\n"
      "        cmp1 := (:rhs(i) = :rhs(i));\n"
      "      end if;\n"
      ...
      "          lhs_atomic_null := (:lhs(j) is null);\n"
      "          if rhs_atomic_null or lhs_atomic_null then\n"
      "            if rhs_atomic_null and lhs_atomic_null then\n"
      "              :lhs.delete(j);\n"
      "              exit;\n"
      "            end if;\n"
      "          else\n"
  ...
  static constexpr char DISTINCT_PL[] =
      "declare\n"
      "has_atomic_null_element boolean := FALSE;\n"
      "has_compare_null_element boolean := FALSE;\n"
      "cmp boolean;\n"
      "begin\n"
      ...
      "    if not has_atomic_null_element then\n"
      "      has_atomic_null_element := TRUE;\n"
      "      :result.extend(1);\n"
      "      :result(:result.count) := :coll(idx);\n"
      "    end if;\n"
      "    continue;\n"
      "  elsif (:coll(idx) = :coll(idx)) is null then\n"
      "    if not has_compare_null_element then\n"
      "      has_compare_null_element := TRUE;\n"
      "      :result.extend(1);\n"
      "      :result(:result.count) := :coll(idx);\n"
      "    end if;\n"
      "    continue;\n"
