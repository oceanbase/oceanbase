                                        ...
                                        CollectionPredRes &result)
{
  ...
  //    FALSE: if some element in LHS can't be mapped to RHS, which is |Ul| > |Nr|
  //    NULL: otherwise, we have |Ul| <= |Nr|, so all elements in LHS can be mapped to RHS, and the result is NULL
  // Oracle additionally returns FALSE when LHS contains both atomically NULL and
  // comparison-NULL elements, but RHS contains no atomically NULL element.
  static constexpr char SUBMULTISET_PL[] =
      "declare\n"
      "cmp boolean := :result;\n"
      "lhs_count pls_integer := :lhs.count;\n"
      "rhs_count pls_integer := :rhs.count;\n"
      "lhs_atomic_null_count pls_integer := 0;\n"
      "lhs_compare_null_count pls_integer := 0;\n"
      "rhs_atomic_null_count pls_integer := 0;\n"
      "rhs_compare_null_count pls_integer := 0;\n"
      "matched pls_integer := 0;\n"  // C
      "null_reuslt boolean := NULL;\n"
      "begin\n"
      "for j in :rhs.first..:rhs.last loop\n"
      "  if :rhs.exists(j) then\n"
      "    if :rhs(j) is null then\n"
      "      rhs_atomic_null_count := rhs_atomic_null_count + 1;\n"
      "      :rhs.delete(j);\n"
      "    elsif (:rhs(j)=:rhs(j)) is null then\n"
      "      rhs_compare_null_count := rhs_compare_null_count + 1;\n"
      "      :rhs.delete(j);\n"
      "    end if;\n"
      "  end if;\n"
      "end loop;\n"
      ...
      "  if :lhs(i) is null then\n"
      "    lhs_atomic_null_count := lhs_atomic_null_count + 1;\n"
      "    continue;\n"
      "  elsif (:lhs(i)=:lhs(i)) is null then\n"
      "    lhs_compare_null_count := lhs_compare_null_count + 1;\n"
      "    continue;\n"
      "  end if;\n"
      "  for j in :rhs.first..:rhs.last loop\n"
      ...
      "  :result := TRUE;\n"
      "elsif lhs_count - matched - lhs_atomic_null_count - lhs_compare_null_count\n"
      "      > rhs_atomic_null_count + rhs_compare_null_count then\n"
      "  :result := FALSE;\n"
      "elsif lhs_atomic_null_count > 0\n"
      "      and lhs_compare_null_count > 0\n"
      "      and rhs_atomic_null_count = 0 then\n"
      "  :result := FALSE;\n"
      "else\n"
      "  :result := null_reuslt;\n"
      "end if;\n"
                                         ...
                                         ObObj &res)
{
  ...
  static constexpr char IS_SET_PL[] =
      "declare\n"
      "has_atomic_null boolean := FALSE;\n"
      "has_compare_null boolean := FALSE;\n"
      "cmp boolean;\n"
      "begin\n"
      ":result := TRUE;\n"
      ...
      "  if :coll(i) is null then\n"
      "    if has_atomic_null then\n"
      "      :result := FALSE;\n"
      "      exit;\n"
      "    else\n"
      "      has_atomic_null := TRUE;\n"
      "      continue;\n"
      "    end if;\n"
      "  end if;\n"
      ...
      "    if has_compare_null then\n"
      "       :result := FALSE;\n"
      "       exit;\n"
      "    else\n"
      "      has_compare_null := TRUE;\n"
      "    end if;\n"
      "  else\n"
      "    :buf.extend(1);\n"
