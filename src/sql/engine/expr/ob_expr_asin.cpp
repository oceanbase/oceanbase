...
static int vector_asin(const ObExpr &expr,
                       ObEvalCtx &ctx,
    ...
    for (int64_t idx = bound.start(); OB_SUCC(ret) && idx < bound.end(); ++idx) {
      if (skip.at(idx) || eval_flags.at(idx)) {
        ...
        if (arg > 1 || arg < -1) {
          ...
          res_vec->set_double(idx, asin(arg));
        }
      } else {
        number::ObNumber res_nmb;
        number::ObNumber radian_nmb(arg_vec->get_number(idx));
        ObEvalCtx::TempAllocGuard alloc_guard(ctx);
        if (OB_FAIL(radian_nmb.asin(res_nmb, alloc_guard.get_allocator()))) {
...
int ObExprAsin::eval_number_asin_vector(const ObExpr &expr,
                                        ObEvalCtx &ctx,
                                        const ObBitVector &skip,
                                        const EvalBound &bound)
{
  ...
  if (OB_FAIL(expr.args_[0]->eval_vector(ctx, skip, bound))) {
    LOG_WARN("fail to eval asin param", K(ret));
  } else {
    VectorFormat arg_format = expr.args_[0]->get_format(ctx);
    VectorFormat res_format = expr.get_format(ctx);
    if (VEC_DISCRETE == arg_format && VEC_DISCRETE == res_format) {
      ret = vector_asin<NumberDiscVec, NumberDiscVec, false>(expr, ctx, skip, bound);
    } else if (VEC_UNIFORM == arg_format && VEC_DISCRETE == res_format) {
      ret = vector_asin<NumberUniVec, NumberDiscVec, false>(expr, ctx, skip, bound);
    } else if (VEC_CONTINUOUS == arg_format && VEC_DISCRETE == res_format) {
      ret = vector_asin<NumberContVec, NumberDiscVec, false>(expr, ctx, skip, bound);
    } else {
      ret = vector_asin<ObVectorBase, ObVectorBase, false>(expr, ctx, skip, bound);
    }
  }
...
int ObExprAsin::eval_double_asin_vector(const ObExpr &expr,
                                        ObEvalCtx &ctx,
                                        const ObBitVector &skip,
                                        const EvalBound &bound)
{
  ...
  if (OB_FAIL(expr.args_[0]->eval_vector(ctx, skip, bound))) {
    ...
    if (VEC_FIXED == arg_format && VEC_FIXED == res_format) {
      ret = vector_asin<DoubleFixedVec, DoubleFixedVec, true>(expr, ctx, skip, bound);
    } else if (VEC_UNIFORM == arg_format && VEC_FIXED == res_format) {
      ret = vector_asin<DoubleUniVec, DoubleFixedVec, true>(expr, ctx, skip, bound);
    } else {
      ret = vector_asin<ObVectorBase, ObVectorBase, true>(expr, ctx, skip, bound);
    }
  }
...
int ObExprAsin::cg_expr(ObExprCGCtx &expr_cg_ctx,
                        ...
                        ObExpr &rt_expr) const
{
  ...
  if (ObDoubleType == rt_expr.args_[0]->datum_meta_.type_) {
    rt_expr.eval_vector_func_ = eval_double_asin_vector;
  } else if (ObNumberType == rt_expr.args_[0]->datum_meta_.type_) {
    rt_expr.eval_vector_func_ = eval_number_asin_vector;
  } else {
