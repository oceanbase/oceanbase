...
public:
  ...
  static int slow_reverse_search(common::ObIAllocator& alloc, const common::ObCollationType& cs_type,
      const common::ObString& str1, const common::ObString& str2, int64_t neg_start, int64_t occ, uint32_t& idx);
  virtual int cg_expr(ObExprCGCtx& expr_cg_ctx, const ObRawExpr& raw_expr, ObExpr& rt_expr) const;
  static int calc_oracle_instr_arg(const ObExpr& expr, ObEvalCtx& ctx, bool& is_null, common::ObDatum*& haystack,
      common::ObDatum*& needle, int64_t& pos_int, int64_t& occ_int, common::ObCollationType& calc_cs_type);
