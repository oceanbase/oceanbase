...
int ObExprRegexContext::init(const ObString &origin_pattern,
                             int cflags,
                             ...
                             const bool reusable)
{
  ...
  } else if (OB_FAIL(preprocess_pattern(string_buf, origin_pattern, pattern))) {
    LOG_WARN("failed to prepare process pattern", K(origin_pattern), K(pattern));
  } else if (reusable &&
                                       ...
                                       ObSEArray<uint32_t, 4> &begin_locations) const
{
  ...
  int64_t tmp_start = 0;
  int64_t start = 0;
  ObSEArray<ObSEArray<ObString, 8>, 8> subexpr_arrays;//用于保存匹配分组内容用于后续替换
  if (OB_UNLIKELY(!inited_)) {
    ret = OB_NOT_INIT;
    LOG_WARN("regexp context not inited yet", K(ret), K(this));
