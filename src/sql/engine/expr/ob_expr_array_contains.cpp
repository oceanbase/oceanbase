...
int ObExprArrayContains::eval_array_contains_ObString(const ObExpr &expr, ObEvalCtx &ctx, ObDatum &res)
{
  int ret = OB_SUCCESS;
  ObEvalCtx::TempAllocGuard tmp_alloc_g(ctx);
  common::ObArenaAllocator &tmp_allocator = tmp_alloc_g.get_allocator();
  ...
  if (OB_FAIL(expr.args_[p0]->eval(ctx, datum))) {
    ...
    bool contains_null = arr_obj->contain_null();
    res.set_bool(contains_null);
  } else if (OB_FAIL(ObTextStringHelper::get_string(expr, tmp_allocator, static_cast<int64_t>(p1), datum_val, val))) {
    LOG_WARN("failed to get search value string", K(ret));
  } else if (OB_FAIL(ObArrayUtil::contains(*arr_obj, val, bret))) {
    LOG_WARN("array contains failed", K(ret));
  } else {
    res.set_bool(bret);
...
int ObExprArrayContains::eval_array_contains_batch_ObString(
    const ObExpr &expr, ObEvalCtx &ctx, const ObBitVector &skip, const int64_t batch_size)
{
  int ret = OB_SUCCESS;
  ObDatumVector res_datum = expr.locate_expr_datumvector(ctx);
    ...
    for (int64_t j = 0; OB_SUCC(ret) && j < batch_size; ++j) {
      ...
      if (src_array.at(j)->is_null()) {
        ...
        res_datum.at(j)->set_bool(contains_null);
                     ...
                     expr, tmp_allocator, static_cast<int64_t>(p1), val_array.at(j), val))) {
        LOG_WARN("failed to get search value string", K(ret));
      } else if (OB_FAIL(ObArrayUtil::contains(*arr_obj, val, bret))) {
        LOG_WARN("array contains failed", K(ret));
      } else {
...
int ObExprArrayContains::eval_array_contains_vector_ObString(
    const ObExpr &expr, ObEvalCtx &ctx, const ObBitVector &skip, const EvalBound &bound)
{
  int ret = OB_SUCCESS;
  uint32_t p0 = expr.extra_ == 1 ? 1 : 0;
    ...
    for (int64_t idx = bound.start(); OB_SUCC(ret) && idx < bound.end(); ++idx) {
      ...
      if (OB_FAIL(ret)) {
        ...
        res_vec->set_bool(idx, contains_null);
                     ...
                     expr, tmp_allocator, static_cast<int64_t>(p1), idx, right_vec, val))) {
        LOG_WARN("failed to get search value string", K(ret));
      } else if (OB_FAIL(ObArrayUtil::contains(*arr_obj, val, bret))) {
        LOG_WARN("array contains failed", K(ret));
      } else {
