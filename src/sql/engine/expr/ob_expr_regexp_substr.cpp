...
int ObExprRegexpSubstr::cg_expr(ObExprCGCtx &op_cg_ctx, const ObRawExpr &raw_expr, ObExpr &rt_expr) const
{
  ...
  if (OB_SUCC(ret)) {
    ...
    if (OB_ISNULL(text) || OB_ISNULL(pattern)) {
      ...
      rt_expr.eval_func_ = is_use_re2 ? eval_re2_regexp_substr : (is_use_hs ? eval_hs_regexp_substr : eval_regexp_substr);
...
int ObExprRegexpSubstr::regexp_substr(const ObExpr &expr, ObEvalCtx &ctx, ObDatum &expr_datum)
{
  ...
  // ICU (ObExprRegexContext) uses UTF16, hyperscan and RE2 use UTF8MB4
  const bool constexpr is_icu_engine = std::is_same<RegExpCtx, ObExprRegexContext>::value;
  ObDatum *text = NULL;
    ...
    if (OB_FAIL(ObExprUtil::get_int_param_val(
      ...
      if (OB_FAIL(ret) || is_final) {
        ...
        const ObCollationType constexpr expected_bin_coll =
          is_icu_engine ? CS_TYPE_UTF16_BIN : CS_TYPE_UTF8MB4_BIN;
        const ObCollationType constexpr expected_ci_coll =
          is_icu_engine ? CS_TYPE_UTF16_GENERAL_CI : CS_TYPE_UTF8MB4_GENERAL_CI;
...
int ObExprRegexpSubstr::eval_re2_regexp_substr(const ObExpr &expr, ObEvalCtx &ctx, ObDatum &expr_datum)
{
  return regexp_substr<ObExprRe2RegexCtx>(expr, ctx, expr_datum);
}
