...
int ObExprArrayRemove::eval_array_remove_ObString(const ObExpr &expr, ObEvalCtx &ctx, ObDatum &res)
{
  int ret = OB_SUCCESS;
  ObEvalCtx::TempAllocGuard tmp_alloc_g(ctx);
  common::ObArenaAllocator &tmp_allocator = tmp_alloc_g.get_allocator();
  ...
  if (OB_FAIL(expr.args_[0]->eval(ctx, datum))) {
    ...
    res_arr_obj = arr_obj;
             ...
             OB_FAIL(ObTextStringHelper::get_string(expr, tmp_allocator, 1, datum_val, val))) {
    LOG_WARN("failed to get remove value string", K(ret));
  } else if (!datum_val->is_null() && OB_FAIL(ObArrayUtil::contains(*arr_obj, val, bret))) {
    LOG_WARN("array contains failed", K(ret));
  } else if (!bret) {
    changed = false;
...
int ObExprArrayRemove::eval_array_remove_batch_ObString(
    const ObExpr &expr, ObEvalCtx &ctx, const ObBitVector &skip, const int64_t batch_size)
{
  int ret = OB_SUCCESS;
  ObDatumVector res_datum = expr.locate_expr_datumvector(ctx);
    ...
    for (int64_t j = 0; OB_SUCC(ret) && j < batch_size; ++j) {
      ...
      if (src_array.at(j)->is_null()) {
        ...
        res_arr_obj = arr_obj;
                 ...
                 OB_FAIL(ObTextStringHelper::get_string(expr, tmp_allocator, 1, val_array.at(j), val))) {
        LOG_WARN("failed to get remove value string", K(ret));
      } else if (!val_array.at(j)->is_null() && OB_FAIL(ObArrayUtil::contains(*arr_obj, val, bret))) {
        LOG_WARN("array contains failed", K(ret));
      } else if (!bret) {
...
int ObExprArrayRemove::eval_array_remove_vector_ObString(
    const ObExpr &expr, ObEvalCtx &ctx, const ObBitVector &skip, const EvalBound &bound)
{
  int ret = OB_SUCCESS;
  if (OB_FAIL(expr.args_[0]->eval_vector(ctx, skip, bound)) ||
    ...
    for (int64_t idx = bound.start(); OB_SUCC(ret) && idx < bound.end(); ++idx) {
      ...
      } else if (OB_FAIL(ObTextStringHelper::get_string(expr, tmp_allocator, 1, idx, right_vec, val))) {
        LOG_WARN("failed to get remove value string", K(ret));
      } else if (OB_FAIL(ObArrayUtil::contains(*arr_obj, val, bret))) {
        LOG_WARN("array contains failed", K(ret));
      } else if (!bret) {
      ...
      if (OB_FAIL(ret)) {
        ...
        if (changed) {
          if (OB_FAIL(ObArrayUtil::clone_except(
                  tmp_allocator, *arr_obj, &val, right_vec->is_null(idx), res_arr_obj))) {
            LOG_WARN("array remove failed", K(ret));
          }
        }
        if (OB_SUCC(ret)) {
