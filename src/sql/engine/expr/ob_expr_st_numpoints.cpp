...
namespace sql
{
ObExprSTNumPoints::ObExprSTNumPoints(common::ObIAllocator &alloc)
    : ObFuncExprOperator(alloc, T_FUN_SYS_ST_NUMPOINTS, N_ST_NUMPOINTS, 1,
          NOT_VALID_FOR_GENERATED_COL, NOT_ROW_DIMENSION)
{}

ObExprSTNumPoints::~ObExprSTNumPoints()
{}

int ObExprSTNumPoints::calc_result_type1(
    ObExprResType &type, ObExprResType &type1, common::ObExprTypeCtx &type_ctx) const
{
  ...
  if (!ob_is_string_type(obj_type1) && !ob_is_geometry(obj_type1) && !ob_is_null(obj_type1)) {
    ...
    LOG_USER_ERROR(OB_ERR_GIS_INVALID_DATA, N_ST_NUMPOINTS);
...
int ObExprSTNumPoints::eval_st_numpoints(
    const ObExpr &expr, ObEvalCtx &ctx, ObDatum &res)
{
  ...
  if (ob_is_null(type1)) {
    ...
    } else if (OB_FAIL(ObGeoExprUtils::build_geometry(temp_allocator, wkb, geo, nullptr,
                                                      N_ST_NUMPOINTS, ObGeoBuildFlag::GEO_ALLOW_3D_DEFAULT))) {
      LOG_WARN("fail to build geometry from wkb", K(ret), K(wkb));      // ObIWkbGeom
