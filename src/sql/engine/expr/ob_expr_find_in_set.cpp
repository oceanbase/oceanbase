...
inline bool memequal(const char* p1, int64_t size1, const char* p2, int64_t size2) {
...
int bin_collation_search(const ObString &str, const ObString &str_list, uint64_t &res_pos) {
  ...
  if (str_list.length() < str.length()) {
    ...
    while (offset < str_list.length()) {
      ...
      const char* p = reinterpret_cast<const char*>(memchr(str_list.ptr() + offset + 1, ',', n));
      if (p != nullptr) {
      ...
      ++num;
      bool is_equal = memequal(str.ptr(), str.length(), str_list.ptr() + pre_offset + 1, offset - pre_offset - 1);
      if (is_equal) {
        res_pos = num;
        break;
      }
...
int ObExprFindInSet::calc_find_in_set_vector_dispatch(VECTOR_EVAL_FUNC_ARG_DECL) {
  ...
  Arg1Vec *strlist_vec = reinterpret_cast<Arg1Vec *>(expr.args_[1]->get_vector(ctx));
...
int ObExprFindInSet::calc_find_in_set_vector(VECTOR_EVAL_FUNC_ARG_DECL) {
  ...
  if (OB_UNLIKELY(2 != expr.arg_cnt_)) {
    ...
    if (VEC_DISCRETE == arg0_format && VEC_DISCRETE == arg1_format && VEC_FIXED == res_format) {
      ret = calc_find_in_set_vector_dispatch<StrDiscVec, StrDiscVec, UIntegerFixedVec>(VECTOR_EVAL_FUNC_ARG_LIST);
    } else if (VEC_DISCRETE == arg0_format && VEC_DISCRETE == arg1_format && VEC_UNIFORM == res_format) {
      ret = calc_find_in_set_vector_dispatch<StrDiscVec, StrDiscVec, UIntegerUniVec>(VECTOR_EVAL_FUNC_ARG_LIST);
    } else if (VEC_DISCRETE == arg0_format && VEC_UNIFORM == arg1_format && VEC_FIXED == res_format) {
      ret = calc_find_in_set_vector_dispatch<StrDiscVec, StrUniVec, UIntegerFixedVec>(VECTOR_EVAL_FUNC_ARG_LIST);
    } else if (VEC_DISCRETE == arg0_format && VEC_UNIFORM == arg1_format && VEC_UNIFORM == res_format) {
      ret = calc_find_in_set_vector_dispatch<StrDiscVec, StrUniVec, UIntegerUniVec>(VECTOR_EVAL_FUNC_ARG_LIST);
    } else if (VEC_UNIFORM == arg0_format && VEC_DISCRETE == arg1_format && VEC_FIXED == res_format) {
      ret = calc_find_in_set_vector_dispatch<StrUniVec, StrDiscVec, UIntegerFixedVec>(VECTOR_EVAL_FUNC_ARG_LIST);
    } else if (VEC_UNIFORM == arg0_format && VEC_DISCRETE == arg1_format && VEC_UNIFORM == res_format) {
      ret = calc_find_in_set_vector_dispatch<StrUniVec, StrDiscVec, UIntegerUniVec>(VECTOR_EVAL_FUNC_ARG_LIST);
    } else if (VEC_UNIFORM == arg0_format && VEC_UNIFORM == arg1_format && VEC_FIXED == res_format) {
