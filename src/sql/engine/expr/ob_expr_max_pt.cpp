            ...
            if (OB_ISNULL(sql_proxy)) {
              ...
              SMART_VAR(ObISQLClient::ReadResult, read_result) {
                ...
                if (OB_FAIL(sql_client_retry_weak.read(read_result, tenant_id, inner_sql.ptr()))) {
                  ...
                  } else if (OB_FAIL(expr.deep_copy_datum(ctx, partition_datum))) {
                              ...
                              ObExpr &rt_expr) const
{
  ...
  if (min_cluster_version >= CLUSTER_VERSION_4_4_2_3 && min_cluster_version < CLUSTER_VERSION_4_5_0_0) {
    ...
    LOG_WARN("MAX_PT not supported", K(GET_MIN_CLUSTER_VERSION()), K(ret));
  }
  return ret;
}
