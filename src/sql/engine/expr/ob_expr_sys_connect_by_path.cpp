...
int ObExprSysConnectByPath::calc_result_type2(
...
int ObExprSysConnectByPath::calc_result2(
    common::ObObj& result, const common::ObObj& left, const common::ObObj& right, common::ObExprCtx& expr_ctx) const
{
  ...
  if (OB_ISNULL(expr_ctx.exec_ctx_)) {
    ...
    if (OB_FAIL(ret)) {
    } else if (OB_FAIL(StringContain(val_str, separator_str, is_contain, result_type_.get_calc_collation_type()))) {
      ...
      LOG_WARN("when using SYS_CONNECT_BY_PATH function, cannot have separator as part of column value");
    } else if (OB_ISNULL(op_ctx = GET_PHY_OPERATOR_CTX(
                             ObPhyOperator::ObPhyOperatorCtx, exec_ctx, expr_ctx.phy_operator_ctx_id_))) {
      ret = OB_ERR_UNEXPECTED;
    ...
    } else if (OB_FAIL(ObExprConcat::calc(node_res, separator_str, val_str, expr_ctx.calc_buf_))) {
      LOG_WARN("fail to concat string", K(right), K(left), K(ret));
    } else if (OB_FAIL(connect_by_ctx->get_sys_parent_path(expr_ctx.sys_connect_by_path_id_, parent_path))) {
      LOG_WARN("fail to get parent path", K(ret));
    } else if (OB_FAIL(ObExprConcat::calc(result, parent_path, node_res.get_string(), expr_ctx.calc_buf_))) {
      LOG_WARN("fail to concat string", K(parent_path), K(node_res), K(ret));
    } else if (OB_FAIL(connect_by_ctx->set_sys_current_path(
                   expr_ctx.sys_connect_by_path_id_, node_res.get_string(), result.get_string()))) {
...
int ObExprSysConnectByPath::StringContain(
    const ObString& src_str, const ObString& sub_str, bool& is_contain, ObCollationType cs_type)
{
  int ret = OB_SUCCESS;
  if (OB_UNLIKELY(CS_TYPE_INVALID == cs_type)) {
    ...
    int64_t start_pos = 1;
    uint32_t idx =
        ObCharset::locate(cs_type, src_str.ptr(), src_str.length(), sub_str.ptr(), sub_str.length(), start_pos);
    is_contain = (idx != 0);
  }
  return ret;
}

int ObExprSysConnectByPath::calc_sys_path(const ObExpr& expr, ObEvalCtx& ctx, ObDatum& expr_datum)
{
  int ret = OB_SUCCESS;
  UNUSED(expr);
  UNUSED(ctx);
...
int ObExprSysConnectByPath::cg_expr(ObExprCGCtx& expr_cg_ctx, const ObRawExpr& raw_expr, ObExpr& rt_expr) const
{
  int ret = OB_SUCCESS;
  UNUSED(expr_cg_ctx);
  UNUSED(raw_expr);
...
int ObExprSysConnectByPath::eval_sys_connect_by_path(
    const ObExpr& expr, ObEvalCtx& ctx, ObDatum& expr_datum, ObNLConnectByOpBase* connect_by_op)
{
  ...
  if (OB_FAIL(expr.eval_param_value(ctx, left, right))) {
    ...
    if (OB_FAIL(StringContain(val_str, separator_str, is_contain, expr.datum_meta_.cs_type_))) {
      ...
      ret = OB_ERR_INVALID_SEPARATOR;
      LOG_WARN("separator can't be part of column value", K(ret));
    } else if (OB_FAIL(ObExprConcat::calc(node_res, separator_str, val_str, &tmp_alloc))) {
      LOG_WARN("fail to concat string", K(right), K(left), K(ret));
      ...
      memcpy(res_buf + parent_path.length(), node_res.get_string().ptr(), node_res.get_string().length());
      expr_datum.set_string(res_buf, res_length);
      if (OB_FAIL(connect_by_op->set_sys_current_path(node_res.get_string(), expr_datum.get_string()))) {
        LOG_WARN("fail to set cur node path", K(result), K(ret));
      }
    }
  }
