...
public:
  ...
  bool is_param_lazy_eval() const
  {
    return param_lazy_eval_;
  }

  void set_param_idx(const uint16_t idx)
  {
    param_idx_ = idx;
  }
  void set_param_num(const uint16_t num)
  {
    param_num_ = num;
  }
  void set_param_lazy_eval()
  {
...
public:
  ...
  ExprItemArray& get_exprs()
  {
    return exprs_;
  }
  const ExprItemArray& get_exprs() const
  {
    return exprs_;
  }
  ...
  OB_INLINE int param_eval(common::ObExprCtx& expr_ctx, const common::ObObj& param) const
  {
    ...
    if (OB_ISNULL(expr_ctx.stack_) || OB_ISNULL(expr_ctx.row1_) || OB_UNLIKELY(pos < 0) ||
        OB_UNLIKELY(pos >= exprs_.count())) {
