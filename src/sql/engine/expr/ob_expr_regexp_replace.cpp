...
int ObExprRegexpReplace::cg_expr(ObExprCGCtx &op_cg_ctx, const ObRawExpr &raw_expr, ObExpr &rt_expr) const
{
  ...
  if (OB_SUCC(ret)) {
    ...
    if (OB_ISNULL(text) || OB_ISNULL(pattern)) {
      ...
      rt_expr.eval_func_ = is_support_hs ? eval_hs_regexp_replace : eval_regexp_replace;
      ...
      if (rt_expr.arg_cnt_ >= 2 && rt_expr.args_[0]->is_batch_result()) {
        ...
        if (vector_flag) {
          rt_expr.eval_vector_func_ = is_support_hs
...
int ObExprRegexpReplace::eval_hs_regexp_replace(
    const ObExpr &expr, ObEvalCtx &ctx, ObDatum &expr_datum)
{
  int ret = OB_SUCCESS;
  ObDatum *text = NULL;
    ...
    if (OB_FAIL(ObExprUtil::get_int_param_val(
      ...
      if (OB_FAIL(ret)) {
        ...
        if (OB_FAIL(ret)) {
                   ...
                   expr.args_[0]->datum_meta_.cs_type_ != CS_TYPE_UTF8MB4_GENERAL_CI) {
          res_coll_type = ObCharset::is_bin_sort(expr.args_[0]->datum_meta_.cs_type_) ? CS_TYPE_UTF8MB4_BIN : CS_TYPE_UTF8MB4_GENERAL_CI;
        ...
        if (OB_FAIL(ret)) {
            ...
            expr.args_[2]->datum_meta_.cs_type_ != CS_TYPE_UTF8MB4_GENERAL_CI)) {
          if (OB_FAIL(ObExprUtil::convert_string_collation(to_str, expr.args_[2]->datum_meta_.cs_type_, to_utf8,
            ObCharset::is_bin_sort(expr.args_[2]->datum_meta_.cs_type_) ? CS_TYPE_UTF8MB4_BIN : CS_TYPE_UTF8MB4_GENERAL_CI,
            tmp_alloc))) {
            LOG_WARN("convert charset failed", K(ret));
          }
        } else {
        ...
        if (OB_FAIL(ret)) {
        } else if (OB_FAIL(regexp_ctx->replace(tmp_alloc, text_utf8, to_utf8, pos - 1,
                                              occur, res_replace))) {
          LOG_WARN("failed to regexp replace str", K(ret));
        } else if (res_replace.empty() && lib::is_oracle_mode()) {
          expr_datum.set_null();
...
int ObExprRegexpReplace::eval_hs_regexp_replace_vector(VECTOR_EVAL_FUNC_ARG_DECL)
{
  ...
  if (OB_FAIL(expr.eval_vector_param_value(ctx, skip, bound))) {
    ...
    VectorFormat res_format = expr.get_format(ctx);
    if (VEC_DISCRETE == arg_format && VEC_DISCRETE == res_format) {
      ret = vector_hs_regexp_replace<TextDiscVec, TextDiscVec>(VECTOR_EVAL_FUNC_ARG_LIST);
    } else if (VEC_UNIFORM == arg_format && VEC_DISCRETE == res_format) {
      ret = vector_hs_regexp_replace<TextUniVec, TextDiscVec>(VECTOR_EVAL_FUNC_ARG_LIST);
    } else if (VEC_CONTINUOUS == arg_format && VEC_DISCRETE == res_format) {
      ret = vector_hs_regexp_replace<TextContVec, TextDiscVec>(VECTOR_EVAL_FUNC_ARG_LIST);
    } else if (VEC_DISCRETE == arg_format && VEC_UNIFORM == res_format) {
      ret = vector_hs_regexp_replace<TextDiscVec, TextUniVec>(VECTOR_EVAL_FUNC_ARG_LIST);
    } else if (VEC_UNIFORM == arg_format && VEC_UNIFORM == res_format) {
      ret = vector_hs_regexp_replace<TextUniVec, TextUniVec>(VECTOR_EVAL_FUNC_ARG_LIST);
    } else if (VEC_CONTINUOUS == arg_format && VEC_UNIFORM == res_format) {
      ret = vector_hs_regexp_replace<TextContVec, TextUniVec>(VECTOR_EVAL_FUNC_ARG_LIST);
    } else {
