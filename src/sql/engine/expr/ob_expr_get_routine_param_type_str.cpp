...
int ObExprGetRoutineParamTypeStr::get_subtype_to_base_type(common::ObIAllocator &alloc,
                                                           ObEvalCtx &ctx,
                                                           ...
                                                           pl::ObPLDataType &dst_pl_type)
{
  ...
  pl::ObPLResolveCtx resolve_ctx(alloc,
                                  ...
                                  *GCTX.sql_proxy_,
                                  false);
  OZ (resolve_ctx.get_user_type(dst_pl_type.get_user_type_id(), user_type, &alloc));
  ...
  if (OB_FAIL(expr.eval_param_value(ctx))) {
    ...
    if (OB_FAIL(ret)) {
      ...
      if (OB_SUCC(ret)) {
        ...
        char *type_str = static_cast<char *>(expr.get_str_res_mem(ctx, OB_MAX_SYS_PARAM_NAME_LENGTH));
        if (OB_ISNULL(type_str)) {
          ...
          int64_t type_pos = 0;
          pl::ObPLDataType dst_pl_type;
          pl::ObPLEnumSetCtx enum_set_ctx(alloc_guard.get_allocator());
          ...
          if (OB_SUCC(ret) && dst_pl_type.is_subtype()) {
            OZ (get_subtype_to_base_type(alloc_guard.get_allocator(), ctx, package_guard, dst_pl_type));
          }
