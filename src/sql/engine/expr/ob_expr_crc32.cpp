...
/// swap endianess
OB_INLINE uint32_t swap(uint32_t x)
{
...
uint32_t ObExprCrc32::crc32_16bytes(uint32_t previousCrc32, unsigned char *data, size_t length)
{

  uint32_t crc = ~previousCrc32;
  uint32_t *current = reinterpret_cast<uint32_t *>(data);

  // calculate 16 bytes at once, this value should be compatible with the size of lookup table.
  constexpr size_t BytesAtOnce = 16;
...
// both big and little endian are supported.
...
#else
    // notice that every variable below is equivalent to four bytes;
  ...
  // rest 1 to 63 bytes use standard algorithm to calculate
  while (length-- != 0) {
...
inline void vector_crc32_inner(const ObString &str_val, ResVec *res_vec, int64_t i)
{
  if (str_val.empty()) {
    res_vec->set_uint(i, 0ULL);
  } else {
    unsigned char* buf = reinterpret_cast<unsigned char*>(const_cast<char*>(str_val.ptr()));
    res_vec->set_uint(i, ObExprCrc32::crc32_16bytes(0, buf, str_val.length()));
  }
}
...
int ObExprCrc32::vector_crc32(VECTOR_EVAL_FUNC_ARG_DECL) {
  int ret = OB_SUCCESS;
  ...
  const ObDatumMeta &datum_meta = expr.args_[0]->datum_meta_;
  bool is_text_tc = ob_is_text_tc(datum_meta.type_);
  for (int64_t i = bound.start(); i < bound.end() && OB_SUCC(ret); i++) {
    if (!(skip.at(i) || eval_flags.at(i))) {
      if (arg_vec->is_null(i)) {
        res_vec->set_null(i);
      } else if (!is_text_tc){
        ObString text_val = arg_vec->get_string(i);
        vector_crc32_inner<ResVec>(text_val, res_vec, i);
      } else {
        ObString text_val = arg_vec->get_string(i);
        if (OB_FAIL(ObTextStringHelper::read_real_string_data(calc_alloc, arg_vec,
...
int ObExprCrc32::calc_crc32_expr_vector(VECTOR_EVAL_FUNC_ARG_DECL) {
  int ret = OB_SUCCESS;
  if (OB_FAIL(expr.args_[0]->eval_vector(ctx, skip, bound))) {
    LOG_WARN("fail to eval crc32 param", K(ret));
  } else {
    VectorFormat arg_format = expr.args_[0]->get_format(ctx);
    VectorFormat res_format = expr.get_format(ctx);
    if (VEC_CONTINUOUS == arg_format && VEC_FIXED == res_format) {
      ret = vector_crc32<StrContVec, UIntegerFixedVec>(VECTOR_EVAL_FUNC_ARG_LIST);
    } else if (VEC_CONTINUOUS == arg_format && VEC_UNIFORM == res_format){
      ret = vector_crc32<StrContVec, UIntegerUniVec>(VECTOR_EVAL_FUNC_ARG_LIST);
    } else if (VEC_DISCRETE == arg_format && VEC_FIXED == res_format){
      ret = vector_crc32<StrDiscVec, UIntegerFixedVec>(VECTOR_EVAL_FUNC_ARG_LIST);
    } else if (VEC_DISCRETE == arg_format && VEC_UNIFORM == res_format){
      ret = vector_crc32<StrDiscVec, UIntegerUniVec>(VECTOR_EVAL_FUNC_ARG_LIST);
    } else if (VEC_UNIFORM == arg_format && VEC_FIXED == res_format){
      ret = vector_crc32<StrUniVec, UIntegerFixedVec>(VECTOR_EVAL_FUNC_ARG_LIST);
    } else if (VEC_UNIFORM == arg_format && VEC_UNIFORM == res_format){
      ret = vector_crc32<StrUniVec, UIntegerUniVec>(VECTOR_EVAL_FUNC_ARG_LIST);
    } else {
      ret = vector_crc32<ObVectorBase, ObVectorBase>(VECTOR_EVAL_FUNC_ARG_LIST);
    }
  }
  return ret;
}
...
int ObExprCrc32::cg_expr(ObExprCGCtx& expr_cg_ctx, const ObRawExpr& raw_expr, ObExpr& rt_expr) const
{
  ...
  rt_expr.eval_vector_func_ = calc_crc32_expr_vector;
  return ret;
}


const uint32_t ObExprCrc32::Crc32Lookup[16][256] =
{
    {
