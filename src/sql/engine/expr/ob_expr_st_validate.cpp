 ...
 * This file contains implementation for st_validate.
 */
...
namespace sql
{
ObExprSTValidate::ObExprSTValidate(ObIAllocator &alloc)
    : ObFuncExprOperator(alloc, T_FUN_SYS_ST_VALIDATE, N_ST_VALIDATE, 1, VALID_FOR_GENERATED_COL, NOT_ROW_DIMENSION)
{
}

int ObExprSTValidate::calc_result_type1(ObExprResType &type,
                                        ObExprResType &type1,
...
int ObExprSTValidate::eval_st_validate(const ObExpr &expr, 
                                      ObEvalCtx &ctx, 
                                      ObDatum &res)
{
  int ret = OB_SUCCESS;
  ...
  if (OB_FAIL(expr.args_[0]->eval(ctx, datum))) {
    ...
    } else if (OB_FAIL(ObGeoExprUtils::get_srs_item(ctx, srs_guard, wkb, srs, 
                                                    true, N_ST_VALIDATE))) {
      LOG_WARN("fail to get srs item", K(ret), K(wkb));
    } else if (OB_FAIL(ObGeoExprUtils::build_geometry(tmp_allocator, wkb, geo, srs, N_ST_VALIDATE, 
    ...
    if (OB_SUCC(ret)) {
      ...
      if (OB_FAIL(gis_context.append_geo_arg(geo))) {
        ...
        ObGeoExprUtils::geo_func_error_handle(ret, N_ST_VALIDATE);
      } else if (!isvalid_res) {
        is_null_result = true;
      }
    }
  ...
  if (OB_SUCC(ret)) {
    if (is_null_result) {
      ...
      res.set_string(datum->get_string());
    }
  }
  return ret;
}

int ObExprSTValidate::cg_expr(ObExprCGCtx &expr_cg_ctx, 
                              const ObRawExpr &raw_expr, 
                              ObExpr &rt_expr) const
{
  UNUSEDx(expr_cg_ctx, raw_expr);
  rt_expr.eval_func_ = eval_st_validate;
  return OB_SUCCESS;
}
