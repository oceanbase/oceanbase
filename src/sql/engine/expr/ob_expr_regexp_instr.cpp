    ...
    if (OB_SUCC(ret)) {
      ...
      switch (param_num) {
        case 7/*subexpr*/:
          ...
          if (is_support_hs) {
            ret = OB_INVALID_ARGUMENT;
            LOG_WARN("hyperscan does not support capturing group yet.", K(is_support_hs), K(ret));
...
int ObExprRegexpInstr::cg_expr(ObExprCGCtx &op_cg_ctx, const ObRawExpr &raw_expr, ObExpr &rt_expr) const
{
  ...
  if (OB_SUCC(ret)) {
    ...
    if (OB_ISNULL(text) || OB_ISNULL(pattern)) {
      ...
      rt_expr.eval_func_ = is_support_hs ? eval_hs_regexp_instr : eval_regexp_instr;
...
int ObExprRegexpInstr::eval_hs_regexp_instr(
    const ObExpr &expr, ObEvalCtx &ctx, ObDatum &expr_datum)
{
  int ret = OB_SUCCESS;
  ObDatum *text = NULL;
    ...
    if (OB_FAIL(ObExprUtil::get_int_param_val(
      ...
      LOG_WARN("get integer parameter value failed", K(ret));
               ...
               && (pos <= 0 || occur < 0
                   ...
                   || (lib::is_oracle_mode() && occurrence != NULL && occur == 0))) {
      ret = OB_INVALID_ARGUMENT;
      LOG_WARN("regexp_instr position or occurrence or return_option is invalid",
               K(ret), K(pos), K(occur));
      LOG_USER_ERROR(OB_INVALID_ARGUMENT, "use position or occurrence or return_option in regexp_instr");
    } else if (lib::is_oracle_mode() && return_opt_val < 0) {
      ret = OB_ERR_ARGUMENT_OUT_OF_RANGE;
      LOG_WARN("regexp_instr return_option is invalid", K(ret), K(return_opt_val));
    } else {
      ...
      if (OB_FAIL(ret)) {
        ...
        if (expr.args_[0]->datum_meta_.cs_type_ != CS_TYPE_UTF8MB4_BIN &&
          ...
          if (OB_FAIL(ObExprUtil::convert_string_collation(
                  text_str, expr.args_[0]->datum_meta_.cs_type_, text_utf8,
                  ObCharset::is_bin_sort(expr.args_[0]->datum_meta_.cs_type_)
                      ? CS_TYPE_UTF8MB4_BIN
      ...
      if (OB_FAIL(ret) || is_null) {
      } else if (OB_FAIL(regexp_ctx->find(tmp_alloc,
                                          text_utf8,
                                          pos - 1,
                                          occur,
                                          return_opt_val,
                                          res_pos))) {
        LOG_WARN("failed to hs regexp find loc", K(ret));
      } else if (lib::is_mysql_mode()) {
        expr_datum.set_int(res_pos);
      } else {
        number::ObNumber nmb;
