...
//
int PartTransTask::revert_by_rollback_savepoint(const uint64_t row_index, const int32_t sql_no)
{
  int ret = OB_SUCCESS;
  const bool is_ddl = is_ddl_trans();

  if (OB_UNLIKELY(OB_INVALID_ID == row_index) || OB_UNLIKELY(sql_no < 0)) {
    LOG_ERROR("invalid argument", K(row_index), K(sql_no));
    ret = OB_INVALID_ARGUMENT;
  } else if (OB_UNLIKELY(! is_ddl)) {
    LOG_ERROR("is not ddl, unexcepted", K(*this));
    ret = OB_ERR_UNEXPECTED;
  // rollabck ddl trans
  } else if (OB_FAIL(revert_ddl_stmt_(sql_no))) {
    LOG_ERROR("revert ddl stmt by rollback savepoint failed", KR(ret), K(is_ddl), K(sql_no));
  } else {
    // succ
  }
...
int PartTransTask::revert_ddl_stmt_(const int32_t rollback_sql_no)
{
  int ret = OB_SUCCESS;
    ...
    if (OB_ISNULL(ddl_task)) {
      ...
      if (stmt_sql_no <= rollback_sql_no) {
        ...
        _LOG_DEBUG("[SAVEPOINT][DDL] ddl_stmt need not revert, schema_version=%ld,"
            "rollback_sql_no=%d, stmt_sql_no=%d", ddl_task->get_op_schema_version(),
            rollback_sql_no, stmt_sql_no);
      } else {
        if (NULL != pre_task) {
  ...
  if (OB_SUCC(ret)) {
    _LOG_INFO("[SAVEPOINT][DDL] ROLLBACK_SQL_NO=%d STMT_CNT=%ld/%ld",
        rollback_sql_no, total_stmt_num, stmt_num);
  }
