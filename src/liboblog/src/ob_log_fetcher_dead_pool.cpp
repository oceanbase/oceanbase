...
int ObLogFetcherDeadPool::push(PartFetchCtx *task)
{
  int ret = OB_SUCCESS;
  if (OB_UNLIKELY(! inited_)) {
  ...
  if (OB_ISNULL(part_fetch_mgr_)) {
    ...
    while (OB_SUCCESS == ret && NULL != task) {
      ...
      if (! task->is_in_use()) {
        ...
        else if (OB_FAIL(part_fetch_mgr_->remove_partition(pkey))) {
          LOG_ERROR("remove partition fail", KR(ret), K(pkey), K(task));
        } else {
