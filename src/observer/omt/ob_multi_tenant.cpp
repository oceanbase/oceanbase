                        ...
                        bool mtl_bind_flag)
{
  ...
  if (OB_SUCC(ret) && mtl_bind_flag) {
    ...
    MTL_BIND2(share::ObBackupDestIOPermissionMgr::mtl_new, share::ObBackupDestIOPermissionMgr::mtl_init, mtl_start_default, mtl_stop_default, mtl_wait_default, mtl_destroy_default);
...
int ObMultiTenant::create_tenant(const ObTenantMeta &meta, bool write_slog, const int64_t abs_timeout_us)
{
  ...
  if (OB_FAIL(ret) && tenant_allocator_created) {
    ...
    recycle_tenant_allocator(tenant_id);
  }
  if (lock_succ) {
    bucket_lock_.unlock(bucket_lock_idx);
  }
