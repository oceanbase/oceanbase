                  ...
                  int32_t level, bool force, ObResourceGroup *group)
{
  ...
  if (!force && tenant->total_worker_cnt() >= tenant->max_worker_cnt()) {
    ...
    worker->set_group_id(static_cast<int32_t>(group_id));
...
void ObThWorker::set_th_worker_thread_name()
{
  ...
  if (serving_tenant_id_ != tenant_->id()) {
    ...
    snprintf(buf, sizeof(buf), "L%d_G%d", get_worker_level(), get_group_id());
...
int ObThWorker::acquire_diagnostic_info(ObDiagnosticInfo *&di, rpc::ObRequest *req)
{
  ...
  if (di != nullptr) {
    ...
    const bool skip_worker_rpc_di = ObDiagnosticInfoContainer::get_di_experimental_feature_flag().skip_worker_rpc_di();
    if (OB_UNLIKELY(skip_worker_rpc_di)) {
      // skip worker rpc di creation
    } else {
