...
ObInfoSchemaPartitionsTable::ObInfoSchemaPartitionsTable() : ObVirtualTableScannerIterator(), tenant_id_(OB_INVALID_ID)
{}
        ...
        for (int64_t i = 0; OB_SUCC(ret) && i < database_schemas.count(); ++i) {
          ...
          if (OB_ISNULL(database_schema)) {
            ...
            SERVER_LOG(WARN, "database not exist", K(ret));
                     ...
                     ObSchemaUtils::is_public_database(
                         database_schema->get_database_name_str(), lib::is_oracle_mode())) {
            continue;
...
int ObInfoSchemaPartitionsTable::add_partitions(
...
{
  int ret = OB_SUCCESS;
  ObSEArray<int64_t, 4> all_part_ids;
  ...
  } else if (OB_FAIL(table_schema.get_physical_partition_ids(all_part_ids))) {
    SERVER_LOG(WARN, "fail to get all partition ids", K(ret));
  }
  int64_t table_id = table_schema.get_table_id();
  for (int64_t i = 0; OB_SUCC(ret) && i < all_part_ids.count(); ++i) {
    int64_t cell_idx = 0;
    const int64_t partition_id = all_part_ids.at(i);
    int64_t part_id = partition_id;
    if (is_twopart(partition_id)) {
      part_id = extract_part_idx(partition_id);
    }
      ...
      switch (col_id) {
        ...
        case PARTITION_NAME: {
          if (PARTITION_LEVEL_ZERO == part_level) {
            ...
            if (OB_FAIL(table_schema.get_partition_by_part_id(part_id, check_dropped_partition, partition))) {
              SERVER_LOG(WARN, "fail to get partition by part_id", K(ret), K(table_id), K(part_id), K(partition_id));
            } else if (OB_ISNULL(partition)) {
              ret = OB_ENTRY_NOT_EXIST;
              SERVER_LOG(WARN, "partition not exist", K(ret), K(table_id), K(part_id), K(partition_id));
            } else {
              cells[cell_idx].set_varchar(partition->get_part_name());
              cells[cell_idx].set_collation_type(ObCharset::get_default_collation(ObCharset::get_default_charset()));
            }
          }
          break;
        ...
        case SUBPARTITION_NAME: {
          if (PARTITION_LEVEL_ZERO == part_level || PARTITION_LEVEL_ONE == part_level) {
            ...
            char* name_buf = static_cast<char*>(allocator_->alloc(OB_MAX_PARTITION_NAME_LENGTH));
            int64_t name_size = 0;
            if (OB_ISNULL(name_buf)) {
              ret = OB_ALLOCATE_MEMORY_FAILED;
              SERVER_LOG(WARN, "name_buf is null", K(ret));
            } else {
              MEMSET(name_buf, 0, OB_MAX_PARTITION_NAME_LENGTH);
              if (OB_FAIL(table_schema.get_partition_name(
                      partition_id, name_buf, OB_MAX_PARTITION_NAME_LENGTH, &name_size))) {
                SERVER_LOG(WARN, "fail to get partition name", K(ret), K(partition_id));
              } else {
                ObString subpartition_name(name_size, name_buf);
                cells[cell_idx].set_varchar(subpartition_name);
                cells[cell_idx].set_collation_type(ObCharset::get_default_collation(ObCharset::get_default_charset()));
              }
            }
          }
        ...
        case PARTITION_ORDINAL_POSITION: {
          // TODO replace by position in __all_meta_table
          cells[cell_idx].set_uint64(static_cast<uint64_t>(i));
          break;
        }
        case SUBPARTITION_ORDINAL_POSITION: {
          // TODO no subpartition
        ...
        case PARTITION_METHOD: {
          ...
          switch (func_type) {
            case PARTITION_FUNC_TYPE_HASH_V2:
            case PARTITION_FUNC_TYPE_HASH: {
              cells[cell_idx].set_varchar("HASH");
              cells[cell_idx].set_collation_type(ObCharset::get_default_collation(ObCharset::get_default_charset()));
              break;
            }
            case PARTITION_FUNC_TYPE_KEY_V3:
            case PARTITION_FUNC_TYPE_KEY_V2:
            case PARTITION_FUNC_TYPE_KEY:
            case PARTITION_FUNC_TYPE_KEY_IMPLICIT:
            case PARTITION_FUNC_TYPE_KEY_IMPLICIT_V2: {
        ...
        case SUBPARTITION_METHOD: {
          if (ObPartitionLevel::PARTITION_LEVEL_TWO == table_schema.get_part_level()) {
        ...
        case PARTITION_DESCRIPTION: {
          // TODO no partition description now
          ObString desc;
          if (table_schema.is_range_part()) {
            if (OB_FAIL(gen_high_bound_val_str(table_schema, part_id, desc))) {
              SERVER_LOG(WARN, "fail to generate PARTITION_DESCRIPTION", K(ret), K(all_part_ids[i]));
            } else {
              cells[cell_idx].set_varchar(desc);
              cells[cell_idx].set_collation_type(ObCharset::get_default_collation(ObCharset::get_default_charset()));
            }
          } else if (table_schema.is_list_part()) {
            if (OB_FAIL(gen_list_bound_val_str(table_schema, part_id, desc))) {
              SERVER_LOG(WARN, "fail to generate PARTITION_DESCRIPTION", K(ret), K(all_part_ids[i]));
            } else {
              cells[cell_idx].set_varchar(desc);
              cells[cell_idx].set_collation_type(ObCharset::get_default_collation(ObCharset::get_default_charset()));
        ...
        case TABLESPACE_NAME: {
          cells[cell_idx].set_varchar(table_schema.get_tablegroup_name());
          cells[cell_idx].set_collation_type(ObCharset::get_default_collation(ObCharset::get_default_charset()));
          break;
        }
        default: {
    ...
    const share::schema::ObTableSchema& table_schema, const int64_t part_id, common::ObString& val_str)
{
  ...
  bool check_dropped_partition = false;
  if (OB_FAIL(table_schema.get_partition_by_part_id(part_id, check_dropped_partition, part))) {
    SERVER_LOG(WARN, "fail to get partition", K(ret), K(part_id));
  } else if (OB_ISNULL(part)) {
    ret = OB_INVALID_ARGUMENT;
    SERVER_LOG(WARN, "partition is null", K(ret), K(part_id));
  } else {
    ...
    if (OB_ISNULL(high_bound_val = static_cast<char*>(allocator_->alloc(OB_MAX_B_HIGH_BOUND_VAL_LENGTH)))) {
      ...
      } else if (OB_FAIL(ObPartitionUtils::convert_rowkey_to_sql_literal(part->get_high_bound_val(),
                     high_bound_val,
    ...
    if (OB_ISNULL(list_val = static_cast<char*>(allocator_->alloc(OB_MAX_B_HIGH_BOUND_VAL_LENGTH)))) {
      ...
      if (OB_FAIL(OTTZ_MGR.get_tenant_tz(tenant_id_, tz_info.get_tz_map_wrap()))) {
        SERVER_LOG(WARN, "get tenant timezone map failed", K(ret), K(tenant_id_));
                     ...
                     part->get_list_row_values(), list_val, OB_MAX_B_PARTITION_EXPR_LENGTH, pos, false, &tz_info))) {
        SERVER_LOG(WARN, "Failed to convert rowkey to sql text", K(tz_info), K(ret));
      } else {
