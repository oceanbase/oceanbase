...
{
  // we don't care about transfer_scn, just using prefix select.
  // if needed, SQL layer will filter the result.
  int ret = OB_SUCCESS;
...
int ObAllVirtualSSTabletMeta::fill_in_rows_(const ObArray<VirtualTabletMetaRow> &row_datas)
{
  int ret = OB_SUCCESS;
  ...
  for (int64_t row_i = 0; OB_SUCC(ret) && row_i < row_count; ++row_i) {
    ...
    for (int64_t col_i = 0; OB_SUCC(ret) && col_i < col_count; ++col_i) {
      ...
      case TABLET_ID:
        ...
        break;
      case TRANSFER_SCN:
        cells[col_i].set_int(row_data.reorganization_scn_.get_val_for_inner_table_field());
        break;
      case META_VERSION:
...
int ObAllVirtualSSTabletMeta::extract_result_(
    ...
    VirtualTabletMetaRow &row)
{
  ...
  if (OB_FAIL(ret)) {
    ...
    row.reorganization_scn_.convert_for_sql(reorganization_scn);
...
int ObAllVirtualSSTabletMeta::get_virtual_rows_remote_(
    common::sqlclient::ObMySQLResult &res,
  ...
  if (OB_UNLIKELY(OB_INVALID_TENANT_ID == tenant_id)) {
    ...
    SMART_VAR(ObISQLClient::ReadResult, result) {
      ...
      if (OB_FAIL(sql.assign_fmt("SELECT * FROM %s WHERE tenant_id=%lu and tablet_id=%lu and ls_id=%lu",
                                 OB_ALL_VIRTUAL_SS_TABLET_META_TNAME,
...
int ObAllVirtualSSTabletMeta::generate_virtual_rows_(ObArray<VirtualTabletMetaRow> &row_datas)
{
  int ret = OB_SUCCESS;
  if (is_sys_tenant(effective_tenant_id_) && effective_tenant_id_ != tenant_id_) {
    if (OB_FAIL(get_virtual_rows_remote_(tenant_id_, ls_id_, tablet_id_, row_datas))) {
    ...
    MTL_SWITCH(tenant_id_)
    {
      ...
      if (OB_FAIL(meta_service->get_max_committed_meta_scn(ls_id_, read_snapshot))) {
        ...
        SERVER_LOG(WARN, "param is invalid", KR(ret), K(param));
      } else if (OB_FAIL(meta_service->get_tablet_iter(param, read_snapshot, tablet_iter_guard))) {
        ...
        while (OB_SUCC(ret)) {
          ...
          share::SCN reorganization_scn;
          if (OB_FAIL(tablet_iter->get_next(allocator, tablet_handle, row_scn,
                                            reorganization_scn, update_meta_info, extra_info))) {
            ...
            ret = OB_INVALID_ARGUMENT;
            SERVER_LOG(WARN, "tablet handle is invalid", KR(ret), K(tablet_handle));
          } else {
            const ObTabletMeta &meta = tablet_handle.get_obj()->get_tablet_meta();
            VirtualTabletMetaRow row;
            row.reorganization_scn_ = reorganization_scn;
            ...
            if (OB_FAIL(row_datas.push_back(row))) {
              SERVER_LOG(WARN, "push_back failed.", K(row_datas), K(row));
            }
