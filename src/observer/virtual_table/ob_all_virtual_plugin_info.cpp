/**
 * Copyright (c) 2021 OceanBase
 * SPDX-License-Identifier: Apache-2.0
 */

#define USING_LOG_PREFIX SERVER

#include "lib/allocator/ob_allocator.h"
#include "observer/virtual_table/ob_all_virtual_plugin_info.h"
#include "observer/ob_server.h"
#include "plugin/legacy/sys/ob_plugin_handle.h"
#include "plugin/legacy/sys/ob_plugin_dl_handle.h"
#include "plugin/legacy/sys/ob_plugin_entry_handle.h"
#include "plugin/legacy/sys/ob_plugin_utils.h"
#include "plugin/legacy/sys/ob_plugin_mgr.h"
#include "plugin/legacy/sys/ob_plugin_helper.h"
#include "plugin/legacy/adaptor/ob_plugin_adaptor.h"

using namespace oceanbase::plugin;
using namespace oceanbase::common;

namespace oceanbase {
namespace observer {

// Column ids mirror the schema definition order (OB_APP_MIN_COLUMN_ID + 0..N)
// in ob_inner_table_schema.12501_12550.cpp. These MUST stay in lockstep with
// the generated schema — the bulk loader (ob_load_inner_table_schema.cpp) is
// regenerated from that function by test_dump_inner_table_schema, so this
// order is what the runtime schema service serves.
enum class ObPluginInfoColumn : int64_t {
  SVR_IP = common::OB_APP_MIN_COLUMN_ID,
  SVR_PORT,
  NAME,
  STATUS,
  TYPE,
  LIBRARY,
  LIBRARY_VERSION,
  LIBRARY_REVISION,
  INTERFACE_VERSION,
  AUTHOR,
  LICENSE,
  DESCRIPTION
};

ObAllVirtualPluginInfo::ObAllVirtualPluginInfo() {}

ObAllVirtualPluginInfo::~ObAllVirtualPluginInfo() { reset(); }

void ObAllVirtualPluginInfo::reset() {
  addr_.reset();
  plugin_entries_.reset();
  ext_plugin_statuses_.reset();
  iter_index_ = -1;
  ext_iter_index_ = -1;
}

int ObAllVirtualPluginInfo::inner_open()
{
  int ret = OB_SUCCESS;
  ObMemAttr mem_attr(MTL_ID(), "Plugin");

  int tmp_ret = plugin::ObPluginHelper::discover_external_sub_plugins();
  if (OB_SUCCESS != tmp_ret) {
    SERVER_LOG(WARN, "failed to discover external sub plugins, continue listing", K(tmp_ret));
  }

  if (FALSE_IT(plugin_entries_.set_attr(mem_attr))) {
  } else if (OB_FAIL(GCTX.plugin_mgr_->list_all_plugin_entries(plugin_entries_))) {
    SERVER_LOG(WARN, "failed to init tenant plugin array");
  } else {
    iter_index_ = 0;
  }

  // Snapshot the external-table format plugins registered in
  // ObExtFormatRegistry (read-only: get_plugin_statuses() runs the registry's
  // own one-shot lazy load from `ext_plugin_config` and copies the per-entry
  // outcomes). A failure here only drops the ext rows, so log and continue.
  if (OB_FAIL(ret)) {
  } else {
    if (OB_FAIL(share::ObExtFormatRegistry::get_instance().get_plugin_statuses(ext_plugin_statuses_))) {
      SERVER_LOG(WARN, "failed to get ext plugin statuses from registry, skip ext plugin rows", K(ret));
      ret = OB_SUCCESS;
      ext_plugin_statuses_.reset();
    }
    ext_iter_index_ = 0;
  }

  return ret;
}

int ObAllVirtualPluginInfo::inner_close() {
  int ret = OB_SUCCESS;
  plugin_entries_.reset();
  ext_plugin_statuses_.reset();
  iter_index_ = -1;
  ext_iter_index_ = -1;
  return ret;
}

const char *ObAllVirtualPluginInfo::load_status_to_str(share::ObExtPluginLoadStatus status)
{
  switch (status) {
    case share::EXT_PLUGIN_NOT_ATTEMPTED:   return "NOT_ATTEMPTED";
    case share::EXT_PLUGIN_SUCCESS:         return "SUCCESS";
    case share::EXT_PLUGIN_ALREADY_LOADED:  return "ALREADY_LOADED";
    case share::EXT_PLUGIN_NOT_FOUND:       return "NOT_FOUND";
    case share::EXT_PLUGIN_DLOPEN_FAILED:   return "DLOPEN_FAILED";
    case share::EXT_PLUGIN_NO_SYMBOL:       return "NO_SYMBOL";
    case share::EXT_PLUGIN_ABI_MISMATCH:    return "ABI_MISMATCH";
    case share::EXT_PLUGIN_NO_FORMAT_NAME:  return "NO_FORMAT_NAME";
    case share::EXT_PLUGIN_INVALID_ARG:     return "INVALID_ARG";
    default:                                return "UNKNOWN";
  }
}

int ObAllVirtualPluginInfo::inner_get_next_row(ObNewRow *&row)
{
  int ret = OB_SUCCESS;
  ObPluginEntryHandle *entry_handle = nullptr;
  const share::ObExtPluginStatus *ext_status = nullptr;
  if (iter_index_ >= 0 && iter_index_ < plugin_entries_.count()) {
    if (OB_FAIL(plugin_entries_.at(iter_index_, entry_handle))) {
      LOG_WARN("failed to get plugin entry handle from plugin entries",
               K(iter_index_), K(ret));
    } else if (OB_ISNULL(entry_handle)) {
      ret = OB_ERR_UNEXPECTED;
      SERVER_LOG(WARN, "iterate plugin handle success but got null",
                 K(ret), K(iter_index_), K(plugin_entries_.count()));
    }
    iter_index_++;
  } else if (ext_iter_index_ >= 0 && ext_iter_index_ < ext_plugin_statuses_.count()) {
    ext_status = &ext_plugin_statuses_.at(ext_iter_index_);
    ext_iter_index_++;
  } else {
    ret = OB_ITER_END;
  }

  if (OB_FAIL(ret)) {
  } else if (nullptr != entry_handle) {
    ret = fill_legacy_row_(entry_handle);
  } else if (nullptr != ext_status) {
    ret = fill_ext_plugin_row_(*ext_status);
  } else {
    ret = OB_ERR_UNEXPECTED;
    SERVER_LOG(WARN, "neither legacy plugin entry nor ext plugin status", K(ret));
  }

  if (OB_SUCC(ret)) {
    row = &cur_row_;
  }

  return ret;
}

int ObAllVirtualPluginInfo::fill_legacy_row_(ObPluginEntryHandle *entry_handle)
{
  int ret = OB_SUCCESS;
  const int64_t version_length = 30;
  const ObPluginEntry &plugin_entry = entry_handle->entry();
  ObPluginHandle *plugin_handle = plugin_entry.plugin_handle;
  ObObj *cells = cur_row_.cells_;
  const int64_t col_count = output_column_ids_.count();
  for (int64_t i = 0; OB_SUCC(ret) && i < col_count; ++i) {
    const ObPluginInfoColumn col_id = static_cast<ObPluginInfoColumn>(output_column_ids_.at(i));
    switch (col_id) {
      case ObPluginInfoColumn::SVR_IP: {
        if (addr_.ip_to_string(ip_buf_, sizeof(ip_buf_))) {
          cells[i].set_varchar(ip_buf_);
          cells[i].set_collation_type(ObCharset::get_default_collation(ObCharset::get_default_charset()));
        } else {
          ret = OB_ERR_UNEXPECTED;
          SERVER_LOG(WARN, "fail to execute ip_to_string", K(ret));
        }
      } break;

      case ObPluginInfoColumn::SVR_PORT: {
        cells[i].set_int(addr_.get_port());
      } break;

      case ObPluginInfoColumn::NAME: {
        cells[i].set_varchar(plugin_entry.name);
        cells[i].set_collation_type(ObCharset::get_default_collation(ObCharset::get_default_charset()));
      } break;

      case ObPluginInfoColumn::STATUS: {
        if (OB_NOT_NULL(entry_handle)) {
          cells[i].set_varchar(ob_plugin_state_to_string(entry_handle->state()));
          cells[i].set_collation_type(ObCharset::get_default_collation(ObCharset::get_default_charset()));
        } else {
          cells[i].set_null();
        }
      } break;

      case ObPluginInfoColumn::TYPE: {
        cells[i].set_varchar(ob_plugin_type_to_string(plugin_entry.interface_type));
        cells[i].set_collation_type(ObCharset::get_default_collation(ObCharset::get_default_charset()));
      } break;

      case ObPluginInfoColumn::LIBRARY: {
        ObPluginDlHandle *dl_handle = nullptr;
        if (OB_ISNULL(plugin_handle) ||
            OB_ISNULL(dl_handle = plugin_handle->dl_handle())) {
          cells[i].set_null();
        } else {
          ObString dl_name = dl_handle->name();
          cells[i].set_varchar(dl_name);
          cells[i].set_collation_type(ObCharset::get_default_collation(ObCharset::get_default_charset()));
        }
      } break;

      case ObPluginInfoColumn::LIBRARY_VERSION: {
        char *version_buffer = nullptr;
        if (OB_ISNULL(plugin_handle) || OB_ISNULL(plugin_handle->plugin()) || OB_ISNULL(allocator_) ||
            OB_ISNULL(version_buffer = static_cast<char *>(allocator_->alloc(version_length)))) {
          cells[i].set_null();
        } else {
          ObPluginVersionAdaptor plugin_version(plugin_handle->plugin()->version);
          plugin_version.to_string(version_buffer, version_length);

          cells[i].set_varchar(version_buffer);
          cells[i].set_collation_type(ObCharset::get_default_collation(ObCharset::get_default_charset()));
        }
      } break;

      case ObPluginInfoColumn::LIBRARY_REVISION: {
        if (OB_ISNULL(plugin_handle) || OB_ISNULL(plugin_handle->build_revision())) {
          cells[i].set_null();
        } else {
          cells[i].set_varchar(plugin_handle->build_revision());
          cells[i].set_collation_type(ObCharset::get_default_collation(ObCharset::get_default_charset()));
        }
      } break;

      case ObPluginInfoColumn::INTERFACE_VERSION: {
        void *descriptor = nullptr;
        char *version_buffer = nullptr;
        uint64_t version = 0;
        if (OB_ISNULL(allocator_) ||
            OB_ISNULL(version_buffer = static_cast<char *>(allocator_->alloc(version_length)))) {
          cells[i].set_null();
        } else {
          ObPluginVersionAdaptor plugin_version(plugin_entry.interface_version);
          plugin_version.to_string(version_buffer, version_length);

          cells[i].set_varchar(version_buffer);
          cells[i].set_collation_type(ObCharset::get_default_collation(ObCharset::get_default_charset()));
        }
      } break;

      case ObPluginInfoColumn::AUTHOR: {
        if (OB_NOT_NULL(plugin_handle) &&
            OB_NOT_NULL(plugin_handle->plugin()) &&
            OB_NOT_NULL(plugin_handle->plugin()->author)) {
          cells[i].set_varchar(plugin_handle->plugin()->author);
          cells[i].set_collation_type(ObCharset::get_default_collation(ObCharset::get_default_charset()));
        } else {
          cells[i].set_null();
        }
      } break;

      case ObPluginInfoColumn::LICENSE: {
        if (OB_NOT_NULL(plugin_handle) &&
            OB_NOT_NULL(plugin_handle->plugin()) &&
            OB_NOT_NULL(plugin_handle->plugin()->license)) {
          cells[i].set_varchar(plugin_handle->plugin()->license);
          cells[i].set_collation_type(ObCharset::get_default_collation(ObCharset::get_default_charset()));
        } else {
          cells[i].set_null();
        }
      } break;

      case ObPluginInfoColumn::DESCRIPTION: {
        if (OB_NOT_NULL(plugin_entry.description)) {
          cells[i].set_varchar(plugin_entry.description);
          cells[i].set_collation_type(ObCharset::get_default_collation(ObCharset::get_default_charset()));
        } else {
          cells[i].set_null();
        }
      } break;

      default: {
        SERVER_LOG(WARN, "unknown cell id", K(i));
        cells[i].set_null();
      }
    }
  }
  return ret;
}

int ObAllVirtualPluginInfo::fill_ext_plugin_row_(const share::ObExtPluginStatus &status)
{
  int ret = OB_SUCCESS;
  ObObj *cells = cur_row_.cells_;
  const int64_t col_count = output_column_ids_.count();
  const ObCollationType default_collation =
      ObCharset::get_default_collation(ObCharset::get_default_charset());
  for (int64_t i = 0; OB_SUCC(ret) && i < col_count; ++i) {
    const ObPluginInfoColumn col_id =
        static_cast<ObPluginInfoColumn>(output_column_ids_.at(i));
    switch (col_id) {
      case ObPluginInfoColumn::SVR_IP: {
        if (addr_.ip_to_string(ip_buf_, sizeof(ip_buf_))) {
          cells[i].set_varchar(ip_buf_);
          cells[i].set_collation_type(default_collation);
        } else {
          ret = OB_ERR_UNEXPECTED;
          SERVER_LOG(WARN, "fail to execute ip_to_string", K(ret));
        }
      } break;

      case ObPluginInfoColumn::SVR_PORT: {
        cells[i].set_int(addr_.get_port());
      } break;

      case ObPluginInfoColumn::NAME: {
        cells[i].set_varchar(ObString(status.plugin_name_));
        cells[i].set_collation_type(default_collation);
      } break;

      case ObPluginInfoColumn::STATUS: {
        cells[i].set_varchar(load_status_to_str(status.status_));
        cells[i].set_collation_type(default_collation);
      } break;

      case ObPluginInfoColumn::TYPE: {
        cells[i].set_varchar(ob_plugin_type_to_string(OBP_PLUGIN_TYPE_EXTERNAL));
        cells[i].set_collation_type(default_collation);
      } break;

      case ObPluginInfoColumn::LIBRARY: {
        // Resident rows: the absolute path actually dlopen'd; failed rows: the
        // intended soname/path the loader tried.
        cells[i].set_varchar(ObString(status.lib_path_));
        cells[i].set_collation_type(default_collation);
      } break;

      case ObPluginInfoColumn::LIBRARY_VERSION: {
        // The plugin's self-reported version (api->plugin_version()); empty for
        // failed loads (no api to query) -> NULL.
        if ('\0' != status.plugin_version_[0]) {
          cells[i].set_varchar(ObString(status.plugin_version_));
          cells[i].set_collation_type(default_collation);
        } else {
          cells[i].set_null();
        }
      } break;

      case ObPluginInfoColumn::DESCRIPTION: {
        // Reused as the load-failure reason for external-table plugins; empty
        // for resident rows -> NULL.
        if ('\0' != status.error_msg_[0]) {
          cells[i].set_varchar(ObString(status.error_msg_));
          cells[i].set_collation_type(default_collation);
        } else {
          cells[i].set_null();
        }
      } break;

      // legacy plugin only columns: NULL for external-table plugin rows
      case ObPluginInfoColumn::LIBRARY_REVISION:
      case ObPluginInfoColumn::INTERFACE_VERSION:
      case ObPluginInfoColumn::AUTHOR:
      case ObPluginInfoColumn::LICENSE: {
        cells[i].set_null();
      } break;

      default: {
        SERVER_LOG(WARN, "unknown cell id", K(i));
        cells[i].set_null();
      }
    }
  }
  return ret;
}

} // namespace observer
} // namespace oceanbase

#undef USING_LOG_PREFIX
