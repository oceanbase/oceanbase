        ...
        if (tenant_id_array_idx_ >= tenant_id_array_.count() || tenant_id_array_idx_ < 0) {
          ...
          if (nullptr == cur_mysql_req_mgr_) {
            ...
            if (OB_FAIL(extract_request_ids(t_id, start_id_, end_id_, is_req_valid))) {
              ...
              SERVER_LOG(DEBUG,
                  "start to get rows from inner table",
                  K(start_id_),
                  K(end_id_),
                  K(cur_id_),
                  K(t_id),
...
int ObVirtualSqlPlanMonitor::extract_request_ids(
    const uint64_t tenant_id, int64_t& start_id, int64_t& end_id, bool& is_valid)
{
  int ret = OB_SUCCESS;
  is_valid = true;
...
int ObVirtualSqlPlanMonitor::convert_node_to_row(ObMonitorNode& node, ObNewRow*& row)
{
  ...
  for (int64_t cell_idx = 0; OB_SUCC(ret) && cell_idx < output_column_ids_.count(); ++cell_idx) {
    ...
    switch (column_id) {
      ...
      case TRACE_ID: {
        const uint64_t* trace_id_p = node.get_trace_id();
