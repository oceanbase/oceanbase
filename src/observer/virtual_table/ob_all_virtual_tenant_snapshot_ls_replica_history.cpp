...
ObAllVirtualTenantSnapshotLSReplicaHistory::ObAllVirtualTenantSnapshotLSReplicaHistory():
...
void ObAllVirtualTenantSnapshotLSReplicaHistory::reset()
{
...
int ObAllVirtualTenantSnapshotLSReplicaHistory::inner_open()
{
  int ret = OB_SUCCESS;
...
int ObAllVirtualTenantSnapshotLSReplicaHistory::inner_get_next_row(ObNewRow *&row)
{
  int ret = OB_SUCCESS;
...
bool ObAllVirtualTenantSnapshotLSReplicaHistory::is_need_process(uint64_t tenant_id)
{
  return is_sys_tenant(effective_tenant_id_) || tenant_id == effective_tenant_id_;
}

int ObAllVirtualTenantSnapshotLSReplicaHistory::process_curr_tenant(ObNewRow *&row)
{
  int ret = OB_SUCCESS;
...
int ObAllVirtualTenantSnapshotLSReplicaHistory::get_tenant_snapshot_ls_replica_entries_()
{
  int ret = OB_SUCCESS;
...
void ObAllVirtualTenantSnapshotLSReplicaHistory::release_last_tenant()
{
  if (OB_NOT_NULL(sql_res_)) {
    sql_res_->reset();
  }
