...
ObAllPlanCacheBase::ObAllPlanCacheBase()
    : ObVirtualTableIterator(), pcm_(NULL), tenant_id_array_(), tenant_id_array_idx_(0)
{}
  ...
  for (int64_t i = 0; OB_SUCC(ret) && i < col_count; ++i) {
    ...
    switch (col_id) {
      ...
      case SVR_PORT: {
        ...
        // sql_num
      case SQL_NUM: {
        cells[i].set_int(plan_cache.count());
        break;
      }
        // mem_used
      case MEM_USED: {
      ...
      case PLAN_NUM: {  // id->plan_stat map size;
        cells[i].set_int(plan_cache.get_plan_num());
        break;
      }
        // mem_limit
      case MEM_LIMIT: {
    ...
    if (OB_SUCC(ret)) {
      ...
      if (OB_ISNULL(start_key_obj_ptr) || OB_ISNULL(end_key_obj_ptr)) {
        ...
        if (!(ObIntType == start_key_obj_ptr[0].get_type())) {
          ...
          ret = OB_ERR_UNEXPECTED;
          SERVER_LOG(WARN, "assert start_key_obj_ptr[0].get_type() == end_key_obj_ptr[0].get_type()", K(ret));
        } else {
          tenant_id = start_key_obj_ptr[0].get_int();
          if (!(tenant_id >= 0)) {
            ret = OB_ERR_UNEXPECTED;
            SERVER_LOG(WARN, "assert tenant_id >= 0", K(ret));
          } else if (OB_FAIL(add_var_to_array_no_dup(tenant_ids_, static_cast<uint64_t>(tenant_id)))) {
