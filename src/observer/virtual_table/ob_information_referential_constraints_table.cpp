    ...
    if (OB_FAIL(schema_guard_->get_database_schemas_in_tenant(tenant_id_, database_schemas))) {
      ...
      for (int64_t i = 0; OB_SUCC(ret) && i < database_schemas.count(); ++i) {
        ...
        if (OB_ISNULL(database_schema)) {
          ...
          continue;
        } else if (OB_FAIL(add_fk_constraints_in_db(*database_schema, cells, output_column_ids_.count()))) {
          SERVER_LOG(WARN, "failed to add fk constraint of database schema", K(ret));
        }
      }
      if (OB_SUCC(ret)) {
...
int ObInfoSchemaReferentialConstraintsTable::add_fk_constraints_in_table(
    const share::schema::ObTableSchema& table_schema, const common::ObString& database_name, common::ObObj* cells,
    const int64_t col_count)
{
  ...
  for (int64_t i = 0; OB_SUCC(ret) && i < foreign_key_infos.count(); i++) {
    ...
    } else if (OB_FAIL(schema_guard_->get_table_schema(foreign_key_info.parent_table_id_, parent_tbl_schema))) {
    ...
    } else if (OB_FAIL(schema_guard_->get_database_schema(parent_tbl_schema->get_database_id(), parent_db_schema))) {
        ...
        switch (col_id) {
          ...
          case UNIQUE_CONSTRAINT_NAME: {
            if (CONSTRAINT_TYPE_PRIMARY_KEY == foreign_key_info.ref_cst_type_) {
              ...
              if (OB_FAIL(schema_guard_->get_table_schema(foreign_key_info.ref_cst_id_, uk_index_schema))) {
                SERVER_LOG(WARN, "get uk_index_schema failed", K(ret));
                break;
              ...
              ret = OB_ERR_UNEXPECTED;
              SERVER_LOG(WARN, "foreign_key_info.ref_cst_type_ is invalid type", K(foreign_key_info.ref_cst_type_));
              break;
            }
            break;
          }
          ...
          case REFERENCED_TABLE_NAME: {
            cells[cell_idx].set_varchar(parent_tbl_schema->get_table_name_str());
            cells[cell_idx].set_collation_type(ObCharset::get_default_collation(ObCharset::get_default_charset()));
            break;
          }
          default: {
