...
int ObTenantVirtualOutlineBase::inner_open()
{
  ...
  if (OB_UNLIKELY(NULL == schema_guard_ || OB_INVALID_TENANT_ID == tenant_id_)) {
    ret = OB_NOT_INIT;
      ...
      switch (col_id) {
        ...
        case OUTLINE_TARGET: {
          ...
          if (outline_info->get_sql_text_str().empty()) {
            ...
            cells[cell_idx].set_collation_type(ObCharset::get_default_collation(ObCharset::get_default_charset()));
          } else if (OB_FAIL(ob_write_string(*allocator_, outline_info->get_outline_target_str(), outline_target))) {
            LOG_WARN(
                "fail to deep copy obstring", K(ret), K(outline_info->get_outline_target_str()), K(outline_target));
          } else {
            cells[cell_idx].set_lob_value(
                ObLongTextType, outline_target.ptr(), static_cast<int32_t>(outline_target.length()));
            cells[cell_idx].set_collation_type(ObCharset::get_default_collation(ObCharset::get_default_charset()));
          }
          break;
        }
        case OUTLINE_SQL: {
          ...
          if (outline_info->get_sql_text_str().empty()) {
            ...
            cells[cell_idx].set_lob_value(
                ObLongTextType, outline_sql.ptr(), static_cast<int32_t>(outline_sql.length()));
            cells[cell_idx].set_collation_type(ObCharset::get_default_collation(ObCharset::get_default_charset()));
          }
          break;
        }
