...
ObMySQLDBTable::ObMySQLDBTable() : ObVirtualTableScannerIterator(), tenant_id_(OB_INVALID_ID)
{}
        ...
        if (OB_FAIL(schema_guard_->get_db_priv_with_tenant_id(tenant_id_, db_array))) {
          ...
          for (int64_t row_idx = 0; OB_SUCC(ret) && row_idx < db_array.count(); ++row_idx) {
            ...
            if ((ObString(ObString(OB_ORA_SYS_SCHEMA_NAME)) == db_priv->get_database_name_str()) ||
              ...
              // oracle mode doesn't need to show database
              continue;
            }
...
int ObMySQLDBTable::get_user_info(const uint64_t user_id, const share::schema::ObUserInfo*& user_info)
{
  int ret = OB_SUCCESS;
  user_info = NULL;
