...
int ObTenantAllTables::get_sequence_value()
{
  ...
  int64_t autoinc_version = OB_INVALID_ID;
  AutoincKey key;
    ...
    if (OB_ISNULL(table_schema)) {
      ...
      if (table_schema->is_order_auto_increment_mode()) {
        if (OB_FAIL(order_autokeys.push_back(key))) {
          SERVER_LOG(WARN, "failed to push back AutoincKey", KR(ret));
        ...
        if (OB_FAIL(noorder_autokeys.push_back(key))) {
          SERVER_LOG(WARN, "failed to push back AutoincKey is order", KR(ret));
