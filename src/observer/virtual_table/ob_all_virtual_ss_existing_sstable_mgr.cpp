...
ObAllVirtualSSExistingSSTableMgr::ObAllVirtualSSExistingSSTableMgr()
...
void ObAllVirtualSSExistingSSTableMgr::reset()
{
  tenant_id_ = 0;
  ls_id_.reset();
  tablet_id_.reset();
...
int ObAllVirtualSSExistingSSTableMgr::inner_get_next_row(common::ObNewRow *&row)
{
  int ret = OB_SUCCESS;
...
#ifdef OB_BUILD_SHARED_STORAGE
int ObAllVirtualSSExistingSSTableMgr::get_primary_key_()
{
  int ret = OB_SUCCESS;
...
int ObAllVirtualSSExistingSSTableMgr::check_rowkey_same_(ObNewRange &key_range)
{
  int ret = OB_SUCCESS;
...
int ObAllVirtualSSExistingSSTableMgr::get_first_key_(ObNewRange &key_range)
{
  int ret = OB_SUCCESS;
  if (OB_UNLIKELY(key_range.get_start_key().get_obj_cnt() < ROWKEY_COL_COUNT || key_range.get_end_key().get_obj_cnt() < ROWKEY_COL_COUNT)) {
    ret = OB_ERR_UNEXPECTED;
...
int ObAllVirtualSSExistingSSTableMgr::get_next_tablet_()
{
  int ret = OB_SUCCESS;
  ...
  if (tablet_ == nullptr) {
    ...
    } else if (OB_FAIL(meta_service->get_tablet(param, range, tablet_iter_guard_))) {
...
int ObAllVirtualSSExistingSSTableMgr::get_next_table_(ObITable *&table)
{
  int ret = OB_SUCCESS;
  table = nullptr;
...
int ObAllVirtualSSExistingSSTableMgr::fill_in_row_(const VirtualSSSSTableRow &row_data, common::ObNewRow *&row)
{
  int ret = OB_SUCCESS;
