...
ObTableLoadDataToDeleteIndexTableChannel::ObTableLoadDataToDeleteIndexTableChannel(
...
int ObTableLoadDataToDeleteIndexTableChannel::create_row_projector()
{
  int ret = OB_SUCCESS;
...
int ObTableLoadDataToDeleteIndexTableChannel::handle_insert_row(
  const ObTabletID &tablet_id, const ObDirectLoadDatumRow &datum_row)
{
  return OB_SUCCESS;
}

int ObTableLoadDataToDeleteIndexTableChannel::handle_insert_row(const ObTabletID &tablet_id,
                                                                const ObDatumRow &datum_row)
{
  return OB_SUCCESS;
}

int ObTableLoadDataToDeleteIndexTableChannel::handle_insert_batch(
  const ObTabletID &tablet_id, const ObBatchDatumRows &datum_rows)
{
  return OB_SUCCESS;
}

int ObTableLoadDataToDeleteIndexTableChannel::handle_delete_row(
  const ObTabletID &tablet_id, const ObDirectLoadDatumRow &datum_row)
{
  int ret = OB_SUCCESS;
  if (IS_NOT_INIT) {
    ret = OB_NOT_INIT;
    LOG_WARN("ObTableLoadDataToDeleteIndexTableChannel not init", KR(ret), KP(this));
    ...
    if (OB_FAIL(table_builder_mgr_.get_table_builder(table_builder))) {
      ...
      if (OB_FAIL(
            row_projector_->projector(tablet_id, datum_row, index_tablet_id, delete_datum_row))) {
        LOG_WARN("fail to project row", KR(ret));
      } else if (OB_FAIL(table_builder->append_row(index_tablet_id, delete_datum_row))) {
...
int ObTableLoadDataToDeleteIndexTableChannel::handle_update_row(
  const ObTabletID &tablet_id, const ObDirectLoadDatumRow &datum_row)
{
  return OB_SUCCESS;
}
  ...
  const ObDirectLoadDatumRow &new_row, const ObDirectLoadDatumRow *result_row)
{
  ...
  if (IS_NOT_INIT) {
    ...
    if (OB_FAIL(table_builder_mgr_.get_table_builder(table_builder))) {
      ...
      ObTabletID index_tablet_id;
      if (OB_FAIL(
            row_projector_->projector(tablet_id, old_row, index_tablet_id, delete_datum_row))) {
...
int ObTableLoadDataToDeleteIndexTableChannel::handle_insert_delete_conflict(
  const ObTabletID &tablet_id, const ObDirectLoadDatumRow &datum_row)
{
  return OB_SUCCESS;
}
