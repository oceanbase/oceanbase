...
int ObTableLoadClientTask::get_column_idxs(ObIArray<int64_t> &column_idxs)
{
  int ret = OB_SUCCESS;
...
int ObTableLoadClientTask::init_instance()
{
  ...
  if (IS_NOT_INIT) {
    ...
    const uint64_t tenant_id = param_.get_tenant_id();
    const uint64_t table_id = param_.get_table_id();
    const int64_t parallel = param_.get_parallel();
    ...
    if (OB_FAIL(GCTX.omt_->get_tenant(tenant_id, tenant))) {
      ...
      ret = OB_ERR_UNEXPECTED;
      LOG_WARN("unexpected empty column idxs", KR(ret));
    } else if (OB_FAIL(get_compressor_type(tenant_id, table_id, parallel, compressor_type))) {
      ...
      load_param.parallel_ = parallel;
      load_param.session_count_ = parallel;
...
int ObTableLoadClientTask::commit_instance()
{
  ...
  if (IS_NOT_INIT) {
    ...
    } else if (OB_FAIL(instance_.commit(result_info_))) {
      LOG_WARN("fail to commit instance", KR(ret));
    }
  }
  return ret;
