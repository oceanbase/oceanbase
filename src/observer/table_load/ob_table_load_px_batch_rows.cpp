                                 ...
                                 const int64_t max_batch_size)
{
  ...
  if (IS_INIT) {
    ...
    LOG_WARN("invalid args", KR(ret), K(col_descs), KP(col_nullables), K(row_flag), K(max_batch_size));
  ...
  if (IS_NOT_INIT) {
    ...
    for (int64_t i = 0; OB_SUCC(ret) && i < get_column_count(); ++i) {
      ...
      if (!ObColumnSchemaV2::is_hidden_pk_column_id(col_desc.col_id_)) {
        ...
        if (OB_FAIL(batch_rows_.get_vectors().at(i)->append_batch(batch_idx, vector, offset, size))) {
  ...
  if (IS_NOT_INIT) {
    ...
    for (int64_t i = 0; OB_SUCC(ret) && i < get_column_count(); ++i) {
      ...
      if (!ObColumnSchemaV2::is_hidden_pk_column_id(col_desc.col_id_)) {
        ...
        if (OB_FAIL(batch_rows_.get_vectors().at(i)->append_batch(batch_idx, datum_vector, offset, size))) {
          LOG_WARN("fail to append batch", KR(ret));
        }
      }
    }
                                             ...
                                             int64_t size)
{
  ...
  if (IS_NOT_INIT) {
    ...
    for (int64_t i = 0; OB_SUCC(ret) && i < get_column_count(); ++i) {
      ...
      if (!ObColumnSchemaV2::is_hidden_pk_column_id(col_desc.col_id_)) {
        ObIVector *vector = vectors.at(i);
        if (OB_FAIL(batch_rows_.get_vectors().at(i)->append_selective(batch_idx, vector, selector, size))) {
                                             ...
                                             const uint16_t *selector, int64_t size)
{
  ...
  if (IS_NOT_INIT) {
    ...
    for (int64_t i = 0; OB_SUCC(ret) && i < get_column_count(); ++i) {
      ...
      if (!ObColumnSchemaV2::is_hidden_pk_column_id(col_desc.col_id_)) {
        const ObDatumVector &datum_vector = datum_vectors.at(i);
        if (OB_FAIL(batch_rows_.get_vectors().at(i)->append_selective(batch_idx, datum_vector, selector, size))) {
          LOG_WARN("fail to append selective", KR(ret));
        }
      }
    }
...
int ObTableLoadPXBatchRows::append_row(const ObDirectLoadDatumRow &datum_row)
{
  ...
  if (IS_NOT_INIT) {
    ...
    for (int64_t i = 0; OB_SUCC(ret) && i < get_column_count(); ++i) {
      ...
      const ObColDesc &col_desc = col_descs_.at(i);
      if (!ObColumnSchemaV2::is_hidden_pk_column_id(col_desc.col_id_)) {
        if (OB_FAIL(batch_rows_.get_vectors().at(i)->append_datum(batch_idx, datum))) {
...
int ObTableLoadPXBatchRows::shallow_copy(const IVectorPtrs &vectors, const int64_t batch_size)
{
  ...
  if (IS_NOT_INIT) {
    ...
    for (int64_t i = 0; OB_SUCC(ret) && i < get_column_count(); ++i) {
      ...
      if (!ObColumnSchemaV2::is_hidden_pk_column_id(col_desc.col_id_)) {
        ObIVector *vector = vectors.at(i);
        if (OB_FAIL(batch_rows_.get_vectors().at(i)->shallow_copy(vector, batch_size))) {
                                         ...
                                         const int64_t batch_size)
{
  ...
  if (IS_NOT_INIT) {
    ...
    for (int64_t i = 0; OB_SUCC(ret) && i < get_column_count(); ++i) {
      ...
      if (!ObColumnSchemaV2::is_hidden_pk_column_id(col_desc.col_id_)) {
        const ObDatumVector &datum_vector = datum_vectors.at(i);
        if (OB_FAIL(batch_rows_.get_vectors().at(i)->shallow_copy(datum_vector, batch_size))) {
          LOG_WARN("fail to shallow copy vector", KR(ret), K(i));
        }
      }
    }
