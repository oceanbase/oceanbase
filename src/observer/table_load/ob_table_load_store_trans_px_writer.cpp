...
int ObTableLoadStoreTransPXWriter::write_row(const ObTabletID &tablet_id, const ObDatumRow &row)
{
  ...
  if (IS_NOT_INIT) {
    ...
    batch_ctx_->datum_row_.storage_datums_ = is_heap_table_ ? row.storage_datums_ + 1 : row.storage_datums_;
    batch_ctx_->datum_row_.count_ = is_heap_table_ ? row.count_ - 1 : row.count_;
