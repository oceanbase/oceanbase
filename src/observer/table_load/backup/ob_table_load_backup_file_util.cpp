                                                ...
                                                ObIAllocator &allocator)
{
  ...
  while (OB_SUCC(ret)) {
    if (OB_FAIL(adapter.list_directories(path, storage_info, op))) {
      LOG_WARN("fail to list directories", K(ret), K(retry_count), K(path));
      if (ret == OB_OSS_ERROR) {
        retry_count++;
                                               ...
                                               int64_t &file_length)
{
  ...
  while (OB_SUCC(ret)) {
    if (OB_FAIL(adapter.get_file_length(path, storage_info, file_length))) {
      LOG_WARN("fail to get file length", K(ret), K(retry_count), K(path));
      if (ret == OB_OSS_ERROR) {
        retry_count++;
        if (retry_count <= MAX_RETRY_COUNT) {
          ret = OB_SUCCESS;
                                                ...
                                                int64_t &read_size)
{
  ...
  while (OB_SUCC(ret)) {
    if (OB_FAIL(adapter.read_single_file(path, storage_info, buf, buf_size, read_size))) {
      LOG_WARN("fail to read single file", K(ret), K(retry_count), K(path));
      if (ret == OB_OSS_ERROR) {
        retry_count++;
        if (retry_count <= MAX_RETRY_COUNT) {
          ret = OB_SUCCESS;
                                              ...
                                              int64_t &read_size)
{
  ...
  ObBackupIoAdapter adapter;
  while (OB_SUCC(ret)) {
    if (OB_FAIL(adapter.read_part_file(path, storage_info, buf, buf_size, offset, read_size))) {
