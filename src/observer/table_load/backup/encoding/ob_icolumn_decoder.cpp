  ...
  if (OB_UNLIKELY(col_ctx.micro_block_header_->row_count_ != result_bitmap.size())
    ...
    if (col_ctx.has_extend_value()) {
      ...
      for (int64_t row_id = 0;
        ...
        if (OB_SUCC(ret) && (value & static_cast<uint64_t>(1))) {
    ...
    int64_t &null_count) const
{
  ...
  for (int64_t i = 0; i < row_cap; ++i) {
    ...
    if (ctx.micro_block_header_->store_row_header_) {
    ...
    if (OB_FAIL(decode(const_cast<ObColumnDecoderCtx &>(ctx), cell, row_id, bs, row_data, row_len))) {
