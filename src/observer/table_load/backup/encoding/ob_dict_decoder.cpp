    ...
    const ObBitStream &bs, const char *data, const int64_t len) const
{
  ...
  if (OB_UNLIKELY(!is_inited())) {
    ...
    if (OB_FAIL(ret)) {
    } else if (OB_FAIL(decode(cell, ref, ctx.col_header_->length_))) {
...
int ObDictDecoder::decode(ObObj &cell, const int64_t ref, const int64_t meta_length) const
{
  int ret = OB_SUCCESS;
  ...
  if (OB_UNLIKELY(!is_inited())) {
    ...
    if (OB_SUCC(ret)) {
      if (ref >= count) {
        ...
        if (OB_FAIL(load_data_to_obj_cell(cell_data, cell_len, cell))) {
    ...
    ObBitmap &result_bitmap) const
{
  ...
  if (OB_UNLIKELY(result_bitmap.size() != col_ctx.micro_block_header_->row_count_
    ...
    if (OB_SUCC(ret) && op_type == ObWhiteFilterOperatorType::WHITE_OP_NE) {
      if (OB_FAIL(result_bitmap.bit_not())) {
        LOG_WARN("Failed to bitwise not on result_bitmap", K(ret));
    ...
    if (count > 0) {
      ...
      if (meta_header_->is_sorted_dict()) {
        ...
        case ObWhiteFilterOperatorType::WHITE_OP_GT: {
          ...
          if (fast_filter_valid) {
            if (OB_FAIL(fast_cmp_ref_and_set_res(col_ctx, bound_ref, col_data,
                ObWhiteFilterOperatorType::WHITE_OP_GE, result_bitmap))) {
        ...
        case ObWhiteFilterOperatorType::WHITE_OP_LT: {
          ...
          if (fast_filter_valid) {
            if (OB_FAIL(fast_cmp_ref_and_set_res(col_ctx, bound_ref, col_data,
                ObWhiteFilterOperatorType::WHITE_OP_LT, result_bitmap))) {
              LOG_WARN("Failed to compare reference and set result bitmap",
                  K(ret), K(bound_ref), K(op_type));
            }
          } else if (OB_FAIL(cmp_ref_and_set_res(parent, col_ctx, bound_ref, col_data,
