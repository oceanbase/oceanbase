    ...
    ObBitmap &result_bitmap) const
{
  ...
  if (OB_UNLIKELY(result_bitmap.size() != col_ctx.micro_block_header_->row_count_)) {
    ...
    if (dict_count == 0) {
      if (ObWhiteFilterOperatorType::WHITE_OP_NU == filter.get_op_type()) {
      ...
      } else if (ObWhiteFilterOperatorType::WHITE_OP_NN == filter.get_op_type()) {
        if (OB_FAIL(result_bitmap.bit_not())) {
    ...
    ObBitmap &result_bitmap) const
{
  ...
  if (OB_UNLIKELY(result_bitmap.size() != col_ctx.micro_block_header_->row_count_
    ...
    if (OB_SUCC(ret) && filter.get_op_type() == ObWhiteFilterOperatorType::WHITE_OP_NE) {
      if (OB_FAIL(result_bitmap.bit_not())) {
    ...
    ObBitmap &result_bitmap) const
{
  ...
  if (OB_UNLIKELY(result_bitmap.size() != col_ctx.micro_block_header_->row_count_
                ...
                || filter.get_op_type() != ObWhiteFilterOperatorType::WHITE_OP_BT)) {
      ret = OB_INVALID_ARGUMENT;
    ...
    ObBitmap &result_bitmap) const
{
  ...
  if (OB_UNLIKELY(result_bitmap.size() != col_ctx.micro_block_header_->row_count_
    ...
    if (dict_count > 0) {
      ...
      if (OB_FAIL(ret)) {
      } else if (found && OB_FAIL(set_res_with_bitset(parent, col_ctx, ref_bitset, result_bitmap))) {
        LOG_WARN("Failed to set result_bitmap", K(ret));
      }
    }
  }
