  ...
  if (OB_ISNULL(cell_datas) || OB_ISNULL(datums)) {
    ...
    case common::OBJ_DATUM_4BYTE_LEN_DATA: {
      for (int64_t i = 0; i < row_cap; ++i) {
        if (!datums[i].is_null()) {
          MEMCPY(
              const_cast<char *>(datums[i].ptr_),
              cell_datas[i] + sizeof(uint64_t),
              sizeof(uint32_t));
          MEMCPY(
              const_cast<char *>(datums[i].ptr_) + sizeof(uint32_t),
              cell_datas[i],
              sizeof(uint64_t));
          datums[i].len_ = sizeof(uint32_t) + sizeof(uint64_t);
        } else {
          datums[i].set_null();
        }
      }
      ...
      for (int64_t i = 0; i < row_cap; ++i) {
        if (!datums[i].is_null()) {
          MEMCPY(
              ...
              sizeof(uint16_t));
          MEMCPY(
              const_cast<char *>(datums[i].ptr_) + sizeof(uint16_t),
              cell_datas[i],
              sizeof(uint64_t));
          datums[i].len_ = sizeof(uint16_t) + sizeof(uint64_t);
        } else {
          datums[i].set_null();
        }
      }
    ...
    case common::OBJ_DATUM_8BYTE_DATA: {
      for (int64_t i = 0; i < row_cap; ++i) {
        if (!datums[i].is_null()) {
          MEMCPY(const_cast<char *>(datums[i].ptr_), cell_datas[i], sizeof(uint64_t));
          datums[i].len_ = sizeof(uint64_t);
        } else {
          datums[i].set_null();
        }
      }
    ...
    default: {
      ret = common::OB_NOT_SUPPORTED;
      STORAGE_LOG(WARN, "Unexpect datum obj mapping type for ots/intva data",
          K(ret), K(datum_type));
    }
    }
  }
...
private:
  int extend(const int64_t block_cnt)
  {
    ...
    if (cur_cnt + block_cnt > MAX_BLOCK_CNT) {
      ...
      common::ObMemAttr ma;
      ma.label_ = OB_ENCODING_LABEL_PIVOT;
