    ...
    ObBitmap &result_bitmap) const
{
  ...
  int ret = OB_SUCCESS;
  const ObWhiteFilterOperatorType op_type = filter.get_op_type();
  if (OB_UNLIKELY(!is_inited())) {
    ret = OB_NOT_INIT;
    LOG_WARN("Const decoder not inited", K(ret));
  } else if (OB_UNLIKELY(op_type >= ObWhiteFilterOperatorType::WHITE_OP_MAX)) {
    ret = OB_INVALID_ARGUMENT;
    ...
    switch (op_type) {
      case ObWhiteFilterOperatorType::WHITE_OP_NU:
      case ObWhiteFilterOperatorType::WHITE_OP_NN: {
      ...
      case ObWhiteFilterOperatorType::WHITE_OP_EQ:
      case ObWhiteFilterOperatorType::WHITE_OP_NE:
      case ObWhiteFilterOperatorType::WHITE_OP_LT:
      case ObWhiteFilterOperatorType::WHITE_OP_GE:
      case ObWhiteFilterOperatorType::WHITE_OP_GT:
      case ObWhiteFilterOperatorType::WHITE_OP_LE: {
      ...
      case ObWhiteFilterOperatorType::WHITE_OP_BT: {
      ...
      case ObWhiteFilterOperatorType::WHITE_OP_IN: {
          ...
          case ObWhiteFilterOperatorType::WHITE_OP_LE: {
            if (OB_UNLIKELY(objs.count() != 1 ||
              ...
              LOG_WARN("Invalid argument", K(ret), K(filter));
                    ...
                    ObPushdownWhiteFilterNode::WHITE_OP_TO_CMP_OP[filter.get_op_type()])) {
              if (OB_FAIL(result_bitmap.bit_not())) {
                LOG_WARN("Failed to do bitwise not on result bitmap", K(ret));
              }
            }
    ...
    ObBitmap &result_bitmap) const
{
  ...
  if (OB_UNLIKELY(result_bitmap.size() != col_ctx.micro_block_header_->row_count_)) {
    ...
    if (OB_SUCC(ret) && filter.get_op_type() == ObWhiteFilterOperatorType::WHITE_OP_NN) {
      if (OB_FAIL(result_bitmap.bit_not())) {
        LOG_WARN("Failed to flip all bits in result bitmap", K(ret));
      }
    }
    ...
    ObBitmap &result_bitmap) const
{
  ...
  if (OB_UNLIKELY(result_bitmap.size() != col_ctx.micro_block_header_->row_count_
                  ...
                  || filter.get_op_type() > ObWhiteFilterOperatorType::WHITE_OP_NE
                  || filter.null_param_contained())) {
    ret = OB_INVALID_ARGUMENT;
    ...
    if (meta_header_->const_ref_ == dict_count) {
      ...
      if (OB_FAIL(ret)) {
              ...
              ObPushdownWhiteFilterNode::WHITE_OP_TO_CMP_OP[op_type])) {
        if (OB_FAIL(result_bitmap.bit_not())) {
          LOG_WARN("Failed to flip all bits in result bitmap", K(ret));
        } else {
    ...
    ObBitmap &result_bitmap) const
{
  ...
  if (OB_UNLIKELY(result_bitmap.size() != col_ctx.micro_block_header_->row_count_
                  ...
                  || filter.get_op_type() != ObWhiteFilterOperatorType::WHITE_OP_IN)) {
