...
struct DictCmpRefAVX512Func_T<1, CMP_TYPE>
{
      ...
      sql::ObBitVector &result)
  {
    ...
    if (CMP_TYPE <= ObWhiteFilterOperatorType::WHITE_OP_LT) {
      ...
      for (int64_t row_id = row_cnt / 16 * 16; row_id < row_cnt; ++row_id) {
        if (value_cmp_t<uint8_t, ObWhiteFilterOperatorType::WHITE_OP_GE>(ref_arr[row_id], casted_dict_cnt)) {
          // null value
...
struct DictCmpRefAVX512Func_T<2, CMP_TYPE>
{
      ...
      sql::ObBitVector &result)
  {
    ...
    if (CMP_TYPE <= ObWhiteFilterOperatorType::WHITE_OP_LT) {
      ...
      for (int64_t row_id = row_cnt / 16 * 16; row_id < row_cnt; ++row_id) {
        if (value_cmp_t<uint16_t, ObWhiteFilterOperatorType::WHITE_OP_GE>(ref_arr[row_id], casted_dict_cnt)) {
          // null value
