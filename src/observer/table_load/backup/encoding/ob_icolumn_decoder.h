...
public:
  ...
  const ObMicroBlockHeaderV2 *micro_block_header_;
...
public:
  ...
  bool int_decode_unbr_; // whether cast to UNUMBER when decoding
    ...
    int64_t &row_len) const
{
  ...
  if (OB_ISNULL(row_index)) {
    ...
    if (col_ctx.micro_block_header_->store_row_header_) {
      ...
      if (col_ctx.has_extend_value()) {
        for (int64_t i = 0; i < row_cap; ++i) {
          if (datums[i].is_null()) {
            ...
            row_datas[i] += row_header_size;
            datums[i].pack_ -= static_cast<int32_t>(row_header_size);
          }
        }
      } else {
        for (int64_t i = 0; i < row_cap; ++i) {
