...
public:
  ...
  {}
  virtual ~ObPushdownAndFilterNode() {}
};
...
public:
  ...
  {}
  virtual ~ObPushdownOrFilterNode() {}
};
...
public:
  ObPushdownBlackFilterNode(common::ObIAllocator &alloc)
      ...
      tmp_expr_(nullptr)
  {}
  virtual ~ObPushdownBlackFilterNode() {}
...
public:
  sql::ExprFixedArray column_exprs_; // 列对应的表达式
  sql::ExprFixedArray filter_exprs_; // 下压filters
  // 下压临时保存的filter，如果发生merge，则所有的filter放入filter_exprs_
  // 如果没有发生merge，则将自己的tmp_expr_放入filter_exprs_中
  sql::ObExpr *tmp_expr_;
};
...
public:
  ...
  {}
  virtual ~ObPushdownWhiteFilterNode() {}
...
public:
  ...
  const ObBitmap *get_result() const { return filter_bitmap_; }
  int init_bitmap(const int64_t row_count, ObBitmap *&bitmap);
...
protected:
  int find_evaluated_datums(
      sql::ObExpr *expr,
      const common::ObIArray<sql::ObExpr*> &calc_exprs,
      common::ObIArray<sql::ObExpr*> &eval_exprs);
  int find_evaluated_datums(
      common::ObIArray<sql::ObExpr*> &src_exprs,
      const common::ObIArray<sql::ObExpr*> &calc_exprs,
      common::ObIArray<sql::ObExpr*> &eval_exprs);
  template<typename T>
...
private:
  int filter(sql::ObEvalCtx &eval_ctx, bool &filtered);
  int eval_exprs_batch(sql::ObBitVector &skip, const int64_t bsize);
...
private:
  ...
  sql::ObEvalInfo **eval_infos_;
  int32_t n_datum_eval_flags_;
  sql::ObBitVector **datum_eval_flags_;
...
public:
  ...
  int set_calc_exprs(const sql::ExprFixedArray &calc_exprs, int64_t max_batch_size)
  {
    max_batch_size_ = max_batch_size;
    return calc_exprs_.assign(calc_exprs);
  }
public:
  sql::ExprFixedArray calc_exprs_; //所有需要下压到存储层的表达式
  sql::ExprFixedArray access_exprs_;
  int64_t max_batch_size_;

  // filters push down to storage.
  sql::ExprFixedArray pushdown_filters_;

  int32_t pd_storage_flag_;
  ObPushdownFilter pd_storage_filters_;
  // used to pushdown aggregate expression now.
  sql::ExprFixedArray pd_storage_aggregate_output_;
  // used by external table
  sql::ExprFixedArray ext_file_column_exprs_;
  sql::ExprFixedArray ext_column_convert_exprs_;
  sql::ObExpr *trans_info_expr_;
};
...
{
public:
  ObPushdownOperator(sql::ObEvalCtx &eval_ctx, const ObPushdownExprSpec &expr_spec);
  ...
  OB_INLINE int filter_row_outside(const sql::ObExprPtrIArray &exprs, bool &filtered);
  // Notice:
...
// filter row for storage callback.
OB_INLINE int ObPushdownOperator::filter_row_outside(const sql::ObExprPtrIArray &exprs, bool &filtered)
{
  int ret = common::OB_SUCCESS;
  ret = sql::ObOperator::filter_row(eval_ctx_, exprs, filtered);
