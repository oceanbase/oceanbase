  ...
  if (OB_UNLIKELY(col_ctx.micro_block_header_->row_count_ != result_bitmap.size()
    ...
    LOG_WARN("Filter pushdown operator: Invalid argument", K(ret), K(col_ctx), K(row_index));
                                       ...
                                       result = ObObjCmpFuncs::compare_oper_nullsafe(
                                           ...
                                           ObPushdownWhiteFilterNode::WHITE_OP_TO_CMP_OP[
                                           filter.get_op_type()]);
                                       return OB_SUCCESS;
                                       }))) {
    ...
    ObBitmap &result_bitmap) const
{
  ...
  if (OB_UNLIKELY(col_ctx.micro_block_header_->row_count_ != result_bitmap.size()
    ...
    if (exceed_stored_value_range) {
      if (ObWhiteFilterOperatorType::WHITE_OP_EQ == op_type) {
        // All false
      } else if (ObWhiteFilterOperatorType::WHITE_OP_NE == op_type) {
        // All true
        if (OB_FAIL(result_bitmap.bit_not())) {
        ...
        if ((~(INTEGER_MASK_TABLE[type_store_size] >> 1)) & node_value) {
          ...
          if (ObWhiteFilterOperatorType::WHITE_OP_GT == op_type || ObWhiteFilterOperatorType::WHITE_OP_GE == op_type) {
            if (OB_FAIL(result_bitmap.bit_not())) {
              LOG_WARN("Failed to set result bitmap to all true", K(ret));
            }
          }
          ...
          if (ObWhiteFilterOperatorType::WHITE_OP_LT == op_type || ObWhiteFilterOperatorType::WHITE_OP_LE == op_type) {
            if (OB_FAIL(result_bitmap.bit_not())) {
              LOG_WARN("Failed to set result bitmap to all true", K(ret));
            }
          }
        ...
        if (ObWhiteFilterOperatorType::WHITE_OP_LT == op_type || ObWhiteFilterOperatorType::WHITE_OP_LE == op_type
            || ObWhiteFilterOperatorType::WHITE_OP_NE == op_type) {
          if (OB_FAIL(result_bitmap.bit_not())) {
            LOG_WARN("Failed to set result bitmap to all true", K(ret));
          }
        }
