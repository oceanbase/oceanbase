...
int ObMultiPrefixTree::HashTable::build(TreeNode &tnode, const int64_t move_step)
{
  ...
  if (OB_UNLIKELY(!created_)) {
    ...
    for (int64_t cid = 0; OB_SUCC(ret) && cid < list_size; ++cid) {
      ...
      if ((common_length + move_step) > cnode->cell_->val_len_) {
        // do nothing on this cell
        cnode = cnode->get_next();
      } else {
        str = cnode->cell_->v_.string_ + common_length;
...
int ObMultiPrefixTree::init_root_node(const ObColValues *col_values, const ObObjType type)
{
  int ret = OB_SUCCESS;
  if (OB_UNLIKELY(!is_inited_)) {
    ret = OB_NOT_INIT;
    ...
    tree_nodes_[0].reset();
    for (int64_t row_id = 0; OB_SUCC(ret) && row_id < col_values->count(); ++row_id) {
      const ObObj &cell = col_values->at(row_id);
      cell_nodes_[row_id].cell_ = &cell;
      if (type != cell.meta_.get_type()) {
        if (cell.is_null()) {
          ++null_cnt_;
          cell_nodes_[row_id].len_ = -1;
        } else if (ObActionFlag::OP_NOP == cell.get_ext()) {
          ++nope_cnt_;
          cell_nodes_[row_id].len_ = -2;
        } else {
          ret = OB_NOT_SUPPORTED;
          LOG_WARN("not supported extend object type", K(ret), K(cell), "column type", type);
        }
      } else {
...
int ObMultiPrefixTree::build_tree(const ObColValues *col_values,
    const ObObjType type, int64_t &last_prefix_length, bool &suitable,
    int64_t &prefix_count, int64_t &prefix_length)
{
  int ret = OB_SUCCESS;
  bool stop = false;
  if (OB_UNLIKELY(!is_inited_)) {
    ret = OB_NOT_INIT;
    LOG_WARN("not init", K(ret));
  } else if (OB_UNLIKELY(cnode_cnt_ != col_values->count())
      || OB_UNLIKELY(0 > last_prefix_length)) {
    ret = OB_INVALID_ARGUMENT;
    LOG_WARN("invalid argument", K(ret), K_(cnode_cnt),
        K(col_values->count()), K(last_prefix_length));
  } else {

    if (OB_UNLIKELY(init_root_node(col_values, type))) {
      LOG_WARN("failed to init root node", K(ret), KP(col_values));
    } else if (OB_UNLIKELY(try_previous_length(last_prefix_length, stop))) {
  ...
  if (OB_UNLIKELY(!is_inited_)) {
    ...
    for (int64_t tid = tnode_cnt_ -  1; OB_SUCC(ret) && 0 <= tid; --tid) {
      ...
      if (tnode.is_leaf()) {
        if (OB_UNLIKELY(0 >= tnode.cells_.get_size())) {
          ...
          tnode.prefix_ = tnode.cells_.get_first()->cell_;
          tnode.ref_ = prefix_count;
          ++prefix_count;
          total_prefix_length += tnode.prefix_->val_len_;
    ...
    for (int64_t tid = 0; OB_SUCC(ret) && tid < tnode_cnt_; ++tid) {
      ...
      if (0 < tnode.cells_.get_size() && 0 != tnode.length_) {
        const ObObj *prefix = tnode.prefix_;
        ...
        while (OB_SUCC(ret) && tnode.cells_.get_header() != cnode) {
          ...
          addition_len = prefix->v_.string_ == cnode->cell_->v_.string_ ?
              prefix->val_len_ - tnode.length_ :
              find_prefix(prefix->v_.string_ + tnode.length_, cnode->cell_->v_.string_ + tnode.length_,
              std::min(prefix->val_len_, cnode->cell_->val_len_) - tnode.length_);
