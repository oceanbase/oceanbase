 ...
 */
int ObTableLoadBackupMacroBlockReader::init(const char *buf, int64_t buf_size)
{
  int ret = OB_SUCCESS;
  if (IS_INIT) {
    ret = OB_INIT_TWICE;
...
int ObTableLoadBackupMacroBlockReader::inner_init()
{
  ...
  if (OB_ISNULL(meta_.endkey_ = static_cast<ObObj *>(allocator_.alloc(sizeof(ObObj) *
                  ObTableLoadBackupColumnMap::OB_TABLE_LOAD_PRE_ROW_MAX_COLUMNS_COUNT)))) {
    ...
    micro_index_ = reinterpret_cast<const ObTableLoadBackupMicroBlockIndex *>(buf_ + pos_ + meta_.micro_block_index_offset_);
  }
...
int ObTableLoadBackupMacroBlockReader::decompress_data(const int32_t micro_block_idx)
{
  int ret = OB_SUCCESS;
  if (IS_NOT_INIT) {
    ret = OB_NOT_INIT;
    ...
    if (OB_UNLIKELY(size <= (int64_t)sizeof(ObTableLoadBackupMicroBlockRecordHeader))) {
      ret = OB_INVALID_ARGUMENT;
      LOG_WARN("invalid args", KR(ret), K(micro_block_idx), K(size));
    } else if (OB_FAIL(ObTableLoadBackupMicroBlockRecordHeader::check_record(buf_ + pos_ + micro_offset_, size))) {
      LOG_WARN("fail to check record", KR(ret));
    } else {
      const ObTableLoadBackupMicroBlockRecordHeader *header = nullptr;
      header = reinterpret_cast<const ObTableLoadBackupMicroBlockRecordHeader*>(buf_ + pos_ + micro_offset_);
      const char *comp_buf = buf_ + pos_ + micro_offset_ + sizeof(ObTableLoadBackupMicroBlockRecordHeader);
      int64_t comp_size = size - sizeof(ObTableLoadBackupMicroBlockRecordHeader);
