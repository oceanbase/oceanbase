    ...
    int64_t subpart_idx)
{
  ...
  } else if (OB_FAIL(schema_info_.assign(schema_info))) {
...
int ObTableLoadBackupPartScanner::init_macro_block_list(const ObString &meta_path)
{
  int ret = OB_SUCCESS;
  int64_t pos = 0;
...
int ObTableLoadBackupPartScanner::locate_subpart_macro_block(int64_t subpart_count, int64_t subpart_idx)
{
  int ret = OB_SUCCESS;
  ...
  LOG_INFO("success to locate subpart macro block", K(subpart_count), K(subpart_idx), K(block_start_idx_), K(block_end_idx_));
  return ret;
}
...
int ObTableLoadBackupPartScanner::init_macro_block_scanner()
{
  int ret = OB_SUCCESS;
  int64_t read_size = 0;
  int64_t pos = 0;
  ...
  if (IS_NOT_INIT) {
    ...
    scanner_.reset();
    if (OB_FAIL(scanner_.init(buf_, MACRO_BLOCK_BUF_SIZE, &schema_info_, &column_ids_))) {
