 ...
 */
template<class T>
inline const T *ObTableLoadBackupRowReader::read()
{
...
int ObTableLoadBackupRowReader::setup_row(
    const char *buf,
...
{
  int ret = OB_SUCCESS;
  int64_t header_size = sizeof(ObTableLoadBackupRowHeaderDummy);
  if (OB_UNLIKELY(buf == nullptr || row_end_pos <= 0 || column_index_count < -1)) {
    ...
    if (column_index_count >= 0) {
      row_header_ = reinterpret_cast<const ObTableLoadBackupRowHeader*>(buf_ + pos);
      if (row_header_->get_version() > 0) {
        header_size = sizeof(ObTableLoadBackupRowHeader);
      }
...
int ObTableLoadBackupRowReader::read_text_store(const ObStoreMeta &store_meta, ObObj &obj)
{
  int ret = OB_SUCCESS;
...
int ObTableLoadBackupRowReader::read_column_no_meta(const ObObjMeta &src_meta, ObObj &obj)
{
  int ret = OB_SUCCESS;
  ...
  if (OB_UNLIKELY(obj.is_null() || obj.is_nop_value())) {
    ...
    switch (meta->type_) {
      ...
      case ObTextStoreType : {
        if (OB_FAIL(read_text_store(*meta, obj))) {
...
int ObTableLoadBackupRowReader::read_column(const ObObjMeta &src_meta, ObObj &obj)
{
  int ret = OB_SUCCESS;
  ...
  if(OB_SUCC(ret)){
    ...
    } else if (ObTextStoreType == meta->type_) {
    //not set type, return varchar
...
void ObTableLoadBackupRowReader::reset()
{
  allocator_.reset();
}

int ObTableLoadBackupRowReader::read_compact_rowkey(
