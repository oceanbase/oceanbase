    ...
    const ObColumnMap *column_map)
{
  ...
  if (IS_INIT) {
  ...
  } else if (OB_UNLIKELY(micro_block_data == nullptr || !micro_block_data->is_valid() || nullptr == column_map || !column_map->is_valid())) {
    ...
    data_end_ = buf + block_header_->row_index_offset_;
...
int ObMicroBlockReader::get_row(const int64_t idx, ObNewRow &row)
{
  ...
  if (IS_NOT_INIT) {
    ...
    ret = OB_ITER_END;
      ...
      data_begin_, index_data_[idx + 1], index_data_[idx], *column_map_, allocator_, row))) {
