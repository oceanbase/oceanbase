...
class ObBloomFilterMacroBlockHeader
{
public:
  ObBloomFilterMacroBlockHeader();
  ~ObBloomFilterMacroBlockHeader() = default;
  void reset();
  bool is_valid() const;
  TO_STRING_KV(K_(header_size),
      K_(version),
...
enum ObBackupColumnChecksumMethod
{
  CCM_UNKOWN = 0,
...
class ObMacroBlockMetaV2 final
{
public:
...
{
public:
  ObFullMacroBlockMetaEntry(ObMacroBlockMetaV2 &meta, ObMacroBlockSchemaInfo &schema);
...
class ObBackupLogicMacroBlockId
{
public:
...
public:
  ...
  ~ObBackupLogicMacroBlockId() = default;
  int64_t hash() const;
  bool operator ==(const ObBackupLogicMacroBlockId &other) const;
  bool operator !=(const ObBackupLogicMacroBlockId &other) const;
  OB_INLINE bool is_valid() const { return data_seq_ >= 0; }
