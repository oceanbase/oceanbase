...
#define DESERIALIZE_TRANS_ID(trans_id_ptr) \
  { \
    if (ObRowHeader::RHV_NO_TRANS_ID == row_header_->get_version()) { \
      LOG_DEBUG("without trans id", KR(ret), K(pos_)); \
      ...
      if (OB_FAIL(deserialize_trans_id(buf_, row_end_pos_, pos_, trans_id_ptr))) { \
        LOG_WARN("failed to deserialize trans id", KR(ret)); \
      } \
    } \
  }
    ...
    transaction::ObTransID *trans_id_ptr)
{
  ...
  if (OB_ISNULL(trans_id_ptr)) {
    ...
    LOG_DEBUG("TransID ptr is null", KR(ret), K(buf), K(row_end_pos), K(pos));
  }
  if (OB_FAIL(trans_id_ptr->deserialize(buf, row_end_pos, pos))) {
    LOG_WARN("Failed to deserialize TransID", KR(ret), K(buf), K(row_end_pos), K(pos));
  } else {
    LOG_DEBUG("Deserialize TransId success", KPC(trans_id_ptr));
  }
  return ret;
}

int ObIRowReader::read_text_store(
      ...
      if (lob_scale.is_in_row()) {
        ...
        if (OB_ISNULL(buf = allocator.alloc(sizeof(ObBackupLobData)))) {
          ...
          if (OB_FAIL(value->deserialize(buf_, row_end_pos_, pos_))) {
            ...
            obj.set_lob_value(store_type, buf_ + tmp_pos, pos_ - tmp_pos);
            obj.set_outrow();
          }
        }
      } else {
    ...
    ObNewRow &row)
{
  ...
  if (OB_FAIL(setup_row(row_buf, row_end_pos, pos, column_map.get_store_count()))) {
...
inline int ObFlatRowReader::setup_row(
    const char *buf,
    const int64_t row_end_pos,
    const int64_t pos,
    const int64_t column_index_count,
    transaction::ObTransID *trans_id_ptr/* = nullptr*/)
{
  int ret = OB_SUCCESS;
  if (OB_UNLIKELY(NULL == buf || row_end_pos <= 0 || column_index_count < -1 || pos < 0 || pos >= row_end_pos)) {
    ret = OB_INVALID_ARGUMENT;
    LOG_WARN("invalid row reader argument", KR(ret), K(OB_P(buf)), K(row_end_pos), K(column_index_count));
  } else {
    // set all position
    buf_ = buf;
    ...
    if (column_index_count >= 0) {
      if (OB_FAIL(analyze_row_header(column_index_count, trans_id_ptr))) { // analyze row header
 ...
 * */
OB_INLINE int ObFlatRowReader::analyze_row_header(
    ...
    transaction::ObTransID *trans_id_ptr)
{
  ...
  if (OB_UNLIKELY(
    ...
    LOG_DEBUG("success to deserialize trans id", KR(ret), KPC(trans_id_ptr), KPC(row_header_));
  }
  return ret;
}
    ...
    ObNewRow &row)
{
  ...
  if (OB_UNLIKELY(
    ...
    ret = OB_INVALID_ARGUMENT;
    LOG_WARN("Invalid argument, ", KR(ret), K(column_map.get_store_count()),
        K(column_map.get_request_count()), K(column_map.get_column_indexs()), K(row));
  } else if (OB_UNLIKELY(
      NULL == row_header_ ||
    ...
    for ( ; OB_SUCC(ret) && i < column_cnt; ++i) { // loop the ColumnIndex array
      if (column_idx[i].store_index_ < 0) { // not exists
        ...
        if (cur_read_idx == column_idx[i].store_index_) { //sequence read at pos
          if (OB_FAIL(read_obj(column_idx[i].request_column_type_, allocator, row.cells_[i]))) {
            LOG_WARN("Fail to read column, ", KR(ret), K(i), K_(pos), K(column_idx[i]));
            for (int j = 0; j < i; ++j) {
              LOG_WARN("Fail to read column", KR(ret), K(j), K(row.cells_[j]), K(column_idx[j]));
            }
          } else {
            cur_read_idx++;
          }
          ...
          if (2 == column_index_bytes) { // get offset
            column_index_offset =
                reinterpret_cast<const int16_t *>(store_column_indexs_)[column_idx[i].store_index_];
          } else if (1 == column_index_bytes) {
            column_index_offset =
                reinterpret_cast<const int8_t *>(store_column_indexs_)[column_idx[i].store_index_];
          } else if (4 == column_index_bytes) {
            column_index_offset =
                reinterpret_cast<const int32_t *>(store_column_indexs_)[column_idx[i].store_index_];
          } else {
            ret = OB_INVALID_DATA;
