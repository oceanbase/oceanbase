 ...
 */
ObBloomFilterMacroBlockHeader::ObBloomFilterMacroBlockHeader()
{
  reset();
}
void ObBloomFilterMacroBlockHeader::reset()
{
  MEMSET(this, 0, sizeof(ObBloomFilterMacroBlockHeader));
}

bool ObBloomFilterMacroBlockHeader::is_valid() const {
  return header_size_ > 0 &&
...
ObMacroBlockMetaV2::ObMacroBlockMetaV2()
  : attr_(0),
    ...
    partition_id_(-1),
    column_checksum_method_(ObBackupColumnChecksumMethod::CCM_UNKOWN),
...
int ObFullMacroBlockMetaEntry::deserialize(const char *buf, const int64_t buf_len, int64_t &pos)
{
  ...
  if (nullptr == buf || buf_len <= 0) {
    ...
    LOG_WARN("deserialization header_version error.", KR(ret), K(header_version), K(buffer_reader.capacity()), K(buffer_reader.pos()));
  } else if (OB_UNLIKELY(ObMacroBlockMetaV2::MACRO_BLOCK_META_VERSION_V2 != header_version)) {
 ...
 * ObBackupMacroData
 */
ObBackupMacroData::ObBackupMacroData(
    blocksstable::ObBufferHolder &meta,
...
int ObRecordHeaderV3::serialize(char *buf, const int64_t buf_len, int64_t &pos) const
{
  ...
  if (OB_UNLIKELY(nullptr == buf || buf_len <= 0)) {
    ...
    buf += pos_orig;
    pos = 0;
    if (OB_UNLIKELY(serialize_size + pos_orig > buf_len)) {
      ret = OB_BUF_NOT_ENOUGH;
      LOG_WARN("buffer not enough", KR(ret), K(serialize_size), K(buf_len), K(pos_orig));
    } else {
