...
static const ObMemAttr tl_decode_attr(common::OB_SERVER_TENANT_ID, "TLD_DecoderCtx", common::ObSubCtxIds::THREAD_LOCAL_DECODE_CTX_ID);
...
ObMicroBlockDecoder::ObMicroBlockDecoder()
  : allocator_(nullptr),
    buf_allocator_("TLD_MBDB_V_3_X"),
    cast_allocator_("TLD_MBDC_V_3_X"),
    decoder_allocator_("TLD_MBDD_V_3_X"),
    column_map_(nullptr),
    request_cnt_(0),
    store_cnt_(0),
    rowkey_cnt_(0),
  ...
  if (IS_INIT) {
    ...
    if (OB_FAIL(get_micro_metas(
      ...
      for (int64_t i = 0; !need_cast_ && i < request_cnt_; ++i) {
        if (!column_map_->get_column_indexs()[i].is_column_type_matched_ &&
            column_map_->get_column_indexs()[i].store_index_ >= 0) {
          need_cast_ = true;
        }
      }
      ...
      if (OB_SUCC(ret)) {
        if (OB_FAIL(init_decoders())) {
          ...
          for (int64_t i = 0; OB_SUCC(ret) && i < request_cnt_; i++) {
            const int64_t idx = column_map->get_column_indexs()[i].store_index_;
...
int ObMicroBlockDecoder::init_decoders()
{
  ...
  if (OB_UNLIKELY(nullptr == block_header_ || nullptr == col_header_)) {
    ...
    int64_t store_idx = -1;
    for (int64_t i = 0; OB_SUCC(ret) && i < request_cnt_; ++i) {
      store_idx = column_map_->get_column_indexs()[i].store_index_;
      obj_meta = column_map_->get_column_indexs()[i].macro_column_type_;
    ...
    common::ObObj *objs)
{
  ...
  for (int64_t i = col_begin; need_cast_ && OB_SUCC(ret) && i < col_end; ++i) {
    ...
    const ObColumnIndexItem &column_index = column_map_->get_column_indexs()[i];
    if (!column_index.is_column_type_matched_ && column_index.store_index_ >= 0) {
      // types don't match, need cast obj
      if (OB_FAIL(storage::cast_obj(column_index.request_column_type_, cast_allocator_, objs[i]))) {
        LOG_WARN("failed to cast obj", KR(ret), K(i), K(objs[i]), K(column_index.request_column_type_));
      }
    }
  }
  return ret;
