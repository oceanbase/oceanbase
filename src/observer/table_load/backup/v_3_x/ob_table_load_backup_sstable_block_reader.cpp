...
ObTableLoadBackupSSTableBlockReader::ObTableLoadBackupSSTableBlockReader()
  : allocator_("TLD_BSBR_V_3_X"),
    buf_allocator_("TLD_BSBRB_V_3_X"),
    buf_(nullptr),
    buf_size_(0),
    pos_(0),
...
int ObTableLoadBackupSSTableBlockReader::inner_init()
{
  ...
  } else if (OB_FAIL(init_full_macro_block_meta_entry())) {
    LOG_WARN("fail to init full_macro_block_meta_entry", KR(ret));
  } else {
...
int ObTableLoadBackupSSTableBlockReader::init_backup_common_header()
{
  ...
  if (OB_ISNULL(backup_common_header_)) {
    ...
    LOG_WARN("buf not enough", KR(ret),  K(*backup_common_header_), K(buf_size_), K(pos_));
  } else if (OB_FAIL(backup_common_header_->check_data_checksum(buf_ + pos_, backup_common_header_->data_zlength_))) {
    LOG_WARN("failed to check data checksum", KR(ret),  K(*backup_common_header_));
  }
  return ret;
}

int ObTableLoadBackupSSTableBlockReader::init_full_macro_block_meta_entry()
{
  int ret = OB_SUCCESS;
  ObBufferReader macro_block_meta_entry_reader;
  ObBufferReader macro_block_reader;
  ObBackupMacroData backup_macro_data(macro_block_meta_entry_reader, macro_block_reader);
  ObMacroBlockMetaV2 macro_block_meta;
  ObMacroBlockSchemaInfo macro_block_schema_info;
  ObFullMacroBlockMetaEntry full_macro_block_meta_entry(macro_block_meta, macro_block_schema_info);
  if (OB_FAIL(backup_macro_data.deserialize(buf_, buf_size_, pos_))) {
    LOG_WARN("fail to deserialize", KR(ret));
  } else if (OB_ISNULL(macro_block_meta.endkey_ = reinterpret_cast<ObObj*>(allocator_.alloc(sizeof(ObObj) * OB_MAX_ROWKEY_COLUMN_NUMBER)))) {
    ret = OB_ALLOCATE_MEMORY_FAILED;
    LOG_WARN("fail to alloc memory for macro block end key", KR(ret));
  } else if (OB_FAIL(macro_block_meta_entry_reader.read_serialize(full_macro_block_meta_entry))) {
    LOG_WARN("fail to read serialize", KR(ret), K(macro_block_meta_entry_reader));
  } else if (OB_UNLIKELY(!full_macro_block_meta_entry.is_valid())) {
    ret = OB_ERR_UNEXPECTED;
    LOG_WARN("full_macro_block_meta_entry is invalid", KR(ret), K(full_macro_block_meta_entry));
  } else {
...
int ObTableLoadBackupSSTableBlockReader::inner_init_data_block()
{
  ...
  if (OB_UNLIKELY(!sstable_macro_block_header_->is_valid())) {
    ...
    if (OB_SUCC(ret)) {
      if (OB_FAIL(col_map_.init(
          allocator_,
          0,/*schema version*/
          sstable_macro_block_header_->rowkey_column_count_,
