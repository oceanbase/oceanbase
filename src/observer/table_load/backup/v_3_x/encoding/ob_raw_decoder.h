...
public:
  ...
  ObRawDecoder() : store_class_(ObExtendSC),
                   integer_mask_(0), meta_data_(NULL)
  ...
  static int locate_cell_data(const char *&cell_data, int64_t &cell_len,
      ...
      const ObMicroBlockHeaderV2 &micro_block_header_, const ObColumnHeader &col_header,
      const Header &header);
        ...
        for (int64_t i = 0; OB_SUCC(ret) && i < row_cap; ++i) {
          if (col_header.has_extend_value() && datums[i].is_null()) {
            ...
            if (OB_FAIL(gen.init(var_data - col_idx_byte, col_idx_byte))) {
              ...
              datums[i].pack_ = datums[i].pack_ - offset - (var_data - datas[i]);
