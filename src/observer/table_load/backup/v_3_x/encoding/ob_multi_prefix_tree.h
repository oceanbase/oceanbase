...
public:
  ...
  struct CellNode
  {
    ...
    TO_STRING_KV(K_(*cell), K_(len), K_(ref));
  ...
  struct TreeNode
  {
    ...
    inline const common::ObObj *get_prefix() const { return prefix_; }
...
public:
  ...
  int build_tree(const ObColValues *col_values,
      const common::ObObjType type, int64_t &last_prefix_length,
      bool &suitable, int64_t &prefix_count, int64_t &prefix_length);
  void reuse();
...
private:
  int init_root_node(const ObColValues *col_values, const common::ObObjType type);
