    ...
    if (ctx.is_bit_packing()) {
      ...
      if (ctx.is_fix_length()) {
        ...
        if (OB_FAIL(locate_cell_data(cell_data, cell_len, data, len,
          ...
          LOG_WARN("locate cell data failed", K(ret), K(len), K(ctx), K(*ctx.col_header_));
        }
      }
      // fill data
      if (OB_FAIL(ret)) {
      } else if (OB_FAIL(load_data_to_obj_cell(cell_data, cell_len, cell))) {
...
bool ObRawDecoder::fast_decode_valid(const ObColumnDecoderCtx &ctx) const
{
  ...
  const ObMicroBlockHeaderV2 *block_header = ctx.micro_block_header_;
  const ObObjTypeStoreClass store_class = get_store_class_map()[ctx.obj_meta_.get_type_class()];
  if (col_header->is_fix_length()) {
    ...
    if (valid && (ObIntSC == store_class || ObUIntSC == store_class)) {
      uint16_t store_size = col_header->length_;
    ...
    valid = !col_header->is_bit_packing()
        && !col_header->has_extend_value()
        && !block_header->store_row_header_
        && row_idx_byte_valid
  ...
  if (col_ctx.is_bit_packing()) {
    ...
    for (int64_t row_id = 0;
      ...
      if (nullptr != parent && parent->can_skip_filter(row_id)) {
        ...
        if (OB_FAIL(ret)) {
        } else if (OB_FAIL(load_data_to_obj_cell(cell_data, cell_len, cur_obj))) {
    ...
    ObObj &load_obj) const
{
  ...
  if (OB_ISNULL(cell_data)) {
    ...
    { // json and text storage class have the same behavior currently
      load_obj.set_scale(static_cast<ObScale>(*reinterpret_cast<const int8_t *>(cell_data)));
      load_obj.val_len_ = static_cast<int32_t>(cell_len - sizeof(int8_t));
      load_obj.v_.string_ = cell_data + sizeof(int8_t);
      break;
    }
    case ObOTimestampSC: {
      if (OB_FAIL(load_obj.read_otimestamp(cell_data, load_obj.get_otimestamp_store_size()))) {
        LOG_WARN("Failed to read otimestamp", K(ret), K(load_obj), K(cell_data));
      }
      break;
    }
    case ObIntervalSC: {
      if (OB_FAIL(load_obj.read_interval(cell_data))) {
        LOG_WARN("Failed to read interval", K(ret), K(load_obj), K(cell_data));
      }
      break;
    }
    default: {
