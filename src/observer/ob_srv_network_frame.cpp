...
#endif
      if (OB_SUCC(ret) && GET_MIN_CLUSTER_VERSION() < CLUSTER_VERSION_4_2_5_2) {
        if (use_sm) {
          ret = OB_NOT_SUPPORTED;
          LOG_WARN("CLUSTER_VERSION < 4.2.5.2", KR(ret));
          LOG_USER_ERROR(OB_NOT_SUPPORTED, "CLUSTER_VERSION < 4.2.5.2, national encryption not supported");
        }
      }
      if (OB_SUCC(ret)) {
        if (!use_bkmi) {
