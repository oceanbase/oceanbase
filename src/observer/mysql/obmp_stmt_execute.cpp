        ...
        for (uint64_t i = 0; OB_SUCC(ret) && i < params_num_; i++) {
          if (OB_FAIL(piece_cache->remove_piece(
            ...
            if (OB_HASH_NOT_EXIST == ret) {
              ret = OB_SUCCESS;
              LOG_INFO("piece hash not exist", K(ret), K(session.get_sessid()), K(stmt_id_), K(i));
            } else {
              need_disconnect = true;
              LOG_WARN("remove piece fail", K(ret), K(session.get_sessid()),
                       K(need_disconnect), K(stmt_id_), K(i));
            }
          }
        }
      } else {
        LOG_DEBUG("piece_cache_ is null", K(ret), K(session.get_sessid()));
      }
      ret = upper_scope_ret;
    }
