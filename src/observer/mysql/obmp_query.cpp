                                        ...
                                        stmt::StmtType &stmt_type)
{
  ...
  LOG_TRACE("check is trans ctrl cmd ", K(sql), K(is_trans_ctrl_cmd), K(stmt_type));
#endif
}
                                               ...
                                               stmt::StmtType stmt_type)
{
  ...
  if (enable_sql_audit) {
    ...
    audit_record.is_multi_stmt_ = ctx_.multi_stmt_item_.is_part_of_multi_stmt();
...
#ifdef ENABLE_DEBUG_LOG
    LOG_INFO("start trans finish ", KR(ret), K(read_only), K(need_disconnect), K(tx_param));
#endif
...
#ifdef ENABLE_DEBUG_LOG
    LOG_INFO("end trans finish ", KR(ret), K(need_trans_cb), K(need_disconnect), K(is_async_end_trans), K(async_resp_used));
#endif
  }
  return ret;
}
