...
int ObMPResetConnection::process()
{
  ...
  if (OB_FAIL(get_session(session))) {
    ...
    ObMaxWaitGuard max_wait_guard(NULL);
    ObTotalWaitGuard total_wait_guard(NULL);
    ...
    */
    ...
    if (OB_SUCC(ret)) {
      ...
      if (OB_FAIL(raw_owner_id.convert_from_client_sessid(session->get_sid(), session->get_client_create_time()))) {
