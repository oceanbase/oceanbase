...
int ObService::backup_completing_log(const obrpc::ObBackupComplLogArg &arg)
{
  ...
  if (!arg.is_valid() || OB_ISNULL(sql_proxy)) {
    ...
    LOG_WARN("failed to get backup dest", KR(ret), K(arg));
      ...
      job_desc, backup_dest, tenant_id, backup_set_desc, ls_id, start_scn, end_scn, is_only_calc_stat, turn_id, retry_id))) {
    ...
    MTL_SWITCH(arg.tenant_id_) {
      ...
      if (OB_ISNULL(ls_service)) {
        ...
        for (int64_t i = 0; OB_SUCC(ret) && i < arg.tablet_ids_.count(); i++) {
          ...
          } else if (OB_FAIL(res.tablet_sizes_.push_back(tablet_handle.get_obj()->get_tablet_meta().space_usage_.all_sstable_required_size_))) {
            LOG_WARN("failed to push back", K(ret));
          }
        }
      }
