...
void ObTableGroupCommitMgr::ObTableGroupUpdateTask::update_ops_task()
{
  int ret = OB_SUCCESS;
  ...
  if (ObTableGroupUtils::is_group_commit_config_enable()) {
    ...
    if (ops.get_ops() >= DEFAULT_ENABLE_GROUP_COMMIT_OPS &&
    ...
    } else if (ops.get_ops() < DEFAULT_ENABLE_GROUP_COMMIT_OPS &&
              !TABLEAPI_GROUP_COMMIT_MGR->is_group_commit_disable()) {
      TABLEAPI_GROUP_COMMIT_MGR->set_group_commit_disable(true);
      LOG_INFO("disable group commit");
    }
    LOG_DEBUG("group commit debug:", K(ops));
