                                                      ...
                                                      ObDASScanCtDef &das_tsc_ctdef)
{
  ...
  if (ctx.is_index_back() && output_count > 0) {
    ...
    if (output_count > 0) {
      ObPushdownFilterUtils::set_aggregate_pushdown_storage(das_tsc_ctdef.pd_expr_spec_.pd_storage_flag_);
...
{
  int ret = OB_SUCCESS;
  const bool pd_agg = ObPushdownFilterUtils::is_aggregate_pushdown_storage(das_tsc_ctdef.pd_expr_spec_.pd_storage_flag_);
  ...
  } else if (pd_agg && OB_FAIL(das_tsc_ctdef.table_param_.convert_agg(das_tsc_ctdef.access_column_ids_,
                                                                      das_tsc_ctdef.aggregate_column_ids_))) {
    LOG_WARN("fail to convert agg", K(ret), K(*table_schema), K(das_tsc_ctdef));
