...
int64_t ObTableTTLDeleteTask::get_timeout_ts()
{
  if (timeout_ts_ == 0) {
    timeout_ts_ = ONE_TASK_TIMEOUT + ObTimeUtility::current_time();
  }
  return timeout_ts_;
}
...
{
  int ret = OB_SUCCESS;
  info_.start_time_ = ObTimeUtility::current_time();
  if (IS_NOT_INIT) {
    ret = OB_NOT_INIT;
    LOG_WARN("not init", K(ret));
  } else if (OB_FAIL(delete_rate_throttler_.init(info_))) {
    LOG_WARN("fail to init delete rate throttler", KR(ret), K(delete_rate_throttler_));
  } else {
...
int ObTableTTLDeleteTask::process_one()
{
  ...
  SMART_VAR(ObTableCtx, scan_ctx, allocator_) {
    ...
    } else if (FALSE_IT(delete_rate_throttler_.record_trans_start_time())) {
  ...
  if (OB_NOT_NULL(trans_param.trans_state_ptr_)
    ...
    } else if (OB_FAIL(delete_rate_throttler_.check_and_try_throttle())) {
      LOG_WARN("fail to check and try throttle", KR(ret));
    }
  ...
  while (OB_SUCC(ret)) {
    ...
    if (OB_FAIL(ttl_row_iter.get_next_row(row))) {
      ...
      if (OB_FAIL(construct_delete_entity(ttl_row_iter.column_pairs_, *row, delete_entity_))) {
        ...
        if (OB_TMP_FAIL(delete_rate_throttler_.check_and_try_throttle(get_timeout_ts()))) {
          if (OB_ITER_END != tmp_ret) {
            LOG_WARN("fail to limit delete rate", KR(tmp_ret));
          }
        }
      }
      if (REACH_TIME_INTERVAL(30 * 1000 * 1000)) { // 30s
        delete_rate_throttler_.print_throttle_info();
      }
