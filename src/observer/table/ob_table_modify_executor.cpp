                                                       ...
                                                       bool &is_row_changed)
{
  ...
  if (tb_ctx_.is_inc_or_append()) {
    ...
    FOREACH_CNT_X(info, upd_ctdef.assign_columns_, OB_SUCC(ret) && !is_row_changed)
    {
      ...
      if (OB_ISNULL(old_row.at(idx)) || OB_ISNULL(new_row.at(idx))) {
        ret = OB_ERR_UNEXPECTED;
        LOG_WARN("old row or new row expr is NULL", KP(old_row.at(idx)), KP(new_row.at(idx)), K(idx));
      } else if (OB_FAIL(old_row.at(idx)->eval(eval_ctx, old_datum)) || OB_FAIL(new_row.at(idx)->eval(eval_ctx, new_datum))) {
