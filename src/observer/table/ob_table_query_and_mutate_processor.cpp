  ...
  if (ObTableEntityType::ET_HKV == arg_.entity_type_) {
    ...
    if (htable_filter.is_valid()) {
      ...
      if (key_ranges.count() != 1) {
        ...
        } else {} // we have to scan additional rows to get result with multi-column
      }
    } else {
      ret = OB_INVALID_ARGUMENT;
...
int ObTableQueryAndMutateP::try_process()
{
  ...
  } else if (FALSE_IT(query_ctx_.param_tablet_id() = tablet_ids.at(0))) {
  } else if (OB_FAIL(get_ls_id(tablet_ids.at(0), query_ctx_.param_ls_id()))) {
    LOG_WARN("failed to get ls id", K(ret));
  } else if (OB_FAIL(start_trans(is_readonly, sql::stmt::T_UPDATE, consistency_level, table_id, query_ctx_.param_ls_id(), get_timeout_ts()))) {
    ...
    if (OB_ITER_END == ret || OB_SUCC(ret)) {
      ...
      switch(mutation.type()) {
        ...
        case ObTableOperationType::INCREMENT:         // Increment
          ...
          if (one_result->get_row_count() > 0) { // not empty result means check passed
            affected_rows = 1;
            SMART_VAR(ObHTableIncrementExecutor, inc_executor, ObTableOperationType::INCREMENT,
                                                   allocator_,
                                                   table_id,
                                                   tablet_ids.at(0),
                                                   query_ctx_.param_ls_id(),
        ...
        case ObTableOperationType::APPEND:             // Append
          ...
          if (one_result->get_row_count() > 0) { // not empty result means check passed
            affected_rows = 1;
            SMART_VAR(ObHTableIncrementExecutor, apd_executor, ObTableOperationType::APPEND,
                                                   allocator_,
                                                   table_id,
                                                   tablet_ids.at(0),
                                                   query_ctx_.param_ls_id(),
  ...
  query_ctx_.destroy_result_iterator(access_service_);
