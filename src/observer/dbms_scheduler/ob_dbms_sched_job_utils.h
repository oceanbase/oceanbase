...
#define DATA_VERSION_SUPPORT_EXECUTE_SESSION_ID(data_version) (DATA_VERSION_4_4_2_0 <= data_version)
...
public:
  ...
  */
  ...
  // Returns true only when the string can be parsed as a valid server address.
  // Unparseable values are treated as zone names by the caller.
  ...
  // Unified entry replacing zone_check_impl at the SET_ATTRIBUTE / RUN_JOB sites.
