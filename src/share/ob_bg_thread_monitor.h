...
public:
  ...
  void add_size(const int64_t size)
  {
    pos_ += (size);
  }
  void sub_size(const int64_t size)
  {
    pos_ -= (size);
  }
...
struct MonitorEntry {
  MonitorEntry();
  ~MonitorEntry();
  void set(const int64_t start_ts, const int64_t warn_ts, const MonitorCallbackWrapper& callback);
  void reset();
