...
public:
  ...
  inline void set_scan_param(const ObVTableScanParam* scan_param)
  {
  ...
  virtual inline void set_session(sql::ObSQLSessionInfo* session)
  {
    session_ = session;
  }
  virtual inline void set_convert_flag()
  {
    need_convert_ = true;
  }
  ...
  virtual const common::ObIArray<common::ObNewRange>& get_key_ranges()
  {
    return key_ranges_;
  }
  void set_scan_flag(const common::ObQueryFlag& scan_flag)
  {
...
private:
  ...
  int convert_key(const common::ObRowkey& src, common::ObRowkey& dst,
