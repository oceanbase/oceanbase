...
namespace table
{
int ObRedisHelper::get_decoded_msg_from_entity(ObIAllocator &allocator,
...
{
  int ret = OB_SUCCESS;
  if (OB_FAIL(ObRedisHelper::get_decoded_msg_from_entity(
...
int ObRedisResponse::set_res_error(const ObString &err_msg)
{
  ...
  } else if (OB_FALSE_IT(obj.set_varchar(err_msg))) {
  } else if (OB_FAIL(res_entity->set_property(ObRedisUtil::REDIS_PROPERTY_NAME, obj))) {
    LOG_WARN("fail to set property", K(ret), K(err_msg));
  }
