        ...
        while (OB_SUCC(ret)) {
          if (OB_FAIL(result->next())) {
            ...
            if (OB_FAIL(result_arr.push_back(task))) {
              ...
              if (OB_SUCC(ret) && OB_NOT_NULL(allocator) && data_version >= DATA_VERSION_4_4_2_0) {
                ObString scan_index;
...
int ObTTLUtil::get_tenant_vector_index_ids(const uint64_t tenant_id, bool &has_ivf_index, common::ObIArray<uint64_t> &table_id_array)
{
  int ret = OB_SUCCESS;
  ObSchemaGetterGuard schema_guard;
  ObMultiVersionSchemaService &schema_service = ObMultiVersionSchemaService::get_instance();
