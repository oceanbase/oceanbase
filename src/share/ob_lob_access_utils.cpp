...
int ObTextStringIter::get_inrow_or_outrow_prefix_data(ObString &data_str, uint32_t prefix_char_len)
{
  ...
  if (!is_init_ || state_ != TEXTSTRING_ITER_INIT) {
    ...
    if (OB_FAIL(loc.get_inrow_data(data_str))) {
      ...
      size_t max_len = ObCharset::strlen_char(cs_type_, data_str.ptr(), data_str.length());
      size_t byte_len = (prefix_char_len > max_len) ? max_len : prefix_char_len;
  ...
  if (!in_obj.has_lob_header()) {
    ...
    if (!loc.is_valid()) {
      ...
      if (OB_FAIL(new_tmp_lob.init(lob_data_byte_len))) {
        ...
        // copy inrow data
        ObArenaAllocator tmp_alloc("ObLobRead", OB_MALLOC_NORMAL_BLOCK_SIZE, MTL_ID());
        ObTextStringIterState state;
        ObString src_block_data;
        ObTextStringIter instr_iter(in_obj);
        if (OB_FAIL(ret)) {
        } else if (OB_FAIL(instr_iter.init(0, session, allocator, &tmp_alloc))) {
          COMMON_LOG(WARN, "Lob: init text string iter failed", K(ret), K(instr_iter));
        } else {
          while (OB_SUCC(ret)
