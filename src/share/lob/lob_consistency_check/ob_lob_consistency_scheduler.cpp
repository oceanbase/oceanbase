 ...
 */
 ...
 #define HANDLE_USER_TTL()                                                                                              \
   if (!is_valid_lob_cmd(arg.cmd_code_)) {                                                                              \
     ...
     } else if (!common::ObTTLUtil::is_support_lob_consistency_data_version(tenant_data_version)) {                    \
       ret = OB_NOT_SUPPORTED;                                                                                          \
       LOG_USER_ERROR(OB_NOT_SUPPORTED, "LOB consistency is not supported for this tenant data version");             \
       LOG_WARN("LOB consistency is not supported for this tenant data version", K(ret), K(tenant_id_),                 \
 ...
 #define GENERATE_ONE_TABLET_TASK(is_repair_task)                                                                       \
   ...
   if (IS_NOT_INIT) {                                                                                                   \
     ...
     if (OB_FAIL(ObLobCheckTask::read_orphan_data_to_pairs(tenant_id_, ls_, 0, 0, tablet_table_pairs))) {               \
 ...
 int ObLobCheckScheduler::check_all_tablet_finished(bool &all_finished)
 {
 ...
 bool ObLobCheckScheduler::can_trigger_lob_clean_task() const {
   ...
   } else if (common::ObTTLUtil::is_support_lob_consistency_data_version(data_version)) {
   ...
   if (OB_ISNULL(exception_tablets_json)) {
     ...
     for (uint64_t i = 0; OB_SUCC(ret) && i < table_count; ++i) {
       ...
       if (OB_FAIL(exception_obj->get_key(i, table_id_key))) {
         ...
         LOG_WARN("[LOB_CHECK]table_id_key: %s, table_id: %lu", K(table_id_key), K(table_id));
 ...
 int ObLobRepairScheduler::check_all_tablet_finished(bool &all_finished)
 {
   DEBUG_SYNC(BEFORE_CHECK_TTL_TASK_FINISH);
   int ret = OB_SUCCESS;
