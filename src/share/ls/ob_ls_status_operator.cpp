    ...
    common::GlobalLearnerList &learner_list)
{
  ...
  if (OB_UNLIKELY(OB_INVALID_TENANT_ID == tenant_id)) {
    ...
    LOG_WARN("tenant id is invalid", KR(ret), K(tenant_id));
                                    ...
                                    member_list, status_info, client, arb_member, learner_list, oceanbase::share::OBCG_DEFAULT))) {
    ...
    bool &contain)
{
  ...
  if (OB_UNLIKELY(OB_INVALID_TENANT_ID == tenant_id || !src_ls_id.is_valid() || !dst_ls_id.is_valid())) {
    ...
    contain = true;
    LOG_INFO("does not have duplicate ls for transfer", KR(ret), K(src_ls_id), K(dst_ls_id));
  }
  return ret;
}
