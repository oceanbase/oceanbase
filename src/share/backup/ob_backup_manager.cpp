...
DEF_TO_STRING(ObBaseBackupInfo)
{
  ...
  //TODO (yanfeng) backup finish it later
...
{
  bool is_valid = true;
  //TODO(yanfeng) backup fix it later
  return is_valid;
}
...
{
  int ret = OB_SUCCESS;
  HEAP_VAR(ObBaseBackupInfo, tmp_info) {
    if (!inited_) {
      ret = OB_NOT_INIT;
...
int ObBackupInfoManager::update_backup_info(
    ...
    ObBackupItemTransUpdater &updater)
{
  //TODO(yanfeng) consider with backup dest
  int ret = OB_SUCCESS;
  if (!info.is_valid()) {
    ret = OB_INVALID_ARGUMENT;
...
int ObBackupInfoManager::get_backup_info(const uint64_t tenant_id, ObBaseBackupInfo &info)
{
  ...
  } else if (OB_FAIL(updater.start(*proxy_, tenant_id))) {
...
int ObBackupInfoManager::get_backup_info(common::ObIArray<ObBaseBackupInfoStruct> &infos)
{
  ...
  if (!inited_) {
    ...
    for (int64_t i = 0; OB_SUCC(ret) && i < tenant_ids_.count(); ++i) {
      const uint64_t tenant_id = tenant_ids_.at(i);
      HEAP_VARS_2((ObBaseBackupInfo, tmp_info),(ObBaseBackupInfoStruct, tmp_info_struct)) {
        if (OB_FAIL(get_backup_info(tenant_id, tmp_info))) {
          LOG_WARN("failed to get backup info", K(ret), K(tenant_id), K(tmp_info));
...
int ObBackupInfoManager::convert_info_to_struct(
    const ObBaseBackupInfo &info, ObBaseBackupInfoStruct &info_struct)
{
  int ret = OB_SUCCESS;
  info_struct.reset();
  ...
  } else if (OB_FAIL(info.backup_snapshot_version_.get_int_value(
      info_struct.backup_snapshot_version_))) {
    LOG_WARN("failed to get info_struct.backup_snapshot_version_", K(ret), K(info));
  } else if (OB_FAIL(info.backup_schema_version_.get_int_value(
      info_struct.backup_schema_version_))) {
    LOG_WARN("failed to get info_struct.backup_schema_version_", K(ret), K(info));
  } else if (OB_FAIL(info.backup_data_version_.get_int_value(
      info_struct.backup_data_version_))) {
...
int ObBackupInfoManager::convert_struct_to_info(
    const ObBaseBackupInfoStruct &info_struct, ObBaseBackupInfo &info)
{
  //TODO(yanfeng) backup fix it later
  int ret = OB_SUCCESS;
  if (!inited_) {
...
int ObBackupInfoManager::get_backup_info_without_trans(
    const uint64_t tenant_id,
    ObBaseBackupInfoStruct &info)
{
  int ret = OB_SUCCESS;
...
int ObBackupInfoManager::get_detected_region(
    const uint64_t tenant_id,
    ObIArray<ObBackupRegion> &detected_region)
{
  int ret = OB_SUCCESS;
  ...
  if (!inited_) {
    ...
    LOG_WARN("failed to find tenant id", K(ret), K(tenant_id));
  } else if (OB_FAIL(ObTenantBackupInfoOperation::load_info_item(*proxy_,
      tenant_id, detected_region_item, need_lock))) {
    ...
    ObBackupInfoStatus &status)
{
  ...
  if (!inited_) {
    ...
    LOG_WARN("failed to find tenant id", K(ret), K(tenant_id));
      ...
      tenant_id, backup_status_item, need_lock))) {
    LOG_WARN("failed to detected info", K(ret), K(tenant_id));
...
int ObBackupInfoManager::get_backup_backup_dest(
    const uint64_t tenant_id,
    common::ObISQLClient &trans,
    ObBaseBackupInfoStruct::BackupDest &dest)
{
  int ret = OB_SUCCESS;
  ...
  if (!inited_) {
    ...
    LOG_WARN("failed to find tenant id", K(ret), K(tenant_id));
      ...
      trans, tenant_id, dest_item, need_lock))) {
    ...
    const int64_t major_version)
{
  ...
  if (!inited_) {
    ...
    ret = OB_INVALID_ARGUMENT;
    LOG_WARN("insert restore tenant base backup version get invalid argument", K(ret), K(major_version),
        K(base_backup_version));
  } else if (OB_FAIL(ObTenantBackupInfoOperation::load_info_item(*proxy_,
      tenant_id, base_backup_version_item, need_lock))) {
    ...
    bool &is_enable)
{
  ...
  if (!inited_) {
    ...
    LOG_WARN("failed to find tenant id", K(ret), K(tenant_id));
      ...
      tenant_id, enable_backup_archivelog_item, need_lock))) {
    LOG_WARN("failed to detected info", K(ret), K(tenant_id));
    ...
    common::ObISQLClient &trans)
{
  ...
  if (!inited_) {
    ...
    LOG_WARN("failed to update enable backup archivelog", K(ret), K(tenant_id),
        K(enable_backup_archivelog_item));
  }
  return ret;
}
