...
struct ObBackupLeaseInfo
{
  ...
  ObBackupLeaseInfo();
  void reset();
  int set_new_lease(const int64_t start_ts, const int64_t epoch,
  ...
  bool is_valid() const;
  TO_STRING_KV(K_(is_leader), K_(lease_start_ts), K_(leader_epoch), K_(leader_takeover_ts),
      K_(round), K_(lease_epoch));
  bool is_leader_;
  int64_t lease_start_ts_;
  int64_t leader_epoch_; // for rs leader
  int64_t leader_takeover_ts_; // for rs leader
  int64_t round_;
  int64_t lease_epoch_; // backup lease epoch
};
...
public:
  ...
  int renew_lease(const int64_t can_be_leader_ts, const int64_t next_round,
      const ObBackupLeaseInfo &old_lease_info, ObBackupLeaseInfo &new_lease_info,
      const char *&msg);
  int clean_backup_lease_info(const int64_t next_round, const ObBackupLeaseInfo &old_lease_info,
      ObBackupLeaseInfo &new_lease_info);
private:
	int get_backup_scheduler_leader_(
      common::ObISQLClient &sql_client,
      common::ObAddr &backup_leader_addr,
      bool &has_leader,
      int64_t &lease_epoch);
  int get_backup_scheduler_leader_v1_(
      common::ObISQLClient &sql_client,
      common::ObAddr &backup_leader_addr,
      bool &has_leader);
  int get_backup_scheduler_leader_v2_(
      common::ObISQLClient &sql_client,
      common::ObAddr &backup_leader_addr,
      bool &has_leader,
      int64_t &lease_epoch);
  int update_backup_scheduler_leader_(
      const common::ObAddr &lease_leader,
      int64_t &lease_epoch,
      common::ObISQLClient &sql_client);
  int update_backup_scheduler_leader_v1_(
      const common::ObAddr &backup_leader_addr,
      common::ObISQLClient &sql_client);
  int update_backup_scheduler_leader_v2_(
      const common::ObAddr &lease_leader,
      const int64_t lease_epoch,
      common::ObISQLClient &sql_client);
  int clean_backup_scheduler_leadear_();
  int clean_backup_scheduler_leadear_v1_();
  int clean_backup_scheduler_leadear_v2_();
  int do_renew_lase_(
...
public:
  ...
  virtual int64_t get_lease_version() const = 0;
...
public:
  ...
  virtual int64_t get_lease_version() const;
};
