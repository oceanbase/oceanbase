...
{
  int ret = OB_SUCCESS;
  if (GET_MIN_CLUSTER_VERSION() < CLUSTER_VERSION_4_2_1_11) {
    ret = OB_NOT_SUPPORTED;
    LOG_WARN("backup zone is not supported under cluster version 4_2_1_11 ", K(ret));
...
int ObBackupChangeExternalStorageDestUtil::change_external_storage_dest(const obrpc::ObAdminSetConfigArg &arg)
{
  ...
  if (GET_MIN_CLUSTER_VERSION() < CLUSTER_VERSION_4_2_1_11) {
    ret = OB_NOT_SUPPORTED;
    LOG_WARN("change external storage dest is not supported under cluster version 4_2_1_11 ", K(ret));
    LOG_USER_ERROR(OB_NOT_SUPPORTED, "change external storage dest is");
  } else if (!arg.is_valid()) {
    ret = OB_INVALID_ARGUMENT;
    LOG_WARN("invalid arg", K(arg), K(ret));
  } else if (2 != arg.items_.count() || arg.items_.at(0).value_.is_empty() || arg.items_.at(1).value_.is_empty()) {
    ret = OB_INVALID_ARGUMENT;
    LOG_WARN("invalid arg", K(arg), K(ret));
  } else {
    ...
    const common::ObFixedLengthString<common::OB_MAX_CONFIG_VALUE_LEN> &attribute = arg.items_.at(1).value_;
    ...
    } else if (OB_FAIL(ObBackupDestAttributeParser::parse(attribute.str(), attribute_option))) {
    ...
    ROOTSERVICE_EVENT_ADD("root_service", "change_external_storage_dest", K(ret), K(path), K(attribute));
  }
