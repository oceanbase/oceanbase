...
int ObIndexBuilderUtil::add_column(
    ...
    const int64_t new_col_name_len,
    const int64_t new_col_id,
    const int64_t index_position)
{
  ...
  if (NULL == data_column) {
  ...
  } else if (-1 == new_col_id && OB_INVALID_INDEX != row_desc.get_idx(
    ...
    if (is_index) {
      ...
      if (OB_ISNULL(store_column_schema)) {
        ...
        store_column_schema->add_column_flag(USER_SPECIFIED_STORING_COLUMN_FLAG);
          ...
          new_col_id != -1 ? OB_INVALID_ID : data_column->get_table_id(), new_col_id != -1 ? new_col_id : data_column->get_column_id()))) {
    ...
    if (OB_FAIL(column.assign(*data_column))) {
      ...
      if (OB_NOT_NULL(new_col_name)) {
        ObString tmp_str;
        tmp_str.assign_ptr(new_col_name, new_col_name_len);
        column.set_column_name(tmp_str);
      }
      if (-1 != new_col_id) {
...
int ObIndexBuilderUtil::set_index_table_columns_for_vector_hnsw(
      ...
      common::ObRowDesc &row_desc)
{
  ...
  OB_ASSERT(1 == arg.index_columns_.count());
  // pks
  const int64_t pk_buffer_len = OB_MAX_COLUMN_NAME_LENGTH + 10;
  char pk_name_buffer[pk_buffer_len];
  int64_t index_column_id = -1;
  int64_t index_position = 0;
  const ObRowkeyInfo &rowkey_info = data_schema.get_rowkey_info();

  const ObColumnSortItem &sort_item = arg.index_columns_.at(0);
  const ObColumnSchemaV2 *vector_data_column = NULL;
  if (NULL == (vector_data_column = data_schema.get_column_schema(sort_item.column_name_))) {
    ...
    index_column_id = data_schema.get_max_used_column_id() + 1;
  }

  const ObString &part_str = data_schema.get_part_option().get_part_func_expr_str();
  for (int64_t i = 0; OB_SUCC(ret) && i < rowkey_info.get_size(); ++i) {
    ...
    if (OB_FAIL(rowkey_info.get_column_id(i, column_id))) {
      ...
      LOG_WARN("get_column_schema failed", "table_id", data_schema.get_table_id(),
          K(column_id), K(ret));
    } else if (OB_FAIL(databuff_printf(pk_name_buffer, pk_buffer_len, pos, "base_pk%ld", i))) {
      LOG_WARN("column name buffer overflow", K(ret));
    } else if (OB_FAIL(add_column(data_column, false, is_rowkey,
              data_column->get_order_in_rowkey(), row_desc, index_schema,
              false /* is_hidden */, false /* is_specified_storing_col */,
              pk_name_buffer, pos, column_id))) {
      LOG_WARN("add column failed", K(ret));
    } else if (data_column->get_column_name_str() == part_str) {  // Column ref only. TODO: shk
      ObString pk_name_str;
      pk_name_str.assign_ptr(pk_name_buffer, pos);
      const_cast<ObPartitionOption&>(data_schema.get_part_option()).set_part_expr(pk_name_str);
    }
  }

  ObColumnSchemaV2 extra_col;
  ObObjMeta obj_meta;
  // level
  if (OB_SUCC(ret)) {
    extra_col.reset();
    obj_meta.set_int();
    extra_col.set_meta_type(obj_meta);
    extra_col.set_column_name("level");
    if (OB_FAIL(add_column(&extra_col, false, true,   // TODO: shk: optimze &extra_col to avoid copy
                extra_col.get_order_in_rowkey(), row_desc, index_schema,
                false /* is_hidden */, false /* is_specified_storing_col */,
                nullptr, 0, index_column_id++))) {
      LOG_WARN("add column failed", K(ret));
    }
  }
  ...
  if (OB_SUCC(ret)) {
    for (int64_t i = 0; OB_SUCC(ret) && i < rowkey_info.get_size(); ++i) {
      ...
      if (OB_FAIL(rowkey_info.get_column_id(i, column_id))) {
        ...
        LOG_WARN("get_column_schema failed", "table_id", data_schema.get_table_id(),
            K(column_id), K(ret));
      } else if (OB_FAIL(databuff_printf(pk_name_buffer, pk_buffer_len, pos, "ref_pk%ld", i))) {
        LOG_WARN("column name buffer overflow", K(ret));
      } else if (OB_FAIL(add_column(data_column, false, true,
                         data_column->get_order_in_rowkey(), row_desc, index_schema,
                         false /* is_hidden */, false /* is_specified_storing_col */,
                         pk_name_buffer, pos, index_column_id++))) {
        LOG_WARN("add column failed", K(ret));
      }
    }
    if (OB_SUCC(ret)) {
      index_schema.set_rowkey_column_num(row_desc.get_column_num());
      LOG_INFO("######## rowkey cnt ##########", K(row_desc.get_column_num()));
    }
  }
  ...
  if (OB_SUCC(ret)) {
    int64_t pos = 0;
    if (OB_UNLIKELY(ObVectorType != vector_data_column->get_meta_type().get_type())) {
      ret = OB_ERR_UNEXPECTED;
      LOG_WARN("unexpected error: column must be a vector", K(ret), K(vector_data_column->get_meta_type().get_type()));
    } else if (OB_FAIL(databuff_printf(pk_name_buffer, pk_buffer_len, pos, "_%.*s",
                                       static_cast<int>(sort_item.column_name_.length()),
                                       sort_item.column_name_.ptr()))) {
      LOG_WARN("column name buffer overflow", K(ret));
    } else if (OB_FAIL(add_column(vector_data_column, true, false,
                                  arg.index_columns_.at(0).order_type_, row_desc, index_schema,
                                  false /* is_hidden */, false /* is_specified_storing_col */,
                                  pk_name_buffer, pos, sort_item.get_column_id(), ++index_position))) {
      LOG_WARN("add column failed", K(ret));
    } else {
      index_schema.set_index_column_num(index_position);
    }
  }
  ...
  if (OB_SUCC(ret)) {
    ...
    obj_meta.set_int();
    extra_col.set_meta_type(obj_meta);
    extra_col.set_column_name("n_neighbors");
    if (OB_FAIL(add_column(&extra_col, false, false,   // TODO: shk: optimze &extra_col to avoid copy
                extra_col.get_order_in_rowkey(), row_desc, index_schema,
                false /* is_hidden */, false /* is_specified_storing_col */,
                nullptr, 0, index_column_id++))) {
      LOG_WARN("add column failed", K(ret));
  ...
  if (OB_SUCC(ret)) {
    ...
    obj_meta.set_double();
    extra_col.set_meta_type(obj_meta);
    extra_col.set_column_name("distance");
    if (OB_FAIL(add_column(&extra_col, false, false,   // TODO: shk: optimze &extra_col to avoid copy
                extra_col.get_order_in_rowkey(), row_desc, index_schema,
                false /* is_hidden */, false /* is_specified_storing_col */,
                nullptr, 0, index_column_id++))) {
    ...
    common::ObRowDesc &row_desc)
{
  ...
  rowkey_col.set_column_name("center_idx");
  if (FAILEDx(add_column(&rowkey_col, false/*is_index_column*/, true/*is_rowkey*/,
      arg.index_columns_.at(0).order_type_, row_desc, index_schema,
      false /* is_hidden */, false /* is_specified_storing_col */,
  ...
  if (OB_SUCC(ret)) { // index table
    for (int64_t i = 0; OB_SUCC(ret) && i < rowkey_info.get_size(); ++i) {
      ...
      if (OB_FAIL(rowkey_info.get_column_id(i, column_id))) {
        LOG_WARN("get_column_id failed", "index", i, K(ret));
      } else if (NULL == (rowkey_data_column = data_schema.get_column_schema(column_id))) {
        ret = OB_ERR_BAD_FIELD_ERROR;
        LOG_WARN("get_column_schema failed", "table_id", data_schema.get_table_id(),
            K(column_id), K(ret));
      } else if (ob_is_text_tc(rowkey_data_column->get_data_type())) {
        ret = OB_ERR_WRONG_KEY_COLUMN;
        LOG_WARN("Lob column should not appear in rowkey position", "data_column", *rowkey_data_column,
            K(is_rowkey), "order_in_rowkey", rowkey_data_column->get_order_in_rowkey(),
            K(row_desc), K(ret));
      } else if (ob_is_extend(rowkey_data_column->get_data_type()) || ob_is_user_defined_sql_type(rowkey_data_column->get_data_type())) {
        ret = OB_ERR_WRONG_KEY_COLUMN;
        LOG_WARN("udt column should not appear in rowkey position", "data_column", *rowkey_data_column,
            K(is_rowkey), "order_in_rowkey", rowkey_data_column->get_order_in_rowkey(),
            K(row_desc), K(ret));
      } else if (ob_is_json_tc(rowkey_data_column->get_data_type())) {
        ret = OB_ERR_JSON_USED_AS_KEY;
        LOG_WARN("JSON column cannot be used in key specification.", "data_column", *rowkey_data_column,
            K(is_rowkey), "order_in_rowkey", rowkey_data_column->get_order_in_rowkey(),
            K(row_desc), K(ret));
      } else if (ob_is_vector_type(rowkey_data_column->get_data_type())) {
        ret = OB_ERR_UNEXPECTED; // TODO(@jingshui)
        LOG_WARN("vector column cannot be used in key specification.", "data_column", *rowkey_data_column,
            K(is_rowkey), "order_in_rowkey", rowkey_data_column->get_order_in_rowkey(),
            K(row_desc), K(ret));
      } else if (OB_FAIL(add_column(rowkey_data_column, false/*is_index_column*/, true/*is_rowkey*/,
          rowkey_data_column->get_order_in_rowkey(), row_desc, index_schema,
          false /* is_hidden */, false /* is_specified_storing_col */))) {
        LOG_WARN("add column failed", K(ret));
      }
    }
    ...
    bool check_data_schema /*=true*/)
{
  ...
  if (OB_SUCC(ret)) {
    if (arg.is_vector_index()) {
      if (arg.index_columns_.empty()) {
        ret = OB_INVALID_ARGUMENT;
        LOG_WARN("invalid index_columns", K(ret), K(ret));
      } else if (arg.index_columns_.count() > 1) {
        ret = OB_NOT_SUPPORTED; // TODO(@jingshui) error code
        LOG_WARN("vector index request only one index column", K(ret), K(arg));
      } else {
        HEAP_VAR(ObRowDesc, row_desc) {
          ...
          if (NULL == (data_column = data_schema.get_column_schema(sort_item.column_name_))) {
            ...
            LOG_WARN("get_column_schema failed", "tenant_id", data_schema.get_tenant_id(),
                    ...
                    "table_name", data_schema.get_table_name(),
                    "column name", sort_item.column_name_, K(ret));
          } else if (!(ob_is_vector_type(data_column->get_data_type()))) {
            ret = OB_ERR_UNEXPECTED; // TODO(@jingshui)
              LOG_WARN("vector column cannot be used in key specification", "tenant_id", data_schema.get_tenant_id(),
                      "database_id", data_schema.get_database_id(),
                      "table_name", data_schema.get_table_name(),
            ...
            if (ObIndexUsingType::USING_IVFFLAT == arg.index_using_type_) {
              ...
              if (OB_FAIL(set_index_table_columns_for_vector_hnsw(arg, data_schema, index_schema, row_desc))) {
                LOG_WARN("failed to set index table columns for hnsw", K(ret));
              }
            }
          }
        }
        ...
        if (OB_SUCC(ret)) {
          ...
          for (int64_t i = 0; OB_SUCC(ret) && i < rowkey_info.get_size(); ++i) {
            ...
            if (OB_FAIL(rowkey_info.get_column_id(i, column_id))) {
              ...
              ret = OB_ERR_UNEXPECTED; // TODO(@jingshui)
              LOG_WARN("vector column cannot be used in key specification.", "data_column", *data_column, K(is_index_column),
                  K(is_rowkey), "order_in_rowkey", data_column->get_order_in_rowkey(),
                  K(row_desc), K(ret));
            } else if (OB_FAIL(add_column(data_column, is_index_column, is_rowkey,
                data_column->get_order_in_rowkey(), row_desc, index_schema,
