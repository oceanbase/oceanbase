    ...
    bool &is_passed)
{
  ...
  } else if (data_version < DATA_VERSION_4_2_1_9) {
    is_passed = true;
    ...
    const common::ObString &job_name)
{
  ...
  if (!is_trigger_job(job_name)) {
    ...
    LOG_USER_ERROR(OB_OP_NOT_ALLOW, "tenant data version is less than 4.2.1.9, enable SCHEDULED_TRIGGER_PARTITION_BALANCE is");
  } else {
    ...
    const common::ObString &job_name)
{
  ...
  if (!is_trigger_job(job_name)) {
    ...
    LOG_USER_ERROR(OB_OP_NOT_ALLOW, "tenant data version is less than 4.2.1.9, disable SCHEDULED_TRIGGER_PARTITION_BALANCE is");
  }
  return ret;
}
