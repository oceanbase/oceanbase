...
public:
  ...
  virtual int get_resource_pools(const common::ObIArray<uint64_t> &pool_ids,
                                 common::ObIArray<ObResourcePool> &pools) const;
  virtual int get_resource_pool_names(const uint64_t tenant_id,
  ...
  int get_units_by_unit_ids(const ObIArray<uint64_t> &unit_ids,
                            common::ObIArray<ObUnit> &units);
  int get_unit_by_id(common::ObISQLClient &client, const uint64_t unit_id, ObUnit &unit) const;
...
private:
  int construct_unit_dml_(
    const ObUnit &unit, const bool include_ug_id, share::ObDMLSqlSplicer &dml);
  int check_unit_conflict_with_clone_(common::ObISQLClient &client, const ObUnit &unit);
