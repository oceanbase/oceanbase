  ...
  inline static int cmp(const ObDatum &l, const ObDatum &r, int &cmp_ret)
  {
    ...
    if (OB_FAIL(l_instr_iter.init(0, NULL, &allocator))) {
      ...
      if (OB_FAIL(j_bin_l.reset_iter())) {
        COMMON_LOG(WARN, "fail to reset left json bin iter", K(ret), K(l_data));
      } else if (OB_FAIL(j_bin_r.reset_iter())) {
        COMMON_LOG(WARN, "fail to reset right json bin iter", K(ret), K(r_data));
...
typedef ObConstIntMapping<0,
    ...
    CS_TYPE_LATIN1_GERMAN1_CI,1,
    CS_TYPE_LATIN1_DANISH_CI,1,
    CS_TYPE_LATIN1_GENERAL_CI,1,
    CS_TYPE_LATIN1_GENERAL_CS,1,
    CS_TYPE_LATIN1_SPANISH_CI,1,
    CS_TYPE_LATIN1_GERMAN2_CI,1,
