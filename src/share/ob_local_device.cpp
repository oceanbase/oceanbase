...
{
  int ret = OB_SUCCESS;
  const ObMemAttr mem_attr = SET_IGNORE_MEM_VERSION(ObMemAttr(OB_SERVER_TENANT_ID, "LDIOSetup"));
    ...
    bool &is_exist)
{
  ...
  if (OB_FAIL(databuff_printf(store_path_, OB_MAX_FILE_NAME_LENGTH, "%s/%s/%s",
    ...
    if (OB_SUCC(ret)) {
      const ObMemAttr mem_attr = SET_IGNORE_MEM_VERSION(ObMemAttr(OB_SERVER_TENANT_ID, "LDBlockBitMap"));
