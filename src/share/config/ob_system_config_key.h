...
inline ObSystemConfigKey::ObSystemConfigKey() : zone_(), server_port_(0), version_(0)
{
...
{
  return (this == &other ||
          (0 == STRCMP(name_, other.name_) && zone_ == other.zone_ && (0 == STRCMP(server_type_, other.server_type_)) &&
              (0 == STRCMP(server_ip_, other.server_ip_)) && server_port_ == other.server_port_));
}
...
inline bool ObSystemConfigKey::match(const ObSystemConfigKey& other) const
{
  bool ret = false;
  ret = (this == &other ||
         (0 == STRCMP(name_, other.name_) && (zone_ == other.zone_ || zone_.is_empty()) &&
             ((0 == STRCMP(server_type_, other.server_type_)) || (0 == STRCMP(server_type_, DEFAULT_VALUE))) &&
             ((0 == STRCMP(server_ip_, other.server_ip_)) || (0 == STRCMP(server_ip_, DEFAULT_VALUE))) &&
...
inline void ObSystemConfigKey::set_name(const ObString& name)
{
  ...
  int64_t pos = 0;
  (void)databuff_printf(name_, OB_MAX_CONFIG_NAME_LEN, pos, "%.*s", static_cast<int>(name_length), name.ptr());
}
...
inline void ObSystemConfigKey::set_server_type(const ObString& server_type)
{
  ...
  snprintf(server_type_, OB_SERVER_TYPE_LENGTH, "%.*s", static_cast<int>(server_type_length), server_type.ptr());
}
...
inline void ObSystemConfigKey::set_server_ip(const ObString& server_ip)
{
  ...
  snprintf(server_ip_, OB_MAX_SERVER_ADDR_SIZE, "%.*s", static_cast<int>(server_ip_length), server_ip.ptr());
}
