...
public:
  ...
  public:
    ...
    int init(const int64_t row_id, ObCoreTableProxy& kv_proxy, const common::ObIArray<Cell>& cells);
    ...
    void set_row_id(const int64_t row_id)
    {
      row_id_ = row_id;
    }

    int64_t get_cell_cnt() const
    {
      return cell_cnt_;
    }
  ...
  bool is_valid() const
  {
    return NULL != table_name_ && NULL != sql_client_;
  }

  void set_sql_client(common::ObISQLClient& sql_client)
  {
  ...
  {
    return all_row_.count();
  }
  const common::ObIArray<Row>& get_all_row() const
  {
    return all_row_;
  }
  ...
  // if the row not exist, execute insert; otherwise update the row while the new is large than the old value of the
  // row. it is used for resolve some value can not backoff. in the RS switch, the new value will be overwrite while old
  // query may execute on new leader.
...
private:
  ...
  int update(const common::ObIArray<UpdateCell>& cells, const bool insert, int64_t& affected_rows);
  int incremental_update(const common::ObIArray<UpdateCell>& cells, const bool insert, int64_t& affected_rows);

  static int check_row_match(const Row& row, const common::ObIArray<UpdateCell>& cells, bool& match);

  // update cell or insert new cell
  int execute_update_sql(const Row& row, const common::ObIArray<UpdateCell>& cells, int64_t& affected_rows);
  int execute_incremental_update_sql(const Row& row, const common::ObIArray<UpdateCell>& cells, int64_t& affected_rows);
