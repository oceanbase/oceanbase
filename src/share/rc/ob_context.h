...
#define WITH_ENTITY_P(condition, entity)                                              \
  ENTITY_P(condition,                                                                 \
      share::Entity2Enum<typename std::remove_pointer<decltype(entity)>::type>::type, \
      share::EntitySource::WITH,                                                      \
      entity)
...
namespace share {
enum class ObEntityType { TENANT_SPACE, TABLE_SPACE, RESOURCE_OWNER };
...
class EntityBase {
  ...
  friend int create_entity(T_Entity*& entity, Args&&... args);
  template <typename T_Entity, typename... Args>
  friend int create_entity(T_Entity& entity, Args&&... args);
  template <typename T_Entity>
...
public:
  ...
  static int guard_init_cb(const ObTableSpace& table_space, char* buf, bool& is_inited);
...
// For example, if you want to switch to TenantSpace, the most convenient interface for the upper layer is to only
// provide tenant_id. The TenantSpace is obtained by the framework, if the tenant_id is used as the parameter of the
...
BIND_ENTITY(ObEntityType::TABLE_SPACE, share::ObTableSpace);
...
public:
  ...
  static Guard& current_guard()
  {
    ...
    if (OB_UNLIKELY(nullptr == cur)) {
      static lib::CoVar<char[sizeof(Guard)]> buf;
      Guard* guard = new (&buf[0]) Guard(T_Entity::root());
...
template <typename T_Entity, typename... Args>
inline int create_entity(T_Entity*& entity, Args&&... args)
{
  int ret = common::OB_SUCCESS;
...
template <ObEntityType ct, EntitySource es>
class _S {};
...
extern int get_tenant_ctx_with_tenant_lock(
    const uint64_t tenant_id, common::ObLDHandle& handle, ObTenantSpace*& tenant_ctx);
extern int get_compat_mode_with_table_id(const uint64_t table_id, ObWorker::CompatMode& compat_mode);
