...
int ObInnerTableSchema::v_ob_ls_log_transport_stat_ora_schema(ObTableSchema &table_schema)
{
  int ret = OB_SUCCESS;
  ...
  table_schema.set_table_id(OB_V_OB_LS_LOG_TRANSPORT_STAT_ORA_TID);
  ...
  if (OB_SUCC(ret)) {
    if (OB_FAIL(table_schema.set_table_name(OB_V_OB_LS_LOG_TRANSPORT_STAT_ORA_TNAME))) {
  ...
  if (OB_SUCC(ret)) {
    if (OB_FAIL(table_schema.set_view_definition(R"__(     SELECT TENANT_ID, LS_ID, SVR_IP, SVR_PORT, ROLE, ENABLED,            END_LSN, END_SCN, LAST_SENT_LSN, LAST_SENT_SCN,            STANDBY_END_LSN, STANDBY_END_SCN, STANDBY_ADDR     FROM SYS.GV$OB_LS_LOG_TRANSPORT_STAT     WHERE TENANT_ID = EFFECTIVE_TENANT_ID()           AND SVR_IP = HOST_IP() AND SVR_PORT = RPC_PORT()   )__"))) {
