...
int ObInnerTableSchema::all_virtual_partition_info_schema(ObTableSchema &table_schema)
{
  ...
  if (OB_SUCC(ret)) {
    ObObj last_replay_log_ts_default;
    last_replay_log_ts_default.set_int(0);
    ADD_COLUMN_SCHEMA_T("last_replay_log_ts", //column_name
      ++column_id, //column_id
      0, //rowkey_id
      0, //index_id
      0, //part_key_pos
      ...
      false, //is_nullable
      false, //is_autoincrement
      last_replay_log_ts_default,
      last_replay_log_ts_default); //default_value
  }
  if (OB_SUCC(ret)) {
    table_schema.get_part_option().set_part_func_type(PARTITION_FUNC_TYPE_HASH);
    if (OB_FAIL(table_schema.get_part_option().set_part_expr("hash (addr_to_partition_id(svr_ip, svr_port))"))) {
