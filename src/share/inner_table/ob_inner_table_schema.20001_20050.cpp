...
int ObInnerTableSchema::tables_schema(ObTableSchema &table_schema)
{
  ...
  if (OB_SUCC(ret)) {
    if (OB_FAIL(table_schema.set_view_definition(R"__(                     select /*+ leading(a) no_use_nl(ts)*/                     cast('def' as char(512)) as TABLE_CATALOG,                     cast(b.database_name as char(64)) as TABLE_SCHEMA,                     cast(a.table_name as char(64)) as TABLE_NAME,                     cast(case when (a.database_id = 201002 or a.table_type = 1) then 'SYSTEM VIEW'                          when a.table_type in (0, 2) then 'SYSTEM TABLE'                          when a.table_type = 4 then 'VIEW'                          when a.table_type = 14 then 'EXTERNAL TABLE'                          else 'BASE TABLE' end as char(64)) as TABLE_TYPE,                     cast(case when a.table_type in (0,3,5,6,7,11,12,13) then 'InnoDB'                         else 'MEMORY' end as char(64)) as ENGINE,                     cast(NULL as unsigned) as VERSION,                     cast(a.store_format as char(10)) as ROW_FORMAT,                     cast( coalesce(ts.row_cnt,0) as unsigned) as TABLE_ROWS,                     cast( coalesce(ts.avg_row_len,0) as unsigned) as AVG_ROW_LENGTH,                     cast( coalesce( su. size,0) as unsigned) as DATA_LENGTH,                     cast(NULL as unsigned) as MAX_DATA_LENGTH,                     cast( coalesce( su_idx.size , 0) as unsigned) as INDEX_LENGTH,                     cast(NULL as unsigned) as DATA_FREE,                     cast(NULL as unsigned) as AUTO_INCREMENT,                     cast(a.gmt_create as datetime) as CREATE_TIME,                     cast(a.gmt_modified as datetime) as UPDATE_TIME,                     cast(NULL as datetime) as CHECK_TIME,                     cast(d.collation as char(32)) as TABLE_COLLATION,                     cast(NULL as unsigned) as CHECKSUM,                     cast(NULL as char(255)) as CREATE_OPTIONS,                     cast(case when a.table_type = 4 then 'VIEW'                              else a.comment end as char(2048)) as TABLE_COMMENT                     from                     (                     select cast(0 as signed) as tenant_id,                            c.database_id,                            c.table_id,                            c.table_name,                            c.collation_type,                            c.table_type,                            usec_to_time(d.schema_version) as gmt_create,                            usec_to_time(c.schema_version) as gmt_modified,                            c.comment,                            c.store_format                     from oceanbase.__all_virtual_core_all_table c                     join oceanbase.__all_virtual_core_all_table d                       on c.tenant_id = d.tenant_id and d.table_name = '__all_core_table'                     where c.tenant_id = effective_tenant_id()                     union all                     select tenant_id,                            database_id,                            table_id,                            table_name,                            collation_type,                            table_type,                            gmt_create,                            gmt_modified,                            comment,                            store_format                     from oceanbase.__all_table where table_mode >> 12 & 15 in (0,1) and index_attributes_set & 16 = 0) a                     join oceanbase.__all_database b                     on a.database_id = b.database_id                     and a.tenant_id = b.tenant_id                     join oceanbase.__tenant_virtual_collation d                     on a.collation_type = d.collation_type                     left join (                       select tenant_id,                              table_id,                              row_cnt,                              avg_row_len                       from oceanbase.__all_table_stat                       where partition_id = -1 or partition_id = table_id) ts                     on a.table_id = ts.table_id                     and a.tenant_id = ts.tenant_id                     left join (                       SELECT                         avt.database_id as database_id,                         avt.table_id as table_id,                         SUM(avtps.required_size) AS size                       FROM oceanbase.__all_virtual_tablet_pointer_status avtps                       INNER JOIN oceanbase.__all_tablet_to_ls avttl                         ON avttl.tablet_id = avtps.tablet_id                       INNER JOIN oceanbase.__all_table avt                         ON avt.table_id = avttl.table_id                           AND avt.table_type != 5                       INNER JOIN oceanbase.__all_database ad                         ON ad.database_id = avt.database_id                       INNER JOIN oceanbase.DBA_OB_LS_LOCATIONS avlmt                         ON avtps.ls_id = avlmt.ls_id                           AND avtps.svr_ip = avlmt.svr_ip                           AND avtps.svr_port = avlmt.svr_port                           AND avlmt.role = 'LEADER'                       group by database_id, table_id                       UNION                       SELECT                         avt.database_id as database_id,                         avt.table_id as table_id,                         SUM(avtps.required_size) AS size                       FROM oceanbase.__all_virtual_tablet_pointer_status avtps                       INNER JOIN oceanbase.__all_table avt                         ON table_type = 0                           AND avt.tablet_id = avtps.tablet_id                       INNER JOIN oceanbase.__all_database ad                         ON ad.database_id = avt.database_id                       INNER JOIN oceanbase.DBA_OB_LS_LOCATIONS avlmt                         ON avtps.ls_id = avlmt.ls_id                           AND avtps.svr_ip = avlmt.svr_ip                           AND avtps.svr_port = avlmt.svr_port                           AND avlmt.role = 'LEADER'                       group by database_id, table_id) su                         ON a.database_id = su.database_id and a.table_id = su.table_id                                              left join (                       SELECT                         avt.database_id as database_id,                         avt.data_table_id as data_table_id,                         SUM(avtps.required_size) AS size                       FROM oceanbase.__all_virtual_tablet_pointer_status avtps                       INNER JOIN oceanbase.__all_tablet_to_ls avttl                         ON avttl.tablet_id = avtps.tablet_id                       INNER JOIN oceanbase.__all_table avt                         ON avt.table_id = avttl.table_id                           AND table_type = 5                       INNER JOIN oceanbase.__all_database ad                         ON ad.database_id = avt.database_id                       INNER JOIN oceanbase.DBA_OB_LS_LOCATIONS avlmt                         ON avtps.ls_id = avlmt.ls_id                           AND avtps.svr_ip = avlmt.svr_ip                           AND avtps.svr_port = avlmt.svr_port                           AND avlmt.role = 'LEADER'                       group by database_id, data_table_id                       UNION                       SELECT                         avt.database_id as database_id,                         avt.data_table_id as data_table_id,                         SUM(avtps.required_size) AS size                       FROM oceanbase.__all_virtual_tablet_pointer_status avtps                       INNER JOIN oceanbase.__all_table avt                         ON table_type = 0                           AND avt.tablet_id = avtps.tablet_id                       INNER JOIN oceanbase.__all_database ad                         ON ad.database_id = avt.database_id                       INNER JOIN oceanbase.DBA_OB_LS_LOCATIONS avlmt                         ON avtps.ls_id = avlmt.ls_id                           AND avtps.svr_ip = avlmt.svr_ip                           AND avtps.svr_port = avlmt.svr_port                           AND avlmt.role = 'LEADER'                       group by database_id, data_table_id) su_idx                         ON su_idx.database_id = a.database_id and su_idx.data_table_id = a.table_id                     left join (                       select e.tenant_id as tenant_id,                              e.data_table_id as data_table_id,                              SUM(f.macro_blk_cnt * 2 * 1024 * 1024) AS index_length                       FROM oceanbase.__all_table e JOIN oceanbase.__all_table_stat f                             ON e.tenant_id = f.tenant_id and e.table_id = f.table_id and (f.partition_id = -1 or f.partition_id = e.table_id)                       WHERE e.index_type in (1, 2, 3, 4, 5, 6, 7, 8, 10, 11, 12) and e.table_type = 5                             group by tenant_id, data_table_id                     ) idx_stat on idx_stat.tenant_id = a.tenant_id and idx_stat.data_table_id = a.table_id                     where a.tenant_id = 0                     and a.table_type in (0, 1, 2, 3, 4, 14)                     and b.database_name != '__recyclebin'                     and b.in_recyclebin = 0                     and 0 = sys_privilege_check('table_acc', effective_tenant_id(), b.database_name, a.table_name) )__"))) {
      LOG_ERROR("fail to set view_definition", K(ret));
    }
  }
  table_schema.set_index_using_type(USING_BTREE);
