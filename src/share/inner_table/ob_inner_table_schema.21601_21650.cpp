...
int ObInnerTableSchema::dba_ob_vector_index_task_history_schema(ObTableSchema &table_schema)
{
  ...
  if (OB_SUCC(ret)) {
    if (OB_FAIL(table_schema.set_view_definition(R"__(   SELECT       table_id as TABLE_ID,       tablet_id as TABLET_ID,       task_id as TASK_ID,       gmt_create as CREATE_TIME,       gmt_modified as MODIFY_TIME,       case trigger_type         when 0 then "AUTO"         when 1 then "MANUAL"         else "INVALID" END AS TRIGGER_TYPE,       case status         when 0 then "PREPARED"         when 1 then "RUNNING"         when 2 then "PENDING"         when 3 then "FINISHED"         else "INVALID" END AS STATUS,       task_type as TASK_TYPE,       target_scn as TASK_SCN,       ret_code as RET_CODE,       trace_id as TRACE_ID,       task_info as TASK_INFO,       exec_addr as EXEC_ADDR,       priority as PRIORITY,       start_time as START_TIME,       end_time as END_TIME,       err_msg as ERR_MSG   FROM oceanbase.__all_vector_index_task_history )__"))) {
      LOG_ERROR("fail to set view_definition", K(ret));
    }
  }
  table_schema.set_index_using_type(USING_BTREE);
