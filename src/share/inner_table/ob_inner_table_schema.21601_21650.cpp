...
int ObInnerTableSchema::proc_schema(ObTableSchema &table_schema)
{
  ...
  if (OB_SUCC(ret)) {
    if (OB_FAIL(table_schema.set_view_definition(R"__(     SELECT       D.DATABASE_NAME AS DB,       R.ROUTINE_NAME AS NAME,       CAST((CASE R.ROUTINE_TYPE         WHEN 1 THEN 'PROCEDURE'         WHEN 2 THEN 'FUNCTION' END) AS CHAR(10)) AS TYPE,       R.ROUTINE_NAME AS SPECIFIC_NAME,       CAST('SQL' AS CHAR(4)) AS LANGUAGE,       CAST((CASE WHEN (R.FLAG & 32768) = 32768 THEN 'NO_SQL'                 WHEN (R.FLAG & 65536) = 65536 THEN 'READS_SQL_DATA'                 WHEN (R.FLAG & 131072) = 131072 THEN 'MODIFIES_SQL_DATA'                 ELSE 'CONTAINS_SQL' END) AS CHAR(32)) AS SQL_DATA_ACCESS,       CAST((CASE WHEN (R.FLAG & 4) = 4 THEN 'YES' ELSE 'NO' END) AS CHAR(4)) AS IS_DETERMINISTIC,       CAST((CASE WHEN (R.FLAG & 16) = 16 THEN 'INVOKER' ELSE 'DEFINER' END) AS CHAR(10)) AS SECURITY_TYPE,       MYSQL_PROC_INFO(R.ROUTINE_ID, 'PARAM_LIST') AS PARAM_LIST,       MYSQL_PROC_INFO(R.ROUTINE_ID, 'RETURNS') AS RETURNS,       MYSQL_PROC_INFO(R.ROUTINE_ID, 'BODY') AS BODY,       CAST(CONCAT('''', REPLACE(R.PRIV_USER, '@', '''@''' ), '''') AS CHAR(77)) AS DEFINER,       R.GMT_CREATE AS CREATED,       R.GMT_MODIFIED AS MODIFIED,       CAST(MYSQL_PROC_INFO(R.ROUTINE_ID, 'SQL_MODE') AS CHAR(8192)) AS SQL_MODE,       NVL(R.COMMENT, '') AS COMMENT,       CAST(MYSQL_PROC_INFO(R.ROUTINE_ID, 'CHARACTER_SET_CLIENT') AS CHAR(128)) AS CHARACTER_SET_CLIENT,       CAST(MYSQL_PROC_INFO(R.ROUTINE_ID, 'COLLATION_CONNECTION') AS CHAR(128)) AS COLLATION_CONNECTION,       CAST(MYSQL_PROC_INFO(R.ROUTINE_ID, 'DB_COLLATION') AS CHAR(128)) AS DB_COLLATION,       MYSQL_PROC_INFO(R.ROUTINE_ID, 'BODY') AS BODY_UTF8       FROM          ((SELECT * FROM oceanbase.__all_routine) R           LEFT JOIN oceanbase.__all_database D ON R.DATABASE_ID = D.DATABASE_ID)       WHERE         D.IN_RECYCLEBIN = 0       AND         R.ROUTINE_TYPE IN (1, 2)   )__"))) {
      LOG_ERROR("fail to set view_definition", K(ret));
    }
  }
  table_schema.set_index_using_type(USING_BTREE);
