...
int ObInnerTableSchema::all_virtual_ash_all_virtual_ash_i1_schema(ObTableSchema &table_schema)
{
  ...
  table_schema.set_max_used_column_id(column_id + 34);
  return ret;
}
...
int ObInnerTableSchema::all_virtual_kvcache_handle_leak_info_schema(ObTableSchema &table_schema)
{
  int ret = OB_SUCCESS;
  uint64_t column_id = OB_APP_MIN_COLUMN_ID - 1;
  ...
  table_schema.set_tablegroup_id(OB_INVALID_ID);
  table_schema.set_database_id(OB_SYS_DATABASE_ID);
  table_schema.set_table_id(OB_ALL_VIRTUAL_KVCACHE_HANDLE_LEAK_INFO_TID);
  table_schema.set_rowkey_split_pos(0);
  table_schema.set_is_use_bloomfilter(false);
  table_schema.set_progressive_merge_num(0);
  table_schema.set_rowkey_column_num(0);
  ...
  if (OB_SUCC(ret)) {
    if (OB_FAIL(table_schema.set_table_name(OB_ALL_VIRTUAL_KVCACHE_HANDLE_LEAK_INFO_TNAME))) {
      LOG_ERROR("fail to set table_name", K(ret));
    }
  }
