...
int ObInnerTableSchema::gv_merge_info_schema(ObTableSchema &table_schema)
{
  ...
  if (OB_SUCC(ret)) {
    if (OB_FAIL(table_schema.set_view_definition(R"__(     SELECT         SVR_IP,         SVR_PORT,         TENANT_ID,         INDEX_ID AS TABLE_ID,         PARTITION_ID,         CASE MERGE_TYPE WHEN 'MAJOR MERGE' THEN 'major' ELSE 'minor' END AS TYPE,         MERGE_TYPE AS ACTION,         CASE MERGE_TYPE WHEN 'MAJOR MERGE' THEN VERSION ELSE SNAPSHOT_VERSION END AS VERSION,         USEC_TO_TIME(MERGE_START_TIME) AS START_TIME,         USEC_TO_TIME(MERGE_FINISH_TIME) AS END_TIME,         MACRO_BLOCK_COUNT,         CASE MACRO_BLOCK_COUNT WHEN 0 THEN 0.00 ELSE ROUND(USE_OLD_MACRO_BLOCK_COUNT/MACRO_BLOCK_COUNT*100, 2) END AS REUSE_PCT,         TOTAL_CHILD_TASK AS PARALLEL_DEGREE     FROM OCEANBASE.__ALL_VIRTUAL_PARTITION_COMPACTION_HISTORY     WHERE         EFFECTIVE_TENANT_ID() = 1 OR tenant_id  = EFFECTIVE_TENANT_ID() )__"))) {
      LOG_ERROR("fail to set view_definition", K(ret));
    }
  }
  table_schema.set_index_using_type(USING_BTREE);
