...
int ObInnerTableSchema::dba_ob_tenants_ora_schema(ObTableSchema &table_schema)
{
  ...
  if (OB_SUCC(ret)) {
    if (OB_FAIL(table_schema.set_view_definition(R"__( SELECT A.TENANT_ID,        TENANT_NAME,        (CASE             WHEN A.TENANT_ID = 1 THEN 'SYS'             WHEN (MOD(A.TENANT_ID, 2)) = 1 THEN 'META'             ELSE 'USER'         END) AS TENANT_TYPE,        A.gmt_create AS CREATE_TIME,        A.gmt_modified AS MODIFY_TIME,        PRIMARY_ZONE,        LOCALITY,        (SELECT TG.TABLEGROUP_NAME           FROM SYS.ALL_VIRTUAL_TABLEGROUP_REAL_AGENT TG          WHERE TG.TENANT_ID = A.TENANT_ID            AND TG.TABLEGROUP_ID = A.DEFAULT_TABLEGROUP_ID) AS TABLEGROUP_NAME,        CASE previous_locality           WHEN '' THEN NULL           ELSE previous_locality        END AS PREVIOUS_LOCALITY,        CASE compatibility_mode           WHEN 0 THEN 'MYSQL'           WHEN 1 THEN 'ORACLE'           ELSE NULL        END AS COMPATIBILITY_MODE,        STATUS,        CASE in_recyclebin           WHEN 0 THEN 'NO'           ELSE 'YES'        END AS IN_RECYCLEBIN,         CASE locked           WHEN 0 THEN 'NO'           ELSE 'YES'        END AS LOCKED,         (CASE             WHEN A.TENANT_ID = 1 THEN 'PRIMARY'             WHEN (MOD(A.TENANT_ID, 2)) = 1 THEN 'PRIMARY'             ELSE TENANT_ROLE         END) AS TENANT_ROLE,         (CASE             WHEN A.TENANT_ID = 1 THEN 'NORMAL'             WHEN (MOD(A.TENANT_ID, 2)) = 1 THEN 'NORMAL'             ELSE SWITCHOVER_STATUS         END) AS SWITCHOVER_STATUS,         (CASE             WHEN A.TENANT_ID = 1 THEN 0             WHEN (MOD(A.TENANT_ID, 2)) = 1 THEN 0             ELSE SWITCHOVER_EPOCH         END) AS SWITCHOVER_EPOCH,         (CASE             WHEN A.TENANT_ID = 1 THEN NULL             WHEN (MOD(A.TENANT_ID, 2)) = 1 THEN NULL             ELSE SYNC_SCN         END) AS SYNC_SCN,         (CASE             WHEN A.TENANT_ID = 1 THEN NULL             WHEN (MOD(A.TENANT_ID, 2)) = 1 THEN NULL             ELSE REPLAYABLE_SCN         END) AS REPLAYABLE_SCN,         (CASE             WHEN A.TENANT_ID = 1 THEN NULL             WHEN (MOD(A.TENANT_ID, 2)) = 1 THEN NULL             ELSE READABLE_SCN         END) AS READABLE_SCN,         (CASE             WHEN A.TENANT_ID = 1 THEN NULL             WHEN (MOD(A.TENANT_ID, 2)) = 1 THEN NULL             ELSE RECOVERY_UNTIL_SCN         END) AS RECOVERY_UNTIL_SCN,         (CASE             WHEN A.TENANT_ID = 1 THEN 'NOARCHIVELOG'             WHEN (MOD(A.TENANT_ID, 2)) = 1 THEN 'NOARCHIVELOG'             ELSE LOG_MODE         END) AS LOG_MODE,        ARBITRATION_SERVICE_STATUS,        UNIT_NUM, ZONE_UNIT_NUM_LIST,        COMPATIBLE,        (CASE             WHEN (SELECT STARTUP_MODE() FROM DUAL) = 'shared_storage' THEN                 CASE                     WHEN (MOD(A.TENANT_ID, 2)) = 1 THEN 1001                     ELSE B.MAX_LS_ID                 END             ELSE                 CASE                     WHEN (MOD(A.TENANT_ID, 2)) = 1 THEN 1                     ELSE B.MAX_LS_ID                 END         END) AS MAX_LS_ID,        (CASE             WHEN A.TENANT_ID = 1 THEN 'NORMAL'             WHEN (MOD(A.TENANT_ID, 2)) = 1 THEN 'NORMAL'             ELSE RESTORE_DATA_MODE         END) AS RESTORE_DATA_MODE,        (CASE           WHEN A.TENANT_ID = 1 THEN NULL           WHEN (MOD(A.TENANT_ID, 2)) = 1 THEN NULL           ELSE E.LATEST_FLASHBACK_LOG_SCN END) AS FLASHBACK_LOG_SCN,         A.INFO AS "COMMENT",         B.PROTECTION_MODE AS PROTECTION_MODE,         (CASE             WHEN B.PROTECTION_LEVEL IS NULL THEN NULL             WHEN B.PROTECTION_MODE = 'MAXIMUM PERFORMANCE' AND B.PROTECTION_LEVEL = 'PRE MAXIMUM PERFORMANCE' THEN 'MAXIMUM PERFORMANCE'             WHEN B.PROTECTION_MODE = 'MAXIMUM PROTECTION' AND B.PROTECTION_LEVEL = 'PRE MAXIMUM PROTECTION' THEN 'MAXIMUM PERFORMANCE'             WHEN B.PROTECTION_MODE = 'MAXIMUM AVAILABILITY' AND B.PROTECTION_LEVEL = 'PRE MAXIMUM PROTECTION' THEN 'RESYNCHRONIZATION'             WHEN B.PROTECTION_MODE = 'MAXIMUM AVAILABILITY' AND B.PROTECTION_LEVEL = 'MAXIMUM PERFORMANCE' THEN 'RESYNCHRONIZATION'             WHEN B.PROTECTION_MODE = 'MAXIMUM AVAILABILITY' AND B.PROTECTION_LEVEL = 'PRE MAXIMUM PERFORMANCE' THEN 'RESYNCHRONIZATION'             ELSE B.PROTECTION_LEVEL END         ) AS PROTECTION_LEVEL FROM SYS.ALL_VIRTUAL_TENANT_SYS_AGENT A LEFT JOIN SYS.ALL_VIRTUAL_TENANT_INFO B     ON A.TENANT_ID = B.TENANT_ID LEFT JOIN     (SELECT TENANT_ID,             (CASE                  WHEN TENANT_ID < 1 THEN NULL                  WHEN TENANT_ID != 1 THEN TENANT_ID - 1                  ELSE NULL              END) AS META_TENANT_ID,             MIN(UNIT_COUNT) AS UNIT_NUM,             LISTAGG(ZONE || ':' || UNIT_COUNT, ',') WITHIN GROUP (ORDER BY ZONE) AS ZONE_UNIT_NUM_LIST      FROM (SELECT TENANT_ID, ZONE, COUNT(*) UNIT_COUNT FROM (            SELECT F.TENANT_ID, G.ZONE FROM SYS.ALL_VIRTUAL_RESOURCE_POOL_SYS_AGENT F            JOIN SYS.ALL_VIRTUAL_UNIT_SYS_AGENT G ON F.RESOURCE_POOL_ID = G.RESOURCE_POOL_ID            WHERE F.TENANT_ID > 0 AND G.STATUS != 'DELETING')            GROUP BY TENANT_ID, ZONE) GROUP BY TENANT_ID) C     ON A.TENANT_ID = C.TENANT_ID OR A.TENANT_ID = C.META_TENANT_ID LEFT JOIN     (SELECT TENANT_ID,             MIN(VALUE) AS COMPATIBLE      FROM SYS.ALL_VIRTUAL_TENANT_PARAMETER      WHERE NAME = 'compatible'      GROUP BY TENANT_ID) D     ON A.TENANT_ID = D.TENANT_ID LEFT JOIN     (SELECT TENANT_ID, MAX(FLASHBACK_LOG_SCN) AS LATEST_FLASHBACK_LOG_SCN      FROM SYS.ALL_VIRTUAL_TENANT_FLASHBACK_LOG_SCN GROUP BY TENANT_ID) E      ON A.TENANT_ID = E.TENANT_ID WHERE A.TENANT_ID = EFFECTIVE_TENANT_ID()   )__"))) {
      LOG_ERROR("fail to set view_definition", K(ret));
    }
  }
  table_schema.set_index_using_type(USING_BTREE);
...
int ObInnerTableSchema::dba_ob_table_opt_stat_gather_history_ora_schema(ObTableSchema &table_schema)
{
  ...
  if (OB_SUCC(ret)) {
    if (OB_FAIL(table_schema.set_view_definition(R"__(     SELECT         CAST(DB.DATABASE_NAME         AS     VARCHAR2(128)) AS OWNER,         CAST(V.TABLE_NAME             AS     VARCHAR2(256)) AS TABLE_NAME,         CAST(STAT.TASK_ID             AS     VARCHAR2(36)) AS TASK_ID,         CAST((CASE WHEN RET_CODE = 0 THEN 'SUCCESS'                 ELSE (CASE WHEN RET_CODE IS NULL THEN NULL                       ELSE (CASE WHEN RET_CODE = -5065 THEN 'CANCELED' ELSE 'FAILED' END) END) END) AS VARCHAR2(8)) AS STATUS,         CAST(STAT.START_TIME          AS     TIMESTAMP(6)) AS START_TIME,         CAST(STAT.END_TIME            AS     TIMESTAMP(6)) AS END_TIME,         CAST(STAT.MEMORY_USED         AS     NUMBER) AS MEMORY_USED,         CAST(STAT.STAT_REFRESH_FAILED_LIST      AS     VARCHAR2(4096)) AS STAT_REFRESH_FAILED_LIST,         CAST(STAT.PROPERTIES       AS     VARCHAR2(4096)) AS PROPERTIES     FROM     (         (SELECT TENANT_ID,                 DATABASE_ID,                 TABLE_ID,                 TABLE_ID AS PARTITION_ID,                 TABLE_NAME,                 NULL AS PARTITION_NAME,                 NULL AS SUBPARTITION_NAME,                 NULL AS PARTITION_POSITION,                 NULL AS SUBPARTITION_POSITION,                'TABLE' AS OBJECT_TYPE           FROM              SYS.ALL_VIRTUAL_CORE_ALL_TABLE         UNION ALL         SELECT TENANT_ID,                DATABASE_ID,                TABLE_ID,                CASE WHEN PART_LEVEL = 0 THEN TABLE_ID ELSE -1 END AS PARTITION_ID,                TABLE_NAME,                NULL AS PARTITION_NAME,                NULL AS SUBPARTITION_NAME,                NULL AS PARTITION_POSITION,                NULL AS SUBPARTITION_POSITION,                'TABLE' AS OBJECT_TYPE         FROM             SYS.ALL_VIRTUAL_TABLE_REAL_AGENT T         WHERE T.TABLE_TYPE IN (0,2,3,8,9,16,17)         AND BITAND((T.TABLE_MODE / 4096), 15) IN (0,1)         /*do not show deleting index*/         AND BITAND(T.INDEX_ATTRIBUTES_SET, 16) = 0)     ) V     JOIN         SYS.ALL_VIRTUAL_DATABASE_REAL_AGENT db         ON db.tenant_id = V.tenant_id         AND db.database_id = V.database_id         AND V.TENANT_ID = EFFECTIVE_TENANT_ID()         AND DB.TENANT_ID = EFFECTIVE_TENANT_ID()     LEFT JOIN         SYS.ALL_VIRTUAL_TABLE_OPT_STAT_GATHER_HISTORY STAT         ON V.TENANT_ID = STAT.TENANT_ID         AND V.TABLE_ID = STAT.TABLE_ID         AND STAT.TENANT_ID = EFFECTIVE_TENANT_ID() )__"))) {
      LOG_ERROR("fail to set view_definition", K(ret));
    }
  }
  table_schema.set_index_using_type(USING_BTREE);
