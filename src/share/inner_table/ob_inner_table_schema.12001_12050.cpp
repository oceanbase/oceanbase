...
int ObInnerTableSchema::tenant_virtual_external_table_error_schema(ObTableSchema &table_schema)
{
  int ret = OB_SUCCESS;
  uint64_t column_id = OB_APP_MIN_COLUMN_ID - 1;
  ...
  table_schema.set_tablegroup_id(OB_INVALID_ID);
  table_schema.set_database_id(OB_SYS_DATABASE_ID);
  table_schema.set_table_id(OB_TENANT_VIRTUAL_EXTERNAL_TABLE_ERROR_TID);
  table_schema.set_rowkey_split_pos(0);
  table_schema.set_is_use_bloomfilter(false);
  table_schema.set_progressive_merge_num(0);
  table_schema.set_rowkey_column_num(0);
  ...
  if (OB_SUCC(ret)) {
    if (OB_FAIL(table_schema.set_table_name(OB_TENANT_VIRTUAL_EXTERNAL_TABLE_ERROR_TNAME))) {
      LOG_ERROR("fail to set table_name", K(ret));
    }
  }
  ...
  if (OB_SUCC(ret)) {
    ObObj error_file_name_default;
    error_file_name_default.set_varchar(ObString::make_string(""));
    ADD_COLUMN_SCHEMA_T("error_file_name", //column_name
      ++column_id, //column_id
      0, //rowkey_id
      0, //index_id
      0, //part_key_pos
      ...
      false, //is_nullable
      false, //is_autoincrement
      error_file_name_default,
      error_file_name_default); //default_value
  }

  if (OB_SUCC(ret)) {
    ObObj error_column_name_default;
    error_column_name_default.set_varchar(ObString::make_string(""));
    ADD_COLUMN_SCHEMA_T("error_column_name", //column_name
      ++column_id, //column_id
      0, //rowkey_id
      0, //index_id
      0, //part_key_pos
      ...
      false, //is_nullable
      false, //is_autoincrement
      error_column_name_default,
      error_column_name_default); //default_value
  }

  if (OB_SUCC(ret)) {
    ObObj error_row_number_default;
    error_row_number_default.set_int(0);
    ADD_COLUMN_SCHEMA_T("error_row_number", //column_name
      ++column_id, //column_id
      0, //rowkey_id
      0, //index_id
      0, //part_key_pos
      ...
      false, //is_nullable
      false, //is_autoincrement
      error_row_number_default,
      error_row_number_default); //default_value
  }

  if (OB_SUCC(ret)) {
    ObObj error_data_default;
    error_data_default.set_varchar(ObString::make_string(""));
    ADD_COLUMN_SCHEMA_T("error_data", //column_name
      ++column_id, //column_id
      0, //rowkey_id
      0, //index_id
      0, //part_key_pos
      ...
      false, //is_autoincrement
      error_data_default,
      error_data_default); //default_value
  }

  if (OB_SUCC(ret)) {
    ObObj error_description_default;
    error_description_default.set_varchar(ObString::make_string(""));
    ADD_COLUMN_SCHEMA_T("error_description", //column_name
      ++column_id, //column_id
      0, //rowkey_id
      0, //index_id
      0, //part_key_pos
      ...
      error_description_default); //default_value
  }
  table_schema.set_index_using_type(USING_HASH);
  table_schema.set_row_store_type(ENCODING_ROW_STORE);
  table_schema.set_store_format(OB_STORE_FORMAT_DYNAMIC_MYSQL);
