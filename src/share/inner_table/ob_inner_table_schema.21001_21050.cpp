...
int ObInnerTableSchema::v_session_event_schema(ObTableSchema &table_schema)
{
  ...
  if (OB_SUCC(ret)) {
    if (OB_FAIL(table_schema.set_view_definition(R"__(select SID,CON_ID,EVENT_ID,EVENT,WAIT_CLASS_ID,                     `WAIT_CLASS#`,WAIT_CLASS,TOTAL_WAITS,TOTAL_TIMEOUTS,TIME_WAITED,                      MAX_WAIT,AVERAGE_WAIT,TIME_WAITED_MICRO                      from oceanbase.gv$session_event                      where SVR_IP=host_ip() and SVR_PORT=rpc_port() )__"))) {
      LOG_ERROR("fail to set view_definition", K(ret));
    }
  }
  table_schema.set_index_using_type(USING_BTREE);
...
int ObInnerTableSchema::v_session_wait_schema(ObTableSchema &table_schema)
{
  ...
  if (OB_SUCC(ret)) {
    if (OB_FAIL(table_schema.set_view_definition(R"__(select SID,CON_ID,EVENT,P1TEXT,P1,                      P2TEXT,P2,P3TEXT,P3,WAIT_CLASS_ID,                      `WAIT_CLASS#`,WAIT_CLASS,STATE,WAIT_TIME_MICRO,TIME_REMAINING_MICRO,                      TIME_SINCE_LAST_WAIT_MICRO                      from oceanbase.gv$session_wait                      where SVR_IP=host_ip() and SVR_PORT=rpc_port() )__"))) {
      LOG_ERROR("fail to set view_definition", K(ret));
    }
  }
  table_schema.set_index_using_type(USING_BTREE);
...
int ObInnerTableSchema::v_session_wait_history_schema(ObTableSchema &table_schema)
{
  ...
  if (OB_SUCC(ret)) {
    if (OB_FAIL(table_schema.set_view_definition(R"__(select SID,CON_ID,`SEQ#`,`EVENT#`,EVENT,                      P1TEXT,P1,P2TEXT,P2,P3TEXT,                      P3,WAIT_TIME_MICRO,TIME_SINCE_LAST_WAIT_MICRO,WAIT_TIME                      from oceanbase.gv$session_wait_history                      where SVR_IP=host_ip() and SVR_PORT=rpc_port() )__"))) {
      LOG_ERROR("fail to set view_definition", K(ret));
    }
  }
  table_schema.set_index_using_type(USING_BTREE);
...
int ObInnerTableSchema::v_sesstat_schema(ObTableSchema &table_schema)
{
  ...
  if (OB_SUCC(ret)) {
    if (OB_FAIL(table_schema.set_view_definition(R"__(select SID,CON_ID,`STATISTIC#`,VALUE from oceanbase.gv$sesstat                      where SVR_IP=host_ip() and SVR_PORT=rpc_port() )__"))) {
      LOG_ERROR("fail to set view_definition", K(ret));
    }
  }
  table_schema.set_index_using_type(USING_BTREE);
...
int ObInnerTableSchema::v_sysstat_schema(ObTableSchema &table_schema)
{
  ...
  if (OB_SUCC(ret)) {
    if (OB_FAIL(table_schema.set_view_definition(R"__(select CON_ID,`STATISTIC#`,VALUE,STAT_ID,NAME,CLASS from oceanbase.gv$sysstat                      where SVR_IP=host_ip() and SVR_PORT=rpc_port() )__"))) {
      LOG_ERROR("fail to set view_definition", K(ret));
    }
  }
  table_schema.set_index_using_type(USING_BTREE);
...
int ObInnerTableSchema::v_system_event_schema(ObTableSchema &table_schema)
{
  ...
  if (OB_SUCC(ret)) {
    if (OB_FAIL(table_schema.set_view_definition(R"__(select CON_ID,EVENT_ID,EVENT,WAIT_CLASS_ID,`WAIT_CLASS#`,                      WAIT_CLASS,TOTAL_WAITS,TOTAL_TIMEOUTS,TIME_WAITED,AVERAGE_WAIT,                      TIME_WAITED_MICRO                      from oceanbase.gv$system_event                      where SVR_IP=host_ip() and SVR_PORT=rpc_port() )__"))) {
      LOG_ERROR("fail to set view_definition", K(ret));
    }
  }
  table_schema.set_index_using_type(USING_BTREE);
...
int ObInnerTableSchema::v_memory_schema(ObTableSchema &table_schema)
{
  ...
  if (OB_SUCC(ret)) {
    if (OB_FAIL(table_schema.set_view_definition(R"__( SELECT     TENANT_ID, CONTEXT, COUNT, USED, ALLOC_COUNT, FREE_COUNT FROM     oceanbase.gv$memory WHERE         IP=HOST_IP()     AND         PORT=RPC_PORT() )__"))) {
      LOG_ERROR("fail to set view_definition", K(ret));
    }
  }
  table_schema.set_index_using_type(USING_BTREE);
...
int ObInnerTableSchema::v_memstore_schema(ObTableSchema &table_schema)
{
  ...
  if (OB_SUCC(ret)) {
    if (OB_FAIL(table_schema.set_view_definition(R"__( SELECT     TENANT_ID,     ACTIVE,     TOTAL,     `FREEZE_TRIGGER`,     `MEM_LIMIT` FROM     oceanbase.gv$memstore WHERE         IP=HOST_IP()     AND         PORT=RPC_PORT() )__"))) {
      LOG_ERROR("fail to set view_definition", K(ret));
    }
  }
  table_schema.set_index_using_type(USING_BTREE);
...
int ObInnerTableSchema::v_memstore_info_schema(ObTableSchema &table_schema)
{
  ...
  if (OB_SUCC(ret)) {
    if (OB_FAIL(table_schema.set_view_definition(R"__( SELECT     TENANT_ID,     IP,     PORT,     TABLE_ID,     PARTITION_ID,     VERSION,     BASE_VERSION,     MULTI_VERSION_START,     SNAPSHOT_VERSION,     IS_ACTIVE,     USED,     HASH_ITEMS,     BTREE_ITEMS FROM     oceanbase.gv$memstore_info WHERE         IP=HOST_IP()     AND         PORT=RPC_PORT() )__"))) {
      LOG_ERROR("fail to set view_definition", K(ret));
    }
  }
  table_schema.set_index_using_type(USING_BTREE);
...
int ObInnerTableSchema::v_plan_cache_stat_schema(ObTableSchema &table_schema)
{
  ...
  if (OB_SUCC(ret)) {
    if (OB_FAIL(table_schema.set_view_definition(R"__(   SELECT tenant_id,svr_ip,svr_port,sql_num,mem_used, mem_hold, access_count,hit_count,   hit_rate,plan_num,mem_limit,hash_bucket,stmtkey_num   FROM oceanbase.gv$plan_cache_stat WHERE svr_ip=HOST_IP() AND svr_port=RPC_PORT() )__"))) {
      LOG_ERROR("fail to set view_definition", K(ret));
    }
  }
  table_schema.set_index_using_type(USING_BTREE);
...
int ObInnerTableSchema::gv_plan_cache_plan_explain_schema(ObTableSchema &table_schema)
{
  ...
  if (OB_SUCC(ret)) {
    if (OB_FAIL(table_schema.set_view_definition(R"__(     SELECT TENANT_ID,                             SVR_IP as IP,                             SVR_PORT as PORT,                             PLAN_ID,                             PLAN_DEPTH,                             PLAN_LINE_ID,                             OPERATOR,                             NAME,                             ROWS,                             COST,                             PROPERTY                             FROM oceanbase.__all_virtual_plan_cache_plan_explain                             WHERE is_serving_tenant(svr_ip, svr_port, effective_tenant_id()) and (EFFECTIVE_TENANT_ID()=1 or TENANT_ID=EFFECTIVE_TENANT_ID()) )__"))) {
      LOG_ERROR("fail to set view_definition", K(ret));
    }
  }
  table_schema.set_index_using_type(USING_BTREE);
...
int ObInnerTableSchema::v_plan_cache_plan_explain_schema(ObTableSchema &table_schema)
{
  ...
  if (OB_SUCC(ret)) {
    if (OB_FAIL(table_schema.set_view_definition(R"__(     SELECT * FROM oceanbase.gv$plan_cache_plan_explain WHERE IP =HOST_IP() AND PORT = RPC_PORT() )__"))) {
      LOG_ERROR("fail to set view_definition", K(ret));
    }
  }
  table_schema.set_index_using_type(USING_BTREE);
...
int ObInnerTableSchema::v_sql_audit_schema(ObTableSchema &table_schema)
{
  ...
  if (OB_SUCC(ret)) {
    if (OB_FAIL(table_schema.set_view_definition(R"__(SELECT * FROM oceanbase.gv$sql_audit WHERE svr_ip=HOST_IP() AND svr_port=RPC_PORT() )__"))) {
      LOG_ERROR("fail to set view_definition", K(ret));
    }
  }
  table_schema.set_index_using_type(USING_BTREE);
...
int ObInnerTableSchema::v_latch_schema(ObTableSchema &table_schema)
{
  ...
  if (OB_SUCC(ret)) {
    if (OB_FAIL(table_schema.set_view_definition(R"__(SELECT * FROM oceanbase.gv$latch where SVR_IP=host_ip() and SVR_PORT=rpc_port() )__"))) {
      LOG_ERROR("fail to set view_definition", K(ret));
    }
  }
  table_schema.set_index_using_type(USING_BTREE);
...
int ObInnerTableSchema::gv_obrpc_outgoing_schema(ObTableSchema &table_schema)
{
  ...
  if (OB_SUCC(ret)) {
    if (OB_FAIL(table_schema.set_view_definition(R"__( SELECT     TENANT_ID,     SVR_IP AS IP,     SVR_PORT AS PORT,     PCODE,     PCODE_NAME,     COUNT,     TOTAL_TIME,     TOTAL_SIZE,     FAILURE,     TIMEOUT,     SYNC,     ASYNC,     LAST_TIMESTAMP FROM     oceanbase.__all_virtual_obrpc_stat WHERE     is_serving_tenant(svr_ip, svr_port, effective_tenant_id())     AND         (EFFECTIVE_TENANT_ID()=1     OR         TENANT_ID=EFFECTIVE_TENANT_ID()) )__"))) {
      LOG_ERROR("fail to set view_definition", K(ret));
    }
  }
  table_schema.set_index_using_type(USING_BTREE);
...
int ObInnerTableSchema::v_obrpc_outgoing_schema(ObTableSchema &table_schema)
{
  ...
  if (OB_SUCC(ret)) {
    if (OB_FAIL(table_schema.set_view_definition(R"__( SELECT     * FROM     gv$obrpc_outgoing WHERE         IP=HOST_IP()     AND         PORT=RPC_PORT() )__"))) {
      LOG_ERROR("fail to set view_definition", K(ret));
    }
  }
  table_schema.set_index_using_type(USING_BTREE);
...
int ObInnerTableSchema::v_obrpc_incoming_schema(ObTableSchema &table_schema)
{
  ...
  if (OB_SUCC(ret)) {
    if (OB_FAIL(table_schema.set_view_definition(R"__( SELECT     * FROM     oceanbase.gv$obrpc_incoming WHERE         IP=HOST_IP()     AND         PORT=RPC_PORT() )__"))) {
      LOG_ERROR("fail to set view_definition", K(ret));
    }
  }
  table_schema.set_index_using_type(USING_BTREE);
...
int ObInnerTableSchema::v_sql_schema(ObTableSchema &table_schema)
{
  ...
  if (OB_SUCC(ret)) {
    if (OB_FAIL(table_schema.set_view_definition(R"__(SELECT * FROM oceanbase.gv$sql         WHERE svr_ip=HOST_IP() AND svr_port=RPC_PORT() )__"))) {
      LOG_ERROR("fail to set view_definition", K(ret));
    }
  }
  table_schema.set_index_using_type(USING_BTREE);
...
int ObInnerTableSchema::v_sql_monitor_schema(ObTableSchema &table_schema)
{
  ...
  if (OB_SUCC(ret)) {
    if (OB_FAIL(table_schema.set_view_definition(R"__(SELECT * from oceanbase.gv$sql_monitor  WHERE svr_ip=HOST_IP() AND     svr_port=RPC_Port() )__"))) {
      LOG_ERROR("fail to set view_definition", K(ret));
    }
  }
  table_schema.set_index_using_type(USING_BTREE);
...
int ObInnerTableSchema::user_recyclebin_schema(ObTableSchema &table_schema)
{
  ...
  if (OB_SUCC(ret)) {
    if (OB_FAIL(table_schema.set_view_definition(R"__( SELECT     OBJECT_NAME,     ORIGINAL_NAME,     case TYPE when 1 then 'TABLE' when 2 then 'INDEX' when 3 then 'VIEW' when 4 then 'DATABASE' else 'INVALID' end as TYPE,     gmt_create as CREATETIME FROM     oceanbase.__all_virtual_recyclebin where tenant_id = effective_tenant_id() )__"))) {
      LOG_ERROR("fail to set view_definition", K(ret));
    }
  }
  table_schema.set_index_using_type(USING_BTREE);
...
int ObInnerTableSchema::gv_concurrent_limit_sql_schema(ObTableSchema &table_schema)
{
  ...
  if (OB_SUCC(ret)) {
    if (OB_FAIL(table_schema.set_view_definition(R"__(     SELECT * from oceanbase.__tenant_virtual_concurrent_limit_sql )__"))) {
      LOG_ERROR("fail to set view_definition", K(ret));
    }
  }
  table_schema.set_index_using_type(USING_BTREE);
...
int ObInnerTableSchema::v_sql_plan_statistics_schema(ObTableSchema &table_schema)
{
  ...
  if (OB_SUCC(ret)) {
    if (OB_FAIL(table_schema.set_view_definition(R"__(SELECT * from oceanbase.gv$sql_plan_statistics WHERE svr_ip=HOST_IP() AND     svr_port=RPC_Port() )__"))) {
      LOG_ERROR("fail to set view_definition", K(ret));
    }
  }
  table_schema.set_index_using_type(USING_BTREE);
...
int ObInnerTableSchema::gv_server_memstore_schema(ObTableSchema &table_schema)
{
  ...
  if (OB_SUCC(ret)) {
    if (OB_FAIL(table_schema.set_view_definition(R"__( SELECT     SVR_IP AS IP,     SVR_PORT AS PORT,     ACTIVE_MEMSTORE_USED AS ACTIVE,     TOTAL_MEMSTORE_USED AS TOTAL,     MAJOR_FREEZE_TRIGGER AS `FREEZE_TRIGGER`,     MEMSTORE_LIMIT AS `MEM_LIMIT` FROM     oceanbase.__all_virtual_server_memory_info WHERE         (EFFECTIVE_TENANT_ID()=1) )__"))) {
      LOG_ERROR("fail to set view_definition", K(ret));
    }
  }
  table_schema.set_index_using_type(USING_BTREE);
...
int ObInnerTableSchema::gv_tenant_schema(ObTableSchema &table_schema)
{
  ...
  if (OB_SUCC(ret)) {
    if (OB_FAIL(table_schema.set_view_definition(R"__(     SELECT tenant_id,            tenant_name,            zone_list,            primary_zone,            collation_type,            info,            read_only,            locality     FROM oceanbase.__all_tenant     WHERE effective_tenant_id() = 1 OR tenant_id = effective_tenant_id() )__"))) {
      LOG_ERROR("fail to set view_definition", K(ret));
    }
  }
  table_schema.set_index_using_type(USING_BTREE);
...
int ObInnerTableSchema::v_unit_schema(ObTableSchema &table_schema)
{
  ...
  if (OB_SUCC(ret)) {
    if (OB_FAIL(table_schema.set_view_definition(R"__(     SELECT *     FROM oceanbase.gv$unit     WHERE svr_ip = host_ip() AND svr_port = rpc_port() )__"))) {
      LOG_ERROR("fail to set view_definition", K(ret));
    }
  }
  table_schema.set_index_using_type(USING_BTREE);
