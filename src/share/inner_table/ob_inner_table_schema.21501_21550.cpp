...
int ObInnerTableSchema::cdb_ob_ls_replica_task_history_schema(ObTableSchema &table_schema)
{
  ...
  if (OB_SUCC(ret)) {
    if (OB_FAIL(table_schema.set_view_definition(R"__(   (   SELECT TENANT_ID,          LS_ID,          TASK_TYPE,          TASK_ID,          TASK_STATUS,          CAST(CASE PRIORITY               WHEN 0 THEN 'HIGH'               WHEN 1 THEN 'LOW'               ELSE NULL END AS CHAR(5)) AS PRIORITY,          TARGET_REPLICA_SVR_IP,          TARGET_REPLICA_SVR_PORT,          TARGET_PAXOS_REPLICA_NUMBER,          TARGET_REPLICA_TYPE,          (CASE SOURCE_REPLICA_SVR_IP               WHEN "" THEN NULL               ELSE SOURCE_REPLICA_SVR_IP END) AS SOURCE_REPLICA_SVR_IP,          SOURCE_REPLICA_SVR_PORT,          SOURCE_PAXOS_REPLICA_NUMBER,          (CASE SOURCE_REPLICA_TYPE               WHEN "" THEN NULL               ELSE SOURCE_REPLICA_TYPE END) AS SOURCE_REPLICA_TYPE,          (CASE DATA_SOURCE_SVR_IP               WHEN "" THEN NULL               ELSE DATA_SOURCE_SVR_IP END) AS DATA_SOURCE_SVR_IP,          DATA_SOURCE_SVR_PORT,          CAST(CASE IS_MANUAL               WHEN 0 THEN 'FALSE'               WHEN 1 THEN 'TRUE'               ELSE NULL END AS CHAR(6)) AS IS_MANUAL,          TASK_EXEC_SVR_IP,          TASK_EXEC_SVR_PORT,          CAST(GMT_CREATE AS DATETIME) AS CREATE_TIME,          CAST(SCHEDULE_TIME AS DATETIME) AS START_TIME,          CAST(GMT_MODIFIED AS DATETIME) AS MODIFY_TIME,          CAST(FINISH_TIME AS DATETIME) AS FINISH_TIME,          (CASE EXECUTE_RESULT               WHEN "" THEN NULL               ELSE EXECUTE_RESULT END) AS EXECUTE_RESULT,          COMMENT,          CONFIG_VERSION   FROM OCEANBASE.__ALL_VIRTUAL_LS_REPLICA_TASK_HISTORY   )   UNION   (   SELECT TENANT_ID,          LS_ID,          TASK_TYPE,          TASK_ID,          TASK_STATUS,          CAST(CASE PRIORITY               WHEN 0 THEN 'HIGH'               WHEN 1 THEN 'LOW'               ELSE NULL END AS CHAR(5)) AS PRIORITY,          TARGET_REPLICA_SVR_IP,          TARGET_REPLICA_SVR_PORT,          TARGET_PAXOS_REPLICA_NUMBER,          TARGET_REPLICA_TYPE,          (CASE SOURCE_REPLICA_SVR_IP               WHEN "" THEN NULL               ELSE SOURCE_REPLICA_SVR_IP END) AS SOURCE_REPLICA_SVR_IP,          SOURCE_REPLICA_SVR_PORT,          SOURCE_PAXOS_REPLICA_NUMBER,          (CASE SOURCE_REPLICA_TYPE               WHEN "" THEN NULL               ELSE SOURCE_REPLICA_TYPE END) AS SOURCE_REPLICA_TYPE,          (CASE DATA_SOURCE_SVR_IP               WHEN "" THEN NULL               ELSE DATA_SOURCE_SVR_IP END) AS DATA_SOURCE_SVR_IP,          DATA_SOURCE_SVR_PORT,          CAST(CASE IS_MANUAL               WHEN 0 THEN 'FALSE'               WHEN 1 THEN 'TRUE'               ELSE NULL END AS CHAR(6)) AS IS_MANUAL,          TASK_EXEC_SVR_IP,          TASK_EXEC_SVR_PORT,          CAST(GMT_CREATE AS DATETIME) AS CREATE_TIME,          CAST(SCHEDULE_TIME AS DATETIME) AS START_TIME,          CAST(GMT_MODIFIED AS DATETIME) AS MODIFY_TIME,          CAST(FINISH_TIME AS DATETIME) AS FINISH_TIME,          (CASE EXECUTE_RESULT               WHEN "" THEN NULL               ELSE EXECUTE_RESULT END) AS EXECUTE_RESULT,          COMMENT,          CONFIG_VERSION   FROM OCEANBASE.__ALL_LS_REPLICA_TASK_HISTORY WHERE TASK_TYPE = 'REPLACE REPLICA' AND TENANT_ID != 1   )   )__"))) {
      LOG_ERROR("fail to set view_definition", K(ret));
    }
  }
  table_schema.set_index_using_type(USING_BTREE);
...
int ObInnerTableSchema::cdb_mvref_run_stats_schema(ObTableSchema &table_schema)
{
  ...
  if (OB_SUCC(ret)) {
    if (OB_FAIL(table_schema.set_view_definition(R"__(     SELECT       B.TENANT_ID AS TENANT_ID,       CAST(A.USER_NAME AS CHAR(128)) AS RUN_OWNER,       B.REFRESH_ID AS REFRESH_ID,       CAST(C.NUM_MVS AS SIGNED) AS NUM_MVS,       CAST(C.MVIEWS AS CHAR(4000)) AS MVIEWS,       CAST(D.BASE_TABLES AS CHAR(4000)) AS BASE_TABLES,       CAST(B.METHOD AS CHAR(4000)) AS METHOD,       CAST(B.ROLLBACK_SEG AS CHAR(4000)) AS ROLLBACK_SEG,       CAST(IF(B.PUSH_DEFERRED_RPC = 1, 'Y', 'N') AS CHAR(1)) AS PUSH_DEFERRED_RPC,       CAST(IF(B.REFRESH_AFTER_ERRORS = 1, 'Y', 'N') AS CHAR(1)) AS REFRESH_AFTER_ERRORS,       B.PURGE_OPTION AS PURGE_OPTION,       B.PARALLELISM AS PARALLELISM,       B.HEAP_SIZE AS HEAP_SIZE,       CAST(IF(B.ATOMIC_REFRESH = 1, 'Y', 'N') AS CHAR(1)) AS ATOMIC_REFRESH,       CAST(IF(B.NESTED = 1, 'Y', 'N') AS CHAR(1)) AS NESTED,       CAST(IF(B.OUT_OF_PLACE = 1, 'Y', 'N') AS CHAR(1)) AS OUT_OF_PLACE,       B.NUMBER_OF_FAILURES AS NUMBER_OF_FAILURES,       B.RESULT AS RESULT,       CAST(B.START_TIME AS DATETIME) AS START_TIME,       CAST(B.END_TIME AS DATETIME) AS END_TIME,       B.ELAPSED_TIME AS ELAPSED_TIME,       CAST(0 AS SIGNED) AS LOG_SETUP_TIME,       B.LOG_PURGE_TIME AS LOG_PURGE_TIME,       CAST(IF(B.COMPLETE_STATS_AVALIABLE = 1, 'Y', 'N') AS CHAR(1)) AS COMPLETE_STATS_AVAILABLE,       B.MVIEW_ID AS MVIEW_ID,       CAST(MT.TABLE_NAME AS CHAR(128)) AS MVIEW_NAME,       B.DATA_TARGET_SCN AS DATA_TARGET_SCN,       B.TRACE_ID AS TRACE_ID     FROM       oceanbase.__all_virtual_user A,       oceanbase.__all_virtual_mview_refresh_run_stats B       LEFT JOIN (         SELECT           C1.TENANT_ID AS TENANT_ID,           C1.REFRESH_ID AS REFRESH_ID,           COUNT(DISTINCT C1.MVIEW_ID) AS NUM_MVS,           GROUP_CONCAT(DISTINCT C2.TABLE_NAME ORDER BY C2.TABLE_NAME SEPARATOR ',') AS MVIEWS         FROM           oceanbase.__all_virtual_mview_refresh_stats C1,           oceanbase.__all_virtual_table C2         WHERE C1.TENANT_ID = C2.TENANT_ID           AND C1.MVIEW_ID = C2.TABLE_ID         GROUP BY TENANT_ID, REFRESH_ID       ) C ON B.TENANT_ID = C.TENANT_ID AND B.REFRESH_ID = C.REFRESH_ID       LEFT JOIN (         SELECT           D1.TENANT_ID AS TENANT_ID,           D1.REFRESH_ID AS REFRESH_ID,           GROUP_CONCAT(DISTINCT D2.TABLE_NAME ORDER BY D2.TABLE_NAME SEPARATOR ',') AS BASE_TABLES         FROM           oceanbase.__all_virtual_mview_refresh_change_stats D1,           oceanbase.__all_virtual_table D2             LEFT JOIN oceanbase.__all_virtual_table D3               ON D3.TENANT_ID = D2.TENANT_ID               AND D3.DATA_TABLE_ID = D2.TABLE_ID               AND D3.TABLE_TYPE = 7         WHERE D1.TENANT_ID = D2.TENANT_ID           AND D1.DETAIL_TABLE_ID = D2.TABLE_ID           AND D3.TABLE_ID IS NULL         GROUP BY TENANT_ID, REFRESH_ID       ) D ON B.TENANT_ID = D.TENANT_ID AND B.REFRESH_ID = D.REFRESH_ID       JOIN oceanbase.__all_virtual_table MT ON B.TENANT_ID = MT.TENANT_ID         AND B.MVIEW_ID = MT.TABLE_ID AND MT.TABLE_TYPE = 7     WHERE A.TENANT_ID = B.TENANT_ID       AND A.USER_ID = B.RUN_USER_ID )__"))) {
      LOG_ERROR("fail to set view_definition", K(ret));
    }
  }
  table_schema.set_index_using_type(USING_BTREE);
...
int ObInnerTableSchema::dba_mvref_run_stats_schema(ObTableSchema &table_schema)
{
  ...
  if (OB_SUCC(ret)) {
    if (OB_FAIL(table_schema.set_view_definition(R"__(     SELECT       CAST(A.USER_NAME AS CHAR(128)) AS RUN_OWNER,       B.REFRESH_ID AS REFRESH_ID,       CAST(C.NUM_MVS AS SIGNED) AS NUM_MVS,       CAST(C.MVIEWS AS CHAR(4000)) AS MVIEWS,       CAST(D.BASE_TABLES AS CHAR(4000)) AS BASE_TABLES,       CAST(B.METHOD AS CHAR(4000)) AS METHOD,       CAST(B.ROLLBACK_SEG AS CHAR(4000)) AS ROLLBACK_SEG,       CAST(IF(B.PUSH_DEFERRED_RPC = 1, 'Y', 'N') AS CHAR(1)) AS PUSH_DEFERRED_RPC,       CAST(IF(B.REFRESH_AFTER_ERRORS = 1, 'Y', 'N') AS CHAR(1)) AS REFRESH_AFTER_ERRORS,       B.PURGE_OPTION AS PURGE_OPTION,       B.PARALLELISM AS PARALLELISM,       B.HEAP_SIZE AS HEAP_SIZE,       CAST(IF(B.ATOMIC_REFRESH = 1, 'Y', 'N') AS CHAR(1)) AS ATOMIC_REFRESH,       CAST(IF(B.NESTED = 1, 'Y', 'N') AS CHAR(1)) AS NESTED,       CAST(IF(B.OUT_OF_PLACE = 1, 'Y', 'N') AS CHAR(1)) AS OUT_OF_PLACE,       B.NUMBER_OF_FAILURES AS NUMBER_OF_FAILURES,       B.RESULT AS RESULT,       CAST(B.START_TIME AS DATETIME) AS START_TIME,       CAST(B.END_TIME AS DATETIME) AS END_TIME,       B.ELAPSED_TIME AS ELAPSED_TIME,       CAST(0 AS SIGNED) AS LOG_SETUP_TIME,       B.LOG_PURGE_TIME AS LOG_PURGE_TIME,       CAST(IF(B.COMPLETE_STATS_AVALIABLE = 1, 'Y', 'N') AS CHAR(1)) AS COMPLETE_STATS_AVAILABLE,       B.MVIEW_ID AS MVIEW_ID,       CAST(MT.TABLE_NAME AS CHAR(128)) AS MVIEW_NAME,       B.DATA_TARGET_SCN AS DATA_TARGET_SCN,       B.TRACE_ID AS TRACE_ID     FROM       oceanbase.__all_user A,       oceanbase.__all_mview_refresh_run_stats B       LEFT JOIN (         SELECT           C1.TENANT_ID AS TENANT_ID,           C1.REFRESH_ID AS REFRESH_ID,           COUNT(DISTINCT C1.MVIEW_ID) AS NUM_MVS,           GROUP_CONCAT(DISTINCT C2.TABLE_NAME ORDER BY C2.TABLE_NAME SEPARATOR ',') AS MVIEWS         FROM           oceanbase.__all_mview_refresh_stats C1,           oceanbase.__all_table C2         WHERE C1.TENANT_ID = C2.TENANT_ID           AND C1.MVIEW_ID = C2.TABLE_ID         GROUP BY TENANT_ID, REFRESH_ID       ) C ON B.TENANT_ID = C.TENANT_ID AND B.REFRESH_ID = C.REFRESH_ID       LEFT JOIN (         SELECT           D1.TENANT_ID AS TENANT_ID,           D1.REFRESH_ID AS REFRESH_ID,           GROUP_CONCAT(DISTINCT D2.TABLE_NAME ORDER BY D2.TABLE_NAME SEPARATOR ',') AS BASE_TABLES         FROM           oceanbase.__all_mview_refresh_change_stats D1,           oceanbase.__all_table D2             LEFT JOIN oceanbase.__all_table D3               ON D3.TENANT_ID = D2.TENANT_ID               AND D3.DATA_TABLE_ID = D2.TABLE_ID               AND D3.TABLE_TYPE = 7         WHERE D1.TENANT_ID = D2.TENANT_ID           AND D1.DETAIL_TABLE_ID = D2.TABLE_ID           AND D3.TABLE_ID IS NULL         GROUP BY TENANT_ID, REFRESH_ID       ) D ON B.TENANT_ID = D.TENANT_ID AND B.REFRESH_ID = D.REFRESH_ID       JOIN oceanbase.__all_table MT ON B.TENANT_ID = MT.TENANT_ID         AND B.MVIEW_ID = MT.TABLE_ID AND MT.TABLE_TYPE = 7     WHERE A.TENANT_ID = B.TENANT_ID       AND A.USER_ID = B.RUN_USER_ID )__"))) {
      LOG_ERROR("fail to set view_definition", K(ret));
    }
  }
  table_schema.set_index_using_type(USING_BTREE);
...
int ObInnerTableSchema::cdb_mvref_stats_schema(ObTableSchema &table_schema)
{
  ...
  if (OB_SUCC(ret)) {
    if (OB_FAIL(table_schema.set_view_definition(R"__(     SELECT/*+opt_param('enable_optimizer_rowgoal','off')*/       B.TENANT_ID AS TENANT_ID,       CAST(A.DATABASE_NAME AS CHAR(128)) AS MV_OWNER,       CAST(B.TABLE_NAME AS CHAR(128)) AS MV_NAME,       C.REFRESH_ID AS REFRESH_ID,       CAST(         CASE C.REFRESH_TYPE           WHEN 0 THEN 'COMPLETE'           WHEN 1 THEN 'FAST'           ELSE NULL         END AS CHAR(30)       ) AS REFRESH_METHOD,       CAST(NULL AS CHAR(4000)) AS REFRESH_OPTIMIZATIONS,       CAST(NULL AS CHAR(4000)) AS ADDITIONAL_EXECUTIONS,       CAST(C.START_TIME AS DATETIME) AS START_TIME,       CAST(C.END_TIME AS DATETIME) AS END_TIME,       C.ELAPSED_TIME AS ELAPSED_TIME,       CAST(0 AS SIGNED) AS LOG_SETUP_TIME,       C.LOG_PURGE_TIME AS LOG_PURGE_TIME,       C.INITIAL_NUM_ROWS AS INITIAL_NUM_ROWS,       C.FINAL_NUM_ROWS AS FINAL_NUM_ROWS,       C.NUM_STEPS AS NUM_STEPS,       C.RESULT AS RESULT,       C.REFRESH_SCN AS REFRESH_SCN,       CAST(C.SVR_IP AS CHAR(64)) AS SVR_IP,       C.SVR_PORT AS SVR_PORT     FROM       oceanbase.__all_virtual_database A,       oceanbase.__all_virtual_table B,       oceanbase.__all_virtual_mview_refresh_stats C     WHERE A.TENANT_ID = B.TENANT_ID       AND A.DATABASE_ID = B.DATABASE_ID       AND B.TENANT_ID = C.TENANT_ID       AND B.TABLE_ID = C.MVIEW_ID       AND B.TABLE_TYPE = 7 )__"))) {
      LOG_ERROR("fail to set view_definition", K(ret));
    }
  }
  table_schema.set_index_using_type(USING_BTREE);
...
int ObInnerTableSchema::cdb_mvref_change_stats_schema(ObTableSchema &table_schema)
{
  ...
  if (OB_SUCC(ret)) {
    if (OB_FAIL(table_schema.set_view_definition(R"__(     SELECT       E.TENANT_ID AS TENANT_ID,       CAST(C.DATABASE_NAME AS CHAR(128)) AS TBL_OWNER,       CAST(IFNULL(F.TABLE_NAME, D.TABLE_NAME) AS CHAR(128)) AS TBL_NAME,       CAST(A.DATABASE_NAME AS CHAR(128)) AS MV_OWNER,       CAST(B.TABLE_NAME AS CHAR(128)) AS MV_NAME,       E.REFRESH_ID AS REFRESH_ID,       E.RETRY_ID AS RETRY_ID,       E.NUM_ROWS_INS AS NUM_ROWS_INS,       E.NUM_ROWS_UPD AS NUM_ROWS_UPD,       E.NUM_ROWS_DEL AS NUM_ROWS_DEL,       CAST(0 AS SIGNED) AS NUM_ROWS_DL_INS,       CAST('N' AS CHAR(1)) AS PMOPS_OCCURRED,       CAST(NULL AS CHAR(4000)) AS PMOP_DETAILS,       E.NUM_ROWS AS NUM_ROWS     FROM       oceanbase.__all_virtual_database A,       oceanbase.__all_virtual_table B,       oceanbase.__all_virtual_database C,       oceanbase.__all_virtual_table D         LEFT JOIN oceanbase.__all_virtual_table F           ON F.TENANT_ID = D.TENANT_ID           AND F.DATA_TABLE_ID = D.TABLE_ID           AND F.TABLE_TYPE = 7,       oceanbase.__all_virtual_mview_refresh_change_stats E     WHERE A.TENANT_ID = B.TENANT_ID       AND A.DATABASE_ID = B.DATABASE_ID       AND C.TENANT_ID = D.TENANT_ID       AND C.DATABASE_ID = D.DATABASE_ID       AND E.TENANT_ID = B.TENANT_ID       AND E.MVIEW_ID = B.TABLE_ID       AND E.TENANT_ID = D.TENANT_ID       AND E.DETAIL_TABLE_ID = D.TABLE_ID )__"))) {
      LOG_ERROR("fail to set view_definition", K(ret));
    }
  }
  table_schema.set_index_using_type(USING_BTREE);
...
int ObInnerTableSchema::dba_mvref_change_stats_schema(ObTableSchema &table_schema)
{
  ...
  if (OB_SUCC(ret)) {
    if (OB_FAIL(table_schema.set_view_definition(R"__(     SELECT       CAST(C.DATABASE_NAME AS CHAR(128)) AS TBL_OWNER,       CAST(IFNULL(F.TABLE_NAME, D.TABLE_NAME) AS CHAR(128)) AS TBL_NAME,       CAST(A.DATABASE_NAME AS CHAR(128)) AS MV_OWNER,       CAST(B.TABLE_NAME AS CHAR(128)) AS MV_NAME,       E.REFRESH_ID AS REFRESH_ID,       E.RETRY_ID AS RETRY_ID,       E.NUM_ROWS_INS AS NUM_ROWS_INS,       E.NUM_ROWS_UPD AS NUM_ROWS_UPD,       E.NUM_ROWS_DEL AS NUM_ROWS_DEL,       CAST(0 AS SIGNED) AS NUM_ROWS_DL_INS,       CAST('N' AS CHAR(1)) AS PMOPS_OCCURRED,       CAST(NULL AS CHAR(4000)) AS PMOP_DETAILS,       E.NUM_ROWS AS NUM_ROWS     FROM       oceanbase.__all_database A,       oceanbase.__all_table B,       oceanbase.__all_database C,       oceanbase.__all_table D         LEFT JOIN oceanbase.__all_table F           ON F.TENANT_ID = D.TENANT_ID           AND F.DATA_TABLE_ID = D.TABLE_ID           AND F.TABLE_TYPE = 7,       oceanbase.__all_mview_refresh_change_stats E     WHERE A.TENANT_ID = B.TENANT_ID       AND A.DATABASE_ID = B.DATABASE_ID       AND C.TENANT_ID = D.TENANT_ID       AND C.DATABASE_ID = D.DATABASE_ID       AND E.TENANT_ID = B.TENANT_ID       AND E.MVIEW_ID = B.TABLE_ID       AND E.TENANT_ID = D.TENANT_ID       AND E.DETAIL_TABLE_ID = D.TABLE_ID )__"))) {
      LOG_ERROR("fail to set view_definition", K(ret));
    }
  }
  table_schema.set_index_using_type(USING_BTREE);
...
int ObInnerTableSchema::cdb_mvref_stmt_stats_schema(ObTableSchema &table_schema)
{
  ...
  if (OB_SUCC(ret)) {
    if (OB_FAIL(table_schema.set_view_definition(R"__(     SELECT       C.TENANT_ID AS TENANT_ID,       CAST(A.DATABASE_NAME AS CHAR(128)) AS MV_OWNER,       CAST(B.TABLE_NAME AS CHAR(128)) AS MV_NAME,       C.REFRESH_ID AS REFRESH_ID,       C.RETRY_ID AS RETRY_ID,       C.STEP AS STEP,       CAST(C.SQLID AS CHAR(32)) AS SQLID,       C.STMT AS STMT,       CAST(C.START_TIME AS DATETIME) AS START_TIME,       C.EXECUTION_TIME AS EXECUTION_TIME,       C.EXECUTION_PLAN AS EXECUTION_PLAN,       C.RESULT AS RESULT,       C.PARALLELISM AS PARALLELISM     FROM       oceanbase.__all_virtual_database A,       oceanbase.__all_virtual_table B,       oceanbase.__all_virtual_mview_refresh_stmt_stats C     WHERE A.TENANT_ID = B.TENANT_ID       AND A.DATABASE_ID = B.DATABASE_ID       AND B.TENANT_ID = C.TENANT_ID       AND B.TABLE_ID = C.MVIEW_ID )__"))) {
      LOG_ERROR("fail to set view_definition", K(ret));
    }
  }
  table_schema.set_index_using_type(USING_BTREE);
...
int ObInnerTableSchema::dba_mvref_stmt_stats_schema(ObTableSchema &table_schema)
{
  ...
  if (OB_SUCC(ret)) {
    if (OB_FAIL(table_schema.set_view_definition(R"__(     SELECT       CAST(A.DATABASE_NAME AS CHAR(128)) AS MV_OWNER,       CAST(B.TABLE_NAME AS CHAR(128)) AS MV_NAME,       C.REFRESH_ID AS REFRESH_ID,       C.RETRY_ID AS RETRY_ID,       C.STEP AS STEP,       CAST(C.SQLID AS CHAR(32)) AS SQLID,       C.STMT AS STMT,       CAST(C.START_TIME AS DATETIME) AS START_TIME,       C.EXECUTION_TIME AS EXECUTION_TIME,       C.EXECUTION_PLAN AS EXECUTION_PLAN,       C.RESULT AS RESULT,       C.PARALLELISM AS PARALLELISM     FROM       oceanbase.__all_database A,       oceanbase.__all_table B,       oceanbase.__all_mview_refresh_stmt_stats C     WHERE A.TENANT_ID = B.TENANT_ID       AND A.DATABASE_ID = B.DATABASE_ID       AND B.TENANT_ID = C.TENANT_ID       AND B.TABLE_ID = C.MVIEW_ID )__"))) {
      LOG_ERROR("fail to set view_definition", K(ret));
    }
  }
  table_schema.set_index_using_type(USING_BTREE);
