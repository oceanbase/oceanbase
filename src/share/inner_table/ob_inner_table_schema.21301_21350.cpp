...
int ObInnerTableSchema::gv_ob_sql_plan_schema(ObTableSchema &table_schema)
{
  ...
  if (OB_SUCC(ret)) {
    if (OB_FAIL(table_schema.set_view_definition(R"__(SELECT                         SVR_IP,                         SVR_PORT,                         PLAN_ID,                         SQL_ID,                         DB_ID,                         PLAN_HASH,                         GMT_CREATE,                         OPERATION,                         OPTIONS,                         OBJECT_NODE,                         `OBJECT#`,                         OBJECT_OWNER,                         OBJECT_NAME,                         OBJECT_ALIAS,                         OBJECT_TYPE,                         OPTIMZIER,                         ID,                         PARENT_ID,                         DEPTH,                         POSITION,                         SEARCH_COLUMNS,                         COST,                         CARDINALITY,                         BYTES,                         ROWSET,                         OTHER_TAG,                         PARTITION_START,                         PARTITION_STOP,                         PARTITION_ID,                         OTHER,                         DISTRIBUTION,                         CPU_COST,                         IO_COST,                         TEMP_SPACE,                         ACCESS_PREDICATES,                         FILTER_PREDICATES,                         STARTUP_PREDICATES,                         PROJECTION,                         SPECIAL_PREDICATES,                         `TIME`,                         QBLOCK_NAME,                         REMARKS,                         OTHER_XML                     FROM OCEANBASE.__ALL_VIRTUAL_SQL_PLAN                     WHERE TENANT_ID = EFFECTIVE_TENANT_ID() )__"))) {
      LOG_ERROR("fail to set view_definition", K(ret));
    }
  }
  table_schema.set_index_using_type(USING_BTREE);
...
int ObInnerTableSchema::v_ob_sql_plan_schema(ObTableSchema &table_schema)
{
  ...
  if (OB_SUCC(ret)) {
    if (OB_FAIL(table_schema.set_view_definition(R"__(SELECT                       SQL_ID,                       DB_ID,                       PLAN_HASH,                       PLAN_ID,                       GMT_CREATE,                       OPERATION,                       OPTIONS,                       OBJECT_NODE,                       `OBJECT#`,                       OBJECT_OWNER,                       OBJECT_NAME,                       OBJECT_ALIAS,                       OBJECT_TYPE,                       OPTIMZIER,                       ID,                       PARENT_ID,                       DEPTH,                       POSITION,                       SEARCH_COLUMNS,                       COST,                       CARDINALITY,                       BYTES,                       ROWSET,                       OTHER_TAG,                       PARTITION_START,                       PARTITION_STOP,                       PARTITION_ID,                       OTHER,                       DISTRIBUTION,                       CPU_COST,                       IO_COST,                       TEMP_SPACE,                       ACCESS_PREDICATES,                       FILTER_PREDICATES,                       STARTUP_PREDICATES,                       PROJECTION,                       SPECIAL_PREDICATES,                       `TIME`,                       QBLOCK_NAME,                       REMARKS,                       OTHER_XML     FROM OCEANBASE.GV$OB_SQL_PLAN     WHERE SVR_IP=HOST_IP() AND SVR_PORT=RPC_PORT() )__"))) {
      LOG_ERROR("fail to set view_definition", K(ret));
    }
  }
  table_schema.set_index_using_type(USING_BTREE);
...
int ObInnerTableSchema::gv_ob_plan_real_info_schema(ObTableSchema &table_schema)
{
  int ret = OB_SUCCESS;
  uint64_t column_id = OB_APP_MIN_COLUMN_ID - 1;
  ...
  table_schema.set_tablegroup_id(OB_INVALID_ID);
  table_schema.set_database_id(OB_SYS_DATABASE_ID);
  table_schema.set_table_id(OB_GV_OB_PLAN_REAL_INFO_TID);
  table_schema.set_rowkey_split_pos(0);
  table_schema.set_is_use_bloomfilter(false);
  table_schema.set_progressive_merge_num(0);
  table_schema.set_rowkey_column_num(0);
  ...
  if (OB_SUCC(ret)) {
    if (OB_FAIL(table_schema.set_table_name(OB_GV_OB_PLAN_REAL_INFO_TNAME))) {
      LOG_ERROR("fail to set table_name", K(ret));
    }
  }
  ...
  if (OB_SUCC(ret)) {
    if (OB_FAIL(table_schema.set_view_definition(R"__(SELECT                         SVR_IP,                         SVR_PORT,                         PLAN_ID,                         SQL_ID,                         PLAN_HASH,                         ID,                         REAL_COST,                         REAL_CARDINALITY,                         CPU_COST,                         IO_COST                     FROM OCEANBASE.__ALL_VIRTUAL_PLAN_REAL_INFO                     WHERE TENANT_ID = EFFECTIVE_TENANT_ID() )__"))) {
      LOG_ERROR("fail to set view_definition", K(ret));
    }
  }
  table_schema.set_index_using_type(USING_BTREE);
...
int ObInnerTableSchema::v_ob_plan_real_info_schema(ObTableSchema &table_schema)
{
  int ret = OB_SUCCESS;
  uint64_t column_id = OB_APP_MIN_COLUMN_ID - 1;
  ...
  table_schema.set_tablegroup_id(OB_INVALID_ID);
  table_schema.set_database_id(OB_SYS_DATABASE_ID);
  table_schema.set_table_id(OB_V_OB_PLAN_REAL_INFO_TID);
  table_schema.set_rowkey_split_pos(0);
  table_schema.set_is_use_bloomfilter(false);
  table_schema.set_progressive_merge_num(0);
  table_schema.set_rowkey_column_num(0);
  ...
  if (OB_SUCC(ret)) {
    if (OB_FAIL(table_schema.set_table_name(OB_V_OB_PLAN_REAL_INFO_TNAME))) {
      LOG_ERROR("fail to set table_name", K(ret));
    }
  }
  ...
  if (OB_SUCC(ret)) {
    if (OB_FAIL(table_schema.set_view_definition(R"__(SELECT                         SQL_ID,                         PLAN_ID,                         PLAN_HASH,                         ID,                         REAL_COST,                         REAL_CARDINALITY,                         CPU_COST,                         IO_COST     FROM OCEANBASE.GV$OB_PLAN_REAL_INFO     WHERE SVR_IP=HOST_IP() AND SVR_PORT=RPC_PORT() )__"))) {
      LOG_ERROR("fail to set view_definition", K(ret));
    }
  }
  table_schema.set_index_using_type(USING_BTREE);
