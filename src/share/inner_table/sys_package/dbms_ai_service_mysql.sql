...
CREATE OR REPLACE PACKAGE dbms_ai_service AUTHID CURRENT_USER
  ...
  PROCEDURE register_provider(IN name VARCHAR(128), IN params JSON);
  PROCEDURE unregister_provider(IN name VARCHAR(128));
  PROCEDURE create_ai_gateway(IN name VARCHAR(128), IN params JSON);
  PROCEDURE alter_ai_gateway(IN name VARCHAR(128), IN params JSON);
  PROCEDURE drop_ai_gateway(IN name VARCHAR(128));
  PROCEDURE alter_model_parameter(IN model VARCHAR(256), IN params JSON);
  PROCEDURE drop_model_parameter(IN model VARCHAR(256));
