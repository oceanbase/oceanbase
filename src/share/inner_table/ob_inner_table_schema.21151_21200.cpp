...
int ObInnerTableSchema::dba_ob_tenants_schema(ObTableSchema &table_schema)
{
  ...
  if (OB_SUCC(ret)) {
    if (OB_FAIL(table_schema.set_view_definition(R"__( SELECT TENANT_ID,        TENANT_NAME,        (CASE             WHEN TENANT_ID = 1 THEN 'SYS'             WHEN (TENANT_ID & 0x1) = 1 THEN 'META'             ELSE 'USER'         END) AS TENANT_TYPE,        gmt_create AS CREATE_TIME,        gmt_modified AS MODIFY_TIME,        PRIMARY_ZONE,        LOCALITY,        CASE previous_locality           WHEN "" THEN NULL           ELSE previous_locality        END AS PREVIOUS_LOCALITY,        CASE compatibility_mode           WHEN 0 THEN 'MYSQL'           WHEN 1 THEN 'ORACLE'           ELSE NULL        END AS COMPATIBILITY_MODE,        STATUS,        CASE in_recyclebin           WHEN 0 THEN 'NO'           ELSE 'YES'        END AS IN_RECYCLEBIN,         CASE locked           WHEN 0 THEN 'NO'           ELSE 'YES'        END AS LOCKED FROM oceanbase.__all_tenant )__"))) {
      LOG_ERROR("fail to set view_definition", K(ret));
    }
  }
  table_schema.set_index_using_type(USING_BTREE);
...
int ObInnerTableSchema::cdb_ob_tablegroups_schema(ObTableSchema &table_schema)
{
  ...
  if (OB_SUCC(ret)) {
    if (OB_FAIL(table_schema.set_view_definition(R"__(   SELECT TENANT_ID,           TABLEGROUP_NAME,           (CASE PART_LEVEL           WHEN 0 THEN 'NONE'           ELSE (CASE PART_FUNC_TYPE                 WHEN 0 THEN 'HASH'                 WHEN 1 THEN 'KEY'                 WHEN 2 THEN 'KEY'                 WHEN 3 THEN 'RANGE'                 WHEN 4 THEN 'RANGE COLUMNS'                 WHEN 5 THEN 'LIST'                 WHEN 6 THEN 'LIST COLUMNS'                 WHEN 7 THEN 'UNKNOWN'                 ELSE 'UNKNOWN'                 END)           END) AS PARTITIONING_TYPE,           (CASE PART_LEVEL           WHEN 2 THEN (CASE SUB_PART_FUNC_TYPE                        WHEN 0 THEN 'HASH'                        WHEN 1 THEN 'KEY'                        WHEN 2 THEN 'KEY'                        WHEN 3 THEN 'RANGE'                        WHEN 4 THEN 'RANGE COLUMNS'                        WHEN 5 THEN 'LIST'                        WHEN 6 THEN 'LIST COLUMNS'                        WHEN 7 THEN 'UNKNOWN'                        ELSE 'UNKNOWN'                        END)           ELSE 'NONE'           END) AS SUBPARTITIONING_TYPE,           (CASE PART_LEVEL           WHEN 0 THEN NULL           ELSE PART_NUM           END) AS PARTITION_COUNT,           CAST(NULL AS SIGNED) AS DEF_SUBPARTITION_COUNT,           (CASE PART_LEVEL           WHEN 0 THEN NULL           ELSE PART_FUNC_EXPR_NUM           END) AS PARTITIONING_KEY_COUNT,           (CASE PART_LEVEL           WHEN 2 THEN SUB_PART_FUNC_EXPR_NUM           ELSE NULL           END) AS SUBPARTITIONING_KEY_COUNT    FROM OCEANBASE.__ALL_VIRTUAL_TABLEGROUP   )__"))) {
      LOG_ERROR("fail to set view_definition", K(ret));
    }
  }
  table_schema.set_index_using_type(USING_BTREE);
