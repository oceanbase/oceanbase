...
int ObInnerTableSchema::column_privileges_schema(ObTableSchema &table_schema)
{
  ...
  if (OB_SUCC(ret)) {
    if (OB_FAIL(table_schema.set_view_definition(R"__(SELECT cast(concat('''', B.user_name, '''', '@', '''', B.host, '''') as char(292)) as GRANTEE,         cast('def' as char(512)) AS TABLE_CATALOG,         cast(DATABASE_NAME as char(64)) AS TABLE_SCHEMA,         cast(TABLE_NAME as char(64)) AS TABLE_NAME,         cast(COLUMN_NAME as char(64)) AS COLUMN_NAME,         cast(CASE WHEN V1.C1 = 0  AND (A.all_priv & 1) != 0 THEN 'SELECT'               WHEN V1.C1 = 1  AND (A.all_priv & 2) != 0 THEN 'INSERT'               WHEN V1.C1 = 2  AND (A.all_priv & 4) != 0 THEN 'UPDATE'               WHEN V1.C1 = 3  AND (A.all_priv & 8) != 0 THEN 'REFERENCES'               END AS char(64)) AS PRIVILEGE_TYPE,         cast(case when priv_grant_option = 1 then 'YES' ELSE 'NO' END as char(3)) AS IS_GRANTABLE   FROM oceanbase.__all_column_privilege A, oceanbase.__all_user B,       (SELECT 0 AS C1         UNION ALL SELECT 1 AS C1         UNION ALL SELECT 2 AS C1         UNION ALL SELECT 3 AS C1) V1   WHERE A.tenant_id = B.tenant_id and A.tenant_id = 0 and A.user_id = B.user_id AND         ((V1.C1 = 0 AND (A.all_priv & 1) != 0) OR (V1.C1 = 1 AND (A.all_priv & 2) != 0)          OR (V1.C1 = 2 AND (A.all_priv & 4) != 0 OR (V1.C1 = 0 AND (A.all_priv & 8) != 0))) )__"))) {
