...
int ObInnerTableSchema::dba_ob_tenants_schema(ObTableSchema &table_schema)
{
  ...
  if (OB_SUCC(ret)) {
    if (OB_FAIL(table_schema.set_view_definition(R"__( SELECT A.TENANT_ID,        TENANT_NAME,        (CASE             WHEN A.TENANT_ID = 1 THEN 'SYS'             WHEN (A.TENANT_ID & 0x1) = 1 THEN 'META'             ELSE 'USER'         END) AS TENANT_TYPE,        A.gmt_create AS CREATE_TIME,        A.gmt_modified AS MODIFY_TIME,        PRIMARY_ZONE,        LOCALITY,        (SELECT TG.TABLEGROUP_NAME           FROM OCEANBASE.__ALL_VIRTUAL_TABLEGROUP TG          WHERE TG.TENANT_ID = A.TENANT_ID            AND TG.TABLEGROUP_ID = A.DEFAULT_TABLEGROUP_ID) AS TABLEGROUP_NAME,        CASE previous_locality           WHEN "" THEN NULL           ELSE previous_locality        END AS PREVIOUS_LOCALITY,        CASE compatibility_mode           WHEN 0 THEN 'MYSQL'           WHEN 1 THEN 'ORACLE'           ELSE NULL        END AS COMPATIBILITY_MODE,        STATUS,        CASE in_recyclebin           WHEN 0 THEN 'NO'           ELSE 'YES'        END AS IN_RECYCLEBIN,         CASE locked           WHEN 0 THEN 'NO'           ELSE 'YES'        END AS LOCKED,         (CASE             WHEN A.TENANT_ID = 1 THEN 'PRIMARY'             WHEN (A.TENANT_ID & 0x1) = 1 THEN 'PRIMARY'             ELSE TENANT_ROLE         END) AS TENANT_ROLE,         (CASE             WHEN A.TENANT_ID = 1 THEN 'NORMAL'             WHEN (A.TENANT_ID & 0x1) = 1 THEN 'NORMAL'             ELSE SWITCHOVER_STATUS         END) AS SWITCHOVER_STATUS,         (CASE             WHEN A.TENANT_ID = 1 THEN 0             WHEN (A.TENANT_ID & 0x1) = 1 THEN 0             ELSE SWITCHOVER_EPOCH         END) AS SWITCHOVER_EPOCH,         (CASE             WHEN A.TENANT_ID = 1 THEN NULL             WHEN (A.TENANT_ID & 0x1) = 1 THEN NULL             ELSE SYNC_SCN         END) AS SYNC_SCN,         (CASE             WHEN A.TENANT_ID = 1 THEN NULL             WHEN (A.TENANT_ID & 0x1) = 1 THEN NULL             ELSE REPLAYABLE_SCN         END) AS REPLAYABLE_SCN,         (CASE             WHEN A.TENANT_ID = 1 THEN NULL             WHEN (A.TENANT_ID & 0x1) = 1 THEN NULL             ELSE READABLE_SCN         END) AS READABLE_SCN,         (CASE             WHEN A.TENANT_ID = 1 THEN NULL             WHEN (A.TENANT_ID & 0x1) = 1 THEN NULL             ELSE RECOVERY_UNTIL_SCN         END) AS RECOVERY_UNTIL_SCN,         (CASE             WHEN A.TENANT_ID = 1 THEN 'NOARCHIVELOG'             WHEN (A.TENANT_ID & 0x1) = 1 THEN 'NOARCHIVELOG'             ELSE LOG_MODE         END) AS LOG_MODE,        ARBITRATION_SERVICE_STATUS,        UNIT_NUM, ZONE_UNIT_NUM_LIST,        COMPATIBLE,        (CASE             WHEN STARTUP_MODE() = 'shared_storage' THEN                 CASE                     WHEN A.TENANT_ID = 1 THEN 1001                     WHEN (MOD(A.TENANT_ID, 2)) = 1 THEN 1002                     ELSE B.MAX_LS_ID                 END             ELSE                 CASE                     WHEN (MOD(A.TENANT_ID, 2)) = 1 THEN 1                     ELSE B.MAX_LS_ID                 END         END) AS MAX_LS_ID,        (CASE             WHEN A.TENANT_ID = 1 THEN 'NORMAL'             WHEN (A.TENANT_ID & 0x1) = 1 THEN 'NORMAL'             ELSE RESTORE_DATA_MODE         END) AS RESTORE_DATA_MODE,        (CASE             WHEN A.TENANT_ID = 1 THEN NULL             WHEN (A.TENANT_ID & 0x1) = 1 THEN NULL             ELSE E.LATEST_FLASHBACK_LOG_SCN         END) AS FLASHBACK_LOG_SCN,         A.INFO AS COMMENT,         B.PROTECTION_MODE AS PROTECTION_MODE,         B.PROTECTION_LEVEL AS PROTECTION_LEVEL,         (SELECT TS.TABLESPACE_NAME            FROM OCEANBASE.__ALL_VIRTUAL_TENANT_TABLESPACE AS TS           WHERE TS.TENANT_ID = A.TENANT_ID             AND TS.TABLESPACE_ID = A.DEFAULT_TABLESPACE_ID) AS DEFAULT_TABLESPACE FROM OCEANBASE.__ALL_VIRTUAL_TENANT_MYSQL_SYS_AGENT AS A LEFT JOIN OCEANBASE.__ALL_VIRTUAL_TENANT_INFO AS B     ON A.TENANT_ID = B.TENANT_ID LEFT JOIN     (SELECT TENANT_ID,             (CASE                  WHEN TENANT_ID < 1 THEN NULL                  WHEN TENANT_ID != 1 THEN TENANT_ID - 1                  ELSE NULL              END) AS META_TENANT_ID,             MIN(UNIT_COUNT) AS UNIT_NUM,             GROUP_CONCAT(ZONE, ':', UNIT_COUNT ORDER BY ZONE SEPARATOR ',') AS ZONE_UNIT_NUM_LIST      FROM (SELECT TENANT_ID, ZONE, COUNT(*) UNIT_COUNT FROM (            SELECT F.TENANT_ID, G.ZONE FROM OCEANBASE.__ALL_VIRTUAL_RESOURCE_POOL_MYSQL_SYS_AGENT AS F            JOIN OCEANBASE.__ALL_VIRTUAL_UNIT_MYSQL_SYS_AGENT AS G ON F.RESOURCE_POOL_ID = G.RESOURCE_POOL_ID            WHERE F.TENANT_ID > 0 AND G.STATUS != 'DELETING')            GROUP BY TENANT_ID, ZONE) GROUP BY TENANT_ID) AS C     ON A.TENANT_ID = C.TENANT_ID OR A.TENANT_ID = C.META_TENANT_ID LEFT JOIN     (SELECT TENANT_ID,             MIN(VALUE) AS COMPATIBLE      FROM OCEANBASE.__ALL_VIRTUAL_TENANT_PARAMETER      WHERE NAME = 'compatible'      GROUP BY TENANT_ID) AS D     ON A.TENANT_ID = D.TENANT_ID LEFT JOIN     (SELECT TENANT_ID, MAX(FLASHBACK_LOG_SCN) AS LATEST_FLASHBACK_LOG_SCN      FROM OCEANBASE.__ALL_VIRTUAL_TENANT_FLASHBACK_LOG_SCN GROUP BY TENANT_ID) AS E      ON A.TENANT_ID = E.TENANT_ID )__"))) {
      LOG_ERROR("fail to set view_definition", K(ret));
    }
  }
  table_schema.set_index_using_type(USING_BTREE);
...
int ObInnerTableSchema::dba_ob_resource_pools_schema(ObTableSchema &table_schema)
{
  ...
  if (OB_SUCC(ret)) {
    if (OB_FAIL(table_schema.set_view_definition(R"__( SELECT RESOURCE_POOL_ID,        NAME,        CASE TENANT_ID WHEN -1 THEN NULL ELSE TENANT_ID END AS TENANT_ID,        gmt_create AS CREATE_TIME,        gmt_modified AS MODIFY_TIME,        UNIT_COUNT,        UNIT_CONFIG_ID,        ZONE_LIST,        CASE replica_type           WHEN 0 THEN "FULL"           WHEN 5 THEN "LOGONLY"           ELSE NULL        END AS REPLICA_TYPE FROM oceanbase.__all_resource_pool )__"))) {
      LOG_ERROR("fail to set view_definition", K(ret));
    }
  }
  table_schema.set_index_using_type(USING_BTREE);
...
int ObInnerTableSchema::dba_ob_databases_schema(ObTableSchema &table_schema)
{
  ...
  if (OB_SUCC(ret)) {
    if (OB_FAIL(table_schema.set_view_definition(R"__(   SELECT D.DATABASE_NAME AS DATABASE_NAME,          (CASE D.IN_RECYCLEBIN WHEN 0 THEN 'NO' ELSE 'YES' END) AS IN_RECYCLEBIN,          C.COLLATION AS COLLATION,          (CASE D.READ_ONLY WHEN 0 THEN 'NO' ELSE 'YES' END) AS READ_ONLY,          D.COMMENT AS COMMENT,          TG.TABLEGROUP_NAME,          (SELECT TS.TABLESPACE_NAME             FROM OCEANBASE.__ALL_TENANT_TABLESPACE AS TS            WHERE TS.TENANT_ID = D.TENANT_ID              AND TS.TABLESPACE_ID = D.DEFAULT_TABLESPACE_ID) AS DEFAULT_TABLESPACE   FROM OCEANBASE.__ALL_DATABASE AS D   LEFT JOIN OCEANBASE.__TENANT_VIRTUAL_COLLATION AS C   ON D.COLLATION_TYPE = C.COLLATION_TYPE   LEFT JOIN OCEANBASE.__ALL_TABLEGROUP AS TG   ON D.TENANT_ID = TG.TENANT_ID AND D.DEFAULT_TABLEGROUP_ID = TG.TABLEGROUP_ID   )__"))) {
      LOG_ERROR("fail to set view_definition", K(ret));
    }
  }
  table_schema.set_index_using_type(USING_BTREE);
...
int ObInnerTableSchema::cdb_ob_databases_schema(ObTableSchema &table_schema)
{
  ...
  if (OB_SUCC(ret)) {
    if (OB_FAIL(table_schema.set_view_definition(R"__(   SELECT D.TENANT_ID,          D.DATABASE_NAME AS DATABASE_NAME,          (CASE D.IN_RECYCLEBIN WHEN 0 THEN 'NO' ELSE 'YES' END) AS IN_RECYCLEBIN,          C.COLLATION AS COLLATION,          (CASE D.READ_ONLY WHEN 0 THEN 'NO' ELSE 'YES' END) AS READ_ONLY,          D.COMMENT AS COMMENT,          TG.TABLEGROUP_NAME,          (SELECT TS.TABLESPACE_NAME             FROM OCEANBASE.__ALL_VIRTUAL_TENANT_TABLESPACE AS TS            WHERE TS.TENANT_ID = D.TENANT_ID              AND TS.TABLESPACE_ID = D.DEFAULT_TABLESPACE_ID) AS DEFAULT_TABLESPACE   FROM OCEANBASE.__ALL_VIRTUAL_DATABASE AS D   LEFT JOIN OCEANBASE.__TENANT_VIRTUAL_COLLATION AS C   ON D.COLLATION_TYPE = C.COLLATION_TYPE   LEFT JOIN OCEANBASE.__ALL_VIRTUAL_TABLEGROUP AS TG   ON D.TENANT_ID = TG.TENANT_ID AND D.DEFAULT_TABLEGROUP_ID = TG.TABLEGROUP_ID   )__"))) {
      LOG_ERROR("fail to set view_definition", K(ret));
    }
  }
  table_schema.set_index_using_type(USING_BTREE);
...
int ObInnerTableSchema::cdb_subpartition_templates_schema(ObTableSchema &table_schema)
{
  ...
  if (OB_SUCC(ret)) {
    if (OB_FAIL(table_schema.set_view_definition(R"__(   SELECT       CAST(DB.TENANT_ID AS NUMBER) CON_ID,       CAST(DB.DATABASE_NAME AS CHAR(128)) USER_NAME,       CAST(TB.TABLE_NAME AS CHAR(128)) TABLE_NAME,       CAST(SP.SUB_PART_NAME AS CHAR(132)) SUBPARTITION_NAME,       CAST(SP.SUB_PART_ID + 1 AS SIGNED) SUBPARTITION_POSITION,       CAST(TP.TABLESPACE_NAME AS CHAR(30)) TABLESPACE_NAME,       CAST(CASE WHEN SP.HIGH_BOUND_VAL IS NULL THEN SP.LIST_VAL            ELSE SP.HIGH_BOUND_VAL END AS CHAR(262144)) HIGH_BOUND,       CAST(NULL AS CHAR(4)) COMPRESSION,       CAST(NULL AS CHAR(4)) INDEXING,       CAST(NULL AS CHAR(4)) READ_ONLY        FROM OCEANBASE.__ALL_VIRTUAL_DATABASE DB        JOIN OCEANBASE.__ALL_VIRTUAL_TABLE TB       ON DB.DATABASE_ID = TB.DATABASE_ID AND DB.TENANT_ID = TB.TENANT_ID          AND TB.TABLE_TYPE IN (3, 6, 8, 9, 16, 17)          AND TB.TABLE_MODE >> 12 & 15 in (0,1)          AND TB.INDEX_ATTRIBUTES_SET & 16 = 0        JOIN OCEANBASE.__ALL_VIRTUAL_DEF_SUB_PART SP       ON TB.TABLE_ID = SP.TABLE_ID AND SP.TENANT_ID = TB.TENANT_ID        LEFT JOIN OCEANBASE.__ALL_VIRTUAL_TENANT_TABLESPACE TP       ON TP.TABLESPACE_ID = SP.TABLESPACE_ID AND TP.TENANT_ID = SP.TENANT_ID       )__"))) {
      LOG_ERROR("fail to set view_definition", K(ret));
    }
  }
  table_schema.set_index_using_type(USING_BTREE);
