...
int ObInnerTableSchema::cdb_ob_backup_delete_jobs_schema(ObTableSchema &table_schema)
{
  ...
  if (OB_SUCC(ret)) {
    if (OB_FAIL(table_schema.set_view_definition(R"__(     SELECT       TENANT_ID,       JOB_ID,       INCARNATION,       INITIATOR_TENANT_ID,       INITIATOR_JOB_ID,       EXECUTOR_TENANT_ID,       TYPE,       CASE         WHEN TYPE = 'DELETE OBSOLETE BACKUP' THEN            CASE              WHEN TENANT_ID = 1 THEN CONCAT('init_time:', USEC_TO_TIME(PARAMETER))             ELSE CONCAT('expired_time:', USEC_TO_TIME(PARAMETER))           END           WHEN TYPE = 'DELETE BACKUP ALL' THEN 'request_delete_path:' || data_backup_path_list || log_archive_path_list         ELSE CONCAT('request_delete_id:', PARAMETER)       END AS PARAMETER,       JOB_LEVEL,       USEC_TO_TIME(START_TS) AS START_TIMESTAMP,       CASE         WHEN END_TS = 0           THEN NULL         ELSE           USEC_TO_TIME(END_TS)         END AS END_TIMESTAMP,       STATUS,       TASK_COUNT,       SUCCESS_TASK_COUNT,       RESULT,       COMMENT     FROM oceanbase.__all_virtual_backup_delete_job )__"))) {
      LOG_ERROR("fail to set view_definition", K(ret));
    }
  }
  table_schema.set_index_using_type(USING_BTREE);
