...
def_table_schema(
  ...
  FROM oceanbase.__all_virtual_database a inner join oceanbase.__tenant_virtual_collation b ON a.collation_type = b.id
...
def_table_schema(
 ...
 view_definition = """
    ...
    FROM OCEANBASE.__ALL_VIRTUAL_PARTITION_COMPACTION_HISTORY
    WHERE
        EFFECTIVE_TENANT_ID() = 1 OR tenant_id  = EFFECTIVE_TENANT_ID()
""".replace("\n", " ")
)
