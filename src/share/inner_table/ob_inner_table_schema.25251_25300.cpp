...
int ObInnerTableSchema::dba_scheduler_running_jobs_ora_schema(ObTableSchema &table_schema)
{
  int ret = OB_SUCCESS;
  uint64_t column_id = OB_APP_MIN_COLUMN_ID - 1;
  ...
  table_schema.set_tablegroup_id(OB_INVALID_ID);
  table_schema.set_database_id(OB_ORA_SYS_DATABASE_ID);
  table_schema.set_table_id(OB_DBA_SCHEDULER_RUNNING_JOBS_ORA_TID);
  table_schema.set_rowkey_split_pos(0);
  table_schema.set_is_use_bloomfilter(false);
  table_schema.set_progressive_merge_num(0);
  table_schema.set_rowkey_column_num(0);
  ...
  if (OB_SUCC(ret)) {
    if (OB_FAIL(table_schema.set_table_name(OB_DBA_SCHEDULER_RUNNING_JOBS_ORA_TNAME))) {
      LOG_ERROR("fail to set table_name", K(ret));
    }
  }
  ...
  if (OB_SUCC(ret)) {
    if (OB_FAIL(table_schema.set_view_definition(R"__(SELECT     CAST(T.POWNER AS VARCHAR2(128)) AS OWNER,     CAST(T.JOB_NAME AS VARCHAR2(128)) AS JOB_NAME,     CAST(NULL AS VARCHAR2(128)) AS JOB_SUBNAME,     CAST(T.JOB_STYLE AS VARCHAR2(17)) AS JOB_STYLE,     CAST(NULL AS VARCHAR2(5)) AS DETACHED,     CAST(T.THIS_EXEC_SESS_ID AS NUMBER) AS SESSION_ID,     CAST(NULL AS NUMBER) AS SLAVE_PROCESS_ID,     CAST(NULL AS NUMBER) AS SLAVE_OS_PROCESS_ID,     CAST(NULL AS NUMBER) AS RUNNING_INSTANCE,     CAST(NULL AS VARCHAR2(32)) AS RESOURCE_CONSUMER_GROUP,     CAST((TIMESTAMP'1970-01-01 08:00:00' + (SYSDATE - T.THIS_DATE) - TIMESTAMP'1970-01-01 08:00:00') AS INTERVAL DAY(3) TO SECOND(2)) AS ELAPSED_TIME,     CAST(NULL AS INTERVAL DAY(3) TO SECOND(2)) AS CPU_USED,     CAST(NULL AS VARCHAR2(128)) AS DESTINATION_OWNER,     CAST(NULL AS VARCHAR2(128)) AS DESTINATION,     CAST(NULL AS VARCHAR2(128)) AS CREDENTIAL_OWNER,     CAST(NULL AS VARCHAR2(128)) AS CREDENTIAL_NAME,     CAST(T.THIS_DATE AS TIMESTAMP(6) WITH TIME ZONE) AS THIS_DATE,     CAST(T.THIS_EXEC_DATE AS TIMESTAMP(6) WITH TIME ZONE) AS THIS_EXEC_DATE,     CAST(T.THIS_EXEC_ADDR AS VARCHAR2(128)) AS THIS_EXEC_ADDR,     CAST(T.THIS_EXEC_TRACE_ID AS VARCHAR2(128)) AS THIS_EXEC_TRACE_ID     FROM SYS.ALL_VIRTUAL_TENANT_SCHEDULER_JOB_REAL_AGENT T WHERE T.JOB_NAME != '__dummy_guard' AND T.JOB > 0 AND T.THIS_DATE IS NOT NULL )__"))) {
      LOG_ERROR("fail to set view_definition", K(ret));
    }
  }
  table_schema.set_index_using_type(USING_BTREE);
...
int ObInnerTableSchema::dba_mvref_run_stats_ora_schema(ObTableSchema &table_schema)
{
  ...
  if (OB_SUCC(ret)) {
    if (OB_FAIL(table_schema.set_view_definition(R"__(     SELECT       CAST(A.USER_NAME AS VARCHAR2(128)) AS RUN_OWNER,       CAST(B.REFRESH_ID AS NUMBER) AS REFRESH_ID,       CAST(C.NUM_MVS AS NUMBER) AS NUM_MVS,       CAST(C.MVIEWS AS VARCHAR2(4000)) AS MVIEWS,       CAST(D.BASE_TABLES AS VARCHAR2(4000)) AS BASE_TABLES,       CAST(B.METHOD AS VARCHAR2(4000)) AS METHOD,       CAST(B.ROLLBACK_SEG AS VARCHAR2(4000)) AS ROLLBACK_SEG,       CAST(DECODE(B.PUSH_DEFERRED_RPC, 1, 'Y', 'N') AS CHAR(1)) AS PUSH_DEFERRED_RPC,       CAST(DECODE(B.REFRESH_AFTER_ERRORS, 1, 'Y', 'N') AS CHAR(1)) AS REFRESH_AFTER_ERRORS,       CAST(B.PURGE_OPTION AS NUMBER) AS PURGE_OPTION,       CAST(B.PARALLELISM AS NUMBER) AS PARALLELISM,       CAST(B.HEAP_SIZE AS NUMBER) AS HEAP_SIZE,       CAST(DECODE(B.ATOMIC_REFRESH, 1, 'Y', 'N') AS CHAR(1)) AS ATOMIC_REFRESH,       CAST(DECODE(B.NESTED, 1, 'Y', 'N') AS CHAR(1)) AS NESTED,       CAST(DECODE(B.OUT_OF_PLACE, 1, 'Y', 'N') AS CHAR(1)) AS OUT_OF_PLACE,       CAST(B.NUMBER_OF_FAILURES AS NUMBER) AS NUMBER_OF_FAILURES,       CAST(B.RESULT AS NUMBER) AS RESULT,       CAST(B.START_TIME AS TIMESTAMP(6)) AS START_TIME,       CAST(B.END_TIME AS TIMESTAMP(6)) AS END_TIME,       CAST(B.ELAPSED_TIME AS NUMBER) AS ELAPSED_TIME,       CAST(0 AS NUMBER) AS LOG_SETUP_TIME,       CAST(B.LOG_PURGE_TIME AS NUMBER) AS LOG_PURGE_TIME,       CAST(DECODE(B.COMPLETE_STATS_AVALIABLE, 1, 'Y', 'N') AS CHAR(1)) AS COMPLETE_STATS_AVAILABLE,       CAST(B.MVIEW_ID AS NUMBER) AS MVIEW_ID,       CAST(MT.TABLE_NAME AS VARCHAR2(128)) AS MVIEW_NAME,       CAST(B.DATA_TARGET_SCN AS NUMBER) AS DATA_TARGET_SCN,       CAST(B.TRACE_ID AS VARCHAR2(64)) AS TRACE_ID     FROM       SYS.ALL_VIRTUAL_USER_REAL_AGENT A,       SYS.ALL_VIRTUAL_MVIEW_REFRESH_RUN_STATS_REAL_AGENT B,       (         SELECT           C2.TENANT_ID AS TENANT_ID,           C2.REFRESH_ID AS REFRESH_ID,           COUNT(DISTINCT C2.TABLE_NAME) AS NUM_MVS,           LISTAGG(C2.TABLE_NAME, ',') WITHIN GROUP (ORDER BY C2.TABLE_NAME) AS MVIEWS         FROM           (SELECT DISTINCT C1I.TENANT_ID, C1I.REFRESH_ID, C2I.TABLE_NAME            FROM SYS.ALL_VIRTUAL_MVIEW_REFRESH_STATS_REAL_AGENT C1I,                 SYS.ALL_VIRTUAL_TABLE_REAL_AGENT C2I            WHERE C1I.TENANT_ID = C2I.TENANT_ID              AND C1I.MVIEW_ID = C2I.TABLE_ID              AND C1I.TENANT_ID = EFFECTIVE_TENANT_ID()              AND C2I.TENANT_ID = EFFECTIVE_TENANT_ID()           ) C2         GROUP BY C2.TENANT_ID, C2.REFRESH_ID       ) C,       (         SELECT           D1.TENANT_ID AS TENANT_ID,           D1.REFRESH_ID AS REFRESH_ID,           LISTAGG(TBL_NAME, ',') WITHIN GROUP (ORDER BY TBL_NAME) AS BASE_TABLES         FROM           (SELECT DISTINCT D1I.TENANT_ID, D1I.REFRESH_ID, D2I.TABLE_NAME AS TBL_NAME            FROM SYS.ALL_VIRTUAL_MVIEW_REFRESH_CHANGE_STATS_REAL_AGENT D1I,                 SYS.ALL_VIRTUAL_TABLE_REAL_AGENT D2I,                 SYS.ALL_VIRTUAL_TABLE_REAL_AGENT D3I            WHERE D1I.TENANT_ID = D2I.TENANT_ID              AND D1I.DETAIL_TABLE_ID = D2I.TABLE_ID              AND D3I.DATA_TABLE_ID(+) = D2I.TABLE_ID              AND D3I.TABLE_TYPE(+) = 7              AND D1I.TENANT_ID = EFFECTIVE_TENANT_ID()              AND D2I.TENANT_ID = EFFECTIVE_TENANT_ID()              AND D3I.TENANT_ID(+) = EFFECTIVE_TENANT_ID()              AND D3I.TABLE_ID IS NULL           ) D1         GROUP BY D1.TENANT_ID, D1.REFRESH_ID       ) D,       SYS.ALL_VIRTUAL_TABLE_REAL_AGENT MT     WHERE A.USER_ID = B.RUN_USER_ID       AND B.REFRESH_ID = C.REFRESH_ID (+)       AND B.REFRESH_ID = D.REFRESH_ID (+)       AND B.TENANT_ID = MT.TENANT_ID       AND B.MVIEW_ID = MT.TABLE_ID       AND MT.TABLE_TYPE = 7       AND A.TENANT_ID = EFFECTIVE_TENANT_ID()       AND B.TENANT_ID = EFFECTIVE_TENANT_ID()       AND C.TENANT_ID (+) = EFFECTIVE_TENANT_ID()       AND D.TENANT_ID (+) = EFFECTIVE_TENANT_ID()       AND MT.TENANT_ID = EFFECTIVE_TENANT_ID() )__"))) {
      LOG_ERROR("fail to set view_definition", K(ret));
    }
  }
  table_schema.set_index_using_type(USING_BTREE);
...
int ObInnerTableSchema::user_mvref_run_stats_ora_schema(ObTableSchema &table_schema)
{
  ...
  if (OB_SUCC(ret)) {
    if (OB_FAIL(table_schema.set_view_definition(R"__(     SELECT       CAST(B.REFRESH_ID AS NUMBER) AS REFRESH_ID,       CAST(C.NUM_MVS AS NUMBER) AS NUM_MVS,       CAST(C.MVIEWS AS VARCHAR2(4000)) AS MVIEWS,       CAST(D.BASE_TABLES AS VARCHAR2(4000)) AS BASE_TABLES,       CAST(B.METHOD AS VARCHAR2(4000)) AS METHOD,       CAST(B.ROLLBACK_SEG AS VARCHAR2(4000)) AS ROLLBACK_SEG,       CAST(DECODE(B.PUSH_DEFERRED_RPC, 1, 'Y', 'N') AS CHAR(1)) AS PUSH_DEFERRED_RPC,       CAST(DECODE(B.REFRESH_AFTER_ERRORS, 1, 'Y', 'N') AS CHAR(1)) AS REFRESH_AFTER_ERRORS,       CAST(B.PURGE_OPTION AS NUMBER) AS PURGE_OPTION,       CAST(B.PARALLELISM AS NUMBER) AS PARALLELISM,       CAST(B.HEAP_SIZE AS NUMBER) AS HEAP_SIZE,       CAST(DECODE(B.ATOMIC_REFRESH, 1, 'Y', 'N') AS CHAR(1)) AS ATOMIC_REFRESH,       CAST(DECODE(B.NESTED, 1, 'Y', 'N') AS CHAR(1)) AS NESTED,       CAST(DECODE(B.OUT_OF_PLACE, 1, 'Y', 'N') AS CHAR(1)) AS OUT_OF_PLACE,       CAST(B.NUMBER_OF_FAILURES AS NUMBER) AS NUMBER_OF_FAILURES,       CAST(B.RESULT AS NUMBER) AS RESULT,       CAST(B.START_TIME AS TIMESTAMP(6)) AS START_TIME,       CAST(B.END_TIME AS TIMESTAMP(6)) AS END_TIME,       CAST(B.ELAPSED_TIME AS NUMBER) AS ELAPSED_TIME,       CAST(0 AS NUMBER) AS LOG_SETUP_TIME,       CAST(B.LOG_PURGE_TIME AS NUMBER) AS LOG_PURGE_TIME,       CAST(DECODE(B.COMPLETE_STATS_AVALIABLE, 1, 'Y', 'N') AS CHAR(1)) AS COMPLETE_STATS_AVAILABLE,       CAST(B.MVIEW_ID AS NUMBER) AS MVIEW_ID,       CAST(MT.TABLE_NAME AS VARCHAR2(128)) AS MVIEW_NAME,       CAST(B.DATA_TARGET_SCN AS NUMBER) AS DATA_TARGET_SCN,       CAST(B.TRACE_ID AS VARCHAR2(64)) AS TRACE_ID     FROM       SYS.ALL_VIRTUAL_USER_REAL_AGENT A,       SYS.ALL_VIRTUAL_MVIEW_REFRESH_RUN_STATS_REAL_AGENT B,       (         SELECT           C2.TENANT_ID AS TENANT_ID,           C2.REFRESH_ID AS REFRESH_ID,           COUNT(DISTINCT C2.TABLE_NAME) AS NUM_MVS,           LISTAGG(C2.TABLE_NAME, ',') WITHIN GROUP (ORDER BY C2.TABLE_NAME) AS MVIEWS         FROM           (SELECT DISTINCT C1I.TENANT_ID, C1I.REFRESH_ID, C2I.TABLE_NAME            FROM SYS.ALL_VIRTUAL_MVIEW_REFRESH_STATS_REAL_AGENT C1I,                 SYS.ALL_VIRTUAL_TABLE_REAL_AGENT C2I            WHERE C1I.TENANT_ID = C2I.TENANT_ID              AND C1I.MVIEW_ID = C2I.TABLE_ID              AND C1I.TENANT_ID = EFFECTIVE_TENANT_ID()              AND C2I.TENANT_ID = EFFECTIVE_TENANT_ID()           ) C2         GROUP BY C2.TENANT_ID, C2.REFRESH_ID       ) C,       (         SELECT           D1.TENANT_ID AS TENANT_ID,           D1.REFRESH_ID AS REFRESH_ID,           LISTAGG(TBL_NAME, ',') WITHIN GROUP (ORDER BY TBL_NAME) AS BASE_TABLES         FROM           (SELECT DISTINCT D1I.TENANT_ID, D1I.REFRESH_ID, D2I.TABLE_NAME AS TBL_NAME            FROM SYS.ALL_VIRTUAL_MVIEW_REFRESH_CHANGE_STATS_REAL_AGENT D1I,                 SYS.ALL_VIRTUAL_TABLE_REAL_AGENT D2I,                 SYS.ALL_VIRTUAL_TABLE_REAL_AGENT D3I            WHERE D1I.TENANT_ID = D2I.TENANT_ID              AND D1I.DETAIL_TABLE_ID = D2I.TABLE_ID              AND D3I.DATA_TABLE_ID(+) = D2I.TABLE_ID              AND D3I.TABLE_TYPE(+) = 7              AND D1I.TENANT_ID = EFFECTIVE_TENANT_ID()              AND D2I.TENANT_ID = EFFECTIVE_TENANT_ID()              AND D3I.TENANT_ID(+) = EFFECTIVE_TENANT_ID()              AND D3I.TABLE_ID IS NULL           ) D1         GROUP BY D1.TENANT_ID, D1.REFRESH_ID       ) D,       SYS.ALL_VIRTUAL_TABLE_REAL_AGENT MT     WHERE A.USER_ID = B.RUN_USER_ID       AND B.REFRESH_ID = C.REFRESH_ID (+)       AND B.REFRESH_ID = D.REFRESH_ID (+)       AND B.TENANT_ID = MT.TENANT_ID       AND B.MVIEW_ID = MT.TABLE_ID       AND MT.TABLE_TYPE = 7       AND A.TENANT_ID = EFFECTIVE_TENANT_ID()       AND B.TENANT_ID = EFFECTIVE_TENANT_ID()       AND C.TENANT_ID (+) = EFFECTIVE_TENANT_ID()       AND D.TENANT_ID (+) = EFFECTIVE_TENANT_ID()       AND MT.TENANT_ID = EFFECTIVE_TENANT_ID()       AND A.USER_NAME = SYS_CONTEXT('USERENV','CURRENT_USER') )__"))) {
      LOG_ERROR("fail to set view_definition", K(ret));
    }
  }
  table_schema.set_index_using_type(USING_BTREE);
...
int ObInnerTableSchema::dba_mvref_change_stats_ora_schema(ObTableSchema &table_schema)
{
  ...
  if (OB_SUCC(ret)) {
    if (OB_FAIL(table_schema.set_view_definition(R"__(     SELECT       CAST(C.DATABASE_NAME AS VARCHAR2(128)) AS TBL_OWNER,       CAST(NVL(F.TABLE_NAME, D.TABLE_NAME) AS VARCHAR2(128)) AS TBL_NAME,       CAST(A.DATABASE_NAME AS VARCHAR2(128)) AS MV_OWNER,       CAST(B.TABLE_NAME AS VARCHAR2(128)) AS MV_NAME,       CAST(E.REFRESH_ID AS NUMBER) AS REFRESH_ID,       CAST(E.RETRY_ID AS NUMBER) AS RETRY_ID,       CAST(E.NUM_ROWS_INS AS NUMBER) AS NUM_ROWS_INS,       CAST(E.NUM_ROWS_UPD AS NUMBER) AS NUM_ROWS_UPD,       CAST(E.NUM_ROWS_DEL AS NUMBER) AS NUM_ROWS_DEL,       CAST(0 AS NUMBER) AS NUM_ROWS_DL_INS,       CAST('N' AS CHAR(1)) AS PMOPS_OCCURRED,       CAST(NULL AS VARCHAR2(4000)) AS PMOP_DETAILS,       CAST(E.NUM_ROWS AS NUMBER) AS NUM_ROWS     FROM       SYS.ALL_VIRTUAL_DATABASE_REAL_AGENT A,       SYS.ALL_VIRTUAL_TABLE_REAL_AGENT B,       SYS.ALL_VIRTUAL_DATABASE_REAL_AGENT C,       SYS.ALL_VIRTUAL_TABLE_REAL_AGENT D,       SYS.ALL_VIRTUAL_MVIEW_REFRESH_CHANGE_STATS_REAL_AGENT E,       SYS.ALL_VIRTUAL_TABLE_REAL_AGENT F     WHERE A.DATABASE_ID = B.DATABASE_ID       AND C.DATABASE_ID = D.DATABASE_ID       AND E.MVIEW_ID = B.TABLE_ID       AND E.DETAIL_TABLE_ID = D.TABLE_ID       AND F.DATA_TABLE_ID(+) = D.TABLE_ID       AND F.TABLE_TYPE(+) = 7       AND A.TENANT_ID = EFFECTIVE_TENANT_ID()       AND B.TENANT_ID = EFFECTIVE_TENANT_ID()       AND C.TENANT_ID = EFFECTIVE_TENANT_ID()       AND D.TENANT_ID = EFFECTIVE_TENANT_ID()       AND E.TENANT_ID = EFFECTIVE_TENANT_ID()       AND F.TENANT_ID(+) = EFFECTIVE_TENANT_ID() )__"))) {
      LOG_ERROR("fail to set view_definition", K(ret));
    }
  }
  table_schema.set_index_using_type(USING_BTREE);
...
int ObInnerTableSchema::user_mvref_change_stats_ora_schema(ObTableSchema &table_schema)
{
  ...
  if (OB_SUCC(ret)) {
    if (OB_FAIL(table_schema.set_view_definition(R"__(     SELECT       CAST(C.DATABASE_NAME AS VARCHAR2(128)) AS TBL_OWNER,       CAST(NVL(F.TABLE_NAME, D.TABLE_NAME) AS VARCHAR2(128)) AS TBL_NAME,       CAST(A.DATABASE_NAME AS VARCHAR2(128)) AS MV_OWNER,       CAST(B.TABLE_NAME AS VARCHAR2(128)) AS MV_NAME,       CAST(E.REFRESH_ID AS NUMBER) AS REFRESH_ID,       CAST(E.RETRY_ID AS NUMBER) AS RETRY_ID,       CAST(E.NUM_ROWS_INS AS NUMBER) AS NUM_ROWS_INS,       CAST(E.NUM_ROWS_UPD AS NUMBER) AS NUM_ROWS_UPD,       CAST(E.NUM_ROWS_DEL AS NUMBER) AS NUM_ROWS_DEL,       CAST(0 AS NUMBER) AS NUM_ROWS_DL_INS,       CAST('N' AS CHAR(1)) AS PMOPS_OCCURRED,       CAST(NULL AS VARCHAR2(4000)) AS PMOP_DETAILS,       CAST(E.NUM_ROWS AS NUMBER) AS NUM_ROWS     FROM       SYS.ALL_VIRTUAL_DATABASE_REAL_AGENT A,       SYS.ALL_VIRTUAL_TABLE_REAL_AGENT B,       SYS.ALL_VIRTUAL_DATABASE_REAL_AGENT C,       SYS.ALL_VIRTUAL_TABLE_REAL_AGENT D,       SYS.ALL_VIRTUAL_MVIEW_REFRESH_CHANGE_STATS_REAL_AGENT E,       SYS.ALL_VIRTUAL_TABLE_REAL_AGENT F     WHERE A.DATABASE_ID = B.DATABASE_ID       AND C.DATABASE_ID = D.DATABASE_ID       AND E.MVIEW_ID = B.TABLE_ID       AND E.DETAIL_TABLE_ID = D.TABLE_ID       AND F.DATA_TABLE_ID(+) = D.TABLE_ID       AND F.TABLE_TYPE(+) = 7       AND A.TENANT_ID = EFFECTIVE_TENANT_ID()       AND B.TENANT_ID = EFFECTIVE_TENANT_ID()       AND C.TENANT_ID = EFFECTIVE_TENANT_ID()       AND D.TENANT_ID = EFFECTIVE_TENANT_ID()       AND E.TENANT_ID = EFFECTIVE_TENANT_ID()       AND F.TENANT_ID(+) = EFFECTIVE_TENANT_ID()       AND A.DATABASE_NAME = SYS_CONTEXT('USERENV','CURRENT_USER') )__"))) {
      LOG_ERROR("fail to set view_definition", K(ret));
    }
  }
  table_schema.set_index_using_type(USING_BTREE);
...
int ObInnerTableSchema::dba_mvref_stmt_stats_ora_schema(ObTableSchema &table_schema)
{
  ...
  if (OB_SUCC(ret)) {
    if (OB_FAIL(table_schema.set_view_definition(R"__(     SELECT       CAST(A.DATABASE_NAME AS VARCHAR2(128)) AS MV_OWNER,       CAST(B.TABLE_NAME AS VARCHAR2(128)) AS MV_NAME,       CAST(C.REFRESH_ID AS NUMBER) AS REFRESH_ID,       CAST(C.RETRY_ID AS NUMBER) AS RETRY_ID,       CAST(C.STEP AS NUMBER) AS STEP,       CAST(C.SQLID AS VARCHAR2(32)) AS SQLID /* TODO: VARCHAR2(14) */,       C.STMT AS STMT /* TODO: CLOB */,       CAST(C.START_TIME AS TIMESTAMP(6)) AS START_TIME,       CAST(C.EXECUTION_TIME AS NUMBER) AS EXECUTION_TIME,       C.EXECUTION_PLAN AS EXECUTION_PLAN /* TODO: XMLTYPE STORAGE BINARY */,       CAST(C.RESULT AS NUMBER) AS RESULT,       CAST(C.PARALLELISM AS NUMBER) AS PARALLELISM     FROM       SYS.ALL_VIRTUAL_DATABASE_REAL_AGENT A,       SYS.ALL_VIRTUAL_TABLE_REAL_AGENT B,       SYS.ALL_VIRTUAL_MVIEW_REFRESH_STMT_STATS_REAL_AGENT C     WHERE A.DATABASE_ID = B.DATABASE_ID       AND B.TABLE_ID = C.MVIEW_ID       AND A.TENANT_ID = EFFECTIVE_TENANT_ID()       AND B.TENANT_ID = EFFECTIVE_TENANT_ID()       AND C.TENANT_ID = EFFECTIVE_TENANT_ID() )__"))) {
      LOG_ERROR("fail to set view_definition", K(ret));
    }
  }
  table_schema.set_index_using_type(USING_BTREE);
