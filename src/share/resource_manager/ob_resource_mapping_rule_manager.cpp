    ...
    const ObString &plan)
{
  ...
  if (OB_SUCC(ret)) {
    ...
    if (OB_FAIL(proxy.get_all_resource_mapping_rules_by_user(tenant_id, plan, rules))) {
      ...
      LOG_INFO("refresh_resource_mapping_rule for user_id->group_id lookup",
               K(ret), K(tenant_id), K(plan), K(rules));
    }
  }

  // group_id -> group_name
  if (OB_SUCC(ret)) {
    ObResourceIdNameMappingRuleSet rules;
    if (OB_FAIL(proxy.get_all_resource_mapping_rules_for_plan(tenant_id, plan, rules))) {
      LOG_WARN("fail get resource mapping rules", K(tenant_id), K(ret));
    } else {
      for (int64_t i = 0; i < rules.count() && OB_SUCC(ret); ++i) {
        ObResourceIdNameMappingRule &rule = rules.at(i);
        if (OB_FAIL(group_map_.set_refactored(
                    common::combine_two_ids(rule.tenant_id_, rule.group_id_),
                    rule.group_name_,
                    0 /* don't overwrite */))) {
    ...
    LOG_INFO("refresh_resource_mapping_rule for group_id->group_name lookup",
             K(ret), K(tenant_id), K(plan), K(rules));
  }
