...
int ObServiceNameProxy::check_is_service_name_enabled(const uint64_t tenant_id)
{
  ...
  if (OB_UNLIKELY(!is_valid_tenant_id(tenant_id) || !is_user_tenant(tenant_id))) {
    ...
    LOG_WARN("tenant_data_version should be [4.2.1.9, 4.2.2.0) or [4.2.4.0, +infinity)",
    ...
    ret = OB_NOT_SUPPORTED;
    LOG_WARN("meta_tenant_data_version should be [4.2.1.9, 4.2.2.0) or [4.2.4.0, +infinity)",
