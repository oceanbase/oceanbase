...
int ObServiceNameProxy::check_is_service_name_enabled(const uint64_t tenant_id)
{
  ...
  } else if (tenant_data_version < DATA_VERSION_4_2_4_0) {
    ret = OB_NOT_SUPPORTED;
    LOG_WARN("tenant_data_version is smaller than 4_2_4_0", KR(ret), K(tenant_data_version));
  ...
  } else if (meta_tenant_data_version < DATA_VERSION_4_2_4_0) {
    ret = OB_NOT_SUPPORTED;
    LOG_WARN("meta_tenant_data_version is smaller than 4_2_4_0", KR(ret), K(meta_tenant_data_version));
  }
  return ret;
}
