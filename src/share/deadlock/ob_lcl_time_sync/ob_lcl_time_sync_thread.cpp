...
int ObLCLTimeSyncThread::update_local_time_from_rs_leader_(
  ...
  if (change_update_local_time_interval) {
    change_lcl_update_time_interval_(rs_leader_addr, resp.get_leader_current_time(), network_delay, client_recv_response_time);
  }
...
void ObLCLTimeSyncThread::change_lcl_update_time_interval_(const ObAddr &rs_leader_addr, int64_t leader_current_time, int64_t network_delay, int64_t client_recv_response_time) {
  if (rs_leader_addr != last_rs_leader_addr_) {
      last_rs_leader_addr_ = rs_leader_addr;
      auto_fit_update_time = UPDATE_TIME_FROM_CLIENT_MIN_INTERVAL;
    }
    if (time_in_expected_range_(network_delay, leader_current_time,
                               client_recv_response_time)) {
      if (auto_fit_update_time < UPDATE_TIME_FROM_CLIENT_MAX_INTERVAL) {
        auto_fit_update_time += ObLCLTimeSyncThread::INTERVAL_CHANGE_UNIT;
        DETECT_LOG(
            INFO,
      ...
      if (auto_fit_update_time > UPDATE_TIME_FROM_CLIENT_MIN_INTERVAL) {
        auto_fit_update_time -= ObLCLTimeSyncThread::INTERVAL_CHANGE_UNIT;
        DETECT_LOG(INFO,
...
int ObLCLTimeSyncThread::get_global_time_from_rs_leader_(
  ...
  if (OB_FAIL(time_sync_rpc_->update_local_time_message(
    ...
    DETECT_LOG(WARN, "update local time failed",
               KR(ret), K(rs_leader_addr));
  }
...
void ObLCLTimeSyncThread::set_lcl_local_time_offset_and_last_update_time_(
  ...
  report_update_lcl_time_info_to_inner_table_(local_real_time, local_offset_, auto_fit_update_time);
...
int64_t ObLCLTimeSyncThread::get_lcl_real_local_time() {
  struct timespec ts;
  clock_gettime(CLOCK_MONOTONIC, &ts);
  int64_t lcl_time_client_offset = ObServerConfig::get_instance()._lcl_time_client_offset;
  return static_cast<int64_t>(ts.tv_sec) * 1000000L +
         static_cast<int64_t>(ts.tv_nsec / 1000L) + (lcl_time_client_offset * client_rand_per_for_test);
}
