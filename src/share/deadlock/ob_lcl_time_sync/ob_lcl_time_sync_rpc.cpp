...
int ObDeadLockTimeSyncMessageP::process_msg(ObDeadLockTimeSyncResp &resp, const ObDeadLockTimeSyncArg &arg) {
  ...
  int64_t lcl_time_skew_rate = ObServerConfig::get_instance().lcl_time_skew_rate;
  int64_t clock_skew_offset_for_test = 0;
  if (lcl_time_skew_rate != 0) {
    // every 10s the offset is clock_skew_rate ms
    double clock_skew_rate = 1.0 * lcl_time_skew_rate / 10000;
  ...
  resp.set_leader_current_us(leader_real_ts);
  return ret;
}
