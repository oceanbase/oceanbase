...
int ObIvfIndexSearchHelper::init(
    ...
    sql::ObSQLSessionInfo *session)
{
  ...
  if (IS_INIT) {
  ...
  } else if (OB_FAIL(get_vector_probes(session, n_probes_))) {
    LOG_WARN("failed to get vector probes", K(ret), K_(n_probes));
  } else {
    ...
    qvector_ = qvector;
    conn_ = conn;
    allocator_.set_attr(lib::ObMemAttr(tenant_id, scan_label_));
    ...
    if (OB_FAIL(schema::ObMultiVersionSchemaService::get_instance().get_tenant_schema_guard(
      ...
      if (n_probes_ == 0) {
        n_probes_ = sqrt(index_table_schema->get_vector_ivf_lists());
...
int ObIvfIndexSearchHelper::alloc_rows(const bool need_objs)
{
  int ret = OB_SUCCESS;
  if (OB_ISNULL(rows_ = static_cast<ObIvfRow*>(allocator_.alloc(ann_k_ * sizeof(ObIvfRow))))) {
...
int ObIvfIndexSearchHelper::reset_centers()
{
  int ret = OB_SUCCESS;
...
int ObIvfIndexSearchHelper::reload_centers(const common::ObIArray<ObTypeVector *> &array)
{
  int ret = OB_SUCCESS;
  if (array.empty()) {
  } else if (OB_FAIL(MTL(ObTenantIvfCenterCache *)->put(index_table_id_, partition_idx_, distance_type_, array))) {
    LOG_WARN("failed to put int cache", K(ret));
  } else if (OB_FAIL(MTL(ObTenantIvfCenterCache *)->get(index_table_id_, partition_idx_, centers_))) {
    LOG_WARN("failed to get center cache", K(ret), K(index_table_id_), K(partition_idx_));
  }
  return ret;
}

int ObIvfIndexSearchHelper::set_partition_name(common::ObTabletID &tablet_id)
{
...
void ObIvfIndexSearchHelper::reuse()
{
  center_heap_.reset();
  row_cnt_ = 0;
}

void ObIvfIndexSearchHelper::destroy()
{
  is_inited_ = false;
...
int ObIvfIndexSearchHelper::get_centers_by_sql(
    schema::ObSchemaGetterGuard &schema_guard,
      ...
      SMART_VAR(ObMySQLProxy::MySQLResult, read_res) {
        ...
        if (OB_FAIL(conn.execute_read(tenant_id_, sql_str.ptr(), read_res))) {
          ...
          while (OB_SUCC(ret)) {
            ...
            } else if (OB_FAIL(update_center_heap(center_heap_, center_idx, tmp_vec, qvector_, distance, distance_type))) {
              LOG_WARN("failed to update center heap", K(ret), K(center_idx), K(tmp_vec), K(distance));
            }
          }
        }
      }
...
int ObIvfIndexSearchHelper::update_center_heap(common::ObBinaryHeap<HeapCenterItem, HeapCompare, 64> &heap, const double center_idx,
    const ObTypeVector &tmp_vec, const ObTypeVector &qvector, double distance, const ObVectorDistanceType distance_type)
{
  int ret = OB_SUCCESS;
  if (center_idx < 0) {
    ret = OB_INVALID_ARGUMENT;
    LOG_WARN("invalid center idx", K(ret), K(center_idx));
  } else if (L2 == distance_type && OB_FAIL(tmp_vec.cal_l2_square(qvector, distance))) {
    ...
    if (heap.count() < n_probes_) {
      if (OB_FAIL(heap.push(HeapCenterItem(distance, center_idx)))) {
        LOG_WARN("failed to push into heap", K(ret), K(distance), K(center_idx));
      }
    } else { // update heap
      const HeapCenterItem &top = heap.top();
      if (distance < top.distance_) {
        HeapCenterItem tmp(distance, center_idx);
        if (OB_FAIL(heap.replace_top(tmp))) {
          LOG_WARN("failed to replace top", K(ret), K(tmp));
        }
      }
    }
  }
  LOG_TRACE("success to get center", K(ret), K(tmp_vec), K(distance), K(heap.top().distance_));
  return ret;
}

int ObIvfIndexSearchHelper::get_centers_by_cache()
{
  int ret = OB_SUCCESS;
  if (OB_ISNULL(centers_)) {
    ...
    for (int64_t i = 0; i < centers_->count(); ++i) {
      ...
      if (OB_FAIL(update_center_heap(center_heap_, i, tmp_vec, qvector_, distance, distance_type))) {
        LOG_WARN("failed to update center heap", K(ret), K(tmp_vec), K(distance));
      }
    }
...
int ObIvfIndexSearchHelper::generate_centers_by_storage(
    const sql::ExprFixedArray *output_exprs,
    sql::ObEvalCtx *eval_ctx,
    ObIAllocator &allocator,
    ObIArray<ObTypeVector *> &centers)
{
  int ret = OB_SUCCESS;
  if (OB_ISNULL(output_exprs) || OB_ISNULL(eval_ctx)) {
    ...
    double distance = 0;
    LOG_TRACE("start to get centers", K(column_cnt), K(distance_type), KPC(output_exprs));
    if (OB_FAIL(parse_output_expr(output_exprs, eval_ctx, idx_array, tmp_vec))) {
      ...
      LOG_WARN("failed to update center heap", K(ret), K(center_idx), K(tmp_vec), K(distance));
    } else if (OB_FAIL(ObTypeVector::alloc_and_copy_vector(allocator, tmp_vec, center))) {
      LOG_WARN("failed to deep copy center", K(ret));
    } else if (OB_FAIL(centers.push_back(center))) {
...
int ObIvfIndexSearchHelper::prepare_rowkey_ranges(
...
int ObIvfIndexSearchHelper::prepare_rowkey_range(
    ObNewRange &key_range,
    const int64_t rowkey_cnt,
...
int ObIvfIndexSearchHelper::init_item_pool(const HeapRowItem &src_item)
{
  int ret = OB_SUCCESS;
...
int ObIvfIndexSearchHelper::alloc_item_from_pool(HeapRowItem &item)
{
  int ret = OB_SUCCESS;
...
int ObIvfIndexSearchHelper::alloc_row_item(
    const sql::ExprFixedArray *output_exprs,
...
int ObIvfIndexSearchHelper::update_row_heap(const HeapRowItem &row_item, const double distance)
{
  int ret = OB_SUCCESS;
  if (OB_SUCC(ret)) {
...
int ObIvfIndexSearchHelper::parse_output_expr(
    const sql::ExprFixedArray *output_exprs,
  ...
  for (int64_t i = 0; OB_SUCC(ret) && i < column_cnt; ++i) {
    ...
    ObDatum &col_datum = expr->locate_expr_datum(*eval_ctx);
    LOG_TRACE("get col datum", K(i), K(col_datum));
    if (OB_FAIL(col_datum.to_obj(tmp_obj, expr->obj_meta_, expr->obj_datum_map_))) {
      LOG_WARN("convert datum to obj failed", K(ret));
    } else if (tmp_obj.get_type() == ObIntType) {
      int64_t idx = -1;
      if (OB_FAIL(tmp_obj.get_int(idx))) {
        LOG_WARN("failed to get int", K(ret), K(tmp_obj));
      } else if (OB_FAIL(idx_array.push_back(idx))) {
        LOG_WARN("failed to push back idx array", K(ret), K(tmp_obj));
      }
    } else if (tmp_obj.get_type() == ObVectorType) { // vector
      if (OB_FAIL(tmp_obj.get_vector(vector))) {
...
int ObIvfIndexSearchHelper::get_rows_by_storage(
    ...
    sql::ObEvalCtx *eval_ctx)
{
  ...
  if (OB_ISNULL(extra_access_exprs) || OB_ISNULL(output_exprs) || OB_ISNULL(eval_ctx)) {
    ...
    ObDatum &col_datum = expr->locate_expr_datum(*eval_ctx);
    LOG_WARN("update row heap", K(ret), K(distance), K(ann_k_), K(row_heap_), K(row_heap_.count()));
    if (OB_FAIL(col_datum.to_obj(tmp_obj, expr->obj_meta_, expr->obj_datum_map_))) {
      LOG_WARN("convert datum to obj failed", K(ret));
      ...
      LOG_WARN("failed to cal distance", K(ret), K(tmp_vec), K(qvector_));
...
int ObIvfIndexSearchHelper::get_row(const int64_t idx, ObIvfRow *&row)
{
  int ret = OB_SUCCESS;
  if (idx >= row_cnt_) {
    ret = OB_ITER_END;
