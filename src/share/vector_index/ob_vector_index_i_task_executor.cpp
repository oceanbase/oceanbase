      ...
      if (OB_ISNULL(task_ctx)) {
        ...
        switch (task_ctx->task_status_.status_) {
          case ObVecIndexAsyncTaskStatus::OB_VECTOR_ASYNC_TASK_PREPARE:
          {
            ...
            int tmp_ret = OB_SUCCESS;
            if (is_conflict) {
              // PREPARE tasks have not started running yet, so they can be canceled and cleaned directly.
          ...
          case ObVecIndexAsyncTaskStatus::OB_VECTOR_ASYNC_TASK_RUNNING:
          {
            int tmp_ret = OB_SUCCESS;
            if (is_conflict) {
              // Running-stage tasks may be executing inner sql. Mark ret_code as
              // canceled first so inner loops can observe it, then cancel the
              // sys task and kill the running inner sql if needed.
