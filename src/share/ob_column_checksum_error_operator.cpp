...
{
  return (tenant_id_ != OB_INVALID_TENANT_ID) && (frozen_scn_ > 0)
    ...
    const ObColumnChecksumErrorInfo &info)
{
  ...
  if (!info.is_valid()
    ...
    LOG_WARN("invalid argument", KR(ret), K(tenant_id), K(info));
            ...
            || OB_FAIL(dml.add_pk_column("frozen_scn", info.frozen_scn_))
    ...
    const uint64_t min_frozen_scn)
{
  ...
  if (OB_UNLIKELY((!is_valid_tenant_id(tenant_id)))
      || (OB_INVALID_VERSION == min_frozen_scn)) {
    ret = OB_INVALID_ARGUMENT;
    LOG_WARN("invalid argument", KR(ret), K(tenant_id), K(min_frozen_scn));
             ...
             OB_ALL_COLUMN_CHECKSUM_ERROR_INFO_TNAME, tenant_id, min_frozen_scn))) {
