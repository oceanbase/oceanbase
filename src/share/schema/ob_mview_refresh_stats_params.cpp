  ...
  if (OB_FAIL(GET_MIN_DATA_VERSION(tenant_id, compat_version))) {
    ...
    if (with_sys_defaults) {
      if (OB_FAIL(sql.assign_fmt(
            "select "
            "  coalesce(p.collection_level, d.collection_level, %ld) as collection_level, "
            "  coalesce(p.retention_period, d.retention_period, %ld) as retention_period "
            "from "
            "  (select "
            "     max(collection_level) as collection_level, "
            "     max(retention_period) as retention_period "
            "   from %s "
            "   where tenant_id = 0) d "
            "inner join "
            "  (select "
            ...
            "     max(retention_period) as retention_period "
            "   from %s "
            "   where tenant_id = 0 and mview_id = %lu) p "
            "on 1 = 1 ",
