    ...
    ObDMLSqlSplicer &dml)
{
  ...
  } else if (data_version < DATA_VERSION_4_2_3_0 && table.is_new_queuing_table_mode()) {
    ret = OB_NOT_SUPPORTED;
    LOG_WARN("moderate/super/extreme table mode is not supported in data version less than 4.2.3", K(ret), K(table));
  } else {}
  if (OB_SUCC(ret)) {
                                              ...
                                              const ObString *ddl_stmt_str/*=NULL*/)
{
  ...
  } else if (data_version < DATA_VERSION_4_2_3_0 && new_table_schema.is_new_queuing_table_mode()) {
    ret = OB_NOT_SUPPORTED;
    LOG_WARN("moderate/super/extreme table mode is not supported in data version less than 4.2.3", K(ret), K(new_table_schema));
  } else if (OB_FAIL(dml.add_pk_column("tenant_id", ObSchemaUtils::get_extract_tenant_id(
                                             exec_tenant_id, tenant_id)))
      || OB_FAIL(dml.add_pk_column("table_id", ObSchemaUtils::get_extract_schema_id(
                                               exec_tenant_id, table_id)))
