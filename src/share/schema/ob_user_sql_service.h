...
public:
  ...
  virtual int create_user(const ObUserInfo& user, const int64_t new_schema_version,
      const common::ObString* ddl_stmt_str, common::ObISQLClient& sql_client);
  virtual int alter_user(const ObUserInfo& user, const int64_t new_schema_version, const common::ObString* ddl_stmt_str,
      common::ObISQLClient& sql_client);
  virtual int drop_user(const uint64_t tenant_id, const uint64_t user_id, const uint64_t new_schema_version,
      const common::ObString* ddl_stmt_str, common::ObISQLClient& sql_client,
      share::schema::ObSchemaGetterGuard& schema_guard);
  virtual int rename_user(const ObUserInfo& user_info, const int64_t new_schema_version,
      const common::ObString* ddl_stmt_str, common::ObISQLClient& sql_client);
  ...
  virtual int grant_revoke_user(const ObUserInfo& user_info, const int64_t new_schema_version,
      const common::ObString* ddl_stmt_str, common::ObISQLClient& sql_client);
  virtual int lock_user(const ObUserInfo& user_info, const int64_t new_schema_version,
      const common::ObString* ddl_stmt_str, common::ObISQLClient& sql_client);
  virtual int update_user_schema_version(const uint64_t tenant_id, const common::ObArray<ObUserInfo>& user_infos,
      const common::ObString* ddl_stmt_str, common::ObISQLClient& sql_client);
  virtual int alter_user_profile(
      const ObUserInfo& user_info, const common::ObString* ddl_stmt_str, common::ObISQLClient& sql_client);
  static int add_user_history(const ObUserInfo& user, const int64_t schema_version, common::ObISQLClient& sql_client);

  static int gen_user_dml(const uint64_t exec_tenant_id, const ObUserInfo& user, share::ObDMLSqlSplicer& dml);
