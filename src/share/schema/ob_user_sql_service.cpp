    ...
    const ObSchemaOperationType type)
{
  ...
  if (OB_INVALID_ID == tenant_id
    ...
    if (OB_SUCC(ret) && ((compat_version >= MOCK_DATA_VERSION_4_4_2_1 && compat_version < DATA_VERSION_4_5_0_0)
    ...
    const bool is_from_inner_sql)
{
  ...
  if (OB_FAIL(ret)) {
    ...
    ret = OB_NOT_SUPPORTED;
    LOG_WARN("some column of user info is not empty in current DATA_VERSION", K(ret), K(user.get_priv(OB_PRIV_APPLICATION_PASSWORD_ADMIN)));
