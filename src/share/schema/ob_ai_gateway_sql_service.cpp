...
namespace schema
{
int ObAIGatewaySqlService::create_ai_gateway(const ObAIGatewaySchema &new_schema,
                                             ...
                                             common::ObISQLClient &sql_client)
{
  ...
  if (OB_FAIL(GET_MIN_DATA_VERSION(tenant_id, data_version))) {
  ...
  } else if (OB_FAIL(sql.add_pk_column("gateway_id", new_schema.get_gateway_id()))) {
    ...
    LOG_WARN("failed to add column", K(ret), K(new_schema));
  } else if (OB_FAIL(sql.add_column("endpoints", ObHexEscapeSqlStr(new_schema.get_endpoints())))) {
    LOG_WARN("failed to add column", K(ret), K(new_schema));
  } else if (OB_FAIL(sql.add_column("circuit_breaker", ObHexEscapeSqlStr(new_schema.get_circuit_breaker())))) {
    LOG_WARN("failed to add column", K(ret), K(new_schema));
  } else if (OB_FAIL(sql.splice_insert_sql(OB_ALL_AI_GATEWAY_TNAME, buffer))) {
  ...
  } else if (OB_FAIL(sql.splice_insert_sql(OB_ALL_AI_GATEWAY_HISTORY_TNAME, buffer))) {
    LOG_WARN("failed to splice_insert_sql", K(ret));
    ...
    opt.ai_gateway_id_ = new_schema.get_gateway_id();
    opt.op_type_ = OB_DDL_CREATE_AI_GATEWAY;
    opt.schema_version_ = new_schema.get_schema_version();
    opt.ai_gateway_name_ = new_schema.get_name();
...
int ObAIGatewaySqlService::alter_ai_gateway(const ObAIGatewaySchema &new_schema,
                                            ...
                                            common::ObISQLClient &sql_client)
{
  ...
  if (OB_SUCC(ret)) {
    ...
    opt.tenant_id_ = new_schema.get_tenant_id();
    opt.ai_gateway_id_ = new_schema.get_gateway_id();
    opt.op_type_ = OB_DDL_ALTER_AI_GATEWAY;
...
int ObAIGatewaySqlService::drop_ai_gateway(const ObAIGatewaySchema &schema,
                                           const int64_t new_schema_version,
                                           const ObString &ddl_stmt,
                                           common::ObISQLClient &sql_client)
{
  ...
  if (OB_FAIL(GET_MIN_DATA_VERSION(tenant_id, data_version))) {
  ...
  } else if (OB_FAIL(sql.add_pk_column("gateway_id", schema.get_gateway_id()))) {
    LOG_WARN("failed to add column", K(ret), K(schema));
  } else if (OB_FAIL(sql.splice_delete_sql(OB_ALL_AI_GATEWAY_TNAME, buffer))) {
    ...
    if (OB_FAIL(sql.add_pk_column("tenant_id", 0))) {
      ...
      LOG_WARN("failed to add column", K(ret), K(schema));
    } else if (OB_FAIL(sql.add_column("endpoints", ObHexEscapeSqlStr(schema.get_endpoints())))) {
      LOG_WARN("failed to add column", K(ret), K(schema));
    } else if (OB_FAIL(sql.add_column("circuit_breaker", ObHexEscapeSqlStr(schema.get_circuit_breaker())))) {
      LOG_WARN("failed to add column", K(ret), K(schema));
    } else if (OB_FAIL(sql.splice_insert_sql(OB_ALL_AI_GATEWAY_HISTORY_TNAME, buffer))) {
      LOG_WARN("failed to splice_insert_sql", K(ret));
    } else if (OB_FAIL(sql_client.write(schema.get_tenant_id(), buffer.ptr(), affected_rows))) {
  ...
  if (OB_SUCC(ret)) {
    ...
    opt.ai_gateway_id_ = schema.get_gateway_id();
    opt.op_type_ = OB_DDL_DROP_AI_GATEWAY;
    opt.schema_version_ = new_schema_version;
    opt.ai_gateway_name_ = schema.get_name();
    opt.ddl_stmt_str_ = ddl_stmt;
