...
ObErrorInfo::ObErrorInfo(common::ObIAllocator* allocator) : ObSchema(allocator)
{
  reset();
}

ObErrorInfo::ObErrorInfo(const ObErrorInfo& src_schema) : ObSchema()
{
  reset();
  *this = src_schema;
}
...
bool ObErrorInfo::is_valid() const
{
  ...
  if (ObSchema::is_valid()) {
    if (is_user_field_valid()) {
      ret = (OB_INVALID_ID != database_id_) && (OB_INVALID_VERSION != schema_version_);
    } else {
    }
  } else {
  }
...
int ObErrorInfo::collect_error_info(const IObErrorInfo* info, const ObWarningBuffer* buf, bool fill_info)
{
  int ret = OB_SUCCESS;
  if (OB_ISNULL(info)) {
...
int ObErrorInfo::add_error(common::ObISQLClient& sql_client, bool is_replace, bool only_history)
{
  int ret = OB_SUCCESS;
...
int ObErrorInfo::gen_error_dml(const uint64_t exec_tenant_id, ObDMLSqlSplicer& dml)
{
  ...
  if (OB_FAIL(dml.add_pk_column("tenant_id", error_info.extract_tenant_id())) ||
      ...
      OB_FAIL(dml.add_column("schema_version", error_info.get_schema_version())) || OB_FAIL(dml.add_gmt_create()) ||
...
int ObErrorInfo::get_error_info_from_table(ObISQLClient& sql_client, ObErrorInfo* old_err_info)
{
  ...
  ObSqlString sql;
  uint64_t obj_seq = OB_INVALID_ID;
  if (false == is_valid()) {
    ret = OB_ERR_UNEXPECTED;
    LOG_WARN("error info is invalid", K(ret));
  } else if (sql.assign_fmt("SELECT * FROM %s WHERE obj_id = %ld AND tenant_id = %ld \
...
int ObErrorInfo::get_error_obj_seq(common::ObISQLClient& sql_client, bool& exist)
{
  int ret = OB_SUCCESS;
  ObErrorInfo& error_info = *this;
  ObSqlString sql;
...
int ObErrorInfo::handle_error_info(ObMySQLTransaction& trans, const IObErrorInfo* info)
{
  int ret = OB_SUCCESS;
  ObErrorInfo& error_info = *this;
  if (OB_FAIL(update_error_info(info))) {
