...
namespace share {
namespace schema {
int64_t ObSequenceHashWrapper::to_string(char* buf, const int64_t buf_len) const
{
  int64_t pos = 0;
  J_OBJ_START();
  J_KV(K_(tenant_id), K_(database_id), K_(sequence_name));
  J_OBJ_END();
  return pos;
}
using namespace std;
...
struct Del_Schema_In_Tenant_Action {
  ...
  {}
  int operator()(ObSequenceSchema* value, ObSequenceMgr::SequenceInfos& infos, ObSequenceMgr::ObSequenceMap& map)
  {
    int ret = OB_SUCCESS;
    ...
    if (OB_ISNULL(value)) {
    ...
    } else if (OB_FAIL(infos.remove_if(
                   value, ObSequenceMgr::compare_sequence, ObSequenceMgr::equal_sequence, value_to_del))) {
      ...
      ObSequenceHashWrapper hash_wrapper(value->get_tenant_id(), value->get_database_id(), value->get_sequence_name());
...
struct Del_Schema_Filter {
  ...
  bool operator()(ObSequenceSchema* value)
  {
    return value->get_tenant_id() == sequence_.tenant_id_ && value->get_sequence_id() == sequence_.sequence_id_;
  }
...
struct Get_Sequence_Schema_Action {
  Get_Sequence_Schema_Action(const ObSequenceSchema*& sequence_schema) : sequence_schema_(sequence_schema)
  {}
  int operator()(ObSequenceSchema* value, ObSequenceMgr::SequenceInfos& infos, ObSequenceMgr::ObSequenceMap& map)
  {
    UNUSED(infos);
...
struct Get_Sequence_Schema_EarlyStopCondition {
  Get_Sequence_Schema_EarlyStopCondition(const ObSequenceSchema*& sequence_schema) : sequence_schema_(sequence_schema)
  {}
  bool operator()()
  {
...
struct Get_Sequence_Filter {
  Get_Sequence_Filter(uint64_t db_id, uint64_t tenant_id) : db_id_(db_id), tenant_id_(tenant_id)
  {}
  bool operator()(ObSequenceSchema* value)
  {
...
struct Get_Sequence_Action {
  Get_Sequence_Action(ObIArray<const ObSequenceSchema*>& sequence_schemas) : sequence_schemas_(sequence_schemas)
  {}
  int operator()(ObSequenceSchema* value, ObSequenceMgr::SequenceInfos& infos, ObSequenceMgr::ObSequenceMap& map)
  {
    int ret = OB_SUCCESS;
...
struct Deep_Copy_Action {
  Deep_Copy_Action(ObSequenceMgr& sequence_mgr) : sequence_mgr_(sequence_mgr)
  {}
  int operator()(ObSequenceSchema* value, ObSequenceMgr::SequenceInfos& infos, ObSequenceMgr::ObSequenceMap& map)
  {
    int ret = OB_SUCCESS;
...
ObSequenceHashWrapper ObGetSequenceKey<ObSequenceHashWrapper, ObSequenceSchema*>::operator()(
    const ObSequenceSchema* sequence) const
{
...
int ObSequenceMgr::add_sequence(const ObSequenceSchema& sequence_schema)
{
  ...
  } else if (OB_FAIL(ObSchemaUtils::alloc_schema(allocator_, sequence_schema, new_sequence_schema))) {
  ...
  } else if (OB_FAIL(sequence_infos_.replace(
                 new_sequence_schema, iter, compare_sequence, equal_sequence, replaced_sequence))) {
...
int ObSequenceMgr::rebuild_sequence_hashmap(const SequenceInfos& sequence_infos, ObSequenceMap& sequence_map)
{
  int ret = OB_SUCCESS;
  sequence_map.clear();
  ...
  for (; iter != sequence_infos.end() && OB_SUCC(ret); ++iter) {
    ...
    if (OB_ISNULL(sequence_schema)) {
      ...
      bool overwrite = true;
      ObSequenceHashWrapper hash_wrapper(
          sequence_schema->get_tenant_id(), sequence_schema->get_database_id(), sequence_schema->get_sequence_name());
...
int ObSequenceMgr::get_sequence_schema_with_name(const uint64_t tenant_id, const uint64_t database_id,
    const ObString& name, const ObSequenceSchema*& sequence_schema) const
{
  int ret = OB_SUCCESS;
  ...
  if (!is_inited_) {
    ...
    if (OB_FAIL(sequence_map_.get_refactored(hash_wrap, tmp_schema))) {
      if (OB_HASH_NOT_EXIST == ret) {
        ret = OB_SUCCESS;
        LOG_INFO("sequence is not exist", K(tenant_id), K(database_id), K(name), "map_cnt", sequence_map_.item_count());
      }
    } else {
...
int ObSequenceMgr::get_object(const uint64_t tenant_id, const uint64_t database_id, const ObString& name,
    uint64_t& obj_database_id, uint64_t& sequence_id, ObString& obj_table_name, bool& do_exist) const
{
  int ret = OB_SUCCESS;
  do_exist = false;
  ...
  for (int64_t i = 0; OB_SUCC(ret) && false == do_exist && i < 2; ++i) {
    ...
    if (OB_FAIL(get_sequence_schema_with_name(tenant_id, cur_database_id, name, sequence_schema))) {
      LOG_WARN("fail to get sequence schema with name", K(tenant_id), K(cur_database_id), K(name), K(ret));
...
int ObSequenceMgr::del_sequence(const ObTenantSequenceId& sequence)
{
  ...
  if (!sequence.is_valid()) {
  ...
  } else if (OB_FAIL(sequence_infos_.remove_if(
                 sequence, compare_with_tenant_sequence_id, equal_to_tenant_sequence_id, schema_to_del))) {
    ...
    ObSequenceHashWrapper sequence_wrapper(
        schema_to_del->get_tenant_id(), schema_to_del->get_database_id(), schema_to_del->get_sequence_name());
...
int ObSequenceMgr::get_sequence_schemas_in_database(
    const uint64_t tenant_id, const uint64_t database_id, ObIArray<const ObSequenceSchema*>& sequence_schemas) const
{
  int ret = OB_SUCCESS;
...
int ObSequenceMgr::del_schemas_in_tenant(const uint64_t tenant_id)
{
  ...
  if (OB_INVALID_ID == tenant_id) {
    ...
    if (OB_FAIL(get_sequence_schemas_in_tenant(tenant_id, schemas))) {
      ...
      {
        ObTenantSequenceId tenant_sequence_id(tenant_id, (*schema)->get_sequence_id());
        if (OB_FAIL(del_sequence(tenant_sequence_id))) {
          LOG_WARN("del sequence failed",
              "tenant_id",
              tenant_sequence_id.tenant_id_,
              "sequence_id",
              tenant_sequence_id.sequence_id_,
              K(ret));
        }
      }
    }
...
int ObSequenceMgr::get_sequence_schema(const uint64_t sequence_id, const ObSequenceSchema*& sequence_schema) const
{
  int ret = OB_SUCCESS;
  sequence_schema = NULL;
...
bool ObSequenceMgr::compare_sequence(const ObSequenceSchema* lhs, const ObSequenceSchema* rhs)
{
  return lhs->get_sequence_id() < rhs->get_sequence_id();
}

bool ObSequenceMgr::equal_sequence(const ObSequenceSchema* lhs, const ObSequenceSchema* rhs)
{
  return lhs->get_sequence_id() == rhs->get_sequence_id();
}

bool ObSequenceMgr::compare_with_tenant_sequence_id(
    const ObSequenceSchema* lhs, const ObTenantSequenceId& tenant_sequence_id)
{
  return NULL != lhs ? (lhs->get_tenant_sequence_id() < tenant_sequence_id) : false;
}

bool ObSequenceMgr::equal_to_tenant_sequence_id(
    const ObSequenceSchema* lhs, const ObTenantSequenceId& tenant_sequence_id)
{
  return NULL != lhs ? (lhs->get_tenant_sequence_id() == tenant_sequence_id) : false;
}

int ObSequenceMgr::get_sequence_schemas_in_tenant(
    const uint64_t tenant_id, ObIArray<const ObSequenceSchema*>& sequence_schemas) const
{
  int ret = OB_SUCCESS;
  ...
  bool is_stop = false;
  for (ConstSequenceIter iter = tenant_sequence_begin; OB_SUCC(ret) && iter != sequence_infos_.end() && !is_stop;
       ++iter) {
    const ObSequenceSchema* sequence = NULL;
    if (OB_ISNULL(sequence = *iter)) {
      ret = OB_ERR_UNEXPECTED;
