...
ObSchemaGuardWrapper::ObSchemaGuardWrapper(const uint64_t tenant_id,
    ...
    : tenant_id_(tenant_id), ddl_service_(nullptr), schema_service_(schema_service),
                                       ...
                                       int64_t &schema_version)
{
  ...
  if (OB_FAIL(check_inner_stat_())) {
    ...
    if (OB_FAIL(local_schema_guard_.get_table_schema(tenant_id_, database_id, table_name, false/*is_index*/, table_schema))) {
    ...
    LOG_WARN("fail to get table id", KR(ret), K_(tenant_id), K(database_id), K(table_name));
  }
  return ret;
}
                                                          ...
                                                          ObIndexSchemaInfo &index_info)
{
          ...
          if (schema_service_impl->schema_name_is_equal(index_name,
            ...
            if (OB_FAIL(ob_write_string(allocator, index_table_schema->get_table_name_str(), tmp_coded_index_name, true/*c_style*/))) {
              LOG_WARN("fail to write string", KR(ret));
                                               ...
                                               index_table_schema->get_schema_version()))) {
  ...
  } else if (OB_FAIL(latest_schema_guard_.get_coded_index_name_info_mysql(allocator, database_id,
                                                                           data_table_id, index_name, index_info))) {
