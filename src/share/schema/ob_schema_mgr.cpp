...
ObSchemaMgr::ObSchemaMgr()
    : local_allocator_(SET_USE_500(ObModIds::OB_SCHEMA_GETTER_GUARD, ObCtxIds::SCHEMA_SERVICE)),
      ...
      ai_provider_mgr_(allocator_),
      ai_gateway_mgr_(allocator_)
{
}
...
int ObSchemaMgr::init(const uint64_t tenant_id)
{
  ...
  } else if (OB_FAIL(ai_provider_mgr_.init())) {
    LOG_WARN("init ai_provider_mgr_ failed", K(ret));
  } else if (OB_FAIL(ai_gateway_mgr_.init())) {
    LOG_WARN("init ai_gateway_mgr_ failed", K(ret));
...
void ObSchemaMgr::reset()
{
  ...
  if (!check_inner_stat()) {
    ...
    ai_model_mgr_.reset();
    ai_provider_mgr_.reset();
    ai_gateway_mgr_.reset();
  }
}
...
int ObSchemaMgr::assign(const ObSchemaMgr &other)
{
  ...
  if (!check_inner_stat()) {
    ...
    if (OB_SUCC(ret)) {
      ...
      } else if (OB_FAIL(ai_provider_mgr_.assign(other.ai_provider_mgr_))) {
        LOG_WARN("assign ai_provider_mgr_ failed", K(ret));
      } else if (OB_FAIL(ai_gateway_mgr_.assign(other.ai_gateway_mgr_))) {
        LOG_WARN("assign ai_gateway_mgr_ failed", K(ret));
...
int ObSchemaMgr::deep_copy(const ObSchemaMgr &other)
{
  ...
  if (!check_inner_stat()) {
    ...
    if (OB_SUCC(ret)) {
      ...
      } else if (OB_FAIL(ai_provider_mgr_.deep_copy(other.ai_provider_mgr_))) {
        LOG_WARN("deep copy ai_provider mgr failed", K(ret));
      } else if (OB_FAIL(ai_gateway_mgr_.deep_copy(other.ai_gateway_mgr_))) {
        LOG_WARN("deep copy ai_gateway mgr failed", K(ret));
...
int ObSchemaMgr::del_schemas_in_tenant(const uint64_t tenant_id)
{
  ...
  if (!check_inner_stat()) {
    ...
    if (OB_SUCC(ret)) {
      ...
      } else if (OB_FAIL(ai_provider_mgr_.del_schemas_in_tenant(tenant_id))) {
        LOG_WARN("del ai_provider in tenant failed", K(ret), K(tenant_id));
      } else if (OB_FAIL(ai_gateway_mgr_.del_schemas_in_tenant(tenant_id))) {
        LOG_WARN("del ai_gateway in tenant failed", K(ret), K(tenant_id));
...
int ObSchemaMgr::get_schema_count(int64_t &schema_count) const
{
  ...
  if (!check_inner_stat()) {
    ...
    int64_t ai_provider_schema_count = 0;
    int64_t ai_gateway_schema_count = 0;
    ...
    if (OB_FAIL(outline_mgr_.get_outline_schema_count(outline_schema_count))) {
    ...
    } else if (OB_FAIL(ai_provider_mgr_.get_ai_provider_schema_count(ai_provider_schema_count))) {
      LOG_WARN("get ai_provider schema count failed", K(ret));
    } else if (OB_FAIL(ai_gateway_mgr_.get_ai_gateway_schema_count(ai_gateway_schema_count))) {
      LOG_WARN("get ai_gateway schema count failed", K(ret));
      ...
      schema_count += (outline_schema_count + routine_schema_count + priv_schema_count
                       ...
                       + ai_provider_schema_count
                       + ai_gateway_schema_count
                      );
    }
  }
  return ret;
...
int ObSchemaMgr::get_schema_statistics(common::ObIArray<ObSchemaStatisticsInfo> &schema_infos) const
{
  ...
  if (!check_inner_stat()) {
    ...
    LOG_WARN("failed to push back schema statistics", K(ret), K(schema_info));
  } else if (OB_FAIL(ai_provider_mgr_.get_schema_statistics(schema_info))) {
    LOG_WARN("failed to get ai_provider statistics", K(ret));
  } else if (OB_FAIL(schema_infos.push_back(schema_info))) {
    LOG_WARN("failed to push back schema statistics", K(ret), K(schema_info));
  } else if (OB_FAIL(ai_gateway_mgr_.get_schema_statistics(schema_info))) {
    LOG_WARN("failed to get ai_gateway statistics", K(ret));
  } else if (OB_FAIL(schema_infos.push_back(schema_info))) {
    LOG_WARN("failed to push back schema statistics", K(ret), K(schema_info));
  }
  return ret;
