...
{
  static_assert(ARRAYSIZEOF(ObPrivMgr::priv_names_) == OB_PRIV_MAX_SHIFT_PLUS_ONE, "incomplete array priv_names_");
}
...
int ObPrivMgr::deep_copy(const ObPrivMgr& other)
{
  ...
  if (this != &other) {
    reset();
    for (DBPrivIter iter = other.db_privs_.begin(); OB_SUCC(ret) && iter != other.db_privs_.end(); iter++) {
    ...
    for (TablePrivIter iter = other.table_privs_.begin(); OB_SUCC(ret) && iter != other.table_privs_.end(); iter++) {
    ...
    for (SysPrivIter iter = other.sys_privs_.begin(); OB_SUCC(ret) && iter != other.sys_privs_.end(); iter++) {
    ...
    for (ObjPrivIter iter = other.obj_privs_.begin(); OB_SUCC(ret) && iter != other.obj_privs_.end(); iter++) {
...
{
  int ret = OB_SUCCESS;
  priv_schema_count = table_privs_.size() + db_privs_.size() + sys_privs_.size() + obj_privs_.size();
  return ret;
}
...
int ObPrivMgr::add_db_priv(const ObDBPriv& db_priv)
{
  ...
  if (OB_FAIL(ObSchemaUtils::alloc_schema(allocator_, db_priv, new_db_priv))) {
...
int ObPrivMgr::get_db_priv_iter(const ObOriginalDBKey& db_priv_key, DBPrivIter& target_db_priv_iter) const
{
  int ret = OB_SUCCESS;
  target_db_priv_iter = NULL;

  ObTenantUserId tenant_user_id(db_priv_key.tenant_id_, db_priv_key.user_id_);
  DBPrivIter tenant_db_priv_begin = db_privs_.lower_bound(tenant_user_id, ObDBPriv::cmp_tenant_user_id);
  bool is_stop = false;
  for (DBPrivIter db_priv_iter = tenant_db_priv_begin; OB_SUCC(ret) && db_priv_iter != db_privs_.end() && !is_stop;
       ++db_priv_iter) {
    const ObDBPriv* db_priv = NULL;
    if (OB_ISNULL(db_priv = *db_priv_iter)) {
      ret = OB_ERR_UNEXPECTED;
      LOG_WARN("NULL ptr", K(db_priv), K(ret));
    } else if (db_priv_key.tenant_id_ != db_priv->get_tenant_id() || db_priv_key.user_id_ != db_priv->get_user_id()) {
      is_stop = true;
...
int ObPrivMgr::get_db_priv(
    const ObOriginalDBKey& db_priv_key, const ObDBPriv*& db_priv, bool db_is_pattern /*default:false*/) const
{
  int ret = OB_SUCCESS;
  db_priv = NULL;
  ...
  ConstDBPrivIter tenant_db_priv_begin = db_privs_.lower_bound(tenant_user_id, ObDBPriv::cmp_tenant_user_id);
  bool is_stop = false;
  for (ConstDBPrivIter db_priv_iter = tenant_db_priv_begin; OB_SUCC(ret) && db_priv_iter != db_privs_.end() && !is_stop;
       ++db_priv_iter) {
    const ObDBPriv* tmp_db_priv = NULL;
    if (OB_ISNULL(tmp_db_priv = *db_priv_iter)) {
      ret = OB_ERR_UNEXPECTED;
      LOG_WARN("NULL ptr", K(tmp_db_priv), K(ret));
    } else if (tenant_id != tmp_db_priv->get_tenant_id() || user_id != tmp_db_priv->get_user_id()) {
      is_stop = true;
    } else if (0 != wild_compare(db, tmp_db_priv->get_database_name_str(), db_is_pattern)) {
      // do-nothing
    } else {
      db_priv = tmp_db_priv;
      is_stop = true;
  ...
  // anonymous user
  if (OB_SUCC(ret)) {
    if (!db_is_pattern && user_id != combine_id(tenant_id, OB_EMPTY_USER_ID) && NULL == db_priv) {
      ObOriginalDBKey new_db_priv_key(tenant_id, combine_id(tenant_id, OB_EMPTY_USER_ID), db);
...
int ObPrivMgr::get_db_priv_set(const ObOriginalDBKey& db_priv_key, ObPrivSet& priv_set, bool is_pattern) const
{
  int ret = OB_SUCCESS;
...
int ObPrivMgr::add_sys_priv(const ObSysPriv& sys_priv)
{
  ...
  if (OB_FAIL(ObSchemaUtils::alloc_schema(allocator_, sys_priv, new_sys_priv))) {
    LOG_WARN("alloc scheam failed", K(ret));
...
int ObPrivMgr::get_sys_priv_iter(const ObSysPrivKey& sys_priv_key, SysPrivIter& target_sys_priv_iter) const
{
  int ret = OB_SUCCESS;
  bool is_stop = false;
  ...
  for (SysPrivIter sys_priv_iter = sys_priv_begin;
       OB_SUCC(ret) && sys_priv_iter != NULL && sys_priv_iter != sys_privs_.end() && !is_stop;
       ++sys_priv_iter) {
...
int ObPrivMgr::get_sys_priv(const ObSysPrivKey& key, const ObSysPriv*& sys_priv) const
{
  int ret = OB_SUCCESS;
...
int ObPrivMgr::add_table_priv(const ObTablePriv& table_priv)
{
  ...
  if (OB_FAIL(ObSchemaUtils::alloc_schema(allocator_, table_priv, new_table_priv))) {
    LOG_WARN("alloc schema failed", K(ret));
  } else if (OB_ISNULL(new_table_priv)) {
    ret = OB_ERR_UNEXPECTED;
    LOG_WARN("NULL ptr", K(ret), K(new_table_priv));
  } else if (OB_FAIL(table_privs_.replace(
                 new_table_priv, iter, ObTablePriv::cmp, ObTablePriv::equal, replaced_table_priv))) {
    ...
    if (OB_SUCCESS != hash_ret && OB_HASH_EXIST != hash_ret) {
      ret = OB_ERR_UNEXPECTED;
      LOG_WARN(
          "Failed to build table_priv hashmap", "table_priv_key", new_table_priv->get_sort_key(), K(ret), K(hash_ret));
    }
  }
  ...
  if (table_privs_.count() != table_priv_map_.item_count()) {
    LOG_WARN("table priv is non-consistent between map and vector",
        "table_privs vector count",
        table_privs_.count(),
        "table_privs map size",
        table_priv_map_.item_count());
  }
...
int ObPrivMgr::add_obj_priv(const ObObjPriv& obj_priv)
{
  ...
  if (OB_FAIL(ObSchemaUtils::alloc_schema(allocator_, obj_priv, new_obj_priv))) {
    LOG_WARN("alloc schema failed", K(ret));
  } else if (OB_ISNULL(new_obj_priv)) {
    ret = OB_ERR_UNEXPECTED;
    LOG_WARN("NULL ptr", K(ret), K(new_obj_priv));
  } else if (OB_FAIL(obj_privs_.replace(new_obj_priv, iter, ObObjPriv::cmp, ObObjPriv::equal, replaced_obj_priv))) {
    ...
    if (OB_SUCCESS != hash_ret && OB_HASH_EXIST != hash_ret) {
      ret = OB_ERR_UNEXPECTED;
      LOG_WARN("Failed to build obj_priv hashmap", "obj_priv_key", new_obj_priv->get_sort_key(), K(ret), K(hash_ret));
    }
  }
  ...
  if (obj_privs_.count() != obj_priv_map_.item_count()) {
    LOG_WARN("obj priv is non-consistent between map and vector",
        "obj_privs vector count",
        obj_privs_.count(),
        "obj_privs map size",
        obj_priv_map_.item_count());
  }
...
int ObPrivMgr::del_table_priv(const ObTablePrivSortKey& table_priv_key)
{
  ...
  if (OB_FAIL(
          table_privs_.remove_if(table_priv_key, ObTablePriv::cmp_sort_key, ObTablePriv::equal_sort_key, table_priv))) {
...
int ObPrivMgr::del_obj_priv(const ObObjPrivSortKey& obj_priv_key)
{
  ...
  if (OB_FAIL(obj_privs_.remove_if(obj_priv_key, ObObjPriv::cmp_sort_key, ObObjPriv::equal_sort_key, obj_priv))) {
...
int ObPrivMgr::get_obj_priv(const ObObjPrivSortKey& obj_priv_key, const ObObjPriv*& obj_priv) const
{
  int ret = OB_SUCCESS;
  obj_priv = NULL;
...
int ObPrivMgr::get_table_priv(const ObTablePrivSortKey& table_priv_key, const ObTablePriv*& table_priv) const
{
  int ret = OB_SUCCESS;
  table_priv = NULL;
...
int ObPrivMgr::get_table_priv_set(const ObTablePrivSortKey& table_priv_key, ObPrivSet& priv_set) const
{
  int ret = OB_SUCCESS;
  priv_set = OB_PRIV_SET_EMPTY;
...
int ObPrivMgr::table_grant_in_db(
    const uint64_t tenant_id, const uint64_t user_id, const ObString& db, bool& is_grant) const
{
  int ret = OB_SUCCESS;
  is_grant = false;
  ...
  if (OB_INVALID_ID == tenant_id || OB_INVALID_ID == user_id || db.length() == 0) {
    ...
    ConstTablePrivIter iter = table_privs_.lower_bound(table_priv_db_key, ObTablePriv::cmp_db_key);
...
int ObPrivMgr::get_db_privs_in_tenant(const uint64_t tenant_id, ObIArray<const ObDBPriv*>& db_privs) const
{
  int ret = OB_SUCCESS;
  db_privs.reset();

  ConstDBPrivIter tenant_db_priv_begin = db_privs_.lower_bound(tenant_id, ObDBPriv::cmp_tenant_id);
  bool is_stop = false;
  for (ConstDBPrivIter iter = tenant_db_priv_begin; OB_SUCC(ret) && iter != db_privs_.end() && !is_stop; ++iter) {
    const ObDBPriv* db_priv = NULL;
...
int ObPrivMgr::get_db_privs_in_user(
    const uint64_t tenant_id, const uint64_t user_id, ObIArray<const ObDBPriv*>& db_privs) const
{
  int ret = OB_SUCCESS;
  db_privs.reset();
  ...
  for (ConstDBPrivIter iter = tenant_db_priv_begin; OB_SUCC(ret) && iter != db_privs_.end() && !is_stop; ++iter) {
    ...
    if (OB_ISNULL(db_priv = *iter)) {
      ret = OB_ERR_UNEXPECTED;
      LOG_WARN("NULL ptr", K(ret), K(db_priv));
    } else if (tenant_id != db_priv->get_tenant_id() || user_id != db_priv->get_user_id()) {
      is_stop = true;
    } else if (OB_FAIL(db_privs.push_back(db_priv))) {
...
int ObPrivMgr::get_table_privs_in_tenant(const uint64_t tenant_id, ObIArray<const ObTablePriv*>& table_privs) const
{
  int ret = OB_SUCCESS;
  table_privs.reset();

  ConstTablePrivIter tenant_table_priv_begin = table_privs_.lower_bound(tenant_id, ObTablePriv::cmp_tenant_id);
  bool is_stop = false;
  for (ConstTablePrivIter iter = tenant_table_priv_begin; OB_SUCC(ret) && iter != table_privs_.end() && !is_stop;
       ++iter) {
    const ObTablePriv* table_priv = NULL;
...
int ObPrivMgr::get_obj_privs_in_tenant(const uint64_t tenant_id, ObIArray<const ObObjPriv*>& obj_privs) const
{
  int ret = OB_SUCCESS;
  obj_privs.reset();

  ConstObjPrivIter tenant_obj_priv_begin = obj_privs_.lower_bound(tenant_id, ObObjPriv::cmp_tenant_id);
  bool is_stop = false;
  for (ConstObjPrivIter iter = tenant_obj_priv_begin; OB_SUCC(ret) && iter != obj_privs_.end() && !is_stop; ++iter) {
...
int ObPrivMgr::get_table_privs_in_user(
    const uint64_t tenant_id, const uint64_t user_id, ObIArray<const ObTablePriv*>& table_privs) const
{
  int ret = OB_SUCCESS;
  table_privs.reset();
  ...
  for (ConstTablePrivIter iter = tenant_table_priv_begin; OB_SUCC(ret) && iter != table_privs_.end() && !is_stop;
    ...
    } else if (tenant_id != table_priv->get_tenant_id() || user_id != table_priv->get_user_id()) {
      is_stop = true;
    } else if (OB_FAIL(table_privs.push_back(table_priv))) {
      LOG_WARN("push back table priv failed", K(ret));
    }
...
int ObPrivMgr::get_obj_privs_in_grantee(
    const uint64_t tenant_id, const uint64_t grantee_id, ObIArray<const ObObjPriv*>& obj_privs) const
{
  int ret = OB_SUCCESS;
  obj_privs.reset();

  ObTenantUserId tenant_user_id(tenant_id, grantee_id);
  ConstObjPrivIter tenant_obj_priv_begin = obj_privs_.lower_bound(tenant_user_id, ObObjPriv::cmp_tenant_user_id);
  bool is_stop = false;
  for (ConstObjPrivIter iter = tenant_obj_priv_begin; OB_SUCC(ret) && iter != obj_privs_.end() && !is_stop; ++iter) {
    const ObObjPriv* obj_priv = NULL;
    if (OB_ISNULL(obj_priv = *iter)) {
    ...
    } else if (tenant_id != obj_priv->get_tenant_id() || grantee_id != obj_priv->get_grantee_id()) {
      is_stop = true;
    } else if (OB_FAIL(obj_privs.push_back(obj_priv))) {
      LOG_WARN("push back obj priv failed", K(ret));
    }
...
int ObPrivMgr::get_obj_privs_in_grantor(
    const uint64_t tenant_id, const uint64_t grantor_id, ObIArray<const ObObjPriv*>& obj_privs, bool reset_flag) const
{
  int ret = OB_SUCCESS;
  if (reset_flag) {
    obj_privs.reset();
  }
  for (ConstObjPrivIter iter = obj_privs_.begin(); OB_SUCC(ret) && iter != obj_privs_.end(); ++iter) {
    const ObObjPriv* obj_priv = NULL;
    if (OB_ISNULL(obj_priv = *iter)) {
      ret = OB_ERR_UNEXPECTED;
      LOG_WARN("NULL ptr", K(ret), K(obj_priv));
    } else if (tenant_id == obj_priv->get_tenant_id() && grantor_id == obj_priv->get_grantor_id()) {
...
int ObPrivMgr::get_obj_privs_in_obj(const uint64_t tenant_id, const uint64_t obj_id, const uint64_t obj_type,
    ObIArray<const ObObjPriv*>& obj_privs, bool reset_flag) const
{
  int ret = OB_SUCCESS;
  if (reset_flag) {
    obj_privs.reset();
  ...
  for (ConstObjPrivIter iter = obj_privs_.begin(); OB_SUCC(ret) && iter != obj_privs_.end(); ++iter) {
    ...
    if (OB_ISNULL(obj_priv = *iter)) {
      ret = OB_ERR_UNEXPECTED;
      LOG_WARN("NULL ptr", K(ret), K(obj_priv));
    } else if (tenant_id == obj_priv->get_tenant_id() && obj_id == obj_priv->get_obj_id() &&
...
int ObPrivMgr::get_obj_privs_in_grantor_obj_id(
    const uint64_t tenant_id, const ObObjPrivSortKey& obj_key, ObIArray<const ObObjPriv*>& obj_privs) const
{
  int ret = OB_SUCCESS;
  ...
  for (ConstObjPrivIter iter = obj_privs_.begin(); OB_SUCC(ret) && iter != obj_privs_.end(); ++iter) {
    ...
    if (OB_ISNULL(obj_priv = *iter)) {
      ...
      LOG_WARN("NULL ptr", K(ret), K(obj_priv));
               ...
               obj_id == obj_priv->get_obj_id() && obj_type == obj_priv->get_objtype() &&
    ...
    const uint64_t tenant_id, const ObObjPrivSortKey& obj_key, ObIArray<const ObObjPriv*>& obj_privs) const
{
  ...
  ConstObjPrivIter tenant_obj_priv_begin = obj_privs_.lower_bound(tenant_ur_obj_id, ObObjPriv::cmp_tenant_ur_obj_id);
  bool is_stop = false;
  for (ConstObjPrivIter iter = tenant_obj_priv_begin; OB_SUCC(ret) && iter != obj_privs_.end() && !is_stop; ++iter) {
    const ObObjPriv* obj_priv = NULL;
    if (OB_ISNULL(obj_priv = *iter)) {
...
int ObPrivMgr::get_sys_privs_in_tenant(const uint64_t tenant_id, ObIArray<const ObSysPriv*>& sys_privs) const
{
  int ret = OB_SUCCESS;
  sys_privs.reset();

  ConstSysPrivIter tenant_sys_priv_begin = sys_privs_.lower_bound(tenant_id, ObSysPriv::cmp_tenant_id);
  bool is_stop = false;
  for (ConstSysPrivIter iter = tenant_sys_priv_begin; OB_SUCC(ret) && iter != sys_privs_.end() && !is_stop; ++iter) {
    const ObSysPriv* sys_priv = NULL;
    if (OB_ISNULL(sys_priv = *iter)) {
      ...
      LOG_WARN("push back sys priv faield", K(ret));
    }
  }
...
int ObPrivMgr::get_sys_priv_in_grantee(const uint64_t tenant_id, const uint64_t grantee_id, ObSysPriv*& sys_priv) const
{
  int ret = OB_SUCCESS;
  ObTenantUserId tenant_user_id(tenant_id, grantee_id);
  ConstSysPrivIter tenant_sys_priv_begin = sys_privs_.lower_bound(tenant_user_id, ObSysPriv::cmp_tenant_grantee_id);
  sys_priv = NULL;
  bool is_stop = false;
  for (ConstSysPrivIter iter = tenant_sys_priv_begin; OB_SUCC(ret) && iter != sys_privs_.end() && !is_stop; ++iter) {
    if (OB_ISNULL(*iter)) {
      ret = OB_ERR_UNEXPECTED;
      LOG_WARN("NULL ptr", K(ret), K(sys_priv));
    } else if (tenant_id != (*iter)->get_tenant_id() || grantee_id != (*iter)->get_grantee_id()) {
      is_stop = true;
    } else {
      sys_priv = *iter;
    }
...
{
  for (DBPrivIter iter = db_privs_.begin(); iter != db_privs_.end(); ++iter) {
  ...
  for (TablePrivIter iter = table_privs_.begin(); iter != table_privs_.end(); ++iter) {
  ...
  for (ObjPrivIter iter = obj_privs_.begin(); iter != obj_privs_.end(); ++iter) {
  ...
  for (SysPrivIter iter = sys_privs_.begin(); iter != sys_privs_.end(); ++iter) {
...
int ObPrivMgr::get_schema_statistics(const ObSchemaType schema_type, ObSchemaStatisticsInfo& schema_info) const
{
  int ret = OB_SUCCESS;
  schema_info.reset();
  if (TABLE_PRIV != schema_type && DATABASE_PRIV != schema_type && SYS_PRIV != schema_type && OBJ_PRIV != schema_type) {
    ret = OB_INVALID_ARGUMENT;
