...
#include "rootserver/freeze/ob_major_freeze_helper.h" // ObGlobalCompactionSchemaHelper
...
int ObSchemaHistoryRecycler::get_recycle_schema_version_by_major_version_(int64_t &recycle_schema_version)
{
  ...
  if (OB_FAIL(check_inner_stat_())) {
    LOG_WARN("fail to check inner stat", KR(ret));
  } else if (OB_FAIL(rootserver::ObGlobalCompactionSchemaHelper::get_global_safe_recycle_schema_version(
                          tenant_id_, *sql_proxy_, *schema_service_, specific_schema_version))) {
    LOG_WARN("fail to get safe recycle schema version by global merge info", KR(ret), K_(tenant_id));
  } else {
    update_min_schema_version_(recycle_schema_version, specific_schema_version);
    LOG_INFO("[SCHEMA_RECYCLE] get recycle schema version by major version",
             KR(ret), K_(tenant_id), K(specific_schema_version), K(recycle_schema_version));
  }
  return ret;
}
