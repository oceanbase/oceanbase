    ...
    ObDMLSqlSplicer &dml)
{
  ...
  if (OB_FAIL(GET_MIN_DATA_VERSION(tenant_id, compat_version))) {
    LOG_WARN("get min data_version failed", KR(ret), K(tenant_id));
      ...
      && (compat_version < MOCK_DATA_VERSION_4_4_2_1
      || (compat_version >= DATA_VERSION_4_5_0_0 && compat_version < DATA_VERSION_4_6_1_0))) {
    ret = OB_NOT_SUPPORTED;
    LOG_WARN("can not handle tablegroup scope before 4.6.1", KR(ret), K(tenant_id));
    LOG_USER_ERROR(OB_NOT_SUPPORTED, "handle tablegroup scope before 4.6.1 is");
