...
static inline bool is_support_oracle_java_routine(const uint64_t &cluster_version, const uint64_t &data_version)
{
  bool cluster_cond = (cluster_version >= MOCK_CLUSTER_VERSION_4_4_2_1 && cluster_version < CLUSTER_VERSION_4_6_0_0)
                        || (cluster_version >= CLUSTER_VERSION_4_6_1_0);
  bool data_cond = (data_version >= MOCK_DATA_VERSION_4_4_2_1 && data_version < DATA_VERSION_4_6_0_0)
                     || (data_version >= DATA_VERSION_4_6_1_0);

  return cluster_cond && data_cond;
}
