  ...
  if (OB_SUCC(ret) && !not_exist && !has_hit) {
    if (OB_FAIL(schema_service_->schema_fetcher_.fetch_schema(schema_type,
      ...
      if (OB_FAIL(ret)) {
                 ...
                 && !ObMultiVersionSchemaService::g_skip_resolve_materialized_view_definition_) {
        ...
        if (OB_FAIL(schema_service_->get_tenant_schema_guard(tenant_id, schema_guard))) {
          ...
          // do-nothing
        } else if (OB_FAIL(schema_service_->get_tenant_schema_guard(tenant_id, schema_guard, schema_version, true))) {
          LOG_WARN("get schema guard failed", K(tenant_id), K(schema_id), K(schema_version), K(ret));
        }
        if (FAILEDx(schema_service_->build_full_materalized_view_schema(schema_guard, allocator, table_schema))) {
    ...
    ObArray<ObRefreshSchemaInfo> &refresh_schema_infos)
{
  ...
  if (refresh_schema_infos.empty()) {
    ...
    const uint64_t local_sequence_id = local_schema_info.get_sequence_id();

    lib::ob_sort(refresh_schema_infos.begin(), refresh_schema_infos.end(), ObRefreshSchemaInfo::less_than);

    uint64_t pre_sequence_id = local_sequence_id;
    for (int64_t i = 0; OB_SUCC(ret) && consecutive && i < refresh_schema_infos.count(); i++) {
      ...
      const uint64_t sequence_id = schema_info.get_sequence_id();
      if (sequence_id != pre_sequence_id + 1) {
        consecutive = false;
      } else {
...
int ObMultiVersionSchemaService::cal_purge_need_timeout(int64_t &cal_timeout)
{
  int ret = OB_SUCCESS;
  if (OB_FAIL(ObShareUtil::get_ctx_timeout(GCONF._ob_ddl_timeout, cal_timeout))) {
