...
ObColumnSchemaV2::ObColumnSchemaV2() : ObSchema()
{
  reset();
}

ObColumnSchemaV2::ObColumnSchemaV2(ObIAllocator* allocator) : ObSchema(allocator)
{
  reset();
}

ObColumnSchemaV2::ObColumnSchemaV2(const ObColumnSchemaV2& src_schema) : ObSchema()
{
  reset();
  *this = src_schema;
}
...
bool ObColumnSchemaV2::is_prefix_column() const
{
  ...
  if (is_hidden() && is_generated_column()) {
    ...
    int64_t min_len = min(column_name_.length(), strlen(prefix_str));
...
{
  bool bret = false;
  if (is_hidden() && is_generated_column() && share::is_oracle_mode()) {
    const char* func_idx_str = "SYS_NC";
    int64_t min_len = min(column_name_.length(), strlen(func_idx_str));
...
OB_DEF_SERIALIZE(ObColumnSchemaV2)
{
  ...
  LST_DO_CODE(OB_UNIS_ENCODE, prev_column_id_, next_column_id_);
...
OB_DEF_DESERIALIZE(ObColumnSchemaV2)
{
  ...
  LST_DO_CODE(OB_UNIS_DECODE, prev_column_id_, next_column_id_);
...
OB_DEF_SERIALIZE_SIZE(ObColumnSchemaV2)
{
  ...
  LST_DO_CODE(OB_UNIS_ADD_LEN, prev_column_id_, next_column_id_);
...
void ObColumnSchemaV2::print(FILE* fd) const
{
  if (OB_ISNULL(fd)) {
    ...
    fprintf(fd,
        " column: %8ld %16ld %24.*s %16s %8d %8d %8d\n",
        column_id_,
        schema_version_,
        column_name_.length(),
        column_name_.ptr(),
        ob_obj_type_str(meta_type_.get_type()),
        accuracy_.get_length(),
        accuracy_.get_precision(),
        accuracy_.get_scale());
  }
}
...
int ObColumnSchemaV2::get_byte_length(int64_t& length, const bool for_check_length) const
{
  int ret = OB_SUCCESS;
  if (CS_TYPE_INVALID == get_collation_type()) {
    ...
    const ObLengthSemantics length_semantic = share::is_oracle_mode() ? get_length_semantics() : LS_CHAR;
...
int ObColumnSchemaV2::deserialize_extended_type_info(const char* buf, const int64_t data_len, int64_t& pos)
{
  int ret = OB_SUCCESS;
