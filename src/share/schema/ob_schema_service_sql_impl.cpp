    ...
    ObIArray<ObAuxTableMetaInfo> &aux_tables)
{
  ...
  SMART_VAR(ObMySQLProxy::MySQLResult, res) {
    ...
    const int64_t snapshot_timestamp = schema_status.snapshot_timestamp_;
    const uint64_t exec_tenant_id = fill_exec_tenant_id(schema_status);
    const static char *FETCH_INDEX_SQL_FORMAT = "SELECT table_id, table_type, index_type FROM "\
                                                "(SELECT /*+ index(%s idx_data_table_id) */ TABLE_ID, SCHEMA_VERSION, TABLE_TYPE, INDEX_TYPE, IS_DELETED, ROW_NUMBER() OVER " \
