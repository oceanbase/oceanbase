...
int ObSchemaServiceSQLImpl::fetch_recycle_object_by_original_name(const uint64_t tenant_id, const common::ObString& original_name,
    const ObRecycleObject::RecycleObjType recycle_obj_type, common::ObISQLClient& sql_client,
    const bool desc, common::ObIArray<ObRecycleObject>& recycle_objs)
{
  int ret = OB_SUCCESS;
