...
int ObUpgradeForAllVersionProcessor::post_upgrade_for_sys_table_progressive_merge_round_()
{
  int ret = OB_SUCCESS;
  ObSqlString sql;
  ...
  if (OB_ISNULL(sql_proxy_) || OB_ISNULL(schema_service_) || OB_ISNULL(common_proxy_)) {
    ret = OB_ERR_UNEXPECTED;
    LOG_WARN("unexpected error", KR(ret), KP(sql_proxy_), KP(schema_service_), KP(common_proxy_));
