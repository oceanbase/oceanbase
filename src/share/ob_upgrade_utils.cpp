...
const uint64_t ObUpgradeChecker::UPGRADE_PATH[DATA_VERSION_NUM] = {
  ...
  CALC_VERSION(4UL, 2UL, 1UL, 8UL),  // 4.2.1.8
  CALC_VERSION(4UL, 2UL, 1UL, 9UL),  // 4.2.1.9
  CALC_VERSION(4UL, 2UL, 1UL, 10UL),  // 4.2.1.10
  CALC_VERSION(4UL, 2UL, 1UL, 11UL),  // 4.2.1.11
};
...
#define CONVERT_CLUSTER_VERSION_TO_DATA_VERSION(CLUSTER_VERSION, DATA_VERSION) \
    ...
    CONVERT_CLUSTER_VERSION_TO_DATA_VERSION(CLUSTER_VERSION_4_2_1_7, DATA_VERSION_4_2_1_7)
    CONVERT_CLUSTER_VERSION_TO_DATA_VERSION(CLUSTER_VERSION_4_2_1_8, DATA_VERSION_4_2_1_8)
    CONVERT_CLUSTER_VERSION_TO_DATA_VERSION(CLUSTER_VERSION_4_2_1_9, DATA_VERSION_4_2_1_9)
    CONVERT_CLUSTER_VERSION_TO_DATA_VERSION(CLUSTER_VERSION_4_2_1_10, DATA_VERSION_4_2_1_10)
    CONVERT_CLUSTER_VERSION_TO_DATA_VERSION(CLUSTER_VERSION_4_2_1_11, DATA_VERSION_4_2_1_11)
...
#define INIT_PROCESSOR_BY_VERSION(MAJOR, MINOR, MAJOR_PATCH, MINOR_PATCH) \
    ...
    INIT_PROCESSOR_BY_VERSION(4, 2, 1, 8);
    INIT_PROCESSOR_BY_VERSION(4, 2, 1, 9);
...
int ObUpgradeFor4217Processor::post_upgrade()
{
  int ret = OB_SUCCESS;
  if (OB_FAIL(check_inner_stat())) {
    LOG_WARN("fail to check inner stat", KR(ret));
...
int ObUpgradeFor4217Processor::post_upgrade_for_spm()
{
  int ret = OB_SUCCESS;
  int64_t start = ObTimeUtility::current_time();
  ObSchemaGetterGuard schema_guard;
...
/* =========== 4217 upgrade processor end ============= */

/* =========== 4219 upgrade processor start ============= */
int ObUpgradeFor4219Processor::post_upgrade()
{
  int ret = OB_SUCCESS;
  if (OB_FAIL(check_inner_stat())) {
    LOG_WARN("fail to check inner stat", KR(ret));
  } else if (OB_FAIL(post_upgrade_for_service_name())) {
    LOG_WARN("post for upgrade for service_name", KR(ret));
...
int ObUpgradeFor4219Processor::post_upgrade_for_service_name()
{
  int ret = OB_SUCCESS;
  int64_t affected_rows = 0;
  if (OB_ISNULL(sql_proxy_)) {
...
int ObUpgradeFor4219Processor::post_upgrade_for_scheduled_trigger_partition_balance()
{
  int ret = OB_SUCCESS;
