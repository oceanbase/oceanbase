...
public:
  ...
  static int get_region_priority(const ObLocalityInfo& locality_info,
      const common::ObIArray<ObLocalityRegion>& tenant_locality_region, uint64_t& region_priority);
  static int get_zone_priority(const ObLocalityInfo& locality_info,
      const common::ObIArray<ObLocalityRegion>& tenant_locality_region, uint64_t& zone_priority);
};
