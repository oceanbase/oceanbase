...
constexpr int64_t ash_refresh_time = 10 * 1000L * 1000L;  // 10s
...
{
  int ret = OB_SUCCESS;
  common::ObTimeGuard time_guard(__func__, ash_refresh_time);
  ...
  if (ret == OB_SUCCESS) {
    ...
    if (require_snapshot_ahead()) {
      // TODO(lianhua): persist one machine's ash buffer use 0.5s, 2000 machine will be 1000s
      int64_t task_timeout_ts = ObTimeUtility::current_time() + ash_refresh_interval * 1000L * 1000L;
...
bool ObAshRefreshTask::require_snapshot_ahead()
{
  ...
  int64_t next_scheduled_snapshot_interval = GCTX.wr_service_->get_snapshot_interval() * 60 - (ObTimeUtility::current_time() - last_scheduled_snapshot_time_) / (1000L * 1000L);
  if (next_scheduled_snapshot_interval <= 0 || next_scheduled_snapshot_interval > GCTX.wr_service_->get_snapshot_interval() * 60) {
  ...
  int64_t cur_speed = (write_pos - prev_write_pos_) / (ash_refresh_interval / 1000 / 1000);
  int64_t expect_speed = free_slots_num / next_scheduled_snapshot_interval;
  bool ret = (cur_speed >= speed_threshold * expect_speed) && (next_scheduled_snapshot_interval > 60);
  return ret;
}
