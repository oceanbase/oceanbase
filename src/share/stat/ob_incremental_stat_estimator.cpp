...
/* Now, the following scenarios will try drive global stat:
 ...
 *
*/
int ObIncrementalStatEstimator::try_drive_global_stat(ObExecContext &ctx,
                                                      ...
                                                      ObIArray<ObOptStat> &opt_stats)
{
  ...
  if (extra.type_ == INVALID_LEVEL || extra.type_ == TABLE_LEVEL ||
    ...
    if (OB_FAIL(drive_part_stats_from_subpart_stats(ctx, param, opt_stats,
      ...
      LOG_WARN("failed to drive part stats from subpart stats", K(ret));
    } else {/*do nothing*/}
  } else if (OB_FAIL(drive_global_stat_from_part_stats(ctx, param,
                                                       approx_part_opt_stats, opt_stats))) {
    LOG_WARN("failed to drive global stat from part stats", K(ret));
  } else {/*do nothing*/}
  return ret;
}

int ObIncrementalStatEstimator::drive_global_stat_from_part_stats(
    ObExecContext &ctx,
    const ObTableStatParam &param,
    ...
    ObIArray<ObOptStat> &opt_stats)
{
  ...
  if (OB_FAIL(append(opt_stats, approx_part_opt_stats))) {
    ...
    //if specify part/subpart name, need get other already existed part stats, but only drive base
    ...
    //get regather partition stats(stats is locked or not stale).
      ...
      if (OB_SUCC(ret)) {
        if (OB_FAIL(get_no_regather_partition_stats(param, no_regather_part_ids,
                                                    table_stats, col_handles, tmp_opt_stats))) {
          LOG_WARN("failed to get locked partition stats", K(ret));
        } else {/*do nothing*/}
      }
    }
    if (OB_SUCC(ret)) {
      if (OB_FAIL(do_derive_global_stat(ctx, param, tmp_opt_stats, need_drive_hist, TABLE_LEVEL, -1,
                                        need_gather_hybrid_hist, global_opt_stat))) {
        ...
        LOG_TRACE("Succeed to drive global stat from part stats", K(opt_stats.count()), K(param));
      }
    }
  }
  return ret;
...
int ObIncrementalStatEstimator::drive_part_stats_from_subpart_stats(
    ObExecContext &ctx,
    const ObTableStatParam &param,
    ...
    ObIArray<ObOptStat> &approx_part_opt_stats)
{
  ...
  if (param.approx_part_infos_.empty()) {
    ...
    LOG_WARN("failed to get locked partition stats", K(ret));
  } else if (OB_FAIL(do_derive_part_stats_from_subpart_stats(ctx, param,
                                                             no_regather_subpart_opt_stats,
                                                             gather_opt_stats,
                                                             approx_part_opt_stats))) {
    LOG_WARN("failed to do derive part stats from subpart stats", K(ret));
  } else {
    LOG_TRACE("Succeed to drive part stats from subpart stats", K(approx_part_opt_stats.count()),
    ...
    ObIArray<ObOptStat> &approx_part_opt_stats)
{
  ...
  for (int64_t i = 0; OB_SUCC(ret) && i < param.approx_part_infos_.count(); ++i) {
    ...
    if (OB_SUCC(ret)) {
      ...
      } else if (OB_FAIL(do_derive_global_stat(ctx, param, subpart_opt_stats, true, PARTITION_LEVEL,
  ...
  if (OB_SUCC(ret) && !gather_hybrid_hist_opt_stats.empty()) {
    ...
    } else if (OB_FAIL(hybrid_est.estimate(part_param, extra, gather_hybrid_hist_opt_stats))) {
...
{
  int ret = OB_SUCCESS;
  if (OB_FAIL(derive_global_tbl_stat(ctx, param,  approx_level, partition_id,
                                     part_opt_stats, global_opt_stat))) {
    LOG_WARN("failed to derive global tbl stat from part tbl stat", K(ret));
  } else if (OB_FAIL(derive_global_col_stat(ctx, param, part_opt_stats, need_drive_hist,
                                                       ...
                                                       ObOptStat &global_opt_stat)
{
  ...
  if (OB_UNLIKELY(part_cnt <= 0 || column_cnt <= 0 ||
    ...
    for (int64_t i = 0; OB_SUCC(ret) && i < column_cnt; ++i) {
      ObOptColumnStat *col_stat = NULL;
      void *ptr = NULL;
      if (OB_ISNULL(ptr = ctx.get_allocator().alloc(sizeof(ObOptColumnStat)))) {
        ret = OB_ALLOCATE_MEMORY_FAILED;
        LOG_WARN("memory is not enough", K(ret), K(ptr));
      } else {
        ...
        if (OB_SUCC(ret)) {
          ...
          col_stat->set_llc_bitmap_size(ObColumnStat::NUM_LLC_BUCKET);
    ...
    ObIArray<ObOptStat> &part_opt_stats)
{
  ...
  if (no_regather_partition_ids.empty()) {
    /*do nothing*/
  } else if (OB_FAIL(get_column_ids(param, column_ids))) {
...
int ObIncrementalStatEstimator::get_column_ids(const ObTableStatParam &param,
                                               ObIArray<uint64_t> &column_ids)
{
  int ret = OB_SUCCESS;
    ...
    ObIArray<ObOptStat> &subpart_opt_stats)
{
  ...
  if (OB_SUCC(ret)) {
    if (OB_FAIL(get_no_regather_partition_stats(param,
