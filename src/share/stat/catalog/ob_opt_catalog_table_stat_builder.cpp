...
ObOptCatalogTableStatBuilder::ObOptCatalogTableStatBuilder()
    : tenant_id_(0), catalog_id_(0), database_name_(), table_name_(), partition_value_(),
      schema_version_(0), row_count_(0), file_num_(0), data_size_(0), last_analyzed_(0),
      partition_num_(0), is_basic_info_set_(false), is_stat_info_set_(false)
{
}
...
void ObOptCatalogTableStatBuilder::reset()
{
  tenant_id_ = 0;
  catalog_id_ = 0;
...
int ObOptCatalogTableStatBuilder::set_basic_info(uint64_t tenant_id,
                                                 uint64_t catalog_id,
                                                 ...
                                                 const common::ObString &partition_value)
{
  ...
  if (OB_UNLIKELY(tenant_id == OB_INVALID_ID || catalog_id == OB_INTERNAL_CATALOG_ID
                  || database_name.empty() || table_name.empty())) {
    ret = OB_ERR_UNEXPECTED;
    LOG_WARN("invalid arguments for basic info",
             K(ret),
             K(tenant_id),
             K(catalog_id),
             K(database_name),
...
int ObOptCatalogTableStatBuilder::set_stat_info(int64_t schema_version,
                                                int64_t row_count,
                                                int64_t file_num,
                                                int64_t data_size,
...
int ObOptCatalogTableStatBuilder::merge_table_stat(const ObOptCatalogTableStat &other)
{
  int ret = OB_SUCCESS;
...
int ObOptCatalogTableStatBuilder::merge_stat_values(int64_t row_count,
                                                    int64_t file_num,
                                                    int64_t data_size)
{
  int ret = OB_SUCCESS;
...
int ObOptCatalogTableStatBuilder::build(ObIAllocator &allocator, ObOptCatalogTableStat *&stat) const
{
  int ret = OB_SUCCESS;
  stat = nullptr;
  ...
  if (OB_UNLIKELY(!is_ready_to_build())) {
    ...
    LOG_WARN("failed to allocate memory for catalog table stat", K(ret), K(size));
  } else {
    int64_t pos = sizeof(ObOptCatalogTableStat);
    ObOptCatalogTableStat *tmp_stat = new (buf) ObOptCatalogTableStat();
    ...
    if (OB_FAIL(ObDbmsStatsUtils::deep_copy_string(buf,
      ...
      LOG_WARN("failed to deep copy partition value", K(ret), K(partition_value_));
    } else {
      // Set statistical information
      tmp_stat->schema_version_ = schema_version_;
      ...
      LOG_TRACE("catalog table stat built successfully",
                K(tenant_id_),
                K(catalog_id_),
                K(database_name_),
                K(table_name_),
...
int64_t ObOptCatalogTableStatBuilder::calculate_size() const
{
  return sizeof(ObOptCatalogTableStat) + database_name_.length() + table_name_.length()
         + partition_value_.length();
}

bool ObOptCatalogTableStatBuilder::is_ready_to_build() const
{
