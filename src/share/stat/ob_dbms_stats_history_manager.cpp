...
#define COLUMN_STAT_MOCK_VALUE_PATTERN "(%lu, %lu, %ld, %lu, usec_to_time(%ld), 0, 0, 0, 0, 0, \
...
{
  int ret = OB_SUCCESS;
  bool no_backup_having_col_stats = false;
  if (having_stat_part_col_map.size() != 0) {
    ...
    if (OB_LIKELY(having_stat_part_col_map.size() == partition_ids.count() * column_ids.count())) {
      ...
      if (OB_SUCC(ret)) {
        ...
        if (partition_list.empty() && part_col_list.empty()) {
          no_backup_having_col_stats = true;
        } else if (OB_FAIL(gen_column_list(column_ids, all_column_list))) {
          LOG_WARN("failed to gen partition list", K(ret));
    ...
    if (OB_SUCC(ret) && !no_backup_having_col_stats) {
      int64_t affected_rows = 0;
