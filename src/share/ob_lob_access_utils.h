...
{
  if (flags.has_read_snapshot_ == 1 && GET_MIN_CLUSTER_VERSION() < CLUSTER_VERSION_4_2_4_0) {
...
public:
    ...
    ctx_(nullptr), err_ret_(OB_SUCCESS), tmp_alloc_(nullptr)
  {
    ...
    cs_type_ = ObLobCharsetUtil::get_collation_type(type, cs_type);
  }
    ...
    datum_str_(obj.get_string()), ctx_(nullptr), err_ret_(OB_SUCCESS), tmp_alloc_(nullptr)
  {
    ...
    cs_type_ = ObLobCharsetUtil::get_collation_type(obj.get_type(), obj.get_collation_type());
  }
  ~ObTextStringIter();
