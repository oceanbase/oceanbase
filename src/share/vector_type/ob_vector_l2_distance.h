...
OB_DECLARE_AVX2_SPECIFIC_CODE(
  ...
  inline static float l2_norm_square(const float *a, size_t len) {
    ...
    while (len >= 16) {
        __m256 mx_0 = _mm256_loadu_ps(a);
        __m256 mx_1 = _mm256_loadu_ps(a + 8);
        msum_0 = _mm256_fmadd_ps(mx_0, mx_0, msum_0);
        __m256 msum_1 = _mm256_mul_ps(mx_1, mx_1);
    ...
    if (len > 0) {
        ...
        __m256 mx = _mm256_set_m128(rest_0, rest_1);
        msum_0 = _mm256_fmadd_ps(mx, mx, msum_0);
    }
    float res = _mm256_reduce_add_ps(msum_0);
    return res;
  }
...
OB_DECLARE_AVX512_SPECIFIC_CODE(
  ...
  inline static float l2_norm_square(const float *a, size_t len) {
    ...
    while (len >= 32) {
      __m512 mx_0 = _mm512_loadu_ps(a);
      __m512 mx_1 = _mm512_loadu_ps(a + 16);
    ...
    if (len > 0) {
      ...
      __m512 mx = _mm512_maskz_loadu_ps(mask, a);
      m512_res = _mm512_fmadd_ps(mx, mx, m512_res);
    }
