    ...
    if (OB_ISNULL(part_array)) {
      ...
      if (FAILEDx(dml.splice_batch_insert_update_sql(OB_ALL_PART_TNAME, part_sql))) {
        ...
        LOG_WARN("fail to write sql", KR(ret), K(part_sql),
            K(affected_rows));
      } else if (OB_UNLIKELY(2 *inc_part_num != affected_rows)) {
        ret = OB_ERR_UNEXPECTED;
        LOG_WARN("unexpected affected rows", KR(ret), K(inc_part_num), K(affected_rows), K(part_sql));
      } else if (FAILEDx(history_dml.splice_batch_insert_update_sql(OB_ALL_PART_HISTORY_TNAME, part_history_sql))) {
      ...
      FLOG_INFO("alter partition policy finish", KR(ret), K(part_sql), K(affected_rows), K(inc_part_num));
    }
  }
  return ret;
}
    ...
    if (OB_ISNULL(part_array)) {
      ...
      for (int64_t i = 0; OB_SUCC(ret) && i < inc_part_num; ++i) {
        ...
        if (OB_ISNULL(inc_part)) {
          ...
          total_subpart_num += inc_subpart_num;
      ...
      if (FAILEDx(dml.splice_batch_insert_update_sql(OB_ALL_SUB_PART_TNAME, subpart_sql))) {
        ...
        LOG_WARN("fail to write sql", KR(ret), K(subpart_sql),
            K(affected_rows));
      } else if (OB_UNLIKELY(2 * total_subpart_num != affected_rows)) {
        ret = OB_ERR_UNEXPECTED;
        LOG_WARN("unexpected affected rows", KR(ret), K(total_subpart_num), K(affected_rows), K(subpart_sql));
      } else if (FAILEDx(history_sub_dml.splice_batch_insert_update_sql(OB_ALL_SUB_PART_HISTORY_TNAME, subpart_history_sql))) {
      ...
      FLOG_INFO("alter subpartition policy success", KR(ret), K(subpart_sql), K(affected_rows), K(total_subpart_num));
    }
  }
  return ret;
}
