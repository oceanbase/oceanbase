  ...
  if (!inited_) {
    ...
    if (OB_FAIL(ObRootUtils::get_rs_default_timeout_ctx(ctx))) {
      LOG_WARN("fail to get timeout ctx", K(ret), K(ctx));
        ...
        "block_migrate_in_time, build_version, stop_time, start_service_time, with_partition "
...
int ObServerTableOperator::update(const ObServerStatus &server_status)
{
  ...
  if (!inited_) {
    ...
    LOG_WARN("get display status str failed", K(ret),
...
int ObServerTableOperator::update_status(const common::ObAddr &server,
...
int ObServerTableOperator::build_server_status(const ObMySQLResult &res,
...
int ObServerTableOperator::update_with_partition(const common::ObAddr &server,
    bool with_partition)
{
  int ret = OB_SUCCESS;
  char svr_ip[OB_IP_STR_BUFF] = "";
