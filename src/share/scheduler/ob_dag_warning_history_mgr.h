...
public:
  ...
  bool is_empty(const int64_t idx) { return free_node_bitset_.has_member(idx); }
...
public:
  ...
  void destroy();
  void clear();
  int get(const Key key, Value *&item);
  int get_with_no_lock(const Key key, Value *&item);
  int del_with_no_lock(const Key key);
  int alloc_and_add_with_no_lock(const Key key, Value *&item);
...
public:
  ...
  TO_STRING_KV(K_(tenant_id), K_(task_id), K_(dag_type), K_(dag_ret), K_(dag_status),
      K_(gmt_create), K_(gmt_modified), K_(retry_cnt), K_(warning_info));
...
public:
  ...
  virtual ~ObDagWarningInfoIterator() { reset(); }
...
int ObNodeArray<T>::get_node(int64_t &idx)
{
  ...
  if (OB_UNLIKELY(!is_inited_)) {
    ret = OB_NOT_INIT;
    COMMON_LOG(WARN, "ObNodeArray has not been inited", K(ret));
  } else if (free_node_bitset_.is_empty()) {
    ret = OB_SIZE_OVERFLOW;
    if (REACH_TIME_INTERVAL(30 * 1000 * 1000)) {
      COMMON_LOG(WARN, "info node is reach uplimits", K(ret), K(max_cnt_));
    }
  } else if (OB_FAIL(free_node_bitset_.find_first(idx)) || idx < 0) {
    COMMON_LOG(WARN, "failed to find free pos", K(ret), K(idx), K(max_cnt_));
  } else if (OB_FAIL(free_node_bitset_.del_member(idx))) {
    COMMON_LOG(WARN, "failed to del bitset member", K(ret), K(idx));
  }
  return ret;
}
...
int ObNodeArray<T>::free_node(const int64_t idx)
{
  ...
  if (OB_UNLIKELY(!is_inited_)) {
    ...
    COMMON_LOG(WARN, "invalid index", K(ret), K(idx), K(max_cnt_));
  } else if (OB_FAIL(free_node_bitset_.add_member(idx))){
    COMMON_LOG(WARN, "failed to add bitset member", K(ret), K(idx));
  } else {
    array_[idx].reset();
  }
  return ret;
...
template <typename Key, typename Value>
int ObInfoManager<Key, Value>:: init(
...
template <typename Key, typename Value>
void ObInfoManager<Key, Value>::destroy()
{
...
template <typename Key, typename Value>
int ObInfoManager<Key, Value>::get(const Key key, Value *&ptr)
{
  common::SpinRLockGuard guard(lock_);
  return get_with_no_lock(key, ptr);
}
...
int ObInfoManager<Key, Value>::get_with_no_lock(const Key key, Value *&ptr)
{
  ...
  if (OB_UNLIKELY(!is_inited_)) {
    ret = OB_NOT_INIT;
    COMMON_LOG(WARN, "ObInfoManager has not been inited", K(ret));
  } else if (OB_FAIL(map_.get_refactored(key, node_index))) {
    if (OB_HASH_NOT_EXIST != ret) {
      COMMON_LOG(WARN, "fail to get from map", K(ret), K(key));
    }
  } else if (NULL == (ptr = node_array_.at(node_index))) {
    ret = OB_ERR_UNEXPECTED;
    COMMON_LOG(WARN, "failed to get from map", K(ret), K(node_index), K(ptr));
  }
  return ret;
}

template <typename Key, typename Value>
int ObInfoManager<Key, Value>::del_with_no_lock(const Key key)
{
  int ret = OB_SUCCESS;
  int64_t node_index = -1;
  if (OB_UNLIKELY(!is_inited_)) {
    ...
    if (OB_HASH_NOT_EXIST != ret) {
      COMMON_LOG(WARN, "fail to get from map", K(ret));
    }
  } else if (OB_FAIL(map_.erase_refactored(key))) {
    COMMON_LOG(WARN, "fail to erase from map", K(ret));
  } else if (OB_FAIL(node_array_.free_node(node_index))) {
    COMMON_LOG(WARN, "fail to free node", K(ret), K(node_index));
  }
  return ret;
}

template <typename Key, typename Value>
int ObInfoManager<Key, Value>::alloc_and_add_with_no_lock(const Key key, Value *&ptr)
{
  int ret = OB_SUCCESS;
  if (OB_UNLIKELY(!is_inited_)) {
    ret = OB_NOT_INIT;
    ...
    if (OB_FAIL(node_array_.get_node(node_idx))) {
      if (OB_SIZE_OVERFLOW != ret) {
        COMMON_LOG(WARN, "fail to allocate memory", K(ret), K(sizeof(Value)));
      }
    } else if (NULL == (ptr = node_array_.at(node_idx))) {
      ret = OB_ERR_UNEXPECTED;
      COMMON_LOG(WARN, "fail to get node ptr from array", K(ret), K(node_idx));
    } else if (OB_FAIL(map_.set_refactored(key, node_idx))) {
      COMMON_LOG(WARN, "fail to set to map", K(ret));
    }
  }
  return ret;
}
