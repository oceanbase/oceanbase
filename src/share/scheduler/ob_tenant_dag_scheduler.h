...
public:
  enum ObITaskType
  {
    ...
    TASK_TYPE_DIRECT_LOAD_WRITE_OP = 116,
    TASK_TYPE_DIRECT_LOAD_MEM_SORT_OP = 117,
    TASK_TYPE_DIRECT_LOAD_COMPACT_TABLE_OP = 118,
    TASK_TYPE_DIRECT_LOAD_INSERT_SSTABLE_OP = 119,
    ...
    TASK_TYPE_DIRECT_LOAD_DIRECT_WRITE = 124,
...
public:
  ...
  bool is_empty() const
  {
    // Hold prio_rwlock_ and also check dag_map_: move_dag_to_list_ removes a dag
...
private:
  ...
  // Please lock prio_rwlock_ before calling the function with _ suffix
...
private:
  ...
  mutable common::SpinRWLock prio_rwlock_;  // Make sure the lock is outside if there are nested locks
  WorkerList waiting_workers_;  // workers waiting for time slice to run
  WorkerList running_workers_;  // running workers // lock with prio_rwlock_
  ObIAllocator* allocator_;
  ObIAllocator* ha_allocator_;
  ObTenantDagScheduler *scheduler_;
  int64_t priority_;
