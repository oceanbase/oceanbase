...
using oceanbase::lib::CoLocalStorage;
...
// Supports arbitrary POD assignment, does not distinguish between types, and compares with 128-bit binary to confirm
// whether they are equal (complement 0 for insufficient bits) Does not distinguish the differences brought about by
// different data types That is to say, int = 1 and uint64_t = 1 and trace_id = (1,0) are regarded as the same task_id
...
public:
  ...
  // The 128-bit split into upper and lower int64 is to facilitate the construction of RPC messages and provide
  // effective to_string messages
  uint64_t first_;
  uint64_t last_;
};
...
public:
  ...
  /// initialized, it will return false get_interrupt_code(): Get interrupt error code clear_status() resets the state
  /// of the inspector to the initial state register_checker(tid) to register yourself on the interrupt manager
  ObInterruptChecker(bool mod, uint64_t rid) : interrupted_(false), array_pos_(0), ref_count_(0), mod_(mod), rid_(rid)
  {
...
private:
  ...
  int64_t array_pos_;
  /*
   * ref_count_ is the reference count of the checker. When the reference count is 1,
   * unregister_interrupt will set interrupted_ to false.
   */
  int64_t ref_count_;
...
struct ObInterruptCheckerNode {
  ObInterruptCheckerNode(ObInterruptChecker* checker) : checker_(checker), next_(nullptr), prev_(nullptr)
  {}
...
public:
  ...
  ObInterruptGetCheckerNodeCall(ObInterruptChecker* checker)
      : checker_(checker), checker_node_(NULL), checker_exist_(false)
  {}
  inline bool is_checker_exist()
  {
    return checker_exist_;
  }
  ObInterruptCheckerNode* get_checker_node()
  {
...
public:
  ...
  typedef hash::ObHashMap<ObInterruptibleTaskID, ObInterruptCheckerNode*, hash::SpinReadWriteDefendMode,
      hash::hash_func<ObInterruptibleTaskID>, hash::equal_to<ObInterruptibleTaskID>,
      hash::SimpleAllocer<typename hash::HashMapTypes<ObInterruptibleTaskID, ObInterruptCheckerNode*>::AllocType,
...
/// The method of obtaining the interrupter, the execution on the coroutine will return a coroutine interrupter, and the
/// execution on the thread will return a thread-private interrupter
