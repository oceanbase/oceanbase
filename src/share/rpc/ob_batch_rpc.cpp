...
int build_batch_packet(const ObAddr& sender, const uint32_t batch_type, const uint32_t sub_type,
    const ObElectionGroupId& eg_id, const ObIFill& req, ObBatchPacket*& pkt, bool& is_dynamic_alloc)
{
  int ret = OB_SUCCESS;
  int64_t limit = 0;
  pkt = (ObBatchPacket*)get_rpc_buffer(limit);
  ...
  if (OB_ISNULL(pkt)) {
    ...
    ObSimpleReqHeader* req_header = (ObSimpleReqHeader*)(pkt + 1);
    ...
    } else if (OB_FAIL(eg_id.serialize(buf, limit - header_end_pos, pos))) {
      RPC_LOG(WARN, "serialize eg_id failed", K(ret), K(sender), K(batch_type), K(sub_type));
...
int build_batch_packet(const ObAddr& sender, const uint32_t batch_type, const uint32_t sub_type,
    const common::ObPartitionKey& pkey, const ObIFill& req, ObBatchPacket*& pkt, bool& is_dynamic_alloc)
{
  int ret = OB_SUCCESS;
  bool is_retry = false;
  ObCurTraceId::TraceId* trace_id = ObCurTraceId::get_trace_id();
  ...
  // Prioritize the allocation of memory locally from the thread.
  // If the allocated memory is not large enough, the memory is allocated dynamically
  while (true) {
    ...
    if (OB_ISNULL(pkt = (ObBatchPacket*)get_rpc_buffer(limit))) {
      ...
      if (req.get_estimate_size() > (limit * 4 / 5) || is_retry) {
        // 1024 more bytes are allocated to fill other fields
    ...
    if (OB_SUCC(ret)) {
      ...
      int64_t total_req_size = 0;
      if ((1 == flag) && OB_FAIL(trace_id->serialize(buf, limit - header_end_pos, pos))) {
        RPC_LOG(WARN, "serialize traceid failed", K(ret), K(sender), K(batch_type), K(sub_type), K(pkey));
      } else if (OB_FAIL(pkey.serialize(buf, limit - header_end_pos, pos))) {
        RPC_LOG(WARN, "serialize pkey failed", K(ret), K(sender), K(batch_type), K(sub_type), K(pkey));
      } else if (OB_FAIL(req.fill_buffer(buf + pos, limit - header_end_pos - pos, filled_size))) {
        if (OB_SIZE_OVERFLOW != ret) {
          RPC_LOG(WARN, "serialize request failed", K(ret), K(sender), K(batch_type), K(sub_type), K(pkey));
        }
      } else {
        total_req_size = pos + filled_size;
        pkt->set((int32_t)(sizeof(*req_header) + total_req_size), sender, (char*)(pkt + 1));
...
int64_t ObRpcBuffer::send(Rpc& rpc, uint64_t tenant_id, const ObAddr& sender, bool send_current)
{
  ...
  if (size > 0) {
    ...
    rpc.post_batch(tenant_id, dest, dst_cluster_id_, *pkt);
    buffer_.reuse(seq);
  }
  return size;
}
...
int ObBatchRpcBase::post(const uint64_t tenant_id, const ObAddr& dest, const int64_t dst_cluster_id,
    const uint32_t batch_type, const uint32_t sub_type, const ObPartitionKey& pkey, const Req& req)
{
  int ret = OB_SUCCESS;
  ...
  if (NULL == buffer || !buffer->fill(sub_type, pkey, req)) {
    Packet* pkt = NULL;
    if (OB_FAIL(build_batch_packet(self_, batch_type, sub_type, pkey, req, pkt, is_dynamic_alloc))) {
      RPC_LOG(WARN, "build_batch_packet fail", K(ret));
    } else if (OB_ISNULL(pkt)) {
      ret = OB_ERR_UNEXPECTED;
      RPC_LOG(WARN, "pkt is NULL", K(ret));
    } else {
      if (OB_FAIL(rpc_->post_batch(tenant_id, new_dest, dst_cluster_id, *pkt))) {
...
int ObBatchRpcBase::post(const uint64_t tenant_id, const ObAddr& dest, const int64_t dst_cluster_id,
    const uint32_t batch_type, const uint32_t sub_type, const ObElectionGroupId& eg_id, const Req& req)
{
  int ret = OB_SUCCESS;
  bool is_dynamic_alloc = false;
  CriticalGuard(get_qs());
  ...
  if (NULL == buffer || !buffer->fill(sub_type, eg_id, req)) {
    Packet* pkt = NULL;
    if (OB_FAIL(build_batch_packet(self_, batch_type, sub_type, eg_id, req, pkt, is_dynamic_alloc))) {
      RPC_LOG(WARN, "build_batch_packet fail", K(ret));
    } else if (OB_ISNULL(pkt)) {
      ret = OB_ERR_UNEXPECTED;
      RPC_LOG(WARN, "pkt is NULL", K(ret));
    } else {
      if (OB_FAIL(rpc_->post_batch(tenant_id, dest, dst_cluster_id, *pkt))) {
        RPC_LOG(WARN, "do_post fail", K(ret));
      }
      if (OB_UNLIKELY(is_dynamic_alloc)) {
        ob_free(pkt);
...
void ObBatchRpcBase::do_work()
{
  if (!is_inited_) {
    ...
    // send package, and count the items that can be gc
    RpcBuffer* iter = NULL;
    ...
    while (NULL != (iter = buffer_map_->quick_next(iter))) {
      ...
      static const int64_t CLEAN_SVR_INTERVAL =
          15 * 24 * 3600 * 1000 * 1000l;  // clean server when idle time reachs 15d
    ...
    for (int64_t i = 0; i < del_list.count(); ++i) {
      ...
      if (0 == buffer_map_->del(&cur_key, cur_buf)) {
        ...
        RPC_LOG(INFO,
            "batch_rpc delete server success",
            K(cur_server),
            K(cluster_id),
            K(tenant_id),
            K(hash_val),
...
ObRpcBuffer* ObBatchRpcBase::create_buffer(const uint64_t tenant_id, const ObAddr& addr, const int64_t dst_cluster_id)
{
  ...
  return (NULL == p) ? NULL
                     : new (p) RpcBuffer(tenant_id, addr, dst_cluster_id, batch_type_, buf_size, BATCH_BUFFER_COUNT);
}
      ...
      if (self_ == addr) {
        ...
        for (int64_t i = 0; i < cnt; ++i) {
          if (addr == dst_list.at(i).get_server()) {
            if (dst_cluster_id == dst_list.at(i).get_cluster_id() || common::OB_INVALID_CLUSTER_ID == dst_cluster_id) {
              // Filter scenarios with the same or invalid cluster_id
              is_need_add = false;
              break;
            }
          }
