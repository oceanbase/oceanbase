                    ...
                    const ObObj &in, ObObj &out, const ObCastMode cast_mode)
{
  ...
  if (CM_IS_COLUMN_CONVERT(cast_mode) && is_mysql_unsupported_json_column_conversion(in.get_type())) {
    ...
    ObString raw_bin;
    if (OB_FAIL(share::ObJsonAccessUtils::get_raw_binary(j_base, raw_bin, params.allocator_v2_))) {
                              ...
                              const ObObj *&res_obj)
{
  ...
  if (CM_IS_COLUMN_CONVERT(cast_ctx.cast_mode_)) {
    ...
    ObString raw_bin;
    if (OB_FAIL(share::ObJsonAccessUtils::get_raw_binary(j_base, raw_bin, cast_ctx.allocator_v2_))) {
