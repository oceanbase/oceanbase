...
OB_INLINE int numeric_range_check(
...
template <>
OB_INLINE int numeric_range_check<int64_t, uint64_t>(
    const int64_t in_val, const uint64_t min_out_val, const uint64_t max_out_val, uint64_t& out_val)
{
  int ret = OB_SUCCESS;
  if (in_val < 0) {
    ret = OB_DATA_OUT_OF_RANGE;
...
template <typename InType, typename OutType>
OB_INLINE int numeric_upper_check(const InType in_val, const OutType max_out_val, OutType& out_val)
{
  int ret = OB_SUCCESS;
...
OB_INLINE int int_range_check(const ObObjType out_type, const InType in_val, int64_t& out_val)
{
  return numeric_range_check(in_val, INT_MIN_VAL[out_type], INT_MAX_VAL[out_type], out_val);
}

template <typename InType>
OB_INLINE int int_upper_check(const ObObjType out_type, InType in_val, int64_t& out_val)
{
...
template <typename InType>
OB_INLINE int uint_range_check(const ObObjType out_type, const InType in_val, uint64_t& out_val)
{
  return numeric_range_check(in_val, static_cast<uint64_t>(0), UINT_MAX_VAL[out_type], out_val);
}

template <typename InType, typename OutType>
OB_INLINE int real_range_check(const ObObjType out_type, const InType in_val, OutType& out_val)
{
  return numeric_range_check(
      in_val, static_cast<OutType>(REAL_MIN_VAL[out_type]), static_cast<OutType>(REAL_MAX_VAL[out_type]), out_val);
}
...
int real_range_check(const ObAccuracy& accuracy, Type& value)
{
  ...
  if (OB_LIKELY(precision > 0) && OB_LIKELY(scale >= 0) && OB_LIKELY(precision >= scale)) {
    ...
    if (OB_FAIL(numeric_range_check(value, min_value, max_value, value))) {
      ...
      value = static_cast<Type>(rint((value - floor(static_cast<double>(value))) * decimal_part) / decimal_part +
...
inline uint64_t hex_to_uint64(const ObString& str)
{
  ...
  if (OB_LIKELY(NULL != p)) {
    for (int32_t i = 0; i < N; ++i, ++p) {
      // compatable with mysql, no overflow check
      value = value * 256 + *p;
    }
  }
  return value;
