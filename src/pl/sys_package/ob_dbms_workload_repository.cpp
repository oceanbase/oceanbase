...
const char *ASH_VIEW_SQL_425_BP3 =
"SELECT"
...
const char *WR_VIEW_SQL_425_BP3 =
"SELECT"
" /*+ LEADING(CDB_WR_ACTIVE_SESSION_HISTORY, CDB_WR_EVENT_NAME) USE_HASH(WR WR_EVENT_NAME) */ "
...
const char *ASH_VIEW_SQL_425_BP0 =
"SELECT"
"  ASH.TIME_MODEL AS TIME_MODEL,"
"  ASH.SVR_IP AS SVR_IP,"
"  ASH.SVR_PORT AS SVR_PORT,"
...
const char *WR_VIEW_SQL_425_BP0 =
"SELECT"
" WR.TIME_MODEL AS TIME_MODEL,"
" WR.SVR_IP AS SVR_IP,"
" WR.SVR_PORT AS SVR_PORT,"
...
#define EXTRACT_UINT_FIELD_FOR_ASH(result, column_name, field, type)              \
  if (OB_SUCC(ret)) {                                                             \
    ...
    if (OB_FAIL((result).get_type(column_name, _col_type))) {                     \
      LOG_WARN("get column type from result failed", K(ret), K(column_name));     \
    } else if (_col_type.is_unumber() || _col_type.is_number()) {                 \
...
#define EXTRACT_UINT_FIELD_FOR_ASH_STR(result, column_name, field, type)         \
  ...
  if (OB_SUCC(ret)) {                                                            \
    ...
    if (OB_FAIL((result).get_type(column_name, col_type))) {                     \
      LOG_WARN("get column type from result failed", K(ret), K(column_name));    \
    } else if (col_type.is_unumber() || col_type.is_number()) {                  \
      EXTRACT_UINT_FIELD_FROM_NUMBER_SKIP_RET(result, column_name, field, type); \
    } else {                                                                     \
      EXTRACT_UINT_FIELD_MYSQL_SKIP_RET(result, column_name, field, type);       \
    }                                                                            \
    ...
    const AshReportParams &ash_report_params, ObSqlString &sql_string)
{
  ...
  } else if (OB_FAIL(sql_string.append_fmt(data_version >= DATA_VERSION_4_2_5_3 ? ASH_VIEW_SQL_425_BP3
                                           : data_version >= DATA_VERSION_4_2_5_0 ? ASH_VIEW_SQL_425_BP0
                                           : data_version >= DATA_VERSION_4_2_4_0 ? ASH_VIEW_SQL_424
                                           : data_version >= DATA_VERSION_4_2_3_0 ? ASH_VIEW_SQL_423
                                           : data_version >= DATA_VERSION_4_2_2_0 ? ASH_VIEW_SQL_422
                                                                                  : ASH_VIEW_SQL_421,
    ...
    const AshReportParams &ash_report_params, ObSqlString &sql_string)
{
  ...
  if (OB_FAIL(GET_MIN_DATA_VERSION(MTL_ID(), data_version))) {
    ...
    LOG_WARN("failed to assign query string", K(ret));
             ...
             OB_FAIL(sql_string.append_fmt(data_version >= DATA_VERSION_4_2_5_3 ? WR_VIEW_SQL_425_BP3
                                           : data_version >= DATA_VERSION_4_2_5_0 ? WR_VIEW_SQL_425_BP0
                                           : data_version >= DATA_VERSION_4_2_4_0 ? WR_VIEW_SQL_424
  ...
  if (OB_FAIL(GET_MIN_DATA_VERSION(MTL_ID(), data_version))) {
    ...
    HEAP_VARS_2((ObISQLClient::ReadResult, res), (ObSqlString, sql_string))
    {
      ...
      if (OB_FAIL(print_section_column_header(ash_report_params, buff, headers))) {
        ...
        "WITH session_data AS ("
          "SELECT plsql_entry_object_id, plsql_entry_subprogram_id, plsql_entry_subprogram_name, "
                  "plsql_object_id, plsql_subprogram_id, plsql_subprogram_name, "
                  "IN_PLSQL_COMPILATION, IN_PLSQL_EXECUTION, IN_SQL_EXECUTION, "
        ...
        "top_plsql_entry AS ("
          ...
          "FROM session_data sd "
          "GROUP BY sd.plsql_entry_object_id, sd.plsql_entry_subprogram_id, sd.plsql_entry_subprogram_name "
        "), "))) {
        LOG_WARN("append sql failed", K(ret));
      } else if (OB_FAIL(sql_string.append_fmt(
        "top_pl_subprogram AS ("
          ...
          "GROUP BY plsql_entry_object_id, plsql_entry_subprogram_id, "
                    "plsql_object_id, plsql_subprogram_id, plsql_subprogram_name "
        "), ",
        lib::is_oracle_mode() ? "BITAND(time_model, 4096) > 0 OR BITAND(time_model, 2048) > 0"
                              : "(time_model & 4096) > 0 OR (time_model & 2048) > 0"))) {
        LOG_WARN("append sql failed", K(ret));
      } else if (OB_FAIL(sql_string.append_fmt(
        "top_sql_subprogram AS ("
          "SELECT plsql_entry_object_id, plsql_entry_subprogram_id, "
                  "NULL AS plsql_object_id, NULL AS plsql_subprogram_id, 'SQL' AS plsql_subprogram_name, "
                  "SUM(count_weight) AS subprogram_cnt "
          "FROM session_data sd "
          "WHERE IN_PLSQL_COMPILATION = 'N' AND IN_PLSQL_EXECUTION = 'N' AND IN_SQL_EXECUTION = 'Y' "
                 "AND (%s) "
          "GROUP BY plsql_entry_object_id, plsql_entry_subprogram_id "
        "), ",
        lib::is_oracle_mode() ? "BITAND(time_model, 16) > 0 AND BITAND(time_model, 4096) = 0 AND BITAND(time_model, 2048) = 0"
                              : "(time_model & 16) > 0 AND (time_model & 4096) = 0 AND (time_model & 2048) = 0"))) {
        LOG_WARN("append sql failed", K(ret));
      } else if (OB_FAIL(sql_string.append_fmt(
        "top_subprogram AS ("
          "SELECT plsql_entry_object_id, plsql_entry_subprogram_id, "
                  "plsql_object_id, plsql_subprogram_id, plsql_subprogram_name, subprogram_cnt, "
                  "ROW_NUMBER() OVER (PARTITION BY plsql_entry_object_id, plsql_entry_subprogram_id ORDER BY subprogram_cnt DESC) AS subprogram_rank "
          "FROM ("
        ...
        "pl_object AS ("
          "SELECT db.database_name AS owner, r.routine_name AS object_name, r.routine_id AS object_id "
          "FROM %s db, %s r "
          "WHERE r.database_id = db.database_id "
          "UNION ALL "
          "SELECT db.database_name AS owner, p.package_name AS object_name, p.package_id AS object_id "
          "FROM %s db, %s p "
          "WHERE p.database_id = db.database_id "
          "UNION ALL "
          "SELECT db.database_name AS owner, t.type_name AS object_name, t.type_id AS object_id "
          "FROM %s db, %s t "
          "WHERE t.database_id = db.database_id "
          "UNION ALL "
          "SELECT db.database_name AS owner, trg.trigger_name AS object_name, trg.trigger_id AS object_id "
          "FROM %s db, %s trg "
          "WHERE trg.database_id = db.database_id "
          "%s"
        ") ",
          lib::is_oracle_mode() ? "SYS.ALL_VIRTUAL_DATABASE_REAL_AGENT" : "oceanbase.__all_database",
          lib::is_oracle_mode() ? "SYS.ALL_VIRTUAL_ROUTINE_REAL_AGENT" : "oceanbase.__all_routine", //FROM %s db, %s r
          lib::is_oracle_mode() ? "SYS.ALL_VIRTUAL_DATABASE_REAL_AGENT" : "oceanbase.__all_database",
          lib::is_oracle_mode() ? "SYS.ALL_VIRTUAL_PACKAGE_REAL_AGENT" : "oceanbase.__all_package", //FROM %s db, %s p
          lib::is_oracle_mode() ? "SYS.ALL_VIRTUAL_DATABASE_REAL_AGENT" : "oceanbase.__all_database",
          lib::is_oracle_mode() ? "SYS.ALL_VIRTUAL_TYPE_REAL_AGENT" : "oceanbase.__all_type", //FROM %s db, %s t
          lib::is_oracle_mode() ? "SYS.ALL_VIRTUAL_DATABASE_REAL_AGENT" : "oceanbase.__all_database",
          lib::is_oracle_mode() ? "SYS.ALL_VIRTUAL_TENANT_TRIGGER_REAL_AGENT" : "oceanbase.__all_tenant_trigger", //FROM %s db, %s trg
          lib::is_oracle_mode() ? "UNION ALL "
                                  "SELECT db.database_name AS owner, ps.package_name AS object_name, ps.package_id AS object_id "
                                  "FROM SYS.ALL_VIRTUAL_DATABASE_REAL_AGENT db, SYS.ALL_VIRTUAL_PACKAGE_SYS_AGENT ps "
        ...
        lib::is_oracle_mode() ? "obj.owner" : "nvl(obj.owner, 'oceanbase')",  // sys package under MySQL mode cannot obtain the owner name
        lib::is_oracle_mode() ? "obj.object_name" : "nvl(obj.object_name, mysql_proc_info(te.plsql_entry_object_id, 'name'))",
        lib::is_oracle_mode() ? "sub_obj.owner" : "nvl(sub_obj.owner, 'oceanbase')",
        lib::is_oracle_mode() ? "sub_obj.object_name" : "nvl(sub_obj.object_name, mysql_proc_info(ts.plsql_object_id, 'name'))"))) {
        LOG_WARN("append sql failed", K(ret));
      } else if (OB_FAIL(sql_proxy->read(res, tenant_id, sql_string.ptr()))) {
        LOG_WARN("failed to execute sql", KR(ret), K(tenant_id), K(sql_string));
      } else if (OB_ISNULL(result = res.get_result())) {
    ...
    HEAP_VARS_2((ObISQLClient::ReadResult, res), (ObSqlString, sql_string))
    {
      ...
      if (OB_FAIL(sql_string.append_fmt(
        ...
        LOG_WARN("append sql failed", K(ret));
                     ...
                     data_version > DATA_VERSION_4_2_5_6
