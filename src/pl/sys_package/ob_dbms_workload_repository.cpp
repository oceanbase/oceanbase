  ...
  if (OB_ISNULL(GCTX.sql_proxy_)) {
    ...
    HEAP_VARS_2((ObISQLClient::ReadResult, res), (ObSqlString, sql_string))
    {
      ...
      if (OB_FAIL(print_section_column_header(
        ...
        "SELECT tg.tenant_id AS TENANT_ID, tg.group_id AS GROUP_ID, tg.group_time AS GROUP_TIME, "
                "tp.program AS PROGRAM, tp.program_time AS PROGRAM_TIME, tp.program_rank AS PROGRAM_RANK, "
        ...
        "ORDER BY tg.group_time DESC, tp.program_rank, tm.module_rank, ta.action_rank"))) {
        LOG_WARN("append sql failed", K(ret));
      } else if (OB_FAIL(sql_proxy->read(res, request_tenant_id, sql_string.ptr()))) {
        LOG_WARN("falied to execute sql", KR(ret), K(request_tenant_id), K(sql_string));
      } else if (OB_ISNULL(result = res.get_result())) {
