...
int ObDBMSVectorMySql::set_attribute(ObPLExecCtx &ctx, ParamStore &params, ObObj &result)
{
  UNUSED(result);
  int ret = OB_SUCCESS;
  CK(GET_MIN_CLUSTER_VERSION() >= CLUSTER_VERSION_4_4_2_0);
...
int ObDBMSVectorMySql::trigger_async_task(ObPLExecCtx &ctx, ParamStore &params, ObObj &result)
{
  ...
  if (OB_SUCC(ret)) {
    ...
    if (task_type_str.case_compare("FOLLOWER_IN_MEMORY_INDEX_REFRESH") != 0) {
      ...
      } else if (OB_FAIL(schema_checker.check_table_show(session_priv,
                                                         session->get_enable_role_array(),
                                                         OB_INTERNAL_CATALOG_ID,
                                                         base_db_name,
                                                         base_name,
                                                         allow_show))) {
        LOG_WARN("fail to check table show", KR(ret), K(base_db_name), K(base_name));
      } else if (!allow_show) {
        ret = OB_ERR_NO_TABLE_PRIVILEGE;
        LOG_WARN("user has no privilege to access base table", KR(ret),
                 K(base_db_name), K(base_name), K(session_priv.user_name_),
                 K(session_priv.host_name_));
        LOG_USER_ERROR(OB_ERR_NO_TABLE_PRIVILEGE,
                       static_cast<int>(sizeof("TRIGGER_ASYNC_TASK") - 1),
                       "TRIGGER_ASYNC_TASK",
