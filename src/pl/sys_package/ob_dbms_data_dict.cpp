    ...
    common::ObObj &result)
{
  ...
  DDLOG(INFO, "trigger dump data_dict begin", K(params));

  if (OB_FAIL(check_op_allowed_(ctx, tenant_id))) {
    DDLOG(WARN, "check_op_allowed failed", KR(ret));
  } else if (OB_UNLIKELY(1 != params.count())) {
    ret = OB_INVALID_ARGUMENT;
    DDLOG(WARN, "invalid params count for trigger_dump_data_dict", KR(ret), K(params));
    ...
    ret = OB_INVALID_ARGUMENT;
    DDLOG(WARN, "get int failed", KR(ret), K(params.at(0)));
    ...
    DDLOG(WARN, "get_tenant_gts failed", KR(ret), K(tenant_id));
  } else if (OB_FAIL(trigger_dump_data_dict_arg.init(base_scn, data_dict_dump_history_retention_sec))) {
    DDLOG(WARN, "init trigger_dump_data_dict_arg failed", KR(ret), K(base_scn), K(data_dict_dump_history_retention_sec), K(params));
  } else if (OB_FAIL(send_trigger_dump_dict_rpc_(tenant_id, trigger_dump_data_dict_arg))) {
    DDLOG(WARN, "send_trigger_dump_dict_rpc_ failed", KR(ret), K(tenant_id), K(trigger_dump_data_dict_arg));
  }

  if (OB_SUCC(ret)) {
    DDLOG(INFO, "trigger dump data_dict succ", KR(ret), K(errsim_code), K(params));
  } else {
    DDLOG(WARN, "trigger dump data_dict failed", KR(ret), K(errsim_code), K(params));
  }
    ...
    common::ObObj &result)
{
  ...
  if (OB_FAIL(check_op_allowed_(ctx, tenant_id))) {
    ...
    DDLOG(WARN, "modify_schedule_job_ attr enable SCHEDULED_TRIGGER_DUMP_DATA_DICT failed",
        KR(ret), K(tenant_id), K(params), K(is_job_enable));
  }

  if (OB_SUCC(ret)) {
    DDLOG(INFO, "enable dump data_dict succ", KR(ret), K(params));
  } else {
    DDLOG(WARN, "enable dump data_dict failed", KR(ret), K(params));
  }
    ...
    common::ObObj &result)
{
  ...
  if (OB_FAIL(check_op_allowed_(ctx, tenant_id))) {
    ...
    if (dump_interval > 0) {
      ...
      DDLOG(WARN, "can't disable dump_data_dict while dict still can dump by config dump_data_dictionary_to_log_interval",
  ...
  if (OB_SUCC(ret)) {
    DDLOG(INFO, "disable dump data_dict succ", KR(ret), K(params));
  } else {
    DDLOG(WARN, "disable dump data_dict failed", KR(ret), K(params));
  }
    ...
    common::ObObj &result)
{
  ...
  if (OB_FAIL(check_op_allowed_(ctx, tenant_id))) {
    ...
    ret = OB_INVALID_ARGUMENT;
    DDLOG(WARN, "invalid params count for modify_interval", KR(ret), K(params));
    ...
    DDLOG(WARN, "modify_schedule_job_ attr repeat_interval SCHEDULED_TRIGGER_DUMP_DATA_DICT failed",
        KR(ret), K(tenant_id), K(params), K(repeat_interval));
  }

  if (OB_SUCC(ret)) {
    DDLOG(INFO, "modify dump interval succ", KR(ret), K(params));
  } else {
    DDLOG(WARN, "modify dump interval failed", KR(ret), K(params));
  }
    ...
    common::ObObj &result)
{
  ...
  if (OB_FAIL(check_op_allowed_(ctx, tenant_id))) {
    ...
    ret = OB_INVALID_ARGUMENT;
    DDLOG(WARN, "invalid params count for modify_duration", KR(ret), K(params));
    ...
    DDLOG(WARN, "modify_schedule_job_ attr max_run_duration SCHEDULED_TRIGGER_DUMP_DATA_DICT failed",
        KR(ret), K(tenant_id), K(params), K(max_run_duration));
  }

  if (OB_SUCC(ret)) {
    DDLOG(INFO, "modify dump duration succ", KR(ret), K(params));
  } else {
    DDLOG(WARN, "modify dump duration failed", KR(ret), K(params));
  }
    ...
    common::ObObj &result)
{
  ...
  if (OB_FAIL(check_op_allowed_(ctx, tenant_id))) {
    ...
    ret = OB_INVALID_ARGUMENT;
    DDLOG(WARN, "invalid params count for modify_retention", KR(ret), K(params));
    ...
    DDLOG(WARN, "invalid data_dict_dump_history_retention_day", K(data_dict_dump_history_retention_day));
  } else if (OB_FAIL(check_scheduled_job_enabled_(tenant_id, is_scheduled_job_enabled))) {
    DDLOG(WARN, "check_scheduled_job_enabled_ failed", KR(ret), K(tenant_id));
  } else if (OB_UNLIKELY(! is_scheduled_job_enabled)) {
    ret = OB_OP_NOT_ALLOW;
    DDLOG(WARN, "scheduled job is not enabled, can not modify retention, please call DBMS_DATA_DICT.ENABLE_DUMP() then retry",
        KR(ret), K(tenant_id));
    ...
    DDLOG(WARN, "build job_action failed", KR(ret), K(tenant_id), K(job_action_str));
    ...
    DDLOG(WARN, "modify_schedule_job_ attr job_action SCHEDULED_TRIGGER_DUMP_DATA_DICT failed",
    ...
    DDLOG(WARN, "get_tenant_gts_ failed", KR(ret), K(tenant_id));
  } else if (OB_FAIL(trigger_dump_data_dict_arg.init(base_scn, data_dict_dump_history_retention_sec))) {
    DDLOG(WARN, "init trigger_dump_data_dict_arg failed", KR(ret), K(tenant_id), K(trigger_dump_data_dict_arg),
  ...
  if (OB_SUCC(ret)) {
    DDLOG(INFO, "modify dict_item retention succ", KR(ret), K(errsim_code), K(params));
  } else {
    DDLOG(WARN, "modify dict_item retention failed", KR(ret), K(errsim_code), K(params));
  }
...
int ObDBMSDataDict::check_op_allowed_(const sql::ObExecContext &ctx, uint64_t &tenant_id)
{
  ...
  if (OB_ISNULL(ctx.get_my_session())) {
    ret = OB_ERR_UNEXPECTED;
    DDLOG(WARN, "unexpected null session info", KR(ret));
  } else if (FALSE_IT(tenant_id = ctx.get_my_session()->get_effective_tenant_id())) {
  } else if (OB_UNLIKELY(!is_user_tenant(tenant_id))) {
    ret = OB_OP_NOT_ALLOW;
    DDLOG(WARN, "not user tenant, trigger_dump_data_dict is not allowed", KR(ret), K(tenant_id));
    LOG_USER_ERROR(OB_OP_NOT_ALLOW, "not user tenant, trigger_dump_data_dict is");
  } else if (OB_FAIL(GET_MIN_DATA_VERSION(tenant_id, data_version))) {
    DDLOG(WARN, "fail to get tenant data version", KR(ret), K(tenant_id), K(data_version));
    ...
    DDLOG(WARN, "not support to trigger partition balance", KR(ret), K(tenant_id), K(data_version));
...
int ObDBMSDataDict::check_scheduled_job_enabled_(const uint64_t tenant_id, bool &is_enabled)
{
  ...
  if (OB_ISNULL(sql_client = GCTX.sql_proxy_)) {
    ret = OB_ERR_UNEXPECTED;
    DDLOG(WARN, "sql_proxy is null", KR(ret));
      ...
      *sql_client,
      ...
      allocator,
      job_info))) {
        DDLOG(WARN, "get_dbms_sched_job_info failed", KR(ret), K(tenant_id));
  } else {
...
int ObDBMSDataDict::get_tenant_gts_(const uint64_t tenant_id, share::SCN &gts)
{
  ...
  do{
    if (ObTimeUtility::current_time() > timeout_ts) {
      ret = OB_TIMEOUT;
      DDLOG(WARN, "get_tenant_gts timeout, wait retry by dbms_scheduler or user_manual", KR(ret), K(timeout_ts));
    } else if (OB_FAIL(OB_TS_MGR.get_gts(tenant_id, stc, NULL, gts, tmp_receive_gts_ts))) {
      if (OB_EAGAIN == ret) {
        ob_usleep(100_ms);
        if (++retry_cnt % PRINT_INTERVAL == 0) {
          DDLOG(WARN, "retry get_gts for data_dict_service failed", KR(ret), K(retry_cnt));
        }
      } else {
      ...
      ret = OB_ERR_UNEXPECTED;
      DDLOG(WARN, "gts invalid", KR(ret), K(tenant_id), K(gts));
    } else {
      DDLOG(TRACE, "get gts", K(tenant_id), K(gts));
    }
  } while (OB_EAGAIN == ret);
  return ret;
}
...
int ObDBMSDataDict::send_trigger_dump_dict_rpc_(const uint64_t tenant_id, const obrpc::ObTriggerDumpDataDictArg &trigger_dump_data_dict_arg)
{
  ...
  if (OB_UNLIKELY(! is_user_tenant(tenant_id))) {
    ret = OB_INVALID_ARGUMENT;
    DDLOG(WARN, "invalid argument", KR(ret), K(tenant_id));
  } else {
    ...
    do {
      ...
      if (ObTimeUtility::current_time() > timeout_ts) {
        ret = OB_TIMEOUT;
        DDLOG(WARN, "send_trigger_dump_dict_rpc_ timeout, wait retry by dbms_scheduler or user_manual", KR(ret), K(timeout_ts));
      } else if (OB_ISNULL(GCTX.location_service_)) {
        ret = OB_ERR_UNEXPECTED;
        DDLOG(WARN, "location_service_ is invalid", KR(ret));
      } else if (OB_ISNULL(GCTX.srv_rpc_proxy_)) {
        ret = OB_ERR_UNEXPECTED;
        DDLOG(WARN, "srv_rpc_proxy is invalid", KR(ret));
      } else if (OB_FAIL(GCTX.location_service_->get_leader_with_retry_until_timeout(
          GCONF.cluster_id,
          tenant_id,
          SYS_LS,
          leader))) { // default 1s timeout
        DDLOG(WARN, "get sys_ls leader failed", KR(ret), "cluster_id", GCONF.cluster_id.get_value(),
        ...
        DDLOG(WARN, "trigger_dump_data_dict rpc failed", KR(ret), K(tenant_id), K(trigger_dump_data_dict_arg));
      } else {
        DDLOG(TRACE, "trigger_dump_data_dict rpc success", KR(ret), K(tenant_id), K(leader), K(trigger_dump_data_dict_arg));
      }
  ...
  if (OB_ISNULL(sql_client = GCTX.sql_proxy_)) {
    ...
    *sql_client,
    ...
    if (OB_ENTRY_NOT_EXIST == ret) {
      DDLOG(WARN, "get_dbms_sched_job_info not exist", KR(ret), K(tenant_id));
    } else {
      DDLOG(WARN, "get_dbms_sched_job_info failed", KR(ret), K(tenant_id));
    }
      ...
      *sql_client,
      ...
      job_attribute_value))) {
    if (OB_ENTRY_NOT_EXIST == ret) {
      DDLOG(INFO, "schedule job didn't change, just skip", K(tenant_id), K(job_attribute_name), K(job_attribute_value));
    } else {
      DDLOG(WARN, "update_dbms_sched_job_info failed", KR(ret),
          KCSTRING(DATA_DICT_SCHEDULED_JOB_NAME), K(tenant_id), K(job_attribute_name), K(job_attribute_value));
    }
  } else {
    DDLOG(INFO, "modify schedule job succ", K(tenant_id),
