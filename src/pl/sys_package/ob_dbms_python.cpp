...
int ObDBMSPython::loadpython_mysql(ObPLExecCtx &ctx, sql::ParamStore &params, common::ObObj &result)
{
  int ret = OB_SUCCESS;
  ...
  if (3 != params.count()) {
  ...
  } else if (OB_FAIL(ObCreateRoutineResolver::check_external_udf_version(tenant_id, true))) {
    LOG_WARN("failed to check external udf version", K(ret), K(tenant_id));
    ...
    LOG_WARN("python py name is empty", K(ret), K(params), K(name));
    ObString var_name = "PY_NAME";
    LOG_USER_ERROR(OB_ERR_WRONG_VALUE_FOR_VAR, var_name.length(), var_name.ptr(), name.length(), name.ptr());
  } else if (content.empty()) {
    ret = OB_ERR_WRONG_VALUE_FOR_VAR;
    LOG_WARN("python py content is empty", K(ret), K(params), K(content));
    ...
    ObCreateExternalResourceArg args(tenant_id, database_id, name, ObSimpleExternalResourceSchema::PYTHON_PY_TYPE, content, comment);
...
int ObDBMSPython::droppython_mysql(ObPLExecCtx & ctx, sql::ParamStore & params,
                               common::ObObj & result)
{
  int ret = OB_SUCCESS;
  ...
  if (1 != params.count()) {
  ...
  } else if (!tenant_config->ob_enable_python_udf) {
    ret = OB_NOT_SUPPORTED;
    LOG_USER_ERROR(OB_NOT_SUPPORTED, "ob_enable_python_udf is not enabled, drop python external resource is");
  } else if (OB_FAIL(GET_MIN_DATA_VERSION(tenant_id, data_version))) {
    LOG_WARN("failed to GET_MIN_DATA_VERSION", K(ret));
  } else if (GET_MIN_CLUSTER_VERSION() < CLUSTER_VERSION_4_4_1_0 || data_version < DATA_VERSION_4_4_1_0) {
    ret = OB_NOT_SUPPORTED;
    LOG_USER_ERROR(OB_NOT_SUPPORTED, "before version 4.4.1, drop python external resource is");
  } else if (OB_FAIL(params.at(0).get_string(name))) {
    LOG_WARN("failed to get name string", K(ret), K(params.at(0)));
  } else if (OB_ISNULL(ctx.exec_ctx_)) {
    ret = OB_ERR_UNEXPECTED;
    ...
    ObDropExternalResourceArg args(tenant_id, database_id, name,
                                   share::schema::ObSimpleExternalResourceSchema::ResourceType::PYTHON_PY_TYPE);
