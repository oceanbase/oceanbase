...
*/
int ObDBMSMViewMysql::purge_log(ObPLExecCtx &pl_ctx, ParamStore &params, ObObj &result)
{
  UNUSED(result);
  int ret = OB_SUCCESS;
...
*/
int ObDBMSMViewMysql::refresh(ObPLExecCtx &pl_ctx, ParamStore &params, ObObj &result)
{
  UNUSED(result);
  int ret = OB_SUCCESS;
  ...
  if (OB_ISNULL(pl_ctx.exec_ctx_)) {
    ret = OB_ERR_UNEXPECTED;
    LOG_WARN("exec context is null", K(ret), K(pl_ctx.exec_ctx_));
  } else {
    ObExecContext &ctx = *(pl_ctx.exec_ctx_);
  ...
  UNUSEDx(async, force);
    ...
    if (OB_FAIL(ret)) {
      ...
      // fill params
      refresh_params.list_ = params.at(ObDBMSMViewRefreshParam::MV_LIST).get_varchar();
      refresh_params.method_ = params.at(ObDBMSMViewRefreshParam::METHOD).is_varchar()
                               ? params.at(ObDBMSMViewRefreshParam::METHOD).get_varchar()
                               : NULL;
      refresh_params.refresh_parallel_ = params.at(ObDBMSMViewRefreshParam::REFRESH_PARALLEL).get_int();
...
RETURN TEXT;
*/
int ObDBMSMViewMysql::refresh_report(ObPLExecCtx &pl_ctx, ParamStore &params, ObObj &result)
{
  int ret = OB_SUCCESS;
  uint64_t tenant_id = OB_INVALID_TENANT_ID;
  ...
  if (OB_ISNULL(pl_ctx.exec_ctx_)) {
    ...
    if (OB_SUCC(ret)) {
      ...
      if (OB_FAIL(report_executor.execute(ctx, report_arg, report_text))) {
        ...
        ObTextStringResult text_res(ObTextType, true, pl_ctx.get_allocator());
...
*/
int ObDBMSMViewMysql::kill(ObPLExecCtx &pl_ctx, ParamStore &params, ObObj &result)
{
  UNUSED(result);
  int ret = OB_SUCCESS;
...
*/
int ObDBMSMViewMysql::set_refresh_params(ObPLExecCtx &pl_ctx, ParamStore &params, ObObj &result)
{
  UNUSED(result);
  int ret = OB_SUCCESS;
  uint64_t tenant_id = OB_INVALID_TENANT_ID;
