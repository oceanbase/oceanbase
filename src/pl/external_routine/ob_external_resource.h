...
/**
 * Python external source - standalone helper for fetching Python UDF source code.
 * The old sub-interpreter-based fetch_impl/caching has been removed.
 * Only static fetch_source methods remain (used by the sandbox path).
*/
class ObExternalURLPy
{
public:
  static int fetch_source(const common::ObString &url, common::ObSqlString &source);
};

class ObExternalSchemaPy
{
public:
  static int fetch_source(const uint64_t database_id,
                          const common::ObString &resource_name,
                          share::schema::ObSchemaGetterGuard &schema_guard,
                          common::ObSqlString &source);
};
