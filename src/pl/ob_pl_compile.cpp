  ...
  const share::schema::ObRoutineInfo &routine, ObPLFunctionAST &func_ast, ObPLFunction &func)
{
  ...
  LOG_INFO(">>>>>>>>Resolve Time: ", K(ret), K(routine.get_routine_id()), K(routine.get_routine_name()), K(resolve_end - parse_end));
  ...
  LOG_INFO(">>>>>>>>CG Time: ", K(ret), K(routine.get_routine_id()), K(routine.get_routine_name()), K(cg_end - resolve_end));

  int64_t final_end = ObTimeUtility::current_time();
  LOG_INFO(">>>>>>>>Final Compile Routine Time: ", K(ret), K(routine.get_routine_id()), K(routine.get_routine_name()), K(final_end - init_start));
...
ObPLCompilerEnvGuard::ObPLCompilerEnvGuard(const ObPackageInfo &info,
                                           ...
                                           int &ret,
                                           const ObPLBlockNS *parent_ns)
  : ret_(ret), session_info_(session_info), allocator_(ObModIds::OB_PL_TEMP, OB_MALLOC_NORMAL_BLOCK_SIZE, MTL_ID())
{
