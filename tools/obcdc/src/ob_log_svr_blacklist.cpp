    ...
    const bool is_sql_server)
{
  ...
  } else if (OB_ISNULL(svrs_buf_[0] = reinterpret_cast<char*>(ob_log_malloc(buf_size)))) {
    LOG_ERROR("alloc svrs buffer 0 fail", KR(ret), K(buf_size));
    ret = OB_ALLOCATE_MEMORY_FAILED;
  } else if (OB_ISNULL(svrs_buf_[1] = reinterpret_cast<char*>(ob_log_malloc(buf_size)))) {
...
void ObLogSvrBlacklist::destroy()
{
  ...
  if (NULL != svrs_buf_[0]) {
    ob_log_free(svrs_buf_[0]);
    svrs_buf_[0] = NULL;
  }

  if (NULL != svrs_buf_[1]) {
    ob_log_free(svrs_buf_[1]);
    svrs_buf_[1] = NULL;
  }
  svrs_buf_size_ = 0;
