...
void ObLogTransMsgSorter::stop()
{
  if (inited_) {
    ...
    LOG_INFO("stop TransMsgSorter succ", "thread_num", ObSimpleThreadPool::get_total_thread_num());
  }
}
...
void ObLogTransMsgSorter::handle(void *data)
{
  ...
  if (OB_UNLIKELY(!inited_)) {
    ...
    LOG_DEBUG("br sorter handle trans done", KR(ret), K_(enable_sort_by_seq_no), KPC(trans));
