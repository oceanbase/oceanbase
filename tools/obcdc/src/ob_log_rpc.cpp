...
#define SEND_RPC(RPC, SVR, TIMEOUT, REQ, ARG) \
    do { \
      if (OB_UNLIKELY(! inited_)) { \
        LOG_ERROR("not init"); \
        ret = OB_NOT_INIT; \
      } else { \
        obrpc::ObLogExternalProxy proxy; \
        if (OB_FAIL(net_client_.get_proxy(proxy))) { \
          ...
          if (OB_FAIL(proxy.by(tenant_id_).trace_time(true).timeout((TIMEOUT))\
              .max_process_handler_time(static_cast<int32_t>(max_rpc_proc_time))\
              .RPC((REQ), (ARG)))) { \
            LOG_ERROR("rpc fail: " #RPC, "svr", (SVR), "rpc_ret", ret, \
...
int ObLogRpc::req_start_log_id_by_tstamp(const common::ObAddr &svr,
      const obrpc::ObLogReqStartLogIdByTsRequestWithBreakpoint& req,
      obrpc::ObLogReqStartLogIdByTsResponseWithBreakpoint& res,
      const int64_t timeout)
{
  int ret = OB_SUCCESS;
  SEND_RPC(req_start_log_id_by_ts_with_breakpoint, svr, timeout, req, res);
  LOG_INFO("rpc: request start log id by tstamp", KR(ret), K(svr), K(timeout), K(req), K(res));
  return ret;
}

int ObLogRpc::req_leader_heartbeat(const common::ObAddr &svr,
    const obrpc::ObLogLeaderHeartbeatReq &req,
    obrpc::ObLogLeaderHeartbeatResp &res,
    const int64_t timeout)
{
  int ret = OB_SUCCESS;
  SEND_RPC(leader_heartbeat, svr, timeout, req, res);
  LOG_DEBUG("rpc: request leader heartbeat", KR(ret), K(svr), K(timeout), K(req), K(res));
  return ret;
}

int ObLogRpc::open_stream(const common::ObAddr &svr,
    const obrpc::ObLogOpenStreamReq &req,
    obrpc::ObLogOpenStreamResp &resp,
    const int64_t timeout)
{
  int ret = OB_SUCCESS;
  SEND_RPC(open_stream, svr, timeout, req, resp);
  LOG_DEBUG("rpc: open stream", KR(ret), K(svr), K(timeout), K(req), K(resp));
  return ret;
}

int ObLogRpc::async_stream_fetch_log(const common::ObAddr &svr,
    const obrpc::ObLogStreamFetchLogReq &req,
    obrpc::ObLogExternalProxy::AsyncCB<obrpc::OB_LOG_STREAM_FETCH_LOG> &cb,
    const int64_t timeout)
{
  int ret = OB_SUCCESS;
  SEND_RPC(async_stream_fetch_log, svr, timeout, req, &cb);
  LOG_DEBUG("rpc: async fetch stream log", KR(ret), K(svr), K(timeout), K(req));
  return ret;
}

int ObLogRpc::init(const uint64_t tenant_id, const int64_t io_thread_num)
{
  int ret = OB_SUCCESS;
  ...
  if (OB_UNLIKELY(inited_)) {
    ...
    LOG_ERROR("invalid argument", K(io_thread_num));
    ...
    inited_ = true;
    LOG_INFO("init rpc succ", K(tenant_id), K(io_thread_num));
  }
  return ret;
}
  ...
  if (enable_ssl_client_authentication) {
    ...
    if (OB_SUCC(ret)) {
      ...
      const uint64_t new_hash_value = is_local_file_mode
        ? observer::ObSrvNetworkFrame::get_ssl_file_hash(OB_CLIENT_SSL_CA_FILE, OB_CLIENT_SSL_CERT_FILE, OB_CLIENT_SSL_KEY_FILE,
            file_exist)
        : ssl_config.hash();
      ...
      if (ssl_config.empty() && ! file_exist) {
        ...
        if (is_local_file_mode) {
          if (EASY_OK != easy_ssl_ob_config_check(OB_CLIENT_SSL_CA_FILE, OB_CLIENT_SSL_CERT_FILE,
                OB_CLIENT_SSL_KEY_FILE, true/* is_from_file */, false/* is_babassl */)) {
        ...
        if (OB_SUCC(ret)) {
          if (OB_FAIL(net_client_.load_ssl_config(ca_cert, public_cert, private_key))) {
