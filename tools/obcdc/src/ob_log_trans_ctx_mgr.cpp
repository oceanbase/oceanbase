...
namespace liboblog
{
void ObLogTransCtxMgr::Scanner::operator() (const transaction::ObTransID &trans_id, TransCtx *trans_ctx)
{
  int ret = OB_SUCCESS;

  if (NULL != trans_ctx && NULL != buffer_ && buffer_size_ > 0 && trans_id.is_valid()) {
    if (pos_ >= buffer_size_) {
      LOG_WARN("print transaction information overflow", K(buffer_size_), K(pos_), K(trans_id), K(*trans_ctx));
    } else if (OB_FAIL(trans_ctx->lock())) {
      LOG_ERROR("lock trans_ctx fail", KR(ret), K(trans_id), K(*trans_ctx));
      ...
      DATA_PRINT(ptr, size, pos_, "trans_id:%s;", to_cstring(trans_id));
      ...
      for (int64_t index = 0; OB_SUCC(ret) && index < participant_count; index++) {
        DATA_PRINT(ptr, size, pos_, "%ld(%ld)",
            participants[index].pkey_.get_table_id(),
            participants[index].pkey_.get_partition_id());
...
int ObLogTransCtxMgr::get_trans_ctx(const transaction::ObTransID &key, TransCtx *&trans_ctx, bool enable_create)
{
  int ret = OB_SUCCESS;
  if (! inited_) {
    ret = OB_NOT_INIT;
...
int ObLogTransCtxMgr::remove_trans_ctx(const transaction::ObTransID &key)
{
  int ret = OB_SUCCESS;
  if (! inited_) {
    ret = OB_NOT_INIT;
...
int64_t ObLogTransCtxMgr::get_trans_count(const int trans_ctx_state)
{
  int64_t ret_count = 0;
