  ...
  if (OB_ISNULL(row_head) || OB_ISNULL(row_tail) || OB_UNLIKELY(row_num <= 0)) {
    ...
    while (OB_SUCC(ret) && NULL != cur_row_index) {
      ...
      if (! is_rollback_row) {
        ...
        if (OB_FAIL(rollback_row_(*cur_row_index))) {
          LOG_ERROR("rollback_row_ fail", KR(ret), KPC(cur_row_index));
        }
      }
...
int SortedDmlRowList::rollback_row_(ObLogRowDataIndex &rollback_row_data_index)
{
  int ret = OB_SUCCESS;
  const int64_t total_row_num = row_num_;
  const int64_t rollback_to_seq = rollback_row_data_index.get_row_seq_no();
  ...
  // 1. If seq_no is not valid(in 22x and 31x), rollback dml stmt by sql_no, otherwise rollback by seq_no
  // 2. Stmt with seq_no/sql_no less than or equal to the seq_no/sql_no specified by rollback savepoint
  // is not processed, find the first seq_no/sql_no greater than rollback_seq_no/rollback_sql_no.
  while (OB_SUCC(ret) && NULL != cur_row_index && !found) {
    const int32_t cur_row_seq = cur_row_index->get_row_seq_no();

    if (cur_row_seq <= rollback_to_seq) {
  ...
  if (OB_SUCC(ret)) {
    _LOG_INFO("[SAVEPOINT][DML] ROLLBACK_TO_SEQ=%ld STMT_CNT=%ld/%ld",
        rollback_to_seq, total_row_num, save_row_num);
  }
