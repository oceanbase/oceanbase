...
struct TransPartInfo
{
  ...
  explicit TransPartInfo(const common::ObPartitionKey &pkey) : obj_(NULL), pkey_(pkey)
  {}
...
private:
  ...
  int64_t                   global_trans_version_;  // Global transaction version
};
...
public:
  ...
  /// @param [in] part_mgr        Partition transaction manager to determine if the participant is served
  ...
  int prepare(PartTransTask &part_trans_task,
  ...
  /// @retval OB_SUCCESS        Success
  /// @retval other_error_code  Fail
  int add_participant(PartTransTask &part_trans_task,
...
private:
  ...
  // INVALID -> PREPARED
  int prepare_(PartTransTask &host,
      IObLogPartMgr &part_mgr,
      const bool print_partition_not_serve_info,
      volatile bool &stop_flag);
...
#endif /* OCEANBASE_LIBOBLOG_TRANS_CTX_ */
