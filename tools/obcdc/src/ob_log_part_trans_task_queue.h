...
#ifndef OCEANBASE_LIBOBLOG_OB_LOG_PART_TRANS_TASK_QUEUE_H__
#define OCEANBASE_LIBOBLOG_OB_LOG_PART_TRANS_TASK_QUEUE_H__
...
private:
  struct DispatchingTaskBasicInfo
  {
    ...
    void reset(bool is_trans_committed,
      ...
      TO_STRING_KV(K(is_dispatching_), K(is_trans_committed_), K(is_trans_ready_to_commit_),
          K(task_type_), K(prepare_log_id_), K(task_timestamp_), K(global_trans_version_));
  };
