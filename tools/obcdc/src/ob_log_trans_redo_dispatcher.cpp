    ...
    PartTransDispatchBudget *part_budget)
{
  ...
  // 3. dispatch_redo_: expected OB_SUCCESS, all kind of other error code are unexpected and liboblog should stop
    ...
    ObLogEntryTask *&log_entry_task)
{
  ...
  } else if (OB_FAIL(log_entry_task->init(part_task.get_partition(), part_task.get_participant_key_str(),
