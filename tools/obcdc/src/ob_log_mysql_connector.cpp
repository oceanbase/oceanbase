...
int ObLogMySQLConnector::query(MySQLQueryBase& query)
{
  ...
  if (OB_UNLIKELY(!inited_)) {
    ...
    // This means that liboblog is connected to a low version of the observer, no replica_type information is available, error code OB_ERR_COLUMN_NOT_FOUND is returned
    //
    ...
    const bool enable_ssl_client_authentication)
{
  ...
  if (NULL == (mysql_ = mysql_init(NULL))) {
    ...
    if (! enable_ssl_client_authentication) {
      ...
      if (0 != (mysql_options(mysql_, MYSQL_OPT_SSL_VERIFY_SERVER_CERT, (void *)&ssl_mode))) {
        LOG_ERROR("failed to set ssl mode for mysql conn",
            K(mysql_error(mysql_)), K(ssl_mode));
        ret = OB_ERR_UNEXPECTED;
      }
    }
