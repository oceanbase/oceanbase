...
#define OCEANBASE_LIBOBLOG_OB_LOG_TASK_POOL_H_
...
public:
  ...
  void destroy()
  {
    ...
    if (OB_UNLIKELY(!inited_)) {
      ...
      CLOG_LOG(ERROR, "err clean prealloc tasks", KR(ret));
    } else {
      if (0 < total_cnt_) {
...
public:
  ...
  TaskType* get(const char *info, const common::ObPartitionKey &partition)
  {
    int ret = common::OB_SUCCESS;
    ...
    if (common::OB_SUCCESS == ret && OB_NOT_NULL(ret_task)) {
      ret_task->set_prealloc_page(get_prealloc_page_());
      ret_task->set_pkey_info(partition, info);
    }
    return ret_task;
  }
