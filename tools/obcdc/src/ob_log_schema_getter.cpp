...
int ObLogSchemaGuard::get_table_schema(const uint64_t table_id,
    const ObTableSchema *&table_schema,
    const int64_t timeout)
{
  int ret = OB_SUCCESS;
  const uint64_t tenant_id = extract_tenant_id(table_id);
  RETRY_ON_FAIL_WITH_TENANT_ID(tenant_id, timeout, guard_, get_table_schema, table_id, table_schema);
  return ret;
}
...
int ObLogSchemaGuard::get_tablegroup_schemas_in_tenant(const uint64_t tenant_id,
    common::ObArray<const share::schema::ObTablegroupSchema *> &tablegroup_schemas,
    const int64_t timeout)
{
  int ret = OB_SUCCESS;
  // You must use the fallback schema guard, otherwise may get an error in following function, here is a precaution
  ret = check_force_fallback_mode_("get_tablegroup_schemas_in_tenant");
  RETRY_ON_FAIL_WITH_TENANT_ID(tenant_id, timeout, guard_, get_tablegroup_schemas_in_tenant, tenant_id, tablegroup_schemas);
  return ret;
}

int ObLogSchemaGuard::get_database_schema(uint64_t database_id,
    const ObDatabaseSchema *&database_schema,
    const int64_t timeout)
{
  int ret = OB_SUCCESS;
  RETRY_ON_FAIL(timeout, guard_, get_database_schema, database_id, database_schema);
  return ret;
}
    ...
    const int64_t timeout)
{
  ...
  ret = check_force_fallback_mode_(extract_tenant_id(database_id), "get_database_schema");
  RETRY_ON_FAIL(timeout, guard_, get_database_schema, database_id, database_schema);
  return ret;
}
...
int ObLogSchemaGuard::get_database_schema_info(uint64_t database_id,
    ...
    const int64_t timeout)
{
  ...
  if (OB_FAIL(guard_.is_lazy_mode(tenant_id, is_lazy))) {
    ...
    if (OB_FAIL(get_database_schema(database_id, db_schema, timeout))) {
      if (OB_TIMEOUT != ret && OB_TENANT_HAS_BEEN_DROPPED != ret) {
        LOG_ERROR("get_database_schema fail when get database name", KR(ret), K(database_id), K(is_lazy));
      }
    ...
    if (OB_FAIL(get_database_schema(database_id, simple_db_schema, timeout))) {
      if (OB_TIMEOUT != ret && OB_TENANT_HAS_BEEN_DROPPED != ret) {
...
int ObLogSchemaGuard::get_tablegroup_schema(uint64_t tablegroup_id,
    const ObTablegroupSchema *&tablegourp_schema,
    const int64_t timeout)
{
  int ret = OB_SUCCESS;
  const uint64_t tenant_id = extract_tenant_id(tablegroup_id);
  RETRY_ON_FAIL_WITH_TENANT_ID(tenant_id, timeout, guard_, get_tablegroup_schema, tablegroup_id, tablegourp_schema);
  return ret;
}

int ObLogSchemaGuard::query_partition_status(const common::ObPartitionKey &pkey,
    oceanbase::share::schema::PartitionStatus &part_status,
    const int64_t timeout)
{
  int ret = OB_SUCCESS;
  const uint64_t tenant_id = pkey.get_tenant_id();
  RETRY_ON_FAIL_WITH_TENANT_ID(tenant_id, timeout, guard_, query_partition_status, pkey, part_status);
  return ret;
}
    ...
    const int64_t max_history_schema_version_count)
{
  ...
  // init Schema Service
          ...
          max_history_schema_version_count, false))) {
    ...
    bool &is_suitable)
{
  ...
  if (OB_FAIL(schema_guard.get_table_schema(table_id, tb_schema, timeout))) {
    LOG_ERROR("get table schema from local schema cache fail", KR(ret), K(table_id),
  ...
  else if (OB_FAIL(schema_guard.get_database_schema(tb_schema->get_database_id(),
      simple_db_schema, timeout))) {
    LOG_ERROR("get database schema from local schema cache fail", KR(ret),
    ...
    const ObSimpleTableSchemaV2 *&tb_schema)
{
  ...
  if (OB_UNLIKELY(! inited_)) {
    LOG_ERROR("schema getter has not been initialized");
    ret = OB_NOT_INIT;
  } else if (OB_FAIL(get_schema_guard_and_simple_table_schema_(table_id, expected_version, timeout, force_lazy,
    ...
    const oceanbase::share::schema::ObTableSchema *&full_tb_schema)
{
  ...
  } else if (OB_FAIL(schema_guard.get_table_schema(table_id, full_tb_schema, timeout))) {
    if (OB_TIMEOUT != ret) {
    ...
    int64_t &latest_version)
{
  ...
  if (OB_UNLIKELY(OB_INVALID_TENANT_ID == tenant_id)
    ...
    if (OB_ENTRY_NOT_EXIST == ret) {
      // Circumvent the problem of liboblog reporting an error on the first refresh of the tenant's schema
      ret = OB_SUCCESS;
      latest_version = OB_INVALID_VERSION;
...
{
  int ret = OB_SUCCESS;
  RETRY_ON_FAIL(timeout, schema_service_, load_split_schema_version, split_schema_version);
  return ret;
}
