...
**OceanBaseデータベース**は分散リレーショナルデータベースです。Ant Groupによって完全に開発されています。OceanBaseデータベースは共通のサーバークラスター上に構築されています。 OceanBase データベースは、[Paxos](https://lamport.azurewebsites.net/pubs/lamport-paxos.pdf) プロトコルとその分散構造に基づいて、高可用性と線形スケーラビリティを実現します。OceanBase データベースは、特定のハードウェア アーキテクチャに依存しません。
...
- **継続的な可用性**: RPO = 0 (データ損失ゼロ)、RTO < 8 秒 (リカバリ時間)
