# 如何使用 OceanBase Deploy 自动化部署多节点集群
...
|   | depends | 依赖的配置节|   通常指定依赖的集群配置，会自动复用集群的 `proxyro` 密码、集群名 `cluster_name` 等等。|
...
|              | rs_list                                  | 172.20.249.52:2881;172.20.249.49:2881;172.20.249.51:2881 | OceanBase 集群 rootservice 服务地址，由 sys 租户的三副本所在节点IP 组成。                                                                                                 |
|              | enable_cluster_checkout                  | FALSE                                                    |                                                                                                                                               |
...
for OceanBase in $IPS;do echo $ob; ssh $ob "ps -ef | grep observer | grep -v grep "; done

输出：
[admin@obce00 oceanbase-ce]$ for OceanBase in $IPS;do echo $ob; ssh $ob "ps -ef | grep observer | grep -v grep "; done
172.20.249.52
...
```bash
IPS="172.20.249.52 172.20.249.49 172.20.249.51"
for OceanBase in $IPS;do echo $ob; ssh $ob "netstat -ntlp"; done

输出：
[admin@obce00 ~]$ for OceanBase in $IPS;do echo $ob; ssh $ob "netstat -ntlp"; done
172.20.249.52
