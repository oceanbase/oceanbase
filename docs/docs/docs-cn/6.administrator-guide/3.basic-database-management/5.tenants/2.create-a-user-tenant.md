...
5. 创建资源池。
   ...
   3. 如果在某个 zone 内找不到足够剩余资源的机器来创建资源单元，资源池会创建失败。
      ...
      详细信息，参考 [资源管理概述](/docs-cn/6.administrator-guide/3.basic-database-management/4.resource-management/1.resource-management-overview/)。
...
6. 创建租户。
   ...
   详细信息，参考 [租户管理概述](/docs-cn/6.administrator-guide/3.basic-database-management/5.tenants/1.tenant-management-overview-1/)。
...
首次使用新租户连接 OceanBase 数据库时，您必须使用 `root` 用户。成功连接后，您可以创建新用户连接 OceanBase 数据库。详细信息，参考 [创建用户](/docs-cn/6.administrator-guide/7.user-rights-management/2.create-user-3/) 文档。
